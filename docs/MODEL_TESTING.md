...
**Defaults migrated 2026-09-23.** Every task (`events`, `actors`, `rules`, `metrics`, `summary`, `analysis`, `referee`) now defaults to `openrouter:meta/muse-spark-1.3-contributor`, with `llm.reasoning_effort` defaulting to `minimal` and `llm.max_tokens` to 3000 – the combination the measurements above show the default model needs to fit its reasoning inside the budget (304 vs 1 599 completion tokens on the same events prompt). Scenarios pinning other models explicitly are unaffected, except that OpenRouter-routed calls now carry `reasoning: {"effort": "minimal"}` unless the scenario overrides it; for non-reasoning instruct models there is nothing to economise and the flag is inert.

**Defaults migrated 2026-08-22 (superseded).** `summary`, `analysis`, and `referee` then defaulted to `qwen/qwen3-235b-a22b-2507`, and all seven scenarios plus the sweden-ai-2030 variants were swept off the withdrawn model. `_default_main` stays `google/gemini-3-flash-preview`: the catalog lists it as reasoning-capable, but probing shows it does not reason unprompted (11 output tokens for a trivial JSON task, no `reasoning` field), so it carries none of the budget risk.
