...
- **Eligibility Gates (optional, per event):** An event may declare an `**Eligible:**` expression in `events.md` (for example `public_sentiment_to_ai < 30`). Python evaluates it against the current metric values with the same sandboxed evaluator used for termination conditions; a false gate removes the event from the rendered events prompt entirely and rejects any candidate for it the model proposed anyway (recorded as a skip reason). Gates exist because long event lists made prose conditions unreliable in practice — models listed ineligible events "just in case", and anything listed gets rolled. Deterministic thresholds become deterministic; prose conditions the metrics cannot express (prior events, portfolio state) stay in `Condition:` for the LLM to judge. Validation rejects gates referencing unknown metrics or failing to evaluate against starting values.
- **The Occurred-Events Record:** Every event that fires is added to `scenario.occurred_events` and persisted to `summary.json`, repeatable or not. This is the run's history — what happened, for gate judgments, precursor analysis and reporting. It is deliberately distinct from `Event.occurred`, the one-shot suppression flag, which is set only for non-repeatable listed events and is what the always-eligible contract checks before skipping a candidate. Resume restores the record in full but re-derives the suppression flag, so a repeatable event in the record is never mistaken for a spent one. Before this separation the record held only non-repeatable events, which silently omitted every precursor (all of them repeat) from per-run analysis; `ensemble.py` and `model_sensitivity.py` had already worked around it by reading `turn-XX/1-events.json` directly, and per-run analysis now reads the same artifacts to build a turn-stamped timeline that works for runs saved before the change.
- **Structured Event History in the Events Prompt:** The events step is given `scenario.event_log` — a turn-stamped record of every event that fired, rendered as one line per completed turn and excluding the current turn — and is told to judge windowed conditions against it and nothing else. Gate conditions are written as windows ("if X occurred in any of the previous 4 completed turns"), which the flat `occurred_events` set cannot answer; before this the step had to recover dates from the narrative and the historical summary, prose that condenses and drops them. The log persists in `summary.json` under `event_log`, is restored by resume, and is truncated at the branch point by branch, so a branch inherits only what had happened by then. Because scenario `user-prompts/events.md` overrides replace the default template rather than merging with it, every such override must render `{{event_history}}` itself; all four in-repo overrides do.
- **Mutually Exclusive Event Families (optional):** `event_groups` in `scenario.yaml` declares families where the members cannot co-occur — an election returning one administration, a crisis that either escalates or is negotiated away. Independent dice cannot express that constraint: two members can fire in one turn, or none can, and neither is a possible world. A group takes one seeded roll for the whole family, keyed `f"{seed}:{turn}:group:{group_id}"` so adding a member does not shift the dice for the others. The events step still prices each member and those prices become the family's *relative* weights, keeping judgment with the LLM and structure with Python. Two resolutions: `exactly_one` (a member fires whenever the group is due; requires `due_turns` and a `default` member for the case where every weight is zero) and `at_most_one` (the summed weight decides whether, the shares decide which). A group may instead select deterministically from the run's own record with `select_by: {kind: most_recent_event, map: {...}, precedence: [...]}` — for an outcome that is a consequence of earlier events rather than a fresh draw; the most recently fired mapped event elects the member, ties inside a turn break by precedence, and no dice are involved. Members are recorded in `1-event-evaluations.json` with `event_group`, `group_roll`, `group_basis` and, for the losers, `suppressed_by_group`; a winner the events step never listed is synthesised into the record with `unlisted_by_model: true`. Because each outcome is its own event id, it enters `occurred_events` and `event_log` like any other event, so later gates, resume, and cross-run analysis read it without parsing narrative. This replaces the older workaround of one event whose description carried every outcome plus a prose rule for choosing between them, which failed twice in practice: multi-sample probability elicitation turned a "certain" event into a probabilistic one when the model omitted it from some samples, and the prose rule was applied against a history the metrics step cannot see, so it silently defaulted. Validation rejects unknown members, a member claimed by two groups, a `due_turn` outside the run, and an `exactly_one` group without `due_turns` or `default`.
...
- **Emerging Developments (opt-in, with emergent events):** When `emergent_events.track_unfired: true` is also set (default `false`, so enabling emergent events alone keeps one-shot semantics), an emergent proposal that did *not* fire is not discarded. Python carries it forward as an emerging development (`EmergingDevelopment`: id, description, first/last turn) for at most `emergent_events.window_turns` (default 3) consecutive listed turns; a proposal that fires leaves the list as an occurred event, and one that stops being re-proposed has fizzled and is dropped. Each turn, the tracked list is rendered into the notepad every LLM step sees (`## Emerging developments (tracked)`), regenerated idempotently so stale copies written back by a model are replaced. The events step is instructed to re-list entries still considered plausible at whatever probability it then judges them to have, read from the world as it currently stands and still capped, or omit them to end them. It is told explicitly not to escalate an entry for having been listed before: an arithmetic ratchet (previously ~1.5–2× per turn) makes the carried-forward set drift toward firing regardless of what the world does, which is the opposite of what tracking them is for; the Game Master is instructed to let tracked developments colour the narrative only as faint, unconfirmed signals. This gives slow-moving dangers narrative presence before they materialise – traces a regulator-grade actor can act on – while Python owns only bookkeeping: what a development means, and how likely it is, remain LLM judgments. The list persists in `summary.json` under `emerging_events` and is restored by resume/branch.
- **Probability Sampling (optional):** `llm.probability_samples: N` (default 1) makes the events step elicit the candidate list N times instead of once. Per event id, the probability used for the dice is the mean across valid samples, counting samples where the event was absent as 0 (absence ≈ conditions judged not met). The evaluation record then includes `probability_samples` (the per-sample values), `samples_present`, and `n_samples`, so probability uncertainty is visible in the artifact. Samples that end in a parse failure are excluded from the denominator; if every sample fails, the turn records the usual parse-failure marker. Multi-sample elicitation naturally down-weights one-off proposals that appear in only some samples; an emerging development that the prompt requires be re-listed every turn appears in all samples and is therefore not diluted.
...
- **`scenario.yaml`**: Configuration (time scale, actors, LLM settings, output language).
  * **LLM Settings:** Includes per-task model configuration (`events`, `actors`, `rules`, `metrics`, `summary`, `analysis`, `referee`). Every task defaults to `openrouter:meta/muse-spark-1.3-contributor` when the scenario does not name a model.
  * **Token Budgets:** Supports global `llm.max_tokens` (default 3000) plus optional per-task overrides via `llm.max_tokens_by_task` (for example, higher cap for `rules` to reduce truncation).
  ...
  * **Reasoning Effort:** `llm.reasoning_effort` (for example `minimal`, `low`, `medium`, `high`; default `minimal`) is passed to providers that accept it, currently OpenRouter, as `reasoning: {"effort": …}`. An explicit null omits the field entirely, so a scenario can still let a model stay on its own default. The default is `minimal` because the default model reasons mandatorily (see below). It matters more than its size suggests, because a model whose reasoning is mandatory spends output tokens on thinking whether or not the step needs it, and reasoning tokens bill as completion tokens. Measured on one events-shaped prompt, `meta/muse-spark-1.3-contributor` spent 1 599 completion tokens and 18.2s at its default effort against 304 tokens and 3.3s at `minimal`, for the same answer – the difference between 2.1x the cost of `qwen/qwen3-235b-a22b-2507` and 0.46x of it. It also decides whether such a model fits `llm.max_tokens` at all: at the 3 000 europe-2032 declares, the events step exhausted the whole budget on reasoning and returned nothing parseable, raising `LLMReasoningBudgetError`. The value is not validated against a list, since each provider names its own levels and an unknown one is the provider's error to report.
  ...
  * **Declared Persistent State:** Optional `store:` block declaring tables of records the framework holds on the actors' behalf, each column carrying an `owner` (`system` | `actor` | `derived`) and a `type`. See *Declared Persistent State: the Store* below. A variant inherits its base's schema; the schema is physics, and arms holding different columns would not be comparable.
  ...
  * **Run-time Config Fields:** `config.json` additionally records `random_seed` (the dice RNG seed for the run), `logging.llm_io`, and, for branch counterfactuals, `event_overrides: {"turn": N, "force": [...], "suppress": [...]}`. `random_seed` and `event_overrides` are set at run time rather than declared in `scenario.yaml`. The run's `llm` block snapshot also records `call_timeout_seconds`, `model_limits` and `reasoning_effort`, so a run's effective limits are traceable from its artifacts. `reasoning_effort` is recorded for the same reason the others are: it moves a reasoning model's cost and wall clock by several times, so a batch whose artifacts do not carry it cannot be compared against any other batch. When a run is given a starting-state draw, `config.json` also records `initial_state` (see Starting-State Draws below). `scenario_source` records the absolute path of the YAML the run was loaded from – the variant file for a variant run, `scenario.yaml` otherwise – so `resume` and `branch` reload the same definition. Without it they resolved a run only to its scenario *directory* and therefore reloaded the base `scenario.yaml`, silently replacing a variant's actors and resource patches with the base ones. Runs predating the field, and runs whose recorded path no longer exists, fall back to the directory; a variant that merely moved is recovered by file name under `variants/`.
...
- **Inheritance:** Scenarios can inherit from others via the `base` field in `scenario.yaml`. Chained inheritance works (a variant whose base is another variant), and a variant's resource patches compose down the chain by extension.
- **Variant Resource Patches:** A variant may declare `patches:` – a list of `{resource, path}` entries applied in order after the base resources load (`resource_patches.py`). Patch files reuse the resource's own markdown grammar and match by stable key: events by `**ID:**`, metric rules by leading number. Present fields override; absent ones inherit; an event section matching no existing ID is a new event when complete (ID + Probability + Description) and a hard error otherwise (a typo'd key must not silently become an addition); `**Remove:** yes` deletes; rule numbers beyond the current maximum append, gaps error. Patchable resources are an explicit allowlist (`events`, `metric_rules`) – actor statement ledgers are living framework state and deliberately out of reach, as is background context (shared by design so arms cannot drift apart). All validators run on the merged result, and applied patches are recorded in the run's `config.json` snapshot and shown by `describe`. Motivation: regime-conditioned prose ("PLATEAU 12%; FAST 18%; …") relied on the events model picking the right column every turn, and an audit of 84 runs showed evaluated probabilities converging across arms instead of tracking their declared figures – flat per-arm patches remove that silent failure mode entirely.
...
- **Resume and branch:** Ledgers are restored from the last completed turn's `<actor_id>-statements.md` files, exactly like notepad and rules, and the actor's own last response from `<actor_id>.md` alongside them. The ledger is the durable substrate: anything a scenario needs an actor to hold across a resume or a branch is safer as a statement than as prose the actor restates each turn.
...
### Declared Persistent State: the Store (`store.py`, `orchestrator.py`)

The statement ledger solves custody for one kind of state. Scenarios need it for others – a measure portfolio, an alliance, a permanent change to the world – and until this existed they got it the way the two failed designs did: the actor re-emitted the state every turn and an entry survived only if the model remembered to write it again.

That was measured rather than assumed. Over the 150-run europe-2032 statistics batch, 4.5% of proposed measures never reached the portfolio at all with no reason given, and 0.6% of turn-to-turn transitions lost an entry with no stated cause; adding the deferrals that were explained once and never revisited, 6.4% of proposals never became measures. Because the metric rules charge political capital per measure in flight, a measure that dropped out silently *stopped costing anything*, so a run that lost one read as better-resourced than it was. Full measurement and design record: `docs/proposals/persistent-state-custody.md`.

The generalisation is not "check the ledger". It is **custody of declared persistent state**: each scenario declares what persists, and the framework guarantees it.

- **Declaration.** A `store:` block in `scenario.yaml`, one entry per table, each column declaring an `owner` and a `type`. Absent in every scenario that predates the mechanism, and absent means nothing changes anywhere.
- **Provenance per column.** `actor` (written under `## Store changes` in an actor's output; a judgement), `world` (written under `## Store changes` in the Game Master response; a standing condition), `system` (stamped by the framework), `derived` (computed). This is the load-bearing part: with `started_turn` owned by the system there is no entry that reaches it, so "copied forward, unchanged" stops being a rule the model obeys by remembering. An unknown column in an entry is a rejection, never an addition.
- **Types few, forgiving on write, strict in store.** `text`, `integer`, `number`, `turn`, `enum`. Values are normalised on the way in – lowercased, stripped of backticks and asterisks, a turn read out of `finishes on turn 7` – because models write `Large`, `` `large` `` and `**large**` for the same thing. What will not normalise is rejected loudly into the turn's changelog. Same move `structured_outputs` makes folding YAML booleans to canonical strings at load time.
- **Derivation is lookup only.** A `map` from another column's value, or a `when_reached` / `else` comparison against the current turn. One step deep, no expressions, no joins. A map over an enum must cover every value. When a scenario wants more than that, the thing it wants is judgement, and judgement belongs to the LLM.
- **Python assigns ids.** Records are addressed as `M1`, `M2` – a per-table prefix, unique across the declared tables – and the id is rendered beside every row. Names are not keys: `scripts/check_portfolio_drift.py` needs a stopword list and a fuzzy word matcher because the Game Master paraphrases measure names between turns, and a command addressed by name does not parse at all.
- **Writes go under one declared header, as JSON.** `## Store changes` in the writer's output, holding one `{"store": [...]}` block with `add` / `update` / `delete` entries. One malformed entry rejects that entry while the rest apply; an absent section is a fault, not a declaration of no change. The JSON form replaced a hand-rolled markdown grammar whose bugs cost three separate fixes; values stay forgiving on write and strict in store. Three reasons the writes are not inline: a turn can be re-run up to `constitutional_enforcement.max_attempts` times, and a scattered write mutates state partially on a partial re-emission; a missing *section* is detectable where a missing inline entry never was; and free-floating syntax means any quoted string containing the pattern writes to state. The section is required every turn, saying `No changes.` (or `{"store": []}`) when there is none, exactly as statements require `No statement changes.`
- **A turn is one transaction.** `Store.begin_turn` restores the pre-turn snapshot before applying, so a turn executed twice replaces its own previous application rather than appending to it. The actor step opens the transaction; the Game Master step's world writes join it without reopening. Individual entries are still accepted or rejected one at a time, as statement proposals are – an all-or-nothing turn would discard good entries because of one typo.
- **Required reports re-ask, not only warn.** `reporting_required`, per table or per column, unifies the two custody models already in the codebase: statements and measures are silence-means-persistence; metrics are silence-means-omission-fault. Those are two settings of one dial. Set, the writer must report the value every turn and an omission is a fault -- and it re-asks once with a targeted prompt, records the outcome in the turn's changelog whether or not the repair worked, and writes carried-forward values explicitly. A flag that only warned would leave the turn's records incomplete. One re-ask mechanism serves two triggers: a required report that is absent, and a declared write block that is absent.
- **Ranges clamp-and-record by default.** `{min: 0, max: 100}`, or `range: [0, 100]`, on a numeric column. An out-of-bounds value clamps into range with a trace in the changelog; `on_out_of_range: error` rejects instead, for columns where a miss means the writer is broken rather than approximate. A silent clamp is the quiet wrongness this project usually refuses; a clamp that leaves a trace is not.
- **`adjust` submits the change, not the new value.** `{"op": "update", ..., "adjust": -9}` moves the table's single numeric writer-owned column by the delta, in Python rather than in the model's head. The artifact records prior value, adjustment, and result, because a wrong delta compounds forever where a wrong absolute value self-corrects next turn. Both forms stay available: some values are set rather than adjusted.
- **Metrics as a world table, behind an adapter.** A scenario may declare a world-scoped `metrics` table holding one record per metric -- the record id is the metric id -- with its levels in a single numeric world-owned column. The loader seeds it from the starting values; the Game Master step writes it with the store's write form (`adjust` where the rules state deltas, field sets where they set levels, a legacy levels map accepted entry by entry so format-fix repairs still land); `scenario.metrics` syncs from the records behind its unchanged interface. Everything downstream -- gates, termination conditions, prompts, analysis, the committed run artifacts -- reads metrics exactly as before, which is what contains the change. The judgement stays with the LLM: `metric-rules.md` reads store values into prose the model still judges. europe-2032 has not migrated: its runs are not comparable across the move, and the migration waits on the before-and-after arithmetic measurement the proposal calls for.
- **Reads render before the model sees them.** `metric-rules.md` was always injected as raw text and never rendered; it is now rendered through the `SandboxedEnvironment` already used for every other template, with a `store` object in scope. That inherits the sandbox, Jinja's filters and the existing undefined-variable behaviour, and keeps a second interpolation syntax out of the design. A scenario with no store, or a rules file with no `{{`, is returned untouched.
- **The query surface is one selector plus reducers.** `store.rows('measures', status='running')` renders the markdown table; `store.rows('measures', ['cost_per_turn'], status='running').sum` yields a number, and `.count` / `.min` / `.max` / `.mean` work the same way. Column selection narrows the rendered table -- the charge selects `['id', 'name', 'cost_per_turn']` while "finishing this turn" keeps the wide row the Game Master prices from. One equality filter, no joins, no arithmetic between reducers. A reducer other than `count` needs exactly one selected column, which the validator enforces. Every extension past that is individually reasonable and collectively a query language with its own bugs, and a wrong aggregate looks exactly as authoritative as a right one. An empty `mean`, `min` or `max` renders `(none)` rather than `0`: these values are read as prose, and an authoritative-looking zero where there is no answer is worse than a visible gap.
- **Itemised rows beside every total.** `design-notes.md` records that the portfolio charge binds *because* "the total cannot be known without summing it". Handing over a pre-computed number alone risks the Game Master ceasing to attend to the portfolio at all, so the rules render the rows and the sum together, and the validator warns when a template aggregates over a table it never renders rows for.
- **Which step sees which.** The **metrics** step, which *applies* the rules, gets them rendered. The **rules** step, which *edits* them, gets the source – hand it totals and it writes those totals back as the rule, and the expression is gone for good. The validator warns when a scenario pairs store expressions with live rule evolution for exactly that reason.
- **Validation reaches references, not only the schema.** An undefined Jinja variable renders as empty text, so a mistyped column would silently zero an arithmetic term and the rule would stop applying with nothing recording it. `validate_store` checks every `store.…` call in `metric-rules.md` and in the scenario's prompt overrides against the declared schema, and unknown tables, unknown columns, unknown operations and second filters are errors. Declared table and column names are also accepted as identifiers when prose in `metric-rules.md` names them, and Jinja expressions are stripped before that file is scanned for metric references – the two namespaces are each checked by the validator that owns them.
- **Persistence.** `turn-XX/2-actors/<actor_id>-store.md` is the readable artifact – every table plus this turn's changelog, written whether or not anything changed, so a diff between consecutive turns is empty unless a command was applied. `<actor_id>-store.json` beside it is the authoritative state that resume reads back, the same division as `4-metrics.json` beside `4-world-state.md`. The resolved schema is recorded in the run's `config.json`: a schema change is a physics change, and two batches under different schemas are not comparable.
- **Resume and branch.** Resume restores the records from the last completed turn. A branch inherits **by value**: `create_branch` copies the turn directories wholesale, so the child's records are its own from the first turn it executes and nothing it does can reach back into the parent.
- **Scope.** `scope: actor` tables are written by actors; `scope: world` tables belong to the run, are written by the Game Master step, and are read by every step. The boundary is enforced, not requested, with one deliberate asymmetry: the Game Master may move records (`update` with stated grounds) on actor tables where the scenario gives it scheduling moves -- it alone sees the whole turn -- but never add or remove there, while actors cannot touch world tables at all. World writes land in the same turn transaction the actor step opened, and persist in `turn-XX/4-world-store.md` (readable changelog) beside `turn-XX/4-world-store.json` (the freshest snapshot -- actor files were written before the Game Master step ran, so `resume` prefers it when present).
- **Seeds, not talk.** A table may declare `initial:` records the run starts with, applied at turn 0 through the same path as any turn's writes. An inherited portfolio never depends on a writer talking it into existence on turn 1.
- **Self-test.** `python -m scenario_lab.store --self-test` checks the mechanism against its own contract, in the pattern `story/check_tree.py` uses.

**What this does not fix.** Custody protects what got into the store. It does nothing about a measure the actor describes in prose and never writes a command for – the never-entered failure, and the larger half of the measured defect. That needs a hook that can *re-ask*, in the pattern of the metrics repair at `orchestrator.py`, and it is not built. The absent section is at least detectable now: it is printed as a warning and recorded as a fault in the turn's artifact. Nor does anything here see an unresolved deferral, where a proposal was correctly deferred with a stated reason and never revisited; that needs tracking of open proposals across turns, a third mechanism.

**Statements have not moved in, deliberately.** They are the same shape and this section's design invites it, but they carry semantics a generic store would flatten – the tier system, the gated relevance check, world pricing of an accepted change. Moving them needs per-table mutation policy first. Statements are the one piece of persistent state in this system that demonstrably does not drift, and rebuilding a working mechanism for architectural tidiness is how it gets lost. The name reflects that: this is the *store*, and "ledger" stays with statements until the two are actually one thing.
...
1. **Events Step**:
  ...
  * **Probability Sampling:** When `llm.probability_samples > 1`, the candidate list is elicited that many times and per-event probabilities are aggregated (mean with absent-as-zero over valid samples) before the single dice roll. Per-sample values are persisted in the evaluation record. The elicitations run concurrently (bounded thread pool, one worker per sample) since they are independent identical prompts; results are consumed in submission order and then sorted by canonical form, so the recorded per-sample arrays stay deterministic regardless of thread scheduling. I/O recording (cost tracking, transcripts) is replayed on the main thread because the recording wrapper buffers on the calling thread's thread-local storage.
...
2. **Actors Step**:
   ...
   * **Parallelization:** Actor prompts are independent and are executed in parallel with bounded concurrency. Each worker carries its buffered I/O transcript back for replay on the main thread (thread-locals do not cross threads), so `--log-llm-io` records every actor call exactly like every other step.
   ...
   * **Store Processing:** Immediately afterwards, and for the same reason, `_process_store_changes` applies the turn's `## Store changes` commands to the declared store and persists `<actor_id>-store.md` and `<actor_id>-store.json`. The turn is one transaction (see *Declared Persistent State* above). A scenario that declares no `store:` block skips this entirely. An actor that wrote no section at all has nothing applied and the fault is printed and recorded – silence is not a declaration of no change.
...
5. **Constitutional Referee Step (Optional)**:
  ...
  * **Model:** Uses dedicated `referee` model (default: `openrouter:meta/muse-spark-1.3-contributor`, like every other task) for cost-effective validation.
...
- **Overrides replace, never merge:** a scenario override fully replaces the default template for that prompt type (or, named `actor_<id>.md`, for that one actor). An actor-prompt override must therefore contain the entire system prompt including the statement-tier norms; several scenarios silently lost the statement contract this way before their stale pre-statement overrides were removed or rebuilt self-contained.
- **User prompt context:** `turn`, `time_period`, `metrics_json`, `world_state`, `historical_summary`, `notepad`, `background_context`, `output_language`, and individual metric variables (`metric_X`). Built by `_get_common_context`.
- **`background_context` and the fixed facts:** `background/context.md` seeds `world_state.narrative`, which the Game Master overwrites at the end of turn 1, so anything a scenario fixes at the start – an election result, a treaty, a map – would otherwise vanish after one turn. `background_context` keeps it available, with two rules that stop it saying the same thing twice. It is **empty in turn 1**, where `world_state` already *is* the context verbatim and rendering both put the entire opening description into the prompt a second time. From turn 2 it renders `background/fixed-facts.md` if the scenario has one, and the full context if not. `fixed-facts.md` is optional and is the compact standing restatement of the opening; scenarios without the file keep the previous behaviour from turn 2 on. A scenario that wants neither can drop the whole block from its own prompt overrides, as `europe-2032` does: its opening description is dated news rather than standing physics, and a block asserting that it outranks the narrative is wrong by the end of a long run. The trade is real — structural facts decay out of the rolling summary because nothing happens to them, so anything that must survive six years belongs in the metric rules or the event catalogue instead.
...
- **Provenance recording (`_render`, `provenance_for`):** every prompt render records which template file was used and where each interpolated value came from, as character spans into the rendered text. Recorded during assembly rather than inferred from the finished text, and keyed by a hash of the rendered prompt so a parallel multi-actor turn cannot mix two prompts up. `orchestrator._persist_llm_io` writes it into the `--log-llm-io` transcripts, and `scripts/render_signoff.py` turns it into the sign-off documents. The variable-to-source map is `PromptBuilder.VARIABLE_SOURCES`, with the turn-dependent entries (`world_state`, `background_context`) resolved by `_variable_sources`; a new context variable worth tracing belongs in that map.
...
- `OpenRouterProvider` – HTTP via `httpx`, reads `OPENROUTER_API_KEY`. The system prompt is sent as content blocks with an Anthropic-style `cache_control` breakpoint (stable across turns, so it caches); OpenRouter honors these on supporting routes and translates them for others, while non-caching routes ignore the hint. The run's dice seed doubles as OpenRouter's `session_id`, pinning sticky routing to one provider endpoint so the cache stays warm (`OPENROUTER_SESSION_ID` overrides). `prompt_tokens_details.cached_tokens` is folded into usage as `cache_read_input_tokens` (priced at the shared 0.1x read multiplier – exact for Anthropic-served models, approximate elsewhere).
- `AnthropicProvider` – official `anthropic` SDK, reads `ANTHROPIC_API_KEY`. System prompts are sent as cache-controlled blocks (ephemeral prompt caching) by default, since a task's system prompt is stable across turns while the user prompt changes; cache write/read tokens are priced with their 1.25x/0.1x multipliers in `cost.py`. Caching can be disabled via the provider constructor (`enable_prompt_caching=False`). Gateway routing follows the Claude Code convention: `ANTHROPIC_BASE_URL` plus `ANTHROPIC_AUTH_TOKEN` (Bearer) instead of the API key; `ANTHROPIC_API_KEY` must be unset or it shadows the gateway route.
- `OpenAICompatibleProvider` (`openai`) – HTTP via `httpx` against any OpenAI chat-completions dialect, endpoint from `OPENAI_BASE_URL` (default `https://api.openai.com/v1`), key from `OPENAI_API_KEY`. Covers OpenAI keys, local servers (Ollama at `http://localhost:11434/v1`, LM Studio, vLLM – any non-empty key such as `ollama`), and OAuth-to-OpenAI proxy bridges.
- `OpenCodeProvider` (`opencode`) – routes each call through an OpenCode server (`opencode serve` API): one fresh session per call, system prompt via the `system` field, `agent: "plan"` with `tools: {}` so the model answers instead of acting. Route model ids are opencode model ids (`opencode:<providerID>/<modelID>`, as listed by `opencode models`), giving access to everything authenticated in OpenCode without managing each auth flow natively. Spawns its own server in an isolated temp working directory by default (so project files do not leak into simulation context); `OPENCODE_SERVER_URL` attaches to a running server instead. Per-call `temperature`/`max_tokens` are owned by the OpenCode model configuration and ignored. Sessions are deleted after each call.
- `CodexProvider` (`codex`) – native ChatGPT-subscription backend (`chatgpt.com/backend-api/codex`) using the Codex CLI credential store (`~/.codex/auth.json`), so calls bill to the subscription, not API credits. Routes are Codex model ids (`codex:gpt-5.6-sol`). The backend requires SSE streaming with `store: false` and rejects `temperature`/`max_tokens`, which are accepted-but-ignored like the opencode provider. On a 401 the provider refreshes OAuth tokens and retries once, writing the new tokens back to the auth file. This is deliberately not the OpenAI Platform API, where ChatGPT OAuth tokens always fail.
...
- `OpenAICompatibleProvider` does the same; local models that reject `response_format` raise `LLMUnsupportedStructuredError` so the legacy parse path is used.
...
- `OpenCodeProvider` does not implement structured output at all (the server message API has no schema mode).
...
- **Event Timeline:** Builds a turn-stamped record of every event that fired from the per-turn `1-events.json` artifacts rather than from `summary.json`'s `occurred_events`. Runs saved before repeatable events entered that record hold only the one-shot ones, so reading the artifacts is both complete and backward-compatible, and the turn stamps are what make a precursor and its escalation legible as a pair.
...
- **Scenario Source:** Resume and branch reload the scenario from the run's recorded `scenario_source` (see Run-time Config Fields), not from the run's directory, so variant runs keep their own actors and resource patches.
...
- **State Loading:**
  ...
  * Reads `turn-XX/2-actors/<actor_id>.md` to restore each actor's `last_actions` – the response the actor prompt renders back as "Your previous response", and the only substrate for state an actor carries forward in prose rather than in its statement ledger, such as a measure portfolio or a standing commitment. Without it a resumed or branched run handed the actor an empty block and that state was lost at the seam, with nothing recording the loss. A missing file leaves the field empty rather than failing
  * Reads `turn-XX/5-notepad.md` to restore game master notes
  * Reads `turn-XX/6-historical-summary.md` to restore turn history
  * Reads `summary.json` to restore the occurred-events record, the turn-stamped `event_log`, and emerging developments. The suppression flag `Event.occurred` is re-derived rather than restored from the record, so repeatable events in it are not treated as spent; branch truncates `event_log` at the branch point
...
**Facilitator UI (`facilitator.py`):** stdlib-only localhost server with a single self-contained page, covering the same loop without a terminal (setup wizard, per-team picks, confirm-and-resolve, print stylesheets). Every action delegates to `live.py`; long LLM calls run as background jobs with per-run locks against double-submit. Same artifacts, so CLI and UI interoperate on one run.
