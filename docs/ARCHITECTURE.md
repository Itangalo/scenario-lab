...
- **Provenance per column.** `actor` (declared in a command; a judgement), `system` (stamped by the framework), `derived` (computed). This is the load-bearing part: with `started_turn` owned by the system there is no command that reaches it, so "copied forward, unchanged" stops being a rule the model obeys by remembering. An unknown column in a command is a rejection, never an addition.
...
- **Writes go under one declared header.** `## Store changes` in the actor's output, holding `add` / `update` / `delete` commands one per bullet, each able to carry an indented `- Grounds:` line. Three reasons it is not inline: a turn can be re-run up to `constitutional_enforcement.max_attempts` times, and a scattered command mutates state partially on a partial re-emission; a missing *section* is detectable where a missing inline command never was; and free-floating syntax means any quoted string containing the pattern writes to state. The section is required every turn, saying `No changes.` when there is none, exactly as statements require `No statement changes.`
- **A turn is one transaction.** `Store.begin_turn` restores the pre-turn snapshot before applying, so a turn executed twice replaces its own previous application rather than appending to it. Individual commands are still accepted or rejected one at a time, as statement proposals are – an all-or-nothing turn would discard good commands because of one typo.
...
- **Scope.** `scope: actor` only. World-scoped tables are a real part of the design but need a writer, and the only step that could write one is the step that also produces the narrative and the metrics JSON; `scope: world` is rejected at load rather than half-wired, so a scenario cannot depend on a table nothing writes to.
