...
- **`background_context` and the fixed facts:** `background/context.md` seeds `world_state.narrative`, which the Game Master overwrites at the end of turn 1, so anything a scenario fixes at the start – an election result, a treaty, a map – would otherwise vanish after one turn. `background_context` keeps it available, with two rules that stop it saying the same thing twice. It is **empty in turn 1**, where `world_state` already *is* the context verbatim and rendering both put the entire opening description into the prompt a second time. From turn 2 it renders `background/fixed-facts.md` if the scenario has one, and the full context if not. `fixed-facts.md` is optional and is the compact standing restatement of the opening: `europe-2032` runs 896 tokens against `context.md`'s 2,528, and adding it cut that scenario's turn-1 actor user prompt from 6,960 tokens to 4,180. Scenarios without the file keep the previous behaviour from turn 2 on.
