...
- **Scope.** `scope: actor` tables are written by actors; `scope: world` tables belong to the run, are written by the Game Master step, and are read by every step. The boundary is enforced, not requested: an actor's entry touching a world table is rejected, and the Game Master cannot reach actor tables. World writes land in the same turn transaction the actor step opened, and persist in `turn-XX/4-world-store.md` (readable changelog) beside `turn-XX/4-world-store.json` (the freshest snapshot -- actor files were written before the Game Master step ran, so `resume` prefers it when present).
