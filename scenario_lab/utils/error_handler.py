"""
Error Handler - Enhanced error handling with detailed messages and recovery strategies (V2)
...
V2 Design:
- No V1 dependencies
- Works with V2 ScenarioState
- Compatible with V2 phases
"""
import sys
import traceback
...
class ErrorHandler:
    """Enhanced error handler with user-friendly messages and recovery suggestions (V2)"""
    ...
    def _get_budget_recovery_actions(self, context: ErrorContext) -> List[RecoveryAction]:
        ...
        # Note: V2 uses ScenarioState for resumable scenarios
        # The run directory structure is the same but orchestrator handles resume
        if context.scenario_name and context.run_number:
            run_dir = f"{context.scenario_name}/runs/run-{context.run_number:03d}"
            actions.append(
                RecoveryAction(
                    "Resume with increased budget limit (if using V2 orchestrator)",
                    command=f"# Use orchestrator to resume from {run_dir}",
                    priority=1
                )
            )
    ...
    def _get_state_recovery_actions(self, context: ErrorContext) -> List[RecoveryAction]:
        ...
        actions = [
            ...
            RecoveryAction(
                "If resuming, verify the state file is valid JSON",
                command="python -c \"import json; print(json.load(open('state.json')))\"",
                priority=2
            )
        ]
    ...
    def _get_configuration_recovery_actions(self, context: ErrorContext) -> List[RecoveryAction]:
        ...
        actions = [
            ...
            RecoveryAction(
                "Validate your scenario YAML using the V2 schemas",
                command="# Use scenario_lab.schemas for validation",
                priority=2
            )
        ]
