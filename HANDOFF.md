...
- [scenarios/europe-2032/design-notes.md](scenarios/europe-2032/design-notes.md) – what the runs so far showed, the open question they left, and why turn 1 stopped being a fork
...
3. **`story/turn-01/superseded-roads/` can go.** Nothing points at it now, and what it was provenance for – the two archived batches – is under gitignored `runs/`, so it does not survive a clone. Everything worth keeping from it is in `design-notes.md`; git history holds the rest.
4. **The tree is larger than it looks.** The naming scheme implies 56 turn pieces per arm and 168 across the three, plus 42 options, each built from ten or more simulations. Worth sizing deliberately before building, at roughly seven minutes per turn per run.
