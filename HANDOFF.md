...
3. **The `Cheapens:` citation drifts.** The price is right; the attribution names whichever event fired most recently rather than the one whose window is open. The audit trail is the only thing making the rule checkable from artifacts. Probably fixed by requiring the window in the line.
4. **The committed sign-off documents are stale.** Generated from `run-20260829-192725`, before the mechanics changes. Regenerate: run 2 turns with `--log-llm-io`, then `python scripts/render_signoff.py <run-dir>`.
