...
**Six events are marked "Always eligible" and must appear in every turn's array, without exception:** `cyber_mass_campaign`, `bio_incident`, `taiwan_blockade`, `capability_jump`, `ai_market_crash`, `rsi_onset`. For those six, and for no other events in the list, conditions are never unmet — only their probability changes. Give each of them its gate-open probability when its gate is open, and its gate-shut probability when it is not. The gate-shut probabilities are small but never zero, and dropping these events from the array because their gate is shut is an error: it removes the surprise the scenario exists to study, by making every escalation look as though it must have been preceded by a warning. `rsi_onset` is the one whose availability depends on the regime rather than on a gate: it appears in your array exactly when it appears in the list above, always at the stated 30%.
...
- **Institutional reactions belong here, not on the list.** When the regulator's own portfolio gives them footing — a flagship restriction in force, measures spanning many jurisdictions, standards with real pull — propose emergent events such as `emergent_court_challenge` (a court suspends a core provision), `emergent_member_state_noncompliance` (a jurisdiction quietly stops implementing), or `emergent_rival_standards_body` (a competing bloc launches lighter rules). These cannot be timed from metrics alone; they arise from what the regulator has actually built, which you can see and it cannot.
- **A description never names this run's trajectory regime.** Never write FAST, PLATEAU or RLVR-LIMITED in a description or id, and never write as though you were announcing what is coming: descriptions reach the regulator when an event fires. Describe the development itself.
