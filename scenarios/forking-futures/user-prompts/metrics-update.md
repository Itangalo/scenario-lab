...
1. **Never write the uppercase labels `FAST`, `PLATEAU` or `RLVR-LIMITED` anywhere in the Narrative — not in prose, not in a subheading, not as "consistent with".** You know which regime this run is in and apply its rules; the regulator does not, and from turn 2 onward the narrative is exactly what the regulator reads. Name behaviours, never the pattern: capability growth that slows, accelerates or splits between domains is described by what it did this turn, full stop.
