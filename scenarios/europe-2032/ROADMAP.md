...
- [~] **The event list is fairly balanced.** *(Superseded 2026-09-10 – the catalogue is now 37 events, not 35: seven cut, four added, several rewritten. The fire-rate finding below described the old list and has to be re-measured after the probability pass. See `design-notes.md`, "The event-list pruning pass".)* All 35 events fire at least once across the 32-run corpus; per-listing fire rates run 1–16% and every event touches at least 5% of runs. Nothing in the catalogue is dead and nothing is scenery. Measured mostly before the open-weight fix, so the incident events gated at `openweight_capability` 55 and 65 were rolled with those branches shut – their true rates are higher than the table says, which does not change the conclusion.
...
**State, 2026-09-11.** (ECHO 2026-09-11) 193 of 207 nodes written on the rerun tree in form A (Sofia Brandt, Claire, dated clippings, stage memos; see `story/README.md`). Remaining: the P22 subtree – turns 6–9 of P22, option-10-P221/P222, and turns 10–13 of P221 and P222 (14 nodes; `grep -L '^status: written' story/tree/*/*.md` lists them). Liberties with the record are noted as HTML comments in the nodes they touch. check_tree clean. Needs a read-through by Johan before anything is published.
...
- [X] **The event list has been re-read since the store redesign.** Done 2026-09-10, before any measured batch as the item required. the catalogue went 37 to 37 by way of 30: seven events cut for scoring mid on both credibility and payload, and seven added — three catastrophic "all bets are off" events, `us_labs_nationalised` as the one event that voids metric rule 9's mechanism rather than merely resisting it, and `knowledge_work_augmented`, `embodied_ai_deployment` and `research_breakthrough` for coverage the list lacked. Downstream references updated in the same pass and the scenario validates. Probabilities are the next pass and are not yet calibrated – several figures in the new and rewritten entries are first guesses. `knowledge_work_augmented`, `embodied_ai_deployment` and `research_breakthrough` were then added to cover the verification-bound arm, embodied AI and AI-driven research, bringing the catalogue to 37. Full account in `design-notes.md`. **Note for the next catalogue change:** `validate` covers the base scenario only. Cutting two events broke all three variants while the base still validated clean, so every arm must be loaded as well.
...
- **Length.** (ECHO 2026-09-11) Settled: each stage is told as one section at the length of a Europe 2031 section, 600–850 words with a ceiling of about 1,000 (`story/README.md`, measured in `story/experiments/voice-from-europe-2031.md`). Samples of one full path are in `story/experiments/stage-sections-sample.md` and wait for review before the full writing starts.
