# Metric rules patch — capital paralysis

Appends one rule. Everything else is inherited from the verification-bounded arm.

19. **Below 20, the Union is no longer in control of its own agenda.** Political capital has so far been a score that fell without changing what the Union could do. It is a budget, and running out of it has to bite.

    While `eu_political_capital` is below 20, and only while it is:
    - **the named priority has no effect.** Rule 10's pull-in-by-one-turn does not apply, and pushing a measure buys nothing. The priority is still named and still costs its −1: the Union is still trying.
