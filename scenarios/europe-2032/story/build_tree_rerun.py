...
# --- stage-3 promotions (2026-09-23) -------------------------------------------
# A path follows rep 1 unless promoted here. Both promotions rebalance the
# catastrophe endings toward what the reps show: A122's rep 1 is the only one
# of ten with a war; V212's rep 8 carries a catastrophic bio incident, and one
# such ending on a non-Acceleration arm matches the ~3% the V/P reps show.
PROMOTIONS = {
    "A122": (2, "rep 1 is the only war among ten reps; rep 2 is the first clean one"),
    "V212": (8, "one non-Acceleration catastrophe ending: catastrophic bio incident in turn 13"),
}
...
def main() -> int:
    ...
    s3paths = {t["path"]: (t["dir"], t["seed"]) for t in
               json.loads((STORY / "stage-3-blocks-rerun.json").read_text())
               if t["rep"] == PROMOTIONS.get(t["path"], (1, None))[0]}
    ...
    for block3, parent2 in STAGE3_PARENTS.items():
        ...
        blocks.append({"block": block3, "arm": block3[0], "stage": 3,
                       ...
                       "option_file": opt_file,
                       "path_rep": PROMOTIONS.get(block3, (1, None))[0],
                       "promotion_reason": PROMOTIONS.get(block3, (1, None))[1],
    ...
    tree = {"scenario": "europe-2032",
            "note": "Rerun source of truth (Muse Spark + store branch, "
                    "2026-09-10/11). 42 path blocks, rep 1 except the stage-3 promotions (A122 rep 2, V212 rep 8; 2026-09-23). "
    ...
    for (path, rep), (d, s) in seed_of.items():
        ...
        if hit and rep == hit[0]["path_rep"] and hit[0]["seed"] != s:
