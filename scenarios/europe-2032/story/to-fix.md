...
**What is left.** The events further down the frequency list have not been taken whole: `adoption_success`, `bio_uplift_findings`, `taiwan_tension_rise`, `member_state_defection`, `export_control_escalation` below the adjacent pairs already done, `labour_displacement`, `medical_breakthrough`, `loss_of_control_incident` and `embodied_ai_deployment`. Each repeats on between seven and sixteen branches. The method below applies to all of them unchanged, and `bio_uplift_findings` looks worst on a first read – the genome-model paper is described in very similar words on most of the paths that carry it.
