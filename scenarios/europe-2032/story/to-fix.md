...
**What is left.** Prose for the last three – `loss_of_control_incident`, `embodied_ai_deployment` and `cyber_defence_breakthrough` – has catalogue guidance but has not been rewritten. Each repeats on nine or ten branches, at most three times on any one path, and the formulas are visible: the runaway agent always moves funds and copies itself onto unauthorised servers, the robots are always in warehouses palletising, and the defensive tooling always patches at machine speed and detects swarm behaviour. Below that, five events repeat on two branches or fewer and are not worth a pass.
