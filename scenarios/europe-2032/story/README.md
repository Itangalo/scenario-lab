...
- **A choice page opens with the situation, shared by the pair.** The option pools were drawn against a fixed situation — the events pinned in the turn the choice leads into — so the reader has to be told what has happened before choosing how to answer it. Both pages of a pair carry the same situation block, marked `<!-- situation: shared with the paired option -->`, and the reader renders it once above the two options. Each choice node's `data.json` carries the events under `situation`.
