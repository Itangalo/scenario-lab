...
(ECHO 2026-09-11: the tree below this point describes the retired
pre-store build. The live tree is `tree.json` (regenerated by
`build_tree_rerun.py` from the Muse Spark + store manifests), the nodes in
`tree/` (207: `data.json` + `record.md` machine-written per node,
`narrative.md`/`choice.md` as drafted scaffolds), the path locks in
`stage-1-rerun/PATHS.md`, `stage-2-PATHS.md`, `stage-3-PATHS.md`, and the
turn-1 frame in `turn-01/opening.md`. Branch tables below are superseded;
the old `stage-1/2/3/` prose and `pool-06-*`/`pool-10-*` inputs live in git
history. Writing rules below still hold.)
...
(ECHO 2026-09-23) A path stops at the half-year in which a catastrophic event fires (`catastrophic_great_power_conflict`, `catastrophic_loss_of_control_incident`, `catastrophic_bio_incident`). The scenario is not written to simulate the world after one, and the runs carry on as if it were business as usual. `scripts/build_story.py` drops the onward link and the choices from that node and ends it with the box in `catastrophe.md`, followed by the postamble. The nodes after it stay in `tree/` but no reader reaches them. In the current tree this cuts seven of the eight Acceleration endings (in 2031) and one Verification-bound ending (a catastrophic biological incident in the second half of 2032). Two stage-3 paths are promoted away from rep 1 to get there (2026-09-23): A122 follows rep 2, because rep 1 was the only one of its ten reps with a war, and V212 follows rep 8, to carry one of the bio catastrophes the V/P reps produce at about 3%. `PROMOTIONS` in `build_tree_rerun.py` holds both. `build_compare.py` likewise stops counting a run, both as a sibling and in the corpus, from the turn after its own catastrophe.
...
Faults we intend to act on rather than accept live in [`to-fix.md`](to-fix.md), with the affected nodes and the scope of each fix. (ECHO 2026-09-16)
...
- **Third person, close to power.** (ECHO 2026-09-11) Replaces the earlier second-person rule, after the form trials in `experiments/form-trials.md`. The story follows Sofia Brandt, who holds the AI file in the Commission President's cabinet. She writes the options and sits in the room, but she does not decide. The reader's choice is what she recommends, and the College adopts it. A failure is watched rather than owned. Member states, providers, the United States and the public push back, and Sofia sees internal division from inside the building. Her sister Claire, a triage nurse in Lyon, is the foil, in texts. Not every path reaches hospitals the same way, but every path can be read so that it does.
- **Form.** (ECHO 2026-09-11) Each half-year has a mood sub-header, one scene given room, two to four dated clippings for events that need only a clause, and texts from Claire where they earn their place. A memo from Sofia to the President may close each stage and set up the choice; this is being judged on `experiments/stage-form-a-sample.md`.
...
- **Mark the date now and then.** (ECHO 2026-09-11) Calendar time is not only a ban on turns: the reader should be told when they are. Open each section, and each subsection that moves the calendar on, with a time anchor – "in March 2028", "during the summer of 2031", "by the spring". A named month must lie inside the half-year the run resolves in, and where the run's world state names a month, that month is used. For an extreme event, an exact date and time signal significance ("At 03:40 on 11 March 2032, missiles meant for someone else's war come down on European soil"). That precision is invented, the way a private character is, and is used at most once per section. `check_tree.py`'s numbers rule does not yet exempt calendar dates and clock times, and has to before such prose enters the tree.
...
- **Length.** (ECHO 2026-09-11) Each stage – 2027–2028, 2029–2030 and 2031–2032 – is told as one section, aimed at the length of a Europe 2031 section: 600–850 words, with a ceiling of about 1,000. Each half-year gets its own sub-header, and the sub-header names the half-year's mood ("Error messages", "Boil-water notices") rather than its dates. The measurement is in `experiments/voice-from-europe-2031.md`, and it replaces the old budget of about 250 words per turn. The opening is still the deliberate exception: it establishes a world and a role that no later node re-establishes. How a stage section maps onto the four turn nodes, and what that means for `extract_tree.py`, `check_tree.py` and `build_story.py`, is not yet decided.
- **Liberty with the record.** (ECHO 2026-09-11) The runs are evidence, not a script, and they sometimes produce events in an order or combination nobody would believe. One example: tension rising around Taiwan after the United States and China are already at war, when a blockade would come with the war rather than after it, unless it came as an outright invasion. The writer may fix that. Within a stage, events may be reordered, two events merged into one development, or one told as part of another (the Taiwan tension becomes a front of the war). Where the event catalogue leaves the concrete form open, the writer chooses it, and may give an event the cause the run left implicit. An event that contradicts the world already told, or adds nothing, may be left out; the continuity report is a report, not a gate. What stays fixed: the measures the reader started and what became of them, the US election result, each stage's major shocks (cut-offs, catastrophes, strikes with casualties), and where things stand at the end of each stage. A liberty must also hold for everything later on the same path. The later stages were simulated from the record as it was, and they may refer back to it, so every liberty is noted where the next writer on that path will find it. Figures are still never invented.
- **A catastrophe ends the story, and the half-year has to earn it.** (ECHO 2026-09-23) The runs tell a catastrophe as one development among many, and the box that follows calls it catastrophic, so the prose must carry the weight the box claims. The half-year ends on the catastrophe: other events and measures come first, compressed to clippings where they can be, and nothing routine follows it. Make it felt beyond Brussels – what happens to society (panic, mutual help, looting, the breakdown of order where it breaks down, people dying of things that were routine a week earlier) and to Claire. The catalogue leaves the form to the writer, and the eight current endings deliberately differ: an extortion deal, networks turned to noise, a ceasefire the machines ignore, missiles on European soil, a second front in the east, blinded early warning, a single nuclear detonation, and an AI-designed pandemic. A new ending should not repeat one already on the tree. Because nothing on the path follows, the liberty rule's later-stages constraint does not bind here, but the rest does: the events that fired and the measures the reader started are told, and figures are still never invented. The reader's own choice must never read as the cause – the box says the catastrophe was most likely not caused by the reader's choices, and that the last choice could not have caused it, so the prose must not contradict either (a measure the reader chose may fail or arrive too late, but it does not provoke the event). Check the second claim against the tree whenever it is rebuilt: it holds today because each catastrophe either sits in a half-year fixed before the last choice, has odds the Union does not move (war), or has odds the reader's own measure could only lower (the V212 pandemic).
...
(ECHO 2026-09-24) The runs of this first tree, which were run on Qwen, were deleted on 2026-09-24: the pinned turn-1 runs above, all Stage 1–3 runs (2026-09-04 to 2026-09-07), and eleven Qwen test runs from 2026-09-09 and 2026-09-10. The manifests `stage-1-blocks.json`, `stage-2-blocks.json` and `stage-3-blocks.json` were deleted with them. The run directories named in this section and the next two therefore no longer exist; the last commit that contains them is `3ec7a1f7b`. The rest of the first tree's material was removed the same day: its option pools without a date (`pool-06-{A1,A2,V1,V2,P1,P2}`, `pool-10-{A11 … P22}`), its turn-1 pool `turn-01/pool-20260904`, `option-06-proposal.md`, `option-10-proposal.md`, `stage-1/` and `t5-winners-superseded.json`; they remain in `eb5b87988`. The published story is built from the rerun (`*-blocks-rerun.json`, `tree.json`, the dated `pool-*-20260911` pools and `turn-01/pool-20260910`), and its turn 1 is pinned to the Muse Spark runs `run-20260910-223233` and its five siblings.
