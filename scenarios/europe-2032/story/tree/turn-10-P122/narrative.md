---
node: turn-10-P122
turn: 10
period: H1 2031
block: P122
status: written
prev: option-10-P122
next: turn-11-P122
---

# Two speeds

> **January 2031.** Research groups on both sides of the Atlantic demonstrate patching and detection tools that finally keep pace with automated probing, closing off a whole class of swarm-like intrusion.

It is genuinely good news for defenders. ENISA pushes the updates through the certified hospitals and the larger utilities where the earlier containment drills had taken hold, and there, exercises that failed a year ago now pass quietly. Where the certificates never arrived, little changes. Smaller town networks and subcontractors cannot deploy the new tooling, and two ransomware waves in March still force manual shutdowns. The engineers speak of a continent running at two speeds: protected cores, brittle edges.

> **Claire:** we got the new patch. the village clinic up the valley didn't
> **Claire:** we sent them paper forms. that's the solidarity
>
> **Sofia:** They're on the list.
>
> **Claire:** everyone's on the list. the list is long

> **February 2031.** A contested biosecurity paper reports that a genome model, prompted by a non-expert, produced what reviewers call a viable design for an organism able to infect humans.

The authors are accused both of hype and of publishing too much. In Brussels the file goes to the health emergency planners and to the new compact's partners for joint scrutiny. No incident follows, but stockpile orders and audits of the detectors quietly rise.

Neither development touches the grievance that dominates. Graduate hiring in law, accountancy, software and customer operations stays frozen. Sofia's guarantee – co-financed income bridging, six-month retraining vouchers and hiring credits in the four sectors, with reporting duties on large deployers of AI – clears the labour ministers quickly, and the President backs it in the College and in public. Then it stumbles in the finance talks. Two net-contributor capitals demand a capped, temporary envelope and hold up the disbursement rules until late spring.

By June the first payments have reached only pilot cohorts, and student unions call it a voucher for jobs that no longer exist.

The middle-power compact formally enters into force, pooling hosting quotas and swapping incident feeds. Dublin still has not unwound its separate supply arrangement, and the cohesion stays cracked even as Brussels claims leverage.
