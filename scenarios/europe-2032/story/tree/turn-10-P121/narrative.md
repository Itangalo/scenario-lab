---
node: turn-10-P121
turn: 10
period: H1 2031
block: P121
status: written
prev: option-10-P121
next: turn-11-P121
---

# Patches where the drills failed

> **February 2031.** A newer generation of automated patching and swarm detection, built elsewhere and freely shared, is released to defenders.

ENISA moves fast. Sofia's sprint pushes the new packages to national response teams, municipal utilities and hospitals, with orders to re-run last year's failed containment exercises, and the President names it the priority, and pays for it in political capital.

In the certified hospitals it works. In two dozen uncertified water and power utilities, the tools catch probing swarms that older, pattern-matching software had missed, and the engineers speak of the first defensive win in years. Elsewhere the rollout stalls. Old control systems cannot take the patcher, tenders slip, and a French association of municipalities publicly blames Brussels for mandating software without funding it.

> **Claire:** the new patch caught a swarm at the water plant next to us
> **Claire:** the engineer came over with cake. to the hospital. for no reason
>
> **Sofia:** There was a reason.
>
> **Claire:** i know. it was nice cake

> **March 2031.** A contested genome-model study circulates. Reviewers say a non-expert with assistance could reach a viable design for an organism able to infect humans.

The authors are accused of alarmism and of publishing too much. The health officials call the health emergency authority, and the authority orders wastewater and clinical sequencing at hospital laboratories, linked to the compact's incident swap. The detection capacity does not arrive in time to settle the argument. The sampling contracts are signed, and the reagents and staff are short.

Dublin again links its cooperation to its separate supply arrangement. It accepts priority access to the defensive tooling and leaves its side deal untouched. The compact itself formally completes its term.

Graduate hiring does not recover. Law and accountancy firms confirm that entry-level posts will stay frozen, and the labour ministers finish their unfunded transition plan for next year. The Commission holds its spending at home flat – the concrete and certificates on old money – betting that visible hardening will buy it room to fund retraining later.

By June the patch coverage is uneven and the bio sampling is partial. But the blackouts have been avoided, and the clinics feel watched over rather than exposed. The relief is cautious, and mixed with dread about the biology paper.
