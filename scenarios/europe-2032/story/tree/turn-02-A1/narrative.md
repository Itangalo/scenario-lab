---
node: turn-02-A1
turn: 2
period: H1 2027
block: A1
status: written
prev: option-02-1
next: turn-03-A1
---

# Copycats

> **January 2027.** A freely downloadable model, close to the leading closed systems, is copied hundreds of thousands of times in its first week. What is new is not the model itself but the box around it: the add-ons that let it use tools and work through tasks on its own ship with the release. What used to need a team needs an afternoon. Unlike a closed model, none of it can be withdrawn.
>
> **February 2027.** Forensic samples of last autumn's grid-intrusion tooling have leaked and been repackaged into kits that need little skill to use, researchers warn.

The probes that follow are clumsy and relentless, and they go where the defenders are thinnest: municipal water operators and small grid suppliers, from the Rhineland to Moravia. Nobody expected the test shot's tools to end up in the hands of amateurs, and nobody is surprised that they have.

Sofia spends the spring turning the Shield towards the towns. Detection that the Union bought jointly is pointed at local utilities. Patch-and-isolate playbooks become mandatory, which means that an operator who finds an intruder cuts the system off first and asks questions later. Backup power and manual-override kits are paid for from Brussels, but only for towns that join the joint exercises. In April an interior minister who spent the winter fighting the obligations signs for the money and complains about the conditions in the same sentence.

It works for the large operators. The small ones get the kit and have nobody to run it, and twice that spring a town isolates itself correctly, keeps the intruders out, and loses its billing and monitoring for days.

The gigafactory programme, meanwhile, stalls without anyone announcing it. It is still twenty billion euros and four to five sites. By June only the two sites near secured power are moving, and the rest are frozen.

> **May 2027.** Military exercises around Taiwan run longer than any in recent years. Shipping insurers raise premiums for the strait.

EU ministers agree that the power to stop exports by ASML, the Dutch maker of the chipmaking machines, is worth more held in reserve than used. Sofia writes that paragraph, and she thinks it is right.

> **Claire:** the triage thing is actually good btw
> **Claire:** waits down all spring. first time since i started
>
> **Sofia:** Good. Enjoy it.
