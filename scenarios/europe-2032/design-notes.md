...
**Listing holds. 26 of 2496 required listings are missing, about 1%** — measured once, on 2026-09-03, by a check that is no longer kept: it read the words "Always eligible" out of this scenario's condition prose, which makes it a europe-2032 check rather than a Scenario Lab one, for a failure mode this small. What `check_events.py` still scores is the contract the framework itself declares, `event_groups` with `due_turns`, which is the one that actually failed here. The six always-eligible events must appear every turn whatever the world does; `taiwan_blockade` accounts for 16 of the misses and `bio_incident` 7, almost all in turns 8–13. The 2028 election family was complete in every one of its due turns, which is the machinery that failed before the group mechanism existed. This is inside the roadmap's stated tolerance of one in ten, and it is the first time the claim has been checked rather than assumed.
...
## Softening the event rates (branch `soften-world`, 2026-09-03)

On the 15-run batch the world reads harsher than the story needs: 2.43 fires/turn/run, cyber alone tripping the harm floor in 7 runs, loss-of-control 5× in one verification run, and the dark-world adders (+10s firing constantly once sentiment and capability cross their thresholds) feeding the doom loop. Decision: soften events only — rules, starting values, constitution and floors untouched, so any difference measures the event catalogue and nothing else.

What changed in `events.md`, all base rates and amplifiers down roughly a quarter to a third, positives and gates untouched: cyber 20/6→14/4 with the openweight adder +8→+5; rsi_onset 30→20; campaign trio 25/30/20→20/25/15 with adders trimmed; eval_anomaly 15→10; capability_jump 15/5→10/4; loss-of-control 12/3→8/2 with the gap adder +10→+6; labour 15→10; taiwan_tension and export_control 15→10; frontier_access_denied 12→10; member_state_defection 10→8 with the capital adder +8→+5; backlash_physical 10→8; supply_chain_coercion 10→8; automated_decision_scandal 8→6. Election weights, bio_incident, blockade, breakthroughs, agreements and adoption_success unchanged.

Measurement: 2 runs per arm (6 runs) against the 15-run baseline — fires/turn, cyber frequency, capital/sentiment finals, floor discrimination. Keep the branch if the world softens without going slack; discard it if harshness was load-bearing.

## Second wave on the branch (2026-09-03, while the 6-run batch is still measuring the trims)

Johan's additions: the running batch jobs loaded their files at startup, so these edits land after the trim measurement and need their own comparison later.

- **Starting `resilience` 35 → 42.** Runway while the 50 absorption floor still binds. Capital stays at 48 on purpose: its reference prose is anchored there, and capital's disease is drain rate, not starting level — a higher start only delays the same collapse.
...
## Emergent examples removed, both halves (2026-09-04)

The mimicry finding: pre-positivity-line, three negative prompt examples became over a third of all emergent proposals (court_challenge 1,289 alone); post-line, the new positive examples were copied the same way (talent_return_wave 63). Examples don't illustrate the distribution, they become it — compounded by track_unfired re-listing. Decision, for speed over attribution: remove all six examples at once and replace with example-free guidance (vary forms widely, invent fresh ids, with the tracked-development re-listing explicitly still required). Structural requirements untouched. If emergent volume collapses without scaffolding, restore the positives. Measuring with 5 runs/arm × 13 turns (~370 proposals expected): distinct-id count and positive share vs the 891-proposal post-line corpus.
...
**Measured on six verification runs, 4–8 turns each (2026-09-10).** Across 35 turns the store took 85 commands. No entry was ever lost, which is structural rather than lucky. What the runs did find, in order of how much they cost:

- **Six measures lost in one eight-turn run** to a command that also wrote `cost_per_turn`. The actor is shown the rendered rows — the rows *are* the portfolio — and it copies the columns it sees. A known-but-unwritable column is now dropped and noted rather than rejecting the command; an unknown one is still a rejection. Re-running the same seed and arm afterwards: 17 commands, none rejected.
- **A two-id delete kept the second record**, silently, because `, M8` was absorbed into the reason clause. This module's own failure mode inside its own parser.
- **The finishing-turn boundary was never written down.** Whether a measure is charged in the turn it finishes had no answer in the text; the framework had decided one way and the Game Master read it the other, which accounts for every charge-line mismatch in the first batch. Rules 6 and 10, the metrics step and the actor prompt now all say the same thing.
- **One initiative was carried twice** for two turns at full cost, because the actor believed its first `add` had not gone through. Noted, not refused: names are not keys.
