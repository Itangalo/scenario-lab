...
4. **Show the portfolio charge before you apply it.** Metric rule 10 charges `eu_political_capital` for every unfinished measure, every turn, on all of them and not only the priority: 2 for a large measure, 1 for a small one, plus 1 for whichever is the named priority, less 1 for any the world plainly favours. Write one line in the Notepad in the form `PORTFOLIO CHARGE: <measure> −N, <measure> −N, … = −TOTAL`, then apply that total. It is written down because it is the rule most easily skipped: a turn in which several measures are unfinished and capital did not fall is a turn this rule was forgotten.
