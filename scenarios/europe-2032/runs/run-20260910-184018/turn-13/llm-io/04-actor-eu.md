...
- characters 456-2744: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2788-5172: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6374-7050: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10492-12640: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): A Europe that keeps essential services running through open frontier AI
- `scarcity_doctrine` (position): With frontier investment collapsing and open weights within months of the frontier, we stop chasing frontier parity and buy resilience, restoration capacity and control over dependencies instead.
...
|---|---|---|---|---|---|---|---|---|---|
| M13 | EU Augmentation Dividend Demonstrators | 9 | small | 12 | 14 | own jurisdiction | public_sentiment up moderately, resilience up slightly via absorption | 2 | running |
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 7 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Assurance Adoption Cell | 5 | small | 2 | 4 | own jurisdiction | ai_safety up moderately | 2 | finished |
| M5 | EU Transition Bridge for AI-Displaced Workers | 7 | small | 3 | 7 | own jurisdiction | resilience up moderately via social absorption, public_sentiment up | 2 | finished |
| M6 | EU Assurance Scale-up and Anomaly Watch | 5 | small | 4 | 8 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via own evaluation capacity | 2 | finished |
| M7 | EU Continuity Switch for Cut-Off Services | 6 | small | 5 | 7 | own jurisdiction | resilience up moderately, public_sentiment up slightly via kept services running | 2 | finished |
| M8 | EU Workplace Augmentation Pact | 9 | small | 7 | 9 | own jurisdiction | public_sentiment up moderately, eu_political_capital up slightly, resilience up slightly via absorption | 2 | finished |
| M9 | EU Municipal Restoration and Dependency Triage | 6 | small | 8 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly via detection | 2 | finished |
| M10 | EU Open-Weight Hardening and Dependency Removal | 6 | small | 9 | 12 | own jurisdiction | resilience up moderately, ai_safety up slightly via detection | 2 | finished |
| M11 | EU Accession to Joint Cyber Command Telemetry Pool | 8 | small | 10 | 12 | coalition | resilience up moderately | 2 | finished |
| M12 | EU Municipal Continuity Surge | 6 | small | 11 | 13 | own jurisdiction | resilience up moderately | 2 | finished |
...
## New measure
**EU Augmentation Dividend Demonstrators**
It puts the measured office productivity gains to visible public use by funding augmentation assistants for municipal front-offices, clinics and SMEs with retained-staff conditions and fraud safeguards.
This now because offices show augmentation raising output without job loss while communes resent borrowed detection and distrust AI, so demonstrating retained jobs plus faster permits and appointments directly serves our two-year commitment to keep essential services running through open frontier AI; as a diffusion and public trust programme it is category 9.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Augmentation Dividend Demonstrators", "category": 9, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up slightly via absorption"}, "grounds": "office productivity gains without job loss amid low trust"}]}
```

## Priority
M12 EU Municipal Continuity Surge, because frozen screens and paper queues still decide legitimacy while the new open-weight release lands, outranking even our own demonstrators this turn.

## In practice
We keep CER/NIS2 crisis coordination live: ENISA triages via the joint telemetry feed, Shield and civil protection surge teams stay in hardest-hit communes with clean backups and re-entry kits paid by reprogrammed Digital Europe and UCPM funds, and distributors/ports hold isolation discipline. Mayors keep 24-hour reporting in exchange for crews; hospital AI stays uncertified and gigafactory plots stay fenced-guarded, no new concrete.

We launch the Demonstrators via Employment/Social and Competitiveness Councils: ESF+ and Digital Europe SME vouchers for supervised office assistants in town halls, clinics and accountancy/legal SMEs, conditioned on no net layoffs and logged human sign-off, with ENISA hardening and fraud-hotline linkage. Employer federations get flexibility, unions get retraining seats and hiring-back audits from the spring evidence, to turn augmentation from resentment into shorter queues.
...
```
## New measure
**EU Essential Services Continuity and Bio-Response Surge**
It keeps hospitals, water, power and registries degrading-not-stopping through the automated ransomware sweep and stands up weeks-long bio containment with detection, PPE stockpiles and cross-border medical aid.
This now because a model-generated cyber sweep plus a real model-linked bio release hit brittle communes and clinics at once, and our two-year commitment to keep essential services running through open frontier AI demands exercised continuity and bio-response over any new build; as contingency plans with exercises plus hardening and detection it is Preparedness and resilience.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Continuity and Bio-Response Surge", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via detection"}, "grounds": "answer simultaneous major cyber and bio incidents under open-weight pressure"}]}
```

## Priority
EU Essential Services Continuity and Bio-Response Surge, because live blackmail freezes and a containing bio agent outrank even our demonstrators while M12 surge aid closes.

## In practice
We trigger CER/NIS2 and UCPM/RescEU together: ENISA triages the sweep via the joint feed with single-redaction to restore speed, Shield isolates updater and restores clean backups, Health Emergency Preparedness and Response Authority with ECDC runs bio detection, contact tracing and stockpiled countermeasures. Council Health and Home Affairs back 24-hour reporting; mayors keep crews only if isolation holds.

We fund from reprogrammed Digital Europe, EU4Health and UCPM, no new money vote: hardened terminals first to clinics and front-offices hosting our Augmentation Demonstrators, supervised assistants paused where networks are compromised, fraud-hotline and human sign-off kept. Double-redacting dispute goes to a time-limited data-retention audit to unblock telemetry without cutting the allied feed.
