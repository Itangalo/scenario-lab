...
- characters 377-1779: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1823-3991: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4047-5157: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5220-27932: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28287-28425: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Shield −3, EU Assurance Adoption Cell −2, priority −1 = −12
PROPOSAL BONUS: EU Assurance Adoption Cell (cat 5, small) +2 — safety_breakthrough this turn and cyber_major_incident exposed assurance gap this answers directly
...
- `emergent_bio_publishing_freeze` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher and preprint servers adopt mandatory AI-assisted screening disclosure and coordinated hold procedures for high-risk biological design papers after the phage controversy.
- `emergent_health_data_leak` -- first noted turn 1, listed in 2 turn(s) so far: Major European research hospitals publish evidence that clinical triage copilots built on open-weight models leaked sensitive patient data through prompt-logging pipelines, triggering cross-border data-protection investigations.
...
- Turn 2 (1 turn(s) ago): cyber_major_incident, safety_breakthrough
