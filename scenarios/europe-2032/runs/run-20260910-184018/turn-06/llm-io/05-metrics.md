...
- characters 105-13534: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13577-13769: `{{metrics_json}}` from the run's live metric values
- characters 13862-14976: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15019-17171: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17227-17873: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Continuity Switch for Cut-Off Services | 2 |
...
## Metrics
```json
{"ai_capability": 62.5, "openweight_capability": 59.4, "ai_safety": 22.0, "resilience": 30.0, "eu_ai_sovereignty": 21.0, "eu_political_capital": 14.0, "public_sentiment": 24.0}
```
## Narrative
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner campaigned on holding advanced AI as strategic asset"}]}
```
```
