...
- characters 454-1947: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1990-4164: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4714-5156: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7810-9751: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Workplace Augmentation Pact | 9 | small | 7 | 9 | own jurisdiction | public_sentiment up moderately, eu_political_capital up slightly, resilience up slightly via absorption | 2 | running |
...
## New measure
**EU Workplace Augmentation Pact**
It turns the measured white-collar productivity gain into visible public benefit by funding augmentation assistants for SMEs, hospitals and administrations on EU-controlled models with retraining tied to hiring retention.
This turn because employment held while output rose among the least experienced, giving a rare window to lift sentiment and capital before the next capability jump, and it directly serves the two-year commitment to run essential services on AI under EU control.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Workplace Augmentation Pact", "category": 9, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, eu_political_capital up slightly, resilience up slightly via absorption"}, "grounds": "productivity-without-layoffs window this turn"}]}
```

## Priority
EU Workplace Augmentation Pact, because with only M6 still costing capital and sovereignty at 15.0, rebuilding public consent through shared gains outranks further hardening this turn.

## In practice
We run this through DG EMPL and DG CNECT on ESF+ and Digital Europe money: EPSCO agreement for wage-insurance conditionality that firms keeping headcount get subsidised augmentation licences, procured via the Continuity Switch stack on open-weight and allied models already re-platformed, with ENISA certification fast-track from M6 Anomaly Watch black-box tests.

We hold M6 on hospital and grid substitute certification now that chain-of-thought is dead, using activation inspection and incident reporting to restore audit coverage. Finished Shield, Transition Bridge and Continuity Switch work is kept sustained by member states without new EU spend, while we use the augmentation story — firms that cut early hiring again — to unblock gigafactory siting councils with mayors and unions.
...
```
## New measure
**EU Municipal Restoration and Dependency Triage**
It restores ransomware-hit public services from Shield fallbacks and maps the compromised dependency blast radius with enforced patching and isolation, to stop cascades.
This turn because a major automated attack is live while defenders are visibly behind, and with capital at 14.0 nothing outranks keeping power, ports and town halls running; it also defends the two-year commitment to run essential services under EU control when that control is being tested.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Restoration and Dependency Triage", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via detection"}, "grounds": "major automated attack landing this turn"}]}
```

## Priority
EU Municipal Restoration and Dependency Triage, because the incident demands it ahead of the Augmentation Pact and the two-year commitment means nothing if services stop during the attack.

## In practice
We run this through ENISA and DG DIGIT under the Critical Systems Shield legal base, with emergency procurement for clean rebuilds of municipal IT and a CER-Directive isolation order for the compromised dependency, funded by reprogrammed Digital Europe resilience money plus bridge loans already proven in 2029. Interior and Telecoms Council in joint session authorises cross-border CERT mutual aid and 24-hour reporting.

We keep the Workplace Augmentation Pact (M8) running on its continuity-stack procurement but pause new hospital certifications until triage clears, using M6 Anomaly Watch black-box logs to distinguish model-generated exploit tooling from mis-configuration. Mayors get restoration teams not lectures, and contractors on gigafactory sites get security guarantees tied to grid-repair priority to prevent the sit-in and cable-cut dynamic spreading.
