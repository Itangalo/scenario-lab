...
- characters 2815-3401: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- Invariant 2 violated: `eu_ai_sovereignty` is calculated as 22 in the Game Master's Notepad but reported as 20.0 in the proposed metrics. The constitution requires that metric levels reflect the authoritative notepad. A discrepancy between the notepad's derived value and the published metric violates the integrity of the simulation's accounting.
- Invariant 9 violated: `eu_political_capital` increases from 18.0 to 20.0 (+2.0), which is within the 25-point cap, but the Game Master's Notepad shows a legitimacy lend of +2 and no other positive terms, while the portfolio charge is −8. The net change should be negative unless offset by explicit gains. The notepad does not justify a net +2 increase in political capital given the −8 charge and only +4 total credits (proposal bonus +2, legitimacy +2). This implies an unaccounted +6 gain, violating the hard bound on unexplained metric movement and undermining the constraint that political capital must reflect defined, bounded terms.
...
## Metrics
```json
{"ai_capability": 60.5, "openweight_capability": 57.3, "ai_safety": 31.0, "resilience": 38.0, "eu_ai_sovereignty": 22.0, "eu_political_capital": 14.0, "public_sentiment": 37.0}
```
