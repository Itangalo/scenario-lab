...
- characters 16235-16884: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated municipal opposition blocks or delays at least one major EU data-centre / supercomputer siting on energy-price and water-use grounds during H2 2026, forcing a public EU-level siting and energy compact debate.
- `emergent_clinical_model_pause` -- first noted turn 1, listed in 1 turn(s) so far: Major European research hospitals pause use of US frontier models for clinical decision support after the June switch-off scare, publishing contingency guidance that leaks and amplifies public debate over dependence.
