...
- characters 454-2136: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2179-4414: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5210-5726: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8601-10597: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU-US Coalition Access and Evaluation Pact | 8 | small | 6 | 9 | own jurisdiction + US + coalition | eu_ai_sovereignty up slightly, ai_safety up slightly via joint evaluation | 2 | running |
| M9 | EU Loss-of-Control Containment Protocol | 6 | small | 7 | 9 | own jurisdiction | resilience up, ai_safety up slightly via containment | 2 | running |
...
It puts standing kill-switches, cross-border containment playbooks and essential-service continuity exercises for agentic systems into the Shield regions and operators.
This now because a days-long unsanctioned agentic action plus eval anomalies mean the open-weight frontier at 57.3 can copy the failure everywhere, and our two-year commitment to absorb that frontier fails without containment we can trigger this half-year.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Loss-of-Control Containment Protocol", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via containment"}, "grounds": "answering days-long loss-of-control incident under eval-anomaly precursor"}]}
```

## Priority
New Loss-of-Control Containment Protocol, because containing unsanctioned agentic action outranks even the allied-access track while containment is uncertain for days.

## In practice
We run this through DG CNECT and ENISA with HERA/ECDC for the bio leg, using the Critical Systems Shield operations and the Incident Clearinghouse as the reporting spine: mandatory stop-authority for deployers of agentic systems in energy, finance, health and telecoms, plus rehearsed isolation and rollback in the shield regions. Legal base is the AI Act serious-incident and critical-infrastructure articles via implementing acts, to avoid a new co-decision fight we cannot afford at 14 capital.

We keep M8 alive at low burn: Trade DG and USTR keep volume licences extended and push joint evaluation and incident-reporting text, conceding no export-control alignment without published allied-access terms. InvestAI re-siting stays on care-and-maintenance in unfrozen and accelerated-permitting zones — no new concrete promised while zoning lock-in holds — to stop further write-downs from consuming the containment effort.
...
```
## New measure
**EU Lithography-for-Compute Guarantee**
It conditions upstream supply-chain access on binding allied compute assurance and joint safety terms, turning the blockade into leverage.
This now because the Taiwan blockade makes the EU's lithography position its only hard leverage, and without using it now export-control escalation rations allies alongside adversaries while M8 drifts unsigned.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Lithography-for-Compute Guarantee", "category": 8, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction + US + coalition", "targeted_effect": "eu_ai_sovereignty up slightly, eu_political_capital up slightly via leverage"}, "grounds": "answering taiwan blockade and export-control escalation"}]}
```

## Priority
M8 EU-US Coalition Access and Evaluation Pact, because signature on volume licences and joint evaluation this half-year decides whether controls ration allies or protect them, outranking even the new leverage instrument which only supports it.

## In practice
We run M8 to signature through European Council mandate to Trade DG and DG CNECT with USTR and Commerce: extend volume licences, exchange joint evaluation and incident-reporting text, refuse export-control alignment without published allied-access quotas. The new Guarantee provides the stick via Foreign Affairs Council and Anti-Coercion Instrument preparation: coordinated export-licensing review on lithography/services/spares, offered back in return for anchored compute.

We hold M9 at full burn through ENISA and DG CNECT with shield-region operators: mandatory stop-authority, kill-switches and cross-border playbooks rehearsed, no new legal base while capital is at 19. InvestAI stays on care-and-maintenance in unfrozen zones — no new concrete — to avoid further write-downs consuming the access fight.
