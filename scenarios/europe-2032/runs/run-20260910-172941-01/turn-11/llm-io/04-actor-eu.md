...
- characters 456-2299: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2343-4837: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5229-5691: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9187-11607: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): A Europe inside the great-power safety floor and hardened to live with open frontier AI
...
|---|---|---|---|---|---|---|---|---|---|
| M12 | EU Trusted Care and Public Service AI Scale-up | 9 | small | 10 | 12 | own jurisdiction | public_sentiment up substantially, resilience up slightly via care continuity, eu_political_capital up slightly | 2 | running |
...
| M4 | EU Incident Reporting and Redress Clearinghouse | 2 | small | 2 | 4 | own jurisdiction | resilience up slightly, ai_safety up slightly, public_sentiment up via redress | 2 | finished |
| M5 | EU Bio-Uplift Assurance and Screening Upgrade | 5 | small | 3 | 5 | own jurisdiction | ai_safety up, resilience up slightly via bio-detection | 2 | finished |
| M6 | EU-US-Allied Compute Assurance and Siting Compact | 8 | small | 4 | 6 | own jurisdiction + US + coalition | eu_ai_sovereignty up slightly, eu_political_capital up slightly via unblock | 2 | finished |
| M7 | EU Bio Detection and Care Continuity Surge | 6 | small | 5 | 7 | own jurisdiction | resilience up, ai_safety up slightly via bio-response | 2 | finished |
| M8 | EU-US Coalition Access and Evaluation Pact | 8 | small | 6 | 9 | own jurisdiction + US + coalition | eu_ai_sovereignty up slightly, ai_safety up slightly via joint evaluation | 2 | finished |
| M9 | EU Loss-of-Control Containment Protocol | 6 | small | 7 | 9 | own jurisdiction | resilience up, ai_safety up slightly via containment | 2 | finished |
| M10 | EU Lithography-for-Compute Guarantee | 8 | small | 8 | 10 | own jurisdiction + US + coalition | eu_ai_sovereignty up slightly, eu_political_capital up slightly via leverage | 2 | finished |
| M11 | EU Accession to the US-China Safety Floor | 8 | small | 9 | 11 | own jurisdiction + US + China + coalition | ai_safety up, eu_political_capital up slightly via inclusion | 2 | finished |
...
## New measure
**EU Trusted Care and Public Service AI Scale-up**
It puts the medical breakthrough and proven public-sector wins onto EU-run, certified infrastructure with fast EU-wide authorization and reimbursement, so benefit is visibly European.
This now because sentiment is at 27 and capital at 22 while capability runs on private servers and Beijing squeezes supply — only attributed, trustworthy deployment rebuilds tolerance to hold lithography leverage and finish accession without losing the public.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trusted Care and Public Service AI Scale-up", "category": 9, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up substantially, resilience up slightly via care continuity, eu_political_capital up slightly"}, "grounds": "converting medical breakthrough and adoption success into European-attributed trust"}]}
```

## Priority
M11 EU Accession to the US-China Safety Floor, because loss-of-control with alien multi-agent cooperation and Chinese gallium retaliation make exclusion from verification and quotas existential and this is the final turn before it finishes.

## In practice
We drive M11 to close via EEAS and the Trade Council: transpose declared-inventory and incident-notification terms into an AI Act implementing act, table shield-region loss-of-control telemetry from the last days-long containment gap and ENISA-certified monitoring as our contribution, and hold the Hague case-by-case lithography line only against published US compute quotas to unlock observer-to-party text despite gallium pressure.

We launch the Scale-up through DG SANTE, DG EMPL and the EHDS Board with EU4Health and ESF+ top-up: emergency CE fast-track for the tailored therapies on EU-sovereign inference, procurement preference for models running in the finished Gigafactory/shield capacity, and rollout of the waiting-list and decision-time use-cases that already worked to hospitals and municipalities hit by fraud, with income-support and retraining desks attached so displacement does not read as abandonment. The M9 containment protocol is re-activated for the unsanctioned agentic incident — grid/health/telecom stop-authority drills, cross-border mutual aid, and mandatory reporting into the M4 clearinghouse — without opening a second instrument this turn.
...
```
## New measure
**EU Swarm-Defence Patching Rollout**
It puts the new automated patching and swarm-behaviour detection directly into hospitals, grids, municipalities and mid-industry via ENISA and the Critical Systems Shield operations.
This now because the breakthrough shifts defence back for the first time in years while resilience sits at 41 and open weights at 61.5, and only rapid deployment turns a lab win into fewer cascades and restored trust.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Swarm-Defence Patching Rollout", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, ai_safety up slightly via reduced cyber harm"}, "grounds": "exploiting defensive breakthrough before offence adapts"}]}
```

## Priority
M12 EU Trusted Care and Public Service AI Scale-up, because with capital at 19 and sentiment at 25 only visibly European cures and working public services rebuild permission to hold any other line.

## In practice
We drive M12 through DG SANTE and the EHDS Board: emergency CE fast-track tied to EU-sovereign inference from the finished Gigafactory capacity, EU4Health reimbursement conditional on certified EU-run hosting, and mayor-led rollout in fraud-hit and short-time-work towns with retraining desks attached so clinics claim credit, not borrowed luck.

We seed the new Rollout via ENISA and the M3 Shield teams without opening a second track: pre-procure certified patching and swarm-detection toolkits, mandate deployment in health, energy and municipal IT under NIS2 implementing acts, and feed hits into the M4 clearinghouse and M9 containment playbooks. On accession, we transpose the observer-to-party telemetry obligations already tabled and hold the Hague servicing line only for published quotas, letting the finished M11 text carry while we spend scarce capital at home.
