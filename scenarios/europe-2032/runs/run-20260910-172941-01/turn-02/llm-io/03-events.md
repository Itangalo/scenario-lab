...
- characters 91-284: `{{metrics_json}}` from the run's live metric values
- characters 377-1614: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1657-4036: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4092-5273: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5336-27671: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28026-28150: `{{event_history}}` from the run's own event record
...
- Turn 1 (1 turn(s) ago): cyber_test_shot, loss_of_control_incident, cyber_defence_breakthrough, openweight_frontier_release
