...
- characters 454-2281: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2324-4641: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5468-5913: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8060-10037: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_siting_backlash_freeze (emergent event):** Municipal resistance to salvaged hyperscale sites escalates into coordinated siting moratoriums and court injunctions over power and water, freezing one InvestAI site for the turn.
...
| M5 | EU Bio-Uplift Assurance and Screening Upgrade | 5 | small | 3 | 5 | own jurisdiction | ai_safety up, resilience up slightly via bio-detection | 2 | running |
...
## New measure
**EU Bio-Uplift Assurance and Screening Upgrade**
It puts the new interpretability/control result into the AI Office evaluation stack and funds hardened nucleic-acid synthesis screening and hospital wastewater bio-detection pilots.
This now because the genome-model bio-uplift finding opens a four-turn bio window while the safety breakthrough gives us something deployable to certify, and export-control tightening makes EU-owned assurance the only leverage we control.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Uplift Assurance and Screening Upgrade", "category": 5, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up, resilience up slightly via bio-detection"}, "grounds": "answering bio-uplift precursor with deployable safety gain"}]}
```

## Priority
EU Bio-Uplift Assurance and Screening Upgrade, because an irretrievable open model plus a credible non-expert bioweapon-assist signal outranks even gigafactory salvage for what must move first.

## In practice
We task the AI Office with AI Act Art.51/55 evaluation powers plus HERA/ECDC for the bio leg, adopting the new interpretability checks as pre-deployment certification for high-compute bio models used in EU procurement and InvestAI sites, and tying Digital Europe/Horizon money to synthesis-provider screening upgrades. Labs wanting EU compute or public contracts adopt the assurance property; synthesis firms get co-funding conditional on verified screening.

In parallel we keep M1/M2 alive on cheap salvage — InvestAI closes distressed cancelled hyperscale sites and grid reservations without reopening state-aid fights — and we drive M4 Clearinghouse to adoption as implementing act, using the 72-hour reports and redress desk to capture the public-sector adoption wins visibly and rebuild trust while the Shield's finished segmentation and swarm-detectors are exercised this autumn.
...
```
## New measure
**EU-US-Allied Compute Assurance and Siting Compact**
It negotiates quota relief and end-use fast-lanes for EU labs/hospitals while offering standardized EU power-water-community benefit terms to unfreeze the moratorium-hit InvestAI site.
This now because Washington rationing plus municipal injunctions jointly stall the sovereignty bet, and a joint supply-plus-siting deal is the only move that unblocks both without opening a second compute programme.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU-US-Allied Compute Assurance and Siting Compact", "category": 8, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction + US + coalition", "targeted_effect": "eu_ai_sovereignty up slightly, eu_political_capital up slightly via unblock"}, "grounds": "answering supply rationing and siting freeze together"}]}
```

## Priority
M1 InvestAI Gigafactories, because salvaging distressed sites at discount is the core of our two-year commitment to run essential AI ourselves and the frozen site must move first.

## In practice
We keep InvestAI execution in DG CNECT and EIB, closing the unfrozen distressed sites and grid reservations while the frozen site goes into mediated procedure: Competitiveness Council mandate for a template community-benefit and water-efficiency package, with co-funding conditional on lifting moratorium, adjudicated with the host region and courts. We link this to the Tech sovereignty package permitting zones so the template becomes the EU standard.

In parallel we task Trade and the AI Office to seek a volume-licence side-letter in the EU-US Trade and Technology channel for verified EU labs and hospitals, using our synthesis-screening and evaluation uptake as assurance, and we drive the Bio-Uplift pilots to refit milestones. The Clearinghouse reporting desk is pushed to municipalities to make the office-productivity gains and waiting-list wins visible, containing backlash while Shield exercises continue.
