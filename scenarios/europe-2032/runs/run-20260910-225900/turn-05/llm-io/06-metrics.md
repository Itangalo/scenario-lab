...
- characters 13501-14651: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14695-16771: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16827-17690: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_energy_siting_backlash` -- first noted turn 5, listed in 1 turn(s) so far: Coordinated protests and local permit challenges stall grid connections for data-centre and fallback inference sites in two member states after energy-price rises are blamed on AI load.
...
## Metrics
```json
{"ai_capability": 61.0, "openweight_capability": 57.0, "ai_safety": 24.0, "resilience": 41.0, "eu_ai_sovereignty": 12.0, "eu_political_capital": 6.0, "public_sentiment": 16.0}
```
