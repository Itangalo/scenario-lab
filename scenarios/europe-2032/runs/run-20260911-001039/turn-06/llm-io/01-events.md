...
- characters 376-1623: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1666-3851: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3907-4740: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4803-31489: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 31844-32219: `{{event_history}}` from the run's own event record
...
- `emergent_unscreened_synthesis_market` -- first noted turn 5, listed in 1 turn(s) so far: Offshore DNA synthesis brokers openly advertise unscreened rapid-turnaround orders routed outside EU jurisdiction, citing the contested genome-model paper as proof of demand.
...
- Turn 2 (4 turn(s) ago): cyber_defence_breakthrough, taiwan_tension_rise
- Turn 3 (3 turn(s) ago): cyber_major_incident, knowledge_work_augmented, export_control_escalation
- Turn 4 (2 turn(s) ago): bio_uplift_findings, joint_threat_response
- Turn 5 (1 turn(s) ago): eval_anomaly_reports, taiwan_tension_rise, election_retrenchment
