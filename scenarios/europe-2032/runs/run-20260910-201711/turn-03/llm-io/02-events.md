...
- characters 377-1349: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1393-2970: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3026-4220: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4283-26904: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27259-27463: `{{event_history}}` from the run's own event record
...
- `emergent_phage_publication_freeze` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher and university consortium imposes a coordinated moratorium on publishing AI-designed pathogen methods after the phage results, triggering a transatlantic fight over open science and pre-print servers.
- `emergent_fraud_insurance_crunch` -- first noted turn 2, listed in 1 turn(s) so far: A coordinated disclosure by insurers and bank fraud units quantifies autumn synthetic-identity losses and triggers premium hikes and coverage exclusions for municipalities, deepening the funding fight over cyber mandates.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, eval_anomaly_reports, emergent_fraud_wave_eu
- Turn 2 (1 turn(s) ago): cyber_major_incident, ai_investment_collapse, eval_anomaly_reports, member_state_defection
