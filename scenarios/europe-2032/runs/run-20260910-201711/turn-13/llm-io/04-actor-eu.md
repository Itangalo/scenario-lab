...
- characters 454-1526: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1570-3775: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5088-5548: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9431-11425: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Rebuild local consent to keep essential services running and EU AI capacity buildable
...
|---|---|---|---|---|---|---|---|---|---|
| M14 | EU Anomaly Triage and Essential-Systems Hold | 5 | small | 12 | 14 | own jurisdiction | ai_safety up small, resilience stabilise, public_sentiment stabilise | 2 | running |
...
| M3 | EU Critical Infrastructure Cyber Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up substantially, ai_safety spillover via incident reporting | 3 | finished |
| M4 | EU Compute Continuity Bridge | 4 | small | 2 | 5 | own jurisdiction | eu_ai_sovereignty stabilise/prevent fall, eu_political_capital small cost | 2 | finished |
| M5 | EU Accession to Joint Cyber Command Telemetry Pact | 8 | small | 3 | 5 | coalition | resilience up, ai_safety spillover via shared attribution | 2 | finished |
...
| M9 | EU Lithography Leverage and Emergency Compute Allocation | 8 | small | 7 | 8 | coalition | eu_ai_sovereignty up via reciprocal access, eu_political_capital stabilise | 2 | finished |
| M10 | EU Local Trust and Redress Compact | 9 | small | 8 | 9 | own jurisdiction | public_sentiment up, eu_political_capital stabilise, resilience up small via siting unfreeze | 2 | finished |
| M11 | EU Displaced Worker Income and Retraining Guarantee | 7 | small | 9 | 10 | own jurisdiction | public_sentiment up, eu_political_capital stabilise, resilience up small via social absorption | 2 | finished |
| M12 | EU Sovereign Continuity Reserve for Essential Services | 6 | small | 10 | 11 | own jurisdiction | resilience up, eu_ai_sovereignty stabilise, eu_political_capital stabilise via visible continuity | 2 | finished |
| M13 | EU Hospital-First Power and Wronged-Citizen Redress Pact | 9 | small | 11 | 12 | own jurisdiction | public_sentiment up, eu_political_capital stabilise, resilience stabilise via siting unfreeze | 2 | finished |
...
## New measure
**EU Anomaly Triage and Essential-Systems Hold**
Stands up an EU-level independent triage cell for the leaked eval anomaly with authority to pause EU essential-service rollouts of the implicated capability class pending verification.
Why this and why now: with safety at 6, open weights at 62 and a frontier jump already breaking hospital workflows, an unexplained eval behaviour plus tightening US export rationing means the Union cannot keep deploying blind; a visible hold-and-check restores minimal trust while buying leverage on supply.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Anomaly Triage and Essential-Systems Hold", "category": 5, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up small, resilience stabilise, public_sentiment stabilise"}, "grounds": "answering eval anomaly and export rationing with safety collapse"}]}
```

## Priority
EU Anomaly Triage and Essential-Systems Hold, because without a credible independent check on the jump system no Hospital-First guarantee or redress payment restores consent to keep services running.

## In practice
We act through the AI Office with JRC and ENISA seconded evaluators, plus Ljubljana-vetted compute from the continuity reserve: a 60-day triage cell gets incident logs, hospital deployment telemetry and vendor model access under the AI Act serious-incident and high-risk audit bases, publishing what it can verify and what it cannot.

We pair it with a narrow hold in Health and Energy Council conclusions: no new rollout of multi-day agentic clinical/logistics workflows in essential services until triage reports, while existing degraded-stable fallbacks stay funded from reallocated cohesion. The large member state's separate US line is invited to submit to the same check rather than confronted, and councils get the published load-shedding and redress timetable as the price for case-by-case permits.
...
```
## New measure
**EU Middle-Power Reciprocal Access Compact**
Secures emergency compute, model access and evaluation sharing with middle-power supply-chain holders to bypass case-by-case US rationing for hospitals and energy.
Why this and why now: with US licences throttling essential services, Taiwan risk rising, and vendors stonewalling the triage cell, only joint withholding power with other middle powers restores supply and gives the triage check teeth; it directly serves rebuilding local consent by keeping wards running.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Middle-Power Reciprocal Access Compact", "category": 8, "size": "small", "finish_turn": 15, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up small, eu_political_capital up, resilience stabilise"}, "grounds": "using middle-power window to secure hospital compute access"}]}
```

## Priority
M14 EU Anomaly Triage and Essential-Systems Hold, because without publishing what can and cannot be verified the new coalition bargain has no legitimacy with mayors and nurses — not the new compact itself, which needs quiet negotiation to land.

## In practice
We keep the 60-day triage cell running through its interim note: AI Office lead, JRC and ENISA evaluators publishing a verifiable / not-verifiable ledger, with the narrow freeze on new agentic clinical rollouts held and existing fallbacks funded. Redacted logs are named as non-compliance under serious-incident powers, and the separate supply line is offered reciprocal middle-power compute in exchange for submitting to the same check.

In parallel we mandate the Commission and External Action Service to align export-licence and joint procurement terms in the middle-power framework — pooling lithography maintenance, evaluation capacity and volume bargaining for hospital/energy inference. Council formation is Competitiveness and Foreign Affairs jointly, funded by reallocated InvestAI continuity funds, offering permit timetables to councils and continuity to ministries to hold cohesion while capital is at 14.
