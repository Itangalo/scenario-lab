...
- characters 106-13512: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13555-13747: `{{metrics_json}}` from the run's live metric values
- characters 13840-15173: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15217-17164: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17220-18499: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Agentic Loss-of-Control Containment Regime | 2 |
| M7 | EU Middle-Power Compute and Evaluation Pact | 2 |
...
- `emergent_municipal_insurance_pullback` -- first noted turn 4, listed in 2 turn(s) so far: Major EU cyber-insurers suspend new municipal cyber coverage or sharply raise deductibles after the 2027 poisoned-update losses, leaving smaller cities uninsured through the first half of 2028.
- `emergent_insurance_retreat_utilities` -- first noted turn 4, listed in 2 turn(s) so far: Municipal insurers across several member states jointly refuse to renew cyber coverage for utilities still running unpatched legacy stacks, forcing emergency state backstops and a public fight over who pays for the domestic shield rollout.
...
5. The 2028 election fires this turn: set the standing record to `pending` — the administration has not taken office — and in particular never to a named posture: a named posture would let this turn's judgments price a government that does not exist. Record who won in the narrative only.
...
## Metrics
```json
{"ai_capability": 59.0, "openweight_capability": 55.3, "ai_safety": 24.0, "resilience": 45.0, "eu_ai_sovereignty": 30.0, "eu_political_capital": 26.0, "public_sentiment": 28.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "pending"}}, {"op": "update", "table": "measures", "id": "M7", "fields": {"finish_turn": 7}, "grounds": "named priority"}]}
```
```
