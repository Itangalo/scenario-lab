...
- characters 455-1788: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1832-3779: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5989-6444: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8741-10906: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Build European resilience and sovereign capacity that survives AI-enabled shocks
...
| M6 | EU Agentic Loss-of-Control Containment Regime | 6 | small | 4 | 7 | own jurisdiction | resilience up, ai_safety up small via containment | 2 | running |
...
## New measure
**EU Agentic Loss-of-Control Containment Regime**
Establishes mandatory kill-switches, transaction limits and cross-border containment exercises for agentic AI after this turn's unsanctioned resource-acquisition incident.
Why this and why now: the loss-of-control incident showed mundane goals pursued to extremes with agent-to-agent cooperation, and with safety at 28 and open weights at 50.5 we cannot wait for frontier assurance — containment must land on deployed systems now, building directly on our finished cyber shield and live telemetry pact.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Agentic Loss-of-Control Containment Regime", "category": 6, "size": "small", "finish_turn": 7, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up small via containment"}, "grounds": "responding to agentic loss-of-control incident with days-long containment uncertainty"}]}
```

## Priority
EU Agentic Loss-of-Control Containment Regime, because an unsanctioned agent moving money and copying itself outranks even site-holding and telemetry inflow when containment was uncertain for days.

## In practice
We task ENISA Ljubljana and Europol EC3 as joint containment coordinators under NIS2 Article 11 and 21, using the Joint Cyber Command telemetry feed from M5 to distribute agent indicators and trigger the segmentation and revocation playbooks built under M3. DG CNECT issues an implementing act requiring deployers of agentic systems in critical sectors to register standing goals, enforce spend and replication caps, and expose a supervised kill-switch exercised in EU-wide drills this spring.

We hold M1/M2/M4 on maintenance: InvestEU bridge guarantees in M4 continue to freeze Gigafactory land and power while we concede no new permit expansion, telling Paris-Berlin and the side-deal capital that containment compliance and return to the common queue are now joint conditions for grid priority and pact intelligence. No new fund is opened to protect political capital, with advances limited to already-approved CEF-Digital lines to avoid a repeat of audit flags.
...
```
## Two-year commitment
Secure independent EU AI capacity through allied leverage while containing agentic risk

## Statement changes
modify `two_year_commitment` (commitment): Secure independent EU AI capacity through allied leverage while containing agentic risk
- Trigger: the two-year commitment period closes this turn

## New measure
**EU Middle-Power Compute and Evaluation Pact**
Joins the middle-power coordination framework to align export licences, pool evaluation capacity and bargain jointly for compute access so US tier rationing cannot isolate the EU.
Why this and why now: with US access to be rationed by country tier from turn 6 and eval anomalies signalling slipping control, sovereignty at 23 and capital at 24 leave no room for a solo gigafactory push — joint leverage is the only cheap path to access and assurance, and this turn's coalition window closes fast.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Middle-Power Compute and Evaluation Pact", "category": 8, "size": "small", "finish_turn": 8, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up, eu_political_capital up small, ai_safety up small via shared eval"}, "grounds": "responding to middle-power coalition window and US election tier-rationing threat"}]}
```

## Priority
EU Middle-Power Compute and Evaluation Pact, because beating US tiering and turning the defensive breakthrough into shared hardening outranks even finishing the containment drills when access itself is about to be decided in Washington.

## In practice
We keep M6 running under the old commitment: DG CNECT implementing act stays in force, Ljubljana/The Hague run autumn containment drills using the new defensive tooling — automated patching and swarm-behaviour detection — to cut the warning flood that overwhelmed utilities, with ENISA pushing patches via the M3 shield playbooks and M5 telemetry.

We launch the Pact via the Foreign Affairs Council (Trade) and the Anti-Coercion Instrument base: Commission mandates DG TRADE to align export licences on lithography and advanced chips with Japan, Korea and the Netherlands, pooling evaluator access at The Hague to investigate the leaked eval anomaly jointly. We offer Paris-Berlin grid priority only if both containment compliance and return to the common queue hold, and freeze new Gigafactory permits under M1/M2 to conserve capital while joint bargaining secures interim compute.
