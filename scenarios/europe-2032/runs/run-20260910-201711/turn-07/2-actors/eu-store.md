# Store: The European Union (turn 7)

## measures

| id | name | category | size | started_turn | finish_turn | applies_to | targeted_effect | cost_per_turn | status |
|---|---|---|---|---|---|---|---|---|---|
| M1 | InvestAI Gigafactories | 4 | large | 0 | 7 | own jurisdiction | eu_ai_sovereignty up, via €200bn for four to five sites | 3 | finished |
| M2 | Tech sovereignty package | 4 | large | 0 | 6 | own jurisdiction | eu_ai_sovereignty up, via €200bn of private capital for AI data centres by 2036 and accelerated-permitting zones | 3 | finished |
| M3 | EU Critical Infrastructure Cyber Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up substantially, ai_safety spillover via incident reporting | 3 | finished |
| M4 | EU Compute Continuity Bridge | 4 | small | 2 | 5 | own jurisdiction | eu_ai_sovereignty stabilise/prevent fall, eu_political_capital small cost | 2 | finished |
| M5 | EU Accession to Joint Cyber Command Telemetry Pact | 8 | small | 3 | 5 | coalition | resilience up, ai_safety spillover via shared attribution | 2 | finished |
| M6 | EU Agentic Loss-of-Control Containment Regime | 6 | small | 4 | 7 | own jurisdiction | resilience up, ai_safety up small via containment | 2 | finished |
| M7 | EU Middle-Power Compute and Evaluation Pact | 8 | small | 5 | 7 | coalition | eu_ai_sovereignty up, eu_political_capital up small, ai_safety up small via shared eval | 2 | finished |
| M8 | EU Essential Services Frontier Fallback | 6 | small | 6 | 7 | own jurisdiction | resilience up, eu_political_capital stabilise via visible continuity | 2 | finished |
| M9 | EU Lithography Leverage and Emergency Compute Allocation | 8 | small | 7 | 9 | coalition | eu_ai_sovereignty up via reciprocal access, eu_political_capital stabilise | 2 | running |

## standing

| id | posture |
|---|---|
| S1 | CONSOLIDATION |

## Changes this turn

- **applied** `M9` — {"op": "add", "table": "measures", "fields": {"name": "EU Lithography Leverage and Emergency Compute Allocation", "category": 8, "size": "small", "finish_turn": 9, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up via reciprocal access, eu_political_capital stabilise"}, "grounds": "responding to Taiwan blockade and frontier denial hitting essential services"}
  - Grounds: responding to Taiwan blockade and frontier denial hitting essential services
