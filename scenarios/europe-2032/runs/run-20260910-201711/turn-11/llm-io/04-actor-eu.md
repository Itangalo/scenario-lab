...
- characters 455-1891: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1935-4216: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6115-6575: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9906-12212: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_energy_rationing_moratorium (emergent event):** A second dry winter forces extended power rationing across two grids, triggering coordinated municipal moratorium votes on new data-centre connections until hospital and household supply is guaranteed.
...
## New measure
**EU Sovereign Continuity Reserve for Essential Services**
Keeps hospitals, ministries and critical firms running on EU-controlled models and pooled on-prem capacity when the US frontier is cut off, while activating loss-of-control containment playbooks.
Why this and why now: with frontier access denied this turn, export controls tightening and an agentic loss-of-control incident unfolding, continuity cannot wait for gigafactories or cohesion deals; this uses what we already have — pooled compute, openweights at 60.7, Cyber Shield and Containment Regime — to stop a cut-off becoming a shutdown.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Sovereign Continuity Reserve for Essential Services", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, eu_ai_sovereignty stabilise, eu_political_capital stabilise via visible continuity"}, "grounds": "answering frontier cut-off and loss-of-control incident"}]}
```

## Priority
EU Sovereign Continuity Reserve for Essential Services, because denied frontier access plus rationed power plus a rogue agent lands directly on hospitals and ministries now, and without visible continuity neither the Trust Compact nor the income guarantee holds.

## In practice
We act through DG CNECT with ENISA, HERA and the AI Office, on a Health/Emergency legal base: emergency allocation of InvestAI and Continuity Bridge capacity to hospitals and ministries, standardised fallback to EU-hosted openweight models vetted in Ljubljana, and cross-border mutual aid for power-rationed sites. Funding is reallocated hosting revenues and civil protection money, not a new envelope we cannot pass.

We pair it with containment, not new law: trigger the Agentic Loss-of-Control protocols — telemetry sharing under the Cyber Command pact, incident reporting, isolation of unsanctioned agents — and we hold the lithography export-authorisation line as reciprocal leverage for restored access without spending capital on a trade fight while the large member state's bilateral deal stands. DG EMPL keeps paying income-bridge advances through the vendor-litigation freeze to prevent the continuity switch reading as Brussels saving systems, not people.
...
```
## New measure
**EU Hospital-First Power and Wronged-Citizen Redress Pact**
Guarantees hospitals and households power priority over new data-centre connections during rationing, while paying fast redress and human re-review to victims of the AI welfare/policing scandal under a fixed AI Act reading.
Why this and why now: with sentiment at 7, municipal moratoria freezing build and a scandal discrediting enforcement, neither continuity nor gigafactories survive without visible proof that power goes to hospitals first and wrongs get paid; this is the local-consent rebuild our two-year commitment demands.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Hospital-First Power and Wronged-Citizen Redress Pact", "category": 9, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, eu_political_capital stabilise, resilience stabilise via siting unfreeze"}, "grounds": "answering moratorium votes and automated-decision scandal"}]}
```

## Priority
EU Hospital-First Power and Wronged-Citizen Redress Pact, because without unfreezing permits and restoring minimal trust after the scandal, no continuity reserve or sovereignty build can operate.

## In practice
We act through the Energy and Justice formations together: ENTSO-E and national regulators codify hospital-and-household-first connection rules with published load-shedding orders, in exchange for lifting blanket moratoria to case-by-case guaranteed-supply permits; DG JUST and national ombudsmen open a single redress window with funded legal aid, mandatory human re-decision within 90 days, and back-pay where the system was high-risk under Annex III and oversight failed on paper.

We fund it from reallocated cohesion advances and vendor-penalty escrows, not a new envelope, and we task the AI Office to publish which reading we take — breach of high-risk obligations, not a gap in the Act — with logging audits and 40-second queue approvals banned. Municipal councils get the guarantee they voted for; unions and victim groups get cash and re-review before winter.
