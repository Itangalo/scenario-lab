...
- characters 454-1474: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1517-3737: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4386-4850: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7489-9661: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_deposit_strike (emergent event):** Municipal hospital federations in three member states suspend daily biosample deposits, citing overtime arrears and privacy rulings, breaking the deposit routine Washington was promised.
...
## New measure
**EU Essential Services Fallback and Continuity Reserve**
It puts standby fallback capacity behind hospitals, ports and municipal services — offline procedures, isolated backup compute and pre-positioned recovery teams — so throttling rented models and absorbing the automated attack degrades rather than stops services. Why this and why now: the major cyber incident plus pre-emptive throttling over tiering fears is exactly the essential-services failure our two-year commitment exists to prevent, and containment alone (M7) cannot keep services running without something to fall back to.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Fallback and Continuity Reserve", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially"}, "grounds": "major automated cyber incident plus tiering-driven throttling of essential services"}]}
```

## Priority
EU Essential Services Fallback and Continuity Reserve, because keeping hospitals, ports and municipalities running through attack and cutoff fears outranks even kill-switches this turn — without fallback, both containment and throttling mean degradation.

## In practice
We act under NIS2 Article 11 and the Cyber Solidarity Act: Commission Implementing Act tasking ENISA and EU-CyCLONe to stand up a Continuity Reserve — exercised offline playbooks, air-gapped backups, and a rostered pool of national CSIRT and HERA surge staff funded by reprogrammed Digital Europe and EU4Health emergency lines, no new budget vote. Health and Transport Council formations are the venue; we buy hospital and municipal compliance by funding overtime arrears and exempting fallback drills from agent-cap penalties.

We hold M7 Agent Containment on track — CSIRT enforcement of spend caps and kill-switches continues — and we treat M1 Gigafactory sites as the physical host for isolated fallback compute where possible. Messaging via Commission and ENISA is operational: what was hit, what fallback is live, and that throttling rented models is sanctioned and covered, not punished.
...
```
## New measure
**EU Assured Operations and Bio-Deposit Restoration**
It deploys the new interpretability/control breakthrough onto the rented agent systems running hospitals, ports and municipalities while buying back the suspended biosample deposits with cleared overtime arrears and privacy safeguards. Why this and why now: the breakthrough gives us the first certifiable control that actually lands on deployed systems, and the deposit strike breaks the early-warning the Shield depends on — without both, services stay unreadable and blind.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Assured Operations and Bio-Deposit Restoration", "category": 5, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, resilience up moderately"}, "grounds": "deploy safety breakthrough to essential services and restore suspended deposits"}]}
```

## Priority
EU Assured Operations and Bio-Deposit Restoration, because it is the only live instrument and it directly serves the two-year commitment to keep essential services running under European control — certifiable control plus restored detection.

## In practice
We act by Commission Recommendation with ENISA and HERA as operators: ENISA pushes the breakthrough control build to essential-service operators as a hardened configuration with pre-observation certification, funded from Digital Europe emergency reprogramming, and counts adoption toward Agent Containment compliance to stop the throttling spiral. HERA negotiates return-to-deposit with the three hospital federations in Health Council — partial overtime arrears paid now from EU4Health, remainder scheduled, plus a data-protection derogation clarifying deposit privacy.

We message through municipalities, not Brussels: drills from the Continuity Reserve (M8) now run on assured systems, recovery teams carry the new monitoring, and every restored deposit is published as early-warning regained. No new budget vote, no Gigafactory diversion — this lives on what is already built and finished.
