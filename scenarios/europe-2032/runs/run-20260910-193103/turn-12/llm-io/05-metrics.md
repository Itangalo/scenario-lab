...
- characters 106-13439: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13482-13670: `{{metrics_json}}` from the run's live metric values
- characters 13763-15070: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15113-17908: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17964-18916: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M12", "fields": {"finish_turn": 14}, "grounds": "left unprioritised several consecutive turns"}]}
```
```
