...
- characters 106-13634: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13677-13869: `{{metrics_json}}` from the run's live metric values
- characters 13962-15864: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15908-18492: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18548-19473: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M8 | EU Essential Services Fallback and Continuity Reserve | 2 |
...
- `emergent_deposit_strike` -- first noted turn 6, listed in 2 turn(s) so far: Municipal hospital federations in three member states suspend daily biosample deposits, citing overtime arrears and privacy rulings, breaking the deposit routine Washington was promised.
