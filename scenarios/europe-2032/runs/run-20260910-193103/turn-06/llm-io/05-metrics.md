...
- characters 14052-15810: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15853-17979: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18035-19224: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Transatlantic Continuity and Diversification Pact | 2 |
| M7 | EU Agent Containment and Kill-Switch Protocol | 2 |
...
- `emergent_deposit_strike` -- first noted turn 6, listed in 1 turn(s) so far: Municipal hospital federations in three member states suspend daily biosample deposits, citing overtime arrears and privacy rulings, breaking the deposit routine Washington was promised.
- `emergent_tiering_scare_throttle` -- first noted turn 6, listed in 1 turn(s) so far: US tiering rules leak in draft form and EU hospitals and ports pre-emptively throttle rented model use, triggering a short-lived scramble for fallback capacity.
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 elected tiered-rationing presidency, takes office turn 6"}]}
```
```
