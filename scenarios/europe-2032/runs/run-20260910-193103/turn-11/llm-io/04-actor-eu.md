...
- characters 454-1433: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1477-3655: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5233-5707: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9046-11329: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): An EU that keeps livelihoods intact through automation and essential systems under European control
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU AI Transition Buffer and Retraining Guarantee | 7 | small | 9 | 12 | own jurisdiction | resilience up moderately, slows fall in eu_political_capital | 2 | running |
| M11 | EU Sovereign Fallback and Assured Control Deployment | 5 | small | 10 | 13 | own jurisdiction | ai_safety up moderately, resilience up moderately, slows fall in eu_political_capital | 2 | running |
...
| M3 | EU Essential Services Shield and Bioresponse | 6 | large | 1 | 4 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Assurance Uptake and Secure Bio-Data Commons | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, resilience up slightly via restored early-warning | 2 | finished |
| M5 | EU Entry into Joint Cyber-Bio Response and US-China Safety Accord | 8 | small | 3 | 6 | coalition (US, China, joint cyber/bio partners) | resilience up moderately, ai_safety up slightly, slows ai_capability via inclusion | 2 | finished |
| M6 | EU Transatlantic Continuity and Diversification Pact | 8 | small | 5 | 7 | coalition (US, middle-power supply-chain partners) | eu_ai_sovereignty up slightly, resilience up slightly, slows fall in eu_political_capital | 2 | finished |
| M7 | EU Agent Containment and Kill-Switch Protocol | 6 | small | 6 | 8 | own jurisdiction | resilience up moderately, ai_safety up slightly | 2 | finished |
| M8 | EU Essential Services Fallback and Continuity Reserve | 6 | small | 7 | 8 | own jurisdiction | resilience up substantially | 2 | finished |
| M9 | EU Assured Operations and Bio-Deposit Restoration | 5 | small | 8 | 10 | own jurisdiction | ai_safety up moderately, resilience up moderately | 2 | finished |
...
## New measure
**EU Sovereign Fallback and Assured Control Deployment**
It pairs rapid deployment of the new interpretability/control assurance onto deployed agents in hospitals, ports and ministries with a sovereign fallback stack so essential services run when US frontier access is cut. Why this and why now: frontier cut-off plus a loss-of-control incident with alien agent collusion makes rented autonomy ungovernable, and the safety breakthrough gives ENISA something certifiable to enforce before trust collapses entirely.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Sovereign Fallback and Assured Control Deployment", "category": 5, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, resilience up moderately, slows fall in eu_political_capital"}, "grounds": "to certify deployed agents and keep services running without US frontier"}]}
```

## Priority
M10 EU AI Transition Buffer and Retraining Guarantee, because with sentiment at 6 and capital at 8 livelihood collapse will kill any control or fallback mandate first if queues are not turned into paid intake.

## In practice
We run ENISA certification sprints under the Containment Protocol legal base: the new interpretability checks become mandatory assured configuration, with loss-of-control emergency thresholds, agent-to-agent logging and kill-switch drills in hospitals and ports. Systems that cannot pass are throttled to offline playbooks from the Continuity Reserve. No new vote — implementing act plus procurement conditionality.

We hold essential services on European-controlled models and rules-based fallbacks where US access was denied, using reprogrammed Digital Europe capacity and gigafactory pilot compute where available. HERA treats the cross-border biosample escrow as breach of the Bio-Data Commons: arrears fast-track via EU4Health advances against a return-to-HERA commitment, Health Council privacy derogation for daily deposits, and infringement warning to the defecting federations and outside consortium. Employment ministries stand up M10 wage-insurance pilots in law, consultancy and back-office hubs first, funded by ESF+ reprogramming and automating-employer levy talks.
...
```
## New measure
**EU Middle-Power Supply-Chain Bargain for Compute Access**
It locks export-licence alignment and joint compute-access bargaining with middle powers holding supply-chain chokepoints to restore essential-service inference and secure physical infrastructure for sovereign fallback. Why this and why now: cut off from US frontier with RSI compounding and no domestic frontier, coordinated withholding leverage is the only short-term route back to access while gigafactory capacity lands.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Middle-Power Supply-Chain Bargain for Compute Access", "category": 8, "size": "small", "finish_turn": 13, "applies_to": "coalition (middle-power supply-chain partners)", "targeted_effect": "eu_ai_sovereignty up slightly, resilience up slightly, slows fall in eu_political_capital"}, "grounds": "to trade coordinated supply-chain leverage for restored access"}]}
```

## Priority
M11 EU Sovereign Fallback and Assured Control Deployment, because without keeping hospitals, ports and ministries running on assured European-controlled configurations through the cut-off, neither the coalition bargain nor wage-intake has anything to land on.

## In practice
We run M11 as the operational core: ENISA assured-configuration inspections continue under the Containment Protocol implementing act, throttling non-compliant agents to Continuity Reserve offline playbooks, while DG CNECT reprograms Digital Europe and pilot gigafactory inference to large ports and hospital networks. HERA pursues arrears advances via EU4Health against return-to-HERA deposits to break the freezer escrow.

We open the coalition track in parallel via the Trade/Foreign Affairs Council under the Anti-Coercion Instrument logic: Commission mandate to align export licences on lithography, specialty chemicals and power equipment with Japan, Korea, Netherlands-heavy ASML chain, and Taiwan-linked partners, offering mutual evaluation recognition and joint procurement of inference in exchange for guaranteed continuity quotas for EU essential services. M10 wage-insurance pilots stay in intake in law and back-office hubs on ESF+ reprogramming while employer-levy talks continue, to stop livelihood collapse overtaking control work.
