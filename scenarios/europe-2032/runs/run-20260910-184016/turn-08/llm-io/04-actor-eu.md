...
- characters 454-2158: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2201-4827: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5668-6169: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8900-11102: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Chokepoint Bargain and Scarcity Pact | 8 | small | 6 | 9 | coalition | eu_ai_sovereignty stabilise, resilience stabilise, eu_political_capital stabilise | 2 | running |
| M9 | EU Cyber Absorb and Patch Surge | 6 | small | 7 | 9 | own jurisdiction | resilience up, ai_safety up slightly, eu_political_capital stabilise | 2 | running |
...
## New measure
**EU Cyber Absorb and Patch Surge**
Deploys the new defensive breakthrough EU-wide to contain the major automated attack — rapid patching, swarm detection in hospitals, water and municipal services, with ENISA-led exercises and mutual aid.
Why this and why now: with capital at 12 and trust fraying, a visible major incident answered by the first real defensive gain in years is the only way to hold essential services and the scarcity pact's legitimacy — this is containment under the two-year commitment made concrete.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cyber Absorb and Patch Surge", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly, eu_political_capital stabilise"}, "grounds": "major automated attack must be contained while defensive breakthrough allows shift"}]}
```

## Priority
EU Cyber Absorb and Patch Surge, because the large automated attack on public services outranks bargaining timelines — without services kept running through it, no chokepoint deal holds.

## In practice
We put ENISA and the Critical Infrastructure Shield teams forward as the visible front: rescEU and Digital Europe funds to push automated patching and swarm-detection into small hospitals, water utilities and municipalities left exposed by the rapid migration to EU-hosted models, with HERA/ECDC reporting lines kept open and mandatory incident reporting enforced via the Health and Telecoms Councils. Cross-border CERT mutual aid is activated for the worst-hit member states, and the Frontier Evaluation Cell triages whether model-generated tooling drove the sweep.

We keep the Chokepoint Bargain running without new spend: Commission holds the single ASML servicing position and Anti-Coercion examination, DG TRADE rations remaining stock and grid to care and telecoms, and outreach to Japan and South Korea trades coordinated lithography terms for maintenance assurance. Unions and youth anger are met with on-site staffing relief tied to the patch surge, not new promises, to stop walkouts merging with protest while containment is proved.
...
```
## New measure
**EU Bio Synthesis Screening Lockdown**
Imposes mandatory screening of DNA synthesis orders and EU-hosted model refusals for pathogen-design assistance, tied to the contested genome-model findings.
Why this and why now: with open weights at near-frontier and a viable human-pathogen design signal now public inside the biosecurity community, the bio gate is open for four turns — a cheap, fast restriction buys containment before diffusion makes it ungovernable, directly serving the two-year commitment to contain bio harm.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio Synthesis Screening Lockdown", "category": 3, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up slightly, resilience stabilise"}, "grounds": "bio uplift precursor opens bio gate for next 4 turns"}]}
```

## Priority
EU Bio Synthesis Screening Lockdown, because the new pathogen-design signal outranks even the ongoing ransomware sweep — cyber damage degrades services, bio uplift enables irreversible harm.

## In practice
We push the lockdown through HERA/ECDC and the Health Council under Article 168 and the Dual-Use Regulation: mandatory customer and sequence screening for all commercial synthesis providers in the Union, emergency update to the Frontier Evaluation Cell's intolerable-risk thresholds to flag genome-model assistance, and procurement conditions on EU-hosted models to block detailed pathogen-design workflows. ENISA's patch surge continues as the visible front in hospitals and water, funded from rescEU/Digital Europe without new spend.

We hold the Chokepoint Bargain as steady-state: DG TRADE keeps the single ASML servicing line and Anti-Coercion examination, rations remaining accelerators to care and telecoms, and banks Tokyo/Seoul maintenance promises while preparing for Taiwan-linked shipping risk. No new industrial promises are made to empty gigafactory halls; ministers are told to show grid readiness, not delivery dates, to stop further capital burn while containment is proved.
