...
- characters 107-12834: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12877-13067: `{{metrics_json}}` from the run's live metric values
- characters 13160-14100: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14144-16202: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16258-16587: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M13 | EU Repair Coalition and Cyber Restoration Pact | 2 |
...
Brussels did not restore alone. Through trade and telecom channels it stood up a joint restoration pool with Japan, Korea and other holders of clean images, spares and response teams, with shared incident reporting to shorten the search for the entry point. In return for no side deals on servicing, capitals got prioritised slots. The common licensing line held, though resentment over who was restored first leaked into the press.

### A distant breakthrough
The half-year's other large technical news belonged to an American lab: a new method that let AI systems design and verify a complex subsystem far faster than before, hailed by specialists as a genuine step. For Europe it changed nothing on the ground. With factory shells empty and no new accelerators, officials recorded it as foreign science with no near-term use.
...
By December the systems were largely clean but trust was thinner. The public saw AI most clearly as the tool that had written the attack, and as the triage agent that rationed their care while it did so.
