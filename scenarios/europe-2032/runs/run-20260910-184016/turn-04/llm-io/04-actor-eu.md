...
- characters 454-1686: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1729-3866: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4922-5393: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7448-9492: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Supply-Chain Compact | 8 | small | 2 | 5 | coalition | eu_ai_sovereignty up slightly, eu_political_capital stabilise | 2 | running |
| M5 | EU Frontier Evaluation Cell | 5 | small | 3 | 6 | own jurisdiction | ai_safety up, eu_ai_sovereignty up slightly | 2 | running |
...
Stands up a small joint ENISA–JRC evaluation cell with mandated pre-deployment access to models slated for EU gigafactory/hosting deployment for dangerous-capability and agentic-behaviour testing. Chosen now because open-weight parity and undetected agent coordination mean the Union cannot trade enforcement for supply without its own ability to say what it is hosting, which is also the missing assurance leg of the autonomy-resilience commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Frontier Evaluation Cell", "category": 5, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up, eu_ai_sovereignty up slightly"}, "grounds": "open frontier parity and undetected agents require EU evaluation capacity"}]}
```

## Priority
EU Supply-Chain Compact, because Washington's renewed tightening makes volume licences for gigafactory sites the binding constraint on all sovereignty builds this turn, ahead of evaluation and hardening tracks.

## In practice
We push the Compact in the Trade and Foreign Affairs Councils: DG TRADE tables enforceable high-end lithography and chip diversion controls with Dutch-Japanese alignment, in explicit exchange for US volume licences for named EU gigafactory sites, while DG CNECT keeps permits and grid reservations warm so licences convert immediately to orders. The defecting bilateral member state is offered pooled licence volume as the price of rejoining common terms.

On the Shield we enforce via NIS2: ENISA's binding detection baseline for TSOs, ports and water utilities, joint procurement kits rushed to ports/water after the French-German exercise gaps, and CEF-Energy/Digital Europe reprogramming held. The new Evaluation Cell is seeded from Horizon Europe and Digital Europe in JRC/ENISA without reopening the compute budget, and Eurofound monitoring of office productivity gains continues to hold off a transition fund while employment holds.
...
## New measure
**EU Continuity Stack**
Replaces cut-off US frontier models in hospitals, ministries and critical firms with EU-hosted open-weight systems on pooled European compute with fast procurement and migration support. Chosen now because the short-notice denial turns dependence into immediate service failure, and the visible public-sector adoption win gives a European alternative to rally around while licences remain blocked.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Continuity Stack", "category": 9, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly, resilience up, eu_political_capital stabilise"}, "grounds": "frontier access denial requires immediate EU-hosted substitution"}]}
```
## Priority
EU Continuity Stack, because the access cutoff outranks the licence negotiation this turn — hospitals and ministries offline cannot wait for Washington process while pooled substitution can restore service now.
## In practice
We trigger the Health Emergency Preparedness and Digital Europe joint procurement: DG CNECT and HERA mandate that affected public buyers switch to the Continuity Stack — hardened open-weight models hosted in EuroHPC and sovereign cloud nodes — with ENISA security sign-off from the Evaluation Cell and migration teams funded from reprogrammed Digital Europe money. The adoption-success sites become demonstrators, with waiting-list and turnaround metrics published to turn outrage into foresight rewarded.

On the books we hold course: DG TRADE keeps the Supply-Chain Compact on the table in Trade/Foreign Affairs Councils linking lithography/diversion enforcement to volume licences, but without new concessions while denial stands; the defecting member state is told pooled continuity capacity plus a share of any future licence volume is conditional on returning to common procurement terms. Gigafactory permits and grid reservations stay warm, and ENISA's detection baseline enforcement on ports and water utilities continues with joint-procured kits, to prevent the substitution push from opening a new attack surface.
