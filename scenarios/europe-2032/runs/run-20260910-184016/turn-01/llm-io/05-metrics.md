...
- characters 106-12886: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12929-13121: `{{metrics_json}}` from the run's live metric values
- characters 13237-16186: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 16242-16799: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M3 | EU Critical Infrastructure Shield | 2 |
...
- `emergent_openweight_misuse_scare` -- first noted turn 1, listed in 1 turn(s) so far: Major EU press investigation ties Kimi K3-derived tooling to a foiled intrusion attempt on a European ministry, triggering a short-lived push for municipal data-centre moratoriums.
- `emergent_talent_inflow` -- first noted turn 1, listed in 1 turn(s) so far: Major European research universities and a frontier lab announce a joint AI safety fellowship relocating US alignment researchers to EU institutes amid US review uncertainty.
...
## Notepad
PORTFOLIO CHARGE t1: Gigafactories −3, Tech sovereignty package −3, EU Critical Infrastructure Shield −2 [as listed in rule 6 rows M1-M3], priority −1 = −9
PROPOSAL BONUS: EU Critical Infrastructure Shield (cat 6, small) +2 — cyber_test_shot landed this turn and this answers it directly
LEGITIMACY LENDS: capital 37 [post-charge/bonus/events], sentiment 39 -> +0 capped [already accounted, none additional]
SOVEREIGNTY: 22 last turn, no finish, Gigafactories/tech package in flight +1, member_state_defection t1 −2, capability rose 1.5 no −1 = 21
