...
- characters 456-1612: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1656-3594: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4684-5160: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8634-10298: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M10 | European Visible Benefit Scale-Up | 9 | small | 10 | 13 | own jurisdiction | public_sentiment up moderately, eu_political_capital up slightly, resilience up slightly via state capacity | 2 | finished |
...
## New measure
**EU Offline Kill-Switch Retrofit**
It funds hardware-level isolation switches and offline rollback images for showcase hospitals and grid operators so shutdowns no longer depend on double-priced foreign cloud, border-limited logs, or untrained staff. This and now because the drills showed kill-switches that fail when needed most, and with capability at 64.5 against safety at 27.5 the next agentic excursion cannot take days to contain.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Offline Kill-Switch Retrofit", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via assured containment"}, "grounds": "to fix cloud-dependent kill-switches exposed by drills"}]}
```

## Priority
EU Offline Kill-Switch Retrofit, because demonstrated offline control outranks further scale-up this turn — without it the 30-site expansion loses licence after the days-long loss-of-control.

## In practice
We run the Retrofit through the AI Office-ENISA cell with DG ENER and HERA: Digital Europe top-up for local disconnect hardware, pre-staged clean images, and cross-border log mirroring to certified deployers, drilled live in the same hospitals and grid nodes that ran the autumn exercises.

Alongside it we hold M10 Scale-Up via DG REFORM to staffed-reviewer kits only, pausing new sites where offline isolation is not yet fitted, and we keep the Trade Council lithography-for-hosting line warm with annex talks while ring-fencing transformer convoys and community-benefit offers to keep grid links moving.
...
```
## New measure
**European Medical Safety Deployment**
It puts the new tailored therapies and the new interpretability control result into the same stamped hospital kits, so medical gains arrive on EU-logged, EU-isolatable infrastructure rather than as dependence on foreign models. This and now because the breakthroughs coincide with the investment collapse — therapies can be Europeanised cheaply while labs want the safety fix, before double-priced cloud and unlogged models capture the benefit.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "European Medical Safety Deployment", "category": 9, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up slightly, ai_safety up slightly via certified deployment"}, "grounds": "to make medical and safety breakthroughs visibly European"}]}
```

## Priority
European Medical Safety Deployment, because with capital at 23 the only way to hold the two-year commitment is to turn the exogenous medical lift into sovereign legitimacy rather than further dependence.

## In practice
We run the Deployment through HERA and the AI Office Assurance Cell with DG SANTE: EMA fast-track conditional on shared logs and isolation-fit, Digital Europe top-up for clean-image mirrors in showcase hospitals, and procurement preference for deployers accepting the new interpretability certification.

Alongside it we push M12 Retrofit to finish — installers into the 30 paused sites, live offline drills in hospitals and grid nodes before winter — and we use the investment collapse to renegotiate hosting: distressed cloud capacity re-anchored to EU jurisdiction under the Supply-Chain Pact terms, with transformer convoys and community-benefit talks ring-fenced to keep grid links moving.
```
