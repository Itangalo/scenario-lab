...
- characters 377-1596: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1640-3354: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3410-4569: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4632-27914: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28269-28406: `{{event_history}}` from the run's own event record
...
- `emergent_kimi_copycat_wave` -- first noted turn 1, listed in 2 turn(s) so far: A coordinated wave of copycat intrusions using fine-tuned Kimi K3 derivatives targets universities and mid-size industrial firms across the EU, producing no strategic damage but forcing a public attribution debate about open-weight liability.
- `emergent_bio_publication_moratorium` -- first noted turn 1, listed in 2 turn(s) so far: A major scientific journal and preprint servers adopt mandatory AI-assisted uplift screening and refuse publication of detailed phage-engineering methods after the bacteriophage results, fragmenting open bio-model research norms.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, loss_of_control_incident, export_control_escalation
- Turn 2 (1 turn(s) ago): adoption_success
