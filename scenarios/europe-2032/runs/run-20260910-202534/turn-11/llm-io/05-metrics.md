...
- characters 107-12922: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12965-13157: `{{metrics_json}}` from the run's live metric values
- characters 13250-14256: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14300-15953: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16009-17045: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M9 | European Supply-Chain Leverage Pact | 2 |
| M10 | European Visible Benefit Scale-Up | 2 |
| M11 | EU Agent Loss-of-Control Response Protocol | 2 |
...
- `emergent_grid_connection_sabotage` -- first noted turn 11, listed in 1 turn(s) so far: Coordinated physical blockade of grid-connection works for EU compute sites spreads beyond the two protest camps, with transformer deliveries sabotaged and construction delayed for months.
- `emergent_care_federation_boycott` -- first noted turn 11, listed in 1 turn(s) so far: Municipal grid operators and hospital IT federations publish a joint refusal to host unlogged frontier medical models, triggering a coordinated procurement boycott that stalls DG REFORM scale-up sites.
