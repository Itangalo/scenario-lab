...
- characters 376-1820: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1863-4119: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4175-5263: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5326-27272: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27627-28070: `{{event_history}}` from the run's own event record
...
- `emergent_crosshost_hash_coalition` -- first noted turn 5, listed in 1 turn(s) so far: Major non-EU cloud and preprint hosts jointly adopt a voluntary hash-sharing feed for the circulating pathogen-methods kit, materially shrinking clear-web availability even as dark-web mirrors persist.
- `emergent_kit_hosting_as_service` -- first noted turn 5, listed in 1 turn(s) so far: Non-EU cloud and mirror hosts openly advertise bulletproof hosting for the stripped pathogen-methods kit, turning fortnightly re-tunes into a paid service with uptime guarantees that hash-blocking cannot chase.
...
- Turn 1 (5 turn(s) ago): cyber_test_shot, loss_of_control_incident, export_control_escalation
- Turn 2 (4 turn(s) ago): adoption_success
- Turn 3 (3 turn(s) ago): loss_of_control_incident, openweight_frontier_release, emergent_kimi_copycat_wave, emergent_bio_publication_moratorium
- Turn 4 (2 turn(s) ago): emergent_screening_bypass_kit, emergent_insurer_open_model_boycott
- Turn 5 (1 turn(s) ago): member_state_defection, election_alliance
