...
- characters 105-13277: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13320-13512: `{{metrics_json}}` from the run's live metric values
- characters 13605-15049: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15092-17348: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17404-18492: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "ALLIANCE"}, "grounds": "turn 5 elected president offering structured allied access; posture takes effect turn 6"}]}
```
```
