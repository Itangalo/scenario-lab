...
- characters 377-1784: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1828-3855: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3911-5052: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5115-29342: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29697-29813: `{{event_history}}` from the run's own event record
...
- `emergent_grid_tooling_copycats` -- first noted turn 2, listed in 1 turn(s) so far: Copycat crews reuse the leaked grid-probing tooling for extortion against European municipal utilities and regional water operators, forcing paid shutdowns and public ransom disputes.
- `emergent_toolkit_teardown_row` -- first noted turn 2, listed in 1 turn(s) so far: Independent university teams publish a reproducible teardown showing how the freely available frontier-class model was adapted into the grid-probing toolkit, triggering repository takedown demands and a cross-border press controversy.
...
- Turn 2 (1 turn(s) ago): bio_uplift_findings, openweight_frontier_release
