...
- characters 454-1291: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1334-3287: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5186-5668: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8412-10223: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_asian_open_stack_adoption (emergent event):** Asian labs release a highly capable open agent stack that EuroHPC-hosted fallbacks rapidly adopt, deepening EU dependence on foreign open weights even as US access stays rationed.
...
|---|---|---|---|---|---|---|---|---|---|
| M7 | EU Critical Services Cyber Shield Surge | 6 | small | 6 | 9 | own jurisdiction | resilience up, incident harm down | 2 | running |
| M8 | EU Lithography and Supply-Chain Leverage Compact | 8 | small | 7 | 9 | own jurisdiction plus coalition partners holding supply-chain chokepoints | eu_ai_sovereignty up, eu_political_capital stabilised | 2 | running |
...
## New measure
**EU Lithography and Supply-Chain Leverage Compact**
Locks EU upstream chokepoints into a coordinated export-licensing and allied compute-access bargain to trade continued supply for chips, models and shipping security during the blockade. Category 8 International coordination and leverage is why now: the quarantine makes the Union's upstream position its only hard leverage, and without joint exercise it will be picked off bilaterally.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Lithography and Supply-Chain Leverage Compact", "category": 8, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction plus coalition partners holding supply-chain chokepoints", "targeted_effect": "eu_ai_sovereignty up, eu_political_capital stabilised"}, "grounds": "answering Taiwan blockade with joint leverage"}]}
```

## Priority
M8 EU Lithography and Supply-Chain Leverage Compact, because the blockade outranks absorption this turn — without secured compute supply nothing else including M6/M7 lands.

## In practice
We run the Compact through the Trade Council and the Anti-Coercion Instrument: DG TRADE with Dutch lithography licensing, coordinated with Japan and the US inward administration for reciprocal compute and chip-paper relief, offering member states shared blockade-cost compensation from InvestEU and EIB to hold unity.

We keep M7 Shield Surge via ENISA/CERT-EU deploying the new defensive breakthrough — automated patching and swarm detection — to hospitals and municipalities hit in February, and M6 Transition Facility via DG EMPL accelerating payouts to keep permit protests from hardening while sentiment sits at 26. We frame fallback continuity and battery pilots as sovereignty-in-use to buy tolerance for siting costs.
...
```
## New measure
**EU Joint Incident and Biosurveillance Accession**
Joins the joint cyber command telemetry pool and the binding biosurveillance pact to get real-time attribution and standing investigation capacity the Union cannot build alone. Category 6 Preparedness and resilience is why now: with RSI compounding, safety at 21 and one attack class already patched, pooling detection is the only fast resilience gain while sovereignty rebuilds.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Joint Incident and Biosurveillance Accession", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction plus joint threat response partners", "targeted_effect": "resilience up, incident harm down"}, "grounds": "accepting joint threat response invitation"}]}
```

## Priority
M7 EU Critical Services Cyber Shield Surge, because holding hospitals and municipalities through RSI-accelerated release cadence outranks even the accession paperwork — without the Shield delivering, pooled telemetry has nothing to plug into.

## In practice
We run M7 through ENISA and CERT-EU as the delivery arm for the accession: connect HERA and ECDC sample-sharing nodes and hospital SOCs to the joint telemetry feed, using the EU4Health and Digital Europe top-up already committed, with Health and Telecoms Council mandates for binding sample-sharing.

We hold M8 Lithography Compact in Trade Council to convert Dutch licensing into reciprocal chips and shipping cover, now widened to the middle-power coalition for export-licence alignment, offering blockade-cost compensation via EIB to keep unity while Asian open-stack dependence grows. DG EMPL sustains the finished M6 Transition Facility payouts for graduate displacement to stop permit protests hardening while sentiment is at 22.
