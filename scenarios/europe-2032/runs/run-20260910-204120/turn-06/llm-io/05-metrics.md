...
- characters 105-13618: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13661-13853: `{{metrics_json}}` from the run's live metric values
- characters 13946-15013: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15056-16981: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17037-18191: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Transition and Absorption Facility | 2 |
| M7 | EU Critical Services Cyber Shield Surge | 2 |
...
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Continuity Stack and Evaluation Authority −2, EU Transition and Absorption Facility −2, priority M5 −1 = −11
PROPOSAL BONUS: EU Transition and Absorption Facility (cat 7, small) +3 — cutoff humiliation plus hiring freezes and hostile permit politics from last two turns and this answers it directly
LEGITIMACY LENDS: capital 23, sentiment 28 -> +2
SOVEREIGNTY: 17 last turn, no capacity measure finishes, Gigafactories in flight +1, export_control_escalation t5 −2, capability rose 3.0 −1 = 15

## Emerging developments (tracked)

- `emergent_asian_open_stack_adoption` -- first noted turn 6, listed in 1 turn(s) so far: Asian labs release a highly capable open agent stack that EuroHPC-hosted fallbacks rapidly adopt, deepening EU dependence on foreign open weights even as US access stays rationed.
- `emergent_asian_access_offer` -- first noted turn 6, listed in 1 turn(s) so far: Asian labs, gaining from US retrenchment, offer the EU cut-price frontier inference tied to data-centre and standards alignment, forcing a public choice between cheap access and sovereignty.
...
## Metrics
```json
{"ai_capability": 68.7, "openweight_capability": 63.6, "ai_safety": 23.0, "resilience": 37.5, "eu_ai_sovereignty": 19.0, "eu_political_capital": 26.0, "public_sentiment": 26.0}
```
...
## Notepad
PORTFOLIO CHARGE t6: M1 InvestAI Gigafactories −3, M6 Transition and Absorption Facility −2, M7 Cyber Shield Surge −2, priority M6 −1 = −8
PROPOSAL BONUS: EU Critical Services Cyber Shield Surge (cat 6, small) +4 — cyber_major_incident landed this turn severe and public and this answers it directly
LEGITIMACY LENDS: capital 24, sentiment 26 -> +2
SOVEREIGNTY: 15 last turn, Tech sovereignty package finishes t6 +4, Gigafactories in flight +1, no capacity event, capability rose 2.2 −1 = 19

## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election winner takes office in turn 6"}]}
```
```
