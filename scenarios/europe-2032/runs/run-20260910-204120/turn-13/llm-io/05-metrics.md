...
- characters 107-13413: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13456-13648: `{{metrics_json}}` from the run's live metric values
- characters 13741-14903: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14947-17152: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17208-18428: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M13 | EU Certified Assurance Rollout for Deployed Systems | 2 |
| M14 | EU Entry-Level Displacement Backstop | 2 |
...
- `emergent_plague_toolchain_attribution_leak` -- first noted turn 12, listed in 2 turn(s) so far: Independent forensic attribution of the engineered plague signature leaks, naming the lab toolchain and AI model family used to design it.
- `emergent_wartime_biocompute_denial` -- first noted turn 12, listed in 2 turn(s) so far: Wartime belligerents impose reciprocal AI-compute blockades on pharmaceutical and pathogen-modeling workloads, freezing cross-border access to sequencing and countermeasure design tools for months.
...
## Metrics
```json
{"ai_capability": 86.5, "openweight_capability": 82.6, "ai_safety": 16.0, "resilience": 58.0, "eu_ai_sovereignty": 14.0, "eu_political_capital": 15.0, "public_sentiment": 7.0}
```
...
## Notepad
PORTFOLIO CHARGE t13: M13 EU Certified Assurance Rollout for Deployed Systems −2, M14 EU Entry-Level Displacement Backstop −2, priority M13 −1 = −5
PROPOSAL BONUS: EU Entry-Level Displacement Backstop (cat 7, small) +2 — labour_displacement landed this turn and this answers it directly
LEGITIMACY LENDS: capital 15, sentiment 7 -> none, capital higher
SOVEREIGNTY: 14 last turn, no capacity event, no category 4 finish or in flight, capability rose 1.2 (below 2 threshold) no −1 = 14
