...
- characters 106-13034: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13077-13269: `{{metrics_json}}` from the run's live metric values
- characters 13385-16334: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 16390-16701: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M3 | EU Critical Systems Hardening and Mutual Aid Pact | 2 |
...
- `emergent_publishing_screen_pact` -- first noted turn 1, listed in 1 turn(s) so far: Major European scientific publishers and preprint servers jointly mandate AI-assisted provenance disclosure and automated screening for dual-use biological designs after the phage results.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 46.0, "ai_safety": 32.0, "resilience": 38.0, "eu_ai_sovereignty": 19.0, "eu_political_capital": 39.0, "public_sentiment": 39.0}
```
...
## Notepad
PORTFOLIO CHARGE t1: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Hardening and Mutual Aid Pact −2, priority M1 −1 = −9
PROPOSAL BONUS: Hardening Pact (cat 6, small) +2 — cyber_test_shot precursor landed this turn and this answers it directly
LEGITIMACY LENDS: none — capital 48, sentiment 42, capital higher
SOVEREIGNTY: 22 last turn, supply_chain_coercion t1 −2, capability rose 3.0 −1 = 19
## Store changes
```json
{"store": []}
```
