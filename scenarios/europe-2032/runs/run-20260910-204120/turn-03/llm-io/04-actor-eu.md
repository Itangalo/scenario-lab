...
- characters 455-1606: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1650-3712: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4341-4830: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6878-9102: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_gulf_compute_bid (emergent event):** A Gulf sovereign fund offers to anchor-finance an EU gigafactory site in exchange for guaranteed capacity share and export rights, forcing a public EU decision on foreign ownership of sovereign compute.
...
| M4 | EU Supply-Chain Solidarity and Anti-Coercion Compact | 8 | small | 2 | 5 | own jurisdiction plus coalition partners holding supply-chain chokepoints | eu_ai_sovereignty up, eu_political_capital stabilised by sharing coercion cost | 2 | running |
...
## New measure
**EU Supply-Chain Solidarity and Anti-Coercion Compact**
Creates a joint EU export-control and anti-coercion front around lithography, servicing and critical inputs so The Hague cannot be picked off bilaterally, pairing a Council mandate for Union-level negotiation with retaliation cover and compensation for ASML. Category 8 International coordination and leverage is why now: Washington is using US-origin technology inside the EU supply chain to coerce a single member state while the grid rehearsal window is open.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Supply-Chain Solidarity and Anti-Coercion Compact", "category": 8, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction plus coalition partners holding supply-chain chokepoints", "targeted_effect": "eu_ai_sovereignty up, eu_political_capital stabilised by sharing coercion cost"}, "grounds": "responding to US coercion through Eindhoven while gate is open"}]}
```

## Priority
M3 EU Critical Systems Hardening and Mutual Aid Pact, because the winter exercise and breaker audits must land before winter while the rehearsal actor is still mapped — without demonstrated hardening the sovereignty lines have no credibility.

## In practice
We are running M3 through ENISA and the NIS2 Cooperation Group on existing CEF-Digital and Civil Protection money: mandatory retrospectives from the two probed transmission operators, staged credential and relay audits spreading to neighbours, and a funded cross-border isolation-and-restore exercise before winter. The publishers' provenance-screening pact we endorse via DG RTD and ECDC linkage for bio-design screening, not as a new measure.

We keep M1 Gigafactories and M2 permitting zones moving through DG CNECT, EuroHPC JU and the Competitiveness Council on grid-connection and state-aid clearance, with EIB de-risking — no new compute line, just execution. On Eindhoven we shift the file to Trade/FAC with DG TRADE leading an Anti-Coercion Instrument examination, offering The Hague Union legal cover and a compensation backstop in exchange for no further bilateral servicing concessions until a common EU-US understanding is reached.
...
```
## New measure
**EU AI Work Transition Facility**
Creates an EU-coordinated wage-insurance, retraining and entry-level re-hiring backstop tied to automating employers, using ESF+ and SURE-type top-ups to stop graduate-market collapse becoming anti-AI politics. Category 7 Labour and social protection is why now: measurable displacement in law, accountancy, software and customer operations is already cutting hiring while sentiment sits at 36 and capital at 32 — without a visible transition offer the gigafactory and anti-coercion lines lose their permission to act.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU AI Work Transition Facility", "category": 7, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up, eu_political_capital stabilised", "targeted_effect_detail": "absorption of displacement shock"}, "grounds": "answering measurable entry-level displacement before it hardens opposition"}]}
```

## Priority
M4 EU Supply-Chain Solidarity and Anti-Coercion Compact, because The Hague refusing a bloc mandate while the Gulf bid forces a sovereignty-ownership decision means the coercion front fails now or never — the labour facility runs in background while trade ministers hold the line.

## In practice
We run M4 through FAC-Trade with DG TRADE's anti-coercion examination: no new bilateral servicing concessions from The Hague, Union legal cover plus a compensation/state-aid backstop for ASML contract loss, and we link the Gulf gigafactory offer to the same doctrine — Gulf capital may take a non-controlling, EU-law-anchored stake with no guaranteed capacity share and no export rights, decided publicly via EuroHPC JU and FDI screening, or no site.

We launch the Transition Facility via DG EMPL and EPSCO on existing ESF+ money: rapid retraining vouchers, short-time and wage-insurance top-ups, and hiring incentives for entry roles in the five named sectors, with ELA tracking non-replacement. We keep M1 gigafactories and M2 permitting zones on DG CNECT execution — grid-connection decisions on the two advancing sites, power/water fix for the stalled one — with no new cash promises this turn, and we bank M3's island-and-restore result into NIS2 winter readiness via ENISA without reopening it.
