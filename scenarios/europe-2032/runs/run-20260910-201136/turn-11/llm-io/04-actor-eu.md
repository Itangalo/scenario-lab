...
- characters 454-1904: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1948-3619: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4519-5236: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8296-10264: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Survive decoupling by keeping essential services running on EU-controlled capacity and hardened resilience
- `sovereign_fallback_now` (position): After cutoff from the leading US model and a major automated attack, assured US access can no longer be assumed and minimum domestic fallback plus resilience becomes the first test of sovereignty.
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Displaced Entry-Work Transition Guarantee | 7 | small | 10 | 12 | own jurisdiction | resilience up slightly via transition capacity, eu_political_capital up slightly if hiring resumes | 2 | running |
...
| M3 | EU Critical Shield and Bio-Response Hardening | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Joint Threat Fusion Membership | 8 | small | 2 | 4 | coalition | resilience up, ai_safety up slightly via shared incident learning | 2 | finished |
| M5 | EU Emergency Cyber Restoration and Mutual Aid Surge | 6 | small | 4 | 5 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
| M6 | EU Assured Access and Fallback Negotiation | 8 | small | 6 | 8 | coalition | eu_ai_sovereignty up slightly, resilience up slightly via supply continuity | 2 | finished |
| M7 | EU Swarm-Defence Rollout | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly via reduced incident load | 2 | finished |
| M8 | EU Middle-Power Supply Chain Pact | 8 | small | 8 | 10 | coalition | eu_ai_sovereignty up slightly, eu_political_capital up slightly via joint leverage | 2 | finished |
| M9 | EU Essential Continuity Switch | 6 | small | 9 | 11 | own jurisdiction | resilience up moderately, eu_ai_sovereignty up slightly via EU-hosted fallback | 2 | finished |
...
## New measure
**EU Displaced Entry-Work Transition Guarantee**
Funds rapid retraining, hiring subsidies and income bridging for entry-level cohorts in law, accountancy, software, customer operations and administration via ESF+ reprogramming and employer co-funding.
Category 7 Labour and social protection is the lever because graduate hiring has stopped while budgets are exhausted and sentiment is hostile, and without a visible adjustment path the continuity fallback loses its political license.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Displaced Entry-Work Transition Guarantee", "category": 7, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up slightly via transition capacity, eu_political_capital up slightly if hiring resumes"}, "grounds": "to answer measurable AI graduate displacement without new borrowing"}]}
```

## Priority
M9 EU Essential Continuity Switch, because hospitals and grids on degraded open-model continuity still outrank a two-turn retraining pipeline — services stopped cannot wait for hiring subsidies.

## In practice
We keep M9 as the operational core: ENISA/EU-CyCLONe triage under NIS2 emergency provisions, joint procurement on EU-hosted open weights at ~63.5 capability and pooled middle-power warm capacity, paid from reprogrammed Digital Europe and Emergency Support lines. No new vote is sought; EPSCO and JHA conclusions extend the requisition of fragmented compute into the common pool.

We stand up the Transition Guarantee through EPSCO with ESF+ reprogramming plus SURE-style bridging, tying automating employers to retraining levies and placement quotas for displaced graduates. With Taiwan exercises raising insurance premia we do not open a new chip or energy build this turn; M1/M2 plots and permitting are held as warm reservations for continuity hosting only, to avoid a second front while capital is at 15.
...
```
## New measure
**EU Bio-Uplift Containment and Certified Control Deployment**
Deploys the new interpretability/control result onto EU-hosted open models and surge-funds bio-synthesis screening plus clinical detection across hospitals and labs.
Category 6 Preparedness and resilience is the lever because a contested but categorical bio-design signal coincides with a usable assurance gain, and with capital at 14 and sentiment at 7 the only affordable move is to harden what we already run rather than build anew.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Uplift Containment and Certified Control Deployment", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately via deployed controls, resilience up slightly via bio detection"}, "grounds": "to answer bio uplift precursor and adopt safety breakthrough on EU fallback"}]}
```

## Priority
M11 EU Bio-Uplift Containment and Certified Control Deployment, because a viable human-infective design signal plus a deployable control result outranks the slower Transition Guarantee when essential services already run on open models at ~64.8 capability.

## In practice
We keep M10 running on ESF+ reprogramming and employer levies through EPSCO, but shift operational focus to ENISA, ECDC and the Health Emergency Preparedness and Response Authority for joint procurement of DNA-synthesis screening, hospital wastewater and clinical sequencing surge, paid from Emergency Support and EU4Health reflows without a new vote.

We mandate via NIS2 and AI Act implementing guidance that all EU-hosted fallback models under M9 adopt the new interpretability/certification checks through the EU AI Office evaluation pool and middle-power pooled evaluators, with weight-security audits on hosting sites. Member states resisting new levies are offered continuity-hosting reservations as compensation, and no new gigafactory build is opened while Taiwan insurance premia remain elevated.
