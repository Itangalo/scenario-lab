...
- characters 1832-3541: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4987-5704: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8828-9962: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_care_protest_blockade (emergent event):** Municipal clinic ransomware failures trigger street protests by health workers and patients' families blaming EU-hosted AI triage, escalating to blockades of a data-centre site.
...
| M10 | EU Displaced Entry-Work Transition Guarantee | 7 | small | 10 | 12 | own jurisdiction | resilience up slightly via transition capacity, eu_political_capital up slightly if hiring resumes | 2 | finished |
| M11 | EU Bio-Uplift Containment and Certified Control Deployment | 6 | small | 11 | 13 | own jurisdiction | ai_safety up moderately via deployed controls, resilience up slightly via bio detection | 2 | finished |
...
## New measure
None this turn, while political capital sits at 13 and public sentiment at 5 and we cannot fund a new build without killing the containment rollout already in flight.

## Store changes
No changes.

## Priority
M11 EU Bio-Uplift Containment and Certified Control Deployment, because with open models at 65.8 capability holding up hospitals and grids and safety at 7.0, certified controls plus bio-detection is the only lever that serves both mandates at once.

## In practice
We keep M11 as the sole operational push: EU AI Office evaluation pool certifying the interpretability control check on all EU-hosted fallback models, ENISA auditing hosting weight-security, and HERA/ECDC joint-procuring DNA-synthesis screening, wastewater monitoring and clinical sequencing from Emergency Support and EU4Health reflows.

We let M10 close into business-as-usual ESF+ retraining without fresh levies, and hold factory plots and supply-pact licences as warm options. No new vote, no new permitting fight, no new promise on hiring — just keeping the degraded fallback exercisable and the bio screen funded until capital rebuilds.
...
```
## New measure
**EU Blockade Continuity and Trusted Care Sustainment**
Keeps hospitals, grids and EU-hosted fallback AI running through the chip blockade while defusing clinic protests with visible adoption wins.
A small preparedness and resilience push (category 6) using joint procurement, ENISA hardening and HERA bio-screening sustained from existing funds, because with capital at 14 and sentiment at 9 we cannot fund a new build but must hold what stuck.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Blockade Continuity and Trusted Care Sustainment", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via sustained controls and bio-detection"}, "grounds": "blockade plus bio-precursor plus clinic blockades require holding fallback exercisable"}]}
```

## Priority
M12 EU Blockade Continuity and Trusted Care Sustainment, because it is the only new spend that serves the two-year commitment to keep essential services running on EU-controlled capacity through decoupling.

## In practice
We run this from the Health and Energy Councils via the AI Office evaluation pool and ENISA, with HERA/ECDC joint procurement for synthesis screening, wastewater and clinical sequencing paid from Emergency Support Instrument and EU4Health reflows — no new vote, no new levy. Upstream lithography leverage is held as bargaining stock in the middle-power pact, not exercised now, to protect licences for spares and legacy-chip substitution.

We answer the data-centre blockade by moving triage audits into clinics: publish the certified control checks, staff municipal clinics with fallback support teams, and point to waiting-list falls where public-sector AI worked. Municipalities get mutual-aid repair crews and spare-parts pooling for legacy systems; hosting sites get weight-security audits and maintenance priority for grid/hospital workloads under the chip quarantine.
