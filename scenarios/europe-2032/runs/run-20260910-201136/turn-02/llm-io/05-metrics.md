...
- characters 105-12945: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12988-13180: `{{metrics_json}}` from the run's live metric values
- characters 13273-14116: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14159-16353: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16409-17533: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Joint Threat Fusion Membership | 2 |
...
- `emergent_compute_buildout_pause` -- first noted turn 1, listed in 2 turn(s) so far: A major non-EU cloud provider announces emergency power-capacity rationing for new AI training workloads after grid-connection delays, freezing several planned EU datacentre expansions for the half-year.
- `emergent_lab_pause_call` -- first noted turn 1, listed in 2 turn(s) so far: A major non-US frontier lab announces a training pause after a containment failure, prompting calls for a coordinated moratorium and briefly tightening talent and compute markets.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 49.7, "ai_safety": 30.0, "resilience": 38.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 29.0, "public_sentiment": 30.0}
```
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Shield and Bio-Response Hardening −3, EU Joint Threat Fusion Membership −2, priority −1 = −12
PROPOSAL BONUS: EU Joint Threat Fusion Membership (cat 8, small) +3 — loss-of-control incident this turn plus autumn staged intrusions/bio event from last turn answered directly via pooled telemetry
LEGITIMACY LENDS: capital 29, sentiment 30 -> +1 but capped, applied
