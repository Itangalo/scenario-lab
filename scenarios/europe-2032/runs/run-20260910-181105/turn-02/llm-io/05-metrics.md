...
- characters 13601-14863: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14906-17126: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17182-18211: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Essential Services Continuity Reserve | 2 |
...
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated protests and local permit blocks delay at least one major EU data-centre / supercomputer expansion for months over power prices and water use.
- `emergent_open_model_hospital_targeting` -- first noted turn 1, listed in 2 turn(s) so far: Major European research hospitals report unauthorised attempts to use Kimi K3-derived cyber tooling to exfiltrate genomic datasets, triggering a cross-border law-enforcement coordination debate.
