...
- characters 107-13291: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13334-13526: `{{metrics_json}}` from the run's live metric values
- characters 13619-14688: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14732-16593: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16649-17507: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M11 | EU Allied Compute Access and Supply Leverage Pact | 2 |
...
- `emergent_strait_medical_supply_crunch` -- first noted turn 10, listed in 2 turn(s) so far: Strait-driven chip-freight and shipping insurance spike triggers spot shortages of imaging consumables and generics in several member states, forcing rationing on the labelled triage path.
