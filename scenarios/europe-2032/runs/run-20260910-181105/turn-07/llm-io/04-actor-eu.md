...
- characters 455-3400: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 3444-5704: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5772-6235: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8673-10937: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M7 | EU Bio-Shield Detection Net | 6 | small | 6 | 9 | own jurisdiction | resilience up, ai_safety spillover via misuse detection | 2 | running |
...
Extends the drilled Continuity Reserve sites with wastewater sequencing, clinical triage flags and emergency stockpile release drills for an AI-assisted bio-design signal, plus locked-down handling rules for the new open-weight frontier models in labs and hospitals. Why this and why now: the contested genome-design finding opens a four-turn bio window just as an open-weight release puts near-frontier capability permanently beyond recall, so with capital at 20 we cannot fund new build and must make detection and response absorb what restriction no longer can.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Shield Detection Net", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety spillover via misuse detection"}, "grounds": "answers bio uplift precursor plus openweight frontier release under low capital"}]}
```

## Priority
M6 EU Loss-of-Control Containment Reserve, because the agent that would not stop plus the fresh open-weight frontier drop means logging, human-release thresholds and cross-border mutual aid must actually land before the bio window widens, outranking even the gigafactory line this semester.

## In practice
We hold M1 InvestAI Gigafactories on warm files only — DG CNECT, DG ENER and EIB keep grid-connection priority and re-tendered co-build terms with EU-jurisdiction anchoring, but we ask Council Competitiveness for no new pledge after the member-state side-deal, using the defection to demand a no-undercutting code rather than cash we will not get.

We run M6 through ENISA and the EU-CERT network on Digital Europe money, pushing mandatory agent-action logging and spending/replication gates to payment, registry and cloud providers, with joint exercises with the three health-ministry kit holders; grid operators keep liability cover without ceding control to get them into drills. We start the Bio-Shield Net via HERA, ECDC and the same five drilled sites: pooled sequencing, hospital triage playbooks, and stockpile-release exercises, framed as health security to survive finance-ministers scrutiny.
...
```
## New measure
**EU Insurable Continuity Standard**
Keeps the five drilled hospital, payment and registry sites running on EU-hosted logged models with human release gates certified to a common insurable standard that municipal insurers can underwrite.
Why this and why now: with capital at 14 and trust collapsing after the side-deal and open-weight release, only a certified continuity label that restores insurance and keeps essential services on EU-controlled fallback serves the rebuild-sovereign-capacity-under-control commitment without asking for money we cannot get.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Insurable Continuity Standard", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up modestly, public_sentiment up via restored trust"}, "grounds": "holds essential-services fallback under control with collapsing capital and trust"}]}
```

## Priority
M7 EU Bio-Shield Detection Net, because wastewater sequencing and triage drills on the five drilled sites must land before the bio window widens, outranking even the new insurability label this semester.

## In practice
We run M7 through HERA, ECDC and ENISA on existing health-security money: pooled sequencing, hospital triage flags, and stockpile-release exercises on the five Continuity Reserve sites, with locked-down handling rules for open-weight models framed as patient safety to survive finance-ministers scrutiny.

We start the Insurable Continuity Standard via DG CNECT, ENISA and EIOPA with the three health-ministry kit holders and payment/registry providers: agent-action logging plus spending/replication human gates audited to a label municipal insurers accept, using liability cover to keep grid control rooms in exercises without ceding control. We hold InvestAI gigafactories to warm files only — permitting zones and grid-connection priority — using the side-deal to press a no-undercutting code in Competitiveness Council rather than a pledge we would lose.
