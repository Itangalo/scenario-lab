...
- characters 16235-16798: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated protests and local permit blocks delay at least one major EU data-centre / supercomputer expansion for months over power prices and water use.
- `emergent_open_model_hospital_targeting` -- first noted turn 1, listed in 1 turn(s) so far: Major European research hospitals report unauthorised attempts to use Kimi K3-derived cyber tooling to exfiltrate genomic datasets, triggering a cross-border law-enforcement coordination debate.
...
## Notepad
PORTFOLIO CHARGE t1: Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Shield −2, priority −1 = −9
PROPOSAL BONUS: EU Critical Systems Shield (cat 6, small) +3 — autumn cross-continent grid intrusion with EU operators affected landed this turn and this answers it directly
LEGITIMACY LENDS: capital 34, sentiment 36 -> +1
SOVEREIGNTY: 22 last turn, no capacity event, no finish, eu_frontier_access_denied t1 −3, capability rose 1.0 no penalty = 19
