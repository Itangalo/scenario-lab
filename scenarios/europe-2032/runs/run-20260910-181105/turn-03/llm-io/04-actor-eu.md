...
- characters 455-1742: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1786-3878: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4245-4718: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6674-8610: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Essential Services Continuity Reserve | 6 | small | 2 | 4 | own jurisdiction | resilience up, eu_ai_sovereignty spillover via EU-hosted fallback | 2 | running |
...
Stands up EU-hosted fallback inference and open-weight stacks for hospitals, grid operators and administrations cut off in November, so the next switch-off degrades rather than stops services.
This is Preparedness and resilience work, not a parallel compute build: it repurposes existing AI Factories capacity and connectivity funds for backup operation while the inherited Gigafactories lines rebuild.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Continuity Reserve", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, eu_ai_sovereignty spillover via EU-hosted fallback"}, "grounds": "November US model switch-off disrupting hospitals and ministries"}]}
```

## Priority
EU Essential Services Continuity Reserve, because the November cut-off proved loss of foreign access is no longer hypothetical and outranks slow gigafactory construction for immediate sovereignty.

## In practice
We direct DG CNECT and ENISA with the Health Emergency Preparedness and Response Authority to contract fallback capacity on existing EuroHPC AI Factories, qualify open-weight models for triage, grid anomaly detection and administrative workflows, and fund member-state migration via reallocated Connecting Europe Facility money. Legal base is CER plus the Health Union regulation; hospital IT vendors resisting re-integration are met with joint procurement.

Alongside it we keep M1 InvestAI Gigafactories and M2 Tech sovereignty package alive without opening a parallel line: accelerated-permitting zones are held, cancelled co-builds are renegotiated toward public-anchored sites, and DG TRADE quietly prepares Anti-Coercion screening of supply-chain leverage. The finished Critical Systems Shield telemetry is fed into the Reserve's backup operations.
...
```
## New measure
**EU Public Benefit AI Deployment Sprint**
Scales the working public-sector AI uses behind this turn's visible wins — shorter waiting lists, faster permits — into a funded EU-wide adoption sprint with European-hosted models.
This is Diffusion, adoption and public trust work, and why now is because a rare attributed European success gives us legitimacy to push adoption while vendor resistance and local protests stall the backup and build lines.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Public Benefit AI Deployment Sprint", "category": 9, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, resilience up modestly via admin capacity"}, "grounds": "visible public-sector adoption success to consolidate"}]}
```

## Priority
EU Essential Services Continuity Reserve, because without a credible fallback the next cutoff still stops hospitals and grids regardless of adoption wins.

## In practice
We keep DG CNECT, ENISA and HERA on the Reserve: forcing joint procurement terms for hospital IT vendors on certification/liability, topping retraining with ESF+ money, and fast-tracking data-handling clearance so the two hospital groups and transmission operator pilot becomes five by December.

Alongside it we launch the Deployment Sprint via the Digital Europe Programme and Technical Support Instrument: member-state health and interior ministries co-fund replication kits using EU-hosted open-weight stacks qualified under the Reserve, with procurement preference for EU-anchored inference. M1 InvestAI Gigafactories and M2 Tech sovereignty package are held on permits and public-anchored renegotiation, no new concrete promised this semester.
