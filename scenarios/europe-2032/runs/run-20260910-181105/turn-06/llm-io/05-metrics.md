...
- characters 105-13315: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13358-13550: `{{metrics_json}}` from the run's live metric values
- characters 13643-15972: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 16015-18173: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18229-18982: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Loss-of-Control Containment Reserve | 2 |
...
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Loss-of-Control Containment Reserve −2, priority −1 = −9
PROPOSAL BONUS: EU Loss-of-Control Containment Reserve (cat 6, small) +3 — loss_of_control_incident fired this turn and this answers it directly
...
- `emergent_insurance_retreat` -- first noted turn 5, listed in 2 turn(s) so far: A major European insurer withdraws cyber cover for municipalities running foreign-hosted AI triage, citing unquantifiable model-dependency risk, forcing a public fight over who insures dependence.
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner campaigned on strategic-asset holding with review and tiered access; posture takes effect turn 6"}]}
```
```
