...
- characters 376-2705: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2748-4906: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4962-5715: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5778-26070: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 26425-26635: `{{event_history}}` from the run's own event record
...
- `emergent_insurance_retreat` -- first noted turn 5, listed in 1 turn(s) so far: A major European insurer withdraws cyber cover for municipalities running foreign-hosted AI triage, citing unquantifiable model-dependency risk, forcing a public fight over who insures dependence.
...
- Turn 1 (5 turn(s) ago): cyber_test_shot, ai_investment_collapse, eu_frontier_access_denied
- Turn 3 (3 turn(s) ago): adoption_success
- Turn 5 (1 turn(s) ago): loss_of_control_incident, election_consolidation
