...
- characters 106-13392: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13435-13627: `{{metrics_json}}` from the run's live metric values
- characters 13720-15253: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15296-17486: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17542-17997: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M10 | EU Entry-Jobs Transition Guarantee | 2 |

     **so 1 measures are in flight and they come to −2 `eu_political_capital` this turn.** That figure is the `cost_per_turn` column of the rows above, added up, and nothing else: the named priority's −1 below is charged on top of it, and is not in it. Your charge line carries one term per measure -- 1 of them before the priority. **A measure is charged in every turn up to but not including its finishing turn. A measure added this turn is charged from this turn -- it appears in the rows above and in your total.**
...
PORTFOLIO CHARGE t9: EU Middle-Power Compute and Evaluation Compact −2, priority −1 = −3
PROPOSAL BONUS: EU Middle-Power Compute and Evaluation Compact (cat 8, small) +3 — answers US rationing and blind oversight from last turns plus secures middle-power window this turn
LEGITIMACY LENDS: capital 22, sentiment 25 -> +2
SOVEREIGNTY: 24 last turn, no capacity event finishes, no cat4 in flight +0, middle_power_coalition t9 +2, capability rose 0.5 −0 = 26
...
## Metrics
```json
{"ai_capability": 62.5, "openweight_capability": 61.3, "ai_safety": 9.0, "resilience": 49.0, "eu_ai_sovereignty": 26.0, "eu_political_capital": 26.0, "public_sentiment": 24.0}
```
...
## Notepad
PORTFOLIO CHARGE t10: EU Entry-Jobs Transition Guarantee −2, priority −1 = −3
PROPOSAL BONUS: EU Entry-Jobs Transition Guarantee (cat 7, small) +2 — answers graduate hiring freeze / labour displacement wave from last two turns directly
LEGITIMACY LENDS: capital 22, sentiment 25 -> +2
SOVEREIGNTY: 26 last turn, no capacity event, no cat4 in flight +0, capability rose 0.5 −0 = 26

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M10", "fields": {"finish_turn": 11}, "grounds": "named priority"}]}
```
```
