...
- characters 105-13588: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13631-13823: `{{metrics_json}}` from the run's live metric values
- characters 13916-15609: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15652-17795: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17851-18939: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Trusted Deployment Compact | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner pledged tiered foreign access; posture takes effect turn 6"}]}
```
```
