...
- characters 105-13069: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13112-13304: `{{metrics_json}}` from the run's live metric values
- characters 13397-14336: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14379-16241: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16297-17650: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Bio-Response and Agent Containment Shield | 3 |

     **so 4 measures are in flight and they come to −12 `eu_political_capital` this turn.** That figure is the `cost_per_turn` column of the rows above, added up, and nothing else: the named priority's −1 below is charged on top of it, and is not in it. Your charge line carries one term per measure -- 4 of them before the priority. **A measure is charged in every turn up to but not including its finishing turn.**
...
PROPOSAL BONUS: EU Critical Systems Shield (cat 6, large) +4 — cyber_test_shot precursor landed this turn and this answers it directly
LEGITIMACY LENDS: capital 39, sentiment 40 -> +1
SOVEREIGNTY: 22 last turn, no finish, Gigafactories/tech package in flight +0, eu_access_secured t1 +2, capability rose 3.0 −1 = 23
Emerging developments (tracked): `emergent_clinical_model_pause` -- faint hospital-level chatter about continuity risk after June scare; `emergent_bioscreening_publishing_pact` -- single publisher-lab disclosure of AI phage details in manuscripts, pact only debated.

## Emerging developments (tracked)

- `emergent_clinical_model_pause` -- first noted turn 1, listed in 2 turn(s) so far: Major European research hospitals pause use of US frontier models for clinical decision support after the June switch-off scare, publicly citing continuity risk and accelerating evaluation of open-weight alternatives.
- `emergent_bioscreening_publishing_pact` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher and a university lab jointly disclose that reviewers caught AI-generated phage design details inserted into submitted manuscripts, triggering a cross-publisher screening pact debate.
...
## Metrics
```json
{"ai_capability": 58.0, "openweight_capability": 50.5, "ai_safety": 30.0, "resilience": 36.0, "eu_ai_sovereignty": 21.0, "eu_political_capital": 33.0, "public_sentiment": 32.0}
```
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Shield −3, EU Bio-Response and Agent Containment Shield −3, priority −1 = −13
PROPOSAL BONUS: EU Bio-Response and Agent Containment Shield (cat 6, large) +6 — bio_incident and loss_of_control_incident both landed this turn and this answers both directly
LEGITIMACY LENDS: capital 31, sentiment 32 -> +2
SOVEREIGNTY: 23 last turn, no capacity build finishes, in flight +0, export_control_escalation t2 −1, capability rose 3.0 −1 = 21

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M4", "fields": {"finish_turn": 4}, "grounds": "named priority"}]}
```
```
