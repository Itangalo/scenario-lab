...
- characters 452-1527: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1570-4781: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7711-8208: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10913-12948: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Clinical Inference Repatriation | 9 | small | 7 | 9 | own jurisdiction | public_sentiment up, resilience up slightly, eu_ai_sovereignty up slightly via anchored inference | 2 | running |
...
## New measure
**EU Clinical Inference Repatriation**
It moves the US tailored-therapy inference and logs onto EU-controlled compute under HERA procurement so remissions no longer depend on revisable Washington hosting terms.
This and now because renting-the-cure anger at sentiment 9 and capital 7 is what is collapsing cohesion, while the adoption-success window and finished shields give us a trusted-delivery story that rebuilds legitimacy without a new Gigafactory build we cannot fund.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Clinical Inference Repatriation", "category": 9, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, resilience up slightly, eu_ai_sovereignty up slightly via anchored inference"}, "grounds": "to answer cure-on-lease dependence while capital is too low for new build"}]}
```

## Priority
EU Clinical Inference Repatriation, because only visible European control of the therapies keeping people alive can lift sentiment from 9 and buy capital for biosecurity and capability gates now open.

## In practice
We use HERA and EU4Health procurement with an Article 122 emergency base in Health Council to require EU-hosted inference and log residency as condition for reimbursement of the US therapies, fronted by EuroHPC capacity and the finished Critical Systems Shield segmentation in hospitals. Pharma pushback on IP is met with liability cover from the finished Deployment Compact pilots, not with new money.

DG EMPL and EPSCO keep the finished Transition Guarantee paying late wage-bridges through PES while we freeze new levies pending court outcomes, and ENISA/HERA hold the 48h playbook and sequencing stockpiles active after the ransomware sweep and contested genome-model uplift, with AI Office logging eval anomalies as black-box uplift cases. No new data-centre construction is ordered; InvestAI sites stay in permit-reservation standby.
...
## New measure
**EU-US State-to-State Continuity Accord**
It negotiates continued clinical and critical-service model access directly with the US government as state-counterparty while shifting qualifying workloads to EU-hosted open-weight fallbacks.
This and now because nationalisation plus access-denial has turned commercial dependence into political cut-off during a major cyber incident and loss-of-control event, and only a state-level accord plus the safety breakthrough gives us leverage to restore access without pretending market instruments still reach Washington.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU-US State-to-State Continuity Accord", "category": 8, "size": "small", "finish_turn": 10, "applies_to": "US", "targeted_effect": "resilience up, eu_ai_sovereignty fall prevented, eu_political_capital stabilised"}, "grounds": "to answer nationalisation and access-denial as state-to-state cut-off"}]}
```

## Priority
EU Clinical Inference Repatriation, because keeping US-designed remissions running on EU-logged hosts is the only visible proof that cut-off does not mean patients die, and without it no accord has domestic standing.

## In practice
We task the HR/VP and DG TRADE with an Anti-Coercion-backed state-to-state track in the Foreign Affairs Council, offering supply-chain cooperation and joint incident reporting plus rapid adoption of the new interpretability control in EU deployments, in exchange for restored humanitarian/critical-service inference and qualified EuroHPC mirroring. HERA and EMA hold reimbursement to EU-logged runs under the Repatriation measure, with EuroHPC allocating emergency inference to hospitals and ministries cut off.

ENISA and HERA run the cyber-major playbook from the finished Critical Systems Shield and the bio/agent containment protocols, with the 48-hour exercise rhythm and sequencing stockpiles re-activated for the loss-of-control containment, while the AI Office logs both incidents as black-box cases without halting care. DG EMPL keeps wage-bridge payments flowing through the finished Transition Guarantee and we freeze new Gigafactory groundbreakings, holding sites in standby to avoid spending capital we do not have.
