...
- characters 455-1362: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1406-3295: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5576-6068: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9097-11037: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Contain open-weight bio and cyber uplift while repairing the AI Act's legitimacy through enforced, trusted deployment
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Embodied Work Transition Shield | 7 | small | 12 | 15 | own jurisdiction | public_sentiment up moderately, resilience up slightly via social absorption | 2 | running |
...
| M3 | EU Critical Infrastructure Cyber Shield | 6 | small | 1 | 4 | own jurisdiction | resilience up moderately, ai_safety spillover via detection | 2 | finished |
| M4 | EU Trusted Sovereign Clinical Deployment | 9 | small | 2 | 5 | own jurisdiction | eu_ai_sovereignty up slightly, public_sentiment up, resilience up slightly via continuity of care | 2 | finished |
| M5 | EU Assurance Uptake Facility | 5 | small | 4 | 6 | own jurisdiction | ai_safety up moderately, resilience up slightly, public_sentiment up slightly | 2 | finished |
| M6 | EU Open-Diffusion Containment Net | 6 | small | 5 | 8 | own jurisdiction | resilience up moderately, ai_safety up slightly via detection | 2 | finished |
| M7 | EU Defensive Breakthrough Deployment | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly via containment | 2 | finished |
| M8 | EU Assured Systems Retrofit | 6 | small | 9 | 11 | own jurisdiction | resilience up moderately, ai_safety up moderately, public_sentiment up slightly | 2 | finished |
| M9 | EU Bio Detection and Act Repair Net | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, ai_safety up slightly, public_sentiment up slightly | 2 | finished |
...
## New measure
**EU Embodied Work Transition Shield**
Pays wage-bridge and rapid retraining for picking, sorting, warehouse and supplier workers displaced by deployed robotics, funded from ESF+ reprioritisation and tied to employers deploying systems. Category 7 Labour and social protection, small, because with capital at 2 and sentiment at 14 we cannot build compute or regulate the frontier — we can only show the Union cushions the manual jobs now falling on the side the robots take.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Embodied Work Transition Shield", "category": 7, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up slightly via social absorption"}, "grounds": "embodied deployment dividing labour market while capital at 2"}]}
```

## Priority
M10 EU Embodied Work Transition Shield, because finishing cyber/bio hardening without answering warehouse and supplier layoffs is why completion reads as maintenance and why the side-deal humiliation sticks.

## In practice
We run this through DG EMPL and national public employment services under ESF+ existing legal base: no new regulation, no procurement fight, automatic income bridge for displaced logistics workers plus paid reskilling vouchers for repair, care-assist and site-supervision roles the robots do not take. Deployment employers co-fund via transition levy offset against training delivered.

We hold the line on what just finished — M8/M9 wrappers, patch feeds and pooled sequencing guidance stay operated by ENISA/ECDC/HERA — and we answer the side-deal and export-control tightening politically: presidency keeps queue-priority offer for re-alignment, and tasks DG TRADE to keep allied volume-licence access without splitting procurement further. No new compute instrument this turn while Gigafactories line is stalled.
...
```
## New measure
None this turn, because with political capital at 2 and M10 already costing 2 we cannot fund a second instrument without collapse, and we are waiting for capital to rebuild enough to enforce Act repair.

## Store changes
No changes.

## Priority
M10 EU Embodied Work Transition Shield, because with sentiment at 10 and the automated-decision scandal making restriction cheap and adoption impossible, holding the only visible cushion for displaced workers outranks starting a new build we cannot pay for.

## In practice
We hold M10 as the sole spend: DG EMPL keeps ESF+ wage-bridges and vouchers flowing through city employment offices while tasking thin rural PES with mobile units, and we tie deploying logistics firms to co-funding to show the Union does not pay alone for Chinese machines running American models.

We work what is already finished for free: we direct ENISA and the hospital wrappers to absorb the cyber-defence breakthrough — automated patching and swarm detection — into the M3/M7/M8 estate without a new programme, and we task the AI Office and EDPS to answer the automated-decision scandal as an enforcement failure, not a new Act: suspend the offending social-insurance/policing scoring use, publish logs and human-oversight timings, and open infringement where Annex III oversight was forty-seconds-a-item. On opaque reasoning we concede CoT monitoring is dead and shift assurance to black-box evals and activation inspection via the M5 facility. No new compute or licensing push while tiered US quotas bind; DG Trade sustains licensed volumes.
