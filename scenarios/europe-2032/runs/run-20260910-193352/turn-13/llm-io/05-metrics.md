...
- characters 107-12926: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12969-13160: `{{metrics_json}}` from the run's live metric values
- characters 13253-14160: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14204-16093: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16149-17205: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M10 | EU Embodied Work Transition Shield | 2 |
...
- `emergent_port_warehouse_blockade` -- first noted turn 13, listed in 1 turn(s) so far: Coordinated wildcat blockades by warehouse and port workers in Rotterdam, Hamburg and Lyon halt automated logistics parks for days, with clashes over Chinese-built humanoids and demands for a deployment moratorium.
- `emergent_logistics_park_blockade` -- first noted turn 13, listed in 1 turn(s) so far: Coordinated blockades of logistics parks at Rotterdam, Hamburg and Lyon by displaced warehouse workers and local communes protesting humanoid deployments disrupt freight handling for days.
...
## Notepad
PORTFOLIO CHARGE t13: M10 EU Embodied Work Transition Shield −2, priority −0 (ineffective, capital below 20) = −2
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 2, sentiment 10 -> +2
SOVEREIGNTY: 17 last turn, no capacity event, no category 4 in flight +0, capability rose 0.7 −0 = 17
## Store changes
```json
{"store": []}
```
