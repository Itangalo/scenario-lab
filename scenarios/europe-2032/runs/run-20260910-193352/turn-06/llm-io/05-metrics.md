...
- characters 105-13545: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13588-13779: `{{metrics_json}}` from the run's live metric values
- characters 13872-15197: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15240-17514: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17570-18055: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Metrics
```json
{"ai_capability": 62.1, "openweight_capability": 56.0, "ai_safety": 31.0, "resilience": 37.0, "eu_ai_sovereignty": 23.0, "eu_political_capital": 11.0, "public_sentiment": 29.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner took office turn 6 - strategic asset / tiered access platform"}]}
```
```
