...
- characters 375-1700: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1743-4017: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4073-5212: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5275-30373: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30728-31144: `{{event_history}}` from the run's own event record
...
- `emergent_shadow_clinical_harm_cluster` -- first noted turn 4, listed in 2 turn(s) so far: Ward-level use of unvetted open-weight models for triage and prescribing causes a documented patient-harm cluster in one member state, triggering a cross-border clinical safety alert and calls for an EU-wide ban on unauthorised clinical inference.
- `emergent_openweight_clinical_harm` -- first noted turn 4, listed in 2 turn(s) so far: A ward-level open-weight clinical error causes serious patient harm in a member state hospital and is publicly attributed to unvetted model advice, triggering malpractice investigations.
...
- Turn 1 (5 turn(s) ago): cyber_test_shot, cyber_major_incident, loss_of_control_incident
- Turn 3 (3 turn(s) ago): openweight_frontier_release, us_china_agreement, automated_decision_scandal, emergent_datacentre_siting_blockade, emergent_clinical_contingency_shift
- Turn 4 (2 turn(s) ago): safety_breakthrough, export_control_escalation
- Turn 5 (1 turn(s) ago): openweight_frontier_release, election_consolidation
