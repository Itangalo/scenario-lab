...
- characters 454-1363: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1406-3241: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4841-5333: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8031-9179: `{{previous_actions}}` from the actor's own response from the previous turn
...
## New measure
None this turn, while we finish the Bio Detection and Act Repair Net and rebuild enough capital to enforce anything at all.

## Store changes
No changes.

## Priority
M9 EU Bio Detection and Act Repair Net, because with capital at 2 and a member-state side-deal undercutting the common line, the only credible hold is to deliver joint audits and pooled sequencing detection visibly rather than open another front we cannot fund.

## In practice
We run M9 through ENISA, ECDC/HERA and the AI Office under existing mandates: extension teams stay commune-by-commune on wrappers and patch feeds, while joint teams publish the first audit read on the social-insurance scandal — breach or gap — and stand up pooled sequencing screening guidance. No new regulation, no new money this turn.

We answer the defection politically, not with a new instrument: Council presidency frames the side-deal as pragmatism that fragments bargaining power, offers the defecting capital queue-priority in the retrofit and detection rollout if it re-aligns procurement, and banks the sovereignty accelerators quietly until M9 finishes and capital recovers.
