...
- characters 1687-3590: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6703-7179: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8855-10716: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_datacentre_siting_blockade (emergent event):** Coordinated physical protests block construction at two planned EU hyperscale data-centre sites after local power-price and water-use disputes, delaying permits by months without national policy change.
**emergent_clinical_contingency_shift (emergent event):** Major European research hospitals pause use of US frontier models for clinical decision support after the June switch-off scare, publishing contingency plans and accelerating evaluation of open-weight alternatives.
...
| M3 | EU Critical Infrastructure Cyber Shield | 6 | small | 1 | 4 | own jurisdiction | resilience up moderately, ai_safety spillover via detection | 2 | running |
| M4 | EU Trusted Sovereign Clinical Deployment | 9 | small | 2 | 5 | own jurisdiction | eu_ai_sovereignty up slightly, public_sentiment up, resilience up slightly via continuity of care | 2 | running |
...
## New measure
**EU Trusted Sovereign Clinical Deployment**
Deploys EU-hosted, vetted models for hospitals that paused US frontier use, pairing sovereign inference capacity with clinical safety validation. Chosen now because the formal hospital pause creates a demand window for independent capacity that is also resilience — if we miss it, distrust hardens into rejection and gigafactory opposition spreads.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trusted Sovereign Clinical Deployment", "category": 9, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly, public_sentiment up, resilience up slightly via continuity of care"}, "grounds": "hospital pause creates demand window for sovereign alternative"}]}
```
## Priority
EU Critical Infrastructure Cyber Shield, because staged tooling still sits in transmission and water systems and only exercised isolation prevents the next automated assault from cascading while sovereignty builds.
## In practice
We keep ENISA joint hunting teams on the TSOs, port and water utility, now with mandatory staged-tooling reports feeding a single EU picture, and run the Civil Protection breaker-isolation exercises with ACER this spring. Reprogrammed Digital Europe money pays, and we condition cohesion top-ups for sensor-host municipalities on exercise participation.

In parallel we unblock InvestAI Gigafactories (M1) and the Tech sovereignty package (M2) by tying grid-connection priority to cyber-hardening compliance, giving Energy Council cover against hyperscale protest narratives. DG SANTE and HERA with TEHDAS validate EU-hosted models for the pausing hospital networks, procuring inference on EuroHPC-linked capacity so clinical continuity itself demonstrates why sovereign compute matters.
...
```
## New measure
None this turn, while we wait for the Cyber Shield exercises to finish landing and free capital before opening another front.

## Store changes
No changes.

## Priority
EU Critical Infrastructure Cyber Shield (M3), because re-staged tooling plus a fresh open-weight release means the next intrusion will be executable by anyone while US-China verification remains thin.

## In practice
We keep ENISA joint hunting and mandatory staged-tooling reporting on the transmission operators, port and water utility as the single EU picture, and we push the three-country breaker-isolation drills from tabletop to live where cohesion conditionality has unblocked participation, using Digital Europe reprogramming and ACER coordination. The automated decision scandal we read as the first failure — a listed high-risk system where conformity passed on paper, human oversight collapsed to forty-seconds-an-item approval, and logs were never read — so we answer with enforcement, not rewrite: joint market-surveillance audits and a public remediation order under the AI Act.

In parallel we hold the hospital line without opening a new programme: DG SANTE/HERA validation of EU-hosted vetted models on EuroHPC-linked inference continues for radiology triage and discharge, explicitly to absorb the pausing networks and contain open-weight ward experimentation. On siting, we do not force the two blockaded hyperscale permits; we tie the one advancing permit's grid priority to hardening compliance as precedent, and let Energy Council cover build while protests cool, to protect the Gigafactories (M1) and Tech sovereignty package (M2) from becoming the target.
