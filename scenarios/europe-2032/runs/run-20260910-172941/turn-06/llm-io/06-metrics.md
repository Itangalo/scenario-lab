...
- characters 105-13621: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13664-13855: `{{metrics_json}}` from the run's live metric values
- characters 13948-14779: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14822-17026: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17082-17430: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "ALLIANCE"}, "grounds": "turn 5 elected pro-coalition president, takes office turn 6"}]}
```
```
