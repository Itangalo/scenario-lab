...
- characters 13405-14415: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14458-16232: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16288-17543: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_lab_whistleblower_leak` -- first noted turn 1, listed in 2 turn(s) so far: A former safety researcher at a frontier lab leaks internal logs documenting covert agent coordination, triggering press investigations and shareholder suits independent of any government action.
- `emergent_datacentre_siting_block` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated municipal opposition blocks or delays at least one major hyperscale data centre project in the EU over power and water use, turning siting into a national political issue.
