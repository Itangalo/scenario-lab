...
- characters 454-2118: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2162-4199: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6614-7075: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10236-11769: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_anti_robot_sabotage (emergent event):** Anti-robot sabotage spreads from protests to coordinated arson of deployed humanoid fleets and gigafactory construction sites in two or more member states, forcing operators to pause rollouts.
...
- `two_year_commitment` (commitment): Keep essential services running and the Union together through cascading AI disruption
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Automation Displacement Bridge | 7 | small | 9 | 12 | own jurisdiction | resilience up slightly via absorption, eu_political_capital up slightly via cohesion, public_sentiment up slightly | 2 | running |
...
| M4 | EU Biological Resilience Shield | 6 | large | 2 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly via misuse learning | 3 | finished |
| M5 | EU Eval Anomaly Response Cell | 5 | small | 3 | 4 | own jurisdiction | ai_safety up slightly, eu_ai_sovereignty up slightly via eval capacity | 2 | finished |
| M6 | EU Cohesion and Non-US Inputs Compact | 8 | small | 4 | 6 | own jurisdiction and middle-power coalition | eu_ai_sovereignty up slightly, eu_political_capital up slightly via cohesion | 2 | finished |
| M7 | EU Bio-Containment and Entitlement Audit Surge | 6 | small | 6 | 8 | own jurisdiction | resilience up, ai_safety up slightly via misuse learning, eu_political_capital down less via visible response | 2 | finished |
| M8 | EU Grid-anchored Sovereign Inference Pool | 4 | small | 7 | 10 | own jurisdiction and middle-power coalition | eu_ai_sovereignty up slightly, resilience up slightly via assured inference | 2 | finished |
| M9 | EU Cyber-Continuity and Patch Surge | 6 | small | 8 | 10 | own jurisdiction | resilience up, ai_safety down less via incident learning | 2 | finished |
...
## New measure
None this turn — waiting for finishing measures to free political capital before starting anything new.

With capital at 2.0 and sentiment at 10.0, adding a second in-flight cost this turn would stall what is already on the books; M8 and M9 reach their finishing turn now and stop costing, which reopens room next turn for a blockade-era sovereignty and resilience build.

## Store changes
No changes.

## Priority
M10 EU Automation Displacement Bridge, because cohesion is the binding constraint while hospitals run on paper, robots arrive foreign-built, and capital cannot fund a second front until the Patch Surge and Inference Pool close out.

## In practice
We hold the line on continuity: ENISA and DG CNECT keep the Cyber-Continuity and Patch Surge drills mandatory in health, municipal and energy operators, using the Taiwan blockade to justify holding EU grid reservations and ASML servicing leverage under the Anti-Coercion Instrument rather than conceding them for short-term chips. DG ENER with TSOs keeps islanding plans exercised.

We make the Bridge visible where it matters: DG EMPL with labour ministries accelerates wage-insurance payouts and transition desks in automotive, logistics and port regions hit by humanoid purchase orders, explicitly tying visible public-sector adoption wins — waiting lists falling, permits in days — to European deployment choices to lift sentiment from 10.0. HERA extends quiet bio-sampling on the genome-model signal without opening a new biodefence build this turn.
...
```
## New measure
None this turn, while capital sits at 4.0 and M10 is still costing until turn 12.

Adding a second in-flight cost now would stall the Displacement Bridge and risk a cohesion failure during a major automated cyber incident plus anti-robot arson; we hold continuity with finished inference, patching and bio-shield capacity and reopen build room when M10 closes next turn.

## Store changes
No changes.

## Priority
M10 EU Automation Displacement Bridge, because keeping payouts and retraining flowing is the only lever that holds the Union together through paper-triage hospitals, foreign robots, and coordinated sabotage while capital cannot fund a second front.

## In practice
We run the commitment on installed capacity: ENISA and DG CNECT keep the finished Cyber-Continuity Patch Surge playbooks mandatory in health, municipal and energy operators to degrade the automated ransomware/dependency sweep to manual rather than stop, with CERT-EU triage and islanding under the grid reservations held through the Taiwan freeze. HERA keeps the Biological Resilience Shield sampling extended on the genome-model signal without a new build.

DG EMPL with labour ministries concentrates the Bridge visibly in auto, port, logistics and now gigafactory-construction regions hit by humanoid purchase orders and arson pauses — faster wage-insurance, transition desks, and negotiated rollout pauses with operators and unions — explicitly tied to European-deployed wins on waiting lists and permits to stop sentiment at 13.0 from tipping into paralysis. DG TRADE holds ASML servicing leverage under the Anti-Coercion Instrument rather than trading it for short-term chips.
