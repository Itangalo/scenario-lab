...
- characters 376-1428: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1471-3547: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3603-4794: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4857-30899: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 31254-31502: `{{event_history}}` from the run's own event record
...
- `emergent_grid_supplychain_disclosure` -- first noted turn 2, listed in 2 turn(s) so far: A major industrial control vendor publishes forensic detail linking the autumn grid intrusions to a compromised software update chain, forcing emergency patching across EU energy and port operators.
- `emergent_siting_backlash` -- first noted turn 2, listed in 2 turn(s) so far: Coordinated local protests and municipal legal challenges stall grid-pledge and permitting-zone sites for AI factories in two candidate regions over energy prices and water use.
...
- Turn 2 (2 turn(s) ago): capability_jump, us_china_agreement, eu_frontier_access_denied, supply_chain_coercion, automated_decision_scandal
- Turn 3 (1 turn(s) ago): bio_uplift_findings, safety_breakthrough
