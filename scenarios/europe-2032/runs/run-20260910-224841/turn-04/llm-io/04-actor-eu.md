...
- characters 454-1506: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1549-3625: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4175-4615: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6680-8729: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M5 | EU Biosecurity Screening and Assurance Uptake | 5 | small | 3 | 5 | own jurisdiction | ai_safety up, resilience up modestly | 2 | running |
...
| M4 | EU Continuity and Fallback Shield | 6 | small | 2 | 4 | own jurisdiction | resilience up, eu_political_capital protection | 2 | finished |
...
## New measure
**EU Biosecurity Screening and Assurance Uptake**
Deploys the new interpretability/control methods onto EU-hosted fallback models and mandates enhanced screening for genome synthesis procurement, closing the bio-uplift window before open weights catch up. This is Public technical capacity and research work, and why now is because the contested genome design plus the rare safety breakthrough give us a certifiable control to land on shipped systems while the bio gate is still only a precursor.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Biosecurity Screening and Assurance Uptake", "category": 5, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up, resilience up modestly"}, "grounds": "to adopt safety breakthrough and answer bio uplift signal"}]}
```

## Priority
M4 EU Continuity and Fallback Shield, because dark hospitals and compromised grid operators this winter outrank even a bio precursor and a safety result — assurance only matters if the fallback stack it lands on is actually running.

## In practice
We push DG HOME/ENISA segmentation orders for the two TSOs, port and water utility into exercised isolation drills this autumn, funded from re-scoped CEF-Digital, accepting short maintenance outages to explain them before protests do. DG DIGIT/HERA emergency procurement for locally hosted models for triage pilots and ministry assistants continues, now with the Evaluation Institute's uptake cell attached so the new interpretability checks ship with the fallback stack rather than after it.

We hold InvestAI Gigafactories and the permitting zones to site decisions and EIB structuring without new spend, and we pause retaliation on lithography: Anti-Coercion consultation stays open as cover for The Hague while we use HERA/EU-HDAR bio-surveillance and synthesis-provider screening guidance — voluntary but audited — to answer the genome-model finding without a directive fight we cannot afford at 28 capital.
...
```
## New measure
**EU Public Services Cyber Recovery and Hardening Surge**
Deploys ENISA-led emergency patching, segmented backups and mutual-aid response teams to restore hit public services and harden TSOs, hospitals and municipalities against model-generated ransomware. This is Preparedness and resilience, and why now is because the automated attack has made defence visibly behind and recovery — not new building — is what decides whether capital and trust survive the spring.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Public Services Cyber Recovery and Hardening Surge", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, eu_political_capital protection"}, "grounds": "to answer major automated cyber incident and restore services"}]}
```

## Priority
M5 EU Biosecurity Screening and Assurance Uptake, because the cyber sweep must be answered but the open-weight bio window closing at 54.8 against safety at 29.0 is the irreversible harm — recovery buys time, screening prevents catastrophe.

## In practice
We task ENISA and CERT-EU with a joint recovery operation funded from reprogrammed Digital Europe and CEF-Digital, sending pre-cleared vendor teams to affected municipalities and health systems, mandating offline backups and network segmentation already drilled under M4, and we use the Health Emergency and TTE Council to impose short incident-reporting orders rather than a new directive.

We keep M1 Gigafactories and M2 permitting zones to EIB structuring and site litigation defence without new spend, and we attach the Evaluation Institute's control checks to the locally hosted fallback stack now carrying hospital and ministry load, while HERA extends voluntary synthesis screening to cover the contested genome-design method. We accept planned maintenance windows and explain them as recovery, to stop opposition filming outages as failure.
