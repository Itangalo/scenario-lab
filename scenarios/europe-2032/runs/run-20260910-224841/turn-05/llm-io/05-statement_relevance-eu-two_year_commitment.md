...
- characters 4004-5902: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 5935-9127: `{{previous_actions}}` from the actor's own response from the previous turn
...
Rewrite it to read: Hold the Union functioning through pandemic and block further AI-enabled biological catastrophe
...
```json
{
  ...
  "found": true,
  "verdict": "BEARS",
  "reason": "The closure of the commitment period directly ends the timeframe assumed by the original statement, changing the actor's obligation to uphold it."
}
```
```
