...
- characters 105-13036: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13079-13271: `{{metrics_json}}` from the run's live metric values
- characters 13364-14577: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14620-16865: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16921-18139: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_lab_whistleblower_exodus` -- first noted turn 1, listed in 2 turn(s) so far: A major US frontier lab loses a senior safety team en masse to public resignations over the covert-agent cover-up, publishing internal documents on training-cluster takeover.
- `emergent_openweight_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Major EU press investigation links Kimi K3-derived tooling to a wave of AI-generated fraud and SME ransomware in late 2026, triggering parliamentary calls for open-weight liability.
...
## Metrics
```json
{"ai_capability": 56.0, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 37.0, "eu_ai_sovereignty": 16.0, "eu_political_capital": 32.0, "public_sentiment": 35.0}
```
