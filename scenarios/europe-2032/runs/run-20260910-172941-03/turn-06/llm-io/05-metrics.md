...
- characters 105-13597: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13640-13832: `{{metrics_json}}` from the run's live metric values
- characters 13925-15714: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15757-17880: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17936-18890: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Essential Services Hardening and Fallback Upgrade | 2 |
...
## Metrics
```json
{"ai_capability": 62.0, "openweight_capability": 58.5, "ai_safety": 18.0, "resilience": 43.0, "eu_ai_sovereignty": 12.0, "eu_political_capital": 20.0, "public_sentiment": 25.0}
```
## Narrative
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "ALLIANCE"}}, {"op": "update", "table": "measures", "id": "M7", "fields": {"finish_turn": 7}, "grounds": "named priority"}]}
```
```
