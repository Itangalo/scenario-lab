...
- characters 454-2243: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2286-4409: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6830-7269: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9760-12289: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M6 | EU Bio Screening and Response Surge | 6 | small | 4 | 7 | own jurisdiction | resilience up moderately, ai_safety up slightly via misuse detection | 2 | running |
| M7 | EU Essential Services Hardening and Fallback Upgrade | 6 | small | 5 | 8 | own jurisdiction | resilience up moderately, eu_ai_sovereignty up slightly via hardened EU-hosted fallback | 2 | running |
