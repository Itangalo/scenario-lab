...
- characters 377-1456: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1500-3492: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3548-4680: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4743-28319: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28674-28767: `{{event_history}}` from the run's own event record
...
- `emergent_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: A coordinated municipal campaign across several shortlisted AI-factory host regions blocks grid-connection permits over power prices and water use, forcing a renegotiation of state-aid pledges.
- `emergent_attribution_diplomatic_row` -- first noted turn 2, listed in 1 turn(s) so far: Competing leaks over grid-intrusion attribution trigger an open diplomatic row between EU capitals and a named foreign power, with expulsions threatened and joint investigation stalled.
