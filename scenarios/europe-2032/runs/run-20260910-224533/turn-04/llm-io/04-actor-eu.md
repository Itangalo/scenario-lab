...
- characters 454-2124: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2167-4435: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5583-6023: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8116-10185: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_siting_backlash (emergent event):** A coordinated municipal campaign across several shortlisted AI-factory host regions blocks grid-connection permits over power prices and water use, forcing a renegotiation of state-aid pledges.
...
| M5 | EU Bio-Detection and Medical Countermeasure Surge | 6 | small | 3 | 5 | own jurisdiction | resilience up, ai_safety up modestly via misuse detection | 2 | running |
...
| M4 | EU Essential Services Continuity Shield | 6 | small | 2 | 3 | own jurisdiction | resilience up, public_sentiment up modestly | 2 | finished |
...
Stands up EU-wide wastewater and clinical sequencing sentinel network plus HERA stockpile and rapid-response protocols for AI-enabled bio designs, run as preparedness. Why this and why now is because the genome-model bio-uplift finding opens a four-turn bio gate where open-weight capability already near 50 makes recipes unrecallable, so detection and response must land before capability does.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Detection and Medical Countermeasure Surge", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up modestly via misuse detection"}, "grounds": "to close bio-detection gap opened by uplift finding while cutoff persists"}]}
```
## Priority
EU Bio-Detection and Medical Countermeasure Surge, because the contested genome-model design is the one window that closes — sovereign compute can slip a semester, a distributed bio capability cannot — and it directly serves resilient capacity under our two-year commitment.
## In practice
We task HERA and ECDC under the Serious Cross-Border Threats Regulation with EU4Health and HERA emergency funds to contract national public-health labs in 12 sentinel states for continuous sequencing, link to ENISA-secured reporting, and pre-position PPE, diagnostics and surge procurement. We sell it to health ministers as money and mandates, not new law, to avoid a Council fight we cannot afford at 28 capital.

The Gigafactories (M1) and Tech sovereignty package (M2) stay in flight but on care-and-maintenance: DG COMP holds state-aid clearances, DG ENER brokers grid-connection queues with TSOs in the two probed states, and we pause new siting fights to stop municipal protests draining capital. The Evaluation Institute (M3) is directed to add a bio-uplift track with vetted researcher access to EU-hosted open models, testing what Europe can run rather than waiting for US frontier access.
...
```
## New measure
**EU Embodied-AI Transition and Shopfloor Trust Pact**
A labour-and-adoption pact for commercially deployed robots: retraining wage insurance tied to automating employers plus EU procurement preference for trustworthy deployments in logistics and care. Why this and why now is because dexterous robots landing on EU factories from Chinese hardware and American models threaten the one industrial base we still lead in while municipal backlash blocks our own AI capacity, so displacement must be cushioned before hostility hardens into physical opposition.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Embodied-AI Transition and Shopfloor Trust Pact", "category": 7, "size": "small", "finish_turn": 7, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, resilience up modestly via labour absorption"}, "grounds": "to absorb embodied-AI shock hitting EU industry while siting fights stall sovereignty builds"}]}
```
## Priority
EU Bio-Detection and Medical Countermeasure Surge, because the unrecallable bio-recipe window still outranks even robots — sequencing and stocks must be live before open-weight uplift closes the chance to detect.
## In practice
We keep M5 as the funded priority: HERA/ECDC contracts go live in 12 sentinels, wastewater sequencing tied to ENISA-secured reporting, diagnostics pre-positioned from emergency health funds. DG SANTE sells it as money not mandates to hold health ministers on board.

We launch the shopfloor pact via DG EMPL and DG GROW under ESF+ and the Single Market programme: works-council agreements in auto and logistics regions, wage-insurance pilots co-funded by deploying firms, and procurement guidance favouring EU-serviced robots. For M1/M2 we do no new build — DG COMP holds aid clearances, DG ENER mediates grid queues with TSOs, and we offer host municipalities revenue-share and power-price caps from existing envelopes to unblock permits without opening a new compute line.
