...
- characters 105-13569: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13612-13804: `{{metrics_json}}` from the run's live metric values
- characters 13897-15567: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15610-17878: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17934-18471: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Embodied-AI Transition and Shopfloor Trust Pact | 2 |
...
## Notepad
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Bio-Detection and Medical Countermeasure Surge −2, EU Embodied-AI Transition and Shopfloor Trust Pact −2, priority −1 = −11
PROPOSAL BONUS: EU Embodied-AI Transition and Shopfloor Trust Pact (cat 7, small) +3 — answers embodied_ai_deployment this turn directly, severe shock to EU industrial base
LEGITIMACY LENDS: capital 17, sentiment 27 -> +2
SOVEREIGNTY: 16 last turn, no capacity finish, Gigafactories in flight +0, embodied_ai_deployment t4 −2, capability rose 3.0 −1 = 13

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 8}, "grounds": "event: coordinated municipal siting backlash blocks grid permits"}]}
```
```
