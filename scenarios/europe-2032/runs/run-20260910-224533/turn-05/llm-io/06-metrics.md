...
- characters 106-13605: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13648-13840: `{{metrics_json}}` from the run's live metric values
- characters 13933-16269: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 16313-18536: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18592-19771: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Essential Services Cyber Recovery and Hardened Fallback | 2 |
...
|---|---|---|---|---|---|---|---|---|---|
| M5 | EU Bio-Detection and Medical Countermeasure Surge | 6 | small | 3 | 5 | own jurisdiction | resilience up, ai_safety up modestly via misuse detection | 2 | finished |
...
- `emergent_open_humanoid_stack` -- first noted turn 5, listed in 1 turn(s) so far: A Chinese lab releases a highly capable open-weight control model for humanoids that runs without US software, rapidly copied in EU logistics pilots and undercutting both US licensing leverage and EU servicing requirements.
- `emergent_robot_shop_floor_strikes` -- first noted turn 5, listed in 1 turn(s) so far: Coordinated wildcat strikes and blockades at logistics and auto plants in Rotterdam, Lyon and Gdansk halt Chinese humanoid pilot lines after a dismissal wave blamed on assembly upgrades.
...
## Notepad
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Embodied-AI Transition and Shopfloor Trust Pact −2, EU Essential Services Cyber Recovery and Hardened Fallback −2, priority −1 = −11
PROPOSAL BONUS: EU Essential Services Cyber Recovery and Hardened Fallback (cat 6, small) +4 — cyber_major_incident landed this turn severe and this answers it directly under rationed access
LEGITIMACY LENDS: capital 10, sentiment 22 -> +1
SOVEREIGNTY: 13 last turn, no capacity finish, Gigafactories in flight +0, member_state_defection t5 −2, capability rose 3.0 −1 = 10

## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "pending"}}, {"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 9}, "grounds": "event: ai_investment_collapse plus permit blocks stall build"}, {"op": "update", "table": "measures", "id": "M2", "fields": {"finish_turn": 9}, "grounds": "event: ai_investment_collapse plus member-state break weakens leverage"}]}
```
```
