...
- characters 453-1647: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1690-3336: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5900-6332: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8239-9247: `{{previous_actions}}` from the actor's own response from the previous turn
...
## Priority
No priority push this turn, because with capital at 2.0 any push would fail and further fragment cohesion — preservation outranks direction.

## In practice
We hold what is on paper and spend no new capital. ENISA and the finished Shield network push the cyber-defence breakthrough into binding CER/NIS2 patching where deadlines still hold — Germany, Poland, parts of France — while elsewhere Shield kits stay in detection/triage mode for the open-weights leak and the loss-of-control incident, with HERA/ECDC on passive bio-detection and no recall pretended.

We absorb the automated-decision scandal in information mode only: Health, Employment and JHA Councils commission a joint audit framing it as enforcement failure under Annex III, not an Act rewrite, to avoid making restriction cheap and adoption impossible for years. No new fund, no enforcement round, no public fight over quotas, Taiwan exposure, or the third-capital outside deal — only quiet derogations-for-return to stop a fourth walk, and permits/grid kept warm.
