...
- characters 105-12970: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13013-13205: `{{metrics_json}}` from the run's live metric values
- characters 13298-15322: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15365-17432: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17488-18612: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 9}, "grounds": "left unprioritised several consecutive turns"}]}
```
```
