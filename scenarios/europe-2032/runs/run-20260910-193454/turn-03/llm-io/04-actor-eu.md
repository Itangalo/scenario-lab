...
- characters 455-2387: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2431-4609: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5339-5788: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7398-9388: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M3 | Grid and Port Cyber Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up, ai_safety up slightly via detection | 3 | running |
| M4 | Trusted Public AI Rollout | 9 | small | 2 | 5 | own jurisdiction | public_sentiment up, eu_political_capital up, resilience up slightly | 2 | running |
...
## New measure
**Trusted Public AI Rollout**
Puts proven public-sector AI assistants into health triage, permit decisions and tutoring across volunteer member states under EU procurement and data-sovereignty terms.
This is Diffusion, adoption and public trust work now because the adoption_success window gives visible benefit to attribute to a European decision, rebuilding capital for the harder resilience and sovereignty build while investment collapse freezes private capacity.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Trusted Public AI Rollout", "category": 9, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, eu_political_capital up, resilience up slightly"}, "grounds": "adoption success creates window for EU-attributed benefit"}]}
```

## Priority
Grid and Port Cyber Shield, because the cyber_defence_breakthrough must be operationalised into OT patching and swarm detection before the quiet intruders return, and it remains the core of our two-year commitment.

## In practice
We push ENISA joint operations into deployment phase: using the Article 122 mandate to require automated patching pilots and swarm-behaviour detection in German, Polish and now French TSOs, co-financed from reprogrammed Digital Europe funds, with liability cover extended to keep energy ministers on board despite derogation requests. Equipment backlog is managed by pooled procurement of hardened relays.

We launch the Rollout via DG DIGIT and DG REFORM with volunteer coalitions — Denmark, Estonia, Spain — using existing EU procurement frameworks favouring EU-hosted models, funded by small-scale repurposing within Digital Europe, explicitly linking waiting-list falls to EU choice. The Gigafactories and tech sovereignty package are held on permitting track without new cash this turn, accepting the investment-collapse pause while we protect sites and grid connections for restart.
...
```
## New measure
**Distributed Capability Containment Net**
Extends the cyber shield into hospitals, municipalities and SMEs with open-weight misuse playbooks, offline backup relays and rapid reporting of AI-enabled intrusions, built to absorb what the unrecallable release puts in every hand.
This is Preparedness and resilience work now because the open-weight frontier jump makes restriction moot and the member-state defection fractures joint leverage, so the only commitment-consistent move is to make disruption degrade rather than cascade while holding the coalition together.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Distributed Capability Containment Net", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly"}, "grounds": "openweight release makes distributed harm unrecallable"}]}
```

## Priority
M3 Grid and Port Cyber Shield, because it remains the operational core of our two-year commitment to withstand disruption, and the quiet autumn intruders plus a frontier-equivalent open model mean patching and swarm detection must finish landing before defection politics slows it.

## In practice
We drive ENISA joint operations to completion in TSOs and ports using Article 122 liability cover and pooled relay procurement, trading limited derogations for binding patching deadlines to keep Germany, Poland and France on board while isolating the defecting member state's bilateral deal as outside EU procurement. The gigafactory and tech-sovereignty lines stay on permitting and site-reservation track with no new cash, preserving restart optionality through the capital strike.

We start the Containment Net via DG CNECT and ENISA with CER Directive implementation funds, pushing behaviour-based detection kits and exercised backup procedures to municipal utilities and mid-size industry in volunteer states, and linking the Trusted Public AI Rollout's local goodwill to siting tolerance for the relays and backup power the shield needs.
