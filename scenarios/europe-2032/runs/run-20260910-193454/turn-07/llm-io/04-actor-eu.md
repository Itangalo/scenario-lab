...
- characters 454-1970: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2014-4303: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5085-5517: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7424-9804: `{{previous_actions}}` from the actor's own response from the previous turn
...
## In practice
We hold what is on paper and spend no new capital. DG ENER and DG COMP keep gigafactory permits and grid reservations warm without cash, and we do not contest the third capital's outside deal publicly, offering only re-entry on derogations-for-participation to stop a fourth walk.

ENISA, HERA and ECDC sustain the finished Shield with existing staff: machine-speed patching where CER/NIS2 deadlines still bind, triage and offline backups elsewhere, and Danish, Estonian and German health pilots kept running as the only visible proof the Union still delivers. Council is Health plus Telecoms in information mode only — no vote, no new fund, no enforcement round we cannot win.
