...
- characters 455-1768: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1812-3949: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6032-6550: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9758-11820: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `act_under_uncertainty` (commitment): We will act to keep systems degradable-not-collapsible and people supported even when frontier models are opaque, open and embodied beyond our control
- `two_year_commitment` (commitment): Keep essential services and livelihoods standing through ungovernable AI, embodied deployment and war
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Opaque-Embodied Shelter Extension | 6 | small | 9 | 12 | own jurisdiction | resilience up to absorb opaque-agent loss-of-control, cyber and embodied disruption | 2 | running |
| M12 | EU Attributed Benefit Retention Drive | 9 | small | 10 | 12 | own jurisdiction | public_sentiment up, resilience up slightly via kept-running services | 2 | running |
...
| M4 | EU Care Continuity Stack | 9 | small | 2 | 4 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up, public_sentiment up | 2 | finished |
| M5 | EU Loss-of-Control Containment Protocol | 6 | small | 3 | 5 | own jurisdiction | resilience up, ai_safety up slightly | 2 | finished |
| M6 | EU Visible Benefit Scale-Up | 9 | small | 4 | 6 | own jurisdiction | public_sentiment up, resilience up slightly | 2 | finished |
| M7 | EU Bio Detection and Response Surge | 6 | small | 5 | 7 | own jurisdiction | resilience up, ai_safety up slightly via detection | 2 | finished |
| M8 | EU Automated Defence Deployment | 6 | small | 6 | 8 | own jurisdiction | resilience up substantially | 2 | finished |
| M9 | EU Wartime Essential Continuity Command | 6 | small | 7 | 10 | own jurisdiction | resilience up substantially to absorb war damage and containment failure | 2 | finished |
| M10 | EU Open Frontier Shelter Hardening | 6 | small | 8 | 11 | own jurisdiction | resilience up to absorb open-weight misuse and eval-anomaly surprise | 2 | finished |
...
## New measure
**EU Attributed Benefit Retention Drive**
Puts the one working public-sector AI success — waiting lists and permit times falling — on EU-branded rails, staffed shifts and backup power so it keeps running through the ransomware sweep and capability jump, and credits it visibly to a European decision. A category 9 Diffusion, adoption and public trust effort, because with sentiment at 10 and capital at 20 nothing else can be funded or enforced unless publics see a benefit that survives.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Attributed Benefit Retention Drive", "category": 9, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, resilience up slightly via kept-running services"}, "grounds": "adoption success attributable to EU amid hostile sentiment and capability jump"}]}
```
## Priority
EU Attributed Benefit Retention Drive, because without converting the single attributed success into sentiment and capital the two shelter measures cannot be staffed or obeyed.
## In practice
We run this through DG REFORM and DG SANTE with the Article 222 continuity command: ring-fence the clinics, municipalities and permit offices where waiting lists actually fell, keep their AI triage and patching agents on protected power and air-gapped backups from M10/M11, and pay overtime and mutual-aid cover from reprogrammed Digital Europe and ESF+ money — no new vote, EPSCO and Health Council conclusions only.

M10 and M11 continue on NIS2 implementing guidance — filters, kill-switches, human-readable logging where agents touch safety-critical controls, manual-override drills — but we pause new obligations on small towns that went dark and lead with kept-open services instead. We tell publics plainly reasoning is uninspectable and weights unrecallable, then show the ward that stayed open and the queue that moved because of an EU-kept system, and hold gigafactories frozen rather than spend the last capital promising sovereignty.
...
```
## New measure
**EU Joint Cyber Shield Accession**
Joins the invited joint cyber command with real-time telemetry sharing and pooled response to absorb the automated sweep while cut off from the leading model, using allied detection we cannot build alone. A category 8 International coordination and leverage effort, because with sovereignty at 12, safety at 0 and access denied, protection must come from a seat at that table rather than a promise of our own frontier.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Joint Cyber Shield Accession", "category": 8, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up to absorb major cyber incident under access denial"}, "grounds": "invited joint response as only viable shield amid cutoff and sweep"}]}
```
## Priority
EU Joint Cyber Shield Accession, because the major incident plus frontier cutoff outranks retention — without shared telemetry the ring-fenced clinics from M12 go dark anyway.
## In practice
We mandate via Foreign Affairs and Telecom Councils under NIS2 Article 11 and the Cyber Solidarity Act: ENISA as single gateway pipes anonymised telemetry from large operators and the M11 kill-switch estate into the joint command, in return for signatures and sinkholing for the tooling adapting faster than our patching agents. No new vote for money — reprogrammed Cyber Emergency Mechanism funds pay seconded analysts.

M11 and M12 continue as the domestic leg: M11 keeps air-gapped records and manual-override drills in care/energy, M12 keeps the branded wards open on protected power as proof the shared intel lands. We tell publics plainly the leading model was cut off without appeal and open weights are now near-frontier and unrecallable, so survival runs through degradable services and allied detection, not through sovereignty we froze rather than lost.
