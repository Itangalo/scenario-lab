...
- characters 105-13585: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13628-13820: `{{metrics_json}}` from the run's live metric values
- characters 13913-15363: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15406-17677: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17733-18872: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Bio Detection and Response Surge | 2 |
| M8 | EU Automated Defence Deployment | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election chose retreat, takes effect turn 6"}]}
```
```
