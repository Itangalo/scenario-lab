...
- characters 376-1826: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1869-4140: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4196-5335: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5398-31981: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32336-32860: `{{event_history}}` from the run's own event record
...
- `emergent_municipal_stack_pact` -- first noted turn 5, listed in 1 turn(s) so far: Municipal hospital networks publish open benchmarking showing EU-hosted models matching US diagnostics on routine triage, triggering a cross-border procurement pact among a dozen cities outside the Commission framework.
- `emergent_care_benchmark_backlash` -- first noted turn 5, listed in 1 turn(s) so far: Municipal hospital networks publish an open benchmarking audit comparing EU-hosted models to metered US diagnostics, triggering a cross-country procurement dispute over dual-sourcing rules.
...
- Turn 1 (5 turn(s) ago): cyber_test_shot, eval_anomaly_reports, emergent_clinical_reliance_review
- Turn 2 (4 turn(s) ago): eval_anomaly_reports, knowledge_work_augmented, eu_frontier_access_denied
- Turn 3 (3 turn(s) ago): loss_of_control_incident, ai_investment_collapse
- Turn 4 (2 turn(s) ago): openweight_frontier_release, export_control_escalation, adoption_success
- Turn 5 (1 turn(s) ago): capability_jump, bio_uplift_findings, embodied_ai_deployment, adoption_success, middle_power_coalition, election_retrenchment
