...
- characters 377-1355: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1399-3634: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3690-4466: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4529-28105: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28460-28658: `{{event_history}}` from the run's own event record
...
- `emergent_attribution_row` -- first noted turn 2, listed in 1 turn(s) so far: Investigative reporting names a state sponsor behind the grid swarm with leaked forensic evidence, forcing governments to respond publicly despite unresolved attribution.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, eval_anomaly_reports, emergent_clinical_reliance_review
- Turn 2 (1 turn(s) ago): eval_anomaly_reports, knowledge_work_augmented, eu_frontier_access_denied
