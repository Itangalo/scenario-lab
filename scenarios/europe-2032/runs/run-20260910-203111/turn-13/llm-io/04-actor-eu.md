...
- characters 455-1546: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1590-3872: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4742-5220: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8256-10258: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential services running on EU-controlled means and cushion AI-driven job loss through exclusion
...
| M3 | Grid Shield Hardening Programme | 6 | small | 1 | 2 | own jurisdiction | resilience up materially, ai_safety flat | 2 | finished |
| M4 | Joint Incident Response and OT Recovery Force | 6 | small | 2 | 4 | own jurisdiction | resilience up materially | 2 | finished |
| M5 | EU Continuity Deployment Stack | 9 | small | 3 | 5 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly, public_sentiment up | 2 | finished |
| M6 | EU Municipal Pooled Cyber Cover Facility | 6 | small | 4 | 5 | own jurisdiction | resilience up materially | 2 | finished |
| M7 | EU Bio Shield Detection Net | 6 | small | 5 | 8 | own jurisdiction | resilience up materially, ai_safety flat | 2 | finished |
| M8 | EU Cutover Continuity Pact | 9 | small | 6 | 7 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly, public_sentiment up slightly | 2 | finished |
| M9 | EU Containment and Trust Recovery Pact | 6 | small | 7 | 9 | own jurisdiction | resilience up materially, public_sentiment up, ai_safety up slightly | 2 | finished |
| M10 | EU Degraded Operations Endurance Fund | 6 | small | 8 | 10 | own jurisdiction | resilience up, public_sentiment up slightly | 2 | finished |
| M11 | EU Entry-Level Transition Guarantee | 7 | small | 9 | 12 | own jurisdiction | public_sentiment up, resilience up slightly | 2 | finished |
| M12 | EU–Middle-Power Supply-Chain Bargain | 8 | small | 12 | 13 | coalition of EU and middle powers holding supply-chain pieces | eu_ai_sovereignty up slightly via secured access, eu_political_capital up | 2 | finished |
...
## New measure
**EU–Middle-Power Supply-Chain Bargain**
Joins EU export-licence alignment, joint compute-access bargaining and shared evaluation capacity with middle powers holding supply-chain chokepoints to secure continued access to capable models on EU terms.
This and now because with sovereignty at 14, capital at 9 and sentiment at zero we cannot build our way out alone, and the coalition window that just opened is the only cheap leverage that restores access without new siting or procurement fights.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU–Middle-Power Supply-Chain Bargain", "category": 8, "size": "small", "finish_turn": 14, "applies_to": "coalition of EU and middle powers holding supply-chain pieces", "targeted_effect": "eu_ai_sovereignty up slightly via secured access, eu_political_capital up"}, "grounds": "middle-power coalition window open while EU access withdrawn"}]}
```

## Priority
M12 EU–Middle-Power Supply-Chain Bargain, because restoring lawful access to frontier capability on joint terms outranks extending the unpaid Transition Guarantee when dependence anger now drives the protests.

## In practice
We run this through the Council trade formation and the Anti-Coercion Instrument toolkit, with DG TRADE and DG CONNECT aligning export-licence positions with coalition partners, pooling evaluation results from the frontier control work, and bargaining compute access jointly rather than member-state by member-state. No new data-centre siting, no new procurement; the offer to partners is coordinated ASML / materials / energy leverage in exchange for guaranteed model access.

We hold the lights-on stack on finished instruments — manual fallback, overtime rosters, ENISA patching and swarm-detection — and keep M11 Transition Guarantee lists open through employment ministries and ESF+ reprogramming, publishing only local audit stability while payments catch up, to avoid staking fresh capital we do not have.
...
Deploys the new interpretability/control result onto EU-used and municipally-run systems via EU evaluators and ENISA patching channels to certify behaviour before harm.
This now because the breakthrough is the first cheap assurance gain in years that labs also want, and with safety at 14 and open weights unrecallable it is the only lever that reduces harm probability without new concrete or access fights, directly serving the keep-services-running commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Assured Deployment Uptake", "category": 5, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up materially, resilience up slightly"}, "grounds": "safety breakthrough adoptable on deployed systems"}]}
```

## Priority
M13 EU Assured Deployment Uptake, because turning the exogenous assurance gain into certified deployed systems outranks any new build when capital is 11 and sentiment is 3.

## In practice
We run this through DG CONNECT and ENISA with the Joint Research Centre evaluators, using the middle-power pooled test results and the frontier-control evaluation access to validate the interpretability/control property on the open-weight and fallback stacks now running hospitals, grids and town halls. Funding is reprogrammed Horizon and Digital Europe evaluation money, no new siting; the offer to member states is free certification and swarm-detection updates in exchange for incident data.

We hold manual fallback, overtime rosters and automated patching as the lights-on base, and keep Transition Guarantee lists open via ESF+ where payments have landed to thin protests, while trade formation holds the middle-power licence alignment so US suppliers do not resume cut-off threats during the uptake.
