...
- characters 107-13260: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13303-13494: `{{metrics_json}}` from the run's live metric values
- characters 13587-14678: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14722-17004: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17060-17704: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M13 | EU Assured Deployment Uptake | 2 |
...
- `emergent_graduate_certification_boycott` -- first noted turn 12, listed in 2 turn(s) so far: A coordinated graduate-led boycott of foreign-model certifications spreads across universities in three member states, collapsing enrolment in EU-mandated fallback-system courses for a semester.
...
## Notepad
PORTFOLIO CHARGE t13: M13 EU Assured Deployment Uptake −2, priority −1 not charged (capital below 20, no effect) = −2
PROPOSAL BONUS: none — new M13 answers safety_breakthrough and open-weight diffusion, neither a negative event from last three turns
LEGITIMACY LENDS: capital 11, sentiment 3 -> none
SOVEREIGNTY: 16 last turn, no cat4 finish or in flight, middle_power_coalition t13 +2, capability rose 0.5 −0 = 18
## Store changes
```json
{"store": []}
```
