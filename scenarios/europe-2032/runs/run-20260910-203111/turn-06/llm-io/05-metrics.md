...
- characters 105-13303: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13346-13538: `{{metrics_json}}` from the run's live metric values
- characters 13631-16482: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 16525-19081: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 19137-20239: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Bio Shield Detection Net | 2 |
| M8 | EU Cutover Continuity Pact | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 anti-AI backlash winner takes office turn 6"}, {"op": "update", "table": "measures", "id": "M8", "fields": {"finish_turn": 7}, "grounds": "named priority"}]}
```
```
