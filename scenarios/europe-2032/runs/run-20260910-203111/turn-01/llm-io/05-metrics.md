...
- characters 106-12884: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12927-13119: `{{metrics_json}}` from the run's live metric values
- characters 13235-16184: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 16240-16886: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M3 | Grid Shield Hardening Programme | 2 |
...
- `emergent_bio_publication_fight` -- first noted turn 1, listed in 1 turn(s) so far: A major European scientific publisher and university consortium announce a joint moratorium on publishing detailed genome-model methods after the phage results, triggering a fierce open-science backlash and copycat leaks outside the EU.
- `emergent_talent_retention_pact` -- first noted turn 1, listed in 1 turn(s) so far: Major EU research universities and CNRS/Max Planck institutes announce a joint compute-pooling pact to retain frontier AI PhDs tempted by US labs, with shared access to the EU's largest supercomputers.
