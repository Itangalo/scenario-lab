...
- characters 453-1515: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1558-3071: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4502-4980: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7792-9071: `{{previous_actions}}` from the actor's own response from the previous turn
...
## New measure
None this turn, while capital sits at 12 and sentiment at zero and the Transition Guarantee still has to turn announced places into paid placements before anything new can be carried.

## Store changes
No changes.

## Priority
M11 EU Entry-Level Transition Guarantee, because paid entry-level hiring on EU-controlled systems is now the only path back to public tolerance for degraded operations.

## In practice
We hold the lights-on stack on finished instruments — Continuity Deployment Stack procedures, municipal pooled cover, Bio Shield sequencing, and regular-budgeted overtime and spares — and fold the ENISA automated patching and swarm-detection into that maintenance without new procurement or new siting fights.

We run M11 as the sole spend through EPSCO, employment ministries and public employment services: ESF+ reprogramming and EIB co-finance for wage-insurance top-ups, retraining places and hiring incentives tied to public-sector and continuity-operator hiring on EU-controlled systems, with the frontier control-certification result as a hiring condition where applicable. We publish local hospital audit stability and municipal containment without claiming recovery, to defend endurance as competence while payments catch up to announcements.
