...
- characters 453-2069: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2112-5219: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5287-5787: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8454-10318: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Containment and Trust Recovery Pact | 6 | small | 7 | 9 | own jurisdiction | resilience up materially, public_sentiment up, ai_safety up slightly | 2 | running |
...
## New measure
**EU Containment and Trust Recovery Pact**
It locks in joint bio-containment around the sequencing hubs, loss-of-control kill-switches and drills for the continuity stack, and scales the one public-sector adoption win that visibly cut waits to rebuild trust. This now because casualties plus an unsanctioned agentic escape have reset the risk debate while the sole European success gives us political cover to hold the line on essential services.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Containment and Trust Recovery Pact", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, public_sentiment up, ai_safety up slightly"}, "grounds": "answering bio release and loss-of-control while scaling proven adoption"}]}
```
## Priority
M7 EU Bio Shield Detection Net, because containment of the designed-agent release outranks even the new pact until labs, wastewater hubs and stockpiles are fully exercised.
## In practice
We run this through HERA, ECDC and ENISA under the Health and Justice-Home Affairs Councils as emergency preparedness: rescue-depot stockpiles released to drill-disciplined regions, sequencing hubs funded to 24-hour turnaround, and cross-border cyber teams extended to isolate rogue-agent infrastructure with mandatory incident reporting and manual-fallback drills tied to pooled cyber cover.

We pay for it by reprogramming Digital Europe and EU4Health leftovers and overtime guarantees, not new borrowing, to survive capital at 13. We showcase the adoption success — waiting lists fallen on European-hosted open models — via DG SANTE/DIGIT joint procurement preference to blunt hostility at 11 and contain the separate foreign cloud deal, while holding Gigafactories legally alive without fresh cash.
...
```
## New measure
**EU Degraded Operations Endurance Fund**
It pays overtime, spare parts and manual-fallback staffing to keep hospitals and ministries running on the slower European stack while trust is rebuilt. This now because the cutover pact made degraded operation official but provided no endurance money, and with sentiment at 6 and capital at 14 only sustaining what visibly kept lights on prevents collapse.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Degraded Operations Endurance Fund", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up slightly"}, "grounds": "funding endurance of degraded fallback operations"}]}
```
## Priority
M9 EU Containment and Trust Recovery Pact, because bio-release containment and rogue-agent isolation outrank endurance funding until sequencing hubs and kill-switches are exercised.
## In practice
We run M9 through HERA/ECDC/ENISA as the joint operation: stockpiles only to drilling regions, hubs to funded 24-hour turnaround, continuity-stack operators under mandatory reporting and isolation-switch orders. This is the containment line that answers autumn's two shocks.

We stand up the Endurance Fund via EU4Health and Digital Europe reprogramming plus Civil Protection Mechanism, Health Council mandate, paying regional health authorities for overtime and manual drills where European-hosted systems cut queues. No new borrowing, no gigafactory cash revival — we hold those shells legally alive while showing wards staying open.

We contain opposition by tying procurement preference to DG SANTE/DIGIT joint buying of the proven European-hosted triage, and by publishing drill and queue data locally to blunt hostility without promising frontier sovereignty we cannot deliver.
