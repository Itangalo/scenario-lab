...
- characters 105-13306: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13349-13541: `{{metrics_json}}` from the run's live metric values
- characters 13634-14761: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14804-17016: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17072-17874: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Loss-of-Control Containment Protocol | 2 |
...
- `emergent_insurance_retreat` -- first noted turn 4, listed in 1 turn(s) so far: Major EU reinsurers jointly suspend new cyber coverage for municipalities lacking 24-hour monitoring, triggering a wave of local service cutbacks and lawsuits over the Commission's uneven hardening rollout.
...
## Metrics
```json
{"ai_capability": 58.0, "openweight_capability": 54.0, "ai_safety": 32.0, "resilience": 45.0, "eu_ai_sovereignty": 18.0, "eu_political_capital": 11.0, "public_sentiment": 27.0}
```
...
## Notepad
PORTFOLIO CHARGE t4: Gigafactories −3, Tech sovereignty package −3, EU Frontier Evaluation Taskforce −2, EU Loss-of-Control Containment Protocol −2, priority −1 = −11
PROPOSAL BONUS: none — new containment protocol answers this turn's unsanctioned incident and open release, not a negative event from the prior three turns
