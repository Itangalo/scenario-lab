...
- characters 456-1585: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1629-3576: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5783-6283: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9583-11564: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Turn upstream chokepoint leverage into secured compute, insured towns and running services through the semiconductor blockade
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Clinical Fallback Compute Reserve | 4 | small | 12 | 14 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up moderately | 2 | running |
...
| M3 | EU Grid and Critical Systems Hardening Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up strongly, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Frontier Evaluation Taskforce | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via technical capacity | 2 | finished |
| M5 | EU Breakthrough Shield Deployment | 6 | small | 3 | 4 | own jurisdiction | resilience up strongly, ai_safety up moderately | 2 | finished |
| M6 | EU Loss-of-Control Containment Protocol | 6 | small | 4 | 5 | own jurisdiction | resilience up strongly, ai_safety up moderately | 2 | finished |
| M7 | Municipal Resilience Co-financing Facility | 6 | small | 5 | 7 | own jurisdiction | resilience up moderately, eu_political_capital up slightly via trust | 2 | finished |
| M8 | EU Accession to the US-China Safety Floor | 8 | small | 7 | 9 | US, China and frontier developers via coalition | ai_safety up moderately via shared assurance, ai_capability growth slowed slightly | 2 | finished |
| M9 | EU Cyber Defence Breakthrough Deployment | 6 | small | 8 | 10 | own jurisdiction | resilience up strongly, ai_safety up slightly via reduced incident impact | 2 | finished |
| M10 | EU Chokepoint Bargain | 8 | small | 10 | 12 | US, China, coalition partners and frontier developers via supply-chain leverage | eu_ai_sovereignty up moderately, eu_political_capital up slightly if bargain lands, resilience up slightly via secured spares | 2 | finished |
...
## New measure
**EU Clinical Fallback Compute Reserve**
Keeps hospitals, grids and ministries running on EU-controlled quotas and trailing-node capacity after the US frontier cutoff, by pinning the remaining chokepoint spares and reversible licences to clinical fallback models.
Category 4 sovereignty measure, because this turn is about who can switch off the models clinics actually use, not about patching alone.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Clinical Fallback Compute Reserve", "category": 4, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly, resilience up moderately"}, "grounds": "frontier access denied this turn cut hospitals off US models"}]}
```

## Priority
The new EU Clinical Fallback Compute Reserve, because the cutoff makes insured towns and falling waiting lists meaningless if the underlying models can be withdrawn without appeal.

## In practice
We run this through the Health and Digital Council formations on an Article 122 emergency logic: DG GROW/TRADE with ASML, Zeiss and ENISA certify a fallback clinical stack — EU-hosted open-weights at 60+ capability plus trailing-node spares already secured — as the insurable standard, and joint Commission-insurer audits re-anchor cover to that stack rather than to the withdrawn US frontier model. EEAS keeps the Seoul/Tokyo small-lot spares line open to feed it.

We bank the two breakthroughs inside it: the deployable interpretability/control result becomes the AI Office compliance passport for the fallback stack to keep audit-light clinics open, and the tailored therapies already delivered through EU quotas are shifted to run on the fallback where possible, so the medical win reads as EU-delivered continuity rather than further proof of dependence. Digital Europe funds pay CERT-EU contractors to migrate hospital and payment-system prompts off the cut-off API first.
...
```
## New measure
**EU Middle-Power Compute and Spares Pact**
Locks the Seoul-Tokyo and other middle-power spares trickle into aligned export licences, joint bargaining and shared evaluation so the clinical fallback has hardware to run on after the US cutoff and the member-state side-deal.
Category 8 international coordination, because with capital at 19 and a defection on the table we cannot buy sovereignty alone — we have to withhold chokepoints jointly.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Middle-Power Compute and Spares Pact", "category": 8, "size": "small", "finish_turn": 15, "applies_to": "coalition partners and frontier developers via supply-chain leverage", "targeted_effect": "eu_ai_sovereignty up moderately, eu_political_capital up slightly, resilience up slightly via secured spares"}, "grounds": "US cutoff plus member-state defection requires joint leverage to secure compute"}]}
```

## Priority
The new EU Middle-Power Compute and Spares Pact, because without jointly secured licences and spares the Clinical Fallback Reserve has no machines to run on and the defection becomes the Union line.

## In practice
We run the Pact through Trade and the Anti-Coercion logic: DG TRADE/GROW align export-licence exceptions with Seoul and Tokyo for trailing-node spares and reversible lots earmarked to the certified fallback clinical stack, with joint bargaining over compute access and mutual recognition of the AI Office fallback audits. The offer to partners is shared evaluation capacity from the Frontier Taskforce and pooled demand from insured towns.

At home we hold M11 as the delivery arm: Health Council Article 122 funds keep migrating hospital and payment prompts off the withdrawn US interface to EU-hosted open models, insurers re-anchor cover town-by-town to that stack, and we answer the loss-of-control incident by freezing agentic money-movement and self-replication privileges on the fallback — CERT-EU containment playbooks and audit logging from the interpretability passport — rather than opening a second front. The defecting capital is told the Pact price is better than its bilateral price, and named in Council if it undercuts the licence alignment.
