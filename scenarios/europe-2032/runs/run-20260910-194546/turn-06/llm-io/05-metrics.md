...
- characters 13321-13512: `{{metrics_json}}` from the run's live metric values
- characters 13605-14835: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14878-16828: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16884-17345: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | Municipal Resilience Co-financing Facility | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election won by anti-AI backlash candidate, posture takes effect turn 6"}]}
```
```
