...
- characters 454-2882: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2925-5524: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6266-6721: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9400-11332: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Accession to the US-China Safety Floor | 8 | small | 7 | 9 | US, China and frontier developers via coalition | ai_safety up moderately via shared assurance, ai_capability growth slowed slightly | 2 | running |
...
It seeks formal EU inclusion in the US-China risk agreement as third party / verifier and aligns EU evaluation and weights-security standards to it, so the capability slowdown covers the EU and shared assurance replaces unilateral cost.
This and now because the agreement is the only event that slows frontier capability and makes safety breakthroughs stick, and with political capital at 6.0 and sovereignty at 18.0 the Union cannot buy its way in — it must leverage supply-chain and regulatory position before the terms harden without it.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Accession to the US-China Safety Floor", "category": 8, "size": "small", "finish_turn": 9, "applies_to": "US, China and frontier developers via coalition", "targeted_effect": "ai_safety up moderately via shared assurance, ai_capability growth slowed slightly"}, "grounds": "to enter US-China agreement while window open"}]}
```

## Priority
EU Accession to the US-China Safety Floor, because being informed afterwards rather than inside locks in dependence while open-weights at 57.5 and export-control rationing decide EU access without it.

## In practice
We mandate EEAS and DG TRADE to seek observer then signatory status to the weights-security / bio-design accord, offering ASML / lithography supply-chain alignment and mutual recognition of the Frontier Evaluation Taskforce results as the EU contribution, and using the Anti-Coercion Instrument posture to avoid rationing under export-control escalation.

We task ENISA, the AI Office and the Joint Research Centre to rapidly adopt the interpretability/control breakthrough into deployment safeguards and the municipal containment playbooks, funded by reprioritising existing Digital Europe money, to lift assurance without a new cash fight towns will read as another invoice.
...
It pushes the new automated patching and swarm-detection tooling into critical operators, municipalities and the isolation playbooks via ENISA, funded by repurposed Digital Europe money.
This and now because the defensive breakthrough is the first shift back to defence in years and with open weights at frontier on unrecallable hardware, rapid diffusion of defence is the only lever that cuts harm without new cash or permits.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cyber Defence Breakthrough Deployment", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up strongly, ai_safety up slightly via reduced incident impact"}, "grounds": "to exploit defensive breakthrough before open-weight misuse spreads"}]}
```

## Priority
EU Cyber Defence Breakthrough Deployment, because containing the newly released open frontier model on municipal servers outranks accession talks while grids and clinics are exposed now.

## In practice
We task ENISA and the CERT-EU shutdown cell to certify the breakthrough patching and swarm-detection stack and push it through the existing containment isolation playbooks and the Municipal Co-financing Facility audit-light window, using repurposed Digital Europe funds so towns face no new invoice. Insurers are brought in to recognise deployed tooling as meeting 24h monitoring for cover restoration.

We keep the Accession track alive at EEAS level with the lithography offer on the table, but shift staff and political weight to deployment pilots in the queued municipalities, universities and utilities holding the new open weights, with AI Office guidance making the interpretability check plus new defences the standard for restored policies.
```
