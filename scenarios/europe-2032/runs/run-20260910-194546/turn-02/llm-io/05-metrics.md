...
- characters 105-12944: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12987-13179: `{{metrics_json}}` from the run's live metric values
- characters 13272-14726: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14769-17293: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17349-18166: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_insurance_retreat` -- first noted turn 1, listed in 2 turn(s) so far: Major EU insurers introduce AI-cyber exclusions or steep premium hikes for municipalities and hospitals after the summer lab incident, triggering a public fight over who bears AI-enabled disruption costs.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 37.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 30.0, "public_sentiment": 34.0}
```
