...
- characters 456-1849: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1893-4184: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5870-6312: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9279-11584: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Rebuild trusted adoption on assured and resilient European capacity
...
| M4 | EU Sovereign Hosting and Supply-Chain Compact | 8 | large | 2 | 5 | own jurisdiction and supply-chain coalition | eu_ai_sovereignty up materially, resilience up slightly, ai_safety up slightly via oversight | 3 | finished |
| M5 | EU Open-Weights Consequence Shield | 6 | small | 3 | 5 | own jurisdiction | resilience up materially, ai_safety up slightly via detection | 2 | finished |
| M6 | European Trustworthy Adoption Accelerator | 9 | small | 5 | 7 | own jurisdiction | public_sentiment up materially, eu_political_capital up slightly, resilience up slightly | 2 | finished |
| M7 | EU Bio-Response and Open-Model Misuse Shield | 6 | small | 6 | 8 | own jurisdiction | resilience up materially, ai_safety up slightly via detection | 2 | finished |
| M8 | EU Municipal Services Cyber Restoration Surge | 6 | small | 8 | 9 | own jurisdiction | resilience up materially, ai_safety up slightly via detection | 2 | finished |
| M9 | EU Assured Deployment Certification Fast-Track | 5 | small | 9 | 10 | own jurisdiction | ai_safety up materially, public_sentiment up slightly, resilience up slightly | 2 | finished |
| M10 | EU Graduate and Displaced-Worker Transition Guarantee | 7 | small | 10 | 11 | own jurisdiction | resilience up materially, public_sentiment up materially, eu_political_capital up slightly | 2 | finished |
...
## New measure
**EU Graduate and Displaced-Worker Transition Guarantee**
Creates an EU-coordinated entry-level and mid-career transition guarantee — employer-tied retraining, wage-insurance top-ups and public-interest placement slots in health, cyber-restoration and certified-AI deployment — funded by repurposing ESF+ and the Adoption Accelerator envelope to answer measurable AI job loss without a fresh budget fight.
This is a Labour and social protection measure because hiring absence in law, accountancy, software and admin, compounded by the benefits scandal, is what will break adoption and cohesion now; a small, visible guarantee outranks new compute or rules while political capital is at 21.0.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Graduate and Displaced-Worker Transition Guarantee", "category": 7, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, public_sentiment up materially, eu_political_capital up slightly"}, "grounds": "measurable AI displacement hitting graduates amid scandal and defection"}]}
```

## Priority
M10 EU Graduate and Displaced-Worker Transition Guarantee, because stopping displacement from becoming hostility and defection outranks certification alone when counters just reopened and the AI Act's legitimacy is on trial.

## In practice
We task EPSCO with DG EMPL to stand up the guarantee from ESF+ reflows and uncommitted Adoption Accelerator funds, with no new ECOFIN ask: short retraining vouchers, six-month wage-insurance for firms that retain-and-retrain, and paid public-interest placements tied to M8 restoration teams and M9 certified deployments in sequencing hubs.

We contain the automated-decision scandal inside existing law: AI Office and national market-surveillance authorities open a joint enforcement audit under the AI Act high-risk track, with the Ombudsman and Fundamental Rights Agency publishing logs and redress, to show enforcement rather than conceding the Act is misdrawn. On defection, we hold the US access and sovereignty line publicly while offering the defecting capital grid and certification priority to return, avoiding a public split we cannot afford to lose.
...
```
## New measure
**EU Transition Guarantee Scale-Up and Redress Facility**
Keeps the transition guarantee alive through the investment bust by scaling paid placements and wage-insurance from reshuffled ESF+ and adding binding redress and human-review floors for the benefits-scoring scandal, to stop displacement plus scandal hardening into permanent hostility.
This is a Labour and social protection measure because with capital at 17 and sentiment at 22, visible jobs and redress outrank new compute or rules, and the investment collapse means private hiring will not return on its own.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Transition Guarantee Scale-Up and Redress Facility", "category": 7, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, public_sentiment up, eu_political_capital up slightly"}, "grounds": "graduate unemployment crisis plus benefits scandal amid investment collapse"}]}
```

## Priority
The new Transition Guarantee Scale-Up and Redress Facility, because only paid slots and redress can prevent the hiring collapse and automated-decision scandal from permanently breaking trusted adoption when no other measure is in flight.

## In practice
We task DG EMPL with EPSCO to extend M10 using ESF+ reflows and unspent adoption funds, no new ECOFIN ask: expand employer wage-insurance, fund 10k-scale public-interest placements tied to health hubs, municipal restoration and certified deployment sites, and publish monthly slots-filled data to make the guarantee real.

We contain the scandal inside enforcement to protect the Act: AI Office with market-surveillance authorities and FRA/Ombudsman run a joint audit, enforce meaningful human-oversight and log-review floors, suspend the offending scoring uses, and pay redress from recoveries. On the investment collapse and side-deal, we hold the common line, protect gigafactory/hosting compacts already built, and offer grid and procurement priority only for return to common terms.
