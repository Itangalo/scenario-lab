...
- characters 105-13335: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13378-13570: `{{metrics_json}}` from the run's live metric values
- characters 13663-15129: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15172-17494: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17550-17994: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | European Trustworthy Adoption Accelerator | 2 |
| M7 | EU Bio-Response and Open-Model Misuse Shield | 2 |
...
PORTFOLIO CHARGE t5: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M6 European Trustworthy Adoption Accelerator −2, priority −1 = −9
PROPOSAL BONUS: none — new Adoption Accelerator answers positive adoption_success, not a negative event from last three turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "ALLIANCE"}, "grounds": "turn 5 election won on coalition access, takes effect turn 6"}]}
```
```
