...
- characters 376-1842: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1885-4207: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4263-5026: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5089-26317: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 26672-27027: `{{event_history}}` from the run's own event record
...
- `emergent_municipal_insurance_pullback` -- first noted turn 4, listed in 2 turn(s) so far: Municipal insurers begin refusing to cover OT and hospital outages tied to unpatched open-weight tooling, forcing a wave of deferred digitisation projects to stall across mid-sized cities.
...
- Turn 2 (4 turn(s) ago): loss_of_control_incident, taiwan_blockade, knowledge_work_augmented, export_control_escalation, emergent_sovereign_hosting_offer
- Turn 3 (3 turn(s) ago): openweight_frontier_release
- Turn 5 (1 turn(s) ago): taiwan_tension_rise, adoption_success, election_alliance
