...
- characters 377-2404: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2448-5260: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5316-6446: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 6509-30736: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 31091-31274: `{{event_history}}` from the run's own event record
...
- `emergent_gigafactory_siting_block` -- first noted turn 2, listed in 1 turn(s) so far: A coordinated municipal backlash stalls two gigafactory shortlist sites with court injunctions over grid priority and water use, delaying final site decisions past mid-2027.
- `emergent_grid_insurance_crunch` -- first noted turn 2, listed in 1 turn(s) so far: European industrial insurers sharply reprice operational-technology cyber cover after the autumn grid intrusions, demanding certified segmentation audits that most municipal utilities cannot yet pass, freezing some Shield procurement.
...
- Turn 2 (1 turn(s) ago): cyber_major_incident, loss_of_control_incident, eval_anomaly_reports, safety_breakthrough, knowledge_work_augmented
