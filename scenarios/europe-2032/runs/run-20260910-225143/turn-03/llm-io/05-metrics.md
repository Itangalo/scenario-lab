...
- characters 106-13200: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13243-13435: `{{metrics_json}}` from the run's live metric values
- characters 13528-15555: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15599-18411: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18467-19276: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Evaluation and Containment Taskforce | 2 |
| M5 | Municipal Shield Audit and Insurance Backstop | 2 |
...
PORTFOLIO CHARGE t2: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M3 EU Critical Services Shield −3, M4 EU Evaluation and Containment Taskforce −2, priority −1 = −12
PROPOSAL BONUS: none — Taskforce (cat 5, small) answers loss-of-control/eval anomaly, but knowledge_work_augmented removes rule 6 bonus with no displacement crisis to point at
LEGITIMACY LENDS: capital 29, sentiment 32 -> +1
SOVEREIGNTY: 21 last turn, no capacity event, Gigafactories in flight +0, capability rose 2.0 −1 = 20

## Emerging developments (tracked)

- `emergent_gigafactory_siting_block` -- first noted turn 2, listed in 2 turn(s) so far: A coordinated municipal backlash stalls two gigafactory shortlist sites with court injunctions over grid priority and water use, delaying final site decisions past mid-2027.
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M3", "fields": {"finish_turn": 6}, "grounds": "named priority"}]}
```
```
