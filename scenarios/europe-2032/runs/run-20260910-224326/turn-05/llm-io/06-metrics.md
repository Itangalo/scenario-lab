...
- characters 106-13643: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13686-13878: `{{metrics_json}}` from the run's live metric values
- characters 13971-14971: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15015-17143: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17199-18384: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Critical Services Cyber Hardening Surge | 2 |
...
|---|---|---|---|---|---|---|---|---|---|
| M5 | EU Displaced Worker Transition Fund | 7 | small | 3 | 5 | own jurisdiction | resilience up, public_sentiment up | 2 | finished |
| M6 | EU Essential Services AI Continuity Shield | 6 | small | 4 | 5 | own jurisdiction | resilience up, eu_political_capital up modestly | 2 | finished |
...
- `emergent_care_procurement_bloc` -- first noted turn 5, listed in 1 turn(s) so far: Several major EU hospital groups jointly suspend new AI procurement and publish a common requirement for on-premise fallback, creating a de facto EU-hosted procurement bloc after the February cutoff.
- `emergent_anti_ai_blockades` -- first noted turn 5, listed in 1 turn(s) so far: Coordinated physical blockades of data-centre construction sites and grid substations spread from siting disputes to a wider anti-AI protest wave, forcing temporary construction halts in two member states.
...
## Metrics
```json
{"ai_capability": 67.0, "openweight_capability": 60.8, "ai_safety": 32.0, "resilience": 38.0, "eu_ai_sovereignty": 14.0, "eu_political_capital": 20.0, "public_sentiment": 19.0}
```
...
## Notepad
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Services Cyber Hardening Surge −2, priority −1 = −9
PROPOSAL BONUS: EU Critical Services Cyber Hardening Surge (cat 6, small) +3 — cyber_major_incident landed this turn hitting hospitals/grids and this answers it directly
LEGITIMACY LENDS: capital 17, sentiment 24 -> +2
SOVEREIGNTY: 15 last turn, no capacity event, Gigafactories in flight +0, capability rose 3.0 −1 = 14

## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "pending"}}, {"op": "update", "table": "measures", "id": "M7", "fields": {"finish_turn": 6}, "grounds": "named priority"}]}
```
```
