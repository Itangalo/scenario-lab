...
- characters 455-1980: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2024-4526: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5532-5972: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7594-9762: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Public AI Accountability Package | 2 | small | 2 | 4 | own jurisdiction | public_sentiment up, resilience up modestly, ai_safety up modestly | 2 | running |
...
## New measure
**EU Public AI Accountability Package**
Mandates incident reporting, public registry and independent audit of AI decision systems in welfare, policing and courts, with redress for those wronged. This is category 2 Transparency and reporting work, and why now is because the social-insurance scandal has exposed the second, more damaging failure — deployments lawful but outside Annex III — leaving the Act looking written for 2024 systems and making restriction cheap unless the Union closes the gap itself.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Public AI Accountability Package", "category": 2, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, resilience up modestly, ai_safety up modestly"}, "grounds": "to answer automated-decision scandal and close Annex III gap"}]}
```

## Priority
EU Public AI Accountability Package, because the scandal has made adoption politically impossible and without restoring trust no gigafactory or evaluation authority survives Council.

## In practice
We keep the Gigafactories (M1) and permitting zones (M2) moving through DG CNECT, DG COMP state-aid clearance and EIB disbursement, but we spend no new political fight there this turn — site pledges from Paris, Berlin, Madrid, Stockholm and Warsaw hold without a vote. The Evaluation Institute (M3) continues staffing under the AI Office with JRC support, framed as the technical arm that will certify the audits.

This turn's fight is in EPSCO and JHA Councils and LIBE Committee: an implementing decision under AI Act Articles 11, 12 and 72 requiring logging to be read, human oversight to be timed and resourced, and all public-sector decision systems — whether classified high-risk or not — to be registered, with ombudsman-triggered review and compensation funded from deployment budgets. We concede to industry that testing is decoupled from market access for private models to buy their silence, while telling Parliament the gap reading is admitted openly: the Act missed these deployments, and we fix it by extension, not by trying harder.
...
```
## New measure
**EU Displaced Worker Transition Fund**
Provides wage insurance, rapid retraining vouchers and hiring incentives for entry-level roles in law, accountancy, software and customer operations hit by AI displacement, funded by a levy-linked ESF+ top-up. This is Labour and social protection work, and why now is because graduate hiring has stopped while trust is already collapsed — without a visible cushion, every sovereignty or resilience build will be read as Brussels protecting systems, not people.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Displaced Worker Transition Fund", "category": 7, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up"}, "grounds": "to answer measurable AI job losses amid trust collapse"}]}
```

## Priority
EU Displaced Worker Transition Fund, because labour displacement this turn outranked our own sovereign-capacity direction — without stemming the jobs shock no factory or institute survives Council.

## In practice
We push the Transition Fund through EPSCO with ESF+ as legal base, asking member states to co-finance retraining desks in public employment services while the Commission covers wage-insurance pilots from uncommitted ESF+ and RRF loan headroom. BusinessEurope gets flexibility language on restructuring in return; ETUC gets the levy-link principle for automating employers conceded for a later turn.

We keep M1/M2 alive on paper through DG CNECT and EIB technical work without spending fresh fights — site pledges hold, permitting-zone financing talks continue — and we direct the AI Office institute (M3) to absorb the safety breakthrough quickly into its test protocols to regain credibility. The Accountability Package (M4) proceeds on registration only, with logging and compensation timetables tied openly to the budget decisions now queued behind the jobs response.
