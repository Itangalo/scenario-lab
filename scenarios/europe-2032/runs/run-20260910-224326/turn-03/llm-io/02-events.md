...
- characters 1946-4448: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4504-5601: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5664-29240: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29595-29689: `{{event_history}}` from the run's own event record
...
- `emergent_toolkit_proliferation` -- first noted turn 2, listed in 1 turn(s) so far: Independent grid-operator audits in the US and Asia publish forensic details of the autumn intrusions, including reusable tooling derived from open models, sharply raising copycat risk.
- `emergent_grid_forensics_fallout` -- first noted turn 2, listed in 1 turn(s) so far: Independent press publishes forensic linkage tying the autumn grid intrusions to tooling derived from the newest open-weight class, triggering insurer repricing of critical-infrastructure cyber cover across the EU.
...
- Turn 2 (1 turn(s) ago): automated_decision_scandal
