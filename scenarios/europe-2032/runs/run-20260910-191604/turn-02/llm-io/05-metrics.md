...
- characters 105-12912: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12955-13147: `{{metrics_json}}` from the run's live metric values
- characters 13240-14434: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14477-16789: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16845-17809: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_ai_act_delay_challenge` -- first noted turn 1, listed in 2 turn(s) so far: EU NGOs and a coalition of MEPs file a legal challenge to the postponement of AI Act high-risk and GPAI obligations to 2027-2028, forcing a public court battle over enforcement timelines.
- `emergent_compute_bottleneck_shock` -- first noted turn 1, listed in 2 turn(s) so far: Major US AI lab announces emergency compute rationing after grid interconnection delays and transformer shortages stall new data centre phases.
