...
- characters 105-13312: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13355-13547: `{{metrics_json}}` from the run's live metric values
- characters 13640-15516: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15559-17662: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17718-18759: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M5 | EU Verification Pillar | 2 |
| M6 | EU Adoption Dividend | 2 |
...
PORTFOLIO CHARGE t3: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Continuity Stack −2, EU Verification Pillar −2 = −10 + priority −1 = −11
PROPOSAL BONUS: none — new Verification Pillar (cat 8, small) answers us_china_agreement which is not a negative event from last three rounds
LEGITIMACY LENDS: capital 17, sentiment 29 -> +2
SOVEREIGNTY: 20 last turn, no capacity event, Gigafactories in flight +0, capability rose 1.0 −0 = 20

## Emerging developments (tracked)

- `emergent_ai_act_court_timetable` -- first noted turn 4, listed in 1 turn(s) so far: ECJ Advocate General issues interim opinion backing NGO/MEP challenge that high-risk and GPAI obligations were unlawfully shelved, forcing the Commission to publish a binding timetable during this semester.
- `emergent_permit_injunction_freeze` -- first noted turn 4, listed in 1 turn(s) so far: Municipal courts in two member states issue interim injunctions halting Gigafactory zone permits on power-grid and environmental grounds, freezing site works through mid-2028.
...
In the middle of that squeeze, one capital broke ranks. A large member state signed its own cloud and model-supply arrangement directly with an American hyperscaler, on pricing and data terms well below what Brussels had been holding out for collectively. The government called it pragmatism for hospitals and industry. In Coreper it was read as an open undercut, and no amount of careful language about non-precedent contained the irritation.

Brussels answered with what it could still point to. Clinics using the vetted open model on shared supercomputing showed waiting-list triage falling from months to weeks, permit offices clearing backlogs, tutoring pilots with measurable results. The Commission repackaged the fallback as a programme — funded deployment, procurement standards, retraining money — and tied grid fast-tracks for new power zones to the reserved inference behind those results.

It half-worked. Patients and mayors liked the visible benefit, and auditors liked that only checked deployments qualified for funds. But court injunctions in two states kept new factory zones frozen, builders said power not money was the constraint, and the court challenge over the delayed rulebook turned hearings into weekly stories about voluntary audits. Essential services stayed up, on borrowed machines, while the infrastructure meant to replace them stalled.

By June, dependence was no longer an abstraction debated in councils. It was a waiting room kept open by someone else's computers, and a side-deal everyone else had to live with.

## Notepad
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Verification Pillar −2, EU Adoption Dividend −2 = −10 + priority no effect, no cost (capital below 20) = −10
PROPOSAL BONUS: EU Adoption Dividend (cat 9, small) +2 — answers investment collapse and defection indirectly by banking adoption win, small size, same turn
LEGITIMACY LENDS: capital 3, sentiment 32 -> +2
SOVEREIGNTY: 20 last turn, no capacity event finishes, Gigafactories in flight +0, member_state_defection t4 −2, capability rose 1.0 −0 = 18
