...
- characters 456-1557: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1601-3783: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5094-5588: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8357-10445: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_restoration_litigation_freeze (emergent event):** Municipal outage maps and restoration prioritization trigger coordinated legal challenges by fringe villages and single-doctor practices alleging discriminatory restoration, freezing ENISA corps deployments pending court rulings.
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Joint Response Accession and Assured Control Deployment | 6 | small | 10 | 12 | own jurisdiction | resilience up, ai_safety up via deployed assurance | 2 | running |
...
## New measure
**EU Joint Response Accession and Assured Control Deployment**
Join the joint cyber-threat telemetry pool and biosurveillance pact on offer while rolling the new interpretability/control result onto EU-run audited builds and restoration kits. This is a preparedness and resilience instrument (category 6), chosen now because the loss-of-control incident plus the invited joint command is the only protection we cannot build alone, and the safety breakthrough gives us something credible to bring to the table.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Joint Response Accession and Assured Control Deployment", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up via deployed assurance"}, "grounds": "joint invitation plus loss-of-control and safety breakthrough in same turn"}]}
```

## Priority
M8 EU Edge Restoration and Containment Corps, because with capital at 15.0 the visible edge restoration must complete first to hold the commitment to keep services running, while the new accession builds behind it.

## In practice
We keep ENISA as operational lead for M8 under NIS2, funded only from reallocated Digital Europe and civil-protection money, finishing clean backups and radio-paper packs for fringe villages and single-doctor practices. Priority stays to audited-build acceptors; we do not reopen the bilateral cloud deal or the frozen factory zones this turn beyond logging the Hague servicing cut.

In parallel we mandate ENISA and HERA to sign the telemetry-sharing and sample-sharing terms, seconding a small EU cell to the joint cyber command and accepting the standing investigation mandate. DG CNECT pushes the new control/interpretability check into the vetted open-model builds that town halls and clinics already run, using incident learning from the loss-of-control case to set escalation thresholds. No new law or ECOFIN vote is sought; we act by implementing act and Council conclusions to fit what capital we have.
...
```
## New measure
None this turn. We are waiting for the restoration-freeze courts to rule and for political capital to rebuild before starting anything new.

## Store changes
No changes.

## Priority
M9 EU Joint Response Accession and Assured Control Deployment, because with deployments frozen by litigation and capital at 10.0 it is the only funded vehicle that still delivers resilience and assured control without a new vote or new money.

## In practice
We put the Commission Legal Service and ENISA under the M9 mandate to answer the fringe-village and single-doctor discrimination suits: publish outage maps and triage criteria, offer renegotiated liability waivers and audited-build acceptance as objective prioritization, and seek interim relief to allow paper packs, detection kits and clean backups to continue pending judgment. No new restoration push is ordered where courts have frozen it.

In parallel we execute the low-cost part of M9: second the small cell to the shared cyber command and biosurveillance pact via Council conclusions, feed live telemetry where legally allowed, and have DG CNECT and ENISA roll the adopted control check into the town-hall and clinic builds already deployed. Funding stays to reallocated Digital Europe and civil-protection lines; we do not touch Gigafactory zones, ASML servicing, or the bilateral cloud deal this turn beyond monitoring.
