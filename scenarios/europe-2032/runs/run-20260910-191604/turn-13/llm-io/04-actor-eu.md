...
- characters 2145-4290: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5190-5684: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8639-10542: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential services running on EU-controlled resilient capacity while containing harm from unrecallable open models
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Municipal Payments and Services Hardening Sprint | 6 | small | 12 | 14 | own jurisdiction | resilience up, public_sentiment up slightly via restored payments | 2 | running |
...
| M5 | EU Verification Pillar | 8 | small | 3 | 6 | US, China, coalition | ai_safety up, eu_ai_sovereignty up slightly via leverage | 2 | finished |
| M6 | EU Adoption Dividend | 9 | small | 4 | 7 | own jurisdiction | public_sentiment up, resilience up slightly via sustained services | 2 | finished |
| M7 | EU Municipal Shield Patch | 6 | small | 7 | 9 | own jurisdiction | resilience up, ai_safety up slightly via incident learning | 2 | finished |
| M8 | EU Edge Restoration and Containment Corps | 6 | small | 9 | 11 | own jurisdiction | resilience up, ai_safety up slightly via incident learning | 2 | finished |
| M9 | EU Joint Response Accession and Assured Control Deployment | 6 | small | 10 | 12 | own jurisdiction | resilience up, ai_safety up via deployed assurance | 2 | finished |
...
## New measure
**EU Municipal Payments and Services Hardening Sprint**
It deploys the new defensive breakthrough — automated patching and swarm-behaviour detection — to municipal payment, reimbursement and core public-service systems, with cash-counter fallback and fraud-cleanup funded centrally. Why this and why now: the payment freeze is actively collapsing trust at sentiment 14.0 and capital 8.0, and this is the first turn in years defence has a technical edge to exploit without new legislation or new money fights.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Payments and Services Hardening Sprint", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up slightly via restored payments"}, "grounds": "payment freeze plus defence breakthrough window"}]}
```

## Priority
EU Municipal Payments and Services Hardening Sprint, because with no measures in flight and courts still freezing edge restoration, stopping the cash-counter run is the only action that rebuilds capital to do anything else.

## In practice
We task ENISA and DG CNECT under existing NIS2 and Digital Europe reallocated lines to push the breakthrough tooling to municipal treasuries and health reimbursement hubs: centrally-signed patches, swarm-detection sensors on direct-debit rails, and exercised manual fallback. No new vote; we use implementing acts and the seconded shared-cyber-command cell for telemetry where lawful.

We pair it with visible relief: EU-funded fraud-reimbursement clearing for households hit in the freeze weeks, and published triage criteria to answer the discrimination writs without violating interim orders. Member-state interior and finance ministries get the kits and paper packs already cleared by judges, while full restoration waits for rulings.
...
```
## New measure
**EU Major Incident Containment and Continuity Operation**
It activates cross-border mutual aid, ENISA-led isolation of the automated attack, and manual-continuity for hit public services using existing continuity funds. Why this and why now: a large model-generated attack is landing as open weights jump to near-frontier, and with capital at 13 and sentiment at 12.5 we cannot afford a new build — only an operation on what is already in flight.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Major Incident Containment and Continuity Operation", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, contain sentiment fall via visible continuity"}, "grounds": "major automated attack plus openweight jump requires immediate containment"}]}
```

## Priority
M10 EU Municipal Payments and Services Hardening Sprint, because restoring payments is the only path to rebuild capital and the new operation depends on its sensors and signed patches to contain the sweep.

## In practice
We task ENISA and the shared cyber command cell under NIS2 Article 11 and existing Digital Europe lines to isolate affected municipal and health systems, push M10's verified patches and swarm-detection, and stand up cash-counter and paper fallback where networks are pulled. No new legislation; Council JHA conclusions give cover, reallocated funds pay overtime and kits.

We direct DG CNECT and member-state treasuries to keep reimbursement flowing with simplified single-page claims, and we publish daily maps of what is up, degraded, or on manual fallback to stop rumour filling the gap. Finance ministries questioning cleanup get a capped EU co-pay tied to exercised continuity, not auditors. Full fringe re-wiring still waits for the courts — we move only sealed kits already cleared.
