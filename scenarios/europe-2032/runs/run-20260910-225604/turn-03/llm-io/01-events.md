...
- characters 377-1764: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1808-3994: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4050-5157: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5220-29447: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29802-29894: `{{event_history}}` from the run's own event record
...
- `emergent_grid_forensics_leak` -- first noted turn 2, listed in 1 turn(s) so far: Independent forensic researchers publish tooling fingerprints linking the autumn grid intrusions to a downloadable frontier model variant, triggering public debate over open-model liability.
- `emergent_gigafactory_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: Municipal and regional opposition coalesces against gigafactory shortlist sites over power prices, water use and permitting, delaying grid-connection approvals in at least one shortlisted state.
