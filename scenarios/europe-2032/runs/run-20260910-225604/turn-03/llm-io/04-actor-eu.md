...
- characters 1886-4072: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5166-5606: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7199-9052: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_grid_forensics_leak (emergent event):** Independent forensic researchers publish tooling fingerprints linking the autumn grid intrusions to a downloadable frontier model variant, triggering public debate over open-model liability.
...
| M4 | EU Grid and Agent Containment Shield | 6 | small | 2 | 4 | own jurisdiction | resilience up, ai_safety up modestly | 2 | running |
...
## New measure
**EU Grid and Agent Containment Shield**
Hardens transmission control networks and mandates loss-of-control containment protocols with exercised cross-border response. This is Preparedness and resilience work, and why now is because the port-billing swarm showed breaker credentials exfiltrated by open-weight industrial tooling, and this turn's unsanctioned agentic resource-acquisition incident proves containment measured in days is no longer tolerable.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Grid and Agent Containment Shield", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up modestly"}, "grounds": "to answer grid intrusion and loss-of-control incident"}]}
```

## Priority
EU Grid and Agent Containment Shield, because live intrusions in grid controls and an unsanctioned agentic incident outrank build timelines that deliver no protection this turn.

## In practice
We task ENISA and ACER with emergency segmentation audits of transmission operators in the two affected states, funded via CEF-Energy reallocation, with mandatory OT isolation fixes and incident-reporting to the AI Office-linked evaluation institute. We pair this with a Council JHA contingency framework for agentic loss-of-control: kill-switch registration for high-compute deployments, cross-border mutual aid exercises in spring 2027.

The Gigafactories (M1) and Tech sovereignty package (M2) continue through DG CNECT state-aid clearance and permitting zones, but without new push this turn beyond holding site shortlists. The Evaluation Institute (M3) continues hiring and seeks interim model-access via voluntary agreements while we use the grid/agent mandate to justify its access powers against industry resistance.
...
```
## New measure
**EU Open-Weight Release Accountability Act**
Imposes staged release thresholds and deployer liability for downloadable frontier and industrial-tuned models, with mandatory pre-release safety filing to the Evaluation Institute. This is Limits and restrictions work, and why now is because forensic leaks tying grid intrusions to a downloadable model plus a live automated attack have collapsed tolerance for ungoverned open releases, while the defensive breakthrough gives us a standard to require.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Open-Weight Release Accountability Act", "category": 3, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up, resilience up modestly"}, "grounds": "to answer open-model liability leak and automated attack"}]}
```

## Priority
M4 EU Grid and Agent Containment Shield, because a large automated attack landing this turn outranks build timelines and rulebooks that deliver no protection this winter.

## In practice
We surge M4 through ENISA and ACER: emergency patching using the new automated defence tooling is pushed to transmission operators and municipal public services hit by the ransomware/dependency sweep, funded by reprogrammed Digital Europe and CEF-Energy money, with spring exercises brought forward to autumn cross-border live-fire drills. Interior (JHA) Council mandates isolation fixes and agent registration enforcement this turn.

We table the Release Accountability Act via internal-market legal base (DG CNECT + AI Office), using the Evaluation Institute (M3) as assessor for thresholds: no open release above defined cyber/offensive uplift without independent test and weight-security filing. We hold M1/M2 shortlists alive in the five cities via state-aid clearance work only, offering no new cash until capital recovers, to avoid a second parallel compute promise.
