...
- characters 376-1944: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1987-4127: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4183-5447: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5510-33058: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 33413-33679: `{{event_history}}` from the run's own event record
...
- `emergent_utility_isolation_backfire` -- first noted turn 3, listed in 1 turn(s) so far: A defensive isolation by a small municipal utility goes wrong, cutting chlorination monitoring for days and causing a brief public-health scare that is blamed on the EU-mandated playbook rather than the intruders.
- `emergent_cyber_insurance_retreat` -- first noted turn 3, listed in 1 turn(s) so far: Major cyber-reinsurers temporarily withdraw or reprice coverage for small municipal utilities after the copycat probe wave, leaving towns unable to insure operational downtime from defensive isolations.
...
- Turn 2 (2 turn(s) ago): openweight_frontier_release, taiwan_tension_rise, emergent_intrusion_tooling_copycats
- Turn 3 (1 turn(s) ago): loss_of_control_incident, capability_jump, eu_frontier_access_denied, adoption_success
