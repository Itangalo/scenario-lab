...
- characters 1802-3930: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4500-4940: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6857-7973: `{{previous_actions}}` from the actor's own response from the previous turn
...
```
## New measure
None this turn, while we conserve collapsing political capital for the Shield to finish and wait for fiscal headroom to use the ASML chokepoint without breaking cohesion.

## Store changes
No changes.

## Priority
M4 EU Grid and Public Services Shield, because brittle grids and unpatched hospitals will decide whether US coercion over ASML plus continued intrusions cascade, outranking unfunded gigafactories this turn.

## In practice
We drive M4 to its finish through ENISA, DG HOME and ACER under NIS2/CER emergency mandates: verified segmentation audits for TSOs, EU-funded response teams held in place in hit municipalities and hospitals, with CEF-Digital bridging the energy-ministry cost fight to keep spares and legacy controllers covered until the Shield lands.

We freeze M1/M2 spend but protect the five site reservations, permits and grid connections via DG CNECT and EIB, and task DG TRADE to prepare an Anti-Coercion Instrument file on the US ASML order — documenting extraterritorial use of EU supply-chain leverage without triggering a rupture we cannot afford at 11 capital — so a joint leverage move can be priced when capital recovers.
