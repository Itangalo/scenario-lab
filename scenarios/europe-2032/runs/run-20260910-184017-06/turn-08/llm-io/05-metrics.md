...
- characters 105-13332: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13375-13565: `{{metrics_json}}` from the run's live metric values
- characters 13658-15297: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15340-17357: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17413-18074: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_embodied_lockin_offer` -- first noted turn 7, listed in 2 turn(s) so far: Chinese humanoid vendors offer EU logistics firms cut-price fleets with on-device models derived from the leaked open frontier, locking in maintenance dependence outside EU jurisdiction.
...
## Metrics
```json
{"ai_capability": 76.0, "openweight_capability": 70.1, "ai_safety": 6.0, "resilience": 34.0, "eu_ai_sovereignty": 9.0, "eu_political_capital": 0.0, "public_sentiment": 2.0}
```
...
## Notepad
PORTFOLIO CHARGE t8: InvestAI Gigafactories −3, priority −1 = −4
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 0, sentiment 2 -> +0, sentiment above capital but floor holds after charges and offsetting event losses/gains
SOVEREIGNTY: 13 last turn, no category 4 finish, Gigafactories in flight +0, supply_chain_coercion t8 −3, capability rose 3.0 −1 = 9
