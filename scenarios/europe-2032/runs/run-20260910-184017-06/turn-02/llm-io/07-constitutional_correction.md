...
- characters 2529-2960: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- Organizational Constraint violated: `eu_political_capital` dropped from 43.0 to 26.0, a decrease of 17.0 points in one turn. While metrics are allowed to fall, the drop exceeds the hard cap of 25 points maximum movement (up or down) in a single turn per Constitutional Constraint #9. Although the narrative and notepad cite multiple negative charges and a scandal, the magnitude of the change violates the 25-point ceiling on absolute movement. A drop of this size must be spread across turns or justified by cascading event gates that still respect the cap.
...
## Metrics
```json
{"ai_capability": 58.0, "openweight_capability": 50.5, "ai_safety": 37.0, "resilience": 36.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 26.0, "public_sentiment": 34.0}
```
