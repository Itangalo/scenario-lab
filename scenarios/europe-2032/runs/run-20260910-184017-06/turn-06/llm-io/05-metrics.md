...
- characters 105-13818: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13861-14052: `{{metrics_json}}` from the run's live metric values
- characters 14145-15324: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15367-17920: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17976-18553: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}}, {"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 9}, "grounds": "event ai_investment_collapse cancelled build-out and blocked servicing"}]}
```
```
