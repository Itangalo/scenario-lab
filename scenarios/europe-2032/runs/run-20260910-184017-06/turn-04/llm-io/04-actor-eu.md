...
- characters 454-2265: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2308-4765: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5874-6337: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8497-10771: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_graduate_protest_wave (emergent event):** Coordinated graduate-led protests across several EU capitals link the hiring freeze to data-centre permits, physically blocking two compute-zone hearings and forcing a national party to adopt an anti-AI jobs platform.
...
| M4 | EU Assurance Adoption Facility | 5 | small | 2 | 5 | own jurisdiction | ai_safety up materially, resilience up slightly via certified deployments | 2 | running |
| M5 | EU Displaced Workers Transition Fund | 7 | small | 3 | 6 | own jurisdiction | resilience up materially via social absorption, eu_political_capital up slightly via public_sentiment | 2 | running |

They cost you 10 political capital this turn, which is the sum of the `cost_per_turn` column above. That column, like `id`, `started_turn` and `status`, is worked out for you and is not yours to write.
...
Pays wage insurance, rapid retraining and hiring incentives for entry-level white-collar roles hollowed out by AI, funded by a levy-linked transition line tied to automating employers.
This is Labour and social protection because graduate hiring has stopped in law, accountancy, software and customer operations while public sentiment sits at 34, and without visible absorption the sovereignty build loses its permit base; why now is the labour displacement event colliding with the cyber incident and welfare scandal that already strain capital.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Displaced Workers Transition Fund", "category": 7, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially via social absorption, eu_political_capital up slightly via public_sentiment"}, "grounds": "measurable graduate displacement must be answered within two-year absorb-shocks commitment"}]}
```

## Priority
EU Critical Systems Shield (M3), because a large automated model-generated attack landing while capability jumps to 58.0 outranks even the hiring freeze — cascading public-service failure is what turns anxiety into hostility.

## In practice
We run the Shield as incident command: ENISA with CERT-EU and ENTSO-E triages the ransomware/dependency blast, ring-fences Digital Europe sensor money to segmented sites that passed winter audits, and publishes daily restoration rates to separate defenders-behind narrative from recovery. DG DIGIT fast-tracks the Assurance Facility checklists to hospitals and energy control rooms hit in the sweep, using the three JRC nodes for post-incident certification.

We stand up the Transition Fund through DG EMPL and ECOFIN: ESF+ repurposed for wage insurance pilots in five hardest-hit labour markets, with EPSCO agreement that firms deploying frontier agents co-fund retraining, while DG CNECT ties gigafactory zone permits to local hiring and apprenticeship quotas to answer councils blocking power. We bank the public-sector adoption win — waiting lists falling — via DG REFORM procurement showcase to prove European decisions deliver benefit, not just restriction.
...
```
## New measure
**EU Compute Communities Dividend**
Links every gigafactory and AI data-centre zone permit to a binding local package of apprenticeship and first-job quotas for graduates, energy-bill relief and audited cyber-hardening for surrounding municipalities and hospitals.
This is Diffusion, adoption and public trust because graduate-led blockades of two hearings and council permit freezes over consumption and insecurity are now the binding constraint on sovereignty, and with sentiment at 24 and capital at 17 no compute build survives without visible local benefit; why now is the emergent protest wave fusing the hiring freeze to data-centre siting.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Compute Communities Dividend", "category": 9, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up materially, eu_political_capital up slightly, eu_ai_sovereignty up slightly via unblocked permits"}, "grounds": "unblock compute permits fused to graduate jobs protest"}]}
```

## Priority
EU Displaced Workers Transition Fund (M5), because graduate unemployment spiking in five markets and a national party adopting an anti-AI jobs platform outranks even permit unblocking — without a payout track started now the Dividend has nothing to offer protesters.

## In practice
We run M5 as emergency disbursement: DG EMPL repurposes ESF+ advances for wage-insurance pilots and paid traineeships in law, accountancy, software and customer-ops in the five hardest-hit labour markets, with EPSCO mandating co-funding from firms cutting entry-level hiring while deploying agents. DG CNECT makes the new Dividend the permit deal — no power connection without apprenticeship quotas, local hiring floors and municipal hardening funds — negotiated directly with blocking councils in Energy Council.

We hold the Shield line finished but sustained: ENISA and the grid coordinator keep daily restoration publishing and post-incident certification via the three interpretability nodes and Assurance Facility checklists in hospitals and energy dispatch, ring-fencing sensor money to sites that cut remote vendor links. DG REFORM keeps touring the two health ministries and education authority wins to show European-procured AI cutting waits, now paired explicitly with first-job placements to answer the jobs charts.
