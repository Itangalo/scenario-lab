...
- characters 454-2495: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2538-4365: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4745-5205: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7323-9423: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M5 | EU-US Volume Licence and Anti-Coercion Buffer | 8 | small | 3 | 5 | US plus middle-power coalition | eu_ai_sovereignty up slightly, eu_political_capital protected | 2 | running |
...
```
## New measure
None this turn, while we hold capital for the licensing negotiation to land and avoid starting a fight we cannot fund.

## Store changes
No changes.

## Priority
M5 EU-US Volume Licence and Anti-Coercion Buffer, because a member-state side-deal now directly undercuts the common volume-licence ask in Washington and must be folded back into it before licensing closes.

## In practice
We keep the continuity stack as finished doctrine: EuroHPC/AI Factory migration teams stay on the dozen cut-off hospital sites, funded from reprogrammed Digital Europe and EU4Health, with ENISA holding the hardened open-weight configuration and no new instrument opened. M3 segmentation orders remain enforced by TTE Council and national SOCs.

On M5 we move DG Trade to the front: we confront the defecting capital in COREPER with a choice between an EU-soil hosting clause for its hyperscaler deal and an Anti-Coercion Instrument examination, while offering it first call on pooled medical-workload quota if it realigns. In Washington we lower the ask to a written medical/public-sector volume assurance plus deferred licensing timetable, pursued jointly with Tokyo and The Hague on spares, to keep M1/M2 permitting tracks legally alive until finance returns.
