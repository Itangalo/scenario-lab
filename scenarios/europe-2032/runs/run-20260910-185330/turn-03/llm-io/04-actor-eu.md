...
- characters 455-2119: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2163-4269: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4701-5161: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7092-9188: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Essential Services Continuity Stack | 9 | small | 2 | 4 | own jurisdiction | resilience up, public_sentiment up; eu_ai_sovereignty up slightly | 2 | running |
...
Stands up EU-hosted fallback inference and support for hospitals, ministries and firms cut off from the leading US model, using EU-controlled open-weight systems and pooled compute.
This is category 9 Diffusion, adoption and public trust because the immediate harm is loss of function in public services, and only a trusted EU-run substitution restores it now; it also buys time for the category 4 gigafactory build while private capital is frozen.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Continuity Stack", "category": 9, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up; eu_ai_sovereignty up slightly"}, "grounds": "frontier access denial requires immediate substitution for essential users"}]}
```

## Priority
EU Essential Services Continuity Stack, because the US cut-off is an active loss of function in hospitals and ministries that outranks even the grid hardening work for this semester.

## In practice
We task DG CNECT and the AI Office with a continuity mandate under the AI Act service-continuity and procurement base, funded by reprogrammed Digital Europe and EU4Health envelopes: EuroHPC sites and the nascent AI Factories host hardened open-weight models for the affected hospitals and ministries, with ENISA vetting the deployment. Member states that built directly on the denied model get migration teams first; we ask Health and TTE Councils for an emergency adequacy waiver to pool health data on EU soil only.

We keep M3 running as the backbone of our two-year commitment — ENISA segmentation orders and SOC co-financing continue — and we refuse to let M1 and M2 die with the investment collapse: EIB and CINECEF permitting tracks stay open on public money and site preparation, while we tell universities and CNRS/Max Planck protesters their sovereign training demand is now the post-collapse justification for keeping public compute spend alive through the freeze.
...
```
## New measure
**EU-US Volume Licence and Anti-Coercion Buffer**
Secures continued allied chip and model access on volume licences while building a joint middle-power stockpile and retaliation posture if allies are rationed alongside adversaries.
This is category 8 International coordination and leverage because the export-control escalation is decided in Washington, not Brussels; with capital at 27 and private compute finance frozen, a leverage and assurance deal is the only move that keeps M1/M2 alive this semester.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU-US Volume Licence and Anti-Coercion Buffer", "category": 8, "size": "small", "finish_turn": 5, "applies_to": "US plus middle-power coalition", "targeted_effect": "eu_ai_sovereignty up slightly, eu_political_capital protected"}, "grounds": "export control escalation threatens remaining compute pipeline"}]}
```

## Priority
M4 EU Essential Services Continuity Stack, because hospitals and ministries cut off in March still lack full continuity and that active loss of function outranks even the export-control negotiation for this semester.

## In practice
We keep M4 as the operational centre: DG CNECT and EuroHPC/AI Factories expand pilot migrations from two hospital networks to all cut-off health users, funded by reprogrammed Digital Europe/EU4Health, with ENISA hardening the open-weight fallback and the Health Council pressed for an EU-soil-only data-pooling waiver. M3 hardening orders stay in force as finished doctrine, implemented by TTE Council and national SOCs without new cost.

On the new buffer, we task DG TRADE and DG GROW to seek a written allied volume-licence carve-out in Washington, offering export-control alignment and joint weight-security standards, while quietly preparing an Anti-Coercion Instrument file with Japan, Korea and the Netherlands on lithography and chemicals leverage. We tell the Council this is what keeps M1/M2 permitting tracks fundable: no parallel compute programme, just keeping the chip door open until private capital returns.
