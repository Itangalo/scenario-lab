...
- characters 455-1491: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1535-3335: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4252-4684: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6329-8479: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M3 | EU Grid and Port Cyber Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up materially, ai_safety flat | 3 | running |
| M4 | EU Incident Reporting and High-Risk Enforcement Sweep | 2 | small | 2 | 5 | own jurisdiction | ai_safety up modestly, public_sentiment stabilised, eu_political_capital protected | 2 | running |
...
## New measure
**EU Incident Reporting and High-Risk Enforcement Sweep**
Requires mandatory reporting of serious AI incidents to ENISA/EU AI Office and launches a joint enforcement sweep of Annex III social-insurance/policing deployments on conformity, human oversight and logging.
Category 2 Transparency and reporting, small because it uses existing AI Act and NIS2 bases rather than new capacity, and why now is the welfare/policing scandal plus the capability jump making voluntary testing unverifiable.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Incident Reporting and High-Risk Enforcement Sweep", "category": 2, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up modestly, public_sentiment stabilised, eu_political_capital protected"}, "grounds": "scandal demands enforcement response and jump demands reporting"}]}
```

## Priority
M3 EU Grid and Port Cyber Shield, because the state-level pre-positioning rehearsal outranks even the scandal and the new frontier jump for the next two turns.

## In practice
We keep ENISA surge and TSO/port segmentation audits under the Energy and Transport Councils as the main spend, funding exercises via Digital Europe repurposing, while DG CNECT holds gigafactory site selection warm without new permits acceleration to avoid a second fight.

On the scandal we choose the first reading: the system was high-risk under Annex III and obligations were breached — paper conformity, 40-second human rubber-stamps, unread logs. The AI Office with national market-surveillance authorities and the Fundamental Rights Agency opens infringement-track audits, suspends the offending deployment, and publishes logs methodology. This contains the frame to enforcement failure, not Act failure.

On the jump and the adoption win we task the AI Office to tie public-sector adoption successes — waiting-list cuts attributed to European procurement — to the new incident registry, so benefit stays visible while the frontier advance is monitored through mandatory reports rather than a new evaluation body this turn.
...
```
## New measure
**EU Evaluation Access and Anomaly Triage Cell**
Stands up a joint AI Office-JRC cell with mandated model access to triage the leaked eval anomaly and run independent agent-behaviour checks.
This is public technical capacity and research, category 5, and why now is the eval anomaly opening the capability/control gate while a member-state side-deal weakens our collective leverage — we cannot answer either without our own testing eyes.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Evaluation Access and Anomaly Triage Cell", "category": 5, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up modestly, eu_ai_sovereignty up slightly via bargaining leverage"}, "grounds": "anomaly demands independent triage and defection demands common evidence base"}]}
```

## Priority
M4 EU Incident Reporting and High-Risk Enforcement Sweep, because the welfare betrayal plus the anomaly leak is what will decide whether the public and the defecting capital believe any Union line at all in the next two turns.

## In practice
We keep ENISA on grids and ports under M3 through Energy and Transport Councils, funded by the repurposed Digital Europe line, and we hold M1/M2 site selection warm without forcing permits while sentiment sits at 36 — DG CNECT and DG ENER broker grid-connection offers to regions to stop further side-deals.

On enforcement we push M4 hardest: AI Office with national watchdogs and FRA publishes the logging methodology, completes suspension and infringement-track audits of the benefits system, and routes the leaked benchmark anomaly into the new incident registry as its first frontier case. The new Triage Cell is stood up inside the AI Office with JRC secondees, using Digital Europe and Horizon top-up, tasked to demand evaluation access from providers serving the EU and to publish a triage note that either confirms or downgrades the anomaly.
