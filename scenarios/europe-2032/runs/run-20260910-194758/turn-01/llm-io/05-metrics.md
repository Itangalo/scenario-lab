...
- characters 13233-16182: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 16238-16821: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_openweight_fraud_wave` -- first noted turn 1, listed in 1 turn(s) so far: Major European press investigation traces Kimi K3-derived phishing and fraud wave to small criminal groups, triggering first EU-wide calls for open-weight liability rules.
- `emergent_journal_moratorium_genome_methods` -- first noted turn 1, listed in 1 turn(s) so far: Major scientific journals impose a temporary moratorium on publishing detailed methods for AI-designed genome work after the phage results, triggering a fight over preprints and open science.
...
## Metrics
```json
{"ai_capability": 53.5, "openweight_capability": 46.0, "ai_safety": 32.0, "resilience": 38.0, "eu_ai_sovereignty": 22.0, "eu_political_capital": 43.0, "public_sentiment": 40.0}
```
