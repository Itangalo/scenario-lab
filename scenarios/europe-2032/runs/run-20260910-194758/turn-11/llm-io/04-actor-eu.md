...
- characters 454-2218: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2262-4321: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5816-6306: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9210-11239: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_infrastructure_sabotage (emergent event):** Coordinated physical attacks on data-centre construction sites and grid interconnects in two member states, forcing a temporary halt to gigafactory build-out and hardening works.
...
- `two_year_commitment` (commitment): Rebuild trust by delivering visible public benefits and keeping essentials running as open AI spreads beyond recall
...
| M3 | EU Grid and Port Cyber Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up materially, ai_safety flat | 3 | finished |
| M4 | EU Incident Reporting and High-Risk Enforcement Sweep | 2 | small | 2 | 5 | own jurisdiction | ai_safety up modestly, public_sentiment stabilised, eu_political_capital protected | 2 | finished |
| M5 | EU Evaluation Access and Anomaly Triage Cell | 5 | small | 3 | 6 | own jurisdiction | ai_safety up modestly, eu_ai_sovereignty up slightly via bargaining leverage | 2 | finished |
| M6 | EU Observer Bridge to the US-China Safety Accord | 8 | small | 4 | 7 | US, China, coalition | ai_safety up, eu_ai_sovereignty up slightly via leverage, eu_political_capital protected | 2 | finished |
| M7 | EU Visible Benefit Adoption Drive | 9 | small | 7 | 8 | own jurisdiction | public_sentiment up materially, eu_political_capital up, resilience up slightly | 2 | finished |
| M8 | EU Displacement Transition Shield | 7 | small | 8 | 9 | own jurisdiction | resilience up, public_sentiment up, eu_political_capital protected | 2 | finished |
| M9 | EU Essentials Continuity Sprint | 6 | small | 9 | 10 | own jurisdiction | resilience up materially, public_sentiment up, eu_political_capital protected | 2 | finished |
| M10 | EU Trusted Cures Deployment | 9 | small | 10 | 11 | own jurisdiction | public_sentiment up materially, eu_political_capital up, resilience up slightly | 2 | finished |
...
## New measure
**EU Trusted Cures Deployment**
It procures the new tailored therapies and diagnostic models for delivery through European-hosted and audited hospital channels with trusted-source labels, so the medical breakthrough lands as a visible public benefit under EU control.
This and now because the breakthrough can lift sentiment sharply and rebuild capital only if accessed on our terms, while export-control tightening and the cyber-defence opening make EU-hosted deployment both urgent and newly defensible; diffusion and adoption, category 9, is the lever that restores room to act.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trusted Cures Deployment", "category": 9, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up materially, eu_political_capital up, resilience up slightly"}, "grounds": "to turn medical and defensive breakthroughs into EU-controlled benefits"}]}
```

## Priority
EU Trusted Cures Deployment, because with no measures in flight and capital at 7 only a delivered health benefit paired with the cyber-defence patching window can lift sentiment from 24 and fund anything else.

## In practice
We run this through DG SANTE and DG CNECT with EMA and ENISA as operators: joint procurement of the breakthrough therapies and model access under EU data-residency and audit terms, co-financed from Digital Europe and EU4Health, decided in EPSCO and JHA. Hospitals already touched by the finished Continuity Sprint are first sites, with automated patching kits pushed alongside clinical rollout.

We condition US volume-licence access on EU hosting and evaluation rights, using the finished Gigafactories legal groundwork and Anti-Coercion leverage without opening a new compute build, and we offer member states staffing relief if hardened clinical and cyber posts are filled from Shield retrainees. No parallel sovereignty programme — thin gigafactory shells stay as the sole compute line.
...
```
## New measure
**EU Continuity and Repair Shield**
It keeps hospitals, grids and ministries running on EU-hosted and audited fallbacks after the US cutoff, while repairing sabotaged gigafactory and interconnect sites under guarded rebuild with ENISA hardening.
This and now because access denial plus physical sabotage directly breaks the two-year commitment to keep essentials running, and with bio uplift and labour losses rising only a delivered continuity fix can stop capital collapsing; preparedness and resilience, category 6, is the lever that holds.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Continuity and Repair Shield", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, eu_political_capital protected, public_sentiment up slightly"}, "grounds": "to hold essentials through cutoff and sabotage"}]}
```

## Priority
EU Continuity and Repair Shield, because it alone directly serves the two-year commitment to keep essentials running when cutoff and sabotage have halted it.

## In practice
We run this through JHA and TTE Councils with ENISA and Europol as operators: guarded rebuild orders for the two hit member states funded from CEF and Internal Security Fund, mandatory backup drills and fallback switch to EU-hosted models for queued hospitals and ministries from the finished Trusted Cures and Continuity lines.

We pair DG SANTE joint procurement volumes with DG HOME site protection, offering construction regions grid-fee relief and local hiring from Shield retrainees to unblock permits and counter protest, while conditioning any restored US access on EU hosting and audit rights. No new compute build — thin shells are secured, not expanded.
