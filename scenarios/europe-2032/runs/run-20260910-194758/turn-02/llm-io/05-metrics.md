...
- characters 13277-14306: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14349-16238: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16294-17488: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_openweight_fraud_wave` -- first noted turn 1, listed in 2 turn(s) so far: Major European press investigation traces Kimi K3-derived phishing and fraud wave to small criminal groups, triggering first EU-wide calls for open-weight liability rules.
- `emergent_journal_moratorium_genome_methods` -- first noted turn 1, listed in 2 turn(s) so far: Major scientific journals impose a temporary moratorium on publishing detailed methods for AI-designed genome work after the phage results, triggering a fight over preprints and open science.
...
## Metrics
```json
{"ai_capability": 58.5, "openweight_capability": 49.5, "ai_safety": 24.0, "resilience": 37.0, "eu_ai_sovereignty": 21.0, "eu_political_capital": 28.0, "public_sentiment": 36.0}
```
