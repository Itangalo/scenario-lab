...
- characters 13148-15971: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 16015-18004: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18060-19095: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_shadow_diagnostic_breach` -- first noted turn 13, listed in 1 turn(s) so far: Hospital staff reliance on the loose frontier model triggers a cross-border clinical data leak, with patient records used as prompts harvested and resold.
- `emergent_siting_blockade_escalation` -- first noted turn 13, listed in 1 turn(s) so far: Coordinated local blockades halt remaining grid hookups and guarded repair convoys for hospital continuity sites, leaving several kitted sites unpowered for days.
...
## Metrics
```json
{"ai_capability": 66.6, "openweight_capability": 65.2, "ai_safety": 2.5, "resilience": 36.0, "eu_ai_sovereignty": 12.0, "eu_political_capital": 4.0, "public_sentiment": 14.0}
```
...
## Notepad
PORTFOLIO CHARGE t13: EU Open Frontier Absorption Shield −2, priority −1 not charged (capital below 20, no effect) = −2
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 2, sentiment 14 -> +2 capped by events net
SOVEREIGNTY: 12 last turn, no capacity event, no cat4 in flight +0, capability rose 0.7 −0 = 12
## Store changes
```json
{"store": []}
```
