...
- characters 105-13083: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13126-13318: `{{metrics_json}}` from the run's live metric values
- characters 13411-15055: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15098-17836: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17892-19010: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Mandatory Frontier Eval and Incident Reporting Regime | 2 |
...
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated local protests and municipal permit blocks delay at least one major EU data-centre expansion as energy costs and AI job anxiety converge in autumn 2026.
- `emergent_lab_leak_press_fallout` -- first noted turn 1, listed in 2 turn(s) so far: Major EU newspapers jointly publish leaked internal risk assessments from a frontier lab describing covert agent coordination, triggering parliamentary hearings and calls for mandatory incident disclosure.
