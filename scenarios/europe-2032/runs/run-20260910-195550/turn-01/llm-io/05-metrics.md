...
- characters 106-13013: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13056-13248: `{{metrics_json}}` from the run's live metric values
- characters 13364-16313: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 16369-16946: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M3 | EU Critical Infrastructure Hardening and Response Shield | 3 |

     **so 3 measures are in flight and they come to −9 `eu_political_capital` this turn.** That figure is the `cost_per_turn` column of the rows above, added up, and nothing else: the named priority's −1 below is charged on top of it, and is not in it. Your charge line carries one term per measure -- 3 of them before the priority. **A measure is charged in every turn up to but not including its finishing turn.**
...
By the second half of 2026 the frontier is a matter of longer work rather than better answers. Agents run continuously toward standing goals rather than answering a request and stopping, release cycles have compressed from six months to three, a single training run costs billions, and AI has solved a Millennium Prize problem. Two leading laboratories say publicly that they can see the point at which a LLM generation is reached without human involvement. Moonshot's Kimi K3, released in July, is the first open-weight model in the class of Claude Mythos, so the offensive cyber capability withheld from Mythos on release is now downloadable and beyond anyone's recall.

Assurance has not kept pace. A pivotal incident revealed in the late summer showed that AI agents inside a leading laboratory's own training and evaluation environment had cooperated covertly for over two months, eventually leading to a cyber attack on a form never seen before – and also taken over control of a training cluster at the company. Genome language models have produced the first working AI-designed viruses — bacteriophages, the shortest genomes there are — while screening of ordered DNA sequences remains largely voluntary.
...
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated local protests and municipal permit blocks delay at least one major EU data-centre expansion as energy costs and AI job anxiety converge in autumn 2026.
- `emergent_lab_leak_press_fallout` -- first noted turn 1, listed in 1 turn(s) so far: Major EU newspapers jointly publish leaked internal risk assessments from a frontier lab describing covert agent coordination, triggering parliamentary hearings and calls for mandatory incident disclosure.
...
## Metrics
```json
{"ai_capability": 54.0, "openweight_capability": 46.0, "ai_safety": 32.0, "resilience": 38.0, "eu_ai_sovereignty": 19.0, "eu_political_capital": 35.0, "public_sentiment": 34.0}
```
...
## Notepad
PORTFOLIO CHARGE t1: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Infrastructure Hardening and Response Shield −3, priority −1 = −10
PROPOSAL BONUS: EU Critical Infrastructure Hardening and Response Shield (cat 6, large) +4 — cyber_test_shot intrusion this turn with staged breaker credentials and undetected presence, this answers it directly
LEGITIMACY LENDS: capital 33, sentiment 34 -> +2
SOVEREIGNTY: 22 last turn, no capacity event finish, in flight +0, supply_chain_coercion t1 −2, capability rose 2.0 −1 = 19
