...
- characters 454-1637: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1681-3828: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5701-6168: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8941-10764: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_infrastructure_backlash (emergent event):** Coordinated physical blockades of data-centre construction and grid-upgrade sites spread across several member states, delaying build-out for months.
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Service Continuity Stack | 9 | small | 10 | 12 | own jurisdiction | resilience up moderately, public_sentiment up, eu_ai_sovereignty up slightly via EU-hosted substitution | 2 | running |
...
## New measure
**EU Service Continuity Stack**
It replaces the cut-off foreign frontier model in hospitals, ministries and firms with EU-hosted and open-weight systems procured and supported centrally, turning the access denial into visible restoration. This and now because adoption success shows public-sector AI can win trust, while the cutoff makes sovereignty a service-continuity issue under the keep-services-running commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Service Continuity Stack", "category": 9, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up, eu_ai_sovereignty up slightly via EU-hosted substitution"}, "grounds": "frontier access cut off for hospitals/ministries amid proven adoption benefit"}]}
```

## Priority
M7 EU Local Continuity and Livelihood Bridge, because with capital at 14 and sentiment at 8 only visible town-by-town restoration holds cohesion while the new Stack ramps up.

## In practice
We run the Bridge as the front line: DG ECHO/ENISA single intake keeps paying overtime, short-time support and segmentation upkeep from cohesion and civil-protection budgets, prioritising eastern and southern town halls and clinics whose portals crashed, with EPSCO sign-off for extended windows and lighter audits.

In parallel we launch the Stack without a new law: DIGIT and HaDEA bulk-procure EU-hosted inference and hardened open-weight packs for health and administration, deployed on existing national clouds and Gigafactory pilot halls, with procurement preference for EU-controlled servicing. DG TRADE maintains screening-without-retaliation while we quietly negotiate fallback servicing, so substitution reads as competence, not escalation.
...
```
## New measure
None this turn, while we let the Continuity Stack land and rebuild capital before starting anything new.

## Store changes
No changes.

## Priority
M8 EU Service Continuity Stack, because with capital at 6 and sentiment at 14 only visible restoration of hospital and administrative tools keeps cohesion while blockades stall new build.

## In practice
We hold the line on M8: DIGIT/HaDEA continue bulk EU-hosted inference and hardened open-weight packs onto national clouds, prioritising Rotterdam, Lyon, Milan and Warsaw workflows hit by the February refusals, with lighter audits and single intake for the livelihood bridge now folded into cohesion funds.

We do not escalate on blockades: we pause new siting decisions, ask member states to police sites through existing civil-protection powers, and direct the stalled Gigafactory pilot halls to servicing and maintenance crews to unblock migrations delayed by residency and procurement disputes. DG TRADE maintains screening-without-retaliation.
