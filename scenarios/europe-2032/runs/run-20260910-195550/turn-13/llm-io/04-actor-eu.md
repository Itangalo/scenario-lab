...
- characters 454-1445: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1489-3797: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7057-7524: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10522-12568: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential services running and communities supported through accelerating AI disruption
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Embodied-Work Transition Shield | 7 | small | 12 | 14 | own jurisdiction | public_sentiment up, resilience up slightly via labour absorption, eu_political_capital up slightly via visible protection | 2 | running |
...
| M3 | EU Critical Infrastructure Hardening and Response Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up strongly, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Mandatory Frontier Eval and Incident Reporting Regime | 1 | small | 2 | 5 | own jurisdiction | ai_safety up, resilience up slightly via early warning | 2 | finished |
| M5 | EU Loss-of-Control Containment and Agent Accountability Protocol | 6 | small | 3 | 6 | own jurisdiction | resilience up, ai_safety up slightly via containment | 2 | finished |
| M6 | EU Degraded-Mode Continuity and Bio-Signal Surge | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly via detection and early warning | 2 | finished |
| M7 | EU Local Continuity and Livelihood Bridge | 7 | small | 9 | 11 | own jurisdiction | resilience up moderately, public_sentiment up, eu_political_capital up slightly via visible restoration | 2 | finished |
| M8 | EU Service Continuity Stack | 9 | small | 10 | 12 | own jurisdiction | resilience up moderately, public_sentiment up, eu_ai_sovereignty up slightly via EU-hosted substitution | 2 | finished |
...
It keeps warehouse, sorting and logistics workers hit by commercial robots on payroll pathways through wage insurance, rapid retraining for repair/maintenance roles, and EU-preference procurement for maintenance depots using repurposed pilot halls. Why this and why now: embodied deployment just split the labour market at the manual-judgement line and open-weight frontier models put automation capability beyond recall, while blockades and sentiment at 18 mean we cannot build new capacity — we must hold people and services where they are.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Embodied-Work Transition Shield", "category": 7, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, resilience up slightly via labour absorption, eu_political_capital up slightly via visible protection"}, "grounds": "embodied deployment dividing labour market while capital at 5"}]}
```

## Priority
EU Embodied-Work Transition Shield, because with no measures in flight and capital at 5 only visible protection against robot-driven displacement can lift sentiment enough to unblock continuity work.

## In practice
We run this through DG EMPL and the cohesion fund already carrying the livelihood bridge, with EPSCO mandate: wage-insurance top-ups and short retraining vouchers for picking/sorting/palletising workers, co-funded by automating logistics employers, and maintenance-technician tracks staffed from the repurposed factory pilot-hall depots.

We do not force new data-centre or substation siting this turn: DIGIT/HaDEA keep EU-hosted inference and hardened open-weight packs running for Rotterdam, Lyon, Milan and Warsaw clinics on existing clouds, member states police blocked gates under existing powers, and procurement favours EU-maintained robot fleets for resupply-type logistics without touching lethality rules. Lighter audits stay to keep transfers flowing east and south.
...
```
## New measure
None this turn, while we absorb a major automated cyber incident with political capital at 7 and let the Transition Shield finish landing.

## Store changes
No changes.

## Priority
EU Embodied-Work Transition Shield, because with sentiment at 16, blockades paused, and a cyber sweep plus a member-state side-deal pulling cohesion apart, only visible holding of logistics workers and clinic continuity can prevent paralysis.

## In practice
We run the Shield through DG EMPL, DG DIGIT and ENISA under existing mandates: wage top-ups and repair vouchers continue, depot technicians in the repurposed halls are seconded to help municipal IT restore public-service systems hit by the automated attack, using the hardened EU-hosted packs and degraded-mode playbooks already on the books.

We push no new siting, no new large build, and no new levy this turn. Council JHA activates cross-border mutual aid for the ransomware/dependency incident, CERT-EU triages, and we use the middle-power coordination opening to jointly bargain patches and compute access, while opening infringement dialogue — not sanctions — with the defecting member state to hold the line without spending capital we do not have.
