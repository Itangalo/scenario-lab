...
- characters 105-13509: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13552-13743: `{{metrics_json}}` from the run's live metric values
- characters 13836-15174: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15217-17543: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17599-18062: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner takes office turn 6; supply-chain coercion posture"}]}
```
```
