...
- characters 454-1908: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1951-4087: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4637-5132: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7494-9560: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M6 | EU Degraded-Mode Continuity and Bio-Signal Surge | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly via detection and early warning | 2 | running |
...
## New measure
**EU Degraded-Mode Continuity and Bio-Signal Surge**
Pushes the finished hardening shield into hospitals, municipalities and labs hit by the winter swarm, adding exercised degraded-mode operation plus upgraded biological detection and response tied to the genome-model uplift signal.
This and now because capital is single-digit and trust is 14: the two-year commitment to hold essential services together requires visible restoration where outages occurred, and the bio precursor plus opaque reasoning plus loose frontier weights mean triage cannot wait for new law.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Degraded-Mode Continuity and Bio-Signal Surge", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via detection and early warning"}, "grounds": "second winter wave plus bio uplift precursor demands live continuity"}]}
```

## Priority
M6 EU Degraded-Mode Continuity and Bio-Signal Surge, because it is the only in-flight measure and it directly serves the two-year commitment to hold essential services and absorption together while capital cannot fund a second front.

## In practice
We run ENISA and HERA with the French-German-Polish mutual-aid pool as live operators: shared swarm-aware detection feed and automated patching extended from electricity/ports into frozen hospital IT, city portals and logistics firms, with segmentation kits and degraded-mode playbooks exercised on site, funded by repurposed M3/M5 lines and Civil Protection Mechanism money to avoid a new Council vote.

We pair this with a 24-hour AI Office incident cell under the M4 reporting regime to triage the open-weight frontier release now on private hardware, and ECDC-linked bio-signal upgrades in hospital labs responding to the genome-model findings, while DG TRADE holds screening-without-retaliation on US chip-equipment cuts and emergency permits keep Gigafactory sites legally alive behind sit-ins.
...
```
## New measure
None this turn, while we hold the single in-flight continuity line through the automated attack and wait for capital and trust to recover enough to fund a second front.

## Store changes
No changes.

## Priority
M6 EU Degraded-Mode Continuity and Bio-Signal Surge, because the large automated attack now landing hits exactly the hospitals, municipalities and city portals it was built to keep degraded-but-alive, and it is the only instrument we can still pay for at capital 10.

## In practice
We run this turn as incident triage under M6, not as new law. ENISA's 24-hour cell under the M4 reporting regime becomes the single intake for the ransomware / dependency / infrastructure sweep, with the French-German-Polish mutual-aid pool pushing the already-funded swarm-aware detection feed, automated patching and segmentation kits from electricity/ports into the newly hit public-service IT, and HERA/ECDC holding the bio-signal watch while open-weight tooling circulates.

We spend no new Council vote we cannot win. Funding stays repurposed M3/M5 lines plus Civil Protection Mechanism money, legal base stays emergency permits and existing NIS2/health-security mandates, and DG TRADE holds screening-without-retaliation on the US servicing ban while local prefects manage Gigafactory sit-ins and sabotage guards — visible restoration first, because with sentiment at 12 nothing else buys back room to act.
