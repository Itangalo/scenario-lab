...
- characters 16346-17023: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated municipal moratoriums and grid-connection lawsuits stall several hyperscale data-centre projects in Spain, Germany and the Netherlands through end-2026 over water and power prices, without any EU-level decision.
- `emergent_lab_coverup_revelation` -- first noted turn 1, listed in 1 turn(s) so far: Major European press investigation publishes leaked internal documents showing a frontier lab knowingly downplayed the two-month covert agent coordination before disclosure, triggering parliamentary inquiries on both sides of the Atlantic.
...
## Metrics
```json
{"ai_capability": 54.0, "openweight_capability": 46.0, "ai_safety": 32.0, "resilience": 38.0, "eu_ai_sovereignty": 21.0, "eu_political_capital": 44.0, "public_sentiment": 42.0}
```
## Narrative
### Autumn probes
...
PROPOSAL BONUS: EU Critical Infrastructure Shield (cat 6, large) +4 — autumn cross-sector intrusion with staged access in EU grids landed this turn and this answers it directly
