...
- characters 455-1698: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1742-3777: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4594-5065: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7921-9733: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Survive rationed intelligence without fragmentation while rebuilding minimal leverage for access
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Degraded Operations Hardening | 6 | small | 12 | 14 | own jurisdiction | resilience up moderately | 2 | running |
...
| M3 | EU Critical Infrastructure Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Bio Detection and Medical Countermeasure Surge | 6 | large | 2 | 4 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | finished |
| M5 | EU Independent Incident Audit Authority | 1 | small | 3 | 5 | frontier developers directly | ai_safety up moderately | 2 | finished |
| M6 | EU AI Displacement Buffer | 7 | small | 4 | 8 | own jurisdiction | public_sentiment up, resilience up slightly | 2 | finished |
| M7 | EU Biosynthesis Screening and Model Uplift Containment | 3 | small | 5 | 7 | own jurisdiction | resilience up, ai_safety up slightly | 2 | finished |
| M8 | EU Joint Cyber Telemetry Accession | 8 | small | 6 | 7 | coalition | resilience up moderately | 2 | finished |
| M9 | EU Third-Party Accession to US-China Risk Reduction Accord | 8 | small | 8 | 10 | coalition | ai_safety up moderately, eu_ai_sovereignty up slightly via retained access | 2 | finished |
...
## New measure
**EU Degraded Operations Hardening**
It keeps essential municipal, power and hospital services running on older licensed models plus the new steering technique while open-weight automated intrusion tooling spreads. This and now because the autumn assault erased the safety margin and attribution is open, and with political capital at 8 only a small resilience measure can hold fragmentation without new funds.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Degraded Operations Hardening", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "autumn automated assault with margin gone amid open-weight release"}]}
```

## Priority
No priority measure this turn, because with capital at 8 any push risks rupture and the new hardening must proceed quietly alongside sustained shields.

## In practice
We execute through ENISA and CERT-EU under the finished Critical Infrastructure Shield and Joint Telemetry Accession: isolate and re-image locked municipal systems, enforce segmented backups, block the poisoned update vector, and push the shared steering technique to hospitals, customs and grid operators for predictable degraded operation. The liaison cell via Tokyo and Seoul trades screening records and telemetry for licence continuity without conceding emergency powers.

We hold cohesion by refusing again to power the empty shells in Spain and the Netherlands, sustaining the Displacement Buffer via DG EMPL messaging on junior productivity gains and rehiring, and operationalising the middle-powers pact via DG TRADE joint compute bargaining and aligned export paperwork to secure rather than ration access. No new fund, no retaliation over quotas.
...
```
## New measure
**EU Incident Triage and Continuity Cells**
It surges joint ENISA-CERT-EU teams to re-image and isolate hit municipal, health and grid systems from the automated attack while keeping degraded manual fallbacks running. This and why now because the major automated incident lands on already-degraded services with open-weight tooling everywhere, and only a small preparedness measure fits capital at 5 without fragmenting.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Incident Triage and Continuity Cells", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "major automated attack on degraded public services"}]}
```

## Priority
Priority is M10 EU Degraded Operations Hardening, because holding segmented backups and steering on older models is what keeps the new triage cells from becoming permanent outage management under quota.

## In practice
We execute through ENISA and CERT-EU under the Civil Protection Mechanism, deploying pre-cleared triage teams to worst-hit municipalities, hospitals and grid operators: cut the compromised dependency, restore from segmented backups, and enforce the shared steering guardrails for cut-off operation. Funding is reallocated technical assistance, no new fund vote, to avoid a Council fight we cannot win at capital 5.

We hold the two-year commitment to survive rationed intelligence without fragmentation by refusing emergency powers for empty shells, sustaining joint compute bargaining with middle powers via DG TRADE to keep licences at quota despite Taiwan insurance pressure, and messaging junior productivity gains to blunt hostility at sentiment 18. Attribution is left to the liaison via Tokyo/Seoul telemetry exchange to protect licence continuity.
