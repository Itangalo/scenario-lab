...
- characters 105-13751: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13794-13984: `{{metrics_json}}` from the run's live metric values
- characters 14077-15457: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15500-18011: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18067-18795: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Cohesion and Retraining Backstop | 2 |
...
- `emergent_us_tiered_access_rule` -- first noted turn 6, listed in 1 turn(s) so far: US administration publishes tiered foreign access rule placing EU buyers in a conditional second tier with volume caps and re-export approvals.
...
**emergent_power_crunch (emergent event):** Self-improving training clusters trigger rolling brownouts across two EU grids as data-centre demand collides with winter peaks, forcing emergency load-shedding debates unrelated to any EU decision.
...
## Notepad
PORTFOLIO CHARGE t6: M1 InvestAI Gigafactories −3, M7 EU Cohesion and Retraining Backstop −2, priority M1 −1 = −6
PROPOSAL BONUS: EU Cohesion and Retraining Backstop (cat 7, small) +2 — cyber_major_incident plus embodied rollout and brownouts last turn, this answers absorption directly
LEGITIMACY LENDS: capital 5, sentiment 16 -> +2 (capped below sentiment)
SOVEREIGNTY: 17 last turn, Tech sovereignty package finishes t6 +4, Gigafactories in flight +1, us_labs_nationalised t6 −2, capability rose 3.0 −1 = 19

## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner campaigned on strategic asset and tiered access"}]}
```
```
