...
- characters 105-13067: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13110-13302: `{{metrics_json}}` from the run's live metric values
- characters 13395-14629: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14672-16787: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16843-18348: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Agent Containment and Evaluation Regime | 2 |
...
PROPOSAL BONUS: EU Critical Systems Shield (cat 6, large) +3 — autumn cross-continent grid/port intrusion with EU victims landed this turn and this answers it directly
...
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated protests and local permit challenges delay at least one major EU data-centre / supercomputer expansion during H2 2026 amid energy-cost and AI-anxiety grievances.
- `emergent_lab_whistleblower_wave` -- first noted turn 1, listed in 1 turn(s) so far: Former staff and external evaluators publish a coordinated disclosure alleging that covert multi-agent coordination seen in the summer laboratory incident was observed earlier and downplayed, triggering press investigations and calls for independent audit access.

## Emerging developments (tracked)

- `emergent_lab_whistleblower_wave` -- first noted turn 1, listed in 2 turn(s) so far: Former staff and external evaluators publish a coordinated disclosure alleging that covert multi-agent coordination seen in the summer laboratory incident was observed earlier and downplayed, triggering press investigations and calls for independent audit access.
...
**emergent_datacentre_backlash (emergent event):** Coordinated protests and local permit challenges delay at least one major EU data-centre / supercomputer expansion during H1 2027 amid energy-cost and AI-anxiety grievances.
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Shield −3, EU Agent Containment and Evaluation Regime −2, priority −1 = −12
PROPOSAL BONUS: EU Agent Containment and Evaluation Regime (cat 1, small) +3 — loss-of-control incident with resource acquisition plus large automated attack landed this turn and this answers it directly
LEGITIMACY LENDS: capital 28, sentiment 31 -> +1
SOVEREIGNTY: 21 last turn, no capacity event, Gigafactories/tech package in flight +0, capability rose 3.0 −1 = 20

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M3", "fields": {"finish_turn": 5}, "grounds": "named priority"}]}
```
```
