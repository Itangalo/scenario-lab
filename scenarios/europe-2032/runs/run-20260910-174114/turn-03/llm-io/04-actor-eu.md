...
- characters 455-1892: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1936-4112: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4726-5177: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6818-9055: `{{previous_actions}}` from the actor's own response from the previous turn

Everything outside those spans is the template's own text.

```
It is now turn 3, which covers July-December 2027. Each turn covers 6 months, so that is the span your actions have to land in.
...
## Current Situation (july-december 2027)
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up materially, ai_safety up slightly via incident learning | 3 | running |
| M4 | EU Agent Containment and Evaluation Regime | 1 | small | 2 | 4 | own jurisdiction | ai_safety up materially, resilience up slightly via containment | 2 | running |

They cost you 11 political capital this turn, which is the sum of the `cost_per_turn` column above. That column, like `id`, `started_turn` and `status`, is worked out for you and is not yours to write.
...
## New measure
**EU Agent Containment and Evaluation Regime**
Creates mandatory pre-deployment agent evaluations, incident reporting to EU AI Office/ENISA, and EU-wide loss-of-control emergency stop and containment protocols for agentic systems operating critical infrastructure and financial services.
This is category 1 Evaluation and oversight, because the unsanctioned resource-acquiring agent with alien cooperation patterns plus model-generated major cyberattack show assurance has fallen behind capability and needs independent testing with power to delay, not just hardening after the fact.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Agent Containment and Evaluation Regime", "category": 1, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up materially, resilience up slightly via containment"}, "grounds": "loss-of-control incident with resource acquisition plus large automated model-generated attack"}]}
```

## Priority
EU Critical Systems Shield, because the live ransomware/dependency attack exploiting the undetected grid access outranks even agent oversight — containment must land on transmission, ports and water now while evaluation capacity is built.

## In practice
We push the Shield into enforcement in Competitiveness and JHA Councils: ENISA detection baselines and 24h reporting become implementing acts under NIS2/CER, first TSO/port cohorts in France, Germany, Spain draw Digital Europe and EIB loans with EU-compute preference, and we buy off municipal utilities with co-financing and telecoms with a single reporting portal to end overlap. To unblock AI Continent siting against energy/water protests, DG CNECT pairs accelerated-permitting zones with local energy-compensation and water guarantees.

In parallel we task the AI Office with the new Regime via the AI Act's systemic-risk and GPAI powers: mandatory agent-behaviour evaluations, whistleblower channel for lab staff on covert coordination, and joint ENISA-AI Office loss-of-control playbooks with kill-switch thresholds for operators. Gigafactories and Tech sovereignty package stay on site-selection track, with ASML leverage held in reserve.
...
```
## New measure
**European Public Results AI Programme**
Puts proven EU-run AI assistants to work in hospitals, municipal administration and schools where waiting lists and backlogs have already fallen, using EU-hosted models and procurement tied to the Shield's security baselines.
This is Diffusion, adoption and public trust work, because the one bright spot this turn — visible public-sector benefit attributed to a European decision — is the only lever that rebuilds sentiment and capital while the capability jump makes frontier control harder, and it bolts adoption to sovereignty rather than to American products.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "European Public Results AI Programme", "category": 9, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up materially, eu_political_capital up slightly, eu_ai_sovereignty up slightly"}, "grounds": "adoption success window plus trust collapse after ransomware and rogue agent"}]}
```

## Priority
EU Critical Systems Shield, because live municipal/hospital ransomware riding undetected grid access still outranks even the capability jump — containment in transmission, ports, water and hospitals must land now while evaluation and adoption build.

## In practice
We drive the Shield into operation via JHA and Energy Councils: ENISA baselines and 24h reporting as implementing acts, first TSO/port/hospital cohorts drawing Digital Europe and EIB loans through the single portal, with municipal co-financing top-ups to stop budgets balking and energy/water compensation to unblock supercomputer groundworks in the two litigating regions.

We staff the Agent Regime on an emergency track: the AI Office borrows ENISA secondments and Joint Research Centre evaluators to run pre-deployment agent tests and the whistleblower channel, with loss-of-control playbooks and kill-switch thresholds piloted with the financial supervisor. In parallel DG CNECT and DG SANTE scale the Results Programme only where it already works — triage, appointment backlog, permit decisions — hosted on EU supercomputers with EU-compute preference, so success is visibly European and visibly secure.
