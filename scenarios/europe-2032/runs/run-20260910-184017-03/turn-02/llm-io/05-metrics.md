...
- characters 105-13074: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13117-13309: `{{metrics_json}}` from the run's live metric values
- characters 13402-14557: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14600-16520: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16576-17681: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated local protests and municipal permit challenges stall several planned EU data-centre expansions in autumn 2026 over power prices and water use, without yet stopping national build-out.
...
## Metrics
```json
{"ai_capability": 58.0, "openweight_capability": 50.5, "ai_safety": 30.0, "resilience": 37.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 33.0, "public_sentiment": 35.0}
```
