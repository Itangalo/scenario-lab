...
- characters 454-1609: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1652-3572: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4146-4615: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6101-8514: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M3 | EU Critical Shield and Bio-Screening Directive | 6 | large | 1 | 3 | own jurisdiction | resilience up strongly, ai_safety up modestly via misuse reduction | 3 | running |
