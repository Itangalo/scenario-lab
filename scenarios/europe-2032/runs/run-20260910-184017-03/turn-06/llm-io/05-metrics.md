...
- characters 105-13720: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13763-13955: `{{metrics_json}}` from the run's live metric values
- characters 14048-15570: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15613-17806: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17862-19053: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | Opaque Reasoning Assurance Programme | 2 |
| M7 | EU Essential Continuity and Biosecurity Hardening | 2 |
| M8 | EU Accord Seat and Sovereign Access Terms | 2 |

     **so 4 measures are in flight and they come to −9 `eu_political_capital` this turn.** That figure is the `cost_per_turn` column of the rows above, added up, and nothing else: the named priority's −1 below is charged on top of it, and is not in it. Your charge line carries one term per measure -- 4 of them before the priority. **A measure is charged in every turn up to but not including its finishing turn.**
...
- `emergent_humanoid_safety_ban` -- first noted turn 6, listed in 1 turn(s) so far: A commercial humanoid unit in a European logistics hub injures workers after a control-stack update, triggering national safety bans and liability litigation against the US vendor.
- `emergent_humanoid_safety_scare` -- first noted turn 6, listed in 1 turn(s) so far: A deployed Chinese-built humanoid fleet in EU logistics suffers a coordinated control-stack failure causing injuries and a temporary warehouse shutdown, fuelling calls for EU robotics certification.
...
## Notepad
PORTFOLIO CHARGE t6: M1 InvestAI Gigafactories −3, M6 Opaque Reasoning Assurance Programme −2, M7 EU Essential Continuity and Biosecurity Hardening −2, M8 EU Accord Seat and Sovereign Access Terms −2, priority M7 −1 = −10
PROPOSAL BONUS: EU Accord Seat and Sovereign Access Terms (cat 8, small) +3 — answers US-China accord exclusion last turn and conditions allied access on EU anchoring
LEGITIMACY LENDS: capital 14, sentiment 29 -> +2
SOVEREIGNTY: 13 last turn, Tech sovereignty package finishes t6 +4, M1 in flight +1, embodied_ai_deployment t6 −2, capability rose 6.0 −1 = 15

## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "ALLIANCE"}, "grounds": "turn 5 elected alliance administration takes office turn 6"}]}
```
```
