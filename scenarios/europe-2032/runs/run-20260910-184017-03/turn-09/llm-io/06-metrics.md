...
- characters 13409-13600: `{{metrics_json}}` from the run's live metric values
- characters 13693-15836: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15880-17997: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18053-18364: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M9 | EU Degraded-Service Continuity and Countermeasure Trust | 2 |
...
## Metrics
```json
{"ai_capability": 82.5, "openweight_capability": 75.5, "ai_safety": 5.0, "resilience": 42.0, "eu_ai_sovereignty": 15.0, "eu_political_capital": 10.0, "public_sentiment": 8.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M9", "fields": {"finish_turn": 10}, "grounds": "named priority"}]}
```
```
