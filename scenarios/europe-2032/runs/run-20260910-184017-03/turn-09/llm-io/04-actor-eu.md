...
- characters 454-2597: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2641-4758: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6201-6682: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9234-10721: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_counterfeit_countermeasure_surge (emergent event):** Counterfeit triage and countermeasure lots circulate during the pandemic surge, with forged cold-chain certificates and AI-generated lab reports, forcing recalls and checkpoint seizures in several member states.
...
## New measure
None this turn. We are waiting for political capital and hospital capacity to recover enough to staff anything new without collapsing the joint telemetry pact and continuity command.

## Store changes
No changes.

## Priority
M8 EU Accord Seat and Sovereign Access Terms, because with only concrete gigafactory shells at home and a member state buying alone abroad, the joint cyber/bio telemetry seat is the only path that keeps essential services running and preserves any sovereign access terms.

## In practice
We run incident command through Health/JHA formations on Article 168/222: HERA/ECDC pool samples and push detection kits under the binding biosurveillance pact, Civil Protection funds isolation wards and triage power, and ENISA keeps cutting infected municipal and hospital networks to degraded-not-dark. We accept the real-time intrusion feeds even where courts previously blocked them, as emergency solidarity.

We keep M8 at low burn through EEAS: seek a formal EU seat in the joint cyber command and biosurveillance investigation mandate, trading synthesis-screening enforcement and export-control alignment for models, cloud and countermeasures access. We do not sanction the breakaway buyer — we log incompatibility with grid and continuity co-funding and condition return to common procurement, while holding the Union line on warehouse-machine certification via the two national bans and liability suits to avoid seizing food and medicine logistics.
