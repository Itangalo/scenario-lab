...
- characters 478-3427: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 4799-5135: `{{statement_ledger}}` from the actor's live statement ledger

Everything outside those spans is the template's own text.

```
It is now turn 1, which covers July-December 2026. Each turn covers 6 months, so that is the span your actions have to land in.
