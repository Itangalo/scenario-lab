...
- characters 107-13427: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13470-13659: `{{metrics_json}}` from the run's live metric values
- characters 13752-15872: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15916-18132: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18188-19489: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M13 | EU Automated Patch and Swarm Defence Rollout | 2 |
...
- `emergent_belligerent_spillover_strike` -- first noted turn 13, listed in 1 turn(s) so far: A belligerent strike or sabotage directly disables EU-hosted power, cable landing or data-centre infrastructure despite declared non-belligerence, forcing emergency rationing of compute and connectivity.
- `emergent_wartime_infrastructure_blackout` -- first noted turn 13, listed in 1 turn(s) so far: Belligerent targeting of undersea cables and satellite ground stations cascades into a weeks-long cross-border outage of clearing, dispatch and clinical systems that drilled EU cities ride out on radio and generators while others lose payments and care.
...
## Metrics
```json
{"ai_capability": 91.5, "openweight_capability": 86.7, "ai_safety": 1.0, "resilience": 53.0, "eu_ai_sovereignty": 10.0, "eu_political_capital": 0.0, "public_sentiment": 4.0}
```
