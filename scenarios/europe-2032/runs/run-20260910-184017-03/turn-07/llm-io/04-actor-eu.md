...
- characters 454-2186: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2230-4681: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6063-6544: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9246-11485: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_humanoid_safety_ban (emergent event):** A commercial humanoid unit in a European logistics hub injures workers after a control-stack update, triggering national safety bans and liability litigation against the US vendor.
...
- `two_year_commitment` (commitment): Rebuild sovereign EU AI capacity that keeps essential services running through bio and embodied disruption
...
|---|---|---|---|---|---|---|---|---|---|
| M7 | EU Essential Continuity and Biosecurity Hardening | 6 | small | 5 | 8 | own jurisdiction | resilience up strongly, ai_safety up modestly via reduced misuse impact | 2 | running |
| M8 | EU Accord Seat and Sovereign Access Terms | 8 | small | 6 | 9 | US, China, coalition | eu_ai_sovereignty up, ai_safety up modestly via shared verification | 2 | running |
...
## New measure
**EU Accord Seat and Sovereign Access Terms**
Negotiates EU inclusion in the US-China risk accord and ties any structured allied frontier access to EU jurisdiction anchoring, safety verification and supply-chain leverage.
This is category 8 International coordination and leverage, because with RSI onset, open bio uplift and a US-China floor set without Brussels, sovereignty now depends on being inside the rule-setting rather than building parallel compute this turn; it directly serves the two-year commitment by making essential-service continuity enforceable on models the EU does not itself host.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Accord Seat and Sovereign Access Terms", "category": 8, "size": "small", "finish_turn": 9, "applies_to": "US, China, coalition", "targeted_effect": "eu_ai_sovereignty up, ai_safety up modestly via shared verification"}, "grounds": "to secure EU seat after US-China agreement and condition access on sovereignty"}]}
```

## Priority
M7 EU Essential Continuity and Biosecurity Hardening, because the automated cyber sweep is live and the bio uplift precursor is open — degraded-not-dark hospitals, grids and water outrank even the accord negotiation this turn.

## In practice
We run M7 as incident response: HERA/ECDC push detection kits and tightened synthesis screening to volunteer continuity nodes, ENISA with the Cybersecurity Agency isolates municipal and hospital systems hit by the model-generated sweep, and the Civil Protection Mechanism exercises fallback triage and robot-incident playbooks where humanoids are already deployed. No sanction on the hyperscaler hospital chain yet; cooperation with black-box bio suites becomes the condition for continuity funding.

We staff the new accord track from EEAS and DG TRADE/CNECT in the Foreign Affairs and Trade formations, using Anti-Coercion Instrument preparation, chip-equipment and robotics leverage, and export-control alignment as entry ticket to the US-China weights-security and bio-design limits, while DG CNECT keeps M1 gigafactory grid/water co-funding on the table and the AI Office pushes M6 opaque-reasoning assurance as the EU verification contribution.
...
```
## New measure
None this turn. We are waiting for pandemic triage and joint biosurveillance terms to stabilise before staking scarce capital on a new instrument we cannot staff.

## Store changes
No changes.

## Priority
M7 EU Essential Continuity and Biosecurity Hardening, because the AI-designed pandemic plus the ongoing ransomware sweep makes degraded-not-dark hospitals, water and municipalities the only mandate that still operates.

## In practice
We run M7 as pandemic incident command: HERA/ECDC and national health-security agencies push pooled sample-sharing and detection under the offered biosurveillance pact, Civil Protection Mechanism funds isolation, fallback triage and continuity kits to volunteer nodes, and ENISA keeps isolating municipal/hospital networks hit by the sweep. Council Health and JHA formations carry this on Article 168/222 solidarity bases; we accept the joint cyber/bio telemetry terms even where courts previously refused model access, because resilience now moves on others' infrastructure.

We keep M8 alive at low burn: EEAS seeks a formal EU seat in the joint threat-response command and in the US-China weights/bio-design follow-on, trading export-control alignment and synthesis-screening enforcement for access. We do not sanction the defecting member state this turn — we log the hyperscaler/Washington/Beijing side-deal as undercutting sovereign access terms and condition future continuity and grid co-funding on return to common procurement, while publicly citing the humanoid bans and liability cases to hold a Union line on embodied safety without spending capital we do not have.
