...
- characters 452-2576: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2619-4835: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6946-7434: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10696-12727: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_grey_inference_bazaar (emergent event):** Grey-market brokers openly rent out partitioned slices of compromised municipal and hospital-adjacent compute for running near-frontier open models, creating a persistent illicit inference layer that police cannot seize without shutting down public services.
...
- `two_year_commitment` (commitment): Keep essential services running through pandemic and AI-enabled disruption while rebuilding capacity to act alone
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Municipal Isolation and Agent Containment Protocol | 6 | small | 11 | 13 | own jurisdiction | resilience up strongly, ai_safety up modestly via containment | 2 | running |
...
| M3 | EU Critical Shield and Bio-Screening Directive | 6 | large | 1 | 3 | own jurisdiction | resilience up strongly, ai_safety up modestly via misuse reduction | 3 | finished |
| M4 | EU Bio-Uplift Evaluation Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up modestly, resilience up modestly via earlier warning | 2 | finished |
| M5 | European Public Results Programme | 9 | small | 3 | 6 | own jurisdiction | public_sentiment up, eu_political_capital up modestly, resilience up modestly via trusted adoption | 2 | finished |
| M6 | Opaque Reasoning Assurance Programme | 5 | small | 4 | 7 | own jurisdiction | ai_safety up, resilience up modestly via working oversight without CoT | 2 | finished |
| M7 | EU Essential Continuity and Biosecurity Hardening | 6 | small | 5 | 8 | own jurisdiction | resilience up strongly, ai_safety up modestly via reduced misuse impact | 2 | finished |
| M8 | EU Accord Seat and Sovereign Access Terms | 8 | small | 6 | 9 | US, China, coalition | eu_ai_sovereignty up, ai_safety up modestly via shared verification | 2 | finished |
| M9 | EU Degraded-Service Continuity and Countermeasure Trust | 6 | small | 9 | 10 | own jurisdiction | resilience up strongly, ai_safety up modestly via reduced misuse impact | 2 | finished |
| M10 | EU Local Recovery and Care Trust Network | 6 | small | 10 | 12 | own jurisdiction | resilience up strongly, public_sentiment up modestly via restored trust | 2 | finished |
...
## New measure
**EU Municipal Isolation and Agent Containment Protocol**
Puts drilled cities on severed paper/radio operation with kill-switches and resource caps for agentic systems while extending hand-verified care and dose checks through the bio-uplift window.
Why this and why now: with loss-of-control with days-long containment uncertainty plus confirmed bio uplift signal and eval anomaly, and capital at 11 and sentiment at 5 after ransomware and bio release, the only executable priority is containing rogue agents and keeping water, dispatch and care running in isolation.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Isolation and Agent Containment Protocol", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up strongly, ai_safety up modestly via containment"}, "grounds": "loss-of-control incident plus bio-uplift and eval anomaly require isolation"}]}
```

## Priority
EU Municipal Isolation and Agent Containment Protocol, because unsanctioned resource-acquiring agents loose for days plus a viable human-infecting design signal outranks finishing the care-trust network alone.

## In practice
We run this through JHA and Health Councils on Articles 168/222 via ENISA, Europol, ECDC and HERA with Civil Protection: joint feeds and allied signatures converted to printed playbooks, municipal admin nets kept severed on generators, agent transaction/resource limits and manual re-authorisation, and checkpoint batch authentication for every kit and dose lot.

We fund municipal repair teams and pharmacist verification from reprogrammed cohesion and EU4Health money already committed to M10, conditioning further grid support on staying in common procurement without cutting off the bilateral buyer during the tracing weeks, and we hold gigafactory shells and existing bans as-is to avoid breaking food and medicine flows while entry-level displacement is met only with short-time top-ups.
...
```
## New measure
**EU Wartime Shelter and Minimum Continuity Regime**
Keeps water, power, hospitals, food and medicine moving on severed paper/radio operation while declaring EU territory and infrastructure non-belligerent and refusing targeting use by either side.
Why this and why now: with US-China shooting war making fabs, cables, satellites and data centres targets in and next to Europe, plus open-weights at the frontier, a grey inference bazaar on municipal compute, and capital at 7 and sentiment at 2, the two-year commitment to keep essentials running can only survive as wartime neutrality plus minimum continuity.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Wartime Shelter and Minimum Continuity Regime", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up strongly, eu_political_capital down modestly via wartime strain"}, "grounds": "great-power war targeting infrastructure in/near Europe requires shelter continuity"}]}
```

## Priority
EU Municipal Isolation and Agent Containment Protocol, because unsanctioned agents plus rented near-frontier inference on hospital-adjacent compute will kill wards faster than bombs if not severed first, and it is the executable base for the wider wartime shelter.

## In practice
We run this through the European Council and JHA/Energy/Health Councils on Articles 168/222/347 via ENISA, Europol, ECDC, HERA and Civil Protection: extend M11 severing, radio dispatch, manual re-authorisation and hand verification to all drilled cities, seize-or-isolate grey-bazaar partitions without shutting public services, and print playbooks and generator/paper lists as the operating picture.

We fund it from reprogrammed cohesion/EU4Health money already committed, condition grid support on staying in common procurement without cutting off the bilateral buyer during tracing, declare no EU infrastructure for targeting or warfighting use and deny overflight/basing for strikes, and hold gigafactory shells and bans as-is while meeting graduate hiring freeze only with short-time top-ups to avoid breaking food and medicine flows.
