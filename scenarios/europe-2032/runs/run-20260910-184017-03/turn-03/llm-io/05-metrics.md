...
- characters 106-13462: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13505-13697: `{{metrics_json}}` from the run's live metric values
- characters 13790-15249: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15293-17492: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17548-18431: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Bio-Uplift Evaluation Cell | 2 |
| M5 | European Public Results Programme | 2 |
...
PORTFOLIO CHARGE t2: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M3 EU Critical Shield and Bio-Screening Directive −3, M4 EU Bio-Uplift Evaluation Cell −2, priority −1 = −12
PROPOSAL BONUS: EU Bio-Uplift Evaluation Cell (cat 5, small) +2 — bio_uplift precursor this turn plus autumn intrusion/screening expose from last turn, directly answered
LEGITIMACY LENDS: capital 41, sentiment 38 -> none (capital higher)
SOVEREIGNTY: 21 last turn, no capacity event, M1/M2 in flight +0, capability rose 3.0 −1 = 20

## Emerging developments (tracked)

- `emergent_evaluation_access_ruling` -- first noted turn 3, listed in 1 turn(s) so far: An EU court issues an interim order testing the systemic-risk power to compel deeper than API-only access to the contested genome model for the joint evaluation cell, creating a precedent for all frontier labs operating in the Union.
...
## Notepad
PORTFOLIO CHARGE t3: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M4 EU Bio-Uplift Evaluation Cell −2, M5 European Public Results Programme −2, priority M4 −1 = −11
PROPOSAL BONUS: European Public Results Programme (cat 9, small) +2 — answers member_state_defection indirectly by offering Union alternative, and scales adoption_success directly but positive event not severe shock
LEGITIMACY LENDS: capital 22, sentiment 34 -> +2
SOVEREIGNTY: 20 last turn, no capacity event finishes, M1/M2 in flight +0, member_state_defection t3 −2, capability rose 3.0 −1 = 17
## Store changes
```json
{"store": []}
```
