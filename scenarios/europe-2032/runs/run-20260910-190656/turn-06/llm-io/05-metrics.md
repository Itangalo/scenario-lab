...
- characters 105-13746: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13789-13981: `{{metrics_json}}` from the run's live metric values
- characters 14074-15418: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15461-17786: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17842-18414: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Blockade Bargaining and Continuity Pact | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election_retrenchment winner takes office in turn 6"}]}
```
```
