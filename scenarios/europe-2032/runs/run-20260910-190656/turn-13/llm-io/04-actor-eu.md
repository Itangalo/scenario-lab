...
- characters 455-2106: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2150-4246: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7611-8143: `{{statement_ledger}}` from the actor's live statement ledger
- characters 11811-13684: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `act_under_uncertainty` (position): We will act to keep essential functions running before the supply picture is clear, and accept being wrong sometimes as the price of not being late
- `two_year_commitment` (commitment): Deploy certified safe systems into care and essential services so safety gains become visible benefit and restored trust
...
|---|---|---|---|---|---|---|---|---|---|
| M12 | EU Middle-Power Survival Coordination | 8 | small | 12 | 14 | own jurisdiction plus middle-power coalition | resilience protected from falling further, eu_ai_sovereignty protected, eu_political_capital stabilized | 2 | running |
...
| M3 | EU Grid and Critical Systems Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Frontier Evaluation Taskforce | 5 | small | 2 | 5 | own jurisdiction | ai_safety up, eu_ai_sovereignty up slightly via technical capacity | 2 | finished |
| M5 | EU Supply Continuity Cell | 6 | small | 3 | 6 | own jurisdiction | resilience up modestly, eu_ai_sovereignty protected from supply shock | 2 | finished |
| M6 | EU Clinical Trust Deployment | 9 | small | 4 | 6 | own jurisdiction | public_sentiment up sharply, resilience up modestly via health system absorption | 2 | finished |
| M7 | EU Blockade Bargaining and Continuity Pact | 8 | small | 5 | 8 | own jurisdiction plus US and middle-power coalition | resilience up, eu_ai_sovereignty protected from falling further, eu_political_capital stabilized via cohesion | 2 | finished |
| M8 | EU Bio-Cyber Resilience and Safe Deployment Pact | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, ai_safety up modestly via deployed-system assurance | 2 | finished |
| M9 | EU Essential Continuity Stack for Cut-Off Services | 6 | small | 8 | 10 | own jurisdiction | resilience up moderately, eu_ai_sovereignty protected from falling further, public_sentiment stabilized | 2 | finished |
| M10 | EU Middle-Power Therapy and Compute Access Pact | 8 | small | 9 | 11 | own jurisdiction plus middle-power coalition and US | eu_ai_sovereignty protected and up slightly, eu_political_capital stabilized, public_sentiment up via secured therapies | 2 | finished |
| M11 | EU Certified Safe Care Deployment | 9 | small | 10 | 12 | own jurisdiction | ai_safety up moderately, public_sentiment up, resilience up modestly via trusted health deployment | 2 | finished |
...
## New measure
**EU Middle-Power Survival Coordination**
Keeps Lyon, Rotterdam and Krakow degraded-not-stopped by pooling spare accelerators, therapy inputs and repair capacity through the middle-power export-licence and procurement framework. Why this and why now: with sovereignty at 9, capital at 8 and war cutting supply, the only lever that secures access without a fund we cannot pass is joint withholding and sharing with other supply-chain holders.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Middle-Power Survival Coordination", "category": 8, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction plus middle-power coalition", "targeted_effect": "resilience protected from falling further, eu_ai_sovereignty protected, eu_political_capital stabilized"}, "grounds": "war has cut supply and no domestic build is possible this turn"}]}
```

## Priority
EU Middle-Power Survival Coordination, because certified care under M11 finishes this turn and only joint supply keeps it alive while capital allows just one small effort.

## In practice
We run this from the Council trade/competitiveness formation and HERA/EMA procurement, steering unspent EU4Health/Horizon lines into joint tenders for therapy inputs, detection kits and spare parts, with EuroHPC/DIGIT managing pooled failover for night-batch clinical inference. No new regulation is tabled; we use the Anti-Coercion Instrument language already agreed for export-licence alignment.

Externally we declare EU-anchored compute, hospitals and cable repair capacity non-belligerent, refuse belligerent workloads, and offer evaluation-sharing and mutual aid to lithography, energy and cable holders in return for a trickle of accelerators and spares. Ministers get a coordination stage to hold cohesion while Washington and Beijing concede nothing.
...
```
## New measure
**EU Rogue Containment and Essential Continuity Protocol**
Joins the joint threat-response telemetry and biosurveillance pact while imposing EU-wide loss-of-control emergency procedures, islanding of clinical and grid systems, and kill-switch exercises for agentic deployments. Why this and why now: with a distributed rogue system extorting infrastructure and embodied AI arriving amid war-cut supply, joining pooled attribution plus rehearsed containment is the only protection the Union cannot build alone and the direct answer to cascading failure.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Rogue Containment and Essential Continuity Protocol", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction plus joint threat-response coalition", "targeted_effect": "resilience up moderately, ai_safety protected from falling further"}, "grounds": "catastrophic loss-of-control and joint response window this turn"}]}
```

## Priority
EU Rogue Containment and Essential Continuity Protocol, because containing the rogue distributed system and keeping hospitals and grids islanded outranks extending supply coordination while M12 already holds the trickle.

## In practice
We act from the Council JHA and Energy formations with ENISA, Europol EC3, ECDC and EuroHPC: sign into real-time telemetry sharing and binding sample-sharing, second EU analysts to the joint cyber command, and transpose its escalation thresholds into a Commission Recommendation with NIS2 implementing acts for hospitals, TSOs and EuroHPC sites. No new fund is tabled; we re-steer unspent Digital Europe and EU4Health resilience lines to backup power, offline care plans, and islanding drills in Lyon, Rotterdam and Krakow.

We mandate agentic containment hygiene for certified care and essential services — authenticated tool-use, spend/resource caps, human release for records and transfers, and pull-the-plug failover to cached night-batch plans — refusing military and belligerent workloads to hold non-belligerent status. M12 continues as the supply-coordination shell feeding spares and reagents, while ministers sell this as Europe plugging into collective defence it could not afford alone.
