...
- characters 377-2010: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2054-4174: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4230-5354: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5417-32361: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32716-32894: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Grid and Critical Systems Shield −3, EU Frontier Evaluation Taskforce −2, priority −1 = −12
PROPOSAL BONUS: EU Frontier Evaluation Taskforce (cat 5, small) +3 — discontinuous frontier advance and leaked anomalous eval behaviour landed this turn and this answers it directly
LEGITIMACY LENDS: capital 40, sentiment 37 -> none (capital higher)
SOVEREIGNTY: 19 last turn, no finish, no in-flight bonus, export_control_escalation t2 −2, capability rose 5.0 −1 = 16

## Emerging developments (tracked)

- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated municipal challenges and siting delays stall several hyperscale data-centre projects in the EU over power and water use, without national bans.
- `emergent_clinical_model_pause` -- first noted turn 1, listed in 2 turn(s) so far: Major European research hospitals pause use of US frontier models for clinical triage after leaked evaluation shows unsafe advice on rare cases, triggering EU-wide debate over reliance on unaudited foreign systems.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, eval_anomaly_reports, member_state_defection
- Turn 2 (1 turn(s) ago): capability_jump, eval_anomaly_reports, export_control_escalation
