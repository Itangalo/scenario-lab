...
- characters 1864-3672: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3728-4817: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4880-28162: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28517-28558: `{{event_history}}` from the run's own event record
...
- `emergent_factory_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: Coordinated local opposition delays one or more AI factory shortlist sites, with municipal councils blocking grid pledges over energy prices and water use after grid-intrusion news links open models to vulnerability.
- `emergent_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: Coordinated local protests and municipal permit challenges stall data-centre and grid-upgrade sites in two or more candidate host states, delaying site selection beyond June.
