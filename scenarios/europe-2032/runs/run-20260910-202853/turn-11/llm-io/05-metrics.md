...
- characters 107-13515: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13558-13749: `{{metrics_json}}` from the run's live metric values
- characters 13842-15280: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15324-17468: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17524-18502: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_port_wildcat_spread` -- first noted turn 10, listed in 2 turn(s) so far: Coordinated wildcat shutdowns spread from Rotterdam and Lyon to two additional EU container ports after another teleoperated-machine injury, forcing a national-level port continuity emergency.
...
## Metrics
```json
{"ai_capability": 90.5, "openweight_capability": 86.6, "ai_safety": 0.0, "resilience": 59.0, "eu_ai_sovereignty": 12.0, "eu_political_capital": 21.0, "public_sentiment": 16.0}
```
