...
- characters 453-1665: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1708-3891: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5162-5640: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8120-10735: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M7 | EU Joint Shield Membership and Hospital-to-Grid Continuity Pact | 6 | small | 5 | 7 | own jurisdiction | resilience up materially, ai_safety up slightly via shared detection | 2 | running |
