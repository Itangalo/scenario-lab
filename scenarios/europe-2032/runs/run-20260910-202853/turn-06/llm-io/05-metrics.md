...
- characters 105-13659: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13702-13893: `{{metrics_json}}` from the run's live metric values
- characters 13986-15198: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15241-17424: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17480-18092: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Joint Shield Membership and Hospital-to-Grid Continuity Pact | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election won on strategic-asset tiered-access platform, taking office turn 6"}]}
```
```
