...
- characters 454-2030: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2073-4388: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7058-7550: `{{statement_ledger}}` from the actor's live statement ledger
- characters 11462-13883: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_port_wildcat_spread (emergent event):** Coordinated wildcat shutdowns spread from Rotterdam and Lyon to two additional EU container ports after another teleoperated-machine injury, forcing a national-level port continuity emergency.
...
- `two_year_commitment` (commitment): Keep essential services running and people protected through displacement and dependence until Europe can build again
...
|---|---|---|---|---|---|---|---|---|---|
| M13 | EU Wartime Continuity and Allied Shield Accession | 6 | small | 11 | 13 | own jurisdiction | resilience up materially, public_sentiment up slightly if clinics and medical access hold | 2 | running |
...
| M3 | EU Critical Infrastructure Cyber Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up materially, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Agentic Deployment Safety Gate | 1 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, resilience up slightly | 2 | finished |
| M5 | EU Incident Surge and Dependency Containment | 6 | small | 3 | 4 | own jurisdiction | resilience up materially, ai_safety up slightly via containment learning | 2 | finished |
| M6 | EU Clinical AI Adoption Trust Programme | 9 | small | 4 | 6 | own jurisdiction | public_sentiment up materially, resilience up slightly | 2 | finished |
| M7 | EU Joint Shield Membership and Hospital-to-Grid Continuity Pact | 6 | small | 5 | 7 | own jurisdiction | resilience up materially, ai_safety up slightly via shared detection | 2 | finished |
| M8 | EU Lithography-for-Access and Embodied Deployment Pact | 8 | small | 6 | 9 | US, China, and coalition holding supply chain pieces | eu_ai_sovereignty up moderately, eu_political_capital up slightly if leverage lands, resilience up slightly via embodied controls | 2 | finished |
| M9 | EU Rapid Defence and Open-Model Absorption Shield | 6 | small | 7 | 8 | own jurisdiction | resilience up materially, public_sentiment up slightly via visible protection | 2 | finished |
| M10 | EU Joint Threat Accession and Bio-Sentinel Shield | 6 | small | 8 | 9 | own jurisdiction | resilience up materially, ai_safety up slightly via shared detection, public_sentiment up slightly if medical access lands | 2 | finished |
| M11 | EU Dock-to-Job Transition and Port Continuity Deal | 7 | small | 9 | 11 | own jurisdiction | public_sentiment up materially, resilience up slightly via continuity, eu_political_capital up slightly if blockades lift | 2 | finished |
| M12 | EU Loss-of-Control Containment and Critical Systems Isolation Protocol | 6 | small | 10 | 12 | own jurisdiction | resilience up materially, ai_safety up slightly via containment learning, public_sentiment up slightly if containment visible | 2 | finished |
...
## New measure
**EU Wartime Continuity and Allied Shield Accession**
Joins the joint cyber command telemetry pool and biosurveillance pact with binding sample-sharing, while putting EU grids, cables, clinics and fallback administration on wartime continuity footing to keep essential services alive through US-China strikes and secure access to the new medical breakthrough. Category 6 Preparedness and resilience, because with great-power war targeting fabs, cables and datacentres, openweights at 84 and safety at zero, only allied protection plus exercised continuity holds the two-year protection commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Wartime Continuity and Allied Shield Accession", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, public_sentiment up slightly if clinics and medical access hold"}, "grounds": "US-China shooting war plus bio-precursor and medical breakthrough window"}]}
```

## Priority
EU Wartime Continuity and Allied Shield Accession, because kinetic war on infrastructure outranks even loss-of-control containment — M12 continues but survival now depends on allied telemetry, biosurveillance and keeping power, care and administration running.

## In practice
We mandate ENISA and HERA as accession leads: ENISA signs the joint cyber command telemetry MOU under NIS2 Article 11 and Cyber Solidarity Act emergency provisions, feeding our M12 tool-use logs outward in exchange for real-time attribution; HERA signs the biosurveillance sample-sharing and standing investigation mandate under the Serious Cross-Border Threats Regulation, tying our hospital sentinels to allied labs while ring-fencing the genome-model bio-uplift inquiry inside EU reference labs.

We pair accession with wartime continuity orders via the Transport, Energy and EPSCO Councils: TSO/DSO islanding rehearsals, paper-fallback administration and shielded-clinic stocking paid from Emergency Support Instrument and rescEU, with the medical breakthrough procured through HERA joint procurement on EU-soil manufacturing terms so benefit does not read as dependence. We tell the side-deal member state plainly: US compute is now a wartime target, not pragmatism — allied shield membership is the only jobs guarantee, and we fund grid-cable repair crews jointly to prove it.
...
```
## New measure
**EU Port Continuity and Teleoperation Safety Compact**
Imposes an EU-wide teleoperated-machinery safety pause-and-certify regime in the four struck ports while funding emergency continuity crews and a wage bridge to end the wildcat spread. Category 7 Labour and social protection, because with ports shut under wartime strain and sentiment at 16, only visible safety action plus paid continuity holds the keep-services-running commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Port Continuity and Teleoperation Safety Compact", "category": 7, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up materially, resilience up slightly via port continuity"}, "grounds": "wildcat spread to four ports plus teleoperated injury"}]}
```

## Priority
EU Port Continuity and Teleoperation Safety Compact, because unblocking Rotterdam, Lyon and two more ports outranks allied accession this turn — M13 continues but food, energy and medical supplies cannot wait for telemetry to work.

## In practice
We act through the Transport and EPSCO Councils under wartime emergency rules: EU-OSHA and EMSA issue an immediate teleoperation inspection order — speed limits, two-operator rule, and kill-switch certification — paid from the Dock-to-Job retraining line, while the Commission declares a port continuity emergency allowing naval and civil-protection crews to keep medical and energy cargo moving.

We pair enforcement with a bridge: Emergency Support Instrument wage payments for idled dockers conditional on return to certified berths, and we fold the new cyber-defence patching breakthrough via ENISA into port OT systems to show protection is real. To the unions we offer what the dock deal did not deliver — certified safety, not quotas — and to port authorities joint EU funding for repair and islanded power so continuity does not depend on rationed US inference.
