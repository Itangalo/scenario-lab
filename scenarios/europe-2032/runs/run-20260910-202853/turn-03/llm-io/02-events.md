...
- characters 377-2048: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2092-3999: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4055-4899: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4962-31906: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32261-32398: `{{event_history}}` from the run's own event record
...
- `emergent_bio_screening_norm` -- first noted turn 1, listed in 2 turn(s) so far: Major EU scientific publishers and universities jointly mandate disclosure and containment standards for AI-designed biological constructs after the phage demonstrations, creating a de facto screening norm ahead of regulation.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, loss_of_control_incident
