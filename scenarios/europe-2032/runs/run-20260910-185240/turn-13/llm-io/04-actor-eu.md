...
- characters 452-1942: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1986-4049: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5755-6230: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8191-9846: `{{previous_actions}}` from the actor's own response from the previous turn
...
```
## New measure
None this turn, while political capital is zero and frontier access cutoff leaves no fundable lever to start.

We hold the two-year commitment to hold essential life together through pandemic and cutoff while preserving a path back to EU capacity by keeping finished Shield and Continuity Switch fallbacks, rationed open-weight triage, and case-by-case medical continuity requests running rather than starting an instrument we cannot pay for, staff, or enforce.
...
## In practice
We keep hospitals, testing sites and grid operations upright on the Shield and Continuity Switch playbooks. HERA, ECDC and health ministries run paper triage and double shifts; EuroHPC rations open weights for dosage and freight while ministries absorb the US frontier cutoff and re-route critical workloads to what still answers.

We treat the US-China risk agreement as floor we are outside of, not leverage we hold. The Commission and EEAS seek observer access and narrow health continuity on Washington's terms, the AI Office logs the cutoff and extortion-agent incidents for incident reporting without launching evaluations we cannot staff, and no new compute build is started while grid-hookup freezes stay defended in court.

We let the tailored-therapy arrivals come through case-by-case US approvals at US prices, publicly grateful and privately recording dependence, because with capital and sentiment near zero contesting terms would cost continuity we cannot replace.
