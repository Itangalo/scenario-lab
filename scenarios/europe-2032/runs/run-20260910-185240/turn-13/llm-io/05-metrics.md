...
- characters 107-12868: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12911-13099: `{{metrics_json}}` from the run's live metric values
- characters 13192-14682: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14726-16789: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16845-17548: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_therapy_grey_market` -- first noted turn 13, listed in 1 turn(s) so far: Unlicensed cross-border networks begin moving US-tailored therapy doses and protocols into Europe outside Washington approval channels, forcing seizures, prosecutions and a public debate over medical smuggling as care.
...
## Notepad
PORTFOLIO CHARGE t13: no measures in flight, no priority = −0
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 0.0, sentiment 1.0 -> +0, capital held at floor, no lend applied while capital at zero with no delivery
SOVEREIGNTY: 4 last turn, no capacity event? no — eu_frontier_access_denied t13 −3, no cat4 finish, no capacity in flight +0, capability rose 1.5 −0 = 1
