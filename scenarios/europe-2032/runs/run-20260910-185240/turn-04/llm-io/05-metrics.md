...
- characters 105-13503: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13546-13738: `{{metrics_json}}` from the run's live metric values
- characters 13831-14871: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14914-17039: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17095-17843: `{{notepad}}` from the Game Master's notepad, carried across turns
...
**emergent_permit_blockade_freeze (emergent event):** Coordinated municipal protests block grid connections and construction access at planned gigafactory sites, forcing a formal permitting freeze through mid-2028.
...
## New measure
None this turn, while we conserve collapsing political capital and wait for the municipal blockade and permitting freeze to be negotiable again.

## Store changes
No changes.

## Priority
M1 InvestAI Gigafactories, because the two-year commitment to independent capacity fails entirely if the coordinated permit blockade is allowed to harden into permanent local vetoes.

## In practice
We are putting M1/M2 into defensive hold through DG CNECT, DG ENER and the EIB: protecting site reservations and grid-connection queue positions legally, pausing compulsory measures, and opening mayor-level talks in the blocking municipalities on blackout resilience, water and power compensation, and construction traffic. No new ground-breaking is forced against blockades this turn.

We are living off the now-finished M3 EU Critical Systems Shield and M4 Monitoring Baseline as the operating spine: ENISA and EU-CyCLONe keep cross-border CERT mutual aid, paper-fallback procedures for hospitals and municipalities, and port/grid isolation playbooks running, while ministries work insurer-by-insurer to prevent cover withdrawal during attribution. Public communication admits dependence on foreign models for both attack tooling and recovery, and frames resilience as the bridge until EU capacity lands.
