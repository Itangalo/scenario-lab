...
- characters 375-1620: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1663-3733: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3789-4432: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4495-30996: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 31351-31765: `{{event_history}}` from the run's own event record
...
- `emergent_datacentre_moratorium_wave` -- first noted turn 4, listed in 2 turn(s) so far: Coordinated local council moratoriums freeze grid connections for new data centres in three member states after blackout-linked protests, delaying EU compute buildout by months.
...
- Turn 1 (5 turn(s) ago): cyber_test_shot, loss_of_control_incident, safety_breakthrough, knowledge_work_augmented, eu_frontier_access_denied
- Turn 2 (4 turn(s) ago): capability_jump, ai_investment_collapse, emergent_cyber_insurance_repricing
- Turn 3 (3 turn(s) ago): cyber_major_incident, capability_jump
- Turn 4 (2 turn(s) ago): emergent_permit_blockade_freeze
