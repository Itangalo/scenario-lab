...
- characters 377-1753: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1797-4246: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4302-5400: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5463-29039: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29394-29547: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU AI Evaluation and Safety Institute −3, EU Grid and Public-Service Cyber Hardening Shield −2, priority −1 = −12
PROPOSAL BONUS: EU Grid and Public-Service Cyber Hardening Shield (cat 6, small) +3 — cyber_major_incident landed this turn severe and this answers it directly
LEGITIMACY LENDS: capital 16, sentiment 39 -> +2
SOVEREIGNTY: 21 last turn, no capacity event finish, Gigafactories in flight +0, supply_chain_coercion t2 −2 (largest of two take-away events), capability rose 3.0 −1 = 18

## Emerging developments (tracked)

- `emergent_grid_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: Coordinated local protests and municipal court injunctions stall two announced AI factory grid-connection works, forcing renegotiation of energy compensation.
- `emergent_siting_backlash_freeze` -- first noted turn 2, listed in 1 turn(s) so far: Coordinated municipal moratoriums and grid-fee lawsuits stall two to three announced AI factory and data-centre sites, forcing renegotiation of power pledges.
...
- Turn 2 (1 turn(s) ago): cyber_major_incident, supply_chain_coercion, member_state_defection, adoption_success
