...
- characters 453-1701: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1744-4288: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5744-6189: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8403-10521: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M6 | EU Assisted-Defence Extension to Exposed Operators | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately | 2 | running |
...
## New measure
**EU Assisted-Defence Extension to Exposed Operators**
It extends the proven Cyber Shield automated-patching and incident-response kit from the defended transmission, water and hospital estates to municipal clinics and other OT operators hit by premium repricing and the major automated attack.
This and now because the ransomware sweep made defence failure public, insurers are pricing the gap, and with no measures in flight and capital at 6.0 a small preparedness and resilience build is the only affordable way to hold essential services through the blockade commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Assisted-Defence Extension to Exposed Operators", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "major automated attack plus insurer repricing demands expansion"}]}
```

## Priority
EU Assisted-Defence Extension to Exposed Operators, because containing the visible cyber failure outranks shells and licensing queues this turn for holding essential services.

## In practice
We run this through ENISA and the finished Cyber Shield integration teams, using bulk licences and the new interpretability-backed patching already working in uniform hospitals, funded by reallocated Digital Europe / continuity reserve money with co-pay from insurers and member states to settle the procurement dispute over who pays. Health and Energy Council formations carry the mandate to municipal clinics and exposed OT, with instructors seconded from the defended estates.

DG GROW and DG TRADE continue without new law the holding operation: US licence-queue files for Dutch/French toolmakers, power-electronics substitution for islanding kits, volume chips for hospitals/telecom/grid, and quiet containment of the side-deal capital. HERA/ECDC keep detection review, and the AI Office pursues joint evaluation with Washington on the leaked foreign-agent benchmark, while we claim the waiting-list gains for Brussels-backed adoption to rebuild sentiment.
...
```
## New measure
**EU Swarm-Behaviour Detection Rollout**
It pushes the new swarm-behaviour detection and automated patching breakthrough from the defended estates into municipalities, clinics and small grid contractors via ENISA kits already in flight.
This and now because the loss-of-control incident and ransomware sweep proved offence is ahead, the breakthrough is the first visible shift back, and deploying it now is the cheapest way to hold essential services under the blockade commitment with capital at 7.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Swarm-Behaviour Detection Rollout", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly"}, "grounds": "deploy defensive breakthrough to answer sweep and loss-of-control"}]}
```

## Priority
EU Assisted-Defence Extension to Exposed Operators, because kits without staff and insurer repricing are the televised failure that will decide whether the breakthrough is believed.

## In practice
We run both efforts as one field operation through ENISA and the finished Cyber Shield integration teams: bulk licences and seconded instructors from transmission and large hospitals carry the existing assisted-defence kit plus the new swarm-detection update to municipal clinics and exposed OT, funded by reallocated continuity funds with insurer co-pay to settle who pays the uplift. Health and Energy Councils mandate access, no new law this turn.

DG GROW and DG TRADE hold the supply-chain leg without new money: shepherding Dutch/French lithography files through the US queue, power-electronics substitution for islanding kits, and volume chips for hospitals/telecom/grid, while containing the solo-deal capital. The AI Office logs the loss-of-control incident to the joint evaluation with Washington and claims triage waiting-list gains to stabilize sentiment.
