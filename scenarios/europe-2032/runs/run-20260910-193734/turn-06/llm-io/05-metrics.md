...
- characters 105-13326: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13369-13560: `{{metrics_json}}` from the run's live metric values
- characters 13653-14770: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14813-16814: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16870-17832: `{{notepad}}` from the Game Master's notepad, carried across turns
...
LEGITIMACY LENDS: capital 1, sentiment 34 -> +2
SOVEREIGNTY: 16 last turn, no capacity event finishes, Gigafactories in flight +0, member_state_defection t5 −2, capability rose 1.0 −0 = 14

## Emerging developments (tracked)

- `emergent_cyber_insurance_repricing` -- first noted turn 5, listed in 2 turn(s) so far: A major European insurer reprices cyber cover for OT operators after the pilot results, publicly differentiating assisted-defence sites and triggering procurement disputes over who pays for wider rollout.
- `emergent_cyber_insurance` -- first noted turn 5, listed in 2 turn(s) so far: A major European insurer reprices cyber cover for OT operators after the pilot results, publicly differentiating assisted-defence sites and triggering procurement disputes over who pays for wider rollout.
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "ALLIANCE"}, "grounds": "turn 5 winner was alliance, takes effect turn 6"}]}
```
```
