...
- characters 1431-3384: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3440-4587: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4650-28283: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28638-28725: `{{event_history}}` from the run's own event record
...
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated physical protests and grid-connection lawsuits stall at least one major EU data-centre project for months, making energy cost and siting a Union-level political issue.
- `emergent_bio_data_freeze` -- first noted turn 1, listed in 2 turn(s) so far: Major EU scientific publishers and universities jointly mandate AI-provenance disclosure and restricted licensing for frontier biological datasets after the phage designs, temporarily slowing open genome-model training data flows.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, cyber_major_incident, ai_investment_collapse
