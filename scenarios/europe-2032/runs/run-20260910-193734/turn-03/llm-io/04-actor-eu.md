...
- characters 455-1465: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1509-3462: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4121-4563: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6171-7966: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M3 | EU Critical Infrastructure Cyber Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up strongly, ai_safety up slightly via incident learning | 3 | running |
| M4 | Essential Services Continuity Reserve | 6 | small | 2 | 5 | own jurisdiction | resilience up moderately | 2 | running |
...
Creates EU-funded backup power, offline recovery kits and mutual-aid repair crews for hospitals, water and grid operators to keep services degrading not stopping during AI-enabled attack.
This is Preparedness and resilience work that must come now because segmentation audits under the Cyber Shield take years while the autumn probes show staged access already inside, so continuity capacity is the only hardening that pays off within this commitment period.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Essential Services Continuity Reserve", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "probes staged in OT with drills showing IT-to-OT reachability"}]}
```

## Priority
EU Critical Infrastructure Cyber Shield, because it is the core of the two-year hardening commitment and detection plus segmentation audits must move before the continuity reserve can be targeted.

## In practice
We push the Shield through TTE-Energy and JHA this turn: Commission implementing act for mandatory segmentation audits on designated TSOs, ports and water utilities, ENISA-procured swarm-detection sensors funded by CEF-Digital reflows, and the five-country live containment drill. We buy French-German assent with co-financing rates and priority grid-connection for AI factory zones.

Alongside we stand up the Continuity Reserve via UCPM / rescEU: stockpiled islanding kits, mobile generators and clean backup images for hospitals and municipal services, plus cross-border repair teams rostered from TSOs. Utilities resisting unfunded mandates get funded spares and crews in exchange for audit compliance.
...
```
## New measure
**EU Swarm-Defence Fast Deployment**
Deploys the new automated patching and swarm-behaviour detection tooling to energy, water, telecom and hospital operators via ENISA-managed procurement and funded integration teams.
This is Preparedness and resilience work and it must come now because the defensive breakthrough finally shifts the offence-defence balance, and because audits and stockpiles alone cannot close IT-to-OT paths before open-weight intrusion tooling scales further.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Swarm-Defence Fast Deployment", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly"}, "grounds": "defensive breakthrough shifts balance while audits stall"}]}
```

## Priority
EU Critical Infrastructure Cyber Shield, because it is the delivery vehicle for the breakthrough tooling and the core of the hardening commitment, and the new fast-deployment measure only works if plugged into its sensors and audits.

## In practice
We use the Cyber Shield implementing act and CEF-Digital to re-direct unspent sensor funds to bulk-licensed automated patching and swarm-detection, with ENISA as central buyer to survive tender challenges. TTE-Energy mandates deployment on designated TSOs and water utilities, trading co-financed integration costs and Continuity Reserve crew priority for completed segmentation audits.

We push the Continuity Reserve through UCPM/rescEU training this turn: regional warehouse kits get rostered operators and funded exercises with hospitals and mayors, fixing uneven distribution. On Taiwan tensions we order a quiet supply-chain watch via DG TRADE and ENISA on chips and islanding-kit components, without diverting money from hardening.
