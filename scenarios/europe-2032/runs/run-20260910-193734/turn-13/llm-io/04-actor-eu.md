...
- characters 456-2101: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2145-4359: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6884-7344: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10098-12138: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Join pooled defence and contain autonomous-agent risk while restoring capacity to act
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Municipal Recovery and Joint Response Bridging | 6 | small | 12 | 14 | own jurisdiction plus joint command partners | resilience up moderately to absorb cyber incident | 2 | running |
...
| M3 | EU Critical Infrastructure Cyber Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up strongly, ai_safety up slightly via incident learning | 3 | finished |
| M4 | Essential Services Continuity Reserve | 6 | small | 2 | 5 | own jurisdiction | resilience up moderately | 2 | finished |
| M5 | EU Swarm-Defence Fast Deployment | 6 | small | 3 | 5 | own jurisdiction | resilience up moderately, ai_safety up slightly | 2 | finished |
| M6 | EU Assisted-Defence Extension to Exposed Operators | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately | 2 | finished |
| M7 | EU Swarm-Behaviour Detection Rollout | 6 | small | 8 | 10 | own jurisdiction | resilience up moderately, ai_safety up slightly | 2 | finished |
| M8 | EU Agent Containment and Joint Telemetry Accession | 6 | small | 10 | 12 | own jurisdiction plus joint command partners | resilience up moderately, ai_safety up slightly | 2 | finished |
...
## New measure
**EU Municipal Recovery and Joint Response Bridging**
Keeps essential municipal services running through the major cyber incident by extending the finished continuity reserve into a joint triage cell with ENISA and the joint threat-response partners.
This and now because the ransomware sweep hits exactly where resilience is thinnest — small municipalities and two-tier insured operators — and the joint telemetry pool plus US-China verification window is the only cheap protection we can borrow while political capital is at 9.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Recovery and Joint Response Bridging", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction plus joint command partners", "targeted_effect": "resilience up moderately to absorb cyber incident"}, "grounds": "major incident plus joint response invitation while capital depleted"}]}
```

## Priority
M9 EU Municipal Recovery and Joint Response Bridging, because it is the only live instrument and it must turn the joint telemetry and joint threat-response offer into visible recovery in municipalities before defection hardens.

## In practice
We run the incident as a preparedness task, not a sovereignty bid. ENISA as EU node forwards joint indicators to municipal CERTs, Health and JHA Councils reactivate the staffed patching rota on the finished Continuity Reserve legal base with insurer co-pay, and the Franco-German verification cell triages false alarms. No new procurement, no conditionality fight.

We contain the side-deal politically: full access to joint signatures and recovery teams for any operator that stays on common reporting, Transport and JHA Councils keep the solidarity file open without sanctions, and DG TRADE holds screening papers warm. We bank the medical breakthrough for trust and note the open-weight release and eval anomaly as reason to stay inside the US-China verification floor rather than chase solo compute we cannot fund.
...
```
## New measure
**EU Breakthrough Patching Scale-Up**
Deploys the new automated patching and swarm-behaviour detection tooling through ENISA's node and the health/energy rota to the exposed municipalities and operators missed in February.
This and now because the defence breakthrough finally shifts the offence-defence balance back, and with capital at 10 and two capitals on side-deals this is the only cheap window to turn joint telemetry into lasting hardening before the next open release wipes it out.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Breakthrough Patching Scale-Up", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "defence breakthrough plus major incident while capital depleted"}]}
```

## Priority
M9 EU Municipal Recovery and Joint Response Bridging, because visible municipal restoration is what keeps the solidarity file open and buys permission for the patching scale-up to land.

## In practice
We keep M9 as the front line: the bridging cell ties municipal CERTs to joint-command indicators, Health and JHA Councils run the rota on the existing Continuity Reserve base with insurer co-pay, and Franco-German tuning handles false positives. No new procurement fight, no sanctions vote we would lose at capital 10 — just tours of restored cities and full access for anyone staying on common reporting.

We seed the scale-up inside that same track: ENISA packages the breakthrough automated-patching playbooks as an overnight update to existing Cyber Shield tooling, piloted first where telemetry already reaches, then pushed to the paper-queue communes via the bridging cell. DG TRADE keeps screening papers warm and transport/interior ministers hold the two-rulebook problem without forcing a break, while we cite the tailored-therapy benefit and the eval-anomaly leak to justify staying inside the Washington-Beijing verification floor.
