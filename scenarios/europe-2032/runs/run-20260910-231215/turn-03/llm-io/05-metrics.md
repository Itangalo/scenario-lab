...
- characters 106-13080: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13123-13315: `{{metrics_json}}` from the run's live metric values
- characters 13408-14652: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14696-16802: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16858-17704: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Critical Systems Shield | 2 |
| M5 | EU AI Incident Reporting Mandate | 2 |
...
- `emergent_grid_tooling_disclosure` -- first noted turn 2, listed in 2 turn(s) so far: Independent grid-security researchers publish a partial reconstruction of the autumn intrusion tooling, showing close derivation from a freely downloadable model and triggering press calls for open-model liability.
...
## Metrics
```json
{"ai_capability": 56.5, "openweight_capability": 51.0, "ai_safety": 28.0, "resilience": 35.5, "eu_ai_sovereignty": 20.0, "eu_political_capital": 30.0, "public_sentiment": 30.0}
```
