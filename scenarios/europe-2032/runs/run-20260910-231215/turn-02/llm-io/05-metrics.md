...
- characters 105-13033: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13076-13268: `{{metrics_json}}` from the run's live metric values
- characters 13361-14592: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14635-17225: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17281-18081: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_grid_tooling_disclosure` -- first noted turn 2, listed in 1 turn(s) so far: Independent grid-security researchers publish a partial reconstruction of the autumn intrusion tooling, showing close derivation from a freely downloadable model and triggering press calls for open-model liability.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 47.0, "ai_safety": 30.0, "resilience": 37.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 32.0, "public_sentiment": 37.0}
```
