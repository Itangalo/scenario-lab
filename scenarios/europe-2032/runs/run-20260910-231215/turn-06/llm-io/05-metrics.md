...
- characters 105-13610: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13653-13845: `{{metrics_json}}` from the run's live metric values
- characters 13938-15999: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 16042-18264: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18320-19200: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Middle-Power Supply-Chain Compact | 2 |
| M8 | EU Joint Cyber Shield Membership | 2 |
...
- `emergent_evaluation_badge_uptake` -- first noted turn 5, listed in 2 turn(s) so far: Several non-EU health systems publicly license the EU evaluation institute's assurance check for their own procurement after the hospital waiting-list results circulate, giving the badge external pull beyond the Union.
...
## Metrics
```json
{"ai_capability": 62.7, "openweight_capability": 58.9, "ai_safety": 27.5, "resilience": 40.0, "eu_ai_sovereignty": 24.0, "eu_political_capital": 31.0, "public_sentiment": 43.5}
```
