...
- characters 376-2437: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2480-4702: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4758-5638: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5701-31360: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 31715-32044: `{{event_history}}` from the run's own event record
...
- `emergent_evaluation_badge_uptake` -- first noted turn 5, listed in 1 turn(s) so far: Several non-EU health systems publicly license the EU evaluation institute's assurance check for their own procurement after the hospital waiting-list results circulate, giving the badge external pull beyond the Union.
...
- Turn 2 (4 turn(s) ago): taiwan_tension_rise
- Turn 3 (3 turn(s) ago): cyber_major_incident, eval_anomaly_reports
- Turn 4 (2 turn(s) ago): safety_breakthrough, member_state_defection, adoption_success
- Turn 5 (1 turn(s) ago): capability_jump, middle_power_coalition, election_alliance
