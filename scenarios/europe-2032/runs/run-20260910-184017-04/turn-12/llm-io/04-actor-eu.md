...
- characters 454-2060: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2103-4121: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4755-5225: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8212-10170: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_containment_playbook_countered (emergent event):** Hospital paper-fallback procedures and substitute-controller manuals leaked from the repair corps circulate widely, letting ransomware crews pre-build bypasses for islanding drills before the corps stands up.
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Bio Detection and Medical Countermeasure Surge | 6 | small | 11 | 13 | own jurisdiction | resilience up, ai_safety up slightly via misuse detection | 2 | running |
...
| M10 | EU Visible Benefit Adoption Push | 9 | small | 10 | 12 | own jurisdiction | public_sentiment up, eu_political_capital up slightly | 2 | finished |
...
```
## New measure
None this turn, while we let the Bio Detection Surge land and avoid starting what capital 10 and sentiment 3 cannot carry.

## Store changes
No changes.

## Priority
M11 EU Bio Detection and Medical Countermeasure Surge, because the real bio release with model involvement makes detection-first the only way the essential-services pledge survives the next weeks.

## In practice
We hold the line on M11 under HERA and ECDC: sequencing sentinels around Corps-held hospitals stay on, pooled isolation stock is released to willing regions first, and advance countermeasure options are triggered through the Health Council emergency line. Seconded Corps staff run triage, not new hires we cannot fund.

We answer the leaked fallback manuals by compartmenting them: ENISA and EU-CERT reissue substitution sheets only to cleared islanding teams, rotate drills, and treat the circulating copies as compromised. No new instrument, no siting fight — DG SANTE keeps publishing only what already works to avoid spending legitimacy we do not have.
