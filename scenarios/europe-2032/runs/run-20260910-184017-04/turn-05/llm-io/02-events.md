...
- characters 377-1662: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1706-4302: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4358-5449: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5512-27186: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27541-27842: `{{event_history}}` from the run's own event record
...
- `emergent_service_manual_leak` -- first noted turn 4, listed in 1 turn(s) so far: ASML field-service engineers in China stage wildcat work stoppages after spare-parts freeze, leaking maintenance manuals that circulate openly and erode service controls.
- `emergent_beijing_materials_retaliation` -- first noted turn 4, listed in 1 turn(s) so far: Beijing retaliates for the widened ASML halt with targeted export licensing on gallium, germanium and lithography spares, squeezing European chip-tool servicing and hospital hardware refreshes within the half.
...
- Turn 2 (3 turn(s) ago): safety_breakthrough, taiwan_tension_rise, eu_frontier_access_denied
- Turn 3 (2 turn(s) ago): supply_chain_coercion
- Turn 4 (1 turn(s) ago): loss_of_control_incident
