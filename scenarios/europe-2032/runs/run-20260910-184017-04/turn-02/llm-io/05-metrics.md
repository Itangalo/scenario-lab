...
- characters 105-12913: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12956-13148: `{{metrics_json}}` from the run's live metric values
- characters 13241-14414: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14457-16569: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16625-17720: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacenter_siting_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated local opposition blocks or delays at least one major hyperscale data centre project in the EU over power prices and water use, forcing a siting rethink in autumn 2026.
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated protests and local permit challenges delay at least one major EU data-centre or supercomputer expansion during the turn, turning energy use and siting into a national political issue.
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Shield −3, EU Continuity Switch −3, priority −1 = −13
PROPOSAL BONUS: EU Continuity Switch (cat 9, large) +5 — eu_frontier_access_denied landed this turn on hospitals/ministries and this answers it directly
LEGITIMACY LENDS: capital 34, sentiment 38 -> +1
SOVEREIGNTY: 22 last turn, no capacity event, Gigafactories in flight +0, Tech sovereignty package in flight +0, eu_frontier_access_denied t2 −2, capability rose 1.5 −0 = 20
