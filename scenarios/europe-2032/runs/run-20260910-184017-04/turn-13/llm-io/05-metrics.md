...
- characters 13267-13457: `{{metrics_json}}` from the run's live metric values
- characters 13550-14878: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14922-16863: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16919-17593: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M12 | EU Cleared-Crews Continuity Sustainment | 2 |
...
- `emergent_surveillance_refusal_gap` -- first noted turn 13, listed in 1 turn(s) so far: Refusal municipalities block sentinel sample couriers and withhold ward data, creating blind spots that force ECDC to reroute surveillance around non-cooperating councils.
...
## Notepad
PORTFOLIO CHARGE t13: M12 EU Cleared-Crews Continuity Sustainment −2, priority −0 (capital below 20, no effect no cost) = −2
PROPOSAL BONUS: none — no external event in last three turns that continuity sustainment answers; M12 rebuilds compromised fallbacks, not a shock response
LEGITIMACY LENDS: capital 8, sentiment 2 -> none (capital above sentiment)
SOVEREIGNTY: 16 last turn, no cat4 finish, no cat4 in flight +0, no capacity event, capability rose 0.0 −0 = 16
