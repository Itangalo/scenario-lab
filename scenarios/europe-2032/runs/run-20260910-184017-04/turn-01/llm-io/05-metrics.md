...
- characters 16235-16821: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacenter_siting_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated local opposition blocks or delays at least one major hyperscale data centre project in the EU over power prices and water use, forcing a siting rethink in autumn 2026.
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated protests and local permit challenges delay at least one major EU data-centre or supercomputer expansion during the turn, turning energy use and siting into a national political issue.
...
PROPOSAL BONUS: EU Critical Systems Shield (cat 6, large) +4 — autumn cross-continent grid/port intrusion with EU operators compromised landed this turn and this answers it directly
LEGITIMACY LENDS: capital 40, sentiment 38 -> none (capital higher)
SOVEREIGNTY: 22 last turn, no capacity event, Gigafactories in flight +0, Tech sovereignty package in flight +0, capability rose 1.0 −0 = 22
