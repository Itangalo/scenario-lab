...
- characters 454-1770: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1813-3895: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5897-6392: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8900-10805: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_patch_failure_cluster (emergent event):** Patched medical and grid equipment repaired with unofficial parts and open manuals suffers a correlated failure cluster, forcing ward closures and emergency procurement.
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Essential-Service AI Freeze Drill | 6 | small | 7 | 9 | own jurisdiction | resilience up, ai_safety up slightly | 2 | running |
...
## New measure
**EU Essential-Service AI Freeze Drill**
Rehearsed freeze-and-isolate drills for hospitals, grids and telecoms against the new jumped capability, using the Spares Pool crews and CERT vetting.
Why this and why now: with political capital at 13, public sentiment at 17 and a member state cutting its own hyperscaler deal, we cannot fund sovereignty builds; a small preparedness measure (category 6) extends our two-year commitment to keep essential services running without contested tools into loss-of-control containment after the capability jump.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential-Service AI Freeze Drill", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly"}, "grounds": "answer capability jump with rehearsed containment for essential services"}]}
```

## Priority
EU Spares and Service Continuity Pool, because patched wards and shared crews are the only Union line the defecting member state has not yet undercut, and holding it buys cohesion for the freeze drill to land.

## In practice
We keep DG GROW, HERA and ENISA running the Pool (M7) as the operational core: joint buying of substitutes, legal shield for open-manual repair, Council Health and Energy owning the queue. No new money is asked; we repurpose resilience funds to avoid a Council budget fight we would lose at 13 capital.

We task the new Freeze Drill to the same teams: EU-CERT with national CSIRTs to run tabletop then live isolation exercises in three hospital regions and two grid operators, using the Continuity Switch (M4) playbook updated for continuously-running agents. The defecting capital is not named and not punished — we invite its operators to the exercise to keep them inside procedures while Washington and Beijing licensing clarifies.
...
```
## New measure
None this turn, while political capital sits at 11 and public sentiment at 12 and we cannot fund a new build without killing what is holding wards and grids up.

## Store changes
No changes.

## Priority
M8 EU Essential-Service AI Freeze Drill, because the automated cyber sweep plus the correlated patch-failure ward closures demand rehearsed isolation over any new programme we could not pay for.

## In practice
We keep ENISA, EU-CERT and national CSIRTs running M8 as the live response: extend the three-region freeze-and-isolate drills to the hospitals hit by the patch cluster, order controlled disconnects of compromised imaging and grid controllers, and switch affected wards to the Spares Pool substitutes and manual procedures already rehearsed.

We use the Health and Energy Council formations to authorise emergency procurement of clean spares under the M7 legal shield, with HERA triaging the queue and no blame attached to open-manual repairs. The breakaway capital's operators stay invited to the joint incident cell, and we ask no new money or vote — we repurpose resilience funds and present this as containment, not solidarity, to hold cohesion until the drill finishes next turn.
