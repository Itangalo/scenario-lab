...
- characters 455-2035: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2079-3890: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5615-6085: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8896-10622: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential services running and absorbing harm as frontier capability spreads beyond recall
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Visible Benefit Adoption Push | 9 | small | 10 | 12 | own jurisdiction | public_sentiment up, eu_political_capital up slightly | 2 | running |
...
| M4 | EU Continuity Switch | 9 | large | 2 | 4 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up, public_sentiment up | 3 | finished |
| M5 | European Chokepoint Compact | 8 | large | 3 | 5 | coalition | eu_ai_sovereignty up, eu_political_capital down then up | 3 | finished |
| M6 | EU Agent Loss-of-Control Protocol | 6 | small | 4 | 5 | own jurisdiction | resilience up, ai_safety up slightly | 2 | finished |
| M7 | EU Spares and Service Continuity Pool | 6 | small | 6 | 8 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly | 2 | finished |
| M8 | EU Essential-Service AI Freeze Drill | 6 | small | 7 | 9 | own jurisdiction | resilience up, ai_safety up slightly | 2 | finished |
| M9 | EU Essential Services Hardening Corps | 6 | small | 9 | 11 | own jurisdiction | resilience up, public_sentiment up slightly | 2 | finished |
...
## New measure
**EU Visible Benefit Adoption Push**
It scales the public-sector AI deployments behind falling waiting lists and faster decisions into a branded European programme with measured outcomes published per hospital and municipality. This and now because the adoption success window finally gives a European-attributed benefit to amplify while sentiment sits at 4 and capital at 18, and without amplification the Corps alone cannot rebuild tolerance for the essential-services pledge.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Visible Benefit Adoption Push", "category": 9, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, eu_political_capital up slightly"}, "grounds": "adoption success must be attributed and scaled before anger buries it"}]}
```

## Priority
M9 EU Essential Services Hardening Corps, because keeping wards and grids running through unrecallable open weights remains the commitment core while the new adoption push needs time to land.

## In practice
We keep the Corps as the operational centre: ENISA-EU-CERT joint cell runs isolation rotations in the hit hospitals and municipalities, HERA triages clean spares under the M7 shield, funded by repurposed health-energy lines to avoid a new vote we cannot win.

We task DG DIGIT and DG SANTE with the Adoption Push as communications-plus-procurement: standardise the waiting-list and decision-time metrics from the successful sites, fund replication kits for willing regions, and require European-jurisdiction hosting labels so benefit reads as a European decision, placating hostile councils without forcing siting fights this turn.
...
It puts HERA-led wastewater and clinical sequencing sentinels, hospital isolation stockpiles, and fast medical countermeasure contracting around the essential-service sites the Corps already holds. This and now because the genome-model bio-uplift finding opens a four-turn bio window while safety sits at 17 and open weights at 64.7, and without detection the essential-services pledge fails at the first engineered pathogen.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio Detection and Medical Countermeasure Surge", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via misuse detection"}, "grounds": "bio uplift precursor must be met with detection before capability spreads"}]}
```

## Priority
EU Bio Detection and Medical Countermeasure Surge, because keeping essential services running now means surviving the bio gate that just opened rather than only the cyber playbook we drilled.

## In practice
We keep M10 running as cover: DG SANTE publishes the per-site waiting-list gains with hosted-in-Europe labels and funds replication kits only for willing mayors, avoiding a siting fight we cannot afford at capital 14 and sentiment 7.

We stand up the bio surge under HERA and ECDC with Health Council mandate, repurposing emergency health lines: sentinel sequencing in Corps hospitals, pooled PPE and isolation spares triage, and advance purchase options for countermeasures. Seconded Corps staff run it; hostile councils are not forced to join, willing regions get first stock.

We task ENISA-EU-CERT to lock down leaked paper-fallback procedures with compartmented substitution sheets, because intruders studying our drills plus eval-anomaly and labour losses mean the next shock will be attributed to us even if we hold the lights on.
