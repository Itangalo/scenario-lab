...
- characters 377-1345: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1389-3026: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3082-4150: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4213-27789: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28144-28207: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Shield −3, EU Evaluation and Incident Hub −2, priority −1 = −12
PROPOSAL BONUS: EU Evaluation and Incident Hub (cat 5, small) +2 — autumn grid intrusion and frontier opacity from last turn argue for assurance, this answers it indirectly
...
- `emergent_sovereign_hosting_offer` -- first noted turn 1, listed in 2 turn(s) so far: A major non-US cloud provider announces EU-only model hosting with data and weights legally ring-fenced in Europe, marketed as continuity insurance after the June switch-off scare.
- `emergent_health_ai_fraud_wave` -- first noted turn 1, listed in 2 turn(s) so far: Major European research hospitals disclose coordinated fraud and misdiagnosis incidents driven by AI-generated referrals and synthetic clinical data, triggering EU-wide review of medical AI validation.
