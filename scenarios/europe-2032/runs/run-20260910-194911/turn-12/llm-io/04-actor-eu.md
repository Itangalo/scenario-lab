...
- characters 452-1529: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1572-3226: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5533-5993: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9061-10033: `{{previous_actions}}` from the actor's own response from the previous turn
...
## New measure
None this turn, while we conserve near-exhausted capital and let the patching and care scale-up finish landing.

## Store changes
No changes.

## Priority
M11 EU Breakthrough Patching and Care Continuity Scale-up, because it is the only funded instrument directly holding hospitals, grids and wards open under the two-year commitment.

## In practice
We keep ENISA and the joint cyber command secondments on ward-by-ward rollout of automated patching and swarm-behaviour detection, prioritising small clinics and the two municipal utilities that reverted, paid from reprogrammed Digital Europe funds with no new Council vote to spend capital we do not have.

We keep HERA and the allocation cell on pooled procurement and licensed-batch distribution of the tailored therapies southward, with DG EMPL overtime support to hold wards open, while we monitor the new open-weight leakage through existing detection rather than opening a new front we cannot fund.
