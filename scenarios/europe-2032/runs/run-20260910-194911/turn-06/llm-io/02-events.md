...
- characters 375-1484: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1527-3568: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3624-4640: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4703-32038: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32393-32723: `{{event_history}}` from the run's own event record
...
- `emergent_grid_shedding_crisis` -- first noted turn 4, listed in 2 turn(s) so far: Prolonged walkouts and unpaid overtime cascade into an unplanned regional grid separation drill failure, forcing rolling load-shedding in the two previously breached states during winter peak.
- `emergent_insurance_repricing` -- first noted turn 4, listed in 2 turn(s) so far: A major European insurer reprices data-centre and AI-liability coverage after the investment freeze, sharply raising premiums for municipalities and hospitals running uncertified AI systems.
...
- Turn 3 (3 turn(s) ago): ai_investment_collapse, eval_anomaly_reports, safety_breakthrough
- Turn 4 (2 turn(s) ago): opaque_reasoning, adoption_success
- Turn 5 (1 turn(s) ago): capability_jump, safety_breakthrough, eu_frontier_access_denied, election_consolidation
