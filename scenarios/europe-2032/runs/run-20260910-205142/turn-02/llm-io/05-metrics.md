...
- characters 13251-14656: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14699-16906: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16962-17679: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Clinical Validation Network | 2 |
...
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated physical protests and grid-connection lawsuits stall several large EU data-centre builds, forcing a siting moratorium debate in one member state.
...
## Metrics
```json
{"ai_capability": 54.5, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 37.0, "eu_ai_sovereignty": 18.0, "eu_political_capital": 25.0, "public_sentiment": 41.0}
```
