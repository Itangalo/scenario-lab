...
- characters 16235-16802: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated physical protests and grid-connection lawsuits stall several large EU data-centre builds, forcing a siting moratorium debate in one member state.
- `emergent_eu_biotech_breakthrough` -- first noted turn 1, listed in 1 turn(s) so far: Major European research hospitals report AI-assisted antibiotic candidate entering clinical validation after screening millions of compounds in weeks, spurring public debate over EU biotech leadership.
...
PROPOSAL BONUS: Grid and Port Cyber Shield (cat 6, large) +3 — autumn swarm intrusion across EU grids landed this turn and this answers it directly
LEGITIMACY LENDS: capital 39, sentiment 40 -> +1
SOVEREIGNTY: 22 last turn, no finish, in flight +0, supply_chain_coercion t1 −2, capability rose 1.5 no charge = 20
