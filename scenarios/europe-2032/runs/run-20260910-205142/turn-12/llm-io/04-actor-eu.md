...
- characters 455-2346: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2389-4395: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5364-5865: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9220-11201: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_eu_open_medical_hosting_offer (emergent event):** A major non-US open-weight lab offers EU hospitals a clinically validated medical model for on-premise EU hosting, easing therapy procurement standoff without US licences.
...
- `two_year_commitment` (commitment): Rebuild essential services to run on resilient EU-controlled capacity that survives foreign model denial and supply disruption
...
| M4 | EU Clinical Validation Network | 9 | small | 2 | 5 | own jurisdiction | public_sentiment up, resilience up slightly via health/biotech capacity | 2 | finished |
| M5 | EU Common Hosting Terms | 8 | small | 3 | 4 | own jurisdiction | eu_political_capital up slightly via cohesion, eu_ai_sovereignty up slightly via leverage | 2 | finished |
| M6 | EU Bio Shield Detection Upgrade | 6 | small | 4 | 6 | own jurisdiction | resilience up via biosecurity detection and response | 2 | finished |
| M7 | Allied Assurance and Supply Pact | 8 | small | 6 | 8 | coalition plus US | eu_ai_sovereignty up slightly, eu_political_capital up slightly via cohesion | 2 | finished |
| M8 | Taiwan-Contingency Continuity Reserve | 6 | small | 7 | 9 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly via assured fallback | 2 | finished |
| M9 | Deployment Accountability and Redress Audit | 1 | small | 8 | 9 | own jurisdiction | eu_political_capital up slightly via credibility, resilience up slightly via trust, ai_safety up slightly | 2 | finished |
| M10 | Essential Services Continuity Shield | 6 | small | 9 | 10 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly via EU-controlled hosting, eu_political_capital up slightly if continuity holds | 2 | finished |
| M11 | EU Displacement Transition Facility | 7 | small | 10 | 11 | own jurisdiction | public_sentiment up, eu_political_capital up via cohesion, resilience up slightly via social absorption | 2 | finished |
| M12 | EU Sovereign Fallback Stack | 6 | small | 11 | 12 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly via EU-controlled hosting, eu_political_capital up slightly if continuity holds | 2 | finished |
...
Stands up EU-controlled inference and essential-service workloads on open-weight models hosted in EU clouds and grid-ready shells, hardened and rationed for hospitals, ministries and telecom under US frontier denial and Strait chip freeze.
Category 6 Preparedness and resilience, small and fast, because the two-year commitment to survive foreign model denial must now be operated with what is already on EU soil, not waited-for machines.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Sovereign Fallback Stack", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, eu_ai_sovereignty up slightly via EU-controlled hosting, eu_political_capital up slightly if continuity holds"}, "grounds": "to operate essential services after US frontier denial amid chip blockade"}]}
```

## Priority
EU Sovereign Fallback Stack, because with the leading model cut off at short notice it is the direct instrument of our two-year commitment to survive foreign denial.

## In practice
We execute through HERA, ENISA and the EU Health Data Space emergency mode: load-shedding protocols from M10 become standing allocation, EU clouds get priority power and procurement, and open-weight models at ~62 capability are validated for clinical, administrative and network-ops use with guardrails and incident logging. Funding is repurposed CEF-Digital and EU4Health emergency lines via ECOFIN, no new cash call.

We hold social and diplomatic flanks steady: the Displacement Facility continues via national employment services to contain graduate anger, and we use the Anti-Coercion Instrument inventory quietly with Tokyo and Seoul for maintenance and spares — no public Washington confrontation that spends the 25 capital we have left. Compassionate-use exemptions for tailored therapies are widened to defuse oncologists while denial lasts.
...
```
## New measure
**EU Validated Medical Hosting**
Takes up the non-US open-weight lab's offer to deploy its clinically validated medical model on-premise in EU hospitals, restoring tailored-therapy and triage capacity without US licences.
This is Category 9 Diffusion, adoption and public trust work, and it is why now: a medical breakthrough the Union cannot otherwise touch meets an on-EU-soil hosting path that eases the procurement standoff and turns dependence into visible relief.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Validated Medical Hosting", "category": 9, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, resilience up via restored clinical capacity"}, "grounds": "to take up on-premise validated medical model offer amid US denial"}]}
```

## Priority
EU Validated Medical Hosting, because it is the only instrument that converts the breakthrough into EU-controlled care within our two-year commitment to survive foreign model denial.

## In practice
We execute through HERA and the EU Health Data Space with EMA validation: emergency procurement for on-premise deployment in university hospitals first, EU4Health funding for inference nodes on existing EU clouds, with logging and double-check protocols carried over from the Fallback Stack to handle hallucination risk.

We hold the wider flanks without new instruments: the Displacement Facility keeps paying via national employment services, and the Anti-Coercion inventory work with Tokyo and Seoul continues quietly for spares while Taiwan insurance pressure rises. No public confrontation with Washington; oncologists get a lawful EU-hosted channel to replace closed compassionate-use routes.
