...
- characters 454-1485: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1528-3649: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4003-4466: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6595-8746: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_relay_procurement_fight (emergent event):** US volume-licence controls on grid relays and compute spark an EU-level procurement lawsuit and emergency stockpile order, temporarily easing relay supply but deepening transatlantic tech-sovereignty dispute.
...
| M5 | EU Trusted Components and Supply-Chain Leverage Pact | 8 | small | 3 | 6 | own jurisdiction and middle-power coalition | resilience up, eu_ai_sovereignty stabilise | 2 | running |
...
```
## New measure
None this turn, while political capital sits at 24 and three measures already burn 8 per turn against hostile hearings and grid queues.

## Store changes
No changes.

## Priority
M5 EU Trusted Components and Supply-Chain Leverage Pact, because the US volume-licence fight over relays and compute makes the joint stockpile-plus-leverage track the only instrument that both keeps segmentation crews supplied and turns lithography leverage into licence relief.

## In practice
We run M5 through DG GROW/ENISA batch-testing and the emergency stockpile order now under litigation, using EIB recall funds via TSOs to front certified lots to the three counterfeit-hit countries first so segmentation restarts before winter. Energy and Competitiveness Councils co-own the file to hold off interior-ministry blame over co-financing and to lodge the EU-level procurement challenge without breaking volume-licence talks.

We keep M1 Gigafactories and M2 tech sovereignty package on life-support via DG ENER pre-permits and DG COMP clearance, redirecting near-term effort from new sites to defending existing compute allocations and grid connections, and we task Trade/DG TRADE with pooling ASML and equipment export screening under the Anti-Coercion Instrument to bargain US relay/compute licences against allied chip allocations. Municipal continuity teams continue offline playbooks as the visible continuity voters still see.
