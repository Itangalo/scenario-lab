...
- characters 455-1373: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1417-3503: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5135-5612: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8555-10732: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_care_rollback_standoff (emergent event):** Municipal hospital federations publish an open letter refusing to reconnect elective-care systems to the ENISA containment hub until liability for rollback-ordered cancellations is clarified, stalling mutual-aid drills.
...
- `two_year_commitment` (commitment): A Europe that keeps essential services running through frontier failures on infrastructure it controls
...
| M3 | EU Grid and Essential Services Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up materially, ai_safety spillover via detection | 3 | finished |
| M4 | EU Municipal Services Continuity Pact | 6 | small | 2 | 4 | own jurisdiction | resilience up modestly, public_sentiment stabilise | 2 | finished |
| M5 | EU Trusted Components and Supply-Chain Leverage Pact | 8 | small | 3 | 6 | own jurisdiction and middle-power coalition | resilience up, eu_ai_sovereignty stabilise | 2 | finished |
| M6 | EU Joint Threat Response Accession | 8 | small | 5 | 6 | own jurisdiction and middle-power coalition | resilience up materially, ai_safety spillover | 2 | finished |
| M7 | EU Critical Services Restoration Surge | 6 | small | 6 | 7 | own jurisdiction | resilience up materially, public_sentiment stabilise | 2 | finished |
| M8 | EU Automated Patch Shield Rollout | 6 | small | 7 | 8 | own jurisdiction | resilience up materially, ai_safety spillover | 2 | finished |
| M9 | EU Essential Services Hardening Mandate | 6 | small | 8 | 9 | own jurisdiction | resilience up materially, public_sentiment stabilise | 2 | finished |
| M10 | EU Frontier Failure Containment Pact | 6 | small | 9 | 10 | own jurisdiction | resilience up materially, ai_safety spillover via containment | 2 | finished |
| M11 | EU Clean Compute Siting Compact | 4 | small | 10 | 11 | own jurisdiction | eu_ai_sovereignty stabilise then up, public_sentiment stabilise | 2 | finished |
...
## New measure
**EU Clean Compute Siting Compact**
Unblocks gigafactory and data-centre siting frozen by coordinated municipal bans through an EU fast-track permit with grid priority and binding local benefit-sharing, keeping sovereign compute buildable.
Category 4 Sovereignty and industrial capacity, small, because with capital at 28 and sentiment at 23 we cannot fund a new frontier build this turn but we must stop the siting freeze from killing the fenced plots and pooled screening leverage while answering the new major cyber shock with the finished containment stock.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Clean Compute Siting Compact", "category": 4, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty stabilise then up, public_sentiment stabilise"}, "grounds": "to break municipal siting freeze blocking sovereign compute"}]}
```

## Priority
EU Clean Compute Siting Compact, because without buildable sites sovereignty stays at 23 and the two-year commitment to run services on infrastructure it controls fails regardless of containment readiness.

## In practice
We task DG ENER and DG GROW with an emergency siting regulation under Article 122/194 TFEU via the Energy and Competitiveness Councils, designating the fenced gigafactory plots and two replacement sites as Projects of Common Interest with 12-month permits, grid-connection priority, and a community dividend from reprogrammed InvestAI envelopes. Anti-AI mayors are offered noise, water and energy-price guarantees plus local hiring quotas in exchange for lifting freezes.

We keep the finished Frontier Failure Containment Pact live through ENISA-ordered isolation drills and automated patching via NIS2 implementing acts, fronted by mayors and hospital operators to limit fallout from the new ransomware sweep, and we use continuity access to the containment hub to pull the defecting member state back to pooled screening. We message the productivity evidence — gains without job loss — to stabilise sentiment while holding US talks frozen.
...
```
## New measure
**EU Bio-Care Continuity Pact**
Puts hospital elective systems back on the ENISA hub by clarifying liability for rollback-ordered cancellations while plugging EU biosurveillance into the joint sample-sharing pact triggered by the genome-model bio-uplift finding.
This now because the bio gate is open for four turns and the care-rollback standoff stalls the drills our two-year commitment to keep essential services running depends on, and the joint threat invitation is a resilience gain we cannot build alone at capital 29 and sentiment 19.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Care Continuity Pact", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction and middle-power coalition", "targeted_effect": "resilience up materially, ai_safety spillover via detection"}, "grounds": "to resolve hospital rollback standoff and join biosurveillance pact after bio-uplift signal"}]}
```

## Priority
EU Bio-Care Continuity Pact, because without liability clarity and drills restored plus outside biosurveillance cover, containment holds on paper while hospitals stay off the hub and bio-capable tooling spreads.

## In practice
We table an EPSCO/Health Council liability clarification under the Cross-Border Healthcare and NIS2 implementing acts: rollback orders from ENISA create EU-solidarity compensation for cancelled elective care, co-funded from EU4Health and the finished Restoration Surge envelope, administered by HERA. Hospital federations rejoin mutual-aid drills in exchange; mayors who lifted siting freezes get priority reconnection crews.

We task HERA, ECDC and ENISA to sign the biosurveillance pact with binding sample-sharing and standing investigation mandate, pooling attribution telemetry from the winter ransomware sweep. DG TRADE aligns export-control licensing to keep allied volume licences while rationing high-risk compute, without reopening the frozen US talks or forcing the defecting cloud deal back to pooled screening now.
