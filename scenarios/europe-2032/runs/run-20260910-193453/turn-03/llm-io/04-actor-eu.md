...
- characters 455-1559: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1603-3897: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4853-5316: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7257-9235: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_grid_component_fraud (emergent event):** Relay and breaker-component shortages trigger an EU-level secondary market with counterfeit parts entering transmission supply chains, forcing recalls and delaying segmentation work.
...
| M4 | EU Municipal Services Continuity Pact | 6 | small | 2 | 4 | own jurisdiction | resilience up modestly, public_sentiment stabilise | 2 | running |
...
## New measure
**EU Municipal Services Continuity Pact**
Stands up exercised backups, offline fallback procedures and shared clean-image recovery for municipal payment, appointment and update-channel systems hit by the automated Christmas strike.
This is Preparedness and resilience work, and why now is the cyber_major_incident: defenders are visibly behind and attribution will take months, so the next 6 months must buy degrade-not-stop continuity while the larger Grid Shield builds.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Services Continuity Pact", "category": 6, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "resilience up modestly, public_sentiment stabilise"}, "grounds": "answer Christmas automated strike on public services"}]}
```

## Priority
EU Grid and Essential Services Shield, because staged breaker access plus a live public-services strike outranks compute build-out and the new continuity work this turn.

## In practice
We push the Shield through ENISA and ACER with emergency NIS2 Article 21 segmentation orders for transmission operators, ports and water, funded by 70% CEF-EIB co-financing for relay replacement and credential vaulting; Energy Council owns it, and we buy off finance/interior ministers with front-loaded EU share for the two hit states and a mutual-aid roster for the rest.

We keep InvestAI Gigafactories and the tech sovereignty package alive without new money: DG ENER fast-tracks grid-connection pre-permits for the four to five sites, DG COMP clears state-aid, and we task the Anti-Coercion screening of ASML leverage to negotiate US volume licences, while offering protest-hit municipalities energy-price rebates and local co-ownership to unblock permits. The new Continuity Pact runs via DIGIT and ENISA joint teams deploying offline paper-to-digital playbooks and clean update-channel images to affected municipalities.
...
```
## New measure
**EU Trusted Components and Supply-Chain Leverage Pact**
Stands up EU-wide certification and recall of transmission relays and breakers to purge counterfeits, while pooling ASML and upstream chokepoints into a joint bargaining position on chips and volume licences.
This is International coordination and leverage work with a resilience core, and why now is the Taiwan blockade plus counterfeit-driven segmentation delays: compute supply is disrupted for years and grid hardening is stalling on fakes, so upstream leverage is both the most valuable thing the Union holds and the instrument to unblock relays.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trusted Components and Supply-Chain Leverage Pact", "category": 8, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction and middle-power coalition", "targeted_effect": "resilience up, eu_ai_sovereignty stabilise"}, "grounds": "answer Taiwan blockade and counterfeit relay crisis"}]}
```

## Priority
EU Trusted Components and Supply-Chain Leverage Pact, because a multi-year chip cutoff plus fake parts in the grid outranks even municipal continuity this turn and directly serves the harden-and-sovereign commitment.

## In practice
We task DG GROW and ENISA with an emergency trusted-components regime under NIS2 and the Chips Act: mandatory batch testing, EU foundry second-sourcing for relays, and EIB-funded recall replacement, run through ACER and transmission operators with front-loaded aid to keep segmentation moving. Energy and Competitiveness Councils co-own it to override interior objections.

We keep InvestAI Gigafactories and the tech sovereignty package alive via DG ENER grid-connection pre-permits and DG COMP clearance, but shift their near-term focus to securing existing compute and power rather than new sites, and we use the Anti-Coercion Instrument screening of lithography and equipment exports to negotiate US volume licences and allied chip allocations. The Municipal Continuity Pact continues via DIGIT joint teams deploying offline playbooks while the Shield audits feed the recall list.
