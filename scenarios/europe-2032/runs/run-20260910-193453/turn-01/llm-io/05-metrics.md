...
- characters 106-12890: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12933-13125: `{{metrics_json}}` from the run's live metric values
- characters 13241-16190: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 16246-16862: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M3 | EU Grid and Essential Services Shield | 3 |
...
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated protests and municipal permit blocks stall at least one major EU data-centre expansion in late 2026, turning energy costs and the June US switch-off order into a sovereignty debate.
- `emergent_screening_norm_shift` -- first noted turn 1, listed in 1 turn(s) so far: Major EU scientific publishers and preprint servers adopt mandatory AI-design screening declarations for genome-synthesis papers after the phage demonstrations, creating a de facto disclosure norm ahead of regulation.
...
## Notepad
PORTFOLIO CHARGE t1: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Grid and Essential Services Shield −3, priority −1 = −10
PROPOSAL BONUS: EU Grid and Essential Services Shield (cat 6, large) +4 — cyber_test_shot and cyber_major_incident landed this turn and this answers them directly
LEGITIMACY LENDS: capital 36, sentiment 36 -> none (capital not below sentiment)
SOVEREIGNTY: 22 last turn, no finishing measure, in-flight +0, export_control_escalation t1 −2, capability rose 1.0 no −1 = 20
Emerging: datacentre backlash faint municipal pushback over power costs; screening norm shift faint publisher discussion after phage news.
