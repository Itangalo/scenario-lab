...
- characters 106-13204: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13247-13438: `{{metrics_json}}` from the run's live metric values
- characters 13531-14223: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14266-16455: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16511-17293: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M11 | EU Clean Compute Siting Compact | 2 |
...
- `emergent_care_rollback_standoff` -- first noted turn 10, listed in 1 turn(s) so far: Municipal hospital federations publish an open letter refusing to reconnect elective-care systems to the ENISA containment hub until liability for rollback-ordered cancellations is clarified, stalling mutual-aid drills.
...
## Notepad
PORTFOLIO CHARGE t10: EU Clean Compute Siting Compact −2, priority −1 = −3

PROPOSAL BONUS: EU Clean Compute Siting Compact (cat 4, small) +3 — emergent_municipal_siting_freeze fired this turn severe and this answers it directly, cyber_major_incident adds context

LEGITIMACY LENDS: capital 29, sentiment 19 -> none, capital higher

SOVEREIGNTY: 23 last turn, no cat4 finishes, no cat4 in flight payout in proposal turn +0, emergent_municipal_siting_freeze t10 −1, capability rose 0.5 −0 = 22
