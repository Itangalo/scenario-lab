...
- characters 105-13635: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13678-13870: `{{metrics_json}}` from the run's live metric values
- characters 13963-15047: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15090-17532: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17588-18385: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Critical Services Restoration Surge | 2 |
...
- `emergent_nonus_frontier_claim` -- first noted turn 6, listed in 1 turn(s) so far: With the incoming US administration signalling data-centre moratoriums and AI curbs, a non-US frontier lab publicly demonstrates a system trained largely outside US compute, narrowing the perceived US lead.
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election won by retrenchment candidate, takes office turn 6"}, {"op": "update", "table": "measures", "id": "M7", "fields": {"finish_turn": 7}, "grounds": "named priority"}]}
```
```
