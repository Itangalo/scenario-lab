...
- characters 376-1995: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2038-4556: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4612-5747: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5810-33358: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 33713-33912: `{{event_history}}` from the run's own event record
...
- `emergent_ot_supply_disclosure` -- first noted turn 2, listed in 2 turn(s) so far: A major industrial equipment vendor publicly discloses that model-generated intrusion tooling adapted from open weights is embedded in maintenance channels used by European grid and port operators, forcing unplanned outages for remediation.
- `emergent_cyber_insurance_crunch` -- first noted turn 2, listed in 2 turn(s) so far: A major European insurer reprices cyber cover for essential operators after the autumn pre-positioning, sharply raising premiums or withdrawing cover for unhardened municipal utilities and hospitals.
...
- Turn 2 (2 turn(s) ago): eval_anomaly_reports, knowledge_work_augmented
- Turn 3 (1 turn(s) ago): capability_jump, eval_anomaly_reports, labour_displacement
