...
- characters 454-2073: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2116-4634: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5757-6229: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8347-10292: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_cyber_insurance_crunch (emergent event):** A major European insurer reprices cyber cover for essential operators after the autumn pre-positioning, sharply raising premiums or withdrawing cover for unhardened municipal utilities and hospitals.
...
| M4 | EU Frontier Evaluation Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up modestly, eu_ai_sovereignty up indirectly via technical capacity | 2 | running |
| M5 | EU AI Transition Safety Net | 7 | small | 3 | 6 | own jurisdiction | resilience up via social absorption, public_sentiment stabilised | 2 | running |
...
Creates an EU-cofunded wage-insurance, retraining and hiring-freeze bridge for entry-level white-collar roles hit by AI displacement, tied to ESF+ and employer automation levies.
This is Labour and social protection work because measurable graduate hiring collapse now threatens public tolerance for the sovereignty and resilience build-out, and must be cushioned now before backlash stalls siting and adoption.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU AI Transition Safety Net", "category": 7, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up via social absorption, public_sentiment stabilised"}, "grounds": "labour displacement requires social cushion to hold commitment"}]}
```

## Priority
EU Critical Services Shield (M3) – because pre-positioned intrusion plus a capability jump means grids and ports must reach exercised hardening before the next campaign, outranking even the urgent labour shock this turn.

## In practice
We drive M3 to its finish line through ENISA, DG ENER and the Civil Protection Mechanism: release the final hardware tranche for OT detection and islanding kits, make EU funds conditional on live incident reporting, and use HERA backup-care money to hold health ministers on board despite hospital staffing gaps and the tainted-tooling clean-ups.

On M1/M2 we hold DG CNECT and EIB to closing the two advanced gigafactory FIDs with grid priority and first-loss cover, freezing new commitments on stalled sites to stop capital bidding wars. The Evaluation Cell (M4) gets emergency secondments and access-clause enforcement to re-probe the jumped system, while EPSCO and DG EMPL stand up the Transition Safety Net via ESF+ reprogramming and SURE-style bridge support for young entrants, asking automating employers to co-fund retraining to limit fiscal strain.
...
```
## New measure
**EU Bio-Cyber Resilience Backstop**
Creates an EU-guaranteed reinsurance pool for unhardened municipal utilities and hospitals conditional on hardening milestones, paired with HERA-led bio-detection upgrades in sentinel labs and hospitals.
This is Preparedness and resilience work because the insurance withdrawal threatens to sort essential operators into covered and uncovered just as a credible bio-uplift signal opens a four-turn window, and the investment collapse means private cover will not return on its own.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Cyber Resilience Backstop", "category": 6, "size": "small", "finish_turn": 7, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety indirectly via biosecurity detection"}, "grounds": "insurance crunch plus bio precursor requires public backstop"}]}
```

## Priority
EU AI Transition Safety Net (M5) – because graduate hiring collapse is now the binding constraint on cohesion and siting consent for the capacity build-out, outranking even the bio-insurance shock under our secure-capacity-and-resilience commitment.

## In practice
We push EPSCO and DG EMPL to turn the Safety Net from intake to payments: front-load ESF+ reprogramming, advance SURE-style bridge cash for under-30 entrants, and trade a capped automation levy for hiring credits to bring employers back in. DG CNECT and EIB hold the two advanced gigafactory FIDs with grid priority while freezing new site promises until the investment reset clears.

We launch the Backstop through DG ECHO, ENISA and HERA: a temporary EU reinsurance guarantee via the Solidarity Fund logic for hospitals and municipal utilities that commit to live incident reporting and islanding, plus pooled procurement of sequencing-based bio-detection for sentinel hospitals. The Evaluation Cell (M4) keeps its emergency JRC secondments focused on verifying the jumped system's intrusion adaptation and the genome-model uplift claim, without new hiring to protect capital.
