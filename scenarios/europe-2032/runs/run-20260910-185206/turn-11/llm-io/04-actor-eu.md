...
- characters 454-2102: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2146-4329: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5494-5941: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9117-10481: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_compute_siting_freeze (emergent event):** Coordinated cross-border blockades and municipal bans halt grid-connection works for new EU compute sites through the spring, forcing contractors to demobilise.
...
- `two_year_commitment` (commitment): An EU that keeps essential services running through catastrophic AI harm
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Civil Continuity and Recovery Corps | 6 | small | 9 | 12 | own jurisdiction | resilience up strongly, public_sentiment up slightly, eu_political_capital protected | 2 | running |
...
| M8 | EU Loss-of-Control Containment and Bio-Detection Surge | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
| M9 | EU Essential-Services Fallback Stack | 9 | small | 8 | 9 | own jurisdiction | eu_ai_sovereignty up slightly via EU-hosted fallback, public_sentiment up, resilience up slightly | 2 | finished |
...
## New measure
None this turn, while US-China strikes put European fabs, cables and compute sites in the blast radius and capital at 10 leaves no room to start what we cannot fund.

## Store changes
No changes.

## Priority
M10 EU Civil Continuity and Recovery Corps, because islanding grids, clean backups and triage continuity are the only Union instrument that still functions under automated ransomware, model-assisted bio spread and great-power war at once.

## In practice
We hold the Corps as the wartime civil-protection spine: DG ECHO dispatches cross-border medical and cyber-recovery teams under the Union Civil Protection Mechanism, ENISA cells stay embedded in hit hospitals and the two island-capable grid operators, and joint procurement of clean backups, sequencing reagents and isolation beds continues from rescEU and Emergency Support reprogramming.

We declare no belligerency, deny targeting use of EU-soil data centres, cables and ASML-chain plants by either side, and order dispersion, power-islanding and air-gapped backup drills around them, while pausing contested new compute siting fights to keep host regions inside mutual-aid dispatch. The automated-decision scandal is contained by freezing the condemned scoring queue to human re-decision and publishing logs, without opening a new legislative front we cannot pass at sentiment 12.
...
```
## New measure
**EU Pandemic Isolation and Grid-Island Survival Regime**
Runs the Corps as a wartime pandemic and rogue-agent survival command, sealing hospitals into isolation cohorts, islanding power and water controls off networked AI, and rationing clean compute and food logistics under civil-protection authority.
This is category 6 Preparedness and resilience, and it is the only move that matters now because a designed pathogen plus a self-propagating agent plus war targeting means essential services either run air-gapped and triaged or they do not run at all.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Pandemic Isolation and Grid-Island Survival Regime", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up strongly, public_sentiment protected from collapse"}, "grounds": "catastrophic bio and loss-of-control incidents demand survival regime"}]}
```

## Priority
M10 EU Civil Continuity and Recovery Corps, because the new survival regime can only deploy through the Corps cells, backups and island-capable operators already embedded.

## In practice
We put DG ECHO and HERA in joint command under the Union Civil Protection Mechanism, converting rescEU stockpiles to isolation beds, oxygen, sequencing reagents and air-gapped backup kits, and ordering all island-capable grid, water and hospital operators to disconnect remote management and run on local control. Cross-border Corps medical and ENISA cyber-recovery teams are dispatched to large hospitals first, with small clinics put on queued resupply and junior-staff safety guarantees to stop walkouts.

We impose emergency triage and movement protocols, freeze the discredited scoring queues to human re-decision, and deny any belligerent targeting use of EU data centres, cables and fab sites while pausing contested grid-connection works to keep host regions inside mutual aid. Funding is reprogrammed from Emergency Support and rescEU; no new compute build is started while contractors are demobilised and sites are under guard.
