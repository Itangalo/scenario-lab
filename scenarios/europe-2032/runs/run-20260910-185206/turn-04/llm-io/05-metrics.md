...
- characters 105-13550: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13593-13785: `{{metrics_json}}` from the run's live metric values
- characters 13878-15848: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15891-18116: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18172-19053: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M5 | EU Displaced Worker Transition Guarantee | 2 |
| M6 | EU Assurance Uptake and Frontier Evaluation Cell | 2 |
...
- `emergent_anti_ai_site_blockade` -- first noted turn 3, listed in 2 turn(s) so far: Coordinated local protests blockade or vandalise a planned Gigafactory or data-centre site, halting construction for weeks amid job-loss and blackout anger.
...
## Metrics
```json
{"ai_capability": 64.0, "openweight_capability": 57.5, "ai_safety": 33.0, "resilience": 40.0, "eu_ai_sovereignty": 16.0, "eu_political_capital": 16.0, "public_sentiment": 26.0}
```
...
## Notepad
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Infrastructure Cyber Shield −3, EU Displaced Worker Transition Guarantee −2, EU Assurance Uptake and Frontier Evaluation Cell −2, priority −1 = −14
PROPOSAL BONUS: EU Assurance Uptake and Frontier Evaluation Cell (cat 5, small) +2 — answers leaked anomaly uncertainty and builds on deployable assurance advance this turn, small size and own-jurisdiction only
