...
- characters 106-13137: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13180-13372: `{{metrics_json}}` from the run's live metric values
- characters 13465-15184: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15228-17518: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17574-18420: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_anti_ai_site_blockade` -- first noted turn 3, listed in 1 turn(s) so far: Coordinated local protests blockade or vandalise a planned Gigafactory or data-centre site, halting construction for weeks amid job-loss and blackout anger.
...
## Metrics
```json
{"ai_capability": 61.0, "openweight_capability": 54.0, "ai_safety": 28.0, "resilience": 34.0, "eu_ai_sovereignty": 17.0, "eu_political_capital": 21.0, "public_sentiment": 24.0}
```
## Narrative
### Holding the line
