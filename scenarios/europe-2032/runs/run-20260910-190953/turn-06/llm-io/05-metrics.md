...
- characters 105-13587: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13630-13822: `{{metrics_json}}` from the run's live metric values
- characters 13915-16013: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 16056-18155: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18211-19040: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Talent Homecoming and Chip Leverage Pact | 2 |
...
- `emergent_local_ai_moratorium_wave` -- first noted turn 5, listed in 2 turn(s) so far: Coordinated municipal moratoriums on new data-centre siting and hospital AI expansion spread across several member states after the February outage, delaying patch rollout and gigafactory site approvals.
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election winner was retrenchment; posture takes effect turn 6"}]}
```
```
