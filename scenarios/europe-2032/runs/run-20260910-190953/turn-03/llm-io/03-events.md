...
- characters 1713-4187: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4243-5347: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5410-28031: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28386-28494: `{{event_history}}` from the run's own event record
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated local opposition blocks or delays at least one major EU data-centre or supercomputer expansion on energy-price and water-use grounds during autumn 2026.
- `emergent_synthesis_screening_push` -- first noted turn 1, listed in 2 turn(s) so far: Major DNA synthesis providers announce a voluntary screening upgrade for genome-model-designed sequences after the phage demonstrations, but coverage remains partial and non-binding outside the US and EU.
...
- Turn 2 (1 turn(s) ago): ai_investment_collapse
