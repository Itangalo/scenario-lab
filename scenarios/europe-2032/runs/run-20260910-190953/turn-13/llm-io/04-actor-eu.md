...
- characters 455-2047: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2091-4125: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5972-6450: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9708-11796: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Rebuild trustworthy AI in Europe that absorbs shocks without breaking essential services or livelihoods
...
| M3 | EU Grid and Essential Services Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up materially, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Transition Safety Net Pilot | 7 | small | 2 | 5 | own jurisdiction | resilience up moderately, public_sentiment up slightly | 2 | finished |
| M5 | EU Trusted Public Services Scale-Up | 9 | small | 3 | 6 | own jurisdiction | public_sentiment up moderately, eu_political_capital up slightly | 2 | finished |
| M6 | EU Automated Defence Rollout | 6 | small | 4 | 6 | own jurisdiction | resilience up materially, ai_safety up slightly | 2 | finished |
| M7 | EU Talent Homecoming and Chip Leverage Pact | 4 | small | 5 | 8 | own jurisdiction | eu_ai_sovereignty up materially | 2 | finished |
| M8 | EU Welfare-Policing AI Redress and Logging Audit | 2 | small | 8 | 9 | own jurisdiction | public_sentiment up moderately, eu_political_capital up slightly, ai_safety up slightly via oversight | 2 | finished |
| M9 | EU Displaced Workers Transition and Essential Services Continuity Fund | 7 | small | 9 | 10 | own jurisdiction | resilience up moderately, public_sentiment up moderately, eu_political_capital up slightly | 2 | finished |
| M10 | EU Essential Services Insurance Backstop and Supply-Chain Leverage Hold | 6 | small | 10 | 11 | own jurisdiction | resilience up materially, eu_political_capital up slightly, eu_ai_sovereignty up slightly | 2 | finished |
| M11 | EU Jump Response Safety Brake | 1 | small | 11 | 12 | own jurisdiction | ai_safety up slightly, resilience up slightly | 2 | finished |
| M12 | EU Bridge to the US-China Safety Floor | 8 | small | 12 | 13 | US, China, coalition | ai_safety up materially, eu_ai_sovereignty up slightly, eu_political_capital up slightly | 2 | finished |
...
## New measure
**EU Bridge to the US-China Safety Floor**
Seek formal EU accession/observer-plus status to the US-China weights-security and bio-design agreement with mutual recognition of EU evaluations and a joint verification cell, turning the bilateral floor into a shared assurance regime.
This and why now because the bilateral deal is the only force slowing capability growth and making safety cooperation checkable, and with safety at 1.0 and capital at 23.0 the Union cannot rebuild assurance alone — it must plug its evaluation hold and incident pool into the one verification channel that now matters.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bridge to the US-China Safety Floor", "category": 8, "size": "small", "finish_turn": 14, "applies_to": "US, China, coalition", "targeted_effect": "ai_safety up materially, eu_ai_sovereignty up slightly, eu_political_capital up slightly"}, "grounds": "US-China agreement opens verifiable safety cooperation window"}]}
```

## Priority
EU Bridge to the US-China Safety Floor, because without a seat in the only working risk-reduction regime the emergency brake stays unilateral paper while the new agent generation diffuses.

## In practice
We mandate the EEAS and DG CNECT/AI Office jointly under the Common Foreign and Security Policy plus AI Act Article 111 systemic-risk base to seek third-party accession: offer the EU incident pool and staged-rollout dossiers as the EU contribution, and ask for reciprocal evaluator access and weights-security audit participation. Foreign Affairs Council gives the negotiating mandate; Competitiveness Council holds the internal hold steady.

We sell it domestically as hold-through-cooperation, not surrender: mayors get that reinsurance conditionality eases once common logging is mutually recognised, the Dutch get no new fund or Act, and Washington/Beijing get what they lack — a neutral verifier and the upstream supply-chain leverage only the EU can bolt on. Seconded ENISA and JRC evaluators staff the joint cell from existing lines.
...
```
## New measure
**EU Bio-Shield Detection and Continuity Surge**
A preparedness and resilience surge that puts rapid bio-detection in hospitals and wastewater, stockpiled countermeasures, and exercised continuity for clinics and registries under the genome-model uplift window.
This and why now because the contested genome-model uplift opens a four-turn bio gate while safety is at 2.0 and small clinics and eastern registries remain exposed, and with capital at 23.0 only a small, existing-powers resilience build can land before the next signal.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Shield Detection and Continuity Surge", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, ai_safety up slightly via detection"}, "grounds": "bio uplift precursor opens four-turn gate with exposed clinics"}]}
```

## Priority
EU Bio-Shield Detection and Continuity Surge, because it is the only in-flight build that directly serves the two-year commitment to absorb shocks without breaking essential services under the bio precursor.

## In practice
We act under the Cross-Border Health Threats Regulation and Civil Protection Mechanism, not a new Act: HERA procures sequenced wastewater and clinical screening kits for sentinel hospitals, ECDC runs the shared signal pool with the incident pool from the Bridge, and ENISA extends the Shield playbooks to registry failover. Health Council mandates it; Budget hawks accept it because it repurposes HERA and UCPM lines and the Treasury reinsurance conditionality.

On the research breakthrough we claim what is ours: the Grenoble JRC–ILL team with AI-assisted neutron analysis closed the solid-state sulfide electrolyte stability problem, a landmark inside materials science that makes a manufacturable high-density cell credible within two turns. We tie first pilot-line offtake to eastern municipalities and retraining intake, to show benefit where sabotage and joblessness fused. Seconded evaluators from the Bridge cell keep the staged-rollout hold steady while the Surge exercises run, and mayors get co-pay relief linked to joining detection and logging.
