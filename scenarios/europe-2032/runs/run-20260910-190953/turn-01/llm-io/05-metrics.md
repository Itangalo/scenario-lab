...
- characters 16246-16831: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated local opposition blocks or delays at least one major EU data-centre or supercomputer expansion on energy-price and water-use grounds during autumn 2026.
- `emergent_synthesis_screening_push` -- first noted turn 1, listed in 1 turn(s) so far: Major DNA synthesis providers announce a voluntary screening upgrade for genome-model-designed sequences after the phage demonstrations, but coverage remains partial and non-binding outside the US and EU.
...
## Metrics
```json
{"ai_capability": 53.5, "openweight_capability": 46.0, "ai_safety": 32.0, "resilience": 38.0, "eu_ai_sovereignty": 22.0, "eu_political_capital": 37.0, "public_sentiment": 41.0}
```
## Narrative
