...
- characters 105-13077: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13120-13312: `{{metrics_json}}` from the run's live metric values
- characters 13405-14410: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14453-16534: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16590-17729: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Agent Loss-of-Control Containment Protocol | 3 |
...
- `emergent_phage_screening_push` -- first noted turn 1, listed in 2 turn(s) so far: A coordinated investigative report across major EU outlets traces Kimi K3-derived cyber tooling to an attempted intrusion at a European research lab, triggering parliamentary calls for mandatory DNA and cyber-model screening.
- `emergent_publishing_freeze_debate` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher and a leading university jointly announce a temporary moratorium on publishing AI-assisted pathogen design methods after the phage results, triggering a wider editorial freeze debate.
...
## Metrics
```json
{"ai_capability": 58.0, "openweight_capability": 50.5, "ai_safety": 30.0, "resilience": 36.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 37.0, "public_sentiment": 35.0}
```
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Infrastructure Shield −3, EU Agent Loss-of-Control Containment Protocol −3, priority −1 = −13
PROPOSAL BONUS: EU Agent Loss-of-Control Containment Protocol (cat 6, large) +4 — loss_of_control incident landed this turn and this answers it directly
LEGITIMACY LENDS: capital 36, sentiment 35 -> none, capital higher
