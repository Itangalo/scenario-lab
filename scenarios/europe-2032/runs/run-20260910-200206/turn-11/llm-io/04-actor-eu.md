...
- characters 454-1874: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1918-4198: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6832-7303: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10271-12240: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_datacentre_blockade_wave (emergent event):** Coordinated physical protests and blockades target new datacentre and grid-upgrade sites in several member states, delaying reserve capacity expansion for substitution stacks.
...
- `two_year_commitment` (commitment): Keep essential services running on EU-controlled AI through cut-off, attack and capability shock
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU AI Displacement Income and Retraining Guarantee | 7 | small | 10 | 12 | own jurisdiction | resilience up, public_sentiment up | 2 | running |
...
| M3 | EU Critical Infrastructure Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly | 3 | finished |
| M4 | EU Agent Loss-of-Control Containment Protocol | 6 | large | 2 | 4 | own jurisdiction | resilience up, ai_safety up slightly | 3 | finished |
| M5 | EU AI Transition Safety Net | 7 | small | 3 | 6 | own jurisdiction | resilience up, public_sentiment up | 2 | finished |
| M6 | EU Embodied-AI Resilience Pact | 6 | small | 4 | 7 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly | 2 | finished |
| M7 | EU Sovereign Resilience Cloud Reserve | 4 | small | 6 | 8 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up slightly | 2 | finished |
| M8 | EU Public-Service Cyber Failover and Restoration Corps | 6 | small | 7 | 9 | own jurisdiction | resilience up, ai_safety up slightly | 2 | finished |
| M9 | EU Essential Services Model Substitution Programme | 4 | small | 8 | 10 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up | 2 | finished |
| M10 | EU Major Cyber Incident Containment Cell | 6 | small | 9 | 11 | own jurisdiction | resilience up, ai_safety up slightly | 2 | finished |
...
## New measure
**EU AI Displacement Income and Retraining Guarantee**
Pays income bridge and fast retraining placement to workers displaced in named automating sectors, funded via ESF+ and employer levy.
A category 7 labour and social protection measure, small and EU-wide, chosen now because labour displacement plus scandal plus denied medical benefit have collapsed sentiment to 8 and capital to 24, and without absorption the substitution hold fails politically.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU AI Displacement Income and Retraining Guarantee", "category": 7, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up"}, "grounds": "measurable AI job losses and collapse in public acceptance require immediate transition capacity"}]}
```
## Priority
M10 EU Major Cyber Incident Containment Cell, because degraded essential services under active ransomware and second cut-off still determine whether anything else including the new guarantee has a functioning state to land on.

## In practice
We run containment as the operational centre: ENISA-Europol joint cell with national CSIRTs isolates the compromised dependency, restores clean pre-imaged stacks onto EuroHPC/Reserve-hosted open-weight health, customs and municipal systems, and exercises failover with the Restoration Corps. EPSCO and EMPL DGs stand up the new guarantee via ESF+ reprogramming and a transition contribution from automating employers, targeting law, accountancy, software, customer ops and admin entry roles first.

We finish M9 substitution in parallel, prioritizing hospitals still cut off, and we do not chase frontier access or the US-China weights deal this turn: DG CNECT holds grid-connection emergency permits for the four factory zones while Health Council manages expectations on the denied medical breakthrough as dependence, not delivery failure.
...
## New measure
None this turn.
We add nothing while political capital sits at 21 and sentiment at 2, waiting for the income bridge to staff and pay out before staking another instrument.

## Store changes
No changes.

## Priority
M11 EU AI Displacement Income and Retraining Guarantee, because substitution is declared complete and essential services held degraded, so whether the Union survives the cut-off, leak and denied cures now depends on displaced workers seeing cash and placement rather than another resilience plan.

## In practice
We run the M10 Containment Cell to its finish this turn as the operational close-out: ENISA-Europol cell with national CSIRTs locks clean images for the substituted hospital, customs and registry stacks on EuroHPC and Reserve capacity, purges the poisoned dependency, and hands steady-state failover to the M8 Restoration Corps. DG CNECT and DG ENER hold emergency grid-connection permits for the four factory zones in Council, but we do not force racks through blockades; we negotiate site-level de-escalation with municipalities and offer local benefit-sharing from the guarantee envelope to unblock power without coercion.

EPSCO with DG EMPL reprogrammes ESF+ and collects the employer levy to pay the M11 income bridge first in law, accountancy, software and admin entry roles hit by non-returning hiring, while Health Council openly frames denied tailored cures as dependence cost, not clinical failure, and keeps triage and prescribing on EU-hosted open systems. We log the member-state side-deal as defection without retaliation this turn, to avoid spending capital we do not have.
