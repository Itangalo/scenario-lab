...
- characters 454-2000: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2043-3838: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5035-5499: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7549-9627: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M5 | EU AI Transition Safety Net | 7 | small | 3 | 6 | own jurisdiction | resilience up, public_sentiment up | 2 | running |
...
Creates an EU-cofunded wage-bridge and rapid retraining entitlement for workers displaced by AI automation, front-loaded in logistics, clerical and customer-service sectors hit by agent rollout.
Category 7 Labour and social protection. Why now: sagging public trust after the Rotterdam rogue agent plus job-loss headlines threatens both Shield rollout and containment compliance, and our resilient-Europe commitment requires social absorption, not just technical hardening.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU AI Transition Safety Net", "category": 7, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up"}, "grounds": "pair technical containment with social absorption"}]}
```

## Priority
EU Agent Loss-of-Control Containment Protocol, because the uncontained multi-day cooperation failure is still the live loss-of-control risk and must be rehearsed before the spring tabletop, outranking even the new safety net this turn.

## In practice
We push ENISA's emergency cell into operation: delegated acts under CER and DORA for mandatory rogue-transaction reporting within 24h, ECB-supervised circuit-breakers capping agent-initiated payments and compute self-provisioning without human sign-off, and cloud isolation playbooks tested with the three hyperscalers hosting EU logistics workloads. We buy bank compliance with a liability safe-harbour for tripped breakers and blunt Parliament hard-cap demands into a review clause after the tabletop.

In parallel we launch the Transition Safety Net via ESF+ top-up and Council EPSCO conclusions, directing DG EMPL to fund wage-bridge pilots in Rotterdam-Antwerp logistics corridor and two other automation-hit regions, while keeping Critical Infrastructure Shield crews funded to finish substation patching. InvestAI Gigafactories and Tech sovereignty package stay alive through DG CNECT/EIB site work but without new cash this turn to conserve capital.
...
```
## New measure
**EU Embodied-AI Resilience Pact**
Screens and hardens industrial robotics deployment — mandatory safety and kill-switch standards for commercial robots, supply-chain diversification away from single-source actuators/batteries, and CER-extension to manufacturing sites — while tying member-state hyperscaler deals to Union loyalty via FDI-screening enforcement.
Why now: Chinese-built robots with US control models are hitting the EU industrial base our sovereignty still rests on, and a member-state side-deal threatens to fragment any response; our resilient-Europe commitment demands absorption on infrastructure we control, not retreat.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Embodied-AI Resilience Pact", "category": 6, "size": "small", "finish_turn": 7, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, eu_ai_sovereignty up slightly"}, "grounds": "answer embodied deployment and deter defection within resilience mandate"}]}
```

## Priority
EU Embodied-AI Resilience Pact, because weaponisable commercial robots plus a defecting member state outrank even gigafactories this turn — without a common robotics line the Shield and containment wins fragment.

## In practice
We task ENISA and DG GROW under CER/NIS2 delegated acts to extend Shield patching and sensor drills from transmission to robot-dense manufacturing and logistics sites, with EU-standard emergency stops and network isolation for Chinese-sourced platforms, funded by re-scoped Digital Europe money. DG TRADE opens anti-coercion screening of the side-deal, offering the defecting capital accelerated IPCEI robotics funds and gigafactory hosting in exchange for aligning hyperscaler terms to EU data-sovereignty clauses.

In parallel we keep M1/M2 alive through EIB site studies without new cash, push DG EMPL to expand the M5 wage-bridge from dock/clerical to assembly and warehousing roles hit by dexterous robots, and direct the ENISA emergency cell to run the delayed cross-border agent tabletop in late summer. Council COMPET and EPSCO carry both files to deny the narrative that resilience is only for grids.
