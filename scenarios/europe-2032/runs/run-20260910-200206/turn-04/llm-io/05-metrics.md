...
- characters 105-13425: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13468-13660: `{{metrics_json}}` from the run's live metric values
- characters 13753-15299: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15342-17137: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17193-18396: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M5 | EU AI Transition Safety Net | 2 |
| M6 | EU Embodied-AI Resilience Pact | 2 |
...
PORTFOLIO CHARGE t3: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Agent Loss-of-Control Containment Protocol −3, EU AI Transition Safety Net −2, priority −1 = −12
PROPOSAL BONUS: EU AI Transition Safety Net (cat 7, small) +2 — sagging trust after Rotterdam rogue agent plus job-loss headlines in last two turns and this pairs containment with absorption
LEGITIMACY LENDS: capital 30, sentiment 37 -> +2
SOVEREIGNTY: 20 last turn, no capacity event, Gigafactories + Tech package in flight +0, capability rose 3.0 −1 = 19

## Emerging developments (tracked)

- `emergent_agent_insurance_freeze` -- first noted turn 4, listed in 1 turn(s) so far: A major European insurer reprices AI-agent liability after the Rotterdam incident, sharply raising premiums for autonomous logistics and payments and prompting several ports firms to pause agent deployments pending the late-summer tabletop.
- `emergent_agent_insurance_pullback` -- first noted turn 4, listed in 1 turn(s) so far: Major European insurers introduce exclusions and steep surcharges for losses traced to autonomous agent transactions, citing the Rotterdam incident, tightening credit and cloud terms for agent-heavy logistics firms.
...
## Notepad
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU AI Transition Safety Net −2, EU Embodied-AI Resilience Pact −2, priority −1 = −11
PROPOSAL BONUS: EU Embodied-AI Resilience Pact (cat 6, small) +3 — embodied deployment and member-state side-deal landed this turn and this answers both directly
LEGITIMACY LENDS: capital 22, sentiment 32 -> +2
SOVEREIGNTY: 19 last turn, no capacity event, Gigafactories + Tech package in flight +0, embodied_ai_deployment t4 −2, capability rose 3.0 −1 = 16

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M6", "fields": {"finish_turn": 6}, "grounds": "named priority"}]}
```
```
