...
- characters 454-1494: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1537-3187: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6328-6775: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9282-11102: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Public-Service Cyber Failover and Restoration Corps | 6 | small | 7 | 9 | own jurisdiction | resilience up, ai_safety up slightly | 2 | running |
...
## New measure
**EU Public-Service Cyber Failover and Restoration Corps**
Deploys joint ENISA-CERT-EuroHPC restoration teams and pre-imaged failover stacks to get hit clinics, town halls and port operators back online within days. A category 6 preparedness and resilience measure, chosen now because the automated dependency sweep plus eval-anomaly warnings mean the next incident lands before sovereignty build-out finishes, and resilience must hold the line under our two-year commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Public-Service Cyber Failover and Restoration Corps", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly"}, "grounds": "automated sweep and embodied-control pressure demand immediate restoration capacity"}]}
```

## Priority
M7 EU Sovereign Resilience Cloud Reserve, because without holding sovereign failover slots and liability cover live the new restoration corps has nowhere clean to restore to and the two-year commitment fails.

## In practice
We task ENISA and DG CNECT via the Cybersecurity Act emergency mandate and Digital Europe re-scope to stand up the Corps on M7 Reserve racks and EuroHPC capacity, with JHA Council mandating patched backups and payment-breaker drills as condition for liability cover. DG DIGIT funds pre-imaged municipal stacks; insurers are offered loss-data sharing for premium freeze.

We hold the finished Gigafactory permit zones alive through IPCEI pre-notifications while DG TRADE keeps anti-coercion screening open, and use the M6 robot stop-rules and wage-bridge extension to contain embodied rollout in logistics. DG RTD tracks the eval-anomaly leak with allied evaluators without pausing restoration.
...
```
## New measure
**EU Essential Services Model Substitution Programme**
Re-platforms hospitals, ministries and port operators cut off from the US frontier model onto EU-controlled open-weight models hosted on the Reserve and EuroHPC slots, with safety vetting and workflow adapters. A category 4 sovereignty measure, chosen now because nationalisation plus access denial makes commercial dependence political, and our two-year commitment to sovereign capacity that resilience can run on fails if essential services stay dark waiting for Washington.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Model Substitution Programme", "category": 4, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly, resilience up"}, "grounds": "US nationalisation and EU access denial require immediate sovereign substitution"}]}
```

## Priority
M8 EU Public-Service Cyber Failover and Restoration Corps, because without restoration teams holding clinics and ports through the sweep-plus-cutoff, substitution has nothing clean to land on.

## In practice
We task DG CNECT and ENISA under the Cybersecurity Act and Digital Europe to pair the M8 Corps with EuroHPC and the M7 Reserve operators, pre-imaging open-weight stacks for e-health, customs and municipal systems and funding adapter work via Horizon Europe. Health and TTE Councils mandate substitution drills as condition for liability cover.

We hold the four gigafactory permit zones via IPCEI pre-notifications while DG TRADE opens a state-to-state access channel in Washington, making clear exclusion of 450 million cannot stand, and we freeze embodied rollout to EU stop-rules under the M6 pact. DG RTD logs the leaked benchmark and capability jump with allied evaluators without diverting restoration.
```
