...
- characters 376-1422: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1465-3744: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3800-4557: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4620-31715: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32070-32417: `{{event_history}}` from the run's own event record
...
- `emergent_agent_insurance_pullback` -- first noted turn 4, listed in 2 turn(s) so far: Major European insurers introduce exclusions and steep surcharges for losses traced to autonomous agent transactions, citing the Rotterdam incident, tightening credit and cloud terms for agent-heavy logistics firms.
...
- Turn 2 (4 turn(s) ago): loss_of_control_incident
- Turn 4 (2 turn(s) ago): embodied_ai_deployment, member_state_defection
- Turn 5 (1 turn(s) ago): cyber_major_incident, loss_of_control_incident, member_state_defection, emergent_agent_insurance_freeze, election_consolidation
