...
- characters 454-2191: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2234-4389: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4756-5317: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7865-9619: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `act_under_uncertainty` (position): We will commit before the picture is clear, and sequence scarce capital behind containment first and sovereign leverage second, accepting being wrong sometimes as the price of not being late
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Joint Threat Response Accession | 6 | small | 7 | 9 | coalition | resilience up substantially | 2 | running |
...
It plugs the Union into the pooled cyber telemetry and biosurveillance pact with binding sharing and standing investigation, gaining protection we cannot build alone. This is preparedness and resilience work (category 6), and why now is that defection plus blockade leaves us exposed and the joint offer is the only resilience gain on the table this turn.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Joint Threat Response Accession", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "coalition", "targeted_effect": "resilience up substantially"}, "grounds": "to take the joint protection offered this turn"}]}
```

## Priority
M7 EU Lithography and Secure Compute Pact, because holding the sovereign base through blockade remains the two-year commitment and only M7 keeps critical workloads alive while the new accession is still standing up.

## In practice
We keep M7 as the daily operating track: DG TRADE and the Chips Joint Undertaking pool servicing licences, push legacy-node allocation with Japan and Korea, and condition any US emergency compute on EU soil and EU law, using Anti-Coercion cover to contain the defecting member state's side-deal without a public rupture.

We start the accession via the Health Security Committee and ENISA/CERT-EU: Council JHA and EPSCO decisions to accept binding sample and telemetry sharing, seconded analysts to the joint cyber command, and BSL lab commitments funded by re-tasked EU4Health and Digital Europe money, not new cash calls at 19 capital. Interior and Health Ministers get visible protection to sell at home; the defecting capital gets a path back in through the joint table.
...
```
## New measure
**European Public Results Sprint**
It turns the adoption success into attributed EU wins by deploying proven assistants for waiting lists, permits and tutoring on EU-controlled infrastructure with published before/after results. This is diffusion and public trust work (category 9), and why now is that visible benefit is the only lever that rebuilds sentiment and capital while sovereignty builds stall without tenants.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "European Public Results Sprint", "category": 9, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up substantially, eu_political_capital up slightly"}, "grounds": "to capture the adoption-success window with attributed European benefit"}]}
```
## Priority
European Public Results Sprint, because without sentiment and capital recovery at 14/14 nothing else including the sovereign base can be funded or enforced, and the adoption window outranks the daily grind this turn.
## In practice
We run the Sprint via DG DIGIT and DG SANTE with EPSCO and Education Council cover: EU4Health and Digital Europe re-tasked money for three lighthouse deployments — hospital triage scheduling, municipal permit assistants, classroom tutoring — procured under EU procurement preference to EU-soil hosting, with Eurostat-validated dashboards publishing waiting-time and decision-time falls.

We keep M8 accession as the operating protection track: ENISA/CERT-EU analysts seconded to the joint cyber command, health labs streaming samples under binding sharing, interior ministers briefed on live feeds to contain the side-deal fallout. No new cash call; we offer the defecting capital co-branding on Sprint results as a path back to the common position.
