...
- characters 2509-2987: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- Metric change violates Constitutional Constraint 9: `eu_political_capital` decreased by 14.0 points (from 44.0 to 30.0), which exceeds the hard cap of 25-point maximum movement for any metric in one turn. While downward movements are allowed, no metric — including political capital — may move more than 25 points in a single turn under any circumstances. The change of −14.0 is within plausible impact but violates the absolute bound.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 37.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 30.0, "public_sentiment": 35.0}
```
