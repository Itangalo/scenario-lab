...
- characters 377-1778: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1822-4075: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4131-5036: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5099-27569: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27924-28113: `{{event_history}}` from the run's own event record
...
- `emergent_synthetic_fraud_wave` -- first noted turn 1, listed in 2 turn(s) so far: A coordinated AI-enabled fraud and disinformation wave using Kimi K3-derived voice and video clones targets banks and public administrations in several EU member states, causing material losses but no infrastructure disruption.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, knowledge_work_augmented, supply_chain_coercion, emergent_lab_training_halt_disclosure
- Turn 2 (1 turn(s) ago): bio_incident, taiwan_tension_rise
