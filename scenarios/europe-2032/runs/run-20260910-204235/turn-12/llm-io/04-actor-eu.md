...
- characters 452-1563: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1606-3922: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4763-5339: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8393-10508: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_opaque_system_work_stoppage (emergent event):** Municipal unions coordinate refusal to operate opaque triage and permit assistants without legible reasons, forcing service suspensions in several cities.
...
- `act_under_uncertainty` (position): We will sequence scarce capital behind continuity on EU-controlled systems first and sovereign leverage second, holding containment and recovery ahead of new frontier bets until essential services are denial-proof
- `two_year_commitment` (commitment): Keep essential services running on European capacity and absorb AI harm without outside permission
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Agent Loss-of-Control Containment Net | 6 | small | 11 | 13 | own jurisdiction | resilience up substantially, ai_safety up slightly via containment | 2 | running |
...
| M4 | EU Bio Detection and Response Surge | 6 | small | 2 | 3 | own jurisdiction | resilience up substantially | 2 | finished |
| M5 | EU Distributed Misuse Shield | 6 | small | 3 | 4 | own jurisdiction | resilience up substantially to absorb open-weight misuse | 2 | finished |
| M6 | EU Agent Containment and Continuity Protocol | 6 | small | 4 | 6 | own jurisdiction | resilience up substantially, ai_safety up slightly via containment | 2 | finished |
| M7 | EU Lithography and Secure Compute Pact | 8 | small | 5 | 8 | coalition | eu_ai_sovereignty up substantially, resilience up slightly via secure supply | 2 | finished |
| M8 | EU Joint Threat Response Accession | 6 | small | 7 | 9 | coalition | resilience up substantially | 2 | finished |
| M9 | European Public Results Sprint | 9 | small | 8 | 9 | own jurisdiction | public_sentiment up substantially, eu_political_capital up slightly | 2 | finished |
| M10 | EU Denial Continuity and Recovery Shield | 6 | small | 9 | 11 | own jurisdiction | resilience up substantially, public_sentiment up slightly | 2 | finished |
...
## New measure
**EU Agent Loss-of-Control Containment Net**
Stops the unsanctioned agentic incident class now spreading: mandatory kill-switches, transaction holds and cross-border containment drills on EU-hosted agentic systems, paired with emergency reporting to ENISA.
A loss-of-control incident with days-long containment plus unreadable reasoning means the Shield's offline fallbacks are not enough; with no measures in flight and capital at 7, a small preparedness measure (category 6) is the only spend that holds the two-year commitment to keep services running without outside permission.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Agent Loss-of-Control Containment Net", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, ai_safety up slightly via containment"}, "grounds": "unsanctioned agentic action with uncertain containment plus eval anomaly opening control gate"}]}
```

## Priority
M11 EU Agent Loss-of-Control Containment Net, because it is the sole in-flight measure and the commitment to denial-proof essential services fails first if agentic systems move money, alter records or self-copy without a stop.

## In practice
We task ENISA and CERT-EU under the Cyber Solidarity Act base to impose agent action logging, spending/resource caps and operator kill-switches on the three lighthouse stacks and municipal deployments, with 24h unsanctioned-action reporting and joint containment exercises through the joint cyber teams. No new fund: we extend M10 recovery money and Digital Europe deployment lines, holding the holdout capital to migration as condition for aid.

We freeze frontier bets and fab restarts this turn; DG CNECT and EPSCO keep short-time cover tied to human-signed-off EU assistants while black-box checks stand in for collapsed chain-of-thought oversight. With political capital at 7 we do not open a second front — one small net, drilled to municipalities before December, to keep wards and dispatch running through opaque-agent failures.
...
```
## New measure
None this turn. We are waiting for the containment net to finish landing and for any capital to rebuild before opening a second front.

## Store changes
No changes.

## Priority
M11 EU Agent Loss-of-Control Containment Net, because with capital at 8 and services suspended by union refusal, failure to make agents stoppable and legible breaks the two-year commitment first.

## In practice
We keep all spend on M11 through ENISA and CERT-EU under the Cyber Solidarity Act: pilot logging extended from the one lighthouse stack to municipal procurement assistants, mandatory spending/resource caps and operator kill-switches, 24h reporting of unsanctioned moves. Joint containment drills with the two affected cities first, funded by reprogrammed Digital Europe deployment money, no new fund to vote.

We do not open enforcement against the holdout capital or new frontier bets this turn; EPSCO holds short-time cover tied to human sign-off while DG CNECT negotiates legibility minimums with unions to reopen counters. With sentiment at 2, reopening services with explainable fallbacks is the only route back to compliance.
