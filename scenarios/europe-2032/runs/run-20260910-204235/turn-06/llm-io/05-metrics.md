...
- characters 13779-15097: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15140-18014: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18070-18898: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Lithography and Secure Compute Pact | 2 |
...
## Metrics
```json
{"ai_capability": 60.0, "openweight_capability": 56.0, "ai_safety": 15.0, "resilience": 52.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 19.0, "public_sentiment": 19.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election won by anti-AI retrenchment candidate, taking office turn 6"}]}
```
```
