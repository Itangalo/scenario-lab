...
- characters 1618-3774: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3830-4493: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4556-24848: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 25203-25656: `{{event_history}}` from the run's own event record
...
- `emergent_stripped_weights_ransomware` -- first noted turn 4, listed in 2 turn(s) so far: Municipal utilities and regional hospitals running stripped October open weights on local servers suffer a coordinated ransomware wave using model-generated exploits, forcing emergency EU cyber-agency intervention.
...
- Turn 2 (4 turn(s) ago): taiwan_tension_rise, eu_frontier_access_denied
- Turn 3 (3 turn(s) ago): ai_investment_collapse, openweight_frontier_release, knowledge_work_augmented, export_control_escalation
- Turn 4 (2 turn(s) ago): export_control_escalation
- Turn 5 (1 turn(s) ago): knowledge_work_augmented, export_control_escalation, emergent_siting_backlash_freeze, election_consolidation
