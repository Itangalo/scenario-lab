...
- characters 106-12905: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12948-13140: `{{metrics_json}}` from the run's live metric values
- characters 13256-16205: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 16261-16864: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated physical protests and grid-connection lawsuits stall at least one major EU hyperscale data-centre project for months, turning siting into a national political issue.
- `emergent_kimi_exploit_wave` -- first noted turn 1, listed in 1 turn(s) so far: Copycat crews weaponise Kimi K3's offensive cyber capability in a wave of low-sophistication intrusions against EU municipalities and mid-sized firms, widely reported but falling short of critical infrastructure disruption.
