...
- characters 455-1447: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1491-3708: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5287-5751: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7836-10320: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Continuity and Reciprocity Instrument | 8 | large | 2 | 4 | own jurisdiction plus US frontier provider and coalition partners | eu_ai_sovereignty up moderately, eu_political_capital stabilise then up if continuity holds, resilience up slightly | 3 | running |
...
## New measure
**EU Continuity and Reciprocity Instrument**
It keeps cut-off hospitals, ministries and firms running on EU-anchored capacity while tying future foreign-model market access and supply-chain cooperation to guaranteed, non-discriminatory EU access.
This and why now because the short-notice cutoff turned abstract dependence into operational failure in health and government, and with Taiwan risk rising this is the moment when leverage — ASML/EUV, chemicals, Anti-Coercion Instrument, procurement — must be linked to continuity rather than left as speeches while M1/M2 capacity is still years away. Category is International coordination and leverage.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Continuity and Reciprocity Instrument", "category": 8, "size": "large", "finish_turn": 5, "applies_to": "own jurisdiction plus US frontier provider and coalition partners", "targeted_effect": "eu_ai_sovereignty up moderately, eu_political_capital stabilise then up if continuity holds, resilience up slightly"}, "grounds": "frontier access denial cut off hospitals ministries firms with Taiwan precursor opening"}]}
```

## Priority
EU Continuity and Reciprocity Instrument, because live loss of the leading model in hospitals and ministries outranks even the grid hardening sprint which continues without extra push this turn.

## In practice
We task DG CNECT and the AI Office with an emergency triage: inventory cut-off deployments, re-route essential health and public-administration workloads to EU-hosted models including Mistral, Aleph Alpha and EuroHPC capacity, funded by reprogrammed Digital Europe and EU4Health money, with ENISA certifying continuity. Health and interior ministers get continuity funding paired with a procurement rule: no new critical-service build on non-guaranteed foreign inference.

We pair this in Foreign Affairs and Trade Councils with quiet reciprocity: market access under the AI Act, data-flow and public-procurement decisions, plus coordinated export-control posture on lithography and materials with Netherlands, Japan and Taiwan-facing partners, are now conditional on restored, contractually guaranteed access with appeal. We do not trigger the Anti-Coercion Instrument this turn, but we prepare its file so the denial reads at home as foresight acted on, not outrage performed. M1/M2 stay on track via EIB structuring and permitting, and M3 detection upgrades continue in TSOs.
...
```
## New measure
**EU Distributed-Capability Resilience Sprint**
It hardens hospitals, energy, telecoms and public administration against misuse of the now-unrecallable near-frontier open-weight release while keeping essential services on EU-hosted capacity.
This and why now because open weights within ~5 points of the frontier plus brittle resilience means restriction no longer reaches the capability that matters, and with investment collapse freezing private build-out this turn the only leverage that still lands is absorption — detection, patching, and continuity — which directly serves our two-year commitment to absorb frontier shocks without depending on foreign control. Category is Preparedness and resilience.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Distributed-Capability Resilience Sprint", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via deployed safeguards"}, "grounds": "open-weight near-frontier release now unrecallable amid brittle resilience"}]}
```

## Priority
EU Continuity and Reciprocity Instrument, because live foreign cutoff plus Taiwan-linked hardware risk still outranks the new sprint, which is designed to run cheaply alongside it while M1/M2 ride out the investment freeze.

## In practice
We keep M4 as the political centre: DG CNECT/AI Office complete triage re-routing to EU-hosted models via reprogrammed Digital Europe/EU4Health funds, Council adopts the guarantee-clause procurement rule for critical builds, and Trade/Foreign Affairs quietly condition AI Act market access and lithography cooperation on restored contractual access — using the US export-control escalation to press volume-licence access for allies rather than rationing.

We launch the Resilience Sprint through ENISA, Europol EC3 and the Health Emergency Preparedness Authority: baseline cyber-hardening audits for cut-over hospitals/ministries, rapid patching playbooks for open-model-enabled intrusion, exercised fallback to EU-hosted models, funded from existing cyber and civil-protection lines. We instruct EIB and DG COMP to hold M1/M2 sites alive through the valuation reset — no new compute programme, just preserve permits, grid queues and distressed private co-funding options for counter-cyclical buy-in.
