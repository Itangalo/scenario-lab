...
- characters 13366-14669: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14712-17078: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17134-18204: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_synthetic_fraud_wave` -- first noted turn 1, listed in 2 turn(s) so far: A coordinated wave of AI-generated voice and video fraud hits banks and public administrations in several member states in autumn 2026, causing nine-figure losses and forcing emergency authentication changes.
- `emergent_open_toolkit_takedown` -- first noted turn 2, listed in 1 turn(s) so far: Hosting providers and code repositories coordinate a cross-jurisdiction takedown of weaponized open-model cyber toolkits derived from the summer open release, briefly disrupting low-skill attacker activity.
...
## Metrics
```json
{"ai_capability": 55.5, "openweight_capability": 50.0, "ai_safety": 17.0, "resilience": 37.0, "eu_ai_sovereignty": 21.0, "eu_political_capital": 29.0, "public_sentiment": 39.0}
```
