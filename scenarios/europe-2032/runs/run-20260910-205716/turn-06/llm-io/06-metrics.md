...
- characters 105-13551: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13594-13786: `{{metrics_json}}` from the run's live metric values
- characters 13879-14948: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14991-17087: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17143-17638: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Metrics
```json
{"ai_capability": 62.0, "openweight_capability": 59.0, "ai_safety": 9.0, "resilience": 47.0, "eu_ai_sovereignty": 23.0, "eu_political_capital": 18.0, "public_sentiment": 32.0}
```
## Narrative
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner for tighter controls and tiered access takes office turn 6"}]}
```
```
