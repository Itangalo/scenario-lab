...
- characters 107-13391: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13434-13625: `{{metrics_json}}` from the run's live metric values
- characters 13718-15078: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15122-17089: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17145-17852: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M12 | EU Eval Anomaly Triage Cell | 2 |
| M13 | EU Loss-of-Control Containment Protocol | 2 |
...
PORTFOLIO CHARGE t10: EU Essential Services Continuity Pact −2, EU Eval Anomaly Triage Cell −2, priority −1 = −5
PROPOSAL BONUS: EU Eval Anomaly Triage Cell (cat 1, small) +3 — eval anomaly reports landed this turn and this answers it directly
...
- `emergent_screening_arbitrage` -- first noted turn 11, listed in 1 turn(s) so far: Synthesis provider shifts DNA order routing through Swiss subsidiary to evade unfunded EU screening paperwork, creating a visible enforcement gap that clinics and press seize on.
...
## Metrics
```json
{"ai_capability": 67.0, "openweight_capability": 65.0, "ai_safety": 0.0, "resilience": 55.0, "eu_ai_sovereignty": 17.0, "eu_political_capital": 20.0, "public_sentiment": 21.0}
```
...
## Notepad
PORTFOLIO CHARGE t11: EU Eval Anomaly Triage Cell −2, EU Loss-of-Control Containment Protocol −2, priority −1 = −5
