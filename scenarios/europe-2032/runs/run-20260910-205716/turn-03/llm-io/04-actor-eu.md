...
- characters 455-2084: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2128-4221: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4832-5298: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7219-9210: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_synthetic_fraud_wave (emergent event):** A coordinated wave of AI-generated voice and video fraud hits banks and public administrations in several member states in autumn 2026, causing nine-figure losses and forcing emergency authentication changes.
**emergent_open_toolkit_takedown (emergent event):** Hosting providers and code repositories coordinate a cross-jurisdiction takedown of weaponized open-model cyber toolkits derived from the summer open release, briefly disrupting low-skill attacker activity.
...
| M4 | EU OT Rapid Reaction Reserve | 6 | small | 2 | 5 | own jurisdiction | resilience up moderately | 2 | running |
...
## New measure
**EU OT Rapid Reaction Reserve**
It stands up pooled operational-technology incident teams that can isolate, rebuild and restart grid, port and water systems within hours of an AI-enabled intrusion.
Why this and why now is because the capability jump in code and long cyber operations means the rehearsal footholds found in autumn can be turned into simultaneous disruptive action within weeks, and the existing Shield audits without hands-on response capacity will not hold.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU OT Rapid Reaction Reserve", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "closing response gap exposed by test-shot plus cyber capability jump"}]}
```

## Priority
M3 EU Critical Infrastructure Cyber Shield, because the open three-turn attack window plus this turn's jump in attacker tooling outranks even the compute build-out and the new Reserve depends on its audits to know where to deploy.

## In practice
We keep ENISA as operational lead for M3, pushing the CER-based implementing act through the Transport, Telecommunications and Energy Council with 75% co-financing from reprogrammed Digital Europe money to bring municipal utilities and mid-size ports into segmentation audits and credential rotation by summer, while DG CNECT and the EIB hold M1 and M2 permitting to lock the first two Gigafactory grid connections before winter peak.

At the same time we task the Emergency Response Coordination Centre and CERT-EU to roster the new Reserve from TSO, naval cyber and large-vendor OT staff, pre-position clean images and manual-override playbooks at the two compromised transmission operators and the affected port, and fund joint exercises with national CSIRTs. We share staged-tooling signatures with US CISA for goodwill but make EU deployment conditional on domestic detection landing first.
...
```
## New measure
**EU Bank-and-State Authentication Hardening**
It funds emergency rollout of phishing-resistant authentication, verified caller/video attestation for banks and administrations, and a shared fraud-signal exchange to blunt the synthetic fraud wave.
Why this and why now is because nine-figure voice/video fraud is draining trust and political room just as grid hardening needs public consent, and the takedown window gives a brief pause to replace weak authentication before toolkits reappear.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bank-and-State Authentication Hardening", "category": 9, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up modestly, public_sentiment up"}, "grounds": "answer autumn fraud wave before trust collapse"}]}
```

## Priority
M4 EU OT Rapid Reaction Reserve, because the intrusion timeline collapsing from months to weeks means hands-on restart capacity outranks even fraud response and the gigafactory build for survival this winter.

## In practice
We keep ENISA and CERT-EU on M4 rostering, paying standby premiums from repurposed Digital Europe funds to break the Franco-German pay dispute and pulling OT engineers from large TSOs on secondment, with clean backups pre-positioned at the compromised operators and spring exercises locked with national CSIRTs. DG ENER pushes the Shield audits onward through the TTE Council, targeting municipal utilities with simplified pass/fail checks to avoid audit fear.

At the same time we task DG FISMA with ECB and national supervisors to mandate Strong Customer Authentication upgrades and verified-communications registries for banks and town halls, financed as an emergency amendment to the Digital Europe work programme, using the eIDAS2 wallet toolbox for attestation. DG CNECT holds M1/M2 to grid-connection milestones only, deferring wider permitting fights to preserve capital.
