...
- characters 455-1704: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1748-3879: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6671-7178: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10544-12632: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): A Europe that keeps essential services running and frontier AI deployments checkable through runaway capability and automation shock
...
|---|---|---|---|---|---|---|---|---|---|
| M14 | EU Proven-Care Scale-Up | 9 | small | 12 | 14 | own jurisdiction | public_sentiment up, resilience up slightly | 2 | running |
...
| M4 | European Chokepoint Response | 8 | large | 2 | 5 | coalition | eu_ai_sovereignty up, eu_political_capital stable | 3 | finished |
| M5 | EU Bio Shield and Workforce Buffer | 6 | small | 3 | 6 | own jurisdiction | resilience up, public_sentiment up slightly | 2 | finished |
| M6 | EU Assurance Adoption Cell | 5 | small | 4 | 6 | own jurisdiction | ai_safety up, eu_political_capital up slightly | 2 | finished |
| M7 | Graduate Transition Guarantee | 7 | small | 5 | 7 | own jurisdiction | public_sentiment up, resilience up slightly | 2 | finished |
| M8 | EU Continuity Adoption for Hospitals and Public Services | 9 | small | 6 | 8 | own jurisdiction | public_sentiment up, resilience up slightly | 2 | finished |
| M9 | EU Opaque-Systems Scrutiny Reserve | 5 | small | 7 | 9 | own jurisdiction | ai_safety up slightly, resilience up slightly | 2 | finished |
| M10 | EU Hospital Continuity Reserve | 6 | small | 8 | 10 | own jurisdiction | resilience up, public_sentiment up slightly | 2 | finished |
| M11 | EU Automation Shock and Continuity Buffer | 6 | small | 9 | 11 | own jurisdiction | resilience up, public_sentiment up slightly | 2 | finished |
| M12 | EU Cyber-Bio Emergency Hardening Surge | 6 | small | 10 | 12 | own jurisdiction | resilience up, ai_safety up slightly | 2 | finished |
| M13 | EU Joint Cyber-Bio Response Tie-In | 6 | small | 11 | 13 | coalition | resilience up substantially | 2 | finished |
...
## New measure
**EU Proven-Care Scale-Up**
Takes the Lyon-Milan-Rotterdam certified triage dosing and other waiting-list-cutting public-sector uses that voters already credit to a European decision and funds their replication to paper-triage and hit municipalities with EU-run checks, printed fallback and short examiner summaries. This is Diffusion, adoption and public trust, and why now is because adoption_success gives a rare window of credited benefit to rebuild tolerance while eval anomalies and 88-level capability make checkable continuity the only holdable story.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Proven-Care Scale-Up", "category": 9, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, resilience up slightly"}, "grounds": "bank credited adoption success to rebuild room to act"}]}
```
## Priority
M13 EU Joint Cyber-Bio Response Tie-In, because with telemetry keys contested and capability at 88.5 the joint attribution and sample-sharing seat landing next turn is what keeps essential services running through the anomaly window, outranking even the scale-up.
## In practice
We run M13 through ENISA as single gateway and HERA/ECDC for binding sample-sharing, with JHA and Health Councils holding the mandate despite two states refusing hospital routing via American cloud keys — we keep their data on EU-hosted degraded nodes rather than forcing the route and losing cohesion. Reprogrammed Digital Europe and EU4Health money continues to direct pooled kits and chips to February-hit and paper-triage sites, with islanding drills as the deliverable this semester.

We launch the scale-up via EPSCO and DG SANTE procurement that favours EU-certified triage models already cutting adverse events, paired with JRC short test summaries so each new deployment stays checkable. DG ENER mapping for the blocked compute site is ring-fenced to continuity loads only, no new build money promised, to avoid another sovereignty promise we cannot fund at capital 22.
...
```
## New measure
**EU Blackout-Bio Containment Continuity**
Puts hospitals, grids and water on islandable manual-fallback operation with pooled bio-detection kits and a logged kill-switch/containment protocol for agentic systems after the major cyber hit and loss-of-control case. This is Preparedness and resilience, and why now is because automated ransomware, unsanctioned agents, open-weight frontier at 84.5 and the bio-uplift precursor mean essential-services continuity is the only holdable core of the two-year commitment at capital 19.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Blackout-Bio Containment Continuity", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, public_sentiment up slightly"}, "grounds": "answer cyber major + loss-of-control + bio gate with continuity"}]}
```
## Priority
M14 EU Proven-Care Scale-Up, because checkable care is the sole credited trust asset holding sentiment at 21 while the new continuity measure spins up, and losing it loses room for everything else.

## In practice
We run M14 through DG SANTE/EPSCO replication teams to the three new ministries, keeping printed fallbacks and JRC examiner summaries as the condition for funds, and we ring-fence it from the American cloud-key fight by leaving the two refusing states on EU-hosted nodes. HERA/ECDC push pooled kits east under M13's tie-in, now extended as bridge to the new continuity stock.

We launch Blackout-Bio Containment via JHA + Health Council emergency mandate and ENISA as gateway: islanding drills become standing orders for the two hit grid operators, hospitals get offline paper-triage fallback funded from reprogrammed EU4Health, and a joint procurement requires logged replayable traces and operator kill-switch for any agentic system in public services. No new compute build money is promised; DG ENER stays on continuity loads only to avoid breaking cohesion after the member-state defection.
