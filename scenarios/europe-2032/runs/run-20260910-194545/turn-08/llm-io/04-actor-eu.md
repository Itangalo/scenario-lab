...
- characters 454-1695: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1738-3833: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3901-4371: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7038-9265: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Opaque-Systems Scrutiny Reserve | 5 | small | 7 | 9 | own jurisdiction | ai_safety up slightly, resilience up slightly | 2 | running |
...
## New measure
**EU Opaque-Systems Scrutiny Reserve**
Stands up black-box and activation-level scrutiny for frontier systems deployed in EU hospitals and critical services now that chain-of-thought is unreadable, with authority to pause EU-hosted deployment on anomaly. This is Public technical capacity and research, and why now is the leaked eval anomaly plus opaque reasoning means our assurance cell can no longer read models it is putting into clinics, so without a new inspection basis continuity adoption loses trust and safety cover at once.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Opaque-Systems Scrutiny Reserve", "category": 5, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up slightly, resilience up slightly"}, "grounds": "replacing lost CoT oversight after opaque-reasoning shift"}]}
```
## Priority
M8 EU Continuity Adoption for Hospitals and Public Services, because with capital at 18 and sentiment at 21 only visible waiting-list gains keep gigafactory permits and any scrutiny mandate politically alive.
## In practice
We keep M8 as the delivery line through Health/Employment Councils: ENISA-certified EU-hosted models stay in the three cut-off states' hospitals, ESF+ vouchers and Graduate Guarantee traineeships tied to entry-hiring firms, DG GROW holding permitting zones and grid loans while mediation continues on the one blocked site.

We start the Scrutiny Reserve inside the Assurance Adoption Cell with JRC and ENISA examiners: pre-deployment black-box evals plus sampled activation inspection for clinical and logistics models, incident reporting to the common body, and a narrow pause power exercised by the Health Council. We fund it by reprogramming Horizon/Digital Europe evaluation money, not a new build, to avoid a second compute fight we cannot pay for.

On export controls we do not open a new front: we instruct DG TRADE to seek volume-licence continuity for allied buyers and stockpile critical inference chips for hospitals from finished Gigafactory procurement, absorbing the tightening rather than retaliating while US cures and US-China robots set the dependence constraint.
...
```
## New measure
**EU Hospital Continuity Reserve**
Keeps EU-hosted clinical AI running through chip shortages and opaque-model anomalies with pooled inference-chip stockpiles, degraded-mode protocols and exercised fallback in the three cut-off states. This is Preparedness and resilience, and why now is because without stockpiled chips and rehearsed degradation, both continuity adoption and the Scrutiny Reserve's pause power mean clinics simply stop when Washington tightens or a model misbehaves.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Hospital Continuity Reserve", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up slightly"}, "grounds": "keep clinics running through chip cuts and opaque-model pauses"}]}
```
## Priority
M9 EU Opaque-Systems Scrutiny Reserve, because with capital at 22 and sentiment at 22 only a credible safety check on unreadable models keeps continuity adoption and gigafactory permits politically alive.
## In practice
We run Health Council + HERA + ENISA as the delivery line: pooled procurement of high-end inference chips from gigafactory lines for hospital reserve, ENISA-certified degraded-mode playbooks where triage falls back to rules-based tools when the Scrutiny Reserve pauses a deployment, and joint exercises in the three cut-off states funded by EU4Health reprogramming, not new money.

We keep the Scrutiny Reserve inside the Assurance Cell with JRC examiners doing black-box evals and sampled activation checks, reporting anomalies to the common incident body. DG TRADE seeks allied volume-licence continuity without retaliation, while DG GROW holds grid and procurement for the closed tranche and continues mediation on the blocked site.

We tie ESF+ vouchers and Graduate Guarantee traineeships to entry-hiring firms around Rotterdam/Hamburg logistics and hospital admin, so visible waiting-list gains and hiring holds carry the trust case while sovereignty rebuilds slowly.
