...
- characters 105-13750: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13793-13985: `{{metrics_json}}` from the run's live metric values
- characters 14078-15428: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15471-17701: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17757-18605: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | Graduate Transition Guarantee | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn-5 retrenchment winner takes office turn 6"}]}
```
```
