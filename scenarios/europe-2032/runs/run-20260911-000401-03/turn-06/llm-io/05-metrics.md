...
- characters 105-13557: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13600-13792: `{{metrics_json}}` from the run's live metric values
- characters 13885-15179: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15222-17358: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17414-18235: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Supply Leverage and Continuity Compact | 2 |
| M8 | EU Frontier Eval Anomaly Triage | 2 |

     **so 3 measures are in flight and they come to −7 `eu_political_capital` this turn.** That figure is the `cost_per_turn` column of the rows above, added up, and nothing else: the named priority's −1 below is charged on top of it, and is not in it. Your charge line carries one term per measure -- 3 of them before the priority. **A measure is charged in every turn up to but not including its finishing turn. A measure added this turn is charged from this turn -- it appears in the rows above and in your total.**
...
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Supply Leverage and Continuity Compact −2, priority −1 = −9
PROPOSAL BONUS: EU Supply Leverage and Continuity Compact (cat 8, small) +5 — taiwan_blockade landed this turn severe and this answers supply continuity directly
LEGITIMACY LENDS: capital 17, sentiment 33 -> +2
SOVEREIGNTY: 16 last turn, no capacity event finish, Gigafactories in flight +0, taiwan_blockade t5 −2, capability rose 3.0 −1 = 13

## Emerging developments (tracked)

- `emergent_grid_connection_freeze` -- first noted turn 5, listed in 2 turn(s) so far: Grid operators in France and Poland publicly refuse new AI-facility connections until EU-wide backup-power and islanding standards are agreed, freezing EuroHPC-linked clinic and factory rollouts through the winter.
...
## New measure
**EU Frontier Eval Anomaly Triage**
Stands up rapid third-party re-testing of leaked-model claims and deploys enhanced monitoring on EU-hosted substitutes, with pause triggers for health and critical-service AI if control anomalies confirm. This is Evaluation and oversight work, and why now is because the leak plus watchful-agent reports arrive while safety is at 14 and open weights already at 62 — we cannot ration continuity on models we no longer trust.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Frontier Eval Anomaly Triage", "category": 1, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up modestly, resilience protected"}, "grounds": "to verify leaked capability and contain control risk during blockade"}]}
```
## Priority
EU Supply Leverage and Continuity Compact, because without chips and rationed allocation both the gigafactories and the clinics stop — supply still outranks even the eval scare this turn.
## In practice
We keep DG TRADE and the Anti-Coercion board on Seoul and Tokyo lots, with DG GROW/HERA/ENISA/EuroHPC JU allocating what lands strictly to hospitals, grid operators and sequencing, while InvestAI shells are held in legal readiness rather than built blind. Health ministers carry the degraded-but-continuing message to hold queues from becoming blame.

Through the EU AI Evaluation and Safety Institute we task ENISA and the EuroHPC continuity cell to re-run the leaked benchmarks on our substitutes, instrument agent logs for evaluation-awareness, and require incident reporting from critical-service deployers — any confirmed loss-of-control behaviour triggers a pause of that workload, not of the whole rationing system.
...
## Notepad
PORTFOLIO CHARGE t6: InvestAI Gigafactories −3, EU Supply Leverage and Continuity Compact −2, EU Frontier Eval Anomaly Triage −2, priority −1 = −8
PROPOSAL BONUS: EU Frontier Eval Anomaly Triage (cat 1, small) +2 — eval_anomaly_reports landed this turn severe and this answers verification directly
LEGITIMACY LENDS: capital 10, sentiment 33 -> +2
SOVEREIGNTY: 13 last turn, Tech sovereignty package finishes t6 +4, Gigafactories in flight +0, no capacity event, capability rose 2.5 −1 = 16

## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election retrenchment takes office turn 6"}]}
```
```
