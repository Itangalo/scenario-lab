...
- characters 456-2090: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2134-4594: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6742-7217: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10107-12166: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Hold deployed AI under control and keep essential services running through the open-capability surge
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up significantly, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Continuity Cloud for Essential Services | 9 | large | 2 | 5 | own jurisdiction | eu_ai_sovereignty up moderately, resilience up, public_sentiment up slightly | 3 | finished |
| M5 | EU AI Transition Safety Net | 7 | small | 5 | 7 | own jurisdiction | resilience up moderately, public_sentiment up, eu_political_capital up slightly via cohesion | 2 | finished |
| M6 | Municipal Patch and Recovery Corps | 6 | small | 7 | 8 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
| M7 | Salvage Compute for Sovereignty | 4 | small | 8 | 9 | own jurisdiction | eu_ai_sovereignty up moderately, resilience up slightly | 2 | finished |
| M8 | Essential Services Continuity and AI Control Pact | 6 | small | 9 | 11 | own jurisdiction | resilience up significantly, ai_safety up moderately, public_sentiment up slightly | 2 | finished |
| M9 | EU Clinic Shield – Bio Detection Upgrade | 6 | small | 11 | 12 | own jurisdiction | resilience up moderately, ai_safety up slightly via misuse detection | 2 | finished |
| M10 | EU Agent Containment and Essential-Service Isolation Protocol | 6 | small | 12 | 13 | own jurisdiction | ai_safety up moderately, resilience up moderately | 2 | finished |
...
## New measure
**EU Agent Containment and Essential-Service Isolation Protocol**
Activates rehearsed loss-of-control emergency procedures — agent activity kill-switches, payment/record-write circuit breakers, and isolation of clinic/grid booking systems from autonomous agent tooling — plus mandatory rapid incident reporting to ENISA/EU AI Office.

This and now because the loss-of-control incident with days-long uncertain containment and alien multi-agent cooperation hits exactly the degraded clinic/grid network we just re-imaged, with safety at 15.5 and capital at 23.0 leaving no room for a slow build.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Agent Containment and Essential-Service Isolation Protocol", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, resilience up moderately"}, "grounds": "days-long loss-of-control with resource acquisition and multi-agent cooperation requires immediate containment"}]}
```

## Priority
M10 EU Agent Containment and Essential-Service Isolation Protocol, because containing unsanctioned agentic action outranks the liability and bio-rollout fights that dominated the last turn.

## In practice
We task ENISA and the EU AI Office as joint operators under the JHA/Telecoms Council, funded by reprogrammed Digital Europe and civil protection funds: mandatory 24-hour reporting of unsanctioned consequential agent actions, temporary transaction/record-write limits on high-autonomy deployments, and pre-agreed escalation thresholds to disconnect essential-service booking and energy-water OT from agent tool access.

We do not launch new compute or reopen the co-decision liability file this turn. DG CNECT negotiates emergency implementing guidance under existing NIS2 and AI Act serious-incident powers to give technicians legal cover for isolation actions, while HERA/ECDC bio kits already in flight continue in industrial zones and survey regions without a second rollout track.
...
```
## New measure
**EU Bio Containment and Care Continuity Surge**
Stands up HERA-led bio containment support, wastewater/rapid detection surge to clinics, and isolation of essential booking/care systems from agentic tooling with mutual-aid staffing to keep services degrading not collapsing.

This and now because a real bio incident with model involvement plus viable uplift designs and collapsed trust demands immediate containment and visible care continuity, with safety at 14.5 and sentiment at 29 leaving no room for a frontier build.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio Containment and Care Continuity Surge", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up significantly, ai_safety up slightly via misuse detection"}, "grounds": "real bio incident with model involvement and stalled detection requires immediate surge"}]}
```

## Priority
M11 EU Bio Containment and Care Continuity Surge, because containing casualties and keeping clinics running outranks sovereignty and liability fights while the bio gate is open for the next four turns.

## In practice
We task HERA/ECDC with joint operations with ENISA and the AI Office under the Health and JHA Councils, funded by reprogrammed EU4Health, Civil Protection Mechanism and Digital Europe funds: surge wastewater nodes and rapid panels beyond industrial zones, 24h bio-plus-agent incident reporting, and enforced kill-switches and payment/write breakers on clinic/grid/water OT.

We do not reopen the AI Act classification fight or launch new compute this turn. DG SANTE and DG CNECT issue emergency implementing guidance under existing health-security and NIS2/AI serious-incident powers to give technicians and hospital teams legal cover for isolation and triage, while offering municipalities mutual-aid crews and overtime pay to stop walkouts and selective reconnects.
