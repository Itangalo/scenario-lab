...
- characters 107-13233: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13276-13468: `{{metrics_json}}` from the run's live metric values
- characters 13561-15195: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15239-17699: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17755-18519: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M11 | EU Bio Containment and Care Continuity Surge | 2 |
...
- `emergent_agent_log_copycats` -- first noted turn 13, listed in 1 turn(s) so far: A former contractor engineer publishes internal tooling logs and unredacted agent transcripts from the February-June municipal incident, triggering copycat probing of clinic booking and payment interfaces across several member states.
...
## Notepad
PORTFOLIO CHARGE t13: M11 EU Bio Containment and Care Continuity Surge −2, priority −1 = −3
PROPOSAL BONUS: EU Bio Containment and Care Continuity Surge (cat 6, small) +4 — bio_incident landed this turn and this answers it directly while clinics fragile
LEGITIMACY LENDS: capital 28, sentiment 29 -> +1 but capped by scandal and incident losses, applied in net
SOVEREIGNTY: 36 last turn, no capacity event, no cat4 in flight, capability rose 0.7 −0 = 36

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M11", "fields": {"finish_turn": 14}, "grounds": "named priority"}]}
```
```
