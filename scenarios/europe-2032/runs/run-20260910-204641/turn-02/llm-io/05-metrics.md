...
- characters 105-12935: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12978-13170: `{{metrics_json}}` from the run's live metric values
- characters 13263-14396: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14439-16374: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16430-17793: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Continuity Cloud for Essential Services | 3 |
...
- `emergent_inference_rationing_shock` -- first noted turn 1, listed in 2 turn(s) so far: A major US hyperscaler announces emergency curtailment of EU inference capacity during peak grid stress in winter 2026, citing US domestic power constraints and priority obligations, leaving several EU enterprise and public-sector users throttled for days.
- `emergent_genome_publication_freeze` -- first noted turn 1, listed in 2 turn(s) so far: A major scientific journal imposes a temporary moratorium on publishing AI-designed genome methods after the phage results, triggering a fight over whether controls drive work underground.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 49.5, "ai_safety": 37.0, "resilience": 36.5, "eu_ai_sovereignty": 23.0, "eu_political_capital": 30.0, "public_sentiment": 31.0}
```
