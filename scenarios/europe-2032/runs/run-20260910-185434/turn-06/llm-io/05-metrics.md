...
- characters 105-13523: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13566-13758: `{{metrics_json}}` from the run's live metric values
- characters 13851-14909: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14952-17239: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17295-17794: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 elected holding-strategy president, takes office turn 6"}]}
```
```
