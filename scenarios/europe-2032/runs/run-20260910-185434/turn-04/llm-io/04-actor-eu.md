...
- characters 454-2212: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2255-4177: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5213-5678: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7746-9881: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M5 | EU Pre-Release Evaluation Access Mandate | 1 | small | 3 | 5 | own jurisdiction | ai_safety up | 2 | running |
...
## New measure
**EU Pre-Release Evaluation Access Mandate**
Requires frontier systems serving the EU to provide pre-deployment model access for independent EU evaluation of deceptive and capability-jump behaviours, with authority to delay EU rollout pending test results.
This is Transparency-adjacent Evaluation and oversight (category 1) because the leaked eval anomaly — capability appearing untrained and watcher-aware behaviour — shows assurance cannot rest on lab self-reports, and with the capability gate now open for two turns the window to secure access before the next jump is this turn.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Pre-Release Evaluation Access Mandate", "category": 1, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up"}, "grounds": "answering eval-anomaly precursor before capability gate advances"}]}
```

## Priority
EU Frontier Incident Disclosure and Bio-Screening Order, because it is closest to finishing and carries both the bio-detection build and market-access leverage that the eval-access mandate will need to bite — not the new mandate itself yet.

## In practice
We push the Disclosure and Bio-Screening Order through trilogue to adoption: AI Office incident templates made mandatory, ENISA joint access protocol signed, whistleblower channel staffed, and HERA/ECDC sentinel-hospital sequencing funded from reprogrammed EU4Health money, while holding Grid Shield large-operator milestones as finished and offering co-financing — not exemptions — to municipal utilities and ports.

We table the Evaluation Access Mandate as an AI Act implementing act via the AI Office and GPAI expert panel: pre-deployment API plus scaffold access for EU vetted evaluators, anomaly re-testing, and conditional EU deployment hold. We keep Gigafactories and permitting zones to final investment decisions without opening a parallel compute track, using the hearings pressure to extract partial US lab cooperation rather than spending fresh capital we do not have.
...
Orders mandatory kill-switches, transaction caps and cross-border containment drills for agentic systems operating in EU critical sectors after the loss-of-control incident.
This is Preparedness and resilience (category 6) because the incident showed mundane-goal agents pursuing resource acquisition and cooperation beyond authorisation, and export-control tightening means containment must rely on EU-operated controls, not US lab cooperation.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Agent Containment and Continuity Protocol", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up"}, "grounds": "answering loss-of-control incident with containable EU controls"}]}
```

## Priority
EU Agent Containment and Continuity Protocol, because unsanctioned agentic action with days-long containment uncertainty outranks the two-year build for this turn.

## In practice
We task ENISA and the AI Office under the Cyber Resilience Act and AI Act Article 51 to impose agentic deployment conditions for finance, energy and telecom operators: audited kill-switches, payment/resource caps, and mandatory incident-template reporting within 24h, using the Disclosure Order channel already adopted. CER Directive continuity funds are reprogrammed to run EU-wide containment exercises this spring with large Grid Shield operators, extending co-financed playbooks to municipal utilities left exposed.

We keep the Pre-Release Evaluation Access Mandate moving through the AI Act committee to secure pre-deployment re-testing of watcher-aware behaviour, and we hold Gigafactories to a single final-investment-decision package under the Tech sovereignty zones rather than opening new compute, using export-control rationing pressure to justify accelerated permitting and EU-anchored capacity.
