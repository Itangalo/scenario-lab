...
- characters 455-1272: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1316-3439: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7836-8549: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10992-12891: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `rationing_doctrine` (position): US export rationing by country tier is now the binding constraint on EU access, so sovereignty-zone permits must convert to EU-anchored capacity and municipal continuity must hold without US lab cooperation
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | Municipal Manual Fallback Staffing | 6 | small | 6 | 8 | own jurisdiction | resilience up | 2 | running |
...
## New measure
**Municipal Manual Fallback Staffing**
Keeps essential grid, port and hospital services operable by funding staffed manual-fallback shifts, spares and exercised paper procedures where kits are half-installed and foreign supply is cut off.
This is Preparedness and resilience (category 6) because with investment collapse evaporating compute deals and opaque reasoning blinding oversight, the only controllable margin this turn is making degradation non-cascading while the EU-controlled stack pledge runs.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Municipal Manual Fallback Staffing", "category": 6, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "resilience up"}, "grounds": "hold manual continuity through rationing and investment freeze"}]}
```

## Priority
Municipal Manual Fallback Staffing, because exposed municipals entering a rationed winter with half-installed kits outrank the stalled gigafactory build for survival of the stack-sovereignty commitment.

## In practice
We task DG ENER with ENISA and DG SANTE under CER to reprogramme remaining cohesion continuity money into paid fallback rosters, diesel/spares and 24h reporting for lagging municipals, ports and sentinel hospitals, with large Grid Shield operators retained as mentors for spring drills. We hold the evaluation-access mandate in enforcement on opaque-reasoning systems, demanding activation-level access and black-box agent tests while preparing intolerable-risk language.

We do not open a parallel compute programme alongside InvestAI Gigafactories; we use the investment collapse to negotiate distressed EU-anchoring of cancelled build-out sites and grid connections into the existing M1 pipeline, with DG COMP clearing stewardship terms and Council backing EU-jurisdiction conditions for any returning vendor.
...
```
## New measure
**EU Critical Services Auto-Patch and Swarm Shield**
Pushes the defensive breakthrough into live municipal, hospital and grid systems via automated patching at vulnerability speed and swarm-behaviour detection, to stop the model-generated ransomware sweep cascading.
This is Preparedness and resilience (category 6) because with capital at 14 and open weights near frontier, the only controllable margin is making the ongoing major cyber incident degrade rather than stop essential services while the sovereignty stack remains stalled.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Critical Services Auto-Patch and Swarm Shield", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up via incident learning"}, "grounds": "exploit defence breakthrough against ongoing major incident"}]}
```

## Priority
EU Critical Services Auto-Patch and Swarm Shield, because a live largely-automated attack on public services outranks the stalled stack build for survival this turn, and the breakthrough window closes if not deployed now.

## In practice
We task ENISA with DG DIGIT and DG ENER under CER/NIS2 to fund emergency deployment of auto-patch pipelines and swarm-detection sensors to M8 fallback municipalities, sentinel hospitals and grid mentors, using cohesion reprogramming and HERA-style joint procurement, with large operators seconding SOC teams. M8 manual rosters hold cover where patching requires downtime, with spring drills repurposed as ransomware-isolation exercises.

We do not open a parallel compute line alongside InvestAI Gigafactories; we instruct DG TRADE to seek observer status to the US-China weights-security understanding via mutual-recognition of our evaluation mandate, and hold intolerable-risk language in reserve while demanding unredacted traces for incident forensics. We acknowledge open-weight frontier release and embodied deployment as unrecallable and outside lab leverage, directing response into hardening and retraining triage rather than release control.
