...
- characters 106-13376: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13419-13611: `{{metrics_json}}` from the run's live metric values
- characters 13704-14521: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14565-16688: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16744-17907: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M8 | Municipal Manual Fallback Staffing | 2 |
| M9 | EU Critical Services Auto-Patch and Swarm Shield | 2 |
...
PORTFOLIO CHARGE t6: InvestAI Gigafactories −3, Municipal Manual Fallback Staffing −2, priority −1 = −6
PROPOSAL BONUS: Municipal Manual Fallback Staffing (cat 6, small) +2 — answers exposed municipals/ports entering rationed winter and November gateway intrusion vector
LEGITIMACY LENDS: capital 14, sentiment 21 -> +2
SOVEREIGNTY: 15 last turn, Tech sovereignty package finishes t6 +5, Gigafactories in flight +0, ai_investment_collapse t6 −2, capability rose 3.0 −1 = 17
