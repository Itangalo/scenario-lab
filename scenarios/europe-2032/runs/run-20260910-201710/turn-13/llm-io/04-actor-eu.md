...
- characters 456-1638: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1682-3436: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4790-5235: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8698-10686: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Absorb unrecallable AI shocks while rebuilding consent for EU capacity
...
|---|---|---|---|---|---|---|---|---|---|
| M13 | EU Shielded Care Restoration | 6 | small | 12 | 14 | own jurisdiction | resilience up, public_sentiment decline contained, ai_safety incident harm down | 2 | running |
...
| M3 | EU Critical Systems Shield | 6 | small | 1 | 3 | own jurisdiction | resilience up moderately, ai_safety incident harm down | 2 | finished |
| M4 | EU Biological Detection and Response Surge | 6 | large | 2 | 4 | own jurisdiction | resilience up substantially, ai_safety incident harm down | 3 | finished |
| M5 | EU Assured Deployment Standard | 5 | small | 3 | 5 | own jurisdiction | ai_safety up moderately, resilience harm down | 2 | finished |
| M6 | EU High-Risk Enforcement and Redress Sweep | 2 | small | 4 | 5 | own jurisdiction | public_sentiment up, eu_political_capital up via restored trust, resilience harm down | 2 | finished |
| M7 | EU Municipal Cyber Telemetry Sovereignty Pact | 6 | small | 6 | 8 | own jurisdiction | resilience up, eu_ai_sovereignty loss contained | 2 | finished |
| M8 | EU Middle-Power Compute and Access Pact | 8 | small | 7 | 8 | coalition | eu_ai_sovereignty up, eu_political_capital up via secured access | 2 | finished |
| M9 | EU Joint Cyber Shield Accession | 6 | small | 8 | 9 | coalition | resilience up, ai_safety incident harm down | 2 | finished |
| M10 | EU Displaced Work Transition Guarantee | 7 | small | 9 | 10 | own jurisdiction | public_sentiment up, eu_political_capital up via restored consent, resilience up via social absorption | 2 | finished |
| M11 | EU Trusted Cure Deployment | 9 | small | 10 | 11 | own jurisdiction | public_sentiment up sharply, eu_political_capital up via restored trust | 2 | finished |
| M12 | EU Care Continuity Reserve | 4 | small | 11 | 12 | own jurisdiction | eu_ai_sovereignty loss contained, resilience up, public_sentiment decline contained | 2 | finished |
...
## New measure
**EU Shielded Care Restoration**
Keeps cancer-care scheduling and hospital logistics running on EU-hosted and middle-power inference while hardening appointment and triage systems against the new verifiable cyber jump, and pairs restoration with redress for the decision scandal.
A category 6 preparedness and resilience measure, small because it extends the Care Continuity Reserve rationing and Joint Cyber Shield telemetry rather than building new compute, and urgent now because the US cutoff plus capability jump plus scandal together threaten both wards and consent.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Shielded Care Restoration", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment decline contained, ai_safety incident harm down"}, "grounds": "cutoff plus cyber jump plus scandal requires hardened clinical continuity"}]}
```

## Priority
EU Shielded Care Restoration, because queued cures and stalled scheduling outrank frontier debate and it directly serves the two-year commitment to absorb shocks while rebuilding consent.

## In practice
We execute through HERA, EMA and ENISA with the Health Council: clinical workloads stay pinned to EU-hosted inference and M8 pact allocations, non-clinical loads stay throttled, and the M9 joint telemetry feed is extended to hospital appointment and logistics systems patched after the spring intrusion to absorb the code/cyber jump.

We pair restoration with accountability for the automated-decision scandal: the AI Office and Fundamental Rights Agency audit the flagged system under the AI Act, publish logs and redress path, and councils blocking compute permits get weekly restoration reporting tied to local service recovery. ESF+ transition payments continue, no new builds are promised at capital 18, and Washington licensing is treated as given while we document need.
...
```
## New measure
**EU Major Cyber Incident Containment**
Activates cross-border containment, backup restoration and emergency patching for the automated ransomware/dependency attack on public services and hospitals, using exercised mutual aid rather than new builds.
A category 6 preparedness and resilience measure, small because it funds response and hardening through ENISA and the Joint Cyber Shield telemetry already in flight, and urgent now because defenders are visibly behind and wards just stayed open.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Major Cyber Incident Containment", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety incident harm down, public_sentiment decline contained"}, "grounds": "automated major cyber incident plus openweight jump requires immediate containment"}]}
```

## Priority
EU Shielded Care Restoration, because keeping oncology scheduling and hospital logistics alive through the cutoff and attack outranks even containment, and it directly serves the two-year commitment to absorb shocks while rebuilding consent.

## In practice
We execute through ENISA, HERA and the Health Council: M13 keeps clinical workloads pinned to EU-hosted inference and middle-power allocations with rationing, while the new containment extends M9 joint telemetry to affected municipalities and hospitals, pushes emergency hardening guidance into patching, and stands up mutual-aid restoration teams for the ransomware/dependency blast radius.

We pair operations with accountability to stop consent collapsing further: the AI Office publishes the welfare-triage audit and redress, the incident reporting feed logs the model-generated tooling for attribution, and councils freezing data-centre hearings get weekly ward-recovery reporting. No new compute promises at capital 12; the middle-power coalition channel is used to keep reagent and inference access flowing while Washington licensing holds.
