...
- characters 13712-14919: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14962-17359: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17415-17750: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Municipal Cyber Telemetry Sovereignty Pact | 2 |
...
**emergent_utility_telemetry_lockin (emergent event):** Municipal utilities hit by scripted intrusions jointly procure a managed detection service from a non-EU frontier lab, locking operational telemetry outside EU jurisdiction.
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election winner takes office turn 6 - inward turn with moratoriums and curbs"}]}
```
```
