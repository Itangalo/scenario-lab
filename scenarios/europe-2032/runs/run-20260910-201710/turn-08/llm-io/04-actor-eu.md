...
- characters 454-1964: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2007-4337: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5955-6413: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8859-10917: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_gigafactory_injunction_wave (emergent event):** Municipal water courts issue coordinated injunctions halting gigafactory groundworks across three member states, turning paper capacity into a litigated freeze that Brussels cannot override without fresh legislation.
...
## New measure
**EU Middle-Power Compute and Access Pact**
Locks export-licence alignment, joint compute bargaining and shared evaluation with supply-chain middle powers to restore frontier access cut off this turn and pool fallback capacity.
Category 8 international coordination and leverage, small because it uses existing trade and ACI legal bases rather than new build, and urgent now because access denial plus investment collapse closes the window for joint leverage.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Middle-Power Compute and Access Pact", "category": 8, "size": "small", "finish_turn": 9, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up, eu_political_capital up via secured access"}, "grounds": "frontier access denied amid investment collapse"}]}
```

## Priority
EU Middle-Power Compute and Access Pact, because hospitals and ministries cut off from the leading model cannot wait for gigafactories to pour concrete, and joint leverage is the only access route that survives the capex reset.

## In practice
We carry the denial as a procurement and continuity emergency under the Health Emergency and Single Market legal bases: ENISA and HERA reroute affected hospitals and ministries to the ENISA-hosted detection stack and to second-best models under M7, while DG TRADE tables the coalition mandate in Foreign Affairs Council for export-licence alignment and pooled bargaining with Japan, Korea, Taiwan and Gulf compute holders.

We use the Trade and Technology venues already staffed to sign reciprocal evaluation recognition and telemetry-residency terms, tying recovery-money conditionality from M7 to the fallback, and we bank the safety breakthrough via the AI Office to certify the fallback deployments quickly. With capital at 9 we do not open a new build front: M1/M2 zones stay in legal maintenance while protests and water suits run, and retraining is steered through existing ESF+ lines for the narrow coding-support displacement rather than a new fund.
...
Joins the joint cyber command with real-time telemetry sharing and surges hardening to municipalities hit by this turn's automated attack, using the open-weight surge as the unrecallable baseline.
Category 6 preparedness and resilience work done through an 8-type coalition vehicle, small because it plugs ENISA and the M7 home-hosted stack into pooled attribution rather than building new capacity, and urgent now because the major incident plus frontier open release outruns what brittle national defences can absorb alone.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Joint Cyber Shield Accession", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "coalition", "targeted_effect": "resilience up, ai_safety incident harm down"}, "grounds": "automated major incident plus open-weight frontier release"}]}
```

## Priority
EU Joint Cyber Shield Accession, because with capital at 14 and no measures in flight it is the only protection we cannot build alone and the direct answer to this turn's incident under our harden-to-survive commitment.

## In practice
We accept the invitation into the joint command via Foreign Affairs and JHA Councils, mandating ENISA as the EU telemetry hub and extending the M7 residency conditionality to the pooled feed, so recovery money keeps data inside Europe while we draw on joint attribution. HERA and the AI Office triage hospitals and ministries already rerouted from the US cutoff, pushing patches and fallback-model guards first to the ransomware-hit public services.

We do not open a fresh gigafactory or sovereignty build this turn: with water-court injunctions freezing three sites, M1/M2 stay in legal maintenance while Commission legal service prepares a single-market override option for when capital allows. Diplomacy under M8 continues on export-licence alignment, and retraining stays on ESF+ lines, because a small accession is all our capital can carry without collapse.
