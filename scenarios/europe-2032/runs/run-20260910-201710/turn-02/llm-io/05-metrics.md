...
- characters 105-13039: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13082-13274: `{{metrics_json}}` from the run's live metric values
- characters 13367-14604: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14647-16844: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16900-17626: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated physical protests and grid-connection lawsuits stall at least one major EU data-centre build in late 2026, driven by energy-price and water-use grievances after the June US controls.
...
## Metrics
```json
{"ai_capability": 56.0, "openweight_capability": 50.5, "ai_safety": 30.0, "resilience": 36.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 37.0, "public_sentiment": 36.0}
```
