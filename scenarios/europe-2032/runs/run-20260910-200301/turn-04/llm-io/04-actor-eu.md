...
- characters 2145-3831: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4751-5206: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7333-9380: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Frontier Observatory | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via technical capacity | 2 | running |
| M5 | EU Transition Safety Net | 7 | small | 3 | 6 | own jurisdiction | resilience up moderately, public_sentiment up, eu_political_capital up slightly | 2 | running |
...
## New measure
**EU Transition Safety Net**
A small EU-coordinated wage-insurance and rapid-retraining facility tied to automating employers, front-loaded through ESF+ and national schemes.
This now because public trust is slipping while use rises and resilience reporting flags labour-market absorption as the weak link that turns displacement into political breakdown, directly threatening the "taken down from within" half of our commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Transition Safety Net", "category": 7, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up, eu_political_capital up slightly"}, "grounds": "displacement shock risks internal legitimacy"}]}
```

## Priority
M1 InvestAI Gigafactories, because without first money disbursed and grid connections locked this turn the sovereignty leg of our two-year commitment stays paper while dependence continues.

## In practice
We force M1/M2 from structuring to spending: DG CNECT and DG ENER take connection offers to the Energy and Competitiveness Councils for fast-track permitting decisions on two to three zones, EIB is instructed to release a first tranche of the sovereignty fund against co-financing from host states, and conditional access terms requiring EU-jurisdiction failover for essential workloads are attached. ENISA and DG HOME sustain the Shield gains without a new instrument: binding audit follow-ups closed, leftover-access eradication verified, and backup-control procurement unblocked via joint procurement.

At the same time we launch the Transition Safety Net via EMPL Council: ESF+ front-loading plus a levy-linked top-up discussion with large deploying employers, PES network tasked with 90-day retraining placement pilots in most-exposed regions, and the Frontier Observatory (M4) staffed with JRC and contracted labs to deliver first replication results by autumn to hold evaluation pressure on foreign labs.
...
```
## New measure
**Joint Chokepoint Defence**
A small coordinated response to the ASML coercion: trigger Anti-Coercion Instrument screening, pool export-licensing decisions at EU level, and offer the Netherlands joint cover while opening a middle-power supply-chain dialogue with Japan and South Korea on joint exercise of bottlenecks.
This now because Washington is using our one chokepoint against us while our Gigafactories still disburse nothing, so the "cannot be switched off from outside" half of our commitment fails this turn if we let a member state be picked off alone, and the open-weight jump means we cannot afford further dependence on revocable foreign closed access.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Joint Chokepoint Defence", "category": 8, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction and middle-power coalition", "targeted_effect": "eu_ai_sovereignty up slightly, eu_political_capital down then up"}, "grounds": "answer US coercion over ASML without fragmenting"}]}
```

## Priority
M1 InvestAI Gigafactories, because disbursement and grid connections remain the only path from paper sovereignty to running capacity while both the open-weight release and the ASML coercion raise the cost of continued dependence.

## In practice
We keep M1/M2 as the spending push: Commission takes the two to three connection-ready zones to Energy and Competitiveness Councils for fast-track permit decisions, EIB is pressed to disburse the structured first tranche against host-state co-financing, with EU-jurisdiction failover for essential workloads kept in the drafts even as foreign providers call it aspirational. The Observatory (M4) is tasked to benchmark the new open-weight release from outside and feed results to ENISA and critical operators, since recall is now impossible.

At the same time we stand up the Joint Chokepoint Defence via Trade and Foreign Affairs Councils: Commission opens Anti-Coercion examination of the extraterritorial servicing ban, export authorisations for lithography are coordinated to prevent bilateral side-deals, the Netherlands is offered EU budget backstop and joint procurement compensation for lost servicing revenue, and a quiet dialogue with Japan, South Korea and Taiwan is opened on joint bottleneck leverage. The Safety Net (M5) continues its 90-day pilots to hold the internal leg while trust stays low.
