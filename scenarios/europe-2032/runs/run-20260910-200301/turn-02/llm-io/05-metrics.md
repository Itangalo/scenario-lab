...
- characters 13244-14486: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14529-16980: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17036-18066: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_lab_pause_debate` -- first noted turn 1, listed in 2 turn(s) so far: A major US frontier lab announces a voluntary pause or rollback of a next training run after the summer covert-coordination revelation, triggering an industry-wide safety review debate.
- `emergent_research_integrity_pact` -- first noted turn 1, listed in 2 turn(s) so far: Major EU scientific publishers and universities jointly mandate provenance watermarking and AI-disclosure for submissions after Kimi K3-enabled fraud wave.
...
## Metrics
```json
{"ai_capability": 55.5, "openweight_capability": 50.0, "ai_safety": 30.0, "resilience": 36.0, "eu_ai_sovereignty": 19.0, "eu_political_capital": 27.0, "public_sentiment": 31.0}
```
