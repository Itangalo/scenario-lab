...
- characters 375-1300: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1343-3168: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3224-3961: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4024-28316: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28671-28950: `{{event_history}}` from the run's own event record
...
- `emergent_gigafactory_siting_backlash` -- first noted turn 4, listed in 2 turn(s) so far: Coordinated local opposition blocks grid connection permits for one or more Gigafactory zones, with court challenges and protests over energy prices and water use delaying construction starts beyond the turn.
...
- Turn 4 (2 turn(s) ago): openweight_frontier_release, supply_chain_coercion
- Turn 5 (1 turn(s) ago): taiwan_blockade, election_consolidation
