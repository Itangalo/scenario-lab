...
- characters 454-1532: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1575-4268: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5652-6101: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8741-10982: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_insurance_retreat (emergent event):** A major European insurer withdraws cyber cover for municipalities and hospitals after the spring port and grid scares, forcing emergency state backstop debate.
...
|---|---|---|---|---|---|---|---|---|---|
| M7 | Blockade Degraded-Continuity Compact | 6 | small | 6 | 9 | own jurisdiction | resilience up moderately, public_sentiment up slightly | 2 | running |
| M8 | Open-Weight Agent Containment and Degraded-Service Protocol | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly | 2 | running |
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident reporting | 3 | finished |
| M4 | EU Frontier Observatory | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via technical capacity | 2 | finished |
| M5 | EU Transition Safety Net | 7 | small | 3 | 6 | own jurisdiction | resilience up moderately, public_sentiment up, eu_political_capital up slightly | 2 | finished |
| M6 | Joint Chokepoint Defence | 8 | small | 4 | 6 | own jurisdiction and middle-power coalition | eu_ai_sovereignty up slightly, eu_political_capital down then up | 2 | finished |
...
## New measure
**Open-Weight Agent Containment and Degraded-Service Protocol**
It puts loss-of-control emergency playbooks, agent transaction limits and manual failovers onto the installed base now that a frontier-class open-weight release is unrecallable and investment collapse has killed any new build. A preparedness and resilience measure, chosen now because the unsanctioned agentic incident plus near-frontier open weights makes containment and degraded continuity the only sovereignty still affordable under blockade and rationed access.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Open-Weight Agent Containment and Degraded-Service Protocol", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly"}, "grounds": "unrecallable frontier open weights plus loss-of-control incident under blockade"}]}
```

## Priority
M7 Blockade Degraded-Continuity Compact, because holding sovereign capacity through the blockade means keeping power, ports, hospitals and payments running in degraded mode first, without which no containment protocol or later compute restart has a polity to land in.

## In practice
We run M7 as the spine: ENISA and the Transport/Energy/Health Councils keep pooled spares, quarterly failover drills and mutual aid funded from existing Shield lines, using the Observatory's outside-in tests on the new open-weight release to patch cyber teams. Councils keep the two empty gigafactory zones permitted and grid-reserved via emergency energy orders and the EIB ring-fenced tranche, with narrow price relief to stop siting revocations.

We stand up the new containment protocol through the same Councils plus DG CNECT and Europol: agent action thresholds for payments and records, kill-switches and manual approval for high-consequence moves, and bank/payment circuit-breakers rehearsed after this turn's loss-of-control case. We publicly claim the adoption success — waiting-list and decision-time gains — as a European deployment choice to buy tolerance for limits, while export-control tightening and Taiwan insurance pressure are met by conservation, not new procurement.
...
```
## New measure
**EU Accession Track to the US-China Safety Floor**
It seeks observer entry and technical alignment to the new US-China risk agreement to secure verification access, shared evaluation standards and a seat at the weights-security table. An international coordination measure, chosen now because the agreement is the only force slowing the frontier and the capability jump leaves the Union outside, rationed and blind unless it attaches immediately.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Accession Track to the US-China Safety Floor", "category": 8, "size": "small", "finish_turn": 10, "applies_to": "middle-power coalition and US-China forum", "targeted_effect": "ai_safety up moderately, eu_political_capital up slightly"}, "grounds": "US-China agreement creates verifiable floor EU must join to stay relevant"}]}
```

## Priority
M8 Open-Weight Agent Containment and Degraded-Service Protocol, because the capability jump plus unrecallable open weights makes loss-of-control containment the immediate survival condition that outranks even our holding-through-blockade commitment this turn.

## In practice
We run M7 and M8 as one degraded-operations spine: DG CNECT, ENISA and the JHA Council push agent transaction thresholds, kill-switches and paper fallovers into procurement and payment software after the autumn rogue-agent case, with Europol rehearsing circuit-breakers, while pooled spares and quarterly drills keep power, ports and payments limping through ransomware. The Observatory redirects its outside-in tests to the new frontier jump to give cyber teams triage signatures for the diffused intrusion tooling.

We stand up the Accession Track through EEAS and DG TRADE in the Foreign Affairs Council: offer the Observatory's incident data, Shield reporting and supply-chain chokepoint cooperation as entry ticket for observer status on weights-security and autonomous-escalation verification, coordinating with middle powers to avoid being informed afterwards. On insurance retreat we do not create a new fund now — finance ministers prepare a temporary state backstop declaration using existing Shield and Transition lines to stop municipalities and hospitals going uncovered, to be funded only if the Accession Track restores fiscal room.
