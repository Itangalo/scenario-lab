...
- characters 376-1454: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1497-4190: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4246-5047: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5110-28741: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29096-29545: `{{event_history}}` from the run's own event record
...
- `emergent_insurance_retreat` -- first noted turn 7, listed in 1 turn(s) so far: A major European insurer withdraws cyber cover for municipalities and hospitals after the spring port and grid scares, forcing emergency state backstop debate.
...
- Turn 4 (4 turn(s) ago): openweight_frontier_release, supply_chain_coercion
- Turn 5 (3 turn(s) ago): taiwan_blockade, election_consolidation
- Turn 7 (1 turn(s) ago): loss_of_control_incident, ai_investment_collapse, openweight_frontier_release, taiwan_tension_rise, export_control_escalation, adoption_success
