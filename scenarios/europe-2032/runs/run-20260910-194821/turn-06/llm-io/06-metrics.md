...
- characters 105-13468: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13511-13703: `{{metrics_json}}` from the run's live metric values
- characters 13796-15117: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15160-17250: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17306-17845: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election won on holding frontier as strategic asset, posture takes effect turn 6"}]}
```
```
