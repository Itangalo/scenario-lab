...
- characters 455-2239: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2283-4632: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5259-5714: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7682-9882: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU–Middle Power Compute and Chokepoint Pact | 8 | large | 2 | 4 | coalition | eu_ai_sovereignty up, eu_political_capital up slightly via secured access | 3 | running |
...
## New measure
**EU–Middle Power Compute and Chokepoint Pact**
Creates a joint EU–Japan–Korea–Dutch-anchored coordination on export-licence alignment, pooled compute access and shared evaluation, using EU lithography leverage to secure non-US supply. Category 8 International coordination and leverage, because the Taiwan blockade makes access a security question overnight and the Union's upstream position is only leverage if exercised jointly rather than picked off state by state.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU–Middle Power Compute and Chokepoint Pact", "category": 8, "size": "large", "finish_turn": 5, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up, eu_political_capital up slightly via secured access"}, "grounds": "Taiwan blockade and export-control squeeze demand joint leverage"}]}
```

## Priority
EU–Middle Power Compute and Chokepoint Pact, not the Shield, because the blockade reorders everything into access and leverage and without secured supply neither hardening nor Gigafactories can run on AI the EU controls.

## In practice
We mandate the Commission under the Anti-Coercion Instrument and EU Chips Act base to negotiate aligned servicing-licence terms with Japan, Korea and Taiwan-facing partners, with the Foreign Affairs and Competitiveness Councils carrying the mandate: no further Dutch servicing concessions without Union authorisation, pooled bargaining for volume-licence compute, and joint evaluation capacity that adopts the new interpretability breakthrough for deployed systems. The Hague gets political cover and side-deal capital is folded into the common pool.

The Essential Services Shield (M1-track hardening) continues on its funds: ENISA directs segmentation verification and staged-tooling hunts at hit TSOs first, with ACER exercising failover before next winter, while DG CNECT keeps Gigafactory zones (M1/M2) alive on permitting and grid connections but throttled for chips until the Pact secures supply. Utilities get co-financing, municipalities blocking data centres get water-power efficiency conditions, not exemptions.
...
## New measure
**EU Deception Eval and Interpretability Cell**
Stands up a dedicated ENISA–JRC evaluation cell to reproduce the leaked deceptive-under-observation finding on models available in the EU and apply the new interpretability technique to deployed systems in essential services. Category 5 Public technical capacity and research, because the anomaly cannot be answered with hardening or pooled procurement alone and needs own testing capacity now.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Deception Eval and Interpretability Cell", "category": 5, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up, resilience up slightly via trusted deployment"}, "grounds": "leaked deceptive-behaviour eval rattling researchers amid darkened mood"}]}
```
## Priority
EU–Middle Power Compute and Chokepoint Pact, not the new Cell, because without unblocking tooling slots and volume-licence compute neither Gigafactories nor controlled AI for essential services can run.

## In practice
We keep the Pact as the Commission's negotiating priority: Trade and Competitiveness Councils extend the no-further-concessions mandate on Dutch servicing, DG TRADE pools the first joint orders for non-US accelerators and second-source spares, and we offer Tokyo and Seoul co-funded evaluation access via the new Cell to close the joint-text gap without ceding licensing sovereignty. Side-deal capital stays inside the pool under Anti-Coercion Instrument cover.

The new Cell runs on existing Horizon and Digital Europe funds: JRC leads reproduction of the observation-sensitive behaviour, ENISA ties pass-fail to Essential Services Shield deployment guidance, and ACER folds comms-gap fixes from the June failover exercise into this winter's rehearsal. Gigafactories (M1/M2) stay on site works, permitting and grid connections, throttled on tooling until the Pact delivers.
