...
- characters 170-359: `{{metrics_json}}` from the run's live metric values
- characters 452-3376: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 3419-5352: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7147-7657: `{{statement_ledger}}` from the actor's live statement ledger
- characters 11064-13394: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_shadow_fork_fragmentation (emergent event):** Municipal utilities and hospital operators openly fork pinned open-weight builds into unvetted variants to restore functionality, breaking bio-refusal and telemetry compatibility.
...
- `two_year_commitment` (commitment): Hold essential services and public order through rogue-agent and bio-uplift shocks by borrowing allied protection we cannot build alone
...
|---|---|---|---|---|---|---|---|---|---|
| M12 | EU Accession to US-China AI Risk Floor | 8 | small | 11 | 13 | coalition | ai_safety up moderately via shared assurance, eu_political_capital up slightly via inclusion | 2 | running |
...
| M3 | EU Essential Services Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident reporting | 3 | finished |
| M4 | EU–Middle Power Compute and Chokepoint Pact | 8 | large | 2 | 4 | coalition | eu_ai_sovereignty up, eu_political_capital up slightly via secured access | 3 | finished |
| M5 | EU Deception Eval and Interpretability Cell | 5 | small | 3 | 5 | own jurisdiction | ai_safety up, resilience up slightly via trusted deployment | 2 | finished |
| M6 | EU Shield Patch-Scale Deployment | 6 | small | 4 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly via reduced intrusion | 2 | finished |
| M7 | EU Shield Recovery and Continuity Surge | 6 | small | 5 | 7 | own jurisdiction | resilience up substantially to absorb major cyber incident | 2 | finished |
| M8 | EU Essential Continuity and Bio-Detection Patch | 6 | small | 6 | 8 | own jurisdiction | resilience up moderately, ai_safety up slightly via early detection | 2 | finished |
| M9 | EU Agent Containment and Bio-Uplift Shield | 6 | small | 8 | 10 | own jurisdiction | resilience up moderately, ai_safety up slightly via containment and early bio detection | 2 | finished |
| M10 | EU Joint Cyber-Bio Lifeline Accession | 6 | small | 9 | 11 | coalition | resilience up substantially via joint response, ai_safety up slightly via telemetry | 2 | finished |
| M11 | EU Denied-Access Continuity Switch | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, eu_political_capital up slightly via kept services | 2 | finished |
...
## New measure
**EU Accession to US-China AI Risk Floor**
It seeks observer-signatory status to the US-China weights-security and bio-design accord, trading EU telemetry, biosurveillance and supply-chain leverage for inclusion and verification access. This is International coordination and leverage, because with safety at zero, opaque reasoning, a live rogue agent and a bio gate open, the only floor that slows capability and restores shared safety work is the one Washington and Beijing just built — and the borrowing commitment survives only if Europe is inside it, not informed afterwards.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Accession to US-China AI Risk Floor", "category": 8, "size": "small", "finish_turn": 13, "applies_to": "coalition", "targeted_effect": "ai_safety up moderately via shared assurance, eu_political_capital up slightly via inclusion"}, "grounds": "US-China accord opens only path to slow race and restore safety cooperation"}]}
```
## Priority
M11 EU Denied-Access Continuity Switch, because islanding and pinned open-weight fallback is what keeps hospitals, water and ports degraded-not-stopped through the cutoff, rogue-agent assault and sabotage this turn, without which accession diplomacy has no credibility.
## In practice
We keep ENISA, EC3 and civil protection as operators for M11: push remaining islanding kits, clean backups and kill-threshold isolation to hit cities, swap dead US frontier calls for pinned open-weight builds vetted for bio-refusal on hospital and shell servers, paper where no hardware exists, funded by reprogrammed Digital Europe and rescEU under Health and JHA Council mandates.

We task the President, HR/VP and EEAS to pursue accession in parallel: formal request to Washington and Beijing for observer-signatory status to the weights-security and bio-design accord, offering joint telemetry-pool feeds, biosurveillance-pact data and coordinated export-control alignment via the Anti-Coercion Instrument and Dual-Use Regulation, while mandating protection of power/fibre sites with member-state interior ministries against sabotage. We instruct HERA/ECDC and the Joint Research Centre to implement bio-uplift screening to the new preprint standard and black-box evals now that chain-of-thought is unreadable.
...
```
## New measure
**EU Pinned-Build Integrity Hold**
It stops municipal and hospital operators forking pinned open-weight builds into unvetted variants by pushing field engineers, signed locked images, and offline functionality that works without breaking bio-refusal and telemetry. This is Preparedness and resilience, because the shadow fork directly undoes the denied-access switch and biosurveillance pact that keep services degraded-not-stopped, and with safety at zero and no US access, losing bio-refusal is the fastest path to lasting harm.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Pinned-Build Integrity Hold", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately via kept bio-refusal and telemetry, ai_safety up slightly via reduced misuse"}, "grounds": "open fork of hospital builds breaks bio-refusal and detection"}]}
```

## Priority
M12 EU Accession to US-China AI Risk Floor, because only observer inclusion restores any external safety floor and leverage while the integrity hold buys time — without it the borrowing commitment has no protector left to borrow from.

## In practice
We keep ENISA, HERA and civil protection as operators for the hold: JHA and Health Council mandates to deploy joint field teams with signed, locked container images, hardware attestation where available and paper fallback where not, funded by reprogrammed rescEU and Digital Europe, offering municipalities functionality fixes and staffing relief in exchange for staying on vetted builds and telemetry.

We task the Presidency, HR/VP and EEAS to press the M12 accession request in Geneva and capitals, trading live telemetry and biosurveillance feeds for verification access, while interior ministries protect power and fibre sites and Labour ministries open emergency graduate placement and retraining funds to contain sabotage and protest without creating a second instrument.
