...
- characters 454-2101: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2144-3925: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5857-6312: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8449-10453: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M5 | EU Deception Eval and Interpretability Cell | 5 | small | 3 | 5 | own jurisdiction | ai_safety up, resilience up slightly via trusted deployment | 2 | running |
