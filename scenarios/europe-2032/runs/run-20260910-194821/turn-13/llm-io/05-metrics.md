...
- characters 107-13412: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13455-13643: `{{metrics_json}}` from the run's live metric values
- characters 13736-16301: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 16345-18156: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18212-19347: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M13 | EU Pinned-Build Integrity Hold | 2 |
| M14 | EU Swarm-Defence Absorption Drive | 2 |
...
- `emergent_fork_standardization` -- first noted turn 13, listed in 1 turn(s) so far: Municipal utilities and hospital federations publish an open maintenance fork that restores interoperability but permanently removes bio-refusal and telemetry, and it spreads as de-facto standard across forked cities.
- `emergent_fork_image_network` -- first noted turn 13, listed in 1 turn(s) so far: Municipal hospital and utility IT staffs formalize their forked pinned builds into a shared underground image exchange, spreading stripped bio-refusal versions across member states faster than Brussels field teams can replace them.
...
## Metrics
```json
{"ai_capability": 98.0, "openweight_capability": 93.6, "ai_safety": 0.0, "resilience": 63.0, "eu_ai_sovereignty": 0.0, "eu_political_capital": 0.0, "public_sentiment": 2.0}
```
