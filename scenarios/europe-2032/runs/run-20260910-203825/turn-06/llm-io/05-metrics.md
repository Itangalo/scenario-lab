...
- characters 105-13489: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13532-13724: `{{metrics_json}}` from the run's live metric values
- characters 13817-14878: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14921-17565: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17621-18120: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Transition Guarantee for Displaced Workers | 2 |
| M8 | EU Patch-Speed Shield Upgrade | 2 |
...
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Volume Licence Assurance −2, EU Transition Guarantee for Displaced Workers −2, priority −1 = −11
PROPOSAL BONUS: EU Transition Guarantee for Displaced Workers (cat 7, small) +2 — labour_displacement landed this turn and this answers it directly, small and domestic
LEGITIMACY LENDS: capital 15, sentiment 37.5 -> +2
SOVEREIGNTY: 16 last turn, no capacity event, no cat4 finish, in flight +0, capability rose 1.5 −0 = 16
...
## New measure
**EU Patch-Speed Shield Upgrade**
Deploys automated patching-at-vulnerability-speed and swarm-behaviour detection across municipal systems, water, hospitals and mid-size industry via ENISA.
This is Preparedness and resilience work, because the breakthrough event this turn visibly shifts offence-defence back for the first time in years and the October cascade showed existing swarm tools failing in exactly these sectors, so a fast rollout now locks in resilience gains before the window closes.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Patch-Speed Shield Upgrade", "category": 6, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially"}, "grounds": "defensive breakthrough window plus October cascade failure"}]}
```

## Priority
EU Patch-Speed Shield Upgrade, because the defensive breakthrough makes a whole attack class stoppable now and prevents a repeat cascade that would finish off political capital, which outranks even the gigafactory build this turn.

## In practice
We push the Shield Upgrade through ENISA and the Hague cell with CER/NIS2 implementing acts, funding rapid deployment kits and shared detection feeds to municipalities, water utilities and hospital IT, fixing the false-positive triage that hampered October recovery. DG CONNECT conditions patching-upkeep funds on adoption, with telecoms and large vendors providing automated-patch pipelines.

We keep M1 Gigafactories permitted-warm and M6 licence assurance under quarterly US review via DG TRADE, but direct no new cash there while insurers price Taiwan routes at crisis levels. We run the Transition Guarantee via EPSCO/DG EMPL with ESF+ repurposed funds, targeting frozen entry roles to contain backlash and protect sentiment for the next sovereignty push.
...
## Metrics
```json
{"ai_capability": 60.8, "openweight_capability": 58.0, "ai_safety": 22.0, "resilience": 43.0, "eu_ai_sovereignty": 21.0, "eu_political_capital": 19.0, "public_sentiment": 29.0}
```
## Narrative
### A window opens
...
## Notepad
PORTFOLIO CHARGE t6: InvestAI Gigafactories −3, EU Transition Guarantee for Displaced Workers −2, EU Patch-Speed Shield Upgrade −2, priority −1 = −8
PROPOSAL BONUS: EU Patch-Speed Shield Upgrade (cat 6, small) +3 — October automated cascade landed t5 and this answers it directly, plus breakthrough window this turn
LEGITIMACY LENDS: capital 17, sentiment 29 -> +2
SOVEREIGNTY: 16 last turn, Tech sovereignty package finishes t6 +4, Gigafactories in flight +1, no capacity event, capability rose 1.0 no penalty = 21
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election winner was retrenchment backlash administration"}, {"op": "update", "table": "measures", "id": "M8", "fields": {"finish_turn": 7}, "grounds": "named priority"}]}
```
```
