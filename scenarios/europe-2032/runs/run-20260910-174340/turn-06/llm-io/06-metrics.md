...
- characters 105-13500: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13543-13734: `{{metrics_json}}` from the run's live metric values
- characters 13827-14817: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14860-16950: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17006-17346: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 winner was retrenchment, takes effect turn 6"}]}
```
```
