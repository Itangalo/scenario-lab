...
- characters 376-1414: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1458-4381: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4437-5255: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5318-30981: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 31336-31604: `{{event_history}}` from the run's own event record
...
- `emergent_municipal_fiscal_crunch` -- first noted turn 4, listed in 1 turn(s) so far: Several smaller EU municipalities face acute budget crises and service cutbacks after prolonged ransomware recovery costs, triggering national bailout debates and further delaying security upgrades.
...
- Turn 2 (3 turn(s) ago): cyber_major_incident, capability_jump, ai_investment_collapse
- Turn 4 (1 turn(s) ago): openweight_frontier_release, automated_decision_scandal, emergent_municipal_backlash_stall
