...
- characters 105-13017: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13060-13252: `{{metrics_json}}` from the run's live metric values
- characters 13345-15037: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15080-17290: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17346-18406: `{{notepad}}` from the Game Master's notepad, carried across turns
...
PROPOSAL BONUS: EU Critical Systems Shield (cat 6, large) +3 — autumn intrusion across EU grids landed this turn and this answers it directly
LEGITIMACY LENDS: capital 48, sentiment 42 -> none, capital higher
SOVEREIGNTY: 22 last turn, no capacity event, no finish, no in-flight bonus for new proposal, capability rose 2.0 −1 = 21
Emerging developments (tracked): `emergent_phage_publication_moratorium_call` -- first noted turn 1, listed in 1 turn(s) so far: faint press and university debate over restraint on publishing genome-model pathogen methods.

## Emerging developments (tracked)

- `emergent_phage_publication_moratorium_call` -- first noted turn 1, listed in 2 turn(s) so far: A coalition of leading virologists and biosecurity researchers publicly calls for a moratorium on publication of genome-language-model-designed pathogen methods after the AI-designed phage results, triggering press coverage and university review debates.
...
## Metrics
```json
{"ai_capability": 56.0, "openweight_capability": 50.0, "ai_safety": 24.0, "resilience": 36.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 31.0, "public_sentiment": 33.0}
```
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Shield −3, EU Dependency Triage −2, priority −1 = −12
PROPOSAL BONUS: EU Dependency Triage (cat 6, small) +2 — cyber_major_incident landed this turn and this answers it directly
LEGITIMACY LENDS: capital 30, sentiment 33 -> +1
SOVEREIGNTY: 21 last turn, no capacity event, no finish, no in-flight bonus, capability rose 2.0 −1 = 20

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M3", "fields": {"finish_turn": 5}, "grounds": "named priority"}, {"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 7}, "grounds": "event: investment collapse cancelled private co-financing"}]}
```
```
