...
- characters 452-2537: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2580-4859: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6982-7460: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9557-11412: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_anti_ai_sabotage_wave (emergent event):** Coordinated physical sabotage and blockades targeting foreign-owned data centres, robot depots and chip shipments in several member states, driven by hostile public sentiment and blamed on distributed open-weight planning tools.
...
- `two_year_commitment` (commitment): Rebuild EU control over AI used in essential services through sovereign capacity and enforceable safety
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up moderately, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Dependency Triage | 6 | small | 2 | 4 | own jurisdiction | resilience up, to stop cascade from current attack | 2 | finished |
| M5 | Municipal Resilience Labour Pact | 7 | small | 4 | 6 | own jurisdiction | resilience up slightly, public_sentiment up slightly by unblocking patching | 2 | finished |
| M6 | EU Essential Services Continuity Corps | 6 | small | 7 | 9 | own jurisdiction | resilience up slightly, public_sentiment up slightly | 2 | finished |
...
## New measure
None this turn. We are waiting for political capital to rise above survival level and for the medical-breakthrough access terms to clarify before any funded instrument could survive a vote.

With capital at 2.0, sentiment at 5.0 and sovereignty at 16.0, even a small measure costing 2 per turn would zero out capacity to act and forfeit the two-year commitment to rebuild EU control over AI in essential services; holding patched networks, logging the hyperscaler break, and using the middle-power framework and HERA guidance costs nothing and preserves ability to build next turn.

## Store changes
No changes.

## Priority
No priority this turn, because no measures are in flight and no new measure can be funded without collapsing the remaining capacity to act under the rebuilding-control commitment.

## In practice
We hold essential services with what is already paid for. DG CNECT, ENISA, CERT-EU and HERA keep Continuity Corps playbooks, patched hospital networks and commune teams running on existing budgets, with hardened open models only where EU capacity allows. No new fund, no parallel compute line while the Gigafactories record stands stalled; that revival belongs to a funded turn, not to a second track now.

We use the turn politically to stop the bleed and prepare a fundable start. We formally log the capital's hyperscaler side-deal for later joint procurement action, align export-licence and compute-bargaining positions inside the middle-power coordination framework, triage delayed chip shipments at Rotterdam and Hamburg on existing mandates, issue biosecurity screening from the genome-modelling paper, and handle the medical breakthrough as dependence management — pushing for EU-terms access so it does not land only as foreign provision — to lift sentiment enough that a sovereignty measure becomes votable.
...
```
## New measure
None this turn. We are waiting for political capital to recover above 1.0 and for sabotage and access cutoff to be contained with existing playbooks before any funded instrument is votable.

With capital at 1.0, sentiment at 3.0, sovereignty at 14.0 and safety at 2.0, even a small measure costing 2 per turn would drive capital negative and collapse enforcement of the two-year commitment to rebuild EU control over AI in essential services; holding patched networks, commune rotas and port triage on finished mandates costs nothing and preserves the ability to build next turn.

## Store changes
No changes.

## Priority
No priority this turn, because no measures are in flight and no new measure can be funded without extinguishing the last point of capacity to act under the rebuilding-control commitment.

## In practice
We hold essential services with what is already paid for. DG CNECT, ENISA, CERT-EU and HERA keep Continuity Corps playbooks, patched hospital networks and commune teams on rota on existing budgets, triaging late chip crates at Rotterdam and Hamburg and filing bio-screening memos from the genome-model finding with no new money attached. No new fund, no parallel compute line while the Gigafactories record stands finished-but-stalled; that revival belongs to a funded turn, not to a second track now.

We use the turn politically to absorb the cutoff and the sabotage without breaking further. We formally log the short-notice leading-model denial for hospitals and ministries and shift those workloads to hardened open models where EU capacity allows, coordinate police and site protection for blocked data centres, robot depots and chip shipments through the Justice and Home Affairs formation without creating a new instrument, log Washington's use of ASML servicing jurisdiction and the capital's hardened hyperscaler deal for later joint-procurement and Anti-Coercion action, and align licence language inside the middle-power framework — to stop the bleed enough that a sovereignty measure becomes votable.
