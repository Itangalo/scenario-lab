...
- characters 13164-14748: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14792-16827: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16883-17928: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_sabotage_escalation` -- first noted turn 13, listed in 1 turn(s) so far: Coordinated blockades and depot attacks spread from four states to port and grid chokepoints, forcing temporary shutdown of a foreign-owned data centre cluster and emergency policing powers debate.
- `emergent_clinician_shadow_network` -- first noted turn 13, listed in 1 turn(s) so far: Coordinated hospital fallback to slow domestic open models triggers a cross-border informal clinician work-around network sharing prompts and triage queues outside official procurement channels.
