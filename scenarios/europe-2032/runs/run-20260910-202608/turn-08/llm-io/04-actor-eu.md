...
- characters 454-2451: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2494-4607: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5096-5549: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8342-10553: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Swarm-Defence Rapid Deployment | 6 | small | 7 | 9 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 2 | running |
...
## New measure
**EU Swarm-Defence Rapid Deployment**
Rolls the breakthrough swarm-behaviour detection and automated patching into hospitals, municipalities, grids and ports via ENISA-coordinated deployment, turning the Shield repair into forward defence.
This is Preparedness and resilience, because the defensive breakthrough is the first window in years where patching can outpace the poisoned-dependency class that caused the 2029 sweep, and with capital at 19 and sentiment at 19 we cannot afford a third visible cascade before winter.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Swarm-Defence Rapid Deployment", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, ai_safety up slightly via detection"}, "grounds": "defensive breakthrough must be deployed before next sweep"}]}
```

## Priority
M8 EU Bio-Capability Evaluation Cell, because the eval-anomaly leak plus contested genome-design claim leaves us blind on bio-uplift while openweight sits at 63.9, and without independent testing neither the scandal response nor the sovereignty commitment is credible.

## In practice
We run M8 as the priority from the AI Office with ECDC and ENISA secondments, using Digital Europe and reshuffled Horizon money to stand up vetted researcher access and pre-deployment bio-screening protocols, while DG CNECT uses the eval-anomaly reports to demand structured access from frontier developers under AI Act Article 21 evaluation mandates.

We deploy the new Swarm-Defence through the existing Commission-ENISA crisis cell under NIS2 Articles 11-12 and the Critical Systems Shield maintenance track: central procurement of the automated-patching tooling, mandatory rollout to the hospitals and municipalities still on manual fallbacks from M7, and joint exercises before year-end to make absorption visible. On the automated-decision scandal we concede the Annex III gap, open infringement screening on human-oversight failures, and task the Fundamental Rights Agency with redress guidance, to contain the political fallout without opening a second legislative front we cannot fund.
...
```
## New measure
**EU Continuity Switch-Over**
Emergency re-platforming of hospitals, ministries and dependent firms cut off from the US frontier model onto EU-hosted open and licensed models with pooled inference, clean fallbacks and procurement cover.
This is Diffusion, adoption and public trust, because the cutoff is an adoption crisis not a new training run — we cannot build new frontier compute in one turn, but we can keep lights and services on with what we control, and make the sovereignty commitment visible as continuity rather than cranes.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Continuity Switch-Over", "category": 9, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly via use under EU control, resilience up moderately via continuity, eu_political_capital up slightly if services stay on"}, "grounds": "frontier access cut demands immediate sovereign fallback"}]}
```

## Priority
M9 EU Swarm-Defence Rapid Deployment, because with capital at 15 and sentiment at 15 a second municipal/hospital cascade during the model cutoff would read as state failure and finish our capacity to act at all.

## In practice
We run the Switch-Over from DG CNECT and DIGIT through the ENISA crisis cell: emergency procurement waiver to fund inference pooling on the France/Spain gigafactory groundwork sites plus existing EuroHPC, model-swap kits and manual-fallback staffing for hospitals and ministries, paid from reshuffled Digital Europe, with French-Spanish presidency cover in Council to block a retaliatory spiral we cannot afford.

We push M9 as the shield for that migration — central purchase of the machine-speed patching and swarm-behaviour detectors to the same hospitals and municipalities being re-platformed, joint exercises before winter, explicit prioritisation of small clinics and eastern municipalities to stop the lag becoming the story. On the frontier-access denial we lodge Anti-Coercion screening and demand tier-licence reasons in Washington, but we do not promise restoration; we promise continuity without permission.
