...
- characters 377-1556: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1600-3206: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3262-3821: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 3884-30620: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30975-31237: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t4: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M4 Supply-Chain Leverage Coalition −2, M5 EU Bio Detection Uplift −2, M6 EU Agent Containment and Bio Response Surge −2, priority M6 −1 = −13
PROPOSAL BONUS: EU Agent Containment and Bio Response Surge (cat 6, small) +5 — bio_incident + loss_of_control_incident fired this turn and this answers both directly
LEGITIMACY LENDS: capital 18, sentiment 30 -> +2 (capped below sentiment)
...
- Turn 1 (4 turn(s) ago): cyber_test_shot, cyber_defence_breakthrough, export_control_escalation, adoption_success, emergent_datacentre_siting_backlash
- Turn 3 (2 turn(s) ago): bio_uplift_findings
- Turn 4 (1 turn(s) ago): bio_incident, loss_of_control_incident
