...
- characters 106-13474: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13517-13708: `{{metrics_json}}` from the run's live metric values
- characters 13801-17052: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 17095-19212: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 19268-20049: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M11 | EU Municipal and Hospital Cyber Triage | 2 |
| M12 | EU Health Data Sovereignty Containment | 2 |
...
- `emergent_substitute_defection` -- first noted turn 9, listed in 2 turn(s) so far: Regional hospital groups that paused migration to European-hosted substitutes publicly revert to US-linked licensed models via third-country intermediaries, fracturing the continuity procurement pool.
...
**emergent_care_outsourcing_bypass (emergent event):** Regional hospital groups that paused EU swap-kits sign emergency staffing contracts with US vendors for remote triage, creating a de-facto parallel health data flow outside EU oversight.
...
## Notepad
PORTFOLIO CHARGE t10: M11 EU Municipal and Hospital Cyber Triage −2, M12 EU Health Data Sovereignty Containment −2, priority −1 not charged (capital below 20, no effect) = −4
PROPOSAL BONUS: EU Health Data Sovereignty Containment (cat 9, small) +2 — emergent bypass fired this turn and this answers it directly as containment
LEGITIMACY LENDS: capital 7, sentiment 4 -> none, capital not above sentiment is false
SOVEREIGNTY: 15 last turn, no capacity event, no cat4 finishes, no cat4 in flight +0, capability rose 3.0 −1 = 14
