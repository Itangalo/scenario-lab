...
- characters 454-3783: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 3827-5855: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 8372-8829: `{{statement_ledger}}` from the actor's live statement ledger
- characters 12337-14473: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential services running on European-controlled means through AI disruption
...
|---|---|---|---|---|---|---|---|---|---|
| M12 | EU Health Data Sovereignty Containment | 9 | small | 10 | 12 | own jurisdiction | eu_ai_sovereignty up slightly via controlled data flows, resilience up slightly via maintained triage, eu_political_capital down slightly if seen as coercive | 2 | running |
...
| M4 | Supply-Chain Leverage Coalition | 8 | small | 2 | 5 | coalition | eu_ai_sovereignty up moderately, eu_political_capital up slightly if successful | 2 | finished |
| M5 | EU Bio Detection Uplift | 6 | small | 3 | 5 | own jurisdiction | resilience up moderately, ai_safety up slightly via misuse detection | 2 | finished |
| M6 | EU Agent Containment and Bio Response Surge | 6 | small | 4 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly via containment | 2 | finished |
| M7 | EU Public Services Cyber Shield Repair | 6 | small | 5 | 7 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 2 | finished |
| M8 | EU Bio-Capability Evaluation Cell | 5 | small | 6 | 8 | own jurisdiction | ai_safety up moderately, resilience up slightly via bio detection | 2 | finished |
| M9 | EU Swarm-Defence Rapid Deployment | 6 | small | 7 | 9 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 2 | finished |
| M10 | EU Continuity Switch-Over | 9 | small | 8 | 10 | own jurisdiction | eu_ai_sovereignty up slightly via use under EU control, resilience up moderately via continuity, eu_political_capital up slightly if services stay on | 2 | finished |
| M11 | EU Municipal and Hospital Cyber Triage | 6 | small | 9 | 11 | own jurisdiction | resilience up moderately via incident containment, eu_political_capital up slightly if services hold | 2 | finished |
...
## New measure
**EU Health Data Sovereignty Containment**
Brings the emergency US remote-triage contracts under temporary EU oversight — mandatory registration, data-localisation gateway via EuroHPC pool, and time-limited authorisation tied to migration back to EU-hosted models.
This and now because with capital at 11 and sentiment at 6 we cannot ban life-saving triage outright, but an uncontrolled parallel health data flow outside EU oversight voids our two-year commitment to keep essential services running on European-controlled means; containment preserves care while closing the bypass.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Health Data Sovereignty Containment", "category": 9, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly via controlled data flows, resilience up slightly via maintained triage, eu_political_capital down slightly if seen as coercive"}, "grounds": "hospital bypass creates parallel US health data flow during cutoff and sweep"}]}
```

## Priority
M11 EU Municipal and Hospital Cyber Triage, because if the automated ransomware sweep cascades through clinics and utilities now on fragile substitutes, neither containment of US contracts nor any sovereignty claim survives politically.

## In practice
We run M11 from the Commission-ENISA crisis cell under Article 122, pooling rescEU teams and locum admins to restore backups and keep appointment systems on EU-hosted open models, prioritising small clinics and eastern municipalities last-served in autumn, and holding drills only where swap-kits are stable.

We pair it with the new containment via DG SANTE and the European Health Data Space emergency article: regional groups must notify US triage contracts within 14 days, route data through the EuroHPC gateway with logging, and accept 12-month sunset tied to re-migration support. Health Council gets pooled inference funding and liability cover in exchange; non-registered flows face procurement exclusion, not criminal penalty, to avoid breaking care during the sweep.
...
```
## New measure
**EU Graduate Transition and Decision Redress**
A flexicurity-style transition fund tied to automating employers plus an ombudsman-led redress and reclassification of the failed welfare/policing decision systems, paid from ESF+ reallocation.
This and now because graduate hiring has frozen and a systemic wronging scandal has destroyed trust in Brussels-run AI — without income bridging plus a visible fix to the Act's gap, neither essential-services continuity nor any sovereignty claim survives with sentiment at 4.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Graduate Transition and Decision Redress", "category": 7, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately via transition absorption, eu_political_capital up slightly if redress credible, public_sentiment up slightly"}, "grounds": "labour displacement freeze and automated decision scandal collapsing trust"}]}
```

## Priority
M12 EU Health Data Sovereignty Containment, because if unlogged US triage flows harden into a permanent bypass while hospitals are on fragile substitutes, our two-year commitment to keep essential services running on European-controlled means fails outright and nothing else is fundable.

## In practice
We run M12 through DG SANTE and the EHDS emergency article from the Commission-ENISA crisis cell: 14-day registration, EuroHPC gateway logging, 12-month sunset with re-migration support, enforced only by pooled-procurement exclusion to avoid breaking care. Pooled response teams stay on municipal/clinic backup rebuilds, holding exercises only where detectors are installed, with eastern and small sites first in the queue this turn.

We stand up the new redress-transition track under EPSCO and Justice: national ombudsmen plus the Fundamental Rights Agency audit the scandal systems — we concede the second, more damaging reading, that the deployments sat in a gap the 2024 Annex III categories do not reach and were therefore lawful — and we table a targeted AI Act amendment to close it, with case review and compensation. In parallel ESF+ is reprogrammed to wage insurance and retraining for law, accountancy, software, customer operations and administration graduates, co-funded by a levy on large automating employers. On the US-China weights/biology agreement we seek observer/consulted status via the Trade and Technology Council residue and offer our bio-evaluation cell output, to avoid being informed afterwards.
