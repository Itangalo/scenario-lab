...
- characters 105-13453: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13496-13688: `{{metrics_json}}` from the run's live metric values
- characters 13781-15136: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15179-17126: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17182-17988: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Public Services Cyber Shield Repair | 2 |
| M8 | EU Bio-Capability Evaluation Cell | 2 |
...
PORTFOLIO CHARGE t5: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M7 EU Public Services Cyber Shield Repair −2, priority M7 −1 = −9
PROPOSAL BONUS: EU Public Services Cyber Shield Repair (cat 6, small) +3 — cyber_major_incident fired this turn and this answers it directly
LEGITIMACY LENDS: capital 22, sentiment 24 -> +2
SOVEREIGNTY: 16 last turn, no capacity event, M1/M2 in flight +0, export_control_escalation t5 −2, capability rose 3.0 −1 = 13

## Emerging developments (tracked)

- `emergent_insurance_capacity_freeze` -- first noted turn 5, listed in 2 turn(s) so far: Major European reinsurers jointly suspend new cyber-biological business-interruption cover for hospitals and municipal utilities pending EU-wide reporting standards, freezing renewals across several member states.
...
## Notepad
PORTFOLIO CHARGE t6: M1 InvestAI Gigafactories −3, M7 EU Public Services Cyber Shield Repair −2, M8 EU Bio-Capability Evaluation Cell −2, priority M1 −1 = −8
PROPOSAL BONUS: EU Bio-Capability Evaluation Cell (cat 5, small) +2 — contested genome-design finding fired this turn and this answers it directly
LEGITIMACY LENDS: capital 17, sentiment 24 -> +2
SOVEREIGNTY: 13 last turn, Tech sovereignty package finishes t6 +4, Gigafactories in flight +0, no capacity event, capability rose 3.0 −1 = 16
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election settled on holding advanced AI as strategic asset with tiered rationing"}]}
```
```
