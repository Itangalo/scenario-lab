...
- characters 106-13188: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13231-13423: `{{metrics_json}}` from the run's live metric values
- characters 13516-15018: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15062-16980: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17036-18222: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Continuity Stack for Essential Operators | 2 |
| M5 | EU Lithography Bargain Shield | 2 |
...
- `emergent_energy_squeeze_backlash` -- first noted turn 2, listed in 2 turn(s) so far: A coordinated winter storm and low-wind period forces extended grid load-shedding in two member states bidding for gigafactories, turning local permitting fights into a national debate over grid priority for data centres versus households.
- `emergent_cyber_insurance_crunch` -- first noted turn 2, listed in 2 turn(s) so far: European industrial insurers sharply reprice operational-technology cyber cover after the autumn grid intrusions, requiring certified segmentation audits for renewal and leaving several municipal utilities temporarily underinsured.
