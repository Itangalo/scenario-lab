...
- characters 455-1957: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2001-3919: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5230-5702: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7353-9177: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Continuity Stack for Essential Operators | 9 | small | 2 | 4 | own jurisdiction | public_sentiment up, eu_ai_sovereignty up slightly via retained users | 2 | running |
...
## New measure
**EU Continuity Stack for Essential Operators**
Stands up EU-controlled fallback inference and support for the hospitals, ministries and firms cut off from the leading US model, via pooled EU capacity and emergency procurement.
This is category 9 Diffusion, adoption and public trust, because the cut-off is an adoption-trust crisis now, not future compute — it keeps essential services running while the gigafactories build.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Continuity Stack for Essential Operators", "category": 9, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, eu_ai_sovereignty up slightly via retained users"}, "grounds": "frontier access cut-off leaves essential users stranded"}]}
```

## Priority
InvestAI Gigafactories (M1) – because only domestic compute anchored in EU law makes the cut-off unrepeatable, and the Continuity Stack is a bridge to it.

## In practice
We are pushing M1/M2 to final investment decision in the Competitiveness Council with InvestAI guarantees and EIB money, trading grid priority for EU legal anchoring to stop regional bidding wars. The Dutch lithography lever stays in reserve for Anti-Coercion discussions.

This turn DG CNECT with ENISA and HERA stands up the Continuity Stack from existing EuroHPC and EU cloud procurement: emergency framework contracts let affected hospitals and ministries switch to EU-hosted models with migration help. On the scandal, we enforce rather than legislate — Commission guidance orders AI Act audits of the flagged social-insurance/policing system, with the AI Office and national market surveillance authorities publishing conformity and oversight findings, to show the Act bites without freezing adoption.
...
```
## New measure
**EU Lithography Bargain Shield**
Uses the Anti-Coercion Instrument and joint export-licensing to condition any further ASML servicing cuts on reciprocal US guarantees of EU model access and compute supply, while pooling servicing spares inside EU jurisdiction.
This is category 8 International coordination and leverage, because the chokepoint is being used against us and only a joint EU lever with retaliation on US cloud and IP enforcement can make refusal survivable for ASML — and why now is the Washington demand on the Netherlands this turn.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Lithography Bargain Shield", "category": 8, "size": "small", "finish_turn": 6, "applies_to": "US and Netherlands / EU-wide", "targeted_effect": "eu_ai_sovereignty up slightly via retained leverage, eu_political_capital protected"}, "grounds": "US coercion of ASML servicing cuts this turn"}]}
```

## Priority
EU Critical Services Shield (M3) – because with capability at 56, open weights at 50 and resilience at 37, the next grid/cyber rehearsal becoming real outranks even gigafactories for the two-year commitment to withstand coercion and absorb harm.

## In practice
We keep M1/M2 on life-support toward FID: Competitiveness Council locks InvestAI guarantees but we concede grid-connection sequencing to regions and municipalities to unblock permits, funding the Continuity Stack (M4) handover of hospitals/ministries to EuroHPC-hosted Mistral/Aleph Alpha models as the bridge. DG CNECT and ENISA push M3 drills and mandatory segmentation reporting, using insurers' audit demands as cover for interior ministers to accept joint detection.

On ASML, Trade and CNECT open an Anti-Coercion examination in the Foreign Affairs Council (Trade), centralising Dutch export licences at EU level and preparing proportionate responses on US hyperscaler procurement and IP enforcement. The message to Washington is explicit: no further servicing cut without volume-licence EU access to leading models and chips, and spares and servicing know-how pooled on EU soil meanwhile. On the benefits-queue scandal, we stay with enforcement — AI Office publishes audit results and bans solely-automated cuts — to rebuild trust without new law that would freeze adoption further.
