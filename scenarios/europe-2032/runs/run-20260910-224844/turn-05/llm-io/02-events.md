...
- characters 377-1184: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1228-3045: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3101-3496: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 3559-32044: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32399-32669: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Lithography Bargain Shield −2, priority −1 = −9 (priority cost waived, capital below 20, charge applied −8)
...
- Turn 2 (3 turn(s) ago): eu_frontier_access_denied, automated_decision_scandal
- Turn 3 (2 turn(s) ago): export_control_escalation, eu_frontier_access_denied, supply_chain_coercion
