...
- characters 169-359: `{{metrics_json}}` from the run's live metric values
- characters 452-1590: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1633-3673: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5343-5788: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8727-11004: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Essential Services Islanding and Access Pact | 6 | small | 6 | 9 | own jurisdiction | resilience up materially, eu_political_capital protected | 2 | running |
| M9 | EU Licence Solidarity and Clinical Continuity Pact | 8 | small | 7 | 10 | own jurisdiction and US | eu_ai_sovereignty up slightly via leverage, eu_political_capital protected | 2 | running |
...
| M3 | EU Critical Shield and Continuity Programme | 6 | large | 1 | 3 | own jurisdiction | resilience up materially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Automating-Employer Transition Fund | 7 | small | 2 | 5 | own jurisdiction | resilience up materially via social absorption, public_sentiment up, eu_political_capital protected | 2 | finished |
| M5 | EU Bridge to the Washington-Beijing Safety Floor | 8 | small | 3 | 4 | US, China and middle-power coalition | ai_safety up materially via shared assurance, eu_ai_sovereignty up slightly via leverage | 2 | finished |
| M6 | EU Essential Services Cyber Surge | 6 | small | 4 | 5 | own jurisdiction | resilience up materially, ai_safety up slightly via incident learning | 2 | finished |
| M7 | EU Assurance and Evaluation Capacity | 5 | small | 5 | 8 | own jurisdiction | ai_safety up materially, eu_ai_sovereignty up slightly via leverage | 2 | finished |
...
Locks member states into joint EU negotiation of AI-model and chip licences with Washington, pooling clinic procurement and Gigafactory off-take so no single capital can trade away revocation safeguards for a bilateral discount.
This now because a member-state side-deal on conditional US licences would hollow the independence commitment just as revocable cures and black-box reasoning leave us with no EU verdict of our own; a small international-coordination measure (category 8) that makes solidarity the price of liability cover and compute access.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Licence Solidarity and Clinical Continuity Pact", "category": 8, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction and US", "targeted_effect": "eu_ai_sovereignty up slightly via leverage, eu_political_capital protected"}, "grounds": "member-state side arrangement undercutting joint licence line"}]}
```

## Priority
EU Licence Solidarity and Clinical Continuity Pact, because stopping the defection-driven unravelling of our common licence position outranks even islanding drills this turn for securing capacity no foreign licence can withdraw.

## In practice
We table the Pact under the Anti-Coercion Instrument logic via the Foreign Affairs and Health Councils together: joint HERA procurement for clinic models with mandatory revocation-compensation and audit-access clauses, linked to Gigafactory capacity reservations. The defecting capital keeps its hyperscaler pilot but only inside the EU envelope, in exchange for first call on continuity stocks and liability cover already offered under the Islanding Pact.

DG CNECT and the AI Office keep the accelerated compute zones and hardened-grid permits moving with DG ENER and ENISA, while our small evaluation lab publishes a joint forensic note on the poisoned component and the opaque clinical models — legibility lost, licence risk confirmed — to justify why no bilateral deal can waive EU-level assurance. Transition Fund payments stay flowing to holdout cities that sign access-for-cover deals, litigated as NIS2 mutual aid rather than compulsion we cannot afford at capital 12.
...
Puts the Union into pandemic mode under HERA and the Civil Protection Mechanism — pooled biothreat sequencing, binding sample-sharing, hospital continuity and essential-service islanding — by joining the offered biosurveillance pact and redirecting Gigafactory reservation and evaluation capacity to continuity.
This now because the AI-designed pathogen outbreak suspends normal politics: without fast detection, care continuity and a shared attribution channel, both independence and safety mandates fail at once, and the joint threat response window closes if we do not take it this turn; a small preparedness and resilience measure (category 6) that carries the safety breakthrough into deployed triage and lab controls.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Pandemic Continuity and Biosurveillance Surge", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction and biosurveillance coalition", "targeted_effect": "resilience up materially, ai_safety up slightly via deployed assurance"}, "grounds": "catastrophic AI-designed bio outbreak requiring immediate joint response"}]}
```

## Priority
EU Pandemic Continuity and Biosurveillance Surge, because pandemic-mode survival and shared biosurveillance outrank even licence solidarity this turn for preventing lasting harm and holding any capacity to determine our future.

## In practice
We trigger the Emergency Support Instrument and UCPM rescEU reserve via Health and JHA Councils together: HERA leads pooled procurement of countermeasures and sequencing, ECDC takes binding sample-sharing under the offered pact, ENISA and DG ENER extend the Islanding Pact's access-for-cover drills to hospitals, labs and energy contractors already isolated. The defecting hyperscaler pilot is frozen into continuity use only — audit-access and revocation-compensation enforced as a condition of liability cover.

DG CNECT and the AI Office pause accelerated-zone grid fights where encampments block works, redeploying transition payments as pandemic income support and retraining bridge, while our evaluation lab with the interpretability breakthrough is tasked to certify triage and surveillance models already deployed. Solidarity Pact joint procurement stays running but subordinated: no new bilateral clinic-model deals until continuity stocks and attribution telemetry are shared.
