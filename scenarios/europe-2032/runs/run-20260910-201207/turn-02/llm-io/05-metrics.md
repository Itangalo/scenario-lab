...
- characters 105-13080: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13123-13315: `{{metrics_json}}` from the run's live metric values
- characters 13408-14603: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14646-17155: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17211-18384: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_provenance_mandate_fight` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher and two national research funders impose mandatory AI-provenance and disclosure rules for submitted papers after the Millennium Prize claim, triggering a high-profile dispute over verification of AI-assisted proofs.
- `emergent_bio_publishing_freeze` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher or preprint server imposes mandatory AI-uplift screening and holds back several genome-model manuscripts after the phage demonstrations, triggering a public fight over censorship versus safety.
...
## Metrics
```json
{"ai_capability": 63.0, "openweight_capability": 55.0, "ai_safety": 22.0, "resilience": 35.0, "eu_ai_sovereignty": 19.0, "eu_political_capital": 26.0, "public_sentiment": 31.0}
```
