...
- characters 455-1657: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1701-3430: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4146-4621: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8123-9127: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M11 | EU Anti-Coercion Supply and Cure Access Shield | 8 | small | 9 | 12 | own jurisdiction, US and middle-power coalition | eu_ai_sovereignty protected/up slightly via leverage, public_sentiment up slightly via therapy access, eu_political_capital protected | 2 | finished |
| M12 | EU Pathogen Early-Warning and Care Continuity Net | 6 | small | 10 | 13 | own jurisdiction and biosurveillance coalition | resilience up materially, ai_safety up slightly via misuse detection | 2 | finished |
...
## New measure
None this turn, while we conserve collapsing capital until M12 lands and winter triage stabilises.

## Store changes
No changes.

## Priority
M12 EU Pathogen Early-Warning and Care Continuity Net, because it is the only funded vehicle holding health continuity and joint attribution together while capital cannot start anything new.

## In practice
We keep execution inside Health Council conclusions: HERA/ECDC run binding sample-sharing and pooled sequencing, ENISA holds the wastewater/clinic telemetry feed to the joint command, and the two energy contractors continue islanding drills. No new vote, no new money — Commission implementation decisions only.

We let M11 close on its narrow track: trade holds lithography open without escalation, joint therapy buying stays conditioned on audit and EU finishing. Readable triage certification stays mandatory to keep queues legible; Gigafactory grid works stay paused where blocked to avoid a siting fight we would lose at 14 sentiment.
...
```
## New measure
**EU Tailored Cure Access and Continuity Deployment**
It puts foreign tailored therapies to work inside EU hospitals under EU finishing, audit and readable triage rather than letting dependence alone decide who gets cured.
This turn pairs the medical breakthrough with the capability jump: deploying cures visibly through public hospitals is the only move that can lift sentiment and capital from 16 while holding continuity, and it uses existing HERA/ECDC and joint-procurement channels so it starts without a vote we would lose.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Tailored Cure Access and Continuity Deployment", "category": 9, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up materially, resilience up slightly via care continuity"}, "grounds": "medical breakthrough must reach wards to rebuild trust"}]}
```

## Priority
EU Tailored Cure Access and Continuity Deployment, because with no measures in flight it is the only funded vehicle that converts the breakthrough into visible cures and political room while the early-warning net holds.

## In practice
Health Council conclusions mandate HERA joint procurement plus EU finishing lots and ECDC allocation under mandatory readable triage, paid from existing HERA/EU4Health envelopes with no new money vote. ENISA wastewater/clinic feed and islanding drills continue by Commission implementation decision to keep wards lit and attribution fast.

We condition cure rollout on audit and EU finishing to keep the lithography file open without forcing a vote, and we pause contested Gigafactory grid works where sentiment at 16 would turn siting into defeat. Success is measured in electives resumed and queues legible, not in sovereignty gained.
