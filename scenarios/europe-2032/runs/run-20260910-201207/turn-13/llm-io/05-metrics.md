...
- characters 107-13406: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13449-13640: `{{metrics_json}}` from the run's live metric values
- characters 13733-14935: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14979-16708: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16764-17114: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M13 | EU Tailored Cure Access and Continuity Deployment | 2 |
...
## Metrics
```json
{"ai_capability": 93.6, "openweight_capability": 88.8, "ai_safety": 0.0, "resilience": 51.0, "eu_ai_sovereignty": 15.0, "eu_political_capital": 14.0, "public_sentiment": 19.0}
```
...
## Notepad
PORTFOLIO CHARGE t13: EU Tailored Cure Access and Continuity Deployment −2, priority −0 (capital below 20, no effect, no cost) = −2
PROPOSAL BONUS: none — medical breakthrough is positive external shock, not negative event to answer; small cat9 gesture earns nothing
LEGITIMACY LENDS: capital 14, sentiment 19 -> +2 (capped by sentiment, after charge/finish/jump)
SOVEREIGNTY: 16 last turn, no capacity event, no cat4 in flight +0, capability rose 2.8 −1 = 15
