...
- characters 455-1510: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1554-3643: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4372-4824: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7126-9013: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_energy_siting_backlash (emergent event):** Coordinated protests and blockades against new data-centre and grid-upgrade sites in France and Germany force temporary construction halts after energy-price rises are blamed on AI infrastructure.
...
|---|---|---|---|---|---|---|---|---|---|
| M6 | EU Cutoff Fallback Deployment | 9 | small | 6 | 8 | own jurisdiction | public_sentiment up, eu_political_capital up slightly, resilience up slightly | 2 | running |
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up materially, ai_safety up slightly via incident reporting | 3 | finished |
| M4 | EU Bio Detection and Response Net | 6 | large | 2 | 5 | own jurisdiction | resilience up materially, ai_safety up slightly via misuse detection | 3 | finished |
| M5 | EU Loss-of-Control Containment Playbook | 6 | small | 3 | 5 | own jurisdiction | resilience up, ai_safety up slightly via containment and reporting | 2 | finished |
...
## New measure
**EU Cutoff Fallback Deployment**
It swaps EU-run models into the hospitals, ministries and firms cut off from the US frontier via EU AI Office procurement and HERA/health ministry channels, turning the cutoff into visible continuity.
Why this and why now: with access denied at short notice and political capital at 10, finishing gigafactories alone does not keep services running this semester, while the adoption-success window gives us domestic benefit to show for it.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cutoff Fallback Deployment", "category": 9, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, eu_political_capital up slightly, resilience up slightly"}, "grounds": "US cutoff leaves hospitals/ministries stranded while adoption success offers legitimation"}]}
```

## Priority
EU Cutoff Fallback Deployment, because restoring service continuity now is what stops defection and rebuilds capital to finish InvestAI Gigafactories.

## In practice
We task the AI Office with an emergency conformity route for EU-hosted open and domestic models, funded by re-scoped Digital Europe deployment money and EIB liquidity, with ENISA hardening the swap and HERA certifying clinical use. Health and Competitiveness Councils give the legal base; defector member state is offered fallback capacity on condition it returns to common procurement.

We keep M1 alive on its existing €200bn envelope without new cash this turn, using DG COMP IPCEI clearance and permitting zones to hold France-Germany-Poland siting, trading grid-connection priority for regional consent while lithography remains quarantined. Shield, Bio Net and Playbook run on staff already assigned, with the joint incident cell focused on eval-anomaly triage and embodied-AI logistics risks.
...
```
## New measure
**Emergency Cyber Hardening for Swapped Public Systems**
It pushes ENISA-led patching, offline backups and misuse monitoring onto the hospitals, tax offices and logistics systems just migrated to EU fallback models after the US cutoff and the verifiable-domain capability jump.
Why this and why now: the cutoff swap left public systems on unfamiliar models exactly as code/cyber attacker capability jumps, and with capital at 12 this small hardening buys resilience without reopening the stalled gigafactory siting fight inflamed by the French-German energy backlash.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Emergency Cyber Hardening for Swapped Public Systems", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly"}, "grounds": "capability jump in cyber/code hits freshly swapped public systems"}]}
```

## Priority
M6 EU Cutoff Fallback Deployment, because keeping clinics and ministries running through the cutoff is what stops further defection and earns the consent needed for any later grid and cyber work.

## In practice
We keep the AI Office emergency conformity route and HERA clinical certification as the front door, with Digital Europe redeployment and EIB liquidity paying ENISA joint teams to stay in Rotterdam, Lyon, Hamburg and Gdansk hospitals and ministries until fallback models are stable. The defector member state is offered fallback slots only if it rejoins common procurement, and Health Council provides cover.

We do not push new cranes or grid priority this turn while France-German blockades halt sites; InvestAI Gigafactories and the sovereignty package sit on existing permits, IPCEI clearance and private pledges, with Commission mediation offering energy-bill relief dialogue to unions in Lille rather than forced siting. The new cyber patch runs through the Shield/Bio Net staff and joint incident cell already assigned, focused on backup restoration drills and monitoring for the leaked-eval anomaly pattern, with no new cash call to spare political capital.
