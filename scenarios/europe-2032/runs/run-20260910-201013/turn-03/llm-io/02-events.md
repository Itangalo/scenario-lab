...
- characters 377-1542: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1586-3540: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3596-4716: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4779-29006: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29361-29531: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M3 EU Critical Systems Shield −3, M4 EU Bio Detection and Response Net −3, priority −1 = −13
PROPOSAL BONUS: EU Bio Detection and Response Net (cat 6, large) +4 — bio_uplift_findings landed this turn opening four-turn bio gate and this answers it directly
...
- `emergent_bio_publishing_freeze` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher and university consortium announces a temporary moratorium on publishing AI-assisted pathogen-design methods after the phage results, triggering a contested preprint freeze debate.
- `emergent_phage_therapy_breakthrough` -- first noted turn 1, listed in 2 turn(s) so far: Major European research hospitals report AI-designed phage therapies clearing antibiotic-resistant infections in compassionate-use cases, creating public pressure to fast-track genome-model access.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, openweight_frontier_release, knowledge_work_augmented, automated_decision_scandal
- Turn 2 (1 turn(s) ago): bio_uplift_findings
