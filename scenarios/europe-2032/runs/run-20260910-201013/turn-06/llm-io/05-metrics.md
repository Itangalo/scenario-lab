...
- characters 105-13369: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13412-13604: `{{metrics_json}}` from the run's live metric values
- characters 13697-14848: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14891-16893: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16949-17956: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Cutoff Fallback Deployment | 2 |
...
- `emergent_energy_siting_backlash` -- first noted turn 5, listed in 2 turn(s) so far: Coordinated protests and blockades against new data-centre and grid-upgrade sites in France and Germany force temporary construction halts after energy-price rises are blamed on AI infrastructure.
- `emergent_winter_grid_strain` -- first noted turn 5, listed in 2 turn(s) so far: A severe winter storm exposes thin power margins in France and Germany, forcing emergency load-shedding that is publicly blamed on delayed grid segmentation and backup relay work.
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 elected inward-turning administration takes office turn 6"}]}
```
```
