...
- characters 107-13303: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13346-13537: `{{metrics_json}}` from the run's live metric values
- characters 13630-15102: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15146-17138: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17194-17585: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M10 | EU Trusted Public Services Adoption Sprint | 2 |
...
## Notepad
PORTFOLIO CHARGE t13: M10 EU Trusted Public Services Adoption Sprint −2, priority −0 (capital below 20, no effect no cost) = −2
PROPOSAL BONUS: none — knowledge_work_augmented removes displacement crisis and adoption_success window is positive not negative; no recent negative event answered
LEGITIMACY LENDS: capital 0, sentiment 27 -> +2 (capped below sentiment, applied after charges and event losses)
SOVEREIGNTY: 15 last turn, no category 4 in flight +0, export_control_escalation t13 −2, capability rose 0.7 −0 = 13
