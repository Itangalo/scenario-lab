...
- characters 1971-3963: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6301-6774: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9423-10581: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `act_under_uncertainty` (commitment): We will hold deployed systems and essential services first, and defer new frontier builds until consent and capital recover
- `two_year_commitment` (commitment): Keep essential public services and livelihoods running through AI-driven disruption
...
| M6 | EU Cutoff Fallback Deployment | 9 | small | 6 | 8 | own jurisdiction | public_sentiment up, eu_political_capital up slightly, resilience up slightly | 2 | finished |
| M7 | Emergency Cyber Hardening for Swapped Public Systems | 6 | small | 7 | 9 | own jurisdiction | resilience up, ai_safety up slightly | 2 | finished |
| M8 | EU Essential Services Continuity Corps | 6 | small | 9 | 11 | own jurisdiction | resilience up materially, public_sentiment up slightly, eu_political_capital up slightly | 2 | finished |
| M9 | EU Verifiable-Capability Patch Cell | 6 | small | 11 | 13 | own jurisdiction | resilience up slightly, ai_safety up slightly | 2 | finished |
...
## New measure
None this turn, while capital sits at 6.0 and the Patch Cell is still the only staffed line.

## Store changes
No changes.

## Priority
M9 EU Verifiable-Capability Patch Cell, because it is the only instrument that keeps essential services open through the open-weight jump and the new intrusion tradecraft without a vote we would lose.

## In practice
We keep the ENISA-AI Office-Lyon/Gdansk cell on hospitals and municipal counters in Rotterdam, Hamburg and the exposed wards, extending quarantine playbooks to the open-weight tooling now on private hardware permanently. No new siting or frontier build is pushed; councils refusing and grid freezes mean it would die and take the last capital with it.

We note the US-China limited agreement from outside: with no domestic frontier, no compute to offer, and enforcement of our own AI Act discredited by the social-insurance risk-scoring affair, we are informed afterwards, not consulted. We use the pause to log incidents and keep rosters and drills alive, and we let the Act failure be read as enforcement failure where possible, to avoid making adoption politically impossible for years.
...
```
## New measure
**EU Trusted Public Services Adoption Sprint**
Puts audited AI assistants into health triage, benefits appeals backlogs and tutoring in willing regions to turn the buried productivity gains into falling waiting lists people can feel. Category 9 Diffusion, adoption and public trust, because with capital at 3 and siting blocked this is the only build that does not need a council vote for a data centre.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trusted Public Services Adoption Sprint", "category": 9, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, eu_political_capital up slightly, resilience up slightly"}, "grounds": "adoption success window plus office productivity gains without layoffs"}]}
```

## Priority
M10 EU Trusted Public Services Adoption Sprint, because it is the only new spend that rebuilds consent for the two-year commitment to keep services running instead of spending the last capital against councils and grid freezes.

## In practice
We run this through DG REFORM and the AI Office with willing cities and hospital groups in Lyon, Gdansk and Rotterdam wards where the Patch Cell already has trust, using EU-procured models run on existing capacity, with ombudsman-audited logs and human sign-off published to answer the risk-scoring scandal directly.

We do not push new siting, frontier builds or export-control retaliation this turn; with opaque reasoning killing chain-of-thought oversight we extend the Patch Cell playbooks as drills and rosters only, and we bank the US-China pause to log incidents. The Sprint's waiting-list and decision-time wins are what we will point to to lift sentiment from 19 and earn back room to harden.
