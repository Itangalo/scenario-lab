...
- characters 377-1939: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1983-4027: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4083-5238: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5301-28877: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29232-29327: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU AI Evaluation and Safety Institute −3, EU Essential Services Cyber Shield −2, priority −1 = −12
PROPOSAL BONUS: Cyber Shield (cat 6, small) +2 — openweight_frontier_release landed this turn and this answers diffusion-driven intrusion risk directly, grid probe context adds weight
...
- `emergent_energy_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: Coordinated local protests and court challenges stall two announced AI factory grid connections over power prices and water use, delaying site works without cancelling the programme.
- `emergent_inference_throttling` -- first noted turn 2, listed in 1 turn(s) so far: A major US hyperscaler quietly throttles inference capacity available to EU enterprise customers during peak demand, citing domestic load and new review requirements, forcing temporary service degradation in firms and public bodies built on its models.
