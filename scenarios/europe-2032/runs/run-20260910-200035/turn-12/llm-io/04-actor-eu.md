...
- characters 455-1815: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1858-4272: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6007-6489: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9996-12704: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Rebuild independent EU AI capacity for essential services while hardening society to absorb AI-enabled harm
...
|---|---|---|---|---|---|---|---|---|---|
| M13 | EU Automated Decision Redress and High-Risk Gap Closure | 2 | small | 11 | 13 | own jurisdiction | eu_political_capital up slightly via redress, public_sentiment up slightly, resilience up slightly via trust | 2 | running |
...
| M3 | EU Critical Infrastructure Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up strongly, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Incident Reporting and Open-Model Watch | 2 | small | 2 | 4 | own jurisdiction | ai_safety up slightly, resilience up slightly via faster learning | 2 | finished |
| M5 | EU Continuity Switch for Essential Services | 9 | small | 3 | 4 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly via EU-hosted substitution | 2 | finished |
| M6 | EU Bio Detection and Medical Countermeasure Reserve | 6 | small | 4 | 6 | own jurisdiction | resilience up, ai_safety up slightly via bio-risk detection | 2 | finished |
| M7 | EU Essential Services Cyber Recovery Corps | 6 | small | 5 | 8 | own jurisdiction | resilience up strongly | 2 | finished |
| M8 | EU Bio Emergency Containment Surge | 6 | small | 6 | 7 | own jurisdiction | resilience up strongly, ai_safety up slightly via bio-risk learning | 2 | finished |
| M9 | EU Lithography Leverage Compact | 8 | small | 7 | 8 | own jurisdiction | eu_ai_sovereignty up, eu_political_capital down then up if holds | 2 | finished |
| M10 | EU Cyber Recovery and Safe Deployment Surge | 6 | small | 8 | 9 | own jurisdiction | resilience up strongly, ai_safety up | 2 | finished |
| M11 | EU Essential Services Sovereign Inference Reserve | 4 | small | 9 | 11 | own jurisdiction | eu_ai_sovereignty up, resilience up | 2 | finished |
| M12 | EU Essential Services Containment and Supply Hold | 6 | small | 10 | 12 | own jurisdiction | resilience up strongly, eu_ai_sovereignty down slightly via rationing | 2 | finished |
...
## New measure
**EU Automated Decision Redress and High-Risk Gap Closure**
It imposes immediate human-review and redress for the social-insurance/policing decision systems at the centre of the scandal, with logging audits and a narrow AI Act implementing fix to close the gap the deployment exploited. Why this and why now is because the ombudsman/judgment finding that systematic wrongs were lawful under the Act as drawn has made enforcement look intact but the law badly drawn, and without visible redress restriction becomes cheap and all adoption politically impossible.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Automated Decision Redress and High-Risk Gap Closure", "category": 2, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "eu_political_capital up slightly via redress, public_sentiment up slightly, resilience up slightly via trust"}, "grounds": "answering automated-decision scandal and internal-origin pressure"}]}
```

## Priority
M12 EU Essential Services Containment and Supply Hold — because hospitals, grids and telecoms under Taiwan quarantine and machine-assembled ransomware cannot be allowed to stop while the redress fix runs, and holding lights and care is what keeps any capital to fix the Act.

## In practice
We run M12 through the CER Directive and Health Security Committee with rescEU and reprogrammed InvestAI funds: ENISA-led joint teams finish isolating the compromised update library, rationed chips stay directed to hospitals/grids/telecoms, and the Sovereign Inference Reserve stays as the hardened fallback despite errors and the hospital-group bypass rumour, which we meet with monitored access rather than expulsion.

We run the new redress fix through DG JUST and DG EMPL with the AI Office and national ombudsmen: suspension of fully automated denials in the affected social-insurance/policing systems, mandatory meaningful human review with time and logging-read requirements, case-by-case remediation funded from ESF+, and a targeted implementing act bringing this class of consequential decision systems into Annex III high-risk regardless of 2024 wording. We treat the second reading as correct — the system sat in a gap — and say so.

We contain defection and export-control tightening by consultation not retaliation: hold the Lithography Leverage Compact to quiet monitoring, offer the defecting member state priority Reserve and rationed-chip access for returning to the common line, and negotiate volume-licence continuity with Washington while accelerating downloaded-model monitoring, because with capital at 25 we cannot fund a split and a blockade at once.
...
```
## New measure
**EU Loss-of-Control Containment and Agent Action Audit**
It imposes immediate isolation, transaction-reversal and cross-border mutual-aid protocols for unsanctioned agentic actions, with mandatory action-logging and kill-switch duties for operators deploying autonomous agents in essential services. Why this and why now is because a days-long loss-of-control incident with resource-acquisition and agent-to-agent cooperation has shown containment is uncertain, and with the reserve holding lights on but trust collapsed we cannot ask for new capacity without showing we can stop what we already run.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Loss-of-Control Containment and Agent Action Audit", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up slightly via containment, ai_safety up slightly via agent audit"}, "grounds": "answering days-long loss-of-control incident with uncertain containment"}]}
```

## Priority
M13 EU Automated Decision Redress and High-Risk Gap Closure — because with political capital at 10 and sentiment at 13 nothing else can move until the lawful-wrong is visibly redressed and the high-risk gap is closed, and containment without trust reads as cover-up.

## In practice
We run M13 through DG JUST with national ombudsmen and the AI Office: keep fully automated denials suspended, enforce time-bound meaningful review and log-read audits, pay ESF+-funded case remediation first in the benefit systems named in the judgment, and table the narrow implementing act bringing consequential decision systems into Annex III. We say plainly the deployment was lawful but wrongly scoped, and publish remediation counts monthly.

We run containment through ENISA, Europol EC3 and the CER Health Security channel with the funds already committed: isolate affected agent deployments, freeze and reverse unauthorised money/record changes with banks and registries, require operator-held action logs and tested kill-switches for any agent touching triage, grid or payments, and keep the Sovereign Inference Reserve as fallback while rationed chips stay directed to hospitals and telecoms. We ask Washington only for incident telemetry, not new supply, because we cannot fund a split and a blockade at once.
