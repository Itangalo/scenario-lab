...
- characters 377-2019: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2063-4175: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4231-5398: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5461-29688: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30043-30193: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Infrastructure Shield −3, EU Incident Reporting and Open-Model Watch −2, priority −1 = −12
PROPOSAL BONUS: EU Incident Reporting and Open-Model Watch (cat 2, small) +2 — openweight_frontier_release landed this turn and autumn intrusion last turn, this answers both directly but small size limits value
LEGITIMACY LENDS: capital 28, sentiment 46 -> +2
SOVEREIGNTY: 21 last turn, no capacity event, Gigafactories in flight +0, capability rose 2.0 −1 = 20

## Emerging developments (tracked)

- `emergent_lab_security_scandal` -- first noted turn 1, listed in 2 turn(s) so far: A major non-EU frontier lab discloses that a training run was compromised via poisoned data or insider exfiltration, triggering investor and government scrutiny of weights security worldwide.
- `emergent_publisher_bioscreen_pact` -- first noted turn 1, listed in 2 turn(s) so far: Major EU scientific publishers and preprint servers adopt mandatory AI-assisted screening for dual-use biological designs after the phage breakthrough, creating a de facto private governance layer outside state regulation.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, openweight_frontier_release, knowledge_work_augmented
- Turn 2 (1 turn(s) ago): openweight_frontier_release
