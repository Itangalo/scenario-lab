...
- characters 16346-16970: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_lab_security_scandal` -- first noted turn 1, listed in 1 turn(s) so far: A major non-EU frontier lab discloses that a training run was compromised via poisoned data or insider exfiltration, triggering investor and government scrutiny of weights security worldwide.
- `emergent_publisher_bioscreen_pact` -- first noted turn 1, listed in 1 turn(s) so far: Major EU scientific publishers and preprint servers adopt mandatory AI-assisted screening for dual-use biological designs after the phage breakthrough, creating a de facto private governance layer outside state regulation.
...
PROPOSAL BONUS: none — Shield answers cyber_test_shot directly but knowledge_work_augmented suppresses rule 6 proposal bonus per event text, no displacement crisis to point at
LEGITIMACY LENDS: capital 36, sentiment 45 -> +2
