...
- characters 105-13071: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13114-13306: `{{metrics_json}}` from the run's live metric values
- characters 13399-14564: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14607-16457: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16513-17597: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated municipal moratoriums on new hyperscale data centres spread across several member states over power prices and water use, forcing at least one announced EU compute expansion to pause for review.
- `emergent_safety_leak_press` -- first noted turn 1, listed in 2 turn(s) so far: Major European press investigation publishes leaked internal safety evaluations from a frontier lab showing withheld cyber-offensive results, triggering parliamentary inquiries and calls for mandatory disclosure.
...
## Metrics
```json
{"ai_capability": 58.0, "openweight_capability": 51.0, "ai_safety": 30.0, "resilience": 36.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 34.0, "public_sentiment": 33.0}
```
