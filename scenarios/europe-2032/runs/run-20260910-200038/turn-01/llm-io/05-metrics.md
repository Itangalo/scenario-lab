...
- characters 106-13017: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13060-13252: `{{metrics_json}}` from the run's live metric values
- characters 13368-16317: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 16373-17001: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated municipal moratoriums on new hyperscale data centres spread across several member states over power prices and water use, forcing at least one announced EU compute expansion to pause for review.
- `emergent_safety_leak_press` -- first noted turn 1, listed in 1 turn(s) so far: Major European press investigation publishes leaked internal safety evaluations from a frontier lab showing withheld cyber-offensive results, triggering parliamentary inquiries and calls for mandatory disclosure.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 46.0, "ai_safety": 32.0, "resilience": 37.0, "eu_ai_sovereignty": 21.0, "eu_political_capital": 44.0, "public_sentiment": 39.0}
```
## Narrative
### The autumn audit
...
PROPOSAL BONUS: EU Critical Systems Cyber Shield (cat 6, large) +4 — cyber_test_shot precursor landed this turn with EU grids affected and this answers it directly
