...
- characters 455-1351: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1395-3708: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4100-4560: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7039-8980: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | Municipal Siting Compact | 4 | small | 6 | 8 | own jurisdiction | eu_ai_sovereignty up, public_sentiment up slightly via unblocking sites | 2 | running |
...
## New measure
**Municipal Siting Compact**
Offers water-neutral cooling, grid-backed power packages and local revenue shares to lift the municipal connection moratoria freezing gigafactory-adjacent sites, paired with fast-track referenda support.
This is a Category 4 Sovereignty and industrial capacity fix because the moratoria wave directly stalls the independent compute our two-year commitment depends on, and RSI makes physical infrastructure the only remaining bottleneck — we cannot outrun capability without unblocking soil, power and water now.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Municipal Siting Compact", "category": 4, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up, public_sentiment up slightly via unblocking sites"}, "grounds": "moratoria wave freezes permitted sites amid RSI infrastructure bottleneck"}]}
```

## Priority
M1 InvestAI Gigafactories, because the moratoria wave plus US tiering threatens to strand our only sovereign-compute line — unblocking sites outranks patching and disclosure this turn.

## In practice
We task DG ENER, DG ENV and the AI Office with a joint siting taskforce: CEF Energy and Cohesion reprogramming for closed-loop cooling retrofits, backup-power guarantees via TSO contracts, and model host-community benefit agreements that give councils a share of connection revenues ahead of local referenda. Competitiveness Council gives mandate; mayors who lifted holds get first disbursement.

Alongside, ENISA keeps pushing the Shield patch surge into thin municipals with mobile crews, and we bank the Disclosure Act first reading for early warning on RSI cadence. We publicly claim the European medical/materials breakthroughs as proof anchored compute delivers public benefit, to soften hostility without opening a second front.
...
## New measure
**EU Patch Commons**
Deploys the breakthrough automated patching and swarm-detection tooling EU-wide via ENISA into critical services and municipalities.
This is a Preparedness and resilience move because the offence-defence shift is a closing window to lock in degradation-not-stoppage before capability jumps again, and it protects the sovereign compute build-out without opening a second large front while capital is thin.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Patch Commons", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "cyber defence breakthrough opens window to lock in hardening"}]}
```

## Priority
M8 Municipal Siting Compact, because unblocking gigafactory-adjacent power and water remains the binding constraint on our two-year commitment to non-withdrawable capacity, and the patch window can ride on existing Shield crews without displacing it.

## In practice
We keep the joint DG ENER-DG ENV-AI Office siting taskforce as the main push: closed-loop cooling kits, TSO backup contracts and host-community revenue shares disbursed first to councils that lift moratoria, with Competitiveness Council cover for fast-track referenda in Flanders, Bavaria and Lombardy. This spends our remaining capital where soil meets sovereignty.

Alongside, ENISA converts the Shield patch surge teams into Patch Commons distributors, pushing the new automated patching and swarm-detection builds into energy, health, telecoms and thin municipalities via pooled SOCs, funded by reprogramming Digital Europe, not a new fund. We claim both Rotterdam/Milan therapies and grid survival as proof anchored capacity plus absorption holds.
