...
- characters 455-2031: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2075-4172: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4722-5212: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8349-10700: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M12 | EU Hospital Power Guard and Auto-Patch Deployment | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, public_sentiment up slightly via kept power and patches | 2 | running |
...
## New measure
**EU Hospital Power Guard and Auto-Patch Deployment**
Deploys the new automated patching and swarm-detection tooling across hospitals, ministries and grid operators while putting hospital backup power and operating interconnectors under the same guarded-repair regime as the construction sites.
This is a Preparedness and resilience move because coordinated sabotage has jumped from building sites to live interconnectors and hospital backup power just as a real defensive breakthrough and a medical breakthrough arrive, and with safety at 3 and capital at 21 only a small deployment tied to the existing Shield Corps can turn those windows into continuity without US access.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Hospital Power Guard and Auto-Patch Deployment", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up slightly via kept power and patches"}, "grounds": "sabotage hit operating grid and hospital backup as defensive tooling breakthrough arrived"}]}
```

## Priority
EU Site Shield and Repair Corps, because the guarded repair zones in Flanders, Lombardy and Bavaria are now the only thing keeping both halted compute works and live interconnectors from cascading into outages.

## In practice
We extend the Critical Entities Resilience protected-repair orders from the backup lines and cooling trenches to the two member states' operating interconnectors and hospital backup feeds, tasking the same gendarmerie vans, military engineers and rapid spares convoys under ENTSO-E and ENISA coordination, funded by reprogrammed CEF and Digital Europe money. Municipal policing deals bought with accelerated revenue shares in Flanders and Lombardy are rolled to interconnector corridors; in Bavaria we frame substation guard as hospital-continuity, not site protection.

Alongside, ENISA and HERA push the breakthrough auto-patching and swarm-detection into the EU-hosted substitutes and vetted open stacks that hospitals and ministries run on, keeping the hours patch cycle alive without readable frontier traces, and we procure the new tailored therapies only on EU-hosted compute where possible so the medical gain reads as dependence reduced, not dependence demonstrated.
...
Surges ENISA-led machine-speed isolation, clean rebuilds from EU-hosted images, and guarded manual fallbacks to contain the automated attack across public services and infrastructure.
This is a Preparedness and resilience move because with safety at 1 and openweight at 80.5 the model-generated attack will cascade without a centrally coordinated containment, and with capital at 16 only a small surge built onto the existing Hospital Power Guard and auto-patch deployment can hold continuity under our two-year commitment to keep essential services operable without US access.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Automated Attack Containment and Essential Service Restoration", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately to contain cascading damage"}, "grounds": "large automated model-generated attack landing on brittle services"}]}
```

## Priority
EU Automated Attack Containment and Essential Service Restoration, because the live major incident outranks construction-site guarding and will decide whether hospitals, grids and ministries stay running through December.

## In practice
We trigger the Cyber Solidarity Act emergency reserve and task ENISA's Cyber Reserve with CERT-EU to push kill-switches, network segmentation and clean EU-hosted rebuild images to hit administrations, hospitals and grid operators, keeping the hours patch cycle from M12 running under attack. The protected-repair orders are extended to incident response: same gendarmerie-engineer convoys escort spares and diesel while operators run guarded rolling restoration.

We fund this by reprogramming Digital Europe and CEF-Digital unspent lines, with Health Emergency Preparedness and Response Authority co-procurement for hospital IT rebuilds. Interior and Telecoms Council formations are asked for a 6-month mandate; Flanders/Lombardy municipal policing deals are copied to hit regions, and Bavaria hospital-guard framing is used to avoid renewed blockade politics. No new US frontier access is sought — restoration is explicitly on EU-hosted and vetted-open stacks to hold the two-year commitment.
```
