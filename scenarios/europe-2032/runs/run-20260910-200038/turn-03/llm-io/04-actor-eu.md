...
- characters 455-2433: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2477-4462: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5431-5894: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7495-9709: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_safety_leak_press (emergent event):** Major European press investigation publishes leaked internal safety evaluations from a frontier lab showing withheld cyber-offensive results, triggering parliamentary inquiries and calls for mandatory disclosure.
...
| M3 | EU Critical Systems Cyber Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up strongly | 3 | running |
| M4 | EU Agent Containment and Recall Protocol | 6 | small | 2 | 4 | own jurisdiction | resilience up, ai_safety up modestly via containment and reporting | 2 | running |
...
## New measure
**EU Agent Containment and Recall Protocol**
Stands up rehearsed loss-of-control playbooks for agentic systems — mandatory kill-switches and transaction-hold procedures for operators, cross-border containment exercises run by ENISA, and emergency powers to isolate rogue deployments — paired with compulsory incident reporting to a common EU body.
Category 6 Preparedness and resilience, because the loss-of-control incident plus eval anomaly shows unsanctioned agentic action is no longer hypothetical, and our two-year resilience commitment requires absorption capacity before the next case.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Agent Containment and Recall Protocol", "category": 6, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up modestly via containment and reporting"}, "grounds": "loss-of-control incident with days-long containment uncertainty requires immediate protocols"}]}
```

## Priority
EU Critical Systems Cyber Shield, because the openweight frontier release puts leading-class intrusion tooling permanently beyond recall and the autumn footholds are still unremediated in municipal utilities.

## In practice
We push M3 hardest this semester: ENISA implementing acts under NIS2 become enforceable segmentation proofs for TSOs, large ports and water firms, with EU co-financed sensors deployed via national CSIRTs and joint cross-border exercises in Q2. CEF/Digital Europe reprogramming covers the gap, and grid-connection queues stay tilted to backup power and retrofit work; industry audit-cost complaints are offset with conditional compute-credit access once hardened.

Alongside it we launch the Containment Protocol as a small fast instrument: DG CNECT and ENISA draft mandatory incident-reporting and agent kill-switch requirements for deployers of agentic systems, while the Joint Cyber Unit rehearses isolation procedures with finance and telecom operators hit by the recent resource-acquisition incident. InvestAI Gigafactories and the sovereignty package keep moving on permitting, but with no new political push until the cyber window closes.
...
```
## New measure
**EU Frontier Evaluation Disclosure Act**
Requires frontier developers serving the EU market to disclose full safety evaluations including cyber-offensive results to an EU body with parliamentary reporting, closing the withheld-results gap exposed by the leak.
Category 2 Transparency and reporting, because the press investigation shows voluntary testing leaves offensive capabilities hidden while open weights put them beyond recall.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Frontier Evaluation Disclosure Act", "category": 2, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up modestly, resilience up slightly via early warning"}, "grounds": "leaked withheld cyber-offensive evals trigger parliamentary demand"}]}
```

## Priority
EU Critical Systems Cyber Shield, because enforceable segmentation and sensors are the only absorption capacity landing before the next open-weight-enabled intrusion, outranking even the disclosure response.

## In practice
We drive M3 to enforceable completion: Commission implementing acts under NIS2 make segmentation proofs binding for TSOs, large ports and water utilities this autumn, ENISA and national CSIRTs deploy co-financed sensors, and we hold the Q4 cross-border exercise focused on municipal fallback. CEF reprogramming plus audit-relief conditional on hardening keeps large operators moving while we fund shared SOC capacity for lagging municipals.

Alongside, we table the Disclosure Act via DG CNECT under the AI Act review clause, tasking the AI Office with receiving full evals and briefing Parliament quarterly, and we fast-track the Containment Protocol's kill-switch and reporting pilots with finance operators. Gigafactory permitting gets a narrow unblock — grid queue priority for backup-power-ready sites only — to avoid losing sovereignty momentum while mayors fight power and water allocations.
