...
- characters 377-1594: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1638-3589: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3645-4751: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4814-32362: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32717-32963: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Frontier Evaluation Disclosure Act −2, Municipal Resilience Backstop −2, priority −1 = −11
PROPOSAL BONUS: none — no external event in last three turns that this cat 6 small directly answers; municipal lag is ongoing condition, not priced event
...
- `emergent_municipal_moratoria_wave` -- first noted turn 4, listed in 1 turn(s) so far: A coordinated wave of municipal moratoria on new data-centre water and power connections spreads across several member states, freezing permitted gigafactory-adjacent sites for months pending local referenda.
- `emergent_shield_noncompliance_revolt` -- first noted turn 4, listed in 1 turn(s) so far: Municipal utilities in two member states publicly refuse to implement mandated segmentation proofs, citing costs and staffing, triggering infringement threats and a court challenge over isolation powers.
...
- Turn 2 (3 turn(s) ago): loss_of_control_incident, eval_anomaly_reports, openweight_frontier_release
- Turn 3 (2 turn(s) ago): openweight_frontier_release, taiwan_tension_rise, emergent_safety_leak_press
