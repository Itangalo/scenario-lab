...
- characters 455-1572: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1616-3681: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6312-6802: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10306-12319: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Defend essential services and EU compute sites through sabotage and make them physically operable without US access
...
|---|---|---|---|---|---|---|---|---|---|
| M14 | EU Degraded Operations Continuity Pact | 6 | small | 12 | 14 | own jurisdiction | resilience up moderately, public_sentiment up slightly via kept services | 2 | running |
...
| M3 | EU Critical Systems Cyber Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up strongly | 3 | finished |
| M4 | EU Agent Containment and Recall Protocol | 6 | small | 2 | 4 | own jurisdiction | resilience up, ai_safety up modestly via containment and reporting | 2 | finished |
| M5 | EU Frontier Evaluation Disclosure Act | 2 | small | 3 | 6 | own jurisdiction | ai_safety up modestly, resilience up slightly via early warning | 2 | finished |
| M6 | Municipal Resilience Backstop | 6 | small | 4 | 5 | own jurisdiction | resilience up moderately | 2 | finished |
| M7 | EU Shield Patch Surge | 6 | small | 5 | 6 | own jurisdiction | resilience up strongly | 2 | finished |
| M8 | Municipal Siting Compact | 4 | small | 6 | 8 | own jurisdiction | eu_ai_sovereignty up, public_sentiment up slightly via unblocking sites | 2 | finished |
| M9 | EU Patch Commons | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately | 2 | finished |
| M10 | EU Essential Continuity Reserve | 6 | small | 8 | 9 | own jurisdiction | resilience up moderately, public_sentiment up slightly via continuity | 2 | finished |
| M11 | EU Site Shield and Repair Corps | 6 | small | 9 | 11 | own jurisdiction | resilience up moderately, eu_ai_sovereignty up slightly via unblocking halted sites | 2 | finished |
| M12 | EU Hospital Power Guard and Auto-Patch Deployment | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, public_sentiment up slightly via kept power and patches | 2 | finished |
| M13 | EU Automated Attack Containment and Essential Service Restoration | 6 | small | 11 | 13 | own jurisdiction | resilience up moderately to contain cascading damage | 2 | finished |
...
## New measure
**EU Degraded Operations Continuity Pact**
Keeps hospitals, grids and municipal services operable on paper-heavy manual fallbacks and guarded EU-hosted rebuilds through the open-weight sweep, without US frontier access.
This is a Preparedness and resilience move because with safety at zero, openweight at 83 and another frontier leak imminent, essential services will only survive if degraded operation is funded and staffed, and with capital at 21 only a small pact extending M13 and M12 can hold our two-year commitment to keep sites physically operable.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Degraded Operations Continuity Pact", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up slightly via kept services"}, "grounds": "open-weight automated sweep with zero safety assurance"}]}
```

## Priority
EU Automated Attack Containment and Essential Service Restoration, because the live containment deciding whether rebuilds land this spring outranks the follow-on continuity pact still starting up.

## In practice
We keep ENISA/CERT-EU segmentation and clean EU-hosted rebuilds from M13 as the main effort, using the Digital Europe emergency reserve and HERA co-procurement, with Interior Council mandate extended to June. Gendarmerie-engineer escorts stay on hospitals and grid rebuild sites, using the Bavaria hospital-guard framing to avoid Munich-type blockades.

We start the Continuity Pact by tasking DG CNECT and DG SANTE to fund diesel, backup tapes, paper fallback kits and overtime for manual operations in Wallonia, Lombardy and Bavaria hit districts, copied from the Flanders municipal policing deals. No new US access is requested; restoration stays on EU-hosted and vetted-open images to hold operability without US access, while export-control tightening is handled by rationing remaining allied compute to rebuilds first.
...
```
## New measure
**EU Wartime Joint Cyber-Bio Defence Accession**
Joins the joint cyber command telemetry pool and biosurveillance sample-sharing pact to plug EU hospitals, grids and compute sites into allied attribution and response while US-China fighting targets cables, fabs and data centres.
This is an International coordination and leverage move now because with a major automated attack landing, bio-uplift opening the bio gate, safety at zero and war making European infrastructure a target, protection we cannot build alone outranks building alone, and it directly extends our two-year commitment to keep essential services physically operable without US access.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Wartime Joint Cyber-Bio Defence Accession", "category": 8, "size": "small", "finish_turn": 15, "applies_to": "coalition", "targeted_effect": "resilience up moderately via joint telemetry and biosurveillance"}, "grounds": "war and concurrent cyber/bio shocks requiring pooled protection"}]}
```

## Priority
EU Wartime Joint Cyber-Bio Defence Accession, because pooled attribution, telemetry and medical countermeasure access during active great-power war outranks even the Continuity Pact for whether degraded services survive this winter.

## In practice
We keep M14 as the operating base: DG CNECT, DG SANTE and ENISA/CERT-EU fund diesel, overtime, paper fallbacks and clean EU-hosted rebuilds in Wallonia, Lombardy and Bavaria under the Interior and Health Council mandate, with gendarmerie-engineer escorts. No new US bilateral access is sought; rationed allied compute stays on rebuilds first.

We execute the accession via Foreign Affairs and General Affairs Councils under the Cyber Solidarity Act and HERA/European Health Union legal bases, seconding ENISA liaison officers to the joint cyber command and ECDC to the biosurveillance standing mechanism, with Digital Europe and EU4Health emergency funds covering telemetry nodes and sample-sharing labs. France, Germany and the Netherlands host the uplinks; neutrality and data-protection objections are met with EU-retained data residency and a war-limited mandate tied to our essential-services commitment.
