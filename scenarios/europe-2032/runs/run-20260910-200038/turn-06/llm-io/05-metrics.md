...
- characters 105-13669: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13712-13904: `{{metrics_json}}` from the run's live metric values
- characters 13997-15059: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15102-17495: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17551-18097: `{{notepad}}` from the Game Master's notepad, carried across turns
...
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Frontier Evaluation Disclosure Act −2, EU Shield Patch Surge −2, priority −1 = −11
PROPOSAL BONUS: EU Shield Patch Surge (cat 6, small) +4 — major automated attack landed this turn and defensive breakthrough gives it something to deploy, answers directly
LEGITIMACY LENDS: capital 20, sentiment 24 -> +2 capped? applied as +1 to reach 20 (reported after all terms)
...
**emergent_municipal_moratoria_wave (emergent event):** A coordinated wave of municipal moratoria on new data-centre water and power connections spreads across several member states, freezing permitted gigafactory-adjacent sites for months pending local referenda.
...
## Metrics
```json
{"ai_capability": 75.0, "openweight_capability": 63.8, "ai_safety": 17.0, "resilience": 50.0, "eu_ai_sovereignty": 19.0, "eu_political_capital": 20.0, "public_sentiment": 31.0}
```
## Narrative
### The cadence breaks
...
## Notepad
PORTFOLIO CHARGE t6: InvestAI Gigafactories −3, Municipal Siting Compact −2, priority −1 = −6
PROPOSAL BONUS: Municipal Siting Compact (cat 4, small) +2 — emergent_municipal_moratoria_wave froze permitted sites this turn and this answers it directly
LEGITIMACY LENDS: capital 19, sentiment 31 -> +1
SOVEREIGNTY: 17 last turn, Tech sovereignty package finishes t6 +5, Gigafactories in flight +0, emergent_municipal_moratoria_wave t6 −2, capability rose 8.0 −1 = 19
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner rationed access by tier, administration takes office turn 6"}]}
```
```
