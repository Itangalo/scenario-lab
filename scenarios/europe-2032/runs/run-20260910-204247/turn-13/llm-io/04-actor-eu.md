...
- characters 455-3748: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 3792-5873: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7868-8361: `{{statement_ledger}}` from the actor's live statement ledger
- characters 11262-13413: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Hold together through ungovernable frontier capability by hardening European systems and binding middle-power leverage
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | Defection Containment and Eastern Backfill | 8 | small | 12 | 14 | own jurisdiction and middle-power coalition | eu_political_capital up slightly via cohesion, resilience up slightly, eu_ai_sovereignty up slightly | 2 | running |
...
| M3 | EU Critical Shield | 6 | large | 1 | 7 | own jurisdiction | resilience up materially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Bio-Sentinel Uplift | 6 | small | 2 | 5 | own jurisdiction | resilience up moderately via bio-detection and response | 2 | finished |
| M5 | Eastern Resilience Repair Cell | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
| M6 | Leaked Frontier Watch | 5 | small | 8 | 10 | own jurisdiction | ai_safety up slightly, resilience up slightly via triage of open-weight uplift | 2 | finished |
| M7 | Joint Patch and Leverage Pact | 8 | small | 9 | 11 | own jurisdiction and middle-power coalition | resilience up moderately, eu_ai_sovereignty up slightly via joint access, eu_political_capital up via coalition | 2 | finished |
| M8 | EU Bio-Agent Containment Surge | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, ai_safety up slightly via containment learning | 2 | finished |
...
## New measure
**Defection Containment and Eastern Backfill**
A small, fast instrument to keep the defecting member state's side-deal from hollowing the common line: ring-fence EU-funded workloads off the cut-price foreign cloud, backfill the affected eastern corridor with joint-procurement compute and machine-speed hardening, and tie the next aid-window payout to single procurement standards.
This and now because with capital at 12 and sentiment at 15 we cannot afford a split, and with casualties fresh the Union must show help arrives before outages, not after.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Defection Containment and Eastern Backfill", "category": 8, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction and middle-power coalition", "targeted_effect": "eu_political_capital up slightly via cohesion, resilience up slightly, eu_ai_sovereignty up slightly"}, "grounds": "member-state defection undercutting common line during containment"}]}
```

## Priority
M9 Defection Containment and Eastern Backfill — because it is the only live instrument, and stopping a public split outranks any new build while containment sustains on finished capacities.

## In practice
We run this through the General Affairs and Ecofin front door, not a new vote: Commission implementing decision to exclude non-aligned cloud from handling of EU-funded health, cyber and Critical Shield data, with HERA/ECDC/ENISA certifying the ring-fence. The joint procurement vehicle from the Leverage Pact stays open with shared Gigafactory-adjacent compute offered at cost to the defecting capital and its neighbours to make staying cheaper than leaving.

We hold finished lines in sustainment without new money: permanent cyber centres filter copycats, Bio-Sentinel sequencing and compulsory synthesis screening stay on, hospital isolation and payment-freeze drills continue in the corridor. Eastern mayors get the single aid window first, paid from reprogrammed Digital Europe/EU4Health funds, explicitly conditioned on common procurement — pragmatism at home must not mean fragmentation for all.
...
```
## New measure
**European Relief Dividend**
It scales the working public-sector AI wins — shorter waiting lists, faster permits, proven tutoring — onto EU-controlled hardened stacks in clinics, municipalities and job-transition desks, so benefit is seen locally before the next outage.
This and now because with capital at 9 and sentiment at 12 we cannot enforce anything else until voters see attributed European benefit, and the autumn cyber sweep plus warehouse-robot shock demand a visible counterweight that also pulls workloads off non-aligned clouds.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "European Relief Dividend", "category": 9, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, eu_political_capital up slightly via visible benefit, resilience up slightly"}, "grounds": "adoption success window amid collapse in trust and major cyber incident"}]}
```

## Priority
M9 Defection Containment and Eastern Backfill — because holding the ring-fence and eastern backfill through the automated sweep outranks expansion, and the new dividend only works if aid and containment hold first.

## In practice
We keep M9 as the front door: HERA/ECDC/ENISA certification of the ring-fence stays, joint procurement at cost plus Gigafactory-adjacent compute stays open, single aid window first-payout from reprogrammed funds for buyers to common standards. Permanent cyber centres move to sweep-triage, sequencing and compulsory screening stay on, isolation and payment-freeze drills continue.

We launch the Dividend through Health and Employment Councils on existing Digital Europe/EU4Health/ESF+ reprogramming, no new vote: standard EU-hardened deployment kits — triage, permit, tutoring modules — procured to common standards, staffed by retrained municipal teams with wage-insurance-linked transition desks in logistics regions hit by warehouse automation. Eastern mayors get first installs alongside backfill compute, directly answering after-outage help and hostile siting meetings.
