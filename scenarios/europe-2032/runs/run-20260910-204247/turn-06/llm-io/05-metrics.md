...
- characters 105-13358: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13401-13592: `{{metrics_json}}` from the run's live metric values
- characters 13685-15284: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15327-17462: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17518-18458: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Metrics
```json
{"ai_capability": 62.3, "openweight_capability": 58.5, "ai_safety": 22.0, "resilience": 39.0, "eu_ai_sovereignty": 26.0, "eu_political_capital": 9.0, "public_sentiment": 38.0}
```
## Narrative
### The attack arrives
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election winner was retrenchment, takes effect turn 6"}]}
```
```
