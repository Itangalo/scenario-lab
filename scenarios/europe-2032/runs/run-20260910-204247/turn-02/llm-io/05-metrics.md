...
- characters 105-13011: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13054-13246: `{{metrics_json}}` from the run's live metric values
- characters 13339-14525: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14568-16802: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16858-17932: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_open_model_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Major EU press investigation links Kimi K3-derived tooling to fraud and intrusion attempts against municipalities and SMEs, triggering coordinated calls for data-centre moratoriums and open-weight restrictions.
- `emergent_medical_fraud_wave` -- first noted turn 1, listed in 2 turn(s) so far: Major European research hospitals disclose coordinated fraud wave using Mythos-class open-weight voice and document forgery to obtain prescriptions and insurance payouts, forcing emergency health-system controls.
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Shield −3, EU Bio-Sentinel Uplift −2, priority −1 = −12
PROPOSAL BONUS: none — Bio-Sentinel answers bio uplift precursor but knowledge_work_augmented removes felt displacement crisis, no bonus per event condition
LEGITIMACY LENDS: capital 30, sentiment 49 -> +2
SOVEREIGNTY: 23 last turn, no capacity event, no finishing measure, in-flight +0, capability rose 1.5 −0 = 23

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M3", "fields": {"finish_turn": 7}, "grounds": "named priority"}]}
```
```
