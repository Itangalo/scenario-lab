...
- characters 455-3613: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 3656-5731: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6111-6604: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9116-10426: `{{previous_actions}}` from the actor's own response from the previous turn
...
## New measure
None this turn — waiting for political capital to recover above single-measure survival before staking a new instrument while a real bio-agent release is running.

## Store changes
No changes.

## Priority
M8 EU Bio-Agent Containment Surge — because with casualties real, containment running for weeks, and capital at 7, finishing the screening, sequencing and freeze/isolate drills already on the books outranks any new start.

## In practice
We put HERA, ECDC and the Health Security Committee in operational lead, with ENISA and ECB supervisors alongside: synthesis-provider screening pilots made mandatory where implementing acts allow, sequencing reporting accelerated, and hospital/lab isolation plus payment-freeze playbooks exercised in the affected corridor first. Reprogrammed EU4Health and Digital Europe money only; no new vote we cannot win.

We hold finished Gigafactories, Critical Shield centres, Bio-Sentinel and the Patch and Leverage Pact in sustainment mode — eastern machine-speed kits stay on, joint compute offers stay open to contain the Gulf-deal capital — but staff and Council time go to the bio incident. Finance and Health Council formations meet jointly to keep treasurers and mayors seeing one front door, because trust at 22 will not survive competing briefings.
