...
- characters 107-12836: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12879-13069: `{{metrics_json}}` from the run's live metric values
- characters 13162-14057: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14101-15687: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 15743-16371: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M11 | EU Graduate Transition and Public AI Works Corps | 2 |
...
- `emergent_insurance_driven_spares_crunch` -- first noted turn 13, listed in 1 turn(s) so far: Insurers impose war-risk surcharges on Taiwan Strait chip cargoes that trigger spot shortages of spare parts for patched grid and hospital systems in unprotected ports and wards.
...
## Notepad
PORTFOLIO CHARGE t13: EU Graduate Transition and Public AI Works Corps −2, priority −0 (capital below 20, no effect, no cost) = −2
PROPOSAL BONUS: EU Graduate Transition and Public AI Works Corps (cat 7, small) +3 — labour_displacement and eu_frontier_access_denied both fired this turn and this answers staffing/continuity directly
LEGITIMACY LENDS: capital 0, sentiment 13 -> +2 (capped below sentiment)
SOVEREIGNTY: 25 last turn, no capacity event? actually eu_frontier_access_denied t13 −2, no capacity build finish or in flight +0, capability rose 0.6 −0 = 23
