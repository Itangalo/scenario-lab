...
- characters 455-1799: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1843-3837: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4706-5196: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7140-9078: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | European Public Services AI Scale-Up | 9 | small | 2 | 4 | own jurisdiction | public_sentiment up moderately, resilience up slightly | 2 | running |
...
## New measure
**European Public Services AI Scale-Up**
Takes the hospital and municipal triage pilots that cut waiting lists and permit times to EU-wide deployment on EU-controlled infrastructure with shared procurement and evaluation.
This is category 9 Diffusion, adoption and public trust, and why now is the adoption_success window — visible benefit attributed to a European decision — plus the permit backlash: we need demonstrated public benefit to hold consent for compute and hardening.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "European Public Services AI Scale-Up", "category": 9, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up slightly"}, "grounds": "scaling proven triage pilots during adoption success window"}]}
```

## Priority
European Public Services AI Scale-Up, because it directly serves the two-year commitment to keep essential services running while converting favourable coverage into consent for Gigafactories and grid hardening.

## In practice
We run this through DG REFORM and DG CNECT under the Digital Europe Programme, with joint procurement via the European Health Data Space and the Living-in.EU municipal network, using the secured US-model terms as bridge while requiring EU-hosted deployment for scale-up sites. Health ministries and mayors who hosted pilots become champions; we offer co-financing for staffing and evaluation to municipalities facing permit fights.

M1 Gigafactories and M2 Tech sovereignty package stay in committee-track: DG COMP clears state aid, site shortlists add grid-connection and water-transparency criteria from M3's Shield to unblock permits. ENISA's finished Shield standards become the condition for scale-up connection — no detection compliance, no scale-up funds — sequencing hardening before expansion without a new instrument.
...
```
## New measure
**Municipal Shield Relief Corps**
Puts funded, shared cyber-implementation teams behind the municipalities now asked to deliver the Shield retrofits and the public-services scale-up at once, so essential services stay staffed through the load spike.
This is Preparedness and resilience work, and why now is that the June bottleneck is crews and municipal IT capacity, not rules — without relief both hardening and adoption stall and the two-year commitment to keep services running fails on implementation.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Municipal Shield Relief Corps", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "unblock municipal implementation overload threatening Shield and scale-up"}]}
```

## Priority
Municipal Shield Relief Corps, because it is the binding constraint on the two-year commitment to keep essential services running — Gigafactories and scale-up both die in municipal IT queues without it.

## In practice
We fund this via Technical Support Instrument top-up plus Digital Europe redeployment, run by DG REFORM with ENISA secondments: regional pools of vetted integrators that do Shield segmentation and EU-hosted deployment for M4 sites, conditional on the March detection standards. Mayors get staff, not just co-financing; transmission operators get a single municipal interface for exercises.

M1 Gigafactories and M2 Tech sovereignty package stay on committee-track: DG COMP accelerates state-aid clearances using the new grid-connection and water-transparency criteria to settle the two stalled shortlists, with substation upgrades sequenced ahead of site awards. M4 scale-up continues joint procurement through EHDS and Living-in.EU, but no new sites connect without Shield compliance and Corps support — hardening before expansion.
