...
- characters 454-2472: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2516-4494: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5920-6420: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8802-10713: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M7 | EU Essential Services Restoration Surge | 6 | small | 6 | 8 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident reporting | 2 | running |
...
| M3 | Grid and Port Cyber Shield | 6 | small | 1 | 2 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident reporting | 2 | finished |
| M4 | European Public Services AI Scale-Up | 9 | small | 2 | 4 | own jurisdiction | public_sentiment up moderately, resilience up slightly | 2 | finished |
| M5 | Municipal Shield Relief Corps | 6 | small | 3 | 4 | own jurisdiction | resilience up moderately | 2 | finished |
| M6 | EU Essential Services Continuity Stack | 4 | large | 4 | 6 | own jurisdiction | eu_ai_sovereignty up, resilience up | 3 | finished |
...
## New measure
**EU Essential Services Restoration Surge**
It surges vetted integrators, ENISA teams and mutual-aid IT staff to restore ransomware-hit municipalities and restart stalled grid/port segmentation, while funding overtime and workaround hosting on the continuity stack.
This turn's automated attack plus sick-outs demand repair before any new build, and with capital at 10 this small resilience measure is the only affordable way to keep the two-year fallback commitment alive.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Restoration Surge", "category": 6, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via incident reporting"}, "grounds": "automated ransomware sweep overwhelms municipalities amid walkouts"}]}
```

## Priority
M1 InvestAI Gigafactories, because without unblocking substation works and holding the two cleared sites through the investment downturn and US rationing threat, there is no sovereign host for the fallback to survive on.

## In practice
We put the new Surge under Article 122 via DG CNECT and ENISA, pooling the finished M5 corps, EuroHPC operations staff and cross-border municipal mutual aid, paid from the M6 continuity envelope and civil-protection funds. It triages the ransomware blast radius, restores backups where possible, completes swap-overs only where staffed, and funds grid/port segmentation catch-up, accepting paper where launches would fail.

We hold M1 live but defensive: no new concrete this turn, only national permitting defence at the two blockaded substation sites, renegotiation of private terms toward continuity hosting, and a Trade Council screening-plus-joint-licensing stance on extended US lithography controls to protect ASML servicing without spending capital we do not have.
...
```
## New measure
**EU Municipal Bio-Detection and Care Continuity Net**
It puts funded bio-surveillance sampling, hospital triage backup protocols and incident reporting onto the existing restoration surge footprint, using EuroHPC/vetted-cloud hosting and ENISA teams already deployed.
Why this and why now: the contested genome-design finding opens a four-turn bio gate just as open weights land near the frontier and hospitals are still on paper from the ransomware sweep, so a small preparedness add-on that reuses M7 staff is the only affordable way to keep the fallback commitment from being blind to the next incident.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Bio-Detection and Care Continuity Net", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately via bio-detection and hospital continuity"}, "grounds": "bio uplift precursor meets ransomware-degraded hospitals and near-frontier open weights"}]}
```

## Priority
M7 EU Essential Services Restoration Surge, because with capital at 6 and queues on television, restoring municipal/port backups and segmentation catch-up must outrank even the bio add-on or nothing else holds.

## In practice
We run M7 as the chassis under Article 122 and Civil Protection Mechanism via DG CNECT, ENISA and HERA: pooled integrators and cross-border municipal aid stay on ransomware restoration and funded overtime, accepting paper where walkouts persist, while we defend the two blockaded grid-connection permits in national review and hold continuity hosting terms with private partners.

We bolt the new Net onto that same deployment rather than standing up a separate programme: HERA/ECDC protocols for wastewater and emergency-department sampling, simplified stockpile and paper-to-cloud triage playbooks for hospitals, and mandatory near-miss reporting from the restored portals into the M6 continuity stack on EuroHPC/vetted clouds. No new concrete, no new compute line alongside finished M1/M2/M6 — just detection and continuity where staff already are.
