...
- characters 376-1115: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1159-3123: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3179-4342: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4405-27713: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28068-29042: `{{event_history}}` from the run's own event record
...
- `emergent_clinical_paper_resistance` -- first noted turn 10, listed in 1 turn(s) so far: Hospital staff in two member states refuse to use the EU-hosted fallback triage model and sustain a paper-only work-to-rule through winter, forcing emergency diversion protocols.
- `emergent_port_care_refusal` -- first noted turn 10, listed in 1 turn(s) so far: Coordinated walkouts by hospital and municipal staff spread to port operators in Rotterdam and Antwerp, refusing to operate EU-hosted open-model triage and scheduling tools after the US cutoff, forcing extended paper-only operations.
...
- Turn 1 (10 turn(s) ago): cyber_test_shot, eu_access_secured, adoption_success, emergent_datacentre_permit_backlash
- Turn 2 (9 turn(s) ago): knowledge_work_augmented, adoption_success
- Turn 3 (8 turn(s) ago): knowledge_work_augmented, adoption_success
- Turn 4 (7 turn(s) ago): cyber_major_incident, bio_uplift_findings, eu_frontier_access_denied, member_state_defection
- Turn 5 (6 turn(s) ago): ai_investment_collapse, supply_chain_coercion, emergent_municipal_it_backlash, election_consolidation
- Turn 6 (5 turn(s) ago): cyber_major_incident, export_control_escalation, emergent_substation_blockade
- Turn 7 (4 turn(s) ago): bio_uplift_findings, openweight_frontier_release, knowledge_work_augmented
- Turn 8 (3 turn(s) ago): bio_uplift_findings, opaque_reasoning, labour_displacement, emergent_public_sector_walkout_spread
- Turn 9 (2 turn(s) ago): bio_uplift_findings, eval_anomaly_reports, eu_frontier_access_denied
- Turn 10 (1 turn(s) ago): joint_threat_response
