...
- characters 454-1193: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1237-3201: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3883-4352: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7243-9029: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Joint Threat Response Accession | 8 | small | 10 | 12 | coalition | resilience up moderately via pooled protection | 2 | running |
...
Join the invited joint cyber telemetry command and biosurveillance sample-sharing pact as the EU's substitute for capacity it cannot build alone.
This and now because with capital at 8.0, sovereignty at 27.0 and safety at 8.0 we cannot harden or detect alone, and the invitation offers pooled attribution, real-time telemetry and binding bio sample-sharing that directly serves our commitment to keep essential services running through cutoff and bio-risk.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Joint Threat Response Accession", "category": 8, "size": "small", "finish_turn": 12, "applies_to": "coalition", "targeted_effect": "resilience up moderately via pooled protection"}, "grounds": "invited pooling replaces unaffordable solo build"}]}
```

## Priority
M10 EU Joint Threat Response Accession, because pooled telemetry and bio-surveillance is the only resilience gain affordable before M9 finishes, outranking court defence of unbuilt sites.

## In practice
We accede via Foreign Affairs and JHA Council decisions using Article 222 solidarity and HERA/ECDC mandates, tasking ENISA as telemetry node into the joint cyber command and ECDC/HERA as sample-sharing node with standing investigation consent. Funding is seconded staff and existing M8 sensors and M6 clean images, not new concrete.

We keep M9 running as delivery vehicle: civil-protection overtime and cross-border municipal teams staff EU-hosted open-weight fallback on EuroHPC/vetted clouds where US calls were cut, while banking near-miss and wastewater data into the joint pact. We defend gigafactory permits on paper only, accept handwriting where staff refuse, and spend no capital on new siting fights.
...
```
## New measure
None this turn, while we let M10 land and rebuild capital before starting anything new.

## Store changes
No changes.

## Priority
M10 EU Joint Threat Response Accession, because with capital at 8.0 the pooled telemetry pact is the only channel to import the defensive breakthrough and to contain the defection without opening a second front we cannot fund.

## In practice
We task ENISA and the ECDC/HERA nodes to pull the new automated patching and swarm-detection tooling straight through the joint network into municipal grids, ports and hospitals, using M9's fallback images and M8 sensors as distribution, not new procurement. Seconded staff and overtime municipal teams do the rollout where trust allows, paper fallback stays where it does not.

We do not retaliate against the defecting member state. We record its hyperscaler deal as a national derogation under the solidarity Article 222 decision, keep its feeds in the joint pact, and defend gigafactory litigation on paper only. With political capital at 8 and sentiment at 22, holding the one pooled lifeline outranks enforcing a common line we cannot hold.
