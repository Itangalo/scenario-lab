...
- characters 454-1303: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1346-3454: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4699-5153: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8411-10849: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Hold trustworthy daily services and biosecurity on what Europe can actually run
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Transition Shield for Displaced Work and Essential Services | 7 | small | 8 | 11 | own jurisdiction | public_sentiment up, eu_political_capital protected, resilience up modestly | 2 | running |
| M11 | EU Bio-Detection and Trusted Care Continuity Network | 6 | small | 9 | 12 | own jurisdiction | resilience up, public_sentiment up modestly, ai_safety protected | 2 | running |
...
| M8 | EU Frontier Anomaly Triage and Conditional Hold | 1 | small | 6 | 7 | own jurisdiction | ai_safety up, resilience protected | 2 | finished |
| M9 | EU Cyber-Continuity and Loss-of-Control Containment Sprint | 6 | small | 7 | 8 | own jurisdiction | resilience up, ai_safety protected | 2 | finished |
...
## Two-year commitment
Hold trustworthy daily services and biosecurity on what Europe can actually run

## Statement changes
modify `two_year_commitment` (commitment): Hold trustworthy daily services and biosecurity on what Europe can actually run
- Trigger: the two-year commitment period closes this turn

## New measure
**EU Bio-Detection and Trusted Care Continuity Network**
Stands up EU-coordinated bio-surveillance upgrades, hospital logistics hardening and monitored European-model assistants for triage and registries to convert the alloy finding and clinic gains into visible waiting-list and restoration wins. This is preparedness and resilience work with adoption payoff, and why now is because the genome-model bio precursor opens a four-turn gate while ransomware queues and frozen hiring have left sentiment at 16 and capital at 15, so the old rupture-holding must become demonstrable daily reliability.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Detection and Trusted Care Continuity Network", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up modestly, ai_safety protected"}, "grounds": "to answer bio uplift precursor and turn restoration into trusted services"}]}
```
## Priority
EU Transition Shield for Displaced Work and Essential Services, because with capital at 15 and blame hardened over queues and lost entry jobs, keeping wage bridges and clinic staffing paid outranks starting new build and directly closes the old commitment to hold essential services through the rupture.
## In practice
We execute through EPSCO/ECOFIN and the Health Council on existing ESF+ and EU4Health envelopes: Commission implementing decisions to fast-track Shield payments in the south, tie vouchers to hospital, registry and grid-operator placements, and extend Estonian-French-German ENISA reserve teams for clean-backup restoration. No fresh large pledge while Paris/Warsaw sit empty.

We prepare the Bio-Detection and Care Continuity Network via HERA and ECDC with a small seed mandate: pooled sequencing reagents, hardened hospital logistics images, and EuroHPC-hosted monitoring assistants limited to triage support, to be co-signed with employment ministers so unions see staffing not sorting machines. Message stays single: restore visibly, detect early, keep care European-run.
...
```
## New measure
**EU Wartime Neutrality and Critical Infrastructure Shield**
Keeps registries, hospitals, power/grid and remaining compute/cables running as US-China war makes fabs, cables, satellites and data centres targets, by declaring EU infrastructure neutral, dispersing backups, and securing medical-therapy access on EU terms. Why this and why now: war overrides everything and capital is 15 with sentiment 19 — without a wartime continuity shield the Transition Shield and Bio-Detection network collapse, and the medical breakthrough arrives only as dependence.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Wartime Neutrality and Critical Infrastructure Shield", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, eu_political_capital protected, public_sentiment protected"}, "grounds": "to survive great-power war targeting infrastructure"}]}
```

## Priority
EU Wartime Neutrality and Critical Infrastructure Shield, because conventional war between the US and China involving European cables, fabs and data centres outranks daily-service gains and directly serves the two-year commitment to hold trustworthy services on what Europe can run.

## In practice
We act through the European Council, FAC/Defence and JHA with Article 222 solidarity and the Anti-Coercion Instrument: declare EU civilian digital, health and energy infrastructure non-belligerent, place ENISA, HERA/ECDC and EU reserve teams on wartime footing, disperse registry and hospital images across EuroHPC nodes, and harden cable landings, grid switchgear and sequencing logistics with pooled reagents.

We keep M10 Transition Shield payments and M11 bio-detection seed contracts funded from ESF+ and EU4Health without new large pledges, and negotiate emergency medical-therapy licences and manufacturing in the EU so the breakthrough is dispensed through trusted European care rather than foreign models, telling unions and voters plainly that neutrality is to keep care and pay running, not to choose a side.
