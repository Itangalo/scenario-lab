...
- characters 454-2200: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2243-4459: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4991-5489: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8931-11757: `{{previous_actions}}` from the actor's own response from the previous turn
...
### Containment holds, trust breaks
The rapid containment protocol finished in time to matter. Clinics and town halls that had joined the telemetry and locked-backup scheme isolated infected segments, cut cloud links on playbook, and restored from clean copies within hours. Auditors called it the reason a bad autumn did not become a cascading winter.

Everything else slipped. With advanced chip slots still cancelled and foundry letters still citing force majeure, the joint licensing offer to The Hague, Tokyo and Seoul stalled. Partners asked for binding lithography controls first, quotas later. The gigafactory site stayed fenced and idle. Insurers welcomed the liability clarification draft but did not resume cover, waiting for law, not promise.

Then finance fled. Valuations in AI reset violently; announced data-centre expansions were cancelled outright, not delayed. Two co-location options Brussels had counted on for anchored capacity evaporated. American labs slowed training plans; second-tier builders talked openly of catching up.

At home, two opposite stories collided. In cooperating municipalities, audited triage assistants cut waiting lists and administrative decisions that took months came back in days. Nurses named the system as European, and voters noticed.

In the same weeks, an ombudsman and then a court found a benefits and policing support system had systematically harmed people for months. Caseworkers had approved machine-ranked queues at forty seconds a case, logs were written and never read, conformity papers were in order. Opposition and press did not argue the Act was broken in enforcement — they argued the deployment had never been high-risk at all under its categories. Everything done had been lawful. That reading stuck.

The Commission froze the system, published logs, promised workload limits, independent re-checks and a gap-closing implementing act, and launched audited roll-outs of the working triage tools. But coverage framed it as lawful harm, and restriction became the cheap position. Deployments froze beyond insurance.

Brussels ended the year with containment proven, hardware missing, and legitimacy split between visible benefit and lawful scandal.
...
- `two_year_commitment` (commitment): Rebuild trusted civic AI and resilient essential services under compute scarcity while securing anchored sovereign capacity
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Civic AI Trust and Adoption Repair | 9 | small | 9 | 11 | own jurisdiction | public_sentiment up, resilience up slightly via restored civic deployments, eu_political_capital protected | 2 | running |
...
| M8 | EU Open-Weight Exploit Containment Patch | 6 | small | 6 | 8 | own jurisdiction | resilience up to prevent cascade from open-weight cyber uplift, ai_safety loss contained | 2 | finished |
| M9 | EU Loss-of-Control Rapid Containment Protocol | 6 | small | 7 | 9 | own jurisdiction | resilience up, ai_safety loss contained via faster containment | 2 | finished |
| M10 | EU Chips-for-Compute and Civic Liability Pact | 8 | small | 8 | 10 | coalition partners holding supply-chain chokepoints plus own jurisdiction | eu_ai_sovereignty up slightly via secured compute, resilience up slightly via restored civic deployments | 2 | finished |
...
## Two-year commitment
Rebuild trusted civic AI and resilient essential services under compute scarcity while securing anchored sovereign capacity

## Statement changes
modify `two_year_commitment` (commitment): Rebuild trusted civic AI and resilient essential services under compute scarcity while securing anchored sovereign capacity
- Trigger: the two-year commitment period closes this turn

## New measure
**EU Civic AI Trust and Adoption Repair**
Puts visibly working, audited public-sector AI back into health, administration and education with enforceable human oversight, readable logs and redress, to answer the automated-decision scandal and unfreeze civic deployments. This is Diffusion, adoption and public trust work, category 9, and why now is that the scandal makes restriction cheap and adoption impossible for years unless the Act's failure is fixed in practice, while the adoption success shows European-attributed benefit still buys legitimacy.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Civic AI Trust and Adoption Repair", "category": 9, "size": "small", "finish_turn": 11, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, resilience up slightly via restored civic deployments, eu_political_capital protected"}, "grounds": "to answer scandal plus insurer freeze with low capital"}]}
```
## Priority
M10 EU Chips-for-Compute and Civic Liability Pact — because without anchored compute quotas and the liability backstop through blockade winter nothing else including trust repair has hardware to run on or insurance to deploy under.
## In practice
We finish under the old commitment by driving M10 through Trade/Foreign Affairs Council and DG TRADE: joint licensing offer on lithography/service/spares with Netherlands, Japan and Korea in exchange for anchored compute quotas to keep the gigafactory build alive, plus DG JUST time-limited liability clarification and EU backstop to coax municipal/hospital insurers back. ENISA and Civil Protection keep telemetry-tied triage money and locked backups, and the cross-border isolation exercise playbooks stay live.

We start the trust repair with DG CNECT, DG SANTE and the AI Office under the AI Act: freeze and audit the scandal system, publish logs and redress, require meaningful human oversight with workload limits and independent conformity re-check for Annex III systems, and close the gap deployment with a targeted implementing act. We pair this with scaling the adoption successes — waiting-list triage and administrative decisions in cooperating clinics/municipalities — funded from finished Shield lines, to show benefit attributed to a European decision, not an American product. No new large build; capital at 17 forces one small in-flight plus one small new charge.
...
Join the pooled attribution, telemetry and response command as a full member, wiring ENISA and national CERTs into its real-time feed and committing locked-backup and isolation playbooks in return.
Why this and why now: containment held only where telemetry was shared, finance and compute options have evaporated, and this is protection we cannot build alone arriving as an invitation rather than a demand.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Joint Cyber Command Accession", "category": 8, "size": "small", "finish_turn": 12, "applies_to": "coalition partners in joint command plus own jurisdiction", "targeted_effect": "resilience up via pooled response, ai_safety loss contained"}, "grounds": "to take invited seat while sovereign build stalled"}]}
```

## Priority
M11 EU Civic AI Trust and Adoption Repair — because with capital at 13 and sentiment at 23, unfreezing lawful civic deployments is the only path to rebuild legitimacy to act at all.

## In practice
We keep the weight on M11 through DG CNECT, DG JUST and the AI Office: enforce the freeze on the scandal system, publish logs with redress, impose workload limits and independent re-checks for high-throughput Annex III deployments, and push the gap-closing implementing act while scaling the audited triage successes from cooperating municipalities with Shield-legacy funds to coax insurers back from promise to cover.

We accede to the joint command via Foreign Affairs and Telecom Councils with ENISA as operator: offer our telemetry pool, locked-backup standard and cross-border isolation playbooks as the EU contribution, accept binding real-time sharing and joint attribution rules, and ring-fence this as shared resilience rather than a new sovereign compute line so the stalled Gigafactories and licensing offer are not duplicated.
