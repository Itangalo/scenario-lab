...
- characters 451-1292: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1335-3477: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4840-5315: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7686-9643: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_anti_datacentre_sabotage (emergent event):** Coordinated sabotage and blockades of data-centre construction sites across several member states force a temporary halt to new builds after clashes with security.
...
- `two_year_commitment` (commitment): Keep essential services running and displaced livelihoods supported through superhuman AI disruption
...
|---|---|---|---|---|---|---|---|---|---|
| M3 | EU Critical Systems Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up, ai_safety up via detection | 3 | finished |
| M4 | Joint Supply-Chain Bargain | 8 | small | 2 | 4 | coalition | eu_ai_sovereignty up, eu_political_capital up via leverage | 2 | finished |
| M5 | EU Critical-Sector Cyber Insurance Backstop | 6 | small | 3 | 6 | own jurisdiction | resilience up, eu_political_capital up via prevented coverage gap | 2 | finished |
| M6 | EU Bio Shield Surge | 6 | small | 4 | 7 | own jurisdiction | resilience up, ai_safety up via bio misuse detection | 2 | finished |
| M7 | EU Essential Services Continuity Rationing | 6 | small | 7 | 8 | own jurisdiction | resilience up, eu_political_capital up via prevented cascade | 2 | finished |
| M8 | EU Bio Incident Containment and Care Continuity | 6 | small | 8 | 9 | own jurisdiction | resilience up, ai_safety up via bio misuse detection | 2 | finished |
| M9 | EU Joint Threat Response Accession | 8 | small | 9 | 10 | coalition | resilience up, ai_safety up via shared detection | 2 | finished |
| M10 | EU Emergency Restoration and Mutual Aid Cell | 6 | small | 11 | 12 | own jurisdiction | resilience up, eu_political_capital up via restored services | 2 | finished |
...
## New measure
**EU Emergency Restoration and Mutual Aid Cell**
Deploys joint cyber-incident restoration teams, manual fallback staffing and cross-border mutual aid to bring ransomware-hit public services back online under the new superhuman capability jump.
This and now because the automated attack has hit with defences visibly behind and safety at 1.0 while the two-year commitment to keep essential services running demands restoration over any new build, and with capital at 5.0 only a small resilience measure can land without collapse.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Emergency Restoration and Mutual Aid Cell", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, eu_political_capital up via restored services"}, "grounds": "major automated cyber incident plus capability jump with services down"}]}
```

## Priority
EU Emergency Restoration and Mutual Aid Cell, because restoring hit services is the commitment itself under this fortnight's attack and nothing else outranks it.

## In practice
We act through the Justice and Home Affairs and Health Councils on Articles 222 and 168, tasking ENISA and the Health Emergency Preparedness Authority to run the Cell from the existing rationing cell: reprogrammed ESF+ and Civil Protection Mechanism funds pay Continuity Corps crews and seconded national CERTs to restore municipal registries, hospital triage and payment systems on isolated older models and paper protocols.

We keep the joint telemetry and biosurveillance feed as EU node despite dependence, accepting outside patches to contain blast radius while logging attribution centrally, and we offer the defecting member state restoration teams in return for returning volumes to the solidarity pool. We pause any sovereignty or evaluation build until restoration is landing and the income bridge payments flow.
...
```
## New measure
**EU Pandemic Care and Essential Functions Keep-Alive**
Keeps hospitals, bio-detection and essential registries running on isolated, auditable models and manual protocols through the AI-designed pandemic and frontier cutoff, using the new interpretability assurance to certify the fallback systems.
This and now because the bio outbreak puts everything else on hold and the cutoff plus sabotage have removed the outside crutch, so the two-year commitment to keep essential services running demands a keep-alive built on what we control, and with capital at 8.0 only a small preparedness and resilience measure can be sustained.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Pandemic Care and Essential Functions Keep-Alive", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up via certified fallback"}, "grounds": "AI-designed pandemic plus frontier cutoff with services at risk"}]}
```

## Priority
EU Pandemic Care and Essential Functions Keep-Alive, because pandemic-mode survival and the cutoff outrank any other build and it is the commitment itself under this turn's events.

## In practice
We act through the Health and Justice and Home Affairs Councils on Articles 168 and 222, tasking HERA and ENISA to run a Keep-Alive cell from the Mutual Aid Cell: reprogrammed EU4Health, Civil Protection Mechanism and ESF+ funds pay continuity crews, seconded CERTs and military medical logistics to hold hospital triage, bio-surveillance, and registries on air-gapped older models certified with the new interpretability result plus paper fallbacks.

We accept outside patches only through EU vetting, ration remaining compute to care and detection, and offer restoration and vaccine-logistics support to all capitals including the defecting one in return for pooling volumes and pausing sabotage-blocked builds. We freeze any new sovereignty build until keep-alive is landing and bridge payments flow.
