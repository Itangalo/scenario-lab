...
- characters 452-2151: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2195-4219: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6620-7095: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9788-11834: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Pandemic Care and Essential Functions Keep-Alive | 6 | small | 12 | 14 | own jurisdiction | resilience up, ai_safety up via certified fallback | 2 | running |
...
```
## New measure
None this turn — waiting for the Keep-Alive to land and for any political capital to rebuild before starting anything new.

## Store changes
No changes.

## Priority
M11 EU Pandemic Care and Essential Functions Keep-Alive, because with pandemic, cutoff, automated attack and RSI onset hitting at once, holding triage, surveillance and registries on what we alone control is the two-year commitment itself.

## In practice
We keep the Health and Home Affairs Keep-Alive cell as the sole operating track: HERA/ENISA-run continuity crews, seconded CERTs and military medical logistics holding hospitals and civil registries on isolated certified fallbacks and paper, with reprogrammed EU4Health and Civil Protection funds and compute rationed to care and detection.

We use the joint threat-response accession channels for telemetry and sample-sharing to absorb the automated attack and bio-uplift signal, admit no outside patch except through EU vetting, and offer logistics support to all capitals without opening a second build track. No new siting push while sabotage and sentiment at zero make construction unenforceable.
