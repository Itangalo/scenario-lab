...
- characters 16367-16972: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_cyber_insurance_crunch` -- first noted turn 1, listed in 1 turn(s) so far: Major EU insurers introduce AI-cyber exclusions or steep premium hikes for critical-sector clients after the Kimi K3 release, triggering a public fight over who bears model-enabled attack losses.
- `emergent_clinical_ai_pause` -- first noted turn 1, listed in 1 turn(s) so far: Major European research hospitals pause elective AI-assisted diagnostics after a widely reported model confabulation causes incorrect triage, triggering calls for a moratorium on clinical frontier models.
...
## Metrics
```json
{"ai_capability": 57.0, "openweight_capability": 46.0, "ai_safety": 25.0, "resilience": 37.0, "eu_ai_sovereignty": 19.0, "eu_political_capital": 40.0, "public_sentiment": 40.0}
```
...
PROPOSAL BONUS: EU Critical Systems Shield (cat 6, large) +3 — cyber_test_shot intrusion with undetected persistence landed this turn and this answers it directly
LEGITIMACY LENDS: capital 39, sentiment 42 -> +1
SOVEREIGNTY: 22 last turn, no capacity measure finishes, Gigafactories in flight +0, supply_chain_coercion t1 −2, capability rose 5.0 −1 = 19
## Store changes
```json
{"store": []}
```
