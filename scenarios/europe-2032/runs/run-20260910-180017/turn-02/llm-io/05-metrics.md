...
- characters 105-13051: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13094-13286: `{{metrics_json}}` from the run's live metric values
- characters 13379-14284: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14327-16226: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16282-17368: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | Joint Supply-Chain Bargain | 2 |
...
- `emergent_cyber_insurance_crunch` -- first noted turn 1, listed in 2 turn(s) so far: Major EU insurers introduce AI-cyber exclusions or steep premium hikes for critical-sector clients after the Kimi K3 release, triggering a public fight over who bears model-enabled attack losses.
- `emergent_clinical_ai_pause` -- first noted turn 1, listed in 2 turn(s) so far: Major European research hospitals pause elective AI-assisted diagnostics after a widely reported model confabulation causes incorrect triage, triggering calls for a moratorium on clinical frontier models.
...
## Metrics
```json
{"ai_capability": 60.0, "openweight_capability": 51.0, "ai_safety": 30.0, "resilience": 35.0, "eu_ai_sovereignty": 16.0, "eu_political_capital": 30.0, "public_sentiment": 38.0}
```
