...
- characters 106-13231: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13274-13463: `{{metrics_json}}` from the run's live metric values
- characters 13556-14922: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14965-16888: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16944-18000: `{{notepad}}` from the Game Master's notepad, carried across turns
...
(none)
     ...
     **so 0 measures are in flight and they come to −0 `eu_political_capital` this turn.** That figure is the `cost_per_turn` column of the rows above, added up, and nothing else: the named priority's −1 below is charged on top of it, and is not in it. Your charge line carries one term per measure -- 0 of them before the priority. **A measure is charged in every turn up to but not including its finishing turn.**
...
PORTFOLIO CHARGE t9: EU Joint Threat Response Accession −2, priority −1 = −3
PROPOSAL BONUS: EU Joint Threat Response Accession (cat 8, small) +4 — joint_threat_response invitation landed this turn and eu_frontier_access_denied + capability_jump directly argue for it
LEGITIMACY LENDS: capital 5, sentiment 12 -> +2 (capped below sentiment)
SOVEREIGNTY: 1 last turn, no cat4 finish, no cat4 in flight, eu_frontier_access_denied t9 −2, capability rose 5.0 −1 = 0
...
## Metrics
```json
{"ai_capability": 88.2, "openweight_capability": 80.5, "ai_safety": 1.0, "resilience": 50.0, "eu_ai_sovereignty": 0.0, "eu_political_capital": 5.0, "public_sentiment": 6.0}
```
