...
- characters 377-2028: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2072-4346: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4402-5527: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5590-29817: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30172-30307: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Cyber Shield −3, EU Incident Reporting and Misuse Telemetry Network −2, priority −1 = −12
PROPOSAL BONUS: EU Incident Reporting and Misuse Telemetry Network (cat 2, small) +3 — openweight frontier release landed this turn and autumn test-shot detection failed, this answers both directly
...
- `emergent_biosecurity_publishing_freeze` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher and a national research funder jointly impose a moratorium on publishing AI-designed pathogen methods after the phage results, triggering a preprint exodus to offshore servers.
- `emergent_eu_talent_retention_pact` -- first noted turn 1, listed in 2 turn(s) so far: Major EU research universities and public labs jointly announce a pooled compute consortium to retain frontier AI talent threatening to leave for US labs.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, taiwan_tension_rise, eu_access_secured
