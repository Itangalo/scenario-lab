...
- characters 377-2601: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2645-4797: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4853-5646: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5709-34194: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 34549-34852: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, Joint Threat Response Accession −2, priority −1 = −9
PROPOSAL BONUS: Joint Threat Response Accession (cat 8, small) +3 — cyber_major_incident landed this turn and this answers it directly via pooled telemetry
...
- `emergent_municipal_copycat_wave` -- first noted turn 4, listed in 1 turn(s) so far: Copycat municipal extortion wave using the openly available intrusion tooling forces several mid-size cities to suspend e-services for weeks, triggering street protests over data-centre siting and recovery spending.
...
- Turn 1 (4 turn(s) ago): cyber_test_shot, taiwan_tension_rise, eu_access_secured
- Turn 2 (3 turn(s) ago): openweight_frontier_release
- Turn 3 (2 turn(s) ago): cyber_major_incident, openweight_frontier_release
