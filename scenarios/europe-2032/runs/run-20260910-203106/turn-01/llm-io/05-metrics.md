...
- characters 16345-16933: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M3 | EU Critical Systems Cyber Shield | 3 |
...
- `emergent_biosecurity_publishing_freeze` -- first noted turn 1, listed in 1 turn(s) so far: A major European scientific publisher and a national research funder jointly impose a moratorium on publishing AI-designed pathogen methods after the phage results, triggering a preprint exodus to offshore servers.
- `emergent_eu_talent_retention_pact` -- first noted turn 1, listed in 1 turn(s) so far: Major EU research universities and public labs jointly announce a pooled compute consortium to retain frontier AI talent threatening to leave for US labs.
...
PROPOSAL BONUS: EU Critical Systems Cyber Shield (cat 6, large) +4 — cyber_test_shot landed this turn, severe precursor with staged access in two member-state grids, answered directly
LEGITIMACY LENDS: capital 41, sentiment 40 -> none (capital higher)
SOVEREIGNTY: 22 last turn, no finishing measure, in-flight partial +0, eu_access_secured t1 +2, capability rose 2.0 −1 = 23
## Store changes
```json
