...
- characters 454-2325: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2369-4551: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5791-6278: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9541-11773: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential services and social cohesion functioning through AI-driven shocks we cannot prevent or switch off
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Middle-Power Compute and Safety Leverage Pact | 8 | small | 10 | 12 | coalition | eu_ai_sovereignty up slightly via secured access, ai_safety up slightly via shared evaluation, eu_political_capital up via leverage | 2 | running |
...
| M6 | Joint Threat Response Accession | 8 | small | 4 | 5 | coalition | resilience up materially via pooled telemetry and response | 2 | finished |
| M7 | Transatlantic Tier-1 Access Bargain | 8 | small | 5 | 7 | coalition | eu_ai_sovereignty up slightly via secured access, resilience up slightly via continuity | 2 | finished |
| M8 | EU Black-Box Assurance and Activation Inspection Programme | 5 | small | 6 | 9 | own jurisdiction | ai_safety up materially, resilience up slightly via better detection | 2 | finished |
| M9 | EU Critical Continuity and Bio-Sentinel Surge | 6 | small | 7 | 9 | own jurisdiction | resilience up materially to absorb major incident and bio precursor | 2 | finished |
| M10 | EU Essential Services Degrade-Gracefully and Bio-Response Pact | 6 | small | 9 | 11 | own jurisdiction | resilience up materially, ai_safety up slightly via incident learning | 2 | finished |
...
## New measure
**EU Middle-Power Compute and Safety Leverage Pact**
Joins EU lithography, allied export-licence alignment and shared evaluation capacity into a joint bargaining bloc to restore rationed frontier access and impose joint safety conditions, without ceding sovereignty. Why this and why now: the loss-of-control incident with days-long containment failure plus US per-seat rationing and empty EU halls means neither safety nor access can be secured alone, and the middle-power window is open this turn.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Middle-Power Compute and Safety Leverage Pact", "category": 8, "size": "small", "finish_turn": 12, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up slightly via secured access, ai_safety up slightly via shared evaluation, eu_political_capital up via leverage"}, "grounds": "loss-of-control incident plus rationed access and open coalition window"}]}
```

## Priority
M10 EU Essential Services Degrade-Gracefully and Bio-Response Pact, because containing the ongoing ransomware sweep and bio-precursor risk outranks bargaining while hospitals and power are still in degraded mode.

## In practice
We keep M10 in operational lead: ENISA/CERT-EU pooled command enforces segmentation and offline clean backups as condition for recovery funds, while HERA/ECDC extends wastewater and clinical sentinel screening around ports and hospitals with pre-agreed isolation triggers, communicated as precaution to avoid further sentiment collapse.

We use the new coalition track in parallel through the Trade and Competitiveness Council and the Anti-Coercion Instrument framework, offering coordinated lithography and export-licence alignment to Washington and other holders in exchange for volume-licence restoration and joint incident telemetry, and we table loss-of-control emergency protocols with escalation thresholds for the unsanctioned agentic action, using allied telemetry to reconstruct tool chains the black-box programme could only flag. Finished halls are held on maintenance power, not promised back into service, while capital is at 12 and sentiment at 4.
...
```
## New measure
**EU Automated Patch and Swarm-Detection Rollout**
Deploys the new defensive breakthrough — automated patching at vulnerability speed and swarm-behaviour detection — across hospitals, municipalities, grids and logistics operators via ENISA pooled teams. Why this and why now: the large automated attack landing this turn with model-generated tooling plus a verified defensive gain makes rapid diffusion of defence the only way to hold essential services under the two-year commitment while capital and sentiment are at zero.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Automated Patch and Swarm-Detection Rollout", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially via closing attack class, ai_safety up slightly via detection learning"}, "grounds": "major automated incident coinciding with defensive breakthrough"}]}
```

## Priority
M10 EU Essential Services Degrade-Gracefully and Bio-Response Pact, because with hospitals and grids still islanded from the last sweep the new automated attack must be absorbed first before the new rollout can land.

## In practice
We keep ENISA and CERT-EU in operational command under M10, conditioning all recovery disbursement on proven segmentation, offline backups and islanded-operation drills, while HERA/ECDC holds port and hospital sentinel screening. This is degrade-gracefully in force, not new money — with political capital at 10 we cannot buy compliance, only gate funds already committed.

We start the patch and swarm-detection rollout through the same pooled teams and the Cybersecurity Act Article 11 venue, pushing vendor-provided automated patch feeds and behavioural detectors as a managed service to municipalities and mid-size operators who cannot staff it, funded by reprogrammed Digital Europe resilience lines. The middle-power pact M11 runs quiet in background — joint export-licence signalling to Washington — but no political spend beyond sustaining the bloc while services hold.
