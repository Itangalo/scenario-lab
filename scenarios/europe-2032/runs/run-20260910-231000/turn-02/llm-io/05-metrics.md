...
- characters 13370-14601: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14644-17234: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17290-18349: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: A coordinated municipal campaign across several host-candidate regions blocks AI factory site surveys over power prices and water use, forcing at least one shortlisted site to be withdrawn this turn.
- `emergent_grid_insurance_crunch` -- first noted turn 2, listed in 1 turn(s) so far: European insurers and reinsurers sharply raise premiums and impose security conditions for grid, port and water operators after the autumn intrusions, forcing deferred maintenance cuts elsewhere.
