...
- characters 105-13052: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13095-13287: `{{metrics_json}}` from the run's live metric values
- characters 13380-14560: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14603-16753: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16809-17866: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated local protests and municipal permit challenges stall several planned EU data-centre expansions over energy prices and water use, without any EU-level decision causing it.
- `emergent_ai_fraud_moral_panic` -- first noted turn 1, listed in 2 turn(s) so far: Major EU press investigation links Kimi K3-derived phishing and fraud wave to elderly victims across several member states, triggering calls for open-weight liability rules.
