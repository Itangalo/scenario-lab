...
- characters 452-1669: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1713-3847: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5761-6219: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9049-11660: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): A Europe that can keep essential services running without the leading foreign model
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Redress and Incident Ledger for Deployed Decision Systems | 2 | small | 12 | 14 | own jurisdiction | ai_safety up slightly, resilience up slightly, public_sentiment protected from further fall | 2 | running |
...
| M3 | EU Critical Infrastructure Cyber Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Lithography Shield and Reciprocity Pact | 8 | small | 2 | 5 | own jurisdiction, United States as counterpart | eu_ai_sovereignty up slightly, eu_political_capital protected against coercion loss | 2 | finished |
| M5 | EU Chip Continuity and Critical Compute Reserve | 6 | small | 3 | 6 | own jurisdiction | resilience up, eu_ai_sovereignty protected from fall | 2 | finished |
| M6 | EU Entry-Level Transition and Fraud Victim Fund | 7 | small | 7 | 9 | own jurisdiction | public_sentiment up, eu_political_capital up, resilience up slightly | 2 | finished |
| M7 | EU Middle-Power Resilience and Access Pact | 8 | small | 11 | 13 | own jurisdiction with middle-power coalition counterparts | eu_political_capital up, resilience up, ai_safety up slightly from deployed-system assurance | 2 | finished |
...
## New measure
**EU Redress and Incident Ledger for Deployed Decision Systems**
It imposes immediate incident reporting, log-preservation and victim redress on AI decision systems in benefits, policing and courts, closing the gap the scandal exposed.
This and now because the ombudsman finding shows the deployments of 2032 were lawful but wrong — the Act as drawn in 2024 does not reach them — and with open weights now within points of the frontier we cannot restrict our way out; we must make what Europe actually runs readable, reportable and reversible to keep essential services running without the leading foreign model.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Redress and Incident Ledger for Deployed Decision Systems", "category": 2, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up slightly, resilience up slightly, public_sentiment protected from further fall"}, "grounds": "to answer the scandal as a coverage gap and restore auditable fallbacks"}]}
```

## Priority
EU Redress and Incident Ledger for Deployed Decision Systems — because the scandal as a coverage gap, not a breach, makes the AI Act itself look badly drawn for 2024 systems against 2030 deployments, and without a redress answer restriction becomes cheap and all adoption becomes politically impossible.

## In practice
We treat the affair in its second, more damaging reading: the system was never classified high-risk at all, the deployment sat in a gap Annex III does not reach, and everything done was lawful. The Commission therefore proposes under the AI Act review clause a narrow implementing act: mandatory incident reporting to the AI Office, preservation and independent sampling of logs that were generated but never read, and a funded redress window via the Transition Fund, run with national ombudsmen and the Fundamental Rights Agency. No new siting, no new compute spend.

At the same time we hold M7 as the delivery track: DG TRADE/CNECT close the reciprocal licence-alignment letters for essential-service inference, while ENISA/HERA and hospital CIOs extend automated patching, swarm detection and the new interpretability checks to the older and open-weight fallback models now permanently on EU servers after this half-year's open-weight frontier release. Legal Service keeps both shells heated and drained and shifts works to the unblocked site under the water-use injunction, and communications stays on the two EU-hosted regions that cut waits, to show something that worked while redress is built.
...
```
## New measure
**EU Visible Benefit Adoption Pact**
It turns the half-year's two successes — EU-hosted triage that cut waits and new tailored therapies reaching ordinary clinics — into attributed European delivery, while ring-fencing embodied-AI deployment to logistics tasks the Union can absorb.
This and now because with open weights unrecallable, sovereignty at 3 and sentiment at 0, the only capital left to earn is proof that European decisions deliver care faster; medical breakthrough plus adoption success is a window to re-attribute benefit to Europe before dependence and the fraud scandal define it.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Visible Benefit Adoption Pact", "category": 9, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, resilience up slightly via clinical deployment"}, "grounds": "to convert medical and triage wins into attributed European benefit"}]}
```

## Priority
EU Visible Benefit Adoption Pact — because redress alone cannot lift sentiment from zero, and only visible, European-attributed clinical benefit outranks the ledger work this turn.

## In practice
We run the Pact through DG SANTE and HERA with hospital CIOs in the two showcase triage regions plus three volunteer systems: EU procurement preference for EU-hosted inference for triage notes and tailored-therapy pathways, with digital signatures and incident-logging from M8 built in, funded by re-scoped EU4Health and Horizon clinical-translation money. Communications names the European decision — hosted in Europe, audited in Europe, redress in Europe — not the foreign model underneath.

At the same time we hold M8 as the trust floor: AI Office incident reporting and FRA/ombudsman redress stay the condition for any adoption funding, and ENISA extends patching and swarm detection to the clinical open-weight stack. On embodied AI we do not build: EPSCO and DG GROW issue logistics-only deployment guidance with social partners, steering robots to picking/sorting/palletising where displacement can go through retraining, and keeping care/repair/construction manual, to avoid a second benefits-style backlash.
