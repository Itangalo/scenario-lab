...
- characters 452-1686: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1729-3356: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3683-4137: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6503-8761: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_anti_datacentre_sabotage (emergent event):** Coordinated physical sabotage and blockades targeting data-centre construction and grid interconnects spread across several member states amid hostile sentiment and water disputes.
...
|---|---|---|---|---|---|---|---|---|---|
| M6 | EU Entry-Level Transition and Fraud Victim Fund | 7 | small | 7 | 9 | own jurisdiction | public_sentiment up, eu_political_capital up, resilience up slightly | 2 | running |
...
## New measure
**EU Entry-Level Transition and Fraud Victim Fund**
It provides rapid income bridging, retraining vouchers and municipal staffing top-ups for the narrow displaced cohorts in routine coding, analysis and tier-one support, plus reimbursement for AI-fraud victims, paid from repurposed cohesion and ESF+ funds.
Why this and why now: with political capital at 2.0 and public sentiment at 10.0 after the bio weeks, job cuts and fraud wave, no large sovereignty or regulatory build can move; a small category 7 labour and social protection measure is the only instrument that can rebuild tolerance to act while holding our absorb-without-foreign-control commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Entry-Level Transition and Fraud Victim Fund", "category": 7, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, eu_political_capital up, resilience up slightly"}, "grounds": "capital at 2.0 allows only one small measure and sentiment collapse demands it"}]}
```

## Priority
M6 EU Entry-Level Transition and Fraud Victim Fund, because without arresting hostility and municipal breakdown no sovereignty, safety or resilience instrument can be enforced or sited.

## In practice
We run this through EPSCO and the Competitiveness Council as a cohesion reprogramming decision, not a new law: DG EMPL redirects existing ESF+ and cohesion envelopes to works-council-registered retraining for the hit entry-level cohorts, with HERA and ENISA finished tools continuing bio and cyber containment without new mandates. Municipalities that received software without staff get funded seconded operators for the detection cell and patching rollout.

We contain the member-state side-deal by keeping it inside joint procurement reporting and re-export approval, offering the defecting capital accelerated access to the Transition Fund and pooled cyber kits in exchange for no separate hyperscaler data-transfer concession. On research, we publicise where the verifiable-advance breakthrough landed without claiming sovereignty gain from it, using it to show finished EU programmes still produce evidence while we rebuild permission to build.
...
```
## New measure
None this turn, while we wait for political capital to rebuild enough to fund and enforce anything new.

## Store changes
No changes.

## Priority
M6 EU Entry-Level Transition and Fraud Victim Fund, because with capital at zero and sabotage spreading it is the only instrument still paying out that keeps municipalities staffed and prevents hostility hardening into permanent veto on siting.

## In practice
We run M6 as the shield for the gigafactory shells: DG EMPL accelerates voucher and operator payouts to the affected construction regions, and we condition municipal top-ups on hosting protection — funded liaison officers, water-compensation packages, and joint patrols with grid operators around interconnect sites. No new siting decision is forced this semester; the two poured sites are mothballed warm with permits extended via the Competitiveness Council.

We contain the breach without a new law: the Commission keeps the defecting hyperscaler deal inside joint-procurement reporting and re-export review, offering access to pooled cyber kits and Transition Fund tranches if no further side letters are signed, while ENISA and HERA finished capacities handle sabotage response and detection with existing mandates. Public communication stays on absorption-without-foreign-control — fraud reimbursement fixes and protein-design countermeasure work — not on sovereignty we cannot currently build.
