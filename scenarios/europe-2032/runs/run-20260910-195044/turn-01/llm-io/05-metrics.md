...
- characters 16352-16920: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated local protests and municipal permit challenges stall several planned EU data-centre expansions over energy prices and water use, without any EU-level decision causing it.
- `emergent_ai_fraud_moral_panic` -- first noted turn 1, listed in 1 turn(s) so far: Major EU press investigation links Kimi K3-derived phishing and fraud wave to elderly victims across several member states, triggering calls for open-weight liability rules.
...
## Metrics
```json
{"ai_capability": 54.0, "openweight_capability": 46.0, "ai_safety": 32.0, "resilience": 38.0, "eu_ai_sovereignty": 19.0, "eu_political_capital": 36.0, "public_sentiment": 36.0}
```
## Narrative
...
SOVEREIGNTY: 22 last turn, no finish, in flight +0, export_control_escalation t1 −2, capability rose 2.0 −1 = 19
## Store changes
```json
{"store": []}
```
