...
- characters 13005-13194: `{{metrics_json}}` from the run's live metric values
- characters 13287-15692: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15736-17676: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17732-18048: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M7 | EU Depot Siting Arbitration and Continuity Pact | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M7", "fields": {"finish_turn": 10}, "grounds": "named priority"}]}
```
```
