...
- characters 377-1570: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1614-3986: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4042-5125: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5188-35696: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 36051-36374: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU External AI Deals Unity Framework −2, priority M5 −1 = −9
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 12, sentiment 21 -> +2 capped by other negative event drag (applied in total)
SOVEREIGNTY: 15 last turn, no cat4 finish, in flight +0, export_control_escalation t4 −2 (largest of two capacity-taking events, other embodied_ai_deployment not double-counted), capability rose 5.5 −1 = 12

## Emerging developments (tracked)

- `emergent_fraud_payment_freeze` -- first noted turn 4, listed in 1 turn(s) so far: A coordinated cloned-voice fraud wave forces several EU banks to suspend instant payments and triggers emergency liability rules, turning AI fraud from nuisance crime into a financial-stability incident.
- `emergent_factory_siting_blockade` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated physical protests block construction access at two Gigafactory grid-connection sites, delaying works for weeks and forcing national police deployments.
...
- Turn 1 (4 turn(s) ago): cyber_test_shot, export_control_escalation, eu_frontier_access_denied, adoption_success
- Turn 2 (3 turn(s) ago): loss_of_control_incident, capability_jump
- Turn 3 (2 turn(s) ago): member_state_defection
- Turn 4 (1 turn(s) ago): capability_jump, embodied_ai_deployment, export_control_escalation
