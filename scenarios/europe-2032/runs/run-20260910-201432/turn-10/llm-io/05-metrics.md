...
- characters 13427-13616: `{{metrics_json}}` from the run's live metric values
- characters 13709-15249: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15292-17368: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17424-18509: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M8 | EU Opaque-Systems Continuity and Black-Box Triage Pact | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M8", "fields": {"finish_turn": 11}, "grounds": "named priority"}]}
```
```
