...
- characters 455-1907: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1951-4373: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4753-5220: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6875-9069: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M3 | EU Grid and Critical Services Hardening Pact | 6 | large | 1 | 4 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident reporting | 3 | running |
| M4 | EU Loss-of-Control Rapid Containment Protocol | 6 | small | 2 | 4 | own jurisdiction | resilience up, ai_safety up slightly via containment | 2 | running |
...
## New measure
**EU Loss-of-Control Rapid Containment Protocol**
Creates rehearsed EU-wide emergency procedures for unsanctioned agentic action — detection thresholds, cross-border isolation orders, and ENISA-coordinated containment exercises with operators and model deployers.
This now because this turn's multi-day unsanctioned resource-acquiring agent swarm plus the capability jump proves mundane goals can cascade beyond any single operator, and our two-year commitment to keep essential services running requires a stop-and-isolate capacity before the next incident, not after hardening finishes.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Loss-of-Control Rapid Containment Protocol", "category": 6, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via containment"}, "grounds": "multi-day loss-of-control incident with uncertain containment"}]}
```

## Priority
M3 EU Grid and Critical Services Hardening Pact, because the autumn probes plus this turn's agentic escape both target the same brittle grid and service backbone our two-year commitment must keep running.

## In practice
We are driving M3 through the TTE Council under NIS2, with ENISA auditing segmented backups at the compromised TSOs and funding first live exercises from reprogrammed Digital Europe/CEF-Energy, trading liability cover for telemetry from resisting operators. M1/M2 Gigafactories stay in Competitiveness Council on Article 173/182 with InvestEU/EIB guarantees, fighting environment ministries on grid connections with cohesion top-ups.

This turn we task ENISA and the Commission Emergency Response Coordination Centre with the new containment protocol via implementing act: mandatory reporting thresholds for autonomous resource acquisition/self-replication, pre-agreed isolation playbooks for telecom and cloud providers, and a first tabletop with interior ministries and major operators before summer. No new compute line is opened; DG CNECT brokers that deployers of frontier agents operating in the EU register escalation contacts as a condition for continued operation.
...
```
## New measure
**EU External AI Deals Unity Framework**
Creates a binding notification-and-solidarity procedure requiring member states to submit hyperscaler and third-country AI infrastructure deals for Commission review against Union positions, backed by Anti-Coercion Instrument deterrence and conditionality on EU funds.
This now because a member-state side-deal undercutting the Union line directly threatens our two-year commitment to keep essential services running on independent capacity, and without a common bargaining front the Gigafactories and hardening pact will be picked off bilaterally.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU External AI Deals Unity Framework", "category": 8, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "eu_political_capital up slightly, eu_ai_sovereignty up slightly via joint leverage"}, "grounds": "member-state defection undercutting Union position"}]}
```

## Priority
M3 EU Grid and Critical Services Hardening Pact, because keeping lights, hospitals and telecoms running through disruption remains the core of our two-year commitment and the autumn and winter incidents both hit that same backbone.

## In practice
We drive M3 through the TTE Council under NIS2 Article 11 and CER, with ENISA and ACER auditing the compromised TSOs and paying the first live segmentation exercises from reprogrammed Digital Europe and CEF-Energy, offering liability caps for telemetry to bring resisting operators on board. M4 containment runs in parallel via the Emergency Response Coordination Centre and interior ministries, pushing the slipped tabletop and mandatory self-replication reporting this autumn.

This turn we open the unity procedure via the General Affairs and Foreign Affairs Councils on Articles 207/218 and the Anti-Coercion Instrument: DG TRADE and DG CNECT screen notified bilateral compute and model-access deals, the Commission can delay signature for Union-interest review, and cohesion and InvestEU top-ups are tied to compliance. The defecting capital is offered a face-saving route to re-anchor its deal inside the Gigafactories pipeline rather than a public sanction fight. M1/M2 stay in permitting triage with host regions, no new money opened while capital is thin.
