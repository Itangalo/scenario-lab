...
- characters 377-1829: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1873-4295: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4351-5485: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5548-32492: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32847-33028: `{{event_history}}` from the run's own event record
...
- `emergent_synthetic_fraud_wave` -- first noted turn 1, listed in 2 turn(s) so far: A coordinated wave of AI-generated fraud using Kimi K3-derived voice and document forgeries hits several EU banks and public administrations, causing material losses but no infrastructure damage.
- `emergent_provenance_mandate` -- first noted turn 1, listed in 2 turn(s) so far: Major EU scientific publishers and universities jointly mandate watermarking and provenance disclosure for AI-assisted papers after Millennium Prize proof controversy.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, export_control_escalation, eu_frontier_access_denied, adoption_success
- Turn 2 (1 turn(s) ago): loss_of_control_incident, capability_jump
