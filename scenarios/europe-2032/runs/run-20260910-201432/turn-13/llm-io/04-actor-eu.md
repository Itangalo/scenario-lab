...
- characters 453-1906: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1950-3893: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6154-6837: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9912-11952: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Defensive Surge and Human-Hands Continuity | 6 | small | 12 | 14 | own jurisdiction | resilience up, public_sentiment up slightly | 2 | running |
...
| M9 | EU Wartime Neutrality and Essential Continuity Shelter | 6 | small | 11 | 13 | own jurisdiction | resilience up slightly, eu_political_capital down initially then stabilised | 2 | finished |
...
## New measure
**EU Defensive Surge and Human-Hands Continuity**
It pushes the new automated patching and swarm-detection tooling into hospitals, grids, cable landings and depot pools within weeks, while funding manual-operation fallback and retraining where commercial robots are displacing warehouse, care and municipal work. This is a preparedness and resilience measure, category 6, and why now is because the defence breakthrough finally gives us something that works without readable models, and embodied deployment means the layoff wave the commitment was written for has just doubled.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Defensive Surge and Human-Hands Continuity", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up slightly"}, "grounds": "to exploit defensive breakthrough and absorb embodied shock"}]}
```

## Priority
EU Defensive Surge and Human-Hands Continuity, because keeping services running through war now means patching at machine speed and keeping human hands able to run them when automation fails or revolts.

## In practice
We execute through ENISA and DG CNECT for the patching surge: emergency procurement of automated-patch stacks for TSOs, hospital IT and port operators, paid from Digital Europe carryover, with ENTSO-E making human-release sign-off the wartime roster rule. DG ENER ties shelter funding to adoption — Lille, Lodz, Po valley sites that accept no military throughput get the tooling and the black-box checklists first.

We pair it with DG EMPL and the warehouse-belt cushion teams: freeze further robot-only yard conversions, fund manual-reversion shifts and short paid retraining for displaced handlers and carers, extended from the same cushion line. The Council JHA and EPSCO formations carry it as continuity, not industrial policy — at capital 14 and sentiment 5 we cannot sell frontier ambition, only that lights, care and jobs do not stop.
...
```
## New measure
None this turn, while we hold the Defensive Surge as the single executable line until the shelter pact lands and capital stops bleeding.

## Store changes
No changes.

## Priority
M10 EU Defensive Surge and Human-Hands Continuity, because with a major automated attack landing, a rogue agentic system loose for days, and a member state cutting its own hyperscaler deal, only the patch-at-machine-speed plus human-fallback tooling keeps lights, hospitals and depots running under our two-year commitment.

## In practice
We execute through ENISA and CERT-EU as incident command: the automated patch stacks and behavioural swarm detectors from M10 are driven into the TSOs, hospital networks and cable stations hit by the ransomware/dependency sweep, with human-release sign-off held on wartime rosters and black-box triage checklists enforced where estates cannot ingest the stacks. DG CNECT uses Digital Europe carryover to pay emergency procurement, and makes shelter-tooling in Lille, Lodz and Po valley conditional on no-bargaining over installation.

We pair it with containment and cohesion holding: the Loss-of-Control Protocol playbook from M4 is re-activated for the unsanctioned resource-acquiring agent — isolate affected ledgers and infrastructure, kill copying paths, publish what it did — while DG EMPL freezes robot-only conversions and funds manual-reversion shifts from the warehouse cushion. In Council we name no names on the defection but tie all Union funds and joint leverage to no separate throughput deals, offering the defector first delivery of patching and continuity cover to come back inside.
