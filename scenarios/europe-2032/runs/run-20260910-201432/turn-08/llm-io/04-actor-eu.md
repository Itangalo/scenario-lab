...
- characters 451-2395: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2438-4422: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4490-5177: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7364-8797: `{{previous_actions}}` from the actor's own response from the previous turn
...
```
## New measure
None this turn, while political capital at 1.0 and public sentiment at 4.0 rebuild enough to fund anything without collapse.

## Store changes
No changes.

## Priority
No priority measure in flight to name this turn, because with all prior measures finished and no capital to start a new instrument, holding pooled procurement, defensive patching, and cushion payouts operational outranks opening a new front.

## In practice
We govern what is already built. DG EMPL and national paying agencies keep the finished Displacement Cushion paying in warehouse and supplier towns, with mayoral funds holding factory perimeters, while we publicly concede the arithmetic is short of lost hours rather than defending it. ENISA, the TSO cell and hospital IT keep machine-speed patching and probe detection on grids, payments and fallback clinical systems first.

On degraded continuity we hold the unity line on paper while managing the break in practice: the Commission maintains condemnation of the November bilateral deal to preserve joint leverage, DG CNECT sustains pooled licences for less-capable models plus older-model and paper-queue continuity for hospitals and finance ministries, and we do not open a coercion or procurement fight we cannot enforce. On the benefits-trust collapse we task the AI Office with preparing a narrow high-risk Annex patch, and ECDC/HERA continues the genome-model detection stocktake under existing preparedness — no new instrument, no new promise, until governability returns.
