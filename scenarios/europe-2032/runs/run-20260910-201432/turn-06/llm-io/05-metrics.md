...
- characters 13445-13634: `{{metrics_json}}` from the run's live metric values
- characters 13727-14955: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14998-17539: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17595-18131: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Automation Displacement Cushion | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 challenger win on moratoriums takes office turn 6"}]}
```
```
