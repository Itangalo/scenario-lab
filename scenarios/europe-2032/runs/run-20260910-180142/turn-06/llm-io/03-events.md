...
- characters 376-2358: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2401-4491: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4547-5786: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5849-29631: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29986-30294: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Municipal Resilience Hardening Fund −2, EU AI Access Bargaining Bloc −2, priority −1 = −11
PROPOSAL BONUS: EU AI Access Bargaining Bloc (cat 8, small) +3 — member-state side deal this turn plus US tiered rationing decided this turn requires joint bloc directly
LEGITIMACY LENDS: capital 10, sentiment 32 -> +2 (capped, applied after other terms)
SOVEREIGNTY: 16 last turn, no capacity event, Gigafactories in flight +0, member_state_defection t5 −2, capability rose 1.0 −0 = 14

## Emerging developments (tracked)

- `emergent_clinical_fallback_liability` -- first noted turn 4, listed in 2 turn(s) so far: European hospital audit finds open-weights clinical fallback hallucinating dosage guidance in edge cases, triggering a cross-border medical liability dispute over provisional AI Office certification.
- `emergent_clinical_hallucination_backlash` -- first noted turn 4, listed in 2 turn(s) so far: European hospital clinicians publish a case series showing open-weight replacement models produced plausible but wrong triage and dosing guidance in edge cases, triggering national press coverage and a formal review of provisional AI Office certifications.
...
- Turn 1 (5 turn(s) ago): cyber_test_shot, openweight_frontier_release
- Turn 2 (4 turn(s) ago): capability_jump
- Turn 3 (3 turn(s) ago): ai_investment_collapse, eu_frontier_access_denied, adoption_success
- Turn 5 (1 turn(s) ago): openweight_frontier_release, member_state_defection, election_consolidation
