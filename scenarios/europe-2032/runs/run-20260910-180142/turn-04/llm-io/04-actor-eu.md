...
- characters 454-1702: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1745-4249: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4317-4782: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6944-9154: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Frontier Evaluation Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via retained talent | 2 | running |
| M5 | EU Continuity Stack for Public Services | 9 | small | 3 | 5 | own jurisdiction | public_sentiment up, eu_political_capital up via visible continuity, resilience up slightly | 2 | running |
...
## New measure
**EU Continuity Stack for Public Services**
Keeps hospitals, ministries and firms running after the US frontier cut-off by switching them to EU-hosted open-weight and allied models on Union compute, and scales the public-sector adoption win that just landed.
This is Diffusion, adoption and public trust work, because the denial is a legitimacy shock that only visible continuity plus visible benefit can contain, and the investment collapse means we must do it with capacity we already control rather than promised hyperscaler build-out.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Continuity Stack for Public Services", "category": 9, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, eu_political_capital up via visible continuity, resilience up slightly"}, "grounds": "frontier access denied to EU users plus domestic adoption success to scale"}]}
```

## Priority
EU Continuity Stack for Public Services, not the Gigafactories, because the cut-off is hitting wards and ministries now and without continuity both sovereignty lines lose political permission to continue.

## In practice
We task DG CNECT and DG SANTE with a continuity mandate under the Health Emergency and NIS2 legal bases: EU4Health and Digital Europe funds reprogrammed to pay for inference on EuroHPC and existing commercial EU-cloud capacity, with the AI Office certifying substitute models from the Evaluation Cell pipeline for clinical and administrative use. Member-state health and interior ministers get a TTE/ EPSCO joint commitment: Brussels pays migration costs if they mandate EU-hosted fallback by December.

We use the adoption success explicitly — the waiting-list and permit-time wins — as the communications and procurement template, extending those contracts to the fallback stack rather than starting new tenders. M1/M2 stay on permitting track but without new cash this turn given the investment reset; we protect the two unblocked sites and pause disbursement expectations elsewhere. The Evaluation Cell (M4) gets first call on testing the substitute models for cyber safety before rollout.
...
```
## New measure
**EU Municipal Resilience Hardening Fund**
Extends the Shield that worked on grids/ports to hospitals, municipalities and mid-size industry with funded cyber-hardening, backup inference, and exercised recovery playbooks.
This is Preparedness and resilience work, because open-weight capability at 53 means distributed offensive cyber cannot be recalled, safety assurance at 21 cannot stop it, and the only lever fully in EU control that serves the two-year commitment to absorb shocks is making essential services degrade rather than stop.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Resilience Hardening Fund", "category": 6, "size": "small", "finish_turn": 7, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, public_sentiment up slightly via kept services"}, "grounds": "open-weight exploit capability distributed while Shield proved hardening contains cascade"}]}
```

## Priority
EU Continuity Stack for Public Services, not the Gigafactories, because wards and permit offices losing frontier access now will decide whether any sovereignty build retains political permission.

## In practice
We keep M5 as the operational lead this turn: DG CNECT/DIGIT fund inference vouchers on EuroHPC and EU clouds via reprogrammed Digital Europe/EU4Health money, AI Office issues provisional certifications for open-weight substitutes with M4 doing rapid cyber-safety checks, and we hold health/interior ministers to the December fallback mandate. M1/M2 are kept alive but on low burn — protect permitting for the two viable sites, no new disbursement promises until power-price and funding reset clarifies.

We launch the hardening fund via UCPM/Cybersecurity Act Article and NIS2 implementing acts: joint procurement of detection/backup stacks for municipalities and hospitals, Athens-run exercises expanded beyond energy/ports, and repair crews on retainer. Cohesion and RRF-reflow money pays first wave; reluctant finance and energy ministries are bought with co-funding and avoided-compensation logic from the October-November contained incidents.
