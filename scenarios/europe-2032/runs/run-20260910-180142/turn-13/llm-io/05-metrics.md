...
- characters 107-12925: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12968-13158: `{{metrics_json}}` from the run's live metric values
- characters 13251-14096: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14140-15999: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16055-16690: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_health_staff_pool` -- first noted turn 13, listed in 1 turn(s) so far: Cross-border locum networks built during the health strikes formalise into a permanent EU health-staff pooling platform, easing elective backlogs but triggering a political fight over wage harmonisation.
...
## Notepad
PORTFOLIO CHARGE t13: M9 EU Strait-Proof Continuity Reserve −2, priority −0 (no effect, capital below 20) = −2
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 0.0, sentiment 16.0 -> +0 capped, capital remains at floor after scandal and displacement losses
SOVEREIGNTY: 19 last turn, no capacity event, no cat4 in flight +0, capability rose 0.8 −0 = 19
