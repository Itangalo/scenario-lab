...
- characters 454-1299: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1343-3202: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4113-4564: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7450-8762: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Rebuild trusted EU-run AI for essential services that no suspension can stop
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Strait-Proof Continuity Reserve | 6 | small | 11 | 14 | own jurisdiction | resilience up moderately, eu_political_capital stabilized via visible continuity | 2 | running |
...
| M3 | EU Critical Infrastructure Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident reporting | 3 | finished |
| M4 | EU Frontier Evaluation Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via retained talent | 2 | finished |
| M5 | EU Continuity Stack for Public Services | 9 | small | 3 | 5 | own jurisdiction | public_sentiment up, eu_political_capital up via visible continuity, resilience up slightly | 2 | finished |
| M6 | EU Municipal Resilience Hardening Fund | 6 | small | 4 | 7 | own jurisdiction | resilience up substantially, public_sentiment up slightly via kept services | 2 | finished |
| M7 | EU AI Access Bargaining Bloc | 8 | small | 5 | 8 | own jurisdiction | eu_ai_sovereignty up slightly, eu_political_capital down then up via held line | 2 | finished |
| M8 | EU Clinical AI Safety Review and Supervised Re-entry | 1 | small | 9 | 11 | own jurisdiction | ai_safety up slightly, public_sentiment up, resilience up slightly | 2 | finished |
...
## New measure
None this turn, while we absorb the walkout and conserve the last of political capital for the continuity reserve already in flight.

## Store changes
No changes.

## Priority
M9 EU Strait-Proof Continuity Reserve, because with capital at 9.0 and wards walking out over manual overload, preventing cascade in hospitals, energy and payments outranks starting any new instrument we cannot fund.

## In practice
We use the M9 envelope as the emergency staffing bridge: Health Council authorises paid overtime, cross-border locum pooling and HERA-funded repeat-test capacity for returning patients, framed explicitly as manual-endurance funding, not re-entry. ENISA telemetry and the AI Office gate stay unchanged — two supervised pilots with physician co-sign only — to avoid trading the walkout for an unsafe restart.

We task the Employment and Health formations jointly to translate the knowledge-work evidence — productivity up, employment holding, junior staff gaining most — into a ward message: supervised re-entry when safety allows reduces overload rather than replacing staff. No new procurement, no new compute promise; Commission holds side cloud deals inside M9 legal anchor and pushes spares stockpiles to the two walkout states first to show the reserve lands where strain is highest.
...
```
## New measure
None this turn, while political capital sits at 3.0 and the reserve still carries its 2-per-turn cost.

The scandal around AI-supported decisions in social insurance / courts — judged as an AI Act enforcement failure, not a gap in the text: obligations existed, oversight did not bite, redress came from a court/ombudsman after harm — demands answer, but we cannot fund a new instrument without killing the overtime/locum bridge holding the wards. Labour displacement is real but narrow — routine coding, standardised analysis, first-draft documentation, tier-one support with entry hiring not recovering — which lets us hold the line: no broad restriction wave, targeted transition instead. We answer both inside M9 and finished instruments this turn and keep the slot empty until M9 finishes in turn 14.

## Store changes
No changes.

## Priority
M9 EU Strait-Proof Continuity Reserve, because with wards on strike, capital near zero and a trust scandal breaking, visible continuity and redress outrank any new build we cannot pay for.

## In practice
We keep M9 as the manual-endurance fund: Health and Employment Councils extend paid overtime, cross-border locum pooling and HERA repeat-test capacity to the two walkout systems first, with spares and lab slots directed visibly to picket hospitals. The two supervised documentation pilots stay frozen with physician co-sign only; we publish the gate and telemetry rule to answer unions demanding a date and conditions — date when the Clinical Review certifies, conditions co-signed and monitored — and we carry the office-productivity evidence to the wards as overload relief, not replacement.

On the automated-decision scandal we do not legislate anew. We task the AI Office and national market-surveillance authorities under the existing AI Act to suspend the offending system, order human re-review of affected cases with compensation via the social-insurance budget, and publish the audit trail — framing it as enforcement that failed and now bites. On displacement we task employment ministries with flexicurity-style retraining and hiring incentives aimed only at the hit functions — junior code, standard analysis, tier-one support — funded from ESF+ reprogramming, while quietly reissuing the broader forecast without acting on it.
