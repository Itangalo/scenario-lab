...
- characters 1475-3254: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3310-4396: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4459-27171: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27526-27638: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Infrastructure Shield −3, EU Frontier Evaluation Cell −2, priority −1 = −12
PROPOSAL BONUS: EU Frontier Evaluation Cell (cat 5, small) +2 — capability_jump landed this turn and this answers it directly
...
- `emergent_datacentre_permit_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated municipal protests across several member states block permitting for new hyperscale data centres over power prices and water use, forcing at least one national pause in late 2026.
- `emergent_clinical_model_pullback` -- first noted turn 1, listed in 2 turn(s) so far: Major European research hospitals pause use of US frontier models for clinical decision support after the June switch-off scare, publishing contingency procurement rules favouring models runnable inside the EU.
...
- Turn 2 (1 turn(s) ago): capability_jump
