...
- characters 106-13558: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13601-13793: `{{metrics_json}}` from the run's live metric values
- characters 13886-15078: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15122-16984: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17040-18186: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M8 | EU Distributed Frontier Shelter Regime | 2 |
...
- `emergent_clinical_trust_boycott` -- first noted turn 7, listed in 1 turn(s) so far: Cross-border patient groups file coordinated lawsuits over opaque clinical recommendations, triggering a temporary clinician boycott of AI triage tools in several member states.
- `emergent_clinical_liability_litigation` -- first noted turn 7, listed in 1 turn(s) so far: Hospitals and patient groups file coordinated cross-border liability suits over the opaque clinical assistant harm, forcing courts to rule on burden of proof for illegible AI traces.
...
## Metrics
```json
{"ai_capability": 83.0, "openweight_capability": 75.0, "ai_safety": 11.0, "resilience": 36.0, "eu_ai_sovereignty": 24.0, "eu_political_capital": 14.0, "public_sentiment": 17.0}
```
...
## Notepad
PORTFOLIO CHARGE t7: M7 EU Clinical AI Harm Review and Liability Protocol −2, M8 EU Distributed Frontier Shelter Regime −2, priority −1 = −5
PROPOSAL BONUS: EU Distributed Frontier Shelter Regime (cat 6, small) +3 — rsi_onset plus openweight frontier release fired this turn and this answers degraded-operation need directly
LEGITIMACY LENDS: capital 12, sentiment 22 -> +2
SOVEREIGNTY: 20 last turn, M1 InvestAI Gigafactories finishes t7 +5, no capacity event, no cat4 in flight +0, capability rose 7.0 −1 = 24
