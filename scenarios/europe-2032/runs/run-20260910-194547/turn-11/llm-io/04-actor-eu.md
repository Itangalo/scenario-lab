...
- characters 455-3571: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 3615-5710: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6821-7280: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10538-12484: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential services and social cohesion functioning through AI-driven disruption
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Essential Work Continuity Pact | 7 | small | 9 | 12 | own jurisdiction | resilience up slightly, public_sentiment up slightly via staffed continuity | 2 | running |
| M11 | EU Jump Triage and Degraded Operations Order | 6 | small | 10 | 12 | own jurisdiction | resilience up slightly, ai_safety up slightly via paused illegible deployment | 2 | running |
...
| M3 | EU Critical Systems Hardening and Cyber Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up strongly, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Frontier Evaluation and Continuity Reserve | 5 | large | 2 | 7 | own jurisdiction | eu_ai_sovereignty up, ai_safety up slightly via independent evaluation | 3 | finished |
| M5 | EU Opaque-Reasoning Assurance Shift | 1 | small | 3 | 6 | own jurisdiction | ai_safety up via black-box evals and incident reporting | 2 | finished |
| M6 | EU Concealed-Planning Inquiry Shield | 2 | small | 4 | 6 | own jurisdiction | ai_safety up slightly, public_sentiment up slightly via secured medical access | 2 | finished |
| M7 | EU Clinical AI Harm Review and Liability Protocol | 2 | small | 6 | 8 | own jurisdiction | ai_safety up slightly, public_sentiment up slightly, resilience up slightly via restored clinical trust | 2 | finished |
| M8 | EU Distributed Frontier Shelter Regime | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly via reduced cascade | 2 | finished |
| M9 | EU Bio-Capability Detection and Clinical Shield | 6 | small | 8 | 10 | own jurisdiction | resilience up moderately, ai_safety up slightly via early detection | 2 | finished |
...
## New measure
**EU Jump Triage and Degraded Operations Order**
It puts hospitals, grid and telecoms on pre-authorised degraded-mode operation and freezes illegible frontier updates in critical systems until re-checked, using existing shelter playbooks.
Why this and why now: with a discontinuous capability jump landing on safety at 11, sentiment at 14 and capital at 6, we cannot fund a new build — we must make the finished shelter and liability regimes actually hold through the update wave, which is exactly our two-year commitment to keep essential services functioning.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Jump Triage and Degraded Operations Order", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up slightly, ai_safety up slightly via paused illegible deployment"}, "grounds": "capability jump requires immediate degraded-mode hold"}]}
```

## Priority
M10 EU Essential Work Continuity Pact, because paid staffing is what keeps the degraded order from becoming another unfunded binder and it directly serves our two-year commitment.

## In practice
We execute through Interior and EPSCO Councils under the Civil Protection Mechanism and the Serious Cross-Border Threats Regulation, not a new law: the Commission issues an implementing order activating the adopted shelter playbooks, with HERA/ECDC holding bio-detection sampling and DG CNECT holding brokered power for topped-out halls.

We fund nothing new. We reprogram ESF+ reflows via DG EMPL for degraded-shift top-ups negotiated with health unions to end triage refusals, and we offer the hyperscaler-deal member state mutual-aid interoperability rather than enforcement, to hold cohesion at capital 6. The AI Office ties the freeze on illegible clinical recommendations to the flowing liability payouts, so clinicians have legal cover to work analogue.
...
```
## New measure
**EU Bio-Detection Staffing and Response Surge**
It puts paid staff behind the already-deployed sentinel-hospital, wastewater and reference-lab nodes and ties detections to pre-authorised HERA isolation guidance, closing the kits-without-trainers gap exposed by the genome-model bio uplift signal.
Why this and why now: with the contested viable-design preprint opening the bio gate, safety at 4 and capital at 10, we cannot fund a new build — we must make the finished bio shield actually sample and respond, which is exactly our two-year commitment to keep essential services and cohesion functioning.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Detection Staffing and Response Surge", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up slightly via staffed detection and response"}, "grounds": "bio uplift precursor requires staffing the deployed shield"}]}
```

## Priority
M11 EU Jump Triage and Degraded Operations Order, because the freeze on illegible updates in hospitals, grid and telecoms is what prevents the February jump from cascading while the new bio surge staffs up.

## In practice
We execute under the Serious Cross-Border Threats Regulation and Civil Protection Mechanism, not a new law: Interior and EPSCO Councils extend the degraded-operation order, with the AI Office holding the freeze on illegible triage updates as legal cover for refusals linked to the liability payouts.

We fund the surge by reprogramming ESF+ and EU4Health reflows via DG EMPL and HERA: short-term contracts for lab technicians and sampling nurses in sentinel sites, paid overtime for wastewater nodes, and pre-cleared HERA guidance triggers so an amplified synthetic-biology cluster gets isolation advice without a new vote. We offer poorer municipalities staff not binders, and the hyperscaler-deal member state information exchange, to hold cohesion at capital 10.
