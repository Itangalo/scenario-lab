...
- characters 454-1556: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1599-3570: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4938-5379: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8088-10467: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_clinical_trust_boycott (emergent event):** Cross-border patient groups file coordinated lawsuits over opaque clinical recommendations, triggering a temporary clinician boycott of AI triage tools in several member states.
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Distributed Frontier Shelter Regime | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly via reduced cascade | 2 | running |
...
## New measure
**EU Distributed Frontier Shelter Regime**
It puts hospitals, power, telecoms and municipal services onto rehearsed degraded-operation playbooks for AI-enabled attack using widely distributed frontier open weights, with pre-positioned analogue fallbacks and cross-border mutual aid triggers.
Why this and why now: with RSI onset compounding capability and an open-weight release now within 5-10 points of the closed frontier on private hardware beyond recall, restriction buys nothing and safety assurance has collapsed to 17; a small preparedness measure (category 6) is the only cheap way to keep essential services degrading rather than stopping while capital is at 12 and sentiment at 22.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Distributed Frontier Shelter Regime", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via reduced cascade"}, "grounds": "RSI onset plus unrecallable near-frontier open weights require degraded-operation capacity"}]}
```

## Priority
M7 EU Clinical AI Harm Review and Liability Protocol, because containing the illegible-recommendation harm and certifying narrower assistants is what stops sentiment collapsing further and keeps any shelter regime credible, even above our access commitment this turn.

## In practice
We run M7 through EPSCO/Health with HERA/EMA as incident registrar: pulled module stays pulled, retained traces reviewed under the failure-prediction method, interim compensation paid from reserves, and backlog publication continues to prove wards still work. DG CNECT and the AI Office hold the two gigafactory sites on brokered power and spares despite US volume-licence friction, without new money, to protect the two-year access commitment.

The new shelter regime runs via DG HOME, ENISA and the Civil Protection Mechanism with JRC support: TTE and JHA Councils adopt degraded-mode checklists for hospitals, energy DSOs and telecoms, pre-stock spares and offline procedures piloted in the M3-hardened sectors, and UCPM mutual-aid triggers for cross-border cascade. We explicitly frame this as living with distributed frontier capability we cannot recall, not as controlling it, to buy cohesion from the separate-deal member state and municipalities.
...
```
## New measure
**EU Bio-Capability Detection and Clinical Shield**
It stands up syndromic-plus-sequencing early warning in sentinel hospitals, wastewater nodes and reference labs, with stockpiled diagnostics and a common HERA-led trigger for model-assisted bio misuse reporting.
Why this and why now: with bio uplift findings opening a 4-turn precursor window, safety at 11 and open weights at 75 unrecallable, a small preparedness measure (category 6) is the only affordable way to cut time-to-detect while the shelter regime holds hospitals together and capital sits at 14.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Capability Detection and Clinical Shield", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via early detection"}, "grounds": "bio uplift precursor plus unrecallable open weights requires detection capacity"}]}
```

## Priority
M8 EU Distributed Frontier Shelter Regime, because with investment collapse evaporating foreign compute dependencies and boycotts hollowing clinical trust, keeping hospitals, grid and telecoms on rehearsed degraded operation outranks even the new bio shield this turn.

## In practice
We run M8 through DG HOME, ENISA and the Civil Protection Mechanism: JHA Council conclusions lock degraded-mode playbooks and analogue fallbacks for hospitals, DSOs and telecoms, UCPM mutual-aid triggers go live, and pilots concentrate in M3-hardened sectors to spare municipalities new costs. We offer the separate-deal member state opt-in interoperability rather than compliance fights to hold cohesion.

We launch the bio shield via HERA, ECDC and JRC under Article 168 and the Serious Cross-Border Threats Regulation: sentinel-site sequencing funds reprogrammed from EU4Health, wastewater expansion through the Urban Wastewater mechanism, and an incident-reporting channel in the AI Office tied to the finished M7 liability protocol. DG CNECT freezes gigafactory spend at topped-out halls on brokered power, protecting the access commitment without new money while valuations reset.
