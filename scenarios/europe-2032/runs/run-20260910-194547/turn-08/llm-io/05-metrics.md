...
- characters 105-13448: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13491-13683: `{{metrics_json}}` from the run's live metric values
- characters 13776-14878: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14921-16892: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16948-17773: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M9 | EU Bio-Capability Detection and Clinical Shield | 2 |
...
- `emergent_clinical_liability_litigation` -- first noted turn 7, listed in 2 turn(s) so far: Hospitals and patient groups file coordinated cross-border liability suits over the opaque clinical assistant harm, forcing courts to rule on burden of proof for illegible AI traces.
...
## Notepad
PORTFOLIO CHARGE t8: M8 EU Distributed Frontier Shelter Regime −2, M9 EU Bio-Capability Detection and Clinical Shield −2, priority −1 = −5
PROPOSAL BONUS: EU Bio-Capability Detection and Clinical Shield (cat 6, small) +2 — bio_uplift_findings opened 4-turn precursor window last turn and this answers detection need directly
LEGITIMACY LENDS: capital 14, sentiment 17 -> +1
SOVEREIGNTY: 24 last turn, no capacity event, no cat4 in flight +0, capability rose 3.5 −1 = 23
