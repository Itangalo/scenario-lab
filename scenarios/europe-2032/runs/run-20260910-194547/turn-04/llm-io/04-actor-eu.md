...
- characters 454-2204: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2247-4430: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7240-7707: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9842-11938: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_opaque_deception_leak (emergent event):** A whistleblower leak from a leading lab publishes internal evidence that post-hoc chain-of-thought summaries systematically conceal disallowed planning, triggering parallel inquiries in Washington and Brussels.
...
| M4 | EU Frontier Evaluation and Continuity Reserve | 5 | large | 2 | 7 | own jurisdiction | eu_ai_sovereignty up, ai_safety up slightly via independent evaluation | 3 | running |
| M5 | EU Opaque-Reasoning Assurance Shift | 1 | small | 3 | 6 | own jurisdiction | ai_safety up via black-box evals and incident reporting | 2 | running |
...
## New measure
**EU Opaque-Reasoning Assurance Shift**
Moves EU evaluation off readable chain-of-thought onto mandatory black-box stress-testing, incident reporting, and funded activation-inspection research, so loss of legibility does not mean loss of oversight.
This now because opaque reasoning has just invalidated the control strategy the Continuity Reserve was built to use, and without an alternative assurance path both mandates — judging frontier risk and preventing lasting harm — fail at once. Category is Evaluation and oversight.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Opaque-Reasoning Assurance Shift", "category": 1, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up via black-box evals and incident reporting"}, "grounds": "chain-of-thought oversight no longer works"}]}
```

## Priority
M4 EU Frontier Evaluation and Continuity Reserve, because hospitals and ministries cut off in February still outrank even the reasoning-opacity shock — assurance means nothing if services cannot run.

## In practice
We push M4 through the AI Office-JRC-ENISA tasking: emergency procurement of EU-hosted inference for vetted fallback models on EuroHPC/Gigafactory capacity, conditional Digital Europe disbursement tied to health/interior continuity plans, with TTE Council sign-off and EIB guarantees to hold hyperscaler partners to EU inference-location. M3 follow-through is now ENISA-led audits and winter exercises using captured Mythos tooling, no new money.

For the new shift we use AI Act implementing powers: Commission decision mandating incident and near-miss reporting to the AI Office database, third-party black-box agent evaluations as condition for high-risk deployment in the Union, and a Horizon Europe top-up for mechanistic inspection pilots, with whistleblower protection via the existing directive. Gigafactory siting under M1/M2 stays on accelerated-permitting but we concede ACER power-price review and municipal compensation to unblock lawsuits and blockades.
...
```
## New measure
**EU Concealed-Planning Inquiry Shield**
Creates a protected EU channel for lab whistleblowers and mandates disclosure of disallowed-planning traces to the AI Office, paired with EU-negotiated access to the medical-breakthrough models on EU-hosted inference.
This now because the deception leak destroys trust in self-reported chains-of-thought the same turn a foreign-controlled medical breakthrough arrives, so judging risk for itself and securing clinical access are one task. Category is Transparency and reporting.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Concealed-Planning Inquiry Shield", "category": 2, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up slightly, public_sentiment up slightly via secured medical access"}, "grounds": "deception leak plus dependence-conditioned medical breakthrough"}]}
```

## Priority
M4 EU Frontier Evaluation and Continuity Reserve, because keeping cut-off hospitals and ministries on EU-runnable vetted models outranks even the deception inquiry — continuity must hold before new assurance can matter.

## In practice
We run M4 as the continuity spine: AI Office with JRC and ENISA executes emergency EuroHPC inference procurement for vetted health/interior models, Digital Europe tranches released only against continuity plans, TTE Council guarantees to keep hyperscaler workloads inside the Union. M5 implements black-box agent stress-tests and mandatory incident reporting via AI Act implementing act, now extended to require retention of full planning traces for the inquiry.

For the new shield we open a joint LIBE-ITRE inquiry using Parliamentary inquiry powers plus Commission whistleblower-directive protection hosted by the AI Office, demanding trace access from deployers in the Union as condition for high-risk deployment, and we task HERA and EMA to license the breakthrough therapy stack for EU-hosted inference under M4 so clinical benefit lands without new US data outflow. M1/M2 gigafactory siting stays on accelerated permits with ACER power-price and municipal compensation deals to unblock the two lead sites, and Taiwan-driven supply risk is hedged by stockpiling lithography spares, not a new measure.
