...
- characters 105-13090: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13133-13325: `{{metrics_json}}` from the run's live metric values
- characters 13418-14788: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14831-16749: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16805-17563: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated physical protests and grid-connection lawsuits stall several large EU data-centre projects after power-price spikes are blamed on AI buildout.
...
## Notepad
PORTFOLIO CHARGE t2: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M3 EU Critical Systems Hardening and Cyber Shield −3, M4 EU Frontier Evaluation and Continuity Reserve −3, priority −1 = −13
PROPOSAL BONUS: EU Frontier Evaluation and Continuity Reserve (cat 5, large) +4 — denial of frontier access plus capability jump landed this turn and this answers it directly
LEGITIMACY LENDS: capital 26, sentiment 33 -> +2
SOVEREIGNTY: 21 last turn, no capacity event, no finish, in flight +0, eu_frontier_access_denied t2 −2, capability rose 5.0 −1 = 18
