...
- characters 454-3428: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 3471-5950: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 8090-8569: `{{statement_ledger}}` from the actor's live statement ledger
- characters 11903-14373: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Social-System Algorithm Audit and Redress | 1 | small | 11 | 13 | own jurisdiction | public_sentiment up slightly, eu_political_capital protected, ai_safety up slightly | 2 | running |
...
| M10 | EU Degraded-Mode Survival and Open-Weights Triage | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, ai_safety up slightly via triage and detection, public_sentiment protected | 2 | finished |
...
```
## New measure
None this turn. We are waiting for political capital to rebuild and for the audit to finish landing before staking another instrument.

## Store changes
No changes.

## Priority
M11 EU Social-System Algorithm Audit and Redress, because with capital at 2 and trust at 16 it is the only instrument we can fund, and suspending and re-reviewing the condemned scorer is what stops the scandal consuming the ward-level continuity work.

## In practice
We hold what is already built. Health, Employment and JHA Councils keep the ransomware-linked continuity cover, segmented offline backups, paper fallbacks and joint HERA-ENISA-Europol drills running on existing EU4Health and reinsurance lines, with recovery money only where scans restore within days and the capped liability backstop still limited to certified domestic standby on Controllable Core and Fallback Reserve. No separate US licence bargain, no Hague servicing concession, no cover for the breakaway hyperscaler capital.

On the forty-second review we enforce rather than legislate. The Commission via the AI Office with national market-surveillance authorities and the ombudsman sustains suspension of the social-insurance/policing scorer, orders case-by-case human re-review and redress from existing social envelopes, and publishes oversight and log-review failures to show Annex III means something. Degraded capacity stays rationed by clinical need while ENISA signatures and radiology-stack patching blunt open-weight-enabled phishing against hospitals.
