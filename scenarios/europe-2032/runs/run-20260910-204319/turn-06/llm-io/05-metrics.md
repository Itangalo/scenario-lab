...
- characters 105-13623: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13666-13857: `{{metrics_json}}` from the run's live metric values
- characters 13950-15533: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15576-18100: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18156-19285: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Care Continuity and Displacement Cushion | 2 |
| M7 | EU Fallback Inference Reserve | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election won on tiered rationing platform, administration takes office turn 6"}]}
```
```
