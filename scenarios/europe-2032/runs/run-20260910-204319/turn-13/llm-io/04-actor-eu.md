...
- characters 453-3020: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 3064-5509: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7918-8397: `{{statement_ledger}}` from the actor's live statement ledger
- characters 11580-13113: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M11 | EU Social-System Algorithm Audit and Redress | 1 | small | 11 | 13 | own jurisdiction | public_sentiment up slightly, eu_political_capital protected, ai_safety up slightly | 2 | finished |
...
```
## New measure
None this turn. We are waiting for political capital and public trust to rebuild enough to fund anything without collapse.

## Store changes
No changes.

## Priority
No priority measure in flight to name; we husband the finished degraded-mode and audit instruments while capital is at 1.

## In practice
We keep wards open on what is already built. Health Council and HERA sustain segmented backups, paper fallbacks and drills on existing EU4Health and reinsurance lines, with ENISA signatures patching radiology stacks against open-weight-enabled intrusion; no separate US licence, no Hague servicing concession, no cover for the breakaway hyperscaler deal.

On the scorer we enforce rather than legislate. The AI Office with national market-surveillance authorities and the ombudsman holds the suspension, continues case-by-case re-review and redress from existing social envelopes, and publishes the forty-second oversight failure to show high-risk rules bind. Degraded capacity stays rationed by clinical need while we absorb the eval-anomaly, breakthrough and Taiwan-precursor news without staking a new instrument we cannot pay for.
