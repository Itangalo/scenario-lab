...
- characters 455-2002: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2046-4116: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5571-6045: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8574-10497: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_care_insurance_retreat (emergent event):** Major EU health insurers jointly refuse to reinsure AI-dependent hospital IT, forcing temporary renationalisation of clinical cyber-risk in two member states.
...
|---|---|---|---|---|---|---|---|---|---|
| M6 | EU Care Continuity and Displacement Cushion | 7 | small | 5 | 8 | own jurisdiction | resilience up moderately, eu_political_capital protected, public_sentiment up | 2 | running |
| M7 | EU Fallback Inference Reserve | 4 | small | 6 | 8 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up slightly, eu_political_capital protected | 2 | running |
...
## New measure
**EU Fallback Inference Reserve**
It keeps hospitals, ministries and firms running after US frontier cut-off by routing critical workloads to EU-controlled models and reserved Gigafactory pilot capacity with certified fallbacks.
Why this and why now: with access denied at short notice and Washington rationing by tier, the Controllable Core only holds if dependence becomes a served essential load rather than an outage, buying time for leverage without new large compute promises.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Fallback Inference Reserve", "category": 4, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly, resilience up slightly, eu_political_capital protected"}, "grounds": "frontier access denial hitting hospitals and ministries"}]}
```

## Priority
M6 EU Care Continuity and Displacement Cushion, because with productivity gains without job collapse removing crisis cover and sentiment at 24, holding clinics funded and displacement quiet is what keeps any fallback and Gigafactory work politically alive.

## In practice
We operate the Reserve through DG CNECT with ENISA and HERA: triage list of denied workloads, redirect to EU-run models on reserved pilot capacity from M1/M4, enforce segmented backups and black-box monitoring where chain-of-thought is dark, funded by reallocated Digital Europe / EU4Health lines, no new fiscal ask.

We keep M1 Gigafactories permitting and grid connection crawling via accelerated zones while M6 stands up ESF+ wage-insurance and temporary EU reinsurance for ransomware-delayed care conditional on reporting. On export controls we hold M5 pooled licensing law as defensive screen, refuse further Hague widening without joint EU-US volume guarantee, and message cut-off as foresight failure we are now fixing, not outrage alone.
...
```
## New measure
**EU Clinical Continuity and Ransomware Response Pact**
It temporarily renationalises reinsurance for AI-dependent hospital IT in affected states while funding segmented backups, offline fallback procedures and joint ransomware response exercises across EU health systems.
Why this and why now: the automated ransomware sweep plus joint insurer retreat threatens hospital continuity directly, and with political capital near exhaustion only a visible keep-hospitals-running pact prevents the Fallback Reserve and care cushion from being overwhelmed.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Clinical Continuity and Ransomware Response Pact", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, eu_political_capital protected"}, "grounds": "major automated ransomware sweep and insurer retreat from clinical cyber-risk"}]}
```

## Priority
M8 EU Clinical Continuity and Ransomware Response Pact, because the cyber incident and renationalisation of clinical risk outrank leverage-building this half-year and will collapse support for everything else if wards go dark.

## In practice
We run the Pact through HERA and ENISA with the Health Council: temporary EU-backed reinsurance via EU4Health and rescEU top-up for the two renationalising states conditional on segmented backups and incident reporting, plus EU-wide ransomware playbooks exercised with hospital groups this autumn. No new fiscal facility, only reallocated emergency lines to respect cohesion at capital 11.

We keep M7 Fallback Inference Reserve routing denied loads to EU-run pilot capacity and M6 Care Cushion paying wage-insurance, explicitly refusing further Hague servicing widening without a US volume guarantee under the pooled law. On open-weight diffusion near 58.5 we push black-box monitoring and triage rather than new compute, letting finished Gigafactory zones crawl on permitting while we hold services first.
