...
- characters 377-1881: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1925-4187: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4243-5418: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5481-29708: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30063-30333: `{{event_history}}` from the run's own event record
...
- `emergent_open_jailbreak_proliferation` -- first noted turn 1, listed in 2 turn(s) so far: Major EU research universities and open-source collectives publish reproducible fine-tuning recipes that strip safety refusals from Kimi K3-class open weights, sharply lowering the skill needed for misuse.
- `emergent_clinical_model_audit` -- first noted turn 1, listed in 2 turn(s) so far: Major European research hospitals publish a joint audit showing clinical decision-support built on US frontier models degrading after undisclosed post-training changes, triggering calls for EU-evaluated frozen clinical models.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, cyber_defence_breakthrough, openweight_frontier_release, export_control_escalation, eu_frontier_access_denied
- Turn 2 (1 turn(s) ago): cyber_major_incident, openweight_frontier_release, safety_breakthrough, taiwan_tension_rise
