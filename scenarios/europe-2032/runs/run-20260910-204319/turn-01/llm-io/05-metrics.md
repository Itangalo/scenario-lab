...
- characters 16339-16985: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_open_jailbreak_proliferation` -- first noted turn 1, listed in 1 turn(s) so far: Major EU research universities and open-source collectives publish reproducible fine-tuning recipes that strip safety refusals from Kimi K3-class open weights, sharply lowering the skill needed for misuse.
- `emergent_clinical_model_audit` -- first noted turn 1, listed in 1 turn(s) so far: Major European research hospitals publish a joint audit showing clinical decision-support built on US frontier models degrading after undisclosed post-training changes, triggering calls for EU-evaluated frozen clinical models.
...
## Metrics
```json
{"ai_capability": 54.0, "openweight_capability": 47.0, "ai_safety": 32.0, "resilience": 38.0, "eu_ai_sovereignty": 19.0, "eu_political_capital": 44.0, "public_sentiment": 36.0}
```
...
PROPOSAL BONUS: EU Critical Systems Shield (cat 6, large) +4 — cyber test-shot intrusion landed this turn and this answers it directly
LEGITIMACY LENDS: capital 48, sentiment 42 -> none, capital higher
SOVEREIGNTY: 22 last turn, no capacity measure finishes, no in-flight bonus, eu_frontier_access_denied t1 −2, capability rose 2.0 −1 = 19
