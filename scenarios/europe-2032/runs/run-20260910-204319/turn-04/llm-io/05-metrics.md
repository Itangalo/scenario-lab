...
- characters 105-13422: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13465-13657: `{{metrics_json}}` from the run's live metric values
- characters 13750-15145: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15188-17670: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17726-18464: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M5 | EU Lithography Leverage Shield | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 7}, "grounds": "event: blockade disrupts compute supply"}]}
```
```
