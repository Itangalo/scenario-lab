...
- characters 455-3045: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 3089-5422: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7155-7634: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10771-13229: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential services running on EU-controlled fallback through rupture and rebuild trust ward by ward
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Degraded-Mode Survival and Open-Weights Triage | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, ai_safety up slightly via triage and detection, public_sentiment protected | 2 | running |
...
| M9 | EU Clinical AI Liability Backstop and Waiting-List Recovery Programme | 9 | small | 8 | 10 | own jurisdiction | public_sentiment up, resilience up moderately, eu_political_capital protected | 2 | finished |
...
## New measure
**EU Degraded-Mode Survival and Open-Weights Triage**
Keeps hospitals, municipalities and essential operators running on segmented offline fallbacks and EU-controlled standby inference while triaging the new open frontier release for cyber and bio misuse. This and now because US keys are revoked, venture co-financing is gone, open capability is unrecallable within points of the frontier, and with zero measures in flight this is the only ward-holding line the next commitment can build on.

Category named in prose: Preparedness and resilience (6), with a small labour/social cushion element executed inside it, not as a separate instrument.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Degraded-Mode Survival and Open-Weights Triage", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via triage and detection, public_sentiment protected"}, "grounds": "US cut-off plus open frontier release with no in-flight cover"}]}
```

## Priority
The new EU Degraded-Mode Survival and Open-Weights Triage, because with political capital at 5 and no measures in flight, holding theatres, imaging and municipal services in degraded mode is what outranks any rebuild of sovereignty this turn.

## In practice
We run this through the Health Council and Justice and Home Affairs Council on the M8/M9 legal bases already voted: EU-backed reinsurance for ransomware-linked failure stays conditional on segmented offline backups, paper fallbacks and joint HERA-ENISA drills, and the capped liability backstop stays limited to certified domestic tools on Controllable Core and Fallback Reserve inference with mandatory incident reporting to the AI Office. No new Hague servicing alignment, no separate volume-licence begging, and no backstop or EU4Health top-up for the defecting capital's hyperscaler deal.

ENISA and Europol triage the open-weights release that landed within points of the frontier — shared signatures, patching of imaging and municipal stacks, and targeted monitoring of ransomware crews and bio-misuse pathways — while EU4Health recovery money goes only to wards that can show chest-scans-in-days and falling elective queues on EU standby. What we can offer the sceptical capitals and hospital managers is filmed, counted reversals ward by ward, not promises of new gigawatt capacity.
...
```
## New measure
**EU Social-System Algorithm Audit and Redress**
Orders independent audit, suspension and redress of the unlawful welfare/policing benefits scoring system, with enforced human-oversight and logging standards under the AI Act.
This and now because the ombudsman judgment shows a high-risk Annex III system passed on paper while caseworkers cleared queues at forty seconds an item and logs went unread, and with sentiment at 21 leaving that unanswered makes both adoption and enforcement politically impossible.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Social-System Algorithm Audit and Redress", "category": 1, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up slightly, eu_political_capital protected, ai_safety up slightly"}, "grounds": "automated decision scandal requires enforced redress"}]}
```

## Priority
M10 EU Degraded-Mode Survival and Open-Weights Triage, because theatres kept open through ransomware and rationing will close if degraded cover lapses, and that outranks even the scandal redress this turn.

## In practice
We hold the ward line first. Health and JHA Councils keep the extended EU-backed ransomware cover conditional on segmented offline backups, paper fallbacks and joint HERA-ENISA-Europol drills, with EU4Health recovery funds only for wards showing scans-in-days and the capped liability backstop still limited to certified domestic tools on Controllable Core and Fallback Reserve. ENISA/Europol continue triaging the openly available frontier-class weights now in ransomware phishing kits — shared signatures, radiology-stack patching, bio-misuse monitoring — with no separate US volume-licence deal, no Hague servicing alignment, and no cover for the breakaway hyperscaler capital. Washington has now rationed allies alongside adversaries, so we ration degraded capacity by clinical need.

On the scandal we act through enforcement, not a new Act. The Commission tasks the AI Office with national market-surveillance authorities and the ombudsman to suspend the condemned social-insurance/policing scorer, order case-by-case human re-review and redress, and enforce Annex III conformity, meaningful human oversight and log-review obligations where they were breached on paper. No new budget line this turn; redress is funded from existing social and recovery envelopes while M10 absorbs the political cost.
