...
- characters 455-2209: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2253-4765: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6395-6915: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9170-10570: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M6 | EU Upstream Leverage Coalition | 8 | small | 5 | 8 | coalition | eu_ai_sovereignty up moderately, eu_political_capital up slightly via leverage | 2 | running |
...
## New measure
None this turn, while we hold capital for the two measures already burning and wait for the Gigafactory line to reach its finishing turn.

## Store changes
No changes.

## Priority
M6 EU Upstream Leverage Coalition, because Washington is now using our one chokepoint against us and a member state is breaking the line — if joint maintenance-for-tranches terms do not hold this turn there is no independent footing left to harden.

## In practice
We put DG TRADE and the Anti-Coercion Instrument in the lead with the Netherlands, using the Foreign Affairs Council (Trade) to block further ASML servicing cuts beyond what US jurisdiction compels, pooling maintenance, spares and chemicals offers to Japan and Korea in exchange for prioritised accelerator tranches and shared stockpiles. The defecting member state's side-deal is contained by making stockpile access conditional on no undercutting, not by public sanction we cannot afford.

We run M1 on care-and-maintenance: InvestEU/EIB guarantees stay on insurance premia and pre-bought stockpile, permits and grid kept warm, no new ground broken until supply is assured. ENISA's joint centre and the finished Shield/Municipal Fund shift to maintain-only — offline-backup drills and hardening guidance that keep EU-conditional funding as the de facto condition for municipal cyber-insurance renewal after the health-insurer repricing.
