...
- characters 106-12983: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13026-13218: `{{metrics_json}}` from the run's live metric values
- characters 13334-16283: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 16339-16980: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_clinical_ai_moratorium` -- first noted turn 1, listed in 1 turn(s) so far: Major European research hospitals publish a joint moratorium on clinical use of frontier AI diagnostics after a faulty open-model deployment causes mistriage, triggering EU-wide calls for medical AI certification.
- `emergent_research_integrity_crackdown` -- first noted turn 1, listed in 1 turn(s) so far: Major EU scientific publishers and universities jointly mandate disclosure and watermarking of AI-assisted research after the Millennium Prize controversy, triggering a high-profile dispute over reproducibility.
...
PROPOSAL BONUS: EU Critical Systems Shield (cat 6, large) +3 — cyber_test_shot precursor landed this turn and this answers it directly
LEGITIMACY LENDS: capital 43, sentiment 41 -> none (capital higher)
SOVEREIGNTY: 22 last turn, no finishing measure, Gigafactories/Tech package in flight +0, eu_access_secured t1 +2, capability rose 2.0 −1 = 23
