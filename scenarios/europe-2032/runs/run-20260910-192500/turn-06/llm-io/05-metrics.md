...
- characters 13413-13605: `{{metrics_json}}` from the run's live metric values
- characters 13698-14849: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14892-17063: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17119-17577: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Upstream Leverage Coalition | 2 |
...
**emergent_cyber_insurance_repricing (emergent event):** Major European health insurers quietly reprice municipal cyber risk after the autumn sweep, making EU-conditional restoration funding a de facto requirement for coverage renewal.
...
## Metrics
```json
{"ai_capability": 61.8, "openweight_capability": 58.0, "ai_safety": 23.0, "resilience": 44.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 13.0, "public_sentiment": 26.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election won by backlash candidate, retrenchment takes effect turn 6"}]}
```
```
