...
- characters 455-1852: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1896-4005: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4897-5373: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7327-9192: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Evaluation Access Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via evaluation leverage | 2 | running |
...
## New measure
**EU Evaluation Access Cell**
Stands up a joint JRC-ENISA-EU AI Office cell with legal demand for pre-deployment evaluation access and weight-security checks after the leaked eval anomaly.
This is category 5 Public technical capacity and research, and why now is the anomaly plus the major automated attack together: assurance is slipping while agents run longer, and without own testing access the Union learns about capability only after deployment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Evaluation Access Cell", "category": 5, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, eu_ai_sovereignty up slightly via evaluation leverage"}, "grounds": "eval anomaly demands independent testing capacity"}]}
```

## Priority
M3 EU Critical Systems Shield, because the large automated attack landing on pre-positioned infrastructure outranks longer-build work and the new Cell needs the Shield's incident data to learn.

## In practice
We keep the Shield as the operational centre: ENISA joint operations with TSO regulators move from audits of the two compromised operators to EU-funded sensors, offline credential restoration and exercised cross-border isolation drills, using NIS2 crisis base and Shield co-financing to hold operator and capital compliance through the live incident.

In parallel we launch the Evaluation Access Cell without opening a second front: DG CNECT tasks the JRC and AI Office to invoke AI Act evaluation provisions for the anomalous system, demanding structured access and incident-linked model information from providers under the written frontier-access terms, while M1 Gigafactories and M2 Tech sovereignty package continue on permitting zones and private-capital pipeline with no new scope this turn.
...
```
## New measure
**EU Municipal Recovery and Isolation Fund**
Extends the finished Shield into municipalities, logistics and health networks with ring-fenced recovery money tied to offline backups and exercised isolation cuts.
This is Preparedness and resilience work, and why now is the automated ransomware sweep landing on staged credentials while attribution drags: the Shield proved sensors and co-financing hold operators, but thin crews and municipal gaps are where services paid or rebuilt, so the same instrument must reach the layer that was actually hit.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Recovery and Isolation Fund", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "automated sweep exposed municipal and health gap"}]}
```

## Priority
EU Municipal Recovery and Isolation Fund, because the large automated attack on pre-positioned infrastructure outranks longer-build compute and eval work and directly serves the absorb-shocks half of the two-year commitment.

## In practice
We keep ENISA joint operations as the operational centre under NIS2 crisis provisions, directing EU-funded sensors, offline credential restoration and real interconnection cuts first to affected municipalities, logistics firms and the two health networks, with co-financing conditional on offline backups and isolation drills to hold operator and capital compliance despite complaints about Brussels directing grids.

In parallel we hold the Evaluation Access Cell on its track: DG CNECT tasks JRC-AI Office to press structured pre-deployment access demands for the anomalous frontier system under AI Act provisions and written terms, banking summaries now while building leverage for weights, and we answer export-control tightening by keeping M1 Gigafactories and M2 sovereignty package on permitting and private-capital pipeline with no new scope, preserving supply-chain leverage without opening a second front this turn.
