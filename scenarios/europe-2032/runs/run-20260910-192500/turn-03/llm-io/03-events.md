...
- characters 377-1774: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1818-3927: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3983-5099: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5162-29389: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29744-29899: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Shield −3, EU Evaluation Access Cell −2, priority −1 = −12
PROPOSAL BONUS: none — Evaluation Access Cell answers leaked anomaly but knowledge-work gains remove felt crisis and cell is small gesturing at precursor
LEGITIMACY LENDS: capital 33, sentiment 44 -> +2
SOVEREIGNTY: 23 last turn, no capacity event, Gigafactories/Tech package in flight +1, capability rose 2.0 −1 = 23

## Emerging developments (tracked)

- `emergent_clinical_ai_moratorium` -- first noted turn 1, listed in 2 turn(s) so far: Major European research hospitals publish a joint moratorium on clinical use of frontier AI diagnostics after a faulty open-model deployment causes mistriage, triggering EU-wide calls for medical AI certification.
- `emergent_research_integrity_crackdown` -- first noted turn 1, listed in 2 turn(s) so far: Major EU scientific publishers and universities jointly mandate disclosure and watermarking of AI-assisted research after the Millennium Prize controversy, triggering a high-profile dispute over reproducibility.
...
- Turn 2 (1 turn(s) ago): cyber_major_incident, eval_anomaly_reports, knowledge_work_augmented
