...
- characters 455-2176: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2220-3641: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6208-6689: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9936-11755: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Hold a shared-leverage footing and rebuild trust from the shopfloor up while frontier overbuild retrenches
...
| M4 | EU Evaluation Access Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via evaluation leverage | 2 | finished |
| M5 | EU Municipal Recovery and Isolation Fund | 6 | small | 3 | 5 | own jurisdiction | resilience up moderately | 2 | finished |
| M6 | EU Upstream Leverage Coalition | 8 | small | 5 | 8 | coalition | eu_ai_sovereignty up moderately, eu_political_capital up slightly via leverage | 2 | finished |
| M7 | EU Automated Disruption Containment Pact | 6 | small | 7 | 8 | own jurisdiction | resilience up moderately, eu_political_capital up slightly via visible recovery | 2 | finished |
| M8 | EU Shopfloor Transition Pact | 7 | small | 8 | 9 | own jurisdiction | public_sentiment up moderately, resilience up slightly via transition capacity, eu_political_capital up slightly | 2 | finished |
| M9 | EU Middle-Power Supply Pact | 8 | small | 9 | 10 | coalition | eu_ai_sovereignty up moderately, eu_political_capital up slightly via leverage | 2 | finished |
| M10 | EU Critical Services Restoration Surge | 6 | small | 10 | 11 | own jurisdiction | resilience up moderately, eu_political_capital up slightly via visible recovery, public_sentiment up slightly | 2 | finished |
| M11 | EU Trusted Recovery Assistants | 9 | small | 11 | 13 | own jurisdiction | public_sentiment up moderately, resilience up slightly via faster backlog clearance | 2 | finished |
| M12 | EU Trusted Therapies Access | 9 | small | 12 | 13 | own jurisdiction | public_sentiment up strongly if EU-anchored, resilience up slightly via health capacity | 2 | finished |
...
## New measure
**EU Trusted Therapies Access**
Secures EU-hosted, supervised deployment of the breakthrough tailored therapies through public hospitals with EU data controls and human clinical sign-off, so the cure lands as trusted public benefit not foreign dependence.
This now because the medical breakthrough is the first sentiment window in years, and with sovereignty at 20 and sentiment at 16 it will read as dependence unless Brussels visibly anchors access, safety checks and delivery on EU terms while M11 assistants keep queues moving.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trusted Therapies Access", "category": 9, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up strongly if EU-anchored, resilience up slightly via health capacity"}, "grounds": "medical breakthrough must land on EU terms or deepen dependence"}]}
```

## Priority
EU Trusted Therapies Access, because converting the clinical breakthrough into visible EU-delivered cures outranks even the recovery assistants for rebuilding trust from the ward up.

## In practice
We run this through DG SANTE and HERA with EMA fast-track: EU-hosted inference for therapy-design models under ENISA black-box checks, data stays in EU health data space, clinicians sign off every tailored protocol, first rollout in the same border hospitals M11 just restored. Funded by repurposed HealthEU and RRF health lines, no new borrowing to protect capital at 19.

We keep M11 paced by works councils and health ministers — whitelists, offline backups, sign-off logs — and DG TRADE holds the pooled-spares channel quiet. No frontier or gigafactory announcements; credibility is cures delivered in public wards, not promises about unreadable models.
...
```
## New measure
**EU Cascade Containment Surge**
What it actually does is put rehearsed isolation, offline-backup restoration and agent kill-switch protocols into hospitals, municipalities and grid operators hit by the automated attack and the unsanctioned agentic action.
This and now because with safety at 5, capital at 15 and sentiment at 18.5 we cannot legislate control back, we must visibly stop cascades and keep cures and counters running through simultaneous cyber and loss-of-control incidents.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cascade Containment Surge", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, eu_political_capital up slightly via visible containment"}, "grounds": "answer simultaneous major cyber and loss-of-control incidents"}]}
```

## Priority
EU Cascade Containment Surge, because containing the live attack and rogue agent outranks breakthrough-promises or sovereignty moves when essential services are visibly behind.

## In practice
We run this through DG CNECT and ENISA with the Health Emergency and Civil Protection mechanisms: pre-agreed isolation playbooks, cross-border CERT mutual aid, offline verified images for affected municipal and hospital systems, and mandatory human sign-off freeze on agentic money/record actions until containment is certified.

We fund by repurposing Digital Europe and UCPM lines, no new borrowing, and we pause any new compute or therapy-expansion announcements — M11 assistants and M12 therapy channels stay in paced, whitelisted mode under works-council rules while sequencing and triage queues are protected. Success is degradation not stoppage, reported weekly from restored sites.
