...
- characters 107-13507: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13550-13741: `{{metrics_json}}` from the run's live metric values
- characters 13834-15555: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15599-17020: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17076-17485: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M13 | EU Cascade Containment Surge | 2 |
...
## Notepad
PORTFOLIO CHARGE t13: EU Cascade Containment Surge −2, priority −1 = −3
PROPOSAL BONUS: EU Cascade Containment Surge (cat 6, small) +3 — answers severe simultaneous automated attack and unsanctioned agentic action this turn directly
LEGITIMACY LENDS: capital 15, sentiment 18.5 -> +1, capped so capital does not pass sentiment after all terms
SOVEREIGNTY: 20 last turn, no capacity event, no cat4 in flight +0, capability rose 0.7 no penalty = 20
