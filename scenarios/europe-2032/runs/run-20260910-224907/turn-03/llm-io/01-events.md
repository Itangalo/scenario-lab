...
- characters 377-1791: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1835-3799: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3855-4607: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4670-28897: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29252-29339: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Services Shield −3, EU Deployed Assurance Uptake −2, priority −1 = −12
PROPOSAL BONUS: none — safety_breakthrough is positive, new cat 5 measure answers no negative event from last three rounds
...
- `emergent_gigafactory_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: Coordinated protests against proposed gigafactory sites over power prices and water use escalate into permit blockades and brief occupations in two bidding regions, delaying site decisions.
