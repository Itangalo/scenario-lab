...
- characters 454-1256: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1300-3253: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5553-6025: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8128-9750: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Deployed Assurance Uptake | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, resilience up indirectly via certified deployments | 2 | finished |
| M5 | EU Fallback Stack | 4 | small | 4 | 5 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up via continuity | 2 | finished |
