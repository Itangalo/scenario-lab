...
- characters 106-13580: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13623-13814: `{{metrics_json}}` from the run's live metric values
- characters 13907-14709: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14753-16706: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16762-17889: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_permit_injunction_freeze` -- first noted turn 4, listed in 2 turn(s) so far: A national administrative court grants an interim injunction freezing grid-connection permits for two contested gigafactory sites after farmer and municipal challenges, pushing final investment decisions past mid-2028.
- `emergent_drought_power_squeeze` -- first noted turn 4, listed in 2 turn(s) so far: A dry winter and spring cut hydro output and constrain cooling water for thermal and nuclear plants in southern Europe, spiking power prices and hardening local opposition to new large grid connections.
