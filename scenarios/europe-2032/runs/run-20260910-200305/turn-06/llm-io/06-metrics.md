...
- characters 105-13594: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13637-13828: `{{metrics_json}}` from the run's live metric values
- characters 13921-15692: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15735-18470: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18526-19108: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election won on holding advanced AI as strategic national asset; administration takes office turn 6"}]}
```
```
