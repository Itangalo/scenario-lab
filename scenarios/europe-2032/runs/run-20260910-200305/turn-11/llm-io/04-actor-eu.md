...
- characters 455-1433: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1477-3591: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6313-6785: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9925-12317: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Hold essential services running on controlled European capacity while containing open-weight harm
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | Recognised Periphery Mutual-Aid Interoperability | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, public_sentiment up slightly via kept-open periphery beds | 2 | running |
...
| M3 | EU Swarm Defence Hardening | 6 | small | 1 | 3 | own jurisdiction | resilience up moderately, ai_safety up slightly via detection | 2 | finished |
| M4 | EU Essential Services Continuity Reserve | 6 | small | 2 | 4 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly via fallback use | 2 | finished |
| M5 | Municipal and Hospital Cyber Shield | 6 | large | 3 | 7 | own jurisdiction | resilience up substantially | 3 | finished |
| M6 | High-Risk Enforcement Audit | 1 | small | 4 | 6 | own jurisdiction | ai_safety up slightly, public_sentiment up slightly via restored trust | 2 | finished |
| M7 | EU Bio-Cyber Incident Containment Reserve | 6 | small | 5 | 7 | own jurisdiction | resilience up moderately | 2 | finished |
| M8 | Certified Care Continuity Corps | 9 | small | 7 | 8 | own jurisdiction | public_sentiment up moderately, resilience up slightly via reopened capacity | 2 | finished |
| M9 | Periphery Care Audit and Extension Pact | 9 | small | 8 | 9 | own jurisdiction | public_sentiment up moderately, resilience up slightly via reopened periphery capacity | 2 | finished |
| M10 | Locally Stationed Care Continuity Detachments | 9 | small | 9 | 11 | own jurisdiction | public_sentiment up moderately, resilience up slightly via staffed periphery reopening | 2 | finished |
...
## New measure
**Recognised Periphery Mutual-Aid Interoperability**
Puts EU money, liability cover and ring-fenced Spanish-French inference behind the emergent periphery hospital-utility mutual-aid compact instead of competing with it, standardising its segmented kits and on-call technician pooling under DG SANTE/ENISA certification.
This and now because the compact is the only staffed capacity actually present after detachments arrived late and understaffed, and with US frontier access cut off and export quotas tightened, Brussels cannot restore fly-in patching — it must legalise and supply what periphery built itself or lose all leverage over essential-services continuity.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Recognised Periphery Mutual-Aid Interoperability", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up slightly via kept-open periphery beds"}, "grounds": "periphery compact formalisation plus frontier access cut-off and export tightening"}]}
```

## Priority
Locally Stationed Care Continuity Detachments, because without converting the last surge rosters into the permanent postings the mutual-aid compact demands to interoperate with, the new recognition has no EU staff or kits to attach to and refusal hardens further.

## In practice
We execute M10 to its finish by reassigning remaining HERA surge and ENISA health teams as permanent detachments co-located with compact technician pools, funded from the capped EU4Health backstop and hyperscaler in-kind inference, with Health Council legal cover for cross-border pooling. DG SANTE certifies compact segmented kits as equivalent to EU kits where logs are EU-hosted, defusing log-ownership lawsuits.

We answer the access denial by ring-fencing all remaining Spain-France pilot inference for triage and utility-linked clinics, telling Washington we will not bid for quota restoration for those workloads, and using the medical-breakthrough and office-productivity news to justify keeping EU-hosted models in care even where less capable — continuity over frontier performance, publicly. Export-control rationing is managed via the Anti-Coercion Instrument consultation, not retaliation we cannot afford at 8 capital.
...
## New measure
None this turn.
We are holding to the single running recognition line while political capital sits at 4 and a major automated attack is still being triaged, waiting for M11 to finish and for capital to recover before opening another front.

## Store changes
No changes.

## Priority
Recognised Periphery Mutual-Aid Interoperability, because with US clinical licences suspended, a major model-generated cyber incident landing, and eval anomalies opening the control gate, the only staffed continuity left is the periphery compact's pooled servers and technicians that must stay interoperable and logged.

## In practice
We keep DG SANTE and ENISA on certification of compact segmented kits as EU-equivalent where logs are EU-hosted, using the liability cover and co-located surge teams to unblock joint sites and defuse log-ownership suits, and we hold the ring-fence of Spanish-French pilot inference for triage and utility-linked clinics — continuity over performance, no bid for Washington restoration.

We answer the ransomware/dependency sweep through the finished Cyber Shield and Bio-Cyber Reserve playbooks plus the compact's technician pools: isolate affected municipal and hospital systems, fail over to logged EU-hosted models, and exercise cross-border mutual aid rather than standing up a new instrument we cannot fund. Labour displacement at the checkable-output edge and embodied-AI pressure on logistics are monitored for the retraining case, but no new transition fund is opened this turn.
