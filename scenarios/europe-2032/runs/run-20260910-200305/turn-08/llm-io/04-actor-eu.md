...
- characters 454-3287: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 3330-4828: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5165-5671: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8067-9749: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_periphery_care_split (emergent event):** Periphery hospital federations announce coordinated refusal to rejoin EU triage system until liability and logging audits conclude, leaving certified continuity corps operating only in capitals.
...
## New measure
**Certified Care Continuity Corps**
Puts certified, EU-hosted triage and admin AI back into wards under human sign-off to reopen beds and show lawful AI working.
This is the trust-rebuild commitment made concrete now because wards partly closed and fraud-scoring ban have collapsed legitimacy to 14, and with capital at 5 only a small, visible restoration can precede any sovereignty build.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Certified Care Continuity Corps", "category": 9, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up slightly via reopened capacity"}, "grounds": "wards partly closed after certification suspensions"}]}
```

## Priority
Certified Care Continuity Corps, because without reopening wards under certified lawful use the two-year commitment to rebuild trust fails and no further measure is fundable.

## In practice
We task the AI Office with HERA and ENISA to run joint certification sprints for EU-hosted open models for triage support and admin, using Article 168 health legal base and existing EU4Health envelope — no new Council vote. Hospitals in Paris, Berlin, Warsaw that already switched get certified first, then periphery sites with surge-team presence.

We implement the CJEU ban fully while ring-fencing care: fraud-scoring stays suspended with audit by EDPS, compensation via member-state social funds, and triage fallbacks only return with logged human clinician sign-off and dosage guardrails. DG SANTE negotiates with hospital federations to lift suspensions site-by-site to end lawsuits.
...
```
## New measure
**Periphery Care Audit and Extension Pact**
Puts independent logging and liability audits into periphery hospitals and extends certified continuity teams there under joint EU-federation oversight.
This and now because periphery federations refuse to rejoin the EU triage system, and without settling liability and delivering surge teams outside capitals the trust-rebuild commitment collapses into a capitals-only corps.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Periphery Care Audit and Extension Pact", "category": 9, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up slightly via reopened periphery capacity"}, "grounds": "periphery federations refuse rejoining triage system"}]}
```

## Priority
Periphery Care Audit and Extension Pact, because the certified corps finished but operates only in capitals and the two-year commitment to keep essential services running fails if periphery wards stay closed.

## In practice
We task DG SANTE with the AI Office, EDPS and HERA to run fast-track logging audits on EU-hosted open triage models, publish dosage guardrails and unread-log fixes, and create a capped EU liability backstop via EU4Health and member-state social funds to settle compensation claims for benefit cuts and dosage errors. Hospital federations get co-signed audit protocol and site-by-site recertification timetable in Health Council.

We redeploy the joint bio-cyber reserve surge rosters and ENISA health teams from large-city shielding to periphery clinics and small utilities-linked hospitals, installing segmented EU-hosted triage with logged clinician sign-off. Washington quota rationing is managed by prioritizing periphery allocations under the hyperscaler no-break pledge and Spanish-French gigafactory pilot capacity, to show capacity Europe controls reaching beyond Paris, Berlin, Warsaw.
