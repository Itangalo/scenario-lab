...
- characters 454-1524: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1567-3931: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6740-7194: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8655-11096: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_datacentre_blockade (emergent event):** A coordinated physical protest movement blocks construction of two hyperscale data centre sites in Spain and Germany over water and power use, delaying commissioning by months and forcing a national permitting review.
...
| M3 | EU Swarm Defence Hardening | 6 | small | 1 | 3 | own jurisdiction | resilience up moderately, ai_safety up slightly via detection | 2 | running |
...
## Two-year commitment
A Europe that can absorb AI-enabled shocks on infrastructure it controls

## Statement changes
add `two_year_commitment` (commitment): Build a Europe that can absorb AI-enabled shocks on infrastructure it controls.

## New measure
**EU Swarm Defence Hardening**
Hardens electricity transmission, ports and water utilities against AI-swarm intrusion with mandatory segmentation, staged-tooling detection and exercised cross-border response.
This is category 6 Preparedness and resilience, and it must come now because the autumn test shot proved segmentation failed and opened a three-turn cyber gate while defensive tooling has just made defence catch-up plausible.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Swarm Defence Hardening", "category": 6, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via detection"}, "grounds": "answer test-shot precursor before gate closes"}]}
```

## Priority
EU Swarm Defence Hardening, because an unactuated swarm with EU footholds outranks the slower sovereignty build when the next three turns carry elevated cyber risk.

## In practice
We task ENISA with a binding emergency hardening standard under the NIS2 legal base, funded by reprogrammed Digital Europe and Connecting Europe Facility money, with TSOs, port authorities and water operators in Council Energy and Transport formations required to segment OT, rotate breaker credentials and deploy swarm-behaviour detection piloted from the breakthrough tooling. France, Germany and Poland lead the first exercise in December; laggard municipalities get EU co-funding tied to compliance.

On the inherited lines, we keep InvestAI Gigafactories and the Tech sovereignty package running but slow-burn them: DG CNECT shepherds permitting zones and grid connections, we defend ASML autonomy quietly against US export pressure without a public break, and we use the AI Act postponement window to prepare 2027 high-risk enforcement rather than reopening the text.

We link the two tracks politically: resilience spending is sold as sovereignty protection — no credible EU cloud without survivable grids — to hold member-state cohesion and blunt industry claims that security diverts from competitiveness, while productivity gains without job losses are used to keep public sentiment neutral.
...
Keeps hospitals, ministries and operators running when the leading foreign model is cut off by switching them to EU-controlled fallback models and manual procedures under a temporary continuity mandate.
This is Preparedness and resilience work, category 6, and it must come now because the access cutoff plus the major automated attack together prove dependence is an active shock vector under our two-year commitment to absorb shocks on infrastructure we control.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Continuity Reserve", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, eu_ai_sovereignty up slightly via fallback use"}, "grounds": "answer frontier cutoff and cyber incident continuity gap"}]}
```

## Priority
EU Essential Services Continuity Reserve, because restoring cut-off hospitals and ministries outranks even swarm hardening when denial is already biting.

## In practice
We activate the Union Civil Protection Mechanism and NIS2 Article 11 crisis process via ENISA and the Health Emergency Preparedness Authority, funded by reprogrammed Digital Europe money, to inventory all public-sector deployments built on the denied model and order fallback to EU-hosted open models and pre-AI procedures within weeks. France, Germany and Poland health ministries lead the swap; laggard providers get emergency procurement waivers tied to EU-hosting.

On the inherited lines we keep EU Swarm Defence Hardening running hard — segmentation orders and December-exercise lessons now extended to ransomware-hit public services with behaviour-detection — while we slow-burn InvestAI Gigafactories and the Tech sovereignty package: DG CNECT mediates the Spain/Germany blockade with water-use concessions and accelerated grid review, and we contain the member-state side-deal quietly in COREPER to avoid a public split. The benefits-scandal we frame as enforcement failure, not Act failure, opening infringement screening to protect legitimacy.
