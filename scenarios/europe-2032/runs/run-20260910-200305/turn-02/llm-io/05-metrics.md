...
- characters 105-13037: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13080-13272: `{{metrics_json}}` from the run's live metric values
- characters 13365-14435: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14478-16842: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16898-17742: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_biosecurity_publishing_freeze` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher and university consortium imposes a coordinated disclosure moratorium on AI-designed pathogen methods after the phage results, prompting a fight over preprints and open science rules.
