...
- characters 107-12932: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12975-13166: `{{metrics_json}}` from the run's live metric values
- characters 13259-15678: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15722-17537: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17593-18683: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_open_log_privacy_breach` -- first noted turn 13, listed in 1 turn(s) so far: Open dosing and quota logs from the suspended oncology pathway are scraped and published with patient re-identification, triggering a cross-border privacy and clinical-safety investigation beyond the Rotterdam inquiry.
- `emergent_oncology_staff_walkout` -- first noted turn 13, listed in 1 turn(s) so far: Coordinated walkouts by oncology nursing staff in two member states over dual-queue triage and unsigned AI-planned regimens force temporary ward closures, independent of any EU decision.
...
## Notepad
PORTFOLIO CHARGE t13: M11 EU Logged-Workload Clinical Safety Reset −2, priority −0 (capital below 20, no effect, no cost) = −2
