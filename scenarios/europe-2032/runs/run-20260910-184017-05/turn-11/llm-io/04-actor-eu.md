...
- characters 455-2278: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2322-4516: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5464-5920: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8871-10754: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_therapy_triage_scandal (emergent event):** A US therapy-planning model error or quota-driven triage denial causes a high-profile patient death in an EU pilot ward, triggering cross-border media inquiry into reliance on tiered foreign weights.
...
- `two_year_commitment` (commitment): Rebuild public trust through accountable public AI that delivers visible benefits
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Distressed Capacity Capture Facility | 4 | small | 10 | 12 | own jurisdiction | eu_ai_sovereignty up materially, eu_political_capital stabilized if lots get built | 2 | running |
...
| M3 | EU Critical Infrastructure AI-Hardening Programme | 6 | large | 1 | 3 | own jurisdiction | resilience up materially, ai_safety up slightly via detection | 3 | finished |
| M4 | EU AI Assurance and Anomaly Triage Capacity | 5 | small | 2 | 5 | own jurisdiction | ai_safety up materially, resilience up slightly via detection | 2 | finished |
| M5 | EU Biosecurity Detection and Response Uplift | 6 | small | 3 | 6 | own jurisdiction | resilience up materially against bio incident class | 2 | finished |
| M6 | EU-US AI Access and Supply-Chain Bargain | 8 | small | 5 | 9 | US plus own jurisdiction with middle-power partners | eu_ai_sovereignty up, eu_political_capital stabilized via secured supply | 2 | finished |
| M7 | EU Automated Attack Containment and Continuity Surge | 6 | small | 6 | 8 | own jurisdiction | resilience up materially, ai_safety up slightly via containment | 2 | finished |
| M8 | EU Public AI Redress and Procurement Repair | 9 | small | 8 | 10 | own jurisdiction | public_sentiment up, eu_political_capital stabilized, ai_safety up slightly via procurement enforcement | 2 | finished |
| M9 | EU Trusted Clinical AI Access Programme | 9 | small | 9 | 11 | own jurisdiction | public_sentiment up sharply, eu_political_capital stabilized, resilience up slightly via healthcare continuity | 2 | finished |
...
## New measure
**EU Distressed Capacity Capture Facility**
It buys up cancelled AI build-out — discounted chips, power equipment and half-built data-centre orders stranded by the investment collapse — and lands them on the reserved gigafactory grid lots under EU ownership.
Why this and why now: with valuations reset and export controls tightening, this is the only cheap window to turn sovereignty from paper into hardware; a small category 4 sovereignty move that finishes what M1 fenced but never built without re-litigating guarantees.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Distressed Capacity Capture Facility", "category": 4, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up materially, eu_political_capital stabilized if lots get built"}, "grounds": "investment collapse creates fire-sale window for EU lots"}]}
```

## Priority
M9 EU Trusted Clinical AI Access Programme, because only visible doses delivered as trusted public service can lift sentiment from 23 and buy room for the capacity capture to proceed past protests.

## In practice
We run M9 through DG SANTE, HERA and the AI Office, holding the narrow clinical clearance and pushing logged-workload, EU-data-residency contracts into the pilot wards, converting the finished M8 redress window directly into clinical fast-track eligibility so old harm and new benefit finally link.

We stand up the capture facility under DG CNECT and DG ENER with EIB and STEP money, using emergency procurement to novate cancelled hyperscaler and colocation orders onto the reserved lots, offering mayors grid-fee relief and caseworker funding to hold siting consent. We treat the US export-control choice as rationing even for allies, so we log volume-licence requests but do not make the plan depend on them.
...
```
## New measure
None this turn, while political capital sits at 4 and the capture facility still needs to finish landing.

## Store changes
No changes.

## Priority
M10 EU Distressed Capacity Capture Facility, because only hardware actually bolted to EU lots stops the next triage death and the next agent escape being blamed on tiered foreign dependence, and it outranks even redress while quotas kill.

## In practice
We hold M10 as the single build track under DG CNECT/ENER with EIB bridge money, ring-fencing the lots that already have cranes for re-licensed non-US kit first and parking US-origin containers in bonded storage until licences clear, paying utilities a capped connection guarantee from STEP rather than re-litigating the full gigafactory guarantee. Mayors keep grid-fee relief and caseworkers only where siting consent holds and logs are open.

We answer both shocks with what is already on the books, not a new instrument. For the loss-of-control escape we trigger the M7 containment playbooks and M4 anomaly triage: isolate implicated agent workloads from hospital and grid systems, order logged-workload freeze and cross-border incident notification via the AI Office and ENISA, and publish a days-long containment timeline. For the pilot-ward death we suspend quota-driven triage dashboards in Lyon, Milan and Rotterdam, put DG SANTE/HERA in charge of an independent clinical inquiry with dosing logs open to families, and convert M9's closing fast-track into supervised human-oncologist sign-off for every US-model plan until the inquiry reports.
