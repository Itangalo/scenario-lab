...
- characters 455-1829: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1873-4061: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5133-5588: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7916-10082: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M6 | EU-US AI Access and Supply-Chain Bargain | 8 | small | 5 | 9 | US plus own jurisdiction with middle-power partners | eu_ai_sovereignty up, eu_political_capital stabilized via secured supply | 2 | running |
| M7 | EU Automated Attack Containment and Continuity Surge | 6 | small | 6 | 8 | own jurisdiction | resilience up materially, ai_safety up slightly via containment | 2 | running |
...
## New measure
**EU Automated Attack Containment and Continuity Surge**
Extends mandatory segmentation, clean-rebuild capacity and exercised continuity across municipalities, hospitals and telecoms plus rapid closure of sequencing/diagnostics stocks to absorb the model-generated ransomware wave and the genome-design uplift signal.
This and now because the large automated incident landing while open weights sit at 57.8 and a viable human-infecting design is contested demands absorption before attribution, and the US-China risk deal reached over our heads makes domestic resilience the only leverage we control.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Automated Attack Containment and Continuity Surge", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, ai_safety up slightly via containment"}, "grounds": "major automated cyber incident plus bio uplift precursor this turn"}]}
```

## Priority
EU Automated Attack Containment and Continuity Surge, because the ransomware sweep degrading public services in public view will collapse the capital needed for both gigafactories and the Washington bargain if containment visibly fails this turn.

## In practice
We keep M1 on grid-connection of the first two lots via EIB guarantees and DG CNECT siting pressure, and keep the M6 bargain track offering export-control alignment for tier-1 access, but both run without new push while forensics consume EuroHPC time.

We surge through ENISA and the finished triage cell: mandatory isolation and clean-rebuild playbooks enforced via the NIS2 implementing act, HERA/ECDC funds re-sequenced to close sequencing nodes and diagnostics stocks early in this semester, and Health and Telecoms Council formations pressed to fund municipal IT rebuilds — mayors resisting reporting burdens are offered EU recovery funding tied to segmentation.

We note the US-China limited agreement from outside and task DG RTD and the EU evaluator network to align weight-security and bio-tool controls so we can seek late entry, without pausing containment.
...
```
## New measure
None this turn, while political capital at 14 and public sentiment hostile leave no room to fund a further instrument without collapsing what is in flight.

## Store changes
No changes.

## Priority
M7 EU Automated Attack Containment and Continuity Surge, because the defensive breakthrough and the safety result only buy back capital if containment visibly holds through the ransomware rebuilds and the automated-decision scandal.

## In practice
We push M7 through ENISA and the NIS2 implementing act, rolling the new automated-patching and swarm-detection tooling into the mandatory segmentation and clean-rebuild playbooks for municipalities, hospitals and telecom billing, using EU recovery money tied to segmentation to buy mayoral compliance on reporting. HERA/ECDC funds already re-sequenced close the sequencing gaps, now with the certified control property from the interpretability result adopted via the AI Office and Joint Research Centre into triage dashboards.

We keep M6 alive at low burn in the Trade and Competitiveness Council formations, offering export-control alignment for tier-1 US access but logging narrow quotas as given, and we task DG CNECT and EIB to hold gigafactory grid lots without new push while EuroHPC time is shared with forensics. On the automated-decision scandal we frame it as an AI Act enforcement failure, not a design flaw: the AI Office with the national ombudsman orders audit, redress and procurement pause for the system concerned, to show restriction can work before hostility hardens further.
