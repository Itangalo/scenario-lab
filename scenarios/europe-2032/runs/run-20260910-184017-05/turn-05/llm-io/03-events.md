...
- characters 377-1479: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1523-3872: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3928-4992: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5055-32025: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32380-32679: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, EU AI Assurance and Anomaly Triage Capacity −2, EU Biosecurity Detection and Response Uplift −2, priority −1 = −8
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 22, sentiment 43 -> +2
SOVEREIGNTY: 18 last turn, no capacity event, no finishing measure, in flight +0, capability rose 2.0 −1 = 17

## Emerging developments (tracked)

- `emergent_journal_biosecurity_screen` -- first noted turn 4, listed in 1 turn(s) so far: Major scientific journals jointly adopt mandatory pre-publication biosecurity review for genome models after the contested human-infecting design paper, delaying publication but creating a de facto screening norm outside government regulation.
- `emergent_replication_rebuttal` -- first noted turn 4, listed in 1 turn(s) so far: A peer replication consortium publishes a negative replication of the contested genome-model organism design claim, briefly cooling the bio debate but leaving health ministries split over whether detection uplift should continue.
...
- Turn 1 (4 turn(s) ago): cyber_test_shot, knowledge_work_augmented, export_control_escalation
- Turn 2 (3 turn(s) ago): eval_anomaly_reports, safety_breakthrough, adoption_success
- Turn 3 (2 turn(s) ago): bio_uplift_findings, knowledge_work_augmented
