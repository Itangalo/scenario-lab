...
- characters 376-2007: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2050-4417: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4473-5686: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5749-30838: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 31193-31565: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, EU Biosecurity Detection and Response Uplift −2, EU-US AI Access and Supply-Chain Bargain −2, priority −1 = −8
PROPOSAL BONUS: EU-US AI Access and Supply-Chain Bargain (cat 8, small) +2 — tiered rationing decided this turn and embodied deployment from US models requires negotiated track
LEGITIMACY LENDS: capital 16, sentiment 37 -> +2
SOVEREIGNTY: 17 last turn, no finishing measure, Gigafactories in flight +1, embodied_ai_deployment t5 −2, capability rose 1.0 no charge = 16

## Emerging developments (tracked)

- `emergent_journal_biosecurity_screen` -- first noted turn 4, listed in 2 turn(s) so far: Major scientific journals jointly adopt mandatory pre-publication biosecurity review for genome models after the contested human-infecting design paper, delaying publication but creating a de facto screening norm outside government regulation.
- `emergent_replication_rebuttal` -- first noted turn 4, listed in 2 turn(s) so far: A peer replication consortium publishes a negative replication of the contested genome-model organism design claim, briefly cooling the bio debate but leaving health ministries split over whether detection uplift should continue.
...
- Turn 1 (5 turn(s) ago): cyber_test_shot, knowledge_work_augmented, export_control_escalation
- Turn 2 (4 turn(s) ago): eval_anomaly_reports, safety_breakthrough, adoption_success
- Turn 3 (3 turn(s) ago): bio_uplift_findings, knowledge_work_augmented
- Turn 4 (2 turn(s) ago): cyber_major_incident
- Turn 5 (1 turn(s) ago): embodied_ai_deployment, election_consolidation
