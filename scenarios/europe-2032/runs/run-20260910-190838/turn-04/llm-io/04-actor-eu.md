...
- characters 454-1738: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1781-3542: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3871-4315: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6237-8254: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_genome_paper_leak (emergent event):** Frustrated authors of the disputed genome-methods paper publish the unredacted manuscript on a foreign preprint server, triggering emergency review and proliferation concerns across Europe.
...
## New measure
**EU Patch-and-Drill Completion Facility**
It closes the Shield gaps by funding credential resets, re-audits and repeat black-start drills for the two ports and failing water utility, and buys dedicated EuroHPC queue slices for hospital and essential-service fallback. A category 6 Preparedness and resilience instrument, small and fast, because the commitment to withstand disruption fails on the operators that missed the spring window and capital cannot carry a new large build now.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Patch-and-Drill Completion Facility", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "to close missed drills and fallback queues under Shield"}]}
```

## Priority
M4 EU Patch-and-Drill Completion Facility, because finishing the missed ports, water re-audit and EuroHPC fallback is the two-year commitment made concrete and must outrank stalled gigafactory builds until delivered.

## In practice
We run this through the Energy and Transport Councils with ENISA-led hunt teams retained on Civil Protection Mechanism emergency funds: compulsory re-audit orders under NIS2 for the laggard ports and water utility, with the temporary telecom levy holiday extended only on attendance. DG-CNECT ring-fences EuroHPC slices for essential-service workloads to cut queue times, paid by reprogrammed Digital Europe funds, not fresh cash.

We keep M1 InvestAI Gigafactories and M2 Tech sovereignty package alive but throttled to mapped permitting zones and grid-queue priority, using EIB pre-financing to hold sites and Anti-Coercion screening to shield lithography suppliers, deferring any fresh cash call until capital rebuilds. On welfare-policing and the genome-model paper we stay inside existing AI Act audit and reporting powers, ordering log preservation and human-oversight checks without opening a second instrument this turn.
...
```
## New measure
**EU Genome-Leak Bio Shield Surge**
It activates emergency biosecurity screening, lab reporting, and health-system detection for the leaked genome methods, with takedown and access-control requests on the foreign preprint.
A category 6 Preparedness and resilience instrument, small and fast, because the unredacted proliferation cannot wait for the stalled sovereignty builds and the two-year commitment to withstand disruption now includes biological disruption.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Genome-Leak Bio Shield Surge", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via misuse containment"}, "grounds": "to contain leaked genome methods proliferation"}]}
```

## Priority
M5 EU Genome-Leak Bio Shield Surge, because an unrecallable methods leak outranks holding gigafactory sites and demands the scarce capital now before it diffuses into open-weight workflows.

## In practice
We run this through Health and Home Affairs Councils with HERA and ECDC leading: emergency BWC/Dual-Use screening guidance to synthesis providers and university BSL labs, mandatory incident reporting under NIS2-adjacent health powers, and reprogrammed EU4Health funds for wastewater and clinical sequencing surge. DG-RTD issues preservation and screening orders while the Commission requests the foreign host to restrict access pending review.

We keep M1 InvestAI Gigafactories and M2 Tech sovereignty package on care-and-maintenance — mapped zones, EIB pre-financing to hold land and grid queues, no new cash call — to avoid a second compute programme while the inherited ones are stalled. Shield fallback slices from M4 are retained for hospitals to keep essential services running through any bio-alert load.
