...
- characters 453-1602: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1646-3464: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4682-5159: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7859-9455: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_municipal_grid_pact (emergent event):** Stalled-zone councils coordinate a formal refusal pact linking grid-connection freezes to demands for EU energy-price compensation, turning local siting blocks into a collective bargaining lever.
