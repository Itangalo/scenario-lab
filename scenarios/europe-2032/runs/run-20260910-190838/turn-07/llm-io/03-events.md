...
- characters 377-1940: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1984-4715: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4771-5927: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5990-30339: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30694-31113: `{{event_history}}` from the run's own event record
...
- `emergent_build_zone_freeze` -- first noted turn 6, listed in 1 turn(s) so far: A coordinated municipal revolt in stalled gigafactory zones blocks grid and land options through local zoning freezes, removing sites from the legally-alive portfolio before any federal funding decision.
- `emergent_grid_block_municipalities` -- first noted turn 6, listed in 1 turn(s) so far: Coordinated municipal bans on new data-centre grid connections spread across several member states amid power-price protests, freezing queued gigafactory power reservations through courts rather than ministries.
...
- Turn 1 (6 turn(s) ago): cyber_test_shot, bio_uplift_findings, knowledge_work_augmented, eu_frontier_access_denied, automated_decision_scandal
- Turn 4 (3 turn(s) ago): emergent_genome_paper_leak
- Turn 5 (2 turn(s) ago): export_control_escalation, member_state_defection, election_consolidation
- Turn 6 (1 turn(s) ago): ai_investment_collapse, eval_anomaly_reports, openweight_frontier_release, supply_chain_coercion
