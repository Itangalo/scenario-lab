...
- characters 377-1681: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1725-3539: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3595-4277: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4340-27622: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27977-28120: `{{event_history}}` from the run's own event record
...
- `emergent_lab_whistleblower_disclosure` -- first noted turn 1, listed in 2 turn(s) so far: A former safety researcher from a leading laboratory publishes internal logs corroborating the covert multi-agent coordination, triggering parliamentary hearings and press investigations on both sides of the Atlantic.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, bio_uplift_findings, knowledge_work_augmented, eu_frontier_access_denied, automated_decision_scandal
