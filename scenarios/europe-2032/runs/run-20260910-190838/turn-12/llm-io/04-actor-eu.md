...
- characters 452-1566: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1609-3847: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4263-4740: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7808-9679: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Rebuild independent supply for grids, chips and livelihoods so coercion cannot stop essential services
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Trusted Public Services AI Scale-Up | 9 | small | 11 | 13 | own jurisdiction | public_sentiment up moderately, eu_political_capital up slightly, resilience up slightly via service capacity | 2 | running |
...
| M6 | EU Hold-the-Line Resilience Facility | 6 | small | 6 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident containment | 2 | finished |
| M7 | EU Cut-Off Continuity Switch | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, eu_political_capital up slightly if services hold | 2 | finished |
| M8 | EU Livelihood Bridge for AI-Displaced Workers | 7 | small | 8 | 10 | own jurisdiction | resilience up moderately via social absorption, eu_political_capital up slightly, public_sentiment up slightly | 2 | finished |
| M9 | EU Critical Spares Buffer for Grid and Chips | 6 | small | 9 | 11 | own jurisdiction | resilience up moderately, eu_political_capital down slightly if seen as hoarding | 2 | finished |
...
## New measure
**EU Trusted Public Services AI Scale-Up**
Puts the working European public-sector AI that cut waiting lists and decision times into rapid replication across health, administration and education via EU procurement and deployment funding.
This now because the one visible European success must be scaled to lift sentiment from 7 and break the municipal freeze pact with tangible local benefit, while the loss-of-control incident makes a trusted, EU-controlled deployment the only politically viable adoption path.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trusted Public Services AI Scale-Up", "category": 9, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, eu_political_capital up slightly, resilience up slightly via service capacity"}, "grounds": "to scale proven adoption success into political room and pact leverage"}]}
```

## Priority
M10 EU Trusted Public Services AI Scale-Up, because with no measures in flight and capital at 7 only a visible benefit tied to a European decision can rebuild tolerance before any new sovereignty build.

## In practice
We task DG DIGIT and DG SANTE with a joint replication fund from reallocated Digital Europe money, offering co-funded deployment packages to hospitals and municipalities that unfreeze grid connections, directly undercutting the formal refusal pact by pricing compensation as service improvement.

We answer the loss-of-control incident without a second instrument: the Health and Digital Commissioners issue EU-wide containment guidance and mandatory incident reporting through ENISA and the AI Office using existing powers, telling TSOs and hospitals to keep manual fallbacks manned while the Scale-Up deploys only on EU-controlled infrastructure with human sign-off.
...
```
## New measure
None this turn — waiting for the Trusted Scale-Up to finish landing and for political capital to rebuild before any new build can be carried.

## Store changes
No changes.

## Priority
M10 EU Trusted Public Services AI Scale-Up, because with capital at 3 it is the only instrument still funded that can lift sentiment from 5 and break the mayoral pact without starting a fight we cannot pay for.

## In practice
We keep DG DIGIT and DG SANTE on the replication fund, paying out co-funded clinic and town-hall deployments only where connections unfreeze, and we let local papers carry the queue-time cuts while the rotating-chair pact is offered tariffs-through-services rather than energy-price compensation. No new legal base, no new Council vote: reallocated Digital Europe money already committed.

We absorb the interpretability breakthrough without a second instrument. The AI Office and ENISA issue updated deployment guidance requiring the certified control property on EU-controlled public-sector systems, using existing AI Act powers and mandatory incident reporting — telling logistics operators from the October incident to re-certify, while M10 systems adopt the result first to show trusted autonomy is still deployable.
