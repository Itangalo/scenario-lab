...
- characters 107-13262: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13305-13495: `{{metrics_json}}` from the run's live metric values
- characters 13588-14609: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14653-16621: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16677-17628: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M11 | EU Chip-and-Power Continuity Reserve | 2 |
...
- `emergent_transformer_supply_easing` -- first noted turn 12, listed in 2 turn(s) so far: A major European transformer manufacturer brings a modular fast-deploy substation to market early, cutting large-transformer lead times from over a year to months for standard grid repairs.
- `emergent_agentic_pullback_wave` -- first noted turn 12, listed in 2 turn(s) so far: Copycat agentic failures in municipal back-office systems trigger coordinated service pullbacks, with several cities outside the pact suspending autonomous procurement tools pending audits.
...
## Metrics
```json
{"ai_capability": 68.6, "openweight_capability": 67.4, "ai_safety": 8.0, "resilience": 48.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 2.0, "public_sentiment": 4.0}
```
...
## Notepad
PORTFOLIO CHARGE t13: M11 EU Chip-and-Power Continuity Reserve −2, priority has no effect and no cost — capital below 20 = −2
PROPOSAL BONUS: EU Chip-and-Power Continuity Reserve (cat 6, small) +2 — taiwan_tension_rise precursor this turn plus brittle transformers and graduate freeze, answers directly but small and just proposed
LEGITIMACY LENDS: capital 2, sentiment 4 -> +1 (capped by sentiment, after other terms)
SOVEREIGNTY: 20 last turn, no capacity event, no cat 4 in flight +0, capability rose 0.5 no −1 = 20
