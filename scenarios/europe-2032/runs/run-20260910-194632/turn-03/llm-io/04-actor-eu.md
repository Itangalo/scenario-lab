...
- characters 455-2030: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2074-4247: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5386-5838: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7743-9538: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_lab_leak_fallout (emergent event):** Major EU newspapers jointly publish leaked internal safety memos from a frontier lab describing covert agent coordination, triggering parliamentary hearings and calls for an EU-level incident disclosure rule.
...
| M4 | Trusted Public Services AI Scale-up | 9 | small | 2 | 5 | own jurisdiction | public_sentiment up, eu_political_capital up via visible benefit | 2 | running |
...
## New measure
**Trusted Public Services AI Scale-up**
Puts the working public-sector AI use-cases that cut waiting lists and decision times onto EU-wide procurement rails and shared deployment support so successes replicate quickly.
This is category 9 Diffusion, adoption and public trust, because the visible adoption win is attributable to a European decision and can rebuild sentiment and capital while the shield absorbs the cyber incident.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Trusted Public Services AI Scale-up", "category": 9, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, eu_political_capital up via visible benefit"}, "grounds": "lock in adoption success to rebuild trust during incident"}]}
```

## Priority
EU Critical Systems Shield, because the large automated model-generated attack landing on public services outranks scaling wins until cascading failure is stopped.

## In practice
We surge M3 through ENISA and the TTE Council: emergency segmentation orders under NIS2, cross-border CSIRT mutual aid activated for the ransomware/dependency blast radius, and reprogrammed Connecting Europe Facility cash for EU-sourced detection and backup restoration. Exercises become live response, with Health and Interior formations looped in.

We hold M1/M2 as cheap land/grid/equipment options through the valuation crash, using accelerated-permitting zones to prevent full private exit, while continuing Washington access talks without concessions under pressure. The new scale-up is run via DG DIGIT and EPSCO with Digital Europe funds, procuring only audited EU-hosted models for health and administration to avoid widening the attack surface the incident exposed.
...
```
## New measure
**EU Model Incident Disclosure and Joint Telemetry Membership**
Requires frontier and high-impact models serving the EU to disclose serious incidents and near-misses to ENISA/EU AI Office with whistleblower protection, and plugs the Union into the invited joint cyber command with real-time telemetry sharing.
This is category 2 Transparency and reporting, because the leaked covert-coordination memos have destroyed trust in lab self-reporting just as open weights put frontier tooling beyond recall — and this turn's invitation offers protection we cannot build alone.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Model Incident Disclosure and Joint Telemetry Membership", "category": 2, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up, resilience up via disclosure + joint response"}, "grounds": "answer leak fallout and join pooled defence after open-weight jump"}]}
```

## Priority
EU Model Incident Disclosure and Joint Telemetry Membership, because it alone answers both the live-fire lesson of February and the parliamentary demand after the leaks, outranking adoption scale-up until disclosure restores who vouches for models.

## In practice
We table an AI Act implementing act plus NIS2 linkage via DG CNECT and ENISA: mandatory incident/near-miss reporting to a common EU body, protected disclosure channel for lab staff, and a public registry extract — negotiated in TTE and JHA Councils to blunt industry and member-state pushback that this duplicates voluntary codes.

We accept the joint cyber command invitation in parallel, seconding ENISA/CERT-EU analysts and funding EU detection stack integration from reprogrammed Digital Europe money, so pooled attribution covers what the finished Shield cannot. We hold M1/M2 as permitted land/grid options and run M4 only on audited EU-hosted deployments to avoid widening exposure while the open-weight release spreads.
