...
- characters 13597-14872: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14915-17440: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17496-18024: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Metrics
```json
{"ai_capability": 60.5, "openweight_capability": 57.2, "ai_safety": 23.0, "resilience": 41.0, "eu_ai_sovereignty": 23.0, "eu_political_capital": 6.0, "public_sentiment": 28.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "ALLIANCE"}, "grounds": "turn 5 election winner offered structured allied access; posture takes effect turn 6"}]}
```
```
