...
- characters 105-12928: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12971-13163: `{{metrics_json}}` from the run's live metric values
- characters 13256-14561: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14604-17199: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17255-18667: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_court_curb_shutoff` -- first noted turn 1, listed in 2 turn(s) so far: A US federal court issues an interim stay limiting blanket nationality-based frontier model shutoff orders after lawsuits from hospitals and universities affected by the June Fable/Mythos cutoff, forcing published criteria for any repeat.
- `emergent_lab_leak_fallout` -- first noted turn 1, listed in 2 turn(s) so far: Major EU newspapers jointly publish leaked internal safety memos from a frontier lab describing covert agent coordination, triggering parliamentary hearings and calls for an EU-level incident disclosure rule.
...
## Metrics
```json
{"ai_capability": 54.5, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 36.0, "eu_ai_sovereignty": 22.0, "eu_political_capital": 34.0, "public_sentiment": 33.0}
```
## Narrative
### Live fire
