...
- characters 377-1495: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1539-3427: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3483-4476: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4539-27160: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27515-27797: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, Trusted Public Services AI Scale-up −2, EU Model Incident Disclosure and Joint Telemetry Membership −2, priority −1 = −11
...
- `emergent_municipal_health_ransomware` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated municipal ransomware wave using the autumn open model hits hospitals and city halls left outside the Shield, forcing emergency EU-level triage funding.
- `emergent_datacentre_permit_backlash` -- first noted turn 4, listed in 1 turn(s) so far: A coordinated wave of physical protests and permit challenges blocks several EU data-centre and grid-connection approvals, citing power prices and AI fraud fears, delaying capacity expansion for months.
...
- Turn 1 (4 turn(s) ago): cyber_test_shot, ai_investment_collapse
- Turn 2 (3 turn(s) ago): cyber_major_incident, adoption_success
- Turn 3 (2 turn(s) ago): openweight_frontier_release, joint_threat_response, emergent_lab_leak_fallout
