...
- characters 455-1513: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1557-3654: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6406-6850: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9861-11903: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_energy_price_revolt (emergent event):** A coordinated municipal utility revolt spreads beyond the two frozen data-centre zones, with several city councils imposing punitive power surcharges and moratoria that stall private grid upgrades across the EU.
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Anti-Coercion Supply-Chain Pact | 8 | small | 10 | 12 | coalition | eu_ai_sovereignty up slightly, eu_political_capital down short-term then up if holds | 2 | running |
...
## New measure
**EU Anti-Coercion Supply-Chain Pact**
This is a small International coordination and leverage response that triggers the Anti-Coercion Instrument process over ASML coercion, pools export-licensing and servicing decisions at EU level, and seeks a joint middle-power front with Japan and South Korea against extraterritorial cut-offs.
Why now is that Washington using our one chokepoint while tightening export controls forces a sovereignty answer we can fund without new money, and the safety breakthrough gives us a cooperative hook to trade assurance for access rather than accepting rationing alone.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Anti-Coercion Supply-Chain Pact", "category": 8, "size": "small", "finish_turn": 12, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up slightly, eu_political_capital down short-term then up if holds"}, "grounds": "to answer ASML coercion without spending capital we lack"}]}
```

## Priority
EU Anti-Coercion Supply-Chain Pact, because losing ASML servicing autonomy by default would finish EU sovereignty while M9 already runs on its own track and grid fights remain frozen.

## In practice
We task DG TRADE and DG CNECT to open an Anti-Coercion examination, with the Foreign Affairs and Competitiveness Councils co-owning: no immediate retaliatory duties, but a single EU servicing-licence regime for lithography and a compensation backstop for ASML from existing InvestEU guarantees so The Hague is not left alone. We link the safety breakthrough to it, offering joint evaluation access and certified deployment safeguards via the AI Office as reason for volume-licence access.

We keep M9 running through HERA/ECDC without new legislation: sequencing feeds and Shield drills continue on reprogrammed money, while we let municipal power-price mediation and Transition Safety Net payouts continue quietly to stop sentiment at 29 falling further. We start no data-centre forced vote and no new fund this turn.
...
```
## New measure
None this turn, while we let political capital rebuild above paralysis and the municipal surcharge revolt clarify before committing scarce funds.

## Store changes
No changes.

## Priority
M10 EU Anti-Coercion Supply-Chain Pact, because losing ASML servicing autonomy now would end sovereignty while energy moratoria and the capability jump demand holding action, not new spending.

## In practice
We keep DG TRADE's anti-coercion examination and the pooled EU lithography servicing licence as the single front: Competitiveness Council owns the file, InvestEU guarantee backstop reassures The Hague and ASML, and the AI Office trades certified evaluation access for volume-licence continuity with Tokyo and Seoul. No retaliatory duties this turn; we cannot fund a trade war at 5.0 capital.

At home we run what is already on the books. HERA/ECDC wastewater and Bio Shield drills continue, Transition Safety Net payouts continue quietly, and we task DG ENER with municipal mediation on power surcharges — temporary relief and grid-connection fast-track in exchange for lifting moratoria — without forcing a data-centre vote we would lose. We circulate the interpretability certification and leaked eval anomaly through joint telemetry to buy legibility while capability jumps.
