...
- characters 454-1925: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1968-4136: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5419-5859: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7993-10053: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M5 | EU Bio-Detection and Critical-Service Hardening Sprint | 6 | small | 3 | 5 | own jurisdiction | resilience up moderately, ai_safety protected indirectly | 2 | running |
...
## New measure
**EU Bio-Detection and Critical-Service Hardening Sprint**
Stands up EU-wide biosecurity detection (genomic surveillance, hospital screening protocols, HERA stockpile triggers) alongside cyber-hardening playbooks for health and energy operators exposed by the capability jump. This is Preparedness and resilience work, and why now is because the bio-uplift finding opens a four-turn bio gate while the frontier jump and open-weight diffusion make both misuse and cascade failure materially more likely before gigafactories land.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Detection and Critical-Service Hardening Sprint", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety protected indirectly"}, "grounds": "to close bio-detection gap before precursor window matures"}]}
```

## Priority
EU Bio-Detection and Critical-Service Hardening Sprint, because an open bio-capability precursor outranks build timelines — sovereignty means nothing if a distributed model enables harm we cannot detect or absorb.

## In practice
We task HERA with ECDC to expand wastewater and clinical genomic sampling in 10 hub cities, fund rapid sequencing kits and reporting duties for hospitals via EU4Health, and pre-authorise common procurement of medical countermeasures. ENISA with DG ENER runs exercised isolation and restoration drills with transmission operators hit last autumn, tying cyber-insurance relief to adopted hardening standards.

The Gigafactories (M1) and permitting zones (M2) continue through state-aid and grid-connection work, now explicitly conditioned on hardened power and security-by-design, while the Evaluation Institute (M3) is directed to add bio-uplift and agent-capability tests to substitute-model certification. Labour displacement is monitored via EMPL scoreboard this turn to prepare a transition instrument next turn, but we do not split capital across two new builds at 28 capital.
...
```
## New measure
**EU Proven-Gains Scale-Up for Health and Public Services**
Puts the public-sector AI uses already showing measurable benefit — shorter waits, faster decisions — plus EU-controlled clinical deployment of new therapies onto EuroHPC and sovereign stacks in every member state, so gains are visibly European. This is Diffusion, adoption and public trust work, and why now is because adoption success plus medical breakthrough have opened a rare sentiment window while eval anomalies and job losses threaten to close it — scaling what works rebuilds capital to sustain gigafactories and hardening.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Proven-Gains Scale-Up for Health and Public Services", "category": 9, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, eu_political_capital up, resilience up modestly"}, "grounds": "to convert adoption and medical wins into European-attributed legitimacy"}]}
```

## Priority
EU Proven-Gains Scale-Up for Health and Public Services, because at 20 capital nothing else finishes — without a visible European benefit now, the bio-hardening sprint and factories lose enforceability before they land.

## In practice
We task DG SANTE with HERA and DG DIGIT to procure scale-up via EU4Health and Digital Europe: joint buying of sequencing-linked therapies run on EuroHPC, and a replica fund for waiting-list triage and permit-decision copilots certified by the Evaluation Institute, with procurement preference for models runnable on EU-controlled compute. EPSCO formation carries it, with health ministers announcing hub-city results.

The Bio-Detection and Hardening Sprint (M5) continues its drills and sampling through to next turn, and Gigafactories (M1) and permitting zones (M2) grind through state-aid and grid hook-ups conditioned on hardening — we do not add money there this turn, only protect their legal base while the scale-up buys the tolerance to keep them alive. EMPL scoreboard on non-refilled entry posts is published alongside to show transition planning without opening a second build.
