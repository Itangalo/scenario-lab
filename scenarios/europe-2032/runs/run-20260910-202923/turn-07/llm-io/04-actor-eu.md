...
- characters 455-3034: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 3078-5466: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6117-6567: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8979-10818: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M7 | EU Trusted Public Services Rollout | 9 | small | 6 | 8 | own jurisdiction | public_sentiment up substantially, resilience up modestly | 2 | running |
...
## New measure
**EU Trusted Public Services Rollout**
It scales the proven European public-sector AI wins — shorter waiting lists and faster decisions — into certified hospitals, municipalities and schools under the audit regime.
This turn brought both a fresh agent loss-of-control and a rare attributed European adoption success; with trust at 17 and capital at 11, expanding what visibly works under proven safety is the only way our trust-rebuilding commitment advances.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trusted Public Services Rollout", "category": 9, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up substantially, resilience up modestly"}, "grounds": "to convert adoption success into trust while safety holds"}]}
```

## Priority
M7 EU Trusted Public Services Rollout, because only visible public benefit under certification can lift sentiment from 17 and fund the rest — containment alone cannot.

## In practice
We keep M1 gigafactories alive but unfunded-push, and enforce the joint-cell freeze and 24h reporting on logistics-finance agents in power, transport, hospitals and finance under NIS2/CER and AI Act Art.73, demanding unredacted logs for the stalled audit. DG HOME/HERA hold sequencing, wastewater and triage stocks as the backstop.

We launch the rollout via DG DIGIT and the AI Office with health and interior ministers fronting: EU4Health and Digital Europe money to replicate the passing hospital/municipal re-audits, procurement preference for EU-certified systems, and published wait-time/decision-time gains district by district to answer blockades and arson with proof. Council EPSCO and Health formations carry it to blunt moratorium parties.
...
```
## New measure
**EU Displaced-Worker Guarantee and Retraining Pact**
It ties automation-linked restructuring to funded wage insurance, rapid retraining and placement via certified public employment services, answering job-loss hostility and stalled robot hype.
With capital at 16 and sentiment at 15, clinics alone cannot rebuild trust while labour fear dominates coverage; this extends the trust-rebuilding commitment from proven services to livelihood security, and the narrow embodied-AI reality gives us a window to act before broader displacement hits.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Displaced-Worker Guarantee and Retraining Pact", "category": 7, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up moderately"}, "grounds": "to address job-loss driven hostility while robots still narrow"}]}
```

## Priority
M7 EU Trusted Public Services Rollout, because visible certified clinic and municipal gains remain the only proof holding audits together while the new pact builds out — trust cannot wait three turns for retraining cheques.

## In practice
We keep M7 fronted by health and interior ministers: DG DIGIT and AI Office replicate passing hospital/municipal audits with EU4Health and Digital Europe money, EPSCO/Health Council procurement preference for certified EU systems, district figures published to counter blockades.

We launch the Pact via DG EMPL with EPSCO Council under ESF+ and SURE-type backstop: qualifying automating employers co-fund wage insurance, PES fast-track retraining tied to certified sectors, legal base Articles 153/175 to blunt moratorium parties and unions resisting data-centres. Joint cell freeze and 24h reporting stay enforced in power, transport, hospitals, finance under NIS2/CER and AI Act Art.73 while M1 gigafactory capacity is shepherded to operational use, not new build.
