...
- characters 106-13304: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13347-13539: `{{metrics_json}}` from the run's live metric values
- characters 13632-14558: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14602-16896: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16952-17521: `{{notepad}}` from the Game Master's notepad, carried across turns
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Grid-Port-Bio Shield −2, EU Incident Registry and Deployment Audit −2, priority −1 = −11
PROPOSAL BONUS: EU Incident Registry and Deployment Audit (cat 2, small) +3 — cyber_major_incident and automated_decision_scandal both landed this turn with harm discovered by victims and logs never read, this answers both directly
...
## Metrics
```json
{"ai_capability": 56.5, "openweight_capability": 52.4, "ai_safety": 28.0, "resilience": 40.5, "eu_ai_sovereignty": 22.0, "eu_political_capital": 26.0, "public_sentiment": 17.0}
```
