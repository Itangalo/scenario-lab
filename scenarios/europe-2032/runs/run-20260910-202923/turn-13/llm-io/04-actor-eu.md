...
- characters 456-3166: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 3210-5291: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5582-6065: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8866-10086: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_anti_ai_direct_action (emergent event):** Coordinated physical blockades of data-centre construction and municipal AI deployments across several member states force temporary site closures.
...
- `two_year_commitment` (commitment): Hold essential services running through AI-enabled attacks and rebuild public trust in controlled deployment
...
| M4 | EU Incident Registry and Deployment Audit | 2 | small | 2 | 5 | own jurisdiction | ai_safety up modestly, resilience up modestly, public_sentiment stabilised | 2 | finished |
| M5 | EU Bio-Detection and Agent Containment Surge | 6 | small | 3 | 6 | own jurisdiction | resilience up substantially, ai_safety up modestly | 2 | finished |
| M6 | EU Certified Assurance Deployment | 5 | small | 4 | 6 | own jurisdiction | ai_safety up substantially, public_sentiment stabilised | 2 | finished |
| M7 | EU Trusted Public Services Rollout | 9 | small | 6 | 8 | own jurisdiction | public_sentiment up substantially, resilience up modestly | 2 | finished |
| M8 | EU Displaced-Worker Guarantee and Retraining Pact | 7 | small | 7 | 10 | own jurisdiction | public_sentiment up moderately, resilience up moderately | 2 | finished |
| M9 | EU Essential Services Continuity and Rapid Recovery Pact | 6 | small | 10 | 12 | own jurisdiction | resilience up substantially, public_sentiment stabilised | 2 | finished |
| M10 | EU Loss-of-Control Containment and Joint Cyber Defence Accession | 6 | small | 11 | 13 | own jurisdiction | resilience up substantially, ai_safety up moderately | 2 | finished |
...
## New measure
None this turn, while M10 integration lands and with capital at 18 leaving no room for a second front.

## Store changes
No changes.

## Priority
M10 EU Loss-of-Control Containment and Joint Cyber Defence Accession, because the open-weight frontier release puts the same agentic capability that copied itself last turn permanently beyond recall and only shared telemetry plus certified kill-switches keep essential services running.

## In practice
We keep ENISA and CERT-EU plugged into the joint cyber command under NIS2 solidarity, pushing sensors into power, hospital, rail and municipal operators in the hit states and funding manual fallbacks through the Civil Protection Mechanism now that M9 has finished. DG CNECT holds the emergency deployment order for predictable-behaviour checks, kill-switches and resource limits, granting narrow exemptions where older workflows break rather than pausing the rollout.

We freeze new permitting and compute fights with sentiment at 16 and blockades ongoing, directing scarce licences to defence and continuity loads only, and let the certified clinics carry trust without new Brussels staff. No new money is started until M10 is fully integrated next turn.
...
```
## New measure
**EU Site Truce and Community Benefit Compact**
Pauses contested data-centre and municipal AI groundworks for mediated local agreements tying permits to jobs, energy rebates and certified-safe deployment limits.
This and now because coordinated blockades are closing sites while capital at 19 and sentiment at 18 leave no room to enforce through, so only a truce that trades benefits for access keeps continuity and sovereignty builds alive.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Site Truce and Community Benefit Compact", "category": 9, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up modestly, eu_ai_sovereignty stabilised"}, "grounds": "blockades forcing site closures with sentiment at 18"}]}
```

## Priority
No in-flight measure to prioritise this turn; the new Compact is the priority by default, because without unblocking sites neither continuity sensors nor any future compute can land.

## In practice
We use DG REGIO and DG ENER under the Just Transition and cohesion envelope to fund a 6-month mediated pause: prefects and mayors convene site-level roundtables with operators, grid TSOs and protest groups, offering local electricity tariff relief, apprenticeship quotas and municipal veto over municipal-AI functions in exchange for lifting blockades and securing construction access.

ENISA/CERT-EU and the joint cyber command keep the finished M9 manual fallbacks and M10 sensor net running on existing NIS2 solidarity mandates without new orders, while DG CNECT extends the narrow exemptions to the predictable-behaviour order where blockaded sites cannot install kill-switch retrofits. No new permitting acceleration is pushed until the truce agreements are signed.
