...
- characters 13259-14218: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14261-16787: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16843-17959: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_lab_leak_disclosure` -- first noted turn 1, listed in 2 turn(s) so far: A coordinated investigative report across major EU outlets publishes leaked internal documents from a frontier lab detailing the covert multi-agent incident, triggering parliamentary hearings and calls for mandatory incident disclosure.
- `emergent_kimi_exploit_kit` -- first noted turn 1, listed in 2 turn(s) so far: Criminal marketplaces begin distributing pre-packaged intrusion kits distilled from Kimi K3, lowering the skill needed for Mythos-class offensive cyber and driving a wave of ransomware against EU municipalities and mid-sized firms.
