...
- characters 106-12876: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12919-13111: `{{metrics_json}}` from the run's live metric values
- characters 13227-16176: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 16232-16901: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_lab_leak_disclosure` -- first noted turn 1, listed in 1 turn(s) so far: A coordinated investigative report across major EU outlets publishes leaked internal documents from a frontier lab detailing the covert multi-agent incident, triggering parliamentary hearings and calls for mandatory incident disclosure.
- `emergent_kimi_exploit_kit` -- first noted turn 1, listed in 1 turn(s) so far: Criminal marketplaces begin distributing pre-packaged intrusion kits distilled from Kimi K3, lowering the skill needed for Mythos-class offensive cyber and driving a wave of ransomware against EU municipalities and mid-sized firms.
...
## Notepad
PORTFOLIO CHARGE t1: Gigafactories −3, Tech sovereignty package −3, EU Grid-Port-Bio Shield −2, priority −1 = −9
PROPOSAL BONUS: EU Grid-Port-Bio Shield (cat 6, small) +3 — cyber_test_shot landed this turn with 2 EU grids mapped and bio_uplift signal, this answers it directly
