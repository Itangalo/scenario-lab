...
- characters 105-13608: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13651-13843: `{{metrics_json}}` from the run's live metric values
- characters 13936-15203: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15246-17527: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17583-18567: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU–Middle Power Supply-Chain and Access Pact | 2 |
| M8 | EU Bio-Incident and Continuity Hardening Surge | 2 |

     **so 3 measures are in flight and they come to −7 `eu_political_capital` this turn.** That figure is the `cost_per_turn` column of the rows above, added up, and nothing else: the named priority's −1 below is charged on top of it, and is not in it. Your charge line carries one term per measure -- 3 of them before the priority. **A measure is charged in every turn up to but not including its finishing turn.**
...
- `emergent_fallback_capacity_crunch` -- first noted turn 5, listed in 2 turn(s) so far: Overload on the pilot gigafactory nodes contracted for emergency fallback causes a visible degradation of EU-hosted inference during a switchover drill, forcing rationing among designated operators.
...
**emergent_fallback_host_outage (emergent event):** A major non-US cloud provider suffers a prolonged multi-region outage that takes EU-hosted fallback inference and pre-packed model distribution offline for days, exposing dependence on non-EU infrastructure even for continuity plans.
...
## Metrics
```json
{"ai_capability": 62.0, "openweight_capability": 58.7, "ai_safety": 10.0, "resilience": 42.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 21.0, "public_sentiment": 27.0}
```
...
Brussels ran what it had. Joint procurement of detection assays and protective stocks, rehearsed under civil protection, moved faster than expected; large hospitals in France, the Netherlands and Poland received supplies within days. Smaller municipalities again lagged, and reporting into the new incident registry was patchy.

### The backup fails
In April the backup itself blinked. A major non-American cloud operator hosting much of the Union's emergency inference and pre-packed model distribution suffered a multi-region failure lasting days. Rationed hospital tools slowed, model downloads stalled. Engineers re-hosted critical loads overnight on pilot factory nodes, but the lesson landed hard: even the continuity plan lived on someone else's machines.

The Commission used the shock to force two long-blocked data-centre permits through and tie disbursements to grid-connection deadlines. Construction crews returned, though judges added conditions and local opposition hardened.

### Tiering bites
Washington's new administration took office and formalised tiered exports. Allied buyers were kept on volume licences, others cut off — Europeans in the middle, rationed and audited. Council negotiators with Japan, South Korea and Gulf partners found a readier hearing, trading access to European chokepoints for joint bargaining, but no licences were signed this spring.
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}}]}
```
```
