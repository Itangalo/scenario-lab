...
- characters 376-1643: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1686-3967: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4023-5326: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5389-30487: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30842-31264: `{{event_history}}` from the run's own event record
...
- `emergent_fallback_capacity_crunch` -- first noted turn 5, listed in 1 turn(s) so far: Overload on the pilot gigafactory nodes contracted for emergency fallback causes a visible degradation of EU-hosted inference during a switchover drill, forcing rationing among designated operators.
- `emergent_fallback_host_outage` -- first noted turn 5, listed in 1 turn(s) so far: A major non-US cloud provider suffers a prolonged multi-region outage that takes EU-hosted fallback inference and pre-packed model distribution offline for days, exposing dependence on non-EU infrastructure even for continuity plans.
...
- Turn 1 (5 turn(s) ago): cyber_test_shot, knowledge_work_augmented, member_state_defection
- Turn 3 (3 turn(s) ago): us_china_agreement, member_state_defection
- Turn 4 (2 turn(s) ago): capability_jump, openweight_frontier_release, knowledge_work_augmented, eu_frontier_access_denied
- Turn 5 (1 turn(s) ago): capability_jump, bio_uplift_findings, eu_frontier_access_denied, middle_power_coalition, election_consolidation
