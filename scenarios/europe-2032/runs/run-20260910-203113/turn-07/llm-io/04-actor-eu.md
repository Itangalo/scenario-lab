...
- characters 455-1974: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2018-4546: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6694-7150: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9782-11855: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_fallback_capacity_crunch (emergent event):** Overload on the pilot gigafactory nodes contracted for emergency fallback causes a visible degradation of EU-hosted inference during a switchover drill, forcing rationing among designated operators.
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Bio-Incident and Continuity Hardening Surge | 6 | small | 6 | 8 | own jurisdiction | resilience up, ai_safety up via incident learning | 2 | running |
...
## New measure
**EU Bio-Incident and Continuity Hardening Surge**
Rationed biodefence detection, hospital isolation capacity and EU-hosted fallback inference rebuilt after the outage, under civil protection.
Why this and why now: the real bio release with model involvement plus the multi-day fallback outage reset all risk arguments — hospitals and continuity nodes cannot wait for Gigafactories to finish, so a category 6 preparedness surge is the only instrument that answers casualties and containment now.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Incident and Continuity Hardening Surge", "category": 6, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up via incident learning"}, "grounds": "bio incident and fallback outage require immediate hardening"}]}
```

## Priority
M1 InvestAI Gigafactories, because without sovereign inference surviving US tiering and the non-US cloud outage, neither bio-response nor the middle-power pact has anything to run on — keeping the build alive outranks even the incident surge this turn.

## In practice
We keep M1 alive by political triage: Commission uses the outage and hospital blackout to force the two blocked permits through, links Connecting Europe and EIB disbursements to grid-connection deadlines, and tasks DG CNECT-ENISA to re-host the continuity reserve on EU-sovereign pilot nodes with pre-packed weights mirrored on public infrastructure, not a single external cloud.

We run the new surge through HERA and the Civil Protection Mechanism: joint procurement of detection assays and PPE, HERA stocktake turned into distribution to Lyon/Rotterdam/Krakow clusters, Health Council mandates incident reporting into the OT Registry, and rescEU exercises extended to bio-containment weeks. We push M7 in Council to demand allied volume-licence access and joint compute bargaining, using export-control escalation as leverage for chokepoint reciprocity.
...
```
## New measure
**EU Black-Box Assurance and Model Inspection Cell**
Stands up EU public evaluator access with black-box stress-tests and early activation-inspection pilots after chain-of-thought went opaque, tied to incident learning.
Why this and why now: the capability jump in code/cyber plus leaked eval anomalies and unreadable reasoning destroyed the remaining oversight basis while safety sits at 10 — a category 5 public capacity build is the only way to test what is deployed and rationed in Europe before the next bio/cyber use.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Black-Box Assurance and Model Inspection Cell", "category": 5, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up, resilience up via better detection"}, "grounds": "opaque reasoning and eval anomalies require independent assurance"}]}
```

## Priority
M8 EU Bio-Incident and Continuity Hardening Surge, because with dozens dead in Lyon, degraded fallback nodes, and rationed hospitals, keeping detection stocks and continuity drills alive outranks even the new inspection build this turn.

## In practice
We run M8 through HERA and the Civil Protection Mechanism to the finish: joint assay/PPE distribution focused on small municipalities that lagged, mandatory reporting into the registry via the Health Council, and rescEU bio-continuity exercises that include the pilot-node switchover drill that just failed, with ENISA re-hosting critical inference across two separate EU sites rather than one cloud.

We seed the new Cell inside the AI Office with JRC and ENISA staff, using Digital Europe funds: negotiated evaluation access to models deployed in essential services under the AI Act, shared black-box cyber/bio uplift tests, and a first activation-inspection research track with Japan/South Korea partners from M7. We contain the defection by offering the defecting member state volume-licence pass-through conditional on EU audit, rather than a public fight we cannot afford at 21 capital.
