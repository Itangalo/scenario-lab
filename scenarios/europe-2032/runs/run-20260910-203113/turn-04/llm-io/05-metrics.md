...
- characters 105-13716: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13759-13951: `{{metrics_json}}` from the run's live metric values
- characters 14044-15281: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15324-17543: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17599-18230: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Essential Services Continuity Reserve | 2 |
...
## Notepad
PORTFOLIO CHARGE t4: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M6 EU Essential Services Continuity Reserve −2, priority M6 −1 = −9
PROPOSAL BONUS: none — M6 (cat 6, small) answers the frontier cutoff directly but knowledge_work_augmented suppresses the rule 6 recent-event bonus with no displacement crisis to point at
LEGITIMACY LENDS: capital 26, sentiment 37 -> +2
SOVEREIGNTY: 16 last turn, no capacity finish, Gigafactories in flight +1, eu_frontier_access_denied t4 −2, capability rose 2.0 −1 = 14
## Store changes
```json
