...
- characters 455-1545: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1589-3564: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5095-5566: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9143-10925: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M13 | EU Accession to Joint Cyber Command Telemetry Pact | 8 | small | 12 | 14 | middle-power coalition | resilience up, ai_safety up via shared detection | 2 | running |
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up, ai_safety up via incident learning | 3 | finished |
| M4 | EU OT Intrusion Shared Registry | 2 | small | 2 | 4 | own jurisdiction | resilience up, ai_safety up via incident learning | 2 | finished |
| M5 | EU Third-Pillar Accession to US-China AI Risk Accord | 8 | small | 3 | 4 | US, China, middle-power coalition | ai_safety up, eu_ai_sovereignty up via inclusion and verification leverage | 2 | finished |
| M6 | EU Essential Services Continuity Reserve | 6 | small | 4 | 6 | own jurisdiction | resilience up, eu_ai_sovereignty up via fallback capacity | 2 | finished |
| M7 | EU–Middle Power Supply-Chain and Access Pact | 8 | small | 5 | 7 | middle-power coalition | eu_ai_sovereignty up, eu_political_capital up via joint leverage | 2 | finished |
| M8 | EU Bio-Incident and Continuity Hardening Surge | 6 | small | 6 | 8 | own jurisdiction | resilience up, ai_safety up via incident learning | 2 | finished |
| M9 | EU Black-Box Assurance and Model Inspection Cell | 5 | small | 7 | 10 | own jurisdiction | ai_safety up, resilience up via better detection | 2 | finished |
| M10 | EU Essential Workloads Continuity and Rationing Reserve | 6 | small | 8 | 11 | own jurisdiction | resilience up, eu_ai_sovereignty up via protected essential loads | 2 | finished |
| M11 | EU Public Benefit AI Deployment with Trusted Signatures | 9 | small | 9 | 12 | own jurisdiction | public_sentiment up, eu_political_capital up, resilience up via trusted adoption | 2 | finished |
| M12 | EU Logistics Displacement Transition and Care Retraining Compact | 7 | small | 10 | 13 | own jurisdiction | public_sentiment up, eu_political_capital up, resilience up via staffed services | 2 | finished |
...
## New measure
**EU Accession to Joint Cyber Command Telemetry Pact**
Joins the joint cyber command offered after the automated ransomware sweep, pooling attribution and real-time telemetry to keep hospitals and grids degrading not stopping. Why this and why now: with capital at 12, sentiment at 17 and no new halls, we cannot buy defence alone, and the invitation is the only protection that arrives within this turn.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Accession to Joint Cyber Command Telemetry Pact", "category": 8, "size": "small", "finish_turn": 14, "applies_to": "middle-power coalition", "targeted_effect": "resilience up, ai_safety up via shared detection"}, "grounds": "joint threat invitation after major automated incident"}]}
```

## Priority
M13 EU Accession to Joint Cyber Command Telemetry Pact, because the automated attack is public now and shared telemetry is the only resilience gain affordable before the ASML coercion fight.

## In practice
We mandate ENISA plus the M4 OT registry cell to connect dual-site detection feeds to the joint command under an Article 222 / NIS2 implementing act, with Council JHA sign-off this semester; open signature logs from M11 are extended to incident telemetry to hold auditors and mayors. Treasury cost is seconded analysts, not new halls, to fit thin funds while M12 runs.

We park the ASML coercion response at protest plus Anti-Coercion Instrument screening preparation, price freight and ration protected loads under M10 doctrine, and push M12 wage-insurance and care-channel placements as the visible answer to warehouse layoffs. No new compute line is opened; procurement keeps bargaining for audit clauses without conceding extraterritorial servicing bans.
...
Joins the middle-power coordination framework to align export licences, pool evaluation, and bargain jointly for compute access after the US lithography cut and investment collapse. Why this and why now: with queues lengthening, private build-out cancelled, and an unrecallable open-weight frontier release landing, joint withholding and joint buying is the only access and leverage still affordable at capital 10 and sentiment 12.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Middle-Power Supply-Chain Bargaining Compact", "category": 8, "size": "small", "finish_turn": 15, "applies_to": "middle-power coalition", "targeted_effect": "eu_ai_sovereignty up, eu_political_capital up via joint access and leverage"}, "grounds": "export-control escalation plus investment collapse and coalition window"}]}
```

## Priority
M13 EU Accession to Joint Cyber Command Telemetry Pact, because live shared detection is the only thing keeping essential services degrading-not-stopping through this semester's ransomware aftermath and open-weight diffusion, outranking even access bargaining while wards and grids are exposed.

## In practice
We keep ENISA secondments and dual-site feeds into the joint command on NIS2 Article 222 authority, with JHA Council sign-off and M11 signature logs extended to incident telemetry to hold mayors and auditors. This is seconded staff and pooled telemetry, not new halls, to fit a thin treasury while M12 transition payments continue.

We mandate DG Trade plus DG CONNECT under the Anti-Coercion Instrument to table licence-alignment and joint compute-procurement terms in the middle-power framework, using coordinated ASML servicing and upstream leverage as the bargaining chip for volume-licence access, and direct any freed procurement to protected essential loads under the M10 doctrine. No parallel EU compute programme is opened; the Gigafactories line remains the compute vehicle.
