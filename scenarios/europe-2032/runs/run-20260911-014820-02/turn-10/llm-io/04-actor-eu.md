...
- characters 453-1433: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1476-3336: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4486-4942: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7262-8707: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M6 | EU Cyber Emergency Repair Cell | 6 | small | 6 | 8 | own jurisdiction | resilience up, eu_political_capital up modestly via visible response | 2 | finished |
| M7 | EU Municipal Continuity and Bio-Sentinel Surge | 6 | small | 8 | 10 | own jurisdiction | resilience up, ai_safety up modestly via detection | 2 | finished |
...
## Two-year commitment
Rebuild trust by keeping essential services standing through the capability storm

## Statement changes
modify `two_year_commitment` (commitment): Rebuild trust by keeping essential services standing through the capability storm
- Trigger: the two-year commitment period closes this turn

## New measure
None this turn. We are waiting for the repair surge to finish landing and for any political headroom to return before starting anything new we cannot pay for.

## Store changes
No changes.

## Priority
M7 EU Municipal Continuity and Bio-Sentinel Surge, because it is our only funded presence still holding clinics, communes and detection together while capital is at 2 and anything new would break cohesion.

## In practice
We stay under the old hold-together commitment this turn: ENISA/CERT-EU keep embedded repair rosters on offline backups and patching via Digital Europe/UCPM reprogramming, with mayors signed to stay-until-holding, and HERA/ECDC keep sequencing and wastewater sentinels running under existing health emergency reporting after the genome-model warning.

We freeze new build votes and retaliatory export moves, keep Gigafactory sites warm, protest the lithography servicing cuts politically while EEAS uses solidarity repair aid to pull the defecting capital back to the joint licensing line. No new instrument is launched because with capital at 2 we cannot fund or enforce one without collapse.
