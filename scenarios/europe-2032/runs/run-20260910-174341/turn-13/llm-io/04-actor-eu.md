...
- characters 454-1720: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1764-3805: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7239-7744: `{{statement_ledger}}` from the actor's live statement ledger
- characters 11287-13115: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): A Europe that remains governable and cared-for through nationalised frontiers and embodied AI despite dependence on foreign models
...
|---|---|---|---|---|---|---|---|---|---|
| M13 | EU Bio Containment Surge and Care Isolation Net | 6 | small | 12 | 14 | own jurisdiction | resilience up materially against bio incident | 2 | running |
...
| M3 | EU Critical Systems Hardening and Response Pact | 6 | large | 1 | 3 | own jurisdiction | resilience up materially, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Frontier Evaluation Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up slightly, resilience up slightly via early warning | 2 | finished |
| M5 | EU Frontier Transparency and Deployment Reporting Act | 2 | small | 3 | 5 | own jurisdiction | ai_safety up slightly via visibility, resilience up slightly via faster detection | 2 | finished |
| M6 | EU Bio-Uplift Screening and Rapid Response Net | 6 | small | 4 | 6 | own jurisdiction | resilience up materially against bio incidents, ai_safety up slightly via misuse detection | 2 | finished |
| M7 | EU Essential Services Continuity Reserve | 6 | small | 6 | 8 | own jurisdiction | resilience up materially, eu_ai_sovereignty up slightly via fallback capacity | 2 | finished |
| M8 | EU Municipal Recovery Corps | 6 | small | 7 | 9 | own jurisdiction | resilience up materially via faster restoration, public_sentiment up slightly | 2 | finished |
| M9 | EU Care Continuity Switch | 6 | small | 8 | 10 | own jurisdiction | resilience up materially, public_sentiment up slightly | 2 | finished |
| M10 | EU Supply-Chain Leverage Defence | 8 | small | 9 | 11 | coalition | eu_ai_sovereignty up slightly, eu_political_capital down then up | 2 | finished |
| M11 | EU Humanoid Shock Absorber and Care Continuity Pact | 7 | small | 10 | 12 | own jurisdiction | resilience up slightly, public_sentiment up slightly | 2 | finished |
| M12 | EU Containment and Continuity Protocol for Rogue Agents | 6 | small | 11 | 13 | own jurisdiction | resilience up slightly, ai_safety up slightly via containment | 2 | finished |
...
## New measure
**EU Bio Containment Surge and Care Isolation Net**
Activates HERA-led bio detection, hospital isolation, and cross-border medical mutual aid for the deliberate bio-agent release now unfolding, using the Bio-Uplift Net and Continuity Reserve fallback.
This and now because casualties are real and containment runs for weeks, and with sovereignty at 10 and sentiment at 9 only a small preparedness measure (category 6) can hold governability under our two-year commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio Containment Surge and Care Isolation Net", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially against bio incident"}, "grounds": "to contain ongoing bio incident with model involvement"}]}
```

## Priority
M12 EU Containment and Continuity Protocol for Rogue Agents, because the rogue-agent quarantine and the bio containment must run as one isolation regime or hospitals and clouds fail together, outranking even the new bio surge this turn.

## In practice
We run the surge through HERA, ECDC and ENISA under the Civil Protection Mechanism and CER base: mandatory reporting from labs and cloud hosts, quarantine of suspect partitions alongside bio contact-tracing, and fallback to offline triage already drilled in two regions extended EU-wide. ESF+ and EU4Health reprogram to pay isolation wards and municipal recovery corps overtime.

We hold DG TRADE and the Anti-Coercion track open for the middle-power coalition window on export-licence alignment and compute bargaining, but commit no new instrument to it this turn; with capital at 13 we cannot fund two fronts. EPSCO keeps M11 wage bridges paying to prevent walkouts tipping into refusal of bio duties.
...
```
## New measure
**EU Entry-Level Work and Humanoid Transition Bridge**
Extends wage bridges and employer-tied retraining to graduate entry roles collapsing under AI displacement and to sites facing Chinese humanoid imports, funded by reprogrammed ESF+.
This and now because labour displacement and embodied AI deployment are hitting together while sentiment is at 7, and our two-year commitment to remain governable and cared-for fails first on hiring freeze and humanoid walkouts, not on frontier access.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Entry-Level Work and Humanoid Transition Bridge", "category": 7, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up slightly, public_sentiment up slightly via jobs cushion"}, "grounds": "to answer AI job losses and humanoid deployment under bleak sentiment"}]}
```

## Priority
M13 EU Bio Containment Surge and Care Isolation Net, because engineered-marker transmission plus a contested genome-model recipe means isolation beds and detection decide whether the Union holds this winter, outranking even the new labour bridge.

## In practice
We run M13 through HERA/ECDC and the Civil Protection Mechanism: pooled detection kits, cross-border isolation-bed mutual aid, and offline triage extended from drilled regions, with ENISA quarantine of suspect cloud partitions kept under the M12 rogue-agent protocol now finishing into standing procedure.

We stand up the new bridge through EPSCO and DG EMPL via ESF+ reprogramming: short-time top-ups extended to law, accountancy, software and customer-ops entry roles, plus care-rota protection where humanoids enter logistics and hospitals. No new funding instrument for the Brussels/The Hague supply-chain coalition or Taiwan-shock stockpiling this turn; with capital at 21 we keep the door open on licence-calendar alignment and compute bargaining but spend only on containment and jobs.
