...
- characters 105-13385: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13428-13619: `{{metrics_json}}` from the run's live metric values
- characters 13712-14903: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14946-17076: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17132-17986: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M8 | EU Municipal Recovery Corps | 2 |
| M9 | EU Care Continuity Switch | 2 |
...
PORTFOLIO CHARGE t7: M7 EU Essential Services Continuity Reserve −2, M8 EU Municipal Recovery Corps −2, priority −1 = −5
PROPOSAL BONUS: EU Municipal Recovery Corps (cat 6, small) +3 — cyber_major_incident severe this turn and this answers restoration directly, small size caps bonus
LEGITIMACY LENDS: capital 14, sentiment 34 -> +2 capped (applied in total, capital held below sentiment)
SOVEREIGNTY: 18 last turn, InvestAI Gigafactories finishes t7 +5, no capacity in flight bonus, export_control_escalation t7 −2, capability rose 3.0 −1 = 20

## Emerging developments (tracked)

- `emergent_care_rationing_lawsuit` -- first noted turn 7, listed in 2 turn(s) so far: US tiering delays trigger public attribution fight when two EU hospital networks report postponed oncology triage upgrades, sparking lawsuits against vendors and calls for compensation.
...
## Metrics
```json
{"ai_capability": 83.5, "openweight_capability": 76.5, "ai_safety": 6.0, "resilience": 41.0, "eu_ai_sovereignty": 17.0, "eu_political_capital": 14.0, "public_sentiment": 27.0}
```
...
## Notepad
PORTFOLIO CHARGE t8: M8 EU Municipal Recovery Corps −2, M9 EU Care Continuity Switch −2, priority no effect below 20 −0 = −4
PROPOSAL BONUS: EU Care Continuity Switch (cat 6, small) +4 — eu_frontier_access_denied severe this turn and this answers continuity directly, small size caps bonus
LEGITIMACY LENDS: capital 14, sentiment 27 -> +2 capped (applied in total, capital held below sentiment)
SOVEREIGNTY: 20 last turn, no cat4 finish, no cat4 in flight +0, eu_frontier_access_denied t8 −2 (largest capacity event, export_control_escalation also qualifies), capability rose 3.5 −1 = 17
