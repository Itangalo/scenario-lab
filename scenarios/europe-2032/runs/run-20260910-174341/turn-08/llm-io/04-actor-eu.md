...
- characters 1687-3817: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6115-6601: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9318-11274: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Municipal Recovery Corps | 6 | small | 7 | 9 | own jurisdiction | resilience up materially via faster restoration, public_sentiment up slightly | 2 | running |
...
Deploys joint ENISA-led recovery teams, clean backups and manual-workaround kits to restore ransomed municipal and health services fast, using the safety-breakthrough controls to re-certify cleaned systems.
This and now because the automated ransomware sweep is the commitment shock arriving with rationed US support, and only a small, visible restoration effort fits capital at 18 while M7 builds fallbacks.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Recovery Corps", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially via faster restoration, public_sentiment up slightly"}, "grounds": "to restore services hit by major automated attack under rationing"}]}
```

## Priority
M7 EU Essential Services Continuity Reserve, because keeping hospitals and grids on EU-controlled fallbacks through rationing plus cyber disruption is the two-year commitment case and outranks even the live recovery work.

## In practice
We run M7 as the main line: ENISA with HERA and ACER inventory US-model dependencies, contract standby inference on EuroHPC and utility clouds, and exercise offline switchovers in the two blackout-test regions, funded from the resilience envelope with member-state standby cost-sharing. We press Washington for tier clarity using Transparency Act incident data, without new money for gigafactory construction.

We stand up the Recovery Corps inside that same mandate: Commission Implementing Decision under the NIS2 crisis mechanism, ENISA pooling national CERT surgeons and pre-positioned clean images, Health and JHA Councils directing first deployments to hit municipalities and throttled imaging suites. Labour displacement is acknowledged via fast-track retraining slots attached to recovery hiring, but no separate fund this turn to protect capital.
...
```
## New measure
**EU Care Continuity Switch**
Keeps hospitals, ministries and municipal services running on EU-controlled fallback inference and manual protocols after the US frontier cutoff, redirecting the medical breakthrough to EU-run deployment.
This and now because access denial plus the loss-of-control incident is the exact rationing/failure case of our two-year commitment, and only a small preparedness operation fits capital at 14.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Care Continuity Switch", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, public_sentiment up slightly"}, "grounds": "to keep essential services running after frontier access denied"}]}
```

## Priority
M8 EU Municipal Recovery Corps, because visible restoration of ransomed municipalities and throttled hospitals is what holds legitimacy to execute the cutoff switchover under the two-year commitment.

## In practice
We run the Corps as the front line with the Continuity Switch inside it: Commission Implementing Decision under NIS2 Article 11 crisis coordination, ENISA plus HERA pooling national CERT surgeons, clean images and the new failure-prediction certification to re-certify cleaned systems, first deployments to paper-triage hospitals and hit municipalities. Health Council directs emergency inference contracts onto EuroHPC and utility clouds, with offline switchover rehearsals in the two blackout-test regions.

We press no new gigafactory build this turn to protect capital, using DG CNECT and EuroHPC JU standby funds with member-state cost-sharing. On bio-uplift and loss-of-control we task the Frontier Evaluation Cell legacy and the Bio Screening Net for monitoring and incident reporting only, acknowledging labour-freeze anger with retraining slots attached to recovery hiring but no separate fund.
