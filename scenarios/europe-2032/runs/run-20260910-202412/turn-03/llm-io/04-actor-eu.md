...
- characters 455-2497: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2541-4385: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5411-5876: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7553-9620: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M3 | EU Critical Infrastructure Cyber Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | running |
| M4 | EU Evaluation and Assurance Capacity | 5 | small | 2 | 6 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via talent and evaluation leverage | 2 | running |
...
## New measure
**EU Evaluation and Assurance Capacity**
Stands up independent EU technical capacity to evaluate frontier models and audit weight security, giving Brussels its own read on capability rather than learning from deployments and cut-offs.
Category 5 Public technical capacity and research – small – because ai_safety is falling behind capability at 54.0 and open weights at 46.0, and neither the Cyber Shield nor the gigafactory lines give us evaluators, researcher access, or assurance we control.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Evaluation and Assurance Capacity", "category": 5, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, eu_ai_sovereignty up slightly via talent and evaluation leverage"}, "grounds": "closing assurance gap after test-shot intrusion and US cut-off"}]}
```

## Priority
EU Critical Infrastructure Cyber Shield, because staged access in two member-state grids still undetected in production makes finishing pilot-to-deployment the precondition for the two-year commitment, ahead of the slower evaluation build.

## In practice
We keep the Shield as the operational push: ENISA with EU-CyCLONe moves from pilots to live swarm-monitoring on transmission operators and ports, using the 70% co-financing to settle cost-allocation fights and a fast-track tender repair; DG ENER ties gigafactory grid-connection approvals to Shield sensor rollout to blunt municipal opposition in Spain, Germany and Netherlands over power/water.

In parallel we seed the new evaluation capacity inside ENISA/EU AI Office with Digital Europe reprogramming: a small vetted evaluator team with pre-release access agreements and weight-security audit mandate, staffed by retaining frontier talent that would otherwise leave, and tasked first with learning from Shield incident data. M1/M2 site selection continues on its track without a new compute programme, preserving capital while the Shield spends it where harm is immediate.
...
```
## New measure
**EU Essential Services Continuity Fallback**
Keeps hospitals, ministries and grid operators running when the leading US model is cut off, by switching them to EU-hosted and open-weight models under ENISA continuity plans.
Category 6 Preparedness and resilience – small – because this is the second short-notice cut-off and the eval anomaly means we cannot wait for gigafactories in 2030 to answer a 2027 denial.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Continuity Fallback", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, eu_ai_sovereignty up slightly via EU-hosted fallback"}, "grounds": "second US model cut-off hitting hospitals and ministries"}]}
```

## Priority
EU Critical Infrastructure Cyber Shield, because it finishes next turn and the anomaly plus continued probing makes live hardening outrank even the new fallback, which can start in its shadow.

## In practice
We push ENISA and DG CNECT with the Health Emergency Preparedness and Response Authority to move Shield pilots from monitoring-only to enforced 24-hour patching on the two transmission networks and the port, using the settled 70% co-financing and closing water-utility gaps with a repaired joint-procurement tender. Council Energy formation carries the cost fight; gigafactory grid-tie linkage stays as the lever on Spanish, German and Dutch municipalities.

We start the fallback in parallel without a new compute programme: DIGIT lists essential public-sector AI dependencies, the AI Office evaluation unit vets EU-hosted replacements, and EU4Health/Digital Europe funds the switch-over playbooks and licences. No new siting, no parallel gigafactory – M1/M2 continue on track while this uses what already runs on EU soil.
