...
- characters 377-2419: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2463-4307: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4363-5549: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5612-29839: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30194-30290: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Infrastructure Cyber Shield −3, EU Evaluation and Assurance Capacity −2 = −11 + priority −1 = −12
PROPOSAL BONUS: EU Evaluation and Assurance Capacity (cat 5, small) +2 — staged intrusion and US cut-off in last two turns exposed no EU evaluator read, this answers it indirectly
LEGITIMACY LENDS: capital 25, sentiment 35.5 -> +2 (capped below sentiment)
SOVEREIGNTY: 19 last turn, no capacity event, no finish, in flight +0, capability rose 2.0 −1 = 18

## Emerging developments (tracked)

- `emergent_datacentre_permit_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated local protests and municipal permit challenges delay several hyperscale data-centre projects in Spain, Germany and the Netherlands over power prices and water use, without turning into a Union-wide movement.
- `emergent_dataset_poisoning_reckoning` -- first noted turn 1, listed in 2 turn(s) so far: A major non-frontier AI lab announces a training-data poisoning compromise affecting widely used open datasets, forcing temporary withdrawals of datasets and retraining delays across open-weight projects.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, cyber_defence_breakthrough, eu_frontier_access_denied
