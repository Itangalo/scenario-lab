...
- characters 106-12937: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12980-13171: `{{metrics_json}}` from the run's live metric values
- characters 13264-14500: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14543-16686: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16742-17103: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M6 | EU Bio-Detection and Essential Continuity Surge | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M6", "fields": {"finish_turn": 13}, "grounds": "named priority"}]}
```
```
