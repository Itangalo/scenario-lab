...
- characters 454-2094: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2138-4554: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4986-5478: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7509-8604: `{{previous_actions}}` from the actor's own response from the previous turn
...
## New measure
None this turn. We are waiting for political capital to recover above survival level before opening any new instrument, while M1 still burns capital we do not have.

## Store changes
No changes.

## Priority
M1 InvestAI Gigafactories, because with Washington tiering access and open weights now near-frontier, only domestic compute under EU jurisdiction restores any ability to determine our own future.

## In practice
We husband what exists. The Commission holds ENISA Shield enforcement on funded grids and port, keeps fallback playbooks live where deployed, and tasks the finished Evaluation unit to triage the leaked eval anomaly and bio-uplift claim into detection tuning — no new mandates, no new spend.

On politics we repair cohesion first. Council formations pause new permitting fights and trade retaliation, DG TRADE keeps the Anti-Coercion coalition warm without triggering it, and we use knowledge-work productivity evidence to stabilise sentiment while M1 deliveries are chased administratively to stop further slippage.
