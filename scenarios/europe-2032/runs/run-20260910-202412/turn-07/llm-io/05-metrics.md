...
- characters 106-13201: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13244-13435: `{{metrics_json}}` from the run's live metric values
- characters 13528-15168: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15212-17628: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17684-18460: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_grid_siting_sabotage` -- first noted turn 7, listed in 1 turn(s) so far: A coordinated physical blockade campaign against grid connections for data centres in Spain, Germany and the Netherlands escalates into sustained sabotage, forcing temporary construction halts and national police protection.
