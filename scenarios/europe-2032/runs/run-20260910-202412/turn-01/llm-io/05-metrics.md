...
- characters 16352-16995: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_permit_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated local protests and municipal permit challenges delay several hyperscale data-centre projects in Spain, Germany and the Netherlands over power prices and water use, without turning into a Union-wide movement.
- `emergent_dataset_poisoning_reckoning` -- first noted turn 1, listed in 1 turn(s) so far: A major non-frontier AI lab announces a training-data poisoning compromise affecting widely used open datasets, forcing temporary withdrawals of datasets and retraining delays across open-weight projects.
