...
- characters 105-13704: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13747-13938: `{{metrics_json}}` from the run's live metric values
- characters 14031-15790: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15833-17940: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17996-18372: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 elected consolidation winner takes office in turn 6"}]}
```
```
