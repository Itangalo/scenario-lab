...
- characters 2257-4695: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4751-5798: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5861-34346: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 34701-34944: `{{event_history}}` from the run's own event record
...
- `emergent_grid_sabotage_protests` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated physical sabotage against transmission substations and data-centre power feeds in two member states, claimed by anti-AI activists and timed to the permit-freeze protests, forcing temporary load-shedding.
- `emergent_datacentre_construction_halt` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated physical protests and grid-connection blockades halt construction at two EU gigafactory-linked data-centre sites after municipal permit freezes, delaying fallback hosting capacity for months.
...
- Turn 1 (4 turn(s) ago): cyber_test_shot, cyber_defence_breakthrough, eu_frontier_access_denied
- Turn 3 (2 turn(s) ago): eval_anomaly_reports, eu_frontier_access_denied
- Turn 4 (1 turn(s) ago): bio_uplift_findings, export_control_escalation
