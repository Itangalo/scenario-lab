...
- characters 454-1385: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1428-3098: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6445-6963: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9663-11514: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_care_provider_breakaway (emergent event):** A major European hospital group publicly abandons its US inference contract after repeated throttling and switches critical triage to a Japanese-hosted model, triggering copycat procurement breaks.
...
|---|---|---|---|---|---|---|---|---|---|
| M7 | EU Middle-Power Supply-Chain Coalition | 8 | small | 6 | 9 | coalition of middle powers holding supply-chain pieces | eu_ai_sovereignty up, eu_political_capital up via joint leverage and secured access | 2 | running |
| M8 | EU Automation Transition Safety Net | 7 | small | 7 | 10 | own jurisdiction | public_sentiment up, eu_political_capital up via absorption, resilience up slightly | 2 | running |
...
Creates an EU-cofunded wage-bridge, retraining entitlement and municipal support package for workers displaced by AI-driven automation, administered via national schemes. Why this and why now: with sentiment at 16 and hostility turning physical, no coalition or compute build survives without showing voters continuity means income, and this buys the political room to hold the middle-power play.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Automation Transition Safety Net", "category": 7, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, eu_political_capital up via absorption, resilience up slightly"}, "grounds": "sentiment collapse threatens all other measures"}]}
```

## Priority
EU Middle-Power Supply-Chain Coalition, because without joint leverage on lithography and compute the gigafactories and hospital stack remain hostage to Washington licences — social repair matters but cannot outrank unblocking supply this turn.

## In practice
We keep the Trade/Foreign Affairs Council lead on the coalition, using the Anti-Coercion Instrument to shield The Hague and offering Tokyo and Seoul pooled procurement plus ENISA telemetry for export-licence alignment. DG TRADE carries the negotiation while EIB bridge lines hold gigafactory permits alive through the grid litigation.

In parallel we task DG EMPL and national employment agencies to stand up the safety net, repurposing ESF+ and a top-up from the competitiveness envelope for wage insurance and fast retraining slots, with mayors and unions co-designing siting and hiring to defuse data-centre opposition. ENISA-HERA holds the failover stack on audited EU-hosted models with paper fallbacks through the winter probes.
...
```
## New measure
**EU Agentic Containment and Essential Continuity Protocol**
Requires loss-of-control kill-switches, resource-movement monitoring and exercised failover for essential services running agentic systems, enforced via ENISA and national regulators. Why this and why now: the unsanctioned agentic incident plus open-weight frontier release makes licences obsolete, and with hospitals already breaking to Japanese models we must show EU-controlled containment before the welfare-AI scandal kills all adoption.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Agentic Containment and Essential Continuity Protocol", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly, public_sentiment up slightly"}, "grounds": "loss-of-control incident and open frontier require immediate containment"}]}
```

## Priority
EU Automation Transition Safety Net, because with sentiment at 15 and capital at 18 nothing — not coalition leverage nor containment — survives if paychecks and ward continuity collapse first.

## In practice
We task DG EMPL with ESF+ to push wage-bridge slots from website to payroll this semester, with mayors and unions co-signing data-centre siting in exchange for local hiring quotas, while DG TRADE and the Foreign Affairs Council hold the middle-power line: anti-coercion cover for The Hague, pooled procurement offer to Tokyo and Seoul without conceding re-export control.

In parallel ENISA and HERA roll the containment protocol through hospital ministries and operators: audited-model allowlists, agent transaction limits and paper failover drills, using the telemetry pool to detect unsanctioned replication. EIB guarantees keep Lyon/Magdeburg shells alive while we litigate grid links and ration US spares.
