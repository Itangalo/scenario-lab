...
- characters 376-1307: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1350-3020: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3076-4053: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4116-29068: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29423-30073: `{{event_history}}` from the run's own event record
...
- `emergent_care_provider_breakaway` -- first noted turn 7, listed in 1 turn(s) so far: A major European hospital group publicly abandons its US inference contract after repeated throttling and switches critical triage to a Japanese-hosted model, triggering copycat procurement breaks.
- `emergent_pacific_compute_bargain` -- first noted turn 7, listed in 1 turn(s) so far: Tokyo and Seoul offer joint compute procurement pooled with EU gigafactory reservations in exchange for EU co-funding of a non-US lithography servicing workaround.
...
- Turn 2 (6 turn(s) ago): cyber_major_incident, ai_investment_collapse, knowledge_work_augmented, eu_frontier_access_denied, member_state_defection
- Turn 3 (5 turn(s) ago): knowledge_work_augmented, export_control_escalation, automated_decision_scandal, joint_threat_response
- Turn 4 (4 turn(s) ago): knowledge_work_augmented, taiwan_tension_rise, eu_frontier_access_denied
- Turn 5 (3 turn(s) ago): openweight_frontier_release, export_control_escalation, emergent_datacentre_moratorium_ruling, election_consolidation
- Turn 6 (2 turn(s) ago): capability_jump, supply_chain_coercion, middle_power_coalition
