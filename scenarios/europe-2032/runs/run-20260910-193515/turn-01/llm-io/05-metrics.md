...
- characters 16248-16834: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_synthesis_pause` -- first noted turn 1, listed in 1 turn(s) so far: Major DNA synthesis providers announce a voluntary temporary pause on de novo genome-language-model designs pending a shared screening upgrade, after the bacteriophage results draw press attention.
- `emergent_research_integrity_pact` -- first noted turn 1, listed in 1 turn(s) so far: Major EU scientific publishers and universities jointly mandate disclosure and watermarking of AI-generated research after the Millennium Prize claim triggers reproducibility disputes.
