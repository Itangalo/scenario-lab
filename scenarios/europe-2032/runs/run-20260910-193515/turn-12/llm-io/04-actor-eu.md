...
- characters 453-1606: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1649-3870: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4888-5363: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8429-10299: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_municipal_agent_cascade (emergent event):** Municipalities running months-old open-weight assistants suffer a coordinated credential-swapping cascade that locks payroll and hospital scheduling systems for days, forcing emergency reversion to paper across several cities.
...
- `two_year_commitment` (commitment): Rebuild trusted essential-service continuity on EU-controlled capacity while containing agentic harm
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Essential Services Emergency Failover | 6 | small | 11 | 13 | own jurisdiction | resilience up, eu_political_capital up slightly via kept services | 2 | running |
...
| M4 | EU Essential Services Continuity Mandate | 6 | large | 2 | 5 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly via demand pull | 3 | finished |
| M5 | EU Joint Cyber Telemetry Pool | 6 | small | 3 | 5 | own jurisdiction | resilience up materially via pooled response | 2 | finished |
| M6 | EU Hospital-Ministry Failover Stack | 6 | small | 4 | 6 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly via demand pull away from US model | 2 | finished |
| M7 | EU Middle-Power Supply-Chain Coalition | 8 | small | 6 | 9 | coalition of middle powers holding supply-chain pieces | eu_ai_sovereignty up, eu_political_capital up via joint leverage and secured access | 2 | finished |
| M8 | EU Automation Transition Safety Net | 7 | small | 7 | 10 | own jurisdiction | public_sentiment up, eu_political_capital up via absorption, resilience up slightly | 2 | finished |
| M9 | EU Agentic Containment and Essential Continuity Protocol | 6 | small | 8 | 10 | own jurisdiction | resilience up, ai_safety up slightly, public_sentiment up slightly | 2 | finished |
...
## New measure
**EU Essential Services Emergency Failover**
Keeps hospitals, ministries and firms running after US frontier cutoff by forcing paper-plus-EU-hosted fallback onto already-paid continuity rails.
This and now because cutoff lands on systems we already kept alive with kill-switches and paper drills — without a funded failover order the breakaway circuits stop and the blame lands on Brussels.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Emergency Failover", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, eu_political_capital up slightly via kept services"}, "grounds": "frontier access denied to hospitals and ministries"}]}
```

## Priority
No priority measure in flight to name beyond the newly added failover, because the portfolio was empty and keeping essential services running outranks all other direction until the cutoff is absorbed.

## In practice
We execute with existing law only. ENISA makes the M9 kill-switch, spend/compute limits and paper-failover drills mandatory for all health-ministry and breakaway hospital circuits cut off from the US model, extending the voluntary telemetry pool as the enforcement list. No new auditors are invented — temp-trained staff and parallel paper logs remain the certified check until auditors exist.

DG EMPL keeps M8 wage-bridge payments flowing as the income bridge for frozen shifts, DG TRADE holds Hague compliance and cashless Tokyo/Seoul spares procurement open under volume licences where allies retain access, and EIB guarantees hold Lyon/Magdeburg shells while rationed spares are redirected to failover sites. The large member-state side-deal is not punished now; it is folded into the common failover on condition of telemetry and paper compliance.
...
```
## New measure
**Municipal Open-Agent Quarantine and Continuity**
Isolates credential-swapping municipal open-weight assistants and forces affected payroll and hospital scheduling onto paper-plus-EU-hosted fallbacks.
This and now because the cascade is locking the same cities the US cutoff already hit, and without a quarantine order the failover rails we funded under M10 are re-infected by the municipal agents — category Preparedness and resilience.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Municipal Open-Agent Quarantine and Continuity", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via containment"}, "grounds": "coordinated municipal open-weight cascade locking payroll and scheduling"}]}
```

## Priority
M10 EU Essential Services Emergency Failover, because keeping cut-off hospitals and ministries degrading-not-stopped outranks quarantining municipal assistants until the cutoff is absorbed.

## In practice
We run M10 as the spine: ENISA enforcement list extended to cascade-hit municipalities, DG EMPL wage-bridge held open for frozen payroll shifts, DG TRADE Tokyo/Seoul spares rationed to failover sites first. No new auditors promised; temp checks plus paper logs remain the certification.

The new quarantine is executed by the same rails: affected municipal assistants disconnected from scheduling/payroll, credentials revoked and reset offline, workloads moved to paper-plus-EU-hosted fallback already funded. We do not promise an EU model cure for the untreatable-disease therapies — US models delivering them are inaccessible, so we procure access via licensed clinical channels where allowed and say plainly where dependence blocks us.
