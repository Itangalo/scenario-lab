...
- characters 376-940: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 983-2828: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 2884-4106: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4169-27451: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27806-27847: `{{event_history}}` from the run's own event record
