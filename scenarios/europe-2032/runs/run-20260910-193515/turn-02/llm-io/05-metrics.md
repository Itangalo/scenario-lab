...
- characters 105-12946: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12989-13181: `{{metrics_json}}` from the run's live metric values
- characters 13274-13838: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 13881-15726: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 15782-17004: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_synthesis_pause` -- first noted turn 1, listed in 2 turn(s) so far: Major DNA synthesis providers announce a voluntary temporary pause on de novo genome-language-model designs pending a shared screening upgrade, after the bacteriophage results draw press attention.
- `emergent_research_integrity_pact` -- first noted turn 1, listed in 2 turn(s) so far: Major EU scientific publishers and universities jointly mandate disclosure and watermarking of AI-generated research after the Millennium Prize claim triggers reproducibility disputes.
...
## Metrics
```json
{"ai_capability": 54.5, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 37.0, "eu_ai_sovereignty": 19.0, "eu_political_capital": 22.0, "public_sentiment": 29.0}
```
