...
- characters 377-1641: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1685-3728: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3784-4905: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4968-29195: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29550-29665: `{{event_history}}` from the run's own event record
...
- `emergent_cyber_insurance_retreat` -- first noted turn 2, listed in 1 turn(s) so far: Insurers sharply raise premiums or withdraw cover for grid and port operators after the autumn intrusions are disclosed, forcing emergency state backstops for cyber-related business interruption in several member states.
- `emergent_gigafactory_permit_freeze` -- first noted turn 2, listed in 1 turn(s) so far: Regional courts grant interim injunctions halting groundworks at two shortlisted gigafactory sites after grid-priority and environmental challenges, delaying construction starts beyond mid-2027.
...
- Turn 2 (1 turn(s) ago): cyber_defence_breakthrough, taiwan_tension_rise
