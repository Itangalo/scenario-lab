...
- characters 377-2415: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2459-4897: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4953-6043: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 6106-32956: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 33311-33650: `{{event_history}}` from the run's own event record
...
- `emergent_clinician_backlash_suspension` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated clinician walkouts and hospital liability suits over errors from fallback triage models after the autumn cutoff force several regions to suspend AI-assisted triage.
- `emergent_compute_referendum_block` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated municipal referendums in Spain and Germany vote to block new compute-zone water and grid connections, forcing a national-level legal challenge over local veto rights.
...
- Turn 1 (4 turn(s) ago): cyber_test_shot, cyber_major_incident, adoption_success
- Turn 2 (3 turn(s) ago): safety_breakthrough, member_state_defection
- Turn 3 (2 turn(s) ago): eu_frontier_access_denied, emergent_eval_coverup_leak
- Turn 4 (1 turn(s) ago): cyber_major_incident, bio_uplift_findings, eval_anomaly_reports, opaque_reasoning
