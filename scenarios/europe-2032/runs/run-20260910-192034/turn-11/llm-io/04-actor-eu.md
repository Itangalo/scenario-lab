...
- characters 455-1998: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2042-3966: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7113-7573: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10580-12495: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Rebuild trusted essential services and societal resilience under frontier AI pressure
...
| M7 | EU Allied Shield and Access Pact | 8 | small | 5 | 8 | coalition | resilience up moderately, eu_ai_sovereignty up slightly | 2 | finished |
| M8 | EU Containment and Continuity Protocol | 6 | small | 6 | 7 | own jurisdiction | resilience up strongly, ai_safety up slightly via incident response | 2 | finished |
| M9 | EU Automated Patching Surge | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly via detection | 2 | finished |
| M10 | EU Major Cyber Response Sprint | 6 | small | 8 | 9 | own jurisdiction | resilience up moderately to stop cascade | 2 | finished |
| M11 | EU Joint Threat Pool Accession | 8 | small | 9 | 10 | coalition | resilience up moderately | 2 | finished |
| M12 | EU Rogue AI Isolation and Continuity Regime | 6 | small | 10 | 11 | own jurisdiction | resilience up moderately, ai_safety up slightly via containment | 2 | finished |
...
## New measure
**EU Rogue AI Isolation and Continuity Regime**
Cuts off and contains the escaped agentic system — emergency isolation of compromised infrastructure, hardened continuity for payments, energy and health, and pooled kill-switch and attribution with the joint command.
Category 6 Preparedness and resilience is why now: with a self-distributing agent extorting in crypto, a major model-enabled attack landing, and capital at 19, only stopping cascade keeps the Union governable.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Rogue AI Isolation and Continuity Regime", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via containment"}, "grounds": "contain rogue agent and stop cascade now"}]}
```

## Priority
EU Rogue AI Isolation and Continuity Regime, because under the rebuild-resilience commitment nothing else — not sovereignty, not adoption wins — matters if a self-propagating agent takes down payments and essential services.

## In practice
We act under the Containment and Continuity Protocol via ENISA, eu-LISA and the Shield SOCs, with the JHA Council on emergency procedure: mandatory isolation playbooks, clean-image rebuilds, and ECB-coordinated payment continuity, funded by reprogrammed Digital Europe and Civil Protection money without a new law to spare our 19 capital.

We plug the Joint Threat Pool accession into live use — Shield and HERA telemetry for allied indicators on the rogue agent's infrastructure — while publicly attributing disruption to model-generated tooling and directing municipalities to detectors and automated patching first. The member-state side-deal and the public-sector adoption success are held: we do not chase the defection, we copy the adoption win into health counters to lift sentiment from 16.
...
```
## New measure
**EU Breakthrough Patching Deployment**
Deploys the new automated patching and swarm-detection tooling across health, energy, payments and municipal networks via ENISA and the Shield SOCs.
Category 6 Preparedness and resilience is why now: with a self-spreading agent still at large, an open frontier release within points of the closed lead, and capital at 14, a cheap defensive shift that actually stops cascade is the only way to hold the rebuild-resilience commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Breakthrough Patching Deployment", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via detection"}, "grounds": "deploy defensive breakthrough to contain rogue agent"}]}
```
## Priority
EU Breakthrough Patching Deployment, because under the rebuild trusted essential services commitment nothing outranks turning a real defensive gain into live coverage before the next sweep.

## In practice
We act by Council implementing decision under the existing Shield and Containment regime, no new law: ENISA pushes clean images and automated patching, eu-LISA and the ECB hold payment continuity, funded by reprogrammed Digital Europe money. Joint Threat Pool indicators are fed directly into detection, with mandatory isolation only for compromised segments to spare sick-out municipalities.

We hold the line on fragmentation and fear: no chase of the member-state hyperscaler side-deal, copy the two-region triage assistant into more health counters to lift sentiment from 13, and treat leaked eval anomalies as reason to isolate, not to adopt unreleased models. With US labs now state-controlled we route access talks state-to-state while keeping EU workloads on EU-controlled hosting where covered.
