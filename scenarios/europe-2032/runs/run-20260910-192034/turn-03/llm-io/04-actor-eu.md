...
- characters 455-1948: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1992-3889: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4610-5074: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7026-9052: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_eval_coverup_leak (emergent event):** A major non-frontier AI lab researcher publicly releases internal documents alleging systematic under-reporting of biosecurity evaluation failures across several 2026 model releases.
...
| M4 | EU Assurance Adoption Programme | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, resilience up slightly via certified detection | 2 | running |
...
## New measure
**EU Assurance Adoption Programme**
Deploys the new interpretability and control result across critical-sector AI systems and embeds it in ENISA and EU evaluators' pre-release checks, turning a lab breakthrough into certified assurance on shipped systems.
Category 5 Public technical capacity and research is why now: the breakthrough only raises safety if adopted quickly before the next capability jump wipes it out, and it gives Brussels a concrete trust anchor to counter both the biosecurity leak distrust and the member-state defection narrative.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Assurance Adoption Programme", "category": 5, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, resilience up slightly via certified detection"}, "grounds": "adopt deployable interpretability breakthrough before capability advance erodes it"}]}
```

## Priority
EU Critical Infrastructure Shield, because the staged sabotage tooling is already inside OT networks and segmentation resets before December outrank even a fast-moving safety gain.

## In practice
We drive the Shield through ENISA and the Energy and Transport Councils, with the two hit TSOs leading breaker-credential resets and cross-border exercises funded by reprogrammed Digital Europe and CEF money, while offering municipalities grid-fee relief to defuse data-centre power resistance in Spain and Germany. DG CNECT keeps Gigafactories permitting zones alive but without new political spend this turn.

In parallel we task the AI Office and Joint Research Centre to codify the interpretability result into common evaluation protocols and procurement requirements for critical-sector AI, inviting the defecting member state back in via joint researcher access rather than public confrontation. City-level adoption pilots on hospital queues and permits continue as visible wins to hold sentiment while hardening dominates attention.
...
Stands up emergency fallback for hospitals, ministries and firms cut off from the leading foreign model by re-hosting their critical workflows on EU-controlled models and pooled Gigafactory-zone capacity under fast-track procurement.
Category 6 Preparedness and resilience is why now: the cutoff is already degrading essential services this turn, and without a visible switch the denial reads as failure of foresight and fractures the common line further, while it also buys time for the sovereignty build-out to land.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Continuity Switch", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, eu_ai_sovereignty up slightly via pooled EU hosting"}, "grounds": "keep essential services running after sudden foreign frontier cutoff"}]}
```

## Priority
EU Essential Services Continuity Switch, because cut-off hospitals and ministries failing this winter outranks even assurance adoption under our two-year resilient-Europe commitment.

## In practice
We trigger the Shield machinery via ENISA, DG CNECT and the Health and Home Affairs Councils, using emergency procurement under the Critical Entities Resilience Directive to fund re-platforming vouchers and a pooled inference reserve hosted in available EU data-centres and early Gigafactory pilot racks. The two hit TSOs' exercise cell is repurposed to triage health and ministry dependencies first, with the defecting capital offered first access to the pool if it aligns its hyperscaler deal to EU security terms.

We keep the Assurance Adoption Programme running through the AI Office and JRC to codify the spring interpretability result into procurement checks, now sharpened by the biosecurity under-reporting leak to require independent bio-eval disclosure, and we keep Gigafactories permitting alive with targeted grid-fee and water-reuse packages for Spanish and German municipalities rather than new political spend. City pilots on hospital queues are folded into the Switch as proof the fallback works.
