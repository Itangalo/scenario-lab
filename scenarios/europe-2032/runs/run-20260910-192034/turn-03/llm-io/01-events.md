...
- characters 377-1870: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1914-3811: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3867-4949: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5012-28588: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28943-29094: `{{event_history}}` from the run's own event record
...
- `emergent_eval_coverup_leak` -- first noted turn 1, listed in 2 turn(s) so far: A major non-frontier AI lab researcher publicly releases internal documents alleging systematic under-reporting of biosecurity evaluation failures across several 2026 model releases.
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated municipal protests block permitting for two planned hyperscale data centres in Spain and Germany over power and water use, forcing a national-level review.
...
- Turn 2 (1 turn(s) ago): safety_breakthrough, member_state_defection
