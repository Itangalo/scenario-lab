...
- characters 16374-16926: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_eval_coverup_leak` -- first noted turn 1, listed in 1 turn(s) so far: A major non-frontier AI lab researcher publicly releases internal documents alleging systematic under-reporting of biosecurity evaluation failures across several 2026 model releases.
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated municipal protests block permitting for two planned hyperscale data centres in Spain and Germany over power and water use, forcing a national-level review.
...
PROPOSAL BONUS: EU Critical Infrastructure Shield (cat 6, large) +4 — test shot precursor landed this turn and this answers it directly
