# Store: The European Union (turn 6)

## measures

| id | name | category | size | started_turn | finish_turn | applies_to | targeted_effect | cost_per_turn | status |
|---|---|---|---|---|---|---|---|---|---|
| M1 | InvestAI Gigafactories | 4 | large | 0 | 7 | own jurisdiction | eu_ai_sovereignty up, via €200bn for four to five sites | 3 | running |
| M2 | Tech sovereignty package | 4 | large | 0 | 6 | own jurisdiction | eu_ai_sovereignty up, via €200bn of private capital for AI data centres by 2036 and accelerated-permitting zones | 3 | finished |
| M3 | EU Critical Infrastructure Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up strongly, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Assurance Adoption Programme | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, resilience up slightly via certified detection | 2 | finished |
| M5 | EU Essential Services Continuity Switch | 6 | small | 3 | 4 | own jurisdiction | resilience up moderately, eu_ai_sovereignty up slightly via pooled EU hosting | 2 | finished |
| M6 | EU Shock Absorption Sprint | 6 | small | 4 | 5 | own jurisdiction | resilience up moderately | 2 | finished |
| M7 | EU Allied Shield and Access Pact | 8 | small | 5 | 8 | coalition | resilience up moderately, eu_ai_sovereignty up slightly | 2 | running |
| M8 | EU Containment and Continuity Protocol | 6 | small | 6 | 8 | own jurisdiction | resilience up strongly, ai_safety up slightly via incident response | 2 | running |

## standing

| id | posture |
|---|---|
| S1 | pending |

## Changes this turn

- **applied** `M8` — {"op": "add", "table": "measures", "fields": {"name": "EU Containment and Continuity Protocol", "category": 6, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "resilience up strongly, ai_safety up slightly via incident response"}, "grounds": "answer bio and loss-of-control incidents under RSI onset"}
  - Grounds: answer bio and loss-of-control incidents under RSI onset
