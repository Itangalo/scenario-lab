...
- characters 452-1238: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1282-3494: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4121-4581: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8100-9876: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M14 | EU Trusted Essential Services Rebuild | 9 | small | 12 | 14 | own jurisdiction | public_sentiment up moderately, resilience up slightly, eu_political_capital up slightly | 2 | running |
...
## New measure
None this turn.
We hold to the single running rebuild and add nothing new while political capital is at 7 and essential-services continuity is still landing, waiting for sentiment and capital to recover enough to carry another instrument.

## Store changes
No changes.

## Priority
M14 EU Trusted Essential Services Rebuild, because it is the only instrument that can turn EU-hosted continuity after the US cutoff into visible trust without spending capital we do not have.

## In practice
We keep all political weight on M14 delivery through this winter: Commission Recommendation implemented via Digital Europe and EU4Health reprogramming, Health Data Space and eu-LISA pooled hosting replacing cut-off US triage models, with mandatory human sign-off, public queue dashboards and ombudsman audit to answer the benefits scandal directly.

We do not open new fronts on ASML servicing, export licences or the member-state side-deal. The Single Market and Health Councils hold a continuity line, ENISA keeps patched grids and payments in degraded-not-down mode, and state-to-state talks quietly seek emergency US volume licences while procurement shifts only where EU-controlled capacity already exists. No retaliation, no new mandate, no parallel compute push.
