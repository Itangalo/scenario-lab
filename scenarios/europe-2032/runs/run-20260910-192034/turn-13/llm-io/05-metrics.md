...
- characters 107-12952: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12995-13183: `{{metrics_json}}` from the run's live metric values
- characters 13276-14062: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14106-16318: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16374-17244: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_patch_refusal_belt` -- first noted turn 12, listed in 2 turn(s) so far: Municipal utilities and hospital groups openly refuse to install centrally pushed AI patching agents after the autumn outages, creating a patch-refusal belt that leaves edge networks exposed.
...
## Notepad
PORTFOLIO CHARGE t13: M14 EU Trusted Essential Services Rebuild −2, priority −0 (capital below 20, priority has no effect and no cost) = −2
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 5, sentiment 6 -> none, capital calculation done before lend check but sentiment not above by rule margin applied
SOVEREIGNTY: 2 last turn, no capacity event, no cat4 in flight +0, capability rose 3.0 −1 = 1
## Store changes
```json
{"store": []}
```
