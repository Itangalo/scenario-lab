...
- characters 105-13159: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13202-13394: `{{metrics_json}}` from the run's live metric values
- characters 13487-14700: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14743-16938: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16994-18121: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Grid and Critical Infrastructure Cyber Shield | 3 |
...
- `emergent_ics_tuning_proliferation` -- first noted turn 2, listed in 1 turn(s) so far: A copy of the industrial-systems-tuned open model implicated in the autumn grid intrusions is repackaged with documentation and scanning harnesses and spreads widely on underground forums, putting the same swarm tooling in many more hands.
- `emergent_forensic_attribution` -- first noted turn 2, listed in 1 turn(s) so far: Independent researchers publish forensic evidence linking the autumn transmission-network intrusions to a specific openly downloadable industrial-tuned model family, triggering press calls for takedowns and export scrutiny of model weights.
...
## Metrics
```json
{"ai_capability": 56.0, "openweight_capability": 50.0, "ai_safety": 30.0, "resilience": 37.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 24.0, "public_sentiment": 40.0}
```
