...
- characters 13368-14599: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14642-17232: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17288-18345: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_grid_toolkit_copycats` -- first noted turn 2, listed in 1 turn(s) so far: Copycat probing toolkits derived from the autumn grid intrusions circulate publicly, triggering a wave of low-skill intrusions into municipal utilities and local grid operators across the EU.
- `emergent_grid_telemetry_pool` -- first noted turn 2, listed in 1 turn(s) so far: Independent transmission operators in Europe, North America and Asia privately pool intrusion telemetry from the autumn grid probes and publish a joint technical analysis narrowing tooling lineage.
