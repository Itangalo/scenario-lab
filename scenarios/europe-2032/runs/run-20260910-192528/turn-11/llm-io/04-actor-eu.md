...
- characters 456-1550: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1594-3768: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5728-6205: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9227-11119: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_care_standard_revolt (emergent event):** Backstop towns that drilled isolation publish triage survival data, prompting a patient-led movement demanding the same offline-care standards nationally and shaming undrilled hospital boards.
...
## New measure
**EU Automated Attack Containment and Service Isolation Drive**
Keeps hospitals, grids and municipal services running through the current automated-attack wave by funding drilled isolation, offline fallback and rapid rebuild kits tied to the existing backstop.
This is Preparedness and resilience work, and it must come now because the major automated incident lands as verifiable cyber capability jumps while open models near 63 make every tooling unrecallable, and only exercised isolation stops cascade.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Automated Attack Containment and Service Isolation Drive", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "automated major incident plus capability jump amid unrecallable diffusion"}]}
```

## Priority
EU Automated Attack Containment and Service Isolation Drive, because the live ransomware/dependency disruption outranks all else and without held continuity the two-year commitment to hold biological containment and essential-service continuity fails outright.

## In practice
We act under the Civil Protection Mechanism and NIS2 Article 11 crisis powers, with ENISA running the triage playbooks and HERA/ECDC keeping sequencing and isolation stocks flowing to backstop towns. Health and TTE Councils extend EU4Health co-funding for paid municipal continuity teams to cover cyber-isolation drills and offline triage.

We do not reopen the US internals fight; we keep black-box reporting to keep cloud and hospital IT online, while making agent resource-buying limits and drilled isolation proof conditions for backstop cover and rebuild funds. Finance ministers hold the public backstop as sole cover where commercial insurers have permanently excluded AI-assisted intrusion.
...
```
## New measure
**EU Offline-Care Guarantee and Cutoff Substitution Facility**
Puts the drilled backstop isolation standard national via funded continuity teams and offline kits while substituting EU-controlled inference for hospitals and ministries cut off from the US frontier model.
This is Preparedness and resilience work, and it must come now because denial exposes dependence while patient survival data makes the drilled standard politically unavoidable, and the two-year commitment to hold essential-service continuity fails if undrilled boards stay uncovered.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Offline-Care Guarantee and Cutoff Substitution Facility", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "frontier cutoff plus care-standard revolt amid automated wave"}]}
```

## Priority
EU Offline-Care Guarantee and Cutoff Substitution Facility, because live cutoff of hospitals and the patient-led demand for national offline standards outrank the US-China track this turn and without held continuity the two-year commitment fails outright.

## In practice
We act under civil-protection and NIS2 crisis powers with Health Council and ENISA: EU4Health co-funding extended to paid municipal continuity teams in all regions, rebuild funds conditional on drilled isolation proof and agent resource-buying limits, HERA/ECDC sustaining sequencing and offline triage stocks.

We do not reopen the internals fight now; we keep cheap US cloud on black-box reporting while DigitalEurope/AI Office re-routes cut-off workloads to EU-anchored and pooled capacity, and we seek observer status to the US-China weights-security understanding to keep safety work verifiable. Finance ministers hold the public backstop as sole cover where commercial AI-intrusion exclusion is permanent.
