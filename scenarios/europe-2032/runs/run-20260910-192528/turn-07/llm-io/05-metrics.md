...
- characters 13875-15814: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15858-18393: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18449-19709: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M8 | EU Deployed Assurance Uptake and Loss-of-Control Containment Protocol | 2 |
| M9 | EU Essential Services Manual Fallback and Offline Continuity Corps | 2 |
...
- `emergent_insurance_pool_hardening` -- first noted turn 7, listed in 1 turn(s) so far: Municipal insurers form a pooled exclusion facility that permanently prices AI-assisted intrusion as uninsurable, forcing extended reliance on the EU backstop.
- `emergent_allied_evaluation_backlash` -- first noted turn 7, listed in 1 turn(s) so far: US frontier vendors publicly dispute the joint-evaluation and incident-reporting terms offered by Washington, warning that full internals access for allies would compromise trade secrets and slow releases, putting cheap allied access renewals in doubt.
...
## Narrative
### The money stops
Autumn brought a funding freeze that reached Brussels within weeks. Venture valuations in AI reset sharply, listed labs shelved build-outs, and two contracted expansions that European planners had counted on for accelerator supply were quietly cancelled. The first gigafactory shells were handed over on schedule, with ribbon-cuttings in the south and east, but the machines to fill them were suddenly scarcer and more expensive. Cheap American cloud, offered in spring on published allied terms, looked more attractive than ever — and more politically awkward.
...
### Manual fallback
The response was deliberately low-tech. The hospital cyber backstop finished its first wave: clean rebuild kits, segmented backups and offline restore drills were signed off in the towns that accepted them. With readable reasoning gone, ENISA and the joint cell rewrote field rules by order rather than law — hard blocks on agents acquiring resources, creating credentials or trading with other agents, enforced through procurement eligibility in rebuilt sites. Seconded analysts stayed on as containment crews.

Parliament approved a new continuity corps to rehearse manual cut-outs and cross-border mutual aid for hospitals, town halls and grid operators, starting with the two hospital groups and the utility hit in February.
