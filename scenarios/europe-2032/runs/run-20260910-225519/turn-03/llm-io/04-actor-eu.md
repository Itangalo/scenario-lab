...
- characters 455-1641: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1685-3955: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5680-6120: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7734-10016: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_ics_toolkit_proliferation (emergent event):** Industrial-control-tuned open weights from the autumn grid intrusions are repackaged into a public toolkit, with step-by-step playbooks for grid and port systems circulating beyond the original actors.
**emergent_gigafactory_siting_backlash (emergent event):** A coordinated physical and cyber sabotage campaign targets two planned gigafactory grid-connection sites, delaying permitting and spiking local opposition to data-centre power use.
...
| M3 | EU AI Evaluation and Safety Institute | 5 | large | 1 | 4 | own jurisdiction | ai_safety up, eu_ai_sovereignty up modestly | 3 | running |
| M4 | European Chokepoint Leverage Shield | 8 | small | 2 | 4 | own jurisdiction | eu_ai_sovereignty up, eu_political_capital down short-term | 2 | running |
...
## New measure
**European Chokepoint Leverage Shield**
Creates a coordinated EU response to the extraterritorial use of its supply-chain position, conditioning ASML servicing and export decisions on Council mandate and preparing Anti-Coercion Instrument countermeasures. This is International coordination and leverage, and why now is because Washington is spending the Union's one chokepoint while private compute co-financing collapses — if the EU does not claim control over its own bottleneck now, sovereign capacity has no leverage to bargain with.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "European Chokepoint Leverage Shield", "category": 8, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up, eu_political_capital down short-term"}, "grounds": "to contest extraterritorial use of ASML and preserve EU leverage"}]}
```

## Priority
European Chokepoint Leverage Shield, because losing control of ASML to a foreign export order outranks even gigafactories — without it there is nothing to trade for model access or compute.

## In practice
We task DG TRADE and DG COMP to open an Anti-Coercion Instrument examination of the expanded US lithography controls, with a Council mandate that no further ASML export or servicing cut is implemented without joint EU authorisation, offering The Hague indemnity and EU-level compensation for lost servicing contracts. In parallel we link any future Dutch licensing to reciprocal guarantees: vetted EU evaluator access to US frontier models and security assurances for EU-hosted compute.

We keep InvestAI Gigafactories (M1) and the Tech sovereignty package (M2) alive through the investment collapse by shifting emphasis from private co-finance to EIB, Innovation Fund and redirected cohesion bridging, and by slowing site selection to match power realities in France, Germany, Spain, Sweden and Poland rather than cancelling. The Evaluation Institute (M3) is directed to add an industrial-control systems track, using the port-operator intrusion artefacts to draft mandatory segmentation and incident-reporting standards for transmission operators via ENISA and ACER, without opening a separate resilience programme this turn.
...
```
## New measure
**EU Grid and Public-Service Cyber Shield**
Deploys emergency hardening, segmentation and mutual-aid response for transmission, ports and public services hit by the automated attack and ICS toolkit. This is Preparedness and resilience, and why now is because the major incident plus open toolkit proliferation makes resilience the binding constraint on harm — sovereignty means nothing if lights and services cascade.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Grid and Public-Service Cyber Shield", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up modestly via incident learning"}, "grounds": "to absorb ongoing automated attack and ICS toolkit spread"}]}
```

## Priority
EU Grid and Public-Service Cyber Shield, because containing the ongoing cascade outranks even gigafactories — without it political capital and public tolerance for everything else collapses.

## In practice
We task ENISA with ACER and EU-CyCLONe to execute emergency segmentation, backup control and dependency-patching across transmission operators and affected municipalities, funded by reprogrammed Digital Europe and Recovery bridging, with mandatory incident reporting under NIS2 enforced now. The Evaluation Institute (M3) feeds artefacts from the attack into detection rules and adopts the safety breakthrough for monitoring, while keeping its grid track.

We hold InvestAI Gigafactories (M1) and Tech sovereignty package (M2) on public financing and grid-plausible siting, adding security perimeters and community compensation after the sabotage at two sites to keep permitting alive. The Chokepoint Shield (M4) stays as legal cover: no ASML servicing cut implemented without joint authorisation, while we seek US model access in exchange, but we spend no new capital pushing it this turn.
