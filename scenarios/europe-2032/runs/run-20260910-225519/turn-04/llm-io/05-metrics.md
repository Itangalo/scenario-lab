...
- characters 105-13684: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13727-13919: `{{metrics_json}}` from the run's live metric values
- characters 14012-15874: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15917-18297: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18353-19221: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Trustworthy Deployment Audit and Redress | 2 |
...
- `emergent_grid_connection_moratorium` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated municipal moratoriums on new data-centre grid connections spread across three member states, citing power prices and autumn outage coverage, freezing site selection independent of sabotage.
...
## Metrics
```json
{"ai_capability": 59.5, "openweight_capability": 53.5, "ai_safety": 34.0, "resilience": 34.0, "eu_ai_sovereignty": 17.0, "eu_political_capital": 5.0, "public_sentiment": 20.0}
```
...
## Notepad
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Grid and Public-Service Cyber Shield −2, EU Trustworthy Deployment Audit and Redress −2, priority −1 = −11
PROPOSAL BONUS: EU Trustworthy Deployment Audit and Redress (cat 2, small) +3 — automated_decision_scandal fired this turn and this answers it directly as enforcement breach, small size limits value
LEGITIMACY LENDS: capital 5, sentiment 20 -> +2 (included in capital calc, capped below sentiment)
