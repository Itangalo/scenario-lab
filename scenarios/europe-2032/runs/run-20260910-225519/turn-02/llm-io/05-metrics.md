...
- characters 105-13146: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13189-13381: `{{metrics_json}}` from the run's live metric values
- characters 13474-14687: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14730-16925: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16981-18018: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_ics_toolkit_proliferation` -- first noted turn 2, listed in 1 turn(s) so far: Industrial-control-tuned open weights from the autumn grid intrusions are repackaged into a public toolkit, with step-by-step playbooks for grid and port systems circulating beyond the original actors.
- `emergent_gigafactory_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: A coordinated physical and cyber sabotage campaign targets two planned gigafactory grid-connection sites, delaying permitting and spiking local opposition to data-centre power use.
