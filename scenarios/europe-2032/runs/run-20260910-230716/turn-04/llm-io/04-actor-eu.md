...
- characters 454-2328: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2371-4624: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5694-6166: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8298-10389: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_synthetic_fraud_wave (emergent event):** Coordinated AI-generated voice and video fraud wave hits several EU banks and public administrations, causing hundreds of millions in losses and forcing emergency authentication changes.
...
| M3 | EU Critical Services Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up substantially, ai_safety indirectly via incident response | 3 | running |
| M5 | EU Critical Supply Buffer for Taiwan Shock | 6 | small | 3 | 5 | own jurisdiction | resilience up modestly, eu_ai_sovereignty protected from supply shock | 2 | running |
...
| id | name | category | size | started_turn | finish_turn | applies_to | targeted_effect | cost_per_turn | status |
|---|---|---|---|---|---|---|---|---|---|
| M4 | EU Essential Services Continuity Stack | 6 | small | 2 | 4 | own jurisdiction | resilience up, eu_ai_sovereignty up modestly via demand pull | 2 | finished |
...
## New measure
**EU Critical Supply Buffer for Taiwan Shock**
Creates a pooled EU stockpile and rerouting plan for advanced chips and critical components plus emergency freight and insurance backstop so a Taiwan Strait disruption degrades rather than stops essential services and industry.
This is Preparedness and resilience work now, not new compute concrete, because the Taiwan precursor opens a three-turn window where sovereignty without stockpiles is paper; a small, fast buffer lands inside the two-year commitment to withstand coercion and absorb harm.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Critical Supply Buffer for Taiwan Shock", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up modestly, eu_ai_sovereignty protected from supply shock"}, "grounds": "Taiwan tension precursor requires pre-positioned buffer"}]}
```

## Priority
EU Critical Services Shield (M3) – because the large automated ransomware/dependency sweep still decides whether hospitals and municipalities stay running, which outranks even the Taiwan buffer this semester.

## In practice
We run M3 as the live war room through ENISA, CERT-EU and HERA: cohesion money stays retargeted to locked municipalities and hospitals, centrally procured automated patching and swarm-detection is pushed to energy, health and local government, with mandatory isolation playbooks to stop defensive shutdowns doing more damage than encryption.

We hold M1 and M2 alive without new cash: DG CNECT and ECOFIN keep guarantees on paper, tie grid-connection priority to EU-anchored sites, and court sovereign and EIB co-investors to replace withdrawn hyperscaler credits, while M4 Continuity Stack is funded via DIGITAL as paid failover on EU-hosted vetted open models for essential operators cut off by the US. The new Buffer is stood up via DG TRADE/GROW and the Emergency Instrument with member-state customs and chip buyers, pooling orders for a 6-month critical-chip reserve and pre-chartered rerouting.
...
```
## New measure
**EU Emergency Authentication Shield against Synthetic Fraud**
Deploys EU-wide verified payment and admin authentication upgrades plus bank–telecom–police fraud-intelligence sharing to stop the voice/video fraud wave and restore trust in digital services.
This is Preparedness and resilience work (category 6), not new compute, because with capital at 20 and hospitals already on degraded EU-hosted failover after the US cutoff, the fraud wave is the cascading harm we can actually stop this semester within our two-year commitment to absorb AI harm.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Emergency Authentication Shield against Synthetic Fraud", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up by restoring trust"}, "grounds": "emergent synthetic fraud wave requires immediate containment"}]}
```

## Priority
EU Critical Services Shield (M3) — because the ransomware sweep plus US model cutoff still decides whether hospitals and municipalities function, which outranks even the fraud wave and Taiwan buffer this semester.

## In practice
We run M3 as the live operations centre through ENISA, CERT-EU and HERA: cohesion funds stay diverted to locked municipalities and hospital groups, centrally procured patching and swarm-detection is pushed to energy, health and rail freight, and isolation playbooks are rewritten so defensive pull-the-plug does not close more wards than the malware itself.

We stand up the new Authentication Shield via DG FISMA, DG CNECT and Europol/EC3 with the ECB and national bank supervisors: emergency PSD2 guidance mandating liveness-checked verification and delayed settlement for high-risk voice/video-instructed transfers, a 48-hour fraud-signal sharing pact with telecoms and major banks, and DIGITAL-funded rollout of EU Digital Identity Wallet signatures for public administrations hit by impersonation. The Taiwan Buffer (M5) proceeds via DG TRADE/GROW as pooled chip orders and chartered freight options, while M1/M2 gigafactory guarantees are kept alive on paper with EIB cover but no new cash until co-investors return.
