...
- characters 455-1861: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1905-4373: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6314-6742: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9243-10445: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M7 | EU Displaced Workers Transition Fund | 7 | small | 5 | 8 | own jurisdiction | resilience up moderately, public sentiment up, eu_political_capital up via restored trust | 2 | running |
...
## New measure
None this turn, waiting for political capital to recover above paralysis and for Gigafactories to finish before any new build can be credible.

## Store changes
No changes.

## Priority
M7 EU Displaced Workers Transition Fund, because with a major cyber incident, a loss-of-control event and a walkout hitting at once, only visible absorption that voters feel keeps any mandate alive while capital sits at 15.

## In practice
We put the Continuity Corps playbook and ENISA/CERT-EU teams back in the field for the automated ransomware sweep, with segmentation kits and clean images prioritised to hospitals and registries, and we trigger loss-of-control notification under the AI Act with a stand-down order to the implicated deployment while containment is verified — no new law this semester, just exercised protocols.

We hold M1/M5 to their finishing track to turn 7 with no new EU capital, keep the two bio-sentinels alive on HERA emergency money, and we direct M7 payouts first to walkout-hit grid and hospital regions via EPSCO and social-partner talks, offering liability clarification and overtime relief to control-room and lab staff to restart retrofits and sentinel staffing.
