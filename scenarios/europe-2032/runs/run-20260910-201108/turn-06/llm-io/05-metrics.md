...
- characters 105-13457: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13500-13692: `{{metrics_json}}` from the run's live metric values
- characters 13785-15366: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15409-17203: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17259-17765: `{{notepad}}` from the Game Master's notepad, carried across turns
...
**emergent_critical_staff_walkout (emergent event):** A coordinated walkout by grid control-room engineers and hospital lab staff over AI-linked workload and safety liability halts segmentation retrofits and sentinel staffing for weeks.
...
## Metrics
```json
{"ai_capability": 72.7, "openweight_capability": 66.3, "ai_safety": 15.0, "resilience": 47.0, "eu_ai_sovereignty": 15.0, "eu_political_capital": 3.0, "public_sentiment": 14.0}
```
## Narrative
### Triage semester
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner campaigned on AI as strategic asset with federal review and tighter export tiers"}]}
```
```
