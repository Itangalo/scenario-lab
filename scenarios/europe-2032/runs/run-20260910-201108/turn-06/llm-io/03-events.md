...
- characters 376-1957: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2000-3794: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3850-4940: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5003-33630: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 33985-34413: `{{event_history}}` from the run's own event record
...
- `emergent_critical_staff_walkout` -- first noted turn 4, listed in 2 turn(s) so far: A coordinated walkout by grid control-room engineers and hospital lab staff over AI-linked workload and safety liability halts segmentation retrofits and sentinel staffing for weeks.
- `emergent_offshore_synthesis_grey_market` -- first noted turn 4, listed in 2 turn(s) so far: Benchtop synthesis providers outside the EU begin advertising no-questions-asked oligo pools marketed to European buyers, exploiting pilot-only screening and idle sentinel staffing.
...
- Turn 2 (4 turn(s) ago): bio_uplift_findings, member_state_defection, emergent_datacentre_siting_backlash
- Turn 3 (3 turn(s) ago): eval_anomaly_reports, taiwan_tension_rise, export_control_escalation
- Turn 4 (2 turn(s) ago): cyber_major_incident, capability_jump, eval_anomaly_reports, automated_decision_scandal
- Turn 5 (1 turn(s) ago): eval_anomaly_reports, election_consolidation
