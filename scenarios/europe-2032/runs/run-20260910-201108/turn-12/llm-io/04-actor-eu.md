...
- characters 451-1593: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1636-3354: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4640-5076: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8015-9378: `{{previous_actions}}` from the actor's own response from the previous turn
...
## New measure
None this turn, while we conserve collapsing capital until the Rationing and Repair Corps finishes landing.

## Store changes
No changes.

## Priority
M10 EU Wartime Inference Rationing and Repair Corps, because with cables dark, US models revoked and extortion in hospitals and banks, rationed inference and physical repair remain the only line holding essentials alive.

## In practice
We execute M10 through HERA, ENISA and EuroHPC JU under health-emergency and Critical Entities Resilience cover: pooled inference stays restricted to emergency care, payroll and interbank settlement, with police, civil protection and seconded grid crews posted at EuroHPC sites, substations, fibre landings and ground stations. The rushed defensive patching stack goes first to health, power and payments where self-repairing footholds reappear after takedowns.

On ASML coercion we mount no new instrument this semester we cannot fund or enforce. DG TRADE and the Dutch government seek quiet licence/queue and servicing carve-outs for installed EU base and existing spares, and we fold the demand into the middle-power coordination framework for joint bargaining, without a public Anti-Coercion confrontation that would spend our last capital and risk company survival. Concrete without machines stays guarded, not connected; no new siting fights are opened.
