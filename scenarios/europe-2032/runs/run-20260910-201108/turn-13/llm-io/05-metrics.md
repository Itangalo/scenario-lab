...
- characters 107-13059: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13102-13290: `{{metrics_json}}` from the run's live metric values
- characters 13383-14737: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14781-16719: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16775-17984: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M12 | EU Synthesis Screening and Bio-Response Continuity Shield | 2 |
...
- `emergent_escrow_generic_bridge` -- first noted turn 13, listed in 1 turn(s) so far: A Gulf-state backed generic-drug consortium offers at-cost synthesis of US-designed tailored therapies under Swiss escrow inspection, breaking the cold-chain bottleneck for EU clinics without US compute access.
- `emergent_swiss_medical_escrow_channel` -- first noted turn 13, listed in 1 turn(s) so far: Swiss-mediated escrow for hospital compute and cold-chain spares unexpectedly clears legal review, opening a narrow neutral channel for diagnostics and therapy planning outside US-China controls.
