...
- characters 452-1806: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1850-3788: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4362-4798: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8070-10397: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential services alive through war and loss of control
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Humanitarian Clinical Access and Embodied-Work Continuity Bridge | 6 | small | 12 | 14 | own jurisdiction | resilience up moderately, public_sentiment up | 2 | running |
...
| M3 | EU Critical Systems Cyber Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Synthesis Screening and Bio-Detection Bridge | 6 | small | 2 | 5 | own jurisdiction | resilience up moderately, ai_safety up slightly via misuse reduction | 2 | finished |
| M5 | EU Frontier Evaluation Access Taskforce | 5 | small | 3 | 7 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via evaluation leverage | 2 | finished |
| M6 | EU Public Services Continuity Corps | 6 | small | 4 | 5 | own jurisdiction | resilience up substantially to absorb cyber incident | 2 | finished |
| M7 | EU Displaced Workers Transition Fund | 7 | small | 5 | 8 | own jurisdiction | resilience up moderately, public sentiment up, eu_political_capital up via restored trust | 2 | finished |
| M8 | EU Lithography Shield and Sovereign Inference Reserve | 4 | small | 7 | 10 | own jurisdiction | eu_ai_sovereignty up moderately, resilience up slightly | 2 | finished |
| M9 | EU Essential Inference Continuity and Infrastructure Guard | 6 | small | 8 | 10 | own jurisdiction | resilience up moderately, eu_ai_sovereignty up slightly via protected usable capacity | 2 | finished |
| M10 | EU Wartime Inference Rationing and Repair Corps | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, eu_ai_sovereignty up slightly via usable protected capacity | 2 | finished |
...
## New measure
**EU Humanitarian Clinical Access and Embodied-Work Continuity Bridge**
Deploys the arriving tailored therapies through the rationing corps and neutral escrow channels while extending continuity protection to care, logistics and industrial maintenance crews facing embodied AI displacement. Category 6 Preparedness and resilience – why this and why now: with services degraded-alive, sentiment at 1 and capital at 7, the medical breakthrough is the only event that can lift public tolerance for continued rationing, but only if it reaches ordinary clinics on EU terms rather than as another demonstration of dependence, and robots arriving in the same semester mean no time to separate health continuity from work continuity.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Humanitarian Clinical Access and Embodied-Work Continuity Bridge", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up"}, "grounds": "to land medical breakthrough via rationing corps while buffering embodied displacement"}]}
```

## Priority
M11 EU Humanitarian Clinical Access and Embodied-Work Continuity Bridge, because keeping essentials alive now depends on turning the one positive shock into delivered care rather than another dependence lesson.

## In practice
We execute through HERA, EMA emergency use, and the EuroHPC/Essential Inference Continuity structures already guarding triage, payroll and settlement: pooled inference slots are re-weighted to diagnostics and tailored-therapy planning, ENISA-hardened hospital pathways get first spares from the Swiss-escrow track DG TRADE is now asked to fund and formalise, and civil-protection repair crews escort cold-chain and maintenance.

On embodied AI we do not open a new industrial programme we cannot fund. We attach logistics, hospital-porter, mine-clearance and facility-maintenance roles to the existing Transition Fund delivery chain and EPSCO social-partner agreements, with procurement guidance favouring repairable, EU-serviceable machines and retraining tied to care and grid-repair corps, while tabling armed-logistics concerns in the middle-power forum without spending our last capital on a treaty we cannot enforce.
...
```
## New measure
**EU Synthesis Screening and Bio-Response Continuity Shield**
Tightens synthesis screening, hospital bio-detection and medical continuity protocols against AI-enabled bioweapon designs now credibly in reach. Category 6 Preparedness and resilience – why this and why now: the genome-model finding opens a four-turn bio gate while hospitals are already degraded-alive and dependent on foreign compute, so the only affordable move under the keep-essentials-alive commitment is to make screening, detection and continuity catch what safety and sovereignty cannot stop.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Synthesis Screening and Bio-Response Continuity Shield", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "to close bio misuse path opened by genome-model uplift finding"}]}
```

## Priority
M11 EU Humanitarian Clinical Access and Embodied-Work Continuity Bridge, because with capital at 5 and sentiment at 1 only delivered cures and protected care jobs keep rationing politically survivable while the bio shield builds.

## In practice
We run M11 to completion through HERA/EMA and civil-protection escorts: pooled EuroHPC slots stay on diagnostics and therapy planning, Swiss-escrow spares go to hardened hospital pathways first, and procurement keeps favouring repairable EU-serviceable robots with retraining into care and grid-repair crews.

We stand up the bio shield on the same corps without a new bureaucracy: HERA plus ECDC and ENISA extend the M4 screening bridge to synthesis providers and hospital labs, add rapid sequencing and isolation drills in rationed hospitals, and pre-position PPE, antivirals and continuity rosters via the Union Civil Protection Mechanism. Health Council QMV and HERA emergency funds carry it; industry accepts mandatory screening in exchange for continued access to pooled inference and repair escorts.
