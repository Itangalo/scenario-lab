...
- characters 377-1530: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1574-3616: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3672-4825: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4888-29115: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29470-29579: `{{event_history}}` from the run's own event record
...
- `emergent_gigafactory_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: A coordinated physical protest campaign blocks construction access at two announced gigafactory sites, delaying permits and forcing a national debate over power and water use.
- `emergent_attribution_split` -- first noted turn 2, listed in 1 turn(s) so far: Attribution reporting on the autumn grid intrusions fragments publicly, with rival commercial threat-intelligence claims blaming different state sponsors and EU governments unable to present a joint attribution statement.
...
- Turn 2 (1 turn(s) ago): cyber_major_incident, taiwan_tension_rise
