...
- characters 106-13660: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13703-13895: `{{metrics_json}}` from the run's live metric values
- characters 13988-15219: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15263-17204: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17260-18193: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_infrastructure_permitting_revolt` -- first noted turn 5, listed in 1 turn(s) so far: A coordinated siting and grid-connection revolt spreads beyond the two frozen Gigafactory sites, with regional courts granting interim injunctions on water and power permits that force a national-level renegotiation of AI infrastructure hosting terms.
...
## Notepad
PORTFOLIO CHARGE t5: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M5 EU Assurance and Anomaly Triage Cell −2, priority −1 = −9
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 1, sentiment 23 -> +2
SOVEREIGNTY: 18 last turn, no capacity build finishing, supply_chain_coercion t5 −2, Gigafactories in flight +0, capability rose 0.5 −0 = 16
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "pending"}}, {"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 9}, "grounds": "event: permitting injunctions and audit freeze stall construction"}]}
```
```
