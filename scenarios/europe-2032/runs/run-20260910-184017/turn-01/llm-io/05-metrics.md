...
- characters 16367-17015: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_kimi_crime_wave` -- first noted turn 1, listed in 1 turn(s) so far: Criminal groups widely distribute ready-made phishing and intrusion kits built on Kimi K3, causing a sharp fraud and SME ransomware wave across Europe that is attributed to open-model proliferation rather than a state actor.
- `emergent_medical_fraud_wave` -- first noted turn 1, listed in 1 turn(s) so far: Major European research hospitals publicly report a wave of AI-generated insurance fraud and forged referrals exploiting clinical workflows built on US frontier models, triggering a cross-border health-data integrity review.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 46.0, "ai_safety": 32.0, "resilience": 38.0, "eu_ai_sovereignty": 21.0, "eu_political_capital": 37.0, "public_sentiment": 40.0}
```
...
PROPOSAL BONUS: EU Critical Systems Shield (cat 6, large) +3 — autumn cross-sector intrusion with undetected dwell in two EU grids landed this turn and this answers it directly
LEGITIMACY LENDS: none — capital 37, sentiment 40 comparison after other terms but rule requires sentiment above capital before charge; at start capital 48 above sentiment 42 so no lend
SOVEREIGNTY: 22 last turn, no capacity event, Gigafactories in flight +0, capability rose 3.0 −1 = 21
Emerging developments (tracked): emergent_kimi_crime_wave faint phishing-kit rumours; emergent_medical_fraud_wave single-source hospital fraud reports.
