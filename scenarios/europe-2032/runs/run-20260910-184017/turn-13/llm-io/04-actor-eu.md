...
- characters 454-1636: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1680-3647: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4197-4685: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8641-10886: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Hold the line through superhuman diffusion — contain agentic harm, harden essentials, and make safety certifiable
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | SME Fraud and Ransomware Triage | 6 | small | 2 | 4 | own jurisdiction | resilience up moderately, public_sentiment up slightly via visible response | 2 | finished |
| M5 | EU Displacement Transition Fund | 7 | small | 3 | 6 | own jurisdiction | resilience up moderately, public_sentiment up slightly, eu_political_capital protected | 2 | finished |
| M6 | EU Evaluation and Assurance Cell | 5 | small | 4 | 5 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via own assurance capacity | 2 | finished |
| M7 | EU Critical Dependency Patching and Recovery Reserve | 6 | small | 5 | 6 | own jurisdiction | resilience up substantially, public_sentiment up slightly via visible recovery | 2 | finished |
| M8 | EU Upstream Leverage and Secure Compute Compact | 8 | small | 6 | 9 | coalition | eu_ai_sovereignty up moderately, eu_political_capital protected, ai_safety up slightly via conditioned access | 2 | finished |
| M9 | EU Joint Threat Shield Accession | 6 | small | 7 | 9 | coalition | resilience up moderately, ai_safety up slightly via shared telemetry and verification | 2 | finished |
| M10 | EU Containment and Lights-On Protocol | 6 | small | 8 | 9 | own jurisdiction | resilience up moderately, public_sentiment up slightly, ai_safety up slightly via containment learning | 2 | finished |
| M11 | EU Certified Containment Retrofit | 5 | small | 9 | 10 | own jurisdiction | ai_safety up substantially, resilience up moderately, public_sentiment up slightly via visible control | 2 | finished |
| M12 | EU Wartime Continuity and Sovereign Fallback | 6 | small | 10 | 11 | own jurisdiction | resilience up substantially, public_sentiment up slightly via kept services, eu_political_capital protected | 2 | finished |
| M13 | EU Shielded Continuity and Certified Medical Uptake | 6 | small | 11 | 12 | own jurisdiction | resilience up substantially, public_sentiment up moderately via kept services and medical benefit, ai_safety up slightly via monitor gate | 2 | finished |
| M14 | EU Certified Benefit Scale-Up and Anti-Carve-Out Enforcement | 9 | small | 12 | 13 | own jurisdiction | public_sentiment up moderately via visible medical and service benefit, resilience up moderately via EU-hosted deployment, eu_political_capital protected via enforcement | 2 | finished |
...
## New measure
**EU Certified Benefit Scale-Up and Anti-Carve-Out Enforcement**
Puts the visible adoption win — shorter waits and tailored therapies delivered on EU-hosted vetted systems — into ordinary clinics and battery pilots under EU certification, while blocking member-state side-deals that strip monitoring and reserves.
This is a Diffusion, adoption and public trust move now because with frontier access cut, open weights unrecallable, and a member state undercutting the line, the only way our hold-the-line commitment survives is to make European benefit attributable and defection costly within this half-year.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Certified Benefit Scale-Up and Anti-Carve-Out Enforcement", "category": 9, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately via visible medical and service benefit, resilience up moderately via EU-hosted deployment, eu_political_capital protected via enforcement"}, "grounds": "adoption win to bank, defection to contain, breakthrough to scale under EU monitors"}]}
```

## Priority
EU Certified Benefit Scale-Up and Anti-Carve-Out Enforcement, because without a certified, attributable benefit and a held line against carve-outs nothing else in containment-hardening-certification holds.

## In practice
We run scale-up through EMA fast-track, EHDS sandboxes and EU4Health procurement: only therapies and diagnostics runnable on EU-hosted vetted open systems gated by certified monitors qualify for reimbursement, with the new interpretability checks as condition. The battery formation advance goes to Innovation Fund pilots tied to EU siting and servicing-pool commitments.

We enforce the line through the Anti-Coercion Instrument and FDI screening formation: distressed power/cloud sales with monitoring stripped are suspended for review, continuity reserves get right of first refusal, and the defecting side-arrangement is brought to the European Council with infringement exposure and loss of continuity rationing priority. DG COMP and ENISA publish degradation attribution to war and cutoff to shield sentiment while operators get relief crews.
...
```
## New measure
**EU Automated Attack Containment and Essential Service Restoration**
Restores telecoms, energy controls and public services hit by the large automated model-generated attack while hardening them against the next wave on isolated vetted systems.
This is a Preparedness and resilience move now because with safety at 6, open capability at 87 and defenders visibly behind, the hold-the-line commitment lives or dies on absorbing the incident publicly and stopping cascade within this half-year.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Automated Attack Containment and Essential Service Restoration", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially via restoration and hardening, public_sentiment up slightly via kept services, ai_safety up slightly via incident learning"}, "grounds": "answer major automated attack and hold essentials"}]}
```

## Priority
EU Automated Attack Containment and Essential Service Restoration, because nothing in containment-hardening-certification holds if the current attack cascades while we argue about carve-outs.

## In practice
We trigger the Critical Systems Shield playbook and the Wartime Continuity reserves: ENISA and EU-CyCLONe coordinate triage, CERT-EU pushes vetted patches and clean images for the compromised dependency, and transmission operators island energy controls and telecom cores onto pre-certified monitors with rationed spares. Recovery is funded by reprogrammed EU4Health and Digital Europe envelopes, with relief crews to exhausted operators.

We pair restoration with enforcement: distressed-asset sale suspension stays, the side-deal sites face infringement exposure and loss of rationing priority unless they re-accept EU monitoring, and attribution and lessons feed the Evaluation Cell to tighten certifiable safety gates. Council JHA and Energy formations carry the mandate, with Health ministers holding clinic continuity as the visible proof.
