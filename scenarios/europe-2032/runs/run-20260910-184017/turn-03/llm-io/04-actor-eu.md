...
- characters 1713-3748: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4201-4652: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6614-8513: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | SME Fraud and Ransomware Triage | 6 | small | 2 | 4 | own jurisdiction | resilience up moderately, public_sentiment up slightly via visible response | 2 | running |
...
## New measure
**SME Fraud and Ransomware Triage**
Stands up rapid triage, clean backups and takedown coordination for SMEs and hospitals hit by the Kimi-K3 crime wave, using the Shield's joint cell.
Category 6 Preparedness and resilience is why now: open-weight kits are causing a sharp, attributable fraud and extortion wave this turn, and withstanding shocks means containing it before trust in digital services collapses.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "SME Fraud and Ransomware Triage", "category": 6, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up slightly via visible response"}, "grounds": "Kimi K3 crime wave hitting SMEs and hospitals"}]}
```

## Priority
EU Critical Systems Shield, because undetected dwell in transmission and ports still risks cascading winter failure that a triage line alone cannot stop.

## In practice
We push the Shield through ENISA, ENTSO-E and the Interior/Energy Councils: monitoring baselines live in the two compromised grids, segmented backups funded from repurposed Digital Europe/CEF, mandatory reporting into the joint cell enforced by co-funding sensors, with the December exercise expanded to five grids. Operators resisting downtime get compensation for test windows; port and water authorities are folded into the same cell.

We launch the Triage as a fast small add-on, not a second programme: Europol EC3, ENISA and national CSIRTs run a single reporting hotline, pre-positioned clean-backup kits and coordinated takedowns of phishing infrastructure, funded by reallocated SME digital funds. Hospitals get prioritised referral-verification fixes. Gigafactories and Tech sovereignty package continue via DG CNECT/EIB site and grid-connection work, but without new money this turn to preserve capital.
...
```
## New measure
**EU Displacement Transition Fund**
Creates a rapid EU-cofunded wage-bridge, retraining voucher and employer-transition levy pilot for knowledge-work occupations where augmentation is tipping into displacement, run through national employment agencies.
Category 7 Labour and social protection is why now: early productivity-without-job-loss studies are already stale, fraud-driven distrust plus looming junior-work displacement risks turning adjustment into shock, and withstanding shocks requires income and retraining capacity before layoffs hit local news.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Displacement Transition Fund", "category": 7, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up slightly, eu_political_capital protected"}, "grounds": "augmentation stage giving way to displacement amid fraud wave"}]}
```

## Priority
SME Fraud and Ransomware Triage, because mirrors-reappearing phishing and paper-fallback hospitals are the active cascading trust failure this turn that outranks even our own build direction.

## In practice
We keep the Shield in sustainment via ENISA and the joint cell: mandatory reporting stays, baselines in the two intruded grids are hardened, and we trade co-funded sensors and test-window compensation for operator agreement to the slipped spring five-grid exercise, while water segmented backups get Digital Europe top-up. The Triage gets the surge: EC3-led hotline and takedowns, clean-backup kits pushed to micro-firms and three-region hospital referral fixes unblocked via emergency procurement waiver and EHDS derogation.

We seed the Transition Fund without new legislation this turn: ESF+ reallocation plus EIB guarantee for vouchers and wage-bridge in France, Germany, Netherlands, Spain hit-sectors, negotiated in EPSCO with employer levy design to start next turn. Gigafactories and Tech sovereignty package are held to DG CNECT siting, permitting-zone and grid-connection milestones only, no new cash, to stay within capital.
