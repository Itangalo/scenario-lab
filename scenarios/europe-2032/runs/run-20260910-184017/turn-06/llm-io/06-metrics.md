...
- characters 105-13796: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13839-14031: `{{metrics_json}}` from the run's live metric values
- characters 14124-15467: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15510-17687: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17743-18282: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "ALLIANCE"}, "grounds": "turn 5 winner was coalition-minded president promising structured allied access"}]}
```
```
