...
- characters 455-1535: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1579-3811: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4518-4958: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6570-8450: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Grid and Critical Services Hardening Sprint | 6 | large | 2 | 4 | own jurisdiction | resilience up strongly, ai_safety up modestly | 3 | running |
...
```
## New measure
None this turn, while we wait for the Hardening Sprint rollout and Evaluation Institute staffing to clear certification and free capital.

## Store changes
No changes.

## Priority
M4 EU Grid and Critical Services Hardening Sprint, because open-weight frontier parity makes mapped grids immediately exploitable and defence cannot wait for factories.

## In practice
We push ENISA/ACER under NIS2/CER to certify the US/EU patching and swarm-detection stack this turn, directing reprogrammed RRF/Cohesion advances plus EIB bridge cover to the two hit TSOs, hit port and water utility first. Energy Council locks host-state linkage: Gigafactory power pledges move only alongside grid integration commitments, to break the Paris-Berlin-Madrid-Stockholm-Warsaw stall.

We hold InvestAI Gigafactories (M1) and Tech sovereignty package (M2) in site/power negotiation via DG CNECT and permitting zones without new money, and we finish the Evaluation Institute (M3) first cohort hiring with AI Office/JRC to handle open-weight incident triage. Taiwan contingency stays in trade/shipping liaison, lengthening chip-stock reporting, not a separate build.
