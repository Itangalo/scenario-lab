...
- characters 454-1680: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1723-3605: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3924-4395: `{{statement_ledger}}` from the actor's live statement ledger
- characters 5845-8133: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_research_provenance_pact (emergent event):** Major EU scientific publishers and universities jointly mandate provenance watermarking and AI-use disclosure for submitted papers after Millennium Prize proof controversy.
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up, ai_safety up via incident reporting | 3 | running |
...
## Two-year commitment
Keep essential European services running on controllable AI and able to absorb AI-enabled attack

## Statement changes
add `two_year_commitment` (commitment): Keep essential European services running on controllable AI and able to absorb AI-enabled attack

## New measure
**EU Critical Systems Shield**
Hardens electricity transmission, ports and water utilities against AI-enabled swarm intrusion with segmented OT, exercised cross-border contingency plans and mutual aid.
Category 6 Preparedness and resilience. Why this and why now: the autumn test shot proved reachable infrastructure is already mapped with Mythos-class open tooling, and the US switch-off proved dependence fails at the same time resilience is needed.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Critical Systems Shield", "category": 6, "size": "large", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up via incident reporting"}, "grounds": "responding to cross-operator test shot precursor"}]}
```

## Priority
EU Critical Systems Shield, because staged breaker credentials with nothing actuated means the next turn is the window to degrade rather than stop an attack, which outranks even gigafactory build-out.

## In practice
We task ENISA and the Electricity Coordination Group under the NIS2 Article 12 crisis framework, with CER Directive obligations as legal base, to force OT segmentation audits and breaker-credential rotation across the two affected TSOs first, then all ENTSO-E members, funded by reprogrammed Connecting Europe Facility and InvestAI resilience envelope. Interior and Energy Councils carry it; grid operators resisting downtime get cost coverage and liability cover for precautionary islanding.

In parallel we keep M1 Gigafactories and M2 sovereignty package moving through accelerated-permitting zones and EIB guarantees, explicitly directing first sovereign inference capacity to hospitals, ministries and TSO control rooms hit by the US nationality cut-off, so dependence is not rebuilt while we harden. The Commission pairs this with a demarche to Washington demanding published criteria and appeal for any frontier cutoff, without pausing the hardening work.
...
```
## New measure
**EU Provenance Clearinghouse**
Stands up an EU-wide verification registry for AI-use disclosure and provenance watermarking in science and public services, building on the publishers' pact to catch disputed proofs and manipulated evidence.
Why this and why now: the Millennium proof controversy shows trust in shared knowledge failing just as hospitals and grids depend on AI outputs, and a cheap registry locks in the emergent pact before it fragments.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Provenance Clearinghouse", "category": 2, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, ai_safety up slightly via incident reporting"}, "grounds": "locking in emergent provenance pact"}]}
```

## Priority
M3 EU Critical Systems Shield, because mapped breaker credentials with tooling left in place means this is the last hardening window before degradation, which outranks even the useful provenance win.

## In practice
We drive M3 through ENISA and the Electricity Coordination Group under NIS2 Article 12, forcing credential rotation and OT segmentation audits on the two hit TSOs first, with reprogrammed CEF money covering downtime and liability for precautionary islanding. Energy and Interior Councils carry the fights with operators over outages, and we push the slipped cross-border exercise back into this autumn's calendar.

We keep M1 Gigafactories and M2 sovereignty package on permitting zones and EIB guarantees, ring-fencing first sovereign inference for the clinics, ministries and control rooms darkened by the US cutoff so dependence is not rebuilt. In parallel we task the EU Publications Office and ENISA with the new Clearinghouse, using the publishers-universities pact as voluntary base and Horizon grant conditions as lever, to provide common watermark verification and disclosure standards for journals and hospital pilots.
