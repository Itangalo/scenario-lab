...
- characters 13248-14474: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14517-16399: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16455-17512: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Provenance Clearinghouse | 2 |
...
- `emergent_open_model_clinical_compromise` -- first noted turn 1, listed in 2 turn(s) so far: Major European research hospitals publish evidence that clinical decision-support built on Kimi K3-derived models was manipulated via poisoned fine-tuning data, triggering an EU-wide review of open-model use in healthcare.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 37.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 24.0, "public_sentiment": 37.0}
```
