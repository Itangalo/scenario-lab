...
- characters 375-2153: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2196-4535: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4591-5365: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5428-25949: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 26304-26745: `{{event_history}}` from the run's own event record
...
- `emergent_municipal_ai_moratorium` -- first noted turn 5, listed in 1 turn(s) so far: Municipal utilities and hospital networks in two member states suspend new AI procurement and disconnect automated triage tools after the welfare and ransomware scandals, creating a de facto local moratorium that spreads via city networks.
...
- Turn 2 (4 turn(s) ago): emergent_research_provenance_pact
- Turn 3 (3 turn(s) ago): knowledge_work_augmented, supply_chain_coercion, member_state_defection
- Turn 4 (2 turn(s) ago): cyber_major_incident, taiwan_blockade, ai_investment_collapse, automated_decision_scandal
- Turn 5 (1 turn(s) ago): bio_uplift_findings, openweight_frontier_release, election_consolidation
