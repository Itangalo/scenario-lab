...
- characters 1743-3769: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4209-4685: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6666-8485: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M3 | EU Critical Infrastructure Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident reporting | 3 | running |
...
## New measure
**EU Continuity Fallback for Critical AI Dependencies**
It stands up an emergency switch for hospitals, ministries and operators cut off from the leading foreign model to EU-hosted open-weight and allied models with shared inference capacity and procurement waivers.
This and now because the cutoff turns latent dependence into operational failure this turn, and without a visible fallback the outrage reads as EU foresight failure and breaks Shield delivery credibility.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Continuity Fallback for Critical AI Dependencies", "category": 9, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, eu_ai_sovereignty up slightly via fallback capacity"}, "grounds": "frontier access denied this turn"}]}
```

## Priority
EU Continuity Fallback for Critical AI Dependencies, because restoring hospitals, ministries and firms cut off this turn outranks even the Shield until continuity is shown.

## In practice
We task DG CNECT, the AI Office and HERA with a 72-hour mapping of cut-off dependencies via NIS2 contact points, then fund fallback inference on EuroHPC and Gigafactory pilot capacity plus allied commercial clouds under Article 122 emergency procurement and reprogrammed Digital Europe funds. Health and interior ministers in the EPSCO/JHA formations are offered cost-cover for re-integration, not new rules.

M3 Shield continues on its ENISA-ACER detection baseline track with audited co-financing, but its staff and money are second call after fallback teams this turn. M1/M2 stay on permitting and grid-queue mediation with EIB, with site decisions explicitly tied to proving domestic inference can carry critical loads when foreign access is withdrawn.
...
```
## New measure
**EU Open-Weight Surge Shield Extension**
It hardens hospitals, grids, ports and municipal IT against the cyber and fraud tooling the new open-weight frontier puts permanently on private hardware, via rapid patching, detection signatures and mutual-aid playbooks.
This and why now because the release puts ~52-level capability beyond recall within months, and with sovereignty at 20 and Shield teams diverted to fallback duty, distributed misuse is the shock our two-year resilience commitment must absorb first.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Open-Weight Surge Shield Extension", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via misuse detection"}, "grounds": "open-weight release within 5-10pts of frontier this turn"}]}
```

## Priority
M3 EU Critical Infrastructure Shield, because fallback continuity held but Shield baselines slipped and the open-weight surge now tests exactly the grid-port-health detection it was built for.

## In practice
We keep DG CNECT and ENISA on Shield delivery as first call again: restore seconded detection teams from fallback duty, use NIS2 implementing-act money and co-financing to force sensor installs in lagging small ports, and run the joint grid-port exercises deferred from spring. The new Extension is run as a Shield work-package, not a parallel bureaucracy — EU-CyCLONe distributes model-abuse signatures, HERA/ECDC adds bio-screening guidance for synthesis providers.

M1/M2 stay alive on a narrow track: EIB and DG ENER mediate grid connections only for pilot sites that prove they can carry critical inference loads, unblocking the conditionality that stalled permitting. We ask Health and JHA Councils for staff retraining cover for the clumsier EU-hosted fallback, in exchange for holding audit and reporting obligations through the surge.
