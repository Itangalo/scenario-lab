...
- characters 377-2073: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2117-3914: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3970-5047: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5110-27944: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28299-28560: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t4: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M3 EU Critical Infrastructure Shield −3, M5 EU Open-Weight Surge Shield Extension −2, priority −1 = −12
PROPOSAL BONUS: EU Open-Weight Surge Shield Extension (cat 6, small) +2 — openweight frontier release landed this turn and this answers it directly
...
- `emergent_datacentre_siting_backlash` -- first noted turn 4, listed in 1 turn(s) so far: Municipal protests and grid-operator lawsuits stall several EU gigafactory and data-centre sites over power prices and water use, forcing a temporary permitting freeze in one member state.
- `emergent_datacentre_power_backlash` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated local protests and grid-operator curtailments force a temporary moratorium on new data-centre connections in two EU member states, delaying EuroHPC and gigafactory ramp-up.
...
- Turn 1 (4 turn(s) ago): cyber_test_shot, safety_breakthrough, knowledge_work_augmented
- Turn 2 (3 turn(s) ago): safety_breakthrough, taiwan_tension_rise
- Turn 3 (2 turn(s) ago): eu_frontier_access_denied
- Turn 4 (1 turn(s) ago): openweight_frontier_release
