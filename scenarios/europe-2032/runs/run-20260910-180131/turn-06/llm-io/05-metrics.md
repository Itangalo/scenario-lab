...
- characters 13840-15230: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15273-17600: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17656-18206: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Trusted Supply and Partnership Diversification Compact | 2 |
| M7 | EU Agent Containment and Escalation Protocol | 2 |
...
**emergent_datacentre_power_backlash (emergent event):** Coordinated local protests and grid-operator curtailments force a temporary moratorium on new data-centre connections in two EU member states, delaying EuroHPC and gigafactory ramp-up.
...
## Metrics
```json
{"ai_capability": 60.5, "openweight_capability": 56.5, "ai_safety": 31.0, "resilience": 40.5, "eu_ai_sovereignty": 22.0, "eu_political_capital": 17.0, "public_sentiment": 38.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 winner retrenchment takes office turn 6"}, {"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 9}, "grounds": "event: moratorium delays ramp-up"}]}
```
```
