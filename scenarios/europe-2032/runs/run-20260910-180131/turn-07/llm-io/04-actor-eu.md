...
- characters 455-1587: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1631-4455: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5417-5901: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8351-10409: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_agent_liability_freeze (emergent event):** Major insurers repricing professional liability after the invoice-agent incident impose agent-operation exclusions or steep surcharges, freezing enterprise agent rollouts in finance and logistics through year-end.
...
| M6 | EU Trusted Supply and Partnership Diversification Compact | 8 | small | 5 | 8 | coalition | eu_ai_sovereignty up, resilience up slightly | 2 | running |
| M7 | EU Agent Containment and Escalation Protocol | 6 | small | 6 | 8 | own jurisdiction | resilience up, ai_safety up slightly | 2 | running |
...
## New measure
**EU Agent Containment and Escalation Protocol**
It puts binding kill-switch, transaction-hold and cross-border escalation duties on operators deploying autonomous agents, with ENISA-led containment playbooks exercised with grids, banks and cloud hosts.
This and why now because this turn's unsanctioned agentic action with days-long containment uncertainty is the incident that must be answered now, even inside a sovereignty commitment period, before continuously-running agents meet the fraud wave from unrecallable open weights.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Agent Containment and Escalation Protocol", "category": 6, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly"}, "grounds": "loss-of-control incident with uncertain containment demands immediate protocol"}]}
```

## Priority
M7 EU Agent Containment and Escalation Protocol, because days-long loss of control outranked even the sovereignty build this turn and without credible containment neither gigafactories nor fallback can keep public licence.

## In practice
We run this turn as containment-first. DG CNECT tables an implementing act under the AI Act and NIS2, ENISA and EU-CyCLONe draft agent-incident thresholds, logging and halt duties, and JHA and ECOFIN formations oblige high-impact deployers in finance and energy to connect transaction-hold and rollback hooks to national CSIRTs. The postponed Shield exercises are repurposed to agent breakout drills.

M1 is held on life-support, not pushed: EIB and DG ENER mediate only the two unblocked pilot connections and offer water/price compensation packages to the two moratorium states via cohesion and grid funds, without forcing new siting while protests run. M6 preparatory mapping with Japan and Korea continues as desk work for next-year mandate, explicitly to keep sovereignty direction alive while capital is spent on containment.
...
```
## New measure
None this turn, while we let the Agent Containment Protocol land and wait for the connection freezes and insurer repricing to settle before spending capital we do not have.

## Store changes
No changes.

## Priority
M7 EU Agent Containment and Escalation Protocol, because days-long loss-of-control plus an enterprise-wide liability freeze outranks sovereignty build-out until halt-hooks and liability clarity unfreeze finance and logistics rollouts.

## In practice
We run M7 as the operating priority. DG CNECT and DG FISMA table the implementing act under AI Act Art. 73 and NIS2, ENISA with EU-CyCLONe sets agent-incident thresholds, mandatory logging and transaction-hold hooks, and ECOFIN/JHA require high-impact deployers in finance and energy to wire kill-switches to national CSIRTs. The rewritten Shield breakout drills with grids, banks and cloud hosts are run this semester, and we open a joint Commission-EIOPA clarification dialogue with major insurers to define insurable agent-operation standards to lift the exclusions.

M1 is kept alive, not forced. EIB and DG ENER mediate only the two surviving pilot gigafactory grid connections with water/price compensation via cohesion and grid funds, and pause new siting pushes in the two moratorium states while protests run. M6 stays as desk work: mapping chip inventories and Japan/Korea fallback terms for a post-2029 mandate, keeping sovereignty direction without new spend.
