...
- characters 455-1354: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1397-3203: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5581-6040: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8949-10929: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_dutch_lithography_standoff (emergent event):** The Dutch government publicly refuses a US demand to suspend servicing of older lithography tools already installed abroad, triggering an open transatlantic dispute over extraterritorial export controls and a retaliatory US licensing threat.
...
- `two_year_commitment` (commitment): Harden essential services and democratic trust to absorb unrecallable open-weight AI
...
| M3 | EU Critical Infrastructure Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident reporting | 3 | finished |
| M4 | EU Continuity Fallback for Critical AI Dependencies | 9 | small | 3 | 4 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly via fallback capacity | 2 | finished |
| M5 | EU Open-Weight Surge Shield Extension | 6 | small | 4 | 6 | own jurisdiction | resilience up, ai_safety up slightly via misuse detection | 2 | finished |
| M6 | EU Trusted Supply and Partnership Diversification Compact | 8 | small | 5 | 8 | coalition | eu_ai_sovereignty up, resilience up slightly | 2 | finished |
| M7 | EU Agent Containment and Escalation Protocol | 6 | small | 6 | 8 | own jurisdiction | resilience up, ai_safety up slightly | 2 | finished |
| M8 | EU Cyber Patch and Recovery Surge | 6 | small | 8 | 9 | own jurisdiction | resilience up, ai_safety up slightly | 2 | finished |
| M9 | EU Deployed Assurance Adoption Programme | 5 | small | 9 | 11 | own jurisdiction | ai_safety up, resilience up slightly | 2 | finished |
| M10 | EU Cut-Off Continuity and Substitution Shield | 6 | small | 10 | 11 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly | 2 | finished |
| M11 | EU Municipal Patch and Certified Recovery Network | 6 | small | 11 | 12 | own jurisdiction | resilience up substantially, ai_safety up slightly | 2 | finished |
...
## New measure
**EU Municipal Patch and Certified Recovery Network**
Puts the new automated patching and certified-predictability tooling directly into hospitals, small councils and telecom operators left exposed by the cut-off and ransomware wave, via ENISA teams and EU-hosted open models.
This now because capital at 16 allows only one cheap deployable win, and the defence and safety breakthroughs plus investment collapse make hardening with what already exists the only way to hold our hardening commitment while services stay lit.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Patch and Certified Recovery Network", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, ai_safety up slightly"}, "grounds": "deploy defence and safety breakthroughs to exposed essential services"}]}
```

## Priority
M11 EU Municipal Patch and Certified Recovery Network, because keeping essential services lit through unrecallable open weights is the core of our two-year hardening commitment and outranks chip fights we cannot afford this turn.

## In practice
We task ENISA with the AI Office and HERA to ship a single hardened stack: auto-patching at vulnerability speed, swarm detection, and the newly certifiable control technique pre-applied to EU-hosted open-weight substitutes from the spring release. Funding comes from reprogrammed Digital Europe and Recovery resilience lines, no new vote, implemented via implementing acts under NIS2 and the AI Act.

We run it as joint cross-border teams triaging the three cut-off states first, then small municipalities and mid-size hospitals that lagged before: 24h reporting stays, but teams do the patching where staff are missing. On Washington and lithography we hold to TTC volume-licence continuity and quiet diversification paperwork, freezing new gigafactory siting fights until trust recovers.
...
```
## New measure
**EU Joint Threat Response and Treatment Continuity Cell**
Joins the joint cyber/biosurveillance response pact with real-time telemetry and sample-sharing while routing the new tailored therapies through EU-hosted substitutes and paper-backup clinical pathways.
This now because cut-offs plus the Dutch lithography standoff leave us with no sovereign lever this turn, and the only hardening that still lands under capital at 22 is protection we borrow through the joint command plus visible medical delivery that rebuilds trust.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Joint Threat Response and Treatment Continuity Cell", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "coalition", "targeted_effect": "resilience up substantially, public_sentiment up slightly"}, "grounds": "answer cut-off and displacement with borrowed protection and delivered treatments"}]}
```

## Priority
M12 EU Joint Threat Response and Treatment Continuity Cell, because it is the only live instrument and it directly serves our two-year hardening commitment when domestic build-outs are frozen.

## In practice
We mandate ENISA, HERA and ECDC via NIS2 and cross-border health legal bases to plug into the joint cyber command telemetry feed and the biosurveillance sample-sharing protocol, with seconded agency patch teams as the EU contribution. Funding is reprogrammed from Digital Europe and EU4Health, no new vote, decided at the Health and Telecoms Councils together to avoid a siting fight.

We pair that with HERA-led procurement of the breakthrough therapies for the three cut-off states first, dispensed through EU-hosted open-weight triage and the municipal patch network hospitals, with paper-backup protocols retained until substitutes clear audit. On ASML servicing we back The Hague publicly but seek a TTC volume-licence carve-out to keep older-tool servicing legal, offering Washington joint attribution in return.
