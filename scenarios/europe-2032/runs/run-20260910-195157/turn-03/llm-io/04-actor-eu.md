...
- characters 2032-4046: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6056-6515: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8488-10498: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_sovereign_hosting_offer (emergent event):** A major non-US cloud provider announces EU-only model hosting with data and weights legally ring-fenced in the Union, marketed as continuity insurance after access scares.
...
| M4 | EU Evaluation and Deployment Review Capacity | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, resilience up slightly via misuse detection | 2 | running |
...
## New measure
**EU Evaluation and Deployment Review Capacity**
Stands up independent EU technical capacity to evaluate frontier and open-weight models for cyber-biological misuse and to feed results into deployment safeguards and incident response.
This is category 5 Public technical capacity and research, because with an open-weight release now within a few months of the closed frontier and permanently unrecallable, the EU cannot govern through release control and must be able to test what is loose and harden against it.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Evaluation and Deployment Review Capacity", "category": 5, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, resilience up slightly via misuse detection"}, "grounds": "open-weight frontier release now unrecallable demands own evaluation capacity"}]}
```

## Priority
EU Critical Infrastructure Cyber Shield, not the Gigafactories, because the autumn intrusion plus a frontier-class open release means distributed industrial-access tooling can strike before any new compute lands.

## In practice
We keep M3 as the funded priority: ENISA and DG ENER drive mandatory segmentation audits and credential resets for transmission operators, with Rotterdam/Antwerp exercises extended to water and port OT, paid from reprogrammed Connecting Europe and Digital Europe money and paired to EU vendors to hold competitiveness critics.

M1 Gigafactories and M2 tech sovereignty package stay on their DG CNECT/EIB site-selection and permitting-zone tracks but deliberately deprioritised for cash this turn; we do not open a parallel compute line. We task the Joint Research Centre and ENISA to start the new evaluation cell now — hiring red-teamers, securing model access for testing, and linking findings to the Cyber Shield detection playbooks — using existing Horizon and Digital Europe research lines to avoid a fresh budget fight.
...
```
## New measure
**EU Ring-Fenced Continuity Hosting**
Sign and anchor the non-US EU-only hosting offer as continuity capacity for essential public and industrial workloads, paired with rapid rollout of the new defensive tooling to Shield operators.
This and now because the hosting window closes fast, the cyber-defence breakthrough makes defended hosting credible for the first time in years, and with frontier open weights unrecallable and gigafactories years out, ring-fenced capacity is the only sovereignty gain available this turn — while knowledge-work gains without job losses give us brief tolerance to spend on insurance the public cannot yet feel.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Ring-Fenced Continuity Hosting", "category": 4, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up moderately, resilience up slightly"}, "grounds": "lock in emergent EU-only hosting offer plus defensive breakthrough before window closes"}]}
```

## Priority
EU Ring-Fenced Continuity Hosting, not the evaluation cell, because the hosting offer and defence breakthrough together offer a closable sovereignty and hardening win while gigafactories stall and open weights spread.

## In practice
We task DG CNECT and ENISA with a fast procurement and legal-anchoring track for the provider's EU-only zone — data and weights residency, EU-law control, continuity priority for transmission operators, ports and water — using Digital Europe and EIB guarantees, not fresh budget, to avoid a Council fight. DIGIT and member-state CIOs are offered first-mover migration for health and administration workloads to fill the pipe.

We direct the finished Cyber Shield operators and the JRC/ENISA evaluation cell to deploy the breakthrough automated-patching and swarm-detection tooling first on that ring-fenced estate and then to Shield-covered transmission and Rotterdam/Antwerp OT, with M1/M2 kept on low-cash permitting tracks. Energy and municipal complaints about unfunded mandates are met with free tooling plus hosting credits, not new mandates.
