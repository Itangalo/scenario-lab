...
- characters 376-1711: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1754-3782: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3838-4660: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4723-31373: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 31728-32074: `{{event_history}}` from the run's own event record
...
- `emergent_triage_bypass_exposure` -- first noted turn 5, listed in 1 turn(s) so far: Independent grid audit finds the EU triage desk's frozen agent rollouts were bypassed via vendor remote-update channels, leaving municipal utilities exposed without triggering reporting obligations.
...
- Turn 3 (3 turn(s) ago): cyber_defence_breakthrough, openweight_frontier_release, knowledge_work_augmented, emergent_sovereign_hosting_offer
- Turn 4 (2 turn(s) ago): eval_anomaly_reports
- Turn 5 (1 turn(s) ago): adoption_success, election_alliance
