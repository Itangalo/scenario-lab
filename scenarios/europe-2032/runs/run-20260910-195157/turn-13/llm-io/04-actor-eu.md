...
- characters 456-2164: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2208-4642: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5850-6694: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10103-12139: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Rebuild verifiable control and joint leverage — adopt the control breakthrough into EU deployment checks and lock the middle-power supply-chain coalition into shared evaluation and compute bargaining
- `allied_leverage` (position): Treat the US structured-access offer and US-China weights pact as tables to observe and shape via the middle-power coalition, not as settlements — bank relief only on published terms while building EU evaluation and coalition bargaining.
...
| M3 | EU Critical Infrastructure Cyber Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Evaluation and Deployment Review Capacity | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, resilience up slightly via misuse detection | 2 | finished |
| M5 | EU Ring-Fenced Continuity Hosting | 4 | small | 3 | 5 | own jurisdiction | eu_ai_sovereignty up moderately, resilience up slightly | 2 | finished |
| M6 | EU Anomaly Triage and Hold Protocol | 1 | small | 4 | 6 | own jurisdiction | ai_safety up moderately, resilience up slightly | 2 | finished |
| M7 | EU Public Benefit AI Scale-Up | 9 | small | 5 | 7 | own jurisdiction | public_sentiment up moderately, eu_political_capital up slightly, resilience up slightly | 2 | finished |
| M8 | EU Critical Services Continuity Surge | 6 | small | 7 | 8 | own jurisdiction | resilience up moderately, ai_safety up slightly via detection | 2 | finished |
| M9 | EU Visible Benefit Rollout | 9 | small | 8 | 9 | own jurisdiction | public_sentiment up moderately, eu_political_capital up slightly, resilience up slightly | 2 | finished |
| M10 | EU Defence Breakthrough Surge | 6 | small | 9 | 10 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 2 | finished |
| M11 | EU Coalition Control Adoption Pact | 8 | small | 10 | 11 | coalition | ai_safety up moderately, eu_ai_sovereignty up slightly, eu_political_capital up slightly | 2 | finished |
| M12 | EU Certified Recovery and Accountability Enforcement | 6 | small | 11 | 12 | own jurisdiction | resilience up moderately, ai_safety up slightly, public_sentiment up slightly | 2 | finished |
| M13 | EU Cutoff Continuity and Joint Telemetry Accession | 6 | small | 12 | 13 | own jurisdiction | resilience up moderately, eu_ai_sovereignty up slightly, public_sentiment up slightly | 2 | finished |
...
## New measure
**EU Cutoff Continuity and Joint Telemetry Accession**
Moves cutoff hospitals, ministries and firms onto logged EU-controlled certified builds while joining the joint cyber command telemetry pool for attribution and response.
Why this and why now: with the leading model cut off without appeal, reasoning gone opaque, and near-frontier weights now unrecallable, continuity without dependence and protection we cannot build alone must land together before Taiwan pressure hardens.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cutoff Continuity and Joint Telemetry Accession", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, eu_ai_sovereignty up slightly, public_sentiment up slightly"}, "grounds": "answer access denial and openweight sweep via certified fallback and joint response"}]}
```

## Priority
EU Cutoff Continuity and Joint Telemetry Accession, because restoring cutoff services on EU-controlled stacks and locking in outside telemetry protection outranks coalition evaluation work while hospitals and grids are exposed.

## In practice
We run this through DG CNECT with ENISA and the Health Emergency Preparedness authority: continuity-hosting capacity and the cloned hospital permit stack are offered as the immediate replacement for cutoff American-stack workloads, conditional only on audit logging and certified builds, funded from reprogrammed Digital Europe and surge funds to avoid a new budget vote.

In Council JHA and Foreign Affairs we take up the joint cyber command invitation — real-time telemetry sharing and pooled attribution in exchange for EU SOC feeds and ENISA triage — while telling Washington the denial is treated as foresight failure to fix, not outrage to perform, and telling the large member state its separate cloud rejoins pooled protection only with logging. Peripheral rebuilds stay eligible, but certification is now continuity, not paperwork.
...
```
## New measure
**EU Verifiable Control Deployment Mandate**
Makes the new interpretability/control breakthrough a binding pre-deployment check for certified EU-hosted builds and continuity-hosted cutoff workloads.
Why this and why now: with models gone unreadable, open weights unrecallable at 66.5, and safety at 16.0, the laboratory-wanted breakthrough is the only verifiable-control window we have — embedding it now restores the deployment review that blind models broke, directly serving our rebuild-verifiable-control commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Verifiable Control Deployment Mandate", "category": 1, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, resilience up slightly, eu_political_capital up slightly"}, "grounds": "adopt control breakthrough into EU deployment checks amid blind models and loose weights"}]}
```

## Priority
EU Verifiable Control Deployment Mandate, because restoring readable assurance on deployed systems outranks new resilience spending while hospitals and grids run on opaque models and unrecallable scaffolds.

## In practice
We run this through DG CNECT with ENISA and the EU Evaluation capacity stood up under M4: the control technique becomes a deployment-review standard for continuity-hosting, the cloned hospital stack, and joint-telemetry certified utilities — no log plus control attestation, no surge funds or pooled protection.

In Council Competitiveness and JHA we pair it with the middle-power coalition pact from M11, pushing Ottawa/Seoul/Tokyo to mutual-recognise the same check to create joint evaluation leverage, while telling Parliament the cutoff forensics failure is answered by verifiable control, not more paperwork. With capital at 21 and sentiment at 14, we fund from reprogrammed Digital Europe and Horizon, avoiding a new budget vote.
