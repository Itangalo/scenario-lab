...
- characters 107-13353: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13396-13588: `{{metrics_json}}` from the run's live metric values
- characters 13681-15389: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15433-17867: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17923-18388: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M14 | EU Verifiable Control Deployment Mandate | 2 |
...
## Notepad
PORTFOLIO CHARGE t13: EU Verifiable Control Deployment Mandate −2, priority −1 = −3
PROPOSAL BONUS: none — new control mandate answers blind-model opacity and loose weights but no negative event in last three turns maps directly as shock it would have helped with; breakthrough is positive opportunity
LEGITIMACY LENDS: capital 20, sentiment 15 -> none, capital higher
SOVEREIGNTY: 32 last turn, no capacity event, no category 4 in flight, capability rose 0.8 −0 = 32
