...
- characters 16352-16932: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_sovereign_hosting_offer` -- first noted turn 1, listed in 1 turn(s) so far: A major non-US cloud provider announces EU-only model hosting with data and weights legally ring-fenced in the Union, marketed as continuity insurance after the June switch-off scare.
- `emergent_bio_publication_moratorium` -- first noted turn 1, listed in 1 turn(s) so far: Major scientific journals and preprint servers impose coordinated mandatory screening and delayed-release review for AI-assisted pathogen design work after the phage demonstrations.
...
## Metrics
```json
{"ai_capability": 54.0, "openweight_capability": 46.0, "ai_safety": 32.0, "resilience": 37.0, "eu_ai_sovereignty": 21.0, "eu_political_capital": 44.0, "public_sentiment": 38.0}
```
...
PROPOSAL BONUS: EU Critical Infrastructure Cyber Shield (cat 6, large) +3 — cyber_test_shot landed this turn and this answers it directly
...
Emerging: sovereign_hosting_offer faint single-source trade press rumour; bio moratorium faint journal editorials only.
