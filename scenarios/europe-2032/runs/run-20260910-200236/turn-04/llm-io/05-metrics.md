...
- characters 105-13247: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13290-13482: `{{metrics_json}}` from the run's live metric values
- characters 13575-15229: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15272-17299: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17355-17857: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M5 | EU Workplace Transition and Income Bridge | 2 |
...
## Notepad
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Workplace Transition and Income Bridge −2, priority −1 = −9
PROPOSAL BONUS: none — new Bridge (cat 7, small) answers productivity plateau which is stabilising not a negative shock from last three turns
LEGITIMACY LENDS: capital 20, sentiment 39.5 -> +2
SOVEREIGNTY: 20 last turn, no capacity event, Gigafactories/tech package in flight +0, capability rose 1.5 −0 = 20

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M5", "fields": {"finish_turn": 7}, "grounds": "named priority"}]}
```
```
