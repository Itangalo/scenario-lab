...
- characters 2004-3660: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3716-4668: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4731-28013: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28368-28462: `{{event_history}}` from the run's own event record
...
- `emergent_provenance_mandate` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher and a national research funder jointly mandate verified provenance and AI-use disclosure for submitted papers after a wave of fabricated citations from agentic literature reviews is exposed.
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated local protests block or delay two or more large EU data-centre projects over power prices and water use, forcing a national permitting pause.
