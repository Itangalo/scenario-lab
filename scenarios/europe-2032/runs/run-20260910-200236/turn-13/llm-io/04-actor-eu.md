...
- characters 456-1401: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1445-2944: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3384-3892: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7198-8998: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M12 | EU Human Front-Door and Trusted Stewards | 9 | small | 12 | 14 | own jurisdiction | public_sentiment up moderately, resilience up slightly | 2 | running |
...
| M11 | EU Local Services Continuity and Livelihood Bridge | 7 | small | 11 | 13 | own jurisdiction | resilience up moderately, public_sentiment up | 2 | finished |
...
```
## New measure
None this turn — we are waiting for the Front-Door stewards to actually fill posts and for the Bridge successor lines to settle before staking another instrument we cannot fund.

## Store changes
No changes.

## Priority
M12 EU Human Front-Door and Trusted Stewards, because with open weights now within months of the frontier and sentiment at 17, only a visible human alternative that people can use this winter stops distrust hardening into bans and obstruction.

## In practice
We keep the Bridge successor funding running through DG EMPL/REGIO and ESF+: overtime and mutual-aid rosters in the three hit states stay conditioned on open desks, clean-image rebuilds and published ombudsman redress, with the mayoral compact held — no forced data-centre push for unblocked hardened rebuilds. DG CNECT and ENISA fold the new open-weight release into the existing restoration playbook: offline fallback procedures, verified clean images, and fraud/impersonation checks at clinic and permit desks, no new mandate.

We push M12 through EPSCO and the health Council track: complete the live-assistant map, convert advertised steward posts into filled posts with Bridge successor money, and move the right to human handling from draft to enforceable pilots with published wait-times in the worst-hit districts. Commission representations run the local mediation on paused server-room works, offering scheduling and noise guarantees to get concrete poured without forcing a confrontation we would lose.
