...
- characters 456-1469: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1513-3628: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3696-4204: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7030-9004: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Rebuild public trust by making AI survivable in daily life through resilient services, real human oversight and livelihood protection
...
| M3 | EU Critical Shield Hardening Programme | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 3 | finished |
| M4 | Cross-Border Repair and Continuity Reserve | 6 | small | 3 | 4 | own jurisdiction | resilience up moderately | 2 | finished |
| M5 | EU Workplace Transition and Income Bridge | 7 | small | 4 | 7 | own jurisdiction | resilience up moderately, public_sentiment up | 2 | finished |
| M6 | EU Agent Loss-of-Control Containment and Mutual Aid Protocol | 6 | small | 5 | 7 | own jurisdiction | resilience up, ai_safety up slightly via containment | 2 | finished |
| M7 | EU Cyber Emergency Containment Surge | 6 | small | 6 | 7 | own jurisdiction | resilience up moderately to limit cyber incident damage | 2 | finished |
| M8 | EU Community-Anchored Compute Compact | 4 | small | 8 | 9 | own jurisdiction | eu_ai_sovereignty up moderately, public_sentiment up slightly | 2 | finished |
| M9 | EU Accountable Deployment and Redress Guarantee | 9 | small | 9 | 10 | own jurisdiction | public_sentiment up, resilience up slightly, ai_safety up slightly via oversight | 2 | finished |
| M10 | EU Cyber Restoration and Safe-Systems Adoption Surge | 6 | small | 10 | 11 | own jurisdiction | resilience up moderately, ai_safety up slightly, public_sentiment up slightly | 2 | finished |
...
## New measure
**EU Cyber Restoration and Safe-Systems Adoption Surge**
It funds emergency restoration of hit public services with hardened rebuilds while rolling the new interpretability/control breakthrough into EU-run and critical-sector AI deployments via audited patches. Why this and why now: the automated attack has made defenders-visibly-behind the lived reality and trust is at 18, so under our rebuild-trust-by-survivability commitment we must show services degrade-not-stop and that safety gains land on shipped systems immediately.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cyber Restoration and Safe-Systems Adoption Surge", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly, public_sentiment up slightly", "grounds": "to contain major cyber incident and deploy safety breakthrough"}}]}
```

## Priority
EU Cyber Restoration and Safe-Systems Adoption Surge, because the major incident outranks concrete and licensing fights — without visible restoration no other pledge is fundable.

## In practice
We task DG CNECT and ENISA with the surge under the Civil Protection Mechanism and Digital Europe: joint triage teams to affected municipalities and hospitals, clean rebuilds from EU-anchored images, and cross-border mutual aid for continuity, paid from the Continuity Reserve and hospital-hardening line.

We pair it with DG DIGIT and the AI Office adopting the interpretability/control result on clinic, school and benefits assistants running on EU servers — mandatory pre-observation prediction checks and certified properties, published failures — while keeping the Accountable Deployment redress and log-reading duties enforced with ombudsmen. EEAS holds the Washington observer track for spare parts and model access, accepting rationing rather than rupture while bans block new sites through 2031.
...
```
## New measure
**EU Local Services Continuity and Livelihood Bridge**
It keeps hit clinics, helpdesks and permit offices staffed in degraded manual mode while funding short-term income and retraining bridges for displaced frontline and admin workers. Why this and why now: with trust at 15 and capital at 23 after the February freezes, under our rebuild-trust-by-survivability commitment only visible continuity of daily services plus livelihood protection can stop hostility hardening into bans and obstruction.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Local Services Continuity and Livelihood Bridge", "category": 7, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up", "grounds": "to hold services and jobs through degraded operations"}}]}
```

## Priority
EU Local Services Continuity and Livelihood Bridge, because without staffed fallback services and paid transition no hardening or certification pledge is believed.

## In practice
We task DG EMPL and DG REGIO with the Bridge under the ESF+ and Civil Protection Mechanism: mutual-aid rosters to keep affected municipalities and hospitals open on paper-plus-clean-image procedures, paid overtime and temporary hires, tied to ombudsman-logged redress payouts already legislated.

We pair it with Council EPSCO and mayoral compacts in the three hit states: no new data-centre siting push this turn, in exchange for lifting de facto blocks on hardened rebuilds and EU-anchored images, with Commission funding conditioned on keeping permit and scheduling desks open in degraded mode. EEAS holds the rationing line with Washington for spares only.
