...
- characters 106-13089: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13132-13324: `{{metrics_json}}` from the run's live metric values
- characters 13417-14441: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14485-16446: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16502-17567: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M3 | EU AI Evaluation and Safety Institute | 3 |
| M4 | EU Grid Shield Hardening Programme | 2 |
| M5 | EU Rapid Cyber Defence Deployment | 2 |

     **so 5 measures are in flight and they come to −13 `eu_political_capital` this turn.** That figure is the `cost_per_turn` column of the rows above, added up, and nothing else: the named priority's −1 below is charged on top of it, and is not in it. Your charge line carries one term per measure -- 5 of them before the priority. **A measure is charged in every turn up to but not including its finishing turn. A measure added this turn is charged from this turn -- it appears in the rows above and in your total.**
...
PORTFOLIO CHARGE t2: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M3 EU AI Evaluation and Safety Institute −3, M4 EU Grid Shield Hardening Programme −2, priority −1 = −12
PROPOSAL BONUS: EU Grid Shield (cat 6, small) +2 — autumn mapped-grid intrusion in last turn and this answers it directly
...
- `emergent_energy_siting_backlash` -- first noted turn 3, listed in 1 turn(s) so far: Coordinated protests and municipal permit blocks stall grid-hardening retrofits and gigafactory power connections in several host regions over costs and energy strain.
- `emergent_grid_siting_backlash` -- first noted turn 3, listed in 1 turn(s) so far: A coordinated physical and cyber protest campaign targets planned gigafactory grid connections and substation upgrades in two host regions, delaying power hookups and forcing a national debate on energy costs.
...
## New measure
**EU Rapid Cyber Defence Deployment**
Deploys automated patching and swarm-behaviour detection from the breakthrough across grid operators, public services and major software dependencies via ENISA-coordinated rollout. Why this and why now: the major automated attack proves offence has outrun uneven hardening, while the defensive breakthrough gives us a closable class of attack to push back on immediately.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Rapid Cyber Defence Deployment", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up"}, "grounds": "to exploit defence breakthrough after major model-generated attack"}]}
```
## Priority
EU Grid Shield Hardening Programme, because the major incident hits the same grids, ports and public services it is already segmenting and the breakthrough only helps if baseline logging and segmentation are in place first.
## In practice
We task ENISA with CERT-EU and national CSIRTs to push the new automated patching and swarm-detection tooling first to the two hit grid operators, the container port, water utility and the compromised public-service stacks, funded by emergency reprogramming plus Digital Europe cybersecurity funds, with TTE Council mandating deployment standards despite municipal capacity gaps.

We keep InvestAI Gigafactories (M1) and Tech sovereignty package (M2) on site-selection and EIB structuring with Paris, Berlin, Madrid, Stockholm, Warsaw, and push the EU AI Evaluation and Safety Institute (M3) toward vetted-access pilots, but accept both stay preparatory while engineers and incident responders absorb the attack and attribution proceeds.
...
## Notepad
PORTFOLIO CHARGE t3: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M3 EU AI Evaluation and Safety Institute −3, M4 EU Grid Shield Hardening Programme −2, M5 EU Rapid Cyber Defence Deployment −2, priority −1 = −14
PROPOSAL BONUS: EU Rapid Cyber Defence Deployment (cat 6, small) +3 — major automated attack landed this turn and this answers it directly plus exploits breakthrough
