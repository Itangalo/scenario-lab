...
- characters 105-13504: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13547-13739: `{{metrics_json}}` from the run's live metric values
- characters 13832-15022: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15065-17136: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17192-18324: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_energy_siting_backlash` -- first noted turn 3, listed in 2 turn(s) so far: Coordinated protests and municipal permit blocks stall grid-hardening retrofits and gigafactory power connections in several host regions over costs and energy strain.
- `emergent_grid_siting_backlash` -- first noted turn 3, listed in 2 turn(s) so far: A coordinated physical and cyber protest campaign targets planned gigafactory grid connections and substation upgrades in two host regions, delaying power hookups and forcing a national debate on energy costs.
...
## Metrics
```json
{"ai_capability": 58.0, "openweight_capability": 53.5, "ai_safety": 27.0, "resilience": 40.0, "eu_ai_sovereignty": 18.0, "eu_political_capital": 22.0, "public_sentiment": 23.0}
```
...
## Notepad
PORTFOLIO CHARGE t4: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M5 EU Rapid Cyber Defence Deployment −2, M6 EU Continuity Stack −2, priority −1 = −11
PROPOSAL BONUS: EU Continuity Stack (cat 4, small) +4 — eu_frontier_access_denied landed this turn and this answers it directly
LEGITIMACY LENDS: capital 20, sentiment 23 -> +2
SOVEREIGNTY: 20 last turn, no capacity event? no — eu_frontier_access_denied t4 −2, Gigafactories in flight +0, capability rose 1.5 −0 = 18
