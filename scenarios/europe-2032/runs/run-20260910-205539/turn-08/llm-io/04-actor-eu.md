...
- characters 452-2527: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2570-5530: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6810-7307: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10109-12298: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_insurer_coverage_freeze (emergent event):** Hospital groups and municipal utilities sue insurers over model-assisted sabotage and pathogen exclusions, triggering a court-ordered coverage freeze and emergency reinsurance intervention.
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Accession Bridge to the US-China Risk Accord | 8 | small | 7 | 9 | US, China, coalition | ai_safety up modestly, eu_ai_sovereignty up slightly, eu_political_capital up slightly | 2 | running |
...
| M3 | EU Critical Systems Cyber Shield | 6 | small | 1 | 2 | own jurisdiction | resilience up moderately, ai_safety flat | 2 | finished |
| M4 | EU Fallback Stack for Essential Services | 9 | small | 2 | 5 | own jurisdiction | eu_ai_sovereignty up modestly, resilience up, public_sentiment up slightly | 2 | finished |
| M5 | EU Critical Services Cyber-Bio Shield Surge | 6 | small | 3 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly | 2 | finished |
| M6 | EU High-Risk Enforcement Reset | 1 | small | 4 | 5 | own jurisdiction | public_sentiment up modestly, ai_safety up slightly, eu_political_capital up slightly | 2 | finished |
| M7 | EU Defensive Shield Deployment | 6 | small | 5 | 7 | own jurisdiction | resilience up substantially, ai_safety up slightly | 2 | finished |
| M8 | EU Bio-Cyber Containment and Care Surge | 6 | small | 6 | 8 | own jurisdiction | resilience up substantially, public_sentiment up modestly | 2 | finished |
...
## New measure
**EU Accession Bridge to the US-China Risk Accord**
Seeks formal EU inclusion in the US-China limited agreement on weights security, autonomous escalation and biological design tools, offering EU sequencing, incident and evaluation data and supply-chain leverage as entry price.
This is International coordination and leverage work, and it must come now because the only thing that slows capability is being negotiated without us while Taiwan tensions rise and hospitals strike — if we are informed afterwards rather than inside, both our sovereignty and safety mandates fail.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Accession Bridge to the US-China Risk Accord", "category": 8, "size": "small", "finish_turn": 9, "applies_to": "US, China, coalition", "targeted_effect": "ai_safety up modestly, eu_ai_sovereignty up slightly, eu_political_capital up slightly"}, "grounds": "to join US-China agreement rather than be briefed afterwards"}]}
```

## Priority
M8 EU Bio-Cyber Containment and Care Surge, because elective-care suspensions and staff refusal over liability cover will cascade into deaths and fragmentation faster than any accord negotiation, and only keeping hospitals open and paid holds cohesion.

## In practice
We run M8 through HERA/ECDC and ENISA this turn: joint procurement extended to elective-care backlogs, ECOFIN emergency backstop converted into explicit EU liability guarantee for AI-assisted diagnostics to end staff refusal, and integration teams paid to unbox detectors in the east. Health Council adopts temporary sequencing and reporting mandate with funded staff, not unfunded mandates.

On the new bridge, the High Representative and DG CNECT carry a single mandate to Washington and Beijing: EU adhesion to the weights-security and bio-tool provisions, mutual recognition of evaluations, and shared verification using EU labs and ASML-adjacent supply-chain leverage. We coordinate the middle-power position with Japan, Korea and the UK so the large member-state side-deal cannot be used to pick us off, and we link gigafactory grid-permit progress to accord-compliant compute standards.
...
```
## New measure
**EU Emergency Care Continuity Reinsurance and Patch Surge**
Keeps hospitals and municipal utilities operating through the court-ordered coverage freeze via a temporary EU reinsurance backstop tied to mandatory patching and backup-care protocols against the new code/cyber jump.
This and now because the insurer freeze plus a verifiable-domain capability jump means elective suspensions return and cyber dwell times spike within weeks, and with capital at 7 and sentiment at 7 we cannot hold cohesion or the US-China bridge unless wards stay open; it directly serves our two-year commitment to hardened services that absorb open-model harm.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Emergency Care Continuity Reinsurance and Patch Surge", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, public_sentiment up slightly, ai_safety up slightly"}, "grounds": "to hold hospitals/utilities through coverage freeze and code-cyber jump"}]}
```

## Priority
M9 EU Accession Bridge to the US-China Risk Accord, because with a member-state side-deal undercutting us and Taiwan opening the gate, being left outside the weights/bio accord permanently loses both mandates even as the new surge holds the wards.

## In practice
We run the new surge through ECOFIN and Health Council under the existing liability-guarantee legal base: EIB emergency reinsurance window to override the freeze for hospitals/utilities that patch, ENISA-led forced patch teams redeployed west-to-east, and HERA sequencing kits funded only where reporting holds. Finance ministers get joint borrowing capped to this window to avoid further blunting of preparedness.

On M9 the High Representative carries a narrow accession ask to Washington and Beijing — EU labs and incident data for verification seats — coordinated with Tokyo/Seoul/London so the defecting member state cannot be played off. We publicly name the defection as fragmentation risk in Council while offering tiered-access members guaranteed fallback capacity from the two grid-connected shells once permits land.
