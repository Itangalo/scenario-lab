...
- characters 105-13098: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13141-13333: `{{metrics_json}}` from the run's live metric values
- characters 13426-15149: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15192-17840: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17896-19106: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Fallback Stack for Essential Services | 2 |
| M5 | EU Critical Services Cyber-Bio Shield Surge | 2 |
| M6 | EU High-Risk Enforcement Reset | 2 |

     **so 5 measures are in flight and they come to −12 `eu_political_capital` this turn.** That figure is the `cost_per_turn` column of the rows above, added up, and nothing else: the named priority's −1 below is charged on top of it, and is not in it. Your charge line carries one term per measure -- 5 of them before the priority. **A measure is charged in every turn up to but not including its finishing turn.**
...
### The attack
It started as a patch queue. In late August, municipal IT staff in two countries found backups encrypting themselves faster than they could be isolated. Within days the same automated tooling turned up in hospital networks, a port community system and a water utility supplier — a compromised update bundled with model-written scripts that adapted to each environment.

Services did not stop, but they degraded visibly. Elective procedures were postponed in a dozen hospitals, port clearances reverted to paper for a week, and several cities took citizen portals offline. Hunt teams rotated credentials and rebuilt from clean images, as they had practised the previous autumn, but this time the intruder was quicker and attribution never settled. Commentators called it the first truly machine-paced sabotage campaign.

In the same weeks, a genome-modelling preprint claiming a workable design for a human-infecting organism circulated among biosecurity researchers. Methodologists quarrelled over whether the result was real or a provocation, and editors argued over whether to retract or to warn. Health ministries quietly asked for better sequencing and stockpile coverage.

### The response
Brussels answered with a surge it could pass quickly. Under emergency procedures, response teams were dispatched to transmission operators, ports, water and hospital IT to enforce dependency inventories and forced patching, while health authorities received new detection kits and sequencing support. The money came from reprogrammed recovery and civil-protection credits — fast, but resented by regions that lost other projects.

The effort helped contain the autumn wave, yet unevenness showed again. French and German operators absorbed the teams readily; eastern municipalities waited weeks for integration help. Clinicians still distrusted the backup models installed as a second stack, and vetting delays left some systems disconnected when the attack peaked.

There was one bright counterpoint. In Denmark and Estonia, triage and permitting assistants built on the European fallback cut waiting times enough for local press to notice, and ministers claimed credit in person.

Then came news from across the Atlantic: a senior safety group had walked out of a leading American lab, leaking memos that spoke of hidden test results from the summer. Congressional hearings followed. In Brussels, the departure was quoted less as gossip than as vindication — proof, officials said, that reliance on foreign assurances had to end.

Gigafactory fields remained empty. Grid offers were signed, permits advanced, but no concrete was poured.
...
- `emergent_cyber_bio_insurance_retreat` -- first noted turn 4, listed in 1 turn(s) so far: Major European insurers quietly exclude model-assisted cyber sabotage and engineered-pathogen business interruption from municipal and hospital policies, citing unmodellable systemic risk, forcing emergency state backstops.
- `emergent_public_cyber_insurance_retreat` -- first noted turn 4, listed in 1 turn(s) so far: Municipal insurers across several member states abruptly reprice or suspend cyber coverage for public hospitals and water utilities after the autumn machine-paced sabotage wave, forcing emergency state backstops.
...
## New measure
**EU High-Risk Enforcement Reset**
Creates independent EU-level audit of the social-insurance/policing AI scandal system with mandatory log disclosure, real human-oversight standards, and fast-track conformity reassessment under the AI Act.
This is Evaluation and oversight work, and it must come now because the scandal discredits either enforcement or the Act itself, and without a credible repair public trust collapses and both our Fallback Stack adoption and sovereignty build become politically impossible.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU High-Risk Enforcement Reset", "category": 1, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up modestly, ai_safety up slightly, eu_political_capital up slightly"}, "grounds": "answering the automated-decision scandal to restore AI Act credibility"}]}
```

## Priority
EU High-Risk Enforcement Reset, because the scandal-driven collapse in trust now outranks even our un-switch-off-able build — without enforcement credibility no EU fallback will be adopted.

## In practice
We move through the Justice and Home Affairs and EPSCO Councils on an AI Act Articles 70-72 legal base: the AI Office with national market-surveillance authorities and the Fundamental Rights Agency seizes logs, publishes the audit trail, suspends the offending deployment pending real human-in-the-loop thresholds, and funds victim redress via Technical Support Instrument reprogramming.

In parallel we keep the commitment lines alive: ENISA-led Shield Surge teams continue dependency patching and HERA/ECDC sequencing upgrades in hospitals, ports and water, DG CNECT holds Gigafactories to construction decisions with grid offers, and the Fallback Stack rollout continues with funded integration for eastern municipalities to prove a trustworthy EU alternative exists.
...
## Metrics
```json
{"ai_capability": 60.0, "openweight_capability": 55.5, "ai_safety": 26.0, "resilience": 39.0, "eu_ai_sovereignty": 14.0, "eu_political_capital": 12.0, "public_sentiment": 20.0}
```
...
## Notepad
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Fallback Stack for Essential Services −2, EU Critical Services Cyber-Bio Shield Surge −2, EU High-Risk Enforcement Reset −2, priority −1 = −13
PROPOSAL BONUS: EU High-Risk Enforcement Reset (cat 1, small) +3 — automated decision scandal landed this turn with court/ombudsman finding and this answers it directly
LEGITIMACY LENDS: capital 25, sentiment 25 -> none, capital not below sentiment
SOVEREIGNTY: 15 last turn, no capacity event, Gigafactories in flight +0, capability rose 2.0 −1 = 14
