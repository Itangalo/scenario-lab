...
- characters 13751-15358: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15401-17724: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17780-18269: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Defensive Shield Deployment | 2 |
...
**emergent_cyber_bio_insurance_retreat (emergent event):** Major European insurers quietly exclude model-assisted cyber sabotage and engineered-pathogen business interruption from municipal and hospital policies, citing unmodellable systemic risk, forcing emergency state backstops.
...
## Metrics
```json
{"ai_capability": 62.0, "openweight_capability": 58.5, "ai_safety": 23.0, "resilience": 40.0, "eu_ai_sovereignty": 16.0, "eu_political_capital": 11.0, "public_sentiment": 10.0}
```
## Narrative
### The release
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner campaigned on tiered access, consolidation takes effect turn 6"}]}
```
```
