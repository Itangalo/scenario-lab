...
- characters 2009-4203: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4259-5267: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5330-27951: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28306-28534: `{{event_history}}` from the run's own event record
...
- `emergent_permitting_freeze` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated municipal backlash halts data-centre and sensor-site permitting in several member states, with court injunctions citing fraud, energy costs and health-system strain.
- `emergent_misuse_liability_litigation` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated municipal lawsuits against distributors and AI deployers over fraud losses and hospital outages force courts to rule on liability for open-model misuse, creating an EU-wide precedent debate.
...
- Turn 4 (1 turn(s) ago): cyber_major_incident, ai_investment_collapse, eval_anomaly_reports
