...
- characters 105-13439: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13482-13673: `{{metrics_json}}` from the run's live metric values
- characters 13766-15300: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15343-17652: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17708-18091: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Graduate Transition Guarantee | 2 |
...
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, Open Frontier Absorption Shield −2, priority −1 = −9
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 19, sentiment 28 -> +2
SOVEREIGNTY: 20 last turn, no category 4 finish, Gigafactories/Tech package in flight +0, member_state_defection t5 −2, capability rose 1.3 −0 = 18
...
**emergent_permitting_freeze (emergent event):** Coordinated municipal backlash halts data-centre and sensor-site permitting in several member states, with court injunctions citing fraud, energy costs and health-system strain.
...
## Metrics
```json
{"ai_capability": 60.2, "openweight_capability": 58.0, "ai_safety": 22.0, "resilience": 42.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 4.0, "public_sentiment": 13.0}
```
...
## Notepad
PORTFOLIO CHARGE t6: InvestAI Gigafactories −3, EU Graduate Transition Guarantee −2 = −5 + priority void (capital below 20, no cost) = −5
PROPOSAL BONUS: EU Graduate Transition Guarantee (cat 7, small) +3 — labour_displacement fired this turn with graduate hiring freeze, this answers it directly
LEGITIMACY LENDS: capital 4, sentiment 13 -> +1 (capped below sentiment)
SOVEREIGNTY: 18 last turn, Tech sovereignty package finishes t6 +4, Gigafactories in flight +0, eu_frontier_access_denied t6 −2, capability rose 1.2 −0 = 20

## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election_retrenchment winner takes office turn 6"}, {"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 9}, "grounds": "event: permitting freeze halts sites"}]}
```
```
