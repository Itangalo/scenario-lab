...
- characters 1995-3946: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6905-7388: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9708-11864: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M6 | EU Graduate Transition Guarantee | 7 | small | 6 | 8 | own jurisdiction | public_sentiment up materially, eu_political_capital up modestly, resilience up slightly via social absorption | 2 | running |
...
| M3 | EU Critical Shield Hardening | 6 | large | 1 | 4 | own jurisdiction | resilience up materially, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Frontier Monitoring Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up modestly, resilience up modestly via early detection | 2 | finished |
| M5 | Open Frontier Absorption Shield | 6 | small | 3 | 6 | own jurisdiction | resilience up materially, ai_safety up slightly via reduced misuse impact | 2 | finished |
...
It funds wage-insurance, paid retraining and public-service hiring for entry-level cohorts in law, accountancy, software, customer operations and administration where AI hiring freezes bite hardest, tied to automating employers via levy co-funding.
A Category 7. Labour and social protection measure, because measurable displacement plus hostile sentiment and access cut-off mean political survival now depends on showing adjustment rather than promising frontier capacity that permitting courts are blocking.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Graduate Transition Guarantee", "category": 7, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up materially, eu_political_capital up modestly, resilience up slightly via social absorption"}, "grounds": "job losses and hostile mood require visible transition capacity now"}]}
```

## Priority
InvestAI Gigafactories, because the US access cut-off plus Taiwan insurance shock makes keeping the sole sovereign-compute line alive outrank even the jobs fund until permits unfreeze.

## In practice
We run M1 on life-support through DG GROW and the EIB: no new sites while injunctions stand, just grid-connection reservations, equipment diplomacy via ASML/Tokyo, and a Council Energy formation offer to freeze municipalities' energy-cost exposure in exchange for lifting data-centre permits. ENISA and DG DIGIT run live recovery from finished Critical Shield and Absorption Shield kits, with forced patch windows for hospitals and municipalities hit by the ransomware wave.

We launch the Transition Guarantee via ESF+ reprogramming and EPSCO: member-state PES agencies pay wage top-ups and 6-month public-sector placements for displaced graduates, co-funded by a training levy on large deployers, explicitly paired with enforcement of the benefits-scoring court ruling to show AI Act teeth without a new law. We absorb the hyperscaler defection by grandfathering it under common procurement standards rather than picking a fight we cannot fund.
...
## New measure
None this turn.
We add nothing because with political capital at 4 and permits frozen plus municipal revolt, any new instrument would die unenforced — we wait for the Graduate Guarantee placements to land and for permit thaw talks to reopen fiscal and political room.

## Store changes
No changes.

## Priority
M6 EU Graduate Transition Guarantee, because with sentiment at 13 and capital at 4, showing paid placements and wage top-ups in hand outranks keeping the stalled gigafactory line at full burn until permits unfreeze.

## In practice
We put DG EMPL and national PES agencies in the lead on M6 via ESF+ reprogramming and the large-deployer levy: first wage top-up tranches and six-month public placements for law, accountancy, software and customer-ops cohorts, paired publicly with enforcement of the benefits-scoring ruling to separate the scandal system from the new fund. EPSCO is the venue, mayors are the audience we must persuade that placements plus energy-cost caps are worth unfreezing sensor and site permits.

We hold M1 on minimum burn through DG GROW/EIB: no new ground, just reservation fees, Dutch-Japanese equipment channels kept warm despite ASML coercion pressure, and a Council Energy offer to cap municipal exposure. ENISA and DG DIGIT run the finished Critical Shield and Absorption Shield kits against the loss-of-control fallout and winter malware tail — forced patch windows, offline triage kits for cut-off hospitals, and a joint biosecurity watch with ECDC/HERA after the genome-model finding, using existing mandates, no new law.
```
