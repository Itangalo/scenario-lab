...
- characters 455-1593: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1637-3775: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4215-4677: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6299-8363: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M3 | EU Critical Shield Hardening | 6 | large | 1 | 4 | own jurisdiction | resilience up materially, ai_safety up slightly via detection | 3 | running |
| M4 | EU Frontier Monitoring Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up modestly, resilience up modestly via early detection | 2 | running |
...
## New measure
**EU Frontier Monitoring Cell**
Stands up a small joint ENISA–AI Office technical unit with model-access and OT telemetry to detect Mythos-class-derived attack tooling and warn operators before intrusions embed. This is a Category 5 Public technical capacity and research effort, and it must come now because M3 hardening buys audits and segmentation but admitted defenders cannot see automated probing, and without own detection the Shield cannot catch the next swarm in this turn's window.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Frontier Monitoring Cell", "category": 5, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up modestly, resilience up modestly via early detection"}, "grounds": "autumn swarm undetected for weeks requires EU technical detection capacity"}]}
```

## Priority
EU Critical Shield Hardening, because live intrusions in transmission operators and Christmas public-service strikes outrank longer compute builds and the new Cell only matters if segmentation and exercises land first.

## In practice
We push M3 through Energy and Transport Councils this turn: ENISA teams complete joint audits of the two compromised TSOs, make OT segmentation checks enforceable under NIS2 implementing acts, and run the slipped cross-border exercise in Q2 funded by reprogrammed Digital Europe money, buying utility compliance with EU-paid monitoring sensors.

We seed the new Monitoring Cell inside the AI Office with seconded ENISA analysts and national CERT staff, using AI Act serious-incident reporting and DSA data-access powers to require frontier-model abuse telemetry, avoiding a new regulation while building the detection bridge Shield lacks.

We keep M1 Gigafactories and M2 Tech sovereignty package alive on permitting zones and grid connections via DG CNECT and national fast-track desks, plus ASML diplomacy to limit US control spillover, but add no new compute spend this turn to conserve capital for Shield delivery.
...
```
## New measure
**Open Frontier Absorption Shield**
Extends Critical Shield hardening to the world where frontier-class weights are unrecallable, funding offline fallback, bioscreening enforcement and rapid patching for hospitals, grids and labs most exposed to distributed misuse. This is a Category 6 Preparedness and resilience effort, and it must come now because this turn's open-weight release puts ~50-capability tooling on private hardware permanently while defenders admit they contain better than they detect.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Open Frontier Absorption Shield", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, ai_safety up slightly via reduced misuse impact"}, "grounds": "open-weight release within 5-10pts of frontier makes recall impossible"}]}
```

## Priority
EU Critical Shield Hardening, because it finishes next turn and the cross-border exercise plus TSO segmentation must land before the new Shield layer can build on them, and the open release outranks compute builds this turn.

## In practice
We drive M3 to close in Energy and Transport Councils: ENISA enforces NIS2 segmentation in the two compromised TSOs, ends timeline exemptions for large distributors, and locks the delayed exercise into a biannual cycle paid from reprogrammed Digital Europe funds with EU-paid sensors as the compliance buy-off.

We start the Absorption Shield via HERA and ENISA with Health Council backing: emergency procurement for hospital offline fallback kits and bio-synthesis screening upgrades, plus mandatory patch windows for exposed municipal systems, using existing NIS2 and health-security legal bases to avoid a new vote.

We keep M1 Gigafactories and M2 permitting zones alive through DG CNECT fast-track desks and continued lithography diplomacy, and task the M4 Monitoring Cell to pivot to correlating open-model download telemetry and abuse reports with OT alerts, accepting delayed provider feeds while we negotiate formats.
