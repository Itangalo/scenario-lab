...
- characters 377-2400: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2444-4469: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4525-5669: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5732-31962: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32317-32644: `{{event_history}}` from the run's own event record
...
- `emergent_clinical_agent_scare` -- first noted turn 4, listed in 1 turn(s) so far: A major European hospital group publicly attributes a near-miss medication dosing error to a widely used AI scribe-agent misreading records, triggering cross-border press coverage and calls for clinical-agent audits.
- `emergent_insurance_repricing` -- first noted turn 4, listed in 1 turn(s) so far: A major European insurer reprices AI-enabled cyber and professional-liability cover after the 2027 ransomware wave, sharply raising premiums for hospitals and municipalities that cannot prove segmented backups and mandatory reporting.
...
- Turn 1 (4 turn(s) ago): cyber_test_shot, ai_investment_collapse, export_control_escalation
- Turn 2 (3 turn(s) ago): cyber_major_incident, capability_jump, bio_uplift_findings, openweight_frontier_release
- Turn 3 (2 turn(s) ago): knowledge_work_augmented
- Turn 4 (1 turn(s) ago): labour_displacement, member_state_defection
