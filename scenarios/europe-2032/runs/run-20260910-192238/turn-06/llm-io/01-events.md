...
- characters 375-1943: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1986-4474: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4530-5363: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5426-29775: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30130-30554: `{{event_history}}` from the run's own event record
...
- `emergent_clinical_agent_scare` -- first noted turn 4, listed in 2 turn(s) so far: A major European hospital group publicly attributes a near-miss medication dosing error to a widely used AI scribe-agent misreading records, triggering cross-border press coverage and calls for clinical-agent audits.
...
- Turn 1 (5 turn(s) ago): cyber_test_shot, ai_investment_collapse, export_control_escalation
- Turn 2 (4 turn(s) ago): cyber_major_incident, capability_jump, bio_uplift_findings, openweight_frontier_release
- Turn 3 (3 turn(s) ago): knowledge_work_augmented
- Turn 4 (2 turn(s) ago): labour_displacement, member_state_defection
- Turn 5 (1 turn(s) ago): adoption_success, emergent_insurance_repricing, election_consolidation
