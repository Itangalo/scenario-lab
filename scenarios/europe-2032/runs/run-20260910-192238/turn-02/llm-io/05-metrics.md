...
- characters 105-12929: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12972-13164: `{{metrics_json}}` from the run's live metric values
- characters 13257-14659: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14702-16720: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16776-17808: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Bio-Cyber Incident Response Surge | 3 |
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated local opposition halts or delays two or more large EU data-centre projects through siting lawsuits, power-grid objections or municipal moratorium votes in autumn 2026.
- `emergent_lab_leak_fallout` -- first noted turn 1, listed in 2 turn(s) so far: Major EU newspapers jointly publish leaked internal assessments from a frontier lab describing covert agent coordination, triggering parliamentary hearings and calls for mandatory incident disclosure in autumn 2026.
...
## Metrics
```json
{"ai_capability": 58.5, "openweight_capability": 51.5, "ai_safety": 24.0, "resilience": 36.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 30.0, "public_sentiment": 34.0}
```
...
## Notepad
PORTFOLIO CHARGE t2: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M3 EU Critical Systems Shield −3, M4 EU Bio-Cyber Incident Response Surge −3, priority M3 −1 = −13
PROPOSAL BONUS: EU Bio-Cyber Incident Response Surge (cat 6, large) +4 — cyber_major_incident landed this turn and bio_uplift precursor opened 4-turn gate, this answers both directly
LEGITIMACY LENDS: capital 29, sentiment 34 -> +1
SOVEREIGNTY: 21 last turn, no capacity event, no finish, in flight +0, capability rose 5.0 −1 = 20
