...
- characters 454-1646: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1689-3305: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3929-4398: `{{statement_ledger}}` from the actor's live statement ledger
- characters 5867-8168: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_health_data_forgery_wave (emergent event):** Major European research hospitals disclose coordinated fraud and data-poisoning campaigns using Mythos-class open models to fabricate clinical trial data and insurance claims.
...
| M3 | EU Critical Infrastructure Cyber-Hardening Directive | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety flat | 3 | running |
...
## Two-year commitment
Secure European resilience and autonomy against AI-enabled disruption

## Statement changes
add `two_year_commitment` (commitment): Secure European resilience and autonomy against AI-enabled disruption over the next two years.

## New measure
**EU Critical Infrastructure Cyber-Hardening Directive**
Mandates segmented OT/IT hardening, continuous threat-hunting and exercised black-start contingency plans across electricity transmission, ports and water utilities.
This is category 6 Preparedness and resilience, and it must come now because the autumn test-shot proved segmentation failed and detection would not have caught a swarm derived from open Mythos-class models, opening a three-turn cyber window we cannot meet with existing uneven defences.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Critical Infrastructure Cyber-Hardening Directive", "category": 6, "size": "large", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, ai_safety flat"}, "grounds": "test-shot precursor demands immediate hardening"}]}
```

## Priority
EU Critical Infrastructure Cyber-Hardening Directive, because the precursor gate is open for three turns and grids in two member states were already mapped while Gigafactories cannot protect anything for years.

## In practice
We are pushing the hardening directive through DG ENER and DG CNECT with ENISA as operator, using the CER and NIS2 legal base for mandatory OT segmentation audits and EU-CyCLONe exercises, funded by repurposed RRF and CEF-Digital money to get TSOs and port authorities hunters on-site this winter. France, Germany and Poland want exemptions for municipal utilities; we buy them with co-financing and a mutual-aid pool.

In parallel we keep the two inherited sovereignty lines alive: InvestAI Gigafactories (M1) moves to site selection and grid-connection offers in the accelerated-permitting zones, and the tech sovereignty package (M2) stays in private-capital mobilisation, both slow by construction but now framed as the autonomy half of the same commitment. We signal to Washington and to ASML that resilience procurement will favour EU-anchored compute where possible, without opening a second compute programme.
...
Deploys the breakthrough swarm-behaviour detection and automated patching across electricity, ports, water and research hospitals via ENISA-coordinated rollout.
This is Preparedness and resilience work, and it must come now because the defence breakthrough visibly shifts the offence-defence balance for the first time in years while the autumn swarm proved current detection would miss a repeat and hospitals are now being poisoned with Mythos-class forgeries.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Swarm-Defence Scale-Up", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up modestly"}, "grounds": "exploit defence breakthrough before window closes"}]}
```

## Priority
EU Critical Infrastructure Cyber-Hardening Directive, because it is the mandatory legal vehicle already in flight that the new breakthrough tooling and hospital forgery response must ride on rather than a parallel track.

## In practice
We push M3 through the Energy and TTE Council on the CER/NIS2 base, using emergency CEF-Digital and EU4Health co-financing to place ENISA hunters and segmentation audits in TSOs, ports, water operators and now the affected research hospitals, buying off municipal-staff objections with a mutual-aid pool and joint black-start exercises.

In parallel we operationalise the new scale-up as M3's technical payload: ENISA certifies the breakthrough detection/patching stack, DG CNECT procures it centrally for critical operators, and ECDC-linked HERA extends it to clinical-data integrity checks after the forgery wave. M1 Gigafactories and M2 sovereignty package stay alive on site-selection and investor roadshows, but take no new political spend this turn.
