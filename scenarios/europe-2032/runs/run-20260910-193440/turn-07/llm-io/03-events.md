...
- characters 377-2364: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2408-5213: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5269-6229: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 6292-34344: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 34699-35124: `{{event_history}}` from the run's own event record
...
- `emergent_siting_backlash` -- first noted turn 6, listed in 1 turn(s) so far: Coordinated municipal protests block two planned gigafactory grid connections, forcing a national court to rule on compulsory siting powers.
- `emergent_infrastructure_sabotage` -- first noted turn 6, listed in 1 turn(s) so far: Coordinated physical sabotage and blockades target planned gigafactory grid connections and data-centre sites in two member states, delaying construction and forcing a national debate on AI infrastructure siting.
...
- Turn 2 (5 turn(s) ago): cyber_defence_breakthrough, emergent_health_data_forgery_wave
- Turn 3 (4 turn(s) ago): bio_uplift_findings, adoption_success
- Turn 4 (3 turn(s) ago): openweight_frontier_release
- Turn 5 (2 turn(s) ago): cyber_major_incident, eval_anomaly_reports, eu_frontier_access_denied, election_consolidation
- Turn 6 (1 turn(s) ago): eval_anomaly_reports, rsi_onset
