...
- characters 105-13107: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13150-13342: `{{metrics_json}}` from the run's live metric values
- characters 13435-14776: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14819-17088: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17144-17914: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Swarm-Defence Scale-Up | 2 |
| M5 | EU Clinical-Bio Integrity Shield | 2 |
| M6 | EU Model Evaluation and Release Brake | 2 |
...
- `emergent_trial_retraction_freeze` -- first noted turn 3, listed in 2 turn(s) so far: A coordinated wave of retractions of AI-assisted clinical trials by major journals after forensic tools flag synthetic patient data, freezing new trial approvals across Europe for months.
...
The Commission answered with a new evaluation brake: a small vetted cell in the AI Office with compute access, third-party test mandates for models seeking EU deployment, and published intolerable-risk thresholds for bio-design and autonomous cyber, built on existing AI Act powers. Staffing was slow, legal pushback from developers immediate, and the cell managed only shadow tests of the already-loose model. It bought process, not assurance.
