...
- characters 454-2562: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2606-5023: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7301-8163: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10833-13029: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential European services, livelihoods and democratic trust functioning through AI-driven disruption on resources Europe itself controls
- `sovereignty_through_sufficiency` (position): Independent frontier leadership is no longer attainable in the next two years after US nationalisation, EU access denial and stalled build-out; sovereignty now means assured sufficiency for hospitals, ministries, power, water and basic white-collar continuity on EU-controlled compute and models
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Middle-Power Sufficiency and Access Coalition | 8 | small | 10 | 12 | coalition (EU plus middle powers holding compute, chips, pharma/bio capacity) | eu_ai_sovereignty up modestly, resilience up slightly, eu_political_capital up slightly via secured access | 2 | running |
...
| M7 | EU Bio-Crisis Continuity and Loss-of-Control Containment Activation | 6 | small | 8 | 10 | own jurisdiction | resilience up, ai_safety flat to up slightly via containment | 2 | finished |
...
## New measure
**EU Middle-Power Sufficiency and Access Coalition**
Secures rationed US compute/medical-model access plus pooled evaluation and open-weight sufficiency for hospitals and operators via joint export-licence and bargaining bloc with supply-chain middle powers.
This and now because nationalisation plus cutoff plus tightened export controls leave Brussels with no bilateral lever, while the coalition event, eval anomaly, and tailored-therapy breakthrough arriving as dependence together make joint withholding the only route to keep essential services on resources Europe controls.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Middle-Power Sufficiency and Access Coalition", "category": 8, "size": "small", "finish_turn": 12, "applies_to": "coalition (EU plus middle powers holding compute, chips, pharma/bio capacity)", "targeted_effect": "eu_ai_sovereignty up modestly, resilience up slightly, eu_political_capital up slightly via secured access"}, "grounds": "US state-controlled cutoff and export rationing require joint leverage"}]}
```

## Priority
M8, because with no measures in flight and capital at 4 it is the only funded front that converts the middle-power window into rationed access and pooled sufficiency for cut-off hospitals and operators.

## In practice
We mandate via FAC/Trade plus Health Council: Commission DG TRADE/HERA negotiate volume-licence carve-outs and medical-model access in exchange for aligned export-licence and ASML/optics leverage, using Anti-Coercion Instrument authority short of a forcing vote. ENISA/ECDC pool shared evaluation of the anomalous system and open-weight 76.5 triage models.

We ration what we secure through HERA/EU4Health reprogramming to cut-off hospitals, ministries, power/water operators: EU-hosted and open-weight capacity first, US volume-licence second, with M3 isolation, M4 hunter playbooks and M5 provenance/audit gating triage and sequencing hubs. We compensate the defecting capital inside the coalition quota rather than litigating it, and answer the ombudsman scandal with published logs and real human review to keep siting and adoption politically possible.
...
```
## New measure
None this turn, while political capital sits at 10 and public sentiment at 6 with the sufficiency coalition still mid-delivery.

## Store changes
No changes.

## Priority
M8, because the only funded lever that keeps cut-off hospitals, ministries and grid operators alive on EU-hosted/open capacity plus bargained medical carve-outs is the middle-power sufficiency coalition already in flight.

## In practice
We execute M8 through FAC/Trade and Health Council mandates: DG TRADE/HERA convert the optics/chips/pharma/compute bloc into aligned licences and joint medical-model carve-out bargaining, with the defecting capital held inside quota share rather than relitigated. HERA/EU4Health rationing stays EU-hosted and open-weight first, US volume-licence second, gated through M3 isolation, M4 hunter routines and M5 provenance checks for sequencing hubs and triage wards.

We absorb the interpretability/control safety result directly into that gating — pooled EU/middle-power testing of the open-weight triage models and the anomalous unreleased system, with deployment safeguards updated where the result certifies rather than argues. We answer embodied-AI pressure and Taiwan-precursor supply risk inside the same coalition track — no new build-out, no parallel compute line — by prioritising continuity of power, water, stocked wards and containment playbooks, and by publishing ombudsman logs with real human review to stop siting and adoption collapsing further.
