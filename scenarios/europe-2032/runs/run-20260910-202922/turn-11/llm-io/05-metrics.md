...
- characters 107-13108: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13151-13342: `{{metrics_json}}` from the run's live metric values
- characters 13435-14411: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14455-16600: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16656-17487: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_shadow_care_network` -- first noted turn 10, listed in 2 turn(s) so far: Municipal clinics and pharmacies begin openly pooling black-market triage prompts and unvetted foreign diagnostic models into a shared informal library, creating a parallel care standard outside EU oversight.
