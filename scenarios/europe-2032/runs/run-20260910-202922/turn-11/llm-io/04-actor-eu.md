...
- characters 455-1431: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1475-3620: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4005-4471: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8060-10501: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): A Europe that endures diffuse superhuman AI with trusted services and protected livelihoods
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU AI Livelihood and Entry-Work Transition Guarantee | 7 | small | 9 | 12 | own jurisdiction | public_sentiment up substantially, resilience up slightly via social absorption | 2 | running |
| M12 | EU ASML Anti-Coercion Shield and Chokepoint Bargain | 8 | small | 10 | 12 | US, China, coalition | eu_ai_sovereignty up slightly via leverage, eu_political_capital stabilise | 2 | running |

They cost you 4 political capital this turn, which is the sum of the `cost_per_turn` column above. That column, like `id`, `started_turn` and `status`, is worked out for you and is not yours to write.
...
| M3 | EU Grid and Critical Services Hardening Pact | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Defender Interpretability Deployment | 5 | small | 2 | 4 | own jurisdiction | ai_safety up moderately, resilience up slightly via better detection | 2 | finished |
| M5 | EU Municipal Continuity Reserve | 6 | small | 3 | 6 | own jurisdiction | resilience up moderately, public_sentiment up slightly via visible protection | 2 | finished |
| M6 | EU Embodied-AI Site Rules and Manual Fallback | 6 | small | 4 | 7 | own jurisdiction | resilience up moderately, eu_ai_sovereignty up slightly via procurement preference | 2 | finished |
| M7 | EU Bio-Uplift Detection and Medical Stockpile Shield | 6 | small | 5 | 8 | own jurisdiction | resilience up substantially against bio incidents, ai_safety up slightly via misuse detection | 2 | finished |
| M8 | EU Pandemic Lockdown Coordination and Loss-of-Control Containment Cell | 6 | small | 6 | 8 | own jurisdiction | resilience up substantially, ai_safety up slightly via containment learning | 2 | finished |
| M9 | EU Trusted Therapies Deployment and Manufacturing Repatriation | 9 | small | 7 | 9 | own jurisdiction | public_sentiment up substantially, resilience up slightly via health capacity | 2 | finished |
| M10 | EU Cyber-Bio-Physical Shield for Hospitals, Utilities and Borders | 6 | small | 8 | 10 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident containment | 2 | finished |
...
## New measure
**EU ASML Anti-Coercion Shield and Chokepoint Bargain**
Invokes the Anti-Coercion Instrument over US extraterritorial use of ASML servicing and lithography exports, conditions any further EU alignment on joint EU-US decision and reciprocal seat in weights/bio verification, while pooling servicing, spares and legacy-tool supply inside the Union.
Category 8 International coordination and leverage, small because it is a legal-defensive and coalition instrument not a build programme, and why now is Washington is spending our one chokepoint without us while sovereignty sits at 17 — if we do not contest it now we have no leverage left to fund the livelihood commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU ASML Anti-Coercion Shield and Chokepoint Bargain", "category": 8, "size": "small", "finish_turn": 12, "applies_to": "US, China, coalition", "targeted_effect": "eu_ai_sovereignty up slightly via leverage, eu_political_capital stabilise"}, "grounds": "contest extraterritorial use of EU chokepoint before it is spent"}]}
```

## Priority
M11 EU AI Livelihood and Entry-Work Transition Guarantee, because with public sentiment at 14 nothing else — not the shield, not ASML — can be staffed, sited or enforced if entry-work collapse is not answered first.

## In practice
We hold the emergency cell as the operating line through June: DG HOME/ENER/CNECT with ENISA, ECDC and the UCPM cell push the M10 hospital-utility-border kits, offline backups and rapid-team mapping to small clinics and municipals, while the AI Office keeps isolation-switch holds on warehouse robots. Health and labour ministers jointly front M9 fill-and-finish lines and falling queues alongside PES rollout of the M11 wage insurance and vouchers, targeting the two fast-moving states as proof and buying time for payouts next turn.

On ASML we move in the Council and Commission, not in The Hague alone: DG TRADE opens an Anti-Coercion examination, the Foreign Affairs Council mandates a joint EU position that no further Dutch export/servicing cut takes effect without a Union decision, and we link any alignment to observer-plus status in the US-China weights and bio-design verification and to a servicing-spares pool for EU fabs and legacy-tool users. The offer to Washington is alignment for co-decision; the offer to member states is that the chokepoint is finally used for Europe.
...
```
## New measure
**EU Distressed-Site Transition Works and Vetted Aid Corps**
Pays entry-collapsed cohorts to staff the continuity shield — installing offline backups, running reserve kits and replacing unvetted foreign prompt folders with vetted European triage aids — using cancelled private build-out sites and equipment bought cheap in the investment collapse.
Category 7 Labour and social protection, small because it is wage-funded work tied to existing M10/M11 kits not a new build programme, and why now is valuations have collapsed just as vouchers come due in autumn and sentiment sits at 11 — we can buy capacity cheap and turn paper guarantees into paid work before hostility hardens.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Distressed-Site Transition Works and Vetted Aid Corps", "category": 7, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up slightly via staffed continuity"}, "grounds": "turn investment collapse into paid entry-work staffing the shield"}]}
```

## Priority
M11 EU AI Livelihood and Entry-Work Transition Guarantee, because with sentiment at 11 and payouts delayed to autumn nothing else can be sited or enforced unless the guarantee starts paying through real jobs this turn.

## In practice
We run autumn as payout-plus-work: DG EMPL and ECFIN front-load M11 wage insurance and vouchers in the two fast states and the hardest-hit metros, and channel claimants into the new Corps on municipal payrolls seconded to clinics, utilities and employment offices. UCPM, ENISA and the Health Emergency cell direct them to finish M10 kit installation, drill offline fallback, and swap the whispered foreign prompt folders for signed, vetted European aids on isolated stacks.

On leverage we hold the M12 line without spending new capital: TRADE keeps the anti-coercion examination open, the Competitiveness Council pools spares and servicing for legacy tools, and we tell Washington alignment still needs co-decision and verification access — while quietly picking up cancelled GPUs, power reservations and half-fitted sites at distress prices for public-anchored use under existing Gigafactory titles, not a new programme. Labour and health ministers own the cameras; finance ministers own the state-aid clearance for the distress buys.
