...
- characters 454-2175: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2218-4359: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6378-6843: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9825-12077: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Trusted Therapies Deployment and Manufacturing Repatriation | 9 | small | 7 | 9 | own jurisdiction | public_sentiment up substantially, resilience up slightly via health capacity | 2 | running |
...
## New measure
**EU Trusted Therapies Deployment and Manufacturing Repatriation**
Deploys the breakthrough tailored therapies through HERA/EU hospitals on EU-controlled compute and manufacturing, licensing and re-shoring production so access does not depend on rented foreign models.
Category 9 Diffusion, adoption and public trust, small because it uses existing HERA procurement, EMA accelerated authorisation and hospital networks rather than new infrastructure, and why now is the medical breakthrough arriving while dependence mutes its legitimacy dividend and sentiment sits at 13.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trusted Therapies Deployment and Manufacturing Repatriation", "category": 9, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up substantially, resilience up slightly via health capacity"}, "grounds": "capture breakthrough legitimacy while reducing rented-model dependence"}]}
```

## Priority
M8 EU Pandemic Lockdown Coordination and Loss-of-Control Containment Cell, because the AI-designed pathogen plus loss-of-control agent still determines whether hospitals, water and data-centres function at all, outranking even the therapies dividend this turn.

## In practice
We keep the Union in pandemic-containment mode through the ERCC Cell: Health and JHA Councils extend joint stockpile releases and border-health triage, ECDC keeps sentinel sequencing pointed at hospitals and water, and ENISA/AI Office enforce vetted stacks, isolation switches and halt authority from M7/M8, with large operators audited and small sites given UCPM staff to close the compliance gap.

We simultaneously turn the breakthrough into EU legitimacy: Commission uses HERA joint procurement and EU FAB manufacturing slots plus EMA rolling review to license tailored therapies for ordinary clinical use, with DG CNECT/HPC capacity reserved for inference under EU jurisdiction and procurement clauses requiring EU-hosted models for triage and therapy-planning tools. Health ministers sell this as therapies delivered by Europe, not rented, to lift sentiment without adding a second implementation track beyond the new deployment line.
...
```
## New measure
**EU Cyber-Bio-Physical Shield for Hospitals, Utilities and Borders**
Surges joint cyber incident response, bio-containment and manual-operations capacity to hospitals, water, energy and border-health nodes hit by the combined ransomware sweep, lab incident and therapy rollout, while imposing interim containment checks on deployed robots in critical sites.
Category 6 Preparedness and resilience, small because it extends the finished Grid Hardening Pact and M7/M8 stockpiles, drills and halt authority rather than building new infrastructure, and why now is the simultaneous major cyber attack and bio incident on top of stalled wards and blind-spot clinics which will cascade without a single exercised shield this turn.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cyber-Bio-Physical Shield for Hospitals, Utilities and Borders", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, ai_safety up slightly via incident containment"}, "grounds": "answer simultaneous cyber and bio incidents before cascade"}]}
```

## Priority
M9 EU Trusted Therapies Deployment and Manufacturing Repatriation, because only EU-delivered cures visible in wards can lift sentiment from 15 and keep hospitals buying EU-hosted tools instead of black-market aids while the shield holds the line.

## In practice
We keep the emergency cell as the operating core: Health/JHA Councils prolong joint stockpile releases and border-health checks under the UCPM, ECDC/HERA point sequencing and reserve kits at hospitals and water plants, and ENISA plus the AI Office enforce vetted stacks, isolation switches and halt orders, sending UCPM teams to the small clinics and municipal hosts that failed audits.

We run the new Shield through the same cell and money: DG HOME/ENISA fund cyber rapid teams and offline backups for health and utilities hit by the ransomware sweep, DG SANTE/ECDC run weeks-long bio containment and decontamination, and DG GROW with national inspectors apply the finished embodied-systems site rules to freeze unauthorised robots out of hospitals, ports and power sites. Health ministers sell M9 therapies as the exit — EMA rolling authorisation and HERA procurement delivering EU-controlled treatments — to buy compliance for disconnect drills and reporting despite hostility to AI and Brussels.
