...
- characters 1693-3695: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4836-5302: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9294-11523: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M13 | EU Distressed-Site Transition Works and Vetted Aid Corps | 7 | small | 11 | 14 | own jurisdiction | public_sentiment up moderately, resilience up slightly via staffed continuity | 2 | running |
| M14 | EU Breakthrough Shield and Cures Deployment | 6 | small | 12 | 14 | own jurisdiction | resilience up moderately, public_sentiment up moderately via cures and stopped cascades | 2 | running |
...
| M11 | EU AI Livelihood and Entry-Work Transition Guarantee | 7 | small | 9 | 12 | own jurisdiction | public_sentiment up substantially, resilience up slightly via social absorption | 2 | finished |
| M12 | EU ASML Anti-Coercion Shield and Chokepoint Bargain | 8 | small | 10 | 12 | US, China, coalition | eu_ai_sovereignty up slightly via leverage, eu_political_capital stabilise | 2 | finished |
...
## New measure
**EU Breakthrough Shield and Cures Deployment**
Rolls the new defensive cyber tooling across hospitals, utilities and municipal systems via ENISA/UCPM while licensing and manufacturing the breakthrough therapies in EU plants for free-clinic distribution, turning two foreign breakthroughs into visible local protection.
Category 6 Preparedness and resilience with a Category 9 diffusion kicker, small because it deploys already-proven tools and licensed cures through M10/M13 crews not new builds, and why now is capability just jumped again while safety sits at 4, bio-uplift opens a 4-turn gate, and sentiment at 12 will kill any shield that does not also heal.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Breakthrough Shield and Cures Deployment", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up moderately via cures and stopped cascades"}, "grounds": "deploy cyber breakthrough and medical breakthrough before bio gate opens"}]}
```

## Priority
M13 EU Distressed-Site Transition Works and Vetted Aid Corps, because without paid local crews installing patches and delivering cures this turn the breakthroughs stay press releases and hostility hardens past siting.

## In practice
We run this as work, not announcements. DG EMPL funds M13 Corps continuation, ENISA pushes the automated-patching stack and swarm-detection rules to hospital and utility operators drilled last autumn, and HERA/ECDC license the breakthrough therapy for EU fill-finish with EMA fast-track, distributed free through the same clinics and reserve-kit routes. Health and EPSCO ministers own delivery; finance holds state-aid clearance under existing titles.

On leverage we spend no new capital abroad: TRADE keeps the lithography examination open as cover while we insist any therapy or defensive-tool license includes EU manufacturing and verification access, so the medical benefit lands as sovereignty not dependence. UCPM and the Health Emergency cell direct crews to swap remaining foreign prompt folders for signed aids during the same visits that deliver patches and doses.
...
```
## New measure
**EU-US State-to-State AI Access and Restraint Pact**
Open a direct government-to-government negotiation with Washington for assured EU access to frontier systems and joint safety/restraint terms now that labs are state assets, trading EU supply-chain, siting and deployment cooperation for contracted capacity and verification.
Category 8 International coordination and leverage, small because it is a negotiating mandate and liaison cell not a build, and why now is nationalisation voids market-access leverage and makes commercial terms political — the only counterparty left is state-to-state, and without a pact EU access and safety review go to zero.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU-US State-to-State AI Access and Restraint Pact", "category": 8, "size": "small", "finish_turn": 15, "applies_to": "US", "targeted_effect": "eu_ai_sovereignty up slightly via assured access, eu_political_capital stabilise via state bargain"}, "grounds": "US nationalisation requires state-level access deal"}]}
```

## Priority
M14 EU Breakthrough Shield and Cures Deployment, because with sentiment at 20 and safety at 0 only visible patches installed and cures delivered hold cohesion while the Washington negotiation runs.

## In practice
We keep delivery first. ENISA and HERA push the patching stack and fill-finish doses through M13 municipal crews to hospitals and utilities, with EPSCO/Health Council owning free-clinic distribution and swapping informal foreign prompts for signed aids on the same visits. DG TRADE keeps lithography pooling and manufacturing-rights demands as the bargaining cover.

On Washington we act as a state. Council mandates the Commission/EEAS to seek a classified-access and supply agreement: assured compute/model access on EU soil, EU evaluator access and incident-notification, in return for lithography spares, energy/siting for allied capacity, and alignment on export and bio-risk red lines. No new build is stood up here; the Gigafactories record stays the compute line.
