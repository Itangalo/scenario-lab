...
- characters 105-13738: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13781-13973: `{{metrics_json}}` from the run's live metric values
- characters 14066-15419: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15462-17921: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17977-18614: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Bio-Uplift Detection and Medical Stockpile Shield | 2 |
| M8 | EU Pandemic Lockdown Coordination and Loss-of-Control Containment Cell | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election won by moratorium challenger, takes office turn 6"}]}
```
```
