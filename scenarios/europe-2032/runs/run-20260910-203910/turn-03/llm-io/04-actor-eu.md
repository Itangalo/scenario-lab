...
- characters 455-1897: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1941-4304: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5022-5470: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7077-8960: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up, ai_safety up slightly via detection | 3 | running |
| M4 | EU Assurance and Weight-Security Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up, resilience up slightly via verification | 2 | running |
...
## New measure
**EU Assurance and Weight-Security Cell**
Stands up independent EU evaluation and weight-security audit capacity in ENISA/EU AI Office to verify segmentation fixes and test deployed Mythos-class systems.
Category 5 Public technical capacity and research. Why this and why now: the Shield mandates baselines but operators and vendors contest them without anyone able to verify, and the sovereign hosting offer needs an EU technical verifier to make ring-fencing credible.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Assurance and Weight-Security Cell", "category": 5, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up, resilience up slightly via verification"}, "grounds": "verifying Shield baselines and vetting hosting offer"}]}
```

## Priority
M3 EU Critical Systems Shield, not M1/M2, because the rehearsal intrusion window still governs and only exercised segmentation keeps political capital from collapsing.

## In practice
We push ENISA under NIS2 to issue the transmission and port segmentation/logging baselines this spring, funded from reprogrammed CEF-Digital, with black-start and manual port exercises as the compliance test. France, Germany, Poland and the Netherlands hold co-sponsorship via grid-connection acceleration; we meet operator unfunded-mandate complaints with co-funded detection upgrades, not waivers.

We keep M1 Gigafactories and M2 Tech sovereignty package alive on paper through the financing trough, using cheap build costs and the non-US EU-only hosting offer as interim bolted-to-EU capacity under M2 partnership terms, not as a new vehicle. The new Cell is staffed from JRC, ENISA and AI Office secondees to audit weights handling and pre-release test the hosted models, giving us a verifier both for Shield and hosting claims.
...
```
## New measure
**Breakthrough Defence Adoption**
Deploys the new automated patching and swarm-detection tooling plus the certified interpretability checks across transmission, ports and the EU-hosted model stack via ENISA baselines.
Category 6 Preparedness and resilience. Why this and why now: both breakthroughs land directly on our two-year commitment to withstand disruption on infrastructure we control, and they are adoptable now because labs and vendors want them too — a cheap window to turn paper baselines into working defence before probes adapt.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Breakthrough Defence Adoption", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via deployable assurance"}, "grounds": "exploiting defence and safety breakthroughs for Shield sectors"}]}
```

## Priority
M3 EU Critical Systems Shield, not the new adoption line, because baselines and exercises are the compliance vehicle the breakthrough tooling must ride on or it becomes another unfunded add-on.

## In practice
We task ENISA under NIS2 to amend the April segmentation/logging baselines with the breakthrough detection/patching profiles, co-funded from the reprogrammed CEF-Digital/InvestAI pot — telling large operators this replaces contested bespoke logging with a certified interoperable format, and telling smaller states the EU pays integration. Autumn black-start exercises become the acceptance test.

We direct the JRC-ENISA-AI Office Cell (M4) to certify the interpretability/control result on the non-US EU-ring-fenced hosted models first, making its autumn reports the gate for wider procurement, and we hold M1/M2 Gigafactory tracks on paper with grid-connection acceleration as the holding bargain. France, Germany, Poland, Netherlands co-sponsor; we offer operators co-funded upgrades, not timeline waivers.
