...
- characters 454-1490: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1534-3863: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 8311-8790: `{{statement_ledger}}` from the actor's live statement ledger
- characters 11653-13735: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): An EU that keeps essentials running cut off from the frontier — substitution, hardening, and livelihoods
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Essential Continuity and Substitution Programme | 6 | small | 9 | 12 | own jurisdiction | resilience up, eu_political_capital stabilised, public_sentiment up slightly | 2 | running |
| M9 | EU Cyber Recovery and Assured Substitution Deployment | 6 | small | 10 | 12 | own jurisdiction | resilience up, ai_safety up slightly, public_sentiment stabilised | 2 | running |
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Assurance and Weight-Security Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up, resilience up slightly via verification | 2 | finished |
| M5 | Breakthrough Defence Adoption | 6 | small | 3 | 6 | own jurisdiction | resilience up, ai_safety up slightly via deployable assurance | 2 | finished |
| M6 | European Automation Transition Fund | 7 | small | 5 | 8 | own jurisdiction | resilience up slightly via social absorption, public_sentiment up, eu_political_capital stabilised | 2 | finished |
| M7 | EU Manufacturing Pilot Pause-and-Restart Compact | 7 | small | 7 | 9 | own jurisdiction | resilience up slightly, public_sentiment up slightly, eu_political_capital stabilised | 2 | finished |
...
## New measure
**EU Cyber Recovery and Assured Substitution Deployment**
Absorbs the automated cyber sweep across public services with emergency restoration and mutual aid, while pushing the new interpretability/control result into the EU-hosted and openweight systems now carrying hospitals, ministries and telecoms. Category 6 Preparedness and resilience — chosen now because the attack lands on already-cut-off and degraded services, and the safety breakthrough gives a rare chance to raise assurance on deployed substitution systems rather than just restore them brittle.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cyber Recovery and Assured Substitution Deployment", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly, public_sentiment stabilised"}, "grounds": "major automated cyber incident hits cut-off services as deployable safety advance arrives"}]}
```

## Priority
The new EU Cyber Recovery and Assured Substitution Deployment, because a visible attack on degraded public services outranks steady substitution work — if restoration fails, continuity fails with it.

## In practice
We run recovery through ENISA, EU-CyCLONe and the Shield continuity teams: triage the ransomware/dependency blast radius, isolate affected municipal and health IT, restore from clean backups onto EU-hosted stacks, and surge cross-border CERT mutual aid to the hardest-hit member states, funded by repurposed Shield operating lines and emergency UCPM support.

We pair restoration with assurance: the Assurance Cell with the Joint Research Centre vets the interpretability/control breakthrough, publishes deployment guidance, and mandates logging and pre-observation checks on the vetted openweight models carrying imaging triage, procurement and customer desks. Health and TTE Councils carry the mandate; telecom vendors on foreign servicing contracts get fallback support only under EU maintenance discipline and incident-reporting conditions.
...
```
## New measure
None this turn. — waiting for the two in-flight continuity deployments to finish landing and for any further commitment to be fundable without collapse.

## Store changes
No changes.

## Priority
M9 EU Cyber Recovery and Assured Substitution Deployment, because unsanctioned agentic action plus bio-uplift signals and an automated sweep demand restoration with logging and pre-checks first — without working registries and hospital IT nothing else holds.

## In practice
We run M9 through ENISA, EU-CyCLONe and national CERTs to completion: isolate, rebuild from clean backups onto EU-hosted stacks, and keep the Assurance Cell/JRC logging and pre-observation checks on the openweight models carrying imaging and procurement. M8 continuity teams keep power, health admin and municipal desks in degraded-but-running mode, with UCPM mutual aid to hardest-hit states and emergency repurposing of Shield operating lines, not new money we do not have.

We use the US-China risk agreement as cover, not as membership we have not earned: align EU incident-reporting and weight-security asks with its thin verification, push HERA and ECDC to lift passive bio-detection, and freeze any new sovereignty build until M8/M9 finish in turn 12. With political capital at 8 and sentiment at 9, we do not start what we cannot enforce; we keep essentials running cut off from the frontier, as committed.
