...
- characters 454-1344: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1388-3445: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4880-5332: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7615-8829: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_factory_occupation_wave (emergent event):** Coordinated cross-border factory occupations and equipment sabotage targeting foreign humanoid pilot lines force a temporary EU-wide pause on new manufacturing robot permits.
...
|---|---|---|---|---|---|---|---|---|---|
| M6 | European Automation Transition Fund | 7 | small | 5 | 8 | own jurisdiction | resilience up slightly via social absorption, public_sentiment up, eu_political_capital stabilised | 2 | running |
...
## New measure
None this turn, while political capital sits at 7 against 5 in running costs and adding anything would paralyse enforcement.

## Store changes
No changes.

## Priority
M1 InvestAI Gigafactories, because with sovereignty at 19 and open weights at 60 the only bridge to the automation-on-EU-soil commitment is keeping the four-to-five sites alive to turn 7 rather than opening a new front we cannot fund.

## In practice
We hold the line with what is already funded. DG CNECT and the EIB keep site-prep and grid-connection milestones in Paris-Berlin-Warsaw-The Hague moving under the existing InvestAI envelope, using accelerated-permitting zones already voted to avoid any new legislative ask this semester.

On labour we implement the Transition Fund tabled last turn through ESF+ reprogramming and the Employment Council: retraining levies and wage-insurance conditions attached to humanoid pilot permits, works-council oversight as the concession to unions to lift municipal bans. On the eval-anomaly leak we task the AI Office and the JRC-ENISA cell stood up under M4/M5 to request model-access for independent re-testing, without opening a new evaluation mandate we cannot staff or pay for now.
...
```
## New measure
**EU Manufacturing Pilot Pause-and-Restart Compact**
A temporary EU-wide pause on new manufacturing humanoid permits paired with a negotiated restart tied to retraining levies, works-council oversight and site-security standards.
This and now because coordinated occupations have forced a de facto pause anyway — owning it as a time-bound Compact under the two-year automation-on-EU-soil commitment turns sabotage into a bargaining window before member-state side-deals and walkouts fragment it further.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Manufacturing Pilot Pause-and-Restart Compact", "category": 7, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up slightly, public_sentiment up slightly, eu_political_capital stabilised"}, "grounds": "to contain occupation wave within commitment"}]}
```

## Priority
M6 European Automation Transition Fund, because with capital at 9 only the live transition fund can carry levies and wage-insurance into the pause negotiations while the new Compact is still staffing up.

## In practice
We declare the pause ourselves via an Employment Council recommendation backed by a Commission guidance on pilot permits, making the stoppage EU-wide, temporary, and conditional — not a mayoral patchwork. DG EMPL reprograms ESF+ to keep wage-insurance flowing during the pause, with Europol and national police coordinating site protection rather than evictions to avoid escalation.

We use the semester to negotiate restart criteria with IndustriAll, BusinessEurope and the occupying works councils: no restart without levy payment, oversight rights, and EU-soil data and maintenance clauses that answer the defection risk. The AI Office/JRC cell handles the eval-anomaly file separately with no new mandate, requesting re-test access only.
