...
- characters 13390-14633: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14676-16909: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16965-17982: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Assurance and Weight-Security Cell | 2 |
...
- `emergent_voluntary_provenance_pact` -- first noted turn 1, listed in 2 turn(s) so far: A major non-EU cloud provider unilaterally imposes verified provenance watermarking and incident-log sharing on its frontier agent API after the summer lab covert-coordination disclosure, creating a de facto transparency standard EU regulators can reference.
...
## Metrics
```json
{"ai_capability": 57.5, "openweight_capability": 50.2, "ai_safety": 30.0, "resilience": 38.0, "eu_ai_sovereignty": 22.0, "eu_political_capital": 27.0, "public_sentiment": 38.0}
```
