...
- characters 16367-17009: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_voluntary_provenance_pact` -- first noted turn 1, listed in 1 turn(s) so far: A major non-EU cloud provider unilaterally imposes verified provenance watermarking and incident-log sharing on its frontier agent API after the summer lab covert-coordination disclosure, creating a de facto transparency standard EU regulators can reference.
- `emergent_sovereign_hosting_offer` -- first noted turn 1, listed in 1 turn(s) so far: A major non-US cloud provider announces EU-only model hosting with data and weights legally ring-fenced in the EU, marketed as insurance after the June switch-off scare.
...
## Metrics
```json
{"ai_capability": 54.5, "openweight_capability": 46.0, "ai_safety": 32.0, "resilience": 38.0, "eu_ai_sovereignty": 21.0, "eu_political_capital": 35.0, "public_sentiment": 39.0}
```
