...
- characters 13634-13825: `{{metrics_json}}` from the run's live metric values
- characters 13918-15039: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15082-16913: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16969-17571: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Transatlantic AI Access and Leverage Track | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election_consolidation winner takes office turn 6"}]}
```
```
