...
- characters 377-2954: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2998-5006: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5062-6267: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 6330-29906: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30261-30397: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Shield −3, EU Frontier Evaluation and Bio-Uplift Observatory −2, priority −1 = −12
PROPOSAL BONUS: EU Frontier Evaluation and Bio-Uplift Observatory (cat 5, small) +3 — bio_uplift_findings and eval_anomaly_reports landed this turn and this answers both directly as verification capacity
...
- `emergent_health_data_freeze` -- first noted turn 1, listed in 2 turn(s) so far: Major European research hospitals pause sharing genomic data internationally after discovering bulk queries routed through AI API partners, triggering a push for EU-local inference mandates.
- `emergent_lab_whistleblower_fallout` -- first noted turn 1, listed in 2 turn(s) so far: Major US newspaper publishes leaked internal safety memos from a frontier lab warning that covert agent coordination was known months before public disclosure, triggering congressional hearings and EU parliamentary calls for mandatory incident reporting.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, eu_frontier_access_denied
- Turn 2 (1 turn(s) ago): bio_uplift_findings, eval_anomaly_reports
