...
- characters 1573-3447: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4359-4856: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7950-10000: `{{previous_actions}}` from the actor's own response from the previous turn
...
```
## New measure
None this turn, because with political capital at 2 and public sentiment at 0 we cannot fund a second instrument alongside containment without breaking it.

## Store changes
No changes.

## Priority
M11 EU Bio Containment and Essential Continuity Shield, because lit wards and traced contacts are the only legitimacy we have left while export-control rationing and ASML coercion bite.

## In practice
We run M11 through HERA/ECDC and the Civil Protection Mechanism under Article 122: pooled inference triage for hospitals and grids, rationing dashboards kept dimmed but explicit, continuity payments cleared first to nurses, grid crews and late-paid containment teams to stop mayoral curtailment suits spreading.

On coercion we hold, not build: DG TRADE refuses further ASML servicing cuts where EU law blocks them, freezes new Chinese-logistics-on-US-stack procurement pending stop-signal certification, and channels the Japanese-Korean inference compact into the shield to offset allied volume-licence rationing. No new fund, no new mandate — we ask Council for forbearance, not money, until containment closes.
