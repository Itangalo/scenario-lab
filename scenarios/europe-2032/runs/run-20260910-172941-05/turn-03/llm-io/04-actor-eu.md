...
- characters 455-1626: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1670-3542: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3934-4409: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6351-8217: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | Municipal and Hospital Patch Sprint | 6 | small | 2 | 4 | own jurisdiction | resilience up modestly, fast | 2 | running |
...
## New measure
**Municipal and Hospital Patch Sprint**
It pushes pre-packaged detection rules, offline backups and emergency patching to municipalities, hospitals and water operators most exposed by downloadable offensive cyber tradecraft.
This is Preparedness and resilience work, and why now is the open-weight release putting grid-intrusion tradecraft on private hardware permanently while our large hardening line needs seasons to land — a small fast sprint buys absorption this turn.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Municipal and Hospital Patch Sprint", "category": 6, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "resilience up modestly, fast"}, "grounds": "responding to unrecallable open-weight offensive cyber release"}]}
```

## Priority
EU Grid and Essential Services Hardening Programme, because joint detection audits and live exercises due by spring are the core of our two-year resilience commitment and the sprint only buys time for them.

## In practice
We keep M3 as the main effort: ENISA and ENTSO-E run the joint detection audits ordered under NIS2/CER, with CEF-Energy and Digital Europe repurposed funds plus EIB loans paying for sensors; TSOs that file mandatory incident reports get connection priority and co-funding. Exercises with transmission, port and water operators run through spring.

Alongside it we launch the Patch Sprint via the Health and Digital Council formations, using HERA and ENISA playbooks, civil-protection stockpiles and procurement of backup kits for municipalities and hospitals that cannot wait for retrofits. InvestAI Gigafactories (M1) and the Tech sovereignty package (M2) stay in husbanding mode — permits, sites, grid reservations and lithography protection — with no new cash calls while capital is fled.
...
```
## New measure
**EU-Wide Defensive Tooling Scale-Up**
It deploys the breakthrough automated patching and swarm-behaviour detection across grid, health, municipal and water operators via ENISA playbooks and repurposed Digital Europe funds.
This is Preparedness and resilience work, and why now is the defence breakthrough shifting offence-defence balance back just as open-weight tradecraft sits permanently on private hardware — a window to turn a lab advance into absorption before attackers adapt.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU-Wide Defensive Tooling Scale-Up", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety spillover via detection"}, "grounds": "exploiting defensive breakthrough to blunt distributed offensive capability"}]}
```

## Priority
EU-Wide Defensive Tooling Scale-Up, because the breakthrough makes our two-year resilience commitment suddenly winnable this turn and outranks husbanding compute permits and sites.

## In practice
We push ENISA and the Health Emergency Preparedness and Response Authority to package the new automated patching and swarm-detection into the existing Municipal and Hospital Patch Sprint (M4), so kits that sat boxed get working detection rules and large-city successes replicate to smaller councils. Funding comes from repurposed Digital Europe and CEF-Energy co-funding tied to mandatory incident reporting, with TSOs keeping connection priority if they install.

InvestAI Gigafactories (M1) and Tech sovereignty package (M2) stay in low-cost husbanding — permits, grid reservations, lithography shielding — with no new cash call while capital is thin. The finished Grid Hardening line (M3) provides the sensor base and joint-audit venues on which the new tooling lands, and the eastern water-operator save is used in Council to overcome local data-centre opposition to siting sensors and relays.
