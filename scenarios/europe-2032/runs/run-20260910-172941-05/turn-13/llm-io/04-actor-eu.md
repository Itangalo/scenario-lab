...
- characters 456-1656: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1700-3855: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4405-4853: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8324-10333: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Absorb AI disruption without losing essential services or social cohesion
...
|---|---|---|---|---|---|---|---|---|---|
| M13 | EU Hardened Clinical IT Replacement Fund | 6 | small | 12 | 14 | own jurisdiction | resilience up, public_sentiment up modestly by restoring care IT | 2 | running |
...
| M3 | EU Grid and Essential Services Hardening Programme | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety spillover via incident reporting | 3 | finished |
| M4 | Municipal and Hospital Patch Sprint | 6 | small | 2 | 4 | own jurisdiction | resilience up modestly, fast | 2 | finished |
| M5 | EU-Wide Defensive Tooling Scale-Up | 6 | small | 3 | 4 | own jurisdiction | resilience up, ai_safety spillover via detection | 2 | finished |
| M6 | EU Incident Surge Response | 6 | small | 4 | 5 | own jurisdiction | resilience up modestly, contain incident damage | 2 | finished |
| M7 | EU Assurance and Sovereign Evaluation Capacity | 5 | small | 5 | 7 | own jurisdiction | ai_safety up, eu_ai_sovereignty up modestly | 2 | finished |
| M8 | EU Frontier Open-Weight Containment Thresholds | 3 | small | 6 | 8 | own jurisdiction | ai_safety up modestly, resilience up modestly via reduced misuse surface | 2 | finished |
| M9 | EU Displaced-Worker Transition Guarantee | 7 | small | 7 | 9 | own jurisdiction | public_sentiment up, eu_political_capital up, resilience up modestly | 2 | finished |
| M10 | EU Supply-Chain Leverage Compact | 8 | small | 8 | 10 | coalition | eu_ai_sovereignty up modestly, eu_political_capital up modestly | 2 | finished |
| M11 | EU Loss-of-Control Containment and Care Continuity Protocol | 6 | small | 10 | 12 | own jurisdiction | resilience up, ai_safety up modestly, public_sentiment up modestly | 2 | finished |
| M12 | EU Cyber Major Incident Surge and Service Continuity Operation | 6 | small | 11 | 13 | own jurisdiction | resilience up, public_sentiment up modestly by containing damage | 2 | finished |
...
## New measure
**EU Hardened Clinical IT Replacement Fund**
Pays for EU-standard hardened clinical systems to replace shadow AI tools and paper fallback, breaking the hospital-insurer refusal by funding the disconnect they demand.
This and now because the care-IT standoff prolongs reinfection and blocks the surge operation, and under the absorb-disruption commitment restoring clean clinical continuity outranks adding new law at capital 17.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Hardened Clinical IT Replacement Fund", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up modestly by restoring care IT"}, "grounds": "to break hospital-insurer standoff prolonging reinfection"}]}
```

## Priority
EU Cyber Major Incident Surge and Service Continuity Operation, because stopping cascade and keeping therapy wards open now outranks the slower clinical-IT rebuild for holding the absorb-disruption commitment.

## In practice
We act through the Health and EPSCO Councils with ENISA, HERA and DG SANTE as joint leads: invoke the Cyber Solidarity reserve and EU4Health / rescEU money to co-fund hardened clinical IT kits, clean images and compatible surge hardware for refusing hospital groups and municipalities, conditional on disconnecting shadow systems and accepting ENISA isolation playbooks.

We pair it with the joint offer on the table: pool attribution and real-time telemetry into the invited joint cyber command via CERT-EU, publish AI Office restoration counts, and adopt the new interpretability control result into hospital and municipal deployment checks so the surge holds while replacements land. Treasuries withholding funds are offered arrears-clearing on income-bridges tied to verified disconnects, and the member state contesting cross-border holds is met with a narrow Article 168/172 health-security legal base limited to clinical continuity.
...
```
## New measure
**EU Municipal Clean Rebuild and Service Restoration Surge**
Stands up joint CERT-EU and national clean-rebuild teams with pre-imaged hardware and paper-to-digital restoration kits to close the unpatched municipal and clinic-contractor tail where the automated sweep persists.
This and now because the new large automated attack lands on the same brittle municipal-clinical surface that stalled the last surge, and under the absorb-disruption commitment keeping wards and town halls running outranks any new build while capital is at 16.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Clean Rebuild and Service Restoration Surge", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up modestly by restoring services"}, "grounds": "to contain the new major automated attack on municipal-clinical IT"}]}
```

## Priority
EU Hardened Clinical IT Replacement Fund, because verified disconnects with funded hardened replacements are the only way to stop reinfection around the refusing hospital groups and hold the absorb-disruption commitment.

## In practice
We act through the JHA and Health Councils with ENISA, CERT-EU and HERA as joint leads: extend the Cyber Solidarity reserve invoked for M12 into municipal rebuild cells, co-funded from EU4Health/rescEU and the M13 replacement fund, delivering clean images, compatible plugs and isolation playbooks conditional on disconnecting shadow assistants.

We pool real-time telemetry into the invited joint cyber command for attribution and patch targeting, publish daily restoration counts via the AI Office to rebuild thin trust, and tie treasury arrears-clearing on income-bridges to verified disconnects. The health-security legal base for cross-border clinical holds is used narrowly to keep therapy stacks isolated while surrounding systems are cleaned, with the new interpretability check mandatory in redeployment reviews.
