...
- characters 377-1805: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1849-3717: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3773-4587: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4650-28877: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29232-29319: `{{event_history}}` from the run's own event record
...
- `emergent_grid_toolkit_leak` -- first noted turn 2, listed in 1 turn(s) so far: A second grid-intrusion toolkit derived from the autumn probes is posted to public forums, letting copycat actors replay relay-mapping against smaller utilities.
