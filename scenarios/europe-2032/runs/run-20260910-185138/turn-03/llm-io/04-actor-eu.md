...
- characters 455-1853: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1897-3972: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4344-4825: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6463-8516: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_eval_leak_pressure (emergent event):** A major European investigative consortium publishes leaked internal documents from a frontier lab detailing withheld cyber evaluations around the Mythos-class models, triggering coordinated calls for an EU-level disclosure mandate.
...
| M3 | EU Critical Systems Cyber Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 3 | running |
| M4 | Joint Supply-Chain Leverage Pact | 8 | small | 2 | 5 | coalition | eu_ai_sovereignty up moderately, eu_political_capital down slightly if fails | 2 | running |
...
## New measure
**Joint Supply-Chain Leverage Pact**
Coordinates the Netherlands, Japan and South Korea on lithography and memory chokepoints to secure allied volume licences for EU gigafactories while aligning onward export screening, using the Anti-Coercion Instrument as backstop.
Category 8 International coordination and leverage is why now: Washington is tightening controls again and the EU will either be rationed alongside adversaries or bargain its upstream position for assured access before fab orders lock.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Joint Supply-Chain Leverage Pact", "category": 8, "size": "small", "finish_turn": 5, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up moderately, eu_political_capital down slightly if fails"}, "grounds": "responding to export_control_escalation"}]}
```

## Priority
EU Critical Systems Cyber Shield, because staged implants are still in grids and ports and hardening must land before the next swarm moves from mapping to disruption, which outranks even the chip bargain this turn.

## In practice
We drive the Shield through ENISA joint teams in TSOs and the two affected operators, with a CER/NIS2 implementing act in the Energy and TTE Council for mandatory segmentation audits and credential rotation. Reprogrammed CEF/Digital Europe funds pay first audits; we offer municipal utilities and the holdout port co-financing and phased deadlines to buy compliance, while DG CNECT ring-fences gigafactory grid connections with EIB guarantees.

In parallel we open the leverage track in the Foreign Affairs Council and Trade configuration: mandate to negotiate volume-licence access for EU sites in exchange for aligned screening on lithography and AI-chip re-exports, linking ASML maintenance and Zeiss optics. We welcome the journals-synthesis moratorium and task HERA and ECDC to map EU DNA-synthesis screening uptake, feeding that baseline into the Shield's bio-detection planning without opening a second instrument now.
...
```
## New measure
**EU Frontier Disclosure Mandate**
Requires frontier developers operating in the EU to disclose pre-release cyber and emergent-capability evaluations, including withheld results, to ENISA and the AI Office under a common template.
Category 2 Transparency and reporting is why now: the Mythos leak shows labs are sitting on cyber evaluations the grid operators needed months ago, and the window to turn outrage into a disclosure rule closes before the news cycle moves on.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Frontier Disclosure Mandate", "category": 2, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, eu_political_capital up slightly if lands"}, "grounds": "responding to emergent_eval_leak_pressure"}]}
```

## Priority
EU Frontier Disclosure Mandate, because the leak has created a rare mandate for oversight that will evaporate if not seized now, and it directly feeds the Shield with the evaluation data operators lacked — outranking even hardening delivery this turn.

## In practice
We table an implementing package under the AI Act's serious-incident and systemic-risk articles in the TTE Council, tasking the AI Office with a binding disclosure template and ENISA as the secure recipient for cyber evaluations, with whistleblower protections attached. DG CNECT negotiates with the large labs for interim voluntary filing while the act is drafted, trading accelerated gigafactory permitting cooperation for early compliance.

In parallel we keep the Shield moving through ENISA joint teams: we close the co-financing deals with municipal utilities and the North Sea port for phased audits, and use the Competitiveness Council to ring-fence grid-crew capacity so gigafactory connections are not pitted against segmentation work. The Supply-Chain Pact continues in Trade/FAC track to lock assured licences, and HERA/ECDC feed the synthesis-screening map into bio-detection planning without new law.
```
