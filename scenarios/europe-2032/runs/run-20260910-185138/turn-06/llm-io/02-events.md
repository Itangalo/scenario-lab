...
- characters 2306-4405: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4461-5475: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5538-27438: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27793-28154: `{{event_history}}` from the run's own event record
...
- `emergent_disclosure_template_freeze` -- first noted turn 4, listed in 2 turn(s) so far: A national constitutional court or the EU General Court suspends application of the AI Office's interim voluntary filing template and whistleblower protections pending a legal-base ruling, freezing pre-release cyber-capability disclosure for months.
- `emergent_full_eval_dump` -- first noted turn 4, listed in 2 turn(s) so far: A protected source releases the full withheld cyber-evaluation files referenced in the September leak, including test dates overlapping last year's grid intrusions, triggering parliamentary inquiries and operator lawsuits.
...
- Turn 2 (4 turn(s) ago): export_control_escalation, emergent_synthesis_screening_pact
- Turn 3 (3 turn(s) ago): emergent_eval_leak_pressure
- Turn 4 (2 turn(s) ago): openweight_frontier_release, taiwan_tension_rise, automated_decision_scandal
- Turn 5 (1 turn(s) ago): export_control_escalation, election_consolidation
