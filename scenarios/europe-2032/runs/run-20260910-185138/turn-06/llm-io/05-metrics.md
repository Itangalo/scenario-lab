...
- characters 105-13272: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13315-13507: `{{metrics_json}}` from the run's live metric values
- characters 13600-15487: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15530-17629: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17685-18019: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Displaced Workers Transition Fund | 2 |
...
SOVEREIGNTY: 20 last turn, no capacity event? export_control_escalation t5 −2, Gigafactories in flight +0, capability rose 1.5 −0 = 18
...
**emergent_disclosure_template_freeze (emergent event):** A national constitutional court or the EU General Court suspends application of the AI Office's interim voluntary filing template and whistleblower protections pending a legal-base ruling, freezing pre-release cyber-capability disclosure for months.
**emergent_full_eval_dump (emergent event):** A protected source releases the full withheld cyber-evaluation files referenced in the September leak, including test dates overlapping last year's grid intrusions, triggering parliamentary inquiries and operator lawsuits.
...
## Notepad
PORTFOLIO CHARGE t6: M1 InvestAI Gigafactories −3, M6 EU Displaced Workers Transition Fund −2, priority −1 = −6
PROPOSAL BONUS: EU Displaced Workers Transition Fund (cat 7, small) +3 — labour_displacement fired this turn with graduate hiring freeze and sentiment at 22, this answers it directly
LEGITIMACY LENDS: capital 10, sentiment 14 -> +2
SOVEREIGNTY: 18 last turn, Tech sovereignty package finishes t6 +4, M1 InvestAI Gigafactories in flight +1, ai_investment_collapse t6 −2, capability rose 1.0 −0 = 21
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election won on tiered rationing, takes effect turn 6"}, {"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 8}, "grounds": "named priority"}]}
```
```
