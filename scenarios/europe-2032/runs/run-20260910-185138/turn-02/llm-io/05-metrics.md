...
- characters 105-12931: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12974-13166: `{{metrics_json}}` from the run's live metric values
- characters 13259-14322: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14365-16318: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16374-17277: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_eval_leak_pressure` -- first noted turn 1, listed in 2 turn(s) so far: A major European investigative consortium publishes leaked internal documents from a frontier lab detailing withheld cyber evaluations around the Mythos-class models, triggering coordinated calls for an EU-level disclosure mandate.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 37.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 36.0, "public_sentiment": 37.0}
```
