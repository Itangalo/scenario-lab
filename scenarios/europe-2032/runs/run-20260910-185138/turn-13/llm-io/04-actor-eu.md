...
- characters 456-1662: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1706-3532: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4730-5212: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8796-10794: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_genome_publication_moratorium_push (emergent event):** WHO-led inquiry into the autumn designed-pathogen outbreak names published genome-model methods as an enabling factor, triggering calls for binding pre-publication review that split journals and funders.
...
- `two_year_commitment` (commitment): Contain AI-enabled bio uplift and hold the Union together through enforced continuity and common bargaining
...
|---|---|---|---|---|---|---|---|---|---|
| M12 | EU Bio-Lab Recovery and Municipal Response Staffing Surge | 6 | small | 12 | 14 | own jurisdiction | resilience up moderately, public_sentiment up slightly, eu_political_capital up slightly if visible | 2 | running |
...
| M3 | EU Critical Systems Cyber Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 3 | finished |
| M4 | Joint Supply-Chain Leverage Pact | 8 | small | 2 | 5 | coalition | eu_ai_sovereignty up moderately, eu_political_capital down slightly if fails | 2 | finished |
| M5 | EU Frontier Disclosure Mandate | 2 | small | 3 | 5 | own jurisdiction | ai_safety up moderately, eu_political_capital up slightly if lands | 2 | finished |
| M6 | EU Displaced Workers Transition Fund | 7 | small | 6 | 8 | own jurisdiction | resilience up moderately, eu_political_capital up slightly, public_sentiment up slightly | 2 | finished |
| M7 | EU Essential Services Continuity Reserve | 6 | small | 7 | 8 | own jurisdiction | resilience up moderately, eu_political_capital up slightly if visible | 2 | finished |
| M8 | EU Youth Construction and Jobs Compact | 7 | small | 8 | 9 | own jurisdiction | public_sentiment up moderately, resilience up slightly, eu_political_capital up slightly if blockades lift | 2 | finished |
| M9 | EU Middle-Power Bargaining and Clinical Deployment Pact | 8 | small | 9 | 10 | coalition | eu_ai_sovereignty up moderately, eu_political_capital up, public_sentiment up if therapies deploy | 2 | finished |
| M10 | EU Bio-Continuity Shield and Single-Line Enforcement | 6 | small | 10 | 12 | own jurisdiction | resilience up substantially, ai_safety up slightly, eu_political_capital up slightly if defections stop | 2 | finished |
| M11 | EU Middle-Power Bio-Containment and Compute Access Compact | 8 | small | 11 | 13 | coalition | eu_ai_sovereignty up moderately, eu_political_capital up, resilience up slightly if containment holds | 2 | finished |
...
## New measure
**EU Bio-Lab Recovery and Municipal Response Staffing Surge**
It funds emergency rehiring, trainee stipends and municipal response teams to restaff exhausted sequencing labs and clinics and restore day-to-day oversight. This and why now because containment held but labs are empty and legitimacy is at 10-12 — without staffed hubs and visible local recovery the common queue and the middle-power compact have no delivery arm.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Lab Recovery and Municipal Response Staffing Surge", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up slightly, eu_political_capital up slightly if visible"}, "grounds": "labs exhausted and trainee uptake stalled after containment"}]}
```
## Priority
M11 EU Middle-Power Bio-Containment and Compute Access Compact, because only joint licences and pooled eval give us leverage to hold the single line while domestic staffing rebuilds, and it needs political push to reach domestic approvals.
## In practice
We run M11 through DG TRADE and FAC-Trade: take the initialled text to capitals for domestic approval, align bio/compute licence lists, and pool the short-staffed eval quota into joint compute bargaining, offering pooled therapy inputs and audited inference slots as the price for no new bilateral hosting deals. The two frozen-queue capitals are offered re-entry on common audit, no lawsuits.

In parallel we stand up the staffing surge via HERA/ECDC and EU4Health plus ESF+: emergency waivers and stipends for sequencing trainees, rehiring for hub labs, and funded municipal teams for wastewater sampling, clinic order and mortuary backup. Mayors get policing and continuity payouts conditional on staying in the common queue. We do not open a new compute build; all inference stays on EU-hosted hardened hubs with mandatory genome-model reporting.
...
```
## New measure
**EU Accession Track to the US-China AI Risk Floor and Joint Verification Cell**
It seeks formal EU inclusion in the weights-security / escalation / bio-design-tools agreement as third verifier, transposing its floor into EU law and offering pooled eval and audited hubs as verification contribution. This and why now because exclusion from the only real capability-slowing deal would strand the Union with no leverage and no safety dividend, while the WHO moratorium push gives us a bio-governance entry point partners need.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Accession Track to the US-China AI Risk Floor and Joint Verification Cell", "category": 8, "size": "small", "finish_turn": 15, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up slightly, ai_safety up moderately if included, eu_political_capital up slightly if inside deal"}, "grounds": "US-China agreement creates inclusion window and verification demand"}]}
```
## Priority
New Accession Track measure, because getting inside the US-China floor before its verification practices harden is the one move that can restore leverage, safety cooperation and political standing at once, outranking even staffing recovery.
## In practice
We task EEAS plus DG TRADE/CNECT to seek observer-then-party status in the US-China follow-on group, tabling the initialled Japan-Korea licence alignment and our eval pool as Europe's verification offer, and we transpose the agreed weights-security and bio-design-tool controls via an implementing act under the AI Act and Dual-Use Regulation. HERA/ECDC enforce the WHO-linked pre-publication review for genome-model work through the common queue: no audited-hub clearance, no funding or publication sign-off.

In parallel we keep M12 running through EU4Health/ESF+ rehires and municipal sampling teams, with mayoral payouts still conditional on staying in the queue, to show the Union can verify what it signs. The two side-deal capitals are told re-entry on common audit is now also the ticket to shared US-China verification slots and therapy inputs — no separate lane.
