...
- characters 16241-16893: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_eval_leak_pressure` -- first noted turn 1, listed in 1 turn(s) so far: A major European investigative consortium publishes leaked internal documents from a frontier lab detailing withheld cyber evaluations around the Mythos-class models, triggering coordinated calls for an EU-level disclosure mandate.
- `emergent_synthesis_screening_pact` -- first noted turn 1, listed in 1 turn(s) so far: Major journals and DNA synthesis firms announce a voluntary moratorium on publishing and fulfilling AI-designed virus genome methods after the phage results, creating a de facto screening norm outside any treaty.
