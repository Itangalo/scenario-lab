...
- characters 452-1891: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1934-3763: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4357-4820: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7156-9060: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_youth_site_blockades (emergent event):** Coordinated graduate-led blockades of data-centre and gigafactory sites in three or more member states force a temporary construction halt and policing response.
...
Stands up an emergency fallback that moves cut-off hospitals, ministries and operators onto EU-controlled models and audited utilities playbooks, with ENISA-led triage for the automated attack. This is preparedness and resilience when denial plus a major incident hits at 12 capital and 14 sentiment, and why now is because continuity cannot wait for gigafactories or payouts months away.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Continuity Reserve", "category": 6, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, eu_political_capital up slightly if visible"}, "grounds": "frontier access cut plus automated attack on public services"}]}
```

## Priority
M6 EU Displaced Workers Transition Fund, because the access denial and cyber incident demand the new Reserve be pushed hardest this turn even above our two-year sovereign rebuild — continuity first or nothing else holds.

## In practice
We trigger the Civil Protection Mechanism and Health Emergency Preparedness and Response Authority with ENISA in lead: isolation-contained segmentation drills become live triage, audited power/ports/water operators get mutual-aid crews, and DG DIGIT brokers emergency inference from the first gigafactory-adjacent capacity and licensed EU-hosted models for hospitals and ministries cut off by Washington.

We keep M1 alive on committed funds only — defending grid queues in TTE/Energy Council, no new cash — and we accelerate M6 payouts via EPSCO and national PES agencies, using visible wage-bridge disbursements and restored services as the sole argument to stop sentiment at 14 tipping into physical opposition. We do not table binding disclosure while the court freeze holds; inquiries get the dumped files.
...
```
## New measure
**EU Youth Construction and Jobs Compact**
It pairs a policed construction-continuity protocol for gigafactory and data-centre sites with guaranteed local hiring, apprenticeships and wage-bridge slots for blockading graduates, run through municipalities and public employment services. This and now because coordinated blockades in three states plus a member-state side-deal have stalled the only sovereign capacity still moving, and with capital at 6 and sentiment at 9 we cannot hold sites by force alone — we must trade jobs for access.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Youth Construction and Jobs Compact", "category": 7, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up slightly, eu_political_capital up slightly if blockades lift"}, "grounds": "graduate-led site blockades halting sovereign builds amid defection"}]}
```
## Priority
M8 EU Youth Construction and Jobs Compact, because unblocking sites and defusing graduate anger outranks even continuity triage this turn — without sites and cohesion no sovereign rebuild holds.
## In practice
We task EPSCO and the Employment Committee with the compact, funded by repurposed ESF+ and the finished Transition Fund envelope: mayors get policing guidelines under the Civil Protection Mechanism that keep one construction lane open while guaranteeing dialogue zones, and contractors on the first gigafactory keep grid queues only if they sign local-hire and paid trainee quotas verified by national PES agencies.

We answer the defection by not chasing it publicly: the Commission offers the defecting capital the same compact terms plus ENISA-audited continuity connection, making the side-deal redundant for hospitals and ministries without triggering an Article 258 fight we cannot afford at 6 capital. DG ENER and ENISA keep mutual-aid crews and emergency inference brokering on existing playbooks, no new instrument, to hold essential services through the spring.
