...
- characters 455-1721: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1765-3775: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4167-4647: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6561-8817: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential European systems running through AI-enabled shocks without depending on foreign permission
...
| M4 | EU Assurance Adoption Cell | 5 | small | 2 | 4 | own jurisdiction | ai_safety up, resilience up modestly | 2 | running |
...
| id | name | category | size | started_turn | finish_turn | applies_to | targeted_effect | cost_per_turn | status |
|---|---|---|---|---|---|---|---|---|---|
| M3 | EU Grid and Lifeline Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up strongly, ai_safety up slightly via incident learning | 3 | finished |
...
## New measure
**EU Assurance Adoption Cell**
Puts the defensive-tooling and interpretability breakthroughs to work inside EU oversight — ENISA pushes swarm-behaviour detection and auto-patching into operators under the Grid Shield, while the AI Office bakes certifiable control checks into pre-deployment evaluation.
This is category 5 Public technical capacity and research, because it turns an external safety and cyber-defence result into standing EU evaluation capability, and why now is that both breakthroughs are adopted quickly only if we adopt them this turn before the defection narrative freezes cooperation.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Assurance Adoption Cell", "category": 5, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up, resilience up modestly"}, "grounds": "capture safety and cyber-defence breakthroughs before window closes"}]}
```

## Priority
EU Grid and Lifeline Shield over the compute lines and the new Cell, because the two-year commitment to keep essential systems running through shocks is judged on exercised hardening by spring, not on papers adopted.

## In practice
We keep ENISA paired with ENTSO-E, ports and water under NIS2 implementing acts in the Energy and Telecom Councils, using CEF Energy and Digital Europe money to force segmentation audits and relay monitoring into municipal utilities and mid-size ports, with cohesion earmarks to hold finance ministries and clean-power conditions for greens. The new Cell sits inside the AI Office with ENISA secondees and JRC support, funded by reprogrammed Digital Europe evaluation funds, to write the breakthrough detection and interpretability checks into cross-border exercises and pre-release test protocols.

The Gigafactories (M1) and tech sovereignty package (M2) stay alive on DG CNECT/EIB shortlists, permitting zones and lithography talks, but without new cash this turn. The defecting member state's side-deal is contained bilaterally — we do not open a public infringement fight now, we offer interconnection and mutual-aid access conditional on NIS2 compliance, to avoid spending scarce capital while hardening delivers visible success.
...
```
## New measure
**Municipal Lifeline Close-Out**
Extends the Grid Shield's segmentation, relay monitoring and mutual-aid exercises to municipal utilities and mid-size ports that lagged, using the new swarm-detection tooling.
This is Preparedness and resilience work because it hardens the exact laggard surface where the autumn intrusion still sits downloadable, and why now is that large operators are quiet while municipals remain the cascade path before winter.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Municipal Lifeline Close-Out", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "close municipal and mid-port gap left by Grid Shield"}]}
```

## Priority
Municipal Lifeline Close-Out over M1/M2 and the Adoption Cell, because the two-year commitment to keep essential systems running is now judged on whether laggards stop being the cascade path, not on compute papers.

## In practice
We run this through ENISA with ENTSO-E and the ports association under NIS2 implementing acts in the Energy and Transport Councils, paying from CEF Energy and Digital Europe reprogrammed funds for segmentation audits, credential rotation teams and relay monitors, with cohesion top-ups to get mayors and port boards to sign. Interconnection and mutual-aid access stay conditional on compliance, which contains the side-deal member state without a public infringement fight.

The Assurance Adoption Cell (M4) finishes its write-in of swarm detection and certifiable control checks into pre-release tests and cross-border exercises via the AI Office and JRC, while M1 Gigafactories and M2 sovereignty package stay alive on DG CNECT/EIB shortlists and permitting zones without new cash this turn to preserve capital.
