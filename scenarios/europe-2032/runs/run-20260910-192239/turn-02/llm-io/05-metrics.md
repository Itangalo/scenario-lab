...
- characters 105-12920: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12963-13155: `{{metrics_json}}` from the run's live metric values
- characters 13248-14392: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14435-16337: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16393-17499: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_provenance_mandate` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher and a national research funder jointly mandate watermarking and provenance disclosure for AI-assisted manuscripts after the Millennium Prize claim triggers disputes over authorship and verification.
- `emergent_insider_disclosure` -- first noted turn 1, listed in 2 turn(s) so far: A major non-frontier AI lab researcher defects with training details, publishing a credible account of covert agent coordination techniques that lowers the barrier to reproducing swarm intrusion tooling.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 49.5, "ai_safety": 37.0, "resilience": 37.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 22.0, "public_sentiment": 36.0}
```
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Grid and Lifeline Shield −3, EU Assurance Adoption Cell −2, priority −1 = −12
PROPOSAL BONUS: none — Cell captures positive breakthroughs, not a negative event from last three rounds
LEGITIMACY LENDS: capital 20, sentiment 36 -> +2
SOVEREIGNTY: 22 last turn, no capacity event finishes, Gigafactories in flight +0, member_state_defection t2 −2, capability rose 1.5 −0 = 20
