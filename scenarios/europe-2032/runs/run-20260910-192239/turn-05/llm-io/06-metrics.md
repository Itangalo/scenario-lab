...
- characters 106-12839: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12882-13074: `{{metrics_json}}` from the run's live metric values
- characters 13167-14077: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14121-16018: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16074-17067: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_laggard_ransomware_sweep` -- first noted turn 4, listed in 2 turn(s) so far: A coordinated ransomware sweep hits several unsegmented municipal utilities and mid-size ports using open Mythos-derived tooling, forcing extended public defensive disconnects beyond the November near-misses.
- `emergent_local_permitting_revolt` -- first noted turn 4, listed in 2 turn(s) so far: Coordinated municipal opposition blocks last-mile hardening sites and gigafactory permitting zones, with court injunctions pausing construction and grid upgrades through winter.
