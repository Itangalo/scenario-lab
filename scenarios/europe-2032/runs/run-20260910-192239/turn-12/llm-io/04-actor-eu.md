...
- characters 454-2026: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2069-4054: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5928-6390: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8599-9685: `{{previous_actions}}` from the actor's own response from the previous turn
...
## New measure
None this turn — waiting for political capital to rebuild above survival level before adding new spend.

## Store changes
No changes.

## Priority
M7 — the EU Middle-Power Leverage Pact is the only low-cash route to restore leverage and must absorb the safety breakthrough before open near-frontier diffusion forecloses it.

## In practice
We hold the ten regions on EU-hosted triage under DG SANTE weekly waits and ENISA/DIGIT logs, leaving the US tiered licence unsigned in COREPER while technical talks continue. No new gigafactory spend is tabled; InvestAI permitting and Anti-Coercion preparation remain our chips.

We push the Pact through HR/VP and Trade via the spring framework: AI Office shared evaluation pool adopts the prediction-before-observation control result as a checkable audit standard for pooled procurement, and joint statements on lithography/power/cloud licences are turned into aligned withholding positions. Clinics on the EU stack get priority inference to extend the prescription/paperwork gains that thinned protests.
