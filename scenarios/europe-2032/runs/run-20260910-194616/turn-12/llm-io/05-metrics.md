...
- characters 106-13346: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13389-13581: `{{metrics_json}}` from the run's live metric values
- characters 13674-14835: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14878-16635: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16691-17536: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M12 | EU Islanded Continuity and Cyber Containment Surge | 2 |
...
PORTFOLIO CHARGE t11: EU Welfare AI Redress and Recertification Freeze −2, priority −1 = −3
PROPOSAL BONUS: EU Welfare AI Redress and Recertification Freeze (cat 3, small) +3 — automated_decision_scandal fired this turn with ombudsman finding and this answers it directly
LEGITIMACY LENDS: capital 18, sentiment 18 -> none, capital not below sentiment
SOVEREIGNTY: 16 last turn, no capacity event, no category 4 build in flight +0, capability rose 1.0 −0 = 16
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M12", "fields": {"finish_turn": 13}, "grounds": "named priority"}]}
```
```
