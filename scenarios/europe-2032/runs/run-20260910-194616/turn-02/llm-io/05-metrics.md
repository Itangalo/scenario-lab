...
- characters 13369-14659: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14702-17158: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17214-17990: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated physical protests and grid-connection lawsuits block or delay at least one major EU data-centre / supercomputer expansion for months, making energy and siting a binding constraint on EU compute plans.
