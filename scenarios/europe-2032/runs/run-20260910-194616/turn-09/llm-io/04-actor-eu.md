...
- characters 455-1898: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1942-4061: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4819-5268: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8063-10449: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Supply-Chain Leverage and Continuity Pact | 8 | small | 8 | 10 | coalition + own jurisdiction | eu_ai_sovereignty stabilise, eu_political_capital stabilise, resilience up slightly | 2 | running |
...
## New measure
**EU Supply-Chain Leverage and Continuity Pact**
It converts the Union's lithography and materials position into joint continuity terms with the US, Japan and the Netherlands during the Taiwan blockade, trading controlled upstream flow for secured compute and inputs for EU essential services. Why this and why now: with advanced semiconductor exports halted for years every AI question is a security question, and the one asset that still gives a capital-starved Union bargaining power is the upstream bottleneck — using it jointly beats hoarding it alone, and it serves the two-year commitment to run essential AI without foreign permission.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Supply-Chain Leverage and Continuity Pact", "category": 8, "size": "small", "finish_turn": 10, "applies_to": "coalition + own jurisdiction", "targeted_effect": "eu_ai_sovereignty stabilise, eu_political_capital stabilise, resilience up slightly"}, "grounds": "blockade makes upstream leverage the only spendable asset"}]}
```

## Priority
M8 EU Supply-Chain Leverage and Continuity Pact, because under blockade no domestic workload, repair pool or bio-preparedness holds without jointly managed chip and compute continuity, and it outranks new builds we cannot fund at capital 16.

## In practice
We put the Trade and Competitiveness Council formations with DG TRADE and DG GROW behind the Pact: Anti-Coercion Instrument readiness and export-control alignment as the legal base, ASML/Netherlands, Japan and the US as the coalition table, offering predictable, conditioned upstream supply in return for prioritised compute, spares and servicing for EU health, energy and telecom operators. ENISA and HERA keep degraded-mode repair pools and manual-check operations running from finished measures without new money.

On bio uplift we do not open a second instrument this turn: we task HERA/ECDC with targeted detection guidance and lab liaison within existing resilience funding, holding a full biopreparedness build for next turn if the precursor strengthens. On knowledge-work gains with no job loss we bank the sentiment stabiliser but claim no dividend, conceding the welfare-system gap remains lawful under the 2024 AI Act and asking Parliament for time for a gap-mapping recast rather than moratoria that would freeze adoption.
...
It puts the new interpretability/control result onto EU-hosted essential systems fast via EU AI Office evaluation protocols and ENISA hardening, certifying behaviour before harm rather than auditing after.
Why this and why now: with capability at 64.5 and open weights at 62.5 against safety at 32, a lab-wanted breakthrough that actually lands on deployed systems is the rare cheap lever for a capital-starved Union, and under Taiwan blockade and tightening export controls it lets degraded-mode services run safer without new chips.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Deployed Assurance Uptake", "category": 5, "size": "small", "finish_turn": 11, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up, resilience up slightly, public_sentiment up slightly"}, "grounds": "adopt lab-wanted interpretability breakthrough on deployed EU systems"}]}
```

## Priority
M8 EU Supply-Chain Leverage and Continuity Pact, because under blockade winter and further export tightening no assurance uptake holds without jointly managed compute, spares and servicing for hospitals, grid and telecoms, and it still answers the closing commitment.

## In practice
We keep the Trade/Competitiveness Council and DG TRADE/GROW behind M8: Anti-Coercion Instrument readiness as legal base, joint continuity table with US, Japan, Netherlands offering predictable lithography servicing, optics and materials for written prioritised compute and parts. ENISA and HERA sustain manual-check degraded operations and repair pools from finished measures.

We task the AI Office with the new uptake alongside ENISA: pre-deployment certification checklists for EU-hosted health/ministry workloads, vetted researcher access to validate the result, no new bio programme beyond detection guidance and lab liaison. We tell Parliament the welfare-flagging gap-mapping recast proceeds in parallel so safety gains are seen as fairness gains too.
