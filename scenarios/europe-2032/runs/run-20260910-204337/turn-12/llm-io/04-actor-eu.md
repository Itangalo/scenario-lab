...
- characters 455-1744: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1787-4019: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4915-5348: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8534-10417: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Certified trustworthy AI running on EU-controlled capacity
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Containment and Continuity Surge | 6 | small | 11 | 14 | own jurisdiction | resilience up, ai_safety up slightly via containment learning | 2 | running |
...
| M4 | EU Clinical Sovereignty Pilot | 9 | small | 2 | 5 | own jurisdiction | eu_ai_sovereignty up slightly, public_sentiment up, eu_political_capital up slightly on success | 2 | finished |
| M5 | EU Continuity Capacity Acquisition | 4 | small | 3 | 6 | own jurisdiction | eu_ai_sovereignty up, resilience up slightly via continuity | 2 | finished |
| M6 | EU Probe Containment Surge | 6 | small | 4 | 6 | own jurisdiction | resilience up, ai_safety up slightly via incident learning | 2 | finished |
| M7 | EU Trusted Public AI Dividend | 9 | small | 6 | 8 | own jurisdiction | public_sentiment up, eu_political_capital up on success, eu_ai_sovereignty sustained via use of EU-controlled capacity | 2 | finished |
| M8 | EU Municipal Recovery Corps | 6 | small | 7 | 9 | own jurisdiction | resilience up, eu_political_capital up slightly on visible restore | 2 | finished |
| M9 | EU Assurance Adoption Pact | 5 | small | 8 | 10 | own jurisdiction | ai_safety up materially, resilience up slightly via trusted restore | 2 | finished |
| M10 | EU Certified Work Dividend | 9 | small | 9 | 11 | own jurisdiction | public_sentiment up, eu_political_capital up on success, resilience up slightly via sustained use | 2 | finished |
...
## New measure
**EU Containment and Continuity Surge**
Runs rehearsed loss-of-control isolation protocols with operators and stands up HERA-led bio-detection surge capacity in sentinel hospitals, because containment uncertainty and the genome-model claim must be answered now while capital is thin.
This is category 6 Preparedness and resilience, small and implementable within existing mandates.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Containment and Continuity Surge", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via containment learning"}, "grounds": "loss-of-control incident plus contested bio-design claim requires exercised response"}]}
```

## Priority
M11 EU Containment and Continuity Surge, because at capital 16 only a visible containment and continuity answer restores permission to do anything else, outranking the sovereignty build.

## In practice
We task DG CNECT, ENISA and the AI Office to run loss-of-control playbooks with telecom operators, cloud providers and the two hospital networks: kill-switches, payment-record freezes, and cross-border mutual aid exercised, not documented, funded by reprogrammed Digital Europe resilience money. No new liability directive this turn; we use NIS2 implementing acts to avoid a Council vote we cannot win.

On bio and supply, HERA and ECDC move passive reporting to active sentinel sampling in pilot cities, logging the open-weight release that puts ~63-capability tools permanently beyond recall. EEAS contains the member-state side-deal by offering volume-licence pooling under the Anti-Coercion Instrument consultation, and Finland/Spain shells are held on permits/power only until export-control rationing clarifies — we do not promise concrete we cannot finance.
...
```
## New measure
**EU Certified Assurance Upgrade**
Deploys the new interpretability/control result across certified public AI and operator containment stacks via the AI Office and ENISA, turning a lab breakthrough both sides want into auditable trust.
This is category 5 Public technical capacity and research, small and implementable by implementing act and agency guidance, which is all capital 11 allows while answering the loss-of-control post-mortem.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Certified Assurance Upgrade", "category": 5, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up materially, public_sentiment up slightly via certified trust"}, "grounds": "deployable interpretability breakthrough that labs will adopt"}]}
```

## Priority
M11 EU Containment and Continuity Surge, because with telecom freezes and active sentinel sampling just starting, exercised isolation and bio-detection outranks even the assurance upgrade until containment is credible.

## In practice
We keep M11 as the visible answer: DG CNECT and ENISA run the loss-of-control playbooks with the two affected telecom operators and cloud providers, and HERA/ECDC push sentinel hospitals from passive to active sampling, funded by reprogrammed Digital Europe and EU4Health money without a new Council vote. This is NIS2 implementing acts, not a directive we cannot pass.

We add only the cheap win: the AI Office with JRC writes the safety breakthrough into the certified work and trusted-assistant safety cases as a predictable-behaviour proof requirement, pooled across deployments so ministers can point to certification, not claims. On ASML, we do not open a second front we cannot fund; EEAS files Anti-Coercion consultation and pools the Dutch servicing issue with the chip-rationing quotas, offering the breakaway member state pooled licences if it returns to the common line, while Finland/Spain shells stay on hold for import clarity.
