...
- characters 107-13041: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13084-13274: `{{metrics_json}}` from the run's live metric values
- characters 13367-15993: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 16037-18030: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18086-18719: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M14 | EU Joint Bio-Cyber Threat Response Accession | 2 |
...
- `emergent_repair_convoy_paralysis` -- first noted turn 12, listed in 2 turn(s) so far: Wartime targeting expands to subsea repair fleet and spares convoys, with insurers withdrawing cover for cable and grid restoration ships in the North Sea and Atlantic.
