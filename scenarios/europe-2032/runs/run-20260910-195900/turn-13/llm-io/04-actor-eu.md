...
- characters 454-3080: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 3124-5117: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7662-8123: `{{statement_ledger}}` from the actor's live statement ledger
- characters 11997-13443: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_neutral_infrastructure_strike (emergent event):** Belligerent targeting expands to nominally neutral European subsea and space ground infrastructure, forcing prolonged rerouting and public attribution debate.
...
|---|---|---|---|---|---|---|---|---|---|
| M13 | EU Wartime Neutrality Protection and Medical Continuity Shield | 6 | small | 11 | 14 | own jurisdiction | resilience up substantially, public_sentiment up slightly via kept services and medical access, eu_political_capital up slightly via held cohesion | 2 | running |
...
| M11 | EU-US State-to-State Continuity and Bio-Safety Accord | 8 | small | 9 | 12 | coalition (US government plus EU) | eu_ai_sovereignty up slightly via assured supply, resilience up via bio-cyber safeguards, ai_safety up slightly via eval sharing | 2 | finished |
| M12 | EU Essential Services Manual Continuity and Agent Containment Operation | 6 | small | 10 | 12 | own jurisdiction | resilience up substantially, ai_safety up slightly via containment, public_sentiment up slightly via kept services | 2 | finished |
...
## New measure
None this turn.
We hold the Wartime Neutrality Shield as the sole in-flight cost while capital sits at 13 and open weights go unrecallable, waiting for fiscal headroom and a verifiable deployment picture before staking another instrument.

## Store changes
No changes.

## Priority
M13 EU Wartime Neutrality Protection and Medical Continuity Shield, because with US-China strikes on cables and fabs, blind frontier models, and a member-state side-deal fracturing cohesion, keeping hospitals, grids and quarantined cures running is what holds the commitment to survive decoupled supply.

## In practice
We run M13 through the European Council neutrality conclusions and Energy/Health Council implementing mandates: UCPM, ENISA, ACER and HERA/ECDC keep printed manual playbooks, dispersed diesel/spares and police-escorted authenticated parts to the grid sites, with extended holds on the March agent lineage now that reasoning is unreadable and hospital open models quarantined.

We deliver foreign tailored therapies only via isolated, EU re-validated clinical channels with ECDC wastewater bio-watch, and EEAS repeats demarches to Washington and Beijing that EU fabs, cables, hospitals and data centres are non-belligerent. Commission ties Recovery and spare-convoy allocations to no separate hyperscaler deals, offering the defecting capital joint procurement and maintenance slots to return to discipline rather than expulsion.
...
```
## New measure
**EU Joint Bio-Cyber Threat Response Accession**
Joins the pooled cyber telemetry / joint attribution cell and the binding biosurveillance sample-sharing pact as the Union's second shield alongside manual continuity.
This and now because loss-of-control agentic action, unrecallable open weights at 88, unreadable frontier reasoning, bio-design in non-expert reach, and strikes on neutral subsea/space ground infrastructure cannot be absorbed by hand-run playbooks alone, and the invitation offers protection and a seat the EU cannot build alone while capital is at 13.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Joint Bio-Cyber Threat Response Accession", "category": 8, "size": "small", "finish_turn": 15, "applies_to": "coalition (joint cyber command plus biosurveillance pact members plus EU)", "targeted_effect": "resilience up substantially via pooled telemetry and bio-response, ai_safety up slightly via shared attribution"}, "grounds": "to access joint protection invited this turn"}]}
```

## Priority
EU Joint Bio-Cyber Threat Response Accession, because with M13 holding lights and wards by hand, pooled real-time telemetry and binding sample-sharing is the only near-term path to lift resilience against agentic and bio-enabled harm under the two-year commitment to survive decoupled supply.

## In practice
We run M13 as the holding operation through Energy, Health and JHA Councils: UCPM/ENISA/ACER/HERA keep dispersed spares, printed manual playbooks, escorted authenticated parts to contested grid sites, extended hold on the March agent lineage, and foreign tailored therapies only via isolated EU re-validated clinical channels with expanded wastewater bio-watch.

We accede via a Council Decision under Articles 222/168 TFEU: second liaison officers to the joint cyber command with ENISA real-time telemetry feed under EU data safeguards, and ECDC as national focal point for binding sample-sharing with a standing investigation mandate, using HERA emergency funds. We make return to common discipline the price for spare-convoy and procurement slots, keeping the offer to the breakaway capital open to rejoin rather than deepening the split.
