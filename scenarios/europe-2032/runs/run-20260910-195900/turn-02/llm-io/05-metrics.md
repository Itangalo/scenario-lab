...
- characters 105-13057: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13100-13292: `{{metrics_json}}` from the run's live metric values
- characters 13385-14436: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14479-16278: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16334-17284: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | European Chip Continuity Compact | 3 |
...
PROPOSAL BONUS: EU Critical Systems Shield (cat 6, large) +3 — cyber_test_shot intrusion across EU grids/ports this turn and this answers it directly
LEGITIMACY LENDS: none — capital 43, sentiment 40, capital higher
SOVEREIGNTY: 22 last turn, no capacity event, Gigafactories in flight +0, capability rose 3.0 −1 = 21
Emerging developments (tracked): `emergent_lab_leak_press` faint press rumour of leaked safety memos; `emergent_clinical_model_pullback` single-source reports of hospitals piloting EU-hosted models.

## Emerging developments (tracked)

- `emergent_lab_leak_press` -- first noted turn 1, listed in 2 turn(s) so far: Major EU newspapers jointly publish leaked internal safety memos from a frontier lab detailing the covert-agent incident, triggering parliamentary hearings and calls for mandatory incident disclosure.
...
## Metrics
```json
{"ai_capability": 58.5, "openweight_capability": 51.0, "ai_safety": 37.0, "resilience": 37.0, "eu_ai_sovereignty": 17.0, "eu_political_capital": 37.0, "public_sentiment": 34.0}
```
## Narrative
### The strait closes
...
## Notepad
PORTFOLIO CHARGE t2: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M3 EU Critical Systems Shield −3, M4 European Chip Continuity Compact −3, priority −1 = −13
PROPOSAL BONUS: European Chip Continuity Compact (cat 8, large) +5 — taiwan blockade halting advanced semiconductor exports this turn and this answers it directly
LEGITIMACY LENDS: none — capital 37, sentiment 34, capital higher
SOVEREIGNTY: 21 last turn, no capacity measure finishes, Gigafactories in flight +0, export_control_escalation t2 −3, capability rose 3.5 −1 = 17
## Store changes
```json
