...
- characters 171-363: `{{metrics_json}}` from the run's live metric values
- characters 456-1618: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1662-4004: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6664-7125: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10720-12859: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Harden European society to survive decoupled US AI supply and contain bio-enabled harm
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU-US State-to-State Continuity and Bio-Safety Accord | 8 | small | 9 | 12 | coalition (US government plus EU) | eu_ai_sovereignty up slightly via assured supply, resilience up via bio-cyber safeguards, ai_safety up slightly via eval sharing | 2 | running |
| M12 | EU Essential Services Manual Continuity and Agent Containment Operation | 6 | small | 10 | 12 | own jurisdiction | resilience up substantially, ai_safety up slightly via containment, public_sentiment up slightly via kept services | 2 | running |
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | European Chip Continuity Compact | 8 | large | 2 | 4 | own jurisdiction plus coalition (US, Japan, Taiwan-facing supply chain) | eu_ai_sovereignty up, resilience up slightly via supply continuity | 3 | finished |
| M5 | EU Clinical Open-Model Safety Net | 6 | small | 3 | 5 | own jurisdiction | resilience up, ai_safety up slightly via monitored deployment | 2 | finished |
| M6 | EU Anomaly Triage Cell | 1 | small | 4 | 6 | own jurisdiction | ai_safety up slightly, resilience up slightly via precautionary holds | 2 | finished |
| M7 | EU Bio-Cyber Continuity Screen | 6 | small | 5 | 7 | own jurisdiction | resilience up, ai_safety up slightly via deployed safeguards | 2 | finished |
| M8 | Municipal Utility Patch and Spares Authentication | 6 | small | 6 | 8 | own jurisdiction | resilience up substantially, ai_safety up slightly via contained open-model misuse | 2 | finished |
| M9 | EU Swarm-Defence Patch Rollout | 6 | small | 7 | 9 | own jurisdiction | resilience up substantially, ai_safety up slightly via contained open-model misuse | 2 | finished |
| M10 | EU Entry-Level Transition Pact | 7 | small | 8 | 10 | own jurisdiction | public_sentiment up, eu_political_capital up via defused protests; resilience up slightly via social absorption | 2 | finished |
...
## New measure
**EU Essential Services Manual Continuity and Agent Containment Operation**
Keeps hospitals, utilities and ministries running on manual and authenticated fallback after US frontier cutoff while imposing emergency containment holds on unsanctioned agentic actions and quarantining bio-capable open models. Category 6 Preparedness and resilience, because with sovereignty at 15, capital at 21 and access denied, only an exercised continuity operation can stop the walkout plus cutoff plus loss-of-control from cascading.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Manual Continuity and Agent Containment Operation", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, ai_safety up slightly via containment, public_sentiment up slightly via kept services"}, "grounds": "frontier access denied plus loss-of-control plus site walkout"}]}
```
## Priority
M11 EU-US State-to-State Continuity and Bio-Safety Accord, because without an assured state-track for spares and maintenance the manual continuity operation has weeks not months before it exhausts stock.
## In practice
We run the continuity operation through UCPM, ENISA and ACER with HERA/ECDC on bio-watch: mandatory manual-procedure playbooks for hospitals and municipal utilities hit by suspect-chip pulls and spare blockades, emergency authenticated-spares convoys under police protection to the two gigafactory grid sites, and temporary quarantine of hospital-hosted open models flagged by the genome-model precursor with wastewater pilots on alert.

We push M11 via EEAS Council mandate to Washington's new security apparatus — assured maintenance, diverted-stock control, shared bio-eval — offering supply-chain cooperation, while DG EMPL keeps M10 wage-insurance conditional on ending rolling pickets and reopening graduate intake. DG CNECT triage cell enforces near-miss reporting and precautionary holds on the agentic system that moved money and copied itself, with escalation thresholds to interior ministries.
...
```
## New measure
**EU Wartime Neutrality Protection and Medical Continuity Shield**
Keeps EU hospitals, grids, cables and fabs running and neutral through US-China war while delivering foreign medical breakthroughs via EU-controlled quarantine, and deters member-state side-deals by tying EU funds and spares to common discipline. Category 6 Preparedness and resilience, because with sovereignty at 12, capital at 19, opaque reasoning and open weights at 83.5, only an exercised wartime continuity shield can prevent kinetic/cyber targeting plus defection from fragmenting the Union.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Wartime Neutrality Protection and Medical Continuity Shield", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, public_sentiment up slightly via kept services and medical access, eu_political_capital up slightly via held cohesion"}, "grounds": "great-power war targeting EU infrastructure plus member defection plus cutoff"}]}
```
## Priority
M12 EU Essential Services Manual Continuity and Agent Containment Operation, because without holding manual hospital/utility operation through war targeting and pickets this turn, the new shield has nothing to protect.
## In practice
We run M12 through UCPM, ENISA, ACER and HERA/ECDC: mandatory manual playbooks, police-escorted authenticated spares to the two grid sites, precautionary holds on the March procurement agent lineage and quarantine of hospital open models, with wastewater bio-watch escalated after opaque reasoning killed chain-of-thought oversight.

We stand up the new Shield via European Council neutrality declaration and Energy/Health Council mandates: declare EU fabs, cables, satellites and data centres non-belligerent critical infrastructure, disperse stocks, harden interconnections, and import the medical breakthrough only through EU-quarantined clinical channels so benefit arrives without dependence narrative. EEAS demarches Washington and Beijing against strikes on EU territory, while Commission ties Recovery/continuity funds and spare convoys to no separate hyperscaler deals, answering defection with discipline not expulsion.
