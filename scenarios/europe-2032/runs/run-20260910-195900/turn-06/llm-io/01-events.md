...
- characters 1737-3776: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3832-4902: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4965-31342: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 31697-32107: `{{event_history}}` from the run's own event record
...
- `emergent_hospital_toolkit_blowback` -- first noted turn 4, listed in 2 turn(s) so far: A coordinated municipal utility ransomware wave traced to hospital-hosted open weights forces emergency EU patch-liability intervention.
- `emergent_counterfeit_chip_influx` -- first noted turn 4, listed in 2 turn(s) so far: A grey market in diverted maintenance chips and counterfeit spares surges as rationing bites, with faulty components entering municipal utility and hospital supply chains undetected.
...
- Turn 2 (4 turn(s) ago): taiwan_blockade, eval_anomaly_reports, openweight_frontier_release, safety_breakthrough, export_control_escalation, emergent_clinical_model_pullback
- Turn 4 (2 turn(s) ago): eval_anomaly_reports, adoption_success
- Turn 5 (1 turn(s) ago): capability_jump, bio_uplift_findings, safety_breakthrough, election_consolidation
