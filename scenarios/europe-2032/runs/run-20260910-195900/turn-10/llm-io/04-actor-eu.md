...
- characters 455-1850: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1893-4422: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6646-7107: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10448-12949: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_site_protection_breakdown (emergent event):** A coordinated walkout by maintenance contractors at the two intermittent gigafactory grid-connection sites, joined by rolling-picket graduates, blocks authenticated spare deliveries for weeks and forces manual operation of local utilities.
