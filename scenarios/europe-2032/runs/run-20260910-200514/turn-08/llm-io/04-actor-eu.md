...
- characters 454-2311: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2354-4523: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7185-7645: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10148-12273: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_robot_update_halt (emergent event):** A major Chinese robotics vendor pushes a mandatory cloud-update that briefly halts picking systems in Rotterdam and Hamburg, triggering an EU-wide debate over operational sovereignty for port logistics.
...
|---|---|---|---|---|---|---|---|---|---|
| M7 | EU Sovereign Automation Core | 4 | small | 7 | 9 | own jurisdiction | eu_ai_sovereignty up moderately, resilience up slightly via industrial continuity | 2 | running |
...
## New measure
**EU Sovereign Automation Core**
It anchors a European control-model and integration stack for deployed robotics on EU compute and procurement, so logistics and industrial robots running in Europe do not depend entirely on American models and Chinese hardware.
This turn embodied deployment outranks paper gigafactories because robots are arriving now in EU warehouses and factories we still lead in, and a small sovereignty measure we can staff rebuilds leverage for our two-year capacity commitment while capital at 18 cannot carry a new large build.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Sovereign Automation Core", "category": 4, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up moderately, resilience up slightly via industrial continuity"}, "grounds": "commercial robotics deployment from US models and Chinese hardware demands EU alternative"}]}
```

## Priority
EU Sovereign Automation Core, because with M6 restoration running and gigafactories finished on paper, the only build that advances our secure-independent-capacity commitment within capital we have is a fundable robotics stack, not another unfunded compute pledge.

## In practice
We run this through DG GROW and DG CNECT under the existing tech sovereignty permitting zones and Important Project of Common European Interest state-aid base, not a new vote: extend grid reservations and site options from M1/M2 to robotics integration pilots in Rotterdam, Hamburg and Lyon logistics hubs, with EIB and reprogrammed Digital Europe money for EU-hosted fine-tuning of control models.

We keep M6 restoration as the delivery track — ENISA joint teams finish segmentation and abuse-feed rollout to lagging municipalities — and tell Competitiveness and EPSCO Councils openly that productivity gains without layoffs give us fiscal room to demand EU-procurement preference and maintenance-data retention in exchange, while offering unions retraining tied to warehouse automation rather than new restrictions industry would block.
...
```
## New measure
**EU Municipal and Port Systems Emergency Restoration**
It surges joint ENISA cyber teams, offline backups and segmented recovery to municipalities, hospitals and halted Rotterdam/Hamburg port robotics, while mandating local operational fallback for foreign cloud-updates.
This turn a major automated attack plus the Chinese vendor halt outrank paper capacity work because essential services are visibly down with capital at 14 and sentiment at 27, and only a delivered restoration keeps any room to hold our secure-independent-capacity commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal and Port Systems Emergency Restoration", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via incident learning"}, "grounds": "major automated attack and port robotics halt demand immediate restoration"}]}
```

## Priority
EU Municipal and Port Systems Emergency Restoration, because the cyber sweep and update-halt have stopped public services and port logistics now, while the Sovereign Automation Core builds over turns and cannot restore trust alone.

## In practice
We run this through ENISA and DG CNECT under the existing NIS2 incident-response base and Civil Protection Mechanism, not a new law: redeploy the M6 joint teams to worst-hit municipalities and hospitals for segmentation, clean backups and shared abuse-feed blocking, and add a port cell in Rotterdam and Hamburg with DG MOVE to restart picking systems on local manual override and EU-hosted maintenance images.

We keep the EU Sovereign Automation Core running as the commitment track — EIB and reprogrammed Digital Europe pilots for EU fine-tuning in the three hubs with procurement preference — but tell the Transport and Telecoms Councils openly that no new large build is fundable at capital 14, and offer operators and unions rapid retraining and continuity funding in exchange for mandatory local-fallback and data-retention clauses to prevent the next remote halt.
