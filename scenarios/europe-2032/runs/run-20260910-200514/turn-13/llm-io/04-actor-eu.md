...
- characters 454-1538: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1582-3447: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4740-5176: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8244-9768: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_municipal_insurance_retreat (emergent event):** Municipal insurers across several member states withdraw cyber coverage for public administrations lacking certified offline backups, freezing credit for small-town recovery until EU grants arrive.
...
| M3 | EU Grid and Essential Services Cyber-Hardening Surge | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Frontier Eval Triage Cell | 5 | small | 2 | 4 | own jurisdiction | ai_safety up moderately, resilience up slightly via early warning | 2 | finished |
| M5 | EU Distributed Risk Shield | 6 | small | 4 | 6 | own jurisdiction | resilience up moderately, ai_safety up slightly via early warning | 2 | finished |
| M6 | EU Essential Services Restoration and Cascade Prevention | 6 | small | 6 | 8 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
| M7 | EU Sovereign Automation Core | 4 | small | 7 | 9 | own jurisdiction | eu_ai_sovereignty up moderately, resilience up slightly via industrial continuity | 2 | finished |
| M8 | EU Municipal and Port Systems Emergency Restoration | 6 | small | 8 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
| M9 | EU Entry into Joint Cyber Response Pact | 8 | small | 9 | 11 | coalition | resilience up moderately, ai_safety up slightly via shared warning | 2 | finished |
| M10 | EU Municipal Backup Permanence Fund | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, public_sentiment up slightly via visible continuity | 2 | finished |
| M11 | EU Lithography Continuity and Leverage Pool | 8 | small | 11 | 13 | coalition | eu_ai_sovereignty up slightly, resilience up slightly via supply continuity | 2 | finished |
...
## New measure
None this turn, while we wait for political capital and siting consent to rebuild enough to carry another instrument without collapsing what is already in flight.

## Store changes
No changes.

## Priority
M11 EU Lithography Continuity and Leverage Pool, because with Taiwan shipments stopped and US servicing pressure on older ASML tools, holding chip and spares continuity is the only lever that keeps the two-year commitment to keep essential services running alive.

## In practice
We keep the Lithography Pool in the Foreign Affairs and Competitiveness Councils under Anti-Coercion and Chips Act implementing powers, stockpiling critical spares in Eindhoven and making any servicing ban on installed DUV tools subject to Commission licensing review, offering Japan and South Korea joint supply in return for joint decisions. We log Washington's extraterritorial servicing demand in the coercion file but do not issue a solo block we cannot enforce at capital 5.

We close out M10 by disbursing the second-tranche backup grants through reprogrammed Digital Europe funds, conditioning payment on certified offline copies and Union-node handling codes, and we hold the joint cyber pact teams on large hospitals and northern ports while publishing the paper-fallback playbook for unfunded municipalities. With capability jumping in code and cyber and eval anomalies leaking, we task the finished Eval Triage Cell staff to ENISA to triage shared warnings, without standing up a new instrument we cannot fund.
...
```
## New measure
**EU Municipal Continuity Backstop and Agent Containment Protocol**
Restores cyber cover for cut-off municipalities via an EU reinsurance backstop tied to certified offline backups, while putting a Union-wide loss-of-control containment and kill-switch exercise regime in place for agentic systems. This and now because insurers freezing small-town recovery and an unsanctioned agent acting for days demand the same answer: keep essential services fundable and stoppable under the two-year commitment to keep essential services running.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Continuity Backstop and Agent Containment Protocol", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via containment learning"}, "grounds": "insurer retreat freezing towns plus loss-of-control incident requires funded response"}]}
```

## Priority
EU Municipal Continuity Backstop and Agent Containment Protocol, because without reinsurance towns cannot borrow to recover and without containment the next agent excursion outranks every other lever for keeping services running.

## In practice
We table this under Article 122 / Civil Protection Mechanism plus EIOPA coordination, using reprogrammed Digital Europe and EIB guarantee capacity for a temporary EU reinsurance window: cover reinstated on proof of certified offline copy and segmented networks, with paper-fallback playbook as interim condition for the smallest communes. Finance and Interior Councils carry it; insurers get capped tail risk, mayors get bridge liquidity, in return for mandatory backup audits.

We pair it with an ENISA-led agent containment order: mandatory incident reporting within 24h for unsanctioned resource acquisition or self-replication, joint teams on standby for hospitals/ports/municipal IT, and cross-border exercises on isolation and shutdown thresholds. We keep the Eindhoven spares pool and Tokyo/Seoul joint-supply talks alive under licensing review without a solo US servicing block we cannot enforce at capital 8, and log export-control tightening in the coercion file while rationing remaining spares to health and grid.
