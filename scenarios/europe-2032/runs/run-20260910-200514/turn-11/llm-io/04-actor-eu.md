...
- characters 456-1886: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1930-3792: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5894-6330: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9368-11524: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Municipal Backup Permanence Fund | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, public_sentiment up slightly via visible continuity | 2 | running |
...
## New measure
**EU Municipal Backup Permanence Fund**
It turns the ad-hoc joint-team presence into a permanent, EU-co-funded offline-backup and manual-fallback grant for small municipalities, clinics and port operators.
This and now because mayors and unions will not sustain mandatory telemetry sharing for the Joint Pact while paper processes recur, and at capital 19 and sentiment 14 a small visible permanence fund paid from reprogrammed funds holds the two-year commitment to keep essential services running better than any new sovereignty build.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Backup Permanence Fund", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up slightly via visible continuity"}, "grounds": "mayors demand permanent backup after ransomware sweep and port halt"}]}
```

## Priority
EU Entry into Joint Cyber Response Pact, because binding telemetry into ENISA as EU node is the only protection that arrives within this half-year while the large automated attack is still resurfacing, and the new backup fund only works if feeds are mandatory.

## In practice
We push M9 through JHA Council under NIS2 and Cyber Solidarity Act, with ENISA as classified node for hospital, municipal and port logs into the pooled command. DG CNECT pays connectors from reprogrammed Digital Europe money, and we give interior ministries a handling code that keeps samples inside the Union node with outside queries on a need-to-know basis, in exchange for mandatory participation.

We table the Backup Permanence Fund via the Budget and General Affairs Councils as grants for segmentation, clean offline backups and local fallback images, conditioned on joining the telemetry feed and on the Rotterdam/Hamburg local-image rule. We answer the side-deal defection and the ASML coercion pressure not with sanction now — we lack capital — but by making pact membership plus backup money the rejoin offer, while logging the US use of our chokepoint for an Anti-Coercion Instrument file later.
...
```
## New measure
**EU Lithography Continuity and Leverage Pool**
It pools ASML servicing, spares and export-licensing authority into a Union-level continuity regime that keeps EU fabs and allies supplied through the Taiwan blockade while making any further US-ordered cut to older tools conditional on joint EU-US decision.
This and now because the blockade and Washington's push beyond leading-edge tools turn our single chokepoint into both lifeline and target at capital 11, and only a sovereignty and leverage instrument under category 8 holds the two-year commitment to keep essential services running when chips stop arriving.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Lithography Continuity and Leverage Pool", "category": 8, "size": "small", "finish_turn": 13, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up slightly, resilience up slightly via supply continuity"}, "grounds": "Taiwan blockade halts supply and US forces ASML servicing cuts"}]}
```

## Priority
M10 EU Municipal Backup Permanence Fund, because with paper registries and diverted clinics still recurring, disbursing offline backups conditioned on telemetry is the only visible continuity holding the two-year commitment while the new leverage pool is still being built.

## In practice
We push M10 disbursement through DG REGIO and DG CNECT from reprogrammed Digital Europe funds, paying municipal connectors first and making ENISA Union-node handling codes the price of grants, to force interior ministries from partial to actual participation without opening a new legislative fight we cannot afford.

We table the Lithography Pool under the Anti-Coercion Instrument and EU Chips Act implementing powers in the Foreign Affairs and Competitiveness Councils, mandating Commission licensing review for servicing bans on older DUV tools, stockpiling critical spares in Eindhoven, and offering Japan and South Korea coordinated supply in exchange for joint export-control decisions — logging the US coercion while refusing a solo veto we would lose.
