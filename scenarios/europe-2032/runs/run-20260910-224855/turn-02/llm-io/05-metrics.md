...
- characters 105-13141: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13184-13376: `{{metrics_json}}` from the run's live metric values
- characters 13469-14659: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14702-16794: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16850-17603: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_grid_toolkit_disclosure` -- first noted turn 2, listed in 1 turn(s) so far: A coordinated disclosure by European grid vendors reveals the autumn implants shared a common open-model-derived toolkit, triggering emergency procurement of network segmentation retrofits.
...
## Metrics
```json
{"ai_capability": 56.0, "openweight_capability": 50.0, "ai_safety": 30.0, "resilience": 36.0, "eu_ai_sovereignty": 18.0, "eu_political_capital": 35.0, "public_sentiment": 29.0}
```
