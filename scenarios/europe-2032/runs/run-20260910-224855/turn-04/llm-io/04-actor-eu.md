...
- characters 454-1607: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1650-4731: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7252-7724: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9866-11842: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M5 | EU Breakthrough Defence Surge | 6 | small | 3 | 5 | own jurisdiction | resilience up, ai_safety up slightly via reduced blast radius | 2 | running |
...
| M4 | EU Fallback Stack for Essential Services | 9 | small | 2 | 4 | own jurisdiction | eu_ai_sovereignty up slightly, public_sentiment up, resilience up by restoring continuity | 2 | finished |
...
## New measure
**EU Breakthrough Defence Surge**
Deploys the new automated patching and swarm-detection tooling across Shield-covered operators and adds AI-assisted bio-screening uplift monitoring after the genome-model finding.
This is category 6 Preparedness and resilience work, because the defence breakthrough only matters if it is installed before the next automated sweep, and the bio signal demands detection now while debate stays technical.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Breakthrough Defence Surge", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via reduced blast radius"}, "grounds": "major automated attack plus defensive breakthrough plus bio uplift precursor"}]}
```

## Priority
EU Critical Services Shield (M3) – because the large automated attack landing now still outranks the bridge and the build, and the breakthrough only helps if Shield installs it.

## In practice
We run M3 as live operations through ENISA and the Civil Protection Mechanism: enforced incident reporting stays on, jointly procured detection kits are triaged to hit grids, port and clinics, and EU hardening money only flows against exercised islanding and manual-operation plans. Interior and health ministers get surge funding via EU4Health and the Emergency Support Instrument, in exchange for accepting the conditions they resisted.

We task DG CNECT, EuroHPC JU and HERA to do two things at once: push the breakthrough patching/detection stack into Shield operators first, smaller utilities next, and stand up the M4 Fallback Stack on reallocated supercomputer/cloud inference for cut-off hospitals and ministries with emergency procurement certification. DG ENER and DG COMP keep M1/M2 gigafactory permitting moving with guarantee and grid-connection offers to regions, but concrete waits while restoration comes first.
...
```
## New measure
**EU Assured Control Adoption**
Stands up EU evaluation capacity to certify and mandate the new interpretability/control breakthrough across Shield-covered essential services and frontier deployments in the Union, paired with loss-of-control containment drills after the unsanctioned agentic incident.
This is category 5 Public technical capacity and research work, because assurance only lands if public evaluators can test it and operators are required to install it — and this turn's breakthrough plus loss-of-control gives both the tool and the mandate window.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Assured Control Adoption", "category": 5, "size": "small", "finish_turn": 7, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up, resilience up slightly via certified controls and containment"}, "grounds": "safety breakthrough plus loss-of-control incident demanding certified controls"}]}
```

## Priority
EU Breakthrough Defence Surge (M5) – because pilots in worst-hit grids and ports must become rollout before the next automated sweep, and it directly serves the two-year commitment to withstand coercion and absorb harm.

## In practice
We run M5 as live deployment through ENISA and DG ENER: Shield incident maps direct the certified patching/swarm-detection kits to hit grid operators and the container terminal first, EU hardening money releases only against proven islanding/manual drills, and HERA funds synthesis-screening uplift in health labs. DG CNECT tasks EuroHPC JU to keep the M4 Fallback Stack on reallocated inference for cut-off hospitals while repair crews finish.

We launch Assured Control Adoption via the AI Office with JRC and ENISA as technical certifiers: vetted access to the new interpretability result, pre-deployment control claims tested by parties able to fail them, and loss-of-control playbooks with escalation thresholds exercised with interior ministries. Legal base is the AI Act implementing powers plus Civil Protection Mechanism; Council JHA and EPSCO are offered Emergency Support and EU4Health surge funds for compliance.

We keep M1/M2 Gigafactories and sovereignty permitting alive on paper with guarantee and grid-connection offers, but no new capex this turn — restoration and rollout absorb crews and the reallocated InvestAI line, and productivity gains without job losses give us breathing room to sequence build after defence.
