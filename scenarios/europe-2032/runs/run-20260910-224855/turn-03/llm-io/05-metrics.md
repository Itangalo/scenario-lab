...
- characters 106-13185: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13228-13420: `{{metrics_json}}` from the run's live metric values
- characters 13513-14977: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15021-17183: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17239-18089: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_grid_toolkit_disclosure` -- first noted turn 2, listed in 2 turn(s) so far: A coordinated disclosure by European grid vendors reveals the autumn implants shared a common open-model-derived toolkit, triggering emergency procurement of network segmentation retrofits.
