...
- characters 105-13395: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13438-13629: `{{metrics_json}}` from the run's live metric values
- characters 13722-15167: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15210-17373: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17429-18397: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Cutoff Continuity Switch | 2 |
...
PORTFOLIO CHARGE t5: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, priority M1 −1 = −7
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 7, sentiment 38 -> +2
SOVEREIGNTY: 20 last turn, no capacity event, M1/M2 in flight +1, member_state_defection t5 −2, capability rose 5.0 −1 = 18

## Emerging developments (tracked)

- `emergent_us_tier_assignment` -- first noted turn 6, listed in 1 turn(s) so far: The incoming US administration publishes its tiered foreign access rule in spring 2029, formally placing the EU in a conditional second tier with volume caps and re-export conditions rather than a full cutoff.
- `emergent_us_tier_rule_published` -- first noted turn 6, listed in 1 turn(s) so far: The incoming US administration publishes its tiering rule for frontier model and compute access in early 2029, placing the EU as a bloc in a conditional middle tier with volume licences tied to export-control alignment.
...
## Metrics
```json
{"ai_capability": 76.0, "openweight_capability": 64.9, "ai_safety": 10.0, "resilience": 41.0, "eu_ai_sovereignty": 19.0, "eu_political_capital": 6.0, "public_sentiment": 32.0}
```
...
## Notepad
PORTFOLIO CHARGE t6: M1 InvestAI Gigafactories −3, M6 EU Cutoff Continuity Switch −2, priority M1 −1 = −6
PROPOSAL BONUS: EU Cutoff Continuity Switch (cat 6, small) +3 — eu_frontier_access_denied landed this turn and this answers it directly
LEGITIMACY LENDS: capital 6, sentiment 32 -> +2
SOVEREIGNTY: 18 last turn, Tech sovereignty package finishes t6 +4, M1 in flight +0, eu_frontier_access_denied t6 −2, capability rose 7.0 −1 = 19
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election_consolidation winner takes office turn 6"}, {"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 9}, "grounds": "named priority"}]}
```
```
