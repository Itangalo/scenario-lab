...
- characters 452-2183: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2226-4118: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6752-7212: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9700-11403: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M7 | EU Middle-Power Supply-Chain Pact | 8 | small | 7 | 9 | own jurisdiction | eu_political_capital up, eu_ai_sovereignty up slightly via joint bargaining | 2 | running |
...
Category 8 International coordination and leverage, small and fast because the tier throttle is biting now and this is the only leverage that does not require concrete poured first.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Middle-Power Supply-Chain Pact", "category": 8, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "eu_political_capital up, eu_ai_sovereignty up slightly via joint bargaining"}, "grounds": "middle-power coalition window open while US tiering bites"}]}
```

## Priority
M1 InvestAI Gigafactories, because only EU-anchored compute ends tiered coercion and the coalition is leverage to buy time and terms for it, not a substitute.

## In practice
We mandate the Commission via the Foreign Affairs/Trade Council to sign the coalition MOU on export-licence alignment and joint compute bargaining, using the Anti-Coercion Instrument as backstop and ASML/lithography plus EU evaluation access as our contribution. DG TRADE leads, with the maverick member state bound by the common licence line as condition for its capped M1 cost-share.

In parallel we keep M1 alive on permits and EIB money already committed without new cash calls this turn, and execute M6 islanding and subsidised EU-hosted inference through HERA/DIGIT and ENISA to keep hospitals and ministries running. No new sovereignty build is launched until capital recovers; survival and bargaining come first.
...
```
## New measure
None this turn, while political capital is at 5 and two measures already consume it all, waiting for M6 continuity savings and coalition bargaining to restore room to start again.

## Store changes
No changes.

## Priority
M7 EU Middle-Power Supply-Chain Pact, because with capital exhausted only joint leverage can unthrottle US compute and buy time for M1 without new cash calls.

## In practice
We task DG TRADE and the EU delegation to the coordination framework to convert the MOU into a joint licensing queue: aligned export-licence criteria, a single EU demand schedule for hospital/ministry inference keys, and pooled evaluation lab results offered to Washington as assurance for restored volumes. ASML servicing commitments and shared testing access are our contribution; the Anti-Coercion Instrument stays as backstop. The maverick member state's separate US channel is tolerated this turn to hold the common line, not confronted.

In parallel we keep M1 InvestAI Gigafactories alive on permits and already-committed EIB funds, with grid-connection acceleration via TEN-E coordinators but no new budget call, and we run islanded health/admin systems on EU-hosted open models through HERA/DIGIT and ENISA. On embodied AI we ask the Commission to map warehouse humanoid dependence and supply-chain exposure for a later labour-resilience move, without launching a new instrument now.
