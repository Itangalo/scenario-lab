...
- characters 107-12968: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13011-13201: `{{metrics_json}}` from the run's live metric values
- characters 13294-14489: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14533-16654: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16710-17477: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_gigafactory_site_sabotage` -- first noted turn 12, listed in 2 turn(s) so far: Coordinated physical sabotage and blockades hit fenced gigafactory sites and grid connections across two member states, fusing anti-AI, anti-rationing and anti-Washington protest into sustained site occupations.
...
## Notepad
PORTFOLIO CHARGE t13: M9 EU–US State-Level AI Access and Restraint Channel −2, priority −1 not charged (capital below 20, no effect) = −2

PROPOSAL BONUS: none — no new measure proposed this turn

LEGITIMACY LENDS: capital 0, sentiment 10 -> +0 (after charge, bonus and event terms, never past sentiment, no lift applied separately)

SOVEREIGNTY: 12 last turn, no cat 4 in flight +0, emergent_infrastructure_sabotage_wave t13 −2, capability rose 1.5 −0 = 10
## Store changes
```json
{"store": []}
```
