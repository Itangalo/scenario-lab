...
- characters 105-13394: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13437-13629: `{{metrics_json}}` from the run's live metric values
- characters 13722-14978: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15021-17001: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17057-18055: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Frontier Evaluation Office | 3 |
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated municipal protests and permit challenges stall two planned hyperscale data-centre sites in the EU over power and water use, forcing a national-level siting debate.
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated protests and local permit challenges stall several planned EU data-centre expansions in autumn 2026 after energy-cost and jobs coverage turns hostile.
...
## Metrics
```json
{"ai_capability": 58.0, "openweight_capability": 50.5, "ai_safety": 30.0, "resilience": 42.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 41.0, "public_sentiment": 41.0}
```
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Frontier Evaluation Office −3, priority −1 = −10
PROPOSAL BONUS: EU Frontier Evaluation Office (cat 5, large) +3 — autumn grid pre-positioning with open-model intrusion tooling last turn and this answers it directly
LEGITIMACY LENDS: none — capital 41, sentiment 41, capital not lower
