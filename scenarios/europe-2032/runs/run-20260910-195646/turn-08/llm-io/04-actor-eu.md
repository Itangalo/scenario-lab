...
- characters 452-1627: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1670-3855: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6699-7167: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9949-12235: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Graduate Transition Guarantee | 7 | small | 7 | 9 | own jurisdiction | resilience up moderately via social absorption, public_sentiment up, eu_political_capital protected | 2 | running |
...
## New measure
**EU Graduate Transition Guarantee**
It funds wage-subsidised first hires, rapid retraining vouchers and income bridging for entry-level roles in law, accountancy, software, customer operations and administration displaced by AI, paid by automating-employer levy plus ESF+.
This and why now because measurable non-replacement of graduate hiring plus sentiment at 32 threatens both political capital at 23 and cohesion after a second side-deal, so under the sovereign-governable commitment we must show absorption before Taiwan/shipping stress and the capability jump turn displacement into hostility to all infrastructure.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Graduate Transition Guarantee", "category": 7, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately via social absorption, public_sentiment up, eu_political_capital protected"}, "grounds": "labour displacement in graduate entry roles with capital at 23"}]}
```

## Priority
M8 EU Assurance Restoration Taskforce, because the discontinuous capability jump on ai_safety at 4 with leaked eval-behaviour anomalies makes probability control the binding constraint that outranks even the labour shock this turn.

## In practice
We drive M8 inside the Evaluation Office with EPSO emergency hires, making mandatory incident reporting under DSA/DMA and AI Act Article 73 the legal base, and trade procurement continuity for log and red-team access at EU-hosted sites, while using the leaked year-long benchmark saturation to justify delay authority to the College and Parliament.

We keep M7 Bio-Cyber Surge moving through Health/JHA Councils via HERA/ECDC and ENISA, rolling screening pilots to all synthesis providers and hospital detection stockpiles to border regions first, and we hold Saclay/Jülich fallback nodes certified under the hold while sequencing grid behind hospitals.

We launch the Transition Guarantee through EPSCO and the Social Fund Committee, asking the defecting capital to re-commit to common chip-queue discipline in exchange for first-wave transition funds and gigafactory apprenticeship slots, containing the side-deal without a court fight we cannot afford at 23 capital.
...
```
## New measure
**EU Municipal Services Cyber Restoration Cell**
It deploys ENISA-led rapid restoration teams, clean backup images and emergency procurement waivers to municipalities and hospitals hit by the automated ransomware sweep, using the Critical Systems Shield fallback stack.
This and why now because the major cyber incident landing on resilience at 39 and capital at 8 demands visible containment before the bio uplift and eval anomaly erode trust further, and under the sovereign-governable commitment we must show absorption with a cheap, fast instrument we can afford.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Services Cyber Restoration Cell", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, eu_political_capital protected"}, "grounds": "major automated cyber incident with resilience at 39 and capital at 8"}]}
```

## Priority
M9 EU Graduate Transition Guarantee, because with sentiment at 26 and graduate unemployment spiking it is the only in-flight measure that can rebuild capital enough to carry the cyber response.

## In practice
We run the Restoration Cell through ENISA and DG CNECT under the NIS2 Article 11 crisis mechanism, seconding national CERT staff and pre-cleared vendors, paying from the Cybersecurity Emergency Reserve to avoid a new budget vote we cannot win at 8 capital. We prioritize health, water and municipal registries, re-imaging from the Fallback Stack and publishing daily restoration counts to show defenders catching up.

We push M9 through EPSCO with ESF+ advances and the automating-employer levy framework, directing first wage-subsidy payouts and retraining vouchers to the three capitals with graduate spikes, and we offer the defecting capitals apprenticeship slots tied to queue discipline without reopening court fights. We hold the finished Assurance Taskforce logs and Bio-Cyber screening pilots as operating protocols, not new builds, while evaluation staff document the agent anomaly for the Frontier Office.
