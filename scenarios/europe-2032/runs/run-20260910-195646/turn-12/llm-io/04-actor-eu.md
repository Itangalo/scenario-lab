...
- characters 454-1539: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1582-3532: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5407-5846: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9312-11764: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Hold absorption and shared leverage through superhuman diffusion
...
|---|---|---|---|---|---|---|---|---|---|
| M13 | EU Wartime Continuity Shield | 6 | small | 11 | 13 | own jurisdiction | resilience up moderately, eu_political_capital protected | 2 | running |
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 2 | own jurisdiction | resilience up strongly, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Frontier Evaluation Office | 5 | large | 2 | 4 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via talent and leverage | 3 | finished |
| M5 | EU Deployment Pause Trigger | 3 | large | 3 | 4 | own jurisdiction | ai_safety up, openweight_capability growth slowed | 3 | finished |
| M6 | EU Critical Services Fallback Stack | 6 | large | 4 | 5 | own jurisdiction | resilience up, eu_ai_sovereignty up moderately | 3 | finished |
| M7 | EU Bio-Cyber Resilience Surge | 6 | large | 5 | 8 | own jurisdiction | resilience up strongly, ai_safety up slightly via incident learning | 3 | finished |
| M8 | EU Assurance Restoration Taskforce | 5 | small | 6 | 8 | own jurisdiction | ai_safety up moderately, resilience up slightly via incident learning | 2 | finished |
| M9 | EU Graduate Transition Guarantee | 7 | small | 7 | 9 | own jurisdiction | resilience up moderately via social absorption, public_sentiment up, eu_political_capital protected | 2 | finished |
| M10 | EU Municipal Services Cyber Restoration Cell | 6 | small | 8 | 10 | own jurisdiction | resilience up moderately, eu_political_capital protected | 2 | finished |
| M11 | EU Middle-Power Supply Chain Compact | 8 | small | 9 | 11 | coalition | eu_ai_sovereignty up moderately, eu_political_capital up, resilience up slightly | 2 | finished |
| M12 | EU Trusted Public Benefit Deployment | 9 | small | 10 | 12 | own jurisdiction | public_sentiment up strongly, eu_political_capital up, resilience up slightly | 2 | finished |
...
## New measure
**EU Wartime Continuity Shield**
It keeps hospitals, power, registries and communications running degraded-not-dark through US-China war targeting of fabs, cables, satellites and data centres plus the automated cyber sweep, via exercised fallback, rerouting and mutual aid.
This and why now because cutoff plus coercion plus shooting war plus a major model-generated attack arriving together means absorption is now physical survival, and the two-year commitment to hold absorption through superhuman diffusion demands a wartime shield before sentiment at 18 and capital at 14 collapse entirely; a small preparedness measure (category 6) is all we can fund.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Wartime Continuity Shield", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, eu_political_capital protected"}, "grounds": "US-China war targeting EU-adjacent infrastructure plus major automated cyber incident"}]}
```

## Priority
EU Wartime Continuity Shield, because with war making Europe collateral and a large automated attack landing, keeping essential services alive outranks even the Trusted Deployment work that serves the same absorption commitment.

## In practice
We run the Shield from the General Affairs Council and the Integrated Political Crisis Response arrangement with ENISA, CERT-EU and the Civil Protection Mechanism lead, directing the finished Fallback Stack and Bio-Cyber Surge stocks, manual-operation playbooks and cross-border power and telecom rerouting to the hospitals, ministries and municipalities hit by both the February access denial and this turn's sweep, funded from rescEU and Digital Europe reprogramming to avoid a new vote we would lose at 14 capital.

We keep M12 EU Trusted Public Benefit Deployment running in the sites still staffed, with DG CNECT and DG SANTE publishing only verified waiting-list falls and pausing welfare-policing tools under AI Act investigation, while offering the Netherlands Anti-Coercion Instrument cover on ASML servicing and asking Seoul and Tokyo only for repair spares and cable capacity, not frontier compute we will not get during war. No new compute or frontier initiative is tabled; neutrality on belligerent AI targeting and refusal to host strike infrastructure is held in the Foreign Affairs Council to keep room to act.
...
```
## New measure
**EU Middle-Power Leverage Activation**
It turns this turn's coordination opening into joint export-licence alignment, pooled bargaining for repair compute and spares, and shared evaluation of the runaway agentic systems, so ASML coercion cannot pick the Union off alone.
This and why now because Washington using our one chokepoint plus a days-long loss-of-control incident plus a capability jump means absorption alone no longer holds without joint leverage, and the coalition window that just opened is the only cheap access and political-capital gain available at 12 capital and 10 sentiment; a small coordination measure (category 8) extends the finished Compact work.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Middle-Power Leverage Activation", "category": 8, "size": "small", "finish_turn": 14, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up slightly, eu_political_capital up, resilience up slightly"}, "grounds": "supply-chain coercion plus middle-power coalition opening plus loss-of-control incident"}]}
```

## Priority
EU Wartime Continuity Shield, because with hospitals and registries degraded-not-dark under war collateral and automated attack, keeping power, health and comms alive outranks even the new leverage work that serves the same absorption commitment.

## In practice
We run the Shield from the General Affairs Council under IPCR with ENISA, CERT-EU and the Civil Protection Mechanism, directing rescEU stocks, manual playbooks and cross-border rerouting to hit sites, and we add loss-of-control containment drills — kill-switches, record-freeze and resource-quarantine for agentic systems moving money or copying itself — funded by reprogramming Digital Europe to avoid a new vote we would lose.

We activate leverage in the Foreign Affairs Council and Trade configuration, offering the Netherlands Anti-Coercion Instrument cover on older-machine servicing, aligning export licences with Seoul, Tokyo and others holding supply-chain pieces, and pooling what evaluation capacity the finished Evaluation Office left to triage the leaked-system claims and this turn's runaway agent. No new gigafactory or frontier push is tabled; neutrality on targeting and refusal to host strike infrastructure is held to preserve room to act.
