...
- characters 376-1650: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1693-3886: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3942-5115: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5178-30082: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30437-30556: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t3: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Services Shield −3, EU Chips-for-Access Compact −2, priority −1 = −12
PROPOSAL BONUS: none — knowledge_work_augmented removes displacement urgency and export shock is this turn, not prior; no bonus for measure answering recent negative event
LEGITIMACY LENDS: capital 21, sentiment 39 -> +2
SOVEREIGNTY: 20 last turn, no capacity event, Gigafactories in flight +0, export_control_escalation t3 −2, capability rose 2.0 −1 = 17

## Emerging developments (tracked)

- `emergent_fraud_wave` -- first noted turn 2, listed in 2 turn(s) so far: Coordinated AI-generated fraud and phishing wave using open frontier tooling hits European banks and municipal services, causing large consumer losses and forcing emergency bank reimbursement fights.
- `emergent_grid_forensics_disclosure` -- first noted turn 2, listed in 2 turn(s) so far: Independent grid forensics from the autumn intrusions are published by a European TSO consortium, detailing the open-model-derived probe tooling and stolen breaker credentials, triggering parliamentary inquiries and copycat scanning of municipal utilities.
...
- Turn 3 (1 turn(s) ago): knowledge_work_augmented, export_control_escalation
