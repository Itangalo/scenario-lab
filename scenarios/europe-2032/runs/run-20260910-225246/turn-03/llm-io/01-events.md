...
- characters 1613-3465: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3521-4532: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4595-28822: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29177-29218: `{{event_history}}` from the run's own event record
...
- `emergent_fraud_wave` -- first noted turn 2, listed in 1 turn(s) so far: Coordinated AI-generated fraud and phishing wave using open frontier tooling hits European banks and municipal services, causing large consumer losses and forcing emergency bank reimbursement fights.
- `emergent_grid_forensics_disclosure` -- first noted turn 2, listed in 1 turn(s) so far: Independent grid forensics from the autumn intrusions are published by a European TSO consortium, detailing the open-model-derived probe tooling and stolen breaker credentials, triggering parliamentary inquiries and copycat scanning of municipal utilities.
