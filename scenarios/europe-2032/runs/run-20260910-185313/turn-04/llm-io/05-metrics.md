...
- characters 105-13450: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13493-13685: `{{metrics_json}}` from the run's live metric values
- characters 13778-15721: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15764-17837: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17893-18742: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Critical Services Continuity Reserve | 2 |
| M5 | Emergency Municipal Service Restoration Pact | 2 |
...
PORTFOLIO CHARGE t3: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M3 EU Critical Infrastructure Cyber Shield −3, M4 EU Critical Services Continuity Reserve −2, priority −1 = −12
PROPOSAL BONUS: Continuity Reserve (cat 5, small) +3 — eu_frontier_access_denied landed this turn severe and this answers it directly plus eval anomaly investigation
LEGITIMACY LENDS: capital 14, sentiment 38 -> +2
SOVEREIGNTY: 18 last turn, no capacity event, M1/M2 in flight +0, eu_frontier_access_denied t3 −2, capability rose 2.0 −1 = 15

## Emerging developments (tracked)

- `emergent_open_toolkit_proliferation` -- first noted turn 4, listed in 1 turn(s) so far: A non-state group publishes a ready-to-run intrusion toolkit distilled from a recent open-weight model, with municipal utilities and hospitals in several countries scanned within days.
...
## Metrics
```json
{"ai_capability": 60.0, "openweight_capability": 55.5, "ai_safety": 22.0, "resilience": 42.0, "eu_ai_sovereignty": 16.0, "eu_political_capital": 21.0, "public_sentiment": 28.0}
```
## Narrative
