...
- characters 454-2397: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2440-4513: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6841-7319: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9321-11485: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Critical Services Continuity Reserve | 5 | small | 3 | 6 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up slightly, ai_safety up slightly via independent eval | 2 | running |
...
## New measure
**EU Critical Services Continuity Reserve**
It stands up EU-controlled fallback inference and independent evaluation for the hospitals, ministries and firms cut off from the leading model, while investigating the leaked eval anomaly.
This and now because the cutoff turns dependence into immediate service risk and the anomaly means we cannot trust the next replacement without our own testing capacity; both serve our two-year commitment to keep essential services running on infrastructure we control.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Critical Services Continuity Reserve", "category": 5, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly, resilience up slightly, ai_safety up slightly via independent eval"}, "grounds": "access denied plus eval anomaly this turn"}]}
```

## Priority
M3 EU Critical Infrastructure Cyber Shield, because patching and swarm-detection half-deployed across municipal utilities still outranks even the cutoff until essential grids, ports and water stop cascading.

## In practice
We keep ENISA and the NIS2 Cooperation Group on the M3 rollout sequence, using Digital Europe reprogrammed funds and joint procurement to push the automated patching stack from the hardened transmission operators to lagging municipal water and smaller ports, with mandatory incident reporting and autumn cross-border drills; Energy Council offers the half-out capital co-financing and grid-priority to adopt common tooling rather than its side supplier's rules.

We launch the Continuity Reserve through DG CNECT and ENISA with HERA and Health Emergency Preparedness funds: emergency rerouting of affected hospitals and ministries to EU-hosted models and pooled Gigafactory pilot capacity, plus a small joint evaluation cell with major publishers and universities under the new research integrity pact to reproduce the leaked eval anomaly and set pre-deployment checks. We do not open a new compute programme alongside M1/M2; site prep and permitting continue on existing lines without new money this turn.
...
```
## New measure
**Emergency Municipal Service Restoration Pact**
It surges ENISA-led restoration teams, offline backups and mutual-aid rerouting to the municipal utilities, hospitals and public services hit by the automated attack, to stop cascades and restore essential services.
This and now because the large model-generated attack lands on the lagging municipal layer our cyber shield had not yet hardened, and our two-year commitment to absorb shocks and keep services running demands restoration first; the capability jump means patching alone no longer suffices without exercised containment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Emergency Municipal Service Restoration Pact", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially"}, "grounds": "major automated cyber incident hitting public services"}]}
```

## Priority
M4 EU Critical Services Continuity Reserve, because cut-off hospitals and ministries cannot wait for restoration alone and the fallback EU-hosted capacity is what keeps them running while defenders catch up — what the world demanded that outranked even the new restoration surge.

## In practice
We keep the new Pact under the NIS2 Cooperation Group and ENISA with Civil Protection Mechanism activation: pre-positioned restoration teams to the worst-hit water plants and municipal IT, mandatory isolation and clean-backup playbooks already drilled in autumn, funded by emergency reprogramming from Digital Europe and the Union Civil Protection reserve. Energy and Health Councils co-sign service-priority orders so grid operators and hospitals get clean systems and rerouted capacity first, with the half-out capital offered mutual-aid access if it plugs its side tools into common reporting.

We push M4 as the operational fallback: DG CNECT accelerates pooled pilot compute and EU-hosted models for the American-cut-off wards and ministries, while the joint testing cell triages the leaked anomaly only enough to gate which replacement models are safe to deploy. M1/M2 gigafactory site works continue on existing permits without new money this turn; we cannot fund build-out and restoration at once with capital at 16.
