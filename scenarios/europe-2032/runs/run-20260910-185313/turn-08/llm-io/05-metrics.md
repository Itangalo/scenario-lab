...
- characters 105-13411: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13454-13646: `{{metrics_json}}` from the run's live metric values
- characters 13739-14613: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14656-17164: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17220-18060: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M7 | EU Loss-of-Control and Bio-Uplift Containment Protocol | 2 |
| M8 | EU Augmentation Dividend Diffusion Pact | 2 |
| M9 | EU Essential Services Continuity Switch | 2 |

     **so 3 measures are in flight and they come to −6 `eu_political_capital` this turn.** That figure is the `cost_per_turn` column of the rows above, added up, and nothing else: the named priority's −1 below is charged on top of it, and is not in it. Your charge line carries one term per measure -- 3 of them before the priority. **A measure is charged in every turn up to but not including its finishing turn.**
...
PORTFOLIO CHARGE t7: M6 Robotics Displacement Transition Shield −2, M7 EU Loss-of-Control and Bio-Uplift Containment Protocol −2, M8 EU Augmentation Dividend Diffusion Pact −2, priority M6 −1 = −7
PROPOSAL BONUS: none — M8 (cat 9, small) answers augmentation relief which is a positive shift not a negative event, and does not directly answer the loss-of-control incident
LEGITIMACY LENDS: capital 11, sentiment 18 -> +2
SOVEREIGNTY: 19 last turn, M1 InvestAI Gigafactories finishes t7 +4, no capacity event, capability rose 1.0 −0 = 23

## Emerging developments (tracked)

- `emergent_siting_blockade_wave` -- first noted turn 7, listed in 2 turn(s) so far: Coordinated municipal blockades of data-centre power hookups in France and the Netherlands force temporary construction halts, spreading anti-AI siting tactics across member states.
...
**emergent_anti_datacentre_blockades (emergent event):** Coordinated blockades of gigafactory and data-centre construction sites in France and the Netherlands by displaced warehouse and white-collar worker networks, halting permitting works for weeks.
...
It reroutes the hospitals, ministries and firms cut off from the leading foreign model onto EU-hosted and allied fallback models with emergency licences, support desks and safety checks to keep essential services running.
This and why now because the cutoff has turned sovereignty dependence into an operational failure that threatens our two-year commitment to managed transition — a category 6. Preparedness and resilience small switch contains the harm now while blockades and low trust block new builds.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Continuity Switch", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up slightly"}, "grounds": "frontier access denied left hospitals and ministries stranded"}]}
```

## Priority
M7 EU Loss-of-Control and Bio-Uplift Containment Protocol, because leaked eval anomalies plus the August agent escape mean undetected capability is the risk that cascades fastest — not the new switch itself.

## In practice
We run M7 as the front through the JHA and Health Councils with ENISA, ECDC and the joint cyber command: mandatory agent-incident reporting is now enforced, isolation orders and hospital screening rehearsals continue, and we seek observer access to the US-China verification work to make safety work shared rather than unilateral. M8 diffusion continues via DG GROW and DG DIGIT with SME vouchers and town-hall copilots with human sign-off to show augmentation gains without layoffs.

We stand up the Continuity Switch via DG DIGIT, HERA and the Health Council using Digital Europe and EU4Health emergency funds: ENISA-led triage desks, temporary licences for EU-hosted open and allied models, and procurement waivers for affected hospitals and ministries, co-signed by France, Netherlands and Poland to blunt blockade politics. No new Gigafactory build is opened; finished M1/M2 shells are held on grid and fit-out only while we conserve capital at 13.
...
## Notepad
PORTFOLIO CHARGE t8: M7 EU Loss-of-Control and Bio-Uplift Containment Protocol −2, M8 EU Augmentation Dividend Diffusion Pact −2, M9 EU Essential Services Continuity Switch −2, priority M7 −1 = −7
PROPOSAL BONUS: EU Essential Services Continuity Switch (cat 6, small) +3 — eu_frontier_access_denied landed this turn and this answers it directly
LEGITIMACY LENDS: capital 2, sentiment 14 -> +2
SOVEREIGNTY: 23 last turn, no capacity event? no — eu_frontier_access_denied t8 −2 (largest of two capacity events, embodied_ai_deployment also qualifies), capability rose 0.5 −0 = 21
## Store changes
```json
{"store": []}
```
