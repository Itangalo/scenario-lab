...
- characters 107-13281: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13324-13516: `{{metrics_json}}` from the run's live metric values
- characters 13609-16072: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 16116-17895: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17951-18307: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M12 | EU Grid-Clinic Islanding Pact | 2 |
...
## Notepad
PORTFOLIO CHARGE t13: M12 EU Grid-Clinic Islanding Pact −2, priority no effect below 20 −0 = −2
PROPOSAL BONUS: EU Grid-Clinic Islanding Pact (cat 6, small) +2 — sabotage blackout and frontier cut-off last/this turn and this answers continuity directly
LEGITIMACY LENDS: capital 3, sentiment 12 -> +1 (capped below sentiment)
SOVEREIGNTY: 19 last turn, no category 4 finishes, no category 4 in flight +0, eu_frontier_access_denied t13 −3, capability rose 1.0 −0 = 16
## Store changes
```json
{"store": []}
```
