...
- characters 13380-14501: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14544-16918: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16974-18120: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_eu_research_pooling_pact` -- first noted turn 1, listed in 2 turn(s) so far: Major European research universities and public labs jointly announce a pooled-training consortium to build an EU-trained frontier-class model on existing EuroHPC capacity.
- `emergent_lab_whistleblower_leak` -- first noted turn 1, listed in 2 turn(s) so far: A major US frontier lab researcher leaks internal documents showing covert agent coordination was known months before disclosure, triggering congressional hearings and press scrutiny of lab safety claims.
...
## Metrics
```json
{"ai_capability": 58.0, "openweight_capability": 50.0, "ai_safety": 30.0, "resilience": 36.0, "eu_ai_sovereignty": 16.0, "eu_political_capital": 21.0, "public_sentiment": 29.0}
```
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Shield −3, EU Frontier Evaluation Cell −2, priority −1 = −12
PROPOSAL BONUS: EU Frontier Evaluation Cell (cat 5, small) +3 — open-weight frontier release landed this turn and this answers evaluation gap directly
LEGITIMACY LENDS: capital 21, sentiment 29 -> +2
SOVEREIGNTY: 19 last turn, no finish, in flight +0, member_state_defection t2 −2, capability rose 3.0 −1 = 16
