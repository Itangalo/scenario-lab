...
- characters 1505-3738: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4485-4967: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7022-8945: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_infrastructure_backlash_freeze (emergent event):** Coordinated municipal protests and court injunctions halt several planned data-centre and grid-segmentation sites, freezing Shield expansion for months.
...
| M4 | EU Frontier Evaluation Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up modestly, resilience up slightly via early warning | 2 | running |
...
## New measure
**EU Recovery Surge**
Deploys ENISA-led rapid recovery teams and the new automated patching/detection tooling to the public services hit by this turn's large automated attack, restoring essential systems first.
This is Preparedness and resilience work, chosen now because a visible major incident with defenders behind demands restoration before anything else, and it is small by design to fit political capital at 21.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Recovery Surge", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "major automated attack on public services requires immediate containment"}]}
```
## Priority
M5 EU Recovery Surge, because containing the ongoing major incident and showing defenders catching up outranks even gigafactory permits and the two-year sovereignty build this turn.

## In practice
We task ENISA with triage across the ransomware/dependency blast radius, funded by reallocated CEF-Digital and Civil Protection Mechanism money, with joint teams to the worst-hit municipalities and hospitals and mandatory incident reporting into the Shield network. We push the cyber defence breakthrough — automated patching and swarm detection — via NIS2 implementing guidance, prioritizing French/German grid controls already segmented and the two refusing health ministries as test cases.

We keep M1 Gigafactories and M2 tech package alive on paper — permits, IPCEI clearance, holding lithography leverage — without new fights, and let the Evaluation Cell continue hiring to attribute the model-generated tooling. We offer the defecting member state recovery support tied to common procurement, and we acknowledge labour displacement publicly but defer a transition fund until capital recovers, to avoid starting a fight we cannot fund.
...
```
## New measure
**EU Transition and Siting Compact**
This is Labour and social protection work that pairs a funded entry-level retraining and wage-bridge for displaced graduates with community benefit and local-veto rules for data-centre and Shield segmentation sites.
We choose this now because with sentiment at 20, injunctions freezing Shield expansion, and unions naming abandonment, neither sovereignty nor resilience can move until siting is legitimate and job loss is answered.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Transition and Siting Compact", "category": 7, "size": "small", "finish_turn": 7, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up slightly via unfreezing Shield sites"}, "grounds": "backlash freeze plus graduate job losses blocking all builds"}]}
```

## Priority
EU Transition and Siting Compact, because unfreezing Shield sites and defusing hostility to infrastructure outranks even the Gigafactory build this turn — without local consent nothing else lands.

## In practice
We table an EPSCO-anchored package funded by reallocated ESF+ and Just Transition money: wage insurance for 12 months for under-30s displaced in law, accountancy, support and customer operations, plus paid reskilling places tied to Shield cyber-recovery and grid-segmentation works. Municipalities hosting Gigafactory, data-centre or segmentation sites get grid-fee rebates and a formal early-consultation right to lift the injunctions.

We keep M1 and M2 alive only as permits and IPCEI clearance, join the joint cyber command telemetry pool on Shield terms via ENISA without ceding procurement, and task the Evaluation Cell to publish attribution of the open-model tooling to justify the compact. We tie recovery aid to the defecting member state to common siting standards, not to cancelling its hyperscaler deal outright.
