...
- characters 452-1447: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1490-3669: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5297-5813: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8227-10330: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M7 | EU Hospital Continuity and Loss-of-Control Containment Reserve | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly | 2 | running |
...
## New measure
**EU Hospital Continuity and Loss-of-Control Containment Reserve**
Keeps hospitals and ministries running on EU-controlled fallback models after US frontier cutoff and stands up exercised containment playbooks for the agentic loss-of-control incident.
This and now because access-denied plus a days-long unsanctioned agent action with capital at 4 and sentiment at 9 demands visible continuity over new build, directly serving the two-year commitment to hold recovery and consent through automation shock.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Hospital Continuity and Loss-of-Control Containment Reserve", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly"}, "grounds": "access cutoff and loss-of-control incident require continuity"}]}
```

## Priority
EU Hospital Continuity and Loss-of-Control Containment Reserve, because with frontier access cut and an unsanctioned agent loose for days, keeping clinical systems up and showing containment outranks any siting or industrial push that cannot move.

## In practice
We task DG SANTE and ENISA via the Health Emergency Preparedness and Response Authority and existing Critical Systems Shield procedures: fallback inference on EU-hosted open-weight models for triage, prescribing and admin in affected hospitals, funded by reallocated EU4Health and Digital Europe envelopes, no new legislation. HERA joint procurement sources EU-anchored compute from the warm Gigafactory permits and extended equipment options.

We pair that with a Health-Interior loss-of-control protocol: mandatory incident reporting from the Shield, exercised isolation and rollback drills in two hospital groups and one ministry IT estate per member state, using Evaluation Cell attribution and continued allied telemetry. We do not force entry on siting injunctions, do not retaliate on ASML coercion this turn, and log medical-breakthrough dependence as the case for fallback rather than new restriction.
...
```
## New measure
**EU Shopfloor Continuity and Displacement Bridge**
Keeps factories and logistics running on non-Chinese, non-cutoff automation inputs while putting a first EU-wide income-and-retraining bridge under workers displaced by embodied AI.
This and now because Chinese robots plus US model cutoff hit the industrial base the two-year commitment names, and with capital at 6 and sentiment at 7 we cannot rebuild compute this turn but can prevent plant closures and consent collapse; a small labour-protection measure (category 7) is the cheapest hold-together move.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Shopfloor Continuity and Displacement Bridge", "category": 7, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up slightly, public_sentiment up slightly"}, "grounds": "embodied AI shock requires industrial continuity and displacement bridge"}]}
```

## Priority
EU Shopfloor Continuity and Displacement Bridge, and not the hospital reserve, because plant-level displacement and Chinese robot dependence is now the faster path to fragmentation than ward fallbacks alone.

## In practice
We run this through DG EMPL and DG GROW under existing ESF+ and Globalisation Adjustment Fund rules, no new regulation: short-time work top-ups and rapid retraining vouchers in auto, logistics and machinery regions most exposed to Chinese humanoids, co-funded by member states with Commission advances. Maintenance and spares pooling for critical automation is handled via HERA-style joint procurement, diversifying away from single-source Chinese stacks where possible.

We keep M7 running in parallel: SANTE/HERA keeps EU-hosted open-model fallbacks for the five cutoff states, ENISA keeps isolation drills, funded by reallocated EU4Health/Digital Europe. We do not reopen Gigafactory earthworks or force siting injunctions this turn, and we log ASML rationing and therapy dependence as supply risks to be raised in Trade Council, not retaliated now.
