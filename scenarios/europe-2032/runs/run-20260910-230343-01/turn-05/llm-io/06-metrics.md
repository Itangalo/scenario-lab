...
- characters 13860-15305: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15349-17803: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17859-19003: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Anchor in the Frontier Risk Accord | 2 |
...
- `emergent_agentic_liability_freeze` -- first noted turn 5, listed in 1 turn(s) so far: Municipal insurers and clearing-house counterparties impose an agentic-system exclusion clause, freezing procurement of autonomous finance and registry tools across several member states until certified kill-switch audits exist.
- `emergent_agentic_insurance_pullback` -- first noted turn 5, listed in 1 turn(s) so far: Major EU reinsurers temporarily exclude losses from autonomous agentic software failures from municipal and hospital cyber policies after the Frankfurt clearing-house incident, forcing emergency state backstops.
