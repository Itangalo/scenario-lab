...
- characters 377-1670: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1714-3633: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3689-4814: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4877-27498: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27853-27965: `{{event_history}}` from the run's own event record
...
PROPOSAL BONUS: EU Frontier Evaluation Cell (cat 5, small) +2 — eval anomaly reports landed this turn and this answers it directly
LEGITIMACY LENDS: capital 33, sentiment 37 -> +2
SOVEREIGNTY: 22 last turn, no capacity event, Gigafactories in flight +0, capability rose 1.2 −0, ai_investment_collapse evaporates private compute arrangements −2 = 20

## Emerging developments (tracked)

- `emergent_chinese_open_charm_offensive` -- first noted turn 2, listed in 1 turn(s) so far: A Chinese laboratory releases a near-frontier open-weight model with permissive licensing and EU-hosted mirrors, explicitly courting European developers and research sites frustrated by US access uncertainty.
- `emergent_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: A coordinated municipal backlash stalls two gigafactory shortlist sites over power prices and water use, forcing renegotiation of grid priority and delaying final investment decisions past mid-year.
...
- Turn 2 (1 turn(s) ago): ai_investment_collapse, eval_anomaly_reports
