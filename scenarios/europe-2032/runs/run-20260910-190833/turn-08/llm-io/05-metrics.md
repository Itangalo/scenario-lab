...
- characters 105-13334: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13377-13568: `{{metrics_json}}` from the run's live metric values
- characters 13661-14832: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14875-17108: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17164-18164: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M7 | Allied Choke-for-Compute Bargain | 2 |
...
- `emergent_graduate_occupations` -- first noted turn 7, listed in 2 turn(s) so far: Coordinated graduate-led occupations block several university administration buildings and a data-centre construction site, demanding hiring guarantees and AI displacement levies, forcing a national political response.
- `emergent_anti_datacentre_blockades` -- first noted turn 7, listed in 2 turn(s) so far: Coordinated graduate-led protests across several member states blockade data-centre construction sites and demand hiring guarantees, turning AI infrastructure into a physical contest.
...
## Notepad
PORTFOLIO CHARGE t8: Allied Choke-for-Compute Bargain −2, priority −1 = −3
PROPOSAL BONUS: Allied Choke-for-Compute Bargain (cat 8, small) +2 — Taiwan quarantine blockade from t7 left accelerator lead times in years and this answers it directly
LEGITIMACY LENDS: capital 4, sentiment 14 -> +2 capped by triple external shock losses
SOVEREIGNTY: 21 last turn, no cat4 finish, no cat4 in flight +0, export_control_escalation t8 −2, capability rose 3.0 −1 = 18
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M7", "fields": {"finish_turn": 9}, "grounds": "named priority"}]}
```
```
