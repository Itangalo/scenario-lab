...
- characters 105-13202: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13245-13437: `{{metrics_json}}` from the run's live metric values
- characters 13530-14725: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14768-17036: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17092-18274: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M3 | Grid Shield and Essential Services Hardening | 3 |
| M4 | EU Frontier Evaluation Facility | 2 |
| M5 | Municipal Response and Dependency Triage Surge | 2 |
| M6 | Containment-Grade Evaluation and Bio-Screening Upgrade | 2 |

     **so 6 measures are in flight and they come to −15 `eu_political_capital` this turn.** That figure is the `cost_per_turn` column of the rows above, added up, and nothing else: the named priority's −1 below is charged on top of it, and is not in it. Your charge line carries one term per measure -- 6 of them before the priority. **A measure is charged in every turn up to but not including its finishing turn.**
...
- `emergent_robot_workplace_accident` -- first noted turn 4, listed in 1 turn(s) so far: A warehouse humanoid in an EU logistics pilot injures workers after a control-software update, triggering national safety suspensions and a wider debate over foreign robotics dependence.
- `emergent_datacentre_blockades` -- first noted turn 4, listed in 1 turn(s) so far: A coordinated physical protest movement blocks several EU data-centre and grid-hardening construction sites, merging anti-AI, energy-price and municipal-ransomware anger into site occupations that delay builds for weeks.
...
## Metrics
```json
{"ai_capability": 67.5, "openweight_capability": 60.1, "ai_safety": 11.0, "resilience": 33.5, "eu_ai_sovereignty": 18.0, "eu_political_capital": 15.0, "public_sentiment": 30.0}
```
