...
- characters 454-1649: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1692-3960: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5659-6127: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7957-10026: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M3 | Grid Shield and Essential Services Hardening | 6 | large | 1 | 6 | own jurisdiction | resilience up substantially, ai_safety spillover via incident learning | 3 | running |
| M4 | EU Frontier Evaluation Facility | 5 | small | 2 | 5 | own jurisdiction | ai_safety up, eu_ai_sovereignty spillover via evaluation leverage | 2 | running |
| M5 | Municipal Response and Dependency Triage Surge | 6 | small | 3 | 6 | own jurisdiction | resilience up, to stop cascade from municipal and supply-chain edge | 2 | running |

They cost you 13 political capital this turn, which is the sum of the `cost_per_turn` column above. That column, like `id`, `started_turn` and `status`, is worked out for you and is not yours to write.
...
## New measure
**Municipal Response and Dependency Triage Surge**
Extends the Grid Shield into municipalities, mid-sized suppliers and the compromised software dependency, with emergency triage teams and funded patching where the major automated attack landed hardest.
This is Category 6 Preparedness and resilience, because the incident shows defence held at transmission level but failed at the edge, and that gap must be closed now while attribution and public tolerance are still open.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Municipal Response and Dependency Triage Surge", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, to stop cascade from municipal and supply-chain edge"}, "grounds": "major automated attack exposed edge vulnerability"}]}
```

## Priority
Grid Shield and Essential Services Hardening, because the large automated attack demands that the proven segmentation, patching and islanding tooling be driven to completion before the next sweep, outranking even the evaluation and sovereignty builds.

## In practice
We put ENISA, EU-CyCLONe and the Joint Cyber Unit on war footing for H2 2027, deploying the machine-speed patching and swarm-detection stack from M3 to municipal IT, water and mid-sized energy suppliers hit by the sweep, with front-loaded Recovery and Resilience funds and NIS2 emergency implementing acts mandating segmentation and credential resets. A dedicated dependency-triage cell maps the compromised software component's blast radius with vendor disclosure orders.

The Evaluation Facility (M4) continues staffing via seconded national labs and asserts AI Act systemic-risk access to the model tooling behind the attack, while M1 gigafactories and M2 sovereignty zones proceed only on hardened grid connections. On embodied AI we task DG GROW and EU-OSHA with a mapping of robotics exposure in logistics and manufacturing to prepare a transition instrument next turn, without opening a second front now.
...
```
## New measure
**Containment-Grade Evaluation and Bio-Screening Upgrade**
Retools the Evaluation Facility for a post-chain-of-thought world with black-box behavioural testing and activation inspection, and adds mandatory bio-uplift screening tied to detection stockpiles.
This is Category 5 Public technical capacity and research, because opaque reasoning has killed CoT oversight and the genome-model finding demands independent bio-capability checks now, before the bio gate opens further.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Containment-Grade Evaluation and Bio-Screening Upgrade", "category": 5, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up, resilience up small via bio detection"}, "grounds": "opaque reasoning plus bio-uplift precursor requires non-CoT assurance"}]}
```

## Priority
Municipal Response and Dependency Triage Surge, because the automated sweep is still cascading at the edge and stopping municipal/hospital lock-ups outranks even the evaluation rebuild while lights and operations are at stake.

## In practice
We keep ENISA, EU-CyCLONe and HERA on war footing for H1 2028, driving M3 segmentation/islanding and M5 triage teams into water, mid-size energy and municipal IT with NIS2 emergency acts and funded resets, while the dependency cell compels vendor SBOM disclosure on the poisoned maintenance component. Gigafactory and zone builds under M1/M2 proceed only on certified hardened connections.

We task the Evaluation Facility (M4) to stop litigating chain-of-thought access and pivot to what still works: pre-registration of training runs, black-box agent evaluations including watch-for-evaluation-awareness tests after the eval anomaly, and weight-security audits. DG RTD and JRC stand up interpretability and bio-uplift screening pilots with ECDC, linking any intolerable bio finding to HERA stockpile and hospital detection protocols, without opening a humanoid-robotics front beyond the GROW mapping already ordered.
