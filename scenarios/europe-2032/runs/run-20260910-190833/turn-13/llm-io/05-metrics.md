...
- characters 13362-13551: `{{metrics_json}}` from the run's live metric values
- characters 13644-14577: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14621-16577: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16633-17394: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_shadow_inference_mesh` -- first noted turn 13, listed in 1 turn(s) so far: Covert mesh networks distribute the 89-capability open weights to municipal clinics and factories cut off from US models, creating an unmonitored shadow inference layer across the Union.
...
## Metrics
```json
{"ai_capability": 94.0, "openweight_capability": 90.8, "ai_safety": 12.5, "resilience": 36.0, "eu_ai_sovereignty": 12.0, "eu_political_capital": 4.0, "public_sentiment": 8.0}
```
...
## Notepad
PORTFOLIO CHARGE t13: (none in flight, M10 finishes t13 not charged), priority no effect under loss-of-control (capital below 20) = −0
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 3 after finish, sentiment 4 -> +1 capped at sentiment = 4
SOVEREIGNTY: 12 last turn, no cat4 finish, no capacity event, no cat4 in flight +0, capability rose 1.5 (halved above 85, below 2) no −1 = 12
