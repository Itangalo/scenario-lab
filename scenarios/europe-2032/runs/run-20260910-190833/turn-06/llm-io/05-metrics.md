...
- characters 105-13783: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13826-14017: `{{metrics_json}}` from the run's live metric values
- characters 14110-15538: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15581-17722: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17778-18238: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Notepad
PORTFOLIO CHARGE t6: InvestAI Gigafactories −3, Containment-Grade Evaluation and Bio-Screening Upgrade −2, priority −1 = −6
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 5, sentiment 21 -> +2 capped by prior event losses
SOVEREIGNTY: 17 last turn, Tech sovereignty package finishes t6 +4, no in-flight cat4 bonus +0, eu_frontier_access_denied t6 −1, capability rose 8.0 −1 = 19

## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "ALLIANCE"}, "grounds": "turn 5 election_alliance winner takes office turn 6"}]}
```
```
