...
- characters 375-1803: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1846-3987: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4043-5122: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5185-33160: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 33515-33884: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, Grid Shield and Essential Services Hardening −3, Municipal Response and Dependency Triage Surge −2, Containment-Grade Evaluation and Bio-Screening Upgrade −2, priority −1 = −14
...
- `emergent_robot_workplace_accident` -- first noted turn 4, listed in 2 turn(s) so far: A warehouse humanoid in an EU logistics pilot injures workers after a control-software update, triggering national safety suspensions and a wider debate over foreign robotics dependence.
- `emergent_datacentre_blockades` -- first noted turn 4, listed in 2 turn(s) so far: A coordinated physical protest movement blocks several EU data-centre and grid-hardening construction sites, merging anti-AI, energy-price and municipal-ransomware anger into site occupations that delay builds for weeks.
...
- Turn 2 (4 turn(s) ago): capability_jump, cyber_defence_breakthrough
- Turn 3 (3 turn(s) ago): cyber_major_incident, embodied_ai_deployment
- Turn 4 (2 turn(s) ago): bio_uplift_findings, eval_anomaly_reports, opaque_reasoning
- Turn 5 (1 turn(s) ago): capability_jump, safety_breakthrough, election_alliance
