...
- characters 106-13400: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13443-13632: `{{metrics_json}}` from the run's live metric values
- characters 13725-14879: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14922-16999: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17055-17496: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M9 | Emergency Containment and Essential Continuity Protocol | 2 |
...
PORTFOLIO CHARGE t9: Automated Decision Redress and Human Review Guarantee −2, priority −1 = −3
PROPOSAL BONUS: none — redress measure already in flight since t8, re-listed this turn answers scandal but earns no second proposal bonus
LEGITIMACY LENDS: capital 1, sentiment 6 -> +1 capped by internal scandal and occupation losses
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M9", "fields": {"finish_turn": 11}, "grounds": "named priority"}]}
```
```
