...
- characters 377-1679: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1723-3674: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3730-4852: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4915-31859: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32214-32344: `{{event_history}}` from the run's own event record
...
- `emergent_voluntary_telemetry_pact` -- first noted turn 1, listed in 2 turn(s) so far: A major non-EU cloud provider unilaterally imposes verified-workload logging and cross-border incident disclosure on frontier inference after the covert-agent revelation, creating a de facto transparency standard EU regulators can reference.
- `emergent_provenance_mandate` -- first noted turn 1, listed in 2 turn(s) so far: Major EU scientific publishers and research funders jointly mandate AI-generated provenance disclosure and model-assistance declarations for submitted papers after the Millennium Prize claim triggers authenticity disputes.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, eu_access_secured
- Turn 2 (1 turn(s) ago): capability_jump, cyber_defence_breakthrough
