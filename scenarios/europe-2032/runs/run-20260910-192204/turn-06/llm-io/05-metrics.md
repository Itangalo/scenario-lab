...
- characters 105-13310: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13353-13545: `{{metrics_json}}` from the run's live metric values
- characters 13638-15559: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15602-17631: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17687-18201: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Trustworthy Adoption Dividend | 2 |
| M7 | Joint Cyber Shield Accession | 2 |
...
PORTFOLIO CHARGE t5: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M6 EU Trustworthy Adoption Dividend −2, priority −1 = −9
PROPOSAL BONUS: none — new Adoption Dividend answers adoption_success (positive) not a negative shock from last three turns
LEGITIMACY LENDS: capital 12 after charges+finishes, sentiment 23 -> +2 (capped below sentiment)
SOVEREIGNTY: 18 last turn, M5 EU Essential-Service AI Fallback finishes t5 +3, Gigafactories in flight +0, no capacity event, capability rose 1.5 −0 = 21
...
**emergent_clinical_backlash_freeze (emergent event):** Municipal hospital federations and sickness funds publish a joint procurement boycott of hallucination-prone fallback triage tools after a medication-form error, freezing EU-funded rollout for months.
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election winner takes office turn 6"}, {"op": "update", "table": "measures", "id": "M7", "fields": {"finish_turn": 7}, "grounds": "named priority"}]}
```
```
