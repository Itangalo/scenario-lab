...
- characters 376-2297: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2340-4369: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4425-5266: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5329-26650: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27005-27356: `{{event_history}}` from the run's own event record
...
- `emergent_clinical_backlash_freeze` -- first noted turn 4, listed in 2 turn(s) so far: Municipal hospital federations and sickness funds publish a joint procurement boycott of hallucination-prone fallback triage tools after a medication-form error, freezing EU-funded rollout for months.
...
- Turn 2 (4 turn(s) ago): cyber_major_incident, export_control_escalation
- Turn 3 (3 turn(s) ago): cyber_major_incident, ai_investment_collapse, eu_frontier_access_denied
- Turn 4 (2 turn(s) ago): emergent_frontier_lab_stall
- Turn 5 (1 turn(s) ago): adoption_success, election_retrenchment
