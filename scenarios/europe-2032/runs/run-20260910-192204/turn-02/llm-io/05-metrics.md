...
- characters 105-12927: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12970-13162: `{{metrics_json}}` from the run's live metric values
- characters 13255-14298: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14341-16577: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16633-17674: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_phage_publication_freeze` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher and a leading lab jointly announce a temporary moratorium on publishing detailed genome-model phage methods after the summer phage results, triggering a fight over open science versus biosecurity.
- `emergent_openweight_fraud_wave` -- first noted turn 1, listed in 2 turn(s) so far: Major EU press investigation traces Kimi K3-derived phishing and fraud kits to organized crime groups operating inside the EU, triggering calls for open-weight liability in the AI Act review.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 49.8, "ai_safety": 30.0, "resilience": 36.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 29.0, "public_sentiment": 35.0}
```
