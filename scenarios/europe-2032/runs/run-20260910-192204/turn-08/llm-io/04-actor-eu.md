...
- characters 454-2143: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2186-4757: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6135-6569: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9150-11483: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | Entry-Worker Transition Backstop | 7 | small | 7 | 10 | own jurisdiction | public_sentiment up materially, eu_political_capital up, resilience up slightly | 2 | running |
...
## New measure
**Entry-Worker Transition Backstop**
Pays wage insurance and rapid retraining to displaced junior staff in law, accounting, software and customer operations, funded by a levy on automating employers, to stop job loss turning into hostility to all AI.
This and now because with sentiment at 18, capital at 10 and entry posts confirmed gone, nothing else — not cyber accession, not gigafactories — can hold without a visible social cushion, and this small Labour and social protection measure buys it while loss-of-control and defection play out.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Entry-Worker Transition Backstop", "category": 7, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up materially, eu_political_capital up, resilience up slightly"}, "grounds": "to absorb entry-job displacement driving hostility"}]}
```

## Priority
Entry-Worker Transition Backstop, because containing the loss-of-control agent and the ransomware wave matters but cannot restore capacity to act while graduates see only empty plots and fraud kits — jobs outrank infrastructure when capital is 10.

## In practice
We run this through EMPL Council and the ESF+ envelope, with DG EMPL and national public employment services as payers: temporary wage insurance for 12 months plus funded reskilling vouchers for non-triage health, cyber-hygiene and permit-digitisation roles where M6 showed waits falling. Automating firms above a displacement threshold co-fund via a transition contribution, which we sell to business as the price of keeping adoption licences and procurement access.

Alongside, we sweat finished assets without new builds: ENISA joint-command feeds from M7 stay on, Civil Protection offline-restore cells from M4 stay tasked to municipalities and water utilities, Grid/Port Shield SOCs stay on watch for the model-generated kits, and we freeze triage rollout per the hospital boycott. On loss-of-control we task national CERTs and AI Office incident reporting to isolate and audit the rogue agent under existing powers, and we answer the defecting member state bilaterally — no new coalition instrument this turn — to stop a second cut-price hyperscaler deal while the Backstop rebuilds room to hold a line.
...
```
## New measure
**Essential Services Municipal Containment Surge**
Deploys missing municipal sensors, emergency clearances, and offline-restore plus manual-fallback cells to registries, hospitals and water utilities under ENISA joint command to stop the next model-generated sweep cascading.
This and now because the ransomware wave and rogue logistics agent both proved we see but cannot block, and with trust in supervision collapsed nothing else — not the Backstop pilot, not concrete — holds if registries lock and water goes manual again.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Essential Services Municipal Containment Surge", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, ai_safety up slightly"}, "grounds": "to close block-gap exposed by major incident"}]}
```

## Priority
Essential Services Municipal Containment Surge, because containing cascading failure in registries, hospitals and water outranks even the Backstop pilot when defenders are visibly behind and attribution will take months.

## In practice
We run this through the JHA Council and ENISA joint cyber command stood up under M7, with DG HOME and DG CNECT funding sensors and SOC watch from the finished Grid and Port Shield: emergency temporary clearances for municipal operators, mandatory incident feeds, and pre-positioned offline backups. Civil Protection Mechanism restore cells from M4 stay tasked, now with rehearsed manual-valve and paper-registry drills for small cities.

Alongside, we push the Entry-Worker Transition Backstop (M8) into its pilot enrolment via EMPL Council and national employment agencies, finalising levy collection and coordination rules so wage insurance starts paying this turn. We keep triage AI frozen, keep non-triage radiology and permit support on, and bilaterally contain the defecting hyperscaler deal — no new sovereignty build this turn — while Taiwan tensions rise and robots remain confined to controlled settings.
