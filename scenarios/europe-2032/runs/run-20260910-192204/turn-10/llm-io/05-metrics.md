...
- characters 106-13395: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13438-13627: `{{metrics_json}}` from the run's live metric values
- characters 13720-15494: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15537-17614: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17670-18888: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M11 | Blockaded Sites Local Benefit and Entry-Jobs Compact | 2 |
...
PORTFOLIO CHARGE t9: Entry-Worker Transition Backstop −2, EU Bio Early-Warning and Health Resilience Surge −2, priority −1 = −5
PROPOSAL BONUS: EU Bio Early-Warning and Health Resilience Surge (cat 6, small) +3 — contested genome-model uplift precursor opened 4-turn bio gate last turn and joint biosurveillance pact offer arrived this turn, this answers both directly
LEGITIMACY LENDS: capital 12, sentiment 8 -> none, capital higher
SOVEREIGNTY: 27 last turn, no cat4 finish, no cat4 in flight +0, no capacity event, capability rose 0.5 −0 = 27

## Emerging developments (tracked)

- `emergent_biosurge_site_backlash` -- first noted turn 10, listed in 1 turn(s) so far: Copycat blockades spread from gigafactory sites to wastewater sequencing hubs and medical stockpiles, with activists alleging bio-surveillance overreach and forcing temporary suspension of Bio Early-Warning installations.
- `emergent_biosurveillance_false_alarm` -- first noted turn 10, listed in 1 turn(s) so far: A contested wastewater sequencing hit from the new Bio Early-Warning Surge triggers a city-wide public health scare and temporary controls before confirmatory testing clears it, discrediting the surge while it is still procurement.
...
## New measure
**Blockaded Sites Local Benefit and Entry-Jobs Compact**
Puts blockaded gigafactory and data-centre sites back to work through binding local-benefit agreements — local hiring quotas for entry workers, community energy and tax shares, and mediated construction restart via Council dialogue — funded by reprogrammed InvestAI regional envelopes.
This and now because with capital at 9 and sentiment at 6 concrete curing in the rain is a political death spiral: no restart without local consent, no sovereignty without restart, and wage cheques alone do not shorten queues.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Blockaded Sites Local Benefit and Entry-Jobs Compact", "category": 7, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up materially, eu_political_capital up, eu_ai_sovereignty up slightly"}, "grounds": "to restart blockaded builds via local consent"}]}
```

## Priority
Blockaded Sites Local Benefit and Entry-Jobs Compact, because it is the only measure that simultaneously unblocks sovereignty builds and answers jobless-transition anger that is driving sentiment to 6.

## In practice
We task DG REGIO and DG ENER with the restart: temporary pause orders stay, but Civil Protection site guard converts to mediated access, with prefects and mayors in the three blocking states convening site-by-site tables with contractors, farmers and graduate pickets. The offer is concrete: earmarked apprenticeships and entry posts tied to the Transition Backstop cohorts, municipal power-price rebates from the substations, and a share of land-lease revenue held locally.

Alongside, HERA/ECDC keep the Bio Early-Warning Surge (M10) procurement moving — wastewater nodes and stockpile pre-positioning — and ENISA joint feeds plus municipal paper kits stay tasked for containment. We do not launch new compute or lift the triage freeze; we contain the bilateral cloud deal and let wage insurance keep paying while the compact tries to turn shells into jobs.
