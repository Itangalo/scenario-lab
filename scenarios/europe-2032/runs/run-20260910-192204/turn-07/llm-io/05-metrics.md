...
- characters 106-13469: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13512-13704: `{{metrics_json}}` from the run's live metric values
- characters 13797-14934: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14978-17028: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17084-18164: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M8 | Entry-Worker Transition Backstop | 2 |
...
PORTFOLIO CHARGE t6: M1 InvestAI Gigafactories −3, M6 EU Trustworthy Adoption Dividend −2, M7 Joint Cyber Shield Accession −2, priority −1 = −8
PROPOSAL BONUS: Joint Cyber Shield Accession (cat 8, small) +3 — cyber_major_incident fired this turn and this answers it directly via pooling
LEGITIMACY LENDS: capital 8, sentiment 18 -> +2
SOVEREIGNTY: 21 last turn, M2 Tech sovereignty package finishes t6 +4, Gigafactories in flight +0, no capacity event, capability rose 1.0 −0 = 25

## Emerging developments (tracked)

- `emergent_site_occupations` -- first noted turn 7, listed in 1 turn(s) so far: Coordinated occupations and blockades of fenced gigafactory plots and planned data-centre sites by graduate and municipal groups, halting site works for weeks and forcing evictions covered as national news.
- `emergent_site_blockade_wave` -- first noted turn 7, listed in 1 turn(s) so far: Coordinated physical blockades of fenced gigafactory plots and data-centre substations spread across three member states, forcing extended construction halts and private security withdrawals.
...
### Backstop
The answer was cheques, not chips. The Transition Backstop passed as a small social measure: 12-month wage insurance plus retraining vouchers for displaced juniors, co-funded by a levy on automating firms. Employment offices began enrolling in December; business lobbies grumbled but paid to keep procurement access. Triage AI stayed frozen, non-triage permits and radiology support stayed on, and offline-restore cells kept touring small towns. It was too early to change hiring charts, but queues at job centres shortened.
## Notepad
PORTFOLIO CHARGE t7: Entry-Worker Transition Backstop −2, priority −1 = −3
PROPOSAL BONUS: Entry-Worker Transition Backstop (cat 7, small) +3 — entry-post collapse and sentiment 18 last two turns and this answers it directly
LEGITIMACY LENDS: capital 10, sentiment 18 -> +2
SOVEREIGNTY: 25 last turn, M1 InvestAI Gigafactories finishes t7 +5, no cat4 in flight +0, member_state_defection t7 −2, capability rose 0.5 −0 = 28
## Store changes
```json
{"store": []}
```
