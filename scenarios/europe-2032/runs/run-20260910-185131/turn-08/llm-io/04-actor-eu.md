...
- characters 453-1473: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1516-3243: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4125-4551: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6538-8155: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M3 | Grid and Port Cyber Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up materially; ai_safety up slightly via incident learning | 3 | finished |
| M4 | Essential Services Continuity Reserve | 6 | large | 2 | 6 | own jurisdiction | resilience up materially | 3 | finished |
| M5 | EU Assurance Adoption Cell | 5 | small | 3 | 5 | own jurisdiction | ai_safety up moderately | 2 | finished |
| M6 | EU Patch Sprint | 6 | small | 7 | 8 | own jurisdiction | resilience up moderately | 2 | finished |
...
## New measure
**EU Patch Sprint**
Deploys the breakthrough defensive tooling – automated patching and swarm-behaviour detection – across critical grid, port and health operators via ENISA-coordinated rollout.
This is cheap, visible and uses existing Cyber Shield certification channels, which is why it fits now when capital is at 2.0 and no new fund can pass, while exploiting a window that closes as attackers adapt.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Patch Sprint", "category": 6, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately"}, "grounds": "exploiting cyber defence breakthrough with no new fund"}]}
```

## Priority
EU Patch Sprint, because it is the only instrument affordable at capital 2.0 that can deliver a visible win and slow the political bleed while gigafactories remain stalled awaiting US tiering.

## In practice
We task ENISA and DG CNECT to push the breakthrough patching and detection stacks through the Grid and Port Cyber Shield vendor list, using existing NIS2 implementing acts – no new vote, no new money, just accelerated certification and joint procurement of the tooling for the two breached operators first, then smaller utilities.

On access and jobs we stay in holding: DG ENER keeps gigafactory grid-connection queues warm, DG TRADE maps US tiering exposure, and we answer graduate unemployment only with the Eurostat dashboard, because finance ministers still block fresh spending and any large initiative would collapse and cost the last of our capital.
...
```
## New measure
**Graduate Junior Posts Bridge**
Funds time-limited paid junior placements plus retraining vouchers in affected university cities, tied to public-sector and SME adoption of AI tools.
Why this and why now: with capital at 4.0 and sentiment at 30, concrete without jobs and a member-state breakaway are bleeding the last cohesion; a small labour measure (category 7) is the only affordable visible answer to town-hall anger over missing junior posts while the productivity plateau removes the transition story.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Graduate Junior Posts Bridge", "category": 7, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up slightly via social absorption"}, "grounds": "answering graduate unemployment with affordable placements"}]}
```

## Priority
Graduate Junior Posts Bridge, because holding the Union line after a defection requires showing jobs now, not gigafactories later, and it outranks access-mapping under our two-year commitment this turn.

## In practice
We run this through DG EMPL and EGF-ESF+ top-ups, no new regulation: Council EPSCO conclusions authorise reprogrammed ESF+ envelopes for 6-12 month paid placements in administration, hospitals and SMEs using EU AI tools, with wage top-up conditional on employers retaining mentoring. University cities with sharpest graduate unemployment get first calls.

On sovereignty we hold without spending: DG CNECT and DG TRADE keep the two poured sites on grid-reserve, push transformer orders via joint procurement, and quietly negotiate a no-undercut pledge in COREPER to contain the defecting member state, while ENISA sustains the Patch Sprint rollout to smaller utilities with existing funds. No new fund is tabled because any large vote would fail at capital 4.0.
