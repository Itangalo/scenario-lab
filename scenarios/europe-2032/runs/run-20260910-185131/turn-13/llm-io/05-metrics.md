...
- characters 107-12819: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12862-13053: `{{metrics_json}}` from the run's live metric values
- characters 13146-14421: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14465-16767: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16823-17512: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M9 | EU Cyber Defence Upgrade Rollout | 2 |
...
- `emergent_research_exit_signal` -- first noted turn 12, listed in 2 turn(s) so far: Major European clinical research consortium announces it will move frontier model training and evaluation outside the EU, citing unreadable model reasoning and lack of enforceable EU evaluation access.
...
## Notepad
PORTFOLIO CHARGE t13: EU Cyber Defence Upgrade Rollout −2, priority −1 = −3
PROPOSAL BONUS: none — new cat6 rollout exploits positive breakthrough, does not answer investment collapse shock
LEGITIMACY LENDS: capital 0.0, sentiment 29.0 -> +2 (sentiment above capital, applied after finish and event terms, capped below sentiment)
SOVEREIGNTY: 29 last turn, ai_investment_collapse t13 −2, no cat4 finish or in flight +0, capability rose 0.6 −0 = 27

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M9", "fields": {"finish_turn": 14}, "grounds": "named priority"}]}
```
```
