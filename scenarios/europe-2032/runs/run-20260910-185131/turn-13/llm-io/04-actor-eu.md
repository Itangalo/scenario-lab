...
- characters 455-1730: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1774-4076: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4763-5221: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7560-8834: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Rebuild trust and continuity after cascading AI harm with what we can still enforce
...
| M7 | Graduate Junior Posts Bridge | 7 | small | 8 | 9 | own jurisdiction | public_sentiment up moderately, resilience up slightly via social absorption | 2 | finished |
| M8 | EU Continuity and Incident Learning Compact | 6 | small | 10 | 11 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
...
## New measure
None this turn, because with political capital at 1.0 we cannot fund even a small measure without collapse and must wait for capital to recover.

## Store changes
No changes.

## Priority
No priority can be enforced this turn, because no measure is in flight and any push would exceed the capital we hold.

## In practice
We hold the line with what has finished landing. ENISA and national cyber centres continue mutual-aid patching and backup restoration under NIS2, and the hospital procurement guidance from the Assurance Adoption Cell stays in force through the AI Act serious-incident channel — no new vote, no new money. The DG REFORM waiting-list results remain our sole communication asset to stabilise sentiment at 31.0.

We absorb the two outside shocks without legislating. The interpretability/control breakthrough is routed through the AI Office and Joint Research Centre into procurement checks and incident-learning logs, since laboratories want it adopted and it costs us no enforcement fight. On the US-China agreement, the EEAS and the Commission presidency seek observer/consulted status and offer EU evaluation and resilience experience as contribution, but table no binding EU commitment until coffers and cohesion allow us to carry one.
...
```
## New measure
**EU Cyber Defence Upgrade Rollout**
Deploys the new automated patching and swarm-detection tooling across critical public services and municipal utilities via ENISA and national cyber centres.
This is cheap, proven outside help that fits our rebuild-trust commitment and must be seized this turn before the window closes, while investment collapse means we cannot afford anything that needs new build-out.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cyber Defence Upgrade Rollout", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via incident learning"}, "grounds": "to exploit defensive breakthrough with no new funds needed"}]}
```

## Priority
EU Cyber Defence Upgrade Rollout, because it is the only in-flight work that converts an outside breakthrough into visible steadier services without spending capital we do not have.

## In practice
We task ENISA with packaging the breakthrough tooling — automated patching at vulnerability speed and swarm-behaviour detection — into the existing mutual-aid crews and NIS2 incident-response track, funded by reprogramming the Continuity Reserve maintenance line, no new Council vote. National cyber centres get updated playbooks and Joint Research Centre validation from the labs' control result already in guidance.

We sell it through the DG REFORM clinic-tour asset: same European-procured assistants, now patched and monitored faster, to stabilise sentiment at 33.0. On investment collapse, DG COMP and DG CNECT quietly scout cancelled private build-out for cheap takeover options, but commit no money this turn; we hold the line until capital recovers above 3.0.
