...
- characters 106-13517: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13560-13752: `{{metrics_json}}` from the run's live metric values
- characters 13845-14691: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14735-16780: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16836-17988: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_biosurveillance_mandate_push` -- first noted turn 7, listed in 1 turn(s) so far: A WHO-led independent review attributes the winter pathogen spread partly to delayed cross-border sequencing disclosure, triggering calls for binding sample-sharing timelines outside EU control.
- `emergent_containment_powers_challenge` -- first noted turn 7, listed in 1 turn(s) so far: Coordinated rural-clinic and municipal lawsuits over containment energy overrides coalesce into a cross-border court challenge constraining EU emergency powers during health crises.
...
The second half of 2029 was supposed to belong to delivery. With no new law proposed, Brussels pushed its public-benefit tools — triage assistants, permit helpers, tutoring and back-office aides — into clinics, schools and town halls. Health and education ministers cut ribbons, auditors cited EU-controlled servers, and for a few weeks the story held.
...
Second, a near-frontier open release landed. Hundreds of thousands of downloads in days put that generation permanently on private machines, including in Europe.
...
That calm undercut Brussels. With no displacement crisis to point at, warnings about rogue agents and unrecallable weights felt abstract. Gigafactory construction still had not started; Spanish and German links stayed conditional, Paris and The Hague still feuded over equipment, now in working groups.
...
By December, the Union had more paper capacity and more public tools, but less authority: useful AI at work, ungovernable AI elsewhere, and a sovereignty project still waiting for concrete.
