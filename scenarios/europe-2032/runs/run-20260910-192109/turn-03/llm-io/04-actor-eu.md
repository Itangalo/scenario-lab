...
- characters 455-1765: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1809-3726: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3794-4274: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6250-8268: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Evaluation and Assurance Capacity | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via talent and evaluation leverage | 2 | running |
...
Stands up independent EU evaluation labs with pre-release access and weight-security audits to close the assurance gap exposed by the grid intrusions.
Category 5 Public technical capacity and research, because assurance currently covers only released models while undetected agent behaviour and open-weight tooling drive the present risk.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Evaluation and Assurance Capacity", "category": 5, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, eu_ai_sovereignty up slightly via talent and evaluation leverage"}, "grounds": "autumn intrusions showed assurance lags capability and open-weight abuse"}]}
```

## Priority
M3 EU Critical Systems Shield, because the mapped-grid swarm window outranks even evaluation build-out under our resilient-and-sovereign commitment.

## In practice
We push M3 through ENISA and the Electricity Coordination Group under CER/NIS2 implementing acts: complete segmentation and breaker-credential audits of the two affected TSOs, start synchronous-area roll-out, and unblock detection procurement with interim standards while 2027 cross-border exercises are staffed. CEF-Digital and Civil Protection funds cover first costs; we offer municipalities co-funding to blunt unfunded-mandate opposition.

In parallel we keep M1 InvestAI Gigafactories and M2 Tech sovereignty package moving via DG CNECT and EIB: site shortlists with reserved grid connections, permitting-zone acceleration, and ASML supply-chain safeguards, linking US lithography pressure to Anti-Coercion Instrument discussion to hold France-Germany-Poland majority. We launch the Evaluation and Assurance Capacity as a small start: Joint Research Centre plus ENISA mandate, seconded national labs, and negotiated evaluator access for staged-tooling detection learning, without opening a parallel compute programme.
...
```
## New measure
**Synchronous-Area Detection Deployment**
Deploys interim-standard intrusion detection across transmission and municipal distribution nodes and runs the postponed cross-border exercises live.
Category 6 Preparedness and resilience, because mapped relays are guarded but visibility gaps and paper-only exercises leave the grid exposed to open-weight tooling.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Synchronous-Area Detection Deployment", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via incident learning"}, "grounds": "detection procurement stalled and exercises remain on paper while intrusions persist"}]}
```

## Priority
M5 Synchronous-Area Detection Deployment, because closing the visibility gap outranks further factory siting until the grid can detect a second swarm.

## In practice
We push the new deployment through ENISA and the Electricity Coordination Group under CER/NIS2 implementing acts: interim detection standards adopted to unblock procurement, CEF-Digital co-funding for municipal inspection costs, and ENTSO-E tasked to run live cross-border exercises in autumn 2027.

In parallel we keep M1 InvestAI Gigafactories and M2 Tech sovereignty package alive via DG CNECT and EIB without extra push: hold site shortlists and reserved grid links, manage the two blocked Spanish and German connections through permitting-zone talks, and handle US lithography pressure in the Trade Council with Anti-Coercion discussion to keep France on board without losing export-dependent states. M4 Evaluation labs continues as JRC-led seconded teams seeking staged-tooling access for detection signatures.
