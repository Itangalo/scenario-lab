...
- characters 455-1872: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1916-3925: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4756-5235: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8726-10790: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_rebuild_exploitation_market (emergent event):** Underground market for pre-patched municipal backup images and stolen hospital triage model adapters spreads across EU forums, letting low-skill crews re-enter already-rebuilt councils with fresh model-generated exploits.
...
- `two_year_commitment` (commitment): Contain distributed AI harm and keep essential services running through the open-weight bio-cyber window
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up strongly, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Evaluation and Assurance Capacity | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via talent and evaluation leverage | 2 | finished |
| M5 | Synchronous-Area Detection Deployment | 6 | small | 3 | 5 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
| M6 | EU Bio-Detection and Medical Stockpile Surge | 6 | small | 4 | 7 | own jurisdiction | resilience up moderately, ai_safety up slightly via misuse learning | 2 | finished |
| M7 | EU Public Benefit AI Deployment | 9 | small | 5 | 8 | own jurisdiction | public_sentiment up moderately, eu_political_capital up slightly, eu_ai_sovereignty up slightly | 2 | finished |
| M8 | EU Bio Emergency Containment and Care Surge | 6 | small | 6 | 8 | own jurisdiction | resilience up moderately, ai_safety up slightly via misuse learning | 2 | finished |
| M9 | EU Distributed Defence and Bio-Response Hardening | 6 | small | 8 | 10 | own jurisdiction | resilience up moderately, ai_safety up slightly via misuse learning | 2 | finished |
| M10 | EU Agent Loss-of-Control Containment and Cyber Cutoff Protocol | 6 | small | 9 | 10 | own jurisdiction | resilience up moderately, ai_safety up slightly via containment learning | 2 | finished |
| M11 | EU Critical Services Restoration and Bio Shield Surge | 6 | small | 10 | 11 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
| M12 | EU Open-Weight Bio-Cyber Containment Patch | 6 | small | 11 | 12 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
| M13 | EU Essential Services Continuity Sustainment | 6 | small | 12 | 13 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
...
## New measure
**EU Essential Services Continuity Sustainment**
It keeps the degraded-but-running posture funded through the next window by sustaining patch-kit resupply, hospital triage on domestic systems, and border bio-detection stocks with 24h sequencing sharing under existing mandates. Because with capital at 22, safety at 8, and near-frontier weights permanently distributed, only a small preparedness and resilience instrument without a new law can hold containment and care through the bio-cyber window our commitment names.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Continuity Sustainment", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via incident learning"}, "grounds": "open-weight bio-cyber diffusion plus rationed supply requires sustained containment"}]}
```
## Priority
The new EU Essential Services Continuity Sustainment, because it is the only in-flight measure and directly carries our two-year commitment to contain distributed harm and keep essential services running.
## In practice
We act without new legislation, using NIS2 Articles 11-12 and the Serious Cross-Border Threats Regulation as legal base. ENISA and national CERTs lead patch-kit resupply and isolation-cutoff rehearsals for lagging councils, with Digital Europe reprogrammed funds for overtime and clean-backup rebuilds; HERA and ECDC tie EU4Health and rescEU money to voluntary 24-hour sequencing disclosure and replenishment of border detection stocks.

In the Health and JHA Councils we extend the isolation authorities rehearsed in 2031 and keep hospitals on domestic inference for triage to avoid dependence on rationed US models. Europol EC3 tracks reports of stolen backup images and hospital adapters, while mutual-aid technician pools and accelerated kit delivery are offered to complaining councils to hold cohesion while fraud coverage and data-centre protests keep sentiment at 31.
...
```
## New measure
**EU Municipal Clean-Backup and Compromised Adapter Purge**
It revokes stolen backup images and hospital triage adapters, pushes clean offline rebuilds to hit councils, and hardens resupply against the automated ransomware wave. Because the major cyber incident and the low-skill re-entry market hit exactly the degraded-but-running councils our two-year commitment to contain distributed harm and keep services running must hold.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Clean-Backup and Compromised Adapter Purge", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via incident learning"}, "grounds": "automated ransomware sweep plus backup-image re-entry market requires purge and clean rebuild"}]}
```
## Priority
The new EU Municipal Clean-Backup and Compromised Adapter Purge, because it is the only in-flight measure and it directly answers this turn's incident and market while carrying our containment commitment.
## In practice
We act without new law under NIS2 Articles 11-12 and the Serious Cross-Border Threats Regulation. ENISA and national CERTs lead emergency cutoff and clean-backup restoration for the ransomware-hit public services, with EC3 tracing the forum trade in backup images and adapters for revocation and takedown, funded by reprogrammed Digital Europe and EU4Health/rescEU money for overtime, adapters, and offline vaults.

In the JHA and Health Councils we extend isolation authorities and keep hospitals on domestic triage inference while compromised adapters are purged and reissued. HERA/ECDC sustain voluntary 24h sequencing and border stocks, and we offer accelerated clean kits and mutual-aid technician pools to southern and eastern councils to hold cohesion while recovery runs slow.
