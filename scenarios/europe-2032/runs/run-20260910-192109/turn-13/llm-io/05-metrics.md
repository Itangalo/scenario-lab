...
- characters 107-13350: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13393-13584: `{{metrics_json}}` from the run's live metric values
- characters 13677-15094: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15138-17147: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17203-17560: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M14 | EU Municipal Clean-Backup and Compromised Adapter Purge | 2 |
...
## Notepad
PORTFOLIO CHARGE t13: EU Municipal Clean-Backup and Compromised Adapter Purge −2, priority −1 = −3
PROPOSAL BONUS: EU Municipal Clean-Backup and Compromised Adapter Purge (cat 6, small) +2 — cyber_major_incident and rebuild-exploitation market landed this turn and this answers them directly
LEGITIMACY LENDS: capital 27, sentiment 24 -> none, capital is higher
SOVEREIGNTY: 23.8 last turn, no capacity event, no cat4 in flight, capability rose 0.7 no charge = 23.8
## Store changes
```json
{"store": []}
