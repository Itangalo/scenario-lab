...
- characters 1878-4039: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4095-5174: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5237-31923: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32278-32648: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Bio-Detection and Medical Stockpile Surge −2, EU Public Benefit AI Deployment −2, priority −1 = −11
PROPOSAL BONUS: none — M7 (cat 9, small) answers adoption_success (positive) not a negative shock; loss_of_control answered by finished M4, not by diffusion rollout
...
- `emergent_detection_legal_freeze` -- first noted turn 4, listed in 2 turn(s) so far: A court in one of the two challenging regions suspends municipal inspection and entry powers under the interim detection standards, freezing distribution-level deployment pending a CJEU ruling.
- `emergent_distribution_outage` -- first noted turn 4, listed in 2 turn(s) so far: A distribution-grid intrusion exploits interim detection gaps and false-positive fatigue, causing localized outages blamed on the stalled municipal inspection regime.
...
- Turn 1 (5 turn(s) ago): cyber_test_shot, openweight_frontier_release, knowledge_work_augmented, taiwan_tension_rise
- Turn 2 (4 turn(s) ago): knowledge_work_augmented
- Turn 4 (2 turn(s) ago): bio_uplift_findings, eval_anomaly_reports, labour_displacement, taiwan_tension_rise
- Turn 5 (1 turn(s) ago): loss_of_control_incident, adoption_success, election_retrenchment
