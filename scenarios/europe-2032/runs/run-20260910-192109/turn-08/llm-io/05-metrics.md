...
- characters 105-13525: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13568-13760: `{{metrics_json}}` from the run's live metric values
- characters 13853-15272: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15315-18372: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18428-19089: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M9 | EU Distributed Defence and Bio-Response Hardening | 2 |
...
- `emergent_containment_powers_challenge` -- first noted turn 7, listed in 2 turn(s) so far: Coordinated rural-clinic and municipal lawsuits over containment energy overrides coalesce into a cross-border court challenge constraining EU emergency powers during health crises.
...
## Notepad
PORTFOLIO CHARGE t8: M9 EU Distributed Defence and Bio-Response Hardening −2, priority −1 = −3
PROPOSAL BONUS: none — knowledge_work_augmented blocks rule 6 bonus for spending against unfelt problem, small cat 6 gesture at this-turn precursor earns nothing
LEGITIMACY LENDS: capital 16, sentiment 38 -> +2
SOVEREIGNTY: 27.8 last turn, no capacity finish, embodied_ai_deployment t8 −2, capability rose 0.6 no charge = 25.8
## Store changes
```json
{"store": []}
```
