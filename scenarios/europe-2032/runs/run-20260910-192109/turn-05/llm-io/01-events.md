...
- characters 377-2233: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2277-4441: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4497-5605: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5668-34153: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 34508-34786: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Evaluation and Assurance Capacity −2, Synchronous-Area Detection Deployment −2, EU Bio-Detection and Medical Stockpile Surge −2, priority −1 = −13
PROPOSAL BONUS: EU Bio-Detection and Medical Stockpile Surge (cat 6, small) +2 — bio_uplift_findings landed this turn and this answers it directly
LEGITIMACY LENDS: capital 12, sentiment 43 -> +2
SOVEREIGNTY: 19 last turn, no capacity event, Gigafactories + Tech package in flight +0, capability rose 2.0 −1 = 18

## Emerging developments (tracked)

- `emergent_detection_legal_freeze` -- first noted turn 4, listed in 1 turn(s) so far: A court in one of the two challenging regions suspends municipal inspection and entry powers under the interim detection standards, freezing distribution-level deployment pending a CJEU ruling.
- `emergent_distribution_outage` -- first noted turn 4, listed in 1 turn(s) so far: A distribution-grid intrusion exploits interim detection gaps and false-positive fatigue, causing localized outages blamed on the stalled municipal inspection regime.
...
- Turn 1 (4 turn(s) ago): cyber_test_shot, openweight_frontier_release, knowledge_work_augmented, taiwan_tension_rise
- Turn 2 (3 turn(s) ago): knowledge_work_augmented
- Turn 4 (1 turn(s) ago): bio_uplift_findings, eval_anomaly_reports, labour_displacement, taiwan_tension_rise
