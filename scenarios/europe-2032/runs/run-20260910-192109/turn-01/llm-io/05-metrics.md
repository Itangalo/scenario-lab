...
- `emergent_open_exploit_proliferation` -- first noted turn 1, listed in 1 turn(s) so far: A Kimi K3-derived automated intrusion toolkit targeting municipal utilities circulates openly on underground forums, with copycat use reported outside the main test-shot campaign.
- `emergent_datacentre_permit_freeze` -- first noted turn 1, listed in 1 turn(s) so far: A coordinated activist campaign blocks grid connections for two hyperscale data-centre projects in Spain and Germany, forcing a national-level permitting review that delays EU compute expansion.
