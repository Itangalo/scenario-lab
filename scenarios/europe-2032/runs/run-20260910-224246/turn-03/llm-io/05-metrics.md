...
- characters 106-13261: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13304-13496: `{{metrics_json}}` from the run's live metric values
- characters 13589-15213: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15257-17136: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17192-18335: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_cyber_insurance_retrenchment` -- first noted turn 2, listed in 2 turn(s) so far: A major European insurer reprices cyber cover after the grid intrusion audit, sharply raising premiums or withdrawing cover for municipal utilities and mid-sized industrials, forcing emergency state backstops.
- `emergent_cyber_insurance_repricing` -- first noted turn 2, listed in 2 turn(s) so far: A consortium of European insurers and reinsurers publishes exclusion clauses for AI-enabled grid and port cyber losses, abruptly repricing industrial cyber cover and forcing operators to disclose segmentation gaps.
