...
- characters 377-2001: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2045-3924: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3980-5123: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5186-27950: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28305-28385: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU AI Evaluation and Safety Institute −3, EU Civil Biodefence and Infrastructure Hardening Shield −3, priority −1 = −13
PROPOSAL BONUS: EU Civil Biodefence and Infrastructure Hardening Shield (cat 6, large) +5 — bio_incident severe this turn and this answers it directly
...
- `emergent_cyber_insurance_retrenchment` -- first noted turn 2, listed in 1 turn(s) so far: A major European insurer reprices cyber cover after the grid intrusion audit, sharply raising premiums or withdrawing cover for municipal utilities and mid-sized industrials, forcing emergency state backstops.
- `emergent_cyber_insurance_repricing` -- first noted turn 2, listed in 1 turn(s) so far: A consortium of European insurers and reinsurers publishes exclusion clauses for AI-enabled grid and port cyber losses, abruptly repricing industrial cyber cover and forcing operators to disclose segmentation gaps.
...
- Turn 2 (1 turn(s) ago): bio_incident
