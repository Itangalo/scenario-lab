...
- characters 455-1612: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1656-3544: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5590-6030: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8340-10534: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M6 | EU Critical Operator Patching and Segmentation Disclosure Mandate | 6 | small | 4 | 6 | own jurisdiction | resilience up moderately via patching and disclosure | 2 | running |
...
| M4 | EU Civil Biodefence and Infrastructure Hardening Shield | 6 | large | 2 | 5 | own jurisdiction | resilience up strongly, ai_safety up modestly via incident learning | 3 | finished |
| M5 | EU Accession to Joint Cyber-Bio Threat Response Pact | 8 | small | 3 | 5 | coalition | resilience up via shared telemetry and biosurveillance | 2 | finished |
