...
- characters 13541-13732: `{{metrics_json}}` from the run's live metric values
- characters 13825-15106: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15150-16913: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16969-17933: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|---|---|---|---|---|---|---|
| M5 | EU Public Services AI Scale-Up | 9 | small | 3 | 5 | own jurisdiction | public_sentiment up, eu_political_capital up indirectly, eu_ai_sovereignty up slightly via anchored demand | 2 | finished |
...
- `emergent_enforcement_freeze_ruling` -- first noted turn 5, listed in 1 turn(s) so far: A national administrative court suspends AI Act enforcement fines for public-sector systems pending human-oversight clarification after the benefits-score scandal, freezing new conformity actions for months.
- `emergent_hospital_cover_litigation_freeze` -- first noted turn 5, listed in 1 turn(s) so far: Municipal hospital associations jointly sue the withdrawn cyber-insurer and the state guarantee scheme over denied claims after rushed-isolation outages, freezing patching funds during litigation.
...
## Metrics
```json
{"ai_capability": 69.0, "openweight_capability": 61.0, "ai_safety": 18.0, "resilience": 41.0, "eu_ai_sovereignty": 18.0, "eu_political_capital": 6.0, "public_sentiment": 22.0}
```
...
SOVEREIGNTY: 18 last turn, no capacity event, InvestAI Gigafactories in flight +1, capability rose 5.0 −1 = 18
