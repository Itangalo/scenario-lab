...
- characters 13332-13524: `{{metrics_json}}` from the run's live metric values
- characters 13617-14433: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14477-16407: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16463-17622: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|---|---|---|---|---|---|---|
| M5 | EU Clinic and Municipal Patch Corps | 6 | small | 4 | 5 | own jurisdiction | resilience up moderately | 2 | finished |
...
- `emergent_relay_botnet_persistence` -- first noted turn 4, listed in 2 turn(s) so far: Forensic reviews find the unpatched relays exploited in the August sweep remain remotely accessible in hundreds of municipal utilities and clinics, forming a persistent botnet that a new actor begins renting for follow-on extortion.
- `emergent_care_digitisation_freeze` -- first noted turn 4, listed in 2 turn(s) so far: Several municipal hospital groups suspend elective digitisation projects and revert to paper triage for months after the ransomware sweep, triggering a coordinated hiring freeze for health IT staff across affected regions.
...
SOVEREIGNTY: 20 last turn, no category 4 finish, Gigafactories in flight +0, ai_investment_collapse t5 −2, capability rose 1.0 −0 = 18
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "pending"}}]}
```
