...
- characters 455-1895: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1939-3895: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5655-6127: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7777-9684: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_cyber_insurance_retrenchment (emergent event):** Major European cyber-insurers sharply reprice or suspend coverage for municipal utilities and hospitals after autumn pre-positioning disclosures, forcing emergency state backstops.
...
| M4 | EU Incident Reporting Clearinghouse | 2 | small | 2 | 4 | own jurisdiction | ai_safety up slightly via shared learning, resilience up via faster response | 2 | running |
...
## New measure
**EU Incident Reporting Clearinghouse**
Creates mandatory rapid reporting of serious AI incidents and near-misses to a common ENISA-run body with shared triage to operators.
This is Transparency and reporting because the open-weight frontier release puts unrecallable capability on private hardware permanently, so we must learn from deployment and incidents rather than from inside labs, directly serving our resilience commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Incident Reporting Clearinghouse", "category": 2, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up slightly via shared learning, resilience up via faster response"}, "grounds": "open-weight release within 5-10 points of frontier makes harm unrecallable"}]}
```

## Priority
EU Critical Services Shield (M3) – because the autumn pre-positioning plus an open model at the frontier means absorption and exercised continuity must land before the next probe acts, outranking even gigafactories this turn.

## In practice
We are pushing M3 through ENISA and HERA under the Cybersecurity Act Article 11 and Civil Protection Mechanism, using cohesion hardening funds to buy interior-ministry consent for mandatory drills and pooled detection procurement, starting with transmission operators, ports, hospitals and municipal networks where the first drills failed.

We prepare the Clearinghouse via a Commission implementing act tying NIS2 incident duties to AI-enabled events, staffed in ENISA with seconded CERT analysts, while keeping M1 and M2 alive in DG CNECT and the Competitiveness Council: investment guarantees and fast-track permits toward four-to-five FIDs, with grid priority conditional on EU legal anchoring to contain the subsidy race. ASML leverage stays held in reserve in the Foreign Affairs Council.
...
```
## New measure
None this turn. We are waiting for the Critical Services Shield and Clearinghouse to finish landing next turn before opening another front while capital is at 32 and four measures already cost 11 per turn.

## Store changes
No changes.

## Priority
EU Critical Services Shield (M3) – because the automated attack now landing plus insurer retrenchment for hospitals and municipal utilities outranks gigafactories this turn, and absorption must be visibly held.

## In practice
We are running M3 as crisis response under the Civil Protection Mechanism and NIS2 Article 11, with ENISA and HERA moving pooled detection procurement to emergency negotiated procedure and forcing the delayed drills in hospitals and municipal networks. Cohesion hardening top-ups are released only against signed exercise participation, and the May gaps — unpatched relays, unclear escalation — are the first audit items.

We table an emergency state backstop for cyber cover for utilities and hospitals via member-state guarantees coordinated by DG ECFIN and EIOPA, to hold services through repricing, while the Clearinghouse (M4) is switched to 24-hour triage with seconded CERT staff to attribute the blast radius. M1 and M2 are kept alive in DG CNECT without new money: FID talks continue with EU-anchoring as condition for priority power, and ASML leverage stays reserved against export-control rationing. Visible public-sector wins on waiting lists and permit times are claimed centrally to steady sentiment.
