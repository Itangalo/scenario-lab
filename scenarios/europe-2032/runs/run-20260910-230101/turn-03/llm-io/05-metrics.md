...
- characters 106-13033: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13076-13268: `{{metrics_json}}` from the run's live metric values
- characters 13361-14801: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14845-16801: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16857-17703: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_cyber_insurance_repricing` -- first noted turn 2, listed in 2 turn(s) so far: A major European insurer reprices cyber cover after the autumn pre-positioning audits, abruptly excluding state-backed intrusion losses and forcing grid and port operators to disclose exposure.
