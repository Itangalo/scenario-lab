...
- characters 105-13032: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13075-13267: `{{metrics_json}}` from the run's live metric values
- characters 13360-14497: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14540-16569: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16625-17658: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Incident Reporting Clearinghouse | 2 |
...
- `emergent_cyber_insurance_retrenchment` -- first noted turn 2, listed in 1 turn(s) so far: Major European cyber-insurers sharply reprice or suspend coverage for municipal utilities and hospitals after autumn pre-positioning disclosures, forcing emergency state backstops.
- `emergent_cyber_insurance_repricing` -- first noted turn 2, listed in 1 turn(s) so far: A major European insurer reprices cyber cover after the autumn pre-positioning audits, abruptly excluding state-backed intrusion losses and forcing grid and port operators to disclose exposure.
