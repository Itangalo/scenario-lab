...
- characters 453-1446: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1489-3807: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5563-6042: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9138-11807: `{{previous_actions}}` from the actor's own response from the previous turn
...
### Cures arrive on someone else's machines
The autumn brought the first tailored therapies for previously untreatable conditions into ordinary clinics. Waiting lists filled within days in France, Spain and Poland. DG SANTE insisted procurement run only on certified, European-accessible systems, and the filmed triage assistants finally scaled beyond pilots. For a moment the public story shifted from frozen graduate hiring to lives extended.

The relief curdled quickly. Doses, scheduling software and the top-end models behind the treatment plans were licensed abroad, with allocation queues set in California and compute billed by the hour. Commentators called it healing by permission. Mayors who had kept bans on logistics machines now asked why Brussels could guarantee a therapy appointment but not a job interview.

### One capital walks, many haggle
The defection broke in October: a member state signed its own compute and robot-supply pact with a foreign hyperscaler, with discounted capacity and early humanoid deliveries. At home it was sold as pragmatism; in Brussels as a breach of the common line. Finance ministers openly quarrelled over who would pay for the Guarantee if capitals could freelance.

The Commission's answer was the new supply and evaluation club with other middle powers — pooled bargaining for frontier access, aligned export-licence criteria, mutual recognition of tests. The Council and EEAS moved fast, offering the wayward capital re-entry on better pooled terms. Talks were cordial, communiqués bold, but lawyers noted nothing yet compelled a lab to open a checkpoint or a vendor to re-route chips. Grid priority for the two permitted gigafactory plots stayed stuck in energy councils.

On the ground, Chinese-built humanoids moved to full commercial shifts in ports and warehouses. Blockades in Germany and Italy hardened; two fenced pilot sites suffered night fires, reported as sabotage on single sources. The humanoid register existed but without staff to enforce it, and certification lagged. Works councils co-signed a few reopenings under the Guarantee, but take-up of bridge pay stayed slow.

Frontier labs released again at a pace no roadmap had announced, with training loops running largely unattended. Brussels evaluators kept reading release notes after the fact.
...
**emergent_infrastructure_sabotage_wave (emergent event):** Coordinated night-time arson and blockade campaign spreads from pilot robot sites to gigafactory grid-connection works, halting construction and forcing emergency energy council intervention.
...
- `two_year_commitment` (commitment): Hold cohesion through shared supply leverage and visible public benefit while containing superhuman harm
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Middle-Power Supply and Evaluation Coalition | 8 | small | 9 | 11 | coalition | eu_ai_sovereignty up modestly, eu_political_capital up modestly, ai_safety up modestly | 2 | running |
...
| M4 | EU Essential Services Continuity Stack | 4 | small | 3 | 4 | own jurisdiction | eu_ai_sovereignty up, resilience up modestly | 2 | finished |
| M5 | EU Bio-Shield and Blockade Continuity Regime | 6 | large | 4 | 6 | own jurisdiction | resilience up substantially, ai_safety up modestly via incident response | 3 | finished |
| M6 | EU Municipal Trust and Transition Pact | 7 | small | 5 | 7 | own jurisdiction | public_sentiment up, resilience up modestly, eu_political_capital up modestly | 2 | finished |
| M7 | EU-US Structured Frontier Access and Joint Evaluation Pact | 8 | small | 6 | 8 | US with EU jurisdiction effect | eu_ai_sovereignty up modestly, ai_safety up modestly, eu_political_capital up modestly | 2 | finished |
| M8 | EU Humanoid Deployment Safety Accord | 3 | small | 7 | 8 | own jurisdiction | public_sentiment up, resilience up modestly, eu_political_capital up modestly | 2 | finished |
| M9 | EU Graduate and Logistics Transition Guarantee | 7 | small | 8 | 10 | own jurisdiction | public_sentiment up, resilience up modestly, eu_political_capital up modestly | 2 | finished |
...
## Two-year commitment
Hold cohesion through shared supply leverage and visible public benefit while containing superhuman harm

## Statement changes
modify `two_year_commitment` (commitment): Hold cohesion through shared supply leverage and visible public benefit while containing superhuman harm
- Trigger: the two-year commitment period closes this turn

## New measure
**EU Middle-Power Supply and Evaluation Coalition**
It joins the EU with other middle powers holding compute, chips, energy and evaluation capacity in aligned export-licence, joint compute-bargaining and shared safety-evaluation arrangements, as International coordination and leverage rather than a new EU compute build.
This and now because with sovereignty at 19, capital at 11 and a member state defecting to a bilateral hyperscaler deal, the Union cannot buy autonomy alone, and the open middle-power window is the only leverage that secures access, enforces filters and funds resilience without another stalled gigafactory promise.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Middle-Power Supply and Evaluation Coalition", "category": 8, "size": "small", "finish_turn": 11, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up modestly, eu_political_capital up modestly, ai_safety up modestly"}, "grounds": "defection and superhuman release pace demand joint supply leverage now"}]}
```

## Priority
EU Middle-Power Supply and Evaluation Coalition — because without joint leverage on compute access and evaluation the Guarantee cannot hold cohesion and medical benefits arrive only as dependence.

## In practice
We run the Coalition through the Council and EEAS with DG TRADE and DG CONNECT, tabling aligned export-licence criteria and a joint bargaining pool for frontier compute access, plus mutual recognition of evaluations to stop reading release notes after the fact. We offer the defecting capital re-entry on better access terms than its bilateral deal, funded by the pool, and we ask finance and energy ministers to link grid priority for the two permitted sites to coalition chip and power commitments.

On existing books we keep DG EMPL pushing the Graduate and Logistics Transition Guarantee through ESF+ top-ups, with works councils co-signing reopenings in Germany and Italy in exchange for bridge pay, and DG SANTE procuring the breakthrough therapies and triage assistants only on EU-accessible, certified models so the medical breakthrough lifts sentiment instead of demonstrating dependence. We enforce the humanoid register through mayors with coalition evaluation support, not new staff we do not have.
...
## New measure
**EU Wartime Energy-Continuity and Infrastructure Protection Regime**
It puts gigafactory grid works, cable landings, clinics and essential services under emergency protection and degraded-mode operation during US-China war, as Preparedness and resilience with military-target hardening.
This and now because war makes European fabs, cables and data centres targets and sabotage has already halted grid works — without protected continuity the therapies, Guarantee and coalition access collapse together.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Wartime Energy-Continuity and Infrastructure Protection Regime", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, eu_political_capital down modestly"}, "grounds": "war targeting plus sabotage halt demands emergency protection now"}]}
```

## Priority
EU Wartime Energy-Continuity and Infrastructure Protection Regime — because under great-power shooting war and coordinated arson, keeping lights, clinics and networks degrading-not-stopping outranks supply bargaining and cohesion promises.

## In practice
We trigger the energy council emergency procedure and Article 222 solidarity logic via the European Council: military-police protection for the two gigafactory grid-connection sites, cable landings and therapy-procurement data centres, with ENTSO-E and EU-CyCLONe running degraded-mode playbooks already exercised under the Shield. DG ENER reallocates connection priority by emergency implementing act; defence and interior ministers co-sign site security to stop the night-fire spread.

On existing books we freeze Coalition work to pooled wartime access — no new compulsion on labs, just keeping California-licensed dose queues and triage models flowing — and we direct the finished Guarantee funds to bridge pay for port/warehouse displaced workers where Chinese humanoids now run full shifts, asking works councils and mayors to trade reopening for protection rather than bans. We do not promise autonomy this turn; we promise continuity.
