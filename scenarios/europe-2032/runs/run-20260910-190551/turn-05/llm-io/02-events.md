...
- characters 377-1948: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1992-3991: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4047-5213: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5276-30180: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30535-30914: `{{event_history}}` from the run's own event record
...
- `emergent_clinical_continuity_pact` -- first noted turn 4, listed in 1 turn(s) so far: Hospitals and clinics hit by the winter cutoff publish a joint open letter demanding EU-guaranteed clinical AI continuity, triggering coordinated procurement of a European-hosted medical model pool.
- `emergent_siting_backlash_blockade` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated physical protests block construction access at two planned EU data-centre and grid-upgrade sites, forcing multi-week delays and a national permitting review.
...
- Turn 1 (4 turn(s) ago): cyber_test_shot, bio_uplift_findings, eval_anomaly_reports
- Turn 2 (3 turn(s) ago): capability_jump, knowledge_work_augmented, adoption_success
- Turn 3 (2 turn(s) ago): loss_of_control_incident, openweight_frontier_release, knowledge_work_augmented, eu_frontier_access_denied, member_state_defection
