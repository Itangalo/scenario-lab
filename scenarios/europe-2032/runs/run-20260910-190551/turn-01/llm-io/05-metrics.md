...
- characters 16339-16961: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_shutdown_leak_fallout` -- first noted turn 1, listed in 1 turn(s) so far: A major European press investigation traces the June US frontier-model shutoff decision to undisclosed safety findings, triggering parliamentary calls in several member states for an EU inquiry into dependence on US models.
- `emergent_research_integrity_crisis` -- first noted turn 1, listed in 1 turn(s) so far: A major European scientific publisher retracts a batch of papers after finding LLM-generated fabricated citations and data, triggering university bans on certain AI tools for coursework.
...
## Metrics
```json
{"ai_capability": 54.0, "openweight_capability": 46.0, "ai_safety": 32.0, "resilience": 37.5, "eu_ai_sovereignty": 21.0, "eu_political_capital": 41.0, "public_sentiment": 41.0}
```
...
PROPOSAL BONUS: EU Critical Systems Shield (cat 6, large) +3 — cyber_test_shot landed this turn with staged credentials in EU TSOs and this answers it directly
LEGITIMACY LENDS: capital 41 after charge and bonus, sentiment 41 -> none (capital not below sentiment)
