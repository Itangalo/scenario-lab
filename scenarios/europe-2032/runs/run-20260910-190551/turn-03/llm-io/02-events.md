...
- characters 377-1291: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1335-3275: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3331-4449: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4512-28739: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29094-29264: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Shield −3, EU Frontier Capability Watch −2, priority −1 = −12
PROPOSAL BONUS: none — Watch answers capability jump but knowledge_work_augmented removes displacement crisis and blocks rule 6 bonus for spending against unfelt problem
LEGITIMACY LENDS: capital 31, sentiment 48 -> +2
SOVEREIGNTY: 21 last turn, no capacity event, Gigafactories + Tech package in flight +0, capability rose 1.5 −0 = 21

## Emerging developments (tracked)

- `emergent_shutdown_leak_fallout` -- first noted turn 1, listed in 2 turn(s) so far: A major European press investigation traces the June US frontier-model shutoff decision to undisclosed safety findings, triggering parliamentary calls in several member states for an EU inquiry into dependence on US models.
- `emergent_research_integrity_crisis` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher retracts a batch of papers after finding LLM-generated fabricated citations and data, triggering university bans on certain AI tools for coursework.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, bio_uplift_findings, eval_anomaly_reports
- Turn 2 (1 turn(s) ago): capability_jump, knowledge_work_augmented, adoption_success
