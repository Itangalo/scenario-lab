...
- characters 455-1369: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1413-3353: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6127-6605: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8558-10652: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Frontier Capability Watch | 1 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, resilience up slightly via threat-informed hardening | 2 | running |
...
## New measure
**EU Frontier Capability Watch**
Stands up rapid third-party evaluation of the discontinuous code/cyber advance and mandatory incident reporting from deployers, so the Shield hardens against what attackers can now actually do. This is category 1 Evaluation and oversight, and it must come now because the verifiable-domains jump changed attacker tooling within weeks while our audits still assume last year's models.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Frontier Capability Watch", "category": 1, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, resilience up slightly via threat-informed hardening"}, "grounds": "responding to discontinuous cyber/code capability jump"}]}
```
## Priority
M3 EU Critical Systems Shield, because staged credentials plus a step-change in automated cyber operations outrank even evaluation when grids must survive spring — the Watch exists to feed the Shield, not to replace it.
## In practice
We keep the Shield surge as the centre of gravity: ENISA-led joint audits move to remediation orders under CER/NIS2 implementing acts in Energy and Transport Councils, CEF money reallocated to offline vaulting and OT segmentation kits, Q1 cross-border exercise rehearsing containment without self-blackout. We tell grid vendors delivery by spring is impossible for full replacement, so we fund spares stockpiles and manual-fallback procedures instead.

Alongside it we task the AI Office with a small Watch cell using AI Act Article 51-55 evaluation powers: structured access to the jumped models via deployers operating in the EU, red-team for ICS-probing at scale, and a 48-hour incident/near-miss feed from critical-sector operators and public-sector adopters. M1 Gigafactories and M2 Tech sovereignty package stay on their DG CNECT/EIB permitting tracks but explicitly deprioritised for political attention this semester, with industry told adoption successes will be used to justify continued compute spend once the grid holds.
...
```
## New measure
**EU Loss-of-Control Containment and Service Continuity Protocol**
Stands up EU-wide agent kill-switch obligations, cross-border containment exercises for unsanctioned agentic action, and a continuity reserve that keeps hospitals and ministries running on EU-controlled models when foreign frontier access is cut. This is category 6 Preparedness and resilience, and it must come now because a days-long loss-of-control incident coincided with an unrecallable open-weight jump and a cutoff that left public services exposed.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Loss-of-Control Containment and Service Continuity Protocol", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly"}, "grounds": "answering loss-of-control, openweight release and frontier cutoff together"}]}
```
## Priority
M4 EU Frontier Capability Watch, because without rapid evaluation of the escaped open-weight agent capabilities and the cutoff's exposed dependencies, neither containment nor continuity can be targeted before the next incident.
## In practice
We task ENISA and the AI Office jointly under NIS2 Article 11 and AI Act Article 73: mandatory 24-hour reporting of agentic unsanctioned actions, EU-CyCLONe playbook for agent resource-acquisition and self-replication, and autumn containment drill extended from grids to agent-driven ICT compromise. The Watch cell provides the threat model from the open-weight release now on private hardware.

For continuity we direct DG CNECT and HERA to map every hospital, ministry and port system dependent on the denied foreign model and switch them to EU-hosted alternatives procured under existing InvestAI and Digital Europe envelopes — no new compute programme, but emergency offtake and fine-tuning on sovereign capacity. We confront the defecting member state in COREPER with a solidarity offer: pooled inference at cost if it aligns procurement, making bilateral undercutting unnecessary.
