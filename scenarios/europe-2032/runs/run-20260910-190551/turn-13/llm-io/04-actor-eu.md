...
- characters 454-1575: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1619-3691: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4043-4520: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7828-8915: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_anti_datacentre_blockades (emergent event):** Coordinated physical protests block construction and power connections at several planned data-centre and logistics-automation sites across multiple member states, delaying builds without any EU decision.
...
| M12 | EU Municipal Ransomware Containment and Verified Recovery Corps | 6 | small | 11 | 13 | own jurisdiction | resilience up moderately, eu_political_capital up slightly | 2 | finished |
...
## New measure
None this turn. We are waiting for the containment corps to finish landing and for verified recoveries to rebuild enough trust and capital to fund anything new without collapse.

## Store changes
No changes.

## Priority
M12, because holding clinics, municipalities and grid operators through verified recovery is the entirety of our two-year commitment to hold essential services while rebuilding trust, and nothing else can outrank it while outages remain public.

## In practice
We keep the work inside NIS2/CER and ENISA/EU-CyCLONe, paying containment pools only on certified clean-restore and funding offline paper-to-cloud fallback kits from reprogrammed cohesion and EU4Health emergency lines. Health and JHA Councils hold the clearing standard; no new mandates on operators this turn.

We use the middle-power compact for joint triage and the capped allied channel for clean images, saying openly that hardware remains rationed. Seconded staff are directed to drill restores where kits already landed, rather than relabelling funds as new capacity we do not have.
...
```
## New measure
**EU Trusted Siting and Local Dividend Pact**
It negotiates a pause-and-benefit deal for contested data-centre and automation sites — local jobs, energy-bill relief and verified safety oversight in exchange for lifting blockades and connecting power. 
This and now because physical blockades are delaying the only sovereign capacity we have left while public sentiment is at 8, and without a trusted siting bargain no compute, resilience or recovery measure can land; a small diffusion/trust instrument (category 9) is the cheapest way to rebuild capital to hold services.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trusted Siting and Local Dividend Pact", "category": 9, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, eu_political_capital up slightly, eu_ai_sovereignty up slightly"}, "grounds": "blockades delaying builds with sentiment collapsed"}]}
```

## Priority
M13 — the Trusted Siting Pact just added — because unblocking builds and rebuilding local consent outranks everything else while outages persist and no other measure is in flight.

## In practice
We run this through Cohesion and Energy Councils with ENISA and national regulators, using reprogrammed cohesion envelopes and grid-connection agreements, not new mandates: mayors get a certified clean-restore and offline-kit guarantee plus a local dividend (training places, power-price rebate) for each site that proceeds.

We ask protesting councils and unions for a monitored construction moratorium — no forced clearing — while independent safety and environmental audits are published per site. Interior and Employment formations hold the line that foreign-run warehouses face the same audit standard, to keep blame from falling outward again and to turn verified recoveries into consent for capacity.
