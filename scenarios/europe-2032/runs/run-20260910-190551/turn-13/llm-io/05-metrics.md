...
- characters 13388-13578: `{{metrics_json}}` from the run's live metric values
- characters 13671-14792: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14836-16908: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16964-17678: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M13 | EU Trusted Siting and Local Dividend Pact | 2 |
...
- `emergent_logistics_robot_stoppage` -- first noted turn 12, listed in 2 turn(s) so far: A coordinated stoppage spreads across foreign-run automated warehouses and ports after a further robotics injury, blocking pallet flows for days and forcing emergency manual handling.
...
## Notepad
PORTFOLIO CHARGE t13: EU Trusted Siting and Local Dividend Pact −2, priority −1 not charged (capital below 20, no effect) = −2
PROPOSAL BONUS: EU Trusted Siting Pact (cat 9, small) +2 — emergent_anti_datacentre_blockades this turn directly answered, small size limits gain
LEGITIMACY LENDS: none — capital 15, sentiment 8, capital higher
SOVEREIGNTY: 16 last turn, no cat4 finish, no cat4 in flight +0, emergent_anti_datacentre_blockades t13 −2, capability rose 1.0 no penalty = 14
