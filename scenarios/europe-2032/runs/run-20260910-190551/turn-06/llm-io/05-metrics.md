...
- characters 105-13808: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13851-14043: `{{metrics_json}}` from the run's live metric values
- characters 14136-16079: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 16122-19700: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 19756-20391: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | Middle-Power Evaluation and Supply-Chain Compact | 2 |
| M8 | EU Clinical and Municipal Cyber-Recovery Pact | 2 |
...
**emergent_clinical_continuity_pact (emergent event):** Hospitals and clinics hit by the winter cutoff publish a joint open letter demanding EU-guaranteed clinical AI continuity, triggering coordinated procurement of a European-hosted medical model pool.
...
## Metrics
```json
{"ai_capability": 62.5, "openweight_capability": 58.0, "ai_safety": 19.0, "resilience": 42.0, "eu_ai_sovereignty": 19.0, "eu_political_capital": 14.0, "public_sentiment": 24.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn-5 winner took office turn 6 on tiered foreign access platform"}]}
```
```
