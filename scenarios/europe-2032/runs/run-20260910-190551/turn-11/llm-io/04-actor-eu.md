...
- characters 456-1707: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1751-3799: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7400-7877: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10994-12942: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Hold essential services through AI-enabled shocks while rebuilding public trust for sovereign capacity
...
| M4 | EU Frontier Capability Watch | 1 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, resilience up slightly via threat-informed hardening | 2 | finished |
| M5 | EU Loss-of-Control Containment and Service Continuity Protocol | 6 | small | 3 | 6 | own jurisdiction | resilience up moderately, ai_safety up slightly | 2 | finished |
| M6 | Allied Volume-Licence and Supply-Chain Bargain | 8 | small | 4 | 6 | US plus middle-power coalition | eu_ai_sovereignty up slightly, eu_political_capital stable | 2 | finished |
| M7 | Middle-Power Evaluation and Supply-Chain Compact | 8 | small | 5 | 8 | middle-power coalition plus US | eu_ai_sovereignty up moderately, eu_political_capital up slightly, ai_safety up slightly | 2 | finished |
| M8 | EU Clinical and Municipal Cyber-Recovery Pact | 6 | small | 6 | 8 | own jurisdiction | resilience up moderately, eu_political_capital up slightly | 2 | finished |
| M9 | EU Grid and Gigafactory Protection Pact | 6 | small | 8 | 9 | own jurisdiction | resilience up moderately, eu_ai_sovereignty up slightly, ai_safety up slightly | 2 | finished |
| M10 | EU Essential Services Continuity and Rapid Patching Drive | 6 | small | 9 | 10 | own jurisdiction | resilience up substantially, ai_safety up slightly | 2 | finished |
| M11 | EU Offline Fallback and Unrecallable-Capability Shield | 6 | small | 10 | 11 | own jurisdiction | resilience up moderately, ai_safety up slightly, eu_political_capital up slightly | 2 | finished |
...
## New measure
**EU Offline Fallback and Unrecallable-Capability Shield**
It puts hospitals, municipalities and grid operators onto offline paper-to-cloud fallback kits and ENISA-pushed guardrails tuned to the newly released open-weight frontier capability, because that capability is now permanent on private hardware. This now because the open-weight jump to near-frontier plus brittle services and hostile sentiment at 19 leaves no trust for new build without proven continuity.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Offline Fallback and Unrecallable-Capability Shield", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly, eu_political_capital up slightly"}, "grounds": "open-weight frontier release makes recall impossible while services remain brittle"}]}
```
## Priority
EU Offline Fallback and Unrecallable-Capability Shield, because holding essential services through the unrecallable open-weight shock outranks any sovereignty push while capital is at 16.
## In practice
We act under NIS2 Article 11 and CER emergency support, with ENISA and EU-CyCLONe pushing the swarm-detection rules and certified-behaviour clearing to all municipal IT and hospital admin hit from Lyon to Łódź, funded by reprogrammed cohesion on pay-on-verified-recovery contracts. Health and JHA Councils approve mutual-aid crews and guarded repair corridors as standing pools, not ad-hoc.

DG CNECT and JRC ship offline fallback kits and model-abuse filters tuned to the open-weight release, with a precautionary inspection hold on foreign humanoid fleets to buy union consent. On export controls we take the allied volume-licence line where Washington still offers it, keeping Japan-Korea-Netherlands mapping technical-only until hardware deliveries resume, and we say plainly one link stays court-blocked.
...
```
## New measure
**EU Municipal Ransomware Containment and Verified Recovery Corps**
It stands up standing EU-funded containment teams and verified-recovery clearing for municipalities, hospitals and grid operators hit by the automated ransomware sweep, cutting dwell time and public outage. This now because the major automated incident landed on brittle services with safety at 19 and capital at 15, so holding essential services through verified recovery outranks any new sovereignty build.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Ransomware Containment and Verified Recovery Corps", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, eu_political_capital up slightly"}, "grounds": "automated ransomware sweep requires standing containment and recovery"}]}
```
## Priority
EU Municipal Ransomware Containment and Verified Recovery Corps, because containing the visible outage and restoring services is what the two-year commitment to hold essential services demands before any other push.
## In practice
We act under NIS2 and CER via ENISA and EU-CyCLONe, reprogramming cohesion and EU4Health emergency lines to fund mutual-aid containment pools on pay-on-verified-recovery contracts. Health and JHA Councils mandate certified-behaviour clearing for hospital and municipal restores, with offline paper-to-cloud fallback where networks stay suspect.

We retune existing ENISA filters and swarm-detection to the loose near-frontier model and the capability jump in cyber tooling, without new mandates on operators. We take the middle-power evaluation and supply-chain compact for joint triage intel and the capped allied licence for clean images, and say plainly hardware remains rationed while patrols continue on the blocked grid link.
