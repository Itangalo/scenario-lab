...
- characters 377-1857: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1901-4235: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4291-5429: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5492-29719: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30074-30234: `{{event_history}}` from the run's own event record
...
- `emergent_gigafactory_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: Coordinated permitting lawsuits and local protests stall two gigafactory shortlist sites over power prices and water use, forcing renegotiation of grid connections.
- `emergent_grid_security_standard` -- first noted turn 2, listed in 1 turn(s) so far: European grid operators and vendors quietly circulate a joint procurement standard for AI-assisted intrusion detection in transmission networks after the autumn mapping incident, creating a de facto market for defensive tooling before EU legislation lands.
...
- Turn 2 (1 turn(s) ago): eval_anomaly_reports, knowledge_work_augmented, adoption_success, automated_decision_scandal
