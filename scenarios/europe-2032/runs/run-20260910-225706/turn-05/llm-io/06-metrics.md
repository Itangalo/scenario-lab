...
- characters 13827-15236: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15280-17769: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17825-18668: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Chip-to-Compute Security Pact | 2 |
...
- `emergent_anti_datacentre_blockade` -- first noted turn 5, listed in 1 turn(s) so far: Coordinated physical protests and blockades halt construction at two remaining EU gigafactory / data-centre sites after the hospital ransomware sweep, framed as opposition to AI infrastructure costs and risks.
...
## Metrics
```json
{"ai_capability": 61.0, "openweight_capability": 57.8, "ai_safety": 31.0, "resilience": 44.0, "eu_ai_sovereignty": 16.0, "eu_political_capital": 7.0, "public_sentiment": 22.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "pending"}}, {"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 9}, "grounds": "event: taiwan blockade plus grid freeze stalls build"}]}
```
```
