...
- characters 106-13229: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13272-13464: `{{metrics_json}}` from the run's live metric values
- characters 13557-14863: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14907-16582: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16638-17696: `{{notepad}}` from the Game Master's notepad, carried across turns
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU AI Evaluation and Safety Institute −3, EU Critical Infrastructure Cyber Shield −3, priority −1 = −13
PROPOSAL BONUS: none — Cyber Shield answers autumn grid intrusion narrative but no qualifying event id in last three turns
LEGITIMACY LENDS: capital 21, sentiment 42 -> +2
SOVEREIGNTY: 21 last turn, no capacity event, Gigafactories in flight +0, capability rose 3.0 −1 = 20

## Emerging developments (tracked)

- `emergent_hyperscaler_pause` -- first noted turn 2, listed in 2 turn(s) so far: A major US hyperscaler publicly pauses two planned EU data-centre expansions, citing grid-connection delays and power prices, freezing private co-financing talks for the permitting zones.
- `emergent_open_model_backlash` -- first noted turn 2, listed in 2 turn(s) so far: Independent forensic reports publicly attribute the autumn grid intrusions to tooling derived from the newest open model family, triggering mainstream press calls for open-weight liability and release restraints.
...
Provides wage insurance, rapid retraining vouchers and employer-linked transition levies for occupations facing AI displacement, administered via ESF+ and national public employment services. This is category 7 Labour and social protection, and why now is because the augmentation narrative has collapsed into displacement just as a major automated attack and an investment bust hit public services, so resilience without income absorption will fail politically.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Displaced Workers Transition Fund", "category": 7, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up via social absorption, public_sentiment up modestly"}, "grounds": "to absorb AI-driven job displacement as adjustment not shock"}]}
```

## Priority
M4 EU Critical Infrastructure Cyber Shield, because the large automated attack landing on still-exposed grids, ports and public services outranks build-out this turn and our two-year commitment to resilient capacity fails if essential services cascade.

## In practice
We drive M4 through ENISA, ACER and the NIS2 Cooperation Group with emergency reallocation from the Connecting Europe Facility and Digital Europe, forcing 24h reporting and isolation exercises on lagging ports and water utilities while ring-fencing the two transmission operators that met milestones as mutual-aid hubs. The safety breakthrough from interpretability is pushed via M3 into the Evaluation Institute's protocols to certify deployed-system monitoring, giving us a quick assurance win to justify market-access linkage.

We keep M1 InvestAI Gigafactories and M2 permitting zones alive in DG COMP state-aid and EIB project preparation without new cash this turn, explicitly pausing private co-financing expectations after the investment collapse and pivoting site work to grid-connection and power-price guarantees. In parallel we launch the small Transition Fund via EPSCO and DG EMPL, using ESF+ front-loading rather than fresh fiscal headroom we do not have, to show displaced workers reach support within months.
...
## Metrics
```json
{"ai_capability": 61.0, "openweight_capability": 54.0, "ai_safety": 35.0, "resilience": 36.0, "eu_ai_sovereignty": 17.0, "eu_political_capital": 5.0, "public_sentiment": 31.0}
```
...
In the labour market, the hopeful studies about augmentation were overtaken. Clerical, support and junior technical roles thinned visibly, and a new wage-insurance and retraining fund was launched through employment services with front-loaded social funds rather than fresh cash. Offices opened, vouchers were promised, but payouts were months away.

One bright spot came from research: an interpretability result that made deployed-system behaviour more predictable was quickly taken up by labs and folded into the new evaluation institute's protocols, giving Brussels a rare assurance win to point to.

By December, hardening was real, capacity was further away, and patience was thin.

## Notepad
PORTFOLIO CHARGE t3: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M3 EU AI Evaluation and Safety Institute −3, M4 EU Critical Infrastructure Cyber Shield −3, M5 EU Displaced Workers Transition Fund −2, priority −1 = −15
PROPOSAL BONUS: Transition Fund (cat 7, small) +1 — answers emerging displacement as augmentation narrative collapses, small and indirect against cyber and investment shock
LEGITIMACY LENDS: capital 3, sentiment 31 -> +2
SOVEREIGNTY: 20 last turn, no cat4 finish, in flight +0, ai_investment_collapse t3 −2, capability rose 3.0 −1 = 17

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M4", "fields": {"finish_turn": 5}, "grounds": "named priority"}]}
```
```
