...
- characters 377-1683: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1727-3402: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3458-4516: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4579-28155: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28510-28551: `{{event_history}}` from the run's own event record
...
- `emergent_hyperscaler_pause` -- first noted turn 2, listed in 1 turn(s) so far: A major US hyperscaler publicly pauses two planned EU data-centre expansions, citing grid-connection delays and power prices, freezing private co-financing talks for the permitting zones.
- `emergent_open_model_backlash` -- first noted turn 2, listed in 1 turn(s) so far: Independent forensic reports publicly attribute the autumn grid intrusions to tooling derived from the newest open model family, triggering mainstream press calls for open-weight liability and release restraints.
