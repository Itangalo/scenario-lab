...
- characters 377-1913: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1957-4153: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4209-5332: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5395-28971: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29326-29485: `{{event_history}}` from the run's own event record
...
- `emergent_ot_insurance_pullback` -- first noted turn 2, listed in 1 turn(s) so far: Major reinsurers temporarily exclude OT cyber-triggered business interruption for ports and grid operators after the autumn pre-positioning revelations, raising premiums and forcing temporary throughput limits.
- `emergent_foundry_supply_squeeze` -- first noted turn 2, listed in 1 turn(s) so far: A prolonged drought in Taiwan and power rationing notices force a major foundry to guide down output for two quarters, tightening global GPU and power-electronics supply and delaying European data-centre build-outs.
...
- Turn 2 (1 turn(s) ago): cyber_major_incident, eval_anomaly_reports, safety_breakthrough, automated_decision_scandal
