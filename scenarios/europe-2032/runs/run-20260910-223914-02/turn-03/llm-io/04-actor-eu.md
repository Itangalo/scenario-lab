...
- characters 455-1991: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2035-4231: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5205-5677: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7323-9396: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_ot_insurance_pullback (emergent event):** Major reinsurers temporarily exclude OT cyber-triggered business interruption for ports and grid operators after the autumn pre-positioning revelations, raising premiums and forcing temporary throughput limits.
**emergent_foundry_supply_squeeze (emergent event):** A prolonged drought in Taiwan and power rationing notices force a major foundry to guide down output for two quarters, tightening global GPU and power-electronics supply and delaying European data-centre build-outs.
...
| M3 | EU Critical Services Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up substantially, ai_safety indirectly via incident response | 3 | running |
| M4 | EU Evaluation and Incident Clearinghouse | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, resilience up slightly via faster learning | 2 | running |
...
## New measure
**EU Evaluation and Incident Clearinghouse**
Stands up joint EU evaluator access to frontier models plus mandatory serious-incident reporting to ENISA/EU AI Office, turning the leaked eval anomaly and the major cyber attack into shared assurance.
This is category 5 Public technical capacity and research with a category 2 reporting arm, because the safety breakthrough gives us something deployable to certify while trust in both labs and the AI Act is failing — own technical eyes now, not more paper rules.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Evaluation and Incident Clearinghouse", "category": 5, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, resilience up slightly via faster learning"}, "grounds": "eval anomaly plus major model-generated attack require EU eyes and reporting"}]}
```

## Priority
EU Critical Services Shield (M3) – because the automated attack landing on hospitals, grids and public services outranks build timelines, and the Shield is the only instrument that absorbs harm this turn.

## In practice
We are surging M3 through ENISA and HERA: EU-funded detection stacks and exercised backup power and manual-fallback plans for the hit operators, with the Civil Protection Mechanism covering costs for municipalities that accept mandatory drills. Interior and health ministers get money, not lectures, in exchange for joint procurement and cross-border mutual aid this semester.

On M1 and M2 we hold the line via DG CNECT and ECOFIN: power reservations and EIB guarantees for the two advanced gigafactory sites, and a standstill on capital bidding wars tied to EU legal anchoring, while the others clear grid and siting. In parallel we task the AI Office with the Clearinghouse — vetted researcher access deals and pre-release eval MOUs with labs under the AI Act's systemic-risk base, plus a single incident-reporting portal — using the new interpretability result as the first certifiable check to deploy.
...
```
## New measure
**EU Displaced-Worker and Claimant Safety Net**
Creates an EU-topped-up wage insurance, rapid retraining voucher and independent audit-and-appeal standard for automated benefit and hiring systems, paid from ESF+ to blunt the welfare-scandal fallout and early displacement.
This is category 7 Labour and social protection with a category 9 trust arm, because with sentiment at 30 and capital at 27 we cannot hold the sovereignty-and-resilience commitment without stopping the domestic bleed — lawful-but-harmful automation is now the binding constraint on everything else.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Displaced-Worker and Claimant Safety Net", "category": 7, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately via social absorption, eu_political_capital up slightly via restored trust"}, "grounds": "welfare scandal plus openweight-driven automation pressure collapsing public tolerance"}]}
```

## Priority
EU Critical Services Shield (M3) – because reinsurers excluding OT interruption and ports/grids throttling throughput after a real hospital-and-logistics hit outranks even the welfare repair this turn, and only exercised fallback keeps services running.

## In practice
We surge M3 through the Civil Protection Mechanism and ENISA as an OT backstop: EU bridge guarantees where reinsurers pulled back conditional on mandatory manual-fallback drills, plus HERA/ENISA detection kits to the hit municipalities and port operators, with health and transport ministers co-signing throughput safeguards. DG CNECT holds M1/M2 to the two viable gigafactory sites with EIB guarantees and power reservations, pausing bidding wars while foundry supply is squeezed and telling other sites plainly that grid comes before concrete.

In parallel we launch the Safety Net via EPSCO and ESF+: member states get top-up funds for wage insurance and 3-month retraining placement only if automated welfare/employment systems submit to the Clearinghouse (M4) audit log and human-appeal standard, using the court ruling as legal base. The Clearinghouse itself pushes the interpretability check as interim certification and logs the new open-weight release for municipal SOCs — we cannot recall it, so we distribute detection instead.
