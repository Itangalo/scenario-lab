...
- characters 106-13651: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13694-13886: `{{metrics_json}}` from the run's live metric values
- characters 13979-15181: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15225-17356: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17412-18607: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Accession to Joint Cyber-Bio Threat Response | 2 |
...
|---|---|---|---|---|---|---|---|---|---|
| M4 | EU Evaluation and Incident Clearinghouse | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, resilience up slightly via faster learning | 2 | finished |
...
- `emergent_siting_blockade` -- first noted turn 4, listed in 2 turn(s) so far: Coordinated local blockades halt construction and grid connection work at one of the two secured gigafactory sites for weeks amid drought rationing and data-centre opposition.
- `emergent_siting_freeze_wave` -- first noted turn 4, listed in 2 turn(s) so far: Coordinated municipal moratoriums on new data-centre grid connections spread across drought-hit regions, with councils citing power rationing and water use, freezing two further AI factory candidate sites for the turn.
...
## Metrics
```json
{"ai_capability": 70.0, "openweight_capability": 62.5, "ai_safety": 24.0, "resilience": 40.0, "eu_ai_sovereignty": 17.0, "eu_political_capital": 12.0, "public_sentiment": 22.0}
```
...
## Notepad
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Displaced-Worker and Claimant Safety Net −2, EU Bio Detection and Medical Countermeasure Surge −2, EU Accession to Joint Cyber-Bio Threat Response −2, priority −1 = −13
PROPOSAL BONUS: EU Accession to Joint Cyber-Bio Threat Response (cat 8, small) +3 — cyber_major_incident landed this turn plus joint invitation, answers directly
LEGITIMACY LENDS: capital 11, sentiment 22 -> +1
