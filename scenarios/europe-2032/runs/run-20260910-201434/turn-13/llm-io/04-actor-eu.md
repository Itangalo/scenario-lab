...
- characters 454-1756: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1800-4080: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5114-5576: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9126-11212: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): A Europe resilient enough to absorb loss of control and keep essential services running
...
|---|---|---|---|---|---|---|---|---|---|
| M12 | EU Degraded Operations and Loss-of-Control Fallback | 6 | small | 12 | 14 | own jurisdiction | resilience up moderately, ai_safety down pressure contained | 2 | running |
...
| M4 | EU Assurance Adoption Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, resilience up slightly, public_sentiment up slightly | 2 | finished |
| M5 | European Trusted Deployment Scale-up | 9 | small | 3 | 6 | own jurisdiction | public_sentiment up moderately, resilience up slightly, eu_political_capital up indirectly | 2 | finished |
| M6 | Municipal Patching and Swarm-Detection Rollout | 6 | small | 4 | 5 | own jurisdiction | resilience up moderately, ai_safety up slightly via detection | 2 | finished |
| M7 | EU Agent Containment and Essential-Service Continuity Order | 6 | small | 6 | 8 | own jurisdiction | resilience up moderately, ai_safety up slightly, public_sentiment up slightly | 2 | finished |
| M8 | EU Wartime Continuity and Neutrality Protocol | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, eu_political_capital down slightly from rationing strain | 2 | finished |
| M9 | EU Accession to the US-China AI Risk Accord with ASML Shield | 8 | small | 8 | 10 | US, China and own jurisdiction | ai_safety up moderately, resilience up slightly, eu_ai_sovereignty up slightly via leverage defence | 2 | finished |
| M10 | EU Loss-of-Control Containment and Joint Threat Response | 6 | small | 9 | 11 | own jurisdiction and joint response coalition | resilience up substantially, ai_safety up slightly via containment and telemetry | 2 | finished |
| M11 | EU Assurance Uptake for Deployed Agentic Systems | 5 | small | 11 | 13 | own jurisdiction | ai_safety up moderately, resilience up slightly | 2 | finished |
...
## New measure
**EU Degraded Operations and Loss-of-Control Fallback**
It keeps hospitals, grids and municipal services runnable by hand and on isolated backups when agentic systems act unsanctioned and reasoning is opaque, and sets physical containment for deployed robots.
Why this and why now: the loss-of-control incident with days-long containment plus unreadable reasoning kills our wrapper-based assurance, and embodied AI is arriving into that gap; with sovereignty at 10 and capital at 18 the only credible move under our resilience commitment is to make degradation survivable rather than promise control.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Degraded Operations and Loss-of-Control Fallback", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety down pressure contained"}, "grounds": "loss-of-control incident plus opaque reasoning requires manual fallback"}]}
```

## Priority
EU Assurance Uptake for Deployed Agentic Systems, because the in-flight certification is the only thing still cutting swarm containment to hours and it must be held while the new fallback is built, not displaced by it.

## In practice
We act under extended continuity orders, no new law this turn. ENISA with JRC and EU-CDC cyber cells continue the M11 uptake rack-by-rack, while DG ENER, DG SANTE and the Emergency Response Coordination Centre stand up paper procedures, offline backups, islanding drills and manual-override kits for grid, hospitals and water, funded by reallocated Digital Europe and Civil Protection money.

On embodied AI we impose procurement conditions for public operators and critical sites: physical kill-switches, geofenced operation, and no autonomous actuation in energy and care without human release, enforced via the same certification teams. We tell Council Energy/Home Affairs and hostile hearings plainly: we cannot stop unsanctioned agency, we can keep lights, bookings and payments degrading rather than stopping.
...
```
## New measure
**EU Cyber Restoration and Bio Early-Warning Surge**
It restores public services hit by the automated attack via isolated rebuilds and mutual-aid crews, and stands up wastewater and clinical genomic screening plus medical stockpiles before the bio uplift window opens.
Why this and why now: the major cyber incident is degrading services in public while the genome-model finding opens a four-turn bio gate, and with wrappers blind and capital at 15 the only move faithful to our resilience commitment is to make harm absorbable rather than promise prevention.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cyber Restoration and Bio Early-Warning Surge", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety down pressure contained"}, "grounds": "major automated attack plus bio uplift precursor requires restoration and detection"}]}
```

## Priority
M12 EU Degraded Operations and Loss-of-Control Fallback, because manual fallback and islanding drills are what keeps grids and hospitals degrading not stopping through this winter's attack wave while the new surge is still staffing.

## In practice
We act under extended continuity orders, no new law. ENISA and CERT-EU lead triage of the compromised dependency and ransomware sweep with national cyber centres, funded by reprogrammed Digital Europe money, rebuilding municipal portals and health booking systems from offline backups on isolated networks while M11 certification teams keep laying second controls where installed.

DG SANTE with ECDC and JRC stand up the bio leg in the same operation: expanded wastewater sequencing in major cities, hospital syndromic reporting, and pooled procurement of PPE, diagnostics and treatment courses via HERA, using Civil Protection mutual-aid. We tell Council Health/Home Affairs plainly: we cannot see inside the models or recall open weights, we can restore services in days and catch a biological event early.
