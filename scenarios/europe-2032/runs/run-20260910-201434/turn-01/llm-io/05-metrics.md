...
- characters 16380-17023: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_eu_eval_consortium` -- first noted turn 1, listed in 1 turn(s) so far: Major European research universities and CNRS/Max Planck institutes announce a joint open evaluation consortium publishing independent red-team results on Kimi K3 and Mythos-class models, forcing public debate on open-weight risk.
- `emergent_open_science_compute_call` -- first noted turn 1, listed in 1 turn(s) so far: Major European research universities and open-science institutes publish a joint call for a compute-sharing pool for safety and evaluation research, attracting press coverage and parliamentary questions.
