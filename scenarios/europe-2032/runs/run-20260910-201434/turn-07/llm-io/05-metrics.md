...
- characters 106-13393: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13436-13628: `{{metrics_json}}` from the run's live metric values
- characters 13721-15234: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15278-17626: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17682-18448: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M8 | EU Wartime Continuity and Neutrality Protocol | 2 |
...
- `emergent_distress_compute_buyout` -- first noted turn 7, listed in 1 turn(s) so far: Gulf and Asian sovereign funds step in to finance the two stalled EU compute parks on distressed terms, offering capital for equity and off-take priority.
...
## Metrics
```json
{"ai_capability": 77.5, "openweight_capability": 69.0, "ai_safety": 23.5, "resilience": 41.5, "eu_ai_sovereignty": 16.0, "eu_political_capital": 9.0, "public_sentiment": 23.0}
```
