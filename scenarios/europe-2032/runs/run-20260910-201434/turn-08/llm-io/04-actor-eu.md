...
- characters 453-1860: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1903-3843: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5841-6272: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9002-11212: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Wartime Continuity and Neutrality Protocol | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, eu_political_capital down slightly from rationing strain | 2 | running |
...
## New measure
**EU Wartime Continuity and Neutrality Protocol**
It puts critical energy, cable, satellite and remaining compute assets under wartime protection, rationing and neutral-jurisdiction control, with paper fallbacks and cross-border mutual aid, to keep essential services alive through blockade and great-power strikes.
This and now because blockade plus US-China shooting plus nationalised labs makes sovereignty-by-build impossible this turn; the Union's only agency is to deny targeting pretext, husband what compute and power remains, and prevent cascade collapse while labour and siting revolts spread.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Wartime Continuity and Neutrality Protocol", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, eu_political_capital down slightly from rationing strain"}, "grounds": "blockade and great-power war makes continuity the binding constraint"}]}
```

## Priority
The new Wartime Continuity Protocol, not M7, because conventional strikes on fabs, cables and data centres plus semiconductor cutoff outrank agent-tripwire drills when essential services may stop, not just degrade.

## In practice
We act through the European Council under TEU Articles 42/222 solidarity and TFEU 122 emergency, tasking ENISA, EU INTCEN and the Emergency Response Coordination Centre to map and guard interconnections, subsea landings, LNG/grid nodes and the stalled gigafactory sites as protected civilian infrastructure, declaring EU non-belligerency and refusing basing/targeting use, while DG ENER imposes temporary power-connection moratorium management and rationing to defuse municipal strikes.

We fund it by freezing M1/M2 new spend and reprogramming CEF, Cohesion and EPF logistics to fuel, spares, paper-fallback kits and cross-border repair teams, using M7 kill-switch drills as the wartime isolation procedure, and we offer Washington and Beijing the same terms: no EU supply-chain leverage transferred under fire without unanimous Council release, with export-control alignment paused to neutrality control until shooting stops.
...
```
## New measure
**EU Accession to the US-China AI Risk Accord with ASML Shield**
It seeks formal EU inclusion in the US-China weights-security and escalation agreement as third verifier, and pairs it with an Anti-Coercion shield refusing further ASML servicing cuts without unanimous Council release, while rolling the new automated-patching and swarm-detection breakthrough across critical services.
This and now because the accord is the only thing slowing capability and making safety cooperation verifiable, Washington is spending the Union's one chokepoint without the Union, and the defensive breakthrough gives a cheap resilience win when political capital cannot fund a build.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Accession to the US-China AI Risk Accord with ASML Shield", "category": 8, "size": "small", "finish_turn": 10, "applies_to": "US, China and own jurisdiction", "targeted_effect": "ai_safety up moderately, resilience up slightly, eu_ai_sovereignty up slightly via leverage defence"}, "grounds": "US-China accord window plus ASML coercion plus defensive breakthrough"}]}
```

## Priority
The new Accord Accession measure, not M8, because being written out of the only capability-slowing deal while ASML is conscripted decides whether any sovereignty remains, and continuity rationing alone cannot recover agency.

## In practice
We act through the European Council and FAC-Trade, mandating the HR/VP to demand observer-verifier status in the US-China weights-security and autonomous-escalation implementation, offering ENISA, EU AI Office and JRC evaluation capacity as the neutral checker both sides need. In parallel we open an Anti-Coercion Instrument examination on the extraterritorial ASML servicing ban, freezing further Dutch compliance beyond existing dual-use rules pending Council unanimity, and offering The Hague CEF and Chips Act compensation for litigation risk.

We fund it without new money by tasking ENISA and CERT-EU to push the automated-patching and swarm-detection tooling already proven this turn into M8-protected energy, cable and hospital systems and the stalled gigafactory land, using DG EMPL retraining lines to pair rollout with graduate hiring for patch-operations crews to answer labour displacement where capital allows.
