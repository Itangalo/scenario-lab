...
- characters 376-1617: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1660-3985: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4041-5045: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5108-32249: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32604-32898: `{{event_history}}` from the run's own event record
...
- `emergent_datacentre_siting_backlash` -- first noted turn 4, listed in 2 turn(s) so far: A coordinated protest and legal blockade against new data-centre grid connections in two member states delays contracted compute expansion for six months, cited by opponents as proof of foreign-model energy costs.
- `emergent_grid_backlash_stall` -- first noted turn 4, listed in 2 turn(s) so far: Municipal utilities hit by staffing shortages and untuned detection trigger coordinated protests and siting blockades against new data-centre and grid upgrades in two follower states, stalling rollout.
...
- Turn 2 (4 turn(s) ago): safety_breakthrough, adoption_success
- Turn 3 (3 turn(s) ago): adoption_success
- Turn 4 (2 turn(s) ago): cyber_defence_breakthrough
- Turn 5 (1 turn(s) ago): cyber_major_incident, export_control_escalation, election_alliance
