...
- characters 105-13512: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13555-13747: `{{metrics_json}}` from the run's live metric values
- characters 13840-14739: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14782-16482: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16538-17686: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Assurance Adoption Cell | 2 |
| M5 | European Trusted Deployment Scale-up | 2 |
| M6 | Municipal Patching and Swarm-Detection Rollout | 2 |
...
PORTFOLIO CHARGE t3: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Infrastructure Cyber Shield −3, EU Assurance Adoption Cell −2, European Trusted Deployment Scale-up −2, priority −1 = −14
PROPOSAL BONUS: none — new scale-up answers adoption_success this turn, which is a positive window not a negative event, so no bonus under rule 6
LEGITIMACY LENDS: capital 21, sentiment 44 -> +2
SOVEREIGNTY: 20 last turn, no capacity event, Gigafactories/tech package in flight +0, capability rose 3.0 −1 = 19

## Emerging developments (tracked)

- `emergent_datacentre_siting_backlash` -- first noted turn 4, listed in 1 turn(s) so far: A coordinated protest and legal blockade against new data-centre grid connections in two member states delays contracted compute expansion for six months, cited by opponents as proof of foreign-model energy costs.
- `emergent_grid_backlash_stall` -- first noted turn 4, listed in 1 turn(s) so far: Municipal utilities hit by staffing shortages and untuned detection trigger coordinated protests and siting blockades against new data-centre and grid upgrades in two follower states, stalling rollout.
...
## New measure
**Municipal Patching and Swarm-Detection Rollout**
Deploys the new automated patching and swarm-behaviour detection tooling to municipal utilities and previously hit port/water sites via ENISA-coordinated kits and managed-service contracts.
This is Preparedness and resilience (6) and this breakthrough window is why now: for the first time the offence-defence balance has shifted back, and pushing it to the lagging tier that auditors flagged twice directly serves our two-year resilience commitment before winter probes return.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Municipal Patching and Swarm-Detection Rollout", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via detection"}, "grounds": "to push defence breakthrough to lagging municipals and hit sites"}]}
```

## Priority
EU Critical Infrastructure Cyber Shield follow-through via the new rollout, because the breakthrough only insures our resilience commitment if it reaches the municipal and port/water gaps that would otherwise cascade.

## In practice
We task ENISA and DG CNECT with packaging the breakthrough tooling into hardened update channels and swarm-detection signatures, funded by CEF reprogramming and Digital Europe, and delivered as managed-service contracts for understaffed municipals. Energy Council and the port/water regulators condition continued liability cover and queue preference on adoption and tuning, closing the spares and staffing loop auditors flagged.

We keep DG REFORM, DG SANTE and the JRC Assurance Cell on the Trusted Deployment Scale-up in the five followers, and let DG CNECT carry M1/M2 gigafactory permitting administratively without new political spend, to hold total costs within our thin capital while the defence window is exploited.
...
## Notepad
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Assurance Adoption Cell −2, European Trusted Deployment Scale-up −2, Municipal Patching and Swarm-Detection Rollout −2, priority −1 = −13
PROPOSAL BONUS: none — new rollout answers defensive breakthrough window which is positive, not a negative event from last three rounds
LEGITIMACY LENDS: capital 14, sentiment 46 -> +2
SOVEREIGNTY: 19 last turn, no capacity event, Gigafactories/tech package in flight +0, capability rose 3.0 −1 = 18
