...
- characters 376-1584: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1627-3905: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3961-5087: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5150-32327: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32682-32999: `{{event_history}}` from the run's own event record
...
- `emergent_copycat_substation_probing` -- first noted turn 4, listed in 2 turn(s) so far: Copycat actors use the circulating ready-to-run ICS toolkits to attempt low-skill intrusions on small municipal utilities, causing brief localized outages that amplify public fear disproportionate to physical damage.
- `emergent_toolkit_substation_trip` -- first noted turn 4, listed in 2 turn(s) so far: Copycat use of the circulating ICS toolkits triggers a short localized substation trip and brief outage in southeastern Europe, claimed with screenshots on underground forums but without confirmed attribution.
...
- Turn 1 (5 turn(s) ago): cyber_test_shot, loss_of_control_incident, emergent_datacentre_blockade
- Turn 3 (3 turn(s) ago): bio_uplift_findings, emergent_kimi_exploit_kits
- Turn 4 (2 turn(s) ago): member_state_defection
- Turn 5 (1 turn(s) ago): eval_anomaly_reports, eu_frontier_access_denied, election_retrenchment
