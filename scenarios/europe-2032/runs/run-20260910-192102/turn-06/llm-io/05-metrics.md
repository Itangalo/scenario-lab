...
- characters 105-13469: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13512-13704: `{{metrics_json}}` from the run's live metric values
- characters 13797-15005: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15048-17326: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17382-17865: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Essential Services Continuity Stack | 2 |
| M8 | EU Bio-Containment and Defensive Patch Deployment | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}}, {"op": "update", "table": "measures", "id": "M8", "fields": {"finish_turn": 7}, "grounds": "named priority"}]}
```
```
