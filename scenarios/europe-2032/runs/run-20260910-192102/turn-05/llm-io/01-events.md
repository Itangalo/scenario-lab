...
- characters 377-1551: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1595-3763: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3819-4933: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4996-32544: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32899-33119: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Jurisdiction Lock for Foreign AI Capacity Deals −2, priority −1 = −9
PROPOSAL BONUS: EU Jurisdiction Lock (cat 8, small) +4 — member_state_defection landed this turn and this answers it directly
LEGITIMACY LENDS: capital 24, sentiment 35 -> +2
SOVEREIGNTY: 19 last turn, no capacity event, no cat4 finish, Gigafactories in flight +0, member_state_defection t4 −2, capability rose 3.0 −1 = 16

## Emerging developments (tracked)

- `emergent_copycat_substation_probing` -- first noted turn 4, listed in 1 turn(s) so far: Copycat actors use the circulating ready-to-run ICS toolkits to attempt low-skill intrusions on small municipal utilities, causing brief localized outages that amplify public fear disproportionate to physical damage.
- `emergent_toolkit_substation_trip` -- first noted turn 4, listed in 1 turn(s) so far: Copycat use of the circulating ICS toolkits triggers a short localized substation trip and brief outage in southeastern Europe, claimed with screenshots on underground forums but without confirmed attribution.
...
- Turn 1 (4 turn(s) ago): cyber_test_shot, loss_of_control_incident, emergent_datacentre_blockade
- Turn 3 (2 turn(s) ago): bio_uplift_findings, emergent_kimi_exploit_kits
- Turn 4 (1 turn(s) ago): member_state_defection
