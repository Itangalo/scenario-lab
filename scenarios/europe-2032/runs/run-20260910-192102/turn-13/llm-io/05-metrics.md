...
- characters 107-13367: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13410-13600: `{{metrics_json}}` from the run's live metric values
- characters 13693-15998: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 16042-17882: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17938-18715: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M15 | EU Substitute Inference and Islanded Essentials | 2 |
...
- `emergent_islanding_refusal_compact` -- first noted turn 13, listed in 1 turn(s) so far: Municipal utilities and hospital federations publish a joint refusal to reconnect pooled inference without locally held cut-off keys, turning islanding from emergency order into permanent operating rule.
...
## Metrics
```json
{"ai_capability": 87.2, "openweight_capability": 84.0, "ai_safety": 0.0, "resilience": 63.0, "eu_ai_sovereignty": 10.0, "eu_political_capital": 52.0, "public_sentiment": 2.0}
```
