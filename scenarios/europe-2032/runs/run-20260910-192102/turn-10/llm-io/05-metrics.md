...
- characters 106-13378: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13421-13612: `{{metrics_json}}` from the run's live metric values
- characters 13705-14810: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14853-17201: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17257-18354: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M12 | EU Blockade-Era Continuity and Bio-Defence Surge | 2 |
...
PORTFOLIO CHARGE t9: EU Loss-of-Control Containment and Continuity Regime −2, priority −1 = −3
PROPOSAL BONUS: EU Loss-of-Control Containment and Continuity Regime (cat 6, small) +4 — loss_of_control_incident landed this turn and this answers it directly
LEGITIMACY LENDS: capital 34, sentiment 23 -> none, capital is higher
SOVEREIGNTY: 14 last turn, no cat4 finishes, no cat4 in flight +0, middle_power_coalition t9 +2, capability rose 2.5 −1 = 15

## Emerging developments (tracked)

- `emergent_shadow_inference_market` -- first noted turn 10, listed in 1 turn(s) so far: Municipal utilities and hospital networks, unable to expand pooled inference, begin buying grey-market inference from unvetted foreign-hosted frontier agents via resellers, creating an unauthorised shadow capacity outside the containment regime.
- `emergent_anti_compute_sabotage` -- first noted turn 10, listed in 1 turn(s) so far: Coordinated physical sabotage and blockades target data-centre construction sites and grid-connection works in moratorium regions, forcing extended shutdowns of pooled inference expansion.
...
## Notepad
PORTFOLIO CHARGE t10: EU Blockade-Era Continuity and Bio-Defence Surge −2, priority −1 = −3
PROPOSAL BONUS: EU Blockade-Era Continuity and Bio-Defence Surge (cat 6, small) +6 — taiwan_blockade plus bio_uplift_findings plus openweight_frontier_release plus capability_jump landed this turn and this answers them directly
LEGITIMACY LENDS: capital 36, sentiment 12 -> none, capital is higher
SOVEREIGNTY: 15 last turn, no cat4 finishes, no cat4 in flight +0, taiwan_blockade t10 −2, capability rose 5.5 −1 = 12

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M12", "fields": {"finish_turn": 11}, "grounds": "named priority"}]}
```
```
