...
- characters 455-1944: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1988-3486: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4264-4723: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6705-8347: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_kimi_exploit_kits (emergent event):** Underground forums circulate ready-to-run offensive toolkits derived from Kimi K3, sharply lowering the skill needed to attack industrial control systems.
...
| M4 | EU OT Intrusion Reporting and Shared Detection Network | 2 | small | 2 | 4 | own jurisdiction | resilience up, ai_safety up slightly via shared learning | 2 | running |
...
## New measure
**EU OT Intrusion Reporting and Shared Detection Network**
Requires transmission, port and water operators to report OT intrusions within 24h to ENISA and share detection signatures across borders. A category 2 Transparency and reporting measure, small and fast because M3 hardens infrastructure but without mandatory reporting the rehearsal pattern stays invisible.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU OT Intrusion Reporting and Shared Detection Network", "category": 2, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via shared learning"}, "grounds": "autumn rehearsal undetected without mandatory sharing"}]}
```

## Priority
EU Critical Systems Hardening and Response Pact, because credential rotation and exercised fallback must land before the next probe window — reporting amplifies it but does not replace it.

## In practice
We push M3 through the Energy and Transport Councils via NIS2 implementing acts, using Digital Europe and CEF money already redirected for OT sensors; ENISA leads joint audits with ACER and EMSA, and we trade fast-track permit compacts for municipal buy-in in Spain and Germany to keep M1 and M2 alive without opening a new compute line.

We table the new reporting mandate as a narrow NIS2 delegated act to avoid a full co-decision fight, with ENISA operating the shared signature hub and national CSIRTs enforcing the 24h rule. Operators resisting on liability get safe-harbour for shared indicators, keeping cost low while M3 does the heavy hardening.
...
```
## New measure
**EU Bio-Shield and OT Emergency Patch Surge**
Deploys joint HERA-ENISA rapid teams for biothreat detection triage and emergency OT patching against Kimi-derived exploit kits in hospitals, labs and grid operators. A category 6 Preparedness and resilience measure, small and fast because the bio precursor and circulating ICS toolkits demand absorption capacity now, not new compute.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Shield and OT Emergency Patch Surge", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via incident learning"}, "grounds": "bio uplift signal plus circulating Kimi exploit kits lowering attack skill"}]}
```

## Priority
EU Bio-Shield and OT Emergency Patch Surge, because circulating ready-to-run ICS kits and a credible bio-design signal outrank building reporting pipes — absorption must land before the next probe window.

## In practice
We execute via the Health Emergency Preparedness and Response Authority with ECDC for bio-detection protocols, wastewater and hospital sequencing uplift, and via ENISA with Energy and Transport Councils for emergency OT segmentation backstops and credential rotation in the most exposed southern utilities. Money comes from redirected EU4Health and Digital Europe funds already used for M3, avoiding a new budget fight.

We keep M4 alive as the 24h reporting delegated act with safe-harbour to feed the signature hub, and we protect M1/M2 gigafactory compacts in Spain and Germany with water and power-price guarantees, but staff and Council time this half go to the surge teams and joint exercises. Operators resisting on cost get EU-funded patch kits and liability cover for shared indicators.
