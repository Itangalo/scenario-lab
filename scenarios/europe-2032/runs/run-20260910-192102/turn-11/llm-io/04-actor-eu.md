...
- characters 455-1729: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1773-3934: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5506-5967: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9288-11737: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_anti_compute_sabotage (emergent event):** Coordinated physical sabotage and blockades target data-centre construction sites and grid-connection works in moratorium regions, forcing extended shutdowns of pooled inference expansion.
...
- `two_year_commitment` (commitment): Contain loss-of-control risk and keep essential services running through AI disruption
...
| M4 | EU OT Intrusion Reporting and Shared Detection Network | 2 | small | 2 | 4 | own jurisdiction | resilience up, ai_safety up slightly via shared learning | 2 | finished |
| M5 | EU Bio-Shield and OT Emergency Patch Surge | 6 | small | 3 | 4 | own jurisdiction | resilience up, ai_safety up slightly via incident learning | 2 | finished |
| M6 | EU Jurisdiction Lock for Foreign AI Capacity Deals | 8 | small | 4 | 5 | own jurisdiction | eu_ai_sovereignty up, eu_political_capital down slightly then up if held | 2 | finished |
| M7 | EU Essential Services Continuity Stack | 6 | small | 5 | 7 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly via substitution | 2 | finished |
| M8 | EU Bio-Containment and Defensive Patch Deployment | 6 | small | 6 | 7 | own jurisdiction | resilience up strongly to absorb bio and cyber cascade | 2 | finished |
| M9 | EU Assured Cures Deployment | 9 | small | 7 | 8 | own jurisdiction | public_sentiment up strongly, resilience up slightly, ai_safety up slightly via certified deployment | 2 | finished |
| M10 | EU Opaque-Model Inspection and Bio-Screening Lab | 5 | small | 8 | 9 | own jurisdiction | ai_safety up, resilience up slightly, eu_ai_sovereignty up slightly via independent assurance | 2 | finished |
| M11 | EU Loss-of-Control Containment and Continuity Regime | 6 | small | 9 | 10 | own jurisdiction | resilience up strongly, ai_safety up slightly via containment learning | 2 | finished |
| M12 | EU Blockade-Era Continuity and Bio-Defence Surge | 6 | small | 10 | 11 | own jurisdiction | resilience up strongly, ai_safety up slightly via containment and screening learning | 2 | finished |
...
## New measure
**EU Blockade-Era Continuity and Bio-Defence Surge**
Keeps hospitals, grid, water and payments running on hardened analogue fallback plus screened EU-controlled inference while surging bio-detection, medical countermeasures and OT patching against open-weight-enabled bio uplift and the agent failures already seen, and uses the Union's lithography and equipment choke-point as joint leverage to secure screened compute and spares during the Taiwan blockade. This is Preparedness and resilience work, category 6, chosen now because containment alone cannot hold when open weights sit within 5 points of a jumped frontier, a credible human-infecting design signal is loose, and semiconductors stop flowing.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Blockade-Era Continuity and Bio-Defence Surge", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up strongly, ai_safety up slightly via containment and screening learning"}, "grounds": "Taiwan blockade plus bio uplift plus near-frontier open weights demand immediate continuity and bio-defence"}]}
```

## Priority
EU Blockade-Era Continuity and Bio-Defence Surge, because with safety at 9 and public sentiment at 18 nothing else survives if essential services cascade or a bio incident lands — this outranks sovereignty build-out this turn.

## In practice
We act under NIS2, the Cyber Solidarity Act and the HERA/ECDC mandate through ENISA, EU-CyCLONe and the M10 Inspection Lab as screener: reprogram Digital Europe, EU4Health and rescEU to fund manual-override staffing, sealed-ward protocols, wastewater and syndromic detection, and agent-behaviour monitors and kill-switches on remaining pooled inference. Municipalities lifting grid-connection moratoria get first continuity funding and screened automation for clinics and ports; foreign humanoid stacks stay quarantined from critical sites.

Externally we activate the middle-power framework and the Anti-Coercion Instrument logic: coordinated export-licence controls on lithography, optics and spares in exchange for joint compute bargaining and shared evaluation, without ceding EU jurisdiction. Finance ministers are told this is rationing and repair, not new Gigafactories: no parallel compute programme, just keeping what exists running through the blockade.
...
```
## New measure
**EU Major-Incident Restoration and Assured Systems Rollout**
Puts ENISA-led restoration teams and manual fallbacks into hit public services after the automated attack while rolling the new interpretability/control result through the Inspection Lab onto deployed pooled systems, paired with protected repair corridors for sabotaged grid/data-centre works and middle-power joint spares access. This is Preparedness and resilience work, category 6, chosen now because the attack proves defence is behind open-weight offence, the safety breakthrough is the first certifiable gain in years, and sabotage plus blockade means restoration fails without secured parts and build sites.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Major-Incident Restoration and Assured Systems Rollout", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up strongly, ai_safety up moderately via certified controls"}, "grounds": "automated major incident plus sabotage plus first deployable assurance gain"}]}
```
## Priority
EU Major-Incident Restoration and Assured Systems Rollout, because with safety at 2 and sentiment at 12 nothing outranks visibly restoring hit services and proving the new control actually lands — this is the two-year commitment in its purest form.
## In practice
We act under NIS2 Article 11/CSOA plus Civil Protection Mechanism: ENISA and EU-CyCLONe triage the ransomware/dependency blast radius, isolate compromised builds, push known-good images and paper/manual overrides to hospitals, municipalities and grid operators, funded by reprogrammed Digital Europe and rescEU continuity money.

The M10 Inspection Lab becomes the rollout hub for the interpretability breakthrough — certifying kill-switches and behaviour monitors on remaining EU-controlled inference before reconnection — while Interior/Energy Councils offer protected-site status and continuity funds to lift sabotage blockades, and we table joint export-licence and spares bargaining in the middle-power coalition to keep hardened chips flowing despite US/Beijing pressure.
