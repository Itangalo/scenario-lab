...
- characters 455-2121: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2165-4123: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5312-5752: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7352-9160: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Grid and Public Services Shield | 6 | large | 2 | 5 | own jurisdiction | resilience up strongly, ai_safety up modestly | 3 | running |
...
```
## New measure
None this turn, while we hold capital for the Shield and Evaluation Institute to land and wait for the investment reset to reprice compute deals.

## Store changes
No changes.

## Priority
M4 EU Grid and Public Services Shield, because live ransomware on hospitals and ports plus the US model cutoff outrank concrete-free gigafactories and the anomaly reports demand exercised containment before new build.

## In practice
We push M4 through ENISA and DG HOME civil-protection: mandatory segmentation audits for TSOs under NIS2/CER, EU-funded incident teams deployed to the six hit states, breaker-credential rotation verified by ACER, with CEF-Digital money to blunt energy-ministry cost resistance.

We keep M1/M2 alive but reframe them for the crash: DG CNECT and EIB pause private co-financing calls, preserve permits and grid reservations at the five sites, and prepare distressed-asset terms for cancelled builds. M3 absorbs the leaked eval anomaly via JRC: mandatory anomaly reporting to the Institute and joint review with frontier labs, without halting deployments until legibility is judged.
```
