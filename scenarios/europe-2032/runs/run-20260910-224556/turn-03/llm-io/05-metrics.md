...
- characters 106-13234: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13277-13469: `{{metrics_json}}` from the run's live metric values
- characters 13562-14664: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14708-16611: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16667-17437: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Critical Infrastructure Cyber Shield | 2 |
| M5 | EU ASML Shield and Supply-Chain Coalition | 2 |
...
- `emergent_grid_toolkit_copycats` -- first noted turn 2, listed in 2 turn(s) so far: Forensic write-ups of the autumn grid intrusion circulate exploit tooling derived from the open model, triggering copycat probing of municipal utilities and ports by smaller actors.
...
## Metrics
```json
{"ai_capability": 63.0, "openweight_capability": 54.0, "ai_safety": 23.0, "resilience": 35.0, "eu_ai_sovereignty": 17.0, "eu_political_capital": 11.0, "public_sentiment": 33.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M5", "fields": {"finish_turn": 5}, "grounds": "named priority"}, {"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 8}, "grounds": "event: permitting freeze and protests stall build-out"}]}
```
```
