...
- characters 13283-13472: `{{metrics_json}}` from the run's live metric values
- characters 13565-14514: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14558-16372: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16428-17067: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_anti_ai_infrastructure_sabotage` -- first noted turn 13, listed in 1 turn(s) so far: Coordinated physical sabotage and blockades targeting municipal data-centre and grid-automation sites in multiple member states, forcing temporary shutdowns and emergency policing.
...
## Metrics
```json
{"ai_capability": 92.5, "openweight_capability": 89.0, "ai_safety": 5.0, "resilience": 36.0, "eu_ai_sovereignty": 12.0, "eu_political_capital": 0.0, "public_sentiment": 1.0}
```
## Narrative
...
## Notepad
PORTFOLIO CHARGE t13: no measures in flight −0, priority −0 = −0
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 0, sentiment 2 -> none (capital not below sentiment at start, sentiment above but capital at floor, no lend applied)
SOVEREIGNTY: 14 last turn, no cat4 finish, no cat4 in flight +0, eu_frontier_access_denied t13 −2 (largest capacity event, embodied_ai_deployment also qualifies but one term only), capability rose 1.5 −0 = 12
## Store changes
```json
{"store": []}
```
