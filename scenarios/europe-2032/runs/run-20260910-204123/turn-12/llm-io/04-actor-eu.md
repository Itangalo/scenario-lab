...
- characters 452-1345: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1388-3364: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4293-4816: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7451-9254: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential services, jobs and democratic control running through loss-of-control and displacement shocks without depending on foreign permission
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Swarm-Defence Hardening Rollout | 6 | small | 11 | 13 | own jurisdiction | resilience up moderately, ai_safety up slightly | 2 | running |
...
| M9 | EU Cut-off Continuity Switch | 6 | small | 8 | 10 | own jurisdiction | resilience up moderately, public_sentiment up slightly | 2 | finished |
...
## New measure
**EU Swarm-Defence Hardening Rollout**
Deploys the new automated patching and swarm-behaviour detection across hospitals, registries, energy and telecom operators via ENISA-coordinated procurement.
This now because the cyber-defence breakthrough finally shifts offence-defence back, and with opaque reasoning killing chain-of-thought oversight and capital at 4 we need a cheap, visible resilience win that serves our two-year commitment to keep services running through unpreventable shocks.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Swarm-Defence Hardening Rollout", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly"}, "grounds": "deploying proven defensive tooling while window is open"}]}
```

## Priority
No active measure to prioritize this turn, because the portfolio has no in-flight measures and the newly proposed hardening rollout is only just starting.

## In practice
With no in-flight measures costing capital, we enforce only what is finished — Shield detection rules, Agent Containment reporting, and the Fallback Stack with cut-off rehearsals — while the AI Office and ENISA triage opaque-reasoning models by shifting to black-box evaluations and activation-inspection pilots.

We launch the Swarm-Defence rollout through ENISA and the NIS2 Cooperation Group, using joint procurement and Digital Europe funds rather than a new law, offering operators free tooling and incident-support in exchange for mandatory patch-speed and swarm-alert sharing. DG EMPL meanwhile stretches existing transition lines toward displaced juniors, framing the hardening as keeping services and jobs running without foreign permission.
...
```
## New measure
None this turn, because with political capital and public sentiment at zero we cannot credibly start anything new until the in-flight hardening finishes and trust stabilises.

## Store changes
No changes.

## Priority
M10, because the EU Swarm-Defence Hardening Rollout is the only in-flight instrument still costing capital and the only defence win that maps to our two-year commitment to keep essential services running without foreign permission.

## In practice
We hold the line on what is finished — Critical Systems Shield detection rules, Agent Containment reporting, the Fallback Stack and cut-off rehearsals — and push the remaining ENISA joint-procurement tranches for automated patching and swarm-behaviour detection to large hospitals, energy and telecom operators, while admitting municipalities remain uncovered.

The AI Office suspends chain-of-thought reliance entirely and triages opaque models through black-box testing and activation-inspection pilots, with DG EMPL stretching existing transition funds to junior-unemployment hotspots. No new law, no new fund, no new promise is tabled this semester; the Commission communicates only delivery on patch-speed data and service continuity.
