...
- characters 452-2248: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2291-4354: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6218-6678: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9012-10853: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Visible Benefit Adoption Drive | 9 | small | 7 | 9 | own jurisdiction | public_sentiment up moderately, resilience up slightly | 2 | running |
...
| M5 | EU Supply-Chain Leverage Pact | 8 | small | 3 | 5 | coalition | eu_ai_sovereignty up moderately, resilience up slightly | 2 | finished |
| M6 | EU Essential Services Fallback Stack | 4 | small | 4 | 5 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up moderately | 2 | finished |
| M7 | EU Displacement Transition Fund | 7 | small | 6 | 8 | own jurisdiction | resilience up moderately, public_sentiment up | 2 | finished |
...
Scales the working public-sector AI deployments that cut waiting lists and decision times into a branded EU-wide rollout through health, administration and schools.
Now is the moment because adoption success is the only win attributed to a European decision amid open-weight shock and US tiering, and with sentiment at 6 and capital at 4 we must turn visible benefit into tolerance to hold the jobs and essential-services commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Visible Benefit Adoption Drive", "category": 9, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up slightly"}, "grounds": "convert adoption success into legitimacy during open-weight shock"}]}
```

## Priority
M7 EU Displacement Transition Fund, because queues longer than payouts and court-challenged levy mean the jobs commitment fails first if retraining cash does not flow, even as we seed the adoption drive.

## In practice
We keep M7 alive via EPSCO and ESF+ bridging: Commission advances national job centres from existing envelopes while DG EMPL defends the deployer levy in court, targeting entry clerical, customer-ops and junior coding freezes with wage-insurance plus fast retraining vouchers.

We launch the Adoption Drive via DG DIGIT and DG SANTE procurement: copy the hospital/grid models using the interpretability checklist and fallback stack hosting, funded by repurposed Digital Europe deployment lines, with mayors and health ministers fronting waiting-list results to rebuild trust. Skeleton gigafactory care-and-maintenance continues from EuroHPC JU envelopes only — no new capital call — to preserve permits and grid slots through US tier-rationing.
...
Moves hospitals, ministries and firms cut off from the US frontier model onto the EU fallback stack and older checkable models within weeks.
This and now because access-denied turns dependence into outage on top of eval-anomaly fear and a US-China deal we are outside of, and our two-year commitment to keep essential services and jobs running on EU-controlled capacity fails first if triage, procurement and admin systems go dark.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cut-off Continuity Switch", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up slightly"}, "grounds": "keep cut-off essential services running on EU-controlled fallback"}]}
```

## Priority
M8 EU Visible Benefit Adoption Drive, because the only EU-controlled capacity that actually works right now is the clinic checklist/fallback hosting it scales, and it must absorb the cut-off load to hold the essential-services commitment.

## In practice
We trigger the fallback stack built under M6 via DG DIGIT and ENISA: HERA and DG SANTE issue emergency continuity guidance letting hospitals and ministries swap US API calls for containerised checkable models on hospital servers and EuroHPC partitions, with islanding drills repurposed as migration playbooks.

We fund it by repurposing Digital Europe and EU4Health deployment lines already behind M8, with health ministers and mayors fronting restored waiting-list numbers to contain backlash at sentiment 8. We ask for observer/consulted status on the US-China weights-security understanding via EEAS and the coordination pact, offering incident-reporting and hospital eval data, but spend no new measure slot on it while capital is at zero.
