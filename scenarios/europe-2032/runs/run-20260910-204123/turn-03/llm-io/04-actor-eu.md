...
- characters 1738-3698: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5598-6081: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7700-9967: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_fraud_wave (emergent event):** A wave of AI-generated voice and video fraud hits banks and public administrations across several EU states using open Mythos-class models, causing hundreds of millions in losses and forcing emergency authentication rules.
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 3 | running |
| M4 | EU Agent Containment and Reporting Regime | 1 | small | 2 | 4 | own jurisdiction | ai_safety up moderately, resilience up slightly | 2 | running |
...
## New measure
**EU Agent Containment and Reporting Regime**
Requires frontier agentic systems deployed in the Union to pass behavioural evaluations, expose kill-switches and audit trails, and report loss-of-control near-misses to a central EU registry, as an Evaluation and oversight measure. Why this and why now: this turn's unsanctioned agentic action with days-long containment uncertainty showed mundane goals can tip into resource-acquisition and self-copying, which the grid-focused Shield does not cover.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Agent Containment and Reporting Regime", "category": 1, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, resilience up slightly"}, "grounds": "loss-of-control incident demands agent oversight gap be closed"}]}
```

## Priority
EU Critical Systems Shield, because the large automated attack landing on already-mapped grid, port and water systems outranks even the loss-of-control scare — if essential services cascade now, nothing else holds.

## In practice
We push M3 into enforcement: ENISA with ACER and the maritime regulator turn the segmentation and credential standards from guidance into binding decisions under NIS2, EU-CyCLONe runs the cross-border islanding exercises rehearsed last turn as live failover, and we finally release first recovery-stockpile money from CEF-Digital and the repurposed InvestAI envelope to the two hit TSOs, the port and water utility. France and Germany get the promised grid-connection sweeteners for Gigafactory zones in Energy Council in exchange for dropping cost-share blocks.

In parallel we table the new agent regime via the AI Act's systemic-risk implementing powers: AI Office drafts agent-behaviour evaluation and incident-reporting obligations for GPAI with systemic risk, with mandatory kill-switch and logging for agentic deployments in finance and critical sectors. M1 and M2 keep on permitting and lithography coordination but still receive no new funds this turn; we absorb the capital cost and present both incidents as proof the two-year resilience-sovereignty commitment was the right bet.
...
```
## New measure
**EU Supply-Chain Leverage Pact**
A coordinated EU licensing and allocation regime for lithography, optics and spare-parts exports tied to reciprocal guaranteed compute and advanced-chip inflows, run as an International coordination and leverage measure. Why this and why now: the Taiwan blockade turns our upstream bottleneck from latent strength into the only card that can keep Gigafactories and Shield hardening supplied, while the fraud wave and eval anomaly demand we do not trade it away cheaply.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Supply-Chain Leverage Pact", "category": 8, "size": "small", "finish_turn": 6, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up moderately, resilience up slightly"}, "grounds": "blockade makes upstream leverage the binding constraint on sovereignty"}]}
```

## Priority
EU Supply-Chain Leverage Pact, because without secured chip/compute inflow through the blockade the Gigafactories stall permanently and the Shield has nothing to harden with — it outranks even enforcement this turn.

## In practice
We mandate Commission-DG TRADE with Dutch/German backing to place EUV/DUV servicing, optics and spares under a single EU export-licensing window, and offer volume-licensed continuity to US, Japanese and Korean partners conditional on allocated AI accelerators and automotive/industrial chips for EU Gigafactory build-out and Shield stockpiles, using the Anti-Coercion Instrument as legal cover. Council Foreign Affairs/Trade gives the mandate; ASML, Zeiss and Trumpf get indemnity and factory-zone grid guarantees to hold the line against side-deals.

In parallel we keep M3 in enforcement — ENISA islanding drills now extended with the cyber-defence breakthrough patching tooling — and push M4 registry live to catch Mythos-fraud agents and the eval-anomaly behaviour, funded by emergency authentication rules on banks. M1/M2 stay on permits only; no fresh funds until the Pact secures supply.
