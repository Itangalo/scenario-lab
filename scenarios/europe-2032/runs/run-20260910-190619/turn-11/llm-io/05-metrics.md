...
- characters 107-13356: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13399-13590: `{{metrics_json}}` from the run's live metric values
- characters 13683-14326: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14370-16154: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16210-16731: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M8 | Defection Containment Compact | 2 |
| M9 | EU Service Continuity Fallback | 2 |
...
PORTFOLIO CHARGE t10: M7 EU Loss-of-Control Containment Drill −2, M8 Defection Containment Compact −2, priority −1 = −4 (priority without effect and uncharged as capital <20, measures −4)
PROPOSAL BONUS: Defection Containment Compact (cat 8, small) +3 — member_state_defection landed this turn severe and this answers it directly with cohesion side-payments
LEGITIMACY LENDS: capital 7, sentiment 22 -> +2
SOVEREIGNTY: 14 last turn, no finish, no cat4 in flight, member_state_defection t10 −2, capability rose 1.0 −0 = 12
...
### Fallback, thinly spread
Brussels answered with what it had. The loss-of-control drill completed in autumn fed directly into emergency playbooks, and patch teams re-pointed denied workflows onto locally hosted open systems on national machines and allied patch nodes. Where the re-hosting landed, wards stayed open and payments cleared.

It was partial. The open substitutes were weaker, slower and unfamiliar; clinicians complained of false flags and extra hours. The defector capital froze its undercut in exchange for first fallback slots, but kept its separate hosting contract. Eastern mayors protested that the queue still favoured the west when engineers were scarce. Concrete for the two factory sites remained unpoured, courts and blockades unmoved.

By December services held where patched, but the Union looked exposed. Press coverage dwelt on dependency: a continent that had been warned, had drilled, and still had to borrow and improvise when the switch was thrown. Anger at providers mixed with anger at Brussels, and talk of building at home grew louder even as the money to do it shrank.

## Notepad
PORTFOLIO CHARGE t11: M8 Defection Containment Compact −2, M9 EU Service Continuity Fallback −2, priority −1 = −4 (priority without effect and uncharged as capital <20, measures −4)
PROPOSAL BONUS: none — M9 already in flight, no new measure started this turn to earn one-off
LEGITIMACY LENDS: capital 3, sentiment 15 -> +2 capped, applied in total
SOVEREIGNTY: 12 last turn, no finish, no cat4 in flight, eu_frontier_access_denied t11 −3, capability rose 2.0 −1 = 9

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M9", "fields": {"finish_turn": 12}, "grounds": "named priority"}]}
```
```
