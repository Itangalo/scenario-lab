...
- characters 376-2205: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2248-4883: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4939-6035: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 6098-31033: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 31388-31721: `{{event_history}}` from the run's own event record
...
- `emergent_grid_sabotage_wave` -- first noted turn 7, listed in 1 turn(s) so far: A coordinated winter wave of physical blockades and sabotage against grid interconnectors and data-centre substations in several member states, organized outside formal unions and amplified by viral footage of sealed hospital wards.
- `emergent_grid_permit_freeze` -- first noted turn 7, listed in 1 turn(s) so far: Coordinated blockades and legal challenges stall grid upgrades for the reserved gigafactory sites through autumn, forcing the Commission to pay reservation fees with no construction start.
...
- Turn 4 (4 turn(s) ago): bio_uplift_findings
- Turn 5 (3 turn(s) ago): cyber_major_incident, supply_chain_coercion, election_alliance
- Turn 6 (2 turn(s) ago): bio_incident, knowledge_work_augmented
- Turn 7 (1 turn(s) ago): knowledge_work_augmented, us_china_agreement
