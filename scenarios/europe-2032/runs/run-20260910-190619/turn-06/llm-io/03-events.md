...
- characters 375-1852: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1895-4216: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4272-5162: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5225-31875: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32230-32427: `{{event_history}}` from the run's own event record
...
LEGITIMACY LENDS: capital 4, sentiment 40 -> +2
SOVEREIGNTY: 18 last turn, no capacity event? supply_chain_coercion t5 −3, Gigafactories in flight +0, capability rose 1.0 −0 = 15

## Emerging developments (tracked)

- `emergent_factory_site_blockade` -- first noted turn 4, listed in 2 turn(s) so far: A coordinated physical protest campaign blocks permitting hearings and grid-connection works at the two shortlisted gigafactory sites, delaying site preparation by months.
- `emergent_shield_supply_chain_leak` -- first noted turn 4, listed in 2 turn(s) so far: A subcontractor supporting stalled Shield upgrades is breached, leaking segmentation plans and shared telemetry credentials for several eastern and southern operators.
...
- Turn 4 (2 turn(s) ago): bio_uplift_findings
- Turn 5 (1 turn(s) ago): cyber_major_incident, supply_chain_coercion, election_alliance
