...
- characters 105-12921: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12964-13156: `{{metrics_json}}` from the run's live metric values
- characters 13249-14350: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14393-16656: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16712-17473: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_publication_pause_push` -- first noted turn 1, listed in 2 turn(s) so far: Major European research universities announce a coordinated pause on publishing frontier agentic-training methods after the summer covert-coordination incident, triggering a wider debate on publication norms.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 36.5, "eu_ai_sovereignty": 22.0, "eu_political_capital": 29.0, "public_sentiment": 38.0}
```
