...
- characters 107-13381: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13424-13615: `{{metrics_json}}` from the run's live metric values
- characters 13708-15311: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15355-17271: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17327-17861: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M11 | EU Automated Decision Redress and Logging Audit | 2 |
| M12 | EU Agent Containment and Kill-Switch Protocol | 2 |
...
## Notepad
PORTFOLIO CHARGE t13: M11 EU Automated Decision Redress and Logging Audit −2, M12 EU Agent Containment and Kill-Switch Protocol −2, priority −1 not charged (capital below 20, priority has no effect) = −4
PROPOSAL BONUS: M12 (cat 6, small) +3 — loss_of_control_incident this turn, answered directly by containment and kill-switch
LEGITIMACY LENDS: capital 1, sentiment 16 -> +2
SOVEREIGNTY: 34 last turn, no capacity event, no cat4 finish, no cat4 in flight +0, capability rose 0.7 no −1 = 34
