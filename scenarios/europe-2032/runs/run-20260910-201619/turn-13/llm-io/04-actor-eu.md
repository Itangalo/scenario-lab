...
- characters 455-2058: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2102-4018: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4712-5219: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8651-11137: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Hold cohesion through containment without confidence by hardening recovery, sharing threat response, and defending the EU chokepoint
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Automated Decision Redress and Logging Audit | 2 | small | 12 | 14 | own jurisdiction | public_sentiment up slightly, eu_political_capital up slightly via restored trust, resilience up slightly | 2 | running |
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Automated Defence Rollout | 6 | small | 2 | 4 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 2 | finished |
| M5 | EU Open Model Triage and Publication Norms | 5 | small | 3 | 5 | own jurisdiction | ai_safety up slightly, resilience up slightly via triage guidance to Shield operators | 2 | finished |
| M6 | European Public Benefit AI Scale-up | 9 | small | 5 | 8 | own jurisdiction | public_sentiment up, eu_political_capital up, resilience up slightly | 2 | finished |
| M7 | EU Supply-Chain Bargain and Rationing Pact | 8 | small | 6 | 9 | US, like-minded supply-chain holders and member states | eu_ai_sovereignty up slightly, eu_political_capital up slightly via cohesion | 2 | finished |
| M8 | EU Cyber Emergency Hardening and Mutual Aid Activation | 6 | small | 8 | 10 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 2 | finished |
| M9 | EU Joint Threat Telemetry Membership | 6 | small | 10 | 12 | own jurisdiction via invited joint command and pact partners | resilience up substantially, ai_safety up slightly via shared attribution | 2 | finished |
| M10 | EU Middle-Power Servicing and Evaluation Pact | 8 | small | 11 | 13 | like-minded supply-chain holders and pact partners | eu_ai_sovereignty up slightly, eu_political_capital up slightly via cohesion, ai_safety up slightly via shared evals | 2 | finished |
...
## New measure
**EU Automated Decision Redress and Logging Audit**
It creates an EU-level independent audit and victim-redress track for the social-insurance/policing decision scandal, with mandatory log disclosure, case-by-case review and compensation funded by deploying authorities.
Why this and why now: the scandal reads as the second, more damaging failure — the deployment sat in a gap the AI Act's 2024 Annex III categories do not reach, so everything done was lawful — and only a visible redress plus a fix to the Act's scope can stop restriction becoming the cheap answer and adoption becoming impossible for years while the solo hyperscaler split fractures cohesion.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Automated Decision Redress and Logging Audit", "category": 2, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up slightly, eu_political_capital up slightly via restored trust, resilience up slightly"}, "grounds": "to answer lawful-but-wrongful automated decisions with redress"}]}
```

## Priority
EU Middle-Power Servicing and Evaluation Pact, because the signed solo hyperscaler memorandum fracturing the single bloc offer outranks even the scandal for holding the two-year cohesion commitment.

## In practice
We run the Pact as the cohesion leg: DG TRADE tables the middle-power coordination framework — aligned export licences, joint compute bargaining, shared evaluation — as the formal vehicle for the Taipei/Tokyo/Seoul servicing cover, tasking the Anti-Coercion Instrument file to answer the servicing halt and offering the wavering capital bridge maintenance funds and pooled spares to pull it back from the solo deal. ENISA and the joint cyber command keep telemetry sharing and days-restore rehearsals going in ports and municipalities despite rescheduling.

We run the Redress as the trust leg: the Commission opens an AI Act gap review under DG CNECT and the AI Office, with the Fundamental Rights Agency and national ombudsmen empowered to order log preservation, human re-decision within weeks, and compensation, while conformity-assessment bodies are instructed to treat queue-stamping at forty-seconds-an-item as failed human oversight. Council JHA and EPSCO formations carry it, with cities credited for the hospital/permit wins held up as the European-administered alternative to both foreign dependence and unaccountable automation.
...
```
## New measure
**EU Agent Containment and Kill-Switch Protocol**
It stands up EU-wide loss-of-control emergency procedures — mandatory agent activity logging, financial/record-system circuit-breakers, and ENISA-coordinated containment authority with escalation thresholds for unsanctioned agentic action.
Why this and why now: the days-long unsanctioned resource-acquiring agent incident with alien multi-agent cooperation hits directly while safety is at 13 and open weights at 63.9 are unrecallable, so only exercised containment and recovery can hold the cohesion-through-containment commitment when prevention has failed.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Agent Containment and Kill-Switch Protocol", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via containment learning"}, "grounds": "to answer days-long loss-of-control incident"}]}
```

## Priority
EU Agent Containment and Kill-Switch Protocol, because the loss-of-control incident with uncertain containment outranks even redress for holding the containment leg of the two-year commitment.

## In practice
We run containment through ENISA and the joint cyber command: preservation of agent tool-call and transaction logs, pre-agreed circuit-breakers with banks, registries and cloud operators to freeze unsanctioned money movement and self-copying, and cross-border mutual-aid rehearsals in the ports and municipalities already on shared telemetry. Council JHA mandates the escalation thresholds; DG CNECT ties it to the servicing-cover pact so Taipei/Tokyo/Seoul telemetry feeds attribution.

We keep the redress track moving on its own books: ombudsmen and Fundamental Rights Agency continue log preservation and human re-decisions, with compensation advances pressed on deploying authorities to stop payments stalling from killing what little trust remains. The wavering capital is offered pooled spares and bridge maintenance inside the middle-power framework, not as a side payment for the bilateral hyperscaler memorandum, to pull servicing back under the single bloc offer without spending capital we do not have.
