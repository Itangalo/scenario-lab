...
- characters 1652-3808: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3876-4332: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6860-9079: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M6 | European Public Benefit AI Scale-up | 9 | small | 5 | 8 | own jurisdiction | public_sentiment up, eu_political_capital up, resilience up slightly | 2 | running |
| M7 | EU Supply-Chain Bargain and Rationing Pact | 8 | small | 6 | 9 | US, like-minded supply-chain holders and member states | eu_ai_sovereignty up slightly, eu_political_capital up slightly via cohesion | 2 | running |
...
## New measure
**EU Supply-Chain Bargain and Rationing Pact**
It converts the Union's upstream lithography, optics and materials position into tiered allied access to compute and models, with joint rationing rules for a blockade winter.
This and now because the Taiwan blockade and US tiered-access posture make supply a security question overnight, and without a collective bargain a member-state defection will sell the Union's best lever cheaply; category 8 International coordination and leverage.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Supply-Chain Bargain and Rationing Pact", "category": 8, "size": "small", "finish_turn": 9, "applies_to": "US, like-minded supply-chain holders and member states", "targeted_effect": "eu_ai_sovereignty up slightly, eu_political_capital up slightly via cohesion"}, "grounds": "to hold a common line on chips and models through blockade and export-control escalation"}]}
```

## Priority
M1 InvestAI Gigafactories, because with accelerators blockaded the only sovereignty that still moves this turn is keeping civil, power and EU-sourced retrofit works alive to turn 7 rather than letting sites stall and capital collapse further.

## In practice
We run the blockade response through the Competitiveness and Foreign Affairs Councils together under the Anti-Coercion Instrument logic: DG TRADE and DG CNECT table a single EU offer — continued serviced upstream supply and coordinated export-control alignment — for volume-licence US accelerator and model access plus joint stockpile and allocation rules, with Taiwan, Japan and Korea in the room. The new US administration is met in the Trade and Technology Council, not bilaterally, to deny a divide-and-rule defection premium.

At home we stretch M1/M2 with reprogrammed STEP and EIB money for grid connection and non-US equipment, accelerate M6 public-benefit deployments that run on European-hosted or lower-compute models to show productivity without new chips, and direct ENISA and the Shield triage cell to re-tune for blockade-era cyber risk while Commission liaison teams offer the defecting capital a funded share of the ration rather than a solo hyperscaler deal.
...
```
## New measure
None this turn, while we wait for political capital to rebuild above paralysis and for the Washington tiered-access channel to answer before spending on another instrument.

## Store changes
No changes.

## Priority
M7 EU Supply-Chain Bargain and Rationing Pact, because holding one common ration and one upstream offer is the only thing preventing a solo-defection cascade that would sell lithography leverage cheaply while halls sit empty.

## In practice
We keep the blockade response in Trade and Competitiveness Council joint session under Anti-Coercion logic: DG TRADE holds the single offer — serviced lithography/optics/materials and aligned controls for volume licences and joint stockpiling — with Taipei, Tokyo and Seoul at the table, and refuses bilateral side-deals. Commission liaison in the defecting capital offers a funded, enforceable share of any US ration plus STEP/EIB grid-connection money, in exchange for pausing the solo hyperscaler MOU.

At home we sweat existing books, not new ones: reprogrammed STEP/EIB keeps M1 civil and power works alive without new commitments, DG CNECT pushes M6 public-benefit deployments that run on European-hosted lower-compute models to bank productivity and sentiment, and ENISA plus the Shield cell retune deferred water/energy nodes for probe persistence with parts-available hardening. No new fund is tabled this turn because with capital at 14 another launch would stall everything.
