...
- characters 106-13433: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13476-13668: `{{metrics_json}}` from the run's live metric values
- characters 13761-15045: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15089-17324: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17380-18546: `{{notepad}}` from the Game Master's notepad, carried across turns
...
PORTFOLIO CHARGE t2: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M3 EU Critical Systems Shield −3, M4 EU Lithography Leverage Compact −2, priority −1 = −12
PROPOSAL BONUS: EU Lithography Leverage Compact (cat 8, small) +2 — answers servicing curb pressure on Netherlands last turn, leverage pooling directly responsive but small and own-jurisdiction only
LEGITIMACY LENDS: capital 39, sentiment 39 -> none, capital not lower
SOVEREIGNTY: 19 last turn, no capacity event, Gigafactories in flight +0, capability rose 3.0 −1 = 18

## Emerging developments (tracked)

- `emergent_ot_insurance_crunch` -- first noted turn 2, listed in 2 turn(s) so far: A major non-EU insurer reprices operational-technology cyber cover after the grid-mapping disclosures, sharply raising premiums for unsegmented utilities and forcing unplanned retrofit spending fights onto municipal budgets.
- `emergent_copycat_ot_probing` -- first noted turn 2, listed in 2 turn(s) so far: Copycat probing kits derived from the Mythos-class tooling circulate on underground forums, prompting unaffiliated actors to re-target EU water and port OT systems disclosed in press coverage.
