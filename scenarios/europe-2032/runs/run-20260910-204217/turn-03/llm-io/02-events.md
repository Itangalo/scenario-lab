...
- characters 377-1661: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1705-3940: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3996-5162: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5225-28801: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29156-29220: `{{event_history}}` from the run's own event record
...
- `emergent_ot_insurance_crunch` -- first noted turn 2, listed in 1 turn(s) so far: A major non-EU insurer reprices operational-technology cyber cover after the grid-mapping disclosures, sharply raising premiums for unsegmented utilities and forcing unplanned retrofit spending fights onto municipal budgets.
- `emergent_copycat_ot_probing` -- first noted turn 2, listed in 1 turn(s) so far: Copycat probing kits derived from the Mythos-class tooling circulate on underground forums, prompting unaffiliated actors to re-target EU water and port OT systems disclosed in press coverage.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, supply_chain_coercion
