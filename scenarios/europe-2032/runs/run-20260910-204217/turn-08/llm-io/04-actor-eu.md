...
- characters 453-1644: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1687-3804: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4152-4620: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7589-9859: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_graduate_protest_wave (emergent event):** Coordinated graduate-led protests across several EU university cities block data-centre grid connections and occupy employment-service offices, demanding hiring guarantees rather than retraining stipends.
...
| M9 | EU Entry-Level Transition Guarantee | 7 | small | 7 | 10 | own jurisdiction | resilience up moderately via social absorption, public_sentiment up moderately, eu_political_capital up slightly if visible | 2 | running |
...
## New measure
None this turn.
We hold new instruments while capital sits at 16 and sentiment at 12, because adding cost now would break delivery of the Guarantee that protests directly target.

## Store changes
No changes.

## Priority
EU Entry-Level Transition Guarantee, and not the Embodied-AI Shield, because graduate occupations of employment offices demanding hiring guarantees over stipends outrank husbanding sites and spares.

## In practice
We run M9 as the protest answer through EPSCO and DG EMPL: ESF+ repurposed tranches to PES offices under occupation, joint tables with student unions and rectors to convert six-month stipends into hiring-linked transition posts in municipalities, health-admin and cyber-hygiene, with automating-employer co-funding enforced via social-partner agreements. First payments go to university cities where blockades sit, explicitly tied to unblocking grid-connection works.

We keep M8 alive on procurement and certification only — DG GROW/CNECT IPCEI track for EU-hosted control, no new money — and let the four factory sites stay at fences and grid queues. UCPM Recovery Corps stay on water/port manual fallback with pre-stocked spares. No retaliation on Washington controls; we quietly use remaining volume licences and diversify spares.
