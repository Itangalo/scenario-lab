...
- characters 107-13404: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13447-13636: `{{metrics_json}}` from the run's live metric values
- characters 13729-15074: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15118-17361: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17417-18625: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M12 | EU Distressed Site Sovereignty Hold | 2 |
| M13 | EU Visible Benefit Adoption Drive | 2 |
...
- `emergent_therapy_licence_freeze` -- first noted turn 13, listed in 1 turn(s) so far: US volume-licence window for tailored-therapy queries is suspended for several weeks after a safety-flag dispute, forcing hospitals in Leuven, Lyon and Warsaw to triage already-prescribed courses.
- `emergent_clinical_quota_triage` -- first noted turn 13, listed in 1 turn(s) so far: A foreign frontier provider abruptly throttles EU clinical inference quotas mid-course, forcing hospitals in Turin and Utrecht to publicly triage tailored therapies and triggering cross-border patient transfers.
...
## Notepad
PORTFOLIO CHARGE t13: EU Distressed Site Sovereignty Hold −2, EU Visible Benefit Adoption Drive −2, priority void as capital below 20, charged −4 = −4
PROPOSAL BONUS: none — adoption_success is positive window not negative shock in last three turns, no severe harm answered
LEGITIMACY LENDS: capital 6, sentiment 11 -> +2, capped below sentiment
SOVEREIGNTY: 9 last turn, no category 4 finish, no category 4 in flight, no capacity event, capability rose 1.1 −0 = 9
