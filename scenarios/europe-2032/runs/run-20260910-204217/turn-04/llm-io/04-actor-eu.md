...
- characters 454-1969: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2012-3990: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5162-5656: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7817-9998: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_copycat_ot_probing (emergent event):** Copycat probing kits derived from the Mythos-class tooling circulate on underground forums, prompting unaffiliated actors to re-target EU water and port OT systems disclosed in press coverage.
...
| M4 | EU Lithography Leverage Compact | 8 | small | 2 | 5 | own jurisdiction | eu_ai_sovereignty up moderately, eu_political_capital up slightly if successful | 2 | running |
| M5 | EU Automation Transition Fund | 7 | small | 3 | 6 | own jurisdiction | resilience up moderately via social absorption, public_sentiment up, eu_political_capital up slightly if visible | 2 | running |
...
## New measure
**EU Automation Transition Fund**
Creates an EU-coordinated wage-insurance, retraining and hiring-support fund targeted at entry-level white-collar displacement, co-financed by ESF+ repurposing and a levy-linked contribution from large deploying employers.
This is Labour and social protection because measurable non-replacement of graduate hiring in law, accountancy, software and customer operations directly threatens public sentiment and political capital needed for the two-year commitment, and a Union transition instrument must be started now before anxiety hardens into hostility to all AI build-out.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Automation Transition Fund", "category": 7, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately via social absorption, public_sentiment up, eu_political_capital up slightly if visible"}, "grounds": "responding to measurable AI job losses and graduate hiring freeze this turn"}]}
```

## Priority
EU Automation Transition Fund, because the labour-displacement reports this turn outrank even the Shield and Compact — without visible income and retraining cover, resentment will block grid shutdowns, data-centre siting and any leverage negotiation.

## In practice
We launch the Fund via EPSCO Council with DG EMPL lead, repurposing uncommitted ESF+ and RRF loans for rapid wage-insurance pilots in the worst-hit graduate labour markets, paired with retraining vouchers delivered through national public employment services. Large deployers get contribution credits for retained retrained hires, to blunt employer resistance without opening a new treaty-level tax fight.

In parallel we keep the in-flight book alive without new money: ENISA pushes draft network codes toward delegated-act status while Energy Council brokers winter-after shutdown scheduling, and DG TRADE runs the joint licensing desk as the single servicing-decision venue to hold The Hague from further bilateral concessions. Gigafactory ground works continue tied to DSO cooperation, but no new disbursement is forced this winter.
...
```
## New measure
**OT Rapid Patch and Swarm-Detection Rollout**
Deploys the breakthrough automated patching and swarm-behaviour detection to EU water, port and interconnector OT systems exposed by prior disclosure and now re-targeted by copycat kits.
This and now because copycat probing is hitting exactly the mapped exposure while a defensive window is briefly open, and because eval anomalies mean we must harden without waiting for capability clarity; as Preparedness and resilience it directly serves the harden-to-survive leg of the two-year commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "OT Rapid Patch and Swarm-Detection Rollout", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via detection"}, "grounds": "answering copycat OT probing with new defensive breakthrough"}]}
```

## Priority
OT Rapid Patch and Swarm-Detection Rollout, because live re-targeting of water and ports outranks the slower sovereignty and transition builds — without stopping a cascade now, there is no capital left for anything else.

## In practice
We task ENISA with emergency certification of the breakthrough patching and swarm-detection tooling and push it via the Shield network codes as an accelerated implementing act, funded by reprioritised Digital Europe cyber funds for municipal utilities that stalled on cost. The two compromised TSOs and the two states that already bought detection become deployment hubs, with auditors verifying live segmentation windows scheduled by the Energy Council for spring low-demand periods.

In parallel we hold the rest of the book without new spend: DG EMPL expands the wage-insurance pilots only to the named Paris/Frankfurt graduate markets using existing ESF+ repurposing, and DG TRADE keeps the joint licensing desk as the sole servicing venue to prevent further bilateral leakage while gigafactory sites move from fencing to grid-connection contracting. No new compute money is committed this turn to stay within collapsing political capital.
