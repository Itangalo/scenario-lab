...
- characters 106-13366: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13409-13601: `{{metrics_json}}` from the run's live metric values
- characters 13694-14995: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15039-17385: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17441-18025: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M9 | EU Entry-Level Transition Guarantee | 2 |
...
## Notepad
PORTFOLIO CHARGE t7: M8 European Embodied-AI Industrial Shield −2, M9 EU Entry-Level Transition Guarantee −2, priority −1 = −5
PROPOSAL BONUS: EU Entry-Level Transition Guarantee (cat 7, small) +2 — labour_displacement landed this turn severe on graduates plus sentiment at 18, this answers it directly, small size limits value
LEGITIMACY LENDS: capital 22, sentiment 18 -> none, capital higher
SOVEREIGNTY: 16 last turn, M1 InvestAI Gigafactories finishes t7 +3, M8 in flight +0, export_control_escalation t7 −2, capability rose 6.0 −1 = 16
## Store changes
```json
{"store": []}
```
