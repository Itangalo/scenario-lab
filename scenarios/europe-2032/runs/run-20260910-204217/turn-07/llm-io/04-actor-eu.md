...
- characters 455-1756: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1800-4146: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6128-6596: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9340-11669: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_copycat_water_port_outage (emergent event):** Unaffiliated copycats using circulating Mythos-derived kits trigger a temporary loss of service at one or more municipal water utilities or port terminals whose patch windows slipped, forcing short-term manual operations without a cascading grid failure.
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | European Embodied-AI Industrial Shield | 4 | small | 6 | 9 | own jurisdiction | eu_ai_sovereignty up moderately, resilience up slightly, public_sentiment up slightly if jobs protected | 2 | running |
...
| M4 | EU Lithography Leverage Compact | 8 | small | 2 | 5 | own jurisdiction | eu_ai_sovereignty up moderately, eu_political_capital up slightly if successful | 2 | finished |
| M5 | EU Automation Transition Fund | 7 | small | 3 | 6 | own jurisdiction | resilience up moderately via social absorption, public_sentiment up, eu_political_capital up slightly if visible | 2 | finished |
| M6 | OT Rapid Patch and Swarm-Detection Rollout | 6 | small | 4 | 5 | own jurisdiction | resilience up moderately, ai_safety up slightly via detection | 2 | finished |
| M7 | Municipal Continuity and Recovery Corps | 6 | small | 5 | 6 | own jurisdiction | resilience up strongly, eu_political_capital up slightly if visible, public_sentiment up slightly | 2 | finished |
...
## New measure
**European Embodied-AI Industrial Shield**
Builds an EU-anchored robotics stack — pooled procurement for EU manufacturers, secured component supply, and EU-hosted control models with safety certification — to keep deployment from becoming pure dependence on Chinese hardware and American models.
This and now because embodied AI has reached commercial deployment on top of a capability jump and automated attacks, directly threatening the industrial base we still lead in while sovereignty sits at 15 and sentiment at 23; as Sovereignty and industrial capacity it carries the rebuild-capacity leg of our commitment forward without opening a parallel compute programme alongside the Gigafactories.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "European Embodied-AI Industrial Shield", "category": 4, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up moderately, resilience up slightly, public_sentiment up slightly if jobs protected"}, "grounds": "answering embodied AI deployment hitting EU industrial base"}]}
```

## Priority
European Embodied-AI Industrial Shield, because the robot deployment plus Taiwan precursor threatens irreversible loss of our industrial core while M1 gigafactories remain paper — if the factory floor flips now there will be no sovereign demand left to compute for.

## In practice
We push the Shield through DG GROW and DG CNECT under the Net-Zero Industry Act and Chips Act precedents: an IPCEI for robotics and actuators, coordinated procurement by automotive, logistics and hospital groups, and a certification track via EU standardization bodies requiring EU-hosted control-model inference for critical-site robots. Funding repurposes unspent InvestAI and Innovation Fund envelopes, not fresh fiscal asks, to spare political capital at 19.

In parallel we hold M1 alive on permitting only — Competitiveness Council keeps grid-connection queues moving for the four to five sites, using the US moratorium window to quietly court talent — while the Recovery Corps teams from last turn stay deployed via UCPM to restore municipal, water and port systems hit in the automated wave. No new export-control fight is opened while Taiwan insurance spikes; we pre-stock spares instead.
...
A Category 7 Labour and social protection backstop that funds paid retraining, wage-insurance and public-sector transition posts for displaced graduates and admin/customer-ops workers, tied to automating employers and delivered through PES and municipal corps.
This and now because measurable graduate and white-collar displacement plus sentiment at 18 threatens cohesion and any further capacity build, while investment collapse makes private hiring freeze durable; as social absorption it carries the second leg of our two-year commitment forward when resilience alone cannot.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Entry-Level Transition Guarantee", "category": 7, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately via social absorption, public_sentiment up moderately, eu_political_capital up slightly if visible"}, "grounds": "answering labour displacement and sentiment collapse"}]}
```

## Priority
EU Entry-Level Transition Guarantee, and not the Embodied-AI Shield, because without stemming graduate hiring collapse and hostility at 18 no sovereignty or hardening measure remains enforceable.

## In practice
We run the Guarantee through EPSCO and the ESF+ / SURE precedent: DG EMPL channels repurposed cohesion and unspent RRF loans to public employment services for 6-month paid reskilling plus municipal, health-admin and cyber-hygiene transition posts, with co-funding levied on large automating employers via social-partner agreements. Member states with graduate unemployment spikes get first tranches; unions get hiring guarantees in exchange for flexibility on restructuring.

We hold M8 alive on procurement and certification only — no new money, IPCEI track via DG GROW/CNECT and CEN-CENELEC for EU-hosted inference — while UCPM Recovery Corps teams stay on water/port manual fallback with pre-stocked spares and paper playbooks. On Taiwan and export controls we take no retaliatory stance this turn, quietly diversify spares and use volume-licence access where Washington still offers it, and let Gigafactory sites sit at permitting/grid-queue stage until capital recovers.
