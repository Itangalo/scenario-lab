...
- characters 376-1548: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1591-3622: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3678-4839: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4902-32079: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32434-32718: `{{event_history}}` from the run's own event record
...
- `emergent_copycat_water_port_outage` -- first noted turn 5, listed in 1 turn(s) so far: Unaffiliated copycats using circulating Mythos-derived kits trigger a temporary loss of service at one or more municipal water utilities or port terminals whose patch windows slipped, forcing short-term manual operations without a cascading grid failure.
- `emergent_datacentre_blockade_protests` -- first noted turn 5, listed in 1 turn(s) so far: Coordinated physical protests block two planned gigafactory grid-connection sites, with local councils suspending permits over power prices and water use through winter.
...
- Turn 1 (5 turn(s) ago): cyber_test_shot, supply_chain_coercion
- Turn 3 (3 turn(s) ago): labour_displacement
- Turn 4 (2 turn(s) ago): cyber_defence_breakthrough, eval_anomaly_reports, emergent_copycat_ot_probing
- Turn 5 (1 turn(s) ago): cyber_major_incident, election_retrenchment
