...
- characters 105-13817: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13860-14052: `{{metrics_json}}` from the run's live metric values
- characters 14145-15317: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15360-17391: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17447-18608: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M8 | European Embodied-AI Industrial Shield | 2 |

     **so 2 measures are in flight and they come to −5 `eu_political_capital` this turn.** That figure is the `cost_per_turn` column of the rows above, added up, and nothing else: the named priority's −1 below is charged on top of it, and is not in it. Your charge line carries one term per measure -- 2 of them before the priority. **A measure is charged in every turn up to but not including its finishing turn.**
...
PORTFOLIO CHARGE t5: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M5 EU Automation Transition Fund −2, M7 Municipal Continuity and Recovery Corps −2, priority −1 = −11
PROPOSAL BONUS: Municipal Continuity and Recovery Corps (cat 6, small) +3 — cyber_major_incident landed this turn severe and this answers recovery directly, small size limits value
LEGITIMACY LENDS: capital 17, sentiment 23 -> +2
SOVEREIGNTY: 16 last turn, no capacity event, Gigafactories in flight +0, capability rose 3.0 −1 = 15

## Emerging developments (tracked)

- `emergent_copycat_water_port_outage` -- first noted turn 5, listed in 2 turn(s) so far: Unaffiliated copycats using circulating Mythos-derived kits trigger a temporary loss of service at one or more municipal water utilities or port terminals whose patch windows slipped, forcing short-term manual operations without a cascading grid failure.
- `emergent_datacentre_blockade_protests` -- first noted turn 5, listed in 2 turn(s) so far: Coordinated physical protests block two planned gigafactory grid-connection sites, with local councils suspending permits over power prices and water use through winter.
...
5. The 2028 election fired last turn: read the winner from the event record and set the standing record to the matching posture (`CONSOLIDATION`, `ALLIANCE` or `RETRENCHMENT`) with an `update` entry. From here it stands: it may not be dropped, reinterpreted or replaced later in the run.
...
## Notepad
PORTFOLIO CHARGE t6: M1 InvestAI Gigafactories −3, M8 European Embodied-AI Industrial Shield −2, priority −1 = −6
PROPOSAL BONUS: European Embodied-AI Industrial Shield (cat 4, small) +4 — embodied_ai_deployment landed this turn severe on industrial base plus capability_jump, this answers it directly, small size limits value
LEGITIMACY LENDS: capital 19, sentiment 23 -> +2 capped by prior terms
SOVEREIGNTY: 15 last turn, M2 Tech sovereignty package finishes t6 +4, no capacity event counted beyond embodied pressure sized in narrative, M1 in flight +0, capability rose 5.0 −1 = 16

## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 election delivered anti-AI administration, retrenchment takes effect turn 6"}]}
```
```
