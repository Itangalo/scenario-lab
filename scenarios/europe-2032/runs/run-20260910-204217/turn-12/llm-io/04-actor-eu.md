...
- characters 452-2880: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2923-4913: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7980-8448: `{{statement_ledger}}` from the actor's live statement ledger
- characters 11802-13843: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_distressed_site_selloff (emergent event):** With EU political capital near zero and four fenced factory lots stalled, a distressed regional government auctions a site and grid allocation to a non-EU hyperscaler for a cut-price sovereign-hosted enclave, fracturing the common procurement line.
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Care-Inference Continuity Guard | 6 | small | 11 | 13 | own jurisdiction | resilience up moderately, public_sentiment up slightly if clinics stay online | 2 | running |
...
| M8 | European Embodied-AI Industrial Shield | 4 | small | 6 | 9 | own jurisdiction | eu_ai_sovereignty up moderately, resilience up slightly, public_sentiment up slightly if jobs protected | 2 | finished |
| M9 | EU Entry-Level Transition Guarantee | 7 | small | 7 | 10 | own jurisdiction | resilience up moderately via social absorption, public_sentiment up moderately, eu_political_capital up slightly if visible | 2 | finished |
| M10 | EU Clinical Benefit Access Programme | 9 | small | 10 | 12 | own jurisdiction | public_sentiment up strongly, resilience up slightly, eu_political_capital up slightly if visible | 2 | finished |
...
## New measure
**EU Care-Inference Continuity Guard**
Keeps the thin clinical rollout alive by hardening power, grid connection and cyber-monitoring for the hospital inference nodes and cross-border remote-access links the cures depend on.
This is Preparedness and resilience work, because with export controls tightening and substation interference rumours spreading, the immediate risk is that first-benefit sites go dark — and with sentiment at 11 and capital at 8 we cannot afford to lose the one visible benefit we have.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Care-Inference Continuity Guard", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up slightly if clinics stay online"}, "grounds": "to keep cure delivery running through grid and cyber interference"}]}
```

## Priority
EU Clinical Benefit Access Programme, because it is the only instrument that can lift sentiment from 11 and buy any capital back — the Guard only matters if clinics are still delivering.

## In practice
We keep M10 as the front line: DG SANTE/HERA bloc procurement, EMA compassionate-use guidance, and EHDS-routed data flows, with per-query licences renegotiated as a Union rather than hospital-by-hospital, and first deliveries ring-fenced to Leuven, Lyon, Turin, Warsaw and Utrecht to show the bridge held.

Around it we stand up the Guard through the Critical Systems Shield operators and ENISA: priority grid-connection for hospital inference nodes, physical security and monitoring for feeding substations, backup routing for audited foreign-API links, and municipal recovery corps shifts to keep primary-care diagnostics online. We ask interior and energy Councils for fast-track protection orders, funded from rescEU and existing shield maintenance, offering wary regional operators liability cover and repair funding to restart slowed works without reviving the old grid-conditionality fight.
...
```
## New measure
**EU Distressed Site Sovereignty Hold**
Freezes the cut-price enclave auction via EU FDI screening and state-aid/energy-allocation review while offering bridge liquidity to the regional government to hold the lot inside common procurement.
This is International coordination and leverage work, because with capital at 9 and four lots stalled the selloff would fracture the Union line permanently, and with bio uplift and eval anomalies breaking we cannot cede the only physical leverage we have left to a foreign enclave; we act now before the auction closes.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Distressed Site Sovereignty Hold", "category": 8, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly via blocking fragmentation, eu_political_capital down slightly if contested"}, "grounds": "to stop enclave selloff fracturing common line"}]}
```

## Priority
EU Distressed Site Sovereignty Hold, because if the auction goes through there is no common capacity or procurement line left to defend — the Guard and clinical access both collapse with it.

## In practice
We trigger Commission FDI Screening Regulation review plus DG COMP and DG ENER grid-allocation state-aid hold on the auction, via College decision and Energy Council backing, pausing signature for examination. DG ECFIN and EIB offer emergency bridge loan against rescEU / shield funds to cover the region's arrears, conditional on no exclusive foreign grid reservation.

Around it we keep M11 running: ENISA and grid operators maintain hospital inference-node power and backup routing for audited foreign queries under volume licences, and DG SANTE holds bloc allocation ring-fenced to the five cities. We publicly link the hold to keeping cures flowing — no enclave carve-out of power that darkens clinics — to contain Turin hardening and protest-city anger.
