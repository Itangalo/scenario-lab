...
- characters 12778-12968: `{{metrics_json}}` from the run's live metric values
- characters 13061-14645: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14689-16750: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16806-17104: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Metrics
```json
{"ai_capability": 63.5, "openweight_capability": 62.5, "ai_safety": 7.0, "resilience": 34.0, "eu_ai_sovereignty": 8.0, "eu_political_capital": 0.0, "public_sentiment": 0.0}
```
## Narrative
### Blind reasoning
...
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 2, sentiment 0 -> none, sentiment not above capital before update
SOVEREIGNTY: 10 last turn, no category 4 finish or in-flight, export_control_escalation t13 −2, capability rose 0.5 −0 = 8
## Store changes
```json
{"store": []}
```
