...
- characters 454-1242: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1286-3065: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6402-6859: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8627-10324: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Loss-of-Control Containment Protocol | 6 | small | 2 | 4 | own jurisdiction | resilience up, ai_safety up slightly via containment learning | 2 | finished |
| M5 | EU Common AI Supply Compact | 8 | small | 3 | 5 | own jurisdiction | eu_ai_sovereignty stabilize, eu_political_capital up slightly if holds | 2 | finished |
...
## New measure
None this turn, while political capital sits at 7 and public sentiment at 19 with blockades still turning back siting work.

We cannot credibly launch a new build or benefit offer until the Taiwan-precursor supply risk is priced and a minimal siting consensus is recovered; this half-year we hold M1 permits and grid reservations warm, keep NIS2/CER containment enforced, and use the notification file to prepare a health-jobs-local-benefit siting package for when capital allows.

## Store changes
No changes.

## Priority
M1 InvestAI Gigafactories, because if permits and grid reservations lapse now there is no EU-anchored capacity left to make socially licensable when the US retrenchment window and Taiwan supply risk widen.

## In practice
We keep M1 alive on life-support through the Competitiveness Council and DG CNECT: extend gigafactory permits, keep paid grid reservations current, no new procurement without private co-financing. We do not force surveys through blockades; we log delays, pay mayors in continued reservation fees, and task the notification file with drafting the conditional siting offer — local health funding, hiring guarantees, energy-bill relief — to be tabled once sentiment and capital move.

Containment stays enforced without new law: ENISA's reporting cell, binding segmentation, credential resets and manual fallbacks in energy, ports and hospitals continue, with the autumn exercise rhythm carried into spring. On Taiwan, we take no export-control move this turn; we ask DG TRADE and the Anti-Coercion watch function to map chip, lithography spares and shipping-insurance exposure for the next three turns so any future compact is not blind.
...
```
## New measure
**EU Supply-Chain Coalition for Compute Access**
Coordinates export-licence alignment and joint compute bargaining with middle powers holding supply-chain chokepoints, using the EU upstream position to secure access during the blockade.
This turn because the Taiwan blockade makes every AI question a security question and the US-China deal will be written without us unless we bring collective leverage now; it is also the only build that does not require forcing siting through blockades while sentiment sits at 18.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Supply-Chain Coalition for Compute Access", "category": 8, "size": "small", "finish_turn": 9, "applies_to": "middle-power coalition", "targeted_effect": "eu_ai_sovereignty stabilize/up slightly, eu_political_capital up if holds"}, "grounds": "blockade plus US-China agreement window demands joint leverage"}]}
```

## Priority
EU Supply-Chain Coalition for Compute Access, because with capital at 3 and no measures in flight it is the only move that secures access, uses our upstream leverage, and can rebuild capital without new siting fights.

## In practice
We task the Commission VP for trade with a Council-mandated coalition cell, using the Anti-Coercion Instrument base and DG TRADE exposure maps to align export licences and pool bargaining over compute, spares and shipping insurance with Japan, Korea and the Netherlands-node partners. No new export ban this turn; we offer coordinated licensing and mutual access guarantees.

At home we keep M1 permits warm on life-support via the Competitiveness Council and keep the Critical Systems Shield containment enforced via ENISA and NIS2/CER, while letting the automated-decision scandal be handled as enforcement failure under the AI Act — audits, logging review, human-oversight remediation — to avoid a new legislative fight we cannot fund. The defecting member state's side-deal is contained by folding its terms into the coalition offer rather than punishing it publicly.
