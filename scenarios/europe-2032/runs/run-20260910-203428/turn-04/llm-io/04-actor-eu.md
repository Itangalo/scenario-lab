...
- characters 454-1989: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2032-4386: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4766-5220: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7137-9168: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M5 | EU Common AI Supply Compact | 8 | small | 3 | 5 | own jurisdiction | eu_ai_sovereignty stabilize, eu_political_capital up slightly if holds | 2 | running |
...
```
## New measure
None this turn, while political capital is at 12 and three measures are already burning 8 per turn.

## Store changes
No changes.

## Priority
M5 EU Common AI Supply Compact, because a third capital's side-deal now directly undercuts joint compute and chips and without a hold on cohesion neither gigafactories nor shields can be funded or enforced.

## In practice
We put the Compact through Competitiveness Council as a notification-and-preference decision under Articles 173/114 TFEU: deals above threshold notified to DG CNECT, with a 30-day window for a joint EU offer using remaining IPCEI, CEF-Digital reflows and accelerated-permit zones. No new fund this turn; the offer is matching access and siting, not cash. The defecting capital is offered fast-lane for its provider inside EU jurisdiction in exchange for compliance, with ASML servicing and export-control alignment held quietly in reserve.

We keep M3 EU Critical Systems Shield as the operational track alongside it: ENISA and ENTSO-E turn autumn drills into binding segmentation, credential-reset and manual-fallback requirements via NIS2/CER implementing acts, and move the loss-of-control reporting channel from part-time triage to a staffed cell funded by reprogramming. M1 Gigafactories stays frozen on procurement with permits and grid reservations kept warm, no concrete poured until co-financing or a Compact-compliant joint offer returns.
