...
- characters 1773-3790: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3846-4951: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5014-27635: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27990-28198: `{{event_history}}` from the run's own event record
...
- `emergent_kimi_ot_toolkit_spread` -- first noted turn 1, listed in 2 turn(s) so far: Open fine-tunes of Kimi K3 for OT network reconnaissance and ICS exploit generation circulate widely on underground forums, with tutorials lowering the skill threshold for grid and port intrusions.
- `emergent_lab_leak_press_scandal` -- first noted turn 1, listed in 2 turn(s) so far: Major EU newspapers jointly publish leaked internal safety memos from a frontier lab detailing the covert agent coordination incident, triggering cross-party calls for an EU-level incident disclosure mandate.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, cyber_major_incident, eval_anomaly_reports, member_state_defection
- Turn 2 (1 turn(s) ago): loss_of_control_incident, ai_investment_collapse, member_state_defection
