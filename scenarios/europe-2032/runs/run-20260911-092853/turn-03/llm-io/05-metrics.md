...
- characters 106-13081: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13124-13316: `{{metrics_json}}` from the run's live metric values
- characters 13409-14921: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14965-16628: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16684-17598: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Grid Shield Hardening Programme | 3 |
| M5 | EU Bio-Cyber Shield Surge | 2 |
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU AI Evaluation and Safety Institute −3, EU Grid Shield Hardening Programme −3, priority −1 = −13
PROPOSAL BONUS: EU Grid Shield Hardening Programme (cat 6, large) +2 — export_control_escalation this turn tightens supply and grid intrusions last turn exposed undetected pre-positioning this answers directly
LEGITIMACY LENDS: capital 35, sentiment 36 -> +1
SOVEREIGNTY: 20 last turn, no finish, Gigafactories in flight +0, export_control_escalation t2 −2, capability rose 1.5 −0 = 18

## Emerging developments (tracked)

- `emergent_siting_backlash_freeze` -- first noted turn 2, listed in 2 turn(s) so far: Coordinated municipal opposition delays AI factory site selection as local councils cite grid strain and evening-news coverage linking open models to grid intrusions, forcing temporary permitting freezes in one candidate region.
...
## Metrics
```json
{"ai_capability": 56.5, "openweight_capability": 49.5, "ai_safety": 28.0, "resilience": 35.5, "eu_ai_sovereignty": 16.0, "eu_political_capital": 19.0, "public_sentiment": 32.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M4", "fields": {"finish_turn": 7}, "grounds": "named priority"}]}
```
```
