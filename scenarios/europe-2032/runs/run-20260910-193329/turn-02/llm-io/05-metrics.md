...
- characters 105-12943: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12986-13178: `{{metrics_json}}` from the run's live metric values
- characters 13271-14739: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14782-17048: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17104-18519: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M3 | EU Critical Shield Hardening Programme | 3 |
| M4 | EU Essential Services Continuity Stack | 2 |
...
PORTFOLIO CHARGE t1: Gigafactories −3, Tech sovereignty package −3, EU Critical Shield Hardening Programme −3, priority −1 = −10
PROPOSAL BONUS: EU Critical Shield Hardening Programme (cat 6, large) +4 — autumn swarm intrusion mapped EU grids and staged tooling this turn and this answers it directly
LEGITIMACY LENDS: capital 44, sentiment 42 -> none, capital higher
SOVEREIGNTY: 22 last turn, eu_access_secured t1 +2, capability rose 1.5 no penalty = 24
Emerging developments (tracked)
- `emergent_bio_data_closure` -- first noted turn 1, listed in 1 turn(s) so far: Major European scientific publishers and universities jointly mandate AI-provenance disclosure and restricted licensing for frontier biological datasets after the phage designs, temporarily constricting open genome-model training data.
- `emergent_datacentre_energy_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Major US cloud provider announces emergency power purchase and grid-interconnection queue that triggers municipal backlash and data-centre siting moratorium proposals in several EU member states.

## Emerging developments (tracked)

- `emergent_datacentre_energy_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Major US cloud provider announces emergency power purchase and grid-interconnection queue that triggers municipal backlash and data-centre siting moratorium proposals in several EU member states.
...
**emergent_bio_data_closure (emergent event):** Major European scientific publishers and universities jointly mandate AI-provenance disclosure and restricted licensing for frontier biological datasets after the phage designs, temporarily constricting open genome-model training data.
...
It moves hospitals, ministries and firms cut off from the leading model onto EU-hosted open-weight and bridge models with pooled evaluation, while deploying the new defensive patching/detection tooling. This now because the cutoff made dependence operational overnight and the investment collapse plus bio-data closure force a sovereign fallback that can actually run on infrastructure we control.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Continuity Stack", "category": 9, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly, resilience up, public_sentiment up slightly"}, "grounds": "frontier access denial requires immediate sovereign fallback"}]}
```

## Priority
EU Essential Services Continuity Stack, because the cutoff is a live denial hitting hospitals and ministries now and outranks even Shield hardening this turn while Gigafactories continue in background.

## In practice
We task DG CNECT and ENISA with a Health and Competitiveness Council mandate: emergency procurement of EU-hosted inference for essential services, funded by reprogrammed Digital Europe and EIB liquidity, using Gigafactory pilot capacity and EuroHPC. HERA and member-state health ministries lead substitution; resistance from vendors locked to US APIs is met with temporary compliance forbearance and co-financed migration.

The Critical Shield (M3) continues joint TSO-port-water audits and exercises, now integrating the cyber-defence breakthrough tooling for swarm detection and automated patching. Gigafactories (M1) and Tech Sovereignty Package (M2) are repriced for the investment collapse: we protect sites with grid connection and permits, use ASML and equipment leverage quietly, and treat written access terms as breached — triggering Anti-Coercion screening without yet invoking it.
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M4", "fields": {"finish_turn": 3}, "grounds": "named priority"}]}
```
```
