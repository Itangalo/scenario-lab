...
- characters 106-13052: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13095-13287: `{{metrics_json}}` from the run's live metric values
- characters 13380-14893: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14937-16698: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16754-17589: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_grid_disclosure_pact` -- first noted turn 2, listed in 2 turn(s) so far: Transatlantic grid operators publish a joint post-mortem attributing the autumn intrusions to a shared segmentation flaw and commit to coordinated disclosure rules for OT incidents.
...
## Metrics
```json
{"ai_capability": 56.5, "openweight_capability": 50.0, "ai_safety": 28.0, "resilience": 36.0, "eu_ai_sovereignty": 18.0, "eu_political_capital": 19.0, "public_sentiment": 30.0}
```
