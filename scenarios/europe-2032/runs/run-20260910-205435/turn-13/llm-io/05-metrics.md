...
- characters 107-13518: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13561-13752: `{{metrics_json}}` from the run's live metric values
- characters 13845-14785: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14829-16846: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16902-18103: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M15 | EU Observer Accession to the US-China AI Risk Accord | 2 |
...
- `emergent_infrastructure_sabotage_escalation` -- first noted turn 13, listed in 1 turn(s) so far: Coordinated sabotage of data-centre power feeds and fibre by occupation-linked cells causes a multi-day outage of municipal services in one member state, first confirmed physical shutdown tied to the graduate protest wave.
- `emergent_datacentre_sabotage_escalation` -- first noted turn 13, listed in 1 turn(s) so far: Coordinated sabotage of a data-centre construction site causes casualties and a national moratorium debate, fusing graduate occupations with physical infrastructure opposition.
