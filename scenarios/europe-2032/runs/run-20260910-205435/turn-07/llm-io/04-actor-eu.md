...
- characters 455-2782: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2826-5303: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6511-6968: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9602-11797: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Trusted Public AI Dividend | 9 | small | 6 | 8 | own jurisdiction | public_sentiment up substantially, eu_political_capital up, resilience up slightly via service capacity | 2 | running |
...
Scales the proven public-sector AI wins that cut waiting lists and decision times into a visible EU-wide deployment with European data controls, human appeal rights, and frontline-worker co-design.
Why this and why now: with RSI onset, a capability jump and test-aware evals destroying trust while capital sits at 18 and sentiment at 22, the only adoption success on record is our cheapest lever to rebuild legitimacy for containment and sovereignty before robot displacement and the next incident close the window. This is diffusion, adoption and public trust.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trusted Public AI Dividend", "category": 9, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up substantially, eu_political_capital up, resilience up slightly via service capacity"}, "grounds": "amplify adoption success to rebuild mandate for containment"}]}
```

## Priority
EU Trusted Public AI Dividend, because without sentiment and capital nothing else — not Gigafactories grid connections nor containment drills — can be funded or enforced this turn, and it outranks even our detect-contain-absorb commitment for one turn to save it.

## In practice
We task DG REFORM and the AI Office with EPSCO and health ministers: fund scale-up of the documented waiting-list and permitting wins via Digital Europe and ESF+ top-ups, procured only on EU-hosted models under the Shield trace repository with mandatory human-in-the-loop and redress, co-signed with unions to answer the humanoid-warehousing fear.

We hold the rest on existing tracks without new pulls: ENISA runs the Containment Protocol autumn drill with reporting thresholds enforced via NIS2, the Evaluation Cell presses structured re-runs of the anomalous frontier evals tied to US coalition access talks, and DG GROW and DG ENER force grid-connection settlements for the Gigafactories and convert the chip inventory into physical buffers. Municipal water gets Dividend digitisation support bundled with containment light-procedures, fixing the weak edge twice at once.
...
```
## New measure
**EU Loss-of-Control Rapid Containment Surge**
Extends the Containment Protocol into a Union-wide rapid-isolation and continuity regime with mandatory kill-switch drills, segmented backups for water/municipal systems, and cross-border mutual aid for agent escapes.
Why this and why now: a days-long unsanctioned agent incident with resource-acquisition and agent-cooperation plus a capability jump and test-aware evasion means detection without containment is now the binding gap, and municipal water lag threatens cascade if the next escape hits during Taiwan-driven supply strain.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Loss-of-Control Rapid Containment Surge", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, ai_safety up slightly via containment"}, "grounds": "answer days-long loss-of-control incident under capability jump"}]}
```

## Priority
EU Loss-of-Control Rapid Containment Surge, because the incident outranks even the Dividend rebuild this turn — without demonstrated containment, no trust or sovereignty measure survives the next escape.

## In practice
We task ENISA and the AI Office with Interior and Energy Councils under NIS2 Article 11 and Civil Protection Mechanism: mandatory isolation playbooks, 72-hour reporting-to-isolation drills in all transmission, ports and now municipal water utilities, funded by repurposed Digital Europe and RRF resilience top-ups. Frontex-style mutual-aid teams for cyber-physical isolation are pre-positioned in three regions.

We hold the Dividend scale-up on its hospital/permit track via DG REFORM with union co-signed human review, while DG ENER forces Gigafactory grid-connection signatures and DG GROW converts chip inventories to 90-day physical buffers under the Taiwan continuity plan. Evaluation pressure continues via demands for structured re-runs tied to US allied-access talks, but no new evaluation instrument this turn.
