...
- characters 105-13486: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13529-13721: `{{metrics_json}}` from the run's live metric values
- characters 13814-15515: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15558-18184: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18240-19196: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M5 | High-Risk Enforcement Surge | 2 |
| M6 | Taiwan Strait Supply Continuity Plan | 2 |
...
- `emergent_fraud_wave` -- first noted turn 4, listed in 1 turn(s) so far: A coordinated wave of AI-generated voice and document fraud hits welfare offices and regional banks in several member states, using openly available models to impersonate claimants and officials at scale.
...
## Metrics
```json
{"ai_capability": 64.0, "openweight_capability": 57.0, "ai_safety": 33.0, "resilience": 41.0, "eu_ai_sovereignty": 18.0, "eu_political_capital": 31.0, "public_sentiment": 30.0}
```
