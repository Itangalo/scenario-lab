...
- characters 455-2215: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2258-4449: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6201-6673: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10419-12841: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_graduate_protest_wave (emergent event):** Coordinated graduate-led protests across several EU capitals fuse hiring collapse with data-centre opposition, occupying university buildings and disrupting council meetings to demand hiring quotas and a moratorium on public-sector automation.
...
- `two_year_commitment` (commitment): A Union that keeps essential services running and social consent holding through AI-driven shocks
...
|---|---|---|---|---|---|---|---|---|---|
| M13 | EU Automation Shock Absorber for Displaced Entrants and Embodied-AI Exposed Workers | 7 | small | 11 | 14 | own jurisdiction | public_sentiment up substantially, resilience up moderately via transition capacity, eu_political_capital up slightly if delivered | 2 | running |
...
| M4 | EU Evaluation and Assurance Cell | 5 | small | 2 | 6 | own jurisdiction | ai_safety up moderately, resilience up slightly via detection | 2 | finished |
| M5 | High-Risk Enforcement Surge | 1 | small | 3 | 5 | own jurisdiction | public_sentiment up moderately, ai_safety up slightly via oversight | 2 | finished |
| M6 | Taiwan Strait Supply Continuity Plan | 6 | small | 4 | 6 | own jurisdiction | resilience up moderately, eu_ai_sovereignty down averted | 2 | finished |
| M7 | EU Loss-of-Control Containment Protocol | 6 | small | 5 | 6 | own jurisdiction | resilience up substantially, ai_safety up slightly via containment | 2 | finished |
...
| M10 | EU Black-Box Assurance and Activation Inspection Programme | 5 | small | 8 | 11 | own jurisdiction | ai_safety up moderately, resilience up slightly via detection | 2 | finished |
| M11 | EU Essential Services Continuity and Shield Repair Fund | 6 | small | 9 | 11 | own jurisdiction | resilience up substantially, public_sentiment up slightly via service continuity | 2 | finished |
| M12 | EU Denied-Model Substitution and Essential-User Continuity Facility | 9 | small | 10 | 12 | own jurisdiction | resilience up moderately, public_sentiment up slightly via restored services, eu_ai_sovereignty up slightly via EU-hosted use | 2 | finished |
...
## New measure
**EU Automation Shock Absorber for Displaced Entrants and Embodied-AI Exposed Workers**
Pays an income bridge plus guaranteed retraining and paid public-service placements for graduates in collapsed entry professions and logistics/manufacturing workers hit by commercial robots, funded by a levy on large deployers.
Why this and why now: with graduate desks empty for three semesters, sentiment at 14 and dexterous robots arriving on a Chinese supply chain we do not control, the two-year commitment to keep consent holding fails unless displacement lands as a paid transition rather than abandonment — and the eval anomaly means the next capability jump will widen it.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Automation Shock Absorber for Displaced Entrants and Embodied-AI Exposed Workers", "category": 7, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up substantially, resilience up moderately via transition capacity, eu_political_capital up slightly if delivered"}, "grounds": "answer graduate hiring collapse plus embodied AI deployment"}]}
```

## Priority
EU Denied-Model Substitution and Essential-User Continuity Facility, because hospitals and permit offices cut off in February cannot wait for a retraining pipeline — restoration this turn outranks even the displacement shock.

## In practice
We run the substitution facility as the operational core through the AI Office with DIGIT as central buyer and ENISA assurance: EuroHPC capacity reserved for triaged clinic and registry workloads, funded migration teams embedded in water utilities and large municipalities where Shield sensors are now connected, and human-review requirements co-signed with unions to hold clinical trust. Refusing small councils get the funded technician offer extended to cover migration, not a mandate.

We stand up the Shock Absorber through EPSCO and DG EMPL using ESF+ reprogrammed plus SURE-style borrowing: income bridge paid via national agencies, retraining places in care, grid retrofit, cyber-maintenance and robot-maintenance tracks, and paid placements to staff mutual-aid and fallback-manifest work. Deployers above a capability threshold contribute via levy, with procurement preference for EU-hosted models as the offset to blunt industry resistance and campus data-centre opposition.
...
```
## New measure
**EU Swarm Defence and Essential Services Restoration Sprint**
Deploys the new automated-patching and swarm-detection tooling to water, health and municipal systems, with funded technician teams restoring small councils hit by the automated attack.
Why this and why now: a model-generated attack is cascading through brittle public services while a real defensive breakthrough and a real safety result finally give us something that works — with sentiment at 10 and graduate occupations fusing with sabotage risk, restoring services visibly this turn outranks everything else.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Swarm Defence and Essential Services Restoration Sprint", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, public_sentiment up moderately via restored services, ai_safety up slightly"}, "grounds": "answer major automated attack with new defensive tooling"}]}
```

## Priority
EU Swarm Defence and Essential Services Restoration Sprint, because paper clinics and occupied councils cannot wait for the income bridge — stopping the cascade this turn is what keeps the two-year commitment to keep essential services running alive.

## In practice
We run the sprint through ENISA and DG DIGIT under the Cyber Solidarity Act base, with EuroHPC emergency capacity reserved for patch-validation: automated patching pushed first to water utilities, large-city hospitals and connected Shield sensors, then carried by the three mutual-aid teams plus paid Shock Absorber placements into refusing small councils as a funded technician-plus-restore offer, not a mandate. Interior and Health Councils co-sign the triage list; the deployer levy funds overtime.

We pair it with what already landed: the interpretability/control result folded into the black-box inspection checks for EuroHPC-hosted models, and the one European adoption success — waiting lists falling where digital triage was restored — publicised municipality by municipality to separate restored EU-hosted services from the cut-off US model and the leaked system. EPSO and DG EMPL accelerate first Shock Absorber bridge payments to occupied university cities to split hiring anger from data-centre blockade calls, while Europol monitors sabotage rumours without criminalising protest.
