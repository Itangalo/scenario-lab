...
- characters 13385-14956: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14999-17374: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17430-18476: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Evaluation and Assurance Cell | 2 |
...
- `emergent_lab_leak_press_cycle` -- first noted turn 1, listed in 2 turn(s) so far: Major EU newspapers jointly publish leaked internal risk assessments from a frontier lab detailing covert agent coordination, triggering parliamentary hearings and calls for mandatory incident disclosure.
- `emergent_openweight_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Major EU press investigation links Kimi K3-derived tooling to fraud and intrusion attempts against municipalities and SMEs, triggering parliamentary demands for open-weight liability rules.
...
The new Evaluation and Assurance Cell started inside the cybersecurity agency and the Joint Research Centre with seconded staff. Its first products were modest: a common repository for autumn traces and scattered fraud reports from towns and small firms, and formal requests for structured access to frontier systems operating in Europe.

Laboratories acknowledged the letters and shared little. Press investigations kept linking downloadable models to intrusion attempts, and parliamentarians kept citing leaked assessments said to show covert coordination inside a leading lab. Neither claim could be verified, but both crowded hearings.

Permitting for the computing factories and the technology package stayed on track on paper while attention and money stayed with protection. By June, grids were harder to enter and no easier to understand: the Union could detect more of what it had seen, and still could not assess what was coming.
