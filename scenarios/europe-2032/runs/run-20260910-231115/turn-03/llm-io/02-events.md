...
- characters 377-1717: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1761-4060: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4116-5201: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5264-27828: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28183-28340: `{{event_history}}` from the run's own event record
...
- `emergent_hyperscaler_pause_eu` -- first noted turn 2, listed in 1 turn(s) so far: A major US hyperscaler publicly pauses two planned EU data-centre expansions, citing permitting delays and grid-connection uncertainty around the AI factory siting process.
- `emergent_grid_copycat_extortion` -- first noted turn 2, listed in 1 turn(s) so far: Criminal or copycat groups reuse tooling and techniques resembling the autumn grid intrusions for extortion attempts against municipal utilities and port operators, without state sponsorship.
...
- Turn 2 (1 turn(s) ago): taiwan_blockade, openweight_frontier_release, safety_breakthrough, member_state_defection
