...
- characters 13721-15467: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15511-17757: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17813-18946: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Middle-Power Supply Pact | 2 |
...
- `emergent_anti_datacentre_blockades` -- first noted turn 5, listed in 1 turn(s) so far: A coordinated physical protest and blockade campaign targets power-ready AI factory shells and transformer deliveries in two member states, delaying site works and forcing a national policing response.
- `emergent_municipal_outsourcing` -- first noted turn 5, listed in 1 turn(s) so far: Municipal utilities and port operators hit by copycat extortion in 2027 jointly procure a shared managed-detection service from a non-EU vendor, creating a de facto external dependency outside the containment network.
...
## Notepad
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Supply-Chain Bargain Coalition −2, EU Middle-Power Supply Pact −2, priority −1 = −11
PROPOSAL BONUS: EU Middle-Power Supply Pact (cat 8, small) +3 — answers tiered-rationing threat and two breakaways / Washington no-exemption directly, converts middle_power_coalition window
LEGITIMACY LENDS: capital 25, sentiment 24 -> none (capital higher)
SOVEREIGNTY: 14 last turn, no capacity finish +0, Gigafactories in flight +0, middle_power_coalition t5 +2, capability rose 1.5 −0 = 16
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "pending"}}]}
```
