...
- characters 376-1659: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1702-4347: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4403-5540: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5603-30507: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30862-30997: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t3: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Services Shield −3, EU Deployed-Systems Assurance Cell −2, priority −1 = −12
PROPOSAL BONUS: none — new Assurance Cell answers eval anomaly reports and exploits safety breakthrough, but knowledge_work_augmented removes crisis basis and no recent negative event argued for it
LEGITIMACY LENDS: capital 19, sentiment 44 -> +2
SOVEREIGNTY: 20 last turn, no capacity event, Gigafactories in flight +0, capability rose 2.0 −1 = 19

## Emerging developments (tracked)

- `emergent_gigafactory_siting_revolt` -- first noted turn 2, listed in 2 turn(s) so far: A coordinated municipal backlash stalls two gigafactory shortlist sites, with regional councils voting to suspend fast-track grid permits pending environmental review and local referendum.
- `emergent_grid_probe_attribution` -- first noted turn 2, listed in 2 turn(s) so far: Independent press investigation publicly links the autumn grid-mapping tooling to a specific freely downloadable frontier model release, triggering lawsuits against the releasing lab and calls for open-weight liability.
...
- Turn 3 (1 turn(s) ago): eval_anomaly_reports, safety_breakthrough, knowledge_work_augmented
