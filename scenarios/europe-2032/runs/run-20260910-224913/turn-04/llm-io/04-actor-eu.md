...
- characters 454-1737: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1780-4425: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5337-5809: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7769-9852: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Deployed-Systems Assurance Cell | 5 | small | 3 | 6 | own jurisdiction | ai_safety up modestly, resilience up slightly via certified safeguards | 2 | running |
...
## New measure
**EU Deployed-Systems Assurance Cell**
It stands up a small EU technical cell to adopt the new interpretability/control result onto deployed systems in critical sectors and to run independent follow-up on the leaked eval anomaly.
This and now because the safety breakthrough gives us assurance that actually lands on shipped systems while the anomaly leak reopens the control question, and knowledge-work gains mean we must spend on invisible safety without a crisis to point at — a small category 5 move we can carry alongside three large programmes.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Deployed-Systems Assurance Cell", "category": 5, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up modestly, resilience up slightly via certified safeguards"}, "grounds": "to exploit safety breakthrough and investigate eval anomaly without new large spend"}]}
```

## Priority
EU Critical Services Shield (M3) — because drills and joint detection are the only thing absorbing harm this turn while gigafactory concrete is still stalled and the anomaly raises control doubts.

## In practice
We push M3 through ENISA and the Justice and Home Affairs Council, using Civil Protection Mechanism money to complete joint detection rollout to transmission operators and extend first drills from three to six countries, trading EU-funded hardening for signed continuity plans to quiet interior ministries complaining about unfunded standards.

We keep M1 and M2 alive via DG CNECT shepherding the four sites to final investment decision, pairing InvestAI guarantees and EIB lines with water and power-price side-deals for the two paused municipalities, holding Dutch lithography leverage in reserve. Alongside, we seed the new Assurance Cell in the AI Office with JRC support, tasked to certify the interpretability result on Shield-relevant deployments and to demand evaluation access on the anomalous system, funded by reallocated Horizon money to avoid a fresh budget fight.
...
```
## New measure
**European Chokepoint Compact**
It ties Dutch lithography servicing and EU upstream inputs to a joint EU negotiating mandate, invoking the Anti-Coercion Instrument to bargain volume-licence access and joint export-control terms with Washington instead of bilateral Dutch capitulation.
This and now because Washington is spending the Union's one chokepoint while gigafactory concrete waits and Shield completion slips — category 8 International coordination and leverage — and if ASML is picked off alone sovereignty falls further with no return.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "European Chokepoint Compact", "category": 8, "size": "small", "finish_turn": 7, "applies_to": "coalition (EU + Netherlands, with US as counterpart)", "targeted_effect": "eu_ai_sovereignty up, eu_political_capital defended via bargained access"}, "grounds": "to convert ASML coercion into joint leverage"}]}
```

## Priority
European Chokepoint Compact (new) — because losing ASML servicing bilaterally surrenders the only leverage that makes independent capacity possible, outranking Shield follow-through and stalled sites this turn.

## In practice
We mandate DG TRADE and DG CNECT to open an Anti-Coercion examination, with the Foreign Affairs and Competitiveness Councils authorising a single EU offer: continued servicing under joint EU-US criteria in exchange for guaranteed volume-licence compute and model access for EU gigafactories. The Hague is offered InvestAI anchor orders and EIB cover to hold the line while talks run.

We keep M1/M2 on life-support through CNECT shepherding the three viable sites to FID, using the US access bargain as the missing input guarantee to unlock municipal consent, and we let M4 Assurance Cell continue scoping certification on Shield deployments with JRC staff, deliberately not pulling its people into the trade file. Shield hardening already delivered is exercised via ENISA, but no new EU hardening promises are made until the Civil Protection funding gap is closed.
