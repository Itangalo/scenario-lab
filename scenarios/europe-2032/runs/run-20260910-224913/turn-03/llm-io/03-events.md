...
- characters 377-1648: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1692-3396: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3452-4427: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4490-28717: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29072-29113: `{{event_history}}` from the run's own event record
...
- `emergent_gigafactory_siting_revolt` -- first noted turn 2, listed in 1 turn(s) so far: A coordinated municipal backlash stalls two gigafactory shortlist sites, with regional councils voting to suspend fast-track grid permits pending environmental review and local referendum.
- `emergent_grid_probe_attribution` -- first noted turn 2, listed in 1 turn(s) so far: Independent press investigation publicly links the autumn grid-mapping tooling to a specific freely downloadable frontier model release, triggering lawsuits against the releasing lab and calls for open-weight liability.
