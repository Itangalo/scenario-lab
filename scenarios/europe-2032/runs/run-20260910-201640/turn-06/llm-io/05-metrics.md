...
- characters 105-13435: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13478-13668: `{{metrics_json}}` from the run's live metric values
- characters 13761-15090: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15133-17440: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17496-18068: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | European Talent Reception and Open-Stack Hardening | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 elected inward ticket taking office turn 6"}]}
```
```
