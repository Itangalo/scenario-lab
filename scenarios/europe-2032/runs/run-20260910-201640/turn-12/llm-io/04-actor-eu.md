...
- characters 452-1585: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1628-3543: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6597-7080: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9539-11222: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Hold essential services and social cohesion together through uncontrollable diffusion until leverage returns
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up materially, ai_safety incident harm down | 3 | finished |
| M4 | EU Sovereign Inference Backbone | 4 | large | 2 | 7 | own jurisdiction | eu_ai_sovereignty up materially, resilience up | 3 | finished |
| M5 | EU Incident Containment and Recovery Cell | 6 | small | 3 | 5 | own jurisdiction | resilience up, incident harm down | 2 | finished |
| M6 | European Talent Reception and Open-Stack Hardening | 4 | small | 5 | 7 | own jurisdiction | eu_ai_sovereignty up modestly, resilience up slightly | 2 | finished |
| M7 | Middle-Power Supply-Chain Coordination Framework | 8 | small | 8 | 10 | coalition of EU and middle powers holding supply-chain pieces | eu_political_capital up, eu_ai_sovereignty up modestly via joint leverage | 2 | finished |
| M8 | EU Essential Services Keep-Alive Cell | 6 | small | 11 | 12 | own jurisdiction | resilience up modestly, incident harm down | 2 | finished |
...
## New measure
**EU Essential Services Keep-Alive Cell**
It keeps hospitals, municipalities and telecom fallback running on existing budgets through ENISA-coordinated segmentation, backups and mutual aid. This and now because the automated attack plus rationing leaves no fiscal room for new build, only for holding what prevents cascade.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Keep-Alive Cell", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up modestly, incident harm down"}, "grounds": "major automated incident with no capital for new build"}]}
```

## Priority
EU Essential Services Keep-Alive Cell, because holding essential services through uncontrollable diffusion is the two-year commitment and the only action affordable at capital 3.

## In practice
We operate through ENISA and national CSIRTs under the existing NIS2 mutual-aid base, with Health Emergency Preparedness and Response Authority on quiet biosecurity watch after the genome-model claims. No new law, no new fund: Council JHA and EPSCO conclusions mandate segmentation, offline backups and joint telemetry on current budgets.

We direct the federated EuroHPC clinical fallback to stay as routine-triage cover for cut-off sites, with complex oncology and rare-disease cases routed via cross-border second opinions. We use the finished middle-power channel only to share test access and screening, without reopening servicing terms while Dutch lithography servicing stays frozen. DG COMM holds public line to degraded-not-stopped services to contain sentiment at 12.
...
```
## New measure
**EU Bio-Containment and Care Continuity Cell**
It runs weeks-long containment, triage and cross-border care fallback on existing HERA/ENISA budgets with no new law, isolating model-assisted bio incident response from the cut-off frontier stack. This and now because a real bio incident with casualties resets all risk politics and essential services cannot pause for containment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Containment and Care Continuity Cell", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up modestly, incident harm down"}, "grounds": "real bio incident with containment running"}]}
```

## Priority
EU Bio-Containment and Care Continuity Cell, because containment plus keeping hospitals and municipalities degraded-not-stopped is the two-year commitment under direct hit.

## In practice
We mandate through EPSCO/JHA conclusions: HERA leads detection, isolation protocols and quiet public guidance, ECDC runs joint telemetry, ENISA and national CSIRTs keep segmentation, offline backups and mutual aid for locked registries and paper-fallback hospitals. No new fund, no AI Act reopening: re-tasked EU4Health and rescEU stocks.

We keep the federated clinical fallback on routine triage to free infectious-disease capacity, with complex oncology routed cross-border as before. DG COMM holds a single line — containment plus continuity — to stop sentiment at 8 collapsing into physical opposition, while we treat frontier cut-off, member-state defection and open-weight diffusion as unanswerable this turn on capital 2.
