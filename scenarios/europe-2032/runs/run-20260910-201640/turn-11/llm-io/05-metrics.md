...
- characters 107-12927: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12970-13160: `{{metrics_json}}` from the run's live metric values
- characters 13253-14339: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14383-16459: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16515-17176: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M8 | EU Essential Services Keep-Alive Cell | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M8", "fields": {"finish_turn": 12}, "grounds": "named priority"}]}
```
```
