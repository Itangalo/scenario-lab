...
- characters 376-2131: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2175-4130: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4186-5294: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5357-31355: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 31710-32600: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t8: Middle-Power Supply-Chain Coordination Framework −2 = −2
PROPOSAL BONUS: Middle-Power Supply-Chain Coordination Framework (cat 8, small) +2 — answers lithography servicing coercion from t7 directly and uses current coalition window
LEGITIMACY LENDS: capital 2, sentiment 10 -> +2 capped by sentiment, included in capital total
SOVEREIGNTY: 11 last turn, no category 4 finish, M7 cat 8 in flight +0, middle_power_coalition t8 +2, capability rose 0.7 no −1 = 13

## Emerging developments (tracked)

- `emergent_care_failure_protests` -- first noted turn 7, listed in 2 turn(s) so far: Coordinated street protests and hospital staff walkouts in states hit by US model revocations after on-camera oncology failures, turning health-care dependence into a national political crisis.
- `emergent_clinical_audit_backlash` -- first noted turn 7, listed in 2 turn(s) so far: Municipal hospital federations publish an open clinical safety audit showing the EU hardened open-model triage stack failing at scale, triggering coordinated procurement freezes and street protests against further rollout.
...
- Turn 1 (8 turn(s) ago): cyber_test_shot, openweight_frontier_release
- Turn 2 (7 turn(s) ago): loss_of_control_incident, openweight_frontier_release, knowledge_work_augmented, eu_frontier_access_denied, emergent_eu_open_compute_pact
- Turn 3 (6 turn(s) ago): cyber_major_incident, eval_anomaly_reports, eu_frontier_access_denied
- Turn 4 (5 turn(s) ago): ai_investment_collapse, knowledge_work_augmented
- Turn 5 (4 turn(s) ago): opaque_reasoning, research_breakthrough, openweight_frontier_release, eu_frontier_access_denied, automated_decision_scandal, emergent_us_talent_exodus, election_retrenchment
- Turn 6 (3 turn(s) ago): cyber_major_incident, eu_frontier_access_denied, joint_threat_response
- Turn 7 (2 turn(s) ago): openweight_frontier_release, supply_chain_coercion, joint_threat_response
- Turn 8 (1 turn(s) ago): labour_displacement, adoption_success, middle_power_coalition
...
IMPORTANT: For events with date-specific conditions (e.g., "September 2026 is included"), check if the current time period (July-December 2030) covers that date.
