...
- characters 455-2380: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2424-4852: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6338-6804: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8402-10329: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up materially, ai_safety incident harm down | 3 | running |
| M4 | EU Sovereign Inference Backbone | 4 | large | 2 | 6 | own jurisdiction | eu_ai_sovereignty up materially, resilience up | 3 | running |
...
## New measure
**EU Sovereign Inference Backbone**
Stands up EU-controlled inference and fine-tuning capacity on pooled university and gigafactory-adjacent compute to replace cut-off US models for hospitals, ministries and firms with EU-run open-frontier models. Category 4 Sovereignty and industrial capacity, because the short-notice US cut-off plus a within-5-points open release makes dependence the binding risk and the university compute pact the window to fix it now.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Sovereign Inference Backbone", "category": 4, "size": "large", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up materially, resilience up"}, "grounds": "US frontier access denied while openweight nears frontier and EU pact offers deployable base"}]}
```

## Priority
EU Sovereign Inference Backbone, because the US cut-off hitting hospitals and ministries outranks even the Shield this turn — resilience without runnable models is empty.

## In practice
We mandate DG CNECT and EuroHPC JU to contract the emergent university open-compute pact into a federated inference backbone, with Digital Europe emergency funds for adapters, hardening and helpdesk migration for affected hospitals and ministries, and fast-track grid connection for the two delayed gigafactory sites via the Competitiveness Council. ENISA certifies the Mythos-class-derived defensive tooling separately so the Backbone ships clean.

In parallel we keep M3 EU Critical Systems Shield in full run — segmentation retrofits and autumn exercises — and hold M1/M2 site selection moving, telling finance ministries the Backbone reuses their co-funding rather than adding a parallel compute programme. To Washington we lodge Anti-Coercion notice on discriminatory cut-offs while quietly shifting critical workloads off appeal-less US review regimes.
...
```
## New measure
**EU Incident Containment and Recovery Cell**
Stands up a temporary EU-level operational cell under ENISA to coordinate patching, backup restoration and mutual aid for the automated attack hitting public services, with emergency procurement for clean images and overtime for operators. Category 6 Preparedness and resilience, because the major model-generated incident is live, defenders are visibly behind, and containment this turn outranks building new capacity.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Incident Containment and Recovery Cell", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, incident harm down"}, "grounds": "large automated attack landing while Shield retrofits still in flight"}]}
```

## Priority
M3 EU Critical Systems Shield, because the live ransomware / supply-chain sweep is exactly what the Shield was built for and the new Cell only buys time unless segmentation and exercises hold.

## In practice
We activate the Shield's contingency clause via the Justice and Home Affairs Council: ENISA leads a 24/7 containment cell in Tallinn and The Hague, with CERT-EU triage, joint procurement of clean backups, and cross-border mutual aid teams seconded from France, Poland and Nordics to the hardest-hit municipalities and hospitals. Digital Europe emergency funds cover operator overtime, bypassing the finance-ministry cost fight for now.

In parallel we keep M4 Sovereign Inference Backbone as the hospital workaround — migrating triage pilots cut off by Washington onto the federated open-weight stack with helpdesks — and we freeze new M1/M2 site fights to two fast-track grid connections, telling Spain and eastern Germany no new money until containment is funded. To the public we name the tooling as model-generated without attributing, and promise a post-mortem incident report to rebuild trust in autonomy.
