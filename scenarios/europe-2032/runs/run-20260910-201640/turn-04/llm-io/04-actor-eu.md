...
- characters 454-1560: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1603-3511: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4937-5403: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7459-9441: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M5 | EU Incident Containment and Recovery Cell | 6 | small | 3 | 5 | own jurisdiction | resilience up, incident harm down | 2 | running |
...
```
## New measure
None this turn. We are waiting for political headroom to rebuild after containment costs and for the investment reset to reveal which compute partnerships survive before we stake new money.

## Store changes
No changes.

## Priority
M5 EU Incident Containment and Recovery Cell, because with capital at 12 we can only afford to finish what restores services visibly this turn rather than start what we cannot fund.

## In practice
We keep ENISA's Tallinn/The Hague cell as the single operational face through spring: Digital Europe emergency reimbursements for municipal overtime, shared clean images, and French-Polish-Nordic mutual-aid vans, while the finished M3 Shield segmentation is exercised, not expanded. No new siting decisions under M1/M2 are forced while finance ministers contest the bill and private build-out cancels.

We use the investment collapse as cover to pause, not kill, the gigafactory pipeline: DG CNECT and EIB renegotiate grid and co-funding terms on the two fast-track connections only, and we steer clinics cut off from US systems onto the M4 federated backbone for basic triage with helpdesks. To a public that now sees productivity gains but no job crisis, we promise the post-mortem and kept services running, not new spending.
