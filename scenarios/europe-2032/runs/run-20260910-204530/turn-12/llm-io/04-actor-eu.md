...
- characters 452-2232: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2275-4761: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5685-6178: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9506-11752: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M11 | EU Bio-Containment and Essential Care Continuity Protocol | 6 | small | 11 | 13 | own jurisdiction | resilience up, ai_safety up slightly via incident learning | 2 | running |
...
```
## New measure
None this turn, while we wait for the Bio-Containment Protocol and Lithography Survival Deal to finish landing without bankrupting our last capital.

## Store changes
No changes.

## Priority
M11 EU Bio-Containment and Essential Care Continuity Protocol, because containing the deliberate pathogen release and keeping hospitals on isolated rationed care outranks bargaining leverage while wards are closed and US model access is suspended.

## In practice
We run M11 through HERA, ECDC and the Health Council as triage only: cross-border medical teams and mutual-aid stocks to the two release cities, genome-triage pilots extended where staff hold, critical wards kept on isolated backups and EuroHPC-rationed compute, with daily restored-service counts as the single public reporting line.

We hold M10 alive through DG TRADE and the Foreign Affairs Council without spending we do not have: no unilateral Dutch servicing cut, joint-licensing offer for servicing/spares in exchange for essential-compute guarantees left on the table, the one powered domestic site kept on hospitals, grid and backups only while Strait shipments remain stopped and courts block new grid links. No new build, no new siting fight, no second instrument until containment holds and capital rebuilds.
