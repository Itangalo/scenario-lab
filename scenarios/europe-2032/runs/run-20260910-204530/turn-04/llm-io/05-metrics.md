...
- characters 105-13740: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13783-13975: `{{metrics_json}}` from the run's live metric values
- characters 14068-14992: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15035-17123: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17179-18017: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Supply-Chain Sovereignty Shield | 2 |
...
- `emergent_compute_insurance_repr icing` -- first noted turn 3, listed in 2 turn(s) so far: A major European insurer reprices power and water risk for AI infrastructure after the siting disputes, raising premiums and requiring independent resource audits for new computing plants.
...
## Metrics
```json
{"ai_capability": 64.0, "openweight_capability": 57.0, "ai_safety": 20.0, "resilience": 40.0, "eu_ai_sovereignty": 23.0, "eu_political_capital": 14.0, "public_sentiment": 25.0}
```
## Narrative
