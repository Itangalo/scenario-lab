...
- characters 453-1797: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1841-3932: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6584-7077: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10221-13034: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Survive automated disruption with essential services intact and secure a working place inside the US-China risk regime
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Lithography Leverage and Essential Compute Survival Deal | 8 | small | 10 | 13 | US, Netherlands, coalition (Japan, like-minded suppliers) | eu_ai_sovereignty protected, eu_political_capital protected, resilience up slightly via compute rationing | 2 | running |
...
| M4 | EU Siting Social Licence Deal | 4 | small | 2 | 4 | own jurisdiction | eu_ai_sovereignty up, public_sentiment up slightly by unblocking sites | 2 | finished |
| M5 | EU High-Risk Redress and Oversight Fix | 2 | small | 3 | 4 | own jurisdiction | public_sentiment up, ai_safety up slightly via enforced oversight | 2 | finished |
| M6 | EU Supply-Chain Sovereignty Shield | 8 | small | 4 | 6 | US, Netherlands, coalition (Japan, like-minded suppliers) | eu_ai_sovereignty up slightly, eu_political_capital protected | 2 | finished |
| M7 | EU Essential Services Continuity Reserve | 6 | small | 5 | 7 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly via fallback demand | 2 | finished |
| M8 | EU Agentic Containment and Essential-Service Isolation Protocol | 6 | small | 6 | 8 | own jurisdiction | resilience up, ai_safety up slightly via containment learning | 2 | finished |
| M9 | EU Essential Services Survival and Mutual Aid Surge | 6 | small | 9 | 11 | own jurisdiction | resilience up, ai_safety up slightly via incident learning | 2 | finished |
...
Uses the Taiwan blockade and Washington's ASML coercion to bargain EU-controlled lithography servicing and incident data for continued compute access and observer place, while rationing remaining chips to essential services.
This is category 8 International coordination and leverage because the blockade makes EU sovereignty a state-to-state bargain not a build programme, and because with capital at 9 and courts freezing grid connections no domestic compute build can move this turn; a small coordinated leverage measure is the only fundable way to serve the two-year commitment to survive disruption and secure a place inside the risk regime.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Lithography Leverage and Essential Compute Survival Deal", "category": 8, "size": "small", "finish_turn": 13, "applies_to": "US, Netherlands, coalition (Japan, like-minded suppliers)", "targeted_effect": "eu_ai_sovereignty protected, eu_political_capital protected, resilience up slightly via compute rationing"}, "grounds": "Taiwan blockade and US coercion of ASML require joint EU leverage response"}]}
```

## Priority
M9 EU Essential Services Survival and Mutual Aid Surge, because with ransomware restoration incomplete, bio-uplift signal opening and safety at 8, holding the protected core outranks any new bargain this turn.

## In practice
We run M9 as the operating line through Health and JHA Councils with ENISA, CERT-EU and HERA: extend isolated fallback machines and freeze-thresholds from listed clinics/ministries to hit municipalities and the payroll-supplier chain, fund cross-border restoration crews from the continuity reserve, and publish daily restored-service counts to contain hostility where sentiment is at 10. No new siting fights while injunctions stand; the one powered gigafactory site stays on essential public workloads only.

We run the new leverage track through the Foreign Affairs Council, DG TRADE Anti-Coercion Instrument cell and The Hague: no unilateral Dutch cut, but joint EU licensing of servicing/spares for older DUV tools tied to US guarantees on EU essential compute access, classified-weights access petitions, and observer status in the US-China weights-security pact, offering containment logs and Dutch supply-chain restraint as ticket. HERA and ECDC quietly add genome-model triage to the M9 drills without publishing recipes, preparing for the bio gate.

We ration what the blockade leaves. Via EuroHPC and the Chips Joint Undertaking we freeze non-essential accelerator orders, redirect stock to hospitals, grids and backup systems, and tell hyperscalers power dates hold but expansion does not, to avoid spending capital we do not have.
...
Activates HERA-led containment, triage and isolated medical continuity using rationed compute and fallback systems to keep hospitals functioning through the deliberate biological release.
This and now because casualties are real and containment runs for weeks while frontier access is denied and open weights are already superhuman, so only a small resilience measure focused on bio-response and essential-service isolation can serve the two-year commitment to survive disruption with services intact when political capital is at 5.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Containment and Essential Care Continuity Protocol", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via incident learning"}, "grounds": "real bio incident with model involvement plus denied frontier access requires immediate containment"}]}
```

## Priority
EU Bio-Containment and Essential Care Continuity Protocol, because containing the ongoing release and keeping hospitals and ministries running outranks the lithography bargain while casualties and quarantine blockades coincide.

## In practice
We run the new protocol through Health and JHA Councils with HERA, ECDC and ENISA: national stockpiles and cross-border medical teams deployed to containment zones, triage and genome-model uplift protocols already added to drills made operational, hospitals and grid operators held on isolated fallbacks and EuroHPC-rationed compute, with daily restored-service reporting to stabilize sentiment at 6.

We keep M10 EU Lithography Leverage and Essential Compute Survival Deal alive as the second line through the Foreign Affairs Council and DG TRADE: no unilateral Dutch servicing cut, joint licensing of servicing/spares tied to US guarantees on essential compute and observer access, while we refuse to spend capital on new siting fights blocked by courts and keep the one powered domestic site on public workloads only. Embodied-AI and export-control exposure is handled by procurement restraint, not a new build, until containment holds.
