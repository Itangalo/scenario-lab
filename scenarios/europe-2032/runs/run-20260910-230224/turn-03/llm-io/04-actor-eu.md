...
- characters 455-1832: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1876-3896: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5913-6385: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8053-10325: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_memory_substrate_squeeze (emergent event):** A non-US, non-China supplier imposes temporary export licensing on a critical AI hardware input such as high-bandwidth memory or advanced substrates, citing domestic demand, spiking EU gigafactory cost estimates and delaying site decisions.
**emergent_datacentre_pause (emergent event):** A US hyperscaler freezes or delays planned EU data-centre expansion after gigafactory site fights over grid connections and priority power spill into public disputes over water and electricity prices.
...
| M4 | EU Semiconductor Allocation and Leverage Regime | 8 | large | 2 | 4 | own jurisdiction plus supply-chain coalition | eu_ai_sovereignty up, resilience up via secured supply | 3 | running |
...
## New measure
**EU Semiconductor Allocation and Leverage Regime**
Creates EU-coordinated allocation of scarce advanced chips for critical services and gigafactory build-out, paired with joint control of lithography maintenance, spares and upstream inputs as bargained leverage.
This is International coordination and leverage because the Taiwan blockade turns every compute question into a security negotiation overnight, and the Union's only card that cannot be switched off from outside is its upstream bottleneck held jointly rather than picked off member-state by member-state.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Semiconductor Allocation and Leverage Regime", "category": 8, "size": "large", "finish_turn": 5, "applies_to": "own jurisdiction plus supply-chain coalition", "targeted_effect": "eu_ai_sovereignty up, resilience up via secured supply"}, "grounds": "blockade disrupts compute supply for years and bio findings raise stakes"}]}
```

## Priority
EU Semiconductor Allocation and Leverage Regime (new) – because without allocated chips and a joint ASML position the gigafactories stall and the Shield has nothing to run on, outranking even our own build-out this turn.

## In practice
We are moving the new regime through the Foreign Affairs Council and the Trade configuration under the Anti-Coercion Instrument and the Chips Act emergency provisions, with DG TRADE and DG CNECT jointly running a clearinghouse for advanced-chip stocks, forward orders and gigafactory requirements. Member states with fabs, packaging and materials are offered priority allocation for essential operators in exchange for accepting EU-level export authorisation for lithography services and spares.

At the same time we keep M1 and M2 alive but sequenced behind allocation: site selection continues, but grid-connection and guarantee decisions are now conditional on secured chip supply, to stop a subsidy race for capacity that cannot be equipped. M3 drills are re-tasked to islanding scenarios — operating grids, ports and hospitals through prolonged supply cutoff — with HERA adding pooled procurement of sequencing-based biodetection to answer the genome-model finding before it leaves the biosecurity community.
...
```
## New measure
None this turn. We are waiting for the Shield and Allocation regime to finish landing next turn before staking new capital we do not have.

## Store changes
No changes.

## Priority
M4 EU Semiconductor Allocation and Leverage Regime – because with the strait closed, HBM/substrate squeeze and US datacentre pause, unblocking allocated supply is the precondition for gigafactories, Shield islanding, and any leverage over export-control escalation.

## In practice
We are running the clearinghouse through DG TRADE/CNECT under Chips Act emergency and Anti-Coercion provisions: pooled stocks and forward orders steered to critical operators and conditional gigafactory sites, with lithography service authorisations held jointly to bargain volume-licence access in Washington and restraint in Beijing. Grid offers stay conditional on secured supply to avoid stranded guarantees, and we negotiate the Dutch maintenance case as a Union position rather than a bilateral retaliation target.

M3 Shield drills continue on islanding for grids, ports and hospitals, with HERA's pooled sequencing-detector orders filling the detection gap administrators flagged. M1/M2 site selection is sequenced behind M4: permits and priority power advance, but final investment decisions wait on HBM/substrate licensing relief and hyperscaler recommitment. On the leaked eval anomaly we task EU evaluators with structured access requests and incident-reporting preparation, without opening a new instrument this turn.
