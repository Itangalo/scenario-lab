...
- characters 105-13044: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13087-13279: `{{metrics_json}}` from the run's live metric values
- characters 13372-14509: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14552-16581: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16637-17724: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Semiconductor Allocation and Leverage Regime | 3 |
...
- `emergent_memory_substrate_squeeze` -- first noted turn 2, listed in 1 turn(s) so far: A non-US, non-China supplier imposes temporary export licensing on a critical AI hardware input such as high-bandwidth memory or advanced substrates, citing domestic demand, spiking EU gigafactory cost estimates and delaying site decisions.
- `emergent_datacentre_pause` -- first noted turn 2, listed in 1 turn(s) so far: A US hyperscaler freezes or delays planned EU data-centre expansion after gigafactory site fights over grid connections and priority power spill into public disputes over water and electricity prices.
...
## Metrics
```json
{"ai_capability": 54.5, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 37.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 34.0, "public_sentiment": 33.0}
```
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Services Shield −3, EU Semiconductor Allocation and Leverage Regime −3, priority −1 = −13
PROPOSAL BONUS: EU Semiconductor Allocation and Leverage Regime (cat 8, large) +4 — Taiwan blockade disrupts compute for years this turn and this answers it directly
LEGITIMACY LENDS: capital 34, sentiment 33 -> none, capital higher
SOVEREIGNTY: 22 last turn, no category 4 finish, in flight +0, taiwan_blockade t2 −2, capability rose 1.0 −0 = 20
