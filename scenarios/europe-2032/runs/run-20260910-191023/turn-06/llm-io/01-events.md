...
- characters 1466-3708: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3764-4783: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4846-26085: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 26440-26833: `{{event_history}}` from the run's own event record
...
- `emergent_buildout_blockade` -- first noted turn 4, listed in 2 turn(s) so far: A coordinated physical protest campaign blocks access to two gigafactory and substation construction sites, delaying works for weeks and forcing a national debate on grid-slot priority.
- `emergent_hosting_pilot_suspension` -- first noted turn 4, listed in 2 turn(s) so far: The US cloud provider suspends its EU-hosted pilot for police and border workloads after US legal orders conflict with EU key-handover and continuity demands, forcing a temporary reversion to degraded fallbacks.
...
- Turn 2 (4 turn(s) ago): cyber_major_incident, loss_of_control_incident, safety_breakthrough, emergent_model_access_freeze
- Turn 3 (3 turn(s) ago): emergent_sovereign_hosting_offer
- Turn 4 (2 turn(s) ago): cyber_major_incident, ai_investment_collapse
- Turn 5 (1 turn(s) ago): knowledge_work_augmented, supply_chain_coercion, election_consolidation
