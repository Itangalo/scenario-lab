...
- characters 105-12844: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12887-13079: `{{metrics_json}}` from the run's live metric values
- characters 13172-14219: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14262-16504: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16560-16973: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Fallback Continuity Operation | 2 |
...
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, priority −0 (capital 9 below 20, priority has no effect and no cost) = −3
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 9, sentiment 21 -> +2 (capped below sentiment, applied before other terms)
SOVEREIGNTY: 21 last turn, no category 4 finish, Gigafactories in flight +0, supply_chain_coercion t5 −3, capability rose 1.5 −0 = 18
...
**emergent_hosting_pilot_suspension (emergent event):** The US cloud provider suspends its EU-hosted pilot for police and border workloads after US legal orders conflict with EU key-handover and continuity demands, forcing a temporary reversion to degraded fallbacks.
...
## Notepad
PORTFOLIO CHARGE t6: InvestAI Gigafactories −3, EU Fallback Continuity Operation −2, priority −0 (capital 10 below 20, priority has no effect and no cost) = −5
PROPOSAL BONUS: EU Fallback Continuity Operation (cat 6, small) +2 — cyber_major_incident landed this turn and hosting suspension forced continuity, this answers directly but small
LEGITIMACY LENDS: capital 2, sentiment 11 -> +2 (capped below sentiment, applied after other terms)
SOVEREIGNTY: 18 last turn, no category 4 finish, Gigafactories in flight +0, export_control_escalation t6 −2, capability rose 1.5 −0 = 16

## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner takes office turn 6"}]}
```
```
