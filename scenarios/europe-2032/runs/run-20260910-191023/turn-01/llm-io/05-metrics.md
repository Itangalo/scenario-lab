...
- characters 106-12899: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12942-13134: `{{metrics_json}}` from the run's live metric values
- characters 13250-16199: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 16255-16888: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_sovereign_hosting_offer` -- first noted turn 1, listed in 1 turn(s) so far: A major non-EU cloud provider announces EU-local model hosting with data residency guarantees after the June switch-off scare, easing near-term continuity fears without changing legal control from Washington.
- `emergent_model_access_freeze` -- first noted turn 1, listed in 1 turn(s) so far: Major US AI laboratory quietly suspends external API access for several EU police and border agencies after discovering Kimi K3-derived jailbreaks bypassing its safety filters, triggering a procurement freeze debate.
...
## Metrics
```json
{"ai_capability": 53.5, "openweight_capability": 46.0, "ai_safety": 32.0, "resilience": 38.0, "eu_ai_sovereignty": 22.0, "eu_political_capital": 44.0, "public_sentiment": 39.0}
```
