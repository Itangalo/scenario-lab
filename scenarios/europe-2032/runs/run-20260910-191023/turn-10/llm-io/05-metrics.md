...
- characters 106-13253: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13296-13488: `{{metrics_json}}` from the run's live metric values
- characters 13581-14735: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14778-16660: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16716-17814: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M9 | EU Entry-Level Transition Shield | 2 |
| M10 | EU Joint Cyber Command Accession | 2 |
...
PORTFOLIO CHARGE t9: European Visible Benefit Deployment −2, EU Entry-Level Transition Shield −2, priority −0 (capital 4 below 20, priority has no effect and no cost) = −4
PROPOSAL BONUS: EU Entry-Level Transition Shield (cat 7, small) +3 — labour_displacement landed this turn with named entry-level losses and this answers it directly
LEGITIMACY LENDS: capital 8, sentiment 11 -> +2
SOVEREIGNTY: 18 last turn, no cat4 finishes, no cat4 in flight +0, no capacity event, capability rose 0.7 −0 = 18
...
## Metrics
```json
{"ai_capability": 62.9, "openweight_capability": 61.4, "ai_safety": 21.0, "resilience": 51.0, "eu_ai_sovereignty": 15.0, "eu_political_capital": 11.0, "public_sentiment": 8.0}
```
