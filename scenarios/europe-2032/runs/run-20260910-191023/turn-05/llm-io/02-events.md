...
- characters 376-1510: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1554-3857: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3913-4995: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5058-27679: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28034-28329: `{{event_history}}` from the run's own event record
...
- `emergent_buildout_blockade` -- first noted turn 4, listed in 1 turn(s) so far: A coordinated physical protest campaign blocks access to two gigafactory and substation construction sites, delaying works for weeks and forcing a national debate on grid-slot priority.
- `emergent_hosting_pilot_suspension` -- first noted turn 4, listed in 1 turn(s) so far: The US cloud provider suspends its EU-hosted pilot for police and border workloads after US legal orders conflict with EU key-handover and continuity demands, forcing a temporary reversion to degraded fallbacks.
...
- Turn 2 (3 turn(s) ago): cyber_major_incident, loss_of_control_incident, safety_breakthrough, emergent_model_access_freeze
- Turn 3 (2 turn(s) ago): emergent_sovereign_hosting_offer
