...
- characters 455-1943: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1987-4378: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5701-6167: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9219-11196: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `act_under_uncertainty` (position): Act to keep services running and hiring alive with what we control, since frontier capacity and supply remain rationed from outside
- `two_year_commitment` (commitment): Hold essential services and livelihoods together through AI disruption
...
|---|---|---|---|---|---|---|---|---|---|
| M12 | EU Cure Access Deployment | 9 | small | 12 | 14 | own jurisdiction | public_sentiment up moderately, resilience up slightly | 2 | running |
...
| M3 | EU Grid and Critical Services Hardening Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up strongly, ai_safety up slightly via detection | 3 | finished |
| M4 | EU AI Incident Containment and Continuity Corps | 6 | large | 2 | 5 | own jurisdiction | resilience up strongly, ai_safety up slightly via containment learning | 3 | finished |
| M5 | EU Sovereign Hosting Accreditation Pact | 8 | small | 3 | 5 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up slightly via continuity | 2 | finished |
| M6 | EU Fallback Continuity Operation | 6 | small | 6 | 8 | own jurisdiction | resilience up moderately, ai_safety flat | 2 | finished |
| M7 | EU Swarm-Defence Rollout and Offline Stack Adoption | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, ai_safety up slightly via detection | 2 | finished |
| M8 | European Visible Benefit Deployment | 9 | small | 8 | 10 | own jurisdiction | public_sentiment up moderately, eu_political_capital up slightly | 2 | finished |
| M9 | EU Entry-Level Transition Shield | 7 | small | 9 | 11 | own jurisdiction | public_sentiment up slightly, eu_political_capital up slightly, resilience up slightly | 2 | finished |
| M10 | EU Joint Cyber Command Accession | 6 | small | 10 | 11 | own jurisdiction | resilience up moderately | 2 | finished |
| M11 | EU Entry-Level Guarantee Top-Up | 7 | small | 11 | 13 | own jurisdiction | public_sentiment up slightly, resilience up slightly | 2 | finished |
...
## New measure
**EU Cure Access Deployment**
Puts the new tailored therapies to work in EU public hospitals and clinics on accredited EU hosting and offline continuity systems, so patients feel benefit as European healthcare delivery rather than dependence on foreign models.
This and why now because the medical breakthrough is the first positive shock in years arriving while sentiment is at 7 and capital at 12, and unless delivery is visibly EU-run it will read as further dependence; tying it to the guarantee placements and cleaned networks makes the two-year commitment tangible.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cure Access Deployment", "category": 9, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up slightly"}, "grounds": "deliver medical breakthrough through EU public system"}]}
```

## Priority
EU Entry-Level Guarantee Top-Up, because with capital at 12 and queues still driving blockades, paid hiring where systems are clean must hold before cure rollout can staff and scale.

## In practice
We run M11 through DG EMPL and EPSCO as the spine: ESF+ top-up advances stay directed to arrears municipalities and hospitals on the offline stack, with time-limited siting continuity for the two blockaded sites exchanged for binding local hiring quotas. Supervisors get a funded training allowance so posts do not collapse into burden complaints.

We prepare cure delivery inside existing health competences: HERA and EMA for accelerated tailored-therapy protocols, EU4Health procurement for diagnostics and cold-chain, and the sovereign hosting pact plus EU-only node for any model-dependent personalization to stay on accredited EU infrastructure. Health Council mandates pilot oncology and rare-disease centres in clean-network cities first, staffed in part by guarantee cohorts, so benefit and jobs arrive together.
...
```
## New measure
**EU Offline Care Continuity Shield**
Keeps hospitals and essential services running on accredited EU-hosted and offline fallback pathways after frontier access cutoff, while adding urgent bio-screening and incident reporting around open personalization tools.
This and why now because access denial plus near-frontier open weights plus a bio uplift signal in the same turn means foreign-rationed cures can stop and distributed misuse can start together; with capital at 8 and sentiment at 10 only a small continuity measure tied to the cure pilots and clean networks can hold.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Offline Care Continuity Shield", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly"}, "grounds": "answer cutoff and bio/openweight shock with continuity"}]}
```

## Priority
EU Offline Care Continuity Shield, because cutoff of clinical queries and unrecallable open capability outranks even the guarantee and cure rollout for holding services alive under the two-year commitment.

## In practice
We run the cutoff as a Health Council and EPSCO continuity operation: HERA/EMA re-route personalization queries to accredited EU hosting and the EU-only node, EU4Health funds offline protocols and paper-to-clean fallbacks in pilot oncology centres, and DG EMPL guarantee cohorts are retasked to triage, records and cold-chain continuity so waiting lists do not collapse where systems are clean.

We pair it with a narrow biosecurity add-on through ECDC and the Incident Corps: mandatory incident reporting for clinical AI misuse, screening for outsourced synthesis queries, and joint procurement of detection in hospitals and labs. We offer arrears cities funded posts and diagnostics in exchange for siting calm, and tell blockaded sites no new data-centre permits without quota compliance.
```
