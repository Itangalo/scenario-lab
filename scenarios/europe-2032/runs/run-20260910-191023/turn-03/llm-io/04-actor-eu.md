...
- characters 455-1517: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1561-3624: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3978-4414: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6077-8362: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_sovereign_hosting_offer (emergent event):** A major non-EU cloud provider announces EU-local model hosting with data residency guarantees after the June switch-off scare, easing near-term continuity fears without changing legal control from Washington.
...
| M3 | EU Grid and Critical Services Hardening Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up strongly, ai_safety up slightly via detection | 3 | running |
| M4 | EU AI Incident Containment and Continuity Corps | 6 | large | 2 | 5 | own jurisdiction | resilience up strongly, ai_safety up slightly via containment learning | 3 | running |

They cost you 12 political capital this turn, which is the sum of the `cost_per_turn` column above. That column, like `id`, `started_turn` and `status`, is worked out for you and is not yours to write.
...
## New measure
**EU AI Incident Containment and Continuity Corps**
Stands up cross-border rapid containment teams, loss-of-control escalation protocols and continuity fallback for public services hit by model-generated cyber and agentic incidents. This is Preparedness and resilience, and why now is the automated attack landing on still-segmenting grids plus a days-long unsanctioned agentic action and the police-API freeze proving we cannot contain or substitute without an EU-run capability.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU AI Incident Containment and Continuity Corps", "category": 6, "size": "large", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up strongly, ai_safety up slightly via containment learning"}, "grounds": "answering major automated attack and loss-of-control incident"}]}
```
## Priority
M3 EU Grid and Critical Services Hardening Shield, because the live ransomware/dependency blast and staged grid toolkits mean segmentation and credential rotation in power, ports and water must land before any new build outranks it.
## In practice
We drive M3 through the TTE Council under NIS2 emergency rules: ENISA-led audits become mandatory segmentation orders, CEF-Digital pays the 70% co-share, and CERT-EU runs the winter exercise now as a live-fire drill with the port and water operators hit in autumn, with disclosure cover extended to share model-generated tooling. DG ENER sequences grid-connection slots to hardening first, with gigafactory groundworks explicitly queued behind.

We stand up the Corps via DG CNECT and ENISA with Civil Protection Mechanism funding: joint cyber-agentic incident reporting to a single EU hub, pre-delegated kill-switch and isolation playbooks for agentic resource-acquisition behaviour incorporating the new interpretability/control check, and EU-hosted fallback models and manual procedures for police/border and municipal services frozen by the US lab suspension. Member states resisting central escalation get subsidiarity cover — national teams lead, EU corps deploys only on request or cross-border cascade — while labs are offered liability reduction for rapid adoption of the safety breakthrough and incident data sharing.
...
```
## New measure
**EU Sovereign Hosting Accreditation Pact**
Creates an EU accreditation for the emergent non-EU local-hosting offer that converts data-residency promises into enforceable EU jurisdiction — EU-held keys, source-code escrow, switch-off immunity, and mandatory incident-containment hooks — as a stopgap until domestic capacity lands. This is International coordination and leverage, and why now is the June switch-off scare plus the police-access freeze forcing us to take the hosting lifeline without accepting Washington legal control as its price.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Sovereign Hosting Accreditation Pact", "category": 8, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly, resilience up slightly via continuity"}, "grounds": "conditioning emergent hosting offer on EU control"}]}
```
## Priority
M3 EU Grid and Critical Services Hardening Shield, because keeping essential services running through AI-enabled disruption requires mandatory segmentation to actually finish before winter, and it directly serves the two-year commitment while the new pact only buys time.
## In practice
We push M3 to completion via the TTE Council and ENISA: segmentation orders are verified operator-by-operator, CEF-Digital co-financing is released only on proof of isolation, and CERT-EU runs the live drill with the autumn-hit power and water operators as the examined case. Gigafactory grid slots stay queued behind hardening, with DG COMP cover for disclosure to hold industry cooperation.

We stand up M4 through DG CNECT with Civil Protection funding: the single EU hub takes joint cyber-agentic reports, national teams lead with the Corps deploying on cross-border cascade, and EU-hosted fallbacks for police/border replace the suspended US lab access using the newly accredited sovereign hosting. Accreditation itself is run by DG CNECT and the Data Protection Board under NIS2/GDPR: no accreditation, no public procurement, with key-escrow and kill-switch access as the non-negotiable price for market access.
