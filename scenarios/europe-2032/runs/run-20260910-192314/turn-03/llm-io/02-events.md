...
- characters 377-1996: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2040-4694: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4750-5592: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5655-29882: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 30237-30405: `{{event_history}}` from the run's own event record
...
- `emergent_eu_compute_commons` -- first noted turn 1, listed in 2 turn(s) so far: Major European research universities and open-source collectives announce a coordinated compute-pooling pact to replicate Mythos-class capabilities within Europe, attracting significant private donations and volunteer compute.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, knowledge_work_augmented
