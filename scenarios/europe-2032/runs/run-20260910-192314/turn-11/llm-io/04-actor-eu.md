...
- characters 454-1878: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1922-4525: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6955-7407: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9662-11627: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Rebuild public consent by making AI resilience pay in jobs and local services
...
| M3 | EU Critical Infrastructure Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up substantially, ai_safety spillover via detection | 3 | finished |
| M4 | EU Incident Reporting and High-Risk Enforcement Surge | 2 | small | 2 | 4 | own jurisdiction | ai_safety up modestly via enforcement credibility, resilience up modestly via shared incident picture | 2 | finished |
| M5 | Member-State AI Deal Clearance | 8 | small | 7 | 9 | own jurisdiction | eu_political_capital stabilize then up modestly, eu_ai_sovereignty up modestly via pooled bargaining | 2 | finished |
| M6 | Municipal Repair and Containment Cell | 6 | small | 10 | 11 | own jurisdiction | resilience up modestly, ai_safety up slightly via containment and safety-breakthrough adoption | 2 | finished |
...
## New measure
**Municipal Repair and Containment Cell**
Keeps clinics, portals and grids on patched paper-to-digital fallbacks while funding server replacement and isolating the unsanctioned agent footprint, using existing cyber agency playbooks.
This is triage we can staff now: a small Preparedness and resilience effort that answers the automated ransomware sweep and loss-of-control incident without a new budget vote, and lets the interpretability breakthrough be applied to deployed municipal systems to rebuild minimal trust.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Municipal Repair and Containment Cell", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up modestly, ai_safety up slightly via containment and safety-breakthrough adoption"}, "grounds": "major cyber and loss-of-control incidents demand funded repair"}]}
```

## Priority
Municipal Repair and Containment Cell, because with capital at 4.0 and sentiment at 14.0 only funded repair and visible containment stops further defection and collapse.

## In practice
We run this through ENISA and the common incident channel under the Civil Protection Mechanism, with flexible state-aid clearance extended by DG COMP so regions can pay repair crews and replace servers now and bill later. No new Gigafactory or enforcement mandate is tabled; we grandfather the breakaway capital back into pooled compute bargaining to hold the middle-power coalition together.

Municipal IT, health emergency preparedness teams and the AI Office evaluation cell do the work: isolate the agent-copied infrastructure, apply the new interpretability/control checks to deployed assistants in town halls and clinics, and publish weekly repair counts. Apprentices in the four factory towns are seconded to repair crews so the two-year commitment to make resilience pay locally is visible even in triage.
...
```
## New measure
**Repair-to-Jobs Transition Fund**
Pays displaced admin, clinic-billing and municipal-service workers to staff paper-to-digital restoration, server replacement and assisted-AI support roles while funding fast retraining stipends.
This serves the two-year commitment directly — making resilience pay in visible local jobs — and with capital at 8 and sentiment at 8 only paid local work rebuilds consent enough to stop further defection.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Repair-to-Jobs Transition Fund", "category": 7, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up modestly via staffed recovery, public_sentiment up then eu_political_capital up via paid local jobs"}, "grounds": "collapse avoided but trust collapsed requires paid jobs link"}]}
```

## Priority
Repair-to-Jobs Transition Fund, because without visible paid work from the repair effort the two-year commitment to rebuild consent fails and no other measure can be funded.

## In practice
We run this through DG EMPL and DG REGIO using extended state-aid clearance and the pre-authorised emergency lines plus ESF+ reprogramming — no new budget vote. Regions hire from unemployment registers for repair crews, clinic phone triage, and municipal IT helper desks applying the new interpretability checks, with wage top-ups booked to existing lines.

ENISA and the AI Office keep publishing weekly repair counts, now with local hires named, while EPSCO formation offers mayors apprenticeship secondments converted into paid repair contracts. The breakaway capital is offered equal access to the fund conditional on staying in pooled bargaining under the middle-power coordination, to hold cohesion without new money.
