...
- characters 453-2046: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2089-4824: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7436-7885: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10091-12104: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M5 | Member-State AI Deal Clearance | 8 | small | 7 | 9 | own jurisdiction | eu_political_capital stabilize then up modestly, eu_ai_sovereignty up modestly via pooled bargaining | 2 | running |
...
## New measure
**Member-State AI Deal Clearance**
Requires member states to notify and clear exclusive hyperscaler or third-country AI capacity deals through the Council before signature, with joint procurement as the alternative offer.
This is an international coordination and leverage instrument (category 8), small and legal rather than fiscal, because with political capital at 2 and a defection actively undercutting the Union line, stopping the bleed outranks starting new builds — and it directly serves our absorb-shocks commitment by keeping sovereign capacity collective.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Member-State AI Deal Clearance", "category": 8, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "eu_political_capital stabilize then up modestly, eu_ai_sovereignty up modestly via pooled bargaining"}, "grounds": "member-state defection undercutting Union line amid US tiering"}]}
```

## Priority
Member-State AI Deal Clearance, because with no measures in flight and a defection in progress, holding a single Union bargaining line is the precondition for any future sovereign build to matter.

## In practice
We move through the Competitiveness Council and the Anti-Coercion Instrument legal base, not through new money. DG TRADE and DG CNECT table an emergency notification regime: any preferential compute, model-access or siting deal gets a 6-week Union review, with the InvestAI consortia and joint procurement offered as the compliant route. The defecting capital is offered grandfathering plus a share of pooled US-tier negotiation, not punishment.

We spend no capital we do not have. With Gigafactories (M1) now finished and the Shield (M3) finished, enforcement runs through existing AI Office and ENISA channels — no new programme, no grid re-sequencing fight with mayors this turn — to let political capital and sentiment bottom out and recover while the clearance rule holds the line.
...
```
## New measure
None this turn. We are waiting for the Deal Clearance to finish staffing and for political capital to recover above survival level before opening a new paid build.

## Store changes
No changes.

## Priority
M5 Member-State AI Deal Clearance, because with capital at 1.0 and a defection in progress, holding one Union bargaining line is the precondition for any absorb-shocks build to matter.

## In practice
We do not open a new programme this turn. The cyber ransomware / poisoned-update sweep is answered with what is already finished and funded: the Critical Infrastructure Shield (M3) playbooks, paper fallbacks and offline backups, ENISA-led triage to cities, clinics and the grid operator, and the Incident Reporting surge (M4) channel for the common picture. No new hardening fund is tabled; mayors get repair crews and heating-aid flexibility, not re-sequencing fights.

We nurse M5 through legal base and staffing in the Competitiveness formation, offering the defecting capital grandfathering plus pooled US-tier negotiation, and we bank the quiet productivity evidence from law, accountancy and administration — more output without layoffs — to stabilise sentiment while lit gigafactories and apprenticeships land locally. Bio-uplift and eval-anomaly signals are logged to the AI Office for watch, not for new thresholds we cannot enforce.
