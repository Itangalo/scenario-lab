...
- characters 107-12955: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12998-13188: `{{metrics_json}}` from the run's live metric values
- characters 13281-15875: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15919-17874: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17930-18350: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Notepad
PORTFOLIO CHARGE t13: Repair-to-Jobs Transition Fund −2, Joint Telemetry Seat −2 = −4 (priority has no effect and no cost as capital 9 below 20)
PROPOSAL BONUS: Joint Telemetry Seat (cat 8, small) +3 — loss-of-control incident this turn with days-uncertain containment and invited joint command answers it directly
LEGITIMACY LENDS: capital 9, sentiment 6 -> none, sentiment not above capital
SOVEREIGNTY: 26 last turn, no category 4 finishes, no capacity event, no in-flight build bonus, capability rose 1.0 no deduction = 26
