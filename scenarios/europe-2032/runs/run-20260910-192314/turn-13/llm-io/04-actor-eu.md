...
- characters 454-3048: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 3092-5047: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6183-6635: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9253-10418: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M7 | Repair-to-Jobs Transition Fund | 7 | small | 11 | 14 | own jurisdiction | resilience up modestly via staffed recovery, public_sentiment up then eu_political_capital up via paid local jobs | 2 | running |
...
## New measure
None this turn, while we wait for the Repair-to-Jobs wages and the defence/safety breakthroughs to land in published repairs before spending scarce capital.

## Store changes
No changes.

## Priority
Repair-to-Jobs Transition Fund, because paid local restoration work is the only instrument that can lift sentiment from 6 and capital from 9 back to a level where anything else can be started.

## In practice
We keep DG EMPL/REGIO hiring on existing ESF+ and emergency lines, with ENISA and the AI Office publishing weekly repair counts tied to named local hires and to the new automated-patching and interpretability checks now adopted in municipal cells. Mayors get cover for containment that held, and apprentices stay on paid repair contracts.

We do not open a new budget fight or a new pooled-bargaining vote this turn. We use the middle-power coordination and the joint compute line to hold the defecting member state inside common terms, offering equal access to restoration jobs and to defender tooling from the theorem-proving advance, while letting the visible public-sector wins — shorter waits, restored portals — do the political work.
...
```
## New measure
**Joint Telemetry Seat**
Joins the invited joint cyber command with real-time telemetry sharing and pooled attribution for the agentic loss-of-control class. This and why now: containment was uncertain for days and the Union cannot build this protection alone, so the seat must be taken this turn while the invitation and the incident window are open.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Joint Telemetry Seat", "category": 8, "size": "small", "finish_turn": 15, "applies_to": "coalition", "targeted_effect": "resilience up modestly via pooled telemetry and response"}, "grounds": "invited to joint command after loss-of-control incident"}]}
```

## Priority
Repair-to-Jobs Transition Fund, because paid local repair jobs remain the only path to lift sentiment from 6.0 and capital from 9.0 back to governable levels, even as the joint seat outranks it on urgency.

## In practice
We keep M7 hiring through ESF+ and regional emergency lines, with ENISA and municipal containment cells publishing weekly repairs tied to named local hires to hold mayors and apprentices on board despite late pay in south and east. Restoration tooling from the code/maths advance stays pooled and free to all municipalities including the separate-deal state.

We take the joint cyber command seat via the Critical Infrastructure Shield structures and Council JHA/Cyber Ambassadors, seconding ENISA/CERT-EU analysts and feeding municipal telemetry under existing incident-reporting bases, offering the defecting capital equal access to joint attribution in return for rejoining pooled bargaining. No new budget vote is opened; costs ride on existing secondments until M7 finishes.
