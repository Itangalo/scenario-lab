...
- characters 377-1588: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1632-3471: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3527-4615: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4678-28254: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28609-28702: `{{event_history}}` from the run's own event record
...
- `emergent_gulf_compute_bid` -- first noted turn 2, listed in 1 turn(s) so far: A Gulf sovereign fund publicly offers co-financing for EU AI factory sites tied to preferential inference access, forcing a Council debate on foreign ownership of sovereign compute.
- `emergent_cyber_insurance_crunch` -- first noted turn 2, listed in 1 turn(s) so far: A major European insurer reprices cyber cover after the autumn grid intrusion audit, sharply raising premiums for operators and municipalities and forcing delayed data-centre grid connections.
