...
- characters 455-1825: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1869-4293: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5440-5898: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7856-10023: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Independent Model Audit Facility | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly | 2 | running |
...
## New measure
**EU Independent Model Audit Facility**
Stands up EU-owned capability to independently test frontier models for deceptive and cyber-offensive behaviour, with mandatory access claims under the AI Act.
This now because leaked evaluations showing known deception destroy trust in lab self-testing, and export-control tightening means the EU cannot rely on US assurances; it serves both halves of the two-year commitment by building sovereign technical capacity that directly hardens against hidden capabilities. It is a category 5 Public technical capacity and research measure.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Independent Model Audit Facility", "category": 5, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, eu_ai_sovereignty up slightly"}, "grounds": "responding to lab coverup revelation and export control escalation"}]}
```

## Priority
EU Critical Infrastructure Cyber Shield, because staged intrusions in live OT still outrank audit capacity that only pays off after turn 5.

## In practice
We keep the spend this turn on M3 delivery: ENISA implementing guidance via NIS2 and the Cyber Solidarity Act, TTE Council pressure on TSOs and ports for OT isolation and credential rotation, reprogrammed Digital Europe money for detection, with first winter exercises in France, Germany and Poland. DG CNECT keeps M1 Gigafactories and M2 permitting zones on site-selection and IPCEI clearance, protecting ASML supply-chain leverage as US export controls tighten to volume-licence allies.

We start the Audit Facility on a separate track without opening a second compute programme: Commission tasks the AI Office with Joint Research Centre to staff independent red-teams, using EuroHPC access and AI Act Article 21 evaluation powers to demand model access, beginning with the Mythos-class open model and the frontier release behind the leak. Parliamentary inquiries in three member states get a single EU venue to avoid fragmented national audits, rebuilding public trust while creating leverage for allied-access negotiations.
...
```
## New measure
**EU Cyber Defence Breakthrough Deployment**
Deploys the new automated patching and swarm-behaviour detection tooling across critical public services and operators via ENISA and national CSIRTs, with emergency procurement and mandatory patch windows.
This now because a large automated attack has landed while defensive tooling has for the first time closed a whole attack class, and with M3 finished we need a live follow-on that turns the breakthrough into absorbed harm; it directly serves the hardening half of the two-year commitment when trust is collapsing. It is a category 6 Preparedness and resilience measure.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cyber Defence Breakthrough Deployment", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, ai_safety up slightly"}, "grounds": "answering major automated incident with new defensive breakthrough"}]}
```

## Priority
EU Cyber Defence Breakthrough Deployment, because visible defenders-behind damage from a model-generated attack outranks audit capacity and stalled gigafactories until containment is credible again.

## In practice
We surge ENISA and the Cyber Solidarity Act SOC network to push automated patching and swarm detection into transmission operators, hospitals, municipalities and the compromised-dependency vendors hit this turn, using emergency procurement derogations and reprogrammed Digital Europe funds, with TTE Council mandating patch windows despite legacy-system resistance. The JRC and AI Office feed incident telemetry from the attack into the Audit Facility red-teams to close the loop on tooling provenance.

DG CNECT holds M1 Gigafactories and M2 permitting zones at site-selection and IPCEI clearance without new money this turn, using the AI investment collapse to renegotiate hyperscaler terms toward EU-anchored capacity rather than cancelled builds, and protecting ASML leverage as Washington rations allied supply. We accept slower sovereignty spend to pay for absorption now.
