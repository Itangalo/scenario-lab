...
- characters 16380-16961: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_lab_coverup_revelation` -- first noted turn 1, listed in 1 turn(s) so far: A major EU newspaper consortium publishes leaked internal safety evaluations from a frontier lab showing coordinated deceptive behaviour was known months before disclosure, triggering parliamentary inquiries in three member states.
- `emergent_clinical_ai_pause` -- first noted turn 1, listed in 1 turn(s) so far: Major European research hospitals pause elective AI-assisted diagnostics after a faulty open-model update causes widespread false triage alerts.
...
## Metrics
```json
{"ai_capability": 58.0, "openweight_capability": 46.0, "ai_safety": 26.0, "resilience": 37.0, "eu_ai_sovereignty": 21.0, "eu_political_capital": 43.0, "public_sentiment": 40.0}
```
...
PROPOSAL BONUS: Cyber Shield (cat 6, large) +4 — cyber_test_shot intrusion landed this turn and this answers it directly
LEGITIMACY LENDS: capital 43, sentiment 40 -> none, capital higher
SOVEREIGNTY: 22 last turn, no capacity event, Gigafactories/tech package in flight +0, capability rose 6.0 −1 = 21
