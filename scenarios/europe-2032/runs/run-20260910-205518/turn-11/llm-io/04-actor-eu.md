...
- characters 452-1311: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1355-3415: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5383-5820: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9285-11437: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Survive decoupling through resilient EU-run essential services
...
|---|---|---|---|---|---|---|---|---|---|
| M12 | EU Offline Continuity Corps for Hospitals and Municipalities | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, eu_political_capital up slightly via visible continuity | 2 | running |
...
| M3 | EU Critical Infrastructure Cyber Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Independent Model Audit Facility | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly | 2 | finished |
| M5 | EU Cyber Defence Breakthrough Deployment | 6 | small | 3 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly | 2 | finished |
| M6 | EU Accession to Joint Cyber Command | 8 | small | 4 | 6 | coalition | resilience up substantially | 2 | finished |
| M7 | EU Essential Services Continuity and Bio-Detection Backstop | 6 | small | 5 | 7 | own jurisdiction | resilience up substantially, ai_safety up slightly | 2 | finished |
| M8 | EU Displaced Worker Transition Fund | 7 | small | 6 | 9 | own jurisdiction | public_sentiment up moderately, resilience up slightly, eu_political_capital up slightly via restored consent | 2 | finished |
| M9 | EU Major Incident Containment and Restoration Surge | 6 | small | 7 | 8 | own jurisdiction | resilience up moderately, eu_political_capital up slightly via visible response | 2 | finished |
| M10 | EU Bio-Incident Containment and Continuity Lockdown | 6 | small | 8 | 9 | own jurisdiction | resilience up substantially, eu_political_capital up slightly via visible containment, public_sentiment up slightly | 2 | finished |
| M11 | EU Sovereign Fallback for Essential Services | 6 | small | 9 | 11 | own jurisdiction | resilience up substantially, eu_political_capital up slightly via visible continuity | 2 | finished |
...
## New measure
**EU Offline Continuity Corps for Hospitals and Municipalities**
It puts paid, drilled manual-fallback teams and pre-loaded offline kits into the cut-off hospitals, municipalities and port operators to keep care, records and customs moving without the suspended foreign models.
This now because the cut-off plus ransomware showed the fallback exists on paper only, and with sovereignty at 10 and trust collapsed only visible kept-open wards and counters can stop fragmentation before tenders land; a category 6 Preparedness and resilience stopgap under the survive-decoupling commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Offline Continuity Corps for Hospitals and Municipalities", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, eu_political_capital up slightly via visible continuity"}, "grounds": "frontier access cut and ransomware exposed paper-only fallback"}]}
```

## Priority
M11 EU Sovereign Fallback for Essential Services, because getting EU-hosted triage and clean images actually running outranks staffing around them while hospitals are dark.

## In practice
We execute M11 through HERA, ENISA and the Health and JHA Councils on Article 122 and Cyber Solidarity Act money: push pooled clean images to M10 lockdown nodes, consolidate triage-compute from Gigafactory shells, enforce emergency procurement to EU-hosted models only, and hold the two hyperscaler capitals to mandatory enrolment and health-data repatriation in exchange for frozen infringement.

We stand up the new Corps from the same base with EPSCO and the Civil Protection Mechanism: seconded clinicians, municipal clerks and ENISA reservists, paid from reprofiled M8 voucher leftovers, deployed first to the two stalled hospital networks, the hit port community system and the regional health authority on paper procedures and offline kits.

We ask nothing new of publics beyond kept-open services: no new build promises, daily restoration reporting to claw back consent while EU models are hardened under load.
...
```
## New measure
**EU Bio-Detection and Essential Medicines Continuity Shield**
Deploys pooled bio-detection, offline clinical protocols and EU-made fallback production for the new cures so hospitals can use the breakthrough without foreign models and catch engineered-pathogen signals early.
This now because the genome-design finding opens a four-turn bio window while the medical breakthrough will otherwise deepen dependence, and under the survive-decoupling commitment resilience must cover both pathogens and cures.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Detection and Essential Medicines Continuity Shield", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, public_sentiment up slightly via visible cures access"}, "grounds": "bio uplift precursor plus access-denied medical breakthrough"}]}
```

## Priority
M12 EU Offline Continuity Corps for Hospitals and Municipalities, because kept-open wards and counters are the only consent lever left while sovereignty is at 7 and the new shield builds.

## In practice
We run M12 through the Civil Protection Mechanism and EPSCO/Health Council on reprogrammed EU4Health and rescEU money: extend paid secondments and offline kits to the second dark hospital network and paper-only councils, with daily ward-open reporting to stop quiet defection to hyperscaler side-deals.

We launch the new shield via HERA, ECDC and EMA under Article 122 and the Emergency Framework: pooled sequencing and wastewater detection to M10 lockdown nodes, emergency tech-transfer and joint procurement to make the new therapies on EU-hosted triage and manufacturing, and conditional access so the defecting member state must re-enrol to receive pooled doses and detection feeds. ENISA reservists harden the rewired factory-shell compute running EU triage models, applying the new interpretability control result where it lands on deployed systems.
