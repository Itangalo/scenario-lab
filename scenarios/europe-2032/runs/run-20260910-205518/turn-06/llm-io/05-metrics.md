...
- characters 105-13598: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13641-13833: `{{metrics_json}}` from the run's live metric values
- characters 13926-14852: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14895-16991: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17047-17668: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Essential Services Continuity and Bio-Detection Backstop | 2 |
| M8 | EU Displaced Worker Transition Fund | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election returned consolidation president, takes office turn 6"}]}
```
```
