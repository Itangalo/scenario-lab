...
- characters 453-1426: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1469-3621: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5767-6143: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9220-10474: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_certified_liability_pool (emergent event):** Several large EU insurers jointly launch a certified-AI liability pool offering discounted cover only where Seville-kit telemetry and incident reporting are in place, creating a de-facto private standard for insurable automation.
...
- `two_year_commitment` (commitment): Hold protection and certifiable resilience while rebuilding leverage from what still works
...
| M10 | EU Shop-Floor and Office Transition Fund | 7 | small | 8 | 10 | own jurisdiction | public_sentiment up, eu_political_capital up slightly via visible protection, resilience up slightly | 2 | finished |
| M11 | EU Assured Systems Adoption Patch | 5 | small | 9 | 11 | own jurisdiction | ai_safety up materially, resilience up slightly via certified deployed systems | 2 | finished |
...
## New measure
None this turn, while we conserve the last point of capital and live off certified islands until political room returns.

## Store changes
No changes.

## Priority
No priority named this turn, because with no measures in flight and no capital to start one, naming a push we cannot fund would only accelerate fragmentation.

## In practice
We hold what holds. DG CNECT and ENISA keep the Seville thresholds and assured-systems guidance live where installed — Rotterdam, Antwerp, patched grids — with Recovery Corps teams logging restores, and DG DIGIT keeps the two city back-office wins running as proof protection can pay. No new build, no new levy, no licence fight we cannot win.

DG ECFIN and DG MOVE roll over the emergency state guarantees for uninsured grid and port operators on existing reprogrammed funds, strictly conditioned on certification and incident reporting, while EPSCO maintains wage-bridges for displaced pickers and warehouse staff. With open weights now at the frontier and robots taking picking, sorting and welding from outside suppliers, we publicly name the boundary — checkable manual tasks gone, judgement work manual — and ask Washington only for spares continuity, to avoid another capital-burning refusal.
...
```
## New measure
**Certified Liability Pool Anchor**
It turns the emergent insurers' certified-AI liability pool into EU-backed insurable standard by co-guaranteeing certified operators and conditioning cover on Seville-kit telemetry and incident reporting.
This and now because with capital at 2, sentiment at 7, frontier access cut off and frontier open weights unrecallable, the only lever that expands protection without new build is making certification insurable where it already works.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Certified Liability Pool Anchor", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, ai_safety up slightly, eu_political_capital up slightly via market cover replacing state guarantees"}, "grounds": "insurers launched certified pool while state guarantees curdle politically"}]}
```

## Priority
Certified Liability Pool Anchor, because replacing taxpayer guarantees with discounted private cover for certified sites is the only way to hold the islands after access denial while bio and control precursors open.

## In practice
DG FISMA and DG CNECT sign a memorandum with the pool insurers anchoring ENISA Seville thresholds as the eligibility test, with DG ECFIN rolling the existing emergency guarantees into a first-loss backstop only for certified grid, port and hospital operators that stream telemetry and report incidents. No new levy this turn; funding is reprogrammed guarantees plus pool capital.

DG EMPL keeps wage-bridges for displaced pickers while Recovery Corps and the two fast-permit back-offices are held as the public proof that certified restores work, and we publicly log the US cutoff and open-weight spread as the reason we certify use rather than chase frontier access we were denied. Contractors in blockaded states are offered certified-site fast-track permitting tied to pool cover to break the siting freeze.
