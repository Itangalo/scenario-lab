...
- characters 377-1417: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1461-3158: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3214-4008: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4071-32556: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32911-33270: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Municipal Recovery Corps −2, priority −1 = −9
PROPOSAL BONUS: EU Municipal Recovery Corps (cat 6, small) +3 — cyber_major_incident landed this turn disrupting municipalities/hospitals and this answers it directly
...
- `emergent_datacentre_backlash_blockade` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated protests block permitting and construction at two planned EU compute sites, citing power costs and distrust after the logistics-agent scandal, delaying domestic capacity expansion by months.
...
- Turn 2 (3 turn(s) ago): capability_jump, eval_anomaly_reports, taiwan_tension_rise, eu_frontier_access_denied
- Turn 3 (2 turn(s) ago): loss_of_control_incident
- Turn 4 (1 turn(s) ago): cyber_major_incident, capability_jump, eval_anomaly_reports, openweight_frontier_release, labour_displacement
