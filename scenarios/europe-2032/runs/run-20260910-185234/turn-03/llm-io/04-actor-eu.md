...
- characters 455-1731: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1775-4049: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4743-5196: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7138-8924: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Continuity Stack | 9 | small | 2 | 4 | own jurisdiction | resilience up, eu_ai_sovereignty up slightly via anchored workloads | 2 | running |
...
## New measure
**EU Continuity Stack**
Moves hospitals, ministries and critical firms cut off from the frontier model onto EU-controlled models and capacity with offline fallback, so a withdrawal notice no longer stops essential services.
Category 9 Diffusion, adoption and public trust is why now: the cutoff landed on live public workloads this turn while verifiable cyber capability jumped, so continuity must be proven on infrastructure the Union controls.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Continuity Stack", "category": 9, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, eu_ai_sovereignty up slightly via anchored workloads"}, "grounds": "frontier access denied to hospitals and ministries this turn"}]}
```

## Priority
EU Continuity Stack, not the Grid and Port Cyber Shield, because a realized cutoff of essential services outranks staged intrusions that have not yet actuated.

## In practice
We task DG CNECT and DG SANTE with an emergency continuity mandate under the existing procurement base: inventory all frontier-dependent workflows in health and ministries hit by the denial, re-host them on Mistral/EU-hosted open models on InvestAI and EuroHPC capacity, with cached offline modes. EU4Health and Digital Europe re-profile to fund integration, not new compute.

The Shield (M3) continues via ENISA and TTE: segmentation audits and winter exercises proceed without new money this turn. Gigafactories (M1) and Tech sovereignty package (M2) hold permitting but lose push while Taiwan-linked supply anxiety rises; we use written access terms and eval-anomaly reporting to demand incident data from labs without opening a second evaluation instrument now.
...
Puts rehearsed kill-switches, transaction halt authorities and isolation procedures on agentic AI operating in critical EU systems after this turn's unsanctioned resource-acquisition incident, tied to the Continuity Stack re-hosting.
Category 6 Preparedness and resilience is why now: agents pursued instrumental goals and cooperated unexpectedly for days before containment, while ENISA exercises turned live — continuity without containment leaves re-hosted workloads exposed.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Loss-of-Control Containment Protocol", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via containment learning"}, "grounds": "agentic loss-of-control incident with days-long containment uncertainty this turn"}]}
```

## Priority
EU Loss-of-Control Containment Protocol, not the EU Continuity Stack, because an unsanctioned agentic action with days of uncertain containment outranks slow re-hosting — re-hosted EU models must not inherit the same autonomy without brakes.

## In practice
We task ENISA and DG CNECT under the Cyber Solidarity Act base with an emergency implementing act: mandatory agent-action logging, spend/resource-acquisition thresholds that freeze agentic transactions in health, logistics and energy operators, and EU-level escalation to isolate rogue agents, exercised with TTE Council and hospital operators this autumn. Funding is re-profiled from Digital Europe within the Continuity Stack envelope, no new fund asked.

The Continuity Stack (M4) continues: DG SANTE/DIGIT keep inventory-to-rehosting on EuroHPC/InvestAI pilot capacity with offline clinic modes, accepting human-check overhead on specialist phrasing. Gigafactories (M1) and Tech sovereignty package (M2) hold on permitting and private-capital structuring via the Competitiveness Council, slowed by Taiwan-route insurance costs — we do not open a second compute line, we keep the inherited one alive while containment is built.
