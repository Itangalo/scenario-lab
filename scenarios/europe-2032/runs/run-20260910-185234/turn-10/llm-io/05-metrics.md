...
- characters 106-13354: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13397-13588: `{{metrics_json}}` from the run's live metric values
- characters 13681-15899: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15942-18212: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18268-18808: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M11 | EU Assured Systems Adoption Patch | 2 |
...
**emergent_municipal_insurance_retreat (emergent event):** Municipal insurers across several member states jointly refuse to underwrite AI-operated grid and port control systems after the January logistics incident, forcing emergency state guarantees.
...
## Metrics
```json
{"ai_capability": 66.0, "openweight_capability": 63.6, "ai_safety": 21.0, "resilience": 44.0, "eu_ai_sovereignty": 13.0, "eu_political_capital": 1.0, "public_sentiment": 9.0}
```
