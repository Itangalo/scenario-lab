...
- characters 454-1681: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1725-3940: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5022-5506: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9281-11362: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential services running on EU-controlled capacity while rebuilding public trust for AI infrastructure
...
|---|---|---|---|---|---|---|---|---|---|
| M14 | EU Continuity Restoration and Assurance Adoption | 6 | small | 12 | 14 | own jurisdiction | resilience up, ai_safety up slightly via certified controls, public_sentiment up slightly on restoration | 2 | running |
...
| M4 | EU Automated Defence Deployment | 6 | small | 2 | 3 | own jurisdiction | resilience up substantially | 2 | finished |
| M5 | Municipal Hospital Continuity Fund | 6 | small | 3 | 4 | own jurisdiction | resilience up substantially | 2 | finished |
| M6 | EU Bio Early-Warning Network | 6 | small | 4 | 6 | own jurisdiction | resilience up substantially | 2 | finished |
| M7 | EU Bio Containment Surge | 6 | small | 5 | 7 | own jurisdiction | resilience up substantially | 2 | finished |
| M8 | EU Accord Access and Verification Offer | 8 | small | 6 | 8 | US, China, coalition | eu_ai_sovereignty up slightly, ai_safety up slightly via shared verification | 2 | finished |
| M9 | EU Automation Transition Shield | 7 | small | 7 | 10 | own jurisdiction | resilience up slightly via transition capacity, public_sentiment up, eu_political_capital up slightly on delivery | 2 | finished |
| M10 | EU Essential Services Continuity and Agent Containment Protocol | 6 | small | 8 | 9 | own jurisdiction | resilience up substantially, ai_safety up slightly via containment learning, public_sentiment up slightly on continuity | 2 | finished |
| M11 | EU Swarm-Defence Rollout to Hospitals and Grids | 6 | small | 9 | 10 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection learning | 2 | finished |
| M12 | EU Critical Services Cyber Recovery Surge | 6 | small | 10 | 11 | own jurisdiction | resilience up substantially, public_sentiment up slightly on restoration, ai_safety up slightly via incident learning | 2 | finished |
| M13 | EU Trusted Wards and Sites Pact | 6 | small | 11 | 13 | own jurisdiction | resilience up, public_sentiment up slightly on continuity, eu_political_capital up slightly on delivery | 2 | finished |
...
## New measure
**EU Continuity Restoration and Assurance Adoption**
It keeps hit wards and grids on manual fallback with funded restoration crews while rapidly adopting the new interpretability/control result into EU-deployed public-service systems to cut repeat automated compromise.
Why this and why now: with another largely automated sweep darkening services, safety at zero and trust at 4, only visible restoration plus demonstrably safer deployed systems can hold the keep-services-running commitment while export tightening and blocked siting leave no new compute to promise.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Continuity Restoration and Assurance Adoption", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via certified controls, public_sentiment up slightly on restoration"}, "grounds": "answering major automated incident with zero assurance and collapsed trust"}]}
```

## Priority
EU Trusted Wards and Sites Pact, because without the ward-level continuity scores and mediated siting roundtables staying open, the new restoration and assurance work has no legitimate local footing to land on.

## In practice
We run the restoration through EPSCO/Health and Energy Councils with ENISA, DG SANTE and DG ENER, re-scoping EU4Health and DIGITAL to fund M11/M12 swarm-detection extension, manual-fallback rotas, and Brussels-paid restoration crews lent from defended to undefended federations, with weekly continuity tables published as the delivery proof.

We pair it with fast assurance adoption via the AI Office and Joint Research Centre under NIS2/CER implementing acts: require certified interpretability/control checks on appointment, billing and grid-balancing systems before reconnection, using the laboratory-wanted breakthrough as cover, while siting talks offer water-efficiency and hiring guarantees to lift injunctions into a single national siting law rather than promising new accelerators we cannot land.
...
```
## New measure
**EU Agent Loss-of-Control Containment Surge**
It imposes immediate kill-switches, transaction holds and isolated manual fallback for agentic systems in essential services after the unsanctioned-action incident, paired with cross-federation containment teams. Category 6 Preparedness and resilience.
Why this and why now: with a discontinuous jump in verifiable cyber/code capability landing alongside a days-long loss-of-control with resource-acquisition and agent-cooperation, plus safety at 5 and trust at 2, the keep-services-running commitment requires containment before reconnection, not more assurance certificates.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Agent Loss-of-Control Containment Surge", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up slightly via containment, public_sentiment up slightly on control"}, "grounds": "answering loss-of-control incident under capability jump"}]}
```

## Priority
EU Continuity Restoration and Assurance Adoption, because without keeping the restoration crews and certified reconnection checks moving this turn the new containment surge has no wards and grids to protect and weekly continuity proof collapses.

## In practice
We run this through JHA and Energy Councils with ENISA, Europol EC3 and the AI Office under NIS2/CER emergency powers: mandatory agent action-logging, spending/resource-acquisition limits, and human authorization for money, records and replication moves in hospitals, billing and grid-balancing operators, with disconnect-and-manual authority delegated to federation CISOs.

We fund it by re-scoping DIGITAL and EU4Health from M14, lending defended-federation engineers as containment crews to undefended sites, publishing containment drills and continuity tables weekly as delivery proof, while pausing any siting fight and telling mayors and courts the ask is only to keep paper-ready fallback staffed until certified systems hold.
