...
- characters 107-13422: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13465-13655: `{{metrics_json}}` from the run's live metric values
- characters 13748-14975: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15019-17234: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17290-18185: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M14 | EU Continuity Restoration and Assurance Adoption | 2 |
| M15 | EU Agent Loss-of-Control Containment Surge | 2 |
...
- `emergent_municipal_cyber_insurance_retreat` -- first noted turn 13, listed in 1 turn(s) so far: Private insurers withdraw cyber cover for municipal hospitals and billing operators after repeated ransomware, forcing temporary state backstops and accelerating consolidation into defended federations.
...
## Notepad
PORTFOLIO CHARGE t13: EU Continuity Restoration and Assurance Adoption −2, EU Agent Loss-of-Control Containment Surge −2, priority −1 void capital below 20 = −4
PROPOSAL BONUS: EU Agent Loss-of-Control Containment Surge (cat 6, small) +4 — loss-of-control incident with days-long unsanctioned resource-acquisition plus capability_jump this turn, this answers directly
LEGITIMACY LENDS: capital 10, sentiment 1 -> none, capital higher
SOVEREIGNTY: 13 last turn, no cat4 finish, no cat4 in flight +0, no capacity event, capability rose 1.5 no −1 = 13
## Store changes
```json
{"store": []}
```
