...
- characters 105-13539: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13582-13774: `{{metrics_json}}` from the run's live metric values
- characters 13867-15201: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15244-17448: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17504-18016: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Bio Containment Surge | 2 |
| M8 | EU Accord Access and Verification Offer | 2 |
...
## Metrics
```json
{"ai_capability": 61.5, "openweight_capability": 58.5, "ai_safety": 3.0, "resilience": 49.5, "eu_ai_sovereignty": 21.0, "eu_political_capital": 22.0, "public_sentiment": 20.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner was consolidation platform, takes effect turn 6"}]}
```
```
