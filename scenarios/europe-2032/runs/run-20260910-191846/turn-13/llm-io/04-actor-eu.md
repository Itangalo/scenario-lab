...
- characters 452-2133: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2177-4604: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6763-7285: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10078-12021: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): A Europe that endures cutoff and ungovernable frontier by hardening essentials, containing agents and bio uplift, and rebuilding sovereign fallback
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Cyber Restoration and Embodied-AI Containment Corps | 6 | small | 12 | 14 | own jurisdiction | resilience up moderately, ai_safety up slightly | 2 | running |
...
| M4 | EU Chip Leverage Compact | 8 | small | 2 | 4 | coalition | eu_ai_sovereignty up moderately, eu_political_capital stabilised | 2 | finished |
| M5 | Municipal Care Continuity Corps | 6 | small | 4 | 6 | own jurisdiction | resilience up moderately, ai_safety up slightly | 2 | finished |
| M6 | EU Displacement and Embodied-AI Transition Shield | 7 | small | 6 | 9 | own jurisdiction | resilience up moderately, eu_political_capital stabilised, public_sentiment up slightly | 2 | finished |
| M7 | EU Cutoff Continuity and Joint Threat Adhesion | 6 | small | 7 | 9 | own jurisdiction | resilience up moderately, eu_political_capital stabilised | 2 | finished |
| M8 | EU Essential Continuity and Agent Containment Pact | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, ai_safety up slightly | 2 | finished |
...
## New measure
**EU Cyber Restoration and Embodied-AI Containment Corps**
What it actually does is put ENISA-led restoration teams and municipal fallback technicians into the ransomware-hit public services while imposing interim containment on deployed robots in critical sites. This and now because the automated attack has made defenders visibly behind and commercial robots are landing on our industrial base with no doctrine, and with zero capital we can only afford triage that keeps essentials lit.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cyber Restoration and Embodied-AI Containment Corps", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly"}, "grounds": "major automated cyber incident plus embodied deployment requires immediate restoration"}]}
```

## Priority
EU Cyber Restoration and Embodied-AI Containment Corps, because with no measures in flight and services hit it is the only instrument that restores payments and care while holding robot deployment inside safety screening.

## In practice
We act by Commission emergency recommendation and Council JHA conclusions, not new law: ENISA directs ring-fenced patching and swarm-detection tooling to the hit registries and payment clearing, EuroHPC reallocates remaining inference quotas to hospitals, power, water and payments, with HERA/ECDC bio-detection on the same footprint.

We fund municipal fallback technicians and Transition wage support from reprogrammed ESF+ and Civil Protection Mechanism, and task national coordinators to enforce kill-switches, caps and synthesis screening on self-copying agents. For embodied AI we impose interim works-council screening and critical-site exclusion for foreign control models, offering pooled procurement to the breakaway member state if it returns to common screening.
...
```
## New measure
**EU Neutrality Corridor and Middle-Power Supply Pact**
What it actually does is declare EU infrastructure non-belligerent and pool export-licence, cable, fab and compute bargaining with middle powers to keep essentials supplied through US-China war. This and now because shooting war makes fabs, cables and data centres targets and Brussels has no leverage alone, while the coalition window is open and the ransomware sweep already proved triage alone cannot hold.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Neutrality Corridor and Middle-Power Supply Pact", "category": 8, "size": "small", "finish_turn": 15, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up moderately, eu_political_capital stabilised"}, "grounds": "great-power war targeting supply chain requires joint middle-power leverage"}]}
```

## Priority
EU Neutrality Corridor and Middle-Power Supply Pact, because without protected cables, fabs access and compute through the war nothing in flight — restoration, hospitals, payments — survives, and it outranks our hardening commitment for this turn.

## In practice
We act by European Council neutrality conclusions plus Anti-Coercion Instrument readiness: no EU territory, cables, fabs or EuroHPC for offensive targeting, ENISA-swarm defence extended to subsea landing stations and satellite ground segments, ring-fenced power/water/payments kept under M9 restoration corps. We offer the breakaway member state re-entry via shared supply, not screening conditionality.

We stand up the pact in Trade/Foreign Affairs Council with Japan, Korea, Singapore, UAE and others holding lithography, packaging, cable repair and cloud: aligned export licences, joint bargaining for US chip/model waivers and Chinese robot spares, shared evaluation of unreleased-system anomaly, paid from reprogrammed Global Gateway and EIB, with works-council robot exclusions held until joint safety screening lands.
