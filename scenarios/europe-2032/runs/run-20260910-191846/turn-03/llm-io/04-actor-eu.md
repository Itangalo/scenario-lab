...
- characters 455-2400: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2444-4631: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5181-5624: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7244-9221: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Chip Leverage Compact | 8 | small | 2 | 4 | coalition | eu_ai_sovereignty up moderately, eu_political_capital stabilised | 2 | running |
...
```
## New measure
None this turn — we are waiting for the Shield to finish landing and for capital to recover before adding new costs.

## Store changes
No changes.

## Priority
M3 EU Critical Systems Shield, because the automated attack now landing outranks our own direction and the Shield is the only instrument that absorbs it without permission.

## In practice
We surge ENISA and EU-CyCLONe as the operational core: swarm detection already pushed to transmission operators, ports, water and telecoms becomes mandatory isolation drills, with Digital Europe and CER Directive emergency powers used to order defensive disconnects and mutual aid rather than negotiate them. HERA stockpiles and health continuity are held on watch, not built, this turn.

We hold M1 Gigafactories and M2 Tech sovereignty package at their current burn without new fights over permitting zones, and we keep M4 Chip Leverage Compact to a holding line in the Trade Council — pooling Dutch-German-French licensing on paper and offering the defecting capital a share of any secured compute to stop the side-deal, while conserving every point of capital for incident absorption.
