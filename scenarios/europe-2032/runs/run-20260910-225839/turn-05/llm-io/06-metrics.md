...
- characters 106-13635: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13678-13869: `{{metrics_json}}` from the run's live metric values
- characters 13962-15530: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15574-17667: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17723-18716: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|---|---|---|---|---|---|---|
| M4 | EU Critical Infrastructure Cyber Shield | 6 | large | 2 | 5 | own jurisdiction | resilience up strongly, ai_safety up modestly via incident learning | 3 | finished |
| M5 | EU AI Incident Reporting and High-Risk Enforcement Fix | 2 | small | 3 | 5 | own jurisdiction | ai_safety up modestly, resilience up modestly, public_sentiment stabilised | 2 | finished |
...
- `emergent_data_centre_siting_freeze` -- first noted turn 5, listed in 1 turn(s) so far: Municipal councils in three member states pass coordinated moratoria on new data-centre grid connections over power prices and water use, halting two gigafactory-adjacent builds for months.
- `emergent_private_compute_pause` -- first noted turn 5, listed in 1 turn(s) so far: Major private co-investors formally pause EU gigafactory build-out commitments for H2 2028, citing political paralysis and power-price uncertainty, leaving sites permitted but unfunded.
...
SOVEREIGNTY: 16 last turn, no cat4 finish, Gigafactories in flight +0, embodied_ai_deployment t5 −2, capability rose 1.0 −0 = 14
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "pending"}}]}
```
