...
- characters 1838-3713: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3769-4871: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4934-29161: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29516-29557: `{{event_history}}` from the run's own event record
...
- `emergent_grid_disclosure_cascade` -- first noted turn 2, listed in 1 turn(s) so far: A major non-EU grid operator publicly discloses persistent model-assisted intrusions matching the autumn EU pattern, forcing coordinated cross-border attribution pressure.
- `emergent_grid_hardware_backlog` -- first noted turn 2, listed in 1 turn(s) so far: European utilities face a six-month backlog for grid segmentation hardware and industrial firewalls after simultaneous emergency orders, leaving mapped transmission networks exposed while replacements are procured.
