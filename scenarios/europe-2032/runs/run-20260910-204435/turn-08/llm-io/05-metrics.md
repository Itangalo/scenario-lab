...
- characters 105-13391: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13434-13626: `{{metrics_json}}` from the run's live metric values
- characters 13719-14732: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14775-16841: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16897-17992: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M10 | EU Upstream Bargain for Compute Continuity | 2 |
...
PORTFOLIO CHARGE t7: EU Breakthrough Defence Rollout to Municipal Shield −2, EU Essential Continuity Stack −2, priority −1 = −5
PROPOSAL BONUS: EU Essential Continuity Stack (cat 4, small) +4 — eu_frontier_access_denied landed this turn and this answers it directly
LEGITIMACY LENDS: capital 29, sentiment 21 -> none, capital is higher
SOVEREIGNTY: 21 last turn, InvestAI Gigafactories finishes t7 +5, no capacity in flight +0, eu_frontier_access_denied t7 −1, capability rose 0.8 −0 = 25
...
## Metrics
```json
{"ai_capability": 65.0, "openweight_capability": 62.5, "ai_safety": 18.0, "resilience": 60.0, "eu_ai_sovereignty": 27.0, "eu_political_capital": 31.0, "public_sentiment": 19.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M10", "fields": {"finish_turn": 9}, "grounds": "named priority"}]}
```
```
