# Store: The European Union (turn 4)

## measures

| id | name | category | size | started_turn | finish_turn | applies_to | targeted_effect | cost_per_turn | status |
|---|---|---|---|---|---|---|---|---|---|
| M1 | InvestAI Gigafactories | 4 | large | 0 | 7 | own jurisdiction | eu_ai_sovereignty up, via €200bn for four to five sites | 3 | running |
| M2 | Tech sovereignty package | 4 | large | 0 | 6 | own jurisdiction | eu_ai_sovereignty up, via €200bn of private capital for AI data centres by 2036 and accelerated-permitting zones | 3 | running |
| M3 | EU Grid and Critical Systems Hardening Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up materially, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Swarm Detection Fast-Track | 6 | small | 2 | 3 | own jurisdiction | resilience up, ai_safety up slightly via detection | 2 | finished |
| M5 | Municipal Equal Protection Deployment | 6 | small | 3 | 4 | own jurisdiction | resilience up materially in lagging operators | 2 | finished |
| M6 | EU Trusted Hosting and Shield Continuity Pact | 6 | small | 4 | 6 | own jurisdiction | resilience up, public_sentiment up slightly by defusing bans | 2 | running |

## standing

| id | posture |
|---|---|
| S1 | undecided |

## Changes this turn

- **applied** `M6` — {"op": "add", "table": "measures", "fields": {"name": "EU Trusted Hosting and Shield Continuity Pact", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up slightly by defusing bans"}, "grounds": "hold hosting base for shield amid bans and new open release"}
  - Grounds: hold hosting base for shield amid bans and new open release
