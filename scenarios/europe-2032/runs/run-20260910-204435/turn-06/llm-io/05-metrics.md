...
- characters 105-13491: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13534-13726: `{{metrics_json}}` from the run's live metric values
- characters 13819-16108: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 16151-18229: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18285-18841: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election won on tiered access platform, administration takes office turn 6"}]}
```
```
