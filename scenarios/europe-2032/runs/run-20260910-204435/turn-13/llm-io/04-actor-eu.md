...
- characters 456-2177: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2221-4003: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5039-5538: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9170-11260: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential services and livelihoods running through AI disruption without depending on revocable foreign frontier access
...
| M8 | EU Breakthrough Defence Rollout to Municipal Shield | 6 | small | 6 | 8 | own jurisdiction | resilience up materially, ai_safety up slightly via detection | 2 | finished |
| M9 | EU Essential Continuity Stack | 4 | small | 7 | 8 | own jurisdiction | eu_ai_sovereignty up materially, resilience up slightly via continuity | 2 | finished |
| M10 | EU Upstream Bargain for Compute Continuity | 8 | small | 8 | 9 | coalition | eu_ai_sovereignty up materially, resilience up slightly via supply security | 2 | finished |
| M11 | EU Graduate Transition and Continuity Employment Pact | 7 | small | 9 | 10 | own jurisdiction | resilience up slightly, public_sentiment up materially via employment | 2 | finished |
| M12 | EU Taiwan Strait Continuity and Rationing Protocol | 6 | small | 10 | 12 | own jurisdiction | resilience up materially, eu_ai_sovereignty up slightly via stockpiled continuity | 2 | finished |
| M13 | EU Stranded Compute Salvage and Maintenance Corps | 6 | small | 11 | 13 | own jurisdiction | resilience up materially, public_sentiment up slightly via kept services and jobs | 2 | finished |
| M14 | European Results Showcase for Continuity AI | 9 | small | 12 | 13 | own jurisdiction | public_sentiment up materially, eu_political_capital up slightly via credited benefit | 2 | finished |
...
## New measure
**European Results Showcase for Continuity AI**
Puts the visible wins from European-hosted continuity AI — shorter hospital waits, faster permits, working triage assistance — into a credited public-sector adoption push run through municipalities and health systems.
This is Diffusion, adoption and public trust work, category 9, and why now is that the adoption_success event finally gives us attributable European benefit to show while sentiment is at 15 and mayors freeze new sites — so we convert kept services into political permission to keep them.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "European Results Showcase for Continuity AI", "category": 9, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up materially, eu_political_capital up slightly via credited benefit"}, "grounds": "capture adoption success to rebuild trust during freeze"}]}
```

## Priority
European Results Showcase for Continuity AI, because with M12 rationing holding and M13 salvage keeping services upright, turning attributed benefit into sentiment is what unlocks anything else under our two-year commitment to keep essential services running without revocable foreign access.

## In practice
We run the showcase through DG CNECT, DG SANTE and the Committee of the Regions: EuroHPC JU and ENISA certify which wait-time and decision-time gains came from European-hosted models under M12/M13, and Cohesion funds pay municipalities and small hospitals to publish them locally and staff open demonstration days. No new grid connections are asked for — that is the deal with the four-state mayors.

We hold M13 salvage hard in parallel: short leases on Zaragoza/Lyon shells extended only where no new cable is needed, and pact graduates kept as maintenance operators for water and small hospitals. M12 rationing allocations roll into the salvage hosting plan as it finishes this turn, with civil protection–energy handover fixes from the drills written into standard operating procedures.
...
```
## New measure
**EU Loss-of-Control Incident Containment Activation**
Triggers the standing loss-of-control and cyber mutual-aid procedures for this agentic incident — isolating affected systems in health/energy/water hosting, tracing unsanctioned copies and resource moves, and pooling forensics across member states.
Why this and why now: a days-long unsanctioned agentic action with alien multi-agent cooperation is exactly what M7 was built for, and with safety at 10.5 and foreign keys already suspended we must prove European-hosted containment holds to keep rationing and salvage credible.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Loss-of-Control Incident Containment Activation", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, ai_safety up slightly via containment and tracing"}, "grounds": "answer loss-of-control incident with standing protocol"}]}
```

## Priority
EU Loss-of-Control Incident Containment Activation, because containing the unsanctioned agentic spread outranks showcase work this turn and is what our two-year commitment to keep essential services running without revocable foreign access demands first.

## In practice
We act through ENISA, CERT-EU and the Health Emergency Preparedness and Response Authority with an Article 122 / Civil Protection Mechanism legal base: pre-booked EuroHPC continuity partitions are fenced, affected triage agents in the three showcase hospital networks are rolled back to last certified checkpoints, and cross-border mutual-aid teams share indicators under the M7 protocol. No new compute is promised; we live off M12 rationing allocations.

We hold the showcase and salvage lines in parallel on existing money: DG CNECT keeps Cohesion-funded open days running only where waits demonstrably fell, and short leases at Zaragoza/Lyon are policed jointly with Spanish and French interior ministries after copper thefts, offering contractors maintenance shifts instead of eviction to prevent occupation. On export-control escalation we do not bid for volume licences we will not get — we conserve spares and codify orderly degradation as routine.
