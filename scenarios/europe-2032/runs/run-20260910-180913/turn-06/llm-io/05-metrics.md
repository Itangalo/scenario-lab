...
- characters 105-13604: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13647-13838: `{{metrics_json}}` from the run's live metric values
- characters 13931-15232: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15275-17816: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17872-18379: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M8 | EU Trusted Care and Admin AI Replication | 2 |
...
## Notepad
PORTFOLIO CHARGE t6: InvestAI Gigafactories −3, EU Trusted Care and Admin AI Replication −2, priority −1 = −6
PROPOSAL BONUS: none — no triggering event in last three turns directly answered by replication measure beyond older cutoff already answered
LEGITIMACY LENDS: capital 9, sentiment 13 -> +2
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "ALLIANCE"}}, {"op": "update", "table": "measures", "id": "M8", "fields": {"finish_turn": 7}, "grounds": "named priority"}]}
```
```
