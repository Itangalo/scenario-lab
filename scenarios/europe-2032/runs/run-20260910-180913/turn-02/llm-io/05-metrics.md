...
- characters 105-13070: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13113-13305: `{{metrics_json}}` from the run's live metric values
- characters 13398-14641: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14684-16857: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16913-17942: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Defensive Shield Scale-Up | 3 |
...
- `emergent_eu_cluster_hedge` -- first noted turn 1, listed in 2 turn(s) so far: A major non-US frontier lab announces a training cluster inside the EU under EU law to hedge against US access cutoffs, prompting a bidding contest among member states.
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated local protests block permitting for two planned hyperscale data centre campuses in Spain and Germany over water and power use, forcing a national-level review of siting rules.
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Hardening and Exercises −3, EU Defensive Shield Scale-Up −3, priority −1 = −13
PROPOSAL BONUS: EU Defensive Shield Scale-Up (cat 6, large) +4 — autumn grid/port intrusions landed last turn and this answers them directly with breakthrough tooling
