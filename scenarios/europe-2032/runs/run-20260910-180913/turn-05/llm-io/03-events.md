...
- characters 377-1463: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1507-3817: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3873-4942: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5005-31605: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 31960-32321: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t4: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Municipal Recovery and Cascade Prevention −2, priority −1 = −9
PROPOSAL BONUS: EU Municipal Recovery and Cascade Prevention (cat 6, small) +3 — cyber_major_incident landed this turn exposing municipal/small-utility gap and this answers it directly
...
- `emergent_siting_blockade_escalation` -- first noted turn 4, listed in 1 turn(s) so far: A coordinated siting blockade across Spain and Germany halts gigafactory pre-site works for weeks after local councils vote moratoriums, citing foreign-dependence and energy costs.
- `emergent_siting_sabotage_wave` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated physical sabotage and blockades hit gigafactory pre-sites and grid substations in Spain and Germany, fusing siting opposition with anti-AI hostility and halting construction for weeks.
...
- Turn 1 (4 turn(s) ago): cyber_test_shot, safety_breakthrough, automated_decision_scandal
- Turn 2 (3 turn(s) ago): capability_jump, cyber_defence_breakthrough, automated_decision_scandal
- Turn 3 (2 turn(s) ago): cyber_defence_breakthrough, eu_frontier_access_denied
- Turn 4 (1 turn(s) ago): cyber_major_incident, opaque_reasoning, automated_decision_scandal
