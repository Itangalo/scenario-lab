...
- characters 106-13415: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13458-13650: `{{metrics_json}}` from the run's live metric values
- characters 13743-14829: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14873-17183: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17239-18308: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Cut-off Substitution and Trusted Adoption | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "pending"}}, {"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 9}, "grounds": "event: ai_investment_collapse cancelled private build-out"}, {"op": "update", "table": "measures", "id": "M7", "fields": {"finish_turn": 6}, "grounds": "named priority"}]}
```
```
