...
- characters 455-2354: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2398-4454: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5405-5855: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9078-11022: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Breakthrough Care Localisation | 9 | small | 10 | 12 | own jurisdiction | public_sentiment up materially, resilience up slightly | 2 | running |
...
## New measure
**EU Breakthrough Care Localisation**
Puts the newly demonstrated tailored therapies into EU hospitals and pharmacies on EU-hosted open models and EU-controlled manufacturing steps, so the cure arrives as local care rather than another refused US inference call. Category 9 Diffusion, adoption and public trust, small because at capital 14 we can fund only rationed localisation, and now because the medical breakthrough is the one event that can lift sentiment 12 if citizens experience it on our terms.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Breakthrough Care Localisation", "category": 9, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up materially, resilience up slightly"}, "grounds": "to turn breakthrough into trusted local care not dependence"}]}
```
## Priority
EU Breakthrough Care Localisation, because with capability jumping again and war cutting off US inference, converting the breakthrough into visible EU-delivered treatment is the only lever that can stop political collapse while M10 holds the line.
## In practice
We run both tracks from the civil-protection recovery cell with HERA, EMA and ENISA: M10 keeps ration lists, islanding and neutral-site notifications going, while the new localisation uses EU4Health and Digital Europe emergency procurement to validate open-model treatment protocols, re-task hospital compounding and municipal fallback packs for last-mile delivery.

We authorise under the Serious Cross-Border Threats Regulation and NIS2 emergency articles, in Health and JHA Council, with nurses and hospital pharmacists fronting triage to avoid second-class-care anger. No new gigafactory build is promised; Spain/Germany sites stay in islanded continuity mode, and the defecting capital stays inside mutual aid on condition it shares any side-deal doses and evaluation data.
