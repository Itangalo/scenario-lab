...
- characters 452-1922: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1966-4416: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6822-7272: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10937-12980: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M12 | EU Entry-Level Transition Shield | 7 | small | 11 | 14 | own jurisdiction | public_sentiment up, eu_political_capital up slightly via visible protection, resilience up slightly | 2 | running |
| M13 | EU Struck Node Islanding and Emergency Rationing Reinforcement | 6 | small | 12 | 14 | own jurisdiction | resilience up materially, public_sentiment up slightly via kept services, eu_political_capital down less fast | 2 | running |
...
| M3 | EU Critical Systems Hardening and Exercises | 6 | large | 1 | 3 | own jurisdiction | resilience up materially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Defensive Shield Scale-Up | 6 | large | 2 | 4 | own jurisdiction | resilience up materially, ai_safety up slightly | 3 | finished |
| M5 | EU Continuity Stack for Essential Services | 9 | small | 3 | 4 | own jurisdiction | eu_ai_sovereignty up slightly, public_sentiment up, resilience up slightly | 2 | finished |
| M6 | EU Municipal Recovery and Cascade Prevention | 6 | small | 4 | 5 | own jurisdiction | resilience up materially, public_sentiment up slightly | 2 | finished |
| M7 | EU Cut-off Substitution and Trusted Adoption | 9 | small | 5 | 6 | own jurisdiction | public_sentiment up materially, resilience up slightly, eu_ai_sovereignty up slightly | 2 | finished |
| M8 | EU Trusted Care and Admin AI Replication | 9 | small | 6 | 7 | own jurisdiction | public_sentiment up materially, resilience up slightly | 2 | finished |
| M9 | EU Essential Services Rapid Recovery Cell | 6 | small | 7 | 9 | own jurisdiction | resilience up materially, public_sentiment up slightly | 2 | finished |
| M10 | EU Wartime Essential Continuity and Neutral Infrastructure Shield | 6 | small | 9 | 11 | own jurisdiction | resilience up materially, public_sentiment up slightly, eu_political_capital down less fast via visible holding action | 2 | finished |
| M11 | EU Breakthrough Care Localisation | 9 | small | 10 | 12 | own jurisdiction | public_sentiment up materially, resilience up slightly | 2 | finished |
...
## New measure
**EU Struck Node Islanding and Emergency Rationing Reinforcement**
Keeps hospitals, landings, stations and islanded data centres running through prolonged isolation after the struck neutral site, with emergency rationing, repair crews and fallback packs. Category Preparedness and resilience, small because at capital 13 we cannot fund more than holding, and now because a notified non-belligerent node was hit and islanding must be proven rather than planned.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Struck Node Islanding and Emergency Rationing Reinforcement", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, public_sentiment up slightly via kept services, eu_political_capital down less fast"}, "grounds": "to hold continuity after neutral-site strike"}]}
```
## Priority
EU Struck Node Islanding and Emergency Rationing Reinforcement, because the strike on a declared civilian node outranks even the Transition Shield this turn — if islanding fails, care, jobs and trust fail together.
## In practice
We run the response through the joint recovery cell and the Union Civil Protection Mechanism with ENISA and HERA, using Articles 196/222 TFEU: islanding plans activated around the struck landing/station/data centre, emergency ration lists and municipal fallback packs reissued, repair and rerouting crews pre-cleared across borders.

M12 continues via EPSCO and ESF+/SURE through public employment services and unions — wage insurance and retraining kept paying while hiring incentives are tightened to retention. M11 care localisation continues on certified open-model protocols with EMA validation of the new control bounds. We lodge protest on the neutral-site violation and on ASML coercion via the Anti-Coercion Instrument track without opening a new front this turn, offering member states solidarity funding and the defecting capital continued mutual aid conditional on sharing.
...
```
## New measure
**EU Joint Threat Response Accession and Blockade Rationing**
Joins the joint cyber command telemetry pool and the biosurveillance pact with binding sample-sharing while rationing remaining compute and spares under Taiwan blockade conditions. Category Preparedness and resilience, small because at capital 8 we can only accede and harden, not build, and now because blockade plus capability jump plus medical breakthrough together force protection we cannot build alone.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Joint Threat Response Accession and Blockade Rationing", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, public_sentiment up slightly via kept care, eu_political_capital down less fast"}, "grounds": "to gain pooled protection after blockade and jump"}]}
```
## Priority
EU Joint Threat Response Accession and Blockade Rationing, because pooled attribution and biosurveillance plus compute rationing outranks even islanding this turn — without external telemetry and spares discipline, isolation fails under blockade.
## In practice
We act through the joint recovery cell with ENISA, HERA/ECDC and the Union Civil Protection Mechanism under Articles 196/222 TFEU: sign telemetry-sharing and sample-sharing accords, second EU analysts to the joint cyber command, extend islanding around struck landing/station to blockade spares rationing for hospitals and grid controllers.

M13 islanding and M12 Transition Shield continue on existing ESF+/SURE and emergency aid lines — wards on cached protocols, wage insurance paying, retention bonuses tightened. We localise the medical breakthrough via EMA/HERA compounding on certified EU open models where US models are rationed, and lodge Anti-Coercion track on lithography servicing cutoff without opening a second front, offering member states solidarity funding conditional on sharing telemetry and spares.
```
