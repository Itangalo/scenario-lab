...
- characters 13350-14487: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14530-16559: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16615-17681: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_hyperscaler_pause_eu_build` -- first noted turn 2, listed in 1 turn(s) so far: A major US hyperscaler publicly pauses two planned EU data-centre expansions, citing grid-connection delays and power pricing, fuelling press narrative that gigafactory site rivalry is deterring private build-out.
- `emergent_gigafactory_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: Coordinated local opposition and municipal permitting challenges stall at least one gigafactory shortlisted site over grid priority and power prices, forcing site renegotiation within the turn.
