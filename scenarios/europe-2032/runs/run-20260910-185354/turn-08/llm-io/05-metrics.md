...
- characters 13630-14698: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14741-17055: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17111-17650: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M9 | EU Foundry Keep-Alive and Grid-Queue Pact | 2 |
...
PORTFOLIO CHARGE t7: M8 EU Edge Resilience and Bio-Detection Surge −2, priority −1 = −3
PROPOSAL BONUS: EU Edge Resilience and Bio-Detection Surge (cat 6, small) +3 — answers verifiable-domain jump and contested bio-design findings from this turn directly, edges brittle
LEGITIMACY LENDS: capital 22, sentiment 32 -> +2 (after charges and finishing payouts, capped below sentiment)
SOVEREIGNTY: 19 last turn, M1 InvestAI Gigafactories finishes t7 +5, no capacity in-flight, member_state_defection t7 −2, capability rose 1.5 no penalty = 22
...
## Metrics
```json
{"ai_capability": 64.5, "openweight_capability": 62.1, "ai_safety": 16.0, "resilience": 42.0, "eu_ai_sovereignty": 22.0, "eu_political_capital": 22.0, "public_sentiment": 34.0}
```
## Narrative
...
## Notepad
PORTFOLIO CHARGE t8: M9 EU Foundry Keep-Alive and Grid-Queue Pact −2, priority −1 = −3
PROPOSAL BONUS: none — M9 answers valuation repricing and pledge stretch, not an event from last three rounds
LEGITIMACY LENDS: capital 20, sentiment 32 -> +2
SOVEREIGNTY: 22 last turn, no capacity event, M9 just proposed +0, capability rose 1.0 no penalty = 22
## Store changes
```json
