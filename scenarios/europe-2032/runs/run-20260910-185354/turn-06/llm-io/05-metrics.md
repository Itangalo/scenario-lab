...
- characters 13867-14929: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14972-17547: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17603-18126: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Defensive Stack Deployment | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "ALLIANCE"}, "grounds": "turn 5 election won by alliance candidate, takes effect turn 6"}]}
```
```
