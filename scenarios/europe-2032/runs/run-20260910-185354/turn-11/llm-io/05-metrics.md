...
- characters 13422-13613: `{{metrics_json}}` from the run's live metric values
- characters 13706-15366: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15410-17717: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17773-18678: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M12 | EU Accord Verification and Defensive Research Cell | 2 |
...
- `emergent_port_strike_escalation` -- first noted turn 10, listed in 2 turn(s) so far: A coordinated dockworker and municipal workers strike wave spreads from port stoppages to sympathy strikes in logistics hubs, blocking delivery of backup power and cyber-response hardware to lagging towns for weeks.
