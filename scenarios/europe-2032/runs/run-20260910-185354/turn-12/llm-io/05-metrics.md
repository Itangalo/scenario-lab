...
- characters 106-13410: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13453-13644: `{{metrics_json}}` from the run's live metric values
- characters 13737-15083: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15126-17053: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17109-17521: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M13 | EU Essential Services Islanding and Joint Response Pact | 2 |
...
PORTFOLIO CHARGE t11: M12 EU Accord Verification and Defensive Research Cell −2, priority −1 = −3
PROPOSAL BONUS: none — M12 (cat 8, small) joins US-China floor, no negative event in last three turns that it answers; inclusion is opportunistic not restorative
LEGITIMACY LENDS: capital 16, sentiment 26 -> +2
SOVEREIGNTY: 22 last turn, no capacity event, no cat4 in flight +0, capability rose 0.5 no penalty = 22
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M13", "fields": {"finish_turn": 13}, "grounds": "named priority"}]}
```
```
