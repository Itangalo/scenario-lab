...
- characters 455-1666: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1710-3705: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5821-6270: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9874-12208: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): A Europe that absorbs AI-enabled harm and keeps essential services running
...
| M3 | EU Grid and Port Cyber Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up strongly, ai_safety up slightly via detection | 3 | finished |
| M4 | European Public Services AI Dividend | 9 | small | 2 | 4 | own jurisdiction | public_sentiment up, eu_political_capital up slightly, eu_ai_sovereignty up slightly via demand pull | 2 | finished |
| M5 | Credible Enforcement Repair | 1 | small | 3 | 6 | own jurisdiction | public_sentiment up, eu_political_capital up slightly, ai_safety up slightly | 2 | finished |
| M6 | Allied Volume-Licence and Bottleneck Compact | 8 | small | 4 | 5 | US plus middle-power coalition | eu_ai_sovereignty up, eu_political_capital down slightly if concession-heavy | 2 | finished |
| M7 | EU Defensive Stack Deployment | 6 | small | 5 | 7 | own jurisdiction | resilience up strongly, ai_safety up slightly, public_sentiment up slightly | 2 | finished |
| M8 | EU Edge Resilience and Bio-Detection Surge | 6 | small | 7 | 8 | own jurisdiction | resilience up, ai_safety up slightly via detection | 2 | finished |
| M9 | EU Foundry Keep-Alive and Grid-Queue Pact | 4 | small | 8 | 10 | own jurisdiction | eu_ai_sovereignty up slightly, eu_political_capital down slightly | 2 | finished |
| M10 | EU Critical Services Restoration Pact | 6 | small | 9 | 10 | own jurisdiction | resilience up strongly, public_sentiment up slightly, eu_political_capital up slightly on delivery | 2 | finished |
| M11 | EU Dock-Logistics Flexicurity Transition | 7 | small | 10 | 11 | own jurisdiction | resilience up slightly via social absorption, public_sentiment up, eu_political_capital up slightly on delivery | 2 | finished |
| M12 | EU Accord Verification and Defensive Research Cell | 8 | small | 11 | 12 | US plus China plus middle-power coalition | ai_safety up, eu_political_capital up slightly on inclusion, eu_ai_sovereignty up slightly via leverage | 2 | finished |
| M13 | EU Essential Services Islanding and Joint Response Pact | 6 | small | 12 | 13 | own jurisdiction | resilience up strongly, public_sentiment up slightly, eu_political_capital up slightly on delivery | 2 | finished |
...
## New measure
**EU Essential Services Islanding and Joint Response Pact**
Stands up degraded-but-open operation for hospitals, grids and ports under the ransomware sweep and access cutoff by islanding critical systems, deploying EU-controlled fallback models, and joining the joint cyber command telemetry pool.
Why this and why now: because the automated attack plus sudden denial of the leading model hits exactly what our two-year commitment covers — keeping essential services running — and with capital at 18 we cannot rebuild sovereignty this half but can stop cascades and buy protection we cannot build alone.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Islanding and Joint Response Pact", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up strongly, public_sentiment up slightly, eu_political_capital up slightly on delivery"}, "grounds": "major automated cyber incident plus EU frontier access denial hitting hospitals and ministries"}]}
```

## Priority
M13 EU Essential Services Islanding and Joint Response Pact, because absorbing the live incident and replacing denied dependencies outranks all other work under our absorb-harm commitment.

## In practice
We task ENISA and the EU Joint Cyber Unit via the Council's TTE and JHA formations to trigger the Cybersecurity Act emergency procedures: mandatory telemetry sharing into the invited joint cyber command, pre-authorised islanding playbooks for frontrunner hospitals and two delayed port hubs, and overtime-funded seconded repair teams extended from M10. DG DIGIT and HERA re-route affected ministries and hospitals from the cut-off US model to Saclay-Barcelona evaluated fallbacks and licensed volume stock already on EU soil, using Digital Europe funds, no new vote needed.

We hold the rest defensive without new instruments: DG EMPL keeps M11 wage-insurance payouts flowing to dock towns hit now also by warehouse robotics, DG TRADE keeps Anti-Coercion preparation over Dutch servicing cuts but does not escalate while Hague talks continue, and the M12 verification cell stays as observer to preserve the only frontier seat we hold. Member states resisting data-sharing get restoration queue priority as the offer to stop.
...
```
## New measure
**EU Bio-Containment and Care Continuity Shield**
Stands up HERA-led isolation, EU-model-assisted triage and decontamination for the deliberate bio release while keeping hospitals islanded and staffed through the concurrent ransomware sweep.
Why this and why now: because casualties are real, containment runs for weeks, and our absorb-harm commitment now means preventing hospitals hit by both bio and cyber from stopping — this is the incident that must be answered now.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Containment and Care Continuity Shield", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up strongly, public_sentiment up slightly, ai_safety up slightly via bio-detection",}, "grounds": "real bio incident with model involvement plus ongoing automated ransomware sweep"}]}
```

## Priority
M14 EU Bio-Containment and Care Continuity Shield, because stopping bio spread and keeping care open outranks sovereignty rebuilding when capital is at 15 and sentiment at 16.

## In practice
We task HERA and ECDC via the Health Council emergency procedure to trigger cross-border mutual aid: pooled PPE, mobile BSL-3 testing, wastewater and clinical sequencing surge funded from EU4Health and rescEU, with mandatory incident and uplift-use reporting into ENISA's joint cyber command pool to link bio and cyber telemetry.

We hold care open in degraded mode: DG SANTE and DG DIGIT keep M13 islanding playbooks on, re-route triage admin to Saclay-Barcelona fallbacks with paper backup, and DG EMPL extends overtime and wage-continuation to seconded hospital and port repair teams. No new vote for sovereignty expansion; we ask frontrunner states to cede sequencing and bed capacity first in exchange for restoration queue priority.
