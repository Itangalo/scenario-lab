...
- characters 377-1761: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1805-4045: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4101-5278: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5341-33826: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 34181-34370: `{{event_history}}` from the run's own event record
...
- `emergent_procurement_legal_freeze` -- first noted turn 4, listed in 1 turn(s) so far: Non-EU cloud vendors file a coordinated challenge to the EU-hosted inference preference, seeking interim relief that would suspend laggard-region procurement until discrimination claims are ruled on.
- `emergent_grid_connection_revolt` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated municipal and regional court challenges temporarily halt data-centre grid connections for gigafactory siting, citing costs and the welfare-scoring scandal, delaying EU compute build-out without a formal EU decision.
...
- Turn 2 (3 turn(s) ago): adoption_success
- Turn 3 (2 turn(s) ago): automated_decision_scandal
