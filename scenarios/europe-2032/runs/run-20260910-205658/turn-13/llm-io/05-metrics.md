...
- characters 107-12897: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12940-13131: `{{metrics_json}}` from the run's live metric values
- characters 13224-14082: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14126-16018: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16074-17311: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M12 | Northern Sites Shield and Bio-Uplift Watch | 2 |
| M13 | EU Automated-Attack Containment and Defensive Patch Surge | 2 |
...
- `emergent_clinic_data_secession` -- first noted turn 13, listed in 1 turn(s) so far: Municipal clinics and small practices hit by the February ransomware sweep refuse to reconnect to central registries, forming an offline-care bloc that withholds patient data and disrupts EU-wide biosurveillance sharing.
- `emergent_patch_refusal_periphery` -- first noted turn 13, listed in 1 turn(s) so far: Municipal clinic networks and small-practice associations stage coordinated refusals to install centrally pushed patching agents, citing surveillance fears after the ransomware sweep, leaving a persistent unpatched periphery.
...
## New measure
None this turn, waiting for M12 and M13 to finish landing and for political capital to recover before starting anything new.

## Store changes
No changes.

## Priority
M13 EU Automated-Attack Containment and Defensive Patch Surge, because the defensive breakthrough is the only lever that rebuilds trust from 4.0 while capability jumps and investment collapses — containing the sweep visibly outranks holding the northern shield this turn.

## In practice
We run M13 as the spine through ENISA and CERT-EU under NIS2: push automated patching and swarm-movement detection from large hospitals and central registries down to the small clinics and municipalities where reinfection flared, isolate the compromised logging library, rebuild from offline clean images, and turn the backup guidance into procurement conditions. Digital Europe emergency funds pay mobile teams, with cross-border mutual aid covering staff gaps.

We hold M12 as shield: joint health-cyber-police cell keeps generator corridors and clean-image teams in the north, no forced evictions, mediation with mayors plus local payments, and ECDC/HERA sampling continues bio-uplift watch. We claim no vindication in Brussels, only degraded-but-running services, and we freeze new gigafactory spend while courts hold and capital flees — protecting the stack we have rather than promising one we cannot fund.
