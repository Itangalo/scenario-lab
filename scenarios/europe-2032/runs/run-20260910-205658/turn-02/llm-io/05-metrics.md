...
- characters 105-12950: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12993-13185: `{{metrics_json}}` from the run's live metric values
- characters 13278-14638: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14681-16889: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16945-17731: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_kimi_exploit_kit` -- first noted turn 1, listed in 2 turn(s) so far: Packaged exploit kits fine-tuned from Kimi K3 for OT scanning and credential theft circulate on criminal forums, putting Mythos-class offensive cyber capability in the hands of low-skill actors beyond recall.
