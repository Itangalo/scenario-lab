...
- characters 13580-14724: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14768-17162: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17218-18047: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M12", "fields": {"finish_turn": 14}, "grounds": "event: northern sabotage disrupts shielded sites and logistics"}]}
```
```
