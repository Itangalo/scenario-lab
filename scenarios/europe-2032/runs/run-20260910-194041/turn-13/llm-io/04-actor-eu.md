...
- characters 453-2020: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2064-3888: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5567-6030: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9344-11610: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M13 | EU Middle-Power Patch and Supply Shield | 8 | small | 12 | 14 | own jurisdiction + middle-power coalition + US | resilience up moderately, eu_political_capital up slightly, eu_ai_sovereignty up slightly via secured supply | 2 | running |
...
| M12 | EU Upstream Bargain and Anti-Defection Hold | 8 | small | 11 | 13 | own jurisdiction + US + coalition | eu_ai_sovereignty up slightly, resilience up slightly via secured supply | 2 | finished |
...
```
## New measure
None this turn. We are waiting for political headroom to rebuild enough to carry a US-China accession track without collapsing the patch-and-supply shield already in flight.

## Store changes
No changes.

## Priority
M13 EU Middle-Power Patch and Supply Shield, because it is the only funded vehicle holding hospitals and grids together while giving us something to trade for insured shipments and for a seat in the US-China risk agreement.

## In practice
We keep the restore corps on hand-carry patching of islanded hospitals, grid operators and registries with the machine-speed tooling, and DG TRADE tables that same shared patch capacity plus the single lithography authorisation in the middle-power forum for insured spares and wider medical/energy licences. Trade Council monthly re-authorisation continues; no new tranche releases until volume licences return.

We use the coalition, not a new instrument, to seek observer/consulted status on the US-China weights-security and bio-design understanding, and task ENISA/HERA/ECDC to log the contested genome-model bio-uplift signal into existing Bio Shield detection and triage playbooks. Mayors get temporary heat/clinic use of non-reserved power only; the guarded halls stay reserved. No new builds, no side-deal ultimatum escalation this turn — we cannot afford a second fight.
