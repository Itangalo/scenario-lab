...
- characters 451-1697: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1740-4043: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4889-5352: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8419-10532: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Hold essential services through the cutoff and rebuild European autonomy from what holds
...
|---|---|---|---|---|---|---|---|---|---|
| M12 | EU Upstream Bargain and Anti-Defection Hold | 8 | small | 11 | 13 | own jurisdiction + US + coalition | eu_ai_sovereignty up slightly, resilience up slightly via secured supply | 2 | running |
...
| M11 | EU Restore and Paper Continuity Corps | 6 | small | 10 | 11 | own jurisdiction | resilience up moderately | 2 | finished |
...
## New measure
**EU Upstream Bargain and Anti-Defection Hold**
Locks lithography, optics and spares under a single EU export-licence and anti-coercion mandate to trade continued supply for US volume licences and chip flows, while freezing any member-state side-deal that undercuts it.
This is International coordination and leverage, small and immediate, because with Taiwan blockaded, US models cut off, capital at 14 and a member state defecting, the only asset that still buys essential-service continuity is the upstream chokepoint held in common — uncoordinated it is picked off, coordinated it buys licences, spares and time.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Upstream Bargain and Anti-Defection Hold", "category": 8, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction + US + coalition", "targeted_effect": "eu_ai_sovereignty up slightly, resilience up slightly via secured supply"}, "grounds": "turn blockade and defection into common leverage for licences"}]}
```

## Priority
M12 EU Upstream Bargain and Anti-Defection Hold, because without a common licence hold and a US volume-licence return, hospitals and grid islanding have nothing to run on and every mayor will cut their own deal.

## In practice
We run this from the Commission under the Anti-Coercion Instrument and Dual-Use Regulation, with Trade Council mandate: all advanced lithography, optics, servicing and spares require EU authorisation, released in tranches tied to US volume licences for medical/energy workloads and to shipping-insured chip deliveries. DG TRADE leads the Washington ask; HERA and ENISA certify the listed hospital/grid end-uses.

We hold cohesion by making defection costly and loyalty paid: side-deals lose access to EU-hosted patch trains, restore teams and islanding tariff protection under NIS2/CER, while loyal sites get priority spares, capped tariffs and bill-relief tied to substation hardening. No new gigafactory build this turn; the two empty shells stay mothballed and guarded for clinics and heating if winter forces seizure.
...
```
## New measure
**EU Middle-Power Patch and Supply Shield**
Deploys the new automated patching and swarm-detection tooling across hospitals, grids and registries via the restore corps, while locking it into the middle-power export-licence and compute-access alignment to trade joint upstream leverage for insured chips and licences.
This is International coordination and leverage now because the breakthrough only matters if it is installed everywhere at once, and the coalition only matters if Europe brings something — common patch capacity plus the single lithography authorisation — to buy back the shipments and capital that defection priced away.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Middle-Power Patch and Supply Shield", "category": 8, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction + middle-power coalition + US", "targeted_effect": "resilience up moderately, eu_political_capital up slightly, eu_ai_sovereignty up slightly via secured supply"}, "grounds": "use defence breakthrough and coalition window to restore supply and trust"}]}
```

## Priority
M12 EU Upstream Bargain and Anti-Defection Hold, because without holding the single authorisation through the bilateral break, the new coalition pact has nothing to bargain with and patch trains have nothing to run on.

## In practice
We keep DG TRADE on the Washington collective file, freezing tranche releases until volume licences for listed medical/energy workloads return, and we make the large member state's bilateral the test case: rejoin the common hold and keep restore teams and tariff cover, or keep the side spares and lose them. The Anti-Coercion Instrument mandate stays the legal base, with the Trade Council re-authorising monthly.

At the same time we task ENISA, HERA and the restore corps to push the breakthrough patch tooling by hand-carry and clean image where networks cannot be trusted, starting with islanded hospitals and grid operators, and we table it in the middle-power coordination framework as Europe's contribution — shared evaluation and joint licence alignment for insured cargo. No new builds; the empty halls stay guarded and available for heat or clinic power if mayors force seizure.
