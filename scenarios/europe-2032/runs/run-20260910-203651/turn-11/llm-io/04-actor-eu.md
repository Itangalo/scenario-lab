...
- characters 454-1133: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1177-3231: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3638-4130: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7086-9079: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_care_backlog_litigation_freeze (emergent event):** Municipal clinics and registries hit by the managed-service intrusions publish paper-record backlogs and delayed-care figures, triggering coordinated patient-advocacy lawsuits that freeze further health-administration AI procurement pending court review.
...
- `two_year_commitment` (commitment): A Union that endures AI-enabled disruption on its own footing and bargains for technology access from shared leverage
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Essential Compute Continuity and Rationing Regime | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, eu_political_capital up slightly if essential services hold | 2 | running |
...
| M3 | EU Grid and Essential Services Hardening Pact | 6 | large | 1 | 3 | own jurisdiction | resilience up strongly, ai_safety up slightly via detection | 3 | finished |
...
| M6 | EU Essential Services Model Continuity Bridge | 6 | small | 4 | 5 | own jurisdiction | resilience up moderately, eu_political_capital up slightly if holds | 2 | finished |
| M7 | EU Bio-Cyber Early Warning and Response Net | 6 | small | 5 | 8 | own jurisdiction | resilience up moderately, ai_safety up slightly via detection | 2 | finished |
| M8 | EU–Middle Power Supply Chain Bargaining Compact | 8 | small | 9 | 11 | coalition (EU, Japan, Korea, Netherlands-linked supply chain holders) | eu_ai_sovereignty up slightly via secured access, eu_political_capital up slightly via joint leverage | 2 | finished |
...
## New measure
**EU Essential Compute Continuity and Rationing Regime**
A small category 6 preparedness measure that inventories, rations and ring-fences remaining advanced compute and spares for grids, hospitals, payment systems and Bridge inference under the blockade.
This and now because the Taiwan blockade makes every chip a security question overnight, and with capital at 11 the Union cannot build its way out but must keep essential services alive while M8 bargains jointly.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Compute Continuity and Rationing Regime", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, eu_political_capital up slightly if essential services hold"}, "grounds": "blockade halts advanced semiconductor exports for years"}]}
```

## Priority
M8 EU–Middle Power Supply Chain Bargaining Compact, because the upstream hold only becomes leverage and servicing flows if Tokyo/Seoul alignment lands this turn before rationing hardens against us.

## In practice
We run the rationing through the Single Market Emergency Instrument and the Chips Act crisis toolbox, with DG GROW and DG CNECT tasking ENISA and the Semiconductor Board to audit data-centre spares, managed-service dependencies from the autumn intrusions, and Bridge inference needs. Health and Energy Councils get priority tiers; nothing new is built, permits and conversion sites stay frozen to fit capital.

We weaponise restraint on the upstream: Competitiveness and Foreign Affairs Councils mandate no unilateral Dutch lithography or optics cut-off, pooling any licence decision into the M8 Tokyo/Seoul framework to jointly defend servicing flows and second-source model access from the US and China. Ministers sell clinic waits and permit times as the reason rationing protects ordinary life first, to stop sentiment at 9 collapsing into physical opposition.
...
```
## New measure
**EU Clinic Backlog Clearing and Procurement Review**
A small category 6 preparedness measure that funds paper-backlog digitisation teams, independent clinical review of delayed-care cases, and a time-limited court-supervised review to unfreeze trusted health-administration AI procurement.
This and now because the coordinated lawsuits freeze exactly the procurement that keeps rationed clinics running, and with capital at 11 and sentiment at 5 only a visible backlog-clearing plus judicial review can prevent the freeze hardening into permanent distrust.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Clinic Backlog Clearing and Procurement Review", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up slightly, public_sentiment up slightly, eu_political_capital up slightly if backlogs fall"}, "grounds": "lawsuits freeze health-administration AI procurement amid paper backlogs"}]}
```

## Priority
M9 EU Essential Compute Continuity and Rationing Regime, because keeping hospitals, grids and payments on ring-fenced inference through the rationing winter is what prevents the litigation freeze and parts shortage cascading into service collapse.

## In practice
We keep M9 as the operating system: DG GROW/CNECT with ENISA and the Semiconductor Board enforce the audited spares inventory and priority tiers, Health and Energy Councils defend hospitals-first ring-fencing, and we use the Single Market Emergency Instrument to keep Bridge inference segregated for essential services while conversion sites stay frozen.

We stand up the backlog measure through DG SANTE and DG JUST with the Health Council and national patient ombudsmen: EU4Health money for mobile digitisation and clinical review teams to town halls and clinics hit by the managed-service intrusions, HERA-style triage protocols for delayed care, and a joint Commission-court expert panel to certify which centrally procured treatments and assistants are safe to reconnect. Procurement stays frozen only pending that review, with staff refusals addressed by independent safety sign-off rather than orders to reconnect.
