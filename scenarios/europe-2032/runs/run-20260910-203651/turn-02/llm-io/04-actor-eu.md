...
- characters 454-1725: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1768-4093: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4456-4928: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6406-8605: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_synthetic_fraud_wave (emergent event):** A wave of AI-generated voice and video fraud using Mythos-class open models hits banks and public administrations across several EU states, causing tens of millions in losses and forcing emergency authentication changes.
...
| M3 | EU Grid and Essential Services Hardening Pact | 6 | large | 1 | 3 | own jurisdiction | resilience up strongly, ai_safety up slightly via detection | 3 | running |
