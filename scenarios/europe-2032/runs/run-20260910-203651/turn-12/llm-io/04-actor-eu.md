...
- characters 452-1261: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1304-3249: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4673-5165: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8343-10564: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_care_ai_boycott (emergent event):** Municipal clinic staff and local press publish coordinated refusals to reconnect EU-procured diagnostic assistants after the managed-service intrusions, turning a cyber-hygiene measure into a sustained non-use movement in several regions.
...
|---|---|---|---|---|---|---|---|---|---|
| M10 | EU Clinic Backlog Clearing and Procurement Review | 6 | small | 11 | 13 | own jurisdiction | resilience up slightly, public_sentiment up slightly, eu_political_capital up slightly if backlogs fall | 2 | running |
...
| M9 | EU Essential Compute Continuity and Rationing Regime | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, eu_political_capital up slightly if essential services hold | 2 | finished |
...
```
## New measure
None this turn, while we wait for the backlog review to restore enough trust and capital to act without collapsing essential-services rationing.

## Store changes
No changes.

## Priority
M10 EU Clinic Backlog Clearing and Procurement Review, because only visible digitisation, independent re-triage and court-accepted safety sign-off can break the boycott, unfreeze trusted procurement, and rebuild minimal legitimacy after the automated attack.

## In practice
We keep the rationing regime as the floor without new spending: ENISA, DG CNECT and the Health/Energy Councils hold segregated inference for hospitals, grids and payments, dole audited spares by tier, and isolate the ransomware-affected municipal services for manual fallback rather than reconnecting compromised assistants.

We put everything political we have into M10: DG SANTE/DG JUST funds mobile scanning and clinical review teams from EU4Health, the joint Commission-court panel publishes which tools are cleared to reconnect with human-readable safeguards given opaque reasoning, and patient ombudsmen and clinic staff get independent sign-off veto to end the boycott. No new build or procurement is pushed until that review lands.
