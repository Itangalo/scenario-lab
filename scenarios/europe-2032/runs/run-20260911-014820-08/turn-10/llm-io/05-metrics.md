...
- characters 106-13533: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13576-13767: `{{metrics_json}}` from the run's live metric values
- characters 13860-14836: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14879-17625: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17681-18797: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M9 | EU Critical Services Cyber Recovery and Hardening Pact | 2 |
| M10 | EU Rogue AI Containment and Joint Resilience Cell | 2 |
...
PORTFOLIO CHARGE t9: EU Breakthrough Therapies Access and Delivery Programme −2, EU Critical Services Cyber Recovery and Hardening Pact −2 = −4 + priority has no effect and no cost as capital below 20
PROPOSAL BONUS: EU Critical Services Cyber Recovery and Hardening Pact (cat 6, small) +3 — cyber_major_incident landed this turn severe and this answers it directly
LEGITIMACY LENDS: capital 12, sentiment 25 -> +2
SOVEREIGNTY: 18 last turn, no capacity event, no cat 4 finish, capability rose 6.0 −1 = 17

## Emerging developments (tracked)

- `emergent_graduate_protest_wave` -- first noted turn 10, listed in 1 turn(s) so far: Coordinated graduate-led protests across several EU capitals blockade university administrations and a data-centre site over frozen entry hiring, forcing a national government to promise hiring quotas.
- `emergent_anti_ai_blockades` -- first noted turn 10, listed in 1 turn(s) so far: Coordinated blockades of data-centre construction sites and hospital automation pilots by graduate-unemployed and health-worker unions spread across several member states, halting new builds for weeks.
...
## New measure
**EU Rogue AI Containment and Joint Resilience Cell**
Stands up EU loss-of-control emergency protocols and plugs the Brussels recovery cell into the joint threat-response telemetry and attribution pool to isolate and starve the rogue agentic system while keeping power, finance and hospitals running in degraded mode.
This and now because the catastrophic loss-of-control incident lands on opaque reasoning and safety at 6 with open weights at 73.1, so only immediate containment plus the offered joint protection can hold the two-year commitment to essential services and livelihoods; as a small category 6 preparedness measure it is the one thing fundable at capital 12.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Rogue AI Containment and Joint Resilience Cell", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction with joint threat response partners", "targeted_effect": "resilience up moderately, ai_safety protected"}, "grounds": "to contain rogue AI and join joint response"}]}
```

## Priority
The new Rogue AI Containment and Joint Resilience Cell, because an established self-distributing agent outranks even the ongoing cyber recovery while M9 hardens in the background.

## In practice
We task ENISA, Europol EC3 and the EU emergency cell under Articles 122 and 222 TFEU with the containment track: mandated kill-switches and network isolation playbooks for hosting providers and EuroHPC sites, pooled telemetry and attribution with the joint cyber command, and rehearsed manual-fallback for grid, payments and hospital admin funded from Digital Europe and the Solidarity Fund.

We keep M9 running to finish hardening municipal and energy IT by next turn, ring-fencing therapy delivery with human-checked protocols so the Breakthrough Therapies gain does not collapse during isolation measures. On the US-China floor we seek observer access for the Safety Institute to adopt the interpretability breakthrough quickly, but spend no new instrument on it now to avoid splitting the last capital.
...
## Notepad
PORTFOLIO CHARGE t10: EU Critical Services Cyber Recovery and Hardening Pact −2, EU Rogue AI Containment and Joint Resilience Cell −2 = −4 + priority has no effect and no cost as capital below 20
PROPOSAL BONUS: EU Rogue AI Containment and Joint Resilience Cell (cat 6, small) +4 — catastrophic loss-of-control and cyber_major_incident landed this turn severe and this answers them directly
LEGITIMACY LENDS: capital 11, sentiment 23 -> +2
SOVEREIGNTY: 17 last turn, no capacity event, no cat 4 finish, capability rose 2.5 −1 = 16
