...
- characters 453-1528: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1571-3639: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4189-4630: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6473-8719: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M5 | EU Lithography and Supply-Chain Leverage Compact | 8 | small | 3 | 6 | coalition of supply-chain holders and US | eu_ai_sovereignty up moderately, eu_political_capital protected via leverage | 2 | running |
...
```
## New measure
None this turn — we are waiting for political headroom to rebuild before any new instrument can be carried.

## Store changes
No changes.

## Priority
EU Grid and Lifeline Hardening Shield (M3), because a large automated attack landing now outranks our own build direction and existing hardening is the only instrument that can absorb it.

## In practice
We put M3 on emergency footing: ENISA and the Cyber Solidarity mechanism coordinate the incident response, re-prioritising relay pilots, detector rollouts and automated patching to the hit public services, with black-start and mutual-aid drills activated and costs held inside existing Digital Europe / civil-protection lines. No new mandate is asked from Council; we report under NIS2 Article response tracks.

We keep M4 Bio Watch running on its HERA/ECDC track — sequencing and stockpile audits continue — and hold M1/M2 and M5 on care-and-maintenance: concrete and power reserved, lithography licensing table kept open for any carved-out slots, but no new spend or siting fight started while capital is at 7. We use the junior-productivity evidence to hold sentiment: tools stay on, dependence is named as the cause of the outage.
