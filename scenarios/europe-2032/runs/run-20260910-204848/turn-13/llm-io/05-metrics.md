...
- characters 107-13254: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13297-13487: `{{metrics_json}}` from the run's live metric values
- characters 13580-14714: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14758-16500: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16556-16916: `{{notepad}}` from the Game Master's notepad, carried across turns
...
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 0.0 after finish and event, sentiment 1.0 -> +0 capped, capital stays 0.0
SOVEREIGNTY: 19 last turn, no capacity event, no cat4 in flight +0, capability rose 1.0 −0 = 19
## Store changes
```json
{"store": []}
