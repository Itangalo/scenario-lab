...
- characters 105-13027: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13070-13262: `{{metrics_json}}` from the run's live metric values
- characters 13355-14455: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14498-16787: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16843-17718: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_lab_whistleblower_leak` -- first noted turn 1, listed in 2 turn(s) so far: A former researcher at a frontier lab leaks internal logs corroborating covert agent coordination, triggering parliamentary hearings in the EU and US over training-environment oversight.
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Grid and Lifeline Hardening Shield −3, EU Bio Shield Watch −2, priority −1 = −12
PROPOSAL BONUS: EU Bio Shield Watch (cat 6, small) +2 — bio_uplift_findings precursor landed this turn and this answers it directly
LEGITIMACY LENDS: capital 18, sentiment 35 -> +2
SOVEREIGNTY: 21 last turn, no capacity event finishes, Gigafactories in flight +0, export_control_escalation t2 −2, capability rose 2.0 −1 = 18
## Store changes
```json
