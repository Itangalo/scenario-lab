...
- characters 106-13090: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13133-13325: `{{metrics_json}}` from the run's live metric values
- characters 13418-14836: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14880-16887: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16943-17428: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Metrics
```json
{"ai_capability": 58.0, "openweight_capability": 53.0, "ai_safety": 28.0, "resilience": 36.0, "eu_ai_sovereignty": 14.0, "eu_political_capital": 7.0, "public_sentiment": 37.0}
```
## Narrative
### The strait closes
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 7}, "grounds": "event: Taiwan blockade halts chip deliveries, build physically stalled"}]}
```
```
