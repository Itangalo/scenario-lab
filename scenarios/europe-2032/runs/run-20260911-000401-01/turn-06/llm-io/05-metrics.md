...
- characters 105-13476: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13519-13710: `{{metrics_json}}` from the run's live metric values
- characters 13803-14624: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14667-16137: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16193-17269: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M5 | Middle-Power Supply-Chain Compact | 2 |
...
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, Middle-Power Supply-Chain Compact −2, priority −0 (capital below 20, no effect no cost) = −8
PROPOSAL BONUS: Middle-Power Supply-Chain Compact (cat 8, small) +2 — answers supply-chain use of Dutch chain last turn and tiered rationing after US election directly
LEGITIMACY LENDS: capital 2, sentiment 22 -> +2
SOVEREIGNTY: 14 last turn, no capacity finish, Gigafactories in flight +0, middle_power_coalition t5 +2, capability rose 2.0 −1 = 15

## Emerging developments (tracked)

- `emergent_gigafactory_site_backlash` -- first noted turn 6, listed in 1 turn(s) so far: Coordinated sabotage and blockades halt construction logistics at two of the five reserved gigafactory sites after power-price protests escalate, forcing ministers to suspend permits.
- `emergent_gigafactory_site_blockade` -- first noted turn 6, listed in 1 turn(s) so far: Coordinated power-price protests at two reserved gigafactory sites escalate into prolonged blockades, forcing a formal suspension of permitting at one site.
...
## New measure
None this turn, while we absorb a major automated attack with capital at 4 and hold the line against a first defection.

## Store changes
No changes.

## Priority
M5 Middle-Power Supply-Chain Compact, because a member state cutting its own deal with Washington or a hyperscaler under tiered US access will unravel joint bargaining faster than the cyber incident will unravel the grid if we do not contain the political cascade first.

## In practice
We activate what M4 left behind without reopening it as spend: ENISA-led incident response under NIS2/CER, extended hospital/port playbooks from the autumn Shield, and the Evaluation Institute for malware triage and attribution support. DG CNECT and DG HOME coordinate patching and failover with national CSIRTs, and we communicate publicly that defenders are behind because openweight at 57.4 has distributed offensive tooling — not because audits were not closed.

We use the Compact as the defection instrument. In the Trade and Foreign Affairs Councils we put the defector's terms on the table against the joint licence-alignment and common Washington ask, offer side-payments in compute allocation and testing access through the Institute, and make clear the Anti-Coercion file stays live. DG TRADE is instructed to hold the Netherlands/Japan/Korea alignment, and the EIB/DG CNECT keep the five gigafactory permits warm but spend nothing until capital recovers.
...
## Notepad
PORTFOLIO CHARGE t6: InvestAI Gigafactories −3, Middle-Power Supply-Chain Compact −2, priority −0 (capital below 20, no effect no cost) = −5
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 0, sentiment 10 -> +0 (capital already floored, no lend applied past constraints)
SOVEREIGNTY: 15 last turn, Tech sovereignty package finishes t6 +4, member_state_defection t6 −2, capability rose 1.0 no charge = 17
