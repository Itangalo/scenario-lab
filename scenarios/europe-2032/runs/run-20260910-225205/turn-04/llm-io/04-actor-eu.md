...
- characters 454-2247: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2290-4050: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5569-6041: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7836-9915: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M5 | Emergency Containment and Continuity Surge | 6 | small | 3 | 5 | own jurisdiction | resilience up, damage contained | 2 | running |
...
```
## New measure
None this turn — waiting for the Shield and Containment lines to finish landing and free capital before opening a new front.

## Store changes
No changes.

## Priority
Emergency Containment and Continuity Surge (M5) – because the discontinuous jump in code/cyber tooling lands directly on the unpatched vectors and uninsured operators from the autumn wave, outranking concrete-pouring for these six months.

## In practice
We run the semester as absorption-plus-patch through ENISA and the Civil Protection Mechanism: Shield money and the Surge are released only against patched remote-access tools, isolated tested backups, and a signed crisis-command protocol or bilateral compact, with EU-paid detection and mutual-aid teams kept on deployment in the three hit states and the hospital group. Interior ministers blocking joint command are not forced this turn; we fund the bilaterals that work and publish recovery-time differentials to make patching the price of coverage.

On sovereignty we hold without new spend: InvestAI guarantees and EIB tranches stay tied to grid priority, EU legal anchoring and notification under the External Deal Screen (M4), showcasing the one rising site while Competitiveness Council mediates water/grid fees on the two stalled sites. We use the productivity evidence — gains without job losses — to defuse compensation quarrels and hold the line against a second breakaway discount, framing restraint as protecting the absorption gains already paid for.
