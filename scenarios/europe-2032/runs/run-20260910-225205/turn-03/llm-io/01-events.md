...
- characters 377-1583: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1627-3491: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3547-4641: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4704-28931: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29286-29376: `{{event_history}}` from the run's own event record
...
- `emergent_cyber_insurance_retreat` -- first noted turn 2, listed in 1 turn(s) so far: A major European insurer reprices cyber cover after the grid intrusions, quietly excluding losses from AI-probed OT compromises and forcing several grid operators and ports to self-insure critical downtime.
- `emergent_grid_forensics_leak` -- first noted turn 2, listed in 1 turn(s) so far: Independent press investigation publishes leaked forensic details from the grid intrusions, naming vendors and unpatched remote-access tools and triggering compensation claims against two European operators.
...
- Turn 2 (1 turn(s) ago): member_state_defection
