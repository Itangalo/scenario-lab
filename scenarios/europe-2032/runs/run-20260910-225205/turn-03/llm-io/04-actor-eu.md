...
- characters 455-1661: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1705-3569: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4378-4850: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6506-8533: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_grid_forensics_leak (emergent event):** Independent press investigation publishes leaked forensic details from the grid intrusions, naming vendors and unpatched remote-access tools and triggering compensation claims against two European operators.
...
| M4 | Union External AI Deal Screen | 8 | small | 2 | 5 | own jurisdiction | eu_political_capital stabilise then up, eu_ai_sovereignty up by preventing fragmentation | 2 | running |
...
## New measure
**Union External AI Deal Screen**
Creates a binding notification-and-review screen for member-state AI compute, cloud and model deals with non-EU providers, tying EU co-financing and Shield funds to compliance.
This is International coordination and leverage, because the defection shows sovereignty is lost through bilateral side-deals faster than gigafactories can build it, and the window to set the rule closes while sites are still being allocated.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Union External AI Deal Screen", "category": 8, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "eu_political_capital stabilise then up, eu_ai_sovereignty up by preventing fragmentation"}, "grounds": "member-state side-deal undercutting common line requires enforceable cohesion"}]}
```

## Priority
InvestAI Gigafactories (M1) – because only poured concrete and signed grid priority removes the pragmatism excuse for bilateral hyperscaler deals that undercut the Union.

## In practice
We are holding the gigafactory race together through the Competitiveness Council and DG CNECT, using InvestAI guarantees and EIB tranches to force four-to-five final site decisions this semester, with grid priority conditional on European legal anchoring and on acceptance of the new deal screen. Regions bidding against each other are told plainly: EU money follows the common rulebook.

We push the Shield via ENISA and HERA with Civil Protection Mechanism funds, trading EU-paid detection stacks and hardening for mandatory drills and exercised continuity plans, while isolating Interior-ministry resistance with bilateral continuity compacts. On the defection, we take the file to the European Council and Foreign Affairs Council under the Anti-Coercion Instrument logic, offering the defecting capital accelerated siting plus Shield uplift in return for re-aligning its arrangement to Union terms, and making future funds conditional.
...
```
## New measure
**Emergency Containment and Continuity Surge**
Stands up EU-coordinated patching of named remote-access vectors, isolated backups and cross-border mutual aid for hit public services and grid operators.
This is Preparedness and resilience, because the automated attack plus the forensic leak naming vendors makes absorption this semester the binding constraint on the two-year commitment to withstand coercion and absorb AI harm, while gigafactories and the deal screen continue on their longer tracks.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Emergency Containment and Continuity Surge", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, damage contained"}, "grounds": "major automated attack and grid forensics leak require immediate containment"}]}
```

## Priority
EU Critical Services Shield (M3) – because drills, command clarity and exercised continuity are what decide whether this semester's automated attack cascades, outranking concrete-pouring for these six months.

## In practice
We run the semester through ENISA and the Civil Protection Mechanism with DG ENER and DG DIGIT: EU-paid detection stacks already delivered become the joint picture, and Shield hardening funds are released only against patched remote tools, tested backups and a signed crisis-command protocol. Interior ministers who stall drills are bypassed with bilateral continuity compacts plus conditionality on the new surge money.

On sovereignty we hold the line without new concrete promises: InvestAI guarantees and EIB tranches stay tied to grid priority, European legal anchoring and notification under the External Deal Screen, with the one building site showcased and the two stalled sites given water/grid-fee mediation via the Competitiveness Council. The breakaway capital keeps notification compliance while we deny the same discount to the two askers, framing co-financing as rulebook-only to stop fragmentation while the Shield absorbs the attack.
