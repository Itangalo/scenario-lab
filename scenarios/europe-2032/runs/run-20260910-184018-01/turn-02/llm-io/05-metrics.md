...
- characters 105-13078: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13121-13313: `{{metrics_json}}` from the run's live metric values
- characters 13406-14630: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14673-16719: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16775-17636: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_us_talent_exodus` -- first noted turn 1, listed in 2 turn(s) so far: A major US frontier lab suffers a high-profile researcher exodus after the June switch-off order, with several safety and capabilities staff relocating to European institutes and startups in autumn 2026.
...
## Metrics
```json
{"ai_capability": 56.0, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 36.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 31.0, "public_sentiment": 32.0}
```
