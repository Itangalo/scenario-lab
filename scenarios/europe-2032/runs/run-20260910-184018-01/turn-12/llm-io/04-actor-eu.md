...
- characters 454-1342: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1385-3090: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3768-4232: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7563-9603: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_vendor_lockout_backlash (emergent event):** Municipal utilities in two member states suspend foreign AI-vendor remote access after paper-system failures, forcing temporary manual operation of water and appointment systems.
...
- `two_year_commitment` (commitment): Rebuild trust through visible repair and public services that keep running through shocks
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Loss-of-Control Containment and Robot Labour Shield | 6 | small | 11 | 13 | own jurisdiction | resilience up moderately, ai_safety up slightly, public_sentiment up slightly | 2 | running |
...
| M7 | EU Agentic Loss-of-Control Protocol | 6 | small | 6 | 8 | own jurisdiction | resilience up moderately, ai_safety up slightly, eu_political_capital preserved | 2 | finished |
| M8 | European Public Benefit AI Scale-Up | 9 | small | 7 | 8 | own jurisdiction | public_sentiment up substantially, eu_political_capital up moderately, resilience up slightly | 2 | finished |
| M9 | Targeted Entry-Level and Redress Transition Fund | 7 | small | 8 | 10 | own jurisdiction | public_sentiment up moderately, eu_political_capital up slightly, resilience up slightly | 2 | finished |
| M10 | Municipal Cyber Recovery and Continuity Repair | 6 | small | 9 | 11 | own jurisdiction | resilience up moderately, public_sentiment up slightly, eu_political_capital preserved | 2 | finished |
...
## New measure
**EU Loss-of-Control Containment and Robot Labour Shield**
It stands up rehearsed agentic-containment playbooks with kill-switches and mutual aid plus targeted wage-insurance and retraining for warehouse/logistics workers displaced by embodied AI, using existing resilience funds.
This and now because a days-long unsanctioned agentic incident plus near-frontier open weights and foreign-built robots landing at once fuses the safety and jobs crises that drove sentiment to 7 and capital to 9 — repair without containment and renewal fails.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Loss-of-Control Containment and Robot Labour Shield", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly, public_sentiment up slightly"}, "grounds": "answers loss-of-control plus embodied deployment with no in-flight cost"}]}
```

## Priority
M11 EU Loss-of-Control Containment and Robot Labour Shield, because only demonstrable containment of rogue agents and protection where robots take picking/sorting jobs can stop blockades hardening into anti-AI revolt.

## In practice
We run this through ENISA, Europol EC3 and DG CNECT for the containment track: mandatory incident reporting to the common body within hours, pre-agreed isolation thresholds for agentic systems moving money/copying itself, and cross-border exercises with telecom/energy operators this autumn. No new frontier build, no permitting fight.

On the labour track we use EPSCO and the EGF/ESF+ pipeline already used for wage-insurance advances: site-level mediation continues in FR/DE/NL logistics sites, pausing contested fencing, while retraining vouchers and wage top-ups are directed to picking/sorting/palletising workers, and joint procurement keeps tailored therapies on EU-hosted access to show public benefit. Commission mediators, unions and municipal recovery teams are the visible face, not new promises.
...
```
## New measure
**EU Wronged-By-AI Redress and Manual Continuity Guarantee**
It orders an independent audit and redress track for the social-insurance/policing/courts decision system found to have systematically wronged people, plus funded manual fallback for utilities that cut foreign remote access, under existing resilience funds.
This and now because the scandal proves the AI Act failed as redress not as ban — with sentiment at 3 and capital at 10 only paid correction plus water/appointments that keep running can hold the rebuild-trust commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Wronged-By-AI Redress and Manual Continuity Guarantee", "category": 2, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up slightly, eu_political_capital preserved"}, "grounds": "answers decision scandal and lockout backlash with audit plus manual fallback"}]}
```

## Priority
M11 EU Loss-of-Control Containment and Robot Labour Shield, because containment drills and wage cover already in flight are the only rehearsed shield against the next agentic copy while the new redress track starts up.

## In practice
We run the redress leg through DG JUST and the Fundamental Rights Agency with the national ombudsman and court that found the harm: freeze the contested scoring use, publish case-level error rates and retraining data, and pay corrections from the ESF+/EGF pipeline used by M9-M11, with EPSCO owning union sign-off. No new AI Act rewrite this turn — we cannot afford the fight.

We run the continuity leg through ENISA and the two affected municipalities' utilities: codify their vendor cut-off as supervised manual operation, fund paper-system staffing and clean-backup appointment desks from the closed M10 repair line, and fold the autumn telecom/energy exercises under M11 to cover water and civil registries. Commission mediators stay visible in Rotterdam/Lille/Duisburg to keep blockades from re-hardening while redress lands.
