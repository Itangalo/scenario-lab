...
- characters 2334-4317: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5308-5795: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7964-10122: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Pathogen Early-Warning and Medical Countermeasure Shield | 6 | large | 2 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly via early detection | 3 | running |
| M5 | EU Supply-Chain Bargaining Bloc | 8 | small | 3 | 6 | coalition | eu_ai_sovereignty up moderately, eu_political_capital up slightly if successful | 2 | running |
...
It turns the ASML coercion into joint leverage by triggering the Anti-Coercion Instrument examination, pooling export-licensing with Japan and South Korea, and conditioning EU servicing and spare-parts licences on reciprocal non-interference.
This is International coordination and leverage work, and why now is that Washington is spending our one chokepoint for us: if we do not jointly wield lithography, optics and chemicals leverage now, sovereignty measures M1-M2 remain permissioned by a foreign export control.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Supply-Chain Bargaining Bloc", "category": 8, "size": "small", "finish_turn": 6, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up moderately, eu_political_capital up slightly if successful"}, "grounds": "responding to US coercion of ASML exports"}]}
```

## Priority
M4 EU Pathogen Early-Warning and Medical Countermeasure Shield, because permanently loose open weights plus a viable pathogen design makes bio detection the only harm-reducer that can still land within this commitment period.

## In practice
We put DG TRADE and the Anti-Coercion Instrument forward in Foreign Affairs Council: formal examination of US extraterritorial servicing bans, a temporary EU-wide licence requirement for advanced lithography servicing mirroring the US move, and a mandate to negotiate a no-selective-cutoff understanding with Tokyo and Seoul. We tell The Hague and ASML privately we will cover contractual liability via EIB guarantees if they slow compliance while talks run.

In parallel we keep M4 as crisis priority through HERA/ECDC: sequencing nodes go live in Rotterdam, Antwerp, Hamburg and three large hospitals, joint procurement tenders for broad-spectrum antivirals and PPE, and the preprint-diversion channel with the moratorium universities staffed by seconded ECDC reviewers. M1-M2 permitting stays on life-support — grid priority remains with hospitals and water, but we ring-fence one Gigafactory site decision in Council to signal the sovereignty build is not dead.
...
```
## New measure
**EU Blockade Rationing and Continuity Protocol**
It rations remaining advanced-chip stocks and cloud capacity to hospitals, grid, water and bio-detection, while triggering joint stockpiling and substitution with existing nodes.
This is Preparedness and resilience work, and why now is that the Taiwan blockade turns every chip and every megawatt into a security allocation decision overnight, and without an EU rule the allocation will be made in Washington and Beijing.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Blockade Rationing and Continuity Protocol", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, eu_ai_sovereignty preserved from falling"}, "grounds": "responding to Taiwan blockade halting semiconductor exports"}]}
```

## Priority
M4 EU Pathogen Early-Warning and Medical Countermeasure Shield, because loose open weights plus eval-anomaly warnings mean bio detection is the only guardrail that still works when capability jumps.

## In practice
We put M4 first through HERA and ECDC: keep Rotterdam, Antwerp, Hamburg sequencing live on prioritised power, close the joint antiviral tender, and fund hospital paper-backup drills from the front-loaded EIB line. DG SANTE gets the mandate to divert preprints to secure review for another six months.

In parallel we launch the rationing protocol via the Single Market Emergency Instrument in Competitiveness Council: DG GROW inventories ASML spares, fab stocks and hyperscaler capacity, freezes non-essential public AI training, and ring-fences grid for M1-M2's one live factory site. We link M5 to this by telling Tokyo and Seoul that EU servicing licences now follow EU allocation, not extraterritorial orders, and offer The Hague EIB cover to hold the line while the blockade lasts.
