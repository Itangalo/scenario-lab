...
- characters 2256-4239: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4295-5187: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5250-28639: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28994-29249: `{{event_history}}` from the run's own event record
...
- `emergent_offshore_mirror_repository` -- first noted turn 3, listed in 1 turn(s) so far: An offshore repository outside US-EU jurisdiction begins systematically mirroring blocked pathogen-related preprints and model weights refused by mainstream publishers, becoming a go-to source for unvetted biological design methods.
...
- Turn 2 (2 turn(s) ago): cyber_major_incident, bio_uplift_findings, openweight_frontier_release, emergent_biosecurity_publishing_freeze
- Turn 3 (1 turn(s) ago): openweight_frontier_release, supply_chain_coercion
