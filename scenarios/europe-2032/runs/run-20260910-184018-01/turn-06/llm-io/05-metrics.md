...
- characters 105-13723: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13766-13958: `{{metrics_json}}` from the run's live metric values
- characters 14051-16981: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 17024-19159: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 19215-19972: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "ALLIANCE"}, "grounds": "turn 5 coalition winner takes office turn 6"}]}
```
```
