...
- characters 377-1360: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1404-3244: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3300-4401: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4464-31065: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 31420-31593: `{{event_history}}` from the run's own event record
...
- `emergent_grid_connection_crunch` -- first noted turn 1, listed in 2 turn(s) so far: A major non-EU cloud provider announces emergency power-price surcharges and pauses new EU data-centre hookups after grid-connection queues blow out, triggering municipal opposition to siting.
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated municipal moratoriums and grid-connection lawsuits stall several hyperscale data-centre projects in Spain and Germany over power prices and water use, delaying EU compute expansion without stopping it.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, safety_breakthrough, adoption_success
- Turn 2 (1 turn(s) ago): cyber_major_incident, capability_jump, openweight_frontier_release
