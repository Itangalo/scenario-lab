...
- characters 455-1438: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1482-3322: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4525-4991: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6638-8964: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_datacentre_siting_backlash (emergent event):** Coordinated municipal moratoriums and grid-connection lawsuits stall several hyperscale data-centre projects in Spain and Germany over power prices and water use, delaying EU compute expansion without stopping it.
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | running |
| M4 | EU Rapid Containment and Recovery Corps | 6 | small | 2 | 4 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | running |
...
## New measure
**EU Rapid Containment and Recovery Corps**
Deploys pre-authorised automated-isolation playbooks, mutual-aid cyber teams and stockpiled recovery kits to keep the ransomware / supply-chain attack from cascading across public services and grid operators.
This is Preparedness and resilience work, and why now is the large automated model-generated attack landing on top of undetected grid presence plus an open-weight frontier release that puts the same offensive tooling permanently on private hardware — containment and recovery within EU control must outrun attribution and recall which we do not have.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Rapid Containment and Recovery Corps", "category": 6, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via incident learning"}, "grounds": "answering major automated cyber incident amid open frontier release"}]}
```

## Priority
M3 EU Critical Systems Shield, because the major incident validates its detection and exercised-recovery core and the new Corps only works if Shield sensors and 24-hour reporting are forced through now.

## In practice
We trigger the Shield's cross-border aid clause early via ENISA and DG HOME, standing up the Corps from CERT-EU plus French, German and Polish national teams with emergency joint procurement for endpoint isolation and clean backups for hospitals, municipalities and the two compromised TSOs. The €2bn hardening fund is re-sequenced: first payouts only for operators who install mandated detection and join the Q1-Q2 exercises, with non-compliant operators named to Energy and JHA Councils.

At the same time we hold the compute lines from stalling: InvestAI Gigafactories and the permitting-zones package stay in site-negotiation mode, using grid-connection priority for Shield-compliant zones and quoting public-sector AI waiting-list wins to blunt Spanish and German municipal opposition. The AI Office bakes the capability jump and open-weight release into evaluation — mandatory pre-deployment checks for agentic persistence and offensive-cyber uplift, using the new interpretability control — while we concede publicly that recall is gone and resilience is the strategy.
...
```
## New measure
**EU Trusted Access and Joint Response Accession**
Joins the joint cyber command telemetry pool offered after the spring attacks while negotiating restored, diversification-bound frontier access so hospitals, ministries and firms cut off this turn fail over to vetted alternatives.
This is International coordination and leverage work, and why now is the short-notice US frontier cutoff landing on top of open-weight-enabled ransomware and municipal siting lawsuits — protection we cannot build alone must be pooled immediately, and dependence exposed as a live outage must be bargained down collectively rather than absorbed bilaterally.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trusted Access and Joint Response Accession", "category": 8, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction plus joint cyber command partners", "targeted_effect": "resilience up moderately, eu_ai_sovereignty up slightly via diversified access terms"}, "grounds": "answering frontier access denial and joint pooling offer"}]}
```

## Priority
M3 EU Critical Systems Shield, because the access cutoff plus ongoing ransomware means exercised detection, 24-hour reporting and Shield-funded recovery are what keep hospitals and grids running while accession and compute lines land.

## In practice
We trigger ENISA and CERT-EU accession to the joint command this autumn: real-time telemetry sharing, pooled attribution, and mutual-aid dispatch under the Shield's cross-border clause, paid from the re-sequenced €2bn hardening fund with first payouts still conditional on detection and exercise participation. DG CNECT and the EEAS table a joint protest and technical workaround for the cutoff — emergency licences, API failover to non-cutoff providers, and clean EU-hosted inference for health and ministries — using the outage as leverage in Council to force the 24-hour reporting and naming of laggards through.

We hold the compute lines without opening a second programme: InvestAI Gigafactories and the permitting-zones package stay in negotiation, offering priority grid connection only to Shield-compliant zones and a water-and-price compact to Spanish and German councils to unblock lawsuits. The AI Office keeps tightened pre-deployment checks for persistence and offensive uplift, conceding containment is the doctrine while open weights circulate.
