...
- characters 455-1929: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1973-3981: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6351-6817: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9253-11749: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M6 | EU Clean Sovereign Stack for Essential Services | 9 | small | 4 | 6 | own jurisdiction | eu_ai_sovereignty up moderately, resilience up moderately, public_sentiment up slightly via visible failover and local benefits | 2 | running |
