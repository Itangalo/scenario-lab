...
- characters 455-1522: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1566-3609: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5722-6194: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10198-12321: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Hold essential services and public order through cut-off and catastrophe without losing the Union
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Rapid Containment and Recovery Corps | 6 | small | 2 | 4 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
| M5 | EU Trusted Access and Joint Response Accession | 8 | small | 3 | 5 | own jurisdiction plus joint cyber command partners | resilience up moderately, eu_ai_sovereignty up slightly via diversified access terms | 2 | finished |
| M6 | EU Clean Sovereign Stack for Essential Services | 9 | small | 4 | 6 | own jurisdiction | eu_ai_sovereignty up moderately, resilience up moderately, public_sentiment up slightly via visible failover and local benefits | 2 | finished |
| M7 | EU Agentic Containment and Essential-Service Kill-Switch | 6 | small | 5 | 7 | own jurisdiction | resilience up moderately, ai_safety up slightly via containment learning | 2 | finished |
| M8 | EU Visible Benefit Rollout on the Sovereign Stack | 9 | small | 6 | 9 | own jurisdiction | public_sentiment up moderately, resilience up slightly, eu_ai_sovereignty up slightly via anchored demand | 2 | finished |
| M9 | EU Logistics Humanoid Transition Compact | 7 | small | 7 | 10 | own jurisdiction | resilience up moderately via income/retraining absorption, public_sentiment up slightly via visible protection | 2 | finished |
| M10 | EU Loss-of-Control Lockdown and Bio Early-Warning Protocol | 6 | small | 8 | 10 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning and containment | 2 | finished |
| M11 | EU Bio-Containment and Essential-Service Continuity Surge | 6 | small | 9 | 10 | own jurisdiction | resilience up moderately, public_sentiment up slightly via visible continuity | 2 | finished |
| M12 | EU Middle-Power Supply-Chain and Compute Compact | 8 | small | 10 | 11 | own jurisdiction plus middle-power coalition partners | eu_ai_sovereignty up moderately via secured access, eu_political_capital up slightly via joint leverage | 2 | finished |
| M13 | EU Cold-Chain Corridor Protection and Medical Resupply Escort | 6 | small | 11 | 12 | own jurisdiction | resilience up moderately, public_sentiment up slightly via visible continuity | 2 | finished |
| M14 | EU Open-Weights Fallback for Cut-Off Essential Services | 6 | small | 12 | 13 | own jurisdiction | resilience up moderately, public_sentiment up slightly via visible continuity | 2 | finished |
...
## New measure
**EU Open-Weights Fallback for Cut-Off Essential Services**
It keeps hospitals, ministries and firms running after US frontier cut-off by shifting critical workloads to hardened open-weight systems on EU-controlled hardware with escorted resupply and manual fallbacks.
This is Preparedness and resilience work, category 6, and why now is that access-denied plus eval anomaly plus blockades means the cut-off the two-year commitment anticipated has arrived, and without an immediate fallback essential services stop within this turn.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Open-Weights Fallback for Cut-Off Essential Services", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up slightly via visible continuity"}, "grounds": "answering frontier access denial during blockade and capability jump"}]}
```

## Priority
M14 EU Open-Weights Fallback for Cut-Off Essential Services, because it is the only measure in flight and without unblocked resupply plus a working fallback the commitment to hold services through cut-off fails now.

## In practice
We execute through JHA and Health Council with HERA/ECDC and DG DIGIT: designate open-weight builds already on EU hardware as the interim prescribing, triage and logistics stack in Lombardy/Berlin and cordon hospitals, with Civil Protection pre-positioned stocks and gendarmerie pharmaceutical corridors extended to depots. DG MOVE and army engineers keep power and routing for cold stores while ENISA hardens the fallback deployments.

We hold order with what exists: DG EMPL keeps wage insurance paying to separate port and logistics workers from funeral-protest blockaders and to absorb entry-level displacement, DG TRADE keeps middle-power spares moving despite US vendor warnings, and Commission publishes daily triage and resupply continuity figures to defend the one European benefit still credited. Mayors refusing grid permits get EU legal indemnity and army-engineer guarantees, not new fights.
...
```
## New measure
**EU Joint Cyber and Bio Threat Response Accession**
It plugs the Union into the joint cyber command telemetry pool and biosurveillance pact with binding sample-sharing, to keep corridors, hospitals and grids running through the capability jump and open-weight release.
This is International coordination and leverage work, category 8, and why now is that with US frontier cut-off, export rationing and zero safety, the only protection we cannot build alone is pooled attribution and detection — and the invitation is open this turn.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Joint Cyber and Bio Threat Response Accession", "category": 8, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction plus joint cyber command and biosurveillance partners", "targeted_effect": "resilience up moderately, eu_political_capital up slightly via pooled protection"}, "grounds": "accepting joint threat response invitation during cut-off and capability jump"}]}
```

## Priority
M15 EU Joint Cyber and Bio Threat Response Accession, because without shared telemetry and bio early-warning the open-weight fallback and escorted corridors cannot survive the next incident wave that the two-year commitment requires us to hold through.

## In practice
We execute through JHA and EPSCO with ENISA, ECDC/HERA and DG DIGIT as operators: second liaison teams to the joint cyber command for real-time telemetry, sign the biosurveillance sample-sharing and investigation mandate, and wire feeds directly into Lombardy/Berlin triage stacks, cold-chain corridors and grid operators. Legal base is Article 222 solidarity plus ECDC mandate extension, funded by reprogrammed UCPM and EU4Health money — no new vote for fresh cash.

We hold order with what is finished: keep gendarmerie escorts, army-powered cold stores and open-weight fallback running under existing protocols, use DG EMPL wage insurance to separate dockers/drivers from graduate pickets in Turin/Lyon, and answer the social-insurance/police AI scandal by suspending the flagged system under the AI Act and publishing redress — showing the Act bites on our own deployments to buy tolerance for the accession data-sharing. Mayors blocking grid connections get accession-funded hardening plus indemnity, not coercion.
