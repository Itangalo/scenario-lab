...
- characters 107-13390: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13433-13624: `{{metrics_json}}` from the run's live metric values
- characters 13717-14784: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14828-16871: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16927-17966: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M15 | EU Joint Cyber and Bio Threat Response Accession | 2 |
...
PORTFOLIO CHARGE t12: M14 EU Open-Weights Fallback for Cut-Off Essential Services −2, priority −1 = −3
PROPOSAL BONUS: none — M14 already in flight, no new measure proposed this turn beyond ongoing fallback
LEGITIMACY LENDS: capital 12, sentiment 12 -> none, sentiment not above capital
SOVEREIGNTY: 12 last turn, no capacity event finish, no cat4 in flight, eu_frontier_access_denied t12 −2, capability rose 1.0 no −1 = 10

## Emerging developments (tracked)

- `emergent_shadow_inference_market` -- first noted turn 13, listed in 1 turn(s) so far: Clandestine broker networks resell smuggled US frontier inference to EU hospitals and firms via third-country cut-outs, creating an unregulated shadow dependency outside corridors and fallback.
- `emergent_community_inference_squats` -- first noted turn 13, listed in 1 turn(s) so far: Graduate-led occupations of shuttered university computing labs to run the EU open fallback for local clinics, creating unsanctioned but tolerated community inference sites outside the guarded corridors.
...
## Metrics
```json
{"ai_capability": 95.0, "openweight_capability": 88.0, "ai_safety": 0.0, "resilience": 57.0, "eu_ai_sovereignty": 7.0, "eu_political_capital": 10.0, "public_sentiment": 7.0}
```
...
## Notepad
PORTFOLIO CHARGE t13: M15 EU Joint Cyber and Bio Threat Response Accession −2, priority −1 = −3
PROPOSAL BONUS: M15 (cat 8, small) +2 — joint threat invitation this turn plus cut-off/export rationing and capability jump from last turns, this answers pooled protection directly
LEGITIMACY LENDS: capital 10, sentiment 7 -> none, sentiment not above capital
SOVEREIGNTY: 10 last turn, no cat4 finish, no cat4 in flight, export_control_escalation t13 −2, capability rose 4.0 −1 = 7

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M15", "fields": {"finish_turn": 14}, "grounds": "named priority"}]}
```
```
