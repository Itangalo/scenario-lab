...
- characters 105-13528: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13571-13763: `{{metrics_json}}` from the run's live metric values
- characters 13856-15233: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15276-17015: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17071-18266: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Clean Sovereign Stack for Essential Services | 2 |
...
- `emergent_municipal_insolvency_wave` -- first noted turn 4, listed in 1 turn(s) so far: Small communes hit by the spring ransomware wave face cascading service insolvencies as recovery contractors and backup costs overwhelm budgets, forcing emergency municipal mergers and state receiverships.
- `emergent_grid_permit_freeze` -- first noted turn 4, listed in 1 turn(s) so far: Municipal utilities in France and Germany suspend approved hyperscale grid connections after court challenges, freezing several AI-ready sites mid-build and pushing developers toward locations outside the EU.
