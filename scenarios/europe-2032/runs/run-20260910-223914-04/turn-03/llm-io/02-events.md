...
- characters 377-1524: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1568-3616: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3672-4794: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4857-28433: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28788-28829: `{{event_history}}` from the run's own event record
...
- `emergent_attribution_leak` -- first noted turn 2, listed in 1 turn(s) so far: A joint investigative reporting consortium publishes forensic evidence credibly linking the autumn grid pre-positioning swarm to specific state-contracted compute clusters, forcing public attribution debate.
- `emergent_datacentre_backlash` -- first noted turn 2, listed in 1 turn(s) so far: Coordinated local protests and municipal permit challenges stall at least one AI factory site, forcing a temporary construction pause and national debate over power prices and water use.
