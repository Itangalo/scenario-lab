...
- characters 1779-3761: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3817-4748: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4811-28093: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28448-28489: `{{event_history}}` from the run's own event record
...
- `emergent_cyber_insurance_crunch` -- first noted turn 2, listed in 1 turn(s) so far: A major European insurer reprices cyber cover after the autumn pre-positioning audits, sharply raising premiums or withdrawing cover for municipal utilities and hospitals that cannot prove hardening.
- `emergent_gigafactory_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: Coordinated local protests and municipal permit challenges stall one of the shortlisted gigafactory sites over power and water use, forcing site renegotiation.
