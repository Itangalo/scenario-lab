...
- characters 455-1821: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1865-3653: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4279-4719: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6326-8412: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_gulf_compute_offer (emergent event):** A Gulf sovereign fund publicly offers multi-billion co-financing for EU gigafactory power and data-centre build-out in exchange for guaranteed inference capacity and equipment-supply preferences.
...
| M3 | EU AI Evaluation and Safety Institute | 5 | large | 1 | 5 | own jurisdiction | ai_safety up, eu_ai_sovereignty up modestly | 3 | running |
| M4 | EU Grid Cyber-Hardening and Segmentation Directive | 6 | small | 2 | 5 | own jurisdiction | resilience up, ai_safety up modestly | 2 | running |
...
## New measure
**EU Grid Cyber-Hardening and Segmentation Directive**
Orders emergency segmentation reviews into binding segmentation, credential-reset and exercised contingency standards for transmission operators, funded via existing resilience envelopes. This is category 6 Preparedness and resilience, and why now is because the autumn swarm proved supposedly isolated grids walkable via open-weight industrial tooling, while the new interpretability breakthrough gives our Evaluation Institute something deployable to pair with hardening.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Grid Cyber-Hardening and Segmentation Directive", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up modestly"}, "grounds": "to answer grid intrusion before next probe causes outage"}]}
```
## Priority
EU AI Evaluation and Safety Institute, because vetted pre-deployment testing and weight-security audits are the precondition for enforcing postponed AI Act obligations and for absorbing the safety breakthrough into deployed systems.
## In practice
We keep InvestAI Gigafactories as the long build through DG CNECT, EIB and state-aid clearance, pressing Paris, Berlin, Madrid, Stockholm and Warsaw to lock grid connections and cohesion side-payments while utilities get permitting guarantees. The Tech sovereignty package runs alongside via AI Continent accelerated-permitting zones, coaxing private co-finance despite power-price resistance.

This turn we operationalise the Evaluation Institute under the AI Office with JRC support, fast-tracking hiring of frontier evaluators and using the interpretability breakthrough as its first deployable assurance upgrade, while demanding — and publicly noting Washington's delay on — vetted access to US frontier models. In parallel we task ENTSO-E, ENISA and national TSOs with the grid-hardening standards, framing segmentation and incident exercises as emergency internal-market security rather than new fiscal burden.
...
```
## New measure
**EU Common Terms for External AI Capacity Deals**
Sets Union-level conditions for any Gulf or hyperscaler co-financing of gigafactory power and compute — EU legal anchoring, no extraterritorial kill-switch, capped and interruptible inference reservation — and routes bilateral side-deals through Council clearance to stop the defection undercutting the line. This is International coordination and leverage, and why now is because the public Gulf billions offer plus a member-state side arrangement arriving together will either fund our sovereign build on our terms or fragment it.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Common Terms for External AI Capacity Deals", "category": 8, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up, eu_political_capital up modestly by restoring cohesion"}, "grounds": "to channel Gulf offer and block defecting bilateral on Union terms"}]}
```
## Priority
EU Common Terms for External AI Capacity Deals, because without a common line the Gulf money and the defecting member state will decide the sovereignty build instead of the Union, outranking even the Evaluation Institute this turn.
## In practice
We run the new terms through the Foreign Affairs and Energy Councils with DG TRADE and DG CNECT drafting, using the Anti-Coercion and FDI-screening bases to require notification of bilateral compute deals, and offer the defecting capital cohesion side-payments and a first gigafactory power tranche to come back inside. The Gulf fund is invited to negotiate solely with the Commission/EIB vehicle tied to InvestAI Gigafactories, not capitals singly.

In parallel we keep M1/M2 alive by converting provisional grid connections in Paris, Berlin, Madrid, Stockholm and Warsaw into final investment decisions, pressing utilities on permitting guarantees while earmarking Gulf co-finance only if it accepts our anchoring terms. M3 auditors continue hospital and grid deployments of the control-evaluation upgrade, and M4 segmentation and credential-reset standards move to exercised drills via ENTSO-E/ENISA, funded without opening a new budget fight.
