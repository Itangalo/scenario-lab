...
- characters 377-1743: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1787-3575: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3631-4779: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4842-29069: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29424-29511: `{{event_history}}` from the run's own event record
...
- `emergent_gulf_compute_offer` -- first noted turn 2, listed in 1 turn(s) so far: A Gulf sovereign fund publicly offers multi-billion co-financing for EU gigafactory power and data-centre build-out in exchange for guaranteed inference capacity and equipment-supply preferences.
- `emergent_ics_toolkit_proliferation` -- first noted turn 2, listed in 1 turn(s) so far: Forensic write-ups of the autumn grid intrusions circulate and a repackaged ICS-probing toolkit derived from a frontier-class open model spreads on underground forums, lowering the bar for copycat grid reconnaissance.
...
- Turn 2 (1 turn(s) ago): safety_breakthrough
