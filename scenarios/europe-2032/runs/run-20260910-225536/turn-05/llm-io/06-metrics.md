...
- characters 106-13756: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13799-13990: `{{metrics_json}}` from the run's live metric values
- characters 14083-15204: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15248-17032: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17088-18147: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|---|---|---|---|---|---|---|
| M3 | EU AI Evaluation and Safety Institute | 5 | large | 1 | 5 | own jurisdiction | ai_safety up, eu_ai_sovereignty up modestly | 3 | finished |
| M4 | EU Grid Cyber-Hardening and Segmentation Directive | 6 | small | 2 | 5 | own jurisdiction | resilience up, ai_safety up modestly | 2 | finished |
| M5 | EU Common Terms for External AI Capacity Deals | 8 | small | 3 | 5 | own jurisdiction | eu_ai_sovereignty up, eu_political_capital up modestly by restoring cohesion | 2 | finished |
...
- `emergent_cyber_insurance_freeze` -- first noted turn 5, listed in 1 turn(s) so far: European cyber-insurers suspend new coverage for municipal IT and regional hospitals after March losses, forcing emergency state backstops and freezing outsourcing renewals through autumn.
- `emergent_insurer_coverage_retreat` -- first noted turn 5, listed in 1 turn(s) so far: European cyber-insurers sharply restrict coverage for municipalities and hospitals after March losses, demanding audited segmentation before renewal and leaving dozens of public bodies temporarily uninsured.
...
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 6, sentiment 30 -> +2
SOVEREIGNTY: 17 last turn, no category 4 finish, Gigafactories in flight +1, middle_power_coalition t5 +2, capability rose 1.0 no deduction = 20

## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "pending"}}, {"op": "update", "table": "measures", "id": "M2", "fields": {"finish_turn": 7}, "grounds": "named priority"}]}
```
```
