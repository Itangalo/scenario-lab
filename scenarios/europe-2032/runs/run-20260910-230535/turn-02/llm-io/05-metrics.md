...
- characters 105-13041: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13084-13276: `{{metrics_json}}` from the run's live metric values
- characters 13369-14506: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14549-16578: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16634-17684: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_interim_hosted_offer` -- first noted turn 2, listed in 1 turn(s) so far: A major US hyperscaler publicly offers interim sovereign-hosted inference capacity to EU governments and hospitals on EU soil while gigafactories are built, easing near-term dependence fears but complicating anchoring negotiations.
- `emergent_hyperscaler_pause_eu_build` -- first noted turn 2, listed in 1 turn(s) so far: A major US hyperscaler publicly pauses two planned EU data-centre expansions, citing grid-connection delays and power-price uncertainty amid gigafactory site bidding.
