...
- characters 377-1391: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1435-4037: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4093-4934: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4997-27618: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27973-28274: `{{event_history}}` from the run's own event record
...
- `emergent_provenance_mandate` -- first noted turn 1, listed in 2 turn(s) so far: Major EU scientific publishers and universities jointly mandate watermarking and provenance disclosure for AI-assisted papers after the Millennium Prize proof faces authenticity challenges.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, openweight_frontier_release, knowledge_work_augmented, eu_access_secured, automated_decision_scandal, emergent_datacentre_siting_backlash
- Turn 2 (1 turn(s) ago): cyber_major_incident, loss_of_control_incident, ai_investment_collapse, member_state_defection
