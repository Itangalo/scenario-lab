...
- characters 455-2022: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2065-3897: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4524-4999: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7894-9939: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Rebuild sovereign capacity and public trust through European AI that delivers visible public benefit
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Trusted Public Services Deployment | 9 | small | 11 | 14 | own jurisdiction | public_sentiment up materially, resilience up slightly | 2 | running |
...
| M3 | EU Grid and Critical Systems Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up materially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Agent Containment and Essential-Service Continuity Protocol | 6 | small | 2 | 4 | own jurisdiction | resilience up, ai_safety up slightly via containment learning | 2 | finished |
| M5 | EU Verification Bridge and Allied Supply Compact | 8 | small | 3 | 7 | coalition | ai_safety up via shared assurance, eu_ai_sovereignty up slightly via secured supply | 2 | finished |
| M6 | EU Bio Shield and Continuity Corps | 6 | small | 5 | 9 | own jurisdiction | resilience up materially, ai_safety up slightly via detection learning | 2 | finished |
| M7 | EU Essential-Service Failover Reserve | 6 | small | 8 | 10 | own jurisdiction | resilience up materially, eu_ai_sovereignty up slightly via EU-hosted serving | 2 | finished |
| M8 | EU Assured Failover Upgrade | 5 | small | 9 | 11 | own jurisdiction | ai_safety up materially, resilience up slightly, public_sentiment up slightly via demonstrably safer services | 2 | finished |
...
## New measure
**EU Trusted Public Services Deployment**
Puts certified EU-hosted fallback builds to work visibly in hospitals, administrations and SMEs with signed outputs and human-review guarantees, paired with wage-insurance and retraining for exposed clerical roles.
This is category 9 diffusion work because only visible public benefit plus a workplace guarantee can lift sentiment from 20 and unblock siting and courts, and now is the moment when the failover that held in spring gives us something safe to deploy and the productivity evidence gives us something to share.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trusted Public Services Deployment", "category": 9, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up materially, resilience up slightly"}, "grounds": "to convert certified failover into visible benefit while capital is scarce"}]}
```

## Priority
EU Trusted Public Services Deployment, because with no measures in flight and capital at 19 only a low-cost trust rebuild that answers hostility, court freezes and the cutoff can restore room to act.

## In practice
We task DG CNECT and DG EMPL via the European Social Fund+ and Digital Europe to fund deployment grants for municipalities and hospitals using the M7/M8 assured catalogue, with AI Office-JRC certification, ENISA signed-output and audit-log requirements, and EPSU-employer workplace agreements on retraining and income bridging. EPSC communications frames the leaked eval anomaly as reason for EU checks, not foreign black-box dependence.

We hold the Anti-Coercion file and infringement line on existing high-risk duties without new legislation, offering blocked municipalities and the holdout capital reserved assured-inference slots and co-managed pilots under written safety guarantees to lift injunctions, while EIB keeps the fenced gigafactory site warm and Taiwan-risk supply monitoring continues through the Verification Bridge coalition.
...
```
## New measure
**EU Anomaly Review and Frontier Watch**
Stands up a rapid JRC-AI Office review of the leaked eval anomaly with vetted researcher access and a public incident note, tying findings to the assured catalogue's interpretability checks.
This is Evaluation and oversight work because the leak is being used to justify EU checks without evidence, and a credible independent read now protects both safety credibility and the deployment trust M9 needs while capital is at 21.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Anomaly Review and Frontier Watch", "category": 1, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up slightly, public_sentiment up slightly via credible review"}, "grounds": "to answer eval anomaly without new restrictions while M9 deploys"}]}
```

## Priority
EU Trusted Public Services Deployment, because only visible benefit in hospitals and counters can lift sentiment from 22 and unblock courts and siting, and it outranks the new review while capital is scarce.

## In practice
We keep DG CNECT and DG EMPL deployment grants flowing to the three hospital states and pilot municipalities under AI Office-JRC certification and ENISA signed-output rules, with EPSU workplace deals on retraining and wage bridging, offering blocked cities reserved assured slots and co-management under written human-review guarantees to lift the two injunctions. No new legislation; Anti-Coercion file stays held.

We task JRC and the AI Office to run the anomaly review with ENISA and vetted academics, requesting model-access via the Verification Bridge coalition where possible, publishing methods and limits, and framing results for EPSC communications as why EU-hosted assured builds exist. EIB keeps the fenced gigafactory warm and Taiwan-risk chip monitoring continues, without reopening a parallel compute build.
