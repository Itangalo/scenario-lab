...
- characters 453-1960: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2004-4170: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4533-5011: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8300-11180: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_permit_freeze_spillover (emergent event):** Coordinated municipal water and planning challenges spread from data-centres to grid-scale battery and substation upgrades, freezing several cross-border power interconnection works counted for resilience this winter.
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Essential-Site Protection and Bio-Cyber Shield | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, ai_safety up slightly via defensive tooling and bio screening | 2 | running |
