...
- characters 105-13731: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13774-13966: `{{metrics_json}}` from the run's live metric values
- characters 14059-15255: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15298-17269: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17325-17721: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Loss-of-Control and Automated-Cyber Containment Protocol | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "ALLIANCE"}, "grounds": "turn 5 election winner takes office turn 6"}]}
```
```
