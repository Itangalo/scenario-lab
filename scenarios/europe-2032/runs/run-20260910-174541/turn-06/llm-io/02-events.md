...
- characters 376-1572: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1615-3586: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3642-4605: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4668-28278: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28633-29003: `{{event_history}}` from the run's own event record
...
- `emergent_local_energy_siting_block` -- first noted turn 4, listed in 2 turn(s) so far: Coordinated municipal protests and local permit blocks delay several EU data-centre and grid-upgrade sites, tightening near-term compute and power headroom without a central EU decision.
- `emergent_rationed_model_degradation` -- first noted turn 4, listed in 2 turn(s) so far: US hyperscaler announces EU-only model hosting delay, citing licence review backlog, leaving permit and hospital assistants running on older versions for months.
...
- Turn 2 (4 turn(s) ago): capability_jump, bio_uplift_findings, member_state_defection
- Turn 3 (3 turn(s) ago): export_control_escalation, adoption_success
- Turn 4 (2 turn(s) ago): eval_anomaly_reports, middle_power_coalition
- Turn 5 (1 turn(s) ago): cyber_major_incident, bio_incident, bio_uplift_findings, election_alliance
