...
- characters 454-2601: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2644-5092: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6276-6728: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9415-11482: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_siting_litigation_freeze (emergent event):** Coordinated municipal lawsuits halt data-centre water permits in two more member states, freezing planned resilience and gigafactory power upgrades for months.
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Entry-Level AI Transition Fund | 7 | small | 7 | 9 | own jurisdiction | resilience up moderately via social absorption, public_sentiment up slightly | 2 | running |
...
| M3 | EU Critical Systems Hardening and Detection Pact | 6 | large | 1 | 5 | own jurisdiction | resilience up materially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Bio Detection and Triage Upgrade | 6 | small | 2 | 5 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
| M5 | European Public Benefit AI Deployment | 9 | small | 3 | 6 | own jurisdiction | public_sentiment up materially, eu_political_capital up slightly | 2 | finished |
| M6 | Middle-Power Supply and Evaluation Pact | 8 | small | 4 | 6 | coalition | eu_ai_sovereignty up moderately, eu_political_capital up slightly | 2 | finished |
| M7 | EU Loss-of-Control and Automated-Cyber Containment Protocol | 6 | small | 6 | 8 | own jurisdiction | resilience up moderately, ai_safety up slightly via containment learning | 2 | finished |
...
## New measure
**EU Entry-Level AI Transition Fund**
Provides wage insurance, rapid retraining and hiring incentives targeted at routine coding, analysis and tier-one support roles displaced by AI, tied to public-sector adoption successes. Category 7 Labour and social protection, because labour displacement is now narrow and checkable and adoption success gives a visible benefit to anchor transition around, which is why now when capital is at 10 and broad measures are impossible.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Entry-Level AI Transition Fund", "category": 7, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately via social absorption, public_sentiment up slightly"}, "grounds": "narrow entry-level displacement meets adoption success window"}]}
```

## Priority
M7 EU Loss-of-Control and Automated-Cyber Containment Protocol, because eval anomaly reports reopen the control gate while safety is at 17 and a second loss-of-control would collapse remaining capital before any new build can land.

## In practice
We push M7 from drafting to tabletop: ENISA, Europol EC3 and HERA get implementing mandates under Civil Protection Mechanism and cross-border health threat regulation, running isolation playbooks for agentic fund/record moves and shared flags from the spring sweep, with Japan/Korea observers folded in to stretch our thin evaluation capacity. No new law, no new money fight.

We launch the Transition Fund as ESF+ reprogramming plus EIB micro-grants, via EPSCO Council: member states with clinic/payroll disruption get first draw, conditioned on hiring retrained juniors into public-sector AI deployment teams where waiting lists have fallen, making the adoption success visible locally. DG EMPL administers, DG CNECT certifies skills. We explicitly pause any new sovereignty spend, holding M1/M2 finished stock and linking US access talks only to test-data exchange to avoid hollowing domestic builds while capital rebuilds.
...
```
## New measure
**EU Black-Box Oversight and Deployment Check Programme**
Rebuilds oversight after legible chain-of-thought is gone with black-box stress-testing, activation-inspection pilots, and binding pre-deployment checks for high-impact systems in critical sectors. Category 5 Public technical capacity and research, because opaque reasoning kills our CoT-dependent controls and we need own evaluation capacity now while sovereignty builds sit empty and capital is at 12.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Black-Box Oversight and Deployment Check Programme", "category": 5, "size": "small", "finish_turn": 11, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up slightly, resilience up slightly via deployment safeguards"}, "grounds": "opaque reasoning removes CoT oversight while safety at 16"}]}
```
## Priority
M8 EU Entry-Level AI Transition Fund, because with sentiment at 20 and capital at 12 only visible payouts to displaced juniors and triage-linked hiring can stop defection and siting revolt from hollowing the rest.
## In practice
We keep the money fight off the table. M8 runs through ESF+ reprogramming and EIB micro-grants via EPSCO, first draw to towns hit by payroll/clinic shutdowns and Rotterdam/Hamburg/Lyon warehousing collapse, conditional on placing retrained juniors into the two regions where triage assistants cut waiting lists and into municipal cyber-hygiene teams. No new fund, no unanimity vote.

We start the black-box programme inside existing mandates: ENISA-EU AI Office joint evaluation cell with Japan/Korea observers, using M7 tabletop flags and incident data from M3/M4, plus Horizon Europe redirection for activation-inspection pilots. For siting freeze and defection we do not legislate: Commission offers water-permit mediation packages and links gigafactory power upgrades to local resilience funds, while holding the US allied-access line to test-data exchange only to deny cover for the bilateral undercut.
