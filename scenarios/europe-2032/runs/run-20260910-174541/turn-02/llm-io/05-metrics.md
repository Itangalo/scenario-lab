...
- characters 105-13054: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13097-13289: `{{metrics_json}}` from the run's live metric values
- characters 13382-14592: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14635-16717: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16773-17905: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Bio Detection and Triage Upgrade | 2 |
...
- `emergent_genome_model_leak` -- first noted turn 1, listed in 2 turn(s) so far: A copy of a leading genome language model with phage-design capability is leaked outside voluntary screening channels and mirrored on foreign file-hosts, widening access to AI-assisted biological design.
- `emergent_biopublish_moratorium_fight` -- first noted turn 1, listed in 2 turn(s) so far: A major EU scientific publisher and a national research funder jointly impose a moratorium on publishing AI-designed pathogen methods after the phage results, triggering a high-profile fight over open science and pre-print servers.
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Hardening and Detection Pact −3, EU Bio Detection and Triage Upgrade −2, priority −1 = −12
PROPOSAL BONUS: EU Bio Detection and Triage Upgrade (cat 6, small) +2 — bio_uplift_findings landed this turn and this answers it directly, plus capability jump cyber pressure
LEGITIMACY LENDS: capital 24, sentiment 33 -> +2
SOVEREIGNTY: 21 last turn, no capacity build finishing, in flight +0, member_state_defection t2 −2, capability rose 2.0 −1 = 18
## Store changes
```json
