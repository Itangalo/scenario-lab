...
- characters 105-13158: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13201-13393: `{{metrics_json}}` from the run's live metric values
- characters 13486-14838: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14881-16728: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16784-17941: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | Middle-Power Supply and Evaluation Pact | 2 |
...
- `emergent_local_energy_siting_block` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated municipal protests and local permit blocks delay several EU data-centre and grid-upgrade sites, tightening near-term compute and power headroom without a central EU decision.
- `emergent_rationed_model_degradation` -- first noted turn 4, listed in 1 turn(s) so far: US hyperscaler announces EU-only model hosting delay, citing licence review backlog, leaving permit and hospital assistants running on older versions for months.
...
## Metrics
```json
{"ai_capability": 60.0, "openweight_capability": 55.5, "ai_safety": 20.0, "resilience": 34.0, "eu_ai_sovereignty": 16.0, "eu_political_capital": 8.0, "public_sentiment": 33.0}
```
## Narrative
