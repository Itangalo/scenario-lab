...
- characters 453-1517: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1561-3660: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4897-5375: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8858-10699: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Hold together through sabotage and uplift: keep lights on, services running, and no engineered pathogen
...
|---|---|---|---|---|---|---|---|---|---|
| M12 | EU Continuity and Redress Deployment | 9 | small | 12 | 15 | own jurisdiction | public_sentiment up materially, eu_political_capital up slightly, resilience up slightly | 2 | running |
...
| M8 | EU Entry-Level AI Transition Fund | 7 | small | 7 | 9 | own jurisdiction | resilience up moderately via social absorption, public_sentiment up slightly | 2 | finished |
| M9 | EU Black-Box Oversight and Deployment Check Programme | 5 | small | 8 | 11 | own jurisdiction | ai_safety up slightly, resilience up slightly via deployment safeguards | 2 | finished |
| M10 | EU Unsanctioned-Agent Containment and Essential-Service Continuity Order | 6 | small | 9 | 11 | own jurisdiction | resilience up moderately, ai_safety up slightly via containment | 2 | finished |
| M11 | EU Essential-Site Protection and Bio-Cyber Shield | 6 | small | 10 | 12 | own jurisdiction | resilience up moderately, ai_safety up slightly via defensive tooling and bio screening | 2 | finished |
...
## New measure
**EU Continuity and Redress Deployment**
It keeps cut-off hospitals and ministries running on EU-hosted models and fixes the scandal-hit decision system with human review and redress, to turn denial and scandal into visible benefit.
Category 9 Diffusion, adoption and public trust is why now: frontier access denial plus an automated-decision scandal plus a real safety breakthrough and a public-sector success give a narrow window to restore service and trust at once while capital is at 4.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Continuity and Redress Deployment", "category": 9, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up materially, eu_political_capital up slightly, resilience up slightly"}, "grounds": "access denied and scandal demand immediate continuity response"}]}
```

## Priority
EU Continuity and Redress Deployment, because with no measures in flight and services cut off, restoring hospitals and answering the scandal outranks ring-fencing alone and serves the hold-together commitment.

## In practice
We task the AI Office and ENISA to switch affected hospitals, ministries and firms to EU-hosted models on existing Gigafactory and EuroHPC capacity, applying the new interpretability control to already-deployed systems, with DIGIT funding continuity contracts. No new siting vote is asked for.

We answer the scandal under the AI Act: Employment/Social Council formation orders suspension, human re-review and ombudsman-led redress in the offending social-insurance/policing system, paid from existing ESF+ envelopes, while HERA/ECDC hold bio-screening and civil protection keeps guarded substation works moving. Mayors get community payments for permit release, not new derogations.
...
```
## New measure
None this turn, while political capital sits at 5 and the investment collapse has frozen the build-out on which any new capacity measure would depend.

## Store changes
No changes.

## Priority
M12 EU Continuity and Redress Deployment, because keeping cut-off hospitals and ministries running on EU-hosted models and delivering redress is the only hold-together work we can still fund through sabotage, denial and collapse.

## In practice
We keep the AI Office, ENISA and DIGIT on the M12 switch: continuity contracts keep patched EuroHPC and early-gigafactory capacity serving triage, procurement and service stacks, with the new interpretability check applied to deployed assistants, and machine-speed patching extended to the cut-off sites. No new siting vote is forced while permits are contested and substation sites are under attack.

We use the two positive signals inside the same envelope: Employment/Social Council formation continues suspension, human re-review and ESF+-paid compensation in the welfare scandal, while health ministries with HERA/ECDC procure the new tailored therapies only via EU-hosted, bio-screened pathways, and cite the measured office productivity gains without job loss to defend retraining via the finished Transition Fund. Mayors get continuity payments and queue-reduction evidence from the two working regions, not new derogations.
