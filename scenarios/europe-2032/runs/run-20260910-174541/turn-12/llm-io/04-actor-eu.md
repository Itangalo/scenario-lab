...
- characters 453-2207: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2250-4275: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6258-6736: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9874-11139: `{{previous_actions}}` from the actor's own response from the previous turn
...
## New measure
None this turn, while capital sits at 3 and sentiment at 9 with interconnection works frozen ahead of winter.
## Store changes
No changes.
## Priority
M11 EU Essential-Site Protection and Bio-Cyber Shield, because with safety at 1.0, open weights at 63.9, and permit-freeze spillover now hitting batteries and interconnectors, keeping lights on and screening bio outputs outranks any new start.
## In practice
We spend no new political capital we do not have. M11 is the operating arm: ENISA and the AI Office push the machine-speed patching and swarm-detection stack into the hospitals, grids and payments operators that missed the first wave, and HERA/ECDC hold the bio-output screening triage on genome-model queries while the disputed paper circulates.

We use Civil Protection and the Energy Council formation to ring-fence winter resilience: co-funded water-grid upgrades and community payments from existing Digital Europe and Cohesion envelopes in exchange for municipal release of the frozen battery and substation permits, with Europol-coordinated site protection continuing in the three sabotage states. No new gigafactory derogations, no new votes — unblock interconnectors, keep manual fallback exercised, and survive to rebuild capital.
