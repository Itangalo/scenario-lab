# Store: The European Union (turn 5)

## measures

| id | name | category | size | started_turn | finish_turn | applies_to | targeted_effect | cost_per_turn | status |
|---|---|---|---|---|---|---|---|---|---|
| M1 | InvestAI Gigafactories | 4 | large | 0 | 7 | own jurisdiction | eu_ai_sovereignty up, via €200bn for four to five sites | 3 | running |
| M2 | Tech sovereignty package | 4 | large | 0 | 6 | own jurisdiction | eu_ai_sovereignty up, via €200bn of private capital for AI data centres by 2036 and accelerated-permitting zones | 3 | running |
| M3 | EU Critical Services Shield | 6 | large | 1 | 4 | own jurisdiction | resilience up substantially, ai_safety indirectly via incident response | 3 | finished |
| M4 | EU Frontier Evaluation Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up modestly, eu_ai_sovereignty up indirectly via talent | 2 | finished |
| M5 | EU Municipal Recovery Surge | 6 | small | 3 | 5 | own jurisdiction | resilience up, public_sentiment stabilised | 2 | finished |
| M6 | EU Entry-Level Transition Fund | 7 | small | 4 | 6 | own jurisdiction | public_sentiment up, eu_political_capital stabilised, resilience up modestly | 2 | running |
| M7 | EU Agent Containment Protocol | 6 | small | 5 | 7 | own jurisdiction | resilience up, ai_safety up modestly, public_sentiment stabilised | 2 | running |

## standing

| id | posture |
|---|---|
| S1 | undecided |

## Changes this turn

- **applied** `M7` — {"op": "add", "table": "measures", "fields": {"name": "EU Agent Containment Protocol", "category": 6, "size": "small", "finish_turn": 7, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety up modestly, public_sentiment stabilised"}, "grounds": "unsanctioned agentic action with days-long containment requires EU-level stop and recovery capacity"}
  - Grounds: unsanctioned agentic action with days-long containment requires EU-level stop and recovery capacity
