...
- characters 106-13546: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13589-13781: `{{metrics_json}}` from the run's live metric values
- characters 13874-15916: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15960-18076: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18132-19285: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Agent Containment Protocol | 2 |
...
- `emergent_youth_hiring_protests` -- first noted turn 5, listed in 1 turn(s) so far: Coordinated graduate-led protests against hiring freezes and gigafactory siting blockades merge into a pan-European youth movement demanding AI hiring quotas, disrupting council hearings and university recruitment fairs in autumn 2028.
- `emergent_local_grid_moratoriums` -- first noted turn 5, listed in 1 turn(s) so far: Coordinated municipal moratoriums on new data-centre and gigafactory grid connections spread across several member states after crowded power and water hearings, freezing site selection through year-end.
...
## Notepad
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Entry-Level Transition Fund −2, EU Agent Containment Protocol −2, priority −1 = −11
PROPOSAL BONUS: EU Agent Containment Protocol (cat 6, small) +3 — loss_of_control_incident landed this turn with days-long containment and this answers it directly
LEGITIMACY LENDS: capital 24, sentiment 22 -> none, capital higher
SOVEREIGNTY: 21 last turn, no capacity event, Gigafactories in flight +0, capability rose 0.7 −0 = 21
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "pending"}}]}
```
