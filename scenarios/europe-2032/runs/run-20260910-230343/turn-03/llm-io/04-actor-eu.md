...
- characters 455-2123: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2167-4442: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4992-5464: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7095-8917: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Frontier Evaluation Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up modestly, eu_ai_sovereignty up indirectly via talent | 2 | running |
...
Stands up independent EU technical capacity to evaluate frontier models before deployment in Europe, with researcher access and authority to flag unsafe systems.
This is Public technical capacity and research, because the discontinuous capability jump has widened the assurance gap and the Union currently learns about capability only after deployment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Frontier Evaluation Cell", "category": 5, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up modestly, eu_ai_sovereignty up indirectly via talent"}, "grounds": "capability jump with falling assurance requires own evaluation capacity"}]}
```

## Priority
EU Critical Services Shield (M3) – because pre-positioning with unrecallable open models means absorption must land before the next probe becomes sabotage, outranking even compute this turn.

## In practice
We push M3 through ENISA and HERA with Justice and Home Affairs Council cover, making cohesion-funded hardening conditional on completing the mandatory drills and joining pooled detection procurement; interior ministries get money, we get exercised continuity plans in energy, ports and hospitals.

We keep M1 and M2 moving via DG CNECT and the Competitiveness Council toward final investment decisions on four to five gigafactory sites, holding priority grid connection to EU legal anchoring to contain the subsidy race. ASML leverage stays in reserve in the Foreign Affairs Council.

This turn we task the Joint Research Centre and ENISA to staff the Evaluation Cell from AI Office and national labs, negotiating pre-deployment access terms for models offered in the EU market to close the lagging-indicator gap.
...
```
## New measure
**EU Municipal Recovery Surge**
Deploys emergency EU cyber-response teams, rapid patch funding and enforced isolation-recovery playbooks to municipalities and hospitals hit by the automated attack.
This is Preparedness and resilience, because the major incident exposed that transmission operators held while municipal and health networks did not, and absorption must be made real before attribution arrives.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Municipal Recovery Surge", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment stabilised"}, "grounds": "major automated attack hitting public services requires immediate absorption"}]}
```

## Priority
EU Critical Services Shield (M3) – because the automated attack is testing exactly the continuity plans just exercised, and finishing drills and pooled detection now outranks gigafactory siting this turn.

## In practice
We run M3 as the recovery vehicle: ENISA-led joint teams with HERA support go to affected municipalities and hospital groups, cohesion disbursements are front-loaded conditional on joining pooled detection and completing isolation-recovery drills, and the Justice and Home Affairs Council gives cover to override the three interior ministries' reporting block for incident data.

We hold M1 and M2 to late-2027 FIDs via DG CNECT, offering priority grid connection tied to EU legal anchoring to stop the four-capital bidding war, while keeping chip-equipment controls in reserve. The Evaluation Cell (M4) is tasked to harvest tooling signatures from this model-generated attack for independent testing, pressing for researcher access despite US review-rule objections.
