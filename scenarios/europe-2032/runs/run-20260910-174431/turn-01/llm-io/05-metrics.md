...
- characters 106-12885: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12928-13120: `{{metrics_json}}` from the run's live metric values
- characters 13236-16185: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 16241-16834: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated physical protests and local permitting blocks halt or delay at least one major EU data-centre project as energy costs and job-loss fears mobilize opposition in late 2026.
- `emergent_cyber_insurance_retrenchment` -- first noted turn 1, listed in 1 turn(s) so far: Major European insurers sharply reprice or exclude systemic cyber cover for energy and port operators after the autumn intrusion disclosures, forcing emergency state backstops in two member states.
...
## Notepad
PORTFOLIO CHARGE t1: Gigafactories −3, Tech sovereignty package −3, European Critical Systems Shield −3, priority −1 = −10
PROPOSAL BONUS: European Critical Systems Shield (cat 6, large) +3 — autumn cross-continent grid intrusion with EU presence landed this turn and this answers it directly
