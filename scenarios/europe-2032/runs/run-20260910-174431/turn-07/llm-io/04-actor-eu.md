...
- characters 455-1440: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1484-3348: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4210-4658: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6999-8219: `{{previous_actions}}` from the actor's own response from the previous turn
...
## New measure
None this turn — waiting for the Continuity Stack to finish landing and for political headroom to rebuild before starting anything new.

## Store changes
No changes.

## Priority
M7 EU Continuity Stack for Cut-Off Public Services, because hospitals and ministries offline on US rationing outrank the low-burn gigafactory build and must be kept running on EU-hosted models first.

## In practice
We hold M1 on low burn through DG CNECT and the EIB facility — site reservations, grid connections and the volume-licence defence stay, no new hyperscaler chase — while all deployable attention goes to M7. The AI Office evaluation unit with ENISA certifies the EU-hosted open-weight replacement stack, funded by repurposed Digital Europe adoption money, made procurement-eligible with health and Shield operators first in the queue.

We sell the switch in the Health and Competitiveness Councils as the two-region waiting-list success writ large, offering finance ministers a no-new-money continuity fix and laggard capitals joint-procurement cover and premium relief under the cyber backstop. ASML coercion stays lodged in the Anti-Coercion Instrument track without opening a second spending front this turn.
