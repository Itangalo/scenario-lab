...
- characters 13272-14405: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14448-16454: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16510-17566: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated physical protests and local permitting blocks halt or delay at least one major EU data-centre project as energy costs and job-loss fears mobilize opposition in late 2026.
- `emergent_cyber_insurance_retrenchment` -- first noted turn 1, listed in 2 turn(s) so far: Major European insurers sharply reprice or exclude systemic cyber cover for energy and port operators after the autumn intrusion disclosures, forcing emergency state backstops in two member states.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 36.5, "eu_ai_sovereignty": 22.0, "eu_political_capital": 34.0, "public_sentiment": 38.0}
```
