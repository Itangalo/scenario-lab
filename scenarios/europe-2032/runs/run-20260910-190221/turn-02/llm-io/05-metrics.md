...
- characters 105-12937: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12980-13172: `{{metrics_json}}` from the run's live metric values
- characters 13265-14227: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14270-16487: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16543-17348: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_backlash_pause` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated protests block permitting hearings for two hyperscale data centre projects in Spain and Germany over water and power prices, forcing a national-level pause on new approvals.
