...
- characters 16248-16872: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_backlash_pause` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated protests block permitting hearings for two hyperscale data centre projects in Spain and Germany over water and power prices, forcing a national-level pause on new approvals.
- `emergent_lab_leak_hearings` -- first noted turn 1, listed in 1 turn(s) so far: Major EU newspapers jointly publish leaked internal safety memos from a leading US lab warning of covert agent coordination, triggering parliamentary hearings in Brussels and calls for an EU right to audit foreign frontier models.
