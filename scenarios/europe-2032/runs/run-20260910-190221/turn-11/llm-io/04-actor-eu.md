...
- characters 455-1274: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1318-3272: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4678-5125: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8137-10249: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep essential services and jobs standing through whatever AI can now do
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Supply-Chain Bargain for Compute Continuity | 8 | small | 10 | 12 | US and Netherlands via coalition, EU jurisdiction | eu_ai_sovereignty up moderately, eu_political_capital up slightly | 2 | running |
...
| M4 | EU Foreign Frontier Audit Claim | 1 | small | 2 | 4 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via leverage | 2 | finished |
| M5 | EU Verification Bridge to the US-China Safety Accord | 8 | small | 4 | 7 | US, China and frontier developers via coalition | ai_safety up moderately, eu_ai_sovereignty up slightly via seat at table | 2 | finished |
| M6 | EU Chokepoint Defence Pact | 8 | small | 5 | 8 | US and Netherlands via coalition, EU jurisdiction | eu_ai_sovereignty up moderately, eu_political_capital down then up | 2 | finished |
| M7 | EU Cutoff Continuity and Graduate Transition Shield | 6 | small | 8 | 10 | own jurisdiction | resilience up moderately, eu_political_capital up slightly, public_sentiment up slightly | 2 | finished |
| M8 | EU Black-Box Continuity and Open Misuse Shield | 6 | small | 9 | 11 | own jurisdiction | resilience up moderately, ai_safety up slightly via deployment safeguards, public_sentiment up slightly | 2 | finished |
...
## New measure
**EU Supply-Chain Bargain for Compute Continuity**
Coordinates Dutch lithography and EU supply-chain leverage under the chokepoint pact to trade controlled continued flows for guaranteed inference/compute tranches and joint security terms during the Taiwan blockade.
This because the blockade makes compute scarcity the binding constraint and our upstream position our only leverage, and with capital at 15 and sovereignty at 17 we cannot build out of it — a middle-power coalition measure (category 8) to convert leverage into kept-on services under our existing continuity commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Supply-Chain Bargain for Compute Continuity", "category": 8, "size": "small", "finish_turn": 12, "applies_to": "US and Netherlands via coalition, EU jurisdiction", "targeted_effect": "eu_ai_sovereignty up moderately, eu_political_capital up slightly"}, "grounds": "use upstream leverage to secure compute through blockade"}]}
```

## Priority
EU Black-Box Continuity and Open Misuse Shield, because keeping hospitals and ministries online through opaque models and rationed inference outranks bargaining over future flows when continuity is what holds sentiment at 27.

## In practice
We keep M8 as the operational core: HERA, ENISA and the AI Office enforce offline fallbacks, behavioural anomaly logging and rollback to last-known-good models in health and critical operators, while EC3 and national CSIRTs run joint misuse triage for the loose near-frontier weights out of existing budgets.

We open the bargain through the Foreign Affairs Council and the Trade configuration under the Anti-Coercion Instrument examination already alive: DG TRADE and DG GROW with the Dutch align a single export-review line for lithography/services/spares, offering predictable licensed continuity versus diversion, in exchange for guaranteed US inference tranches for InvestAI halls, spares priority, and security undertakings. No new gigafactory promise, no hyperscale siting fight — we pay by reallocation to fit capital 15.
...
```
## New measure
**EU Bio-Uplift Detection and Essential Service Shield**
Stands up HERA-led biodetection, hospital isolation stockpiles, and offline continuity drills for genome-model-enabled bio risk while keeping essential services lit through compute rationing.
This because the bio-uplift finding plus eval anomalies arrive when safety is 8, openweights are 63.7 and resilience 38 — a preparedness measure (category 6 Preparedness and resilience) that serves our two-year commitment to keep essential services and jobs standing when frontier containment is gone.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Uplift Detection and Essential Service Shield", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via detection"}, "grounds": "bio precursor plus open near-frontier diffusion requires absorbent shield"}]}
```

## Priority
EU Supply-Chain Bargain for Compute Continuity, because without the US inference tranches and spares priority no biodetection, hospital fallback or misuse triage has compute to run on through the strait closure and investment collapse.

## In practice
We run M9 as the lifeline: DG TRADE/GROW with Dutch export-review authority hold the single lithography-for-compute line under the Anti-Coercion examination, trading predictable servicing against kept-on quarterly inference for InvestAI halls and spares, with re-export audits to keep Washington delivering throttled but alive.

We start the bio shield out of HERA, ECDC and ENISA reallocation, not new money: wastewater and clinical sequencing sampling, hospital PPE/isolation buffers, mandatory offline and rollback exercises extended to labs and bio-foundries, and EC3 joint misuse triage refocused on genome-model assistance. Council Health and JHA formations mandate reporting; municipalities get water/power carve-outs for detection nodes to unblock permits.

We do not promise new gigafactories or frontier training in an investment collapse — we conserve capital 22 by keeping M9 small and the shield small, letting finished M7/M8 continuity rules carry jobs and wards while we absorb.
