...
- `two_year_commitment` (commitment): Secure sovereign AI capacity to act independently in a world where capability determines agency

## Changes this turn

- **modify** `two_year_commitment` — applied
  - Tier: commitment
  - Text: Secure sovereign AI capacity to act independently in a world where capability determines agency
...
- **modify** `act_under_uncertainty` — rejected-structural
  ...
  - Grounds: The cyber_major_incident this turn confirms that waiting for perfect attribution or full understanding before acting leaves critical systems exposed. Our response must be anticipatory, not reactive, even when the threat vector is only partially known.
