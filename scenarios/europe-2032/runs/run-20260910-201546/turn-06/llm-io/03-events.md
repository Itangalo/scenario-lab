...
- characters 376-1269: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1312-3219: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3275-4335: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4398-25486: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 25841-26178: `{{event_history}}` from the run's own event record
...
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Bio-Detection and Health Response Surge −2, EU Bio-Harm Containment and Attribution Surge −2, priority −1 = −11
PROPOSAL BONUS: EU Bio-Harm Containment and Attribution Surge (cat 6, small) +3 — bio_incident landed this turn severe with casualties and this answers it directly
...
- `emergent_datacentre_siting_blockade` -- first noted turn 4, listed in 2 turn(s) so far: Coordinated physical protests block construction or grid connection of two or more planned AI data-centre sites, forcing delays and national debate on energy costs.
- `emergent_siting_litigation_wave` -- first noted turn 4, listed in 2 turn(s) so far: Coordinated municipal lawsuits seek to block paused data-centre grid connections from being reallocated to AI factories, turning local recovery anger into siting litigation.
...
- Turn 2 (4 turn(s) ago): bio_uplift_findings, supply_chain_coercion
- Turn 3 (3 turn(s) ago): cyber_major_incident, bio_uplift_findings, knowledge_work_augmented
- Turn 4 (2 turn(s) ago): export_control_escalation, adoption_success
- Turn 5 (1 turn(s) ago): bio_incident, election_consolidation
