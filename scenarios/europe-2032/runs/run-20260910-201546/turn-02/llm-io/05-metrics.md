...
- characters 105-12949: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12992-13184: `{{metrics_json}}` from the run's live metric values
- characters 13277-14467: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14510-16858: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16914-17764: `{{notepad}}` from the Game Master's notepad, carried across turns
...
PROPOSAL BONUS: EU Critical Systems Hardening and Response Pact (cat 6, large) +3 — cyber_test_shot intrusion precursor this turn, directly answered
LEGITIMACY LENDS: capital 39, sentiment 42 -> +3 capped? +1 applied, final 42 — actually: 48-10+3=41, +1 =42
SOVEREIGNTY: 22 last turn, no capacity event, Gigafactories in flight +0, capability rose 1.5 −0 = 22

## Emerging developments (tracked)

- `emergent_screening_norm_push` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher and a national research funder jointly mandate DNA synthesis screening verification for publication and grant disbursement, creating a de facto screening norm beyond voluntary schemes.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 37.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 31.0, "public_sentiment": 34.0}
```
