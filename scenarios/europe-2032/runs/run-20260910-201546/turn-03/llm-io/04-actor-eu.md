...
- characters 1765-3844: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5380-5849: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7498-9583: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M3 | EU Critical Systems Hardening and Response Pact | 6 | large | 1 | 4 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | running |
| M4 | EU Chokepoint Defence and Coalition | 8 | small | 2 | 4 | coalition | eu_ai_sovereignty up slightly, eu_political_capital protected | 2 | running |
...
## New measure
**EU Chokepoint Defence and Coalition**
Creates a coordinated EU response to Washington's use of ASML jurisdiction, pairing Anti-Coercion Instrument preparation with a joint export-control position with Japan and other equipment holders. Why this and why now: the Union's one real bottleneck is being spent by someone else, and if we do not contest its use now we lose both sovereignty leverage and the compute pipeline our resilience commitment depends on.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Chokepoint Defence and Coalition", "category": 8, "size": "small", "finish_turn": 4, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up slightly, eu_political_capital protected"}, "grounds": "contesting extraterritorial use of ASML chokepoint this turn"}]}
```

## Priority
EU Critical Systems Hardening and Response Pact, because the bio-uplift precursor plus the demonstrated grid reachability means absorption capacity in three turns outranks even the chokepoint fight.

## In practice
We keep the Hardening Pact as the main effort: ENISA and DG ENER push OT-monitoring joint procurement to contract, TSOs in the two hit states get funded secondees from the Cybersecurity Reserve, and we extend the Q1-Q2 2027 exercise series to water and port operators while folding the new bio-uplift signal into Health Emergency Preparedness and Response Authority detection work — no new bio instrument yet, just incident-learning inside M3.

On compute we do not open a new programme: DG COMP clears two gigafactory state-aid cases, DG CNECT holds the permitting-zone talks with host regions, and grid-connection queues are arbitrated with TSOs so hardening and build-out stop competing for the same engineers. On coercion we task DG TRADE to open the Anti-Coercion evidence file on servicing bans, Council Foreign Affairs to mandate a joint approach to Tokyo and Seoul on lithography servicing, and The Hague to delay compliance while that coalition forms — defensive, jurisdictional, and cheap in capital.
...
```
## New measure
**EU Bio-Detection and Health Response Surge**
Deploys wastewater genomic sequencing upgrades, hospital syndromic reporting links and HERA stockpile pre-positioning in the five exercise states first, then Union-wide. Why this and why now: the contested genome-model uplift plus a live automated cyber incident means the bio gate is open for the next two years and our resilience commitment requires absorption capacity before the next signal clarifies.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio-Detection and Health Response Surge", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via early warning"}, "grounds": "answering bio uplift precursor within resilience commitment"}]}
```

## Priority
EU Critical Systems Hardening and Response Pact, because the large automated attack now hitting public services outranks even the bio precursor and it is the only instrument already contracted and staffed to absorb it.

## In practice
We surge ENISA and the Cybersecurity Reserve into containment of the ongoing incident: joint OT-monitoring contracts are re-tasked to incident forensics, secondees stay embedded in the hit transmission and public-service operators, and the Council JHA formation authorises cross-border mutual aid for the ransomware/dependency blast radius while attribution is pending. DG ENER arbitrates data-centre grid queues to free engineering hours for recovery, not new connections.

We stand up the Bio Surge inside HERA and ECDC without waiting for a new regulation: using EU4Health money and the M3 exercise network, we fund sequencing kits, hospital reporting APIs and medical countermeasure pre-positioning in the five drilling states, folding the contested genome-model findings into detection thresholds. DG TRADE keeps the Anti-Coercion file and Tokyo/Seoul servicing talks alive defensively, and DG COMP clears gigafactory aid, but no new capital is spent there this turn.
