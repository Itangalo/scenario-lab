...
- characters 455-1386: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1429-3529: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4397-4868: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8049-10206: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Harden essential services and rebuild public trust while keeping sovereign power for European AI
...
| M3 | EU Critical Systems Hardening and Response Pact | 6 | large | 1 | 4 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Chokepoint Defence and Coalition | 8 | small | 2 | 4 | coalition | eu_ai_sovereignty up slightly, eu_political_capital protected | 2 | finished |
| M5 | EU Bio-Detection and Health Response Surge | 6 | small | 3 | 6 | own jurisdiction | resilience up moderately, ai_safety up slightly via early warning | 2 | finished |
| M6 | European Public Results Programme | 9 | small | 4 | 5 | own jurisdiction | public_sentiment up, eu_political_capital up moderately | 2 | finished |
| M7 | EU Bio-Harm Containment and Attribution Surge | 6 | small | 5 | 8 | own jurisdiction | resilience up substantially, ai_safety up slightly | 2 | finished |
| M8 | EU Assurance Uptake and Open-Weight Triage | 5 | small | 6 | 8 | own jurisdiction | ai_safety up moderately, resilience up slightly | 2 | finished |
| M9 | EU Cyber Shield Deployment | 6 | small | 7 | 9 | own jurisdiction | resilience up substantially | 2 | finished |
| M10 | EU Last-Mile Resilience Corps | 6 | small | 9 | 10 | own jurisdiction | resilience up moderately, public_sentiment up slightly | 2 | finished |
| M11 | EU Municipal Staffing and Care Access Guarantee | 9 | small | 10 | 11 | own jurisdiction | public_sentiment up moderately, resilience up slightly, eu_ai_sovereignty up slightly via unblocked power | 2 | finished |
| M12 | EU Automated Ransomware Containment and Recovery Surge | 6 | small | 11 | 12 | own jurisdiction | resilience up moderately, public_sentiment up slightly, ai_safety up slightly via incident learning | 2 | finished |
...
## New measure
**EU Automated Ransomware Containment and Recovery Surge**
Deploys joint EU containment teams, clean backups and emergency patching to the public services hit by the automated attack while keeping hospitals degrading-not-stopping, and routes the new research breakthrough through EU labs and procurement. This is preparedness and resilience work, category 6, and why now is because the major incident demands visible containment before trust collapses further, and the breakthrough gives us a benefit to show if it lands on European terms.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Automated Ransomware Containment and Recovery Surge", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up slightly, ai_safety up slightly via incident learning"}, "grounds": "to contain automated attack and tie breakthrough to recovery"}]}
```

## Priority
M12 EU Automated Ransomware Containment and Recovery Surge, because with capital at 23 and sentiment at 32 the incident will decide whether anything else remains fundable.

## In practice
We activate the Cyber Shield and Last-Mile Corps playbooks under ENISA and CERT-EU with DG DIGIT and HERA: ring-fenced recovery funding from the Resilience envelope, mandatory incident reporting into the common cell, and EU-funded field teams re-deployed to hit municipalities and hospitals first with clean images and detectors. Restoration priority is published daily to end the leverage row, and grid-connection permits for the second factory site are mediated in parallel, not as a condition for recovery.

We attribute via the AI Office and Europol EC3 using the adopted interpretability check on model-generated tooling, and we channel the research breakthrough — materials/compute-relevant work co-produced with EU institutes — into joint procurement and hospital/industrial pilots so cures and recovery arrive together. Council via JHA and EPSCO, mayors and TSOs are the veto points; we offer written staffing top-ups already committed, not new promises.
...
```
## New measure
**EU Clinical Continuity and Sovereign Inference Reserve**
Keeps hospitals, ministries and firms running after the frontier cutoff by switching clinical and essential workloads to EU-controlled inference and open-weight triaged models, and ties the new tailored therapies to EU procurement and production so the medical benefit lands as sovereignty, not dependence. This is sovereignty and industrial capacity work, category 4, and why now is because denial plus breakthrough together decide whether Europe is cut off from both the leading model and the cures it enables.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Clinical Continuity and Sovereign Inference Reserve", "category": 4, "size": "large", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up substantially, resilience up moderately, public_sentiment up moderately"}, "grounds": "to survive frontier cutoff and land medical breakthrough on EU terms"}]}
```
## Priority
M13 EU Clinical Continuity and Sovereign Inference Reserve, because the cutoff is already degrading hospitals and firms and only a visible EU-run fallback restores function and turns the cures into trust.
## In practice
We activate under Articles 122 and 168 via Health and Competitiveness Councils, with HERA, ENISA and EuroHPC JU: ring-fenced reserve inference on EU-anchored gigafactory and EuroHPC capacity, emergency licences and clean model images pushed to hospital IT and ministries, and daily continuity list to stop a new favouritism row. Funding comes from re-directed InvestAI and EU4Health envelopes, with staffing top-ups to communes already promised.

We pair recovery with benefit: joint procurement for the tailored therapies through HERA and EMA pilots in university hospitals in Delft, Grenoble, Munich and beyond, local manufacturing conditions and data-governance terms that keep inference under EU jurisdiction. Suppliers blocking transformers and hyperscalers are offered accelerated permits and procurement in exchange for EU-anchored capacity; mayors and TSOs get mediated grid decisions without making recovery conditional.
