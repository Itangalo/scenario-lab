...
- characters 106-12966: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13009-13201: `{{metrics_json}}` from the run's live metric values
- characters 13294-13979: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14022-15981: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16037-16757: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M11 | EU Shielded Operations Upgrade | 2 |
...
PORTFOLIO CHARGE t9: EU Shielded Operations Upgrade −2, priority −1 = −3
PROPOSAL BONUS: none — knowledge_work_augmented states no displacement crisis to point at, rule 6 bonus does not apply; capability_jump answered by in-flight shield but bonus blocked by political effect
LEGITIMACY LENDS: capital 36, sentiment 27 -> none, capital higher
SOVEREIGNTY: 24 last turn, no capacity event, no cat4 in flight +0, capability rose 1.5 −0 = 24

## Emerging developments (tracked)

- `emergent_datacentre_backlash` -- first noted turn 10, listed in 1 turn(s) so far: Coordinated physical protests and grid-connection lawsuits stall several EU data-centre expansions, forcing a temporary moratorium debate in two member states.
...
## Metrics
```json
{"ai_capability": 66.5, "openweight_capability": 64.0, "ai_safety": 25.0, "resilience": 50.0, "eu_ai_sovereignty": 24.0, "eu_political_capital": 36.0, "public_sentiment": 20.0}
```
