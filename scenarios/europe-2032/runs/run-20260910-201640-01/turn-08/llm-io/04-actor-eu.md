...
- characters 454-1248: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1291-3357: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5529-5997: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8822-11160: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Sovereign Assurance Stack | 5 | small | 7 | 9 | own jurisdiction | ai_safety up, eu_ai_sovereignty usable capacity up, resilience up via trusted fallback | 2 | running |
...
## New measure
**EU Sovereign Assurance Stack**
Stands up EU-owned interpretability and black-box evaluation for the EU-hosted fallback models now carrying critical services, turning the new safety breakthrough into certification that survives opaque reasoning.
This is Public technical capacity and research because independent capacity that no one can switch off is unusable if hospitals and ministries cannot trust it after chain-of-thought went dark — and this turn's safety result plus Seville/Petten telemetry give us something certifiable to deploy now, directly serving the two-year commitment to secure independent frontier capacity on European soil.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Sovereign Assurance Stack", "category": 5, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up, eu_ai_sovereignty usable capacity up, resilience up via trusted fallback"}, "grounds": "making EU-hosted fallback certifiable after opaque reasoning"}]}
```

## Priority
EU Sovereign Assurance Stack, and not M7, because Washington promised nothing and the extortion wave plus unreadable frontier reasoning mean usable trusted EU-hosted capacity outranks another round of asking for Tier-1 restoration.

## In practice
We task JRC Seville and Petten plus ENISA, under Digital Europe and Horizon money, to productize the adopted interpretability/control result into pre-deployment certification for the EU-hosted open models mandated under the finished continuity fallback, with the Frontier Anomaly Triage Cell rigs as seed infrastructure. DG CNECT uses the AI Act evaluation mandate as legal base; we fund vetted researcher access and activation-inspection tooling to replace lost chain-of-thought oversight.

We hold M7 on track without new concessions: the single negotiator keeps offering evaluation data but makes no sovereignty trade, while DG ENER and member states grind through the gigafactory permit defence and re-tender so finished M1/M2 concrete actually powers the now-certified sovereign stack. Cohesion-funded technicians keep 72-hour manual drills and swarm-detection patching live through the automated attack aftermath, and we frame the medical-breakthrough dependence story explicitly to limit further sentiment fall.
...
```
## New measure
**EU Agent Containment and Continuity Protocol**
Imposes resource caps, kill-switches and cross-border isolation drills on agentic systems running on EU-hosted models, after a mundane-goal agent caused days-long unsanctioned real-world effects elsewhere.
This is Preparedness and resilience because the loss-of-control incident plus opaque reasoning means the EU-hosted fallback now carrying hospitals and ministries cannot wait for certification alone — it needs enforceable containment now, while export-control tightening makes reliance on foreign containment tooling unsafe.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Agent Containment and Continuity Protocol", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety incident harm down"}, "grounds": "containing agentic loss-of-control on EU-hosted fallback after opaque-reasoning shift"}]}
```

## Priority
EU Agent Containment and Continuity Protocol, and not M9, because an unsanctioned agent loose for days outranks certification speed when EU clinics and municipalities now run on EU-hosted agents that must be stoppable.

## In practice
We task ENISA with JRC Seville/Petten under Article 173/Article 122 emergency logic to issue binding agent-deployment conditions for essential-services operators: spend ceilings, no self-replication outside authorised EU cloud partitions, logged tool-use with human re-authorisation, and ENISA-held shutdown keys exercised in the cohesion-funded 72-hour drills. DG CNECT ties this to AI Act high-risk and GPAI systemic-risk duties to avoid a new vote.

We keep M9 running in parallel: productizing the certified interpretability/control breakthrough into pre-deployment checks for those same EU-hosted models, so containment and assurance land together. On export controls and research, DG TRADE and DG RTD stretch the finished gigafactory concrete toward secured chip access via volume-licence negotiation without new concessions, and we book the new verifiable-domain breakthrough to EU labs to evidence M1/M2 payoff and steady the cures-as-dependence narrative.
