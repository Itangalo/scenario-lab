...
- characters 456-1757: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1801-3833: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 8347-8823: `{{statement_ledger}}` from the actor's live statement ledger
- characters 12322-14472: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Ensure essential services keep running on controllable EU-hosted AI that no outside power can disrupt
...
| M3 | EU Critical Systems Cyber Shield | 6 | small | 1 | 2 | own jurisdiction | resilience up moderately, ai_safety incident harm down | 2 | finished |
| M4 | EU Incident Containment and Recovery Surge | 6 | small | 2 | 3 | own jurisdiction | resilience up, ai_safety incident harm down | 2 | finished |
| M5 | EU Bio Shield – Screening and Detection Surge | 6 | small | 3 | 4 | own jurisdiction | resilience up, ai_safety incident harm down, bio misuse detection up | 2 | finished |
| M6 | EU Frontier Anomaly Triage Cell | 5 | small | 4 | 5 | own jurisdiction | ai_safety up, eval coverage up | 2 | finished |
| M7 | EU Allied Access and Chokepoint Compact | 8 | small | 5 | 8 | US plus middle-power coalition, own jurisdiction | eu_ai_sovereignty up via secured access and pooled leverage, ai_capability access preserved | 2 | finished |
| M8 | EU Essential Services Continuity and Sovereign Fallback | 6 | small | 6 | 7 | own jurisdiction | resilience up, eu_ai_sovereignty access loss mitigated | 2 | finished |
| M9 | EU Sovereign Assurance Stack | 5 | small | 7 | 9 | own jurisdiction | ai_safety up, eu_ai_sovereignty usable capacity up, resilience up via trusted fallback | 2 | finished |
| M10 | EU Agent Containment and Continuity Protocol | 6 | small | 8 | 9 | own jurisdiction | resilience up, ai_safety incident harm down | 2 | finished |
| M11 | EU Shielded Operations Upgrade | 6 | small | 9 | 11 | own jurisdiction | resilience up, ai_safety incident harm down | 2 | finished |
| M12 | EU Small-Operator Shield Surge | 6 | small | 10 | 12 | own jurisdiction | resilience up, ai_safety incident harm down | 2 | finished |
| M13 | EU Joint Shield Accession | 8 | small | 11 | 13 | US plus middle-power coalition, own jurisdiction | resilience up, ai_safety incident harm down via pooled telemetry and biosurveillance | 2 | finished |
| M14 | EU Unrecallable-Capability Absorption | 6 | small | 12 | 13 | own jurisdiction | resilience up, ai_safety incident harm down despite openweight jump | 2 | finished |
...
## New measure
**EU Unrecallable-Capability Absorption**
Extends the shield to a world where frontier-near kits are permanently on private hardware: default-deny agent permissions, enforced offline backups, and mandatory bio-screening updates for enrolled small operators plus exercised loss-of-control drills.
This is Preparedness and resilience because the open-weight release makes recall impossible, so with essential-services continuity as our two-year commitment the only remaining lever this turn is to make clinics, municipalities and grids degrade-not-stop when those kits are used against them.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Unrecallable-Capability Absorption", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, ai_safety incident harm down despite openweight jump"}, "grounds": "open-weight frontier-near release now unrecallable"}]}
```

## Priority
EU Unrecallable-Capability Absorption, because the open-weight jump outranks even Joint Shield Accession — pooled telemetry only helps if endpoints default-deny autonomous agent actions and can run on paper-plus-backup.

## In practice
We carry it as an implementing push under NIS2 and the Cyber Solidarity Act via ENISA and the Health Emergency Preparedness and Response Authority: Digital Europe and EU4Health reimburse the managed-service providers for pushing default-deny agent profiles, tested offline restore, and updated screening libraries to every enrolled municipality, clinic and energy subcontractor, with premium relief now conditional on accepting all three.

We keep Joint Shield Accession (M13) running as the second track — Council mandates real-time shield telemetry to the joint cyber command and binding sample-sharing to the standing investigation team — while mayors with falling waiting lists get the stage to hold off blockades and court challenges to grid connections. JRC folds the logistics-agent alien methods and the contested genome-design claim into drill thresholds without pausing the controllable EU-hosted fallback.
...
```
## New measure
**EU Sovereign Fallback Activation**
Forces essential services cut off from the leading foreign model onto the controllable EU-hosted stack and rations remaining compute under the Taiwan blockade, with emergency cyber-containment for the automated attack and loss-of-control system.
This and now because access-denied plus blockade plus a major automated incident makes our two-year commitment — essential services keep running on controllable EU-hosted AI no outside power can disrupt — a live test this turn, not a plan; knowledge-work gains do not offset public hostility at 16 and open-weight kits mean the fallback must run default-deny.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Sovereign Fallback Activation", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, eu_ai_sovereignty usable capacity up, ai_safety incident harm down"}, "grounds": "frontier access cut plus Taiwan blockade plus major automated attack"}]}
```

## Priority
EU Sovereign Fallback Activation, because nothing outranks keeping hospitals, ministries and grids running on EU-controlled systems when the foreign frontier is withdrawn and chips are blockaded.

## In practice
We activate it under NIS2 and the Cyber Solidarity Act via ENISA, the EU Health Emergency Preparedness and Response Authority and EuroHPC: Digital Europe emergency funds pay managed-service teams to re-point enrolled clinics, municipalities and energy subcontractors from the denied US model to the Sovereign Assurance Stack, with default-deny agent permissions and offline restores as the condition for reconnection.

We pair that with a Council blockade cell that pools upstream leverage — lithography, grid-connection fast-tracking for the finished Gigafactory sites — to barter maintenance supply and pooled allied compute, while JRC triage folds the loss-of-control alien-cooperation pattern and the verifiable-domain capability jump into containment thresholds. Showcase mayors are stood down; premium relief is now for operators who complete the EU-hosted migration.
