...
- characters 1853-3886: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4782-5241: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7238-9117: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_relay_supply_crunch (emergent event):** Spring mandatory grid checks reveal a shortage of certified relays and control-system engineers, forcing several member states to request derogations and delaying replacement in power and water sites through mid-2027.
...
| M4 | EU Incident Reporting and Software Dependency Registry | 2 | small | 2 | 4 | own jurisdiction | ai_safety up slightly, resilience up slightly via faster learning and scoping | 2 | running |
...
## New measure
**EU Incident Reporting and Software Dependency Registry**
Requires rapid reporting of AI-enabled serious incidents and maps critical software dependencies to scope the blast radius.
Category 2 Transparency and reporting. This now because the major automated attack exposed unknown blast radius while defenders lag, and the Taiwan precursor demands visibility before supply shocks compound it.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Incident Reporting and Software Dependency Registry", "category": 2, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up slightly, resilience up slightly via faster learning and scoping"}, "grounds": "major incident with unknown blast radius demands reporting"}]}
```

## Priority
M3 EU Critical Infrastructure Cyber Shield, not the new registry, because automated disruption of services outranks mapping while attribution lags.

## In practice
We push M3 into emergency mode: ENISA and national CSIRTs execute the spring segmentation audits and cross-border exercises under NIS2, with Digital Europe / CEF co-funding for relay replacement to hold energy ministries, now extended to the services hit in this sweep for isolation and restoration.

We table the reporting registry via DG CNECT under NIS2/CRA bases, tasked to ENISA with a 72-hour serious-incident duty for operators and frontier deployers, plus a dependency inventory for power, ports, water and health IT. Council TTE and JHA formations carry it to keep interior ministries onside.

M1 Gigafactories and M2 tech sovereignty package stay on site-selection and permitting track in Competitiveness Council; on lithography we coordinate export positions and quietly stockpile critical spares as Taiwan insurance premiums rise, without breaking the coalition line.
...
```
## New measure
**EU Relay Surge and OT Engineer Corps**
Jointly procures certified relays and fast-tracks certification and deployment of control-system specialists to clear the grid-check backlog with compensating controls where derogations are unavoidable.
Category 6 Preparedness and resilience. This now because mandatory spring checks exposed a physical bottleneck — missing relays and engineers — that emergency audits and exercises cannot work around, and because eval anomalies mean we must harden while assurance is falling.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Relay Surge and OT Engineer Corps", "category": 6, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially via replacement backlog cleared"}, "grounds": "spring grid checks reveal certified relay and engineer shortage delaying power/water hardening"}]}
```

## Priority
M4 EU Incident Reporting and Software Dependency Registry, not the new surge, because mapping the unknown blast radius exposed by the automated sweep outranks even the relay backlog while attribution lags.

## In practice
We keep M4 on fast legislative track: DG CNECT drafts under NIS2/CRA, ENISA tasked for 72-hour serious-incident intake and dependency inventory for power, ports, water and health IT, pushed through TTE and JHA Councils with interior ministries carrying operators who complain about crisis paperwork by offering standardized templates and co-funded mapping support.

On the relay crunch we launch joint procurement via the Civil Protection Mechanism and Digital Europe/CEF co-funding, with ACER and ENTSO-E coordinating derogations tied to compensating segmentation and monitoring, and accelerated certification of OT engineers through national TSOs and vocational fast-tracks. M1 Gigafactories and M2 tech package stay on permitting track in Competitiveness Council, with quiet chip stockpiling and coordinated export stance as Taiwan insurance costs rise, and local siting opposition handled with water/power mitigation offers to councillors.
