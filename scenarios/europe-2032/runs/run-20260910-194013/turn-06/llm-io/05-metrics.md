...
- characters 105-13670: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13713-13905: `{{metrics_json}}` from the run's live metric values
- characters 13998-15354: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15397-17834: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17890-18428: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Essential Services Continuity and Cross-Border Mutual Aid | 2 |
...
**emergent_technician_licensing_backlash (emergent event):** Fast-tracked OT technician licences trigger coordinated walkouts and safety refusals by transmission-operator unions in two member states, freezing relay installations despite joint procurement.
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 elected consolidation winner takes office turn 6"}]}
```
```
