...
- characters 105-13386: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13429-13621: `{{metrics_json}}` from the run's live metric values
- characters 13714-14993: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15036-17142: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17198-18421: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M10 | EU Essential Services Continuity Stack | 2 |
...
- `emergent_probe_as_a_service` -- first noted turn 7, listed in 2 turn(s) so far: Independent red-team labs publish evidence that self-rewriting intrusion probes are now routinely offered as a paid service on cybercrime forums, collapsing the skill barrier for grid and registry attacks.
- `emergent_us_tiered_access_queue` -- first noted turn 7, listed in 2 turn(s) so far: US tiered-access licensing rules are published and EU-hosted inference of the newest closed models is placed in a delayed review queue, forcing hospitals and ministries built on US APIs onto throttled quotas through autumn.
...
## Notepad
PORTFOLIO CHARGE t8: M9 EU Supply-Chain Shield and Joint Response Accession −2, M10 EU Essential Services Continuity Stack −2, priority M10 −1 = −5
PROPOSAL BONUS: EU Essential Services Continuity Stack (cat 4, small) +4 — eu_frontier_access_denied landed this turn and this answers it directly via sovereign fallback
LEGITIMACY LENDS: capital 21, sentiment 11 -> none, capital is higher
SOVEREIGNTY: 25 last turn, no cat4 finish, M10 in flight +1, eu_frontier_access_denied t8 −2, capability rose 1.0 −0 = 24
