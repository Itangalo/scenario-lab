...
- characters 105-13560: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13603-13795: `{{metrics_json}}` from the run's live metric values
- characters 13888-15333: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15376-17761: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17817-18367: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | European Compute and Talent Anchorage Pact | 2 |
| M7 | EU–Middle Power Supply and Evaluation Compact | 2 |
...
PORTFOLIO CHARGE t5: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Certified Assurance Uptake −2, European Compute and Talent Anchorage Pact −2 = −10 + priority void under capital<20, total −10
PROPOSAL BONUS: European Compute and Talent Anchorage Pact (cat 4, small) +2 — election_retrenchment landed this turn loosening talent/capacity and this answers it directly as anchorage window
LEGITIMACY LENDS: capital 14, sentiment 41 -> +2
...
**emergent_clinic_system_collapse (emergent event):** Municipal hospital ransomware cascade forces several regions to revert to paper triage for weeks, turning uneven patching coverage into a national debate over EU-funded rebuilds.
...
## Metrics
```json
{"ai_capability": 62.3, "openweight_capability": 59.3, "ai_safety": 22.5, "resilience": 42.0, "eu_ai_sovereignty": 26.0, "eu_political_capital": 18.0, "public_sentiment": 33.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 elected retrenchment, takes effect turn 6"}, {"op": "update", "table": "measures", "id": "M7", "fields": {"finish_turn": 8}, "grounds": "named priority"}]}
```
```
