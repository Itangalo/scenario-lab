...
- characters 455-1607: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1651-4012: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5591-6041: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8386-10402: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M6 | European Compute and Talent Anchorage Pact | 4 | small | 5 | 8 | own jurisdiction | eu_ai_sovereignty up moderately | 2 | running |
| M7 | EU–Middle Power Supply and Evaluation Compact | 8 | small | 6 | 8 | coalition | eu_ai_sovereignty up moderately, eu_political_capital up | 2 | running |
...
## New measure
**EU–Middle Power Supply and Evaluation Compact**
It locks export-licence alignment, joint bargaining for compute access and shared evaluation capacity with middle powers holding supply-chain chokepoints into a standing coordination framework.
This now because defection plus the hospital cascade shows the Union cannot hold its line alone while Washington retrenches, and this window for joint leverage that secures access without new spending will close; category 8 International coordination and leverage.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU–Middle Power Supply and Evaluation Compact", "category": 8, "size": "small", "finish_turn": 9, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up moderately, eu_political_capital up"}, "grounds": "to convert US retrenchment window and defection risk into joint leverage"}]}
```

## Priority
EU–Middle Power Supply and Evaluation Compact, because with capital at 16 it is the only move that rebuilds cohesion and secures access while the hospital ransomware cascade and automated attack demand a response we cannot fund alone.

## In practice
We run the emergency through existing tools, not a new programme: the Shield joint cell with ENISA and HERA coordinates hospital triage, segmentation waivers for paper-return regions, and EIB-Digital Europe reflows to funded rebuilds for small clinics, with the defecting capital offered validation inside common procurement terms or outside protected-zone benefits. No new resilience instrument is stood up this turn.

For the commitment we spend diplomacy, not money we lack: Trade and Competitiveness Councils mandate the Commission to sign licence-alignment and joint-procurement terms with Japan, Korea, Singapore and Gulf compute holders, anchoring US-loosened researchers and workloads via the Anchorage Pact chairs and EuroHPC slots, while M1 Gigafactories stays legally alive but unfunded until the Compact restores fiscal room.
...
```
## New measure
**Blockade-Era Upstream Bargain**
It conditions continued EU upstream servicing, spares and export licences on reciprocal compute supply, EU-anchored builds and joint-evaluation recognition from partners.
This now because the Taiwan blockade has made the Union's lithography and equipment position its only hard leverage for compute access, while the automated cyber sweep and capability jump demand sovereign capacity we cannot buy on the open market; category 4 Sovereignty and industrial capacity.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Blockade-Era Upstream Bargain", "category": 4, "size": "small", "finish_turn": 10, "applies_to": "coalition", "targeted_effect": "eu_ai_sovereignty up moderately"}, "grounds": "to convert blockade upstream leverage into secured compute"}]}
```

## Priority
Blockade-Era Upstream Bargain, because with semiconductors blockaded and capital at 18 it is the only move that serves the two-year commitment to un-switch-off-able capacity without new money, outranking even the hospital rebuild.

## In practice
We run the cyber response through the finished Shield and its joint cell with ENISA and HERA: signatures, segmentation waivers and EIB reflows to paper-triage regions, with certified public-sector AI wins — falling waiting lists where adoption held — publicised by DG SANTE and DG DIGIT to hold sentiment. The defecting capital is offered re-entry inside common terms or stays outside protected-zone benefits; no new resilience line is opened.

For sovereignty we task the Trade and Competitiveness Councils under the Anti-Coercion Instrument logic to mandate the Commission to tie upstream licences to Japan-Korea-Singapore-Gulf pooled procurement from the M7 Compact, while DG RTD and EuroHPC use M6 Anchorage chairs and slots to land displaced talent and workloads. Gigafactories stay legally alive, unfunded builds are sequenced behind secured supply.
