...
- characters 106-12990: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13033-13225: `{{metrics_json}}` from the run's live metric values
- characters 13341-16290: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 16346-16952: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_research_integrity_crisis` -- first noted turn 1, listed in 1 turn(s) so far: A major European scientific publisher retracts a batch of papers after discovering LLM-generated citations and fabricated data slipped through peer review, triggering university-level bans on AI-assisted submissions.
- `emergent_provenance_mandate_push` -- first noted turn 1, listed in 1 turn(s) so far: Major EU research universities and publishers jointly announce machine-readable provenance mandates for AI-generated research outputs after the Millennium Prize claim dispute.
...
## Metrics
```json
{"ai_capability": 54.0, "openweight_capability": 46.0, "ai_safety": 32.0, "resilience": 38.0, "eu_ai_sovereignty": 24.0, "eu_political_capital": 44.0, "public_sentiment": 47.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M3", "fields": {"finish_turn": 3}, "grounds": "named priority"}]}
```
```
