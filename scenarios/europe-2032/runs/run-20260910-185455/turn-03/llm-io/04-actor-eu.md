...
- characters 455-1747: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1791-4079: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5149-5622: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7229-9393: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M3 | EU Critical Infrastructure Shield | 6 | large | 1 | 5 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | running |
| M4 | EU Breakthrough Defence Deployment | 6 | small | 2 | 4 | own jurisdiction | resilience up substantially | 2 | running |
...
## New measure
**EU Breakthrough Defence Deployment**
Pushes the new automated patching and swarm-behaviour detection tooling into the grid, ports, water and public-service systems covered by the Shield, with EU-funded licences and joint deployment teams.
This is Preparedness and resilience work because the major incident shows offence still outruns defence, but the concurrent defensive breakthrough gives us a closable class of attack to close now, directly serving the shock-absorbing commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Breakthrough Defence Deployment", "category": 6, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially"}, "grounds": "exploiting defensive breakthrough after major automated attack"}]}
```

## Priority
EU Critical Infrastructure Shield, because the large automated attack landing on staged access demands exercised containment and 24-hour reporting hold before the new tooling can be absorbed, outranking even its own deployment vehicle this turn.

## In practice
We keep the Shield as the operating spine: ENISA's joint cell moves to daily staging reports under the NIS2 emergency base, Energy and Home Affairs Councils enforce OT segmentation deadlines, and Digital Europe co-financing is re-weighted to operators hit or probed in the sweep, with missed-report penalties waived if teams join cross-border exercises this spring. Municipal staffing gaps are filled by seconded TSO cyber teams.

Through that same cell we procure and push the breakthrough defensive stack — automated patch-at-speed and swarm-detection — via joint procurement, starting with transmission operators and large hospitals and public administrations caught in the ransomware/dependency sweep. We pause new Gigafactory site decisions after the investment collapse, protecting EIB guarantees and permitting zones already opened, and use the lithography-linked access reprieve to demand vendor cooperation on patch feeds and evaluation access while offices' productivity gains keep public tolerance for disruption spending alive.
...
```
## New measure
None this turn, while we hold the Shield through the sweep and force the defection back inside common terms before spending capital we do not have.

## Store changes
No changes.

## Priority
EU Critical Infrastructure Shield, because containing the automated sweep across municipalities and grid networks is the precondition for the shock-absorbing commitment, outranking even the breakthrough tooling and the sovereignty fallout this turn.

## In practice
We run everything through the joint ENISA cell on daily staging: Energy and JHA Councils hold OT segmentation deadlines, Digital Europe and EIB co-financing stays re-weighted to hit smaller utilities and hospitals, and seconded TSO teams plus joint-procured licences push the machine-speed patching and swarm detection first to transmission and large hospitals, with municipal legacy installs funded as emergency procurement.

On the defection we contain, not escalate: Commission opens infringement screening and Anti-Coercion assessment of the bilateral hyperscaler/state deal, offers the defecting capital the protected permitting zones and guarantee umbrella if it re-aligns procurement to EU evaluation-access and data-residency terms, and we freeze new Gigafactory site decisions, protecting opened zones while using lithography leverage to extract patch feeds. Juniors-productivity evidence is used with social partners to sustain tolerance for disruption spending.
