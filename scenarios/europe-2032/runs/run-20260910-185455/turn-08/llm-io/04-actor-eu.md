...
- characters 454-1537: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1580-3819: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4176-4626: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7092-9060: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | Blockade-Era Upstream Bargain | 4 | small | 7 | 9 | coalition | eu_ai_sovereignty up moderately | 2 | running |
...
```
## New measure
None this turn, while we wait for M8 ratification to land and for political capital to recover above paralysis.

## Store changes
No changes.

## Priority
M8, because converting lithography servicing leverage into reciprocal compute and anchored builds is the only path to un-switch-off-able capacity that costs no new money while capital sits at 12.

## In practice
We run everything through the Trade Council mandate under Anti-Coercion logic: DG TRADE conditions servicing licences and spares on pooled compute and EU-anchored builds with Japan, Korea and Singapore, with the Gulf kept as intent only until domestic demand eases. DG RTD and EuroHPC direct M6 chairs and slots to land US-leaving talent against those secured queues.

For resilience we work the finished Shield only — ENISA joint cell pushes segmentation and clean-component swaps to paper-triage municipalities, EIB reflows cover emergency rebuild, and DG SANTE tours the certified clinics where waits fell to hold sentiment at 27. The separate-deal capital is offered re-entry on common terms or remains outside protected-zone support; no parallel instrument is opened.
