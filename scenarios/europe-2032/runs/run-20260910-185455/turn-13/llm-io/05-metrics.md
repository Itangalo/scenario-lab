...
- characters 107-13315: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13358-13550: `{{metrics_json}}` from the run's live metric values
- characters 13643-15430: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15474-17485: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17541-18250: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M13 | EU Anchored Industrial Inference Stack | 2 |
...
- `emergent_insurance_coverage_freeze` -- first noted turn 13, listed in 1 turn(s) so far: Major European cyber-insurers suspend ransomware coverage for municipalities and hospitals after the spring sweep, freezing renewal negotiations and forcing emergency state backstops.
...
## Notepad
PORTFOLIO CHARGE t13: EU Anchored Industrial Inference Stack −2, priority −1 = −3
PROPOSAL BONUS: EU Anchored Industrial Inference Stack (cat 4, small) +2 — embodied_ai_deployment landed this turn and this answers it directly
LEGITIMACY LENDS: capital 20, sentiment 24 -> +2
SOVEREIGNTY: 20 last turn, no capacity event, no cat4 finishes, embodied_ai_deployment t13 −2, capability rose 0.8 −0 = 18
## Store changes
```json
{"store": []}
```
