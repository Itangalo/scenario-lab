...
- characters 456-2243: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2287-4298: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6144-6593: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9525-11574: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Keep independent European AI capacity that no outside power can switch off
...
| M4 | EU Breakthrough Defence Deployment | 6 | small | 2 | 4 | own jurisdiction | resilience up substantially | 2 | finished |
| M5 | EU Certified Assurance Uptake | 5 | small | 4 | 6 | own jurisdiction | ai_safety up moderately | 2 | finished |
| M6 | European Compute and Talent Anchorage Pact | 4 | small | 5 | 8 | own jurisdiction | eu_ai_sovereignty up moderately | 2 | finished |
| M7 | EU–Middle Power Supply and Evaluation Compact | 8 | small | 6 | 8 | coalition | eu_ai_sovereignty up moderately, eu_political_capital up | 2 | finished |
| M8 | Blockade-Era Upstream Bargain | 4 | small | 7 | 9 | coalition | eu_ai_sovereignty up moderately | 2 | finished |
| M9 | EU Trusted Medical AI Deployment | 9 | small | 9 | 10 | own jurisdiction | public_sentiment up sharply, resilience up slightly | 2 | finished |
| M10 | EU Assurance Uptake for Deployed Frontier Systems | 5 | small | 10 | 11 | own jurisdiction | ai_safety up substantially | 2 | finished |
| M11 | EU Cutoff Continuity Reserve | 6 | small | 11 | 12 | own jurisdiction | resilience up moderately, eu_political_capital up slightly | 2 | finished |
| M12 | EU Cyber Recovery and Hardening Surge | 6 | small | 12 | 13 | own jurisdiction | resilience up moderately, eu_political_capital up slightly | 2 | finished |
...
## New measure
**EU Cyber Recovery and Hardening Surge**
It restores hit public services from the automated ransomware/dependency attack via segmented rebuilds and EU-anchored clean images, then hardens hospitals, ministries and logistics operators against model-generated tooling. This and now because the major cyber incident landed visibly while open-weight frontier diffusion makes re-infection unrecallable, and with capital at 21 only a small resilience surge can hold continuity without breaking the un-switch-off-able capacity commitment.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cyber Recovery and Hardening Surge", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, eu_political_capital up slightly"}, "grounds": "to absorb the major automated attack and stop cascade while sustaining cutoff continuity"}]}
```
## Priority
EU Cyber Recovery and Hardening Surge, because the public cyber failure outranks even sovereignty building now — without restored services the continuity reserve and the two-year commitment lose credibility visibly.
## In practice
We act through ENISA and the Cyber Solidarity Mechanism with Civil Protection mutual aid: Health and Telecom Councils mandate segmented rebuilds from EuroHPC-hosted clean backups, CERT-EU triages the compromised dependency with unknown blast radius, and the continuity reserve's older queues carry clinical workloads during rebuild. No new appropriation; we reallocate Digital Europe cyber funds and press insurers to cover extended stays.

We contain the member-state side-deal by naming it pragmatism we will out-deliver: offer fallback inference and hardened hosting to its hospitals and operators on the same terms, while freezing the lithography mandate and keeping ECDC bio-screening quiet. Ministers keep pointing to falling waiting lists in European-triage hospitals to rebuild capital, resisting an unenforceable open-weight ban at safety 13.
...
```
## New measure
**EU Anchored Industrial Inference Stack**
It bolts industrial and embodied-AI inference to EU-anchored compute and procurement terms so factories and logistics can use foreign robots/models without surrendering switch-off power. This and now because opaque reasoning kills oversight while Chinese/US embodied systems arrive on the industrial base we still lead in, and with capital at 22 only a small sovereignty-anchored adoption can hold the un-switch-off-able capacity commitment without breaking continuity.
## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Anchored Industrial Inference Stack", "category": 4, "size": "small", "finish_turn": 16, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up moderately, resilience up slightly"}, "grounds": "to keep embodied deployment from becoming new dependence while sustaining independent capacity"}]}
```
## Priority
EU Anchored Industrial Inference Stack, because it is the only live measure serving the two-year commitment to keep un-switch-off-able capacity after the side-deal defection.
## In practice
We act through DG GROW and DG CNECT with EuroHPC JU: Competitiveness Council mandates anchored inference and hardened hosting for publicly procured robotics/logistics AI, funded by reallocating Gigafactory follow-on and Digital Europe money, no new appropriation. The defecting capital gets the same fallback offer for its hospitals/logistics hubs if it returns to common procurement conditions.

We absorb opaque reasoning by shifting assurance to black-box evaluation and activation inspection via ENISA and the AI Office, keeping ECDC synthesis screening quiet. Ministers keep pointing to segmented rebuilds and falling waiting lists where European triage runs to contain insurers, unions and pickets.
