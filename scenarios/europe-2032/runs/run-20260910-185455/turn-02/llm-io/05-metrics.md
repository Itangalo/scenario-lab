...
- characters 105-13038: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13081-13273: `{{metrics_json}}` from the run's live metric values
- characters 13366-14894: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14937-17085: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17141-18369: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M3 | EU Critical Infrastructure Shield | 3 |
| M4 | EU Breakthrough Defence Deployment | 2 |
...
PROPOSAL BONUS: EU Critical Infrastructure Shield (cat 6, large) +4 — cyber_test_shot precursor intrusion staged access in two member states last turn and this answers it directly
LEGITIMACY LENDS: capital 42, sentiment 47 -> +2
SOVEREIGNTY: 22 last turn, eu_access_secured t1 +2, Gigafactories in flight +1, capability rose 2.0 −1 = 24
Emerging: emergent_research_integrity_crisis faint rumour of retractions; emergent_provenance_mandate_push single-source push for provenance mandates.

## Emerging developments (tracked)

- `emergent_research_integrity_crisis` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher retracts a batch of papers after discovering LLM-generated citations and fabricated data slipped through peer review, triggering university-level bans on AI-assisted submissions.
- `emergent_provenance_mandate_push` -- first noted turn 1, listed in 2 turn(s) so far: Major EU research universities and publishers jointly announce machine-readable provenance mandates for AI-generated research outputs after the Millennium Prize claim dispute.
