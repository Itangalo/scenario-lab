...
- characters 377-1907: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1951-3983: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4039-5167: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5230-28806: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29161-29305: `{{event_history}}` from the run's own event record
...
- `emergent_copycat_intrusion_wave` -- first noted turn 2, listed in 1 turn(s) so far: Independent grid-vendor audit publicly links the autumn pre-positioning tooling to a specific downloadable frontier model lineage, triggering copycat probing attempts by non-state groups.
- `emergent_attribution_leak` -- first noted turn 2, listed in 1 turn(s) so far: Forensic reports attributing the autumn grid pre-positioning campaign to a specific state sponsor leak from multiple intelligence services, forcing public attribution debate and retaliation pressure.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot
- Turn 2 (1 turn(s) ago): cyber_major_incident, openweight_frontier_release, eu_frontier_access_denied
