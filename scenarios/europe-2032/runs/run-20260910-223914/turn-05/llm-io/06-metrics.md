...
- characters 13895-15597: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15641-17606: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17662-18728: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | Joint Resilience and Attribution Pool | 2 |
...
- `emergent_insured_airbridge_chips` -- first noted turn 5, listed in 1 turn(s) so far: Gulf and Asian insurers create a state-backed war-risk pool that restores limited air-freight chip shipments from Taiwan at 5-8x premiums, easing but not ending the blockade shortage.
- `emergent_air_freight_insurance_freeze` -- first noted turn 5, listed in 1 turn(s) so far: Major insurers extend war-risk exclusion to air freight of high-end accelerators and lithography spares, grounding fallback compute shipments for weeks.
...
## Metrics
```json
{"ai_capability": 67.0, "openweight_capability": 60.9, "ai_safety": 17.0, "resilience": 48.0, "eu_ai_sovereignty": 13.0, "eu_political_capital": 12.0, "public_sentiment": 21.0}
```
## Narrative
