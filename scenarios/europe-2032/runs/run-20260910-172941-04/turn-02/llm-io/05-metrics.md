...
- characters 105-13034: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13077-13269: `{{metrics_json}}` from the run's live metric values
- characters 13362-14536: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14579-16398: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16454-17744: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Public Service AI Replication Fund | 2 |
...
- `emergent_datacentre_permit_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated local opposition delays or blocks several planned EU data-centre and grid-connection permits in late 2026 over power prices and water use, without any EU-level decision causing it.
- `emergent_bio_norm_pause` -- first noted turn 1, listed in 2 turn(s) so far: Major European research universities jointly announce a voluntary moratorium on open publication of genome-language-model weights above phage-level capability, creating a de facto screening norm ahead of regulation.
...
## Metrics
```json
{"ai_capability": 56.0, "openweight_capability": 50.0, "ai_safety": 30.0, "resilience": 36.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 28.0, "public_sentiment": 42.0}
```
## Narrative
...
## Notepad
PORTFOLIO CHARGE t2: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M3 EU Critical Systems Shield −3, M4 EU Public Service AI Replication Fund −2, priority M3 −1 = −12
PROPOSAL BONUS: none — new Replication Fund answers positive adoption window, not a negative event from last three rounds
