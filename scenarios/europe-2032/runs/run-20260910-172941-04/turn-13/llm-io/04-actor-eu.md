...
- characters 456-1425: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1469-3526: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5344-5825: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9149-11309: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Clinical and Essential Services Continuity Reserve | 6 | small | 12 | 14 | own jurisdiction | resilience up, eu_political_capital down slightly less via avoided collapse | 2 | running |
...
| M10 | EU Trustworthy Care Rollout | 9 | small | 11 | 13 | own jurisdiction | public_sentiment up, eu_political_capital up slightly via visible health benefit | 2 | finished |
...
```
## New measure
None this turn, while political capital is at 17 and public sentiment at 16 and we cannot fund a second track without killing the Reserve that is keeping hospitals degraded-not-stopped.

## Store changes
No changes.

## Priority
M11 EU Clinical and Essential Services Continuity Reserve, because with cutoff plus automated attack fused and gigafactories still empty shells, holding continuity is the only thing that prevents Trustworthy Care gains collapsing into lasting distrust.

## In practice
We execute the Reserve through HERA, ENISA and the joint cyber command feed: DG SANTE mandates EU-hosted substitute inference and paper-manual fallback for cut-off hospital and ministry workloads, funded from EU4Health and Digital Europe, with freeze-and-isolate drills extended to clinics, ports and water operators. AI Office evaluators are triaged to restoration first, certification queues second.

We keep M10 Trustworthy Care Rollout as the attribution anchor — joint procurement switched mid-contract to auditable substitutes, pooled municipal self-insurance held via ECOFIN — and we tell Health and JHA Councils plainly that sovereignty cannot answer this semester, continuity must. The Commission frames the foreign cutoff as proof EU audit/hosting was foresight, while conceding concrete-without-machines and offering mayors restoration teams, not promises.
