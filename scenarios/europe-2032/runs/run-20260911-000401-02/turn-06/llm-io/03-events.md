...
- characters 376-1745: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1788-3937: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3993-5172: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5235-27217: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27572-27889: `{{event_history}}` from the run's own event record
...
- `emergent_insurer_certificate_mandate` -- first noted turn 5, listed in 1 turn(s) so far: Major EU insurers move to make municipal cyber and business-interruption cover conditional on the new ENISA pass certificate, triggering a scramble for re-tests and a public fight over premiums in smaller cities.
- `emergent_insurer_conditional_cover` -- first noted turn 5, listed in 1 turn(s) so far: Major EU insurers move from threats to action, making cyber cover for hospitals and municipal utilities conditional on ENISA pass certificates, triggering a scramble for certification and political backlash over uncovered operators.
...
- Turn 3 (3 turn(s) ago): adoption_success, emergent_gigafactory_siting_backlash
- Turn 4 (2 turn(s) ago): openweight_frontier_release, knowledge_work_augmented, taiwan_tension_rise
- Turn 5 (1 turn(s) ago): eval_anomaly_reports, member_state_defection, election_retrenchment
