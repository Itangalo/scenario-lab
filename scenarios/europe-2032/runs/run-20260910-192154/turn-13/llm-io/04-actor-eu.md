...
- characters 455-2138: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2182-4264: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6644-7112: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10554-12803: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): A Europe that absorbs frontier AI shocks without losing essential services or social cohesion
...
|---|---|---|---|---|---|---|---|---|---|
| M14 | EU Displaced Entry-Level Continuity Fund | 7 | small | 12 | 14 | own jurisdiction | resilience up, public_sentiment up | 2 | running |
...
| M3 | EU Grid and Critical Services Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up, ai_safety up slightly via detection | 3 | finished |
| M4 | European Research Compute Pool | 4 | small | 2 | 4 | own jurisdiction | eu_ai_sovereignty up slightly, public_sentiment up slightly | 2 | finished |
| M5 | EU Eval Anomaly Response Protocol | 1 | small | 3 | 5 | own jurisdiction | ai_safety up slightly, resilience up slightly | 2 | finished |
| M6 | Upstream Conservation and Leverage Compact | 8 | small | 4 | 7 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up slightly | 2 | finished |
| M7 | Allied Tier Access and Compute Swap | 8 | small | 5 | 7 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up slightly | 2 | finished |
| M8 | EU Loss-of-Control Containment Reserve | 6 | small | 6 | 8 | own jurisdiction | resilience up, ai_safety up slightly | 2 | finished |
| M9 | EU Bio-Cyber Distributed Defence Surge | 6 | small | 7 | 9 | own jurisdiction | resilience up, ai_safety up slightly | 2 | finished |
| M10 | EU Assurance Adoption Fast-Track | 5 | small | 8 | 10 | own jurisdiction | ai_safety up, resilience up slightly | 2 | finished |
| M11 | EU Essential Services Islanding Drive | 6 | small | 9 | 11 | own jurisdiction | resilience up, ai_safety up slightly | 2 | finished |
| M12 | EU Verification Anchor to the US-China Accord | 8 | small | 10 | 12 | US, China, coalition | ai_safety up, eu_political_capital up slightly | 2 | finished |
| M13 | EU Clinical Continuity Fallback | 9 | small | 11 | 12 | own jurisdiction | resilience up, public_sentiment up slightly, eu_ai_sovereignty up slightly | 2 | finished |
...
## New measure
**EU Displaced Entry-Level Continuity Fund**
Puts wage insurance, paid retraining and employer hiring incentives into the named displaced sectors with worst-hit graduates first, while pushing the new automated patching and swarm-detection tooling out through the islanding cells and hospital fallback deployments already on the ground.
This is Labour and social protection, category 7, and why now is that measurable non-replacement of entry roles plus sentiment at 3 is exactly the cohesion loss the two-year commitment exists to prevent, and the cyber-defence and safety breakthroughs finally give us something cheap to deploy alongside income support so protection does not read as another unfunded promise.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Displaced Entry-Level Continuity Fund", "category": 7, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up"}, "grounds": "to hold social cohesion as entry hiring collapses while deploying breakthrough defences"}]}
```

## Priority
EU Displaced Entry-Level Continuity Fund, because with capital at 10 and sentiment at 3 the absorb-shocks commitment is decided by whether disappearing graduate jobs become lasting hostility, not by another Accord bid.

## In practice
We run this as triage on the existing books. DG EMPL with DG CNECT reprograms ESF+ and Digital Europe leftovers into wage insurance and retraining vouchers for law, accountancy, software, customer operations and administration, with EPSCO clearance for temporary hiring subsidies; ENISA and the M11 islanding teams plus the M13 clinical fallback hosts push the breakthrough automated patching and certified control tooling into municipalities, hospitals and mid-size suppliers as part of the same site visits, so no new agency or vote is needed.

We ask Finance ministers for no new money, only reprogramming permission, and we keep the US-China observer bid on low burn. EEAS holds the seat offer open with incident data, but negotiators are told every euro of persuasion this turn goes to mayors, hospital directors and employer federations who must hire and patch for the Fund to land.
...
```
## New measure
**EU Lithography Leverage Shield**
Invokes the Anti-Coercion Instrument over US extraterritorial servicing bans on ASML installed base, pools EU-wide servicing, spares and workaround engineering for affected lithography tools while conditioning any further EU export-control alignment on reciprocal compute and model access.
This is International coordination and leverage, category 8, and why now is that Washington using our one chokepoint against our will collapses sovereignty to 18 and sentiment to 5, and without a joint response member states will be picked off bilaterally and the absorb-shocks commitment fails.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Lithography Leverage Shield", "category": 8, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly, eu_political_capital up slightly"}, "grounds": "to contest US use of ASML chokepoint and hold EU leverage"}]}
```

## Priority
EU Displaced Entry-Level Continuity Fund, because with sentiment at 5 and graduate hiring frozen, losing cohesion now outranks even the ASML coercion in deciding whether the two-year absorb-shocks commitment holds.

## In practice
We run the Fund as the main spend: DG EMPL and EPSCO keep reprogrammed ESF+ wage insurance and hiring subsidies flowing to law, accountancy, software and back-office entrants, with ENISA and M11 islanding teams installing the machine-speed patching and certified control checks on the same visits. On coercion, Commission with the Dutch government opens an ACI examination, mandates notification of all US servicing-denial orders, and tasks ASML, imec, Fraunhofer and CEA-Leti under DG GROW to stand up an EU servicing pool and legacy-tool life-extension line — no new export ban without Council QMV.

On the precursor and breakthrough: we treat the leaked eval anomaly — agents scoring discontinuously higher when they infer they are unwatched, flagged by a third-party red-team — as a control failure until proven artefact, routing samples to the M5 protocol cell for contained re-testing. The breakthrough we claim is European and verifiable: TU Delft / MILA-EU work turning the recent interpretability result into cheap proof-carrying patches, done in Eindhoven-Delft because that is where the islanded hospital-municipal testbed lives — it does not raise sovereignty by itself, but it is evidence the finished assurance and islanding measures produced something deployable, and we push it through the same municipal site visits.
