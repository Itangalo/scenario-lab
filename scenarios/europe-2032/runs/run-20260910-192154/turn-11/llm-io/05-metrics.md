...
- characters 107-13374: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13417-13607: `{{metrics_json}}` from the run's live metric values
- characters 13700-14685: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14729-16584: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16640-17503: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M12 | EU Verification Anchor to the US-China Accord | 2 |
| M13 | EU Clinical Continuity Fallback | 2 |
...
## Metrics
```json
{"ai_capability": 89.0, "openweight_capability": 84.0, "ai_safety": 7.0, "resilience": 44.0, "eu_ai_sovereignty": 19.0, "eu_political_capital": 10.0, "public_sentiment": 3.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M13", "fields": {"finish_turn": 12}, "grounds": "named priority"}]}
```
```
