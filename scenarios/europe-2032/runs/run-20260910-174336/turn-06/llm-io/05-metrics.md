...
- characters 13626-13817: `{{metrics_json}}` from the run's live metric values
- characters 13910-15164: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15207-16888: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16944-17369: `{{notepad}}` from the Game Master's notepad, carried across turns
...
## Metrics
```json
{"ai_capability": 60.5, "openweight_capability": 56.9, "ai_safety": 23.0, "resilience": 40.0, "eu_ai_sovereignty": 24.0, "eu_political_capital": 9.0, "public_sentiment": 24.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner for tiered access takes office turn 6"}]}
```
```
