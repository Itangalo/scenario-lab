...
- characters 105-13021: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13064-13256: `{{metrics_json}}` from the run's live metric values
- characters 13349-14770: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14813-16848: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16904-18081: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M5 | EU Model Evaluation and Assurance Office | 2 |
| M6 | EU AI Incident and Dependency Registry | 2 |
...
PORTFOLIO CHARGE t3: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Essential Services Shield and Cross-Border Contingency Pact −3, EU Model Evaluation and Assurance Office −2, priority −1 = −12
PROPOSAL BONUS: EU Model Evaluation and Assurance Office (cat 5, small) +2 — benefits fraud-risk scandal and opaque frontier reasoning in last two turns and this answers enforcement gap directly
LEGITIMACY LENDS: capital 26, sentiment 35 -> +2
SOVEREIGNTY: 22 last turn, no capacity event, Gigafactories in flight +0, capability rose 1.5 −0 = 22

## Emerging developments (tracked)

- `emergent_local_permit_freeze` -- first noted turn 4, listed in 1 turn(s) so far: Several municipal councils in stalled gigafactory regions impose temporary moratoria on new data-centre grid connections after protests over outages and energy costs, freezing site permitting through the spring.
- `emergent_heatwave_grid_crunch` -- first noted turn 4, listed in 1 turn(s) so far: A severe early-summer heatwave forces grid curtailments in southern Europe, pitting stalled gigafactory grid reservations and data-centre power demand against hospital cooling and public anger over outages.
...
## Metrics
```json
{"ai_capability": 57.5, "openweight_capability": 52.2, "ai_safety": 26.0, "resilience": 38.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 14.0, "public_sentiment": 25.0}
```
