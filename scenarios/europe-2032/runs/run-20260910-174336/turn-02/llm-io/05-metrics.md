...
- characters 105-13319: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13362-13554: `{{metrics_json}}` from the run's live metric values
- characters 13647-14754: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14797-16975: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17031-18249: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Essential Services Shield and Cross-Border Contingency Pact | 3 |
...
PORTFOLIO CHARGE t1: Gigafactories −3, Tech sovereignty package −3, Resilience Surge −2, priority −1 = −9
PROPOSAL BONUS: EU Grid-Port-Bio Detection and Hardening Sprint (cat 6, small) +2 — autumn cross-continent intrusion landed this turn and bio precursor opened gate, this answers both directly
LEGITIMACY LENDS: capital 40, sentiment 45 -> +1
SOVEREIGNTY: 22 last turn, no capacity event, Gigafactories in flight +0, capability rose 1.5 −0 = 22
Emerging developments (tracked): synthesis screening pact rumour faint; EU open frontier pooled-compute talk faint single-source.

## Emerging developments (tracked)

- `emergent_synthesis_screening_pact` -- first noted turn 1, listed in 2 turn(s) so far: Major DNA synthesis providers announce a voluntary cross-industry screening pact with shared deny-lists for AI-designed pathogen sequences after the phage results, ahead of any regulation.
- `emergent_eu_open_frontier_push` -- first noted turn 1, listed in 2 turn(s) so far: Major European research universities and open-source collectives announce a joint pooled-compute training run for a frontier-class open model to be built entirely on EU infrastructure, drawing political attention and US supplier scrutiny.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 40.5, "eu_ai_sovereignty": 22.0, "eu_political_capital": 34.0, "public_sentiment": 35.0}
```
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Essential Services Shield and Cross-Border Contingency Pact −3, priority −1 = −10
PROPOSAL BONUS: EU Essential Services Shield and Cross-Border Contingency Pact (cat 6, large) +4 — automated attack landed this turn and this answers it directly
LEGITIMACY LENDS: capital 33, sentiment 35 -> +1
SOVEREIGNTY: 22 last turn, no capacity event, Gigafactories in flight +0, capability rose 1.5 −0 = 22
## Store changes
```json
