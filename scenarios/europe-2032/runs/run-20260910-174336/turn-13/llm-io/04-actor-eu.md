...
- characters 455-2388: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2432-4509: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4876-5369: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8375-10123: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M10 | EU Visible Benefit Adoption Scale-Up | 9 | small | 10 | 12 | own jurisdiction | public_sentiment up moderately, eu_political_capital up slightly, resilience up slightly | 2 | finished |
...
## New measure
None this turn, while political capital sits at 4.0 and public sentiment at 13.0 with no in-flight vehicle to carry new costs.

We are waiting for the safety-breakthrough adoption and the stabilised employment picture to restore a minimum of consent before tabling any new funded instrument that would fail on contact.

## Store changes
No changes.

## Priority
No priority named this turn, because the portfolio holds no in-flight measure to push and naming a finished line as priority would misdirect what little capital remains.

## In practice
We hold the line on what is already built rather than starting anything new. DG CNECT and the EU Model Evaluation and Assurance Office are instructed to take the interpretability/control result that the labs already want and apply it to the two audited health-administration models still running under EU hosting, publishing the certification rather than legislating it. ENISA sustains the paper-fallback and generator routines that held during the autumn sweep under existing CER and civil-protection powers, with no new hardening money tabled.

We use the ordinary-offices evidence — productivity up, employment not down — as the consent story the two-year commitment needs: Commission filming stays on clinics where waits fell and on offices where junior staff kept jobs, Interior Council continues emergency policing escorts for blocked builds in Spain, Netherlands and France under existing public-order coordination, and DG EMPL lets the finished voucher and wage-insurance lines pay out where job centres still process them. Where the research breakthrough lands — US or Chinese labs first — we do not chase it; we note it for procurement later, once hosting is accepted again.
...
```
## New measure
**EU Clinic Queue Replication Pact**
It funds replication of the two certified health-administration models to willing regions with EU hosting and certification, paired with visible queue-time publication.
This is category 9 Diffusion, adoption and public trust, and now is the moment because the adoption_success event finally attributes falling waits to a European decision, giving the two-year commitment to rebuild consent something concrete to scale while capital is at 9.0.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Clinic Queue Replication Pact", "category": 9, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, eu_political_capital up slightly, resilience up slightly"}, "grounds": "to scale certified benefit while consent window open"}]}
```

## Priority
No priority named this turn, because with political capital at 9.0 naming a new in-flight measure as priority before it has entered would overcommit what little can be spent.

## In practice
We hold to the two-year commitment to rebuild consent by keeping services running. DG SANTE and DG CNECT, via the EU Model Evaluation and Assurance Office, extend the existing certificates to new volunteer hospitals where EU hosting is accepted, with no new law and no new mandate, publishing wait-time deltas clinic by clinic.

Interior Council continues existing public-order escorts for builds in Spain, Netherlands and France, and DG EMPL lets finished voucher and wage-insurance lines pay where job centres function. Commission communication stays on filmed queues and retained junior jobs, not on new promises, to let the adoption success land before we spend further.
