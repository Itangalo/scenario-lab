...
- characters 107-12820: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12863-13054: `{{metrics_json}}` from the run's live metric values
- characters 13147-15080: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15124-17201: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17257-18205: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M11 | EU Clinic Queue Replication Pact | 2 |
...
- `emergent_utility_walkout` -- first noted turn 12, listed in 2 turn(s) so far: Coordinated municipal utility operators in two member states stage wildcat walkouts after automated cyberattack overtime and data-centre police escort duties, briefly halting grid maintenance crews.
- `emergent_substation_sabotage_blackouts` -- first noted turn 12, listed in 2 turn(s) so far: Anti data-centre blockades escalate into coordinated sabotage of grid substations feeding construction sites, causing localized blackouts blamed on the protest movement.
...
## Notepad
PORTFOLIO CHARGE t13: EU Clinic Queue Replication Pact −2 = −2
PROPOSAL BONUS: none — new Clinic Queue Replication Pact answers adoption_success positive benefit, not a negative event from last three rounds
LEGITIMACY LENDS: capital 7.0 after charge, sentiment 20.0 -> +2 (capped by sentiment) = 9.0
SOVEREIGNTY: 17.5 last turn, no capacity event, no cat4 finish or in-flight build, capability rose 0.6 -0, consolidation decay with no build -1 = 16.5
