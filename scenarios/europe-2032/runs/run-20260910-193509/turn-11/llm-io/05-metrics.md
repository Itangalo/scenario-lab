...
- characters 107-13372: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13415-13605: `{{metrics_json}}` from the run's live metric values
- characters 13698-14730: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14774-17109: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17165-17686: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M12 | EU Grid-Curtailed Bio-Cyber Survival Protocol | 2 |
...
## Metrics
```json
{"ai_capability": 87.5, "openweight_capability": 82.5, "ai_safety": 9.0, "resilience": 54.0, "eu_ai_sovereignty": 13.0, "eu_political_capital": 29.0, "public_sentiment": 5.0}
```
...
## Notepad
PORTFOLIO CHARGE t11: EU Grid-Curtailed Bio-Cyber Survival Protocol −2, priority −1 = −3
PROPOSAL BONUS: EU Grid-Curtailed Bio-Cyber Survival Protocol (cat 6, small) +4 — bio uplift signal plus eval anomaly plus grid curtailment plus cutoff aftermath landed this turn/last turn and this answers them directly
LEGITIMACY LENDS: capital 29, sentiment 5 -> none, capital is higher
SOVEREIGNTY: 16 last turn, no capacity finish, no cat4 in flight +0, export_control_escalation t11 −2, capability rose 1.5 −1 = 13

## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M12", "fields": {"finish_turn": 12}, "grounds": "named priority"}]}
```
```
