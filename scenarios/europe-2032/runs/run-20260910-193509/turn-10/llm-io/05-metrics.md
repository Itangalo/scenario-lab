...
- characters 106-13490: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13533-13724: `{{metrics_json}}` from the run's live metric values
- characters 13817-15977: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 16020-18348: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18404-19606: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M11 | EU Continuity and Substitution Shield | 2 |
...
## Metrics
```json
{"ai_capability": 86.0, "openweight_capability": 79.1, "ai_safety": 4.0, "resilience": 50.0, "eu_ai_sovereignty": 16.0, "eu_political_capital": 28.0, "public_sentiment": 7.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M11", "fields": {"finish_turn": 11}, "grounds": "named priority"}]}
```
```
