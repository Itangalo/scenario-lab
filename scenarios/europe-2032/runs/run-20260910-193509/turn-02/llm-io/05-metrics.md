...
- characters 105-13416: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13459-13651: `{{metrics_json}}` from the run's live metric values
- characters 13744-14957: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15000-17708: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17764-18533: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_bio_publication_moratorium` -- first noted turn 1, listed in 2 turn(s) so far: Major European research universities jointly announce a voluntary pause on publishing frontier genome-model weights pending new screening standards, prompting debate over open science norms.
