...
- characters 377-1547: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1591-3654: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3710-4832: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4895-32443: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 32798-33155: `{{event_history}}` from the run's own event record
...
- `emergent_foreign_genome_bypass` -- first noted turn 4, listed in 1 turn(s) so far: A non-EU frontier lab publishes open weights for a genome model matching the paused university models, explicitly routing around the European moratorium and collapsing the publication hold.
- `emergent_foreign_genome_release` -- first noted turn 4, listed in 1 turn(s) so far: A non-EU lab publishes open weights for a frontier genome design model with uplift comparable to the disputed claim, mirrored widely before any screening standard is agreed, directly undercutting the university moratorium.
...
- Turn 1 (4 turn(s) ago): cyber_test_shot, loss_of_control_incident, openweight_frontier_release
- Turn 2 (3 turn(s) ago): bio_uplift_findings, taiwan_tension_rise, emergent_lab_talent_exodus
- Turn 3 (2 turn(s) ago): emergent_bio_publication_moratorium
- Turn 4 (1 turn(s) ago): cyber_defence_breakthrough, eval_anomaly_reports, openweight_frontier_release
