...
- characters 455-1871: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1915-4331: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4670-5136: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7103-8951: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_bio_publication_moratorium (emergent event):** Major European research universities jointly announce a voluntary pause on publishing frontier genome-model weights pending new screening standards, prompting debate over open science norms.
...
| M4 | European Evaluation Corps and Talent Reception | 5 | small | 2 | 4 | own jurisdiction | ai_safety up moderately, eu_ai_sovereignty up slightly via talent | 2 | running |
...
## New measure
**European Evaluation Corps and Talent Reception**
Stands up EU-owned frontier evaluation capacity inside JRC/ENISA-linked institutes, fast-track visas and chairs for the exiting US researchers and their tooling, plus a quiet bio-uplift monitoring cell tied to HERA.
Category 5 Public technical capacity and research is why now: the lab exodus brings evaluation tooling that cannot be hired later, and the genome-model bio signal needs independent EU verification before the bio window hardens.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "European Evaluation Corps and Talent Reception", "category": 5, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, eu_ai_sovereignty up slightly via talent"}, "grounds": "capturing exodus tooling and answering bio uplift signal"}]}
```

## Priority
European Evaluation Corps and Talent Reception, because the researcher window closes this semester while gigafactories cannot, and it directly serves our withstand-disruption commitment via detection and assurance.

## In practice
We task DG RTD and JRC with 15-20 senior chairs and vetted-access compute on EuroHPC, using Horizon Europe Widening funds and member-state university pledges, with the Competitiveness Council clearing fast-track admission. The Corps gets legal mandate for pre-deployment third-party checks coordinated with AI Office, absorbing the imported eval harnesses.

In parallel we keep InvestAI Gigafactories and the tech sovereignty package in permitting, and direct HERA/ECDC to quietly scope genome-model uplift with the new cell without public alarm. We ask Washington for forensics cooperation on intrusion tooling as before, but make the talent offer unconditional and decoupled from export-control talks.
...
```
## New measure
**EU Bio Screening and Verification Shield**
Stands up binding screening standards for genome-model weights and DNA synthesis plus HERA-led independent verification of bio-uplift claims, turning the university moratorium into EU-wide practice.
Category 6 Preparedness and resilience is why now: the moratorium opens a brief window to set screening norms before publication resumes, and the genome-model claim needs EU-checked verification while the new Evaluation Corps can do it.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Bio Screening and Verification Shield", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via bio verification", "grounds": "locking in moratorium window and answering bio uplift signal"}}]}
```

## Priority
EU Bio Screening and Verification Shield, because the voluntary pause expires if not codified this semester while gigafactories and evaluation capacity can run a turn without top cover, and it directly serves our withstand-disruption commitment via biological detection.

## In practice
We task DG HERA and ECDC with a synthesis-screening implementing act under the Health Emergency framework, with the AI Office attaching weight-publication screening guidance, using EU4Health funds for screening tooling at universities and synthesis providers. The Health Council gives political cover; we offer research ministries opt-in funding to offset open-science concerns.

In parallel we keep InvestAI Gigafactories and the tech sovereignty package in permitting hearings tied to energy guarantees, and direct JRC/ENISA Evaluation Corps to run the quiet genome-model verification runs for HERA using the imported harnesses. Grid Shield sustainment continues via regulators without new money.
