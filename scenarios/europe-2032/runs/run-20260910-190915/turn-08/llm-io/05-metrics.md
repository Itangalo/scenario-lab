...
- characters 105-13471: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13514-13706: `{{metrics_json}}` from the run's live metric values
- characters 13799-14709: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14752-16691: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16747-17205: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M9 | EU Energy-Site Security and Local Repair Compact | 2 |
...
## Metrics
```json
{"ai_capability": 63.6, "openweight_capability": 61.4, "ai_safety": 18.5, "resilience": 52.5, "eu_ai_sovereignty": 27.0, "eu_political_capital": 30.0, "public_sentiment": 8.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M9", "fields": {"finish_turn": 9}, "grounds": "named priority"}]}
```
```
