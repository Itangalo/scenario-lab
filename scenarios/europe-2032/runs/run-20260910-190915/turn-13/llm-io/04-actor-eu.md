...
- characters 171-361: `{{metrics_json}}` from the run's live metric values
- characters 454-1853: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1897-4503: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6320-7042: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10756-13117: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_care_chain_contagion_strike (emergent event):** Coordinated walkouts by clinical support and municipal care staff spread beyond the five striking countries, forcing temporary closure of paper-based triage sites and prompting cross-border patient transfers.
**emergent_support_staff_paralysis (emergent event):** Cross-border walkout of outsourced hospital IT and cleaning staff over infection risk and arrears closes triage-adjacent services in several crisis wards, distinct from the clinical staffing compact.
...
- `two_year_commitment` (commitment): Keep essential services running on EU-controlled capacity through cutoff and attack
- `bio_containment_first` (position): Pandemic mode outranks all else — joint biosurveillance, clinical triage, and keeping power, payments and care running on EU-controlled capacity come before sovereign-build, adoption and restriction until transmission breaks.
...
|---|---|---|---|---|---|---|---|---|---|
| M13 | EU Chip Chokepoint Hold and Continuity Procurement | 8 | small | 12 | 14 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up slightly, eu_political_capital down slightly then up if hold holds | 2 | running |
...
| M3 | European Critical Systems Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Frontier Evaluation Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, resilience up slightly via early warning | 2 | finished |
| M5 | EU Incident Surge and Bio-Detection Network | 6 | small | 3 | 5 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
| M6 | EU Municipal and Hospital Cyber Backstop | 6 | small | 5 | 7 | own jurisdiction | resilience up moderately, eu_political_capital down slightly then up if it holds | 2 | finished |
| M7 | EU Bio Containment and Clinical Triage Shield | 6 | small | 6 | 8 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
| M8 | EU Essential Services Cyber Restoration Surge | 6 | small | 7 | 8 | own jurisdiction | resilience up moderately | 2 | finished |
| M9 | EU Energy-Site Security and Local Repair Compact | 6 | small | 8 | 9 | own jurisdiction | resilience up moderately, public_sentiment up slightly, eu_political_capital up slightly if it holds | 2 | finished |
| M10 | EU Essential Continuity Stack | 9 | small | 9 | 10 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up moderately, public_sentiment up slightly if it holds | 2 | finished |
| M11 | EU Pandemic Triage and Joint Biosurveillance Surge | 6 | small | 10 | 11 | own jurisdiction | resilience up substantially if it holds, public_sentiment up slightly if essentials hold | 2 | finished |
| M12 | EU Clinical Retention and Emergency Staffing Compact | 7 | small | 11 | 13 | own jurisdiction | resilience up moderately, public_sentiment up slightly, eu_political_capital up slightly if walkouts end | 2 | finished |
...
## New measure
**EU Chip Chokepoint Hold and Continuity Procurement**
It locks ASML servicing and export decisions to a Union-level authorisation and pools remaining chip and compute procurement to keep hospitals, power and payments running through the Taiwan blockade and US coercion.
This and now because Washington is spending the Union's one chokepoint while Taipei supply halts and a member state shops its own deal — without a common hold Europe loses both leverage and the hardware its continuity stack needs, which is exactly what the two-year commitment to keep essentials on EU-controlled capacity requires.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Chip Chokepoint Hold and Continuity Procurement", "category": 8, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly, resilience up slightly, eu_political_capital down slightly then up if hold holds"}, "grounds": "blockade plus US coercion of ASML plus defection risk require common EU authorisation"}]}
```

## Priority
EU Clinical Retention and Emergency Staffing Compact, because wards at minimum staffing will fail before any chip hold matters — staffing relief outranks all else until walkouts end.

## In practice
We run the staffing compact as the operational priority: EPSCO emergency procedure, HERA/ECDC crisis staffing certification, direct EU4Health plus civil-protection reallocation disbursed to hospitals with national co-funding agreements, rescEU mobile teams to the dozen minimum-staffed emergency departments. Unions get enforceable overtime pay and rotation caps this half-year; managers get backfill cash conditional on reporting.

We table the chokepoint hold under the Anti-Coercion Instrument and Dual-Use Regulation in Trade/Foreign Affairs Council: no further ASML service cut without Council authorisation, Commission as single negotiator with Washington, pooled DIGIT/HERA procurement of mature-node chips and spares for hospitals, energy and payments, and infringement warning to any separate hyperscaler or Beijing deal. We keep audited Gigafactory inference and the joint cyber-bio cell in degraded-hold through the automated ransomware sweep and loss-of-control containment, isolating hospital networks to paper fallback where re-infection recurs.
...
```
## New measure
**EU Joint Defence Accession and Care Continuity Deployment**
It joins the joint cyber command telemetry pool and biosurveillance pact on offer, deploys the new automated-patching / swarm-detection tooling to hospitals, power and payments, and extends emergency retention cash and infection protection to clinical-support, cleaning and outsourced IT staff to reopen paper triage sites.
This and now because the ransomware sweep plus fund-moving agent showed defenders behind while a working defence and a joint seat appeared in the same half-year, and because walkouts have now spread from clinicians to the support staff who keep wards open — without this accession and extension the two-year commitment to keep essentials running fails.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Joint Defence Accession and Care Continuity Deployment", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly if joint telemetry holds"}, "grounds": "joint command invitation plus defence breakthrough plus spreading care/support walkouts require immediate accession and deployment"}]}
```

## Priority
EU Joint Defence Accession and Care Continuity Deployment, because the joint telemetry seat and defensive tooling window closes this turn while triage closures from support-staff paralysis will kill before the chip hold matters.

## In practice
We run the accession as the operational priority: Foreign Affairs / JHA Council mandate for ENISA and ECDC/HERA to sign telemetry-sharing and sample-sharing, seconded CERT-EU and hospital CISOs to the joint cyber command, EU4Health plus civil-protection reallocation to push automated patching and swarm detection to pooled hospital, energy and payments networks under the Critical Systems Shield playbooks, with paper-fallback held where reinfection persists.

We keep the M13 chip hold in degraded-hold: Commission as sole negotiator asserting Council sign-off on any further lithography service cut, DIGIT pooled procurement of mature chips/spares for hospitals/power/payments where disbursements can move, infringement warning held on the defiant hyperscaler deal. We extend the finished Staffing Compact logic by emergency EPSCO decision to support and outsourced staff — arrears clearance conditional on co-funding, infection PPE and rotation guarantees, rescEU mobile cleaning/IT relief to closed triage sites — to reopen sites and enable cross-border transfers.
