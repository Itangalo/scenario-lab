...
- characters 106-13542: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13585-13777: `{{metrics_json}}` from the run's live metric values
- characters 13870-15403: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15447-17505: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17561-18435: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M7 | EU Bio Containment and Clinical Triage Shield | 2 |
| M8 | EU Essential Services Cyber Restoration Surge | 2 |
...
- `emergent_anti_infrastructure_backlash` -- first noted turn 7, listed in 1 turn(s) so far: Coordinated physical protests and sabotage attempts against data-centre and grid-connection sites for stalled Gigafactories, fuelled by fear after the pathogen release and anger over health queues.
...
## Metrics
```json
{"ai_capability": 62.8, "openweight_capability": 60.0, "ai_safety": 20.5, "resilience": 45.5, "eu_ai_sovereignty": 27.0, "eu_political_capital": 32.0, "public_sentiment": 10.0}
```
## Narrative
### Two fronts
...
## Notepad
PORTFOLIO CHARGE t7: EU Bio Containment and Clinical Triage Shield −2, EU Essential Services Cyber Restoration Surge −2, priority −1 = −5
PROPOSAL BONUS: none — M8 already in flight per rule 6 portfolio; no new measure proposed this turn that earns a bonus
LEGITIMACY LENDS: capital 28, sentiment 16 -> none, capital higher
SOVEREIGNTY: 22 last turn, InvestAI Gigafactories finishes t7 +5, no cat4 in flight +0, no capacity event, capability rose 0.8 −0 = 27
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M8", "fields": {"finish_turn": 8}, "grounds": "named priority"}]}
```
```
