...
- characters 377-1780: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1824-4007: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4063-5185: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5248-29475: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29830-29922: `{{event_history}}` from the run's own event record
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 2 turn(s) so far: A coordinated campaign of municipal permit denials and grid-connection challenges stalls several planned EU data-centre expansions after the June US switch-off scare fuels sovereignty anxieties.
- `emergent_research_provenance_pact` -- first noted turn 1, listed in 2 turn(s) so far: Major EU scientific publishers and universities jointly mandate disclosure and watermarking of AI-generated research after the Millennium Prize controversy, triggering a global reproducibility review.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, eval_anomaly_reports, openweight_frontier_release
