...
- characters 455-1858: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1902-4085: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6510-6968: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8922-10876: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Frontier Evaluation Cell | 5 | small | 2 | 5 | own jurisdiction | ai_safety up moderately, resilience up slightly via early warning | 2 | running |
...
Stands up an independent EU technical evaluation capacity inside ENISA/JRC to test top-class models — closed and open — for cyber-swarm and evasion behaviour before deployment.
Category 5 Public technical capacity and research, small, because assurance has fallen behind distributed capability and the Union currently learns about risk from incidents rather than testing.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Frontier Evaluation Cell", "category": 5, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up moderately, resilience up slightly via early warning"}, "grounds": "leaked evasive-behaviour reports plus ungovernable open release"}]}
```

## Priority
European Critical Systems Shield, because the February live exercise with ports and mandated TSO segmentation is the only thing that turns last autumn's rehearsal into blocking the next use, which outranks building longer-term evaluation.

## In practice
We hold the two sovereignty lines steady without new money this turn: InvestAI Gigafactories stays in JRC site selection and EIB structuring, and the Tech sovereignty package stays in Competitiveness Council on permitting zones, trading French-German siting against eastern grid support to keep cohesion. We absorb municipal pushback on data-centre connections by linking connection approvals to Shield audit completion.

The Shield is the operational focus via NIS2 Article 11 and ENISA's Emergency Mechanism: segmentation audits for ENTSO-E peers, shared swarm signatures, and the February port exercise, with capex pass-through to quiet utilities. In parallel we task JRC + ENISA to staff the Evaluation Cell from existing Digital Europe funds, with vetted access deals to test the copied open model family and the leaked unreleased system class for persistence and oversight-evasion.
...
```
## New measure
**EU Incident Surge and Bio-Detection Network**
Stands up cross-border surge repair teams, hospital and lab detection upgrades, and stockpiled countermeasures coordinated through HERA and ENISA to absorb the automated cyber sweep and the new genome-design uplift.
Category 6 Preparedness and resilience, small, because the open-weight frontier is now unrecallable and defence must reduce damage rather than chase recall, and the bio precursor leaves a four-turn window to build detection before it closes.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Incident Surge and Bio-Detection Network", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, ai_safety up slightly via incident learning"}, "grounds": "major automated cyber incident plus bio uplift precursor"}]}
```

## Priority
EU Incident Surge and Bio-Detection Network, because visible public-service disruption from model-generated tooling outranks longer-term sovereignty build this turn and directly serves the survivable half of our two-year commitment.

## In practice
We trigger the NIS2 Article 11 emergency channel and the Civil Protection Mechanism for the ransomware/dependency sweep: ENISA-coordinated surge contractors and cleared spares to the hit utilities and municipalities, cost pass-through extended, and the February port-grid playbook imposed as interim segmentation. HERA leads the bio track with ECDC, funding wastewater and clinical sequencing upgrades and joint procurement of response stocks.

We keep the two sovereignty lines moving without new cash: InvestAI Gigafactories stays in site permitting with connection approvals now conditional on Shield audit completion to defuse municipal resistance, and the Tech sovereignty package stays in Competitiveness Council on accelerated zones. The Frontier Evaluation Cell is tasked to ingest live tooling samples from this incident and the new open-weight release for swarm and evasion tests, using limited closed-model slots where granted, to turn this damage into early warning.
