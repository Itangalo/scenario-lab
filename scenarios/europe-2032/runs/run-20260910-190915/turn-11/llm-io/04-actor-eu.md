...
- characters 454-1596: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1640-3747: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6144-6866: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9972-12722: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_clinical_staff_walkout (emergent event):** Hospital staff walkouts over paper-triage overload and unpaid overtime spread across several member states, forcing emergency staffing measures during containment.
...
## Statement changes
- modify `bio_containment_first` (position): Pandemic mode outranks all else — joint biosurveillance, clinical triage, and keeping power, payments and care running on EU-controlled capacity come before sovereign-build, adoption and restriction until transmission breaks.
  - Trigger: AI-designed pandemic pathogen causing global outbreak alongside cutoff and re-infecting cyberattacks
  - Grounds: bio uplift findings have become a lethal contagious outbreak that puts everything else on hold; essentials cannot hold without pandemic triage layered onto the continuity stack

## New measure
**EU Pandemic Triage and Joint Biosurveillance Surge**
It puts HERA/ECDC-led sample-sharing, detection, clinical triage and essential-worker protection onto EU-controlled capacity and into the offered joint biosurveillance pact, with offline fallback when systems re-infect.
This and now because the contested genome-model design is now a contagious lethal outbreak during cutoff and eval-anomaly loss-of-control fears; this is preparedness and resilience, category 6, that makes the continuity commitment survivable in pandemic mode.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Pandemic Triage and Joint Biosurveillance Surge", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially if it holds, public_sentiment up slightly if essentials hold"}, "grounds": "catastrophic AI-designed bio outbreak during cutoff requires pandemic triage and joint response"}]}
```

## Priority
EU Pandemic Triage and Joint Biosurveillance Surge, because pandemic-mode transmission plus dark triage and re-locking registries outranks all else — without it the continuity stack has no staff or clinics to run on.

## In practice
We run this as a health emergency under Article 168 and the Serious Cross-Border Threats Regulation: Health Council mandates HERA/ECDC pooled procurement of countermeasures, mandatory sample-sharing and standing investigation under the offered biosurveillance pact, with ENISA-audited offline triage images from M10 via DIGIT so paper queues do not become ward closures. Audit-conditional EU4Health and civil-protection money funds nurse overtime relief and clinic-queue support carried from M9.

We tell member states plainly: permits and prosecution stay the price of guards and payouts, foreign-frontier restoration is frozen, and the Commission absorbs blame for entering the pandemic without EU-controlled fallback. TSOs keep power on in degraded mode, hospitals migrate cut-off workloads to Gigafactory-hosted models only where ENISA clears them, and all else waits until transmission breaks.
...
```
## New measure
**EU Clinical Retention and Emergency Staffing Compact**
It funds paid overtime, emergency staffing pools and paper-triage relief teams to end hospital walkouts and keep containment staffing during the outbreak.
This and now because walkouts spreading across member states during lethal transmission plus cutoff-dark triage will collapse the continuity stack if wards close — this is labour and social protection, category 7, that makes the two-year commitment to keep essentials running survivable.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Clinical Retention and Emergency Staffing Compact", "category": 7, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up slightly, eu_political_capital up slightly if walkouts end"}, "grounds": "multi-state staff walkouts during lethal pandemic and cutoff require emergency retention"}]}
```

## Priority
EU Clinical Retention and Emergency Staffing Compact, because without staff no triage, biosurveillance or EU-hosted fallback runs — it outranks all else until wards stay open.

## In practice
We run this as an EPSCO + Health Council emergency package under EU4Health and the Civil Protection Mechanism: HERA/ECDC certify crisis staffing need, Commission disburses audit-conditional overtime and sick-leave backfill funds directly to hospitals, and rescEU medical corps plus DIGIT-deployed offline triage teams relieve paper-triage overload. Unions get paid overtime guarantees and rotation limits; member states get ward-closure avoidance in exchange for mandatory staffing reporting.

We freeze foreign frontier restoration and sovereign-build expansion this turn, keep ENISA-audited EU Gigafactory inference and joint cyber-bio cell telemetry in degraded-hold, and seek observer status to the US-China risk agreement via the offered biosurveillance pact to slow the next bio uplift. The Commission continues to absorb blame for entering without domestic fallback while essentials degrade-but-hold.
