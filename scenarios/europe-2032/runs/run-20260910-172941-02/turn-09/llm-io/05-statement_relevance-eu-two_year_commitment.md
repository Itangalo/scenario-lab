...
- characters 3157-5112: `{{world_state}}` from background/context.md, seeded as the opening world state
- characters 5145-7533: `{{previous_actions}}` from the actor's own response from the previous turn
...
Rewrite it to read: Survive cascading bio and rogue AI shocks through containment, continuity and allied leverage
...
```json
{
  ...
  "found": true,
  "verdict": "BEARS",
  "reason": "The closure of the commitment period directly ends the timeframe for the original statement, necessitating a new orientation."
}
```
```
