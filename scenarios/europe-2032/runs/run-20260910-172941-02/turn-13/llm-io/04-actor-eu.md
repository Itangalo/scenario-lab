...
- characters 452-2600: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2644-4519: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7450-7804: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10699-12859: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Secure survival and leverage through middle-power coordination and US-China risk floor while containing bio and rogue-agent harm
...
| M4 | EU Entry-Level Transition Fund | 7 | small | 2 | 5 | own jurisdiction | resilience up moderately via social absorption, public_sentiment up slightly | 2 | finished |
| M5 | EU Model Scrutiny Access Mandate | 1 | small | 3 | 6 | own jurisdiction | ai_safety up moderately, resilience up slightly via incident learning | 2 | finished |
| M6 | EU Frontier Evaluation Taskforce | 5 | small | 4 | 7 | own jurisdiction | ai_safety up moderately | 2 | finished |
| M7 | EU Wartime Continuity and Joint Cyber Shield | 6 | small | 7 | 9 | own jurisdiction | resilience up substantially | 2 | finished |
| M8 | EU Middle-Power Alignment and Verification Cell | 8 | small | 10 | 11 | coalition | eu_political_capital up moderately, eu_ai_sovereignty up slightly via secured access, ai_safety up slightly via shared evaluation | 2 | finished |
| M9 | EU Rogue-Agent Containment and Essential-Service Islanding Protocol | 6 | small | 11 | 12 | own jurisdiction | resilience up moderately, ai_safety up slightly via incident learning | 2 | finished |
| M10 | EU Essential-Service Survival and Cutoff Substitution Protocol | 6 | small | 12 | 13 | own jurisdiction | resilience up moderately, public_sentiment up slightly via kept services | 2 | finished |
...
## New measure
**EU Essential-Service Survival and Cutoff Substitution Protocol**
Keeps hospitals, grids, water and payments running on islanded manual fallbacks and EU-hosted substitutes after US frontier cutoff while absorbing the automated cyber sweep, via ENISA-led triage and rationed domestic capacity.
This and now because cutoff plus major incident together hit lit hospitals and ministries at once, and with capital at 8 and sovereignty at 12 only a small resilience protocol (category 6) using existing continuity cells and islanding can land in this turn.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential-Service Survival and Cutoff Substitution Protocol", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up slightly via kept services"}, "grounds": "surviving cutoff and automated attack with islanded services"}]}
```

## Priority
EU Essential-Service Survival and Cutoff Substitution Protocol, because it is the only in-flight effort and holding lights, care and payments through cutoff and sweep outranks any coalition or build ambition this turn.

## In practice
We run this through the wartime continuity cell already staffed: ENISA with Europol EC3 and national CERTs to sinkhole occupant and sweep-related C2, freeze cash-outs, and push pre-signed islanding orders to TSOs via ENTSO-E and telecom ISACs; HERA/ECDC to fund fever-clinic oxygen/bed triage and hospital backup power from existing EU4Health and rescEU lines, with Justice and Home Affairs Council cover for police guard.

We substitute cutoff workloads by force-rationing what little domestic compute remains to hospitals, ministries and grid operators under the Health Emergency and NIS2 legal bases, downgrading to older EU-hosted and open models where safe and to paper-manual procedures where not, and we tell the defecting member state plainly that joint cyber aid and backup capacity now prioritise aligned systems. No new build is promised; survival paid from existing budgets is the offer capitals can still accept.
...
```
## New measure
**EU Islanded Survival and Joint Threat Shelters**
Keeps hospitals, grids, water and payments islanded on EU-hosted and manual fallbacks while plugging into the joint cyber/bio threat response for telemetry and sample-sharing, to absorb the automated sweep, open-frontier intrusions and labour shock.
This and now because with US cutoff, safety at zero, capital at 9 and open weights at frontier, only a small preparedness measure (category 6) using existing continuity cells, ENISA and HERA can hold essentials and gain outside protection this turn.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Islanded Survival and Joint Threat Shelters", "category": 6, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately via islanding and joint response"}, "grounds": "surviving cutoff, major cyber incident and bio uplift with joint help"}]}
```

## Priority
EU Islanded Survival and Joint Threat Shelters, because holding lights, care and payments through cutoff and sweep outranks any build or coalition ambition while capital and trust are near zero.

## In practice
We run this through the wartime continuity cell already staffed: ENISA with Europol EC3 and national CERTs to sinkhole sweep and occupant C2 and freeze cash-outs, ENTSO-E transmission operators on pre-signed islanding orders, HERA/ECDC funding fever-clinic oxygen, backup power and paper-manual triage from existing EU4Health and rescEU lines under NIS2 and Health Emergency bases.

We force-ration remaining domestic compute to hospitals, ministries and grid, downgrade to older EU-hosted and open models only where safe, and accept the joint cyber command / biosurveillance pact terms for real-time telemetry and sample-sharing to get protection we cannot build alone. Joint aid prioritises aligned systems, and we tell the separately-dealt member state that solidarity now follows alignment. No new build is promised.
