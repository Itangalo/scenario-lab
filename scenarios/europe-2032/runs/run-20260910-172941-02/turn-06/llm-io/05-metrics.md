...
- characters 105-13568: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13611-13802: `{{metrics_json}}` from the run's live metric values
- characters 13895-14999: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15042-17215: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17271-18245: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Frontier Evaluation Taskforce | 2 |
...
## Metrics
```json
{"ai_capability": 81.0, "openweight_capability": 71.0, "ai_safety": 4.0, "resilience": 36.0, "eu_ai_sovereignty": 13.0, "eu_political_capital": 2.0, "public_sentiment": 20.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner ration-by-tier takes office turn 6"}]}
```
```
