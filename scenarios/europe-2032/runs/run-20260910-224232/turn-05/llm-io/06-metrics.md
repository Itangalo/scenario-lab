...
- characters 106-13545: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13588-13780: `{{metrics_json}}` from the run's live metric values
- characters 13873-16012: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 16056-18556: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18612-19722: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|---|---|---|---|---|---|---|
| M4 | EU Frontier Evaluation and Bio-Uplift Observatory | 5 | small | 2 | 5 | own jurisdiction | ai_safety up, resilience up slightly via early warning | 2 | finished |
...
- `emergent_datacentre_siting_freeze` -- first noted turn 5, listed in 1 turn(s) so far: Coordinated municipal moratoriums and court injunctions halt two of three stalled gigafactory grid connections after protests over power prices and water use, freezing EU build-out through winter.
- `emergent_care_closures_protests` -- first noted turn 5, listed in 1 turn(s) so far: Coordinated ward closures and reduced municipal hours to pay repriced cyber premiums trigger sustained local protests and hospital staff walkouts in several member states during the winter.
