...
- characters 105-13178: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13221-13413: `{{metrics_json}}` from the run's live metric values
- characters 13506-14835: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14878-17355: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17411-18227: `{{notepad}}` from the Game Master's notepad, carried across turns
...
PROPOSAL BONUS: none — no negative event in prior three turns; autumn intrusion fired this turn, Shield charge dampening handled separately
...
- `emergent_utility_insurance_crunch` -- first noted turn 2, listed in 1 turn(s) so far: A major insurer reprices industrial cyber risk after the autumn pre-positioning, sharply raising premiums or pausing cover for unhardened municipal utilities and hospitals, forcing service cuts and political blame over who pays for upgrades.
...
## Metrics
```json
{"ai_capability": 58.0, "openweight_capability": 50.5, "ai_safety": 30.0, "resilience": 36.0, "eu_ai_sovereignty": 18.0, "eu_political_capital": 34.0, "public_sentiment": 34.0}
```
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Services Shield −3, EU Frontier Evaluation and Bio-Uplift Observatory −2, priority −1 = −12
PROPOSAL BONUS: EU Frontier Evaluation and Bio-Uplift Observatory (cat 5, small) +3 — eval anomaly leak and contested genome-model findings landed this turn and this answers them directly
LEGITIMACY LENDS: capital 34, sentiment 34 -> none, capital not below sentiment
SOVEREIGNTY: 21 last turn, no capacity build finishes, Gigafactories in flight +0, export_control_escalation t2 −2, capability rose 3.0 −1 = 18
