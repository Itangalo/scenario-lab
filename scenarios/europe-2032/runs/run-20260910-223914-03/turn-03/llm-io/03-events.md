...
- characters 2053-4073: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4129-4964: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5027-28603: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28958-29048: `{{event_history}}` from the run's own event record
...
- `emergent_factory_siting_backlash` -- first noted turn 2, listed in 1 turn(s) so far: Coordinated local protests and municipal permit challenges stall one or more EU AI factory sites over power prices and water use, forcing a siting delay without a formal policy change.
