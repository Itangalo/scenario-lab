...
- characters 106-13608: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13651-13842: `{{metrics_json}}` from the run's live metric values
- characters 13935-15647: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15691-18011: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 18067-19264: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M5 | EU Essential Services Continuity Switch | 6 | small | 3 | 5 | own jurisdiction | resilience up, eu_ai_sovereignty stabilise | 2 | finished |
...
- `emergent_grid_curtailment_crisis` -- first noted turn 5, listed in 1 turn(s) so far: A major European grid operator imposes emergency curtailment on AI data centres during winter peak, forcing throttled training and inference across the continent.
- `emergent_sabotage_construction_halt` -- first noted turn 5, listed in 1 turn(s) so far: A coordinated physical sabotage wave hits two EU data-centre construction sites and grid interconnects, driven by local anti-data-centre groups, halting works for weeks and spiking regional power-price protests.
...
## Metrics
```json
{"ai_capability": 74.5, "openweight_capability": 66.0, "ai_safety": 17.0, "resilience": 41.0, "eu_ai_sovereignty": 17.0, "eu_political_capital": 10.0, "public_sentiment": 10.0}
```
...
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 9, sentiment 10 -> +1
SOVEREIGNTY: 17 last turn, no capacity event, M1/M2 in flight +1, capability rose 3.5 −1 = 17
