...
- characters 377-1720: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1764-3378: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3434-4841: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4904-29131: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29486-29605: `{{event_history}}` from the run's own event record
...
- `emergent_sovereign_hosting_offer` -- first noted turn 1, listed in 2 turn(s) so far: A major non-US cloud provider announces EU-only model hosting with data and weights legally ring-fenced in the EU, marketed as continuity insurance after the June switch-off scare.
- `emergent_eu_hosting_guarantee` -- first noted turn 1, listed in 2 turn(s) so far: A major non-US cloud provider announces EU-only model hosting with legally binding non-withdrawal guarantees, giving EU enterprise customers a partial hedge against another Fable-style switch-off.
