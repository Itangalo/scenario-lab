...
- characters 455-1798: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1842-3456: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5053-5555: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7534-9202: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_sovereign_hosting_offer (emergent event):** A major non-US cloud provider announces EU-only model hosting with data and weights legally ring-fenced in the EU, marketed as continuity insurance after the June switch-off scare.
**emergent_eu_hosting_guarantee (emergent event):** A major non-US cloud provider announces EU-only model hosting with legally binding non-withdrawal guarantees, giving EU enterprise customers a partial hedge against another Fable-style switch-off.
...
| M4 | EU Essential Services Continuity Stack | 9 | large | 2 | 4 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up, public_sentiment up slightly | 3 | running |
...
Stands up EU-hosted fallback models and migration support for hospitals, ministries and operators cut off from the leading foreign model, via emergency procurement and EU-cloud inference capacity. Why this and why now: the cutoff has turned dependence into immediate service failure, and keeping essential services running through disruption is the core of our two-year commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Continuity Stack", "category": 9, "size": "large", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly, resilience up, public_sentiment up slightly"}, "grounds": "answering frontier access cutoff hitting essential services"}]}
```

## Priority
EU Essential Services Continuity Stack, because active loss of the leading model in hospitals and ministries outranks even the grid hunt this turn.

## In practice
We task DG CNECT and DG DIGIT with the AI Office to run an emergency continuity cell: inventory affected deployments in health and administration, offer funded migration to EU-hosted open and licensed models on Gaia-X / EUCS cloud, with temporary state-aid clearance via the Competitiveness Council.

In parallel we keep M3 Grid Shield as operator: ENISA pushes segmentation audits and credential rotation through, using reprogrammed CEF funds, while M1/M2 gigafactory permitting continues on existing legal bases without new cash. We frame the cutoff publicly as foresight failure we are now fixing, not as outrage alone, to protect capital for delivery.
...
```
## New measure
**EU Automated Incident Containment Pact**
Stands up cross-border automated containment, emergency patching and mutual-aid recovery teams for public services hit by the model-generated ransomware/dependency attack. Why this and why now: the large automated attack landing on brittle systems is the immediate threat to keeping essential services running under our two-year commitment, and it outranks longer build-outs this turn.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Automated Incident Containment Pact", "category": 6, "size": "large", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, public_sentiment up slightly"}, "grounds": "answering large automated model-generated attack on public services"}]}
```

## Priority
EU Automated Incident Containment Pact, because active cascading disruption of public services outranks even the continuity-stack migration this turn.

## In practice
We task ENISA and the EU-CyCLONe network with the containment pact: pre-authorised isolation playbooks, joint hunt-and-patch teams drawn from M3 Shield staff, and CER-Directive emergency orders for segmentation and backups, funded by reprogrammed Digital Europe and CEF-Digital money via the Competitiveness and JHA Councils.

In parallel we keep M4 Continuity Stack as operator: DG DIGIT continues funded migration of hospitals and ministries cut off in February to EU-hosted open models, now explicitly steered toward the emergent non-US EU-ring-fenced hosting offers with non-withdrawal clauses to avoid a second switch-off, while M1/M2 gigafactory permitting continues on paper without new cash given the investment collapse. We frame both publicly as keeping wards and town halls running through attack and cutoff, to stabilise sentiment while recovery is visible.
