...
- characters 13927-15021: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15064-16844: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16900-17393: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU High-Risk AI Redress and Audit Reset | 2 |
| M8 | EU Essential Fallback for Hospitals and Administrations | 2 |
...
## Notepad
PORTFOLIO CHARGE t6: InvestAI Gigafactories −3, EU High-Risk AI Redress and Audit Reset −2, EU Essential Fallback for Hospitals and Administrations −2, priority −1 = −8
PROPOSAL BONUS: EU Essential Fallback for Hospitals and Administrations (cat 9, small) +4 — eu_frontier_access_denied fired this turn and this answers it directly, small size limits gain
LEGITIMACY LENDS: capital 10, sentiment 15 -> +2
SOVEREIGNTY: 16 last turn, Tech sovereignty package finishes t6 +4, no other finish, eu_frontier_access_denied t6 −1, capability rose 1.0 no penalty, in flight +0 = 19

## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "CONSOLIDATION"}, "grounds": "turn 5 election winner takes office turn 6"}, {"op": "update", "table": "measures", "id": "M8", "fields": {"finish_turn": 7}, "grounds": "named priority"}]}
```
```
