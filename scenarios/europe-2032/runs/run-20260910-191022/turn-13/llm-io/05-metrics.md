...
- characters 107-13344: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13387-13579: `{{metrics_json}}` from the run's live metric values
- characters 13672-15085: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15129-17007: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17063-17838: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M15 | EU Trusted Workplaces and Care Queues Pact | 2 |
...
- `emergent_care_queue_backlash` -- first noted turn 12, listed in 2 turn(s) so far: Hospital waiting lists for EU-hosted dose-planning leak to foreign press, triggering cross-border medical tourism brokers offering tailored-therapy slots abroad and a parliamentary inquiry into queue allocation.
...
## Notepad
PORTFOLIO CHARGE t13: M15 EU Trusted Workplaces and Care Queues Pact −2, priority −1 = −3
PROPOSAL BONUS: none — knowledge_work_augmented explicitly blocks bonus, eval leak is precursor not a negative event this measure answers
LEGITIMACY LENDS: capital 27, sentiment 15 -> none, capital higher
SOVEREIGNTY: 19 last turn, no category 4 finish, no capacity event, no in-flight build bonus, capability rose 1.0 −0 = 19
