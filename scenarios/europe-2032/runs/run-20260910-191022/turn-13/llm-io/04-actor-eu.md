...
- characters 456-1869: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1913-3791: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5459-5959: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9644-11720: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Rebuild EU-controlled capable AI for health and essential services while keeping offline continuity through cutoff and attack
...
| M3 | EU Grid and Essential Services Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up slightly via incident learning | 3 | finished |
| M4 | EU Essential Services Continuity Stack | 9 | large | 2 | 4 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up, public_sentiment up slightly | 3 | finished |
| M5 | EU Automated Incident Containment Pact | 6 | large | 3 | 4 | own jurisdiction | resilience up substantially, public_sentiment up slightly | 3 | finished |
| M6 | EU Bio Detection and Screening Shield | 6 | small | 4 | 6 | own jurisdiction | resilience up moderately, ai_safety up slightly | 2 | finished |
| M7 | EU High-Risk AI Redress and Audit Reset | 1 | small | 5 | 7 | own jurisdiction | public_sentiment up, eu_political_capital up slightly, ai_safety up slightly | 2 | finished |
| M8 | EU Essential Fallback for Hospitals and Administrations | 9 | small | 6 | 7 | own jurisdiction | eu_ai_sovereignty up slightly, resilience up slightly, public_sentiment up slightly | 2 | finished |
| M9 | EU Hospitals and Grids Cyber Containment Surge | 6 | small | 7 | 8 | own jurisdiction | resilience up moderately, public_sentiment up slightly | 2 | finished |
| M10 | EU Supply-Chain Bargain for Continuity | 8 | small | 8 | 9 | own jurisdiction with middle-power coalition | eu_ai_sovereignty up slightly, resilience up slightly | 2 | finished |
| M11 | EU Essential Services Offline Continuity Corps | 6 | small | 9 | 10 | own jurisdiction | resilience up moderately, public_sentiment up slightly, eu_political_capital up slightly | 2 | finished |
| M12 | EU Sovereign Clinical AI Access Pact | 9 | small | 10 | 11 | own jurisdiction with middle-power coalition | public_sentiment up moderately, resilience up slightly, eu_ai_sovereignty up slightly | 2 | finished |
| M13 | EU Hosted Clinical Inference Sprint | 4 | small | 11 | 12 | own jurisdiction | eu_ai_sovereignty up slightly, public_sentiment up moderately, resilience up slightly | 2 | finished |
| M14 | EU Cutoff-Proof Operations Stack | 9 | small | 12 | 13 | own jurisdiction | resilience up moderately, public_sentiment up moderately, eu_ai_sovereignty up slightly | 2 | finished |
...
## New measure
**EU Cutoff-Proof Operations Stack**
Puts EU-hosted auditable models and non-coercible automation into hospitals, administrations and warehouse-logistics operators so the US cutoff does not stop care, permits or supply, and Chinese-American robots do not own the shop floor.
Why this and why now: with the frontier cut at short notice, safety at 13 and sentiment at 20, only a diffusion-and-resilience stack on EU-cleared clouds proves the waiting room and the office keep running by right, not by permission — the core of our two-year commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cutoff-Proof Operations Stack", "category": 9, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up moderately, public_sentiment up moderately, eu_ai_sovereignty up slightly"}, "grounds": "keeping care admin and logistics running through cutoff and robot wave"}]}
```

## Priority
EU Cutoff-Proof Operations Stack, because the access-denied event outranks everything else — without runnable EU-hosted fallback this turn, queues become shutdowns.

## In practice
We execute via Health and Competitiveness Councils with HERA, EMA, ENISA and EuroHPC: Emergency Implementing Decision extending the cleared hospital-cloud regime to admin and logistics clouds, reprogrammed EU4Health, Digital Europe and ESF+ envelopes for EU-hosted inference adapters, auditable fallback models from the Continuity Corps, and joint procurement for EU-assembled sensors, actuators and control software.

Member states second hospital IT, municipal digitisation and port-logistics teams and accept triage — oncology dose-planning, emergency care, permits and port throughput first — while we offer accelerated permitting, grid connection and pooled orders to EU integrators and Japan/Korea suppliers for non-US control stacks, publishing weekly EU-run patient and permit counts. Frontier vendors get a single choice: license inspectable weights for EU hosting or lose EU public procurement.
...
```
## New measure
**EU Trusted Workplaces and Care Queues Pact**
Puts EU-hosted auditable assistants into offices, hospitals and permit desks under works-council oversight with published queue and productivity gains, so the cutoff and robot wave read as shared benefit not foreign permission.
Why this and why now: with sentiment at 15, capital at 27 and queues brokered abroad after the February cutoff, only visible EU-run benefit in the waiting room and the ordinary office rebuilds tolerance to keep sovereignty work alive; the leaked eval anomaly also demands inspectable EU-hosted deployment to learn from.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trusted Workplaces and Care Queues Pact", "category": 9, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up moderately, resilience up slightly, eu_ai_sovereignty up slightly"}, "grounds": "rebuilding trust through EU-hosted benefit after cutoff"}]}
```

## Priority
EU Trusted Workplaces and Care Queues Pact, because without sentiment recovery no sovereignty or resilience measure can be funded or sited, and it directly serves our two-year commitment to EU-controlled AI for health and essential services.

## In practice
We execute via EPSCO and Health Councils with EU-OSHA, EMA and EuroHPC: an Implementing Decision extending the cleared hospital-cloud regime to workplace assistants, reprogrammed ESF+ and Digital Europe envelopes for EU-hosted adapters and audit logging, and joint procurement for EU-assembled inference capacity.

Member states second works councils, hospital IT and municipal teams and accept triage — dose-planning queues, permits and law/admin/accounting pilots first — while we offer social-partner co-governance, published weekly EU-run patient/permit counts and retraining guarantees to unions and mayors resisting data-centre siting, and a single procurement rule to vendors: inspectable weights for EU hosting or no public contract.
