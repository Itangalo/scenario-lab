...
- characters 105-13655: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13698-13890: `{{metrics_json}}` from the run's live metric values
- characters 13983-15169: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15212-17361: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17417-18022: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M6 | EU Bio Detection and Response Surge | 2 |
| M7 | EU Automation Transition Safety Net | 2 |
| M8 | EU Continuity Switch to European-Run Models | 2 |
...
**emergent_datacentre_permitting_freeze (emergent event):** Coordinated local referendums and court injunctions halt several gigafactory-linked data-centre builds over water and power use, freezing permitted capacity for months.
...
## Notepad
PORTFOLIO CHARGE t6: M1 InvestAI Gigafactories −3, M6 EU Bio Detection and Response Surge −2, M7 EU Automation Transition Safety Net −2, M8 EU Continuity Switch to European-Run Models −2, priority M1 −1 = −10
PROPOSAL BONUS: EU Continuity Switch to European-Run Models (cat 9, small) +3 — eu_frontier_access_denied landed this turn stranding hospitals/ministries and this answers it directly
LEGITIMACY LENDS: capital 9, sentiment 18 -> +2
SOVEREIGNTY: 13 last turn, Tech sovereignty package finishes t6 +4, M1 in flight +1, eu_frontier_access_denied t6 −2, capability rose 1.0 −0 = 16

## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "RETRENCHMENT"}, "grounds": "turn 5 winner takes office turn 6"}, {"op": "update", "table": "measures", "id": "M1", "fields": {"finish_turn": 9}, "grounds": "event: datacentre permitting freeze halts gigafactory-linked builds"}]}
```
```
