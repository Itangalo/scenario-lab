...
- characters 106-13440: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13483-13675: `{{metrics_json}}` from the run's live metric values
- characters 13768-15023: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15067-17750: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17806-18307: `{{notepad}}` from the Game Master's notepad, carried across turns
...
PORTFOLIO CHARGE t2: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M3 EU Critical Systems Shield −3, M4 Shield Scale-Up with Defensive Breakthrough −3, priority −1 = −13
PROPOSAL BONUS: Shield Scale-Up (cat 6, large) +3 — autumn pre-positioning intrusions in grids/port/water landed last turn and this answers it directly
LEGITIMACY LENDS: capital 21, sentiment 31 -> +2
...
## Metrics
```json
{"ai_capability": 58.0, "openweight_capability": 51.0, "ai_safety": 28.0, "resilience": 40.0, "eu_ai_sovereignty": 18.0, "eu_political_capital": 26.0, "public_sentiment": 29.0}
```
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M4", "fields": {"finish_turn": 6}, "grounds": "named priority"}]}
```
```
