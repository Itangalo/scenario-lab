...
- characters 454-1647: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1691-3595: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4257-4713: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7626-9887: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_stranded_gigafactory_writeoff (emergent event):** Coordinated physical blockades and municipal court injunctions permanently strand the Lyon and Magdeburg gigafactory shells without power or fit-out, forcing the Commission to write them off as stranded assets this turn.
...
- `two_year_commitment` (commitment): Hold essential services and social cohesion together through AI-driven disruption
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | Municipal Night-Cover Mutual Aid | 6 | small | 10 | 12 | own jurisdiction | resilience up modestly, public_sentiment up slightly, eu_political_capital up slightly via visible repair | 2 | running |
...
| M4 | Shield Scale-Up with Defensive Breakthrough | 6 | large | 2 | 6 | own jurisdiction | resilience up substantially, ai_safety up slightly via detection | 3 | finished |
| M5 | Trust Repair Audits with Secure Adoption Pilots | 9 | small | 3 | 5 | own jurisdiction | public_sentiment up, eu_political_capital up slightly, resilience up slightly via adoption | 2 | finished |
| M6 | EU Bio Detection and Response Surge | 6 | small | 4 | 7 | own jurisdiction | resilience up, ai_safety up slightly via detection | 2 | finished |
| M7 | EU Automation Transition Safety Net | 7 | small | 5 | 9 | own jurisdiction | public_sentiment up, resilience up slightly via social absorption, eu_political_capital up slightly | 2 | finished |
| M8 | EU Continuity Switch to European-Run Models | 9 | small | 6 | 8 | own jurisdiction | eu_ai_sovereignty up slightly, public_sentiment up slightly, resilience up slightly | 2 | finished |
...
## New measure
**Municipal Night-Cover Mutual Aid**
It puts ENISA-coordinated night and weekend cover onto small-town clinics, water utilities and municipal IT via cross-border watch-centre pooling on existing national budgets.
This and now because the October ransomware sweep and the US frontier cut-off proved protection stops at big cities and hospitals built on foreign models, and with political capital at 2 and public sentiment at 10 we cannot fund concrete or new clusters, only staff time and shared telemetry we already own.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Municipal Night-Cover Mutual Aid", "category": 6, "size": "small", "finish_turn": 12, "applies_to": "own jurisdiction", "targeted_effect": "resilience up modestly, public_sentiment up slightly, eu_political_capital up slightly via visible repair"}, "grounds": "automated attack exposed small-service gap while frontier denial forces continuity"}]}
```

## Priority
No priority named this turn, because with a single small survival measure in flight and political capital at 2, naming it priority adds enforcement cost we cannot pay without breaking mayors' freeze talks further.

## In practice
We run this through ENISA and the France-Germany-Netherlands watch centres that held in October, using the Critical Systems Shield procedures and Bio Detection surge staff already on payroll, seconding analysts to uncovered municipalities under Article 173 and Civil Protection Mechanism mutual-aid requests. No new siting, no new procurement: DG DIGIT repoints existing telemetry sharing, DG SANTE keeps Marseille/Essen/Brno European-hosted coverage running to absorb hospitals cut off from the US lead model.

We tell the Council Competitiveness and EPSCO formations plainly that Lyon/Magdeburg fit-out stays paused until equipment co-financing is replaced, and offer mayors policing-cost reimbursement and grid-link protection via the Committee of the Regions to keep the freeze from becoming sabotage. Retraining and wage-insurance arrears in Rotterdam, Antwerp and Lodz are triaged through ESF+ advances, not new promises, to hold cohesion while attribution on the attack remains pending.
...
## New measure
None this turn.
We add nothing because with political capital at 2 and a single small mutual-aid measure already costing 2, any new instrument breaks enforcement and deepens the member-state defection and blockade crisis we must absorb first.

## Store changes
No changes.

## Priority
M9 — Municipal Night-Cover Mutual Aid, because holding essential services together is our two-year commitment and it is the only instrument we can still fund and staff while Lyon/Magdeburg are written off and a member state undercuts the common line.

## In practice
We formally write off the Lyon and Magdeburg shells as stranded assets this turn: Commission accounting decision, municipal injunctions noted as force majeure, police guard withdrawn to grid-link protection only, no further fit-out spending sought. We tell Council and Parliament plainly that InvestAI Gigafactories (M1) and the sovereignty package (M2) delivered no online sovereign capacity, and we refuse to stand up a parallel compute programme to hide it.

We hold M9 as the whole of action: ENISA and the France-Germany-Netherlands watch centres continue night/weekend cover to small clinics, water utilities and municipal IT on existing payroll, DG DIGIT repoints telemetry, DG SANTE sustains Marseille/Essen/Brno European-hosted continuity after the US cut-off. On the defection we do not retaliate — with capital at 2 we cannot — we contain: bilateral pragmatism noted, Anti-Coercion Instrument not invoked, cohesion work limited to ESF+ emergency advances for Rotterdam/Antwerp/Lodz arrears.
