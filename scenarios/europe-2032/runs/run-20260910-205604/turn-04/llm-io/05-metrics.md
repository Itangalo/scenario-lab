...
- characters 105-13110: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13153-13345: `{{metrics_json}}` from the run's live metric values
- characters 13438-14856: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14899-17145: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17201-18327: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | Shield Scale-Up with Defensive Breakthrough | 3 |
| M5 | Trust Repair Audits with Secure Adoption Pilots | 2 |
...
PORTFOLIO CHARGE t3: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M4 Shield Scale-Up with Defensive Breakthrough −3, M5 Trust Repair Audits with Secure Adoption Pilots −2, priority −1 = −12
PROPOSAL BONUS: Trust Repair Audits (cat 9, small) +2 — welfare-fraud bias scandal landed last turn and this answers it directly but small and trust repair takes time
LEGITIMACY LENDS: capital 24, sentiment 29 -> +2
SOVEREIGNTY: 20 last turn, no capacity finish, Gigafactories/tech package in flight +0, export_control_escalation t3 −1, capability rose 2.0 −1 = 18

## Emerging developments (tracked)

- `emergent_datacentre_permitting_freeze` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated local referendums and court injunctions halt several gigafactory-linked data-centre builds over water and power use, freezing permitted capacity for months.
- `emergent_datacentre_permit_freeze` -- first noted turn 4, listed in 1 turn(s) so far: Coordinated municipal moratoriums on new data-centre water and power connections spread across several member states, freezing gigafactory site permitting for months.
...
## Metrics
```json
{"ai_capability": 60.0, "openweight_capability": 54.5, "ai_safety": 26.0, "resilience": 39.0, "eu_ai_sovereignty": 15.0, "eu_political_capital": 18.0, "public_sentiment": 27.0}
```
