...
- characters 455-1563: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1607-3725: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5520-5970: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8675-10707: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M7 | EU Automation Transition Safety Net | 7 | small | 5 | 9 | own jurisdiction | public_sentiment up, resilience up slightly via social absorption, eu_political_capital up slightly | 2 | running |
| M8 | EU Continuity Switch to European-Run Models | 9 | small | 6 | 8 | own jurisdiction | eu_ai_sovereignty up slightly, public_sentiment up slightly, resilience up slightly | 2 | running |
...
## New measure
**EU Continuity Switch to European-Run Models**
Moves hospitals, ministries and firms cut off from the leading foreign model onto European-hosted and open-weight systems with emergency support, procurement preference and migration funding.
Category 9 diffusion, adoption and public trust is why now: the cutoff has left essential services stranded this turn, and only a visible EU-run fallback restores function while proving the independent-stack commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Continuity Switch to European-Run Models", "category": 9, "size": "small", "finish_turn": 8, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly, public_sentiment up slightly, resilience up slightly"}, "grounds": "answer frontier cutoff and unblock sovereignty legitimacy"}]}
```

## Priority
M1 InvestAI Gigafactories, because the access denial makes conditional foreign compute an immediate outage rather than a future risk and only landing EU-anchored capacity restores leverage.

## In practice
We run the cutoff as the crisis it is: DG CNECT and DIGIT with ENISA stand up a continuity cell that inventories affected hospitals, ministries and contractors, funds re-platforming to EU-hosted Mistral/Aleph Alpha and hardened open-weight deployments via DIGITAL and EU4Health reprogramming, and writes EU-hosting requirements into new health and public-admin procurement. DG TRADE lodges the denial as coercion evidence while demanding durable, non-discriminatory access terms in parallel.

We do not open a second compute line: DG ENER and DG REGIO concentrate on unfreezing M1/M2 sites hit by referendums and injunctions, pairing the M7 wage-insurance pilots with water-efficiency and grid-upgrade co-funding to lift councils, while HERA and ECDC push the M6 bio-detection kits into Shield watch centres. With capital at 14 we sequence, not multiply: gigafactories first, continuity switch second, everything else maintenance.
...
```
## New measure
None this turn, while we let two measures finish and political capital rebuild before starting anything new.

## Store changes
No changes.

## Priority
M8 EU Continuity Switch to European-Run Models, because with capital at 11 only a finishing, visible restoration of hospitals and ministries can stop sentiment collapsing further before gigafactories can unfreeze.

## In practice
We put everything into landing what is almost done: the continuity cell under DG CNECT/DIGIT with ENISA finishes re-platforming the cut-off hospitals in Marseille, Essen, Brno onto EU-hosted models, using DIGITAL/EU4Health money for migration teams and writing EU-hosting into the next health procurement round, while HERA closes the M6 bio-detection rollout into Shield watch centres.

On M1 we do not open new ground: DG ENER and DG REGIO with the Council energy formation offer only water-efficiency and substation co-funding tied to the M7 wage-insurance pilots at Lyon and Magdeburg, to keep councils talking without spending capital we do not have. On embodied AI we take no EU-wide robotics line this turn — we task EU-OSHA and DG EMPL to map warehouse/logistics displacement from Chinese and US systems into the M7 safety-net targeting, and hold autonomous-lethality discussion inside the existing Shield exercises.
