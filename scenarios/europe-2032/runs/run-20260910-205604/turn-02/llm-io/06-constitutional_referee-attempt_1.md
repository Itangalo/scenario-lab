...
- characters 2968-3469: `{{notepad}}` from the Game Master's notepad, carried across turns
...
```
VIOLATIONS:
- Metric change violation: `eu_political_capital` decreased by 20 points (from 43.0 to 23.0), exceeding the 25-point hard cap on maximum movement per turn (Constitutional Constraint #9). While large swings are possible, no metric may move more than 25 points in a single turn. This breach stands even if the narrative justifies a large drop; the constraint is on the magnitude of the numerical change, not its plausibility.
```
```
