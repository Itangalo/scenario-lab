...
- characters 107-13268: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13311-13500: `{{metrics_json}}` from the run's live metric values
- characters 13593-14513: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14557-16564: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16620-17339: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_therapy_licence_squeeze` -- first noted turn 12, listed in 2 turn(s) so far: A foreign frontier provider abruptly tightens clinical-use licensing for its biomedical models, requiring per-patient clearance that delays tailored therapies in EU hospitals reliant on them.
...
## Metrics
```json
{"ai_capability": 69.3, "openweight_capability": 67.4, "ai_safety": 0.0, "resilience": 50.0, "eu_ai_sovereignty": 11.0, "eu_political_capital": 0.0, "public_sentiment": 0.0}
```
## Narrative
...
## Notepad
PORTFOLIO CHARGE t13: none in flight = −0
PROPOSAL BONUS: none — no new measure proposed this turn
LEGITIMACY LENDS: capital 0.0, sentiment 0.0 -> none, capital not lower
SOVEREIGNTY: 14 last turn, no category 4 finish, no category 4 in flight +0, eu_frontier_access_denied t13 −3, capability rose 0.8 −0 = 11
## Store changes
```json
{"store": []}
```
