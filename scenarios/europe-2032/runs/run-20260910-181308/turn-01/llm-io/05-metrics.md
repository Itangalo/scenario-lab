...
- characters 16339-17022: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_biosecurity_publishing_freeze` -- first noted turn 1, listed in 1 turn(s) so far: A major European scientific publisher and university consortium imposes a coordinated moratorium on publishing AI-designed pathogen methods after the phage results, triggering a global dispute over pre-print servers outside EU jurisdiction.
- `emergent_cyber_insurance_pullback` -- first noted turn 1, listed in 1 turn(s) so far: A major insurer reprices cyber cover after the Kimi K3 release, sharply raising premiums or withdrawing cover for municipalities and mid-sized operators, making the open-weight offensive uplift an immediate financial shock.
