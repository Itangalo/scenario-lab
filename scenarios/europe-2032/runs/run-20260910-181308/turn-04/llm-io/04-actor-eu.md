...
- characters 454-2080: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2123-4627: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5197-5707: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7845-10012: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M5 | EU Trust and Transition Repair | 7 | small | 3 | 5 | own jurisdiction | public_sentiment up, eu_political_capital up, resilience up slightly via social absorption | 2 | running |
...
| id | name | category | size | started_turn | finish_turn | applies_to | targeted_effect | cost_per_turn | status |
|---|---|---|---|---|---|---|---|---|---|
| M3 | EU Critical Systems Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up materially, ai_safety up slightly via incident reporting | 3 | finished |
| M4 | EU Incident Containment Reserve | 6 | small | 2 | 4 | own jurisdiction | resilience up, ai_safety up slightly via containment and reporting | 2 | finished |
...
## New measure
**EU Trust and Transition Repair**
Creates fast redress for the benefits/policing AI scandal with ombudsman-led compensation plus wage-insurance and retraining pilots tied to firms showing productivity gains without layoffs.
Category 7 Labour and social protection, because with sentiment at 25 and capital at 24 we cannot harden or build anything if the public blocks siting and enforcement, and the augmented-work evidence gives us a window to pair redress with transition before displacement hits.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trust and Transition Repair", "category": 7, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, eu_political_capital up, resilience up slightly via social absorption"}, "grounds": "automated decision scandal plus low trust blocking Shield and sovereignty build"}]}
```

## Priority
EU Incident Containment Reserve, not the new Repair, because the largely automated attack wave is active now and without visible containment the redress promise reads as words while services stay down.

## In practice
We run the Shield as finished audits into enforced fixes: Commission implementing acts under NIS2/CER oblige the audited TSOs and health funds to close top findings by year-end, funded by CEF reallocation, with wastewater reporting via HERA grants to blunt insurer repricing. ENISA's Reserve gets its first 24h roster from seconded national CERTs, with pre-authorised isolation playbooks exercised with two cloud providers on EU soil.

We launch the Repair via Employment/Social Affairs Council and the AI Act Article 70 redress route: ESF+ pilots for wage insurance and short retraining in the five most-augmented occupations, conditional on social-partner agreements not to cut headcount early, plus a dedicated benefits-scandal redress desk co-funded with member states to show the AI Act can remedy, not just fine. Gigafactories and Tech sovereignty permitting continue on accelerated zones with no new cash push this turn to conserve capital under Taiwan-driven energy and shipping pressure.
...
```
## New measure
**ASML Sovereignty Shield**
Uses the Anti-Coercion Instrument and joint export-control alignment with the Netherlands and Japan to block extraterritorial servicing bans on lithography kit and to condition any further EU cooperation on Washington not using EU chokepoints without EU consent.
Category 8 International coordination and leverage, because Washington is spending our one chokepoint and with capital at 14 and sentiment at 17 we cannot buy sovereignty with cash — only with coordinated leverage that defends Gigafactories supply and proves the two-year commitment to run without foreign permission.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "ASML Sovereignty Shield", "category": 8, "size": "small", "finish_turn": 7, "applies_to": "own jurisdiction", "targeted_effect": "eu_ai_sovereignty up slightly, eu_political_capital up slightly via leverage"}, "grounds": "US coercion of ASML beyond leading-edge to older nodes"}]}
```

## Priority
M5 EU Trust and Transition Repair, not the new Shield, because with sentiment at 17 and payouts stalled after the benefits-scandal ruling nothing — not Shield fixes, not ASML talks — can be enforced or sited if the public blocks it.

## In practice
We keep M1 Gigafactories and M2 Tech sovereignty package alive on permitting and private-capital tracks only, with no new cash call this turn to avoid breaking cohesion under Taiwan shipping/energy strain, while tasking DG TRADE and DG CNECT to prepare the Anti-Coercion examination on US extraterritorial servicing restrictions and to align a common servicing-licence position in the Foreign Affairs Council (Trade).

We push M5 Repair as the visible delivery: ESF+ reallocation via the Employment Council for a benefits redress desk with the ombudsman and member-state ministries, plus wage-insurance and short-course pilots in the five most-augmented office occupations conditional on no-early-layoff agreements, to turn finished Shield audits into trust before hardening is re-funded. ENISA's finished Reserve roster is held on 24h secondment and exercised with the two clouds, with local teams still carrying response while we renegotiate staffing.
