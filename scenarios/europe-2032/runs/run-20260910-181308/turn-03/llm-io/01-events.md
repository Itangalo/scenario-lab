...
- characters 377-1490: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1534-4110: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4166-5089: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5152-27864: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28219-28438: `{{event_history}}` from the run's own event record
...
- `emergent_biosecurity_publishing_freeze` -- first noted turn 1, listed in 2 turn(s) so far: A major European scientific publisher and university consortium imposes a coordinated moratorium on publishing AI-designed pathogen methods after the phage results, triggering a global dispute over pre-print servers outside EU jurisdiction.
...
- Turn 2 (1 turn(s) ago): cyber_major_incident, loss_of_control_incident, bio_uplift_findings, emergent_cyber_insurance_pullback
