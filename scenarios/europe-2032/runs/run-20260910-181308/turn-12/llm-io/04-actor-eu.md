...
- characters 452-2007: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2050-4026: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5624-6127: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8989-10999: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Survive the open-weight-enabled incident wave by keeping essential services running and preventing fragmentation into side deals
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Essential Services Keep-Running Cells | 6 | small | 11 | 13 | own jurisdiction | resilience up, public_sentiment up slightly, eu_political_capital up slightly via visible delivery | 2 | running |
...
| M5 | EU Trust and Transition Repair | 7 | small | 3 | 5 | own jurisdiction | public_sentiment up, eu_political_capital up, resilience up slightly via social absorption | 2 | finished |
| M6 | ASML Sovereignty Shield | 8 | small | 4 | 7 | own jurisdiction | eu_ai_sovereignty up slightly, eu_political_capital up slightly via leverage | 2 | finished |
| M7 | Essential Services Continuity Pact | 6 | small | 5 | 7 | own jurisdiction | resilience up materially, public_sentiment up slightly, eu_political_capital up slightly via visible delivery | 2 | finished |
| M8 | EU Joint Threat Response Accession | 6 | small | 7 | 9 | coalition | resilience up materially via pooled telemetry | 2 | finished |
...
## New measure
**EU Essential Services Keep-Running Cells**
Keeps clinics, town halls and grid operators running through blackouts and mutating extortionware via pre-positioned islandable power, clean backups and travelling ENISA repair teams.
This is category 6 Preparedness and resilience, small and fundable at capital 4, because the February sweep plus July heat blackouts plus frontier open weights landing together is exactly the incident wave our two-year commitment names, and visible keep-running delivery is the only way to rebuild sentiment from 1.0 and stop further side deals.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Essential Services Keep-Running Cells", "category": 6, "size": "small", "finish_turn": 13, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up slightly, eu_political_capital up slightly via visible delivery"}, "grounds": "blackouts plus open-weight cyber wave demands funded continuity now"}]}
```

## Priority
No priority measure in flight this turn, because the single new Cells measure is just starting and priority will attach once staffing begins — this turn all effort is launch.

## In practice
We task ENISA operations room and HERA as joint leads under the Civil Protection Mechanism, using reprogrammed cohesion advances plus the middle-power pooled telemetry feed to push clean images, isolation guides and diesel-plus-battery island kits to the municipalities hit hardest in February and the two blackout states. No new Gigafactory promise; we say plainly shells stay empty while we keep lights and appointments on.

We pair this with quiet bio-gap mapping: health ministries share detection protocols under the May coordination framework, no funds committed yet, and we do not retaliate on the autumn side deal this turn to avoid spending the 4.0 capital we need to launch the Cells. Mayors get redress desks and seconded engineers first if they stay inside the common feed.
...
```
## New measure
None this turn, because with political capital at 2.0 and a global AI-designed pandemic putting everything else on hold we cannot fund a second instrument alongside the Keep-Running Cells without collapsing delivery.

## Store changes
No changes.

## Priority
M9 EU Essential Services Keep-Running Cells, because it is the only funded delivery vehicle left and in pandemic mode keeping clinics, power-islanded care sites and clean backups running is what the two-year commitment to keep essential services running means.

## In practice
We put the Union into pandemic mode under HERA and the Civil Protection Mechanism: M9's islandable power kits, clean images and ENISA travelling repair teams are retasked first to hospitals, labs and appointment systems in the two blackout states and the February-fallback municipalities, paid from already-reprogrammed cohesion advances — no new money vote we cannot pass.

We seek observer inclusion in the US-China bio-design accord on weights security and biological design tools, offering pooled EU clinical and detection telemetry from M8's middle-power feed and accepting allied-licence export-control terms to keep therapeutic compute and detection reagents flowing; we do not launch any sovereignty or industrial build while sentiment is at 2.0 and capital is at 2.0. Public messaging is triage-only: what stays open, where to get care, no new promises.
