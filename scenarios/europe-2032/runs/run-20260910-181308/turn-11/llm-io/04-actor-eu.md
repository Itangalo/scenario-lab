...
- characters 453-1540: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1584-3860: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5801-6304: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8808-11305: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_grid_strain_blackouts (emergent event):** Prolonged heatwave plus throttled data-centre power and postponed grid maintenance trigger rolling blackouts in two member states, blamed publicly on AI electricity demand and US quota throttling.
