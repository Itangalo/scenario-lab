...
- characters 169-358: `{{metrics_json}}` from the run's live metric values
- characters 451-1545: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1588-3941: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5461-5917: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8572-10393: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_tiered_access_selloff (emergent event):** A major US lab announces tiered foreign pricing and throttled inference speeds for lower-tier allies, with contract terms leaking early and triggering pre-emptive hoarding and resale of API quotas by European intermediaries.
...
|---|---|---|---|---|---|---|---|---|---|
| M8 | EU Joint Threat Response Accession | 6 | small | 7 | 9 | coalition | resilience up materially via pooled telemetry | 2 | running |
...
Joins the pooled cyber telemetry / biosurveillance pact as invited third party, plugging ENISA and HERA into real-time sharing and joint investigation.
Joining pooled protection we could not build alone is the only resilience gain available with capital at 1 and sentiment at 3, and answers the access cut and verifiable-domain capability jump by hardening without new builds.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Joint Threat Response Accession", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "coalition", "targeted_effect": "resilience up materially via pooled telemetry"}, "grounds": "invited to joint command after autumn attack and access denial"}]}
```

## Priority
M8 EU Joint Threat Response Accession, because with no in-flight measures costing capital and continuity pact finished, externally pooled hardening is the only deliverable that does not require a vote or cash we lack.

## In practice
We mandate ENISA as single EU node to the joint cyber command and HERA/ECDC to the biosurveillance sample-sharing arrangement, via Council JHA/Health conclusions rather than a new regulation to avoid a co-decision fight. Seconded analysts from the finished Containment Reserve roster staff the feed, funded by reprogrammed Civil Protection Mechanism operating lines.

We accept tiered-access reality for now: no retaliation on lithography servicing or Anti-Coercion vote, continued Tokyo talks and permitting hold on Gigafactories, while hospitals and ministries cut off from the US frontier model are shifted to EU-hosted fallbacks and pooled telemetry rules. Redress desks stay open to contain blockades and benefits-scoring protests until pooled detection visibly shortens outages.
...
```
## New measure
None this turn — waiting for political capital to recover above paralysis and for M8 pooled telemetry to finish landing before starting any new build.

## Store changes
No changes.

## Priority
M8 EU Joint Threat Response Accession, because with capital at 4 and sentiment at 1 the only deliverable that hardens without a vote, new cash, or US permission is the already-mandated pooled cyber/bio feed through the major automated incident.

## In practice
We execute M8 as emergency triage, not accession paperwork. ENISA as single EU node takes pooled signatures and isolation playbooks from the joint cyber command into hospitals, ministries and municipal networks hit by the model-generated ransomware/dependency sweep, using the Continuity Pact offline backups and the Containment Reserve roster for seconded analysts funded from reprogrammed Civil Protection lines. HERA runs shared biosurveillance sampling in parallel. No new regulation, JHA/Health Council conclusions only.

We absorb the open-weight frontier jump and US tiered foreign pricing/throttling as unrecallable facts: no Anti-Coercion vote, no servicing curbs, no retaliation for the late-summer cutoff. Throttled US-dependent workloads stay on slower EU-hosted fallbacks, API-quota hoarding/resale by intermediaries is tolerated and monitored rather than banned to avoid a further supply stop, Tokyo talks and Gigafactory fit-out permits continue. Redress desks and benefits-protest mediation stay open to hold what little cohesion remains until pooled detection visibly shortens outages.
