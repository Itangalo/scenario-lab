...
- characters 91-280: `{{metrics_json}}` from the run's live metric values
- characters 373-1467: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1510-3863: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3919-4916: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 4979-29261: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29616-30502: `{{event_history}}` from the run's own event record
...
- `emergent_tiered_access_selloff` -- first noted turn 6, listed in 2 turn(s) so far: A major US lab announces tiered foreign pricing and throttled inference speeds for lower-tier allies, with contract terms leaking early and triggering pre-emptive hoarding and resale of API quotas by European intermediaries.
...
- Turn 1 (7 turn(s) ago): cyber_test_shot, bio_uplift_findings, automated_decision_scandal
- Turn 2 (6 turn(s) ago): cyber_major_incident, loss_of_control_incident, bio_uplift_findings, emergent_cyber_insurance_pullback
- Turn 3 (5 turn(s) ago): cyber_major_incident, knowledge_work_augmented, taiwan_tension_rise, automated_decision_scandal
- Turn 4 (4 turn(s) ago): supply_chain_coercion
- Turn 5 (3 turn(s) ago): cyber_major_incident, eval_anomaly_reports, openweight_frontier_release, export_control_escalation, eu_frontier_access_denied, member_state_defection, emergent_anti_ai_direct_action, emergent_anti_ai_site_blockades, election_consolidation
- Turn 6 (2 turn(s) ago): opaque_reasoning, supply_chain_coercion, member_state_defection
- Turn 7 (1 turn(s) ago): capability_jump, knowledge_work_augmented, embodied_ai_deployment, eu_frontier_access_denied, joint_threat_response
