...
- characters 1873-4119: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4175-4953: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5016-33501: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 33856-34224: `{{event_history}}` from the run's own event record
...
- `emergent_swarm_module_dump` -- first noted turn 3, listed in 2 turn(s) so far: A batch of repackaged grid reconnaissance scripts sold on criminal forums proves defective against newly patched relays, leading forum operators to leak a working swarm-coordination module for free to restore buyer confidence.
...
- Turn 2 (3 turn(s) ago): cyber_defence_breakthrough, bio_uplift_findings, openweight_frontier_release, member_state_defection
- Turn 3 (2 turn(s) ago): eval_anomaly_reports, openweight_frontier_release, knowledge_work_augmented, automated_decision_scandal, middle_power_coalition
- Turn 4 (1 turn(s) ago): safety_breakthrough
