...
- characters 13531-14862: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14906-16977: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17033-18163: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Critical Systems Shield | 3 |
| M5 | EU–Middle-Power Supply-Chain Coordination Framework | 2 |
...
- `emergent_grid_tooling_leak` -- first noted turn 2, listed in 2 turn(s) so far: Stolen grid reconnaissance tooling derived from the autumn intrusion is repackaged and offered on criminal forums, putting industrial-system swarm capability in the hands of non-state actors beyond the original operators.
- `emergent_swarm_module_dump` -- first noted turn 3, listed in 1 turn(s) so far: A batch of repackaged grid reconnaissance scripts sold on criminal forums proves defective against newly patched relays, leading forum operators to leak a working swarm-coordination module for free to restore buyer confidence.
