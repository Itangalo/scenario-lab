...
- characters 455-1786: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1830-3901: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 7674-8114: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9736-11683: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Critical Systems Shield | 6 | large | 2 | 4 | own jurisdiction | resilience up materially, ai_safety up modestly via reduced incident impact | 3 | running |
...
Deploys automated swarm-detection and rapid patching across transmission grids and port systems plus upgraded bio-detection nodes, using the defensive breakthrough to harden what the autumn intrusion walked through. This is category 6 Preparedness and resilience, and why now is because the grid intrusion plus contested bio-uplift signal means resilience must land before the next open-weight release makes both attack surfaces unrecallable.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Critical Systems Shield", "category": 6, "size": "large", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, ai_safety up modestly via reduced incident impact"}, "grounds": "to close grid and bio detection gap exposed this turn"}]}
```

## Priority
EU Critical Systems Shield, because the swarm that mapped breakers without breaking anything will not wait for gigafactories to pour concrete, and the defensive window closes fast.

## In practice
We task ENISA with ENTSO-E and national TSOs to roll emergency segmentation, credential rotation and swarm-behaviour monitoring to the two hit states first, funded via CEF-Energy repurposing and justified under NIS2 Article 11, buying utilities' cooperation with co-paid patching tooling from the breakthrough. HERA gets the bio-sentinel uplift: sequencing-based detection pilots in major hubs tied to the genome-model warning.

We keep InvestAI Gigafactories (M1) and Tech sovereignty package (M2) moving through DG COMP state-aid and Council Competitiveness by letting France-Germany-Spain-Sweden-Poland keep site horse-trading while we quietly ring-fence grid power for data-centre zones, and we push the Evaluation Institute (M3) hiring despite Washington delays and the member-state side-deal, framing defection as the reason EU vetting cannot depend on US access.
...
```
## New measure
**EU–Middle-Power Supply-Chain Coordination Framework**
Stands up a joint export-licence, compute-access bargaining and shared-evaluation cell with supply-chain middle powers to stop bilateral undercutting and secure access on common terms. This is International coordination and leverage, and why now is because the member-state side-deal plus the frontier open-weight release have shown the Union cannot hold a line alone while attack code and capability leak faster than shields land.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU–Middle-Power Supply-Chain Coordination Framework", "category": 8, "size": "small", "finish_turn": 5, "applies_to": "coalition", "targeted_effect": "eu_political_capital up, eu_ai_sovereignty up modestly via secured access"}, "grounds": "to hold common line after side-deal and frontier open release"}]}
```

## Priority
EU–Middle-Power Supply-Chain Coordination Framework, because without a joint line to stop bilateral undercutting and secure access, the Shield has nothing to run on and the gigafactories have nothing to bargain with — restoring cohesion outranks even patching this turn.

## In practice
We task DG TRADE and DG CNECT with a small secretariat mandate under the Anti-Coercion Instrument logic, taking export-licence alignment and joint hyperscaler terms to the Foreign Affairs Council (Trade) and inviting Japan, Korea, Netherlands-relevant ASML equities, and others holding compute, lithography and energy chokepoints. The offer to the defecting large member state is re-entry on better joint compute-access prices, not censure.

We keep the Critical Systems Shield (M4) and Evaluation Institute (M3) running through ENISA/ENTSO-E pilots and JRC hiring, using the automated-decision scandal — a high-risk Annex III deployment where human oversight collapsed to 40-second approvals — as enforcement failure to be fixed by audit of logging and oversight, not a rewrite of the Act, while letting M1/M2 site horse-trading continue without new money until capital recovers.
