...
- characters 377-1847: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1891-4511: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4567-5645: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5708-28329: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28684-28863: `{{event_history}}` from the run's own event record
...
- `emergent_research_integrity_pact` -- first noted turn 1, listed in 2 turn(s) so far: Major EU scientific publishers and universities jointly mandate disclosure and limits on AI-generated research after the Millennium Prize claim triggers integrity disputes.
- `emergent_openweight_crime_wave` -- first noted turn 1, listed in 2 turn(s) so far: Major EU press investigation links Kimi K3-derived tooling to a wave of ransomware against mid-sized industrial firms, triggering calls for open-weight liability in the European Parliament.
...
- Turn 1 (2 turn(s) ago): cyber_test_shot, export_control_escalation, eu_access_secured
- Turn 2 (1 turn(s) ago): ai_investment_collapse, eval_anomaly_reports, safety_breakthrough
