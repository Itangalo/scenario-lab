...
- characters 455-1925: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1969-4589: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4657-5133: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7106-9174: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | EU Assurance and Eval Rapid Uptake | 5 | small | 2 | 4 | own jurisdiction | ai_safety up materially, resilience up slightly via assured deployment | 2 | running |
...
## New measure
**EU Assurance and Eval Rapid Uptake**
Stands up EU evaluation capacity to reproduce the leaked eval anomaly and rapidly certify and deploy the new interpretability/control breakthrough onto EU-deployed frontier systems.
This is category 5 Public technical capacity and research, because the anomaly precursor and the safety breakthrough both demand independent EU testing and assurance capability now, while the investment collapse freezes private build-out.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Assurance and Eval Rapid Uptake", "category": 5, "size": "small", "finish_turn": 4, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up materially, resilience up slightly via assured deployment"}, "grounds": "eval anomaly plus deployable safety breakthrough during investment freeze"}]}
```

## Priority
EU Critical Infrastructure Cyber Shield, not the new eval uptake, because the swarm that chose not to act still sits in grid and port OT and exercises in early 2027 are the binding constraint.

## In practice
We keep the Cyber Shield as the main effort: ENISA with DG ENER and DG MOVE pushes the NIS2/CER hardening standards to delegated-act stage, CEF-Digital and InvestEU money flows to TSOs and port operators for segmentation and swarm detection, and CyCLONe runs the cross-border exercises this spring. Co-financing holds operator resistance, while interior and energy Councils carry the mandate.

In parallel we task the AI Office and ENISA Joint Cyber Unit, with JRC and national AI Safety Institutes, to reproduce the anomalous eval on bridge frontier models under our evaluation-rights access and to certify the interpretability breakthrough for EU public-sector and critical-sector deployments. We protect the Gigafactories and Tech sovereignty package tracks — permitting zones, grid connections, ASML coordination — by ring-fencing committed EU funds and renegotiating phased private commitments after the valuation reset, without opening a second compute programme.
...
```
## New measure
**EU Hardening Standards Enforcement and Continuity**
Puts the spring-exercise lessons into binding delegated acts under NIS2/CER and funds backup continuity for transmission and port operators to lock in segmentation gains.
This is Preparedness and resilience work, because the dormant probes plus investment freeze mean exercised defences will decay unless standards become enforceable and continuity is funded now.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Hardening Standards Enforcement and Continuity", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially"}, "grounds": "lock in exercise gains before retrenchment erodes them"}]}
```

## Priority
M4 EU Assurance and Eval Rapid Uptake, because the interpretability lead and eval anomaly are a closing window to certify before capability jumps again, outranking even the gigafactory life-support this turn.

## In practice
We keep M1 InvestAI Gigafactories and M2 Tech sovereignty package on life-support: DG CNECT and DG ENER use ring-fenced EU funds for permitting zones and grid connections, renegotiate phased private co-financing with EIB/InvestEU cover, and hold volume licences as political cover while build-out slows. No new compute line is opened.

In parallel we push M4 hard: AI Office with JRC and national safety institutes reproduces the anomalous benchmark on bridge models under evaluation-rights access and starts formal certification testing of the interpretability/control technique for public and critical-sector deployment. ENISA and DG ENER/MOVE carry the new enforcement measure through the Council energy and transport formations toward delegated acts, directing CEF-Digital money to TSOs and ports for segmentation maintenance and continuity backup.
