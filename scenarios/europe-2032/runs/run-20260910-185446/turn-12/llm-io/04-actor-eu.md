...
- characters 455-2643: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2686-4665: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5312-5772: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9188-11192: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_gulf_compute_financing_offer (emergent event):** A Gulf sovereign fund publicly offers to finance EU gigafactory build-out and secure inference capacity in exchange for hosted capacity and energy partnerships, forcing a sovereignty-versus-dependence debate.
...
- `two_year_commitment` (commitment): Keep essential services running and displaced workers supported through AI disruption
...
|---|---|---|---|---|---|---|---|---|---|
| M11 | EU Cyber-Bio Containment and Essential Service Recovery Surge | 6 | small | 11 | 13 | own jurisdiction | resilience up materially, eu_political_capital up if services recover, public_sentiment up slightly | 2 | running |
...
| M4 | EU Assurance and Eval Rapid Uptake | 5 | small | 2 | 4 | own jurisdiction | ai_safety up materially, resilience up slightly via assured deployment | 2 | finished |
| M5 | EU Hardening Standards Enforcement and Continuity | 6 | small | 3 | 6 | own jurisdiction | resilience up materially | 2 | finished |
| M6 | EU Supply-Chain Leverage Coalition | 8 | small | 5 | 8 | coalition (NL, JP, KR plus EU jurisdiction) | eu_ai_sovereignty up materially, eu_political_capital down then up | 2 | finished |
| M7 | EU Essential-Service AI Continuity Reserve | 6 | small | 6 | 8 | own jurisdiction | resilience up materially, eu_ai_sovereignty up slightly via fallback use | 2 | finished |
| M8 | EU Bridge to the US-China Safety Accord | 8 | small | 8 | 9 | coalition (EU plus US, China, NL, JP, KR) | ai_safety up materially, eu_ai_sovereignty up slightly via insider access, eu_political_capital up if admitted | 2 | finished |
| M9 | EU Essential-Service Fallback Repair and Care Liability Pact | 6 | small | 9 | 10 | own jurisdiction | resilience up materially, eu_political_capital up if outages stop, public_sentiment up slightly | 2 | finished |
| M10 | EU Youth Entry and Care Transition Guarantee | 7 | small | 10 | 12 | own jurisdiction | public_sentiment up materially, eu_political_capital up if hiring resumes, resilience up slightly via staffed fallback | 2 | finished |
...
## New measure
**EU Cyber-Bio Containment and Essential Service Recovery Surge**
Runs emergency patching, mutual-aid recovery teams and exercised fallback for the ransomware sweep, plus accelerated bio-detection sampling tied to the genome-model uplift signal.
Why this and why now: the automated cyber incident is degrading the wards and ports the fallback standard just repaired, and the bio precursor opens a four-turn gate — without a visible containment surge the continuity repair and youth-staffed cover collapse politically now.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Cyber-Bio Containment and Essential Service Recovery Surge", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, eu_political_capital up if services recover, public_sentiment up slightly"}, "grounds": "major automated cyber incident plus bio uplift precursor demand immediate containment"}]}
```

## Priority
EU Cyber-Bio Containment and Essential Service Recovery Surge, because the cyber sweep hitting repaired hospitals and the ASML coercion fight both outrank hiring pace this turn — without services staying up the two-year commitment fails first.

## In practice
We execute through ENISA-led cyber mutual aid with the Cyber Shield operations centres, DG SANTE/HERA for hospital continuity and bio-sampling, and DG HOME for incident reporting under NIS2: reprogrammed Digital Europe and Civil Protection funds pay recovery teams, mandated fallback drills in affected municipalities, and ECDC wastewater and screening uplift.

We keep M10 Youth Guarantee staffing the manual cover and recovery desks, with late reimbursements fixed via front-loaded ESF+ advances, and we hold the Netherlands line — no further servicing concessions without Anti-Coercion joint assessment — while Washington uses jurisdiction to force cuts, framing recovery as proof the Union protects its own wards first.
...
```
## New measure
**EU Graduate Entry Re-hiring Bridge**
Pays wage-subsidised entry posts and rapid retraining for displaced graduates in law, accountancy, software, customer ops and admin, tied to fallback staffing.
Why this and why now: entry hiring has frozen and the Youth Guarantee finished without restoring hiring, so the two-year commitment to keep displaced workers supported fails unless a funded bridge replaces make-work criticism with paid posts now.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Graduate Entry Re-hiring Bridge", "category": 7, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up materially, eu_political_capital up if hiring resumes, resilience up slightly via staffed fallback"}, "grounds": "entry-level hiring freeze across named sectors amid finished youth guarantee"}]}
```

## Priority
M11 EU Cyber-Bio Containment and Essential Service Recovery Surge, because wards and ports hit by the automated sweep must stay up first or neither the re-hiring bridge nor any Gulf deal has political ground to stand on.

## In practice
We run M11 through ENISA mutual-aid teams, DG CNECT and DG HOME under NIS2, and HERA/ECDC for hospital drills and wastewater sampling, using reprogrammed Digital Europe and Civil Protection money; DG EMPL front-loads ESF+ advances to finally pay youth-staffed recovery desks. We hold the Hague line politically while Washington forces servicing cuts, preparing an Anti-Coercion assessment track without conceding further machines this turn.

We launch the Re-hiring Bridge via EPSCO and DG EMPL with ESF+ and employer co-funding, offering wage insurance to firms that re-open entry posts and paid fallback roles in municipalities and hospitals. On the Gulf gigafactory offer we negotiate terms only — no parallel build decision this turn — demanding EU legal anchoring, energy conditionality and hosted-capacity control before any acceptance.
