...
- characters 107-12908: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12951-13143: `{{metrics_json}}` from the run's live metric values
- characters 13236-14530: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14574-16267: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16323-17381: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M10 | EU Youth Entry and Care Transition Guarantee | 2 |
| M11 | EU Cyber-Bio Containment and Essential Service Recovery Surge | 2 |
...
## Store changes
```json
{"store": [{"op": "update", "table": "measures", "id": "M11", "fields": {"finish_turn": 13}, "grounds": "named priority"}]}
```
```
