...
- characters 376-2728: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2771-5370: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5426-6325: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 6388-27627: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 27982-28233: `{{event_history}}` from the run's own event record
...
- `emergent_hardening_decay_backlash` -- first noted turn 4, listed in 2 turn(s) so far: Municipal utilities quietly defer credential rotation and detection staffing over winter, and a local press investigation frames EU hardening funds as paying for audits while defences decay.
...
- Turn 2 (4 turn(s) ago): ai_investment_collapse, eval_anomaly_reports, safety_breakthrough
