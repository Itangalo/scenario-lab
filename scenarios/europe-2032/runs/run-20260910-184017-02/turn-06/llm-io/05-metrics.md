...
- characters 105-13401: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13444-13636: `{{metrics_json}}` from the run's live metric values
- characters 13729-15467: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15510-17596: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17652-18103: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M7 | EU Allied Shield Accession | 2 |
| M8 | EU Chokepoint Bargain | 2 |
...
PORTFOLIO CHARGE t5: M1 InvestAI Gigafactories −3, M2 Tech sovereignty package −3, M7 EU Allied Shield Accession −2, priority −1 = −9
PROPOSAL BONUS: EU Allied Shield Accession (cat 8, small) +3 — cyber_major_incident landed this turn and this answers it directly via joint telemetry
LEGITIMACY LENDS: none — capital 36, sentiment 32, capital higher
SOVEREIGNTY: 19 last turn, no capacity event, Gigafactories in flight +1, capability rose 2.0 −1 = 19
...
## Store changes
```json
{"store": [{"op": "update", "table": "standing", "id": "S1", "fields": {"posture": "ALLIANCE"}, "grounds": "turn-5 election won by coalition-minded presidency, taking office turn 6"}]}
```
```
