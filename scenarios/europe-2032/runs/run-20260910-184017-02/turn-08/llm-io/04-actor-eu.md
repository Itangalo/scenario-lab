...
- characters 454-1613: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1656-4027: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6373-6814: `{{statement_ledger}}` from the actor's live statement ledger
- characters 9500-11450: `{{previous_actions}}` from the actor's own response from the previous turn
...
|---|---|---|---|---|---|---|---|---|---|
| M9 | EU Containment Surge | 6 | small | 7 | 9 | own jurisdiction | resilience up substantially, ai_safety up modestly | 2 | running |
...
## New measure
**EU Containment Surge**
A small Category 6 Preparedness and resilience measure that puts cross-border cyber response teams, hospital/grid isolation and backup procedures, and Bio-Sentinel sampling on emergency footing to absorb the automated attack and the bio-uplift signal through the investment freeze.
Chosen now because the major incident makes resilience the binding constraint on both mandates, and the investment collapse means we must hold what we have rather than buy new capacity this turn.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Containment Surge", "category": 6, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, ai_safety up modestly"}, "grounds": "automated major cyber incident plus bio-uplift precursor during investment collapse"}]}
```

## Priority
EU Containment Surge, because public services visibly failing would collapse the capital needed for both the chokepoint bargain and sovereign capacity, and containment must outrank direction this turn.

## In practice
We run the Surge through the Health Emergency Preparedness and Response Authority and ENISA under the Critical Shield framework already finished: emergency procurement extended for the patching toolkit, mandatory isolation drills in hospitals and transmission operators, and EU-CyCLONe mutual-aid teams deployed to the worst-hit member states. Bio-Sentinel wastewater and clinical sampling is raised to weekly reporting while the genome-model claim is triaged.

We hold M8 on its track in parallel without new concessions: DG TRADE maintains the freeze on further ASML servicing cuts pending a joint control list, linking any alignment to written access quotas, while the gigafactory hosts are told concrete poured stays funded and injunctions are fought in court — no new compute money is promised until valuations settle.
...
```
## New measure
**EU Lithography Shield**
A small Category 8 International coordination and leverage response that activates the Anti-Coercion Instrument over ASML servicing, blocks extraterritorial servicing cuts without a jointly agreed list and written quotas, and pools a member-state compensation and legal-defence fund to hold the common line after the defection.
Chosen now because Washington is spending our one chokepoint and a member state is breaking ranks — without a joint shield the Hague freeze collapses and sovereign capacity loses its only leverage.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Lithography Shield", "category": 8, "size": "small", "finish_turn": 10, "applies_to": "US and coalition", "targeted_effect": "eu_ai_sovereignty up modestly, eu_political_capital up or down depending on outcome"}, "grounds": "US coercion of ASML servicing plus member-state defection undercutting EU line"}]}
```

## Priority
EU Lithography Shield, because if ASML servicing is dictated from Washington and defections go unanswered, neither containment nor gigafactory capacity can stand alone — holding the chokepoint outranks even the ongoing Surge this turn.

## In practice
We run the Shield through DG TRADE and the Foreign Affairs Council: opening an Anti-Coercion examination of the US servicing demand, adopting an implementing act requiring EU authorisation for servicing cuts on lithography tools beyond the jointly agreed list, and tying any future alignment strictly to published volume licences and access quotas for EU fabs and gigafactories. The defecting capital is offered a choice in COREPER — rejoin the pooled quota and share in power-price guarantees for the second gigafactory site, or face exclusion from InvestAI capacity allocation.

We keep the Containment Surge (M9) running in parallel without new money: ENISA and HERA continue patching-toolkit deployment and weekly Bio-Sentinel sampling after the ransomware sweep and genome-model claim, while the grid-ready gigafactory is sweated for inference and sovereign workloads to show autonomy dividends while valuations are frozen. No new compute build is promised this turn; the fight is to keep what we have switchable only in Brussels.
