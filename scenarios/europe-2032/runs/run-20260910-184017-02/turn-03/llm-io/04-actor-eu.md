...
- characters 2002-4179: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4822-5264: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7230-9214: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_ai_fraud_wave (emergent event):** A coordinated campaign uses Kimi K3-derived voice and video clones for large-scale fraud against EU banks and public administrations in autumn 2026, causing tens of millions in losses and forcing emergency authentication changes.
**emergent_lab_whistleblower_leak (emergent event):** A leading US lab researcher publicly leaks internal documents showing frontier safety testing was curtailed before the June access shutoff, triggering congressional hearings and press scrutiny on both sides of the Atlantic.
...
## New measure
**EU Verification Bridge to the US-China Accord**
Seek formal EU inclusion/observer status in the US-China risk agreement and offer EU verification assets — model evaluation capacity, weight-security audits, and cyber-defence tooling — as the EU contribution.
Category 8 International coordination and leverage, chosen now because the accord is the one lever that slows capability growth and exclusion would leave the EU rule-taker on weights security and bio-design controls.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Verification Bridge to the US-China Accord", "category": 8, "size": "small", "finish_turn": 4, "applies_to": "US, China and middle-power coalition", "targeted_effect": "ai_safety up, eu_ai_sovereignty up modestly via inclusion"}, "grounds": "limited window to join US-China agreement before terms harden"}]}
```

## Priority
EU Verification Bridge to the US-China Accord, because being informed afterwards instead of inside would lock in dependence for the whole commitment period, outranking even Critical Shield this turn.

## In practice
We mandate EEAS and DG CNECT to seek annex accession in the weights-security and bio-design pillars, offering AI Office evaluations and ENISA-deployed defensive patching/detection stack proven this turn as EU in-kind verification. We coordinate via G7 plus Singapore/Seoul middle-power caucus and ASML supply-chain leverage to make exclusion costly, using the Anti-Coercion Instrument posture quietly.

M3 Critical Shield/Bio-Sentinel continues as joint ENISA-HERA operation: co-funded SOCs buy DE/FR/Nordic grid operator compliance on audits, cross-border exercises run, DNA-synthesis screening enforcement starts. M1 Gigafactories and M2 Tech sovereignty package stay on slow DG CNECT/Competitiveness Council track — shortlist and permitting zones advance without fresh cash — while we feed the cyber-defence breakthrough directly into shield hardening.
...
```
## New measure
**EU Trust Shield Against AI Fraud**
Roll out EU-wide verified authentication, digital signatures for trusted sources, and emergency bank/public-admin standards to contain Kimi-derived clone fraud.
Category 9 Diffusion, adoption and public trust, chosen now because the autumn fraud wave is draining tens of millions and public tolerance, and without trusted authentication neither resilience nor adoption can hold.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Trust Shield Against AI Fraud", "category": 9, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "resilience up, public_sentiment up"}, "grounds": "autumn clone-fraud wave forcing emergency auth changes"}]}
```

## Priority
EU Trust Shield Against AI Fraud, because active losses in banks and administrations outrank even gigafactories this turn for holding the absorb-shocks commitment.

## In practice
We task DG CNECT and ENISA with the Trust Shield via eIDAS2 wallet acceleration and ECB/Banking Authority coordination: mandatory liveness and step-up authentication for high-risk transfers, signed official communications, and rapid reimbursement rules to restore confidence. Funding reprogrammed from Digital Europe, with banks co-funding fraud-detection sharing.

M1 Gigafactories and M2 Tech sovereignty package stay on Competitiveness Council track — we push permitting-zone decisions to keep sovereignty alive despite no fresh cash, and use the US-lab whistleblower leak to demand AI Office evaluation access in the Verification Bridge technical groups. Critical Shield SOCs absorb the new defensive patching stack and keep cross-border drills running.
