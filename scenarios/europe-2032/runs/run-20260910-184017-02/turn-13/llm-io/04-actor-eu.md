...
- characters 456-1702: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1746-3445: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4676-5098: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8582-10715: `{{previous_actions}}` from the actor's own response from the previous turn
...
- `two_year_commitment` (commitment): Harden Europe to live with AI it cannot control
...
|---|---|---|---|---|---|---|---|---|---|
| M14 | EU Blockade Continuity and Restoration Corps | 6 | small | 12 | 14 | own jurisdiction | resilience up substantially, eu_ai_sovereignty up modestly | 2 | running |
...
| M3 | EU Critical Shield and Bio-Sentinel | 6 | large | 1 | 3 | own jurisdiction | resilience up substantially, ai_safety up modestly | 3 | finished |
| M4 | EU Verification Bridge to the US-China Accord | 8 | small | 2 | 3 | US, China and middle-power coalition | ai_safety up, eu_ai_sovereignty up modestly via inclusion | 2 | finished |
| M5 | EU Trust Shield Against AI Fraud | 9 | small | 3 | 4 | own jurisdiction | resilience up, public_sentiment up | 2 | finished |
| M6 | EU Cyber-Bio Surge Response | 6 | small | 4 | 5 | own jurisdiction | resilience up substantially, ai_safety up modestly | 2 | finished |
| M7 | EU Allied Shield Accession | 8 | small | 5 | 7 | coalition and US | resilience up substantially, ai_safety up modestly | 2 | finished |
| M8 | EU Chokepoint Bargain | 8 | small | 6 | 8 | US and coalition | eu_ai_sovereignty up, eu_political_capital up or down depending on outcome | 2 | finished |
| M9 | EU Containment Surge | 6 | small | 7 | 9 | own jurisdiction | resilience up substantially, ai_safety up modestly | 2 | finished |
| M10 | EU Lithography Shield | 8 | small | 8 | 9 | US and coalition | eu_ai_sovereignty up modestly, eu_political_capital up or down depending on outcome | 2 | finished |
| M11 | EU Municipal and Hospital Hard Shield | 6 | small | 9 | 11 | own jurisdiction | resilience up substantially, ai_safety up modestly | 2 | finished |
| M12 | EU Breakthrough Shield Rollout | 6 | small | 10 | 11 | own jurisdiction | resilience up substantially, ai_safety up modestly | 2 | finished |
| M13 | EU Black-Box Assurance Initiative | 5 | small | 11 | 13 | own jurisdiction | ai_safety up modestly, resilience up modestly | 2 | finished |
...
## New measure
**EU Blockade Continuity and Restoration Corps**
Keeps essential services and compute running through the chip blockade and major cyber incident by rationing scarce compute, pre-positioning spares, and deploying joint ENISA restoration teams, using the Union's lithography position as bargaining cover.
Why this and why now: the Taiwan blockade turns every AI dependency into a security exposure at the exact moment a model-generated attack has shown defenders behind, so the hardening commitment now means continuity under scarcity, not just patching — and the defensive breakthrough gives us something that actually works to roll out while we ration.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Blockade Continuity and Restoration Corps", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up substantially, eu_ai_sovereignty up modestly"}, "grounds": "answer blockade scarcity and major cyber incident within hardening commitment"}]}
```

## Priority
EU Blockade Continuity and Restoration Corps, because the blockade plus active disruption outranks even black-box assurance — if hospitals and grids stop, there is nothing left to assure.

## In practice
We activate the Civil Protection Mechanism and ENISA rapid units as a standing restoration corps, funded by reprogramming Digital Europe and unspent Shield maintenance money, with a Competitiveness and Energy Council mandate to prioritise hospital, grid and telecom workloads for scarce compute and to push the new auto-patching and swarm-detection stacks to first-hit municipalities.

We pair that at home with quiet external action through the Anti-Coercion Instrument and the middle-power coalition: no formal export ban, but coordinated licensing delays on lithography spares and services to buy continued compute access and shipping insurance relief, telling Washington and Tokyo that Europe will not be picked off while telling member states and mayors that rationing foreign rented compute now is what keeps waiting lists falling later.
...
```
## New measure
**EU Certified Assurance Fast-Track**
Puts the new interpretability/control breakthrough straight onto deployed systems in hospitals, grids and public administration via EU evaluation capacity and joint procurement of certified patches.
Why this and why now: the breakthrough finally gives assurance that works on shipped systems not toy models, and with knowledge-work augmentation showing AI paying off without mass layoffs, fast certification lets us keep benefits while export-control tightening forces us to live with what we already have.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Certified Assurance Fast-Track", "category": 5, "size": "small", "finish_turn": 15, "applies_to": "own jurisdiction", "targeted_effect": "ai_safety up substantially, resilience up modestly"}, "grounds": "deploy safety breakthrough within hardening commitment"}]}
```

## Priority
M14 EU Blockade Continuity and Restoration Corps, because rationed compute and dark hospitals outrank even a real safety gain — assurance only matters if services stay on.

## In practice
We keep the Civil Protection/ENISA restoration corps as the operating spine, using Digital Europe reprogramming to hold priority workloads for hospitals/grids/telecoms and to push auto-patching plus the new certified control properties to first-hit municipalities, with spares pre-positioned and licensing delays on lithography held as quiet cover for compute access.

We pair that with a fast-track mandate to the JRC and ENISA with AI Office support: validate the breakthrough on EU-deployed stacks, publish certification checklists, and tie reimbursement and procurement to certified versions, telling member states and social partners that augmentation gains in law, admin and health fund the retraining and transition that keeps sentiment from collapsing further.
