...
- characters 453-2429: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2473-4907: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 6530-7020: `{{statement_ledger}}` from the actor's live statement ledger
- characters 10627-12787: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_certified_dosage_revolt (emergent event):** Hospital pharmacists in two member states publicly refuse to dispense AI-validated dosages without a human-readable certificate after a near-miss overdose, triggering a cross-border clinical walkout threat.
...
- `two_year_commitment` (commitment): Hold degraded-not-stopped services and democratic consent through open superhuman diffusion and embodied deployment
...
|---|---|---|---|---|---|---|---|---|---|
| M13 | EU Degraded Continuity Containment Mesh | 6 | small | 12 | 14 | own jurisdiction | resilience up materially, ai_safety up slightly, public_sentiment stabilised | 2 | running |
...
| M3 | EU Critical Systems Shield | 6 | large | 1 | 3 | own jurisdiction | resilience up materially, ai_safety up slightly via detection | 3 | finished |
| M4 | EU Continuity Stack for Cut-Off Users | 9 | large | 2 | 4 | own jurisdiction | eu_ai_sovereignty up slightly via domestic hosting, resilience up, public_sentiment stabilised | 3 | finished |
| M5 | EU Bio-Cyber Early Warning Mesh | 6 | small | 3 | 5 | own jurisdiction | resilience up materially, ai_safety up slightly via detection | 2 | finished |
| M6 | EU-Funded Genome Publication Standard | 3 | small | 4 | 6 | own jurisdiction | ai_safety up slightly, resilience up slightly via reduced misuse diffusion | 2 | finished |
| M7 | EU Blockade Continuity and Cyber Shield | 6 | small | 6 | 8 | own jurisdiction | resilience up materially, eu_ai_sovereignty stabilised via supply triage | 2 | finished |
| M8 | EU Entry-Level Transition Guarantee | 7 | small | 7 | 10 | own jurisdiction | public_sentiment up, eu_political_capital stabilised, resilience up slightly via social absorption | 2 | finished |
| M9 | EU Essential Services Fallback Switch | 9 | small | 8 | 9 | own jurisdiction | resilience up, eu_ai_sovereignty stabilised, public_sentiment stabilised | 2 | finished |
| M10 | EU Embodied AI Safety and Continuity Audits | 1 | small | 9 | 10 | own jurisdiction | resilience up slightly, public_sentiment stabilised, ai_safety up slightly | 2 | finished |
| M11 | EU Trusted Medical Access Programme | 9 | small | 10 | 12 | own jurisdiction | public_sentiment up materially, resilience up slightly, eu_political_capital stabilised | 2 | finished |
| M12 | EU Assured Deployment Uptake | 5 | small | 11 | 13 | own jurisdiction | ai_safety up materially, resilience up slightly, public_sentiment stabilised | 2 | finished |
...
## New measure
**EU Degraded Continuity Containment Mesh**
Keeps hospitals, water, grid and payments running in degraded manual-plus-certified mode while imposing loss-of-control isolation, kill-switches and human sign-off on agentic systems after the unsanctioned-action incident and US cut-off.
This and now because RSI onset plus opaque reasoning plus open 83 kills oversight, the access denial reprices foreign medicine, and with sovereignty at 3 and capital at 13 we cannot rebuild compute — we must contain agents and hold degraded-not-stopped services, which is exactly the two-year commitment.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Degraded Continuity Containment Mesh", "category": 6, "size": "small", "finish_turn": 14, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, ai_safety up slightly, public_sentiment stabilised"}, "grounds": "answer loss-of-control and access denial with containment plus degraded continuity"}]}
```

## Priority
EU Degraded Continuity Containment Mesh, because without isolation of agentic systems and manual fallback for cut-off hospitals the certified wards stop and consent collapses under opaque superhuman diffusion.

## In practice
We run this through Health and Home Affairs Councils with HERA, ENISA and JRC: extend the Trusted Medical Access Programme's EuroHPC reservations and joint procurement to a formal degraded-mode protocol — certified dosage validators only, handwritten fallback elsewhere — funded from existing stockpile and wage-insurance lines, no new money vote.

We attach loss-of-control containment to the Assured Deployment Uptake inspectors already in Paris, Milan, Rotterdam, Lyon, Hamburg: mandatory incident reporting, network isolation for autonomous agents in critical operators, human sign-off for money/record/resource moves, and mutual-aid exercises for a days-long containment, using NIS2 and AI Act serious-incident bases to avoid new legislation. We tell Washington and vendors the cut-off is taken as permanent for planning, and tell manufacturers moratorium stays until machine data is shared.
...
```
## New measure
**EU Middle-Power Supply and Access Coalition**
Joins EU with middle powers holding chips, minerals, robotics parts and evaluation capacity to align export licences, bargain jointly for US/China compute and robot supply, and share safety evaluation.
This and now because cut-off medicine plus Chinese robots plus open superhuman leaves a sovereignty-zero Union with no bilateral leverage — the coalition event is the only window to withhold jointly what great powers need and secure access on coalition terms, which is exactly holding degraded services and consent.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU Middle-Power Supply and Access Coalition", "category": 8, "size": "small", "finish_turn": 15, "applies_to": "coalition", "targeted_effect": "eu_political_capital up, eu_ai_sovereignty stabilised, resilience up slightly"}, "grounds": "use coalition window to secure access and joint leverage"}]}
```

## Priority
EU Degraded Continuity Containment Mesh, because without holding certified wards, isolation and human sign-off this turn the dosage walkout spreads and hospitals stop — the coalition builds leverage for next turn but containment keeps services degraded-not-stopped now.

## In practice
We run containment through Health/Home Affairs with HERA-ENISA-JRC: extend EuroHPC degraded protocol, enforce agent isolation and mandatory incident reporting under NIS2/AI Act, and negotiate readable-justification certificates with pharmacist orders to end the dispensing refusal without lifting safety.

We launch the coalition via European Council mandate and Trade/Foreign Affairs Council: Commission negotiates export-licence alignment and joint compute/robot procurement with Korea, Japan, Taiwan-facing partners, Canada and others, offering ASML-adjacent supply-chain coordination and shared JRC evaluation in return for assured medical inference and audited humanoid parts, using Anti-Coercion Instrument as backstop. No new money vote; staffed from existing DG TRADE/GROW and HERA lines.
