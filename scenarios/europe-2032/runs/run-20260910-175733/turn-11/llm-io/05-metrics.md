...
- characters 107-12998: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13041-13231: `{{metrics_json}}` from the run's live metric values
- characters 13324-14886: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14930-17446: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17502-18036: `{{notepad}}` from the Game Master's notepad, carried across turns
...
|---|---|---|
| M11 | EU Trusted Medical Access Programme | 2 |
...
PORTFOLIO CHARGE t10: M11 EU Trusted Medical Access Programme −2, priority −1 not charged (capital below 20, no effect) = −2
PROPOSAL BONUS: EU Trusted Medical Access Programme (cat 9, small) +2 — capability jump landed this turn and medical breakthrough access shock this turn, this answers dependence directly but benefit not yet delivered
LEGITIMACY LENDS: capital 15, sentiment 14 -> none (capital higher)
SOVEREIGNTY: 9 last turn, no cat4 finish, no cat4 in flight +0, export_control_escalation t10 −2, capability rose 4.0 −1 = 6
...
## Metrics
```json
{"ai_capability": 90.0, "openweight_capability": 83.0, "ai_safety": 6.0, "resilience": 36.0, "eu_ai_sovereignty": 3.0, "eu_political_capital": 13.0, "public_sentiment": 17.0}
```
