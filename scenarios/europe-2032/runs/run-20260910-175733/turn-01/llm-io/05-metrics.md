...
- characters 16367-17039: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_transatlantic_trust_fallout` -- first noted turn 1, listed in 1 turn(s) so far: A coordinated investigative leak publishes internal safety memos around the June Fable/Mythos switch-off order, triggering parliamentary hearings in several member states and a short-lived consumer boycott of a US hyperscaler.
- `emergent_publisher_bio_screen` -- first noted turn 1, listed in 1 turn(s) so far: A major European scientific publisher and preprint server impose coordinated mandatory screening and delayed release for AI-assisted genome design papers after the phage results, slowing open dissemination without any state mandate.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 46.0, "ai_safety": 32.0, "resilience": 37.0, "eu_ai_sovereignty": 21.0, "eu_political_capital": 43.0, "public_sentiment": 40.0}
```
