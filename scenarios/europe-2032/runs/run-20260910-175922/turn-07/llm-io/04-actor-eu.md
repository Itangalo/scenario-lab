...
- characters 1748-4271: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4928-5375: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7410-8572: `{{previous_actions}}` from the actor's own response from the previous turn
...
## New measure
None this turn, while political capital sits at 13 and a regional moratorium wave blocks the gigafactory sites M1 must still connect.

## Store changes
No changes.

## Priority
M1 InvestAI Gigafactories, because renegotiating the moratorium-blocked power/water siting is the only way the two-year commitment to un-switch-off-able EU capacity survives Washington tiering.

## In practice
We treat the moratorium wave as a siting renegotiation, not a permitting defeat: Commission energy DG with the Energy Council formation offers host regions TEN-E accelerated-connection status plus EU-funded water-efficiency retrofits for the two blocked backup-control sites, in exchange for lifting pause motions and reopening joint-review siting. No new build line is opened — builders get connections, not labels.

We bank the interpretability breakthrough through ENISA deployment guidance and municipal audit checklists already paid for, without spending capital on a new oversight instrument. Shield kits stay delivered; crews work only where concessions exist, and elsewhere we let mayors own the crates rather than burn our last capital forcing entry.
