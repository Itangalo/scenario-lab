...
- characters 376-1726: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1769-3888: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 3944-4977: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5040-28372: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 28727-29213: `{{event_history}}` from the run's own event record
...
- `emergent_permit_litigation_freeze` -- first noted turn 7, listed in 1 turn(s) so far: A coordinated EU court challenge suspends pending data-centre power permits across additional host regions, extending the siting freeze into a de facto judicial moratorium for the turn.
- `emergent_us_tier_schedule_release` -- first noted turn 7, listed in 1 turn(s) so far: Washington publishes the formal AI compute tier schedule, placing the EU in a conditional middle tier with volume caps and end-use vetting rather than a full cutoff.
...
- Turn 2 (6 turn(s) ago): member_state_defection
- Turn 3 (5 turn(s) ago): openweight_frontier_release, member_state_defection
- Turn 4 (4 turn(s) ago): knowledge_work_augmented
- Turn 5 (3 turn(s) ago): eval_anomaly_reports, safety_breakthrough, election_consolidation
- Turn 6 (2 turn(s) ago): safety_breakthrough, emergent_siting_moratorium_wave
- Turn 7 (1 turn(s) ago): member_state_defection, adoption_success
