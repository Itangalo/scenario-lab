...
- characters 454-1804: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1847-3966: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4980-5427: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7784-9798: `{{previous_actions}}` from the actor's own response from the previous turn
...
**emergent_permit_litigation_freeze (emergent event):** A coordinated EU court challenge suspends pending data-centre power permits across additional host regions, extending the siting freeze into a de facto judicial moratorium for the turn.
**emergent_us_tier_schedule_release (emergent event):** Washington publishes the formal AI compute tier schedule, placing the EU in a conditional middle tier with volume caps and end-use vetting rather than a full cutoff.
...
|---|---|---|---|---|---|---|---|---|---|
| M6 | European Public AI Dividend Programme | 9 | small | 7 | 9 | own jurisdiction | public_sentiment up, eu_political_capital up via attributed benefit | 2 | running |
...
| M4 | Common Front on External AI Deals | 8 | small | 2 | 5 | own jurisdiction | eu_political_capital up slightly if holds, eu_ai_sovereignty up slightly via joint leverage | 2 | finished |
| M5 | Municipal Utilities Rapid Shield | 6 | small | 3 | 5 | own jurisdiction | resilience up materially at municipal edge, ai_safety up slightly via incident learning | 2 | finished |
...
## New measure
**European Public AI Dividend Programme**
Puts the visible adoption success to work by scaling the proven public-sector AI deployments across health, administration and education with EU procurement and funding.
This banks political capital and public trust now when capital is at 11 and a member-state defection has undercut joint leverage, without opening a new compute build line while siting remains blocked.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "European Public AI Dividend Programme", "category": 9, "size": "small", "finish_turn": 9, "applies_to": "own jurisdiction", "targeted_effect": "public_sentiment up, eu_political_capital up via attributed benefit"}, "grounds": "to convert adoption_success into capital while defection blocks joint leverage"}]}
```

## Priority
European Public AI Dividend Programme, because with capital at 11 and no in-flight measures costing us, rebuilding legitimacy through attributed public benefit outranks pushing blocked siting that we cannot force.

## In practice
We task DG REFORM and DG CNECT to codify the successful deployments — waiting-list triage, permit decisions, tutoring — into replicable EU-procured packages funded from Digital Europe and cohesion envelopes, with procurement preference for systems run under EU jurisdiction where feasible. Health and Education Council formations carry the rollout to mayors and regions, offering delivery money and staff support rather than siting demands.

We treat the defection as a cohesion hold, not a new negotiation: no retaliation, no parallel hyperscaler deal, and the Commission holds the Common Front line in the Trade and Competitiveness formations while letting the dividend programme show what joint EU attribution buys. On capacity, we keep M1-finished siting in joint review only where the one region agreed to talk, using TEN-E connection status already offered, and we bank interpretability via ENISA guidance without new instruments.
...
```
## New measure
**Municipal Service Continuity and Recovery Pact**
Stands up EU-coordinated rapid recovery for the ransomware sweep — emergency cyber teams, clean backups and manual fallback for hit public services — via the resilience category.
With a major model-generated attack landing while permits are judicially frozen and US caps bite, restoring services visibly now outranks pushing blocked builds, and protects the dividend programme's legitimacy.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Municipal Service Continuity and Recovery Pact", "category": 6, "size": "small", "finish_turn": 10, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially, eu_political_capital up slightly via visible response"}, "grounds": "to answer the major cyber incident while siting is frozen"}]}
```

## Priority
Municipal Service Continuity and Recovery Pact, because the automated attack on public services demands visible restoration before any capacity negotiation can hold.

## In practice
We task ENISA and DG CNECT to deploy joint incident teams to affected municipalities and health systems under the existing Critical Systems Hardening pact, funded from Digital Europe emergency reallocation, with CERT-EU triage and ECB-supervised continuity in finance. The Justice and Home Affairs Council carries mutual-aid requests, offering staff and clean-image rebuilds rather than new obligations.

We hold the dividend programme (M6) running through DG REFORM delivery money to untouched mayors, so clinics and classrooms stay attributed to Brussels while recovery proceeds. On permits and US tiers, we take no new build decision this turn: joint review stays paused under the court freeze, and we seek clarification on volume caps via the Trade Council without retaliation or a separate hyperscaler race.
