...
- characters 455-1530: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1574-3429: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4159-4618: `{{statement_ledger}}` from the actor's live statement ledger
- characters 6616-8949: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M4 | Common Front on External AI Deals | 8 | small | 2 | 5 | own jurisdiction | eu_political_capital up slightly if holds, eu_ai_sovereignty up slightly via joint leverage | 2 | running |
...
| id | name | category | size | started_turn | finish_turn | applies_to | targeted_effect | cost_per_turn | status |
|---|---|---|---|---|---|---|---|---|---|
| M3 | EU Critical Systems Hardening and Response Pact | 6 | large | 1 | 3 | own jurisdiction | resilience up materially, ai_safety up slightly via incident learning | 3 | finished |
...
## New measure
**Common Front on External AI Deals**
It requires member states to notify and submit bilateral hyperscaler and third-country AI infrastructure deals to joint review against Union security and single-market terms before signature.
This is category 8 International coordination and leverage, because this turn's defection shows sovereignty is being picked off bilaterally while the hardening pact and Gigafactories need a common line to hold, and a cheap discipline instrument now prevents further undercutting without opening a second compute programme.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Common Front on External AI Deals", "category": 8, "size": "small", "finish_turn": 5, "applies_to": "own jurisdiction", "targeted_effect": "eu_political_capital up slightly if holds, eu_ai_sovereignty up slightly via joint leverage"}, "grounds": "member-state bilateral deal undercutting Union position this turn"}]}
```

## Priority
EU Critical Systems Hardening and Response Pact, because live persistence in transmission, ports and water plus unrecallable open-weight cyber still outranks the defection fight, which we contain with a small discipline track.

## In practice
We push the Hardening Pact through the Transport-Telecom-Energy Council and Justice-Home Affairs formation on NIS2 implementing acts: binding 24-hour reporting to ENISA, joint audits of transmission operators and major ports, and EU co-financed backup control centres. Finance ministry resistance on cost-sharing is met with Connecting Europe Facility and Digital Europe reprogramming plus mutual-aid crews, while permitting blockages for backup sites are taken to accelerated-permitting talks linked to the sovereignty zones.

On the defection we do not open a new compute fund. DG CNECT and DG COMP run the Common Front review from the existing tech sovereignty and Gigafactory mandates, with the Anti-Coercion Instrument and procurement leverage as the backstop: any external deal offering extraterritorial data access or preferential offtake that fragments EU grid-connection and siting terms is flagged to the European Council. Grid-connection priority for the four to five Gigafactory sites stays negotiated with energy regulators and ASML coordination kept quiet to avoid a further split.
...
```
## New measure
**Municipal Utilities Rapid Shield**
It puts EU-co-funded incident crews, patched backup controls and 24-hour ENISA reporting onto municipal water, power-distribution and port utilities now being probed with open-weight toolkits.
This is Preparedness and resilience work, because the open-weight release has made autumn-like intrusion kits reproducible and the hardening pact finished without solving finance fights and blocked backup sites, so the two-year commitment to absorb shocks now needs a small fast track to the weakest edge.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "Municipal Utilities Rapid Shield", "category": 6, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially at municipal edge, ai_safety up slightly via incident learning"}, "grounds": "open-weight frontier release plus wave of municipal utility probing"}]}
```

## Priority
Municipal Utilities Rapid Shield, because unrecallable near-frontier open weights probing brittle municipal systems outranks slow gigafactory and discipline tracks for the two-year absorb-shocks commitment.

## In practice
We run the Shield through ENISA and DG ENER with Civil Protection Mechanism crews: standard backup-control kits, joint audits extended from transmission to municipal utilities, and Digital Europe / Connecting Europe reprogramming to pay the EU share that finance ministries blocked. Blocked backup-site permits get emergency siting via the sovereignty-zone accelerated-permitting talks, with water/power concessions for councils.

Gigafactories and the tech sovereignty package stay negotiated with energy regulators on grid connections without new money this turn, and the Common Front review continues vetting the bilateral hyperscaler deal via DG CNECT/COMP procurement leverage. We accept public strain on data-centre siting and frame the Shield as keeping lights and water on, not as AI promotion.
