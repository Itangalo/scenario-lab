...
- characters 455-2341: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2385-4202: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4559-4998: `{{statement_ledger}}` from the actor's live statement ledger
- characters 7771-9496: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M9 | Municipal Automated Patching Surge | 6 | small | 12 | 13 | own jurisdiction | resilience up materially, ai_safety up slightly via reduced incident spread | 2 | finished |
...
```
## New measure
None this turn. We are waiting for political capital to recover above the cost of any fundable instrument and for the patching surge to finish landing before staking another measure.

## Store changes
No changes.

## Priority
No priority named this turn, because no measure is in flight and no new measure can be funded at 1.0 political capital without collapse.

## In practice
We keep ENISA/CERT-EU relay work running on existing maintenance authority through December, with big-city utilities acting as relays and the offline triage kit circulating to understaffed councils, and DG SANTE continuing foreign-computed therapies with labelling — no new demands for co-funding or access.

We use the Taiwan precursor window only to task contingency stock-checking inside existing services — essential-service inventories, shipping-exposure of medical and spare inputs — without opening a new instrument, since any chip or Taiwan-facing move now would fail on permits, budget and capital and further damage cohesion.
