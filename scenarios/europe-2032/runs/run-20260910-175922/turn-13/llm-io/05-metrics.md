...
- characters 107-13135: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13178-13369: `{{metrics_json}}` from the run's live metric values
- characters 13462-15348: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15392-17209: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17265-18074: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_grassroots_triage_kit` -- first noted turn 12, listed in 2 turn(s) so far: Municipal IT staff circulate a stripped-down offline triage toolkit built from the ENISA playbooks that spreads peer-to-peer among small towns, partially offsetting dependence on foreign models for incident response.
...
## Metrics
```json
{"ai_capability": 66.1, "openweight_capability": 64.8, "ai_safety": 11.0, "resilience": 46.0, "eu_ai_sovereignty": 11.0, "eu_political_capital": 3.0, "public_sentiment": 26.0}
```
## Narrative
...
LEGITIMACY LENDS: capital 1, sentiment 26 -> +2 (capped, after charges, finishes and events)
SOVEREIGNTY: 11 last turn, no capacity event, no category 4 finish, no category 4 in flight +0, capability rose 0.7 −0 = 11
## Store changes
```json
{"store": []}
```
