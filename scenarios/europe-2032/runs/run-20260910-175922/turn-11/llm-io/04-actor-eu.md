...
- characters 455-2250: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 2294-4402: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5322-5761: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8356-9911: `{{previous_actions}}` from the actor's own response from the previous turn
...
## New measure
None this turn.
We are waiting for political capital to recover above survival level and for the Swarm Defence rollout to finish landing before staking another instrument.

## Store changes
No changes.

## Priority
M8 EU Swarm Defence and Loss-of-Control Containment Rollout, because with capital at 2.0 it is the only instrument still costing us and the only build that can move while permits are frozen and US caps bind, and it directly serves our two-year commitment to keep essential services running.

## In practice
We keep ENISA and CERT-EU joint teams on M8 standardisation, funded by Digital Europe reallocation, pushing automated patching and swarm-behaviour flagging into the town halls, hospitals and shared supplier hit in the 2030 sweep, plus tabletop loss-of-control drills for the next agentic requisition. The Health and Competitiveness Councils carry procurement waivers and mutual-aid staffing to get past municipal staffing gaps; we do not force new builds.

We treat the tailored-therapies breakthrough as delivered dependence: DG SANTE procures access on EU terms where possible and labels provenance plainly, without claiming sovereign credit we do not have. On the new hyperscaler side-deal we log a cohesion risk in the Trade Council and seek only technical clarification from Washington, holding the common line without retaliation we cannot afford. The measured office productivity gains we note without new labour legislation — jobs held, transition borrowing stopped — to avoid spending capital we do not have.
