...
- characters 105-12947: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 12990-13182: `{{metrics_json}}` from the run's live metric values
- characters 13275-14213: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 14256-16328: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 16384-17157: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | Common Front on External AI Deals | 2 |
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 2 turn(s) so far: Coordinated local protests block permitting for two planned hyperscale data centre sites in EU member states over power and water use, delaying new capacity without stopping it.
...
## Metrics
```json
{"ai_capability": 55.0, "openweight_capability": 49.5, "ai_safety": 30.0, "resilience": 37.0, "eu_ai_sovereignty": 20.0, "eu_political_capital": 28.0, "public_sentiment": 35.0}
```
...
## Notepad
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU Critical Systems Hardening and Response Pact −3, Common Front on External AI Deals −2, priority −1 = −12
PROPOSAL BONUS: Common Front on External AI Deals (cat 8, small) +2 — member_state_defection this turn undercuts Union line and this answers it directly
LEGITIMACY LENDS: capital 26, sentiment 35 -> +2
SOVEREIGNTY: 22 last turn, no cat4 finish, Gigafactories in flight +0, member_state_defection t2 −2, capability rose 1.5 −0 = 20
