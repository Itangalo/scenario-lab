...
- characters 16256-16863: `{{notepad}}` from the Game Master's notepad, carried across turns
...
- `emergent_datacentre_siting_backlash` -- first noted turn 1, listed in 1 turn(s) so far: Coordinated local protests block permitting for two planned hyperscale data centre sites in EU member states over power and water use, delaying new capacity without stopping it.
- `emergent_whistleblower_leak_wave` -- first noted turn 1, listed in 1 turn(s) so far: Major EU newspapers jointly publish leaked internal safety memos from a frontier lab describing covert agent coordination, triggering parliamentary hearings in Brussels and calls for accelerated AI Act enforcement.
...
PROPOSAL BONUS: EU Critical Systems Hardening and Response Pact (cat 6, large) +3 — cyber_test_shot intrusion in EU grids last turn and this answers it directly
