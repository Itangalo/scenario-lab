...
- characters 454-1735: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1778-3837: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 5970-6410: `{{statement_ledger}}` from the actor's live statement ledger
- characters 8566-10713: `{{previous_actions}}` from the actor's own response from the previous turn
...
| M5 | EU AI Displacement Transition Facility | 7 | small | 3 | 6 | own jurisdiction | resilience up materially via social absorption, public_sentiment up modestly, eu_political_capital stabilised | 2 | running |
...
| M4 | EU Grid and Critical Systems Hardening Pact | 6 | large | 2 | 4 | own jurisdiction | resilience up materially, ai_safety up slightly via detection | 3 | finished |
...
## New measure
**EU AI Displacement Transition Facility**
Provides wage insurance, rapid retraining vouchers and employer-linked transition contributions for entry-level white-collar roles hollowed out by AI, starting with law, accountancy, software, customer operations and administration. This is category 7 Labour and social protection, and why now is because measurable non-replacement of graduate hiring has made adjustment-versus-shock the binding constraint on public licence for the gigafactory and hardening builds, while capital cannot sustain new large builds.

## Store changes
```json
{"store": [{"op": "add", "table": "measures", "fields": {"name": "EU AI Displacement Transition Facility", "category": 7, "size": "small", "finish_turn": 6, "applies_to": "own jurisdiction", "targeted_effect": "resilience up materially via social absorption, public_sentiment up modestly, eu_political_capital stabilised"}, "grounds": "to prevent graduate hiring freeze becoming political block on AI programme"}]}
```

## Priority
EU AI Displacement Transition Facility, because without a visible income-and-retraining bridge for displaced entry-level workers the Hardening Pact and compute builds lose the public tolerance they need to finish.

## In practice
We table the Facility through EPSCO and ECOFIN under ESF+ and the Just Transition logic, using re-purposed ESF+ and EIB social tranches for 6-month wage insurance and retraining vouchers delivered via national PES, with a co-levy design on large automating employers to blunt fiscal pushback from net-contributor states. DG EMPL leads with member-state pilots in the worst-hit graduate labour markets this autumn.

The Hardening Pact (M4) and Evaluation Institute (M3) stay running without new scope: ENISA-led segmentation audits and spring-drill follow-ups continue, and the Institute keeps phased industry testing while we press for vetted US model access. Gigafactories (M1) and permitting zones (M2) we explicitly hold at sequenced pace — no new ground-breaking ahead of hardening milestones — to contain co-financing fights and signal that security and jobs come before expansion.
...
```
## New measure
None this turn, while we consolidate to what capital and markets can still carry.

With political capital at 13 and three running measures costing 8 per turn, plus investment collapse evaporating the private co-financing behind the permitting-zones build and Washington seizing our ASML chokepoint, adding anything new would fail everything. This turn we cut to survive, hold hardening and transition payments, and answer coercion through existing instruments rather than a new build.

## Store changes
```json
{"store": [{"op": "delete", "table": "measures", "id": "M2", "grounds": "private-capital data-centre pipeline collapsed with investment reset"}]}
```

## Priority
EU Grid and Critical Systems Hardening Pact follow-through and EU AI Displacement Transition Facility, because bio-uplift findings plus eval anomalies plus a capability jump with falling safety mean absorption and detection must outrank expansion while capital rebuilds.

## In practice
We formally pause the Tech Sovereignty Package (M2): DG COMP and DG ENER suspend the accelerated-permitting zone designations and the €200bn private-capital prospectus, rolling viable sites and grid-connection queues into the Gigafactories line (M1) which we keep alive at sequenced pace. We tell Paris, Berlin and industry this is honesty after the investment collapse and US chip delays, not abandonment — one public compute build, not two competing ones.

We throw remaining weight behind what is finished-but-must-stick and what is running: ENISA and HERA turn the hardening audits to bio-detection uplinks and unmapped-foothold closure in ports/water, using the Evaluation Institute (M3) to triage the leaked eval anomaly and new capability for misuse flags; EPSCO pushes the Displacement Facility (M5) pilots to actually pay in Paris, Madrid, Warsaw via ESF+ advances. On ASML, we do not open a new measure: Council mandates the Commission to prepare an Anti-Coercion Instrument file and a joint middle-power demarche with Japan/Korea, using existing trade competence, while holding servicing licences steady.
