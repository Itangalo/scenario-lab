...
- characters 106-13235: `{{metric_rules}}` from metric-rules.md as it currently stands, including any variant patch
- characters 13278-13470: `{{metrics_json}}` from the run's live metric values
- characters 13563-15137: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 15181-17556: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 17612-18665: `{{notepad}}` from the Game Master's notepad, carried across turns
...
| M4 | EU Grid and Critical Systems Hardening Pact | 3 |
| M5 | EU AI Displacement Transition Facility | 2 |
...
PORTFOLIO CHARGE t2: InvestAI Gigafactories −3, Tech sovereignty package −3, EU AI Evaluation and Safety Institute −3, EU Grid and Critical Systems Hardening Pact −3, priority −1 = −13
PROPOSAL BONUS: none — Hardening Pact answers autumn intrusion but intrusion was not a formal event in last three turns
LEGITIMACY LENDS: capital 16, sentiment 41 -> +2
SOVEREIGNTY: 21 last turn, no finished cat4, in flight +0, export_control_escalation t2 −1, capability rose 3.0 −1 = 19

## Emerging developments (tracked)

- `emergent_cyber_insurance_retrenchment` -- first noted turn 2, listed in 2 turn(s) so far: A major European insurer reprices cyber cover after the grid intrusion, imposing open-model provenance exclusions that strand municipalities and mid-size utilities without affordable cover.
- `emergent_cloud_redundancy_demand` -- first noted turn 2, listed in 2 turn(s) so far: A major non-EU cloud provider suffers a prolonged multi-region outage traced to AI-generated misconfiguration, prompting enterprise customers to seek EU-hosted redundancy.
...
## Metrics
```json
{"ai_capability": 61.0, "openweight_capability": 54.3, "ai_safety": 28.0, "resilience": 35.0, "eu_ai_sovereignty": 18.0, "eu_political_capital": 13.0, "public_sentiment": 34.0}
```
