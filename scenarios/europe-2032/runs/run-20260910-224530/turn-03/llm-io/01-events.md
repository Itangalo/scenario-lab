...
- characters 377-1951: `{{historical_summary}}` from the run's rolling summary, written by the Game Master
- characters 1995-4370: `{{world_state}}` from the Game Master's narrative from the previous turn
- characters 4426-5479: `{{notepad}}` from the Game Master's notepad, carried across turns
- characters 5542-29118: `{{events_list}}` from events.md, parsed to id / condition / probability / description per event -- the prose sections of that file are NOT rendered
- characters 29473-29588: `{{event_history}}` from the run's own event record
...
- `emergent_cyber_insurance_retrenchment` -- first noted turn 2, listed in 1 turn(s) so far: A major European insurer reprices cyber cover after the grid intrusion, imposing open-model provenance exclusions that strand municipalities and mid-size utilities without affordable cover.
- `emergent_cloud_redundancy_demand` -- first noted turn 2, listed in 1 turn(s) so far: A major non-EU cloud provider suffers a prolonged multi-region outage traced to AI-generated misconfiguration, prompting enterprise customers to seek EU-hosted redundancy.
...
- Turn 2 (1 turn(s) ago): eval_anomaly_reports, export_control_escalation
