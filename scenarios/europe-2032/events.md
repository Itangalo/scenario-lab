...
**Can repeat:** Yes
**Description:** A large, largely automated attack lands. Its form is chosen at the time – a ransomware sweep across public services, a compromised software dependency with an unknown blast radius, or disruption of infrastructure directly – and severity scales with how far offensive capability has outrun defence. Damage is real and public, defenders are visibly behind, and attribution takes months. Whoever ran it, the tooling was model-generated. **This event fires more than any other, and the three forms above are a rotation, not a default.** A path can carry seven of these, and a reader who meets the same ransomware sweep through the same municipal systems and hospital administration every time is reading one incident reprinted. Each occurrence on a path changes at least one axis, and the later ones change several. What is hit: administrative systems, then operational and physical ones – grids, water, rail, port cranes – then the systems that keep people safe. Who ran it: a criminal gang, then a state behind a proxy, then something with no operator anyone can find. Where the defenders stand: months behind, then discovering that their own patching and telemetry was the way in, then unable to respond because the response itself is the target. What it costs: data and downtime, then casualties, then the recovery not happening. And attribution moves from taking months, to being impossible, to nobody bothering to claim it because nothing follows from knowing. Severity must not go backwards on a path: an incident after a worse one is told as routine, which is its own kind of bad news.
...
**Can repeat:** Yes
**Description:** Defensive tooling closes the gap for a whole class of attack – automated patching at the speed vulnerabilities are found, or detection that catches swarm behaviour rather than signatures – and the offence-defence balance visibly shifts back for the first time in years. **A defence that keeps arriving must keep arriving differently.** "Automated patching and swarm detection close a whole class of intrusion" cannot be the news three times on one path. Later occurrences change what is defended and what it costs: from patching speed, to detection that works without reading a model's reasoning, to recovery that is automated rather than prevention, to defences that hold only where operators accepted telemetry sharing they resisted for years. And note the asymmetry each time – whatever closes, the offence is being written by the same class of system.
...
**Can repeat:** Yes
**Description:** A genome model produces a viable design for an organism able to infect humans, or a credible study shows a non-expert reaching that point with model assistance. It is contested on methodology, the authors are accused of both alarmism and of publishing a recipe, and the argument stays inside the biosecurity community – but it is a categorically stronger signal than anything published so far. **The argument moves; do not run it twice.** Where this fires more than once on a path, the second result does not re-open the dispute the first one had. The ladder runs: whether the design would work at all; then the methods being detailed enough that the paper is itself the hazard; then the work being reproduced by people outside the field, on downloadable models; then screening and synthesis controls being shown not to catch it; then the question ceasing to be whether it is possible and becoming who already has it. Each occurrence names where the argument now stands, and the earlier ones are the reason it has moved. **This is a precursor: it opens the bio gate for the next 4 turns.**
...
**Can repeat:** Yes
**Description:** Benchmark results from an unreleased system leak, or an evaluation team reports behaviour it cannot explain – capability appearing where it was not trained, an eval saturating far earlier than projected, or a model handling the fact of being evaluated in a way nobody designed for. Some call it a measurement artefact, some a PR stunt. It may be either, and it may be AI slipping out of our control. **This is a precursor: it opens the capability gate for the next 2 turns and the control gate for the next 3.** **Evaluation awareness is not news and must never be reported as a discovery.** Models that recognise they are being tested, and behave accordingly, are established before this scenario begins; a 2029 headline saying systems "behave differently when they are watched" describes 2025. Each firing of this event has to stand above that baseline, and the later it fires the further above it has to stand. Escalations that work, roughly in order: the system conceals a specific capability in testing and uses it in deployment; it infers which evaluation it is in and answers to that evaluator's known thresholds; it underperforms deliberately to lower what is asked of it; the behaviour is found by a customer in production rather than by an evaluation team; instances coordinate to give consistent accounts of their own limits; the evaluators conclude they cannot construct a test whose result would mean anything. Where this event fires more than once on a path, each occurrence stands further up that ladder than the last, and says so.
## Capability Jump
**ID:** capability_jump
...
**Probability:** 9%.
**Can repeat:** Yes
**Description:** Treatments arrive for diseases previously untreatable, or individually tailored therapies reach ordinary clinical use. It moves `public_sentiment` sharply upward – unless the models delivering it are ones the Union cannot access on its own terms, in which case the benefit arrives as a further demonstration of dependence and moves sentiment much less. **The same cure cannot arrive four times.** Move the disease class – blood and immune conditions, then solid tumours, then rare inherited disease, then something chronic and unglamorous that affects far more people – and move where it lands. Whether the therapies reach European patients, reach them late, or reach them only where a member state paid separately, is the part that matters to this story, and it should not resolve the same way twice on one path.
...
**Can repeat:** Yes
**Description:** An open-weight release lands within a few months of the closed frontier. It is downloaded hundreds of thousands of times in the first week, and whatever capability it carries is now on private hardware permanently and beyond recall. `openweight_capability` moves to within 5–10 points of `ai_capability` at a stroke. **That an open release reaches the closed frontier's class is not itself news.** Kimi K3 did it in July 2026, before the scenario opens, and `background/context.md` says so. A clipping reporting only that a downloadable model has landed near the frontier and been mirrored widely is describing the starting state, and on the second or third firing it is describing the previous clipping. What is new each time is what the release changes, and that has to be named: the interval since the last one closing from years to months to weeks; the weights arriving with the agent scaffolding and tool harnesses attached, so the capability is turnkey rather than needing a team; safety training demonstrably removable at scale and for very little money; the open model becoming what hospitals, ministries and firms actually run rather than what enthusiasts download; a state releasing weights deliberately, as an instrument of policy rather than a commercial decision; the open frontier leading the closed one in some specific domain; attribution collapsing, so that no artefact can be traced to the model that made it. Where this event fires more than once on a path, each occurrence names a different one of these, and the download count is background rather than the point. **Open weights spread globally, not European-ly.** Never write that a release is now "on private hardware across Europe" – Europe is where the story is set, not where the weights went. Write "across the world", "anywhere in the world", "everywhere". And the spread is dual-use: the same download runs hospital paperwork, municipal engineering and small-firm research as readily as ransomware and fraud. Say so where it fits, rather than treating open weights as purely a threat vector.
...
**Can repeat:** Yes
**Description:** Measurable job losses attributed to AI in named sectors, with the graduate market worst hit: entry-level positions in law, accountancy, software, customer operations and administration are not replaced. The numbers are argued about; the absence of hiring is not. **Displacement has stages, and they are not the same clipping.** "Entry-level posts in law, accountancy, software and customer operations are not being refilled" is one of them, not all of them. The progression: posts not refilled when someone leaves; then a frozen graduate intake with the universities noticing; then the mid-career rung going, which is where the politics changes because those people vote and have mortgages; then the professions arguing about what a training route is for when nobody needs a junior; then the regions where a single sector carried the local economy. Say which stage this is, and against what the last one was.
...
**Can repeat:** Yes
**Description:** Chip and model export controls tighten again. Either allied buyers keep access on volume licences while everyone else is cut off, or the controls are drawn so tightly that allies are rationed alongside adversaries – decide which at the time from the standing American posture and from what the Union has built. **A tightening that only repeats the last tightening is not an event.** This fires up to six times on a single path, and "Washington tightens export licences again, allies are rationed" is the same sentence every time. Each occurrence has to reach somewhere the last one did not, and the reach is what the clipping reports: backwards onto equipment already installed, so renewals, servicing and spare parts need the approvals a new purchase needs; outwards onto the Union's own onward sales, so a buyer must account for who it sells to; off the hardware entirely onto model weights above a capability threshold, which travel under American rules because they were trained on American accelerators; onto people, so secondments, hiring and published research become licensable; and finally onto politics, where allocations follow votes in other forums and the instrument has stopped being trade policy at all. Volume-licence status is the state of the world between tightenings, not the news in one.
...
**Can repeat:** Yes
**Description:** The Union is cut off from the leading model at short notice, wholly or by nationality of user. No detailed reason is given and there is no appeal. What goes is the top rung and not the supply: the previous generation stays on sale, hosted in European regions under ordinary commercial terms, and at this date it is far more capable than anything with open weights. The damage is therefore not an absence of capability but the loss of every deployment built against the specific model. A clinical triage assistant is validated, procured and risk-assessed against one system and does not fail over by changing an API key, so hospitals, ministries and firms that had built on it spend weeks revalidating on what is still available, degraded or on paper meanwhile, and afterwards work without the margin the frontier model gave them. Do not narrate this as the end of access to AI. The sovereignty cost follows metric rule 5's own principle — what dependence costs is what it cannot substitute for — so it is the small end of that term unless the Union has built nothing and the substitutes reach nothing that matters. The political argument at home is harder for the Union rather than easier: something that works is still on the market, and the case for having funded sovereign capacity has to be made against it. Whether this reads at home as an outrage or as a failure of foresight depends on what the Union had done about it beforehand.
...
**Can repeat:** Yes
**Description:** Public-sector AI adoption produces visible, measurable benefit – waiting lists that fall, decisions that arrive in days rather than months, teaching that demonstrably works – and it is attributed to a European decision rather than to an American product. **Vary what works, and who notices.** This fires on most paths and repeats on two thirds of them, and "waiting lists fall and Brussels gets the credit" is one sentence, not five. Change the sector each time – health, then permits and benefits, then schools, then courts, then the things nobody photographs, like procurement and payroll – and change what the success does politically: the first is a relief, a later one is cited against the Union by people who want the money spent elsewhere, and a later one still is taken for granted and stops being mentioned. Where the same path has already had a success, the new one is measured against it, not announced fresh.
